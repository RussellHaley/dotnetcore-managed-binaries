...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/434481/b434481_GenericRecurInit b434481_GenericRecurInit.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/434481/b434481_GenericRecurInit b434481_GenericRecurInit.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b434481_GenericRecurInit.dasm.il b434481_GenericRecurInit.exe
    "$CORE_ROOT/ildasm" -raweh -out=b434481_GenericRecurInit.dasm.il b434481_GenericRecurInit.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b434481_GenericRecurInit.asm.exe  b434481_GenericRecurInit.dasm.il
    "$CORE_ROOT/ilasm" -output=b434481_GenericRecurInit.asm.exe  b434481_GenericRecurInit.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b434481_GenericRecurInit
    if [ ! -f b434481_GenericRecurInit.org ]; then
        TakeLock
        if [ ! -f b434481_GenericRecurInit.org ]; then
          mkdir IL
          cp b434481_GenericRecurInit.exe IL/b434481_GenericRecurInit.exe
          mv b434481_GenericRecurInit.exe b434481_GenericRecurInit.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b434481_GenericRecurInit.org /out b434481_GenericRecurInit.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b434481_GenericRecurInit.org /out b434481_GenericRecurInit.exe
          __cgExitCode=$?
...
ExePath=b434481_GenericRecurInit.exe
...
then
  ...
  # Use hints for reflection roots, if provided in b434481_GenericRecurInit.reflect.xml
  if [ -f b434481_GenericRecurInit.reflect.xml ]
then
    ReflectionRoots="-x b434481_GenericRecurInit.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/b434481_GenericRecurInit.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b434481_GenericRecurInit.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b434481_GenericRecurInit.asm.exe $CLRTestExecutionArguments
