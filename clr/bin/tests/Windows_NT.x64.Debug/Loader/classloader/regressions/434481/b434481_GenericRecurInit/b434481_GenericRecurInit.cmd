...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\434481\b434481_GenericRecurInit b434481_GenericRecurInit.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\434481\b434481_GenericRecurInit b434481_GenericRecurInit.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b434481_GenericRecurInit.dasm.il b434481_GenericRecurInit.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b434481_GenericRecurInit.dasm.il b434481_GenericRecurInit.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b434481_GenericRecurInit.asm.exe  b434481_GenericRecurInit.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b434481_GenericRecurInit.asm.exe  b434481_GenericRecurInit.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b434481_GenericRecurInit
    if not exist "b434481_GenericRecurInit.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b434481_GenericRecurInit.org" (
            mkdir IL
            copy b434481_GenericRecurInit.exe IL\b434481_GenericRecurInit.exe 
            ren b434481_GenericRecurInit.exe b434481_GenericRecurInit.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b434481_GenericRecurInit.org /out b434481_GenericRecurInit.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b434481_GenericRecurInit.org /out b434481_GenericRecurInit.exe
...
set ExePath=b434481_GenericRecurInit.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b434481_GenericRecurInit.reflect.xml
    IF EXIST b434481_GenericRecurInit.reflect.xml set ReflectionRoots=-x b434481_GenericRecurInit.reflect.xml
    ...
    set ExePath=%LinkBin%\b434481_GenericRecurInit.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b434481_GenericRecurInit.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b434481_GenericRecurInit.asm.exe %CLRTestExecutionArguments% %Host_Args%
