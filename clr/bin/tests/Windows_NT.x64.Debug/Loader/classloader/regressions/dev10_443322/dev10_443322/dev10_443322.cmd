...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\dev10_443322\dev10_443322 dev10_443322.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\dev10_443322\dev10_443322 dev10_443322.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=dev10_443322.dasm.il dev10_443322.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=dev10_443322.dasm.il dev10_443322.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=dev10_443322.asm.exe -QUIET -NOLOGO -DEBUG dev10_443322.dasm.il
    %CORE_ROOT%\ilasm.exe /output=dev10_443322.asm.exe -QUIET -NOLOGO -DEBUG dev10_443322.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=dev10_443322
    if not exist "dev10_443322.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "dev10_443322.org" (
            mkdir IL
            copy dev10_443322.exe IL\dev10_443322.exe 
            ren dev10_443322.exe dev10_443322.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dev10_443322.org /out dev10_443322.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dev10_443322.org /out dev10_443322.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in dev10_443322.reflect.xml
    IF EXIST dev10_443322.reflect.xml set ReflectionRoots=-x dev10_443322.reflect.xml
    ...
    set ExePath=%LinkBin%\dev10_443322.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% dev10_443322.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% dev10_443322.asm.exe %CLRTestExecutionArguments% %Host_Args%
