...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/dev10_443322/dev10_443322 dev10_443322.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/dev10_443322/dev10_443322 dev10_443322.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=dev10_443322.dasm.il dev10_443322.exe
    "$CORE_ROOT/ildasm" -raweh -out=dev10_443322.dasm.il dev10_443322.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=dev10_443322.asm.exe -QUIET -NOLOGO -DEBUG dev10_443322.dasm.il
    "$CORE_ROOT/ilasm" -output=dev10_443322.asm.exe -QUIET -NOLOGO -DEBUG dev10_443322.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=dev10_443322
    if [ ! -f dev10_443322.org ]; then
        TakeLock
        if [ ! -f dev10_443322.org ]; then
          mkdir IL
          cp dev10_443322.exe IL/dev10_443322.exe
          mv dev10_443322.exe dev10_443322.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dev10_443322.org /out dev10_443322.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dev10_443322.org /out dev10_443322.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in dev10_443322.reflect.xml
  if [ -f dev10_443322.reflect.xml ]
then
    ReflectionRoots="-x dev10_443322.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/dev10_443322.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" dev10_443322.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" dev10_443322.asm.exe $CLRTestExecutionArguments
