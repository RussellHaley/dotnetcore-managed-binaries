...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/529206/vsw529206StaticCctor vsw529206StaticCctor.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/529206/vsw529206StaticCctor vsw529206StaticCctor.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=vsw529206StaticCctor.dasm.il vsw529206StaticCctor.exe
    "$CORE_ROOT/ildasm" -raweh -out=vsw529206StaticCctor.dasm.il vsw529206StaticCctor.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=vsw529206StaticCctor.asm.exe  vsw529206StaticCctor.dasm.il
    "$CORE_ROOT/ilasm" -output=vsw529206StaticCctor.asm.exe  vsw529206StaticCctor.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=vsw529206StaticCctor
    if [ ! -f vsw529206StaticCctor.org ]; then
        TakeLock
        if [ ! -f vsw529206StaticCctor.org ]; then
          mkdir IL
          cp vsw529206StaticCctor.exe IL/vsw529206StaticCctor.exe
          mv vsw529206StaticCctor.exe vsw529206StaticCctor.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vsw529206StaticCctor.org /out vsw529206StaticCctor.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vsw529206StaticCctor.org /out vsw529206StaticCctor.exe
          __cgExitCode=$?
...
ExePath=vsw529206StaticCctor.exe
...
then
  ...
  # Use hints for reflection roots, if provided in vsw529206StaticCctor.reflect.xml
  if [ -f vsw529206StaticCctor.reflect.xml ]
then
    ReflectionRoots="-x vsw529206StaticCctor.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/vsw529206StaticCctor.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" vsw529206StaticCctor.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" vsw529206StaticCctor.asm.exe $CLRTestExecutionArguments
