...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\529206\vsw529206StaticCctor vsw529206StaticCctor.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\529206\vsw529206StaticCctor vsw529206StaticCctor.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=vsw529206StaticCctor.dasm.il vsw529206StaticCctor.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=vsw529206StaticCctor.dasm.il vsw529206StaticCctor.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=vsw529206StaticCctor.asm.exe  vsw529206StaticCctor.dasm.il
    %CORE_ROOT%\ilasm.exe /output=vsw529206StaticCctor.asm.exe  vsw529206StaticCctor.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=vsw529206StaticCctor
    if not exist "vsw529206StaticCctor.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "vsw529206StaticCctor.org" (
            mkdir IL
            copy vsw529206StaticCctor.exe IL\vsw529206StaticCctor.exe 
            ren vsw529206StaticCctor.exe vsw529206StaticCctor.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in vsw529206StaticCctor.org /out vsw529206StaticCctor.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in vsw529206StaticCctor.org /out vsw529206StaticCctor.exe
...
set ExePath=vsw529206StaticCctor.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in vsw529206StaticCctor.reflect.xml
    IF EXIST vsw529206StaticCctor.reflect.xml set ReflectionRoots=-x vsw529206StaticCctor.reflect.xml
    ...
    set ExePath=%LinkBin%\vsw529206StaticCctor.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% vsw529206StaticCctor.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% vsw529206StaticCctor.asm.exe %CLRTestExecutionArguments% %Host_Args%
