...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\347422\b347422 b347422.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\347422\b347422 b347422.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b347422.dasm.il b347422.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b347422.dasm.il b347422.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b347422.asm.exe  b347422.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b347422.asm.exe  b347422.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b347422
    if not exist "b347422.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b347422.org" (
            mkdir IL
            copy b347422.exe IL\b347422.exe 
            ren b347422.exe b347422.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b347422.org /out b347422.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b347422.org /out b347422.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b347422.reflect.xml
    IF EXIST b347422.reflect.xml set ReflectionRoots=-x b347422.reflect.xml
    ...
    set ExePath=%LinkBin%\b347422.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b347422.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b347422.asm.exe %CLRTestExecutionArguments% %Host_Args%
