...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/347422/b347422 b347422.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/347422/b347422 b347422.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b347422.dasm.il b347422.exe
    "$CORE_ROOT/ildasm" -raweh -out=b347422.dasm.il b347422.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b347422.asm.exe  b347422.dasm.il
    "$CORE_ROOT/ilasm" -output=b347422.asm.exe  b347422.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b347422
    if [ ! -f b347422.org ]; then
        TakeLock
        if [ ! -f b347422.org ]; then
          mkdir IL
          cp b347422.exe IL/b347422.exe
          mv b347422.exe b347422.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b347422.org /out b347422.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b347422.org /out b347422.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b347422.reflect.xml
  if [ -f b347422.reflect.xml ]
then
    ReflectionRoots="-x b347422.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b347422.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b347422.asm.exe $CLRTestExecutionArguments
