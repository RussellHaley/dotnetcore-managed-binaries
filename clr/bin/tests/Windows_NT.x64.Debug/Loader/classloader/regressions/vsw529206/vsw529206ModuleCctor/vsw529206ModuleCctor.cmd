...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\vsw529206\vsw529206ModuleCctor vsw529206ModuleCctor.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\vsw529206\vsw529206ModuleCctor vsw529206ModuleCctor.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=vsw529206ModuleCctor.dasm.il vsw529206ModuleCctor.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=vsw529206ModuleCctor.dasm.il vsw529206ModuleCctor.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=vsw529206ModuleCctor.asm.exe  vsw529206ModuleCctor.dasm.il
    %CORE_ROOT%\ilasm.exe /output=vsw529206ModuleCctor.asm.exe  vsw529206ModuleCctor.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=vsw529206ModuleCctor
    if not exist "vsw529206ModuleCctor.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "vsw529206ModuleCctor.org" (
            mkdir IL
            copy vsw529206ModuleCctor.exe IL\vsw529206ModuleCctor.exe 
            ren vsw529206ModuleCctor.exe vsw529206ModuleCctor.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in vsw529206ModuleCctor.org /out vsw529206ModuleCctor.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in vsw529206ModuleCctor.org /out vsw529206ModuleCctor.exe
...
set ExePath=vsw529206ModuleCctor.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in vsw529206ModuleCctor.reflect.xml
    IF EXIST vsw529206ModuleCctor.reflect.xml set ReflectionRoots=-x vsw529206ModuleCctor.reflect.xml
    ...
    set ExePath=%LinkBin%\vsw529206ModuleCctor.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% vsw529206ModuleCctor.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% vsw529206ModuleCctor.asm.exe %CLRTestExecutionArguments% %Host_Args%
