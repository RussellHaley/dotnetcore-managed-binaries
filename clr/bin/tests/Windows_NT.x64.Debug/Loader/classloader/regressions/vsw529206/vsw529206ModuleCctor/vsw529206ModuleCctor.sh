...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/vsw529206/vsw529206ModuleCctor vsw529206ModuleCctor.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/vsw529206/vsw529206ModuleCctor vsw529206ModuleCctor.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=vsw529206ModuleCctor.dasm.il vsw529206ModuleCctor.exe
    "$CORE_ROOT/ildasm" -raweh -out=vsw529206ModuleCctor.dasm.il vsw529206ModuleCctor.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=vsw529206ModuleCctor.asm.exe  vsw529206ModuleCctor.dasm.il
    "$CORE_ROOT/ilasm" -output=vsw529206ModuleCctor.asm.exe  vsw529206ModuleCctor.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=vsw529206ModuleCctor
    if [ ! -f vsw529206ModuleCctor.org ]; then
        TakeLock
        if [ ! -f vsw529206ModuleCctor.org ]; then
          mkdir IL
          cp vsw529206ModuleCctor.exe IL/vsw529206ModuleCctor.exe
          mv vsw529206ModuleCctor.exe vsw529206ModuleCctor.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vsw529206ModuleCctor.org /out vsw529206ModuleCctor.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vsw529206ModuleCctor.org /out vsw529206ModuleCctor.exe
          __cgExitCode=$?
...
ExePath=vsw529206ModuleCctor.exe
...
then
  ...
  # Use hints for reflection roots, if provided in vsw529206ModuleCctor.reflect.xml
  if [ -f vsw529206ModuleCctor.reflect.xml ]
then
    ReflectionRoots="-x vsw529206ModuleCctor.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/vsw529206ModuleCctor.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" vsw529206ModuleCctor.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" vsw529206ModuleCctor.asm.exe $CLRTestExecutionArguments
