...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/523654/test532654_b test532654_b.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/regressions/523654/test532654_b test532654_b.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=test532654_b.dasm.il test532654_b.exe
    "$CORE_ROOT/ildasm" -raweh -out=test532654_b.dasm.il test532654_b.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=test532654_b.asm.exe  test532654_b.dasm.il
    "$CORE_ROOT/ilasm" -output=test532654_b.asm.exe  test532654_b.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=test532654_b
    if [ ! -f test532654_b.org ]; then
        TakeLock
        if [ ! -f test532654_b.org ]; then
          mkdir IL
          cp test532654_b.exe IL/test532654_b.exe
          mv test532654_b.exe test532654_b.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test532654_b.org /out test532654_b.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test532654_b.org /out test532654_b.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in test532654_b.reflect.xml
  if [ -f test532654_b.reflect.xml ]
then
    ReflectionRoots="-x test532654_b.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/test532654_b.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" test532654_b.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" test532654_b.asm.exe $CLRTestExecutionArguments
