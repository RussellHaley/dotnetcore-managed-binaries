...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\523654\test532654_b test532654_b.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\regressions\523654\test532654_b test532654_b.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=test532654_b.dasm.il test532654_b.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=test532654_b.dasm.il test532654_b.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=test532654_b.asm.exe  test532654_b.dasm.il
    %CORE_ROOT%\ilasm.exe /output=test532654_b.asm.exe  test532654_b.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=test532654_b
    if not exist "test532654_b.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "test532654_b.org" (
            mkdir IL
            copy test532654_b.exe IL\test532654_b.exe 
            ren test532654_b.exe test532654_b.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test532654_b.org /out test532654_b.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test532654_b.org /out test532654_b.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in test532654_b.reflect.xml
    IF EXIST test532654_b.reflect.xml set ReflectionRoots=-x test532654_b.reflect.xml
    ...
    set ExePath=%LinkBin%\test532654_b.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% test532654_b.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% test532654_b.asm.exe %CLRTestExecutionArguments% %Host_Args%
