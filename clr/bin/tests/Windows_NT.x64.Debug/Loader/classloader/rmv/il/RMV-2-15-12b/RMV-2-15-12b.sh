...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/rmv/il/RMV-2-15-12b RMV-2-15-12b.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/rmv/il/RMV-2-15-12b RMV-2-15-12b.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=RMV-2-15-12b.dasm.il RMV-2-15-12b.exe
    "$CORE_ROOT/ildasm" -raweh -out=RMV-2-15-12b.dasm.il RMV-2-15-12b.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=RMV-2-15-12b.asm.exe -QUIET -NOLOGO -DEBUG RMV-2-15-12b.dasm.il
    "$CORE_ROOT/ilasm" -output=RMV-2-15-12b.asm.exe -QUIET -NOLOGO -DEBUG RMV-2-15-12b.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=RMV-2-15-12b
    if [ ! -f RMV-2-15-12b.org ]; then
        TakeLock
        if [ ! -f RMV-2-15-12b.org ]; then
          mkdir IL
          cp RMV-2-15-12b.exe IL/RMV-2-15-12b.exe
          mv RMV-2-15-12b.exe RMV-2-15-12b.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RMV-2-15-12b.org /out RMV-2-15-12b.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RMV-2-15-12b.org /out RMV-2-15-12b.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in RMV-2-15-12b.reflect.xml
  if [ -f RMV-2-15-12b.reflect.xml ]
then
    ReflectionRoots="-x RMV-2-15-12b.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/RMV-2-15-12b.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" RMV-2-15-12b.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" RMV-2-15-12b.asm.exe $CLRTestExecutionArguments
