...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\rmv\il\RMV-2-15-12b RMV-2-15-12b.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\rmv\il\RMV-2-15-12b RMV-2-15-12b.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=RMV-2-15-12b.dasm.il RMV-2-15-12b.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=RMV-2-15-12b.dasm.il RMV-2-15-12b.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=RMV-2-15-12b.asm.exe -QUIET -NOLOGO -DEBUG RMV-2-15-12b.dasm.il
    %CORE_ROOT%\ilasm.exe /output=RMV-2-15-12b.asm.exe -QUIET -NOLOGO -DEBUG RMV-2-15-12b.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=RMV-2-15-12b
    if not exist "RMV-2-15-12b.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "RMV-2-15-12b.org" (
            mkdir IL
            copy RMV-2-15-12b.exe IL\RMV-2-15-12b.exe 
            ren RMV-2-15-12b.exe RMV-2-15-12b.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RMV-2-15-12b.org /out RMV-2-15-12b.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RMV-2-15-12b.org /out RMV-2-15-12b.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in RMV-2-15-12b.reflect.xml
    IF EXIST RMV-2-15-12b.reflect.xml set ReflectionRoots=-x RMV-2-15-12b.reflect.xml
    ...
    set ExePath=%LinkBin%\RMV-2-15-12b.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% RMV-2-15-12b.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% RMV-2-15-12b.asm.exe %CLRTestExecutionArguments% %Host_Args%
