...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CircularCctors/CircularCctorFourThreadsBFI CircularCctorFourThreadsBFI.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CircularCctors/CircularCctorFourThreadsBFI CircularCctorFourThreadsBFI.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CircularCctorFourThreadsBFI.dasm.il CircularCctorFourThreadsBFI.exe
    "$CORE_ROOT/ildasm" -raweh -out=CircularCctorFourThreadsBFI.dasm.il CircularCctorFourThreadsBFI.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CircularCctorFourThreadsBFI.asm.exe -QUIET -NOLOGO -DEBUG CircularCctorFourThreadsBFI.dasm.il
    "$CORE_ROOT/ilasm" -output=CircularCctorFourThreadsBFI.asm.exe -QUIET -NOLOGO -DEBUG CircularCctorFourThreadsBFI.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CircularCctorFourThreadsBFI
    if [ ! -f CircularCctorFourThreadsBFI.org ]; then
        TakeLock
        if [ ! -f CircularCctorFourThreadsBFI.org ]; then
          mkdir IL
          cp CircularCctorFourThreadsBFI.exe IL/CircularCctorFourThreadsBFI.exe
          mv CircularCctorFourThreadsBFI.exe CircularCctorFourThreadsBFI.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CircularCctorFourThreadsBFI.org /out CircularCctorFourThreadsBFI.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CircularCctorFourThreadsBFI.org /out CircularCctorFourThreadsBFI.exe
          __cgExitCode=$?
...
ExePath=CircularCctorFourThreadsBFI.exe
...
then
  ...
  # Use hints for reflection roots, if provided in CircularCctorFourThreadsBFI.reflect.xml
  if [ -f CircularCctorFourThreadsBFI.reflect.xml ]
then
    ReflectionRoots="-x CircularCctorFourThreadsBFI.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CircularCctorFourThreadsBFI.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CircularCctorFourThreadsBFI.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CircularCctorFourThreadsBFI.asm.exe $CLRTestExecutionArguments
