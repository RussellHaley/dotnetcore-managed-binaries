...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CircularCctors\CircularCctorFourThreadsBFI CircularCctorFourThreadsBFI.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CircularCctors\CircularCctorFourThreadsBFI CircularCctorFourThreadsBFI.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CircularCctorFourThreadsBFI.dasm.il CircularCctorFourThreadsBFI.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CircularCctorFourThreadsBFI.dasm.il CircularCctorFourThreadsBFI.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CircularCctorFourThreadsBFI.asm.exe -QUIET -NOLOGO -DEBUG CircularCctorFourThreadsBFI.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CircularCctorFourThreadsBFI.asm.exe -QUIET -NOLOGO -DEBUG CircularCctorFourThreadsBFI.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CircularCctorFourThreadsBFI
    if not exist "CircularCctorFourThreadsBFI.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CircularCctorFourThreadsBFI.org" (
            mkdir IL
            copy CircularCctorFourThreadsBFI.exe IL\CircularCctorFourThreadsBFI.exe 
            ren CircularCctorFourThreadsBFI.exe CircularCctorFourThreadsBFI.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CircularCctorFourThreadsBFI.org /out CircularCctorFourThreadsBFI.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CircularCctorFourThreadsBFI.org /out CircularCctorFourThreadsBFI.exe
...
set ExePath=CircularCctorFourThreadsBFI.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CircularCctorFourThreadsBFI.reflect.xml
    IF EXIST CircularCctorFourThreadsBFI.reflect.xml set ReflectionRoots=-x CircularCctorFourThreadsBFI.reflect.xml
    ...
    set ExePath=%LinkBin%\CircularCctorFourThreadsBFI.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CircularCctorFourThreadsBFI.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CircularCctorFourThreadsBFI.asm.exe %CLRTestExecutionArguments% %Host_Args%
