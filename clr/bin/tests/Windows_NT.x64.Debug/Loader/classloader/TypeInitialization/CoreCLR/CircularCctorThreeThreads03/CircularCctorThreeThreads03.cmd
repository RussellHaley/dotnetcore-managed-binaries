...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CoreCLR\CircularCctorThreeThreads03 CircularCctorThreeThreads03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CoreCLR\CircularCctorThreeThreads03 CircularCctorThreeThreads03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CircularCctorThreeThreads03.dasm.il CircularCctorThreeThreads03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CircularCctorThreeThreads03.dasm.il CircularCctorThreeThreads03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CircularCctorThreeThreads03.asm.exe  CircularCctorThreeThreads03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CircularCctorThreeThreads03.asm.exe  CircularCctorThreeThreads03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CircularCctorThreeThreads03
    if not exist "CircularCctorThreeThreads03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CircularCctorThreeThreads03.org" (
            mkdir IL
            copy CircularCctorThreeThreads03.exe IL\CircularCctorThreeThreads03.exe 
            ren CircularCctorThreeThreads03.exe CircularCctorThreeThreads03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CircularCctorThreeThreads03.org /out CircularCctorThreeThreads03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CircularCctorThreeThreads03.org /out CircularCctorThreeThreads03.exe
...
set ExePath=CircularCctorThreeThreads03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CircularCctorThreeThreads03.reflect.xml
    IF EXIST CircularCctorThreeThreads03.reflect.xml set ReflectionRoots=-x CircularCctorThreeThreads03.reflect.xml
    ...
    set ExePath=%LinkBin%\CircularCctorThreeThreads03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CircularCctorThreeThreads03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CircularCctorThreeThreads03.asm.exe %CLRTestExecutionArguments% %Host_Args%
