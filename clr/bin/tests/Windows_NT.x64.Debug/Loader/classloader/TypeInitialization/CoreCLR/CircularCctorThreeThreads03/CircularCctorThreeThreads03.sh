...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CoreCLR/CircularCctorThreeThreads03 CircularCctorThreeThreads03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CoreCLR/CircularCctorThreeThreads03 CircularCctorThreeThreads03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CircularCctorThreeThreads03.dasm.il CircularCctorThreeThreads03.exe
    "$CORE_ROOT/ildasm" -raweh -out=CircularCctorThreeThreads03.dasm.il CircularCctorThreeThreads03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CircularCctorThreeThreads03.asm.exe  CircularCctorThreeThreads03.dasm.il
    "$CORE_ROOT/ilasm" -output=CircularCctorThreeThreads03.asm.exe  CircularCctorThreeThreads03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CircularCctorThreeThreads03
    if [ ! -f CircularCctorThreeThreads03.org ]; then
        TakeLock
        if [ ! -f CircularCctorThreeThreads03.org ]; then
          mkdir IL
          cp CircularCctorThreeThreads03.exe IL/CircularCctorThreeThreads03.exe
          mv CircularCctorThreeThreads03.exe CircularCctorThreeThreads03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CircularCctorThreeThreads03.org /out CircularCctorThreeThreads03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CircularCctorThreeThreads03.org /out CircularCctorThreeThreads03.exe
          __cgExitCode=$?
...
ExePath=CircularCctorThreeThreads03.exe
...
then
  ...
  # Use hints for reflection roots, if provided in CircularCctorThreeThreads03.reflect.xml
  if [ -f CircularCctorThreeThreads03.reflect.xml ]
then
    ReflectionRoots="-x CircularCctorThreeThreads03.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CircularCctorThreeThreads03.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CircularCctorThreeThreads03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CircularCctorThreeThreads03.asm.exe $CLRTestExecutionArguments
