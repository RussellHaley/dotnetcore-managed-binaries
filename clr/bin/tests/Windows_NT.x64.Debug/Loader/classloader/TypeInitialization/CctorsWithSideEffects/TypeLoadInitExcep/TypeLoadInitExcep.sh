...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/TypeLoadInitExcep TypeLoadInitExcep.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/TypeLoadInitExcep TypeLoadInitExcep.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=TypeLoadInitExcep.dasm.il TypeLoadInitExcep.exe
    "$CORE_ROOT/ildasm" -raweh -out=TypeLoadInitExcep.dasm.il TypeLoadInitExcep.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=TypeLoadInitExcep.asm.exe  TypeLoadInitExcep.dasm.il
    "$CORE_ROOT/ilasm" -output=TypeLoadInitExcep.asm.exe  TypeLoadInitExcep.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=TypeLoadInitExcep
    if [ ! -f TypeLoadInitExcep.org ]; then
        TakeLock
        if [ ! -f TypeLoadInitExcep.org ]; then
          mkdir IL
          cp TypeLoadInitExcep.exe IL/TypeLoadInitExcep.exe
          mv TypeLoadInitExcep.exe TypeLoadInitExcep.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TypeLoadInitExcep.org /out TypeLoadInitExcep.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TypeLoadInitExcep.org /out TypeLoadInitExcep.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in TypeLoadInitExcep.reflect.xml
  if [ -f TypeLoadInitExcep.reflect.xml ]
then
    ReflectionRoots="-x TypeLoadInitExcep.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/TypeLoadInitExcep.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" TypeLoadInitExcep.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" TypeLoadInitExcep.asm.exe $CLRTestExecutionArguments
