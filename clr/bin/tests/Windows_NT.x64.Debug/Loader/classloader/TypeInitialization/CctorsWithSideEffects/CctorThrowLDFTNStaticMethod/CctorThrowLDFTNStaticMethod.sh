...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowLDFTNStaticMethod CctorThrowLDFTNStaticMethod.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowLDFTNStaticMethod CctorThrowLDFTNStaticMethod.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CctorThrowLDFTNStaticMethod.dasm.il CctorThrowLDFTNStaticMethod.exe
    "$CORE_ROOT/ildasm" -raweh -out=CctorThrowLDFTNStaticMethod.dasm.il CctorThrowLDFTNStaticMethod.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CctorThrowLDFTNStaticMethod.asm.exe -QUIET -NOLOGO -DEBUG CctorThrowLDFTNStaticMethod.dasm.il
    "$CORE_ROOT/ilasm" -output=CctorThrowLDFTNStaticMethod.asm.exe -QUIET -NOLOGO -DEBUG CctorThrowLDFTNStaticMethod.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CctorThrowLDFTNStaticMethod
    if [ ! -f CctorThrowLDFTNStaticMethod.org ]; then
        TakeLock
        if [ ! -f CctorThrowLDFTNStaticMethod.org ]; then
          mkdir IL
          cp CctorThrowLDFTNStaticMethod.exe IL/CctorThrowLDFTNStaticMethod.exe
          mv CctorThrowLDFTNStaticMethod.exe CctorThrowLDFTNStaticMethod.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowLDFTNStaticMethod.org /out CctorThrowLDFTNStaticMethod.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowLDFTNStaticMethod.org /out CctorThrowLDFTNStaticMethod.exe
          __cgExitCode=$?
...
ExePath=CctorThrowLDFTNStaticMethod.exe
...
then
  ...
  # Use hints for reflection roots, if provided in CctorThrowLDFTNStaticMethod.reflect.xml
  if [ -f CctorThrowLDFTNStaticMethod.reflect.xml ]
then
    ReflectionRoots="-x CctorThrowLDFTNStaticMethod.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CctorThrowLDFTNStaticMethod.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CctorThrowLDFTNStaticMethod.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CctorThrowLDFTNStaticMethod.asm.exe $CLRTestExecutionArguments
