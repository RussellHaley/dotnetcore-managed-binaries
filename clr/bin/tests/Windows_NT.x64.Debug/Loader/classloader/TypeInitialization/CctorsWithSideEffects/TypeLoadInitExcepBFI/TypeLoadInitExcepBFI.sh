...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/TypeLoadInitExcepBFI TypeLoadInitExcepBFI.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/TypeLoadInitExcepBFI TypeLoadInitExcepBFI.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=TypeLoadInitExcepBFI.dasm.il TypeLoadInitExcepBFI.exe
    "$CORE_ROOT/ildasm" -raweh -out=TypeLoadInitExcepBFI.dasm.il TypeLoadInitExcepBFI.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=TypeLoadInitExcepBFI.asm.exe -QUIET -NOLOGO -DEBUG TypeLoadInitExcepBFI.dasm.il
    "$CORE_ROOT/ilasm" -output=TypeLoadInitExcepBFI.asm.exe -QUIET -NOLOGO -DEBUG TypeLoadInitExcepBFI.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=TypeLoadInitExcepBFI
    if [ ! -f TypeLoadInitExcepBFI.org ]; then
        TakeLock
        if [ ! -f TypeLoadInitExcepBFI.org ]; then
          mkdir IL
          cp TypeLoadInitExcepBFI.exe IL/TypeLoadInitExcepBFI.exe
          mv TypeLoadInitExcepBFI.exe TypeLoadInitExcepBFI.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TypeLoadInitExcepBFI.org /out TypeLoadInitExcepBFI.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TypeLoadInitExcepBFI.org /out TypeLoadInitExcepBFI.exe
          __cgExitCode=$?
...
ExePath=TypeLoadInitExcepBFI.exe
...
then
  ...
  # Use hints for reflection roots, if provided in TypeLoadInitExcepBFI.reflect.xml
  if [ -f TypeLoadInitExcepBFI.reflect.xml ]
then
    ReflectionRoots="-x TypeLoadInitExcepBFI.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/TypeLoadInitExcepBFI.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" TypeLoadInitExcepBFI.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" TypeLoadInitExcepBFI.asm.exe $CLRTestExecutionArguments
