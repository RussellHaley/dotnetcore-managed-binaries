...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowMethodAccess CctorThrowMethodAccess.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowMethodAccess CctorThrowMethodAccess.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CctorThrowMethodAccess.dasm.il CctorThrowMethodAccess.exe
    "$CORE_ROOT/ildasm" -raweh -out=CctorThrowMethodAccess.dasm.il CctorThrowMethodAccess.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CctorThrowMethodAccess.asm.exe  CctorThrowMethodAccess.dasm.il
    "$CORE_ROOT/ilasm" -output=CctorThrowMethodAccess.asm.exe  CctorThrowMethodAccess.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CctorThrowMethodAccess
    if [ ! -f CctorThrowMethodAccess.org ]; then
        TakeLock
        if [ ! -f CctorThrowMethodAccess.org ]; then
          mkdir IL
          cp CctorThrowMethodAccess.exe IL/CctorThrowMethodAccess.exe
          mv CctorThrowMethodAccess.exe CctorThrowMethodAccess.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowMethodAccess.org /out CctorThrowMethodAccess.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowMethodAccess.org /out CctorThrowMethodAccess.exe
          __cgExitCode=$?
...
ExePath=CctorThrowMethodAccess.exe
...
then
  ...
  # Use hints for reflection roots, if provided in CctorThrowMethodAccess.reflect.xml
  if [ -f CctorThrowMethodAccess.reflect.xml ]
then
    ReflectionRoots="-x CctorThrowMethodAccess.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CctorThrowMethodAccess.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CctorThrowMethodAccess.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CctorThrowMethodAccess.asm.exe $CLRTestExecutionArguments
