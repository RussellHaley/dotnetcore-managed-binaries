...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowMethodAccess CctorThrowMethodAccess.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowMethodAccess CctorThrowMethodAccess.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowMethodAccess.dasm.il CctorThrowMethodAccess.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowMethodAccess.dasm.il CctorThrowMethodAccess.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CctorThrowMethodAccess.asm.exe  CctorThrowMethodAccess.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CctorThrowMethodAccess.asm.exe  CctorThrowMethodAccess.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CctorThrowMethodAccess
    if not exist "CctorThrowMethodAccess.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CctorThrowMethodAccess.org" (
            mkdir IL
            copy CctorThrowMethodAccess.exe IL\CctorThrowMethodAccess.exe 
            ren CctorThrowMethodAccess.exe CctorThrowMethodAccess.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowMethodAccess.org /out CctorThrowMethodAccess.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowMethodAccess.org /out CctorThrowMethodAccess.exe
...
set ExePath=CctorThrowMethodAccess.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CctorThrowMethodAccess.reflect.xml
    IF EXIST CctorThrowMethodAccess.reflect.xml set ReflectionRoots=-x CctorThrowMethodAccess.reflect.xml
    ...
    set ExePath=%LinkBin%\CctorThrowMethodAccess.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CctorThrowMethodAccess.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CctorThrowMethodAccess.asm.exe %CLRTestExecutionArguments% %Host_Args%
