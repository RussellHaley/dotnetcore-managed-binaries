...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowStaticField CctorThrowStaticField.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowStaticField CctorThrowStaticField.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CctorThrowStaticField.dasm.il CctorThrowStaticField.exe
    "$CORE_ROOT/ildasm" -raweh -out=CctorThrowStaticField.dasm.il CctorThrowStaticField.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CctorThrowStaticField.asm.exe  CctorThrowStaticField.dasm.il
    "$CORE_ROOT/ilasm" -output=CctorThrowStaticField.asm.exe  CctorThrowStaticField.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CctorThrowStaticField
    if [ ! -f CctorThrowStaticField.org ]; then
        TakeLock
        if [ ! -f CctorThrowStaticField.org ]; then
          mkdir IL
          cp CctorThrowStaticField.exe IL/CctorThrowStaticField.exe
          mv CctorThrowStaticField.exe CctorThrowStaticField.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowStaticField.org /out CctorThrowStaticField.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowStaticField.org /out CctorThrowStaticField.exe
          __cgExitCode=$?
...
ExePath=CctorThrowStaticField.exe
...
then
  ...
  # Use hints for reflection roots, if provided in CctorThrowStaticField.reflect.xml
  if [ -f CctorThrowStaticField.reflect.xml ]
then
    ReflectionRoots="-x CctorThrowStaticField.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CctorThrowStaticField.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CctorThrowStaticField.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CctorThrowStaticField.asm.exe $CLRTestExecutionArguments
