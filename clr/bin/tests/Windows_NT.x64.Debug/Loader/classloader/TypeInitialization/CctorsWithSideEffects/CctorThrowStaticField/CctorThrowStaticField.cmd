...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowStaticField CctorThrowStaticField.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowStaticField CctorThrowStaticField.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowStaticField.dasm.il CctorThrowStaticField.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowStaticField.dasm.il CctorThrowStaticField.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CctorThrowStaticField.asm.exe  CctorThrowStaticField.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CctorThrowStaticField.asm.exe  CctorThrowStaticField.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CctorThrowStaticField
    if not exist "CctorThrowStaticField.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CctorThrowStaticField.org" (
            mkdir IL
            copy CctorThrowStaticField.exe IL\CctorThrowStaticField.exe 
            ren CctorThrowStaticField.exe CctorThrowStaticField.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowStaticField.org /out CctorThrowStaticField.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowStaticField.org /out CctorThrowStaticField.exe
...
set ExePath=CctorThrowStaticField.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CctorThrowStaticField.reflect.xml
    IF EXIST CctorThrowStaticField.reflect.xml set ReflectionRoots=-x CctorThrowStaticField.reflect.xml
    ...
    set ExePath=%LinkBin%\CctorThrowStaticField.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CctorThrowStaticField.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CctorThrowStaticField.asm.exe %CLRTestExecutionArguments% %Host_Args%
