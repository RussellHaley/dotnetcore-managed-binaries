...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/UntrustedCodeBFI UntrustedCodeBFI.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/UntrustedCodeBFI UntrustedCodeBFI.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=UntrustedCodeBFI.dasm.il UntrustedCodeBFI.exe
    "$CORE_ROOT/ildasm" -raweh -out=UntrustedCodeBFI.dasm.il UntrustedCodeBFI.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=UntrustedCodeBFI.asm.exe -QUIET -NOLOGO -DEBUG UntrustedCodeBFI.dasm.il
    "$CORE_ROOT/ilasm" -output=UntrustedCodeBFI.asm.exe -QUIET -NOLOGO -DEBUG UntrustedCodeBFI.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=UntrustedCodeBFI
    if [ ! -f UntrustedCodeBFI.org ]; then
        TakeLock
        if [ ! -f UntrustedCodeBFI.org ]; then
          mkdir IL
          cp UntrustedCodeBFI.exe IL/UntrustedCodeBFI.exe
          mv UntrustedCodeBFI.exe UntrustedCodeBFI.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in UntrustedCodeBFI.org /out UntrustedCodeBFI.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in UntrustedCodeBFI.org /out UntrustedCodeBFI.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in UntrustedCodeBFI.reflect.xml
  if [ -f UntrustedCodeBFI.reflect.xml ]
then
    ReflectionRoots="-x UntrustedCodeBFI.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/UntrustedCodeBFI.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" UntrustedCodeBFI.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" UntrustedCodeBFI.asm.exe $CLRTestExecutionArguments
