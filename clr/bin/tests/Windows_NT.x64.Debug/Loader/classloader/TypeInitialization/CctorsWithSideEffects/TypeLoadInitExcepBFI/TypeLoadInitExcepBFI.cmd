...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\TypeLoadInitExcepBFI TypeLoadInitExcepBFI.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\TypeLoadInitExcepBFI TypeLoadInitExcepBFI.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=TypeLoadInitExcepBFI.dasm.il TypeLoadInitExcepBFI.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=TypeLoadInitExcepBFI.dasm.il TypeLoadInitExcepBFI.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=TypeLoadInitExcepBFI.asm.exe -QUIET -NOLOGO -DEBUG TypeLoadInitExcepBFI.dasm.il
    %CORE_ROOT%\ilasm.exe /output=TypeLoadInitExcepBFI.asm.exe -QUIET -NOLOGO -DEBUG TypeLoadInitExcepBFI.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=TypeLoadInitExcepBFI
    if not exist "TypeLoadInitExcepBFI.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "TypeLoadInitExcepBFI.org" (
            mkdir IL
            copy TypeLoadInitExcepBFI.exe IL\TypeLoadInitExcepBFI.exe 
            ren TypeLoadInitExcepBFI.exe TypeLoadInitExcepBFI.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TypeLoadInitExcepBFI.org /out TypeLoadInitExcepBFI.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TypeLoadInitExcepBFI.org /out TypeLoadInitExcepBFI.exe
...
set ExePath=TypeLoadInitExcepBFI.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in TypeLoadInitExcepBFI.reflect.xml
    IF EXIST TypeLoadInitExcepBFI.reflect.xml set ReflectionRoots=-x TypeLoadInitExcepBFI.reflect.xml
    ...
    set ExePath=%LinkBin%\TypeLoadInitExcepBFI.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% TypeLoadInitExcepBFI.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% TypeLoadInitExcepBFI.asm.exe %CLRTestExecutionArguments% %Host_Args%
