...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowInlinedStatic CctorThrowInlinedStatic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowInlinedStatic CctorThrowInlinedStatic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CctorThrowInlinedStatic.dasm.il CctorThrowInlinedStatic.exe
    "$CORE_ROOT/ildasm" -raweh -out=CctorThrowInlinedStatic.dasm.il CctorThrowInlinedStatic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CctorThrowInlinedStatic.asm.exe  CctorThrowInlinedStatic.dasm.il
    "$CORE_ROOT/ilasm" -output=CctorThrowInlinedStatic.asm.exe  CctorThrowInlinedStatic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CctorThrowInlinedStatic
    if [ ! -f CctorThrowInlinedStatic.org ]; then
        TakeLock
        if [ ! -f CctorThrowInlinedStatic.org ]; then
          mkdir IL
          cp CctorThrowInlinedStatic.exe IL/CctorThrowInlinedStatic.exe
          mv CctorThrowInlinedStatic.exe CctorThrowInlinedStatic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowInlinedStatic.org /out CctorThrowInlinedStatic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowInlinedStatic.org /out CctorThrowInlinedStatic.exe
          __cgExitCode=$?
...
ExePath=CctorThrowInlinedStatic.exe
...
then
  ...
  # Use hints for reflection roots, if provided in CctorThrowInlinedStatic.reflect.xml
  if [ -f CctorThrowInlinedStatic.reflect.xml ]
then
    ReflectionRoots="-x CctorThrowInlinedStatic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CctorThrowInlinedStatic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CctorThrowInlinedStatic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CctorThrowInlinedStatic.asm.exe $CLRTestExecutionArguments
