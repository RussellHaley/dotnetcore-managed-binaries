...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\TypeLoadInitExcep TypeLoadInitExcep.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\TypeLoadInitExcep TypeLoadInitExcep.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=TypeLoadInitExcep.dasm.il TypeLoadInitExcep.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=TypeLoadInitExcep.dasm.il TypeLoadInitExcep.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=TypeLoadInitExcep.asm.exe  TypeLoadInitExcep.dasm.il
    %CORE_ROOT%\ilasm.exe /output=TypeLoadInitExcep.asm.exe  TypeLoadInitExcep.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=TypeLoadInitExcep
    if not exist "TypeLoadInitExcep.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "TypeLoadInitExcep.org" (
            mkdir IL
            copy TypeLoadInitExcep.exe IL\TypeLoadInitExcep.exe 
            ren TypeLoadInitExcep.exe TypeLoadInitExcep.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TypeLoadInitExcep.org /out TypeLoadInitExcep.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TypeLoadInitExcep.org /out TypeLoadInitExcep.exe
...
set ExePath=TypeLoadInitExcep.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in TypeLoadInitExcep.reflect.xml
    IF EXIST TypeLoadInitExcep.reflect.xml set ReflectionRoots=-x TypeLoadInitExcep.reflect.xml
    ...
    set ExePath=%LinkBin%\TypeLoadInitExcep.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% TypeLoadInitExcep.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% TypeLoadInitExcep.asm.exe %CLRTestExecutionArguments% %Host_Args%
