...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\UntrustedCodeBFI UntrustedCodeBFI.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\UntrustedCodeBFI UntrustedCodeBFI.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=UntrustedCodeBFI.dasm.il UntrustedCodeBFI.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=UntrustedCodeBFI.dasm.il UntrustedCodeBFI.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=UntrustedCodeBFI.asm.exe -QUIET -NOLOGO -DEBUG UntrustedCodeBFI.dasm.il
    %CORE_ROOT%\ilasm.exe /output=UntrustedCodeBFI.asm.exe -QUIET -NOLOGO -DEBUG UntrustedCodeBFI.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=UntrustedCodeBFI
    if not exist "UntrustedCodeBFI.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "UntrustedCodeBFI.org" (
            mkdir IL
            copy UntrustedCodeBFI.exe IL\UntrustedCodeBFI.exe 
            ren UntrustedCodeBFI.exe UntrustedCodeBFI.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in UntrustedCodeBFI.org /out UntrustedCodeBFI.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in UntrustedCodeBFI.org /out UntrustedCodeBFI.exe
...
set ExePath=UntrustedCodeBFI.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in UntrustedCodeBFI.reflect.xml
    IF EXIST UntrustedCodeBFI.reflect.xml set ReflectionRoots=-x UntrustedCodeBFI.reflect.xml
    ...
    set ExePath=%LinkBin%\UntrustedCodeBFI.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% UntrustedCodeBFI.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% UntrustedCodeBFI.asm.exe %CLRTestExecutionArguments% %Host_Args%
