...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowInlinedStatic CctorThrowInlinedStatic.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowInlinedStatic CctorThrowInlinedStatic.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowInlinedStatic.dasm.il CctorThrowInlinedStatic.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowInlinedStatic.dasm.il CctorThrowInlinedStatic.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CctorThrowInlinedStatic.asm.exe  CctorThrowInlinedStatic.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CctorThrowInlinedStatic.asm.exe  CctorThrowInlinedStatic.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CctorThrowInlinedStatic
    if not exist "CctorThrowInlinedStatic.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CctorThrowInlinedStatic.org" (
            mkdir IL
            copy CctorThrowInlinedStatic.exe IL\CctorThrowInlinedStatic.exe 
            ren CctorThrowInlinedStatic.exe CctorThrowInlinedStatic.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowInlinedStatic.org /out CctorThrowInlinedStatic.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowInlinedStatic.org /out CctorThrowInlinedStatic.exe
...
set ExePath=CctorThrowInlinedStatic.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CctorThrowInlinedStatic.reflect.xml
    IF EXIST CctorThrowInlinedStatic.reflect.xml set ReflectionRoots=-x CctorThrowInlinedStatic.reflect.xml
    ...
    set ExePath=%LinkBin%\CctorThrowInlinedStatic.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CctorThrowInlinedStatic.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CctorThrowInlinedStatic.asm.exe %CLRTestExecutionArguments% %Host_Args%
