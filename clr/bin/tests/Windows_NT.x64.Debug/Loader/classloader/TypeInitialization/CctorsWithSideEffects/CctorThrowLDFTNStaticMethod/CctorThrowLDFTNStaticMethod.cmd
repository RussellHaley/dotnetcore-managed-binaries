...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowLDFTNStaticMethod CctorThrowLDFTNStaticMethod.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowLDFTNStaticMethod CctorThrowLDFTNStaticMethod.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowLDFTNStaticMethod.dasm.il CctorThrowLDFTNStaticMethod.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowLDFTNStaticMethod.dasm.il CctorThrowLDFTNStaticMethod.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CctorThrowLDFTNStaticMethod.asm.exe -QUIET -NOLOGO -DEBUG CctorThrowLDFTNStaticMethod.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CctorThrowLDFTNStaticMethod.asm.exe -QUIET -NOLOGO -DEBUG CctorThrowLDFTNStaticMethod.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CctorThrowLDFTNStaticMethod
    if not exist "CctorThrowLDFTNStaticMethod.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CctorThrowLDFTNStaticMethod.org" (
            mkdir IL
            copy CctorThrowLDFTNStaticMethod.exe IL\CctorThrowLDFTNStaticMethod.exe 
            ren CctorThrowLDFTNStaticMethod.exe CctorThrowLDFTNStaticMethod.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowLDFTNStaticMethod.org /out CctorThrowLDFTNStaticMethod.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowLDFTNStaticMethod.org /out CctorThrowLDFTNStaticMethod.exe
...
set ExePath=CctorThrowLDFTNStaticMethod.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CctorThrowLDFTNStaticMethod.reflect.xml
    IF EXIST CctorThrowLDFTNStaticMethod.reflect.xml set ReflectionRoots=-x CctorThrowLDFTNStaticMethod.reflect.xml
    ...
    set ExePath=%LinkBin%\CctorThrowLDFTNStaticMethod.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CctorThrowLDFTNStaticMethod.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CctorThrowLDFTNStaticMethod.asm.exe %CLRTestExecutionArguments% %Host_Args%
