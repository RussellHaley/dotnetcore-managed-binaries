...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowStaticFieldBFI CctorThrowStaticFieldBFI.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\TypeInitialization\CctorsWithSideEffects\CctorThrowStaticFieldBFI CctorThrowStaticFieldBFI.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowStaticFieldBFI.dasm.il CctorThrowStaticFieldBFI.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CctorThrowStaticFieldBFI.dasm.il CctorThrowStaticFieldBFI.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CctorThrowStaticFieldBFI.asm.exe -QUIET -NOLOGO -DEBUG CctorThrowStaticFieldBFI.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CctorThrowStaticFieldBFI.asm.exe -QUIET -NOLOGO -DEBUG CctorThrowStaticFieldBFI.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CctorThrowStaticFieldBFI
    if not exist "CctorThrowStaticFieldBFI.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CctorThrowStaticFieldBFI.org" (
            mkdir IL
            copy CctorThrowStaticFieldBFI.exe IL\CctorThrowStaticFieldBFI.exe 
            ren CctorThrowStaticFieldBFI.exe CctorThrowStaticFieldBFI.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowStaticFieldBFI.org /out CctorThrowStaticFieldBFI.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CctorThrowStaticFieldBFI.org /out CctorThrowStaticFieldBFI.exe
...
set ExePath=CctorThrowStaticFieldBFI.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CctorThrowStaticFieldBFI.reflect.xml
    IF EXIST CctorThrowStaticFieldBFI.reflect.xml set ReflectionRoots=-x CctorThrowStaticFieldBFI.reflect.xml
    ...
    set ExePath=%LinkBin%\CctorThrowStaticFieldBFI.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CctorThrowStaticFieldBFI.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CctorThrowStaticFieldBFI.asm.exe %CLRTestExecutionArguments% %Host_Args%
