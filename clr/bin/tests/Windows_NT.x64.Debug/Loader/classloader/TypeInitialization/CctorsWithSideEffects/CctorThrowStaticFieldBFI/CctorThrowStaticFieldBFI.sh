...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowStaticFieldBFI CctorThrowStaticFieldBFI.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/TypeInitialization/CctorsWithSideEffects/CctorThrowStaticFieldBFI CctorThrowStaticFieldBFI.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CctorThrowStaticFieldBFI.dasm.il CctorThrowStaticFieldBFI.exe
    "$CORE_ROOT/ildasm" -raweh -out=CctorThrowStaticFieldBFI.dasm.il CctorThrowStaticFieldBFI.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CctorThrowStaticFieldBFI.asm.exe -QUIET -NOLOGO -DEBUG CctorThrowStaticFieldBFI.dasm.il
    "$CORE_ROOT/ilasm" -output=CctorThrowStaticFieldBFI.asm.exe -QUIET -NOLOGO -DEBUG CctorThrowStaticFieldBFI.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CctorThrowStaticFieldBFI
    if [ ! -f CctorThrowStaticFieldBFI.org ]; then
        TakeLock
        if [ ! -f CctorThrowStaticFieldBFI.org ]; then
          mkdir IL
          cp CctorThrowStaticFieldBFI.exe IL/CctorThrowStaticFieldBFI.exe
          mv CctorThrowStaticFieldBFI.exe CctorThrowStaticFieldBFI.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowStaticFieldBFI.org /out CctorThrowStaticFieldBFI.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CctorThrowStaticFieldBFI.org /out CctorThrowStaticFieldBFI.exe
          __cgExitCode=$?
...
ExePath=CctorThrowStaticFieldBFI.exe
...
then
  ...
  # Use hints for reflection roots, if provided in CctorThrowStaticFieldBFI.reflect.xml
  if [ -f CctorThrowStaticFieldBFI.reflect.xml ]
then
    ReflectionRoots="-x CctorThrowStaticFieldBFI.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CctorThrowStaticFieldBFI.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CctorThrowStaticFieldBFI.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CctorThrowStaticFieldBFI.asm.exe $CLRTestExecutionArguments
