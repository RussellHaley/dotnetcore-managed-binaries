...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase07 AbstractBase07.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase07 AbstractBase07.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbstractBase07.dasm.il AbstractBase07.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbstractBase07.dasm.il AbstractBase07.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbstractBase07.asm.exe  AbstractBase07.dasm.il
    "$CORE_ROOT/ilasm" -output=AbstractBase07.asm.exe  AbstractBase07.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbstractBase07
    if [ ! -f AbstractBase07.org ]; then
        TakeLock
        if [ ! -f AbstractBase07.org ]; then
          mkdir IL
          cp AbstractBase07.exe IL/AbstractBase07.exe
          mv AbstractBase07.exe AbstractBase07.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase07.org /out AbstractBase07.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase07.org /out AbstractBase07.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbstractBase07.reflect.xml
  if [ -f AbstractBase07.reflect.xml ]
then
    ReflectionRoots="-x AbstractBase07.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AbstractBase07.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbstractBase07.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbstractBase07.asm.exe $CLRTestExecutionArguments
