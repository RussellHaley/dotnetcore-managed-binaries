...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface02 NestedInterface02.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface02 NestedInterface02.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedInterface02.dasm.il NestedInterface02.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedInterface02.dasm.il NestedInterface02.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedInterface02.asm.exe  NestedInterface02.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedInterface02.asm.exe  NestedInterface02.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedInterface02
    if [ ! -f NestedInterface02.org ]; then
        TakeLock
        if [ ! -f NestedInterface02.org ]; then
          mkdir IL
          cp NestedInterface02.exe IL/NestedInterface02.exe
          mv NestedInterface02.exe NestedInterface02.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface02.org /out NestedInterface02.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface02.org /out NestedInterface02.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedInterface02.reflect.xml
  if [ -f NestedInterface02.reflect.xml ]
then
    ReflectionRoots="-x NestedInterface02.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedInterface02.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedInterface02.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedInterface02.asm.exe $CLRTestExecutionArguments
