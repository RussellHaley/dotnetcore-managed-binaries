...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass01 NestedBaseClass01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass01 NestedBaseClass01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass01.dasm.il NestedBaseClass01.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass01.dasm.il NestedBaseClass01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedBaseClass01.asm.exe  NestedBaseClass01.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedBaseClass01.asm.exe  NestedBaseClass01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedBaseClass01
    if [ ! -f NestedBaseClass01.org ]; then
        TakeLock
        if [ ! -f NestedBaseClass01.org ]; then
          mkdir IL
          cp NestedBaseClass01.exe IL/NestedBaseClass01.exe
          mv NestedBaseClass01.exe NestedBaseClass01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass01.org /out NestedBaseClass01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass01.org /out NestedBaseClass01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedBaseClass01.reflect.xml
  if [ -f NestedBaseClass01.reflect.xml ]
then
    ReflectionRoots="-x NestedBaseClass01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedBaseClass01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedBaseClass01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedBaseClass01.asm.exe $CLRTestExecutionArguments
