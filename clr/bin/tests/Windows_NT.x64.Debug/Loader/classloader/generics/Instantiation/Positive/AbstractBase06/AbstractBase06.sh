...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase06 AbstractBase06.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase06 AbstractBase06.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbstractBase06.dasm.il AbstractBase06.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbstractBase06.dasm.il AbstractBase06.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbstractBase06.asm.exe  AbstractBase06.dasm.il
    "$CORE_ROOT/ilasm" -output=AbstractBase06.asm.exe  AbstractBase06.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbstractBase06
    if [ ! -f AbstractBase06.org ]; then
        TakeLock
        if [ ! -f AbstractBase06.org ]; then
          mkdir IL
          cp AbstractBase06.exe IL/AbstractBase06.exe
          mv AbstractBase06.exe AbstractBase06.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase06.org /out AbstractBase06.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase06.org /out AbstractBase06.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbstractBase06.reflect.xml
  if [ -f AbstractBase06.reflect.xml ]
then
    ReflectionRoots="-x AbstractBase06.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AbstractBase06.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbstractBase06.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbstractBase06.asm.exe $CLRTestExecutionArguments
