...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase01 AbstractBase01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase01 AbstractBase01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase01.dasm.il AbstractBase01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase01.dasm.il AbstractBase01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbstractBase01.asm.exe  AbstractBase01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbstractBase01.asm.exe  AbstractBase01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbstractBase01
    if not exist "AbstractBase01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbstractBase01.org" (
            mkdir IL
            copy AbstractBase01.exe IL\AbstractBase01.exe 
            ren AbstractBase01.exe AbstractBase01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase01.org /out AbstractBase01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase01.org /out AbstractBase01.exe
...
set ExePath=AbstractBase01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbstractBase01.reflect.xml
    IF EXIST AbstractBase01.reflect.xml set ReflectionRoots=-x AbstractBase01.reflect.xml
    ...
    set ExePath=%LinkBin%\AbstractBase01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbstractBase01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbstractBase01.asm.exe %CLRTestExecutionArguments% %Host_Args%
