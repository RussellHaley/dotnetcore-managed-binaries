...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface05 NestedInterface05.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface05 NestedInterface05.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedInterface05.dasm.il NestedInterface05.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedInterface05.dasm.il NestedInterface05.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedInterface05.asm.exe  NestedInterface05.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedInterface05.asm.exe  NestedInterface05.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedInterface05
    if [ ! -f NestedInterface05.org ]; then
        TakeLock
        if [ ! -f NestedInterface05.org ]; then
          mkdir IL
          cp NestedInterface05.exe IL/NestedInterface05.exe
          mv NestedInterface05.exe NestedInterface05.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface05.org /out NestedInterface05.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface05.org /out NestedInterface05.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedInterface05.reflect.xml
  if [ -f NestedInterface05.reflect.xml ]
then
    ReflectionRoots="-x NestedInterface05.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedInterface05.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedInterface05.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedInterface05.asm.exe $CLRTestExecutionArguments
