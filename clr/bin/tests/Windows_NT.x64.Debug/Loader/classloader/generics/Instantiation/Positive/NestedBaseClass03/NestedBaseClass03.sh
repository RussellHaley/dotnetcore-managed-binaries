...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass03 NestedBaseClass03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass03 NestedBaseClass03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass03.dasm.il NestedBaseClass03.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass03.dasm.il NestedBaseClass03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedBaseClass03.asm.exe  NestedBaseClass03.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedBaseClass03.asm.exe  NestedBaseClass03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedBaseClass03
    if [ ! -f NestedBaseClass03.org ]; then
        TakeLock
        if [ ! -f NestedBaseClass03.org ]; then
          mkdir IL
          cp NestedBaseClass03.exe IL/NestedBaseClass03.exe
          mv NestedBaseClass03.exe NestedBaseClass03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass03.org /out NestedBaseClass03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass03.org /out NestedBaseClass03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedBaseClass03.reflect.xml
  if [ -f NestedBaseClass03.reflect.xml ]
then
    ReflectionRoots="-x NestedBaseClass03.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedBaseClass03.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedBaseClass03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedBaseClass03.asm.exe $CLRTestExecutionArguments
