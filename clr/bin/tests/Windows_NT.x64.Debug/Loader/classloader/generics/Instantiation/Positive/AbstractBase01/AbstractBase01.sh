...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase01 AbstractBase01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase01 AbstractBase01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbstractBase01.dasm.il AbstractBase01.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbstractBase01.dasm.il AbstractBase01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbstractBase01.asm.exe  AbstractBase01.dasm.il
    "$CORE_ROOT/ilasm" -output=AbstractBase01.asm.exe  AbstractBase01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbstractBase01
    if [ ! -f AbstractBase01.org ]; then
        TakeLock
        if [ ! -f AbstractBase01.org ]; then
          mkdir IL
          cp AbstractBase01.exe IL/AbstractBase01.exe
          mv AbstractBase01.exe AbstractBase01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase01.org /out AbstractBase01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase01.org /out AbstractBase01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbstractBase01.reflect.xml
  if [ -f AbstractBase01.reflect.xml ]
then
    ReflectionRoots="-x AbstractBase01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AbstractBase01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbstractBase01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbstractBase01.asm.exe $CLRTestExecutionArguments
