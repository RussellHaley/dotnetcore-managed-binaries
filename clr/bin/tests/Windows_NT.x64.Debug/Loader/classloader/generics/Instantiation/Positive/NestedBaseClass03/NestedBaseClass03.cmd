...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass03 NestedBaseClass03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass03 NestedBaseClass03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass03.dasm.il NestedBaseClass03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass03.dasm.il NestedBaseClass03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedBaseClass03.asm.exe  NestedBaseClass03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedBaseClass03.asm.exe  NestedBaseClass03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedBaseClass03
    if not exist "NestedBaseClass03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedBaseClass03.org" (
            mkdir IL
            copy NestedBaseClass03.exe IL\NestedBaseClass03.exe 
            ren NestedBaseClass03.exe NestedBaseClass03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass03.org /out NestedBaseClass03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass03.org /out NestedBaseClass03.exe
...
set ExePath=NestedBaseClass03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedBaseClass03.reflect.xml
    IF EXIST NestedBaseClass03.reflect.xml set ReflectionRoots=-x NestedBaseClass03.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedBaseClass03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedBaseClass03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedBaseClass03.asm.exe %CLRTestExecutionArguments% %Host_Args%
