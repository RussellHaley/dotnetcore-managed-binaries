...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface06 NestedInterface06.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface06 NestedInterface06.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface06.dasm.il NestedInterface06.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface06.dasm.il NestedInterface06.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedInterface06.asm.exe  NestedInterface06.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedInterface06.asm.exe  NestedInterface06.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedInterface06
    if not exist "NestedInterface06.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedInterface06.org" (
            mkdir IL
            copy NestedInterface06.exe IL\NestedInterface06.exe 
            ren NestedInterface06.exe NestedInterface06.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface06.org /out NestedInterface06.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface06.org /out NestedInterface06.exe
...
set ExePath=NestedInterface06.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedInterface06.reflect.xml
    IF EXIST NestedInterface06.reflect.xml set ReflectionRoots=-x NestedInterface06.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedInterface06.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedInterface06.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedInterface06.asm.exe %CLRTestExecutionArguments% %Host_Args%
