...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase06 AbstractBase06.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase06 AbstractBase06.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase06.dasm.il AbstractBase06.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase06.dasm.il AbstractBase06.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbstractBase06.asm.exe  AbstractBase06.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbstractBase06.asm.exe  AbstractBase06.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbstractBase06
    if not exist "AbstractBase06.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbstractBase06.org" (
            mkdir IL
            copy AbstractBase06.exe IL\AbstractBase06.exe 
            ren AbstractBase06.exe AbstractBase06.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase06.org /out AbstractBase06.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase06.org /out AbstractBase06.exe
...
set ExePath=AbstractBase06.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbstractBase06.reflect.xml
    IF EXIST AbstractBase06.reflect.xml set ReflectionRoots=-x AbstractBase06.reflect.xml
    ...
    set ExePath=%LinkBin%\AbstractBase06.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbstractBase06.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbstractBase06.asm.exe %CLRTestExecutionArguments% %Host_Args%
