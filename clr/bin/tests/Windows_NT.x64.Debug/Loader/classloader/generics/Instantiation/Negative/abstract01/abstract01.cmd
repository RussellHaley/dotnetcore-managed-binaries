...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Negative\abstract01 abstract01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Negative\abstract01 abstract01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=abstract01.dasm.il abstract01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=abstract01.dasm.il abstract01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=abstract01.asm.exe -QUIET -NOLOGO -DEBUG abstract01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=abstract01.asm.exe -QUIET -NOLOGO -DEBUG abstract01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=abstract01
    if not exist "abstract01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "abstract01.org" (
            mkdir IL
            copy abstract01.exe IL\abstract01.exe 
            ren abstract01.exe abstract01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in abstract01.org /out abstract01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in abstract01.org /out abstract01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in abstract01.reflect.xml
    IF EXIST abstract01.reflect.xml set ReflectionRoots=-x abstract01.reflect.xml
    ...
    set ExePath=%LinkBin%\abstract01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% abstract01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% abstract01.asm.exe %CLRTestExecutionArguments% %Host_Args%
