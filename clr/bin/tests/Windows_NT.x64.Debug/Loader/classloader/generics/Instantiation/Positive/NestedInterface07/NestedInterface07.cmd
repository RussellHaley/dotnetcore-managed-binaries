...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface07 NestedInterface07.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface07 NestedInterface07.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface07.dasm.il NestedInterface07.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface07.dasm.il NestedInterface07.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedInterface07.asm.exe  NestedInterface07.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedInterface07.asm.exe  NestedInterface07.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedInterface07
    if not exist "NestedInterface07.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedInterface07.org" (
            mkdir IL
            copy NestedInterface07.exe IL\NestedInterface07.exe 
            ren NestedInterface07.exe NestedInterface07.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface07.org /out NestedInterface07.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface07.org /out NestedInterface07.exe
...
set ExePath=NestedInterface07.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedInterface07.reflect.xml
    IF EXIST NestedInterface07.reflect.xml set ReflectionRoots=-x NestedInterface07.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedInterface07.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedInterface07.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedInterface07.asm.exe %CLRTestExecutionArguments% %Host_Args%
