...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface01 NestedInterface01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface01 NestedInterface01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedInterface01.dasm.il NestedInterface01.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedInterface01.dasm.il NestedInterface01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedInterface01.asm.exe  NestedInterface01.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedInterface01.asm.exe  NestedInterface01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedInterface01
    if [ ! -f NestedInterface01.org ]; then
        TakeLock
        if [ ! -f NestedInterface01.org ]; then
          mkdir IL
          cp NestedInterface01.exe IL/NestedInterface01.exe
          mv NestedInterface01.exe NestedInterface01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface01.org /out NestedInterface01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface01.org /out NestedInterface01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedInterface01.reflect.xml
  if [ -f NestedInterface01.reflect.xml ]
then
    ReflectionRoots="-x NestedInterface01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedInterface01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedInterface01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedInterface01.asm.exe $CLRTestExecutionArguments
