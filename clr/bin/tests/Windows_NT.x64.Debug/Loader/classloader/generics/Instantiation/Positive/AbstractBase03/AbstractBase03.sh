...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase03 AbstractBase03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase03 AbstractBase03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbstractBase03.dasm.il AbstractBase03.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbstractBase03.dasm.il AbstractBase03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbstractBase03.asm.exe  AbstractBase03.dasm.il
    "$CORE_ROOT/ilasm" -output=AbstractBase03.asm.exe  AbstractBase03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbstractBase03
    if [ ! -f AbstractBase03.org ]; then
        TakeLock
        if [ ! -f AbstractBase03.org ]; then
          mkdir IL
          cp AbstractBase03.exe IL/AbstractBase03.exe
          mv AbstractBase03.exe AbstractBase03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase03.org /out AbstractBase03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase03.org /out AbstractBase03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbstractBase03.reflect.xml
  if [ -f AbstractBase03.reflect.xml ]
then
    ReflectionRoots="-x AbstractBase03.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AbstractBase03.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbstractBase03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbstractBase03.asm.exe $CLRTestExecutionArguments
