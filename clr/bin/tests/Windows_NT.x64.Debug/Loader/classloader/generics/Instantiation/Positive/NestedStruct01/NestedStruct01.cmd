...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedStruct01 NestedStruct01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedStruct01 NestedStruct01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedStruct01.dasm.il NestedStruct01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedStruct01.dasm.il NestedStruct01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedStruct01.asm.exe  NestedStruct01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedStruct01.asm.exe  NestedStruct01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedStruct01
    if not exist "NestedStruct01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedStruct01.org" (
            mkdir IL
            copy NestedStruct01.exe IL\NestedStruct01.exe 
            ren NestedStruct01.exe NestedStruct01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedStruct01.org /out NestedStruct01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedStruct01.org /out NestedStruct01.exe
...
set ExePath=NestedStruct01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedStruct01.reflect.xml
    IF EXIST NestedStruct01.reflect.xml set ReflectionRoots=-x NestedStruct01.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedStruct01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedStruct01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedStruct01.asm.exe %CLRTestExecutionArguments% %Host_Args%
