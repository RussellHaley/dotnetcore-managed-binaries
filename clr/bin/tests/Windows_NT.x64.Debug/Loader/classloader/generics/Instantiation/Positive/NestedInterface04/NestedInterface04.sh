...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface04 NestedInterface04.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface04 NestedInterface04.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedInterface04.dasm.il NestedInterface04.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedInterface04.dasm.il NestedInterface04.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedInterface04.asm.exe  NestedInterface04.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedInterface04.asm.exe  NestedInterface04.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedInterface04
    if [ ! -f NestedInterface04.org ]; then
        TakeLock
        if [ ! -f NestedInterface04.org ]; then
          mkdir IL
          cp NestedInterface04.exe IL/NestedInterface04.exe
          mv NestedInterface04.exe NestedInterface04.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface04.org /out NestedInterface04.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface04.org /out NestedInterface04.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedInterface04.reflect.xml
  if [ -f NestedInterface04.reflect.xml ]
then
    ReflectionRoots="-x NestedInterface04.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedInterface04.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedInterface04.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedInterface04.asm.exe $CLRTestExecutionArguments
