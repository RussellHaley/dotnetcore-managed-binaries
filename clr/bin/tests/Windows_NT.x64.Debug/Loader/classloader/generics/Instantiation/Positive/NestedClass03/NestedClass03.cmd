...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedClass03 NestedClass03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedClass03 NestedClass03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedClass03.dasm.il NestedClass03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedClass03.dasm.il NestedClass03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedClass03.asm.exe  NestedClass03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedClass03.asm.exe  NestedClass03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedClass03
    if not exist "NestedClass03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedClass03.org" (
            mkdir IL
            copy NestedClass03.exe IL\NestedClass03.exe 
            ren NestedClass03.exe NestedClass03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedClass03.org /out NestedClass03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedClass03.org /out NestedClass03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedClass03.reflect.xml
    IF EXIST NestedClass03.reflect.xml set ReflectionRoots=-x NestedClass03.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedClass03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedClass03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedClass03.asm.exe %CLRTestExecutionArguments% %Host_Args%
