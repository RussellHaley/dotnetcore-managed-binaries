...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface02 NestedInterface02.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface02 NestedInterface02.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface02.dasm.il NestedInterface02.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface02.dasm.il NestedInterface02.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedInterface02.asm.exe  NestedInterface02.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedInterface02.asm.exe  NestedInterface02.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedInterface02
    if not exist "NestedInterface02.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedInterface02.org" (
            mkdir IL
            copy NestedInterface02.exe IL\NestedInterface02.exe 
            ren NestedInterface02.exe NestedInterface02.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface02.org /out NestedInterface02.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface02.org /out NestedInterface02.exe
...
set ExePath=NestedInterface02.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedInterface02.reflect.xml
    IF EXIST NestedInterface02.reflect.xml set ReflectionRoots=-x NestedInterface02.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedInterface02.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedInterface02.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedInterface02.asm.exe %CLRTestExecutionArguments% %Host_Args%
