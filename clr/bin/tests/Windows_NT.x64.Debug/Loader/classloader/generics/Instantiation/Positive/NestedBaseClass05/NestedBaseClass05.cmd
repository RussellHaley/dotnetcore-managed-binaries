...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass05 NestedBaseClass05.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass05 NestedBaseClass05.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass05.dasm.il NestedBaseClass05.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass05.dasm.il NestedBaseClass05.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedBaseClass05.asm.exe  NestedBaseClass05.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedBaseClass05.asm.exe  NestedBaseClass05.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedBaseClass05
    if not exist "NestedBaseClass05.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedBaseClass05.org" (
            mkdir IL
            copy NestedBaseClass05.exe IL\NestedBaseClass05.exe 
            ren NestedBaseClass05.exe NestedBaseClass05.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass05.org /out NestedBaseClass05.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass05.org /out NestedBaseClass05.exe
...
set ExePath=NestedBaseClass05.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedBaseClass05.reflect.xml
    IF EXIST NestedBaseClass05.reflect.xml set ReflectionRoots=-x NestedBaseClass05.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedBaseClass05.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedBaseClass05.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedBaseClass05.asm.exe %CLRTestExecutionArguments% %Host_Args%
