...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase03 AbstractBase03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase03 AbstractBase03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase03.dasm.il AbstractBase03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase03.dasm.il AbstractBase03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbstractBase03.asm.exe  AbstractBase03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbstractBase03.asm.exe  AbstractBase03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbstractBase03
    if not exist "AbstractBase03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbstractBase03.org" (
            mkdir IL
            copy AbstractBase03.exe IL\AbstractBase03.exe 
            ren AbstractBase03.exe AbstractBase03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase03.org /out AbstractBase03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase03.org /out AbstractBase03.exe
...
set ExePath=AbstractBase03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbstractBase03.reflect.xml
    IF EXIST AbstractBase03.reflect.xml set ReflectionRoots=-x AbstractBase03.reflect.xml
    ...
    set ExePath=%LinkBin%\AbstractBase03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbstractBase03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbstractBase03.asm.exe %CLRTestExecutionArguments% %Host_Args%
