...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass04 NestedBaseClass04.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass04 NestedBaseClass04.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass04.dasm.il NestedBaseClass04.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass04.dasm.il NestedBaseClass04.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedBaseClass04.asm.exe  NestedBaseClass04.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedBaseClass04.asm.exe  NestedBaseClass04.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedBaseClass04
    if [ ! -f NestedBaseClass04.org ]; then
        TakeLock
        if [ ! -f NestedBaseClass04.org ]; then
          mkdir IL
          cp NestedBaseClass04.exe IL/NestedBaseClass04.exe
          mv NestedBaseClass04.exe NestedBaseClass04.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass04.org /out NestedBaseClass04.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass04.org /out NestedBaseClass04.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedBaseClass04.reflect.xml
  if [ -f NestedBaseClass04.reflect.xml ]
then
    ReflectionRoots="-x NestedBaseClass04.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedBaseClass04.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedBaseClass04.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedBaseClass04.asm.exe $CLRTestExecutionArguments
