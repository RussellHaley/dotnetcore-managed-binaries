...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface03 NestedInterface03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface03 NestedInterface03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedInterface03.dasm.il NestedInterface03.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedInterface03.dasm.il NestedInterface03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedInterface03.asm.exe  NestedInterface03.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedInterface03.asm.exe  NestedInterface03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedInterface03
    if [ ! -f NestedInterface03.org ]; then
        TakeLock
        if [ ! -f NestedInterface03.org ]; then
          mkdir IL
          cp NestedInterface03.exe IL/NestedInterface03.exe
          mv NestedInterface03.exe NestedInterface03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface03.org /out NestedInterface03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface03.org /out NestedInterface03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedInterface03.reflect.xml
  if [ -f NestedInterface03.reflect.xml ]
then
    ReflectionRoots="-x NestedInterface03.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedInterface03.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedInterface03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedInterface03.asm.exe $CLRTestExecutionArguments
