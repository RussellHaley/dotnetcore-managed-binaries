...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface08 NestedInterface08.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface08 NestedInterface08.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface08.dasm.il NestedInterface08.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface08.dasm.il NestedInterface08.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedInterface08.asm.exe  NestedInterface08.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedInterface08.asm.exe  NestedInterface08.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedInterface08
    if not exist "NestedInterface08.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedInterface08.org" (
            mkdir IL
            copy NestedInterface08.exe IL\NestedInterface08.exe 
            ren NestedInterface08.exe NestedInterface08.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface08.org /out NestedInterface08.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface08.org /out NestedInterface08.exe
...
set ExePath=NestedInterface08.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedInterface08.reflect.xml
    IF EXIST NestedInterface08.reflect.xml set ReflectionRoots=-x NestedInterface08.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedInterface08.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedInterface08.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedInterface08.asm.exe %CLRTestExecutionArguments% %Host_Args%
