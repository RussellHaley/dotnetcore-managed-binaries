...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedStruct01 NestedStruct01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedStruct01 NestedStruct01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedStruct01.dasm.il NestedStruct01.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedStruct01.dasm.il NestedStruct01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedStruct01.asm.exe  NestedStruct01.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedStruct01.asm.exe  NestedStruct01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedStruct01
    if [ ! -f NestedStruct01.org ]; then
        TakeLock
        if [ ! -f NestedStruct01.org ]; then
          mkdir IL
          cp NestedStruct01.exe IL/NestedStruct01.exe
          mv NestedStruct01.exe NestedStruct01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedStruct01.org /out NestedStruct01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedStruct01.org /out NestedStruct01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedStruct01.reflect.xml
  if [ -f NestedStruct01.reflect.xml ]
then
    ReflectionRoots="-x NestedStruct01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedStruct01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedStruct01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedStruct01.asm.exe $CLRTestExecutionArguments
