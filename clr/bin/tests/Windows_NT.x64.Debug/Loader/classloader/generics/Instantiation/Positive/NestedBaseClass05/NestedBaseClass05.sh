...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass05 NestedBaseClass05.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass05 NestedBaseClass05.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass05.dasm.il NestedBaseClass05.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass05.dasm.il NestedBaseClass05.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedBaseClass05.asm.exe  NestedBaseClass05.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedBaseClass05.asm.exe  NestedBaseClass05.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedBaseClass05
    if [ ! -f NestedBaseClass05.org ]; then
        TakeLock
        if [ ! -f NestedBaseClass05.org ]; then
          mkdir IL
          cp NestedBaseClass05.exe IL/NestedBaseClass05.exe
          mv NestedBaseClass05.exe NestedBaseClass05.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass05.org /out NestedBaseClass05.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass05.org /out NestedBaseClass05.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedBaseClass05.reflect.xml
  if [ -f NestedBaseClass05.reflect.xml ]
then
    ReflectionRoots="-x NestedBaseClass05.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedBaseClass05.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedBaseClass05.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedBaseClass05.asm.exe $CLRTestExecutionArguments
