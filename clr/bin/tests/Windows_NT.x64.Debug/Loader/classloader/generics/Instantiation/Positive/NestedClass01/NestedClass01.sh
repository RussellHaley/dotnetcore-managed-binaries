...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedClass01 NestedClass01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedClass01 NestedClass01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedClass01.dasm.il NestedClass01.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedClass01.dasm.il NestedClass01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedClass01.asm.exe  NestedClass01.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedClass01.asm.exe  NestedClass01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedClass01
    if [ ! -f NestedClass01.org ]; then
        TakeLock
        if [ ! -f NestedClass01.org ]; then
          mkdir IL
          cp NestedClass01.exe IL/NestedClass01.exe
          mv NestedClass01.exe NestedClass01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedClass01.org /out NestedClass01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedClass01.org /out NestedClass01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedClass01.reflect.xml
  if [ -f NestedClass01.reflect.xml ]
then
    ReflectionRoots="-x NestedClass01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedClass01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedClass01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedClass01.asm.exe $CLRTestExecutionArguments
