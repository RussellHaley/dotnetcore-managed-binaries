...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass04 NestedBaseClass04.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass04 NestedBaseClass04.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass04.dasm.il NestedBaseClass04.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass04.dasm.il NestedBaseClass04.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedBaseClass04.asm.exe  NestedBaseClass04.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedBaseClass04.asm.exe  NestedBaseClass04.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedBaseClass04
    if not exist "NestedBaseClass04.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedBaseClass04.org" (
            mkdir IL
            copy NestedBaseClass04.exe IL\NestedBaseClass04.exe 
            ren NestedBaseClass04.exe NestedBaseClass04.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass04.org /out NestedBaseClass04.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass04.org /out NestedBaseClass04.exe
...
set ExePath=NestedBaseClass04.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedBaseClass04.reflect.xml
    IF EXIST NestedBaseClass04.reflect.xml set ReflectionRoots=-x NestedBaseClass04.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedBaseClass04.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedBaseClass04.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedBaseClass04.asm.exe %CLRTestExecutionArguments% %Host_Args%
