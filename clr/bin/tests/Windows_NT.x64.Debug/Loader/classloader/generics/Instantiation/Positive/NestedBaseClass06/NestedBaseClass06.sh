...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass06 NestedBaseClass06.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass06 NestedBaseClass06.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass06.dasm.il NestedBaseClass06.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass06.dasm.il NestedBaseClass06.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedBaseClass06.asm.exe  NestedBaseClass06.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedBaseClass06.asm.exe  NestedBaseClass06.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedBaseClass06
    if [ ! -f NestedBaseClass06.org ]; then
        TakeLock
        if [ ! -f NestedBaseClass06.org ]; then
          mkdir IL
          cp NestedBaseClass06.exe IL/NestedBaseClass06.exe
          mv NestedBaseClass06.exe NestedBaseClass06.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass06.org /out NestedBaseClass06.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass06.org /out NestedBaseClass06.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedBaseClass06.reflect.xml
  if [ -f NestedBaseClass06.reflect.xml ]
then
    ReflectionRoots="-x NestedBaseClass06.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedBaseClass06.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedBaseClass06.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedBaseClass06.asm.exe $CLRTestExecutionArguments
