...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface03 NestedInterface03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface03 NestedInterface03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface03.dasm.il NestedInterface03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface03.dasm.il NestedInterface03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedInterface03.asm.exe  NestedInterface03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedInterface03.asm.exe  NestedInterface03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedInterface03
    if not exist "NestedInterface03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedInterface03.org" (
            mkdir IL
            copy NestedInterface03.exe IL\NestedInterface03.exe 
            ren NestedInterface03.exe NestedInterface03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface03.org /out NestedInterface03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface03.org /out NestedInterface03.exe
...
set ExePath=NestedInterface03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedInterface03.reflect.xml
    IF EXIST NestedInterface03.reflect.xml set ReflectionRoots=-x NestedInterface03.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedInterface03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedInterface03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedInterface03.asm.exe %CLRTestExecutionArguments% %Host_Args%
