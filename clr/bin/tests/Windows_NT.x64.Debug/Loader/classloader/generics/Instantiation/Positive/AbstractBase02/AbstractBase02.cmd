...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase02 AbstractBase02.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase02 AbstractBase02.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase02.dasm.il AbstractBase02.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase02.dasm.il AbstractBase02.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbstractBase02.asm.exe  AbstractBase02.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbstractBase02.asm.exe  AbstractBase02.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbstractBase02
    if not exist "AbstractBase02.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbstractBase02.org" (
            mkdir IL
            copy AbstractBase02.exe IL\AbstractBase02.exe 
            ren AbstractBase02.exe AbstractBase02.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase02.org /out AbstractBase02.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase02.org /out AbstractBase02.exe
...
set ExePath=AbstractBase02.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbstractBase02.reflect.xml
    IF EXIST AbstractBase02.reflect.xml set ReflectionRoots=-x AbstractBase02.reflect.xml
    ...
    set ExePath=%LinkBin%\AbstractBase02.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbstractBase02.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbstractBase02.asm.exe %CLRTestExecutionArguments% %Host_Args%
