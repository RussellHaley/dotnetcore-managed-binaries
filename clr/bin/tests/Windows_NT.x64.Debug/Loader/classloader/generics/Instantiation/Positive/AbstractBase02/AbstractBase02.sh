...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase02 AbstractBase02.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase02 AbstractBase02.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbstractBase02.dasm.il AbstractBase02.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbstractBase02.dasm.il AbstractBase02.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbstractBase02.asm.exe  AbstractBase02.dasm.il
    "$CORE_ROOT/ilasm" -output=AbstractBase02.asm.exe  AbstractBase02.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbstractBase02
    if [ ! -f AbstractBase02.org ]; then
        TakeLock
        if [ ! -f AbstractBase02.org ]; then
          mkdir IL
          cp AbstractBase02.exe IL/AbstractBase02.exe
          mv AbstractBase02.exe AbstractBase02.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase02.org /out AbstractBase02.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase02.org /out AbstractBase02.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbstractBase02.reflect.xml
  if [ -f AbstractBase02.reflect.xml ]
then
    ReflectionRoots="-x AbstractBase02.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AbstractBase02.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbstractBase02.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbstractBase02.asm.exe $CLRTestExecutionArguments
