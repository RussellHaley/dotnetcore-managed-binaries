...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass02 NestedBaseClass02.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedBaseClass02 NestedBaseClass02.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass02.dasm.il NestedBaseClass02.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedBaseClass02.dasm.il NestedBaseClass02.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedBaseClass02.asm.exe  NestedBaseClass02.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedBaseClass02.asm.exe  NestedBaseClass02.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedBaseClass02
    if [ ! -f NestedBaseClass02.org ]; then
        TakeLock
        if [ ! -f NestedBaseClass02.org ]; then
          mkdir IL
          cp NestedBaseClass02.exe IL/NestedBaseClass02.exe
          mv NestedBaseClass02.exe NestedBaseClass02.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass02.org /out NestedBaseClass02.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedBaseClass02.org /out NestedBaseClass02.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedBaseClass02.reflect.xml
  if [ -f NestedBaseClass02.reflect.xml ]
then
    ReflectionRoots="-x NestedBaseClass02.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedBaseClass02.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedBaseClass02.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedBaseClass02.asm.exe $CLRTestExecutionArguments
