...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedStruct03 NestedStruct03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedStruct03 NestedStruct03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedStruct03.dasm.il NestedStruct03.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedStruct03.dasm.il NestedStruct03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedStruct03.asm.exe  NestedStruct03.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedStruct03.asm.exe  NestedStruct03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedStruct03
    if [ ! -f NestedStruct03.org ]; then
        TakeLock
        if [ ! -f NestedStruct03.org ]; then
          mkdir IL
          cp NestedStruct03.exe IL/NestedStruct03.exe
          mv NestedStruct03.exe NestedStruct03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedStruct03.org /out NestedStruct03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedStruct03.org /out NestedStruct03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedStruct03.reflect.xml
  if [ -f NestedStruct03.reflect.xml ]
then
    ReflectionRoots="-x NestedStruct03.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedStruct03.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedStruct03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedStruct03.asm.exe $CLRTestExecutionArguments
