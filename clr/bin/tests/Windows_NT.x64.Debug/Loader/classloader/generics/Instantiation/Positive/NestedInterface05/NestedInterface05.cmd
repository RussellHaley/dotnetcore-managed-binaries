...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface05 NestedInterface05.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface05 NestedInterface05.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface05.dasm.il NestedInterface05.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface05.dasm.il NestedInterface05.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedInterface05.asm.exe  NestedInterface05.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedInterface05.asm.exe  NestedInterface05.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedInterface05
    if not exist "NestedInterface05.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedInterface05.org" (
            mkdir IL
            copy NestedInterface05.exe IL\NestedInterface05.exe 
            ren NestedInterface05.exe NestedInterface05.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface05.org /out NestedInterface05.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface05.org /out NestedInterface05.exe
...
set ExePath=NestedInterface05.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedInterface05.reflect.xml
    IF EXIST NestedInterface05.reflect.xml set ReflectionRoots=-x NestedInterface05.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedInterface05.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedInterface05.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedInterface05.asm.exe %CLRTestExecutionArguments% %Host_Args%
