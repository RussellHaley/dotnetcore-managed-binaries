...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedClass03 NestedClass03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedClass03 NestedClass03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedClass03.dasm.il NestedClass03.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedClass03.dasm.il NestedClass03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedClass03.asm.exe  NestedClass03.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedClass03.asm.exe  NestedClass03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedClass03
    if [ ! -f NestedClass03.org ]; then
        TakeLock
        if [ ! -f NestedClass03.org ]; then
          mkdir IL
          cp NestedClass03.exe IL/NestedClass03.exe
          mv NestedClass03.exe NestedClass03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedClass03.org /out NestedClass03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedClass03.org /out NestedClass03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedClass03.reflect.xml
  if [ -f NestedClass03.reflect.xml ]
then
    ReflectionRoots="-x NestedClass03.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedClass03.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedClass03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedClass03.asm.exe $CLRTestExecutionArguments
