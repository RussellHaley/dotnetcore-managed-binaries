...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface04 NestedInterface04.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface04 NestedInterface04.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface04.dasm.il NestedInterface04.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface04.dasm.il NestedInterface04.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedInterface04.asm.exe  NestedInterface04.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedInterface04.asm.exe  NestedInterface04.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedInterface04
    if not exist "NestedInterface04.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedInterface04.org" (
            mkdir IL
            copy NestedInterface04.exe IL\NestedInterface04.exe 
            ren NestedInterface04.exe NestedInterface04.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface04.org /out NestedInterface04.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface04.org /out NestedInterface04.exe
...
set ExePath=NestedInterface04.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedInterface04.reflect.xml
    IF EXIST NestedInterface04.reflect.xml set ReflectionRoots=-x NestedInterface04.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedInterface04.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedInterface04.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedInterface04.asm.exe %CLRTestExecutionArguments% %Host_Args%
