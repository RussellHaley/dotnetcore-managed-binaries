...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass06 NestedBaseClass06.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass06 NestedBaseClass06.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass06.dasm.il NestedBaseClass06.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass06.dasm.il NestedBaseClass06.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedBaseClass06.asm.exe  NestedBaseClass06.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedBaseClass06.asm.exe  NestedBaseClass06.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedBaseClass06
    if not exist "NestedBaseClass06.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedBaseClass06.org" (
            mkdir IL
            copy NestedBaseClass06.exe IL\NestedBaseClass06.exe 
            ren NestedBaseClass06.exe NestedBaseClass06.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass06.org /out NestedBaseClass06.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass06.org /out NestedBaseClass06.exe
...
set ExePath=NestedBaseClass06.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedBaseClass06.reflect.xml
    IF EXIST NestedBaseClass06.reflect.xml set ReflectionRoots=-x NestedBaseClass06.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedBaseClass06.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedBaseClass06.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedBaseClass06.asm.exe %CLRTestExecutionArguments% %Host_Args%
