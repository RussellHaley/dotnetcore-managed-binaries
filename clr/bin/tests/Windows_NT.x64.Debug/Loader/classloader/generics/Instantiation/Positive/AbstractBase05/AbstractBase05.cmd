...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase05 AbstractBase05.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase05 AbstractBase05.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase05.dasm.il AbstractBase05.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase05.dasm.il AbstractBase05.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbstractBase05.asm.exe  AbstractBase05.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbstractBase05.asm.exe  AbstractBase05.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbstractBase05
    if not exist "AbstractBase05.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbstractBase05.org" (
            mkdir IL
            copy AbstractBase05.exe IL\AbstractBase05.exe 
            ren AbstractBase05.exe AbstractBase05.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase05.org /out AbstractBase05.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase05.org /out AbstractBase05.exe
...
set ExePath=AbstractBase05.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbstractBase05.reflect.xml
    IF EXIST AbstractBase05.reflect.xml set ReflectionRoots=-x AbstractBase05.reflect.xml
    ...
    set ExePath=%LinkBin%\AbstractBase05.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbstractBase05.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbstractBase05.asm.exe %CLRTestExecutionArguments% %Host_Args%
