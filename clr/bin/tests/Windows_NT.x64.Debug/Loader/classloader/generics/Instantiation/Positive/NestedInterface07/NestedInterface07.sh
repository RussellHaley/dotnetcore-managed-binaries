...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface07 NestedInterface07.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface07 NestedInterface07.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedInterface07.dasm.il NestedInterface07.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedInterface07.dasm.il NestedInterface07.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedInterface07.asm.exe  NestedInterface07.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedInterface07.asm.exe  NestedInterface07.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedInterface07
    if [ ! -f NestedInterface07.org ]; then
        TakeLock
        if [ ! -f NestedInterface07.org ]; then
          mkdir IL
          cp NestedInterface07.exe IL/NestedInterface07.exe
          mv NestedInterface07.exe NestedInterface07.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface07.org /out NestedInterface07.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface07.org /out NestedInterface07.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedInterface07.reflect.xml
  if [ -f NestedInterface07.reflect.xml ]
then
    ReflectionRoots="-x NestedInterface07.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedInterface07.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedInterface07.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedInterface07.asm.exe $CLRTestExecutionArguments
