...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedStruct03 NestedStruct03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedStruct03 NestedStruct03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedStruct03.dasm.il NestedStruct03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedStruct03.dasm.il NestedStruct03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedStruct03.asm.exe  NestedStruct03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedStruct03.asm.exe  NestedStruct03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedStruct03
    if not exist "NestedStruct03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedStruct03.org" (
            mkdir IL
            copy NestedStruct03.exe IL\NestedStruct03.exe 
            ren NestedStruct03.exe NestedStruct03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedStruct03.org /out NestedStruct03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedStruct03.org /out NestedStruct03.exe
...
set ExePath=NestedStruct03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedStruct03.reflect.xml
    IF EXIST NestedStruct03.reflect.xml set ReflectionRoots=-x NestedStruct03.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedStruct03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedStruct03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedStruct03.asm.exe %CLRTestExecutionArguments% %Host_Args%
