...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface08 NestedInterface08.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface08 NestedInterface08.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedInterface08.dasm.il NestedInterface08.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedInterface08.dasm.il NestedInterface08.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedInterface08.asm.exe  NestedInterface08.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedInterface08.asm.exe  NestedInterface08.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedInterface08
    if [ ! -f NestedInterface08.org ]; then
        TakeLock
        if [ ! -f NestedInterface08.org ]; then
          mkdir IL
          cp NestedInterface08.exe IL/NestedInterface08.exe
          mv NestedInterface08.exe NestedInterface08.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface08.org /out NestedInterface08.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface08.org /out NestedInterface08.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedInterface08.reflect.xml
  if [ -f NestedInterface08.reflect.xml ]
then
    ReflectionRoots="-x NestedInterface08.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedInterface08.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedInterface08.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedInterface08.asm.exe $CLRTestExecutionArguments
