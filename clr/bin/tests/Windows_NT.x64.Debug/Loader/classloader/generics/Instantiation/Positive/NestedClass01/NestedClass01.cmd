...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedClass01 NestedClass01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedClass01 NestedClass01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedClass01.dasm.il NestedClass01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedClass01.dasm.il NestedClass01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedClass01.asm.exe  NestedClass01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedClass01.asm.exe  NestedClass01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedClass01
    if not exist "NestedClass01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedClass01.org" (
            mkdir IL
            copy NestedClass01.exe IL\NestedClass01.exe 
            ren NestedClass01.exe NestedClass01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedClass01.org /out NestedClass01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedClass01.org /out NestedClass01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedClass01.reflect.xml
    IF EXIST NestedClass01.reflect.xml set ReflectionRoots=-x NestedClass01.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedClass01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedClass01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedClass01.asm.exe %CLRTestExecutionArguments% %Host_Args%
