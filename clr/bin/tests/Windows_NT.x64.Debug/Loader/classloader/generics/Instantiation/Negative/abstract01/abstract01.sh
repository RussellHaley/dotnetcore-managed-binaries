...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Negative/abstract01 abstract01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Negative/abstract01 abstract01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=abstract01.dasm.il abstract01.exe
    "$CORE_ROOT/ildasm" -raweh -out=abstract01.dasm.il abstract01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=abstract01.asm.exe -QUIET -NOLOGO -DEBUG abstract01.dasm.il
    "$CORE_ROOT/ilasm" -output=abstract01.asm.exe -QUIET -NOLOGO -DEBUG abstract01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=abstract01
    if [ ! -f abstract01.org ]; then
        TakeLock
        if [ ! -f abstract01.org ]; then
          mkdir IL
          cp abstract01.exe IL/abstract01.exe
          mv abstract01.exe abstract01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in abstract01.org /out abstract01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in abstract01.org /out abstract01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in abstract01.reflect.xml
  if [ -f abstract01.reflect.xml ]
then
    ReflectionRoots="-x abstract01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/abstract01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" abstract01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" abstract01.asm.exe $CLRTestExecutionArguments
