...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface06 NestedInterface06.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/NestedInterface06 NestedInterface06.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NestedInterface06.dasm.il NestedInterface06.exe
    "$CORE_ROOT/ildasm" -raweh -out=NestedInterface06.dasm.il NestedInterface06.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NestedInterface06.asm.exe  NestedInterface06.dasm.il
    "$CORE_ROOT/ilasm" -output=NestedInterface06.asm.exe  NestedInterface06.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NestedInterface06
    if [ ! -f NestedInterface06.org ]; then
        TakeLock
        if [ ! -f NestedInterface06.org ]; then
          mkdir IL
          cp NestedInterface06.exe IL/NestedInterface06.exe
          mv NestedInterface06.exe NestedInterface06.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface06.org /out NestedInterface06.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NestedInterface06.org /out NestedInterface06.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NestedInterface06.reflect.xml
  if [ -f NestedInterface06.reflect.xml ]
then
    ReflectionRoots="-x NestedInterface06.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NestedInterface06.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NestedInterface06.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NestedInterface06.asm.exe $CLRTestExecutionArguments
