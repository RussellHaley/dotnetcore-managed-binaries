...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass02 NestedBaseClass02.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass02 NestedBaseClass02.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass02.dasm.il NestedBaseClass02.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass02.dasm.il NestedBaseClass02.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedBaseClass02.asm.exe  NestedBaseClass02.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedBaseClass02.asm.exe  NestedBaseClass02.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedBaseClass02
    if not exist "NestedBaseClass02.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedBaseClass02.org" (
            mkdir IL
            copy NestedBaseClass02.exe IL\NestedBaseClass02.exe 
            ren NestedBaseClass02.exe NestedBaseClass02.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass02.org /out NestedBaseClass02.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass02.org /out NestedBaseClass02.exe
...
set ExePath=NestedBaseClass02.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedBaseClass02.reflect.xml
    IF EXIST NestedBaseClass02.reflect.xml set ReflectionRoots=-x NestedBaseClass02.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedBaseClass02.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedBaseClass02.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedBaseClass02.asm.exe %CLRTestExecutionArguments% %Host_Args%
