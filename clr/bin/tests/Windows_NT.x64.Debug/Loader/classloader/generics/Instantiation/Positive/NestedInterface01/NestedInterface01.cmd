...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface01 NestedInterface01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedInterface01 NestedInterface01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface01.dasm.il NestedInterface01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedInterface01.dasm.il NestedInterface01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedInterface01.asm.exe  NestedInterface01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedInterface01.asm.exe  NestedInterface01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedInterface01
    if not exist "NestedInterface01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedInterface01.org" (
            mkdir IL
            copy NestedInterface01.exe IL\NestedInterface01.exe 
            ren NestedInterface01.exe NestedInterface01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface01.org /out NestedInterface01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedInterface01.org /out NestedInterface01.exe
...
set ExePath=NestedInterface01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedInterface01.reflect.xml
    IF EXIST NestedInterface01.reflect.xml set ReflectionRoots=-x NestedInterface01.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedInterface01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedInterface01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedInterface01.asm.exe %CLRTestExecutionArguments% %Host_Args%
