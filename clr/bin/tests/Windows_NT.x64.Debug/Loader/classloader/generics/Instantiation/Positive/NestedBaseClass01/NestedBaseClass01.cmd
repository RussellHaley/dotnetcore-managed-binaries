...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass01 NestedBaseClass01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\NestedBaseClass01 NestedBaseClass01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass01.dasm.il NestedBaseClass01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NestedBaseClass01.dasm.il NestedBaseClass01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NestedBaseClass01.asm.exe  NestedBaseClass01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NestedBaseClass01.asm.exe  NestedBaseClass01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NestedBaseClass01
    if not exist "NestedBaseClass01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NestedBaseClass01.org" (
            mkdir IL
            copy NestedBaseClass01.exe IL\NestedBaseClass01.exe 
            ren NestedBaseClass01.exe NestedBaseClass01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass01.org /out NestedBaseClass01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NestedBaseClass01.org /out NestedBaseClass01.exe
...
set ExePath=NestedBaseClass01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NestedBaseClass01.reflect.xml
    IF EXIST NestedBaseClass01.reflect.xml set ReflectionRoots=-x NestedBaseClass01.reflect.xml
    ...
    set ExePath=%LinkBin%\NestedBaseClass01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NestedBaseClass01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NestedBaseClass01.asm.exe %CLRTestExecutionArguments% %Host_Args%
