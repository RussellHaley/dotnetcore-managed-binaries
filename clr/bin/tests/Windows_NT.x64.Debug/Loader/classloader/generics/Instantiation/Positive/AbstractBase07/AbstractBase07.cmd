...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase07 AbstractBase07.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Instantiation\Positive\AbstractBase07 AbstractBase07.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase07.dasm.il AbstractBase07.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbstractBase07.dasm.il AbstractBase07.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbstractBase07.asm.exe  AbstractBase07.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbstractBase07.asm.exe  AbstractBase07.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbstractBase07
    if not exist "AbstractBase07.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbstractBase07.org" (
            mkdir IL
            copy AbstractBase07.exe IL\AbstractBase07.exe 
            ren AbstractBase07.exe AbstractBase07.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase07.org /out AbstractBase07.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbstractBase07.org /out AbstractBase07.exe
...
set ExePath=AbstractBase07.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbstractBase07.reflect.xml
    IF EXIST AbstractBase07.reflect.xml set ReflectionRoots=-x AbstractBase07.reflect.xml
    ...
    set ExePath=%LinkBin%\AbstractBase07.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbstractBase07.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbstractBase07.asm.exe %CLRTestExecutionArguments% %Host_Args%
