...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase05 AbstractBase05.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Instantiation/Positive/AbstractBase05 AbstractBase05.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbstractBase05.dasm.il AbstractBase05.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbstractBase05.dasm.il AbstractBase05.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbstractBase05.asm.exe  AbstractBase05.dasm.il
    "$CORE_ROOT/ilasm" -output=AbstractBase05.asm.exe  AbstractBase05.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbstractBase05
    if [ ! -f AbstractBase05.org ]; then
        TakeLock
        if [ ! -f AbstractBase05.org ]; then
          mkdir IL
          cp AbstractBase05.exe IL/AbstractBase05.exe
          mv AbstractBase05.exe AbstractBase05.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase05.org /out AbstractBase05.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbstractBase05.org /out AbstractBase05.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbstractBase05.reflect.xml
  if [ -f AbstractBase05.reflect.xml ]
then
    ReflectionRoots="-x AbstractBase05.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AbstractBase05.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbstractBase05.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbstractBase05.asm.exe $CLRTestExecutionArguments
