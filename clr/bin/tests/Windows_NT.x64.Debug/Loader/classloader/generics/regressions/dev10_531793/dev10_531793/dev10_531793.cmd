...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\regressions\dev10_531793\dev10_531793 dev10_531793.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\regressions\dev10_531793\dev10_531793 dev10_531793.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=dev10_531793.dasm.il dev10_531793.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=dev10_531793.dasm.il dev10_531793.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=dev10_531793.asm.exe -QUIET -NOLOGO -DEBUG dev10_531793.dasm.il
    %CORE_ROOT%\ilasm.exe /output=dev10_531793.asm.exe -QUIET -NOLOGO -DEBUG dev10_531793.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=dev10_531793
    if not exist "dev10_531793.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "dev10_531793.org" (
            mkdir IL
            copy dev10_531793.exe IL\dev10_531793.exe 
            ren dev10_531793.exe dev10_531793.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dev10_531793.org /out dev10_531793.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dev10_531793.org /out dev10_531793.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in dev10_531793.reflect.xml
    IF EXIST dev10_531793.reflect.xml set ReflectionRoots=-x dev10_531793.reflect.xml
    ...
    set ExePath=%LinkBin%\dev10_531793.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% dev10_531793.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% dev10_531793.asm.exe %CLRTestExecutionArguments% %Host_Args%
