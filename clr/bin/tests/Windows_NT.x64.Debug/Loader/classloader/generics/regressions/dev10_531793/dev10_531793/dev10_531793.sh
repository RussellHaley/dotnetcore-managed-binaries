...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/regressions/dev10_531793/dev10_531793 dev10_531793.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/regressions/dev10_531793/dev10_531793 dev10_531793.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=dev10_531793.dasm.il dev10_531793.exe
    "$CORE_ROOT/ildasm" -raweh -out=dev10_531793.dasm.il dev10_531793.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=dev10_531793.asm.exe -QUIET -NOLOGO -DEBUG dev10_531793.dasm.il
    "$CORE_ROOT/ilasm" -output=dev10_531793.asm.exe -QUIET -NOLOGO -DEBUG dev10_531793.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=dev10_531793
    if [ ! -f dev10_531793.org ]; then
        TakeLock
        if [ ! -f dev10_531793.org ]; then
          mkdir IL
          cp dev10_531793.exe IL/dev10_531793.exe
          mv dev10_531793.exe dev10_531793.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dev10_531793.org /out dev10_531793.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dev10_531793.org /out dev10_531793.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in dev10_531793.reflect.xml
  if [ -f dev10_531793.reflect.xml ]
then
    ReflectionRoots="-x dev10_531793.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/dev10_531793.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" dev10_531793.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" dev10_531793.asm.exe $CLRTestExecutionArguments
