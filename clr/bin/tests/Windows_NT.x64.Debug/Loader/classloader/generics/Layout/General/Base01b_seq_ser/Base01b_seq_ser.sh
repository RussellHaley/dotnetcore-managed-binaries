...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Layout/General/Base01b_seq_ser Base01b_seq_ser.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Layout/General/Base01b_seq_ser Base01b_seq_ser.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Base01b_seq_ser.dasm.il Base01b_seq_ser.exe
    "$CORE_ROOT/ildasm" -raweh -out=Base01b_seq_ser.dasm.il Base01b_seq_ser.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Base01b_seq_ser.asm.exe  Base01b_seq_ser.dasm.il
    "$CORE_ROOT/ilasm" -output=Base01b_seq_ser.asm.exe  Base01b_seq_ser.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Base01b_seq_ser
    if [ ! -f Base01b_seq_ser.org ]; then
        TakeLock
        if [ ! -f Base01b_seq_ser.org ]; then
          mkdir IL
          cp Base01b_seq_ser.exe IL/Base01b_seq_ser.exe
          mv Base01b_seq_ser.exe Base01b_seq_ser.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Base01b_seq_ser.org /out Base01b_seq_ser.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Base01b_seq_ser.org /out Base01b_seq_ser.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Base01b_seq_ser.reflect.xml
  if [ -f Base01b_seq_ser.reflect.xml ]
then
    ReflectionRoots="-x Base01b_seq_ser.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Base01b_seq_ser.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Base01b_seq_ser.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Base01b_seq_ser.asm.exe $CLRTestExecutionArguments
