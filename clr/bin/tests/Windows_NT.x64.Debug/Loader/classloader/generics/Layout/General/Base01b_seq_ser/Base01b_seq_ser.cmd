...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Layout\General\Base01b_seq_ser Base01b_seq_ser.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Layout\General\Base01b_seq_ser Base01b_seq_ser.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Base01b_seq_ser.dasm.il Base01b_seq_ser.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Base01b_seq_ser.dasm.il Base01b_seq_ser.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Base01b_seq_ser.asm.exe  Base01b_seq_ser.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Base01b_seq_ser.asm.exe  Base01b_seq_ser.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Base01b_seq_ser
    if not exist "Base01b_seq_ser.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Base01b_seq_ser.org" (
            mkdir IL
            copy Base01b_seq_ser.exe IL\Base01b_seq_ser.exe 
            ren Base01b_seq_ser.exe Base01b_seq_ser.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Base01b_seq_ser.org /out Base01b_seq_ser.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Base01b_seq_ser.org /out Base01b_seq_ser.exe
...
set ExePath=Base01b_seq_ser.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Base01b_seq_ser.reflect.xml
    IF EXIST Base01b_seq_ser.reflect.xml set ReflectionRoots=-x Base01b_seq_ser.reflect.xml
    ...
    set ExePath=%LinkBin%\Base01b_seq_ser.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Base01b_seq_ser.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Base01b_seq_ser.asm.exe %CLRTestExecutionArguments% %Host_Args%
