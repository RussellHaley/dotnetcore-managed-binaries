...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\Unbox006 Unbox006.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\Unbox006 Unbox006.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Unbox006.dasm.il Unbox006.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Unbox006.dasm.il Unbox006.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Unbox006.asm.exe  Unbox006.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Unbox006.asm.exe  Unbox006.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Unbox006
    if not exist "Unbox006.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Unbox006.org" (
            mkdir IL
            copy Unbox006.exe IL\Unbox006.exe 
            ren Unbox006.exe Unbox006.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unbox006.org /out Unbox006.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unbox006.org /out Unbox006.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Unbox006.reflect.xml
    IF EXIST Unbox006.reflect.xml set ReflectionRoots=-x Unbox006.reflect.xml
    ...
    set ExePath=%LinkBin%\Unbox006.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Unbox006.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Unbox006.asm.exe %CLRTestExecutionArguments% %Host_Args%
