...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/Unbox002 Unbox002.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/Unbox002 Unbox002.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Unbox002.dasm.il Unbox002.exe
    "$CORE_ROOT/ildasm" -raweh -out=Unbox002.dasm.il Unbox002.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Unbox002.asm.exe  Unbox002.dasm.il
    "$CORE_ROOT/ilasm" -output=Unbox002.asm.exe  Unbox002.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Unbox002
    if [ ! -f Unbox002.org ]; then
        TakeLock
        if [ ! -f Unbox002.org ]; then
          mkdir IL
          cp Unbox002.exe IL/Unbox002.exe
          mv Unbox002.exe Unbox002.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unbox002.org /out Unbox002.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unbox002.org /out Unbox002.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Unbox002.reflect.xml
  if [ -f Unbox002.reflect.xml ]
then
    ReflectionRoots="-x Unbox002.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Unbox002.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Unbox002.asm.exe $CLRTestExecutionArguments
