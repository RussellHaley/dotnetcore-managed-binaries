...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\Unbox003 Unbox003.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\Unbox003 Unbox003.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Unbox003.dasm.il Unbox003.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Unbox003.dasm.il Unbox003.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Unbox003.asm.exe  Unbox003.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Unbox003.asm.exe  Unbox003.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Unbox003
    if not exist "Unbox003.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Unbox003.org" (
            mkdir IL
            copy Unbox003.exe IL\Unbox003.exe 
            ren Unbox003.exe Unbox003.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unbox003.org /out Unbox003.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unbox003.org /out Unbox003.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Unbox003.reflect.xml
    IF EXIST Unbox003.reflect.xml set ReflectionRoots=-x Unbox003.reflect.xml
    ...
    set ExePath=%LinkBin%\Unbox003.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Unbox003.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Unbox003.asm.exe %CLRTestExecutionArguments% %Host_Args%
