...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/Unbox003 Unbox003.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/Unbox003 Unbox003.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Unbox003.dasm.il Unbox003.exe
    "$CORE_ROOT/ildasm" -raweh -out=Unbox003.dasm.il Unbox003.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Unbox003.asm.exe  Unbox003.dasm.il
    "$CORE_ROOT/ilasm" -output=Unbox003.asm.exe  Unbox003.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Unbox003
    if [ ! -f Unbox003.org ]; then
        TakeLock
        if [ ! -f Unbox003.org ]; then
          mkdir IL
          cp Unbox003.exe IL/Unbox003.exe
          mv Unbox003.exe Unbox003.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unbox003.org /out Unbox003.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unbox003.org /out Unbox003.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Unbox003.reflect.xml
  if [ -f Unbox003.reflect.xml ]
then
    ReflectionRoots="-x Unbox003.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Unbox003.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Unbox003.asm.exe $CLRTestExecutionArguments
