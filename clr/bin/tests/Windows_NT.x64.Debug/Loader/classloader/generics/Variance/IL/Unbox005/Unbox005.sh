...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/Unbox005 Unbox005.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/Unbox005 Unbox005.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Unbox005.dasm.il Unbox005.exe
    "$CORE_ROOT/ildasm" -raweh -out=Unbox005.dasm.il Unbox005.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Unbox005.asm.exe  Unbox005.dasm.il
    "$CORE_ROOT/ilasm" -output=Unbox005.asm.exe  Unbox005.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Unbox005
    if [ ! -f Unbox005.org ]; then
        TakeLock
        if [ ! -f Unbox005.org ]; then
          mkdir IL
          cp Unbox005.exe IL/Unbox005.exe
          mv Unbox005.exe Unbox005.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unbox005.org /out Unbox005.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unbox005.org /out Unbox005.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Unbox005.reflect.xml
  if [ -f Unbox005.reflect.xml ]
then
    ReflectionRoots="-x Unbox005.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Unbox005.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Unbox005.asm.exe $CLRTestExecutionArguments
