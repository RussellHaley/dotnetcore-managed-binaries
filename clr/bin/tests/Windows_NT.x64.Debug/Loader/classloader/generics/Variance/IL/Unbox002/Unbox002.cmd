...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\Unbox002 Unbox002.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\Unbox002 Unbox002.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Unbox002.dasm.il Unbox002.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Unbox002.dasm.il Unbox002.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Unbox002.asm.exe  Unbox002.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Unbox002.asm.exe  Unbox002.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Unbox002
    if not exist "Unbox002.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Unbox002.org" (
            mkdir IL
            copy Unbox002.exe IL\Unbox002.exe 
            ren Unbox002.exe Unbox002.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unbox002.org /out Unbox002.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unbox002.org /out Unbox002.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Unbox002.reflect.xml
    IF EXIST Unbox002.reflect.xml set ReflectionRoots=-x Unbox002.reflect.xml
    ...
    set ExePath=%LinkBin%\Unbox002.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Unbox002.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Unbox002.asm.exe %CLRTestExecutionArguments% %Host_Args%
