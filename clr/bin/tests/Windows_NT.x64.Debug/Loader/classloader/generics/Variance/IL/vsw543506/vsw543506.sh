...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/vsw543506 vsw543506.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/vsw543506 vsw543506.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=vsw543506.dasm.il vsw543506.exe
    "$CORE_ROOT/ildasm" -raweh -out=vsw543506.dasm.il vsw543506.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=vsw543506.asm.exe  vsw543506.dasm.il
    "$CORE_ROOT/ilasm" -output=vsw543506.asm.exe  vsw543506.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=vsw543506
    if [ ! -f vsw543506.org ]; then
        TakeLock
        if [ ! -f vsw543506.org ]; then
          mkdir IL
          cp vsw543506.exe IL/vsw543506.exe
          mv vsw543506.exe vsw543506.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vsw543506.org /out vsw543506.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vsw543506.org /out vsw543506.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in vsw543506.reflect.xml
  if [ -f vsw543506.reflect.xml ]
then
    ReflectionRoots="-x vsw543506.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" vsw543506.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" vsw543506.asm.exe $CLRTestExecutionArguments
