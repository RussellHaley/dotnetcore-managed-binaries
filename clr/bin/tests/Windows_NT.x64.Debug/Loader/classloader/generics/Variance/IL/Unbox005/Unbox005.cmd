...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\Unbox005 Unbox005.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\Unbox005 Unbox005.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Unbox005.dasm.il Unbox005.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Unbox005.dasm.il Unbox005.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Unbox005.asm.exe  Unbox005.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Unbox005.asm.exe  Unbox005.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Unbox005
    if not exist "Unbox005.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Unbox005.org" (
            mkdir IL
            copy Unbox005.exe IL\Unbox005.exe 
            ren Unbox005.exe Unbox005.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unbox005.org /out Unbox005.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unbox005.org /out Unbox005.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Unbox005.reflect.xml
    IF EXIST Unbox005.reflect.xml set ReflectionRoots=-x Unbox005.reflect.xml
    ...
    set ExePath=%LinkBin%\Unbox005.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Unbox005.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Unbox005.asm.exe %CLRTestExecutionArguments% %Host_Args%
