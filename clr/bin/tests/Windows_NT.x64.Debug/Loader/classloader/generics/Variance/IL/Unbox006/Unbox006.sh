...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/Unbox006 Unbox006.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/generics/Variance/IL/Unbox006 Unbox006.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Unbox006.dasm.il Unbox006.exe
    "$CORE_ROOT/ildasm" -raweh -out=Unbox006.dasm.il Unbox006.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Unbox006.asm.exe  Unbox006.dasm.il
    "$CORE_ROOT/ilasm" -output=Unbox006.asm.exe  Unbox006.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Unbox006
    if [ ! -f Unbox006.org ]; then
        TakeLock
        if [ ! -f Unbox006.org ]; then
          mkdir IL
          cp Unbox006.exe IL/Unbox006.exe
          mv Unbox006.exe Unbox006.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unbox006.org /out Unbox006.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unbox006.org /out Unbox006.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Unbox006.reflect.xml
  if [ -f Unbox006.reflect.xml ]
then
    ReflectionRoots="-x Unbox006.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Unbox006.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Unbox006.asm.exe $CLRTestExecutionArguments
