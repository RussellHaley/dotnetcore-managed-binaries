...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\vsw543506 vsw543506.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\generics\Variance\IL\vsw543506 vsw543506.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=vsw543506.dasm.il vsw543506.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=vsw543506.dasm.il vsw543506.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=vsw543506.asm.exe  vsw543506.dasm.il
    %CORE_ROOT%\ilasm.exe /output=vsw543506.asm.exe  vsw543506.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=vsw543506
    if not exist "vsw543506.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "vsw543506.org" (
            mkdir IL
            copy vsw543506.exe IL\vsw543506.exe 
            ren vsw543506.exe vsw543506.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in vsw543506.org /out vsw543506.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in vsw543506.org /out vsw543506.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in vsw543506.reflect.xml
    IF EXIST vsw543506.reflect.xml set ReflectionRoots=-x vsw543506.reflect.xml
    ...
    set ExePath=%LinkBin%\vsw543506.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% vsw543506.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% vsw543506.asm.exe %CLRTestExecutionArguments% %Host_Args%
