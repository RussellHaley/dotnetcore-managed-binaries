...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/TSAmbiguities/SameMethodImpl/Override/HelloWorld HelloWorld.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/TSAmbiguities/SameMethodImpl/Override/HelloWorld HelloWorld.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=HelloWorld.dasm.il HelloWorld.exe
    "$CORE_ROOT/ildasm" -raweh -out=HelloWorld.dasm.il HelloWorld.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=HelloWorld.asm.exe -QUIET -NOLOGO -DEBUG HelloWorld.dasm.il
    "$CORE_ROOT/ilasm" -output=HelloWorld.asm.exe -QUIET -NOLOGO -DEBUG HelloWorld.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=HelloWorld
    if [ ! -f HelloWorld.org ]; then
        TakeLock
        if [ ! -f HelloWorld.org ]; then
          mkdir IL
          cp HelloWorld.exe IL/HelloWorld.exe
          mv HelloWorld.exe HelloWorld.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in HelloWorld.org /out HelloWorld.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in HelloWorld.org /out HelloWorld.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in HelloWorld.reflect.xml
  if [ -f HelloWorld.reflect.xml ]
then
    ReflectionRoots="-x HelloWorld.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/HelloWorld.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" HelloWorld.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" HelloWorld.asm.exe $CLRTestExecutionArguments
