...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\TSAmbiguities\SameMethodImpl\Override\HelloWorld HelloWorld.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\TSAmbiguities\SameMethodImpl\Override\HelloWorld HelloWorld.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=HelloWorld.dasm.il HelloWorld.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=HelloWorld.dasm.il HelloWorld.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=HelloWorld.asm.exe -QUIET -NOLOGO -DEBUG HelloWorld.dasm.il
    %CORE_ROOT%\ilasm.exe /output=HelloWorld.asm.exe -QUIET -NOLOGO -DEBUG HelloWorld.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=HelloWorld
    if not exist "HelloWorld.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "HelloWorld.org" (
            mkdir IL
            copy HelloWorld.exe IL\HelloWorld.exe 
            ren HelloWorld.exe HelloWorld.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in HelloWorld.org /out HelloWorld.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in HelloWorld.org /out HelloWorld.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in HelloWorld.reflect.xml
    IF EXIST HelloWorld.reflect.xml set ReflectionRoots=-x HelloWorld.reflect.xml
    ...
    set ExePath=%LinkBin%\HelloWorld.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% HelloWorld.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% HelloWorld.asm.exe %CLRTestExecutionArguments% %Host_Args%
