...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\..\dasm\Loader\classloader\v1\Beta1\Layout\Matrix\cs\L-1-4-1 L-1-4-1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\..\dasm\Loader\classloader\v1\Beta1\Layout\Matrix\cs\L-1-4-1 L-1-4-1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=L-1-4-1.dasm.il L-1-4-1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=L-1-4-1.dasm.il L-1-4-1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=L-1-4-1.asm.exe  L-1-4-1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=L-1-4-1.asm.exe  L-1-4-1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=L-1-4-1
    if not exist "L-1-4-1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "L-1-4-1.org" (
            mkdir IL
            copy L-1-4-1.exe IL\L-1-4-1.exe 
            ren L-1-4-1.exe L-1-4-1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in L-1-4-1.org /out L-1-4-1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in L-1-4-1.org /out L-1-4-1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in L-1-4-1.reflect.xml
    IF EXIST L-1-4-1.reflect.xml set ReflectionRoots=-x L-1-4-1.reflect.xml
    ...
    set ExePath=%LinkBin%\L-1-4-1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% L-1-4-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% L-1-4-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
