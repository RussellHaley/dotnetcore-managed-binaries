...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../../dasm/Loader/classloader/v1/Beta1/Layout/Matrix/cs/L-1-4-1 L-1-4-1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../../dasm/Loader/classloader/v1/Beta1/Layout/Matrix/cs/L-1-4-1 L-1-4-1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=L-1-4-1.dasm.il L-1-4-1.exe
    "$CORE_ROOT/ildasm" -raweh -out=L-1-4-1.dasm.il L-1-4-1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=L-1-4-1.asm.exe  L-1-4-1.dasm.il
    "$CORE_ROOT/ilasm" -output=L-1-4-1.asm.exe  L-1-4-1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=L-1-4-1
    if [ ! -f L-1-4-1.org ]; then
        TakeLock
        if [ ! -f L-1-4-1.org ]; then
          mkdir IL
          cp L-1-4-1.exe IL/L-1-4-1.exe
          mv L-1-4-1.exe L-1-4-1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in L-1-4-1.org /out L-1-4-1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in L-1-4-1.org /out L-1-4-1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in L-1-4-1.reflect.xml
  if [ -f L-1-4-1.reflect.xml ]
then
    ReflectionRoots="-x L-1-4-1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" L-1-4-1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" L-1-4-1.asm.exe $CLRTestExecutionArguments
