...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case8 case8.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case8 case8.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case8.dasm.il case8.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case8.dasm.il case8.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case8.asm.exe  case8.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case8.asm.exe  case8.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case8
    if not exist "case8.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case8.org" (
            mkdir IL
            copy case8.exe IL\case8.exe 
            ren case8.exe case8.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case8.org /out case8.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case8.org /out case8.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case8.reflect.xml
    IF EXIST case8.reflect.xml set ReflectionRoots=-x case8.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case8.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case8.asm.exe %CLRTestExecutionArguments% %Host_Args%
