...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case7 case7.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case7 case7.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case7.dasm.il case7.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case7.dasm.il case7.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case7.asm.exe  case7.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case7.asm.exe  case7.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case7
    if not exist "case7.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case7.org" (
            mkdir IL
            copy case7.exe IL\case7.exe 
            ren case7.exe case7.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case7.org /out case7.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case7.org /out case7.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case7.reflect.xml
    IF EXIST case7.reflect.xml set ReflectionRoots=-x case7.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case7.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case7.asm.exe %CLRTestExecutionArguments% %Host_Args%
