...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case6 case6.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case6 case6.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case6.dasm.il case6.exe
    "$CORE_ROOT/ildasm" -raweh -out=case6.dasm.il case6.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case6.asm.exe  case6.dasm.il
    "$CORE_ROOT/ilasm" -output=case6.asm.exe  case6.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case6
    if [ ! -f case6.org ]; then
        TakeLock
        if [ ! -f case6.org ]; then
          mkdir IL
          cp case6.exe IL/case6.exe
          mv case6.exe case6.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case6.org /out case6.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case6.org /out case6.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case6.reflect.xml
  if [ -f case6.reflect.xml ]
then
    ReflectionRoots="-x case6.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case6.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case6.asm.exe $CLRTestExecutionArguments
