...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case4 case4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case4 case4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case4.dasm.il case4.exe
    "$CORE_ROOT/ildasm" -raweh -out=case4.dasm.il case4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case4.asm.exe  case4.dasm.il
    "$CORE_ROOT/ilasm" -output=case4.asm.exe  case4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case4
    if [ ! -f case4.org ]; then
        TakeLock
        if [ ! -f case4.org ]; then
          mkdir IL
          cp case4.exe IL/case4.exe
          mv case4.exe case4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case4.org /out case4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case4.org /out case4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case4.reflect.xml
  if [ -f case4.reflect.xml ]
then
    ReflectionRoots="-x case4.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case4.asm.exe $CLRTestExecutionArguments
