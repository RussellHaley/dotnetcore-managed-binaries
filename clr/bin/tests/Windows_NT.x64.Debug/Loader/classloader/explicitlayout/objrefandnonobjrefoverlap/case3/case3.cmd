...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case3 case3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case3 case3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case3.dasm.il case3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case3.dasm.il case3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case3.asm.exe  case3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case3.asm.exe  case3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case3
    if not exist "case3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case3.org" (
            mkdir IL
            copy case3.exe IL\case3.exe 
            ren case3.exe case3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case3.org /out case3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case3.org /out case3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case3.reflect.xml
    IF EXIST case3.reflect.xml set ReflectionRoots=-x case3.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case3.asm.exe %CLRTestExecutionArguments% %Host_Args%
