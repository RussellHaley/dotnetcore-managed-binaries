...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case2 case2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case2 case2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case2.dasm.il case2.exe
    "$CORE_ROOT/ildasm" -raweh -out=case2.dasm.il case2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case2.asm.exe  case2.dasm.il
    "$CORE_ROOT/ilasm" -output=case2.asm.exe  case2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case2
    if [ ! -f case2.org ]; then
        TakeLock
        if [ ! -f case2.org ]; then
          mkdir IL
          cp case2.exe IL/case2.exe
          mv case2.exe case2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case2.org /out case2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case2.org /out case2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case2.reflect.xml
  if [ -f case2.reflect.xml ]
then
    ReflectionRoots="-x case2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case2.asm.exe $CLRTestExecutionArguments
