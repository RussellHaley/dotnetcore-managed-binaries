...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case1 case1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case1 case1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case1.dasm.il case1.exe
    "$CORE_ROOT/ildasm" -raweh -out=case1.dasm.il case1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case1.asm.exe  case1.dasm.il
    "$CORE_ROOT/ilasm" -output=case1.asm.exe  case1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case1
    if [ ! -f case1.org ]; then
        TakeLock
        if [ ! -f case1.org ]; then
          mkdir IL
          cp case1.exe IL/case1.exe
          mv case1.exe case1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case1.org /out case1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case1.org /out case1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case1.reflect.xml
  if [ -f case1.reflect.xml ]
then
    ReflectionRoots="-x case1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case1.asm.exe $CLRTestExecutionArguments
