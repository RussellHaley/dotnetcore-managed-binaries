...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case7 case7.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case7 case7.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case7.dasm.il case7.exe
    "$CORE_ROOT/ildasm" -raweh -out=case7.dasm.il case7.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case7.asm.exe  case7.dasm.il
    "$CORE_ROOT/ilasm" -output=case7.asm.exe  case7.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case7
    if [ ! -f case7.org ]; then
        TakeLock
        if [ ! -f case7.org ]; then
          mkdir IL
          cp case7.exe IL/case7.exe
          mv case7.exe case7.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case7.org /out case7.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case7.org /out case7.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case7.reflect.xml
  if [ -f case7.reflect.xml ]
then
    ReflectionRoots="-x case7.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case7.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case7.asm.exe $CLRTestExecutionArguments
