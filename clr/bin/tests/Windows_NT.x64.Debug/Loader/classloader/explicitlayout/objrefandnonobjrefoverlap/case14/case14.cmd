...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case14 case14.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case14 case14.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case14.dasm.il case14.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case14.dasm.il case14.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case14.asm.exe  case14.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case14.asm.exe  case14.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case14
    if not exist "case14.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case14.org" (
            mkdir IL
            copy case14.exe IL\case14.exe 
            ren case14.exe case14.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case14.org /out case14.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case14.org /out case14.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case14.reflect.xml
    IF EXIST case14.reflect.xml set ReflectionRoots=-x case14.reflect.xml
    ...
    set ExePath=%LinkBin%\case14.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case14.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case14.asm.exe %CLRTestExecutionArguments% %Host_Args%
