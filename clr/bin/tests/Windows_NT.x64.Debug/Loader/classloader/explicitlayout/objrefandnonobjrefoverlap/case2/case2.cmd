...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case2 case2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case2 case2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case2.dasm.il case2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case2.dasm.il case2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case2.asm.exe  case2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case2.asm.exe  case2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case2
    if not exist "case2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case2.org" (
            mkdir IL
            copy case2.exe IL\case2.exe 
            ren case2.exe case2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case2.org /out case2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case2.org /out case2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case2.reflect.xml
    IF EXIST case2.reflect.xml set ReflectionRoots=-x case2.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case2.asm.exe %CLRTestExecutionArguments% %Host_Args%
