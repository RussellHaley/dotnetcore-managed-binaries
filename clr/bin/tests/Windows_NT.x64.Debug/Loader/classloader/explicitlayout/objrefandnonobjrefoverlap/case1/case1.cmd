...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case1 case1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case1 case1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case1.dasm.il case1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case1.dasm.il case1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case1.asm.exe  case1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case1.asm.exe  case1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case1
    if not exist "case1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case1.org" (
            mkdir IL
            copy case1.exe IL\case1.exe 
            ren case1.exe case1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case1.org /out case1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case1.org /out case1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case1.reflect.xml
    IF EXIST case1.reflect.xml set ReflectionRoots=-x case1.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case1.asm.exe %CLRTestExecutionArguments% %Host_Args%
