...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case6 case6.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case6 case6.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case6.dasm.il case6.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case6.dasm.il case6.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case6.asm.exe  case6.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case6.asm.exe  case6.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case6
    if not exist "case6.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case6.org" (
            mkdir IL
            copy case6.exe IL\case6.exe 
            ren case6.exe case6.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case6.org /out case6.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case6.org /out case6.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case6.reflect.xml
    IF EXIST case6.reflect.xml set ReflectionRoots=-x case6.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case6.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case6.asm.exe %CLRTestExecutionArguments% %Host_Args%
