...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case11 case11.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case11 case11.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case11.dasm.il case11.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case11.dasm.il case11.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case11.asm.exe  case11.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case11.asm.exe  case11.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case11
    if not exist "case11.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case11.org" (
            mkdir IL
            copy case11.exe IL\case11.exe 
            ren case11.exe case11.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case11.org /out case11.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case11.org /out case11.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case11.reflect.xml
    IF EXIST case11.reflect.xml set ReflectionRoots=-x case11.reflect.xml
    ...
    set ExePath=%LinkBin%\case11.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case11.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case11.asm.exe %CLRTestExecutionArguments% %Host_Args%
