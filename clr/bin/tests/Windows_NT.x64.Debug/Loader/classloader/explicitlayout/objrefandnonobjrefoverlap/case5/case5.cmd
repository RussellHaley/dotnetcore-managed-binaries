...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case5 case5.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case5 case5.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case5.dasm.il case5.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case5.dasm.il case5.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case5.asm.exe  case5.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case5.asm.exe  case5.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case5
    if not exist "case5.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case5.org" (
            mkdir IL
            copy case5.exe IL\case5.exe 
            ren case5.exe case5.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case5.org /out case5.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case5.org /out case5.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case5.reflect.xml
    IF EXIST case5.reflect.xml set ReflectionRoots=-x case5.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case5.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case5.asm.exe %CLRTestExecutionArguments% %Host_Args%
