...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case4 case4.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case4 case4.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case4.dasm.il case4.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case4.dasm.il case4.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case4.asm.exe  case4.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case4.asm.exe  case4.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case4
    if not exist "case4.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case4.org" (
            mkdir IL
            copy case4.exe IL\case4.exe 
            ren case4.exe case4.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case4.org /out case4.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case4.org /out case4.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case4.reflect.xml
    IF EXIST case4.reflect.xml set ReflectionRoots=-x case4.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case4.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case4.asm.exe %CLRTestExecutionArguments% %Host_Args%
