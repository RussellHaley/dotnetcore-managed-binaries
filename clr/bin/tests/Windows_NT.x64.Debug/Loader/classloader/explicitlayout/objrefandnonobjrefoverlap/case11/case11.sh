...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case11 case11.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case11 case11.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case11.dasm.il case11.exe
    "$CORE_ROOT/ildasm" -raweh -out=case11.dasm.il case11.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case11.asm.exe  case11.dasm.il
    "$CORE_ROOT/ilasm" -output=case11.asm.exe  case11.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case11
    if [ ! -f case11.org ]; then
        TakeLock
        if [ ! -f case11.org ]; then
          mkdir IL
          cp case11.exe IL/case11.exe
          mv case11.exe case11.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case11.org /out case11.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case11.org /out case11.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case11.reflect.xml
  if [ -f case11.reflect.xml ]
then
    ReflectionRoots="-x case11.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case11.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case11.asm.exe $CLRTestExecutionArguments
