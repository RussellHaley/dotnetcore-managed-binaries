...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case8 case8.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case8 case8.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case8.dasm.il case8.exe
    "$CORE_ROOT/ildasm" -raweh -out=case8.dasm.il case8.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case8.asm.exe  case8.dasm.il
    "$CORE_ROOT/ilasm" -output=case8.asm.exe  case8.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case8
    if [ ! -f case8.org ]; then
        TakeLock
        if [ ! -f case8.org ]; then
          mkdir IL
          cp case8.exe IL/case8.exe
          mv case8.exe case8.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case8.org /out case8.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case8.org /out case8.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case8.reflect.xml
  if [ -f case8.reflect.xml ]
then
    ReflectionRoots="-x case8.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case8.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case8.asm.exe $CLRTestExecutionArguments
