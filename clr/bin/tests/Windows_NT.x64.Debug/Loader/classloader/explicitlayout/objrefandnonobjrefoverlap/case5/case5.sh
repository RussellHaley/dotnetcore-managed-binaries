...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case5 case5.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case5 case5.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case5.dasm.il case5.exe
    "$CORE_ROOT/ildasm" -raweh -out=case5.dasm.il case5.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case5.asm.exe  case5.dasm.il
    "$CORE_ROOT/ilasm" -output=case5.asm.exe  case5.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case5
    if [ ! -f case5.org ]; then
        TakeLock
        if [ ! -f case5.org ]; then
          mkdir IL
          cp case5.exe IL/case5.exe
          mv case5.exe case5.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case5.org /out case5.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case5.org /out case5.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case5.reflect.xml
  if [ -f case5.reflect.xml ]
then
    ReflectionRoots="-x case5.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case5.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case5.asm.exe $CLRTestExecutionArguments
