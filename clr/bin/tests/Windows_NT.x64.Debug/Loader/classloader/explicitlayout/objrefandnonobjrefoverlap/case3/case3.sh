...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case3 case3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case3 case3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case3.dasm.il case3.exe
    "$CORE_ROOT/ildasm" -raweh -out=case3.dasm.il case3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case3.asm.exe  case3.dasm.il
    "$CORE_ROOT/ilasm" -output=case3.asm.exe  case3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case3
    if [ ! -f case3.org ]; then
        TakeLock
        if [ ! -f case3.org ]; then
          mkdir IL
          cp case3.exe IL/case3.exe
          mv case3.exe case3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case3.org /out case3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case3.org /out case3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case3.reflect.xml
  if [ -f case3.reflect.xml ]
then
    ReflectionRoots="-x case3.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case3.asm.exe $CLRTestExecutionArguments
