...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case9 case9.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case9 case9.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case9.dasm.il case9.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case9.dasm.il case9.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case9.asm.exe  case9.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case9.asm.exe  case9.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case9
    if not exist "case9.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case9.org" (
            mkdir IL
            copy case9.exe IL\case9.exe 
            ren case9.exe case9.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case9.org /out case9.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case9.org /out case9.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case9.reflect.xml
    IF EXIST case9.reflect.xml set ReflectionRoots=-x case9.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case9.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case9.asm.exe %CLRTestExecutionArguments% %Host_Args%
