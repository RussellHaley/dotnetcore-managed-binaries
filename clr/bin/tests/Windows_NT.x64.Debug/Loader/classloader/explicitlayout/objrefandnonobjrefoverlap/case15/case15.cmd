...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case15 case15.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case15 case15.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case15.dasm.il case15.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case15.dasm.il case15.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case15.asm.exe  case15.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case15.asm.exe  case15.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case15
    if not exist "case15.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case15.org" (
            mkdir IL
            copy case15.exe IL\case15.exe 
            ren case15.exe case15.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case15.org /out case15.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case15.org /out case15.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case15.reflect.xml
    IF EXIST case15.reflect.xml set ReflectionRoots=-x case15.reflect.xml
    ...
    set ExePath=%LinkBin%\case15.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case15.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case15.asm.exe %CLRTestExecutionArguments% %Host_Args%
