...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case9 case9.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case9 case9.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case9.dasm.il case9.exe
    "$CORE_ROOT/ildasm" -raweh -out=case9.dasm.il case9.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case9.asm.exe  case9.dasm.il
    "$CORE_ROOT/ilasm" -output=case9.asm.exe  case9.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case9
    if [ ! -f case9.org ]; then
        TakeLock
        if [ ! -f case9.org ]; then
          mkdir IL
          cp case9.exe IL/case9.exe
          mv case9.exe case9.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case9.org /out case9.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case9.org /out case9.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case9.reflect.xml
  if [ -f case9.reflect.xml ]
then
    ReflectionRoots="-x case9.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case9.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case9.asm.exe $CLRTestExecutionArguments
