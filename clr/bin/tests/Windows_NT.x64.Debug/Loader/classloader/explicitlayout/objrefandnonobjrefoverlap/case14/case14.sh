...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case14 case14.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case14 case14.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case14.dasm.il case14.exe
    "$CORE_ROOT/ildasm" -raweh -out=case14.dasm.il case14.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case14.asm.exe  case14.dasm.il
    "$CORE_ROOT/ilasm" -output=case14.asm.exe  case14.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case14
    if [ ! -f case14.org ]; then
        TakeLock
        if [ ! -f case14.org ]; then
          mkdir IL
          cp case14.exe IL/case14.exe
          mv case14.exe case14.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case14.org /out case14.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case14.org /out case14.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case14.reflect.xml
  if [ -f case14.reflect.xml ]
then
    ReflectionRoots="-x case14.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case14.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case14.asm.exe $CLRTestExecutionArguments
