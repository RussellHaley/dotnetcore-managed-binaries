...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case12 case12.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\explicitlayout\objrefandnonobjrefoverlap\case12 case12.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=case12.dasm.il case12.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=case12.dasm.il case12.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=case12.asm.exe  case12.dasm.il
    %CORE_ROOT%\ilasm.exe /output=case12.asm.exe  case12.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=case12
    if not exist "case12.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "case12.org" (
            mkdir IL
            copy case12.exe IL\case12.exe 
            ren case12.exe case12.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case12.org /out case12.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in case12.org /out case12.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in case12.reflect.xml
    IF EXIST case12.reflect.xml set ReflectionRoots=-x case12.reflect.xml
    ...
    set ExePath=%LinkBin%\case12.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% case12.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% case12.asm.exe %CLRTestExecutionArguments% %Host_Args%
