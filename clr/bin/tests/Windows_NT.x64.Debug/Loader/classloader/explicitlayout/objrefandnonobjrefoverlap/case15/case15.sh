...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case15 case15.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case15 case15.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case15.dasm.il case15.exe
    "$CORE_ROOT/ildasm" -raweh -out=case15.dasm.il case15.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case15.asm.exe  case15.dasm.il
    "$CORE_ROOT/ilasm" -output=case15.asm.exe  case15.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case15
    if [ ! -f case15.org ]; then
        TakeLock
        if [ ! -f case15.org ]; then
          mkdir IL
          cp case15.exe IL/case15.exe
          mv case15.exe case15.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case15.org /out case15.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case15.org /out case15.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case15.reflect.xml
  if [ -f case15.reflect.xml ]
then
    ReflectionRoots="-x case15.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case15.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case15.asm.exe $CLRTestExecutionArguments
