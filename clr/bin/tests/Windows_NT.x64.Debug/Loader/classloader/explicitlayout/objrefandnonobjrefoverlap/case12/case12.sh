...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case12 case12.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/explicitlayout/objrefandnonobjrefoverlap/case12 case12.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=case12.dasm.il case12.exe
    "$CORE_ROOT/ildasm" -raweh -out=case12.dasm.il case12.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=case12.asm.exe  case12.dasm.il
    "$CORE_ROOT/ilasm" -output=case12.asm.exe  case12.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=case12
    if [ ! -f case12.org ]; then
        TakeLock
        if [ ! -f case12.org ]; then
          mkdir IL
          cp case12.exe IL/case12.exe
          mv case12.exe case12.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case12.org /out case12.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in case12.org /out case12.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in case12.reflect.xml
  if [ -f case12.reflect.xml ]
then
    ReflectionRoots="-x case12.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" case12.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" case12.asm.exe $CLRTestExecutionArguments
