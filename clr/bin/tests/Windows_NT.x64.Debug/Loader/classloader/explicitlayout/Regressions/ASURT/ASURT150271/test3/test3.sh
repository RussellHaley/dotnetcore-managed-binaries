...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/Loader/classloader/explicitlayout/Regressions/ASURT/ASURT150271/test3 test3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/Loader/classloader/explicitlayout/Regressions/ASURT/ASURT150271/test3 test3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=test3.dasm.il test3.exe
    "$CORE_ROOT/ildasm" -raweh -out=test3.dasm.il test3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=test3.asm.exe -QUIET -NOLOGO -DEBUG test3.dasm.il
    "$CORE_ROOT/ilasm" -output=test3.asm.exe -QUIET -NOLOGO -DEBUG test3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=test3
    if [ ! -f test3.org ]; then
        TakeLock
        if [ ! -f test3.org ]; then
          mkdir IL
          cp test3.exe IL/test3.exe
          mv test3.exe test3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test3.org /out test3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test3.org /out test3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in test3.reflect.xml
  if [ -f test3.reflect.xml ]
then
    ReflectionRoots="-x test3.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" test3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" test3.asm.exe $CLRTestExecutionArguments
