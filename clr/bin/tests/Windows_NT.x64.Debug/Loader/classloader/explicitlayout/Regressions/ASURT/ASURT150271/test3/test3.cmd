...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\Loader\classloader\explicitlayout\Regressions\ASURT\ASURT150271\test3 test3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\Loader\classloader\explicitlayout\Regressions\ASURT\ASURT150271\test3 test3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=test3.dasm.il test3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=test3.dasm.il test3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=test3.asm.exe -QUIET -NOLOGO -DEBUG test3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=test3.asm.exe -QUIET -NOLOGO -DEBUG test3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=test3
    if not exist "test3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "test3.org" (
            mkdir IL
            copy test3.exe IL\test3.exe 
            ren test3.exe test3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test3.org /out test3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test3.org /out test3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in test3.reflect.xml
    IF EXIST test3.reflect.xml set ReflectionRoots=-x test3.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% test3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% test3.asm.exe %CLRTestExecutionArguments% %Host_Args%
