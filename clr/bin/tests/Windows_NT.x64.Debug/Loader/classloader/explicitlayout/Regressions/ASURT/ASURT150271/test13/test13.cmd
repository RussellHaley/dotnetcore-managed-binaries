...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\Loader\classloader\explicitlayout\Regressions\ASURT\ASURT150271\test13 test13.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\Loader\classloader\explicitlayout\Regressions\ASURT\ASURT150271\test13 test13.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=test13.dasm.il test13.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=test13.dasm.il test13.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=test13.asm.exe -QUIET -NOLOGO -DEBUG test13.dasm.il
    %CORE_ROOT%\ilasm.exe /output=test13.asm.exe -QUIET -NOLOGO -DEBUG test13.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=test13
    if not exist "test13.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "test13.org" (
            mkdir IL
            copy test13.exe IL\test13.exe 
            ren test13.exe test13.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test13.org /out test13.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test13.org /out test13.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in test13.reflect.xml
    IF EXIST test13.reflect.xml set ReflectionRoots=-x test13.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% test13.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% test13.asm.exe %CLRTestExecutionArguments% %Host_Args%
