...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/Loader/classloader/explicitlayout/Regressions/ASURT/ASURT150271/test13 test13.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/Loader/classloader/explicitlayout/Regressions/ASURT/ASURT150271/test13 test13.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=test13.dasm.il test13.exe
    "$CORE_ROOT/ildasm" -raweh -out=test13.dasm.il test13.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=test13.asm.exe -QUIET -NOLOGO -DEBUG test13.dasm.il
    "$CORE_ROOT/ilasm" -output=test13.asm.exe -QUIET -NOLOGO -DEBUG test13.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=test13
    if [ ! -f test13.org ]; then
        TakeLock
        if [ ! -f test13.org ]; then
          mkdir IL
          cp test13.exe IL/test13.exe
          mv test13.exe test13.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test13.org /out test13.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test13.org /out test13.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in test13.reflect.xml
  if [ -f test13.reflect.xml ]
then
    ReflectionRoots="-x test13.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" test13.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" test13.asm.exe $CLRTestExecutionArguments
