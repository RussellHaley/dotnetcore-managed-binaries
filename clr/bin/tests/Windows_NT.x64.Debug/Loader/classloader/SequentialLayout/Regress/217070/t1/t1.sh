...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/SequentialLayout/Regress/217070/t1 t1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/Loader/classloader/SequentialLayout/Regress/217070/t1 t1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=t1.dasm.il t1.exe
    "$CORE_ROOT/ildasm" -raweh -out=t1.dasm.il t1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=t1.asm.exe -QUIET -NOLOGO -DEBUG t1.dasm.il
    "$CORE_ROOT/ilasm" -output=t1.asm.exe -QUIET -NOLOGO -DEBUG t1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=t1
    if [ ! -f t1.org ]; then
        TakeLock
        if [ ! -f t1.org ]; then
          mkdir IL
          cp t1.exe IL/t1.exe
          mv t1.exe t1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in t1.org /out t1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in t1.org /out t1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in t1.reflect.xml
  if [ -f t1.reflect.xml ]
then
    ReflectionRoots="-x t1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" t1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" t1.asm.exe $CLRTestExecutionArguments
