...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\SequentialLayout\Regress\217070\t1 t1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\Loader\classloader\SequentialLayout\Regress\217070\t1 t1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=t1.dasm.il t1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=t1.dasm.il t1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=t1.asm.exe -QUIET -NOLOGO -DEBUG t1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=t1.asm.exe -QUIET -NOLOGO -DEBUG t1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=t1
    if not exist "t1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "t1.org" (
            mkdir IL
            copy t1.exe IL\t1.exe 
            ren t1.exe t1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in t1.org /out t1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in t1.org /out t1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in t1.reflect.xml
    IF EXIST t1.reflect.xml set ReflectionRoots=-x t1.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% t1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% t1.asm.exe %CLRTestExecutionArguments% %Host_Args%
