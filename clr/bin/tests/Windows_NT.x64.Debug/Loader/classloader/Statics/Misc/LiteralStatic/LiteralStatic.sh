...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/Statics/Misc/LiteralStatic LiteralStatic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/Loader/classloader/Statics/Misc/LiteralStatic LiteralStatic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LiteralStatic.dasm.il LiteralStatic.exe
    "$CORE_ROOT/ildasm" -raweh -out=LiteralStatic.dasm.il LiteralStatic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LiteralStatic.asm.exe -QUIET -NOLOGO -DEBUG LiteralStatic.dasm.il
    "$CORE_ROOT/ilasm" -output=LiteralStatic.asm.exe -QUIET -NOLOGO -DEBUG LiteralStatic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LiteralStatic
    if [ ! -f LiteralStatic.org ]; then
        TakeLock
        if [ ! -f LiteralStatic.org ]; then
          mkdir IL
          cp LiteralStatic.exe IL/LiteralStatic.exe
          mv LiteralStatic.exe LiteralStatic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LiteralStatic.org /out LiteralStatic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LiteralStatic.org /out LiteralStatic.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LiteralStatic.reflect.xml
  if [ -f LiteralStatic.reflect.xml ]
then
    ReflectionRoots="-x LiteralStatic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/LiteralStatic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LiteralStatic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LiteralStatic.asm.exe $CLRTestExecutionArguments
