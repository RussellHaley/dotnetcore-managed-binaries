...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\Statics\Misc\LiteralStatic LiteralStatic.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\Loader\classloader\Statics\Misc\LiteralStatic LiteralStatic.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LiteralStatic.dasm.il LiteralStatic.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LiteralStatic.dasm.il LiteralStatic.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LiteralStatic.asm.exe -QUIET -NOLOGO -DEBUG LiteralStatic.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LiteralStatic.asm.exe -QUIET -NOLOGO -DEBUG LiteralStatic.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LiteralStatic
    if not exist "LiteralStatic.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LiteralStatic.org" (
            mkdir IL
            copy LiteralStatic.exe IL\LiteralStatic.exe 
            ren LiteralStatic.exe LiteralStatic.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LiteralStatic.org /out LiteralStatic.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LiteralStatic.org /out LiteralStatic.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LiteralStatic.reflect.xml
    IF EXIST LiteralStatic.reflect.xml set ReflectionRoots=-x LiteralStatic.reflect.xml
    ...
    set ExePath=%LinkBin%\LiteralStatic.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LiteralStatic.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LiteralStatic.asm.exe %CLRTestExecutionArguments% %Host_Args%
