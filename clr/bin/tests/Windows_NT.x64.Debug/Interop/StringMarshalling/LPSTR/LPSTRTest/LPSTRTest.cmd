...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\StringMarshalling\LPSTR\LPSTRTest LPSTRTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\StringMarshalling\LPSTR\LPSTRTest LPSTRTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LPSTRTest.dasm.il LPSTRTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LPSTRTest.dasm.il LPSTRTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LPSTRTest.asm.exe  LPSTRTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LPSTRTest.asm.exe  LPSTRTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LPSTRTest
    if not exist "LPSTRTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LPSTRTest.org" (
            mkdir IL
            copy LPSTRTest.exe IL\LPSTRTest.exe 
            ren LPSTRTest.exe LPSTRTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LPSTRTest.org /out LPSTRTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LPSTRTest.org /out LPSTRTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LPSTRTest.reflect.xml
    IF EXIST LPSTRTest.reflect.xml set ReflectionRoots=-x LPSTRTest.reflect.xml
    ...
    set ExePath=%LinkBin%\LPSTRTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LPSTRTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LPSTRTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
