...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/StringMarshalling/LPSTR/LPSTRTest LPSTRTest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/StringMarshalling/LPSTR/LPSTRTest LPSTRTest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LPSTRTest.dasm.il LPSTRTest.exe
    "$CORE_ROOT/ildasm" -raweh -out=LPSTRTest.dasm.il LPSTRTest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LPSTRTest.asm.exe  LPSTRTest.dasm.il
    "$CORE_ROOT/ilasm" -output=LPSTRTest.asm.exe  LPSTRTest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LPSTRTest
    if [ ! -f LPSTRTest.org ]; then
        TakeLock
        if [ ! -f LPSTRTest.org ]; then
          mkdir IL
          cp LPSTRTest.exe IL/LPSTRTest.exe
          mv LPSTRTest.exe LPSTRTest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LPSTRTest.org /out LPSTRTest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LPSTRTest.org /out LPSTRTest.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LPSTRTest.reflect.xml
  if [ -f LPSTRTest.reflect.xml ]
then
    ReflectionRoots="-x LPSTRTest.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LPSTRTest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LPSTRTest.asm.exe $CLRTestExecutionArguments
