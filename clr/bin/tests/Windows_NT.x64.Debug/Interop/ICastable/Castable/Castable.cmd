...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Interop\ICastable\Castable Castable.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Interop\ICastable\Castable Castable.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Castable.dasm.il Castable.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Castable.dasm.il Castable.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Castable.asm.exe  Castable.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Castable.asm.exe  Castable.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Castable
    if not exist "Castable.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Castable.org" (
            mkdir IL
            copy Castable.exe IL\Castable.exe 
            ren Castable.exe Castable.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Castable.org /out Castable.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Castable.org /out Castable.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Castable.reflect.xml
    IF EXIST Castable.reflect.xml set ReflectionRoots=-x Castable.reflect.xml
    ...
    set ExePath=%LinkBin%\Castable.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Castable.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Castable.asm.exe %CLRTestExecutionArguments% %Host_Args%
