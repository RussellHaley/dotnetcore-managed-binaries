...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Interop/ICastable/Castable Castable.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Interop/ICastable/Castable Castable.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Castable.dasm.il Castable.exe
    "$CORE_ROOT/ildasm" -raweh -out=Castable.dasm.il Castable.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Castable.asm.exe  Castable.dasm.il
    "$CORE_ROOT/ilasm" -output=Castable.asm.exe  Castable.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Castable
    if [ ! -f Castable.org ]; then
        TakeLock
        if [ ! -f Castable.org ]; then
          mkdir IL
          cp Castable.exe IL/Castable.exe
          mv Castable.exe Castable.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Castable.org /out Castable.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Castable.org /out Castable.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Castable.reflect.xml
  if [ -f Castable.reflect.xml ]
then
    ReflectionRoots="-x Castable.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Castable.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Castable.asm.exe $CLRTestExecutionArguments
