...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\StructMarshalling\PInvoke\MarshalStructAsLayoutSeq MarshalStructAsLayoutSeq.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\StructMarshalling\PInvoke\MarshalStructAsLayoutSeq MarshalStructAsLayoutSeq.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MarshalStructAsLayoutSeq.dasm.il MarshalStructAsLayoutSeq.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MarshalStructAsLayoutSeq.dasm.il MarshalStructAsLayoutSeq.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MarshalStructAsLayoutSeq.asm.exe  MarshalStructAsLayoutSeq.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MarshalStructAsLayoutSeq.asm.exe  MarshalStructAsLayoutSeq.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MarshalStructAsLayoutSeq
    if not exist "MarshalStructAsLayoutSeq.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MarshalStructAsLayoutSeq.org" (
            mkdir IL
            copy MarshalStructAsLayoutSeq.exe IL\MarshalStructAsLayoutSeq.exe 
            ren MarshalStructAsLayoutSeq.exe MarshalStructAsLayoutSeq.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MarshalStructAsLayoutSeq.org /out MarshalStructAsLayoutSeq.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MarshalStructAsLayoutSeq.org /out MarshalStructAsLayoutSeq.exe
...
set ExePath=MarshalStructAsLayoutSeq.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MarshalStructAsLayoutSeq.reflect.xml
    IF EXIST MarshalStructAsLayoutSeq.reflect.xml set ReflectionRoots=-x MarshalStructAsLayoutSeq.reflect.xml
    ...
    set ExePath=%LinkBin%\MarshalStructAsLayoutSeq.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MarshalStructAsLayoutSeq.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MarshalStructAsLayoutSeq.asm.exe %CLRTestExecutionArguments% %Host_Args%
