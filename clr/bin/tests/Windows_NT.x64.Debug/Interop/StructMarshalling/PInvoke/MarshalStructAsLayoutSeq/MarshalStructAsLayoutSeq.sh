...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/StructMarshalling/PInvoke/MarshalStructAsLayoutSeq MarshalStructAsLayoutSeq.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/StructMarshalling/PInvoke/MarshalStructAsLayoutSeq MarshalStructAsLayoutSeq.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MarshalStructAsLayoutSeq.dasm.il MarshalStructAsLayoutSeq.exe
    "$CORE_ROOT/ildasm" -raweh -out=MarshalStructAsLayoutSeq.dasm.il MarshalStructAsLayoutSeq.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MarshalStructAsLayoutSeq.asm.exe  MarshalStructAsLayoutSeq.dasm.il
    "$CORE_ROOT/ilasm" -output=MarshalStructAsLayoutSeq.asm.exe  MarshalStructAsLayoutSeq.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MarshalStructAsLayoutSeq
    if [ ! -f MarshalStructAsLayoutSeq.org ]; then
        TakeLock
        if [ ! -f MarshalStructAsLayoutSeq.org ]; then
          mkdir IL
          cp MarshalStructAsLayoutSeq.exe IL/MarshalStructAsLayoutSeq.exe
          mv MarshalStructAsLayoutSeq.exe MarshalStructAsLayoutSeq.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MarshalStructAsLayoutSeq.org /out MarshalStructAsLayoutSeq.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MarshalStructAsLayoutSeq.org /out MarshalStructAsLayoutSeq.exe
          __cgExitCode=$?
...
ExePath=MarshalStructAsLayoutSeq.exe
...
then
  ...
  # Use hints for reflection roots, if provided in MarshalStructAsLayoutSeq.reflect.xml
  if [ -f MarshalStructAsLayoutSeq.reflect.xml ]
then
    ReflectionRoots="-x MarshalStructAsLayoutSeq.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/MarshalStructAsLayoutSeq.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MarshalStructAsLayoutSeq.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MarshalStructAsLayoutSeq.asm.exe $CLRTestExecutionArguments
