...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\GetObjectForNativeVariant\GetObjectForNativeVariant GetObjectForNativeVariant.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\GetObjectForNativeVariant\GetObjectForNativeVariant GetObjectForNativeVariant.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GetObjectForNativeVariant.dasm.il GetObjectForNativeVariant.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GetObjectForNativeVariant.dasm.il GetObjectForNativeVariant.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GetObjectForNativeVariant.asm.exe  GetObjectForNativeVariant.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GetObjectForNativeVariant.asm.exe  GetObjectForNativeVariant.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GetObjectForNativeVariant
    if not exist "GetObjectForNativeVariant.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GetObjectForNativeVariant.org" (
            mkdir IL
            copy GetObjectForNativeVariant.exe IL\GetObjectForNativeVariant.exe 
            ren GetObjectForNativeVariant.exe GetObjectForNativeVariant.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetObjectForNativeVariant.org /out GetObjectForNativeVariant.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetObjectForNativeVariant.org /out GetObjectForNativeVariant.exe
...
set ExePath=GetObjectForNativeVariant.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GetObjectForNativeVariant.reflect.xml
    IF EXIST GetObjectForNativeVariant.reflect.xml set ReflectionRoots=-x GetObjectForNativeVariant.reflect.xml
    ...
    set ExePath=%LinkBin%\GetObjectForNativeVariant.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GetObjectForNativeVariant.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GetObjectForNativeVariant.asm.exe %CLRTestExecutionArguments% %Host_Args%
