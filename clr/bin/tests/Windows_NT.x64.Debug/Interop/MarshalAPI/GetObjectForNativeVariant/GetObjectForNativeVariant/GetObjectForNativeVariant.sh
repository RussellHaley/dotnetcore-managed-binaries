...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/GetObjectForNativeVariant/GetObjectForNativeVariant GetObjectForNativeVariant.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/GetObjectForNativeVariant/GetObjectForNativeVariant GetObjectForNativeVariant.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GetObjectForNativeVariant.dasm.il GetObjectForNativeVariant.exe
    "$CORE_ROOT/ildasm" -raweh -out=GetObjectForNativeVariant.dasm.il GetObjectForNativeVariant.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GetObjectForNativeVariant.asm.exe  GetObjectForNativeVariant.dasm.il
    "$CORE_ROOT/ilasm" -output=GetObjectForNativeVariant.asm.exe  GetObjectForNativeVariant.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GetObjectForNativeVariant
    if [ ! -f GetObjectForNativeVariant.org ]; then
        TakeLock
        if [ ! -f GetObjectForNativeVariant.org ]; then
          mkdir IL
          cp GetObjectForNativeVariant.exe IL/GetObjectForNativeVariant.exe
          mv GetObjectForNativeVariant.exe GetObjectForNativeVariant.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetObjectForNativeVariant.org /out GetObjectForNativeVariant.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetObjectForNativeVariant.org /out GetObjectForNativeVariant.exe
          __cgExitCode=$?
...
ExePath=GetObjectForNativeVariant.exe
...
then
  ...
  # Use hints for reflection roots, if provided in GetObjectForNativeVariant.reflect.xml
  if [ -f GetObjectForNativeVariant.reflect.xml ]
then
    ReflectionRoots="-x GetObjectForNativeVariant.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GetObjectForNativeVariant.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GetObjectForNativeVariant.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GetObjectForNativeVariant.asm.exe $CLRTestExecutionArguments
