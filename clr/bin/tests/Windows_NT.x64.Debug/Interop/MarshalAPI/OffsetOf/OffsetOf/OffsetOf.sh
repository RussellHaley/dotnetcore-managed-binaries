...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/OffsetOf/OffsetOf OffsetOf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/OffsetOf/OffsetOf OffsetOf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=OffsetOf.dasm.il OffsetOf.exe
    "$CORE_ROOT/ildasm" -raweh -out=OffsetOf.dasm.il OffsetOf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=OffsetOf.asm.exe  OffsetOf.dasm.il
    "$CORE_ROOT/ilasm" -output=OffsetOf.asm.exe  OffsetOf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=OffsetOf
    if [ ! -f OffsetOf.org ]; then
        TakeLock
        if [ ! -f OffsetOf.org ]; then
          mkdir IL
          cp OffsetOf.exe IL/OffsetOf.exe
          mv OffsetOf.exe OffsetOf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in OffsetOf.org /out OffsetOf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in OffsetOf.org /out OffsetOf.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in OffsetOf.reflect.xml
  if [ -f OffsetOf.reflect.xml ]
then
    ReflectionRoots="-x OffsetOf.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" OffsetOf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" OffsetOf.asm.exe $CLRTestExecutionArguments
