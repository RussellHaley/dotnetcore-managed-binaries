...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\OffsetOf\OffsetOf OffsetOf.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\OffsetOf\OffsetOf OffsetOf.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=OffsetOf.dasm.il OffsetOf.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=OffsetOf.dasm.il OffsetOf.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=OffsetOf.asm.exe  OffsetOf.dasm.il
    %CORE_ROOT%\ilasm.exe /output=OffsetOf.asm.exe  OffsetOf.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=OffsetOf
    if not exist "OffsetOf.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "OffsetOf.org" (
            mkdir IL
            copy OffsetOf.exe IL\OffsetOf.exe 
            ren OffsetOf.exe OffsetOf.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in OffsetOf.org /out OffsetOf.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in OffsetOf.org /out OffsetOf.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in OffsetOf.reflect.xml
    IF EXIST OffsetOf.reflect.xml set ReflectionRoots=-x OffsetOf.reflect.xml
    ...
    set ExePath=%LinkBin%\OffsetOf.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% OffsetOf.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% OffsetOf.asm.exe %CLRTestExecutionArguments% %Host_Args%
