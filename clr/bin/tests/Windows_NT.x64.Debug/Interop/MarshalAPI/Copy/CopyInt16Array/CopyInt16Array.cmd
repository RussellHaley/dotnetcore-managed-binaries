...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyInt16Array CopyInt16Array.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyInt16Array CopyInt16Array.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CopyInt16Array.dasm.il CopyInt16Array.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CopyInt16Array.dasm.il CopyInt16Array.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CopyInt16Array.asm.exe  CopyInt16Array.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CopyInt16Array.asm.exe  CopyInt16Array.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CopyInt16Array
    if not exist "CopyInt16Array.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CopyInt16Array.org" (
            mkdir IL
            copy CopyInt16Array.exe IL\CopyInt16Array.exe 
            ren CopyInt16Array.exe CopyInt16Array.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyInt16Array.org /out CopyInt16Array.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyInt16Array.org /out CopyInt16Array.exe
...
set ExePath=CopyInt16Array.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CopyInt16Array.reflect.xml
    IF EXIST CopyInt16Array.reflect.xml set ReflectionRoots=-x CopyInt16Array.reflect.xml
    ...
    set ExePath=%LinkBin%\CopyInt16Array.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CopyInt16Array.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CopyInt16Array.asm.exe %CLRTestExecutionArguments% %Host_Args%
