...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyInt16Array CopyInt16Array.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyInt16Array CopyInt16Array.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CopyInt16Array.dasm.il CopyInt16Array.exe
    "$CORE_ROOT/ildasm" -raweh -out=CopyInt16Array.dasm.il CopyInt16Array.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CopyInt16Array.asm.exe  CopyInt16Array.dasm.il
    "$CORE_ROOT/ilasm" -output=CopyInt16Array.asm.exe  CopyInt16Array.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CopyInt16Array
    if [ ! -f CopyInt16Array.org ]; then
        TakeLock
        if [ ! -f CopyInt16Array.org ]; then
          mkdir IL
          cp CopyInt16Array.exe IL/CopyInt16Array.exe
          mv CopyInt16Array.exe CopyInt16Array.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyInt16Array.org /out CopyInt16Array.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyInt16Array.org /out CopyInt16Array.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CopyInt16Array.reflect.xml
  if [ -f CopyInt16Array.reflect.xml ]
then
    ReflectionRoots="-x CopyInt16Array.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CopyInt16Array.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CopyInt16Array.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CopyInt16Array.asm.exe $CLRTestExecutionArguments
