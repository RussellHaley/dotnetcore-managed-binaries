...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyIntPtrArray CopyIntPtrArray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyIntPtrArray CopyIntPtrArray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CopyIntPtrArray.dasm.il CopyIntPtrArray.exe
    "$CORE_ROOT/ildasm" -raweh -out=CopyIntPtrArray.dasm.il CopyIntPtrArray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CopyIntPtrArray.asm.exe  CopyIntPtrArray.dasm.il
    "$CORE_ROOT/ilasm" -output=CopyIntPtrArray.asm.exe  CopyIntPtrArray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CopyIntPtrArray
    if [ ! -f CopyIntPtrArray.org ]; then
        TakeLock
        if [ ! -f CopyIntPtrArray.org ]; then
          mkdir IL
          cp CopyIntPtrArray.exe IL/CopyIntPtrArray.exe
          mv CopyIntPtrArray.exe CopyIntPtrArray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyIntPtrArray.org /out CopyIntPtrArray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyIntPtrArray.org /out CopyIntPtrArray.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CopyIntPtrArray.reflect.xml
  if [ -f CopyIntPtrArray.reflect.xml ]
then
    ReflectionRoots="-x CopyIntPtrArray.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CopyIntPtrArray.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CopyIntPtrArray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CopyIntPtrArray.asm.exe $CLRTestExecutionArguments
