...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyIntPtrArray CopyIntPtrArray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyIntPtrArray CopyIntPtrArray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CopyIntPtrArray.dasm.il CopyIntPtrArray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CopyIntPtrArray.dasm.il CopyIntPtrArray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CopyIntPtrArray.asm.exe  CopyIntPtrArray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CopyIntPtrArray.asm.exe  CopyIntPtrArray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CopyIntPtrArray
    if not exist "CopyIntPtrArray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CopyIntPtrArray.org" (
            mkdir IL
            copy CopyIntPtrArray.exe IL\CopyIntPtrArray.exe 
            ren CopyIntPtrArray.exe CopyIntPtrArray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyIntPtrArray.org /out CopyIntPtrArray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyIntPtrArray.org /out CopyIntPtrArray.exe
...
set ExePath=CopyIntPtrArray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CopyIntPtrArray.reflect.xml
    IF EXIST CopyIntPtrArray.reflect.xml set ReflectionRoots=-x CopyIntPtrArray.reflect.xml
    ...
    set ExePath=%LinkBin%\CopyIntPtrArray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CopyIntPtrArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CopyIntPtrArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
