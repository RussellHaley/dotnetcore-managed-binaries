...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyByteArray CopyByteArray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyByteArray CopyByteArray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CopyByteArray.dasm.il CopyByteArray.exe
    "$CORE_ROOT/ildasm" -raweh -out=CopyByteArray.dasm.il CopyByteArray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CopyByteArray.asm.exe  CopyByteArray.dasm.il
    "$CORE_ROOT/ilasm" -output=CopyByteArray.asm.exe  CopyByteArray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CopyByteArray
    if [ ! -f CopyByteArray.org ]; then
        TakeLock
        if [ ! -f CopyByteArray.org ]; then
          mkdir IL
          cp CopyByteArray.exe IL/CopyByteArray.exe
          mv CopyByteArray.exe CopyByteArray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyByteArray.org /out CopyByteArray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyByteArray.org /out CopyByteArray.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CopyByteArray.reflect.xml
  if [ -f CopyByteArray.reflect.xml ]
then
    ReflectionRoots="-x CopyByteArray.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CopyByteArray.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CopyByteArray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CopyByteArray.asm.exe $CLRTestExecutionArguments
