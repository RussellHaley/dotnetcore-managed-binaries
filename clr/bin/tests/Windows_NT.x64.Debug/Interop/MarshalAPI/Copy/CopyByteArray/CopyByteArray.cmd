...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyByteArray CopyByteArray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyByteArray CopyByteArray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CopyByteArray.dasm.il CopyByteArray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CopyByteArray.dasm.il CopyByteArray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CopyByteArray.asm.exe  CopyByteArray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CopyByteArray.asm.exe  CopyByteArray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CopyByteArray
    if not exist "CopyByteArray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CopyByteArray.org" (
            mkdir IL
            copy CopyByteArray.exe IL\CopyByteArray.exe 
            ren CopyByteArray.exe CopyByteArray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyByteArray.org /out CopyByteArray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyByteArray.org /out CopyByteArray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CopyByteArray.reflect.xml
    IF EXIST CopyByteArray.reflect.xml set ReflectionRoots=-x CopyByteArray.reflect.xml
    ...
    set ExePath=%LinkBin%\CopyByteArray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CopyByteArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CopyByteArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
