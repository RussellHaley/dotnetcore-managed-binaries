...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyInt64Array CopyInt64Array.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyInt64Array CopyInt64Array.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CopyInt64Array.dasm.il CopyInt64Array.exe
    "$CORE_ROOT/ildasm" -raweh -out=CopyInt64Array.dasm.il CopyInt64Array.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CopyInt64Array.asm.exe  CopyInt64Array.dasm.il
    "$CORE_ROOT/ilasm" -output=CopyInt64Array.asm.exe  CopyInt64Array.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CopyInt64Array
    if [ ! -f CopyInt64Array.org ]; then
        TakeLock
        if [ ! -f CopyInt64Array.org ]; then
          mkdir IL
          cp CopyInt64Array.exe IL/CopyInt64Array.exe
          mv CopyInt64Array.exe CopyInt64Array.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyInt64Array.org /out CopyInt64Array.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyInt64Array.org /out CopyInt64Array.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CopyInt64Array.reflect.xml
  if [ -f CopyInt64Array.reflect.xml ]
then
    ReflectionRoots="-x CopyInt64Array.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CopyInt64Array.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CopyInt64Array.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CopyInt64Array.asm.exe $CLRTestExecutionArguments
