...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyInt64Array CopyInt64Array.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyInt64Array CopyInt64Array.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CopyInt64Array.dasm.il CopyInt64Array.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CopyInt64Array.dasm.il CopyInt64Array.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CopyInt64Array.asm.exe  CopyInt64Array.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CopyInt64Array.asm.exe  CopyInt64Array.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CopyInt64Array
    if not exist "CopyInt64Array.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CopyInt64Array.org" (
            mkdir IL
            copy CopyInt64Array.exe IL\CopyInt64Array.exe 
            ren CopyInt64Array.exe CopyInt64Array.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyInt64Array.org /out CopyInt64Array.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyInt64Array.org /out CopyInt64Array.exe
...
set ExePath=CopyInt64Array.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CopyInt64Array.reflect.xml
    IF EXIST CopyInt64Array.reflect.xml set ReflectionRoots=-x CopyInt64Array.reflect.xml
    ...
    set ExePath=%LinkBin%\CopyInt64Array.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CopyInt64Array.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CopyInt64Array.asm.exe %CLRTestExecutionArguments% %Host_Args%
