...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyInt32Array CopyInt32Array.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyInt32Array CopyInt32Array.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CopyInt32Array.dasm.il CopyInt32Array.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CopyInt32Array.dasm.il CopyInt32Array.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CopyInt32Array.asm.exe  CopyInt32Array.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CopyInt32Array.asm.exe  CopyInt32Array.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CopyInt32Array
    if not exist "CopyInt32Array.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CopyInt32Array.org" (
            mkdir IL
            copy CopyInt32Array.exe IL\CopyInt32Array.exe 
            ren CopyInt32Array.exe CopyInt32Array.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyInt32Array.org /out CopyInt32Array.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyInt32Array.org /out CopyInt32Array.exe
...
set ExePath=CopyInt32Array.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CopyInt32Array.reflect.xml
    IF EXIST CopyInt32Array.reflect.xml set ReflectionRoots=-x CopyInt32Array.reflect.xml
    ...
    set ExePath=%LinkBin%\CopyInt32Array.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CopyInt32Array.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CopyInt32Array.asm.exe %CLRTestExecutionArguments% %Host_Args%
