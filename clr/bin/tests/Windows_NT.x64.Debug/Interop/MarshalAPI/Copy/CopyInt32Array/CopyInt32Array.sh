...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyInt32Array CopyInt32Array.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyInt32Array CopyInt32Array.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CopyInt32Array.dasm.il CopyInt32Array.exe
    "$CORE_ROOT/ildasm" -raweh -out=CopyInt32Array.dasm.il CopyInt32Array.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CopyInt32Array.asm.exe  CopyInt32Array.dasm.il
    "$CORE_ROOT/ilasm" -output=CopyInt32Array.asm.exe  CopyInt32Array.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CopyInt32Array
    if [ ! -f CopyInt32Array.org ]; then
        TakeLock
        if [ ! -f CopyInt32Array.org ]; then
          mkdir IL
          cp CopyInt32Array.exe IL/CopyInt32Array.exe
          mv CopyInt32Array.exe CopyInt32Array.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyInt32Array.org /out CopyInt32Array.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyInt32Array.org /out CopyInt32Array.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CopyInt32Array.reflect.xml
  if [ -f CopyInt32Array.reflect.xml ]
then
    ReflectionRoots="-x CopyInt32Array.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CopyInt32Array.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CopyInt32Array.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CopyInt32Array.asm.exe $CLRTestExecutionArguments
