...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopySingleArray CopySingleArray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopySingleArray CopySingleArray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CopySingleArray.dasm.il CopySingleArray.exe
    "$CORE_ROOT/ildasm" -raweh -out=CopySingleArray.dasm.il CopySingleArray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CopySingleArray.asm.exe  CopySingleArray.dasm.il
    "$CORE_ROOT/ilasm" -output=CopySingleArray.asm.exe  CopySingleArray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CopySingleArray
    if [ ! -f CopySingleArray.org ]; then
        TakeLock
        if [ ! -f CopySingleArray.org ]; then
          mkdir IL
          cp CopySingleArray.exe IL/CopySingleArray.exe
          mv CopySingleArray.exe CopySingleArray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopySingleArray.org /out CopySingleArray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopySingleArray.org /out CopySingleArray.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CopySingleArray.reflect.xml
  if [ -f CopySingleArray.reflect.xml ]
then
    ReflectionRoots="-x CopySingleArray.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CopySingleArray.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CopySingleArray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CopySingleArray.asm.exe $CLRTestExecutionArguments
