...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopySingleArray CopySingleArray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopySingleArray CopySingleArray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CopySingleArray.dasm.il CopySingleArray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CopySingleArray.dasm.il CopySingleArray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CopySingleArray.asm.exe  CopySingleArray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CopySingleArray.asm.exe  CopySingleArray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CopySingleArray
    if not exist "CopySingleArray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CopySingleArray.org" (
            mkdir IL
            copy CopySingleArray.exe IL\CopySingleArray.exe 
            ren CopySingleArray.exe CopySingleArray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopySingleArray.org /out CopySingleArray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopySingleArray.org /out CopySingleArray.exe
...
set ExePath=CopySingleArray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CopySingleArray.reflect.xml
    IF EXIST CopySingleArray.reflect.xml set ReflectionRoots=-x CopySingleArray.reflect.xml
    ...
    set ExePath=%LinkBin%\CopySingleArray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CopySingleArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CopySingleArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
