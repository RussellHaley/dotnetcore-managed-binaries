...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyDoubleArray CopyDoubleArray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyDoubleArray CopyDoubleArray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CopyDoubleArray.dasm.il CopyDoubleArray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CopyDoubleArray.dasm.il CopyDoubleArray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CopyDoubleArray.asm.exe  CopyDoubleArray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CopyDoubleArray.asm.exe  CopyDoubleArray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CopyDoubleArray
    if not exist "CopyDoubleArray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CopyDoubleArray.org" (
            mkdir IL
            copy CopyDoubleArray.exe IL\CopyDoubleArray.exe 
            ren CopyDoubleArray.exe CopyDoubleArray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyDoubleArray.org /out CopyDoubleArray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyDoubleArray.org /out CopyDoubleArray.exe
...
set ExePath=CopyDoubleArray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CopyDoubleArray.reflect.xml
    IF EXIST CopyDoubleArray.reflect.xml set ReflectionRoots=-x CopyDoubleArray.reflect.xml
    ...
    set ExePath=%LinkBin%\CopyDoubleArray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CopyDoubleArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CopyDoubleArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
