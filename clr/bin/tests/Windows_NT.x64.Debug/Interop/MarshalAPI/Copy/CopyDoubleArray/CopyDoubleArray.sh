...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyDoubleArray CopyDoubleArray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyDoubleArray CopyDoubleArray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CopyDoubleArray.dasm.il CopyDoubleArray.exe
    "$CORE_ROOT/ildasm" -raweh -out=CopyDoubleArray.dasm.il CopyDoubleArray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CopyDoubleArray.asm.exe  CopyDoubleArray.dasm.il
    "$CORE_ROOT/ilasm" -output=CopyDoubleArray.asm.exe  CopyDoubleArray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CopyDoubleArray
    if [ ! -f CopyDoubleArray.org ]; then
        TakeLock
        if [ ! -f CopyDoubleArray.org ]; then
          mkdir IL
          cp CopyDoubleArray.exe IL/CopyDoubleArray.exe
          mv CopyDoubleArray.exe CopyDoubleArray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyDoubleArray.org /out CopyDoubleArray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyDoubleArray.org /out CopyDoubleArray.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CopyDoubleArray.reflect.xml
  if [ -f CopyDoubleArray.reflect.xml ]
then
    ReflectionRoots="-x CopyDoubleArray.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CopyDoubleArray.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CopyDoubleArray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CopyDoubleArray.asm.exe $CLRTestExecutionArguments
