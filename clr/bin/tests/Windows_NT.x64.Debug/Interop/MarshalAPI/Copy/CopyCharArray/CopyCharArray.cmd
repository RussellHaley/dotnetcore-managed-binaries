...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyCharArray CopyCharArray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Copy\CopyCharArray CopyCharArray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CopyCharArray.dasm.il CopyCharArray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CopyCharArray.dasm.il CopyCharArray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CopyCharArray.asm.exe  CopyCharArray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CopyCharArray.asm.exe  CopyCharArray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CopyCharArray
    if not exist "CopyCharArray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CopyCharArray.org" (
            mkdir IL
            copy CopyCharArray.exe IL\CopyCharArray.exe 
            ren CopyCharArray.exe CopyCharArray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyCharArray.org /out CopyCharArray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CopyCharArray.org /out CopyCharArray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CopyCharArray.reflect.xml
    IF EXIST CopyCharArray.reflect.xml set ReflectionRoots=-x CopyCharArray.reflect.xml
    ...
    set ExePath=%LinkBin%\CopyCharArray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CopyCharArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CopyCharArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
