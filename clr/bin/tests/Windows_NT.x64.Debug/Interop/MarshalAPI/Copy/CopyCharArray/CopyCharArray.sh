...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyCharArray CopyCharArray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Copy/CopyCharArray CopyCharArray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CopyCharArray.dasm.il CopyCharArray.exe
    "$CORE_ROOT/ildasm" -raweh -out=CopyCharArray.dasm.il CopyCharArray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CopyCharArray.asm.exe  CopyCharArray.dasm.il
    "$CORE_ROOT/ilasm" -output=CopyCharArray.asm.exe  CopyCharArray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CopyCharArray
    if [ ! -f CopyCharArray.org ]; then
        TakeLock
        if [ ! -f CopyCharArray.org ]; then
          mkdir IL
          cp CopyCharArray.exe IL/CopyCharArray.exe
          mv CopyCharArray.exe CopyCharArray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyCharArray.org /out CopyCharArray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CopyCharArray.org /out CopyCharArray.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CopyCharArray.reflect.xml
  if [ -f CopyCharArray.reflect.xml ]
then
    ReflectionRoots="-x CopyCharArray.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CopyCharArray.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CopyCharArray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CopyCharArray.asm.exe $CLRTestExecutionArguments
