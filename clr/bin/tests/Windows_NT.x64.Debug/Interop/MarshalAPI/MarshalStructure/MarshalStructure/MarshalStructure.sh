...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/MarshalStructure/MarshalStructure MarshalStructure.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/MarshalStructure/MarshalStructure MarshalStructure.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MarshalStructure.dasm.il MarshalStructure.exe
    "$CORE_ROOT/ildasm" -raweh -out=MarshalStructure.dasm.il MarshalStructure.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MarshalStructure.asm.exe  MarshalStructure.dasm.il
    "$CORE_ROOT/ilasm" -output=MarshalStructure.asm.exe  MarshalStructure.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MarshalStructure
    if [ ! -f MarshalStructure.org ]; then
        TakeLock
        if [ ! -f MarshalStructure.org ]; then
          mkdir IL
          cp MarshalStructure.exe IL/MarshalStructure.exe
          mv MarshalStructure.exe MarshalStructure.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MarshalStructure.org /out MarshalStructure.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MarshalStructure.org /out MarshalStructure.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in MarshalStructure.reflect.xml
  if [ -f MarshalStructure.reflect.xml ]
then
    ReflectionRoots="-x MarshalStructure.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/MarshalStructure.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MarshalStructure.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MarshalStructure.asm.exe $CLRTestExecutionArguments
