...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\MarshalStructure\MarshalStructure MarshalStructure.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\MarshalStructure\MarshalStructure MarshalStructure.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MarshalStructure.dasm.il MarshalStructure.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MarshalStructure.dasm.il MarshalStructure.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MarshalStructure.asm.exe  MarshalStructure.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MarshalStructure.asm.exe  MarshalStructure.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MarshalStructure
    if not exist "MarshalStructure.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MarshalStructure.org" (
            mkdir IL
            copy MarshalStructure.exe IL\MarshalStructure.exe 
            ren MarshalStructure.exe MarshalStructure.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MarshalStructure.org /out MarshalStructure.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MarshalStructure.org /out MarshalStructure.exe
...
set ExePath=MarshalStructure.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MarshalStructure.reflect.xml
    IF EXIST MarshalStructure.reflect.xml set ReflectionRoots=-x MarshalStructure.reflect.xml
    ...
    set ExePath=%LinkBin%\MarshalStructure.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MarshalStructure.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MarshalStructure.asm.exe %CLRTestExecutionArguments% %Host_Args%
