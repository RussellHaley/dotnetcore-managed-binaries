...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/IUnknown/IUnknownTest IUnknownTest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/IUnknown/IUnknownTest IUnknownTest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=IUnknownTest.dasm.il IUnknownTest.exe
    "$CORE_ROOT/ildasm" -raweh -out=IUnknownTest.dasm.il IUnknownTest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=IUnknownTest.asm.exe  IUnknownTest.dasm.il
    "$CORE_ROOT/ilasm" -output=IUnknownTest.asm.exe  IUnknownTest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=IUnknownTest
    if [ ! -f IUnknownTest.org ]; then
        TakeLock
        if [ ! -f IUnknownTest.org ]; then
          mkdir IL
          cp IUnknownTest.exe IL/IUnknownTest.exe
          mv IUnknownTest.exe IUnknownTest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in IUnknownTest.org /out IUnknownTest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in IUnknownTest.org /out IUnknownTest.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in IUnknownTest.reflect.xml
  if [ -f IUnknownTest.reflect.xml ]
then
    ReflectionRoots="-x IUnknownTest.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/IUnknownTest.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" IUnknownTest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" IUnknownTest.asm.exe $CLRTestExecutionArguments
