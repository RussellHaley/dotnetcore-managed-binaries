...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\IUnknown\IUnknownTest IUnknownTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\IUnknown\IUnknownTest IUnknownTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=IUnknownTest.dasm.il IUnknownTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=IUnknownTest.dasm.il IUnknownTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=IUnknownTest.asm.exe  IUnknownTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=IUnknownTest.asm.exe  IUnknownTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=IUnknownTest
    if not exist "IUnknownTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "IUnknownTest.org" (
            mkdir IL
            copy IUnknownTest.exe IL\IUnknownTest.exe 
            ren IUnknownTest.exe IUnknownTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in IUnknownTest.org /out IUnknownTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in IUnknownTest.org /out IUnknownTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in IUnknownTest.reflect.xml
    IF EXIST IUnknownTest.reflect.xml set ReflectionRoots=-x IUnknownTest.reflect.xml
    ...
    set ExePath=%LinkBin%\IUnknownTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% IUnknownTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% IUnknownTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
