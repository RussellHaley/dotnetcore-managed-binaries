...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteInt64 ReadWriteInt64.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteInt64 ReadWriteInt64.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteInt64.dasm.il ReadWriteInt64.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteInt64.dasm.il ReadWriteInt64.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ReadWriteInt64.asm.exe  ReadWriteInt64.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ReadWriteInt64.asm.exe  ReadWriteInt64.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ReadWriteInt64
    if not exist "ReadWriteInt64.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ReadWriteInt64.org" (
            mkdir IL
            copy ReadWriteInt64.exe IL\ReadWriteInt64.exe 
            ren ReadWriteInt64.exe ReadWriteInt64.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteInt64.org /out ReadWriteInt64.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteInt64.org /out ReadWriteInt64.exe
...
set ExePath=ReadWriteInt64.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ReadWriteInt64.reflect.xml
    IF EXIST ReadWriteInt64.reflect.xml set ReflectionRoots=-x ReadWriteInt64.reflect.xml
    ...
    set ExePath=%LinkBin%\ReadWriteInt64.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ReadWriteInt64.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ReadWriteInt64.asm.exe %CLRTestExecutionArguments% %Host_Args%
