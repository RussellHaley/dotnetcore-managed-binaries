...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteInt64 ReadWriteInt64.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteInt64 ReadWriteInt64.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ReadWriteInt64.dasm.il ReadWriteInt64.exe
    "$CORE_ROOT/ildasm" -raweh -out=ReadWriteInt64.dasm.il ReadWriteInt64.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ReadWriteInt64.asm.exe  ReadWriteInt64.dasm.il
    "$CORE_ROOT/ilasm" -output=ReadWriteInt64.asm.exe  ReadWriteInt64.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ReadWriteInt64
    if [ ! -f ReadWriteInt64.org ]; then
        TakeLock
        if [ ! -f ReadWriteInt64.org ]; then
          mkdir IL
          cp ReadWriteInt64.exe IL/ReadWriteInt64.exe
          mv ReadWriteInt64.exe ReadWriteInt64.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteInt64.org /out ReadWriteInt64.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteInt64.org /out ReadWriteInt64.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ReadWriteInt64.reflect.xml
  if [ -f ReadWriteInt64.reflect.xml ]
then
    ReflectionRoots="-x ReadWriteInt64.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ReadWriteInt64.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ReadWriteInt64.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ReadWriteInt64.asm.exe $CLRTestExecutionArguments
