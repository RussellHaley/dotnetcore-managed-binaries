...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteInt32 ReadWriteInt32.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteInt32 ReadWriteInt32.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteInt32.dasm.il ReadWriteInt32.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteInt32.dasm.il ReadWriteInt32.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ReadWriteInt32.asm.exe  ReadWriteInt32.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ReadWriteInt32.asm.exe  ReadWriteInt32.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ReadWriteInt32
    if not exist "ReadWriteInt32.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ReadWriteInt32.org" (
            mkdir IL
            copy ReadWriteInt32.exe IL\ReadWriteInt32.exe 
            ren ReadWriteInt32.exe ReadWriteInt32.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteInt32.org /out ReadWriteInt32.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteInt32.org /out ReadWriteInt32.exe
...
set ExePath=ReadWriteInt32.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ReadWriteInt32.reflect.xml
    IF EXIST ReadWriteInt32.reflect.xml set ReflectionRoots=-x ReadWriteInt32.reflect.xml
    ...
    set ExePath=%LinkBin%\ReadWriteInt32.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ReadWriteInt32.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ReadWriteInt32.asm.exe %CLRTestExecutionArguments% %Host_Args%
