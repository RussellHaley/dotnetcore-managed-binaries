...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteInt32 ReadWriteInt32.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteInt32 ReadWriteInt32.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ReadWriteInt32.dasm.il ReadWriteInt32.exe
    "$CORE_ROOT/ildasm" -raweh -out=ReadWriteInt32.dasm.il ReadWriteInt32.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ReadWriteInt32.asm.exe  ReadWriteInt32.dasm.il
    "$CORE_ROOT/ilasm" -output=ReadWriteInt32.asm.exe  ReadWriteInt32.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ReadWriteInt32
    if [ ! -f ReadWriteInt32.org ]; then
        TakeLock
        if [ ! -f ReadWriteInt32.org ]; then
          mkdir IL
          cp ReadWriteInt32.exe IL/ReadWriteInt32.exe
          mv ReadWriteInt32.exe ReadWriteInt32.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteInt32.org /out ReadWriteInt32.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteInt32.org /out ReadWriteInt32.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ReadWriteInt32.reflect.xml
  if [ -f ReadWriteInt32.reflect.xml ]
then
    ReflectionRoots="-x ReadWriteInt32.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ReadWriteInt32.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ReadWriteInt32.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ReadWriteInt32.asm.exe $CLRTestExecutionArguments
