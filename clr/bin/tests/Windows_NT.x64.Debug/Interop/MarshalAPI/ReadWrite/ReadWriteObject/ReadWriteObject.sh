...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteObject ReadWriteObject.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteObject ReadWriteObject.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ReadWriteObject.dasm.il ReadWriteObject.exe
    "$CORE_ROOT/ildasm" -raweh -out=ReadWriteObject.dasm.il ReadWriteObject.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ReadWriteObject.asm.exe  ReadWriteObject.dasm.il
    "$CORE_ROOT/ilasm" -output=ReadWriteObject.asm.exe  ReadWriteObject.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ReadWriteObject
    if [ ! -f ReadWriteObject.org ]; then
        TakeLock
        if [ ! -f ReadWriteObject.org ]; then
          mkdir IL
          cp ReadWriteObject.exe IL/ReadWriteObject.exe
          mv ReadWriteObject.exe ReadWriteObject.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteObject.org /out ReadWriteObject.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteObject.org /out ReadWriteObject.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ReadWriteObject.reflect.xml
  if [ -f ReadWriteObject.reflect.xml ]
then
    ReflectionRoots="-x ReadWriteObject.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ReadWriteObject.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ReadWriteObject.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ReadWriteObject.asm.exe $CLRTestExecutionArguments
