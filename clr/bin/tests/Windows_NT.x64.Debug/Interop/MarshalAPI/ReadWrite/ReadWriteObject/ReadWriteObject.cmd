...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteObject ReadWriteObject.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteObject ReadWriteObject.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteObject.dasm.il ReadWriteObject.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteObject.dasm.il ReadWriteObject.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ReadWriteObject.asm.exe  ReadWriteObject.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ReadWriteObject.asm.exe  ReadWriteObject.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ReadWriteObject
    if not exist "ReadWriteObject.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ReadWriteObject.org" (
            mkdir IL
            copy ReadWriteObject.exe IL\ReadWriteObject.exe 
            ren ReadWriteObject.exe ReadWriteObject.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteObject.org /out ReadWriteObject.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteObject.org /out ReadWriteObject.exe
...
set ExePath=ReadWriteObject.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ReadWriteObject.reflect.xml
    IF EXIST ReadWriteObject.reflect.xml set ReflectionRoots=-x ReadWriteObject.reflect.xml
    ...
    set ExePath=%LinkBin%\ReadWriteObject.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ReadWriteObject.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ReadWriteObject.asm.exe %CLRTestExecutionArguments% %Host_Args%
