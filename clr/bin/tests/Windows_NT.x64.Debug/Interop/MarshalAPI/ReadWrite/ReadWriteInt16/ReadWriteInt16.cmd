...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteInt16 ReadWriteInt16.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteInt16 ReadWriteInt16.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteInt16.dasm.il ReadWriteInt16.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteInt16.dasm.il ReadWriteInt16.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ReadWriteInt16.asm.exe  ReadWriteInt16.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ReadWriteInt16.asm.exe  ReadWriteInt16.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ReadWriteInt16
    if not exist "ReadWriteInt16.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ReadWriteInt16.org" (
            mkdir IL
            copy ReadWriteInt16.exe IL\ReadWriteInt16.exe 
            ren ReadWriteInt16.exe ReadWriteInt16.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteInt16.org /out ReadWriteInt16.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteInt16.org /out ReadWriteInt16.exe
...
set ExePath=ReadWriteInt16.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ReadWriteInt16.reflect.xml
    IF EXIST ReadWriteInt16.reflect.xml set ReflectionRoots=-x ReadWriteInt16.reflect.xml
    ...
    set ExePath=%LinkBin%\ReadWriteInt16.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ReadWriteInt16.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ReadWriteInt16.asm.exe %CLRTestExecutionArguments% %Host_Args%
