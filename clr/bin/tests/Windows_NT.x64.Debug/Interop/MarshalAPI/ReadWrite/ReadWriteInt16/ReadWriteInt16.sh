...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteInt16 ReadWriteInt16.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteInt16 ReadWriteInt16.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ReadWriteInt16.dasm.il ReadWriteInt16.exe
    "$CORE_ROOT/ildasm" -raweh -out=ReadWriteInt16.dasm.il ReadWriteInt16.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ReadWriteInt16.asm.exe  ReadWriteInt16.dasm.il
    "$CORE_ROOT/ilasm" -output=ReadWriteInt16.asm.exe  ReadWriteInt16.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ReadWriteInt16
    if [ ! -f ReadWriteInt16.org ]; then
        TakeLock
        if [ ! -f ReadWriteInt16.org ]; then
          mkdir IL
          cp ReadWriteInt16.exe IL/ReadWriteInt16.exe
          mv ReadWriteInt16.exe ReadWriteInt16.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteInt16.org /out ReadWriteInt16.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteInt16.org /out ReadWriteInt16.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ReadWriteInt16.reflect.xml
  if [ -f ReadWriteInt16.reflect.xml ]
then
    ReflectionRoots="-x ReadWriteInt16.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ReadWriteInt16.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ReadWriteInt16.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ReadWriteInt16.asm.exe $CLRTestExecutionArguments
