...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteByte ReadWriteByte.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteByte ReadWriteByte.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ReadWriteByte.dasm.il ReadWriteByte.exe
    "$CORE_ROOT/ildasm" -raweh -out=ReadWriteByte.dasm.il ReadWriteByte.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ReadWriteByte.asm.exe  ReadWriteByte.dasm.il
    "$CORE_ROOT/ilasm" -output=ReadWriteByte.asm.exe  ReadWriteByte.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ReadWriteByte
    if [ ! -f ReadWriteByte.org ]; then
        TakeLock
        if [ ! -f ReadWriteByte.org ]; then
          mkdir IL
          cp ReadWriteByte.exe IL/ReadWriteByte.exe
          mv ReadWriteByte.exe ReadWriteByte.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteByte.org /out ReadWriteByte.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteByte.org /out ReadWriteByte.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ReadWriteByte.reflect.xml
  if [ -f ReadWriteByte.reflect.xml ]
then
    ReflectionRoots="-x ReadWriteByte.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ReadWriteByte.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ReadWriteByte.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ReadWriteByte.asm.exe $CLRTestExecutionArguments
