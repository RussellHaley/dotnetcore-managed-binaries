...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteByte ReadWriteByte.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteByte ReadWriteByte.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteByte.dasm.il ReadWriteByte.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteByte.dasm.il ReadWriteByte.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ReadWriteByte.asm.exe  ReadWriteByte.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ReadWriteByte.asm.exe  ReadWriteByte.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ReadWriteByte
    if not exist "ReadWriteByte.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ReadWriteByte.org" (
            mkdir IL
            copy ReadWriteByte.exe IL\ReadWriteByte.exe 
            ren ReadWriteByte.exe ReadWriteByte.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteByte.org /out ReadWriteByte.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteByte.org /out ReadWriteByte.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ReadWriteByte.reflect.xml
    IF EXIST ReadWriteByte.reflect.xml set ReflectionRoots=-x ReadWriteByte.reflect.xml
    ...
    set ExePath=%LinkBin%\ReadWriteByte.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ReadWriteByte.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ReadWriteByte.asm.exe %CLRTestExecutionArguments% %Host_Args%
