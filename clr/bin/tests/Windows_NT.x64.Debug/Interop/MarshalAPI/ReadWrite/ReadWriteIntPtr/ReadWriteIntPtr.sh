...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteIntPtr ReadWriteIntPtr.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/ReadWrite/ReadWriteIntPtr ReadWriteIntPtr.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ReadWriteIntPtr.dasm.il ReadWriteIntPtr.exe
    "$CORE_ROOT/ildasm" -raweh -out=ReadWriteIntPtr.dasm.il ReadWriteIntPtr.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ReadWriteIntPtr.asm.exe  ReadWriteIntPtr.dasm.il
    "$CORE_ROOT/ilasm" -output=ReadWriteIntPtr.asm.exe  ReadWriteIntPtr.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ReadWriteIntPtr
    if [ ! -f ReadWriteIntPtr.org ]; then
        TakeLock
        if [ ! -f ReadWriteIntPtr.org ]; then
          mkdir IL
          cp ReadWriteIntPtr.exe IL/ReadWriteIntPtr.exe
          mv ReadWriteIntPtr.exe ReadWriteIntPtr.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteIntPtr.org /out ReadWriteIntPtr.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReadWriteIntPtr.org /out ReadWriteIntPtr.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ReadWriteIntPtr.reflect.xml
  if [ -f ReadWriteIntPtr.reflect.xml ]
then
    ReflectionRoots="-x ReadWriteIntPtr.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ReadWriteIntPtr.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ReadWriteIntPtr.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ReadWriteIntPtr.asm.exe $CLRTestExecutionArguments
