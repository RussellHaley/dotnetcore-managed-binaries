...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteIntPtr ReadWriteIntPtr.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\ReadWrite\ReadWriteIntPtr ReadWriteIntPtr.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteIntPtr.dasm.il ReadWriteIntPtr.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ReadWriteIntPtr.dasm.il ReadWriteIntPtr.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ReadWriteIntPtr.asm.exe  ReadWriteIntPtr.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ReadWriteIntPtr.asm.exe  ReadWriteIntPtr.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ReadWriteIntPtr
    if not exist "ReadWriteIntPtr.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ReadWriteIntPtr.org" (
            mkdir IL
            copy ReadWriteIntPtr.exe IL\ReadWriteIntPtr.exe 
            ren ReadWriteIntPtr.exe ReadWriteIntPtr.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteIntPtr.org /out ReadWriteIntPtr.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReadWriteIntPtr.org /out ReadWriteIntPtr.exe
...
set ExePath=ReadWriteIntPtr.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ReadWriteIntPtr.reflect.xml
    IF EXIST ReadWriteIntPtr.reflect.xml set ReflectionRoots=-x ReadWriteIntPtr.reflect.xml
    ...
    set ExePath=%LinkBin%\ReadWriteIntPtr.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ReadWriteIntPtr.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ReadWriteIntPtr.asm.exe %CLRTestExecutionArguments% %Host_Args%
