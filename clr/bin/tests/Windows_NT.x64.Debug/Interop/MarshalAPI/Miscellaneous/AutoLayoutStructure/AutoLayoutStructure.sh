...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Miscellaneous/AutoLayoutStructure AutoLayoutStructure.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Miscellaneous/AutoLayoutStructure AutoLayoutStructure.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AutoLayoutStructure.dasm.il AutoLayoutStructure.exe
    "$CORE_ROOT/ildasm" -raweh -out=AutoLayoutStructure.dasm.il AutoLayoutStructure.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AutoLayoutStructure.asm.exe  AutoLayoutStructure.dasm.il
    "$CORE_ROOT/ilasm" -output=AutoLayoutStructure.asm.exe  AutoLayoutStructure.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AutoLayoutStructure
    if [ ! -f AutoLayoutStructure.org ]; then
        TakeLock
        if [ ! -f AutoLayoutStructure.org ]; then
          mkdir IL
          cp AutoLayoutStructure.exe IL/AutoLayoutStructure.exe
          mv AutoLayoutStructure.exe AutoLayoutStructure.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AutoLayoutStructure.org /out AutoLayoutStructure.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AutoLayoutStructure.org /out AutoLayoutStructure.exe
          __cgExitCode=$?
...
ExePath=AutoLayoutStructure.exe
...
then
  ...
  # Use hints for reflection roots, if provided in AutoLayoutStructure.reflect.xml
  if [ -f AutoLayoutStructure.reflect.xml ]
then
    ReflectionRoots="-x AutoLayoutStructure.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AutoLayoutStructure.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AutoLayoutStructure.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AutoLayoutStructure.asm.exe $CLRTestExecutionArguments
