...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Miscellaneous\AutoLayoutStructure AutoLayoutStructure.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Miscellaneous\AutoLayoutStructure AutoLayoutStructure.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AutoLayoutStructure.dasm.il AutoLayoutStructure.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AutoLayoutStructure.dasm.il AutoLayoutStructure.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AutoLayoutStructure.asm.exe  AutoLayoutStructure.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AutoLayoutStructure.asm.exe  AutoLayoutStructure.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AutoLayoutStructure
    if not exist "AutoLayoutStructure.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AutoLayoutStructure.org" (
            mkdir IL
            copy AutoLayoutStructure.exe IL\AutoLayoutStructure.exe 
            ren AutoLayoutStructure.exe AutoLayoutStructure.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AutoLayoutStructure.org /out AutoLayoutStructure.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AutoLayoutStructure.org /out AutoLayoutStructure.exe
...
set ExePath=AutoLayoutStructure.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AutoLayoutStructure.reflect.xml
    IF EXIST AutoLayoutStructure.reflect.xml set ReflectionRoots=-x AutoLayoutStructure.reflect.xml
    ...
    set ExePath=%LinkBin%\AutoLayoutStructure.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AutoLayoutStructure.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AutoLayoutStructure.asm.exe %CLRTestExecutionArguments% %Host_Args%
