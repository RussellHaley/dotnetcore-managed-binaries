...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Miscellaneous/MarshalClassTests MarshalClassTests.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/Miscellaneous/MarshalClassTests MarshalClassTests.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MarshalClassTests.dasm.il MarshalClassTests.exe
    "$CORE_ROOT/ildasm" -raweh -out=MarshalClassTests.dasm.il MarshalClassTests.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MarshalClassTests.asm.exe  MarshalClassTests.dasm.il
    "$CORE_ROOT/ilasm" -output=MarshalClassTests.asm.exe  MarshalClassTests.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MarshalClassTests
    if [ ! -f MarshalClassTests.org ]; then
        TakeLock
        if [ ! -f MarshalClassTests.org ]; then
          mkdir IL
          cp MarshalClassTests.exe IL/MarshalClassTests.exe
          mv MarshalClassTests.exe MarshalClassTests.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MarshalClassTests.org /out MarshalClassTests.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MarshalClassTests.org /out MarshalClassTests.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in MarshalClassTests.reflect.xml
  if [ -f MarshalClassTests.reflect.xml ]
then
    ReflectionRoots="-x MarshalClassTests.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/MarshalClassTests.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MarshalClassTests.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MarshalClassTests.asm.exe $CLRTestExecutionArguments
