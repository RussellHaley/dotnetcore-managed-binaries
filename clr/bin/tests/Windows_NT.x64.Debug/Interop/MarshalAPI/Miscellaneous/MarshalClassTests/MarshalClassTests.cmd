...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Miscellaneous\MarshalClassTests MarshalClassTests.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\Miscellaneous\MarshalClassTests MarshalClassTests.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MarshalClassTests.dasm.il MarshalClassTests.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MarshalClassTests.dasm.il MarshalClassTests.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MarshalClassTests.asm.exe  MarshalClassTests.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MarshalClassTests.asm.exe  MarshalClassTests.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MarshalClassTests
    if not exist "MarshalClassTests.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MarshalClassTests.org" (
            mkdir IL
            copy MarshalClassTests.exe IL\MarshalClassTests.exe 
            ren MarshalClassTests.exe MarshalClassTests.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MarshalClassTests.org /out MarshalClassTests.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MarshalClassTests.org /out MarshalClassTests.exe
...
set ExePath=MarshalClassTests.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MarshalClassTests.reflect.xml
    IF EXIST MarshalClassTests.reflect.xml set ReflectionRoots=-x MarshalClassTests.reflect.xml
    ...
    set ExePath=%LinkBin%\MarshalClassTests.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MarshalClassTests.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MarshalClassTests.asm.exe %CLRTestExecutionArguments% %Host_Args%
