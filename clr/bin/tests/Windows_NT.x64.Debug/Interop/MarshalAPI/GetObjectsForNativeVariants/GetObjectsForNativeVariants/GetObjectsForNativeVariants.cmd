...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\GetObjectsForNativeVariants\GetObjectsForNativeVariants GetObjectsForNativeVariants.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\GetObjectsForNativeVariants\GetObjectsForNativeVariants GetObjectsForNativeVariants.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GetObjectsForNativeVariants.dasm.il GetObjectsForNativeVariants.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GetObjectsForNativeVariants.dasm.il GetObjectsForNativeVariants.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GetObjectsForNativeVariants.asm.exe  GetObjectsForNativeVariants.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GetObjectsForNativeVariants.asm.exe  GetObjectsForNativeVariants.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GetObjectsForNativeVariants
    if not exist "GetObjectsForNativeVariants.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GetObjectsForNativeVariants.org" (
            mkdir IL
            copy GetObjectsForNativeVariants.exe IL\GetObjectsForNativeVariants.exe 
            ren GetObjectsForNativeVariants.exe GetObjectsForNativeVariants.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetObjectsForNativeVariants.org /out GetObjectsForNativeVariants.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetObjectsForNativeVariants.org /out GetObjectsForNativeVariants.exe
...
set ExePath=GetObjectsForNativeVariants.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GetObjectsForNativeVariants.reflect.xml
    IF EXIST GetObjectsForNativeVariants.reflect.xml set ReflectionRoots=-x GetObjectsForNativeVariants.reflect.xml
    ...
    set ExePath=%LinkBin%\GetObjectsForNativeVariants.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GetObjectsForNativeVariants.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GetObjectsForNativeVariants.asm.exe %CLRTestExecutionArguments% %Host_Args%
