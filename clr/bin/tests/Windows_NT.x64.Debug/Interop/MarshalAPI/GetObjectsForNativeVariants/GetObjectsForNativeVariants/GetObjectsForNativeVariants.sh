...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/GetObjectsForNativeVariants/GetObjectsForNativeVariants GetObjectsForNativeVariants.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/GetObjectsForNativeVariants/GetObjectsForNativeVariants GetObjectsForNativeVariants.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GetObjectsForNativeVariants.dasm.il GetObjectsForNativeVariants.exe
    "$CORE_ROOT/ildasm" -raweh -out=GetObjectsForNativeVariants.dasm.il GetObjectsForNativeVariants.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GetObjectsForNativeVariants.asm.exe  GetObjectsForNativeVariants.dasm.il
    "$CORE_ROOT/ilasm" -output=GetObjectsForNativeVariants.asm.exe  GetObjectsForNativeVariants.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GetObjectsForNativeVariants
    if [ ! -f GetObjectsForNativeVariants.org ]; then
        TakeLock
        if [ ! -f GetObjectsForNativeVariants.org ]; then
          mkdir IL
          cp GetObjectsForNativeVariants.exe IL/GetObjectsForNativeVariants.exe
          mv GetObjectsForNativeVariants.exe GetObjectsForNativeVariants.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetObjectsForNativeVariants.org /out GetObjectsForNativeVariants.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetObjectsForNativeVariants.org /out GetObjectsForNativeVariants.exe
          __cgExitCode=$?
...
ExePath=GetObjectsForNativeVariants.exe
...
then
  ...
  # Use hints for reflection roots, if provided in GetObjectsForNativeVariants.reflect.xml
  if [ -f GetObjectsForNativeVariants.reflect.xml ]
then
    ReflectionRoots="-x GetObjectsForNativeVariants.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GetObjectsForNativeVariants.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GetObjectsForNativeVariants.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GetObjectsForNativeVariants.asm.exe $CLRTestExecutionArguments
