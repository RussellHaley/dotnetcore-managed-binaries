...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/GetNativeVariantForObject/GetNativeVariantForObject GetNativeVariantForObject.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/GetNativeVariantForObject/GetNativeVariantForObject GetNativeVariantForObject.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GetNativeVariantForObject.dasm.il GetNativeVariantForObject.exe
    "$CORE_ROOT/ildasm" -raweh -out=GetNativeVariantForObject.dasm.il GetNativeVariantForObject.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GetNativeVariantForObject.asm.exe  GetNativeVariantForObject.dasm.il
    "$CORE_ROOT/ilasm" -output=GetNativeVariantForObject.asm.exe  GetNativeVariantForObject.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GetNativeVariantForObject
    if [ ! -f GetNativeVariantForObject.org ]; then
        TakeLock
        if [ ! -f GetNativeVariantForObject.org ]; then
          mkdir IL
          cp GetNativeVariantForObject.exe IL/GetNativeVariantForObject.exe
          mv GetNativeVariantForObject.exe GetNativeVariantForObject.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetNativeVariantForObject.org /out GetNativeVariantForObject.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetNativeVariantForObject.org /out GetNativeVariantForObject.exe
          __cgExitCode=$?
...
ExePath=GetNativeVariantForObject.exe
...
then
  ...
  # Use hints for reflection roots, if provided in GetNativeVariantForObject.reflect.xml
  if [ -f GetNativeVariantForObject.reflect.xml ]
then
    ReflectionRoots="-x GetNativeVariantForObject.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GetNativeVariantForObject.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GetNativeVariantForObject.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GetNativeVariantForObject.asm.exe $CLRTestExecutionArguments
