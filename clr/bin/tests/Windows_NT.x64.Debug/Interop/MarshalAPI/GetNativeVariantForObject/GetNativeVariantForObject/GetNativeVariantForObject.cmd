...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\GetNativeVariantForObject\GetNativeVariantForObject GetNativeVariantForObject.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\GetNativeVariantForObject\GetNativeVariantForObject GetNativeVariantForObject.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GetNativeVariantForObject.dasm.il GetNativeVariantForObject.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GetNativeVariantForObject.dasm.il GetNativeVariantForObject.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GetNativeVariantForObject.asm.exe  GetNativeVariantForObject.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GetNativeVariantForObject.asm.exe  GetNativeVariantForObject.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GetNativeVariantForObject
    if not exist "GetNativeVariantForObject.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GetNativeVariantForObject.org" (
            mkdir IL
            copy GetNativeVariantForObject.exe IL\GetNativeVariantForObject.exe 
            ren GetNativeVariantForObject.exe GetNativeVariantForObject.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetNativeVariantForObject.org /out GetNativeVariantForObject.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetNativeVariantForObject.org /out GetNativeVariantForObject.exe
...
set ExePath=GetNativeVariantForObject.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GetNativeVariantForObject.reflect.xml
    IF EXIST GetNativeVariantForObject.reflect.xml set ReflectionRoots=-x GetNativeVariantForObject.reflect.xml
    ...
    set ExePath=%LinkBin%\GetNativeVariantForObject.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GetNativeVariantForObject.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GetNativeVariantForObject.asm.exe %CLRTestExecutionArguments% %Host_Args%
