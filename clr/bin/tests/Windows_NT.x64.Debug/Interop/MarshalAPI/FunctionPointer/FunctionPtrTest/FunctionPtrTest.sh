...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/FunctionPointer/FunctionPtrTest FunctionPtrTest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/FunctionPointer/FunctionPtrTest FunctionPtrTest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=FunctionPtrTest.dasm.il FunctionPtrTest.exe
    "$CORE_ROOT/ildasm" -raweh -out=FunctionPtrTest.dasm.il FunctionPtrTest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=FunctionPtrTest.asm.exe  FunctionPtrTest.dasm.il
    "$CORE_ROOT/ilasm" -output=FunctionPtrTest.asm.exe  FunctionPtrTest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=FunctionPtrTest
    if [ ! -f FunctionPtrTest.org ]; then
        TakeLock
        if [ ! -f FunctionPtrTest.org ]; then
          mkdir IL
          cp FunctionPtrTest.exe IL/FunctionPtrTest.exe
          mv FunctionPtrTest.exe FunctionPtrTest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FunctionPtrTest.org /out FunctionPtrTest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FunctionPtrTest.org /out FunctionPtrTest.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in FunctionPtrTest.reflect.xml
  if [ -f FunctionPtrTest.reflect.xml ]
then
    ReflectionRoots="-x FunctionPtrTest.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/FunctionPtrTest.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" FunctionPtrTest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" FunctionPtrTest.asm.exe $CLRTestExecutionArguments
