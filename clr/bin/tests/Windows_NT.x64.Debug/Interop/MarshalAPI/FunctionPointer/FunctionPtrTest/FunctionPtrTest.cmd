...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\FunctionPointer\FunctionPtrTest FunctionPtrTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\FunctionPointer\FunctionPtrTest FunctionPtrTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=FunctionPtrTest.dasm.il FunctionPtrTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=FunctionPtrTest.dasm.il FunctionPtrTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=FunctionPtrTest.asm.exe  FunctionPtrTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=FunctionPtrTest.asm.exe  FunctionPtrTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=FunctionPtrTest
    if not exist "FunctionPtrTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "FunctionPtrTest.org" (
            mkdir IL
            copy FunctionPtrTest.exe IL\FunctionPtrTest.exe 
            ren FunctionPtrTest.exe FunctionPtrTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FunctionPtrTest.org /out FunctionPtrTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FunctionPtrTest.org /out FunctionPtrTest.exe
...
set ExePath=FunctionPtrTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in FunctionPtrTest.reflect.xml
    IF EXIST FunctionPtrTest.reflect.xml set ReflectionRoots=-x FunctionPtrTest.reflect.xml
    ...
    set ExePath=%LinkBin%\FunctionPtrTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% FunctionPtrTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% FunctionPtrTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
