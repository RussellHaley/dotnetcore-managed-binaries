...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\GetExceptionForHR\GetExceptionForHR GetExceptionForHR.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\MarshalAPI\GetExceptionForHR\GetExceptionForHR GetExceptionForHR.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GetExceptionForHR.dasm.il GetExceptionForHR.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GetExceptionForHR.dasm.il GetExceptionForHR.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GetExceptionForHR.asm.exe  GetExceptionForHR.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GetExceptionForHR.asm.exe  GetExceptionForHR.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GetExceptionForHR
    if not exist "GetExceptionForHR.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GetExceptionForHR.org" (
            mkdir IL
            copy GetExceptionForHR.exe IL\GetExceptionForHR.exe 
            ren GetExceptionForHR.exe GetExceptionForHR.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetExceptionForHR.org /out GetExceptionForHR.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetExceptionForHR.org /out GetExceptionForHR.exe
...
set ExePath=GetExceptionForHR.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GetExceptionForHR.reflect.xml
    IF EXIST GetExceptionForHR.reflect.xml set ReflectionRoots=-x GetExceptionForHR.reflect.xml
    ...
    set ExePath=%LinkBin%\GetExceptionForHR.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GetExceptionForHR.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GetExceptionForHR.asm.exe %CLRTestExecutionArguments% %Host_Args%
