...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/GetExceptionForHR/GetExceptionForHR GetExceptionForHR.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/MarshalAPI/GetExceptionForHR/GetExceptionForHR GetExceptionForHR.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GetExceptionForHR.dasm.il GetExceptionForHR.exe
    "$CORE_ROOT/ildasm" -raweh -out=GetExceptionForHR.dasm.il GetExceptionForHR.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GetExceptionForHR.asm.exe  GetExceptionForHR.dasm.il
    "$CORE_ROOT/ilasm" -output=GetExceptionForHR.asm.exe  GetExceptionForHR.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GetExceptionForHR
    if [ ! -f GetExceptionForHR.org ]; then
        TakeLock
        if [ ! -f GetExceptionForHR.org ]; then
          mkdir IL
          cp GetExceptionForHR.exe IL/GetExceptionForHR.exe
          mv GetExceptionForHR.exe GetExceptionForHR.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetExceptionForHR.org /out GetExceptionForHR.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetExceptionForHR.org /out GetExceptionForHR.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GetExceptionForHR.reflect.xml
  if [ -f GetExceptionForHR.reflect.xml ]
then
    ReflectionRoots="-x GetExceptionForHR.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GetExceptionForHR.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GetExceptionForHR.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GetExceptionForHR.asm.exe $CLRTestExecutionArguments
