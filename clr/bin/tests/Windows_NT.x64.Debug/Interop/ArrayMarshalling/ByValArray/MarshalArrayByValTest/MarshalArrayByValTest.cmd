...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\ArrayMarshalling\ByValArray\MarshalArrayByValTest MarshalArrayByValTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Interop\ArrayMarshalling\ByValArray\MarshalArrayByValTest MarshalArrayByValTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MarshalArrayByValTest.dasm.il MarshalArrayByValTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MarshalArrayByValTest.dasm.il MarshalArrayByValTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MarshalArrayByValTest.asm.exe  MarshalArrayByValTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MarshalArrayByValTest.asm.exe  MarshalArrayByValTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MarshalArrayByValTest
    if not exist "MarshalArrayByValTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MarshalArrayByValTest.org" (
            mkdir IL
            copy MarshalArrayByValTest.exe IL\MarshalArrayByValTest.exe 
            ren MarshalArrayByValTest.exe MarshalArrayByValTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MarshalArrayByValTest.org /out MarshalArrayByValTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MarshalArrayByValTest.org /out MarshalArrayByValTest.exe
...
set ExePath=MarshalArrayByValTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MarshalArrayByValTest.reflect.xml
    IF EXIST MarshalArrayByValTest.reflect.xml set ReflectionRoots=-x MarshalArrayByValTest.reflect.xml
    ...
    set ExePath=%LinkBin%\MarshalArrayByValTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MarshalArrayByValTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MarshalArrayByValTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
