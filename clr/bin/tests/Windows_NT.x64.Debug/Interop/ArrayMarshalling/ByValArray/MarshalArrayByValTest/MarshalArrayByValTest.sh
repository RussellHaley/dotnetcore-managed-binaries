...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/ArrayMarshalling/ByValArray/MarshalArrayByValTest MarshalArrayByValTest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Interop/ArrayMarshalling/ByValArray/MarshalArrayByValTest MarshalArrayByValTest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MarshalArrayByValTest.dasm.il MarshalArrayByValTest.exe
    "$CORE_ROOT/ildasm" -raweh -out=MarshalArrayByValTest.dasm.il MarshalArrayByValTest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MarshalArrayByValTest.asm.exe  MarshalArrayByValTest.dasm.il
    "$CORE_ROOT/ilasm" -output=MarshalArrayByValTest.asm.exe  MarshalArrayByValTest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MarshalArrayByValTest
    if [ ! -f MarshalArrayByValTest.org ]; then
        TakeLock
        if [ ! -f MarshalArrayByValTest.org ]; then
          mkdir IL
          cp MarshalArrayByValTest.exe IL/MarshalArrayByValTest.exe
          mv MarshalArrayByValTest.exe MarshalArrayByValTest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MarshalArrayByValTest.org /out MarshalArrayByValTest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MarshalArrayByValTest.org /out MarshalArrayByValTest.exe
          __cgExitCode=$?
...
ExePath=MarshalArrayByValTest.exe
...
then
  ...
  # Use hints for reflection roots, if provided in MarshalArrayByValTest.reflect.xml
  if [ -f MarshalArrayByValTest.reflect.xml ]
then
    ReflectionRoots="-x MarshalArrayByValTest.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/MarshalArrayByValTest.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MarshalArrayByValTest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MarshalArrayByValTest.asm.exe $CLRTestExecutionArguments
