...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/baseservices/varargs/varargsupport varargsupport.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/baseservices/varargs/varargsupport varargsupport.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=varargsupport.dasm.il varargsupport.exe
    "$CORE_ROOT/ildasm" -raweh -out=varargsupport.dasm.il varargsupport.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=varargsupport.asm.exe -QUIET -NOLOGO -DEBUG varargsupport.dasm.il
    "$CORE_ROOT/ilasm" -output=varargsupport.asm.exe -QUIET -NOLOGO -DEBUG varargsupport.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=varargsupport
    if [ ! -f varargsupport.org ]; then
        TakeLock
        if [ ! -f varargsupport.org ]; then
          mkdir IL
          cp varargsupport.exe IL/varargsupport.exe
          mv varargsupport.exe varargsupport.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in varargsupport.org /out varargsupport.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in varargsupport.org /out varargsupport.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in varargsupport.reflect.xml
  if [ -f varargsupport.reflect.xml ]
then
    ReflectionRoots="-x varargsupport.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/varargsupport.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" varargsupport.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" varargsupport.asm.exe $CLRTestExecutionArguments
