...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\baseservices\varargs\varargsupport varargsupport.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\baseservices\varargs\varargsupport varargsupport.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=varargsupport.dasm.il varargsupport.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=varargsupport.dasm.il varargsupport.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=varargsupport.asm.exe -QUIET -NOLOGO -DEBUG varargsupport.dasm.il
    %CORE_ROOT%\ilasm.exe /output=varargsupport.asm.exe -QUIET -NOLOGO -DEBUG varargsupport.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=varargsupport
    if not exist "varargsupport.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "varargsupport.org" (
            mkdir IL
            copy varargsupport.exe IL\varargsupport.exe 
            ren varargsupport.exe varargsupport.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in varargsupport.org /out varargsupport.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in varargsupport.org /out varargsupport.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in varargsupport.reflect.xml
    IF EXIST varargsupport.reflect.xml set ReflectionRoots=-x varargsupport.reflect.xml
    ...
    set ExePath=%LinkBin%\varargsupport.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% varargsupport.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% varargsupport.asm.exe %CLRTestExecutionArguments% %Host_Args%
