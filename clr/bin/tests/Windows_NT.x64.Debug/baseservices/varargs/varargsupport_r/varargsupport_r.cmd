...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\baseservices\varargs\varargsupport_r varargsupport_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\baseservices\varargs\varargsupport_r varargsupport_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=varargsupport_r.dasm.il varargsupport_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=varargsupport_r.dasm.il varargsupport_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=varargsupport_r.asm.exe -QUIET -NOLOGO -DEBUG varargsupport_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=varargsupport_r.asm.exe -QUIET -NOLOGO -DEBUG varargsupport_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=varargsupport_r
    if not exist "varargsupport_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "varargsupport_r.org" (
            mkdir IL
            copy varargsupport_r.exe IL\varargsupport_r.exe 
            ren varargsupport_r.exe varargsupport_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in varargsupport_r.org /out varargsupport_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in varargsupport_r.org /out varargsupport_r.exe
...
set ExePath=varargsupport_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in varargsupport_r.reflect.xml
    IF EXIST varargsupport_r.reflect.xml set ReflectionRoots=-x varargsupport_r.reflect.xml
    ...
    set ExePath=%LinkBin%\varargsupport_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% varargsupport_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% varargsupport_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
