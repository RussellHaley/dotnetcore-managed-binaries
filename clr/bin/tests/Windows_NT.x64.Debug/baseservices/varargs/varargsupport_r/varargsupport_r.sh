...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/baseservices/varargs/varargsupport_r varargsupport_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/baseservices/varargs/varargsupport_r varargsupport_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=varargsupport_r.dasm.il varargsupport_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=varargsupport_r.dasm.il varargsupport_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=varargsupport_r.asm.exe -QUIET -NOLOGO -DEBUG varargsupport_r.dasm.il
    "$CORE_ROOT/ilasm" -output=varargsupport_r.asm.exe -QUIET -NOLOGO -DEBUG varargsupport_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=varargsupport_r
    if [ ! -f varargsupport_r.org ]; then
        TakeLock
        if [ ! -f varargsupport_r.org ]; then
          mkdir IL
          cp varargsupport_r.exe IL/varargsupport_r.exe
          mv varargsupport_r.exe varargsupport_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in varargsupport_r.org /out varargsupport_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in varargsupport_r.org /out varargsupport_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in varargsupport_r.reflect.xml
  if [ -f varargsupport_r.reflect.xml ]
then
    ReflectionRoots="-x varargsupport_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/varargsupport_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" varargsupport_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" varargsupport_r.asm.exe $CLRTestExecutionArguments
