...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\baseservices\compilerservices\RuntimeWrappedException\RuntimeWrappedException RuntimeWrappedException.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\baseservices\compilerservices\RuntimeWrappedException\RuntimeWrappedException RuntimeWrappedException.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=RuntimeWrappedException.dasm.il RuntimeWrappedException.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=RuntimeWrappedException.dasm.il RuntimeWrappedException.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=RuntimeWrappedException.asm.exe  RuntimeWrappedException.dasm.il
    %CORE_ROOT%\ilasm.exe /output=RuntimeWrappedException.asm.exe  RuntimeWrappedException.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=RuntimeWrappedException
    if not exist "RuntimeWrappedException.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "RuntimeWrappedException.org" (
            mkdir IL
            copy RuntimeWrappedException.exe IL\RuntimeWrappedException.exe 
            ren RuntimeWrappedException.exe RuntimeWrappedException.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RuntimeWrappedException.org /out RuntimeWrappedException.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RuntimeWrappedException.org /out RuntimeWrappedException.exe
...
set ExePath=RuntimeWrappedException.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in RuntimeWrappedException.reflect.xml
    IF EXIST RuntimeWrappedException.reflect.xml set ReflectionRoots=-x RuntimeWrappedException.reflect.xml
    ...
    set ExePath=%LinkBin%\RuntimeWrappedException.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% RuntimeWrappedException.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% RuntimeWrappedException.asm.exe %CLRTestExecutionArguments% %Host_Args%
