...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/baseservices/compilerservices/RuntimeWrappedException/RuntimeWrappedException RuntimeWrappedException.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/baseservices/compilerservices/RuntimeWrappedException/RuntimeWrappedException RuntimeWrappedException.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=RuntimeWrappedException.dasm.il RuntimeWrappedException.exe
    "$CORE_ROOT/ildasm" -raweh -out=RuntimeWrappedException.dasm.il RuntimeWrappedException.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=RuntimeWrappedException.asm.exe  RuntimeWrappedException.dasm.il
    "$CORE_ROOT/ilasm" -output=RuntimeWrappedException.asm.exe  RuntimeWrappedException.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=RuntimeWrappedException
    if [ ! -f RuntimeWrappedException.org ]; then
        TakeLock
        if [ ! -f RuntimeWrappedException.org ]; then
          mkdir IL
          cp RuntimeWrappedException.exe IL/RuntimeWrappedException.exe
          mv RuntimeWrappedException.exe RuntimeWrappedException.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RuntimeWrappedException.org /out RuntimeWrappedException.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RuntimeWrappedException.org /out RuntimeWrappedException.exe
          __cgExitCode=$?
...
ExePath=RuntimeWrappedException.exe
...
then
  ...
  # Use hints for reflection roots, if provided in RuntimeWrappedException.reflect.xml
  if [ -f RuntimeWrappedException.reflect.xml ]
then
    ReflectionRoots="-x RuntimeWrappedException.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/RuntimeWrappedException.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" RuntimeWrappedException.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" RuntimeWrappedException.asm.exe $CLRTestExecutionArguments
