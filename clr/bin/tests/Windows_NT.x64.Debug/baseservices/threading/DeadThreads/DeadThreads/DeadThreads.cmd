...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\baseservices\threading\DeadThreads\DeadThreads DeadThreads.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\baseservices\threading\DeadThreads\DeadThreads DeadThreads.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DeadThreads.dasm.il DeadThreads.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DeadThreads.dasm.il DeadThreads.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DeadThreads.asm.exe  DeadThreads.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DeadThreads.asm.exe  DeadThreads.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DeadThreads
    if not exist "DeadThreads.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DeadThreads.org" (
            mkdir IL
            copy DeadThreads.exe IL\DeadThreads.exe 
            ren DeadThreads.exe DeadThreads.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DeadThreads.org /out DeadThreads.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DeadThreads.org /out DeadThreads.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DeadThreads.reflect.xml
    IF EXIST DeadThreads.reflect.xml set ReflectionRoots=-x DeadThreads.reflect.xml
    ...
    set ExePath=%LinkBin%\DeadThreads.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DeadThreads.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DeadThreads.asm.exe %CLRTestExecutionArguments% %Host_Args%
