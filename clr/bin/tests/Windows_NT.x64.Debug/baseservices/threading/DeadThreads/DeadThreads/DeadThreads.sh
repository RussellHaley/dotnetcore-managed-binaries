...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/baseservices/threading/DeadThreads/DeadThreads DeadThreads.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/baseservices/threading/DeadThreads/DeadThreads DeadThreads.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DeadThreads.dasm.il DeadThreads.exe
    "$CORE_ROOT/ildasm" -raweh -out=DeadThreads.dasm.il DeadThreads.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DeadThreads.asm.exe  DeadThreads.dasm.il
    "$CORE_ROOT/ilasm" -output=DeadThreads.asm.exe  DeadThreads.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DeadThreads
    if [ ! -f DeadThreads.org ]; then
        TakeLock
        if [ ! -f DeadThreads.org ]; then
          mkdir IL
          cp DeadThreads.exe IL/DeadThreads.exe
          mv DeadThreads.exe DeadThreads.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DeadThreads.org /out DeadThreads.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DeadThreads.org /out DeadThreads.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DeadThreads.reflect.xml
  if [ -f DeadThreads.reflect.xml ]
then
    ReflectionRoots="-x DeadThreads.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DeadThreads.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DeadThreads.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DeadThreads.asm.exe $CLRTestExecutionArguments
