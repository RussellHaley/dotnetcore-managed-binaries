...
if not defined CLRTestExecutionArguments (set CLRTestExecutionArguments=false )
...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\baseservices\threading\paramthreadstart\ThreadStartBool ThreadStartBool.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\baseservices\threading\paramthreadstart\ThreadStartBool ThreadStartBool.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ThreadStartBool.dasm.il ThreadStartBool.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ThreadStartBool.dasm.il ThreadStartBool.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ThreadStartBool.asm.exe  ThreadStartBool.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ThreadStartBool.asm.exe  ThreadStartBool.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ThreadStartBool
    if not exist "ThreadStartBool.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ThreadStartBool.org" (
            mkdir IL
            copy ThreadStartBool.exe IL\ThreadStartBool.exe 
            ren ThreadStartBool.exe ThreadStartBool.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ThreadStartBool.org /out ThreadStartBool.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ThreadStartBool.org /out ThreadStartBool.exe
...
set ExePath=ThreadStartBool.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ThreadStartBool.reflect.xml
    IF EXIST ThreadStartBool.reflect.xml set ReflectionRoots=-x ThreadStartBool.reflect.xml
    ...
    set ExePath=%LinkBin%\ThreadStartBool.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ThreadStartBool.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ThreadStartBool.asm.exe %CLRTestExecutionArguments% %Host_Args%
