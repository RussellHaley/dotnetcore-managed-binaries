...
if [ -z ${CLRTestExecutionArguments+x} ]; then export CLRTestExecutionArguments='false'; fi
...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/baseservices/threading/paramthreadstart/ThreadStartBool ThreadStartBool.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/baseservices/threading/paramthreadstart/ThreadStartBool ThreadStartBool.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ThreadStartBool.dasm.il ThreadStartBool.exe
    "$CORE_ROOT/ildasm" -raweh -out=ThreadStartBool.dasm.il ThreadStartBool.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ThreadStartBool.asm.exe  ThreadStartBool.dasm.il
    "$CORE_ROOT/ilasm" -output=ThreadStartBool.asm.exe  ThreadStartBool.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ThreadStartBool
    if [ ! -f ThreadStartBool.org ]; then
        TakeLock
        if [ ! -f ThreadStartBool.org ]; then
          mkdir IL
          cp ThreadStartBool.exe IL/ThreadStartBool.exe
          mv ThreadStartBool.exe ThreadStartBool.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ThreadStartBool.org /out ThreadStartBool.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ThreadStartBool.org /out ThreadStartBool.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ThreadStartBool.reflect.xml
  if [ -f ThreadStartBool.reflect.xml ]
then
    ReflectionRoots="-x ThreadStartBool.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ThreadStartBool.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ThreadStartBool.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ThreadStartBool.asm.exe $CLRTestExecutionArguments
