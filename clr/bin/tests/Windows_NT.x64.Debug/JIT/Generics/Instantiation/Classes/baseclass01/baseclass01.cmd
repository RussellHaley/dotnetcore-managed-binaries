...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Classes\baseclass01 baseclass01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Classes\baseclass01 baseclass01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=baseclass01.dasm.il baseclass01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=baseclass01.dasm.il baseclass01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=baseclass01.asm.exe  baseclass01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=baseclass01.asm.exe  baseclass01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=baseclass01
    if not exist "baseclass01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "baseclass01.org" (
            mkdir IL
            copy baseclass01.exe IL\baseclass01.exe 
            ren baseclass01.exe baseclass01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in baseclass01.org /out baseclass01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in baseclass01.org /out baseclass01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in baseclass01.reflect.xml
    IF EXIST baseclass01.reflect.xml set ReflectionRoots=-x baseclass01.reflect.xml
    ...
    set ExePath=%LinkBin%\baseclass01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% baseclass01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% baseclass01.asm.exe %CLRTestExecutionArguments% %Host_Args%
