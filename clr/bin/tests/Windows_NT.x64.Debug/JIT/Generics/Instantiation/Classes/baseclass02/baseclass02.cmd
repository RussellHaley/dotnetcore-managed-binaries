...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Classes\baseclass02 baseclass02.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Classes\baseclass02 baseclass02.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=baseclass02.dasm.il baseclass02.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=baseclass02.dasm.il baseclass02.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=baseclass02.asm.exe  baseclass02.dasm.il
    %CORE_ROOT%\ilasm.exe /output=baseclass02.asm.exe  baseclass02.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=baseclass02
    if not exist "baseclass02.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "baseclass02.org" (
            mkdir IL
            copy baseclass02.exe IL\baseclass02.exe 
            ren baseclass02.exe baseclass02.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in baseclass02.org /out baseclass02.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in baseclass02.org /out baseclass02.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in baseclass02.reflect.xml
    IF EXIST baseclass02.reflect.xml set ReflectionRoots=-x baseclass02.reflect.xml
    ...
    set ExePath=%LinkBin%\baseclass02.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% baseclass02.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% baseclass02.asm.exe %CLRTestExecutionArguments% %Host_Args%
