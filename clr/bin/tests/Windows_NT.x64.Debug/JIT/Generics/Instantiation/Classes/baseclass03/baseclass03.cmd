...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Classes\baseclass03 baseclass03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Classes\baseclass03 baseclass03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=baseclass03.dasm.il baseclass03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=baseclass03.dasm.il baseclass03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=baseclass03.asm.exe  baseclass03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=baseclass03.asm.exe  baseclass03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=baseclass03
    if not exist "baseclass03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "baseclass03.org" (
            mkdir IL
            copy baseclass03.exe IL\baseclass03.exe 
            ren baseclass03.exe baseclass03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in baseclass03.org /out baseclass03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in baseclass03.org /out baseclass03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in baseclass03.reflect.xml
    IF EXIST baseclass03.reflect.xml set ReflectionRoots=-x baseclass03.reflect.xml
    ...
    set ExePath=%LinkBin%\baseclass03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% baseclass03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% baseclass03.asm.exe %CLRTestExecutionArguments% %Host_Args%
