...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Classes/baseclass01 baseclass01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Classes/baseclass01 baseclass01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=baseclass01.dasm.il baseclass01.exe
    "$CORE_ROOT/ildasm" -raweh -out=baseclass01.dasm.il baseclass01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=baseclass01.asm.exe  baseclass01.dasm.il
    "$CORE_ROOT/ilasm" -output=baseclass01.asm.exe  baseclass01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=baseclass01
    if [ ! -f baseclass01.org ]; then
        TakeLock
        if [ ! -f baseclass01.org ]; then
          mkdir IL
          cp baseclass01.exe IL/baseclass01.exe
          mv baseclass01.exe baseclass01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in baseclass01.org /out baseclass01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in baseclass01.org /out baseclass01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in baseclass01.reflect.xml
  if [ -f baseclass01.reflect.xml ]
then
    ReflectionRoots="-x baseclass01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/baseclass01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" baseclass01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" baseclass01.asm.exe $CLRTestExecutionArguments
