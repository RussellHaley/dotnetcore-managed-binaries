...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Classes/baseclass05 baseclass05.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Classes/baseclass05 baseclass05.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=baseclass05.dasm.il baseclass05.exe
    "$CORE_ROOT/ildasm" -raweh -out=baseclass05.dasm.il baseclass05.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=baseclass05.asm.exe  baseclass05.dasm.il
    "$CORE_ROOT/ilasm" -output=baseclass05.asm.exe  baseclass05.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=baseclass05
    if [ ! -f baseclass05.org ]; then
        TakeLock
        if [ ! -f baseclass05.org ]; then
          mkdir IL
          cp baseclass05.exe IL/baseclass05.exe
          mv baseclass05.exe baseclass05.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in baseclass05.org /out baseclass05.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in baseclass05.org /out baseclass05.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in baseclass05.reflect.xml
  if [ -f baseclass05.reflect.xml ]
then
    ReflectionRoots="-x baseclass05.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/baseclass05.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" baseclass05.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" baseclass05.asm.exe $CLRTestExecutionArguments
