...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Classes\baseclass05 baseclass05.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Classes\baseclass05 baseclass05.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=baseclass05.dasm.il baseclass05.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=baseclass05.dasm.il baseclass05.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=baseclass05.asm.exe  baseclass05.dasm.il
    %CORE_ROOT%\ilasm.exe /output=baseclass05.asm.exe  baseclass05.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=baseclass05
    if not exist "baseclass05.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "baseclass05.org" (
            mkdir IL
            copy baseclass05.exe IL\baseclass05.exe 
            ren baseclass05.exe baseclass05.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in baseclass05.org /out baseclass05.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in baseclass05.org /out baseclass05.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in baseclass05.reflect.xml
    IF EXIST baseclass05.reflect.xml set ReflectionRoots=-x baseclass05.reflect.xml
    ...
    set ExePath=%LinkBin%\baseclass05.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% baseclass05.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% baseclass05.asm.exe %CLRTestExecutionArguments% %Host_Args%
