...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Classes/baseclass02 baseclass02.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Classes/baseclass02 baseclass02.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=baseclass02.dasm.il baseclass02.exe
    "$CORE_ROOT/ildasm" -raweh -out=baseclass02.dasm.il baseclass02.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=baseclass02.asm.exe  baseclass02.dasm.il
    "$CORE_ROOT/ilasm" -output=baseclass02.asm.exe  baseclass02.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=baseclass02
    if [ ! -f baseclass02.org ]; then
        TakeLock
        if [ ! -f baseclass02.org ]; then
          mkdir IL
          cp baseclass02.exe IL/baseclass02.exe
          mv baseclass02.exe baseclass02.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in baseclass02.org /out baseclass02.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in baseclass02.org /out baseclass02.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in baseclass02.reflect.xml
  if [ -f baseclass02.reflect.xml ]
then
    ReflectionRoots="-x baseclass02.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/baseclass02.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" baseclass02.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" baseclass02.asm.exe $CLRTestExecutionArguments
