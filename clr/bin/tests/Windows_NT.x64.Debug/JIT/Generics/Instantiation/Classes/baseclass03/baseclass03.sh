...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Classes/baseclass03 baseclass03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Classes/baseclass03 baseclass03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=baseclass03.dasm.il baseclass03.exe
    "$CORE_ROOT/ildasm" -raweh -out=baseclass03.dasm.il baseclass03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=baseclass03.asm.exe  baseclass03.dasm.il
    "$CORE_ROOT/ilasm" -output=baseclass03.asm.exe  baseclass03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=baseclass03
    if [ ! -f baseclass03.org ]; then
        TakeLock
        if [ ! -f baseclass03.org ]; then
          mkdir IL
          cp baseclass03.exe IL/baseclass03.exe
          mv baseclass03.exe baseclass03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in baseclass03.org /out baseclass03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in baseclass03.org /out baseclass03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in baseclass03.reflect.xml
  if [ -f baseclass03.reflect.xml ]
then
    ReflectionRoots="-x baseclass03.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/baseclass03.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" baseclass03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" baseclass03.asm.exe $CLRTestExecutionArguments
