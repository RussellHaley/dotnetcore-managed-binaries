...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct02 struct02.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct02 struct02.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=struct02.dasm.il struct02.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=struct02.dasm.il struct02.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=struct02.asm.exe  struct02.dasm.il
    %CORE_ROOT%\ilasm.exe /output=struct02.asm.exe  struct02.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=struct02
    if not exist "struct02.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "struct02.org" (
            mkdir IL
            copy struct02.exe IL\struct02.exe 
            ren struct02.exe struct02.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct02.org /out struct02.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct02.org /out struct02.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in struct02.reflect.xml
    IF EXIST struct02.reflect.xml set ReflectionRoots=-x struct02.reflect.xml
    ...
    set ExePath=%LinkBin%\struct02.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% struct02.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% struct02.asm.exe %CLRTestExecutionArguments% %Host_Args%
