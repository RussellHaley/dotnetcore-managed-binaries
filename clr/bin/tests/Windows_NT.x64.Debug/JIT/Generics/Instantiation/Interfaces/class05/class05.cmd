...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\class05 class05.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\class05 class05.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=class05.dasm.il class05.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=class05.dasm.il class05.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=class05.asm.exe  class05.dasm.il
    %CORE_ROOT%\ilasm.exe /output=class05.asm.exe  class05.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=class05
    if not exist "class05.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "class05.org" (
            mkdir IL
            copy class05.exe IL\class05.exe 
            ren class05.exe class05.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class05.org /out class05.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class05.org /out class05.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in class05.reflect.xml
    IF EXIST class05.reflect.xml set ReflectionRoots=-x class05.reflect.xml
    ...
    set ExePath=%LinkBin%\class05.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% class05.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% class05.asm.exe %CLRTestExecutionArguments% %Host_Args%
