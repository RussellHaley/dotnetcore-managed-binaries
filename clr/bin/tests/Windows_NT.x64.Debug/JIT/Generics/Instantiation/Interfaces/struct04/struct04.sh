...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct04 struct04.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct04 struct04.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=struct04.dasm.il struct04.exe
    "$CORE_ROOT/ildasm" -raweh -out=struct04.dasm.il struct04.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=struct04.asm.exe  struct04.dasm.il
    "$CORE_ROOT/ilasm" -output=struct04.asm.exe  struct04.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=struct04
    if [ ! -f struct04.org ]; then
        TakeLock
        if [ ! -f struct04.org ]; then
          mkdir IL
          cp struct04.exe IL/struct04.exe
          mv struct04.exe struct04.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct04.org /out struct04.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct04.org /out struct04.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in struct04.reflect.xml
  if [ -f struct04.reflect.xml ]
then
    ReflectionRoots="-x struct04.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" struct04.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" struct04.asm.exe $CLRTestExecutionArguments
