...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct03 struct03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct03 struct03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=struct03.dasm.il struct03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=struct03.dasm.il struct03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=struct03.asm.exe  struct03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=struct03.asm.exe  struct03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=struct03
    if not exist "struct03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "struct03.org" (
            mkdir IL
            copy struct03.exe IL\struct03.exe 
            ren struct03.exe struct03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct03.org /out struct03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct03.org /out struct03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in struct03.reflect.xml
    IF EXIST struct03.reflect.xml set ReflectionRoots=-x struct03.reflect.xml
    ...
    set ExePath=%LinkBin%\struct03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% struct03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% struct03.asm.exe %CLRTestExecutionArguments% %Host_Args%
