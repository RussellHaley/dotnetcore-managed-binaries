...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\class03 class03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\class03 class03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=class03.dasm.il class03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=class03.dasm.il class03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=class03.asm.exe  class03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=class03.asm.exe  class03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=class03
    if not exist "class03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "class03.org" (
            mkdir IL
            copy class03.exe IL\class03.exe 
            ren class03.exe class03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class03.org /out class03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class03.org /out class03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in class03.reflect.xml
    IF EXIST class03.reflect.xml set ReflectionRoots=-x class03.reflect.xml
    ...
    set ExePath=%LinkBin%\class03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% class03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% class03.asm.exe %CLRTestExecutionArguments% %Host_Args%
