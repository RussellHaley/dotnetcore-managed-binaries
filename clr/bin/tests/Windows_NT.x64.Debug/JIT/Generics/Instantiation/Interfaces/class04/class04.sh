...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/class04 class04.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/class04 class04.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=class04.dasm.il class04.exe
    "$CORE_ROOT/ildasm" -raweh -out=class04.dasm.il class04.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=class04.asm.exe  class04.dasm.il
    "$CORE_ROOT/ilasm" -output=class04.asm.exe  class04.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=class04
    if [ ! -f class04.org ]; then
        TakeLock
        if [ ! -f class04.org ]; then
          mkdir IL
          cp class04.exe IL/class04.exe
          mv class04.exe class04.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class04.org /out class04.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class04.org /out class04.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in class04.reflect.xml
  if [ -f class04.reflect.xml ]
then
    ReflectionRoots="-x class04.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" class04.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" class04.asm.exe $CLRTestExecutionArguments
