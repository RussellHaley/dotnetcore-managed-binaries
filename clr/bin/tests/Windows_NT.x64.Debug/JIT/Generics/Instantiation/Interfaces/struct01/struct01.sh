...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct01 struct01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct01 struct01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=struct01.dasm.il struct01.exe
    "$CORE_ROOT/ildasm" -raweh -out=struct01.dasm.il struct01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=struct01.asm.exe  struct01.dasm.il
    "$CORE_ROOT/ilasm" -output=struct01.asm.exe  struct01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=struct01
    if [ ! -f struct01.org ]; then
        TakeLock
        if [ ! -f struct01.org ]; then
          mkdir IL
          cp struct01.exe IL/struct01.exe
          mv struct01.exe struct01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct01.org /out struct01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct01.org /out struct01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in struct01.reflect.xml
  if [ -f struct01.reflect.xml ]
then
    ReflectionRoots="-x struct01.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" struct01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" struct01.asm.exe $CLRTestExecutionArguments
