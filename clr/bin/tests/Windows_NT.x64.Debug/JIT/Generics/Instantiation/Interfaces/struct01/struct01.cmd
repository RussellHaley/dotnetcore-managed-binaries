...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct01 struct01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct01 struct01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=struct01.dasm.il struct01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=struct01.dasm.il struct01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=struct01.asm.exe  struct01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=struct01.asm.exe  struct01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=struct01
    if not exist "struct01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "struct01.org" (
            mkdir IL
            copy struct01.exe IL\struct01.exe 
            ren struct01.exe struct01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct01.org /out struct01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct01.org /out struct01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in struct01.reflect.xml
    IF EXIST struct01.reflect.xml set ReflectionRoots=-x struct01.reflect.xml
    ...
    set ExePath=%LinkBin%\struct01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% struct01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% struct01.asm.exe %CLRTestExecutionArguments% %Host_Args%
