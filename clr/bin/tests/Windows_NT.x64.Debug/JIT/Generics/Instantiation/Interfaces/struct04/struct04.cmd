...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct04 struct04.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct04 struct04.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=struct04.dasm.il struct04.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=struct04.dasm.il struct04.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=struct04.asm.exe  struct04.dasm.il
    %CORE_ROOT%\ilasm.exe /output=struct04.asm.exe  struct04.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=struct04
    if not exist "struct04.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "struct04.org" (
            mkdir IL
            copy struct04.exe IL\struct04.exe 
            ren struct04.exe struct04.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct04.org /out struct04.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct04.org /out struct04.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in struct04.reflect.xml
    IF EXIST struct04.reflect.xml set ReflectionRoots=-x struct04.reflect.xml
    ...
    set ExePath=%LinkBin%\struct04.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% struct04.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% struct04.asm.exe %CLRTestExecutionArguments% %Host_Args%
