...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\class04 class04.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\class04 class04.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=class04.dasm.il class04.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=class04.dasm.il class04.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=class04.asm.exe  class04.dasm.il
    %CORE_ROOT%\ilasm.exe /output=class04.asm.exe  class04.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=class04
    if not exist "class04.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "class04.org" (
            mkdir IL
            copy class04.exe IL\class04.exe 
            ren class04.exe class04.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class04.org /out class04.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class04.org /out class04.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in class04.reflect.xml
    IF EXIST class04.reflect.xml set ReflectionRoots=-x class04.reflect.xml
    ...
    set ExePath=%LinkBin%\class04.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% class04.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% class04.asm.exe %CLRTestExecutionArguments% %Host_Args%
