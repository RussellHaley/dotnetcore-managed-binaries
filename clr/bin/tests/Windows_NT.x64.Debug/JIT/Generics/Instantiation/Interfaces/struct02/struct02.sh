...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct02 struct02.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct02 struct02.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=struct02.dasm.il struct02.exe
    "$CORE_ROOT/ildasm" -raweh -out=struct02.dasm.il struct02.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=struct02.asm.exe  struct02.dasm.il
    "$CORE_ROOT/ilasm" -output=struct02.asm.exe  struct02.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=struct02
    if [ ! -f struct02.org ]; then
        TakeLock
        if [ ! -f struct02.org ]; then
          mkdir IL
          cp struct02.exe IL/struct02.exe
          mv struct02.exe struct02.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct02.org /out struct02.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct02.org /out struct02.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in struct02.reflect.xml
  if [ -f struct02.reflect.xml ]
then
    ReflectionRoots="-x struct02.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" struct02.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" struct02.asm.exe $CLRTestExecutionArguments
