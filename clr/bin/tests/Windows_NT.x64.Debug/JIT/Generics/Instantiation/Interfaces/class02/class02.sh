...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/class02 class02.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/class02 class02.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=class02.dasm.il class02.exe
    "$CORE_ROOT/ildasm" -raweh -out=class02.dasm.il class02.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=class02.asm.exe  class02.dasm.il
    "$CORE_ROOT/ilasm" -output=class02.asm.exe  class02.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=class02
    if [ ! -f class02.org ]; then
        TakeLock
        if [ ! -f class02.org ]; then
          mkdir IL
          cp class02.exe IL/class02.exe
          mv class02.exe class02.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class02.org /out class02.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class02.org /out class02.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in class02.reflect.xml
  if [ -f class02.reflect.xml ]
then
    ReflectionRoots="-x class02.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" class02.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" class02.asm.exe $CLRTestExecutionArguments
