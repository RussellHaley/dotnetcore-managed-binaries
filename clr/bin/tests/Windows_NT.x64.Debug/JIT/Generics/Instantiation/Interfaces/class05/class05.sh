...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/class05 class05.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/class05 class05.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=class05.dasm.il class05.exe
    "$CORE_ROOT/ildasm" -raweh -out=class05.dasm.il class05.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=class05.asm.exe  class05.dasm.il
    "$CORE_ROOT/ilasm" -output=class05.asm.exe  class05.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=class05
    if [ ! -f class05.org ]; then
        TakeLock
        if [ ! -f class05.org ]; then
          mkdir IL
          cp class05.exe IL/class05.exe
          mv class05.exe class05.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class05.org /out class05.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class05.org /out class05.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in class05.reflect.xml
  if [ -f class05.reflect.xml ]
then
    ReflectionRoots="-x class05.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" class05.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" class05.asm.exe $CLRTestExecutionArguments
