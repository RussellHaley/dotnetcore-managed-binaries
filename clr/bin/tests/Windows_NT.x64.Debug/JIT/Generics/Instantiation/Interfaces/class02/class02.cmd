...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\class02 class02.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\class02 class02.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=class02.dasm.il class02.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=class02.dasm.il class02.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=class02.asm.exe  class02.dasm.il
    %CORE_ROOT%\ilasm.exe /output=class02.asm.exe  class02.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=class02
    if not exist "class02.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "class02.org" (
            mkdir IL
            copy class02.exe IL\class02.exe 
            ren class02.exe class02.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class02.org /out class02.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class02.org /out class02.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in class02.reflect.xml
    IF EXIST class02.reflect.xml set ReflectionRoots=-x class02.reflect.xml
    ...
    set ExePath=%LinkBin%\class02.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% class02.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% class02.asm.exe %CLRTestExecutionArguments% %Host_Args%
