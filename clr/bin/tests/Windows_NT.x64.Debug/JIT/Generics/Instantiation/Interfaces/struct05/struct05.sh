...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct05 struct05.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct05 struct05.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=struct05.dasm.il struct05.exe
    "$CORE_ROOT/ildasm" -raweh -out=struct05.dasm.il struct05.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=struct05.asm.exe  struct05.dasm.il
    "$CORE_ROOT/ilasm" -output=struct05.asm.exe  struct05.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=struct05
    if [ ! -f struct05.org ]; then
        TakeLock
        if [ ! -f struct05.org ]; then
          mkdir IL
          cp struct05.exe IL/struct05.exe
          mv struct05.exe struct05.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct05.org /out struct05.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct05.org /out struct05.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in struct05.reflect.xml
  if [ -f struct05.reflect.xml ]
then
    ReflectionRoots="-x struct05.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" struct05.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" struct05.asm.exe $CLRTestExecutionArguments
