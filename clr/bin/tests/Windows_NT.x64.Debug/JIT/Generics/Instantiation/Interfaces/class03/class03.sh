...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/class03 class03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/class03 class03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=class03.dasm.il class03.exe
    "$CORE_ROOT/ildasm" -raweh -out=class03.dasm.il class03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=class03.asm.exe  class03.dasm.il
    "$CORE_ROOT/ilasm" -output=class03.asm.exe  class03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=class03
    if [ ! -f class03.org ]; then
        TakeLock
        if [ ! -f class03.org ]; then
          mkdir IL
          cp class03.exe IL/class03.exe
          mv class03.exe class03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class03.org /out class03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class03.org /out class03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in class03.reflect.xml
  if [ -f class03.reflect.xml ]
then
    ReflectionRoots="-x class03.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" class03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" class03.asm.exe $CLRTestExecutionArguments
