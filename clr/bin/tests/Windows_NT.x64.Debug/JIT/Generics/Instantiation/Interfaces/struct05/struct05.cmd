...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct05 struct05.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Instantiation\Interfaces\struct05 struct05.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=struct05.dasm.il struct05.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=struct05.dasm.il struct05.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=struct05.asm.exe  struct05.dasm.il
    %CORE_ROOT%\ilasm.exe /output=struct05.asm.exe  struct05.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=struct05
    if not exist "struct05.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "struct05.org" (
            mkdir IL
            copy struct05.exe IL\struct05.exe 
            ren struct05.exe struct05.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct05.org /out struct05.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct05.org /out struct05.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in struct05.reflect.xml
    IF EXIST struct05.reflect.xml set ReflectionRoots=-x struct05.reflect.xml
    ...
    set ExePath=%LinkBin%\struct05.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% struct05.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% struct05.asm.exe %CLRTestExecutionArguments% %Host_Args%
