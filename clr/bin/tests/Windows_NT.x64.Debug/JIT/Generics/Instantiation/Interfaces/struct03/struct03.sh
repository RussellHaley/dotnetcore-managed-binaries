...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct03 struct03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Instantiation/Interfaces/struct03 struct03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=struct03.dasm.il struct03.exe
    "$CORE_ROOT/ildasm" -raweh -out=struct03.dasm.il struct03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=struct03.asm.exe  struct03.dasm.il
    "$CORE_ROOT/ilasm" -output=struct03.asm.exe  struct03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=struct03
    if [ ! -f struct03.org ]; then
        TakeLock
        if [ ! -f struct03.org ]; then
          mkdir IL
          cp struct03.exe IL/struct03.exe
          mv struct03.exe struct03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct03.org /out struct03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct03.org /out struct03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in struct03.reflect.xml
  if [ -f struct03.reflect.xml ]
then
    ReflectionRoots="-x struct03.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" struct03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" struct03.asm.exe $CLRTestExecutionArguments
