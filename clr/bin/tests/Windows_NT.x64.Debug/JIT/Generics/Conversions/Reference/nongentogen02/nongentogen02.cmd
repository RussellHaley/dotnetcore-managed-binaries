...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nongentogen02.dasm.il nongentogen02.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nongentogen02.dasm.il nongentogen02.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nongentogen02.asm.exe  nongentogen02.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nongentogen02.asm.exe  nongentogen02.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nongentogen02
    if not exist "nongentogen02.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nongentogen02.org" (
            mkdir IL
            copy nongentogen02.exe IL\nongentogen02.exe 
            ren nongentogen02.exe nongentogen02.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nongentogen02.org /out nongentogen02.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nongentogen02.org /out nongentogen02.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nongentogen02.reflect.xml
    IF EXIST nongentogen02.reflect.xml set ReflectionRoots=-x nongentogen02.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nongentogen02.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nongentogen02.asm.exe %CLRTestExecutionArguments% %Host_Args%
