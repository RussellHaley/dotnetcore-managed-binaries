...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/gentogen02 gentogen02.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/gentogen02 gentogen02.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gentogen02.dasm.il gentogen02.exe
    "$CORE_ROOT/ildasm" -raweh -out=gentogen02.dasm.il gentogen02.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gentogen02.asm.exe  gentogen02.dasm.il
    "$CORE_ROOT/ilasm" -output=gentogen02.asm.exe  gentogen02.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gentogen02
    if [ ! -f gentogen02.org ]; then
        TakeLock
        if [ ! -f gentogen02.org ]; then
          mkdir IL
          cp gentogen02.exe IL/gentogen02.exe
          mv gentogen02.exe gentogen02.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gentogen02.org /out gentogen02.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gentogen02.org /out gentogen02.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gentogen02.reflect.xml
  if [ -f gentogen02.reflect.xml ]
then
    ReflectionRoots="-x gentogen02.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/gentogen02.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gentogen02.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gentogen02.asm.exe $CLRTestExecutionArguments
