...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/gentogen01 gentogen01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/gentogen01 gentogen01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gentogen01.dasm.il gentogen01.exe
    "$CORE_ROOT/ildasm" -raweh -out=gentogen01.dasm.il gentogen01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gentogen01.asm.exe  gentogen01.dasm.il
    "$CORE_ROOT/ilasm" -output=gentogen01.asm.exe  gentogen01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gentogen01
    if [ ! -f gentogen01.org ]; then
        TakeLock
        if [ ! -f gentogen01.org ]; then
          mkdir IL
          cp gentogen01.exe IL/gentogen01.exe
          mv gentogen01.exe gentogen01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gentogen01.org /out gentogen01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gentogen01.org /out gentogen01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gentogen01.reflect.xml
  if [ -f gentogen01.reflect.xml ]
then
    ReflectionRoots="-x gentogen01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/gentogen01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gentogen01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gentogen01.asm.exe $CLRTestExecutionArguments
