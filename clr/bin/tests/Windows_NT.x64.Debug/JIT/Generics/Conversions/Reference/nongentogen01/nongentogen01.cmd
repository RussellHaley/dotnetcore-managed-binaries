...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\nongentogen01 nongentogen01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\nongentogen01 nongentogen01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nongentogen01.dasm.il nongentogen01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nongentogen01.dasm.il nongentogen01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nongentogen01.asm.exe  nongentogen01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nongentogen01.asm.exe  nongentogen01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nongentogen01
    if not exist "nongentogen01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nongentogen01.org" (
            mkdir IL
            copy nongentogen01.exe IL\nongentogen01.exe 
            ren nongentogen01.exe nongentogen01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nongentogen01.org /out nongentogen01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nongentogen01.org /out nongentogen01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nongentogen01.reflect.xml
    IF EXIST nongentogen01.reflect.xml set ReflectionRoots=-x nongentogen01.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nongentogen01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nongentogen01.asm.exe %CLRTestExecutionArguments% %Host_Args%
