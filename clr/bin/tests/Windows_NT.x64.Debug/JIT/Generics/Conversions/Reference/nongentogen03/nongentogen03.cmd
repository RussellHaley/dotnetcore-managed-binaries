...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nongentogen03.dasm.il nongentogen03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nongentogen03.dasm.il nongentogen03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nongentogen03.asm.exe  nongentogen03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nongentogen03.asm.exe  nongentogen03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nongentogen03
    if not exist "nongentogen03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nongentogen03.org" (
            mkdir IL
            copy nongentogen03.exe IL\nongentogen03.exe 
            ren nongentogen03.exe nongentogen03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nongentogen03.org /out nongentogen03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nongentogen03.org /out nongentogen03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nongentogen03.reflect.xml
    IF EXIST nongentogen03.reflect.xml set ReflectionRoots=-x nongentogen03.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nongentogen03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nongentogen03.asm.exe %CLRTestExecutionArguments% %Host_Args%
