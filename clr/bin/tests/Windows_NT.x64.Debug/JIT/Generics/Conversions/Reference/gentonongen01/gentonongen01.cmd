...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\gentonongen01 gentonongen01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\gentonongen01 gentonongen01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gentonongen01.dasm.il gentonongen01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gentonongen01.dasm.il gentonongen01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gentonongen01.asm.exe  gentonongen01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gentonongen01.asm.exe  gentonongen01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gentonongen01
    if not exist "gentonongen01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gentonongen01.org" (
            mkdir IL
            copy gentonongen01.exe IL\gentonongen01.exe 
            ren gentonongen01.exe gentonongen01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gentonongen01.org /out gentonongen01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gentonongen01.org /out gentonongen01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gentonongen01.reflect.xml
    IF EXIST gentonongen01.reflect.xml set ReflectionRoots=-x gentonongen01.reflect.xml
    ...
    set ExePath=%LinkBin%\gentonongen01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gentonongen01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gentonongen01.asm.exe %CLRTestExecutionArguments% %Host_Args%
