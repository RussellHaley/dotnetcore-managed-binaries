...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/nongentogen01 nongentogen01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/nongentogen01 nongentogen01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nongentogen01.dasm.il nongentogen01.exe
    "$CORE_ROOT/ildasm" -raweh -out=nongentogen01.dasm.il nongentogen01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nongentogen01.asm.exe  nongentogen01.dasm.il
    "$CORE_ROOT/ilasm" -output=nongentogen01.asm.exe  nongentogen01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nongentogen01
    if [ ! -f nongentogen01.org ]; then
        TakeLock
        if [ ! -f nongentogen01.org ]; then
          mkdir IL
          cp nongentogen01.exe IL/nongentogen01.exe
          mv nongentogen01.exe nongentogen01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nongentogen01.org /out nongentogen01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nongentogen01.org /out nongentogen01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in nongentogen01.reflect.xml
  if [ -f nongentogen01.reflect.xml ]
then
    ReflectionRoots="-x nongentogen01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nongentogen01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nongentogen01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nongentogen01.asm.exe $CLRTestExecutionArguments
