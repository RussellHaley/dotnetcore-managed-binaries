...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\gentogen03 gentogen03.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\gentogen03 gentogen03.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gentogen03.dasm.il gentogen03.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gentogen03.dasm.il gentogen03.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gentogen03.asm.exe  gentogen03.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gentogen03.asm.exe  gentogen03.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gentogen03
    if not exist "gentogen03.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gentogen03.org" (
            mkdir IL
            copy gentogen03.exe IL\gentogen03.exe 
            ren gentogen03.exe gentogen03.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gentogen03.org /out gentogen03.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gentogen03.org /out gentogen03.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gentogen03.reflect.xml
    IF EXIST gentogen03.reflect.xml set ReflectionRoots=-x gentogen03.reflect.xml
    ...
    set ExePath=%LinkBin%\gentogen03.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gentogen03.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gentogen03.asm.exe %CLRTestExecutionArguments% %Host_Args%
