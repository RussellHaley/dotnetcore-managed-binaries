...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/gentonongen01 gentonongen01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/gentonongen01 gentonongen01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gentonongen01.dasm.il gentonongen01.exe
    "$CORE_ROOT/ildasm" -raweh -out=gentonongen01.dasm.il gentonongen01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gentonongen01.asm.exe  gentonongen01.dasm.il
    "$CORE_ROOT/ilasm" -output=gentonongen01.asm.exe  gentonongen01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gentonongen01
    if [ ! -f gentonongen01.org ]; then
        TakeLock
        if [ ! -f gentonongen01.org ]; then
          mkdir IL
          cp gentonongen01.exe IL/gentonongen01.exe
          mv gentonongen01.exe gentonongen01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gentonongen01.org /out gentonongen01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gentonongen01.org /out gentonongen01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gentonongen01.reflect.xml
  if [ -f gentonongen01.reflect.xml ]
then
    ReflectionRoots="-x gentonongen01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/gentonongen01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gentonongen01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gentonongen01.asm.exe $CLRTestExecutionArguments
