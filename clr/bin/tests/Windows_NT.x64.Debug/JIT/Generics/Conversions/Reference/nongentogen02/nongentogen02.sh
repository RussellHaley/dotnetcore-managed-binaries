...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/nongentogen02 nongentogen02.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/nongentogen02 nongentogen02.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nongentogen02.dasm.il nongentogen02.exe
    "$CORE_ROOT/ildasm" -raweh -out=nongentogen02.dasm.il nongentogen02.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nongentogen02.asm.exe  nongentogen02.dasm.il
    "$CORE_ROOT/ilasm" -output=nongentogen02.asm.exe  nongentogen02.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nongentogen02
    if [ ! -f nongentogen02.org ]; then
        TakeLock
        if [ ! -f nongentogen02.org ]; then
          mkdir IL
          cp nongentogen02.exe IL/nongentogen02.exe
          mv nongentogen02.exe nongentogen02.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nongentogen02.org /out nongentogen02.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nongentogen02.org /out nongentogen02.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in nongentogen02.reflect.xml
  if [ -f nongentogen02.reflect.xml ]
then
    ReflectionRoots="-x nongentogen02.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nongentogen02.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nongentogen02.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nongentogen02.asm.exe $CLRTestExecutionArguments
