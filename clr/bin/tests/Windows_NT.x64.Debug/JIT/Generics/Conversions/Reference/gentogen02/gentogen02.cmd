...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\gentogen02 gentogen02.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\gentogen02 gentogen02.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gentogen02.dasm.il gentogen02.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gentogen02.dasm.il gentogen02.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gentogen02.asm.exe  gentogen02.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gentogen02.asm.exe  gentogen02.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gentogen02
    if not exist "gentogen02.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gentogen02.org" (
            mkdir IL
            copy gentogen02.exe IL\gentogen02.exe 
            ren gentogen02.exe gentogen02.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gentogen02.org /out gentogen02.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gentogen02.org /out gentogen02.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gentogen02.reflect.xml
    IF EXIST gentogen02.reflect.xml set ReflectionRoots=-x gentogen02.reflect.xml
    ...
    set ExePath=%LinkBin%\gentogen02.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gentogen02.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gentogen02.asm.exe %CLRTestExecutionArguments% %Host_Args%
