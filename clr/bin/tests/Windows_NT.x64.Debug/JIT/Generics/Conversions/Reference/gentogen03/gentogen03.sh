...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/gentogen03 gentogen03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/gentogen03 gentogen03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gentogen03.dasm.il gentogen03.exe
    "$CORE_ROOT/ildasm" -raweh -out=gentogen03.dasm.il gentogen03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gentogen03.asm.exe  gentogen03.dasm.il
    "$CORE_ROOT/ilasm" -output=gentogen03.asm.exe  gentogen03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gentogen03
    if [ ! -f gentogen03.org ]; then
        TakeLock
        if [ ! -f gentogen03.org ]; then
          mkdir IL
          cp gentogen03.exe IL/gentogen03.exe
          mv gentogen03.exe gentogen03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gentogen03.org /out gentogen03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gentogen03.org /out gentogen03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gentogen03.reflect.xml
  if [ -f gentogen03.reflect.xml ]
then
    ReflectionRoots="-x gentogen03.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/gentogen03.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gentogen03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gentogen03.asm.exe $CLRTestExecutionArguments
