...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\gentogen01 gentogen01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Generics\Conversions\Reference\gentogen01 gentogen01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gentogen01.dasm.il gentogen01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gentogen01.dasm.il gentogen01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gentogen01.asm.exe  gentogen01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gentogen01.asm.exe  gentogen01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gentogen01
    if not exist "gentogen01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gentogen01.org" (
            mkdir IL
            copy gentogen01.exe IL\gentogen01.exe 
            ren gentogen01.exe gentogen01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gentogen01.org /out gentogen01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gentogen01.org /out gentogen01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gentogen01.reflect.xml
    IF EXIST gentogen01.reflect.xml set ReflectionRoots=-x gentogen01.reflect.xml
    ...
    set ExePath=%LinkBin%\gentogen01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gentogen01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gentogen01.asm.exe %CLRTestExecutionArguments% %Host_Args%
