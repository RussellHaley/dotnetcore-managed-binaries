...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/nongentogen03 nongentogen03.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Generics/Conversions/Reference/nongentogen03 nongentogen03.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nongentogen03.dasm.il nongentogen03.exe
    "$CORE_ROOT/ildasm" -raweh -out=nongentogen03.dasm.il nongentogen03.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nongentogen03.asm.exe  nongentogen03.dasm.il
    "$CORE_ROOT/ilasm" -output=nongentogen03.asm.exe  nongentogen03.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nongentogen03
    if [ ! -f nongentogen03.org ]; then
        TakeLock
        if [ ! -f nongentogen03.org ]; then
          mkdir IL
          cp nongentogen03.exe IL/nongentogen03.exe
          mv nongentogen03.exe nongentogen03.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nongentogen03.org /out nongentogen03.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nongentogen03.org /out nongentogen03.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in nongentogen03.reflect.xml
  if [ -f nongentogen03.reflect.xml ]
then
    ReflectionRoots="-x nongentogen03.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nongentogen03.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nongentogen03.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nongentogen03.asm.exe $CLRTestExecutionArguments
