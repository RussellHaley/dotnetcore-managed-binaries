...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Generics\Exceptions\general_struct_instance01 general_struct_instance01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Generics\Exceptions\general_struct_instance01 general_struct_instance01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=general_struct_instance01.dasm.il general_struct_instance01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=general_struct_instance01.dasm.il general_struct_instance01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=general_struct_instance01.asm.exe  general_struct_instance01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=general_struct_instance01.asm.exe  general_struct_instance01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=general_struct_instance01
    if not exist "general_struct_instance01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "general_struct_instance01.org" (
            mkdir IL
            copy general_struct_instance01.exe IL\general_struct_instance01.exe 
            ren general_struct_instance01.exe general_struct_instance01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in general_struct_instance01.org /out general_struct_instance01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in general_struct_instance01.org /out general_struct_instance01.exe
...
set ExePath=general_struct_instance01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in general_struct_instance01.reflect.xml
    IF EXIST general_struct_instance01.reflect.xml set ReflectionRoots=-x general_struct_instance01.reflect.xml
    ...
    set ExePath=%LinkBin%\general_struct_instance01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% general_struct_instance01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% general_struct_instance01.asm.exe %CLRTestExecutionArguments% %Host_Args%
