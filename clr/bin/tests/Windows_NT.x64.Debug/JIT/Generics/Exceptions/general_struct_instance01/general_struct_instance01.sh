...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/Exceptions/general_struct_instance01 general_struct_instance01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/Exceptions/general_struct_instance01 general_struct_instance01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=general_struct_instance01.dasm.il general_struct_instance01.exe
    "$CORE_ROOT/ildasm" -raweh -out=general_struct_instance01.dasm.il general_struct_instance01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=general_struct_instance01.asm.exe  general_struct_instance01.dasm.il
    "$CORE_ROOT/ilasm" -output=general_struct_instance01.asm.exe  general_struct_instance01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=general_struct_instance01
    if [ ! -f general_struct_instance01.org ]; then
        TakeLock
        if [ ! -f general_struct_instance01.org ]; then
          mkdir IL
          cp general_struct_instance01.exe IL/general_struct_instance01.exe
          mv general_struct_instance01.exe general_struct_instance01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in general_struct_instance01.org /out general_struct_instance01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in general_struct_instance01.org /out general_struct_instance01.exe
          __cgExitCode=$?
...
ExePath=general_struct_instance01.exe
...
then
  ...
  # Use hints for reflection roots, if provided in general_struct_instance01.reflect.xml
  if [ -f general_struct_instance01.reflect.xml ]
then
    ReflectionRoots="-x general_struct_instance01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/general_struct_instance01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" general_struct_instance01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" general_struct_instance01.asm.exe $CLRTestExecutionArguments
