...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/Exceptions/general_class_static01 general_class_static01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/Exceptions/general_class_static01 general_class_static01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=general_class_static01.dasm.il general_class_static01.exe
    "$CORE_ROOT/ildasm" -raweh -out=general_class_static01.dasm.il general_class_static01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=general_class_static01.asm.exe  general_class_static01.dasm.il
    "$CORE_ROOT/ilasm" -output=general_class_static01.asm.exe  general_class_static01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=general_class_static01
    if [ ! -f general_class_static01.org ]; then
        TakeLock
        if [ ! -f general_class_static01.org ]; then
          mkdir IL
          cp general_class_static01.exe IL/general_class_static01.exe
          mv general_class_static01.exe general_class_static01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in general_class_static01.org /out general_class_static01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in general_class_static01.org /out general_class_static01.exe
          __cgExitCode=$?
...
ExePath=general_class_static01.exe
...
then
  ...
  # Use hints for reflection roots, if provided in general_class_static01.reflect.xml
  if [ -f general_class_static01.reflect.xml ]
then
    ReflectionRoots="-x general_class_static01.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/general_class_static01.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" general_class_static01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" general_class_static01.asm.exe $CLRTestExecutionArguments
