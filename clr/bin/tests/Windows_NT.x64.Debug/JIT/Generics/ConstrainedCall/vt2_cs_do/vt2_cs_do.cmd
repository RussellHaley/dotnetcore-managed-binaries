...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Generics\ConstrainedCall\vt2_cs_do vt2_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Generics\ConstrainedCall\vt2_cs_do vt2_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=vt2_cs_do.dasm.il vt2_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=vt2_cs_do.dasm.il vt2_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=vt2_cs_do.asm.exe  vt2_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=vt2_cs_do.asm.exe  vt2_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=vt2_cs_do
    if not exist "vt2_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "vt2_cs_do.org" (
            mkdir IL
            copy vt2_cs_do.exe IL\vt2_cs_do.exe 
            ren vt2_cs_do.exe vt2_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in vt2_cs_do.org /out vt2_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in vt2_cs_do.org /out vt2_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in vt2_cs_do.reflect.xml
    IF EXIST vt2_cs_do.reflect.xml set ReflectionRoots=-x vt2_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\vt2_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% vt2_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% vt2_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
