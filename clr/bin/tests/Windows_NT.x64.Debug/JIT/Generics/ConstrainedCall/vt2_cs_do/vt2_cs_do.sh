...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/ConstrainedCall/vt2_cs_do vt2_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/ConstrainedCall/vt2_cs_do vt2_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=vt2_cs_do.dasm.il vt2_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=vt2_cs_do.dasm.il vt2_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=vt2_cs_do.asm.exe  vt2_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=vt2_cs_do.asm.exe  vt2_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=vt2_cs_do
    if [ ! -f vt2_cs_do.org ]; then
        TakeLock
        if [ ! -f vt2_cs_do.org ]; then
          mkdir IL
          cp vt2_cs_do.exe IL/vt2_cs_do.exe
          mv vt2_cs_do.exe vt2_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vt2_cs_do.org /out vt2_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vt2_cs_do.org /out vt2_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in vt2_cs_do.reflect.xml
  if [ -f vt2_cs_do.reflect.xml ]
then
    ReflectionRoots="-x vt2_cs_do.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" vt2_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" vt2_cs_do.asm.exe $CLRTestExecutionArguments
