...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/ConstrainedCall/vt2_cs_ro vt2_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/ConstrainedCall/vt2_cs_ro vt2_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=vt2_cs_ro.dasm.il vt2_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=vt2_cs_ro.dasm.il vt2_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=vt2_cs_ro.asm.exe  vt2_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=vt2_cs_ro.asm.exe  vt2_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=vt2_cs_ro
    if [ ! -f vt2_cs_ro.org ]; then
        TakeLock
        if [ ! -f vt2_cs_ro.org ]; then
          mkdir IL
          cp vt2_cs_ro.exe IL/vt2_cs_ro.exe
          mv vt2_cs_ro.exe vt2_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vt2_cs_ro.org /out vt2_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vt2_cs_ro.org /out vt2_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in vt2_cs_ro.reflect.xml
  if [ -f vt2_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x vt2_cs_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" vt2_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" vt2_cs_ro.asm.exe $CLRTestExecutionArguments
