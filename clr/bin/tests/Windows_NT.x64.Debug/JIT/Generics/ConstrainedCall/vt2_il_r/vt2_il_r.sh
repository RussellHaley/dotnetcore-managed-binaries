...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/ConstrainedCall/vt2_il_r vt2_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/ConstrainedCall/vt2_il_r vt2_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=vt2_il_r.dasm.il vt2_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=vt2_il_r.dasm.il vt2_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=vt2_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT vt2_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=vt2_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT vt2_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=vt2_il_r
    if [ ! -f vt2_il_r.org ]; then
        TakeLock
        if [ ! -f vt2_il_r.org ]; then
          mkdir IL
          cp vt2_il_r.exe IL/vt2_il_r.exe
          mv vt2_il_r.exe vt2_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vt2_il_r.org /out vt2_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vt2_il_r.org /out vt2_il_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in vt2_il_r.reflect.xml
  if [ -f vt2_il_r.reflect.xml ]
then
    ReflectionRoots="-x vt2_il_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" vt2_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" vt2_il_r.asm.exe $CLRTestExecutionArguments
