...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/ConstrainedCall/vt2_cs_d vt2_cs_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/ConstrainedCall/vt2_cs_d vt2_cs_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=vt2_cs_d.dasm.il vt2_cs_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=vt2_cs_d.dasm.il vt2_cs_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=vt2_cs_d.asm.exe  vt2_cs_d.dasm.il
    "$CORE_ROOT/ilasm" -output=vt2_cs_d.asm.exe  vt2_cs_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=vt2_cs_d
    if [ ! -f vt2_cs_d.org ]; then
        TakeLock
        if [ ! -f vt2_cs_d.org ]; then
          mkdir IL
          cp vt2_cs_d.exe IL/vt2_cs_d.exe
          mv vt2_cs_d.exe vt2_cs_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vt2_cs_d.org /out vt2_cs_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in vt2_cs_d.org /out vt2_cs_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in vt2_cs_d.reflect.xml
  if [ -f vt2_cs_d.reflect.xml ]
then
    ReflectionRoots="-x vt2_cs_d.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" vt2_cs_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" vt2_cs_d.asm.exe $CLRTestExecutionArguments
