...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Generics\Arrays\TypeParameters\Jagged\class01 class01.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Generics\Arrays\TypeParameters\Jagged\class01 class01.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=class01.dasm.il class01.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=class01.dasm.il class01.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=class01.asm.exe  class01.dasm.il
    %CORE_ROOT%\ilasm.exe /output=class01.asm.exe  class01.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=class01
    if not exist "class01.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "class01.org" (
            mkdir IL
            copy class01.exe IL\class01.exe 
            ren class01.exe class01.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class01.org /out class01.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in class01.org /out class01.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in class01.reflect.xml
    IF EXIST class01.reflect.xml set ReflectionRoots=-x class01.reflect.xml
    ...
    set ExePath=%LinkBin%\class01.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% class01.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% class01.asm.exe %CLRTestExecutionArguments% %Host_Args%
