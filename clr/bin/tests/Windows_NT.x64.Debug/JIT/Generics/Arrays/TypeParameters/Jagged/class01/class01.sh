...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Generics/Arrays/TypeParameters/Jagged/class01 class01.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Generics/Arrays/TypeParameters/Jagged/class01 class01.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=class01.dasm.il class01.exe
    "$CORE_ROOT/ildasm" -raweh -out=class01.dasm.il class01.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=class01.asm.exe  class01.dasm.il
    "$CORE_ROOT/ilasm" -output=class01.asm.exe  class01.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=class01
    if [ ! -f class01.org ]; then
        TakeLock
        if [ ! -f class01.org ]; then
          mkdir IL
          cp class01.exe IL/class01.exe
          mv class01.exe class01.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class01.org /out class01.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in class01.org /out class01.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in class01.reflect.xml
  if [ -f class01.reflect.xml ]
then
    ReflectionRoots="-x class01.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" class01.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" class01.asm.exe $CLRTestExecutionArguments
