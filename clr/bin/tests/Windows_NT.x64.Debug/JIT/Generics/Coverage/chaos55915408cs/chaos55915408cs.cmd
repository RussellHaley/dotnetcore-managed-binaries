...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Generics\Coverage\chaos55915408cs chaos55915408cs.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Generics\Coverage\chaos55915408cs chaos55915408cs.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=chaos55915408cs.dasm.il chaos55915408cs.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=chaos55915408cs.dasm.il chaos55915408cs.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=chaos55915408cs.asm.exe  chaos55915408cs.dasm.il
    %CORE_ROOT%\ilasm.exe /output=chaos55915408cs.asm.exe  chaos55915408cs.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=chaos55915408cs
    if not exist "chaos55915408cs.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "chaos55915408cs.org" (
            mkdir IL
            copy chaos55915408cs.exe IL\chaos55915408cs.exe 
            ren chaos55915408cs.exe chaos55915408cs.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in chaos55915408cs.org /out chaos55915408cs.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in chaos55915408cs.org /out chaos55915408cs.exe
...
set ExePath=chaos55915408cs.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in chaos55915408cs.reflect.xml
    IF EXIST chaos55915408cs.reflect.xml set ReflectionRoots=-x chaos55915408cs.reflect.xml
    ...
    set ExePath=%LinkBin%\chaos55915408cs.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% chaos55915408cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% chaos55915408cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
