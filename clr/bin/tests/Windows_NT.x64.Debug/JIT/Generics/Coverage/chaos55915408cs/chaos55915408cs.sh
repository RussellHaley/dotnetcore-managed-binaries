...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/Coverage/chaos55915408cs chaos55915408cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/Coverage/chaos55915408cs chaos55915408cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=chaos55915408cs.dasm.il chaos55915408cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=chaos55915408cs.dasm.il chaos55915408cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=chaos55915408cs.asm.exe  chaos55915408cs.dasm.il
    "$CORE_ROOT/ilasm" -output=chaos55915408cs.asm.exe  chaos55915408cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=chaos55915408cs
    if [ ! -f chaos55915408cs.org ]; then
        TakeLock
        if [ ! -f chaos55915408cs.org ]; then
          mkdir IL
          cp chaos55915408cs.exe IL/chaos55915408cs.exe
          mv chaos55915408cs.exe chaos55915408cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in chaos55915408cs.org /out chaos55915408cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in chaos55915408cs.org /out chaos55915408cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in chaos55915408cs.reflect.xml
  if [ -f chaos55915408cs.reflect.xml ]
then
    ReflectionRoots="-x chaos55915408cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/chaos55915408cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" chaos55915408cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" chaos55915408cs.asm.exe $CLRTestExecutionArguments
