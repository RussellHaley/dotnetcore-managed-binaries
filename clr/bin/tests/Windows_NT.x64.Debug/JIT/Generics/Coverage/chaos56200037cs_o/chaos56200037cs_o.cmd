...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Generics\Coverage\chaos56200037cs_o chaos56200037cs_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Generics\Coverage\chaos56200037cs_o chaos56200037cs_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=chaos56200037cs_o.dasm.il chaos56200037cs_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=chaos56200037cs_o.dasm.il chaos56200037cs_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=chaos56200037cs_o.asm.exe  chaos56200037cs_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=chaos56200037cs_o.asm.exe  chaos56200037cs_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=chaos56200037cs_o
    if not exist "chaos56200037cs_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "chaos56200037cs_o.org" (
            mkdir IL
            copy chaos56200037cs_o.exe IL\chaos56200037cs_o.exe 
            ren chaos56200037cs_o.exe chaos56200037cs_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in chaos56200037cs_o.org /out chaos56200037cs_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in chaos56200037cs_o.org /out chaos56200037cs_o.exe
...
set ExePath=chaos56200037cs_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in chaos56200037cs_o.reflect.xml
    IF EXIST chaos56200037cs_o.reflect.xml set ReflectionRoots=-x chaos56200037cs_o.reflect.xml
    ...
    set ExePath=%LinkBin%\chaos56200037cs_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% chaos56200037cs_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% chaos56200037cs_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
