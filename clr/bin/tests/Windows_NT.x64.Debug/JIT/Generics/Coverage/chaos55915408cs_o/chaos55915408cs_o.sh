...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/Coverage/chaos55915408cs_o chaos55915408cs_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Generics/Coverage/chaos55915408cs_o chaos55915408cs_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=chaos55915408cs_o.dasm.il chaos55915408cs_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=chaos55915408cs_o.dasm.il chaos55915408cs_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=chaos55915408cs_o.asm.exe  chaos55915408cs_o.dasm.il
    "$CORE_ROOT/ilasm" -output=chaos55915408cs_o.asm.exe  chaos55915408cs_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=chaos55915408cs_o
    if [ ! -f chaos55915408cs_o.org ]; then
        TakeLock
        if [ ! -f chaos55915408cs_o.org ]; then
          mkdir IL
          cp chaos55915408cs_o.exe IL/chaos55915408cs_o.exe
          mv chaos55915408cs_o.exe chaos55915408cs_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in chaos55915408cs_o.org /out chaos55915408cs_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in chaos55915408cs_o.org /out chaos55915408cs_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in chaos55915408cs_o.reflect.xml
  if [ -f chaos55915408cs_o.reflect.xml ]
then
    ReflectionRoots="-x chaos55915408cs_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/chaos55915408cs_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" chaos55915408cs_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" chaos55915408cs_o.asm.exe $CLRTestExecutionArguments
