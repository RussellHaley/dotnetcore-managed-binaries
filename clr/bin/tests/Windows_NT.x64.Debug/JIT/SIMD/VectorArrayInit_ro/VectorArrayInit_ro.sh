...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArrayInit_ro VectorArrayInit_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArrayInit_ro VectorArrayInit_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorArrayInit_ro.dasm.il VectorArrayInit_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorArrayInit_ro.dasm.il VectorArrayInit_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorArrayInit_ro.asm.exe  VectorArrayInit_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorArrayInit_ro.asm.exe  VectorArrayInit_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorArrayInit_ro
    if [ ! -f VectorArrayInit_ro.org ]; then
        TakeLock
        if [ ! -f VectorArrayInit_ro.org ]; then
          mkdir IL
          cp VectorArrayInit_ro.exe IL/VectorArrayInit_ro.exe
          mv VectorArrayInit_ro.exe VectorArrayInit_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArrayInit_ro.org /out VectorArrayInit_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArrayInit_ro.org /out VectorArrayInit_ro.exe
          __cgExitCode=$?
...
ExePath=VectorArrayInit_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in VectorArrayInit_ro.reflect.xml
  if [ -f VectorArrayInit_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorArrayInit_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorArrayInit_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorArrayInit_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorArrayInit_ro.asm.exe $CLRTestExecutionArguments
