...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArrayInit_ro VectorArrayInit_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArrayInit_ro VectorArrayInit_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorArrayInit_ro.dasm.il VectorArrayInit_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorArrayInit_ro.dasm.il VectorArrayInit_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorArrayInit_ro.asm.exe  VectorArrayInit_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorArrayInit_ro.asm.exe  VectorArrayInit_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorArrayInit_ro
    if not exist "VectorArrayInit_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorArrayInit_ro.org" (
            mkdir IL
            copy VectorArrayInit_ro.exe IL\VectorArrayInit_ro.exe 
            ren VectorArrayInit_ro.exe VectorArrayInit_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArrayInit_ro.org /out VectorArrayInit_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArrayInit_ro.org /out VectorArrayInit_ro.exe
...
set ExePath=VectorArrayInit_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorArrayInit_ro.reflect.xml
    IF EXIST VectorArrayInit_ro.reflect.xml set ReflectionRoots=-x VectorArrayInit_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorArrayInit_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorArrayInit_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorArrayInit_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
