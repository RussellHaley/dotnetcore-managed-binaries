...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorConvert_r VectorConvert_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorConvert_r VectorConvert_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorConvert_r.dasm.il VectorConvert_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorConvert_r.dasm.il VectorConvert_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorConvert_r.asm.exe  VectorConvert_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorConvert_r.asm.exe  VectorConvert_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorConvert_r
    if [ ! -f VectorConvert_r.org ]; then
        TakeLock
        if [ ! -f VectorConvert_r.org ]; then
          mkdir IL
          cp VectorConvert_r.exe IL/VectorConvert_r.exe
          mv VectorConvert_r.exe VectorConvert_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorConvert_r.org /out VectorConvert_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorConvert_r.org /out VectorConvert_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorConvert_r.reflect.xml
  if [ -f VectorConvert_r.reflect.xml ]
then
    ReflectionRoots="-x VectorConvert_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorConvert_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorConvert_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorConvert_r.asm.exe $CLRTestExecutionArguments
