...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorConvert_r VectorConvert_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorConvert_r VectorConvert_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorConvert_r.dasm.il VectorConvert_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorConvert_r.dasm.il VectorConvert_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorConvert_r.asm.exe  VectorConvert_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorConvert_r.asm.exe  VectorConvert_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorConvert_r
    if not exist "VectorConvert_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorConvert_r.org" (
            mkdir IL
            copy VectorConvert_r.exe IL\VectorConvert_r.exe 
            ren VectorConvert_r.exe VectorConvert_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorConvert_r.org /out VectorConvert_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorConvert_r.org /out VectorConvert_r.exe
...
set ExePath=VectorConvert_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorConvert_r.reflect.xml
    IF EXIST VectorConvert_r.reflect.xml set ReflectionRoots=-x VectorConvert_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorConvert_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorConvert_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorConvert_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
