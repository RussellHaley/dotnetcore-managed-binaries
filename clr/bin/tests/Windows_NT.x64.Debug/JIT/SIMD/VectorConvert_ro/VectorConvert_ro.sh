...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorConvert_ro VectorConvert_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorConvert_ro VectorConvert_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorConvert_ro.dasm.il VectorConvert_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorConvert_ro.dasm.il VectorConvert_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorConvert_ro.asm.exe  VectorConvert_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorConvert_ro.asm.exe  VectorConvert_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorConvert_ro
    if [ ! -f VectorConvert_ro.org ]; then
        TakeLock
        if [ ! -f VectorConvert_ro.org ]; then
          mkdir IL
          cp VectorConvert_ro.exe IL/VectorConvert_ro.exe
          mv VectorConvert_ro.exe VectorConvert_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorConvert_ro.org /out VectorConvert_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorConvert_ro.org /out VectorConvert_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorConvert_ro.reflect.xml
  if [ -f VectorConvert_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorConvert_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorConvert_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorConvert_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorConvert_ro.asm.exe $CLRTestExecutionArguments
