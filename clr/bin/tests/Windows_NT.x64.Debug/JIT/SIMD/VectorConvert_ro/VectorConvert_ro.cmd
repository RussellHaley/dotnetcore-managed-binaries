...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorConvert_ro VectorConvert_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorConvert_ro VectorConvert_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorConvert_ro.dasm.il VectorConvert_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorConvert_ro.dasm.il VectorConvert_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorConvert_ro.asm.exe  VectorConvert_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorConvert_ro.asm.exe  VectorConvert_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorConvert_ro
    if not exist "VectorConvert_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorConvert_ro.org" (
            mkdir IL
            copy VectorConvert_ro.exe IL\VectorConvert_ro.exe 
            ren VectorConvert_ro.exe VectorConvert_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorConvert_ro.org /out VectorConvert_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorConvert_ro.org /out VectorConvert_ro.exe
...
set ExePath=VectorConvert_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorConvert_ro.reflect.xml
    IF EXIST VectorConvert_ro.reflect.xml set ReflectionRoots=-x VectorConvert_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorConvert_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorConvert_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorConvert_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
