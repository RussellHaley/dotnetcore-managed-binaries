...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorInit_ro VectorInit_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorInit_ro VectorInit_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorInit_ro.dasm.il VectorInit_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorInit_ro.dasm.il VectorInit_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorInit_ro.asm.exe  VectorInit_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorInit_ro.asm.exe  VectorInit_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorInit_ro
    if not exist "VectorInit_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorInit_ro.org" (
            mkdir IL
            copy VectorInit_ro.exe IL\VectorInit_ro.exe 
            ren VectorInit_ro.exe VectorInit_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorInit_ro.org /out VectorInit_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorInit_ro.org /out VectorInit_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorInit_ro.reflect.xml
    IF EXIST VectorInit_ro.reflect.xml set ReflectionRoots=-x VectorInit_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorInit_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorInit_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorInit_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
