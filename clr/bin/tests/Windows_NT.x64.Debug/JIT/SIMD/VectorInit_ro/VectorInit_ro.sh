...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorInit_ro VectorInit_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorInit_ro VectorInit_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorInit_ro.dasm.il VectorInit_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorInit_ro.dasm.il VectorInit_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorInit_ro.asm.exe  VectorInit_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorInit_ro.asm.exe  VectorInit_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorInit_ro
    if [ ! -f VectorInit_ro.org ]; then
        TakeLock
        if [ ! -f VectorInit_ro.org ]; then
          mkdir IL
          cp VectorInit_ro.exe IL/VectorInit_ro.exe
          mv VectorInit_ro.exe VectorInit_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorInit_ro.org /out VectorInit_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorInit_ro.org /out VectorInit_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorInit_ro.reflect.xml
  if [ -f VectorInit_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorInit_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorInit_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorInit_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorInit_ro.asm.exe $CLRTestExecutionArguments
