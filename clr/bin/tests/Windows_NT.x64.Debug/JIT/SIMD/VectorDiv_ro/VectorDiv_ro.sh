...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorDiv_ro VectorDiv_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorDiv_ro VectorDiv_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorDiv_ro.dasm.il VectorDiv_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorDiv_ro.dasm.il VectorDiv_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorDiv_ro.asm.exe  VectorDiv_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorDiv_ro.asm.exe  VectorDiv_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorDiv_ro
    if [ ! -f VectorDiv_ro.org ]; then
        TakeLock
        if [ ! -f VectorDiv_ro.org ]; then
          mkdir IL
          cp VectorDiv_ro.exe IL/VectorDiv_ro.exe
          mv VectorDiv_ro.exe VectorDiv_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorDiv_ro.org /out VectorDiv_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorDiv_ro.org /out VectorDiv_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorDiv_ro.reflect.xml
  if [ -f VectorDiv_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorDiv_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorDiv_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorDiv_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorDiv_ro.asm.exe $CLRTestExecutionArguments
