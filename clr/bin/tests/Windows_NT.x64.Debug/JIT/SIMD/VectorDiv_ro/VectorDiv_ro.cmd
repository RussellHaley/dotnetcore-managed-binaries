...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorDiv_ro VectorDiv_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorDiv_ro VectorDiv_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorDiv_ro.dasm.il VectorDiv_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorDiv_ro.dasm.il VectorDiv_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorDiv_ro.asm.exe  VectorDiv_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorDiv_ro.asm.exe  VectorDiv_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorDiv_ro
    if not exist "VectorDiv_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorDiv_ro.org" (
            mkdir IL
            copy VectorDiv_ro.exe IL\VectorDiv_ro.exe 
            ren VectorDiv_ro.exe VectorDiv_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorDiv_ro.org /out VectorDiv_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorDiv_ro.org /out VectorDiv_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorDiv_ro.reflect.xml
    IF EXIST VectorDiv_ro.reflect.xml set ReflectionRoots=-x VectorDiv_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorDiv_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorDiv_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorDiv_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
