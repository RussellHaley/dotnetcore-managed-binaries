...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorRelOp_ro VectorRelOp_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorRelOp_ro VectorRelOp_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorRelOp_ro.dasm.il VectorRelOp_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorRelOp_ro.dasm.il VectorRelOp_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorRelOp_ro.asm.exe  VectorRelOp_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorRelOp_ro.asm.exe  VectorRelOp_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorRelOp_ro
    if [ ! -f VectorRelOp_ro.org ]; then
        TakeLock
        if [ ! -f VectorRelOp_ro.org ]; then
          mkdir IL
          cp VectorRelOp_ro.exe IL/VectorRelOp_ro.exe
          mv VectorRelOp_ro.exe VectorRelOp_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorRelOp_ro.org /out VectorRelOp_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorRelOp_ro.org /out VectorRelOp_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorRelOp_ro.reflect.xml
  if [ -f VectorRelOp_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorRelOp_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorRelOp_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorRelOp_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorRelOp_ro.asm.exe $CLRTestExecutionArguments
