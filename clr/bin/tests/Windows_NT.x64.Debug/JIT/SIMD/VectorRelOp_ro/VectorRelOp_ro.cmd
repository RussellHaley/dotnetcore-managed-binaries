...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorRelOp_ro VectorRelOp_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorRelOp_ro VectorRelOp_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorRelOp_ro.dasm.il VectorRelOp_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorRelOp_ro.dasm.il VectorRelOp_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorRelOp_ro.asm.exe  VectorRelOp_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorRelOp_ro.asm.exe  VectorRelOp_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorRelOp_ro
    if not exist "VectorRelOp_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorRelOp_ro.org" (
            mkdir IL
            copy VectorRelOp_ro.exe IL\VectorRelOp_ro.exe 
            ren VectorRelOp_ro.exe VectorRelOp_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorRelOp_ro.org /out VectorRelOp_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorRelOp_ro.org /out VectorRelOp_ro.exe
...
set ExePath=VectorRelOp_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorRelOp_ro.reflect.xml
    IF EXIST VectorRelOp_ro.reflect.xml set ReflectionRoots=-x VectorRelOp_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorRelOp_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorRelOp_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorRelOp_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
