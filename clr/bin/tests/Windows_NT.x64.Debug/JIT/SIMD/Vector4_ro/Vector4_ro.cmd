...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Vector4_ro Vector4_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Vector4_ro Vector4_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Vector4_ro.dasm.il Vector4_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Vector4_ro.dasm.il Vector4_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Vector4_ro.asm.exe  Vector4_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Vector4_ro.asm.exe  Vector4_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Vector4_ro
    if not exist "Vector4_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Vector4_ro.org" (
            mkdir IL
            copy Vector4_ro.exe IL\Vector4_ro.exe 
            ren Vector4_ro.exe Vector4_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector4_ro.org /out Vector4_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector4_ro.org /out Vector4_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Vector4_ro.reflect.xml
    IF EXIST Vector4_ro.reflect.xml set ReflectionRoots=-x Vector4_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Vector4_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Vector4_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Vector4_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
