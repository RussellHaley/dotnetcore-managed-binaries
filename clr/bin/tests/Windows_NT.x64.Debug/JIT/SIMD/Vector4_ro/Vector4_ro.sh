...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector4_ro Vector4_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector4_ro Vector4_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Vector4_ro.dasm.il Vector4_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Vector4_ro.dasm.il Vector4_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Vector4_ro.asm.exe  Vector4_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Vector4_ro.asm.exe  Vector4_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Vector4_ro
    if [ ! -f Vector4_ro.org ]; then
        TakeLock
        if [ ! -f Vector4_ro.org ]; then
          mkdir IL
          cp Vector4_ro.exe IL/Vector4_ro.exe
          mv Vector4_ro.exe Vector4_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector4_ro.org /out Vector4_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector4_ro.org /out Vector4_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Vector4_ro.reflect.xml
  if [ -f Vector4_ro.reflect.xml ]
then
    ReflectionRoots="-x Vector4_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Vector4_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Vector4_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Vector4_ro.asm.exe $CLRTestExecutionArguments
