...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Ldfld_r Ldfld_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Ldfld_r Ldfld_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Ldfld_r.dasm.il Ldfld_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Ldfld_r.dasm.il Ldfld_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Ldfld_r.asm.exe  Ldfld_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Ldfld_r.asm.exe  Ldfld_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Ldfld_r
    if not exist "Ldfld_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Ldfld_r.org" (
            mkdir IL
            copy Ldfld_r.exe IL\Ldfld_r.exe 
            ren Ldfld_r.exe Ldfld_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ldfld_r.org /out Ldfld_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ldfld_r.org /out Ldfld_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Ldfld_r.reflect.xml
    IF EXIST Ldfld_r.reflect.xml set ReflectionRoots=-x Ldfld_r.reflect.xml
    ...
    set ExePath=%LinkBin%\Ldfld_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Ldfld_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Ldfld_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
