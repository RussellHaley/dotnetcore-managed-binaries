...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Ldfld_r Ldfld_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Ldfld_r Ldfld_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Ldfld_r.dasm.il Ldfld_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=Ldfld_r.dasm.il Ldfld_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Ldfld_r.asm.exe  Ldfld_r.dasm.il
    "$CORE_ROOT/ilasm" -output=Ldfld_r.asm.exe  Ldfld_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Ldfld_r
    if [ ! -f Ldfld_r.org ]; then
        TakeLock
        if [ ! -f Ldfld_r.org ]; then
          mkdir IL
          cp Ldfld_r.exe IL/Ldfld_r.exe
          mv Ldfld_r.exe Ldfld_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ldfld_r.org /out Ldfld_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ldfld_r.org /out Ldfld_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Ldfld_r.reflect.xml
  if [ -f Ldfld_r.reflect.xml ]
then
    ReflectionRoots="-x Ldfld_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Ldfld_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Ldfld_r.asm.exe $CLRTestExecutionArguments
