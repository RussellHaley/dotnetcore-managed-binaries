...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorAbs_ro VectorAbs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorAbs_ro VectorAbs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorAbs_ro.dasm.il VectorAbs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorAbs_ro.dasm.il VectorAbs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorAbs_ro.asm.exe  VectorAbs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorAbs_ro.asm.exe  VectorAbs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorAbs_ro
    if [ ! -f VectorAbs_ro.org ]; then
        TakeLock
        if [ ! -f VectorAbs_ro.org ]; then
          mkdir IL
          cp VectorAbs_ro.exe IL/VectorAbs_ro.exe
          mv VectorAbs_ro.exe VectorAbs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorAbs_ro.org /out VectorAbs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorAbs_ro.org /out VectorAbs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorAbs_ro.reflect.xml
  if [ -f VectorAbs_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorAbs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorAbs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorAbs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorAbs_ro.asm.exe $CLRTestExecutionArguments
