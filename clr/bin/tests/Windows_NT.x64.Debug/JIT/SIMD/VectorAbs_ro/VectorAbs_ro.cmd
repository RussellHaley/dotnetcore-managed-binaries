...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorAbs_ro VectorAbs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorAbs_ro VectorAbs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorAbs_ro.dasm.il VectorAbs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorAbs_ro.dasm.il VectorAbs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorAbs_ro.asm.exe  VectorAbs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorAbs_ro.asm.exe  VectorAbs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorAbs_ro
    if not exist "VectorAbs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorAbs_ro.org" (
            mkdir IL
            copy VectorAbs_ro.exe IL\VectorAbs_ro.exe 
            ren VectorAbs_ro.exe VectorAbs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorAbs_ro.org /out VectorAbs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorAbs_ro.org /out VectorAbs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorAbs_ro.reflect.xml
    IF EXIST VectorAbs_ro.reflect.xml set ReflectionRoots=-x VectorAbs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorAbs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorAbs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorAbs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
