...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMatrix_ro VectorMatrix_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMatrix_ro VectorMatrix_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorMatrix_ro.dasm.il VectorMatrix_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorMatrix_ro.dasm.il VectorMatrix_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorMatrix_ro.asm.exe  VectorMatrix_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorMatrix_ro.asm.exe  VectorMatrix_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorMatrix_ro
    if not exist "VectorMatrix_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorMatrix_ro.org" (
            mkdir IL
            copy VectorMatrix_ro.exe IL\VectorMatrix_ro.exe 
            ren VectorMatrix_ro.exe VectorMatrix_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMatrix_ro.org /out VectorMatrix_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMatrix_ro.org /out VectorMatrix_ro.exe
...
set ExePath=VectorMatrix_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorMatrix_ro.reflect.xml
    IF EXIST VectorMatrix_ro.reflect.xml set ReflectionRoots=-x VectorMatrix_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorMatrix_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorMatrix_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorMatrix_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
