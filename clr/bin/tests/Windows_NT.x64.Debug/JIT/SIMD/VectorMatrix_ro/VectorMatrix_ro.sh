...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMatrix_ro VectorMatrix_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMatrix_ro VectorMatrix_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorMatrix_ro.dasm.il VectorMatrix_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorMatrix_ro.dasm.il VectorMatrix_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorMatrix_ro.asm.exe  VectorMatrix_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorMatrix_ro.asm.exe  VectorMatrix_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorMatrix_ro
    if [ ! -f VectorMatrix_ro.org ]; then
        TakeLock
        if [ ! -f VectorMatrix_ro.org ]; then
          mkdir IL
          cp VectorMatrix_ro.exe IL/VectorMatrix_ro.exe
          mv VectorMatrix_ro.exe VectorMatrix_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMatrix_ro.org /out VectorMatrix_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMatrix_ro.org /out VectorMatrix_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorMatrix_ro.reflect.xml
  if [ -f VectorMatrix_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorMatrix_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorMatrix_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorMatrix_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorMatrix_ro.asm.exe $CLRTestExecutionArguments
