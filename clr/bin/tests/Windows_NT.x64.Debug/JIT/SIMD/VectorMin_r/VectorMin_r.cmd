...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMin_r VectorMin_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMin_r VectorMin_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorMin_r.dasm.il VectorMin_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorMin_r.dasm.il VectorMin_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorMin_r.asm.exe  VectorMin_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorMin_r.asm.exe  VectorMin_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorMin_r
    if not exist "VectorMin_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorMin_r.org" (
            mkdir IL
            copy VectorMin_r.exe IL\VectorMin_r.exe 
            ren VectorMin_r.exe VectorMin_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMin_r.org /out VectorMin_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMin_r.org /out VectorMin_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorMin_r.reflect.xml
    IF EXIST VectorMin_r.reflect.xml set ReflectionRoots=-x VectorMin_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorMin_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorMin_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorMin_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
