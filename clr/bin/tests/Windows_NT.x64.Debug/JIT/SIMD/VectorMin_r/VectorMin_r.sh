...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMin_r VectorMin_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMin_r VectorMin_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorMin_r.dasm.il VectorMin_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorMin_r.dasm.il VectorMin_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorMin_r.asm.exe  VectorMin_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorMin_r.asm.exe  VectorMin_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorMin_r
    if [ ! -f VectorMin_r.org ]; then
        TakeLock
        if [ ! -f VectorMin_r.org ]; then
          mkdir IL
          cp VectorMin_r.exe IL/VectorMin_r.exe
          mv VectorMin_r.exe VectorMin_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMin_r.org /out VectorMin_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMin_r.org /out VectorMin_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorMin_r.reflect.xml
  if [ -f VectorMin_r.reflect.xml ]
then
    ReflectionRoots="-x VectorMin_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorMin_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorMin_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorMin_r.asm.exe $CLRTestExecutionArguments
