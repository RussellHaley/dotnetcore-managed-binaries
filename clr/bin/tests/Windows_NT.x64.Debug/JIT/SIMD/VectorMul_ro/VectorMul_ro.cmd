...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMul_ro VectorMul_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMul_ro VectorMul_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorMul_ro.dasm.il VectorMul_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorMul_ro.dasm.il VectorMul_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorMul_ro.asm.exe  VectorMul_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorMul_ro.asm.exe  VectorMul_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorMul_ro
    if not exist "VectorMul_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorMul_ro.org" (
            mkdir IL
            copy VectorMul_ro.exe IL\VectorMul_ro.exe 
            ren VectorMul_ro.exe VectorMul_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMul_ro.org /out VectorMul_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMul_ro.org /out VectorMul_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorMul_ro.reflect.xml
    IF EXIST VectorMul_ro.reflect.xml set ReflectionRoots=-x VectorMul_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorMul_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorMul_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorMul_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
