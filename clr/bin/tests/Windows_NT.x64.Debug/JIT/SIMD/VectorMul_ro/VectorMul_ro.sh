...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMul_ro VectorMul_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMul_ro VectorMul_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorMul_ro.dasm.il VectorMul_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorMul_ro.dasm.il VectorMul_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorMul_ro.asm.exe  VectorMul_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorMul_ro.asm.exe  VectorMul_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorMul_ro
    if [ ! -f VectorMul_ro.org ]; then
        TakeLock
        if [ ! -f VectorMul_ro.org ]; then
          mkdir IL
          cp VectorMul_ro.exe IL/VectorMul_ro.exe
          mv VectorMul_ro.exe VectorMul_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMul_ro.org /out VectorMul_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMul_ro.org /out VectorMul_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorMul_ro.reflect.xml
  if [ -f VectorMul_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorMul_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorMul_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorMul_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorMul_ro.asm.exe $CLRTestExecutionArguments
