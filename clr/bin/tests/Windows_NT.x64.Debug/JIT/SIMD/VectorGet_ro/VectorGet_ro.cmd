...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorGet_ro VectorGet_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorGet_ro VectorGet_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorGet_ro.dasm.il VectorGet_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorGet_ro.dasm.il VectorGet_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorGet_ro.asm.exe  VectorGet_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorGet_ro.asm.exe  VectorGet_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorGet_ro
    if not exist "VectorGet_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorGet_ro.org" (
            mkdir IL
            copy VectorGet_ro.exe IL\VectorGet_ro.exe 
            ren VectorGet_ro.exe VectorGet_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorGet_ro.org /out VectorGet_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorGet_ro.org /out VectorGet_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorGet_ro.reflect.xml
    IF EXIST VectorGet_ro.reflect.xml set ReflectionRoots=-x VectorGet_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorGet_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorGet_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorGet_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
