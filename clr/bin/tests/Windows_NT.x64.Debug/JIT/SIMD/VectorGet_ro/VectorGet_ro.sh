...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorGet_ro VectorGet_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorGet_ro VectorGet_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorGet_ro.dasm.il VectorGet_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorGet_ro.dasm.il VectorGet_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorGet_ro.asm.exe  VectorGet_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorGet_ro.asm.exe  VectorGet_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorGet_ro
    if [ ! -f VectorGet_ro.org ]; then
        TakeLock
        if [ ! -f VectorGet_ro.org ]; then
          mkdir IL
          cp VectorGet_ro.exe IL/VectorGet_ro.exe
          mv VectorGet_ro.exe VectorGet_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorGet_ro.org /out VectorGet_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorGet_ro.org /out VectorGet_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorGet_ro.reflect.xml
  if [ -f VectorGet_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorGet_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorGet_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorGet_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorGet_ro.asm.exe $CLRTestExecutionArguments
