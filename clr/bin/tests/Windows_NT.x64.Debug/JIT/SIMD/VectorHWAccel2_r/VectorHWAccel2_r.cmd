...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorHWAccel2_r VectorHWAccel2_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorHWAccel2_r VectorHWAccel2_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorHWAccel2_r.dasm.il VectorHWAccel2_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorHWAccel2_r.dasm.il VectorHWAccel2_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorHWAccel2_r.asm.exe  VectorHWAccel2_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorHWAccel2_r.asm.exe  VectorHWAccel2_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorHWAccel2_r
    if not exist "VectorHWAccel2_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorHWAccel2_r.org" (
            mkdir IL
            copy VectorHWAccel2_r.exe IL\VectorHWAccel2_r.exe 
            ren VectorHWAccel2_r.exe VectorHWAccel2_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorHWAccel2_r.org /out VectorHWAccel2_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorHWAccel2_r.org /out VectorHWAccel2_r.exe
...
set ExePath=VectorHWAccel2_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorHWAccel2_r.reflect.xml
    IF EXIST VectorHWAccel2_r.reflect.xml set ReflectionRoots=-x VectorHWAccel2_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorHWAccel2_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorHWAccel2_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorHWAccel2_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
