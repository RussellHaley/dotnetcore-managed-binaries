...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorHWAccel2_r VectorHWAccel2_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorHWAccel2_r VectorHWAccel2_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorHWAccel2_r.dasm.il VectorHWAccel2_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorHWAccel2_r.dasm.il VectorHWAccel2_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorHWAccel2_r.asm.exe  VectorHWAccel2_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorHWAccel2_r.asm.exe  VectorHWAccel2_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorHWAccel2_r
    if [ ! -f VectorHWAccel2_r.org ]; then
        TakeLock
        if [ ! -f VectorHWAccel2_r.org ]; then
          mkdir IL
          cp VectorHWAccel2_r.exe IL/VectorHWAccel2_r.exe
          mv VectorHWAccel2_r.exe VectorHWAccel2_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorHWAccel2_r.org /out VectorHWAccel2_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorHWAccel2_r.org /out VectorHWAccel2_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorHWAccel2_r.reflect.xml
  if [ -f VectorHWAccel2_r.reflect.xml ]
then
    ReflectionRoots="-x VectorHWAccel2_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorHWAccel2_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorHWAccel2_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorHWAccel2_r.asm.exe $CLRTestExecutionArguments
