...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AbsSqrt_r AbsSqrt_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AbsSqrt_r AbsSqrt_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbsSqrt_r.dasm.il AbsSqrt_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbsSqrt_r.dasm.il AbsSqrt_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbsSqrt_r.asm.exe  AbsSqrt_r.dasm.il
    "$CORE_ROOT/ilasm" -output=AbsSqrt_r.asm.exe  AbsSqrt_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbsSqrt_r
    if [ ! -f AbsSqrt_r.org ]; then
        TakeLock
        if [ ! -f AbsSqrt_r.org ]; then
          mkdir IL
          cp AbsSqrt_r.exe IL/AbsSqrt_r.exe
          mv AbsSqrt_r.exe AbsSqrt_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbsSqrt_r.org /out AbsSqrt_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbsSqrt_r.org /out AbsSqrt_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbsSqrt_r.reflect.xml
  if [ -f AbsSqrt_r.reflect.xml ]
then
    ReflectionRoots="-x AbsSqrt_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbsSqrt_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbsSqrt_r.asm.exe $CLRTestExecutionArguments
