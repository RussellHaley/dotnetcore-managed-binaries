...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AbsSqrt_r AbsSqrt_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AbsSqrt_r AbsSqrt_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbsSqrt_r.dasm.il AbsSqrt_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbsSqrt_r.dasm.il AbsSqrt_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbsSqrt_r.asm.exe  AbsSqrt_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbsSqrt_r.asm.exe  AbsSqrt_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbsSqrt_r
    if not exist "AbsSqrt_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbsSqrt_r.org" (
            mkdir IL
            copy AbsSqrt_r.exe IL\AbsSqrt_r.exe 
            ren AbsSqrt_r.exe AbsSqrt_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbsSqrt_r.org /out AbsSqrt_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbsSqrt_r.org /out AbsSqrt_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbsSqrt_r.reflect.xml
    IF EXIST AbsSqrt_r.reflect.xml set ReflectionRoots=-x AbsSqrt_r.reflect.xml
    ...
    set ExePath=%LinkBin%\AbsSqrt_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbsSqrt_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbsSqrt_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
