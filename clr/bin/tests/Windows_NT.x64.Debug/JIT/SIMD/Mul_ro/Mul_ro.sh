...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Mul_ro Mul_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Mul_ro Mul_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Mul_ro.dasm.il Mul_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Mul_ro.dasm.il Mul_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Mul_ro.asm.exe  Mul_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Mul_ro.asm.exe  Mul_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Mul_ro
    if [ ! -f Mul_ro.org ]; then
        TakeLock
        if [ ! -f Mul_ro.org ]; then
          mkdir IL
          cp Mul_ro.exe IL/Mul_ro.exe
          mv Mul_ro.exe Mul_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Mul_ro.org /out Mul_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Mul_ro.org /out Mul_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Mul_ro.reflect.xml
  if [ -f Mul_ro.reflect.xml ]
then
    ReflectionRoots="-x Mul_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Mul_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Mul_ro.asm.exe $CLRTestExecutionArguments
