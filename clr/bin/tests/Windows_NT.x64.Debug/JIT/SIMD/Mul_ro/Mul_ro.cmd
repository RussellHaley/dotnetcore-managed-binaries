...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Mul_ro Mul_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Mul_ro Mul_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Mul_ro.dasm.il Mul_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Mul_ro.dasm.il Mul_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Mul_ro.asm.exe  Mul_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Mul_ro.asm.exe  Mul_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Mul_ro
    if not exist "Mul_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Mul_ro.org" (
            mkdir IL
            copy Mul_ro.exe IL\Mul_ro.exe 
            ren Mul_ro.exe Mul_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Mul_ro.org /out Mul_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Mul_ro.org /out Mul_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Mul_ro.reflect.xml
    IF EXIST Mul_ro.reflect.xml set ReflectionRoots=-x Mul_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Mul_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Mul_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Mul_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
