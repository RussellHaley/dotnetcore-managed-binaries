...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Ctors_ro Ctors_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Ctors_ro Ctors_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Ctors_ro.dasm.il Ctors_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Ctors_ro.dasm.il Ctors_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Ctors_ro.asm.exe  Ctors_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Ctors_ro.asm.exe  Ctors_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Ctors_ro
    if [ ! -f Ctors_ro.org ]; then
        TakeLock
        if [ ! -f Ctors_ro.org ]; then
          mkdir IL
          cp Ctors_ro.exe IL/Ctors_ro.exe
          mv Ctors_ro.exe Ctors_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ctors_ro.org /out Ctors_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ctors_ro.org /out Ctors_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Ctors_ro.reflect.xml
  if [ -f Ctors_ro.reflect.xml ]
then
    ReflectionRoots="-x Ctors_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Ctors_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Ctors_ro.asm.exe $CLRTestExecutionArguments
