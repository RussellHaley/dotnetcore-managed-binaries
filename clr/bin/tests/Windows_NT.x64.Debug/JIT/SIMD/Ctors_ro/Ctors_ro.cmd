...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Ctors_ro Ctors_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Ctors_ro Ctors_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Ctors_ro.dasm.il Ctors_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Ctors_ro.dasm.il Ctors_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Ctors_ro.asm.exe  Ctors_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Ctors_ro.asm.exe  Ctors_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Ctors_ro
    if not exist "Ctors_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Ctors_ro.org" (
            mkdir IL
            copy Ctors_ro.exe IL\Ctors_ro.exe 
            ren Ctors_ro.exe Ctors_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ctors_ro.org /out Ctors_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ctors_ro.org /out Ctors_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Ctors_ro.reflect.xml
    IF EXIST Ctors_ro.reflect.xml set ReflectionRoots=-x Ctors_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Ctors_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Ctors_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Ctors_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
