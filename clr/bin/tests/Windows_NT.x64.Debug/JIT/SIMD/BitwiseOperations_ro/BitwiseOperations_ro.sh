...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/BitwiseOperations_ro BitwiseOperations_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/BitwiseOperations_ro BitwiseOperations_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=BitwiseOperations_ro.dasm.il BitwiseOperations_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=BitwiseOperations_ro.dasm.il BitwiseOperations_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=BitwiseOperations_ro.asm.exe  BitwiseOperations_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=BitwiseOperations_ro.asm.exe  BitwiseOperations_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=BitwiseOperations_ro
    if [ ! -f BitwiseOperations_ro.org ]; then
        TakeLock
        if [ ! -f BitwiseOperations_ro.org ]; then
          mkdir IL
          cp BitwiseOperations_ro.exe IL/BitwiseOperations_ro.exe
          mv BitwiseOperations_ro.exe BitwiseOperations_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BitwiseOperations_ro.org /out BitwiseOperations_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BitwiseOperations_ro.org /out BitwiseOperations_ro.exe
          __cgExitCode=$?
...
ExePath=BitwiseOperations_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in BitwiseOperations_ro.reflect.xml
  if [ -f BitwiseOperations_ro.reflect.xml ]
then
    ReflectionRoots="-x BitwiseOperations_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/BitwiseOperations_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" BitwiseOperations_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" BitwiseOperations_ro.asm.exe $CLRTestExecutionArguments
