...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\BitwiseOperations_ro BitwiseOperations_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\BitwiseOperations_ro BitwiseOperations_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=BitwiseOperations_ro.dasm.il BitwiseOperations_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=BitwiseOperations_ro.dasm.il BitwiseOperations_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=BitwiseOperations_ro.asm.exe  BitwiseOperations_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=BitwiseOperations_ro.asm.exe  BitwiseOperations_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=BitwiseOperations_ro
    if not exist "BitwiseOperations_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "BitwiseOperations_ro.org" (
            mkdir IL
            copy BitwiseOperations_ro.exe IL\BitwiseOperations_ro.exe 
            ren BitwiseOperations_ro.exe BitwiseOperations_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BitwiseOperations_ro.org /out BitwiseOperations_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BitwiseOperations_ro.org /out BitwiseOperations_ro.exe
...
set ExePath=BitwiseOperations_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in BitwiseOperations_ro.reflect.xml
    IF EXIST BitwiseOperations_ro.reflect.xml set ReflectionRoots=-x BitwiseOperations_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\BitwiseOperations_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% BitwiseOperations_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% BitwiseOperations_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
