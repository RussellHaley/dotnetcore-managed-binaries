...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorHWAccel2_ro VectorHWAccel2_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorHWAccel2_ro VectorHWAccel2_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorHWAccel2_ro.dasm.il VectorHWAccel2_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorHWAccel2_ro.dasm.il VectorHWAccel2_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorHWAccel2_ro.asm.exe  VectorHWAccel2_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorHWAccel2_ro.asm.exe  VectorHWAccel2_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorHWAccel2_ro
    if [ ! -f VectorHWAccel2_ro.org ]; then
        TakeLock
        if [ ! -f VectorHWAccel2_ro.org ]; then
          mkdir IL
          cp VectorHWAccel2_ro.exe IL/VectorHWAccel2_ro.exe
          mv VectorHWAccel2_ro.exe VectorHWAccel2_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorHWAccel2_ro.org /out VectorHWAccel2_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorHWAccel2_ro.org /out VectorHWAccel2_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorHWAccel2_ro.reflect.xml
  if [ -f VectorHWAccel2_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorHWAccel2_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorHWAccel2_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorHWAccel2_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorHWAccel2_ro.asm.exe $CLRTestExecutionArguments
