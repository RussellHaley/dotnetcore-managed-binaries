...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorHWAccel2_ro VectorHWAccel2_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorHWAccel2_ro VectorHWAccel2_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorHWAccel2_ro.dasm.il VectorHWAccel2_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorHWAccel2_ro.dasm.il VectorHWAccel2_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorHWAccel2_ro.asm.exe  VectorHWAccel2_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorHWAccel2_ro.asm.exe  VectorHWAccel2_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorHWAccel2_ro
    if not exist "VectorHWAccel2_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorHWAccel2_ro.org" (
            mkdir IL
            copy VectorHWAccel2_ro.exe IL\VectorHWAccel2_ro.exe 
            ren VectorHWAccel2_ro.exe VectorHWAccel2_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorHWAccel2_ro.org /out VectorHWAccel2_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorHWAccel2_ro.org /out VectorHWAccel2_ro.exe
...
set ExePath=VectorHWAccel2_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorHWAccel2_ro.reflect.xml
    IF EXIST VectorHWAccel2_ro.reflect.xml set ReflectionRoots=-x VectorHWAccel2_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorHWAccel2_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorHWAccel2_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorHWAccel2_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
