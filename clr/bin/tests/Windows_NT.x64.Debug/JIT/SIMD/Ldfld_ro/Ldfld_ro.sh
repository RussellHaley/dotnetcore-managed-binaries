...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Ldfld_ro Ldfld_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Ldfld_ro Ldfld_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Ldfld_ro.dasm.il Ldfld_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Ldfld_ro.dasm.il Ldfld_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Ldfld_ro.asm.exe  Ldfld_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Ldfld_ro.asm.exe  Ldfld_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Ldfld_ro
    if [ ! -f Ldfld_ro.org ]; then
        TakeLock
        if [ ! -f Ldfld_ro.org ]; then
          mkdir IL
          cp Ldfld_ro.exe IL/Ldfld_ro.exe
          mv Ldfld_ro.exe Ldfld_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ldfld_ro.org /out Ldfld_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ldfld_ro.org /out Ldfld_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Ldfld_ro.reflect.xml
  if [ -f Ldfld_ro.reflect.xml ]
then
    ReflectionRoots="-x Ldfld_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Ldfld_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Ldfld_ro.asm.exe $CLRTestExecutionArguments
