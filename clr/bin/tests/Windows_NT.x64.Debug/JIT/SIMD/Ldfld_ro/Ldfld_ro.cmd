...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Ldfld_ro Ldfld_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Ldfld_ro Ldfld_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Ldfld_ro.dasm.il Ldfld_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Ldfld_ro.dasm.il Ldfld_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Ldfld_ro.asm.exe  Ldfld_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Ldfld_ro.asm.exe  Ldfld_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Ldfld_ro
    if not exist "Ldfld_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Ldfld_ro.org" (
            mkdir IL
            copy Ldfld_ro.exe IL\Ldfld_ro.exe 
            ren Ldfld_ro.exe Ldfld_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ldfld_ro.org /out Ldfld_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ldfld_ro.org /out Ldfld_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Ldfld_ro.reflect.xml
    IF EXIST Ldfld_ro.reflect.xml set ReflectionRoots=-x Ldfld_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Ldfld_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Ldfld_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Ldfld_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
