...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMul_r VectorMul_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMul_r VectorMul_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorMul_r.dasm.il VectorMul_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorMul_r.dasm.il VectorMul_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorMul_r.asm.exe  VectorMul_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorMul_r.asm.exe  VectorMul_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorMul_r
    if not exist "VectorMul_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorMul_r.org" (
            mkdir IL
            copy VectorMul_r.exe IL\VectorMul_r.exe 
            ren VectorMul_r.exe VectorMul_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMul_r.org /out VectorMul_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMul_r.org /out VectorMul_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorMul_r.reflect.xml
    IF EXIST VectorMul_r.reflect.xml set ReflectionRoots=-x VectorMul_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorMul_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorMul_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorMul_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
