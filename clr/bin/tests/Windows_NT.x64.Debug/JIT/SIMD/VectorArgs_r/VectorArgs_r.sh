...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArgs_r VectorArgs_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArgs_r VectorArgs_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorArgs_r.dasm.il VectorArgs_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorArgs_r.dasm.il VectorArgs_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorArgs_r.asm.exe  VectorArgs_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorArgs_r.asm.exe  VectorArgs_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorArgs_r
    if [ ! -f VectorArgs_r.org ]; then
        TakeLock
        if [ ! -f VectorArgs_r.org ]; then
          mkdir IL
          cp VectorArgs_r.exe IL/VectorArgs_r.exe
          mv VectorArgs_r.exe VectorArgs_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArgs_r.org /out VectorArgs_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArgs_r.org /out VectorArgs_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorArgs_r.reflect.xml
  if [ -f VectorArgs_r.reflect.xml ]
then
    ReflectionRoots="-x VectorArgs_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorArgs_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorArgs_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorArgs_r.asm.exe $CLRTestExecutionArguments
