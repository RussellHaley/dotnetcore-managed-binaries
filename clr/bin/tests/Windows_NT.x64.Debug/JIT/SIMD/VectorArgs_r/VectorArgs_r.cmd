...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArgs_r VectorArgs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArgs_r VectorArgs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorArgs_r.dasm.il VectorArgs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorArgs_r.dasm.il VectorArgs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorArgs_r.asm.exe  VectorArgs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorArgs_r.asm.exe  VectorArgs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorArgs_r
    if not exist "VectorArgs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorArgs_r.org" (
            mkdir IL
            copy VectorArgs_r.exe IL\VectorArgs_r.exe 
            ren VectorArgs_r.exe VectorArgs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArgs_r.org /out VectorArgs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArgs_r.org /out VectorArgs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorArgs_r.reflect.xml
    IF EXIST VectorArgs_r.reflect.xml set ReflectionRoots=-x VectorArgs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorArgs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorArgs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorArgs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
