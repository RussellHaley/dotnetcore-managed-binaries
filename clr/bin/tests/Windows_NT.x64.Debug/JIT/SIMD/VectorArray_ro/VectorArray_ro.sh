...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArray_ro VectorArray_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArray_ro VectorArray_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorArray_ro.dasm.il VectorArray_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorArray_ro.dasm.il VectorArray_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorArray_ro.asm.exe  VectorArray_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorArray_ro.asm.exe  VectorArray_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorArray_ro
    if [ ! -f VectorArray_ro.org ]; then
        TakeLock
        if [ ! -f VectorArray_ro.org ]; then
          mkdir IL
          cp VectorArray_ro.exe IL/VectorArray_ro.exe
          mv VectorArray_ro.exe VectorArray_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArray_ro.org /out VectorArray_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArray_ro.org /out VectorArray_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorArray_ro.reflect.xml
  if [ -f VectorArray_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorArray_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorArray_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorArray_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorArray_ro.asm.exe $CLRTestExecutionArguments
