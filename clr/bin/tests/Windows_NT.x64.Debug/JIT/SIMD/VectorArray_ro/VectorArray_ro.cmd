...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArray_ro VectorArray_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArray_ro VectorArray_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorArray_ro.dasm.il VectorArray_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorArray_ro.dasm.il VectorArray_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorArray_ro.asm.exe  VectorArray_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorArray_ro.asm.exe  VectorArray_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorArray_ro
    if not exist "VectorArray_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorArray_ro.org" (
            mkdir IL
            copy VectorArray_ro.exe IL\VectorArray_ro.exe 
            ren VectorArray_ro.exe VectorArray_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArray_ro.org /out VectorArray_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArray_ro.org /out VectorArray_ro.exe
...
set ExePath=VectorArray_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorArray_ro.reflect.xml
    IF EXIST VectorArray_ro.reflect.xml set ReflectionRoots=-x VectorArray_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorArray_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorArray_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorArray_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
