...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorIntEquals_ro VectorIntEquals_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorIntEquals_ro VectorIntEquals_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorIntEquals_ro.dasm.il VectorIntEquals_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorIntEquals_ro.dasm.il VectorIntEquals_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorIntEquals_ro.asm.exe  VectorIntEquals_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorIntEquals_ro.asm.exe  VectorIntEquals_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorIntEquals_ro
    if [ ! -f VectorIntEquals_ro.org ]; then
        TakeLock
        if [ ! -f VectorIntEquals_ro.org ]; then
          mkdir IL
          cp VectorIntEquals_ro.exe IL/VectorIntEquals_ro.exe
          mv VectorIntEquals_ro.exe VectorIntEquals_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorIntEquals_ro.org /out VectorIntEquals_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorIntEquals_ro.org /out VectorIntEquals_ro.exe
          __cgExitCode=$?
...
ExePath=VectorIntEquals_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in VectorIntEquals_ro.reflect.xml
  if [ -f VectorIntEquals_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorIntEquals_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorIntEquals_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorIntEquals_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorIntEquals_ro.asm.exe $CLRTestExecutionArguments
