...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorIntEquals_ro VectorIntEquals_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorIntEquals_ro VectorIntEquals_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorIntEquals_ro.dasm.il VectorIntEquals_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorIntEquals_ro.dasm.il VectorIntEquals_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorIntEquals_ro.asm.exe  VectorIntEquals_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorIntEquals_ro.asm.exe  VectorIntEquals_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorIntEquals_ro
    if not exist "VectorIntEquals_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorIntEquals_ro.org" (
            mkdir IL
            copy VectorIntEquals_ro.exe IL\VectorIntEquals_ro.exe 
            ren VectorIntEquals_ro.exe VectorIntEquals_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorIntEquals_ro.org /out VectorIntEquals_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorIntEquals_ro.org /out VectorIntEquals_ro.exe
...
set ExePath=VectorIntEquals_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorIntEquals_ro.reflect.xml
    IF EXIST VectorIntEquals_ro.reflect.xml set ReflectionRoots=-x VectorIntEquals_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorIntEquals_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorIntEquals_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorIntEquals_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
