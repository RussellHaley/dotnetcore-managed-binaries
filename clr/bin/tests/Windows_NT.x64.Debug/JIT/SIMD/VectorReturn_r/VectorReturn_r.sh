...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorReturn_r VectorReturn_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorReturn_r VectorReturn_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorReturn_r.dasm.il VectorReturn_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorReturn_r.dasm.il VectorReturn_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorReturn_r.asm.exe  VectorReturn_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorReturn_r.asm.exe  VectorReturn_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorReturn_r
    if [ ! -f VectorReturn_r.org ]; then
        TakeLock
        if [ ! -f VectorReturn_r.org ]; then
          mkdir IL
          cp VectorReturn_r.exe IL/VectorReturn_r.exe
          mv VectorReturn_r.exe VectorReturn_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorReturn_r.org /out VectorReturn_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorReturn_r.org /out VectorReturn_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorReturn_r.reflect.xml
  if [ -f VectorReturn_r.reflect.xml ]
then
    ReflectionRoots="-x VectorReturn_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorReturn_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorReturn_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorReturn_r.asm.exe $CLRTestExecutionArguments
