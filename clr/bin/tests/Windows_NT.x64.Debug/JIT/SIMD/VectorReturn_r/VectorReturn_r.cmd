...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorReturn_r VectorReturn_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorReturn_r VectorReturn_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorReturn_r.dasm.il VectorReturn_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorReturn_r.dasm.il VectorReturn_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorReturn_r.asm.exe  VectorReturn_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorReturn_r.asm.exe  VectorReturn_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorReturn_r
    if not exist "VectorReturn_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorReturn_r.org" (
            mkdir IL
            copy VectorReturn_r.exe IL\VectorReturn_r.exe 
            ren VectorReturn_r.exe VectorReturn_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorReturn_r.org /out VectorReturn_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorReturn_r.org /out VectorReturn_r.exe
...
set ExePath=VectorReturn_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorReturn_r.reflect.xml
    IF EXIST VectorReturn_r.reflect.xml set ReflectionRoots=-x VectorReturn_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorReturn_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorReturn_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorReturn_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
