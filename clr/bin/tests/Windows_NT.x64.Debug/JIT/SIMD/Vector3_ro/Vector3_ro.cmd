...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Vector3_ro Vector3_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Vector3_ro Vector3_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Vector3_ro.dasm.il Vector3_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Vector3_ro.dasm.il Vector3_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Vector3_ro.asm.exe  Vector3_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Vector3_ro.asm.exe  Vector3_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Vector3_ro
    if not exist "Vector3_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Vector3_ro.org" (
            mkdir IL
            copy Vector3_ro.exe IL\Vector3_ro.exe 
            ren Vector3_ro.exe Vector3_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector3_ro.org /out Vector3_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector3_ro.org /out Vector3_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Vector3_ro.reflect.xml
    IF EXIST Vector3_ro.reflect.xml set ReflectionRoots=-x Vector3_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Vector3_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Vector3_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Vector3_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
