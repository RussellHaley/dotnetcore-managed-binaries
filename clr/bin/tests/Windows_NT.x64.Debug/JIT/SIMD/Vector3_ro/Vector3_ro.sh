...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector3_ro Vector3_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector3_ro Vector3_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Vector3_ro.dasm.il Vector3_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Vector3_ro.dasm.il Vector3_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Vector3_ro.asm.exe  Vector3_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Vector3_ro.asm.exe  Vector3_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Vector3_ro
    if [ ! -f Vector3_ro.org ]; then
        TakeLock
        if [ ! -f Vector3_ro.org ]; then
          mkdir IL
          cp Vector3_ro.exe IL/Vector3_ro.exe
          mv Vector3_ro.exe Vector3_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector3_ro.org /out Vector3_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector3_ro.org /out Vector3_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Vector3_ro.reflect.xml
  if [ -f Vector3_ro.reflect.xml ]
then
    ReflectionRoots="-x Vector3_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Vector3_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Vector3_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Vector3_ro.asm.exe $CLRTestExecutionArguments
