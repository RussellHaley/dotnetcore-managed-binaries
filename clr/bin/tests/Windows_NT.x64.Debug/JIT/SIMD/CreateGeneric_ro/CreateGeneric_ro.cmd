...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CreateGeneric_ro CreateGeneric_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CreateGeneric_ro CreateGeneric_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CreateGeneric_ro.dasm.il CreateGeneric_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CreateGeneric_ro.dasm.il CreateGeneric_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CreateGeneric_ro.asm.exe  CreateGeneric_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CreateGeneric_ro.asm.exe  CreateGeneric_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CreateGeneric_ro
    if not exist "CreateGeneric_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CreateGeneric_ro.org" (
            mkdir IL
            copy CreateGeneric_ro.exe IL\CreateGeneric_ro.exe 
            ren CreateGeneric_ro.exe CreateGeneric_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CreateGeneric_ro.org /out CreateGeneric_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CreateGeneric_ro.org /out CreateGeneric_ro.exe
...
set ExePath=CreateGeneric_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CreateGeneric_ro.reflect.xml
    IF EXIST CreateGeneric_ro.reflect.xml set ReflectionRoots=-x CreateGeneric_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\CreateGeneric_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CreateGeneric_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CreateGeneric_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
