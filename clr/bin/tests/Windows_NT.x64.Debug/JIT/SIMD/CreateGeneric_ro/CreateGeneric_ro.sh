...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CreateGeneric_ro CreateGeneric_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CreateGeneric_ro CreateGeneric_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CreateGeneric_ro.dasm.il CreateGeneric_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=CreateGeneric_ro.dasm.il CreateGeneric_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CreateGeneric_ro.asm.exe  CreateGeneric_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=CreateGeneric_ro.asm.exe  CreateGeneric_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CreateGeneric_ro
    if [ ! -f CreateGeneric_ro.org ]; then
        TakeLock
        if [ ! -f CreateGeneric_ro.org ]; then
          mkdir IL
          cp CreateGeneric_ro.exe IL/CreateGeneric_ro.exe
          mv CreateGeneric_ro.exe CreateGeneric_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CreateGeneric_ro.org /out CreateGeneric_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CreateGeneric_ro.org /out CreateGeneric_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CreateGeneric_ro.reflect.xml
  if [ -f CreateGeneric_ro.reflect.xml ]
then
    ReflectionRoots="-x CreateGeneric_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CreateGeneric_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CreateGeneric_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CreateGeneric_ro.asm.exe $CLRTestExecutionArguments
