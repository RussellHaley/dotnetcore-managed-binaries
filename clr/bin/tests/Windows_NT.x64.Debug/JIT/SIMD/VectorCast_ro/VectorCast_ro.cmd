...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorCast_ro VectorCast_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorCast_ro VectorCast_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorCast_ro.dasm.il VectorCast_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorCast_ro.dasm.il VectorCast_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorCast_ro.asm.exe  VectorCast_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorCast_ro.asm.exe  VectorCast_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorCast_ro
    if not exist "VectorCast_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorCast_ro.org" (
            mkdir IL
            copy VectorCast_ro.exe IL\VectorCast_ro.exe 
            ren VectorCast_ro.exe VectorCast_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorCast_ro.org /out VectorCast_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorCast_ro.org /out VectorCast_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorCast_ro.reflect.xml
    IF EXIST VectorCast_ro.reflect.xml set ReflectionRoots=-x VectorCast_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorCast_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorCast_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorCast_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
