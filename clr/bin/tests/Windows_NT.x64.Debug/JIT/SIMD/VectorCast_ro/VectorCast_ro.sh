...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorCast_ro VectorCast_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorCast_ro VectorCast_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorCast_ro.dasm.il VectorCast_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorCast_ro.dasm.il VectorCast_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorCast_ro.asm.exe  VectorCast_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorCast_ro.asm.exe  VectorCast_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorCast_ro
    if [ ! -f VectorCast_ro.org ]; then
        TakeLock
        if [ ! -f VectorCast_ro.org ]; then
          mkdir IL
          cp VectorCast_ro.exe IL/VectorCast_ro.exe
          mv VectorCast_ro.exe VectorCast_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorCast_ro.org /out VectorCast_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorCast_ro.org /out VectorCast_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorCast_ro.reflect.xml
  if [ -f VectorCast_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorCast_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorCast_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorCast_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorCast_ro.asm.exe $CLRTestExecutionArguments
