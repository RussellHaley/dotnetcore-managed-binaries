...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorExp_ro VectorExp_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorExp_ro VectorExp_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorExp_ro.dasm.il VectorExp_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorExp_ro.dasm.il VectorExp_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorExp_ro.asm.exe  VectorExp_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorExp_ro.asm.exe  VectorExp_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorExp_ro
    if not exist "VectorExp_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorExp_ro.org" (
            mkdir IL
            copy VectorExp_ro.exe IL\VectorExp_ro.exe 
            ren VectorExp_ro.exe VectorExp_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorExp_ro.org /out VectorExp_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorExp_ro.org /out VectorExp_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorExp_ro.reflect.xml
    IF EXIST VectorExp_ro.reflect.xml set ReflectionRoots=-x VectorExp_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorExp_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorExp_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorExp_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
