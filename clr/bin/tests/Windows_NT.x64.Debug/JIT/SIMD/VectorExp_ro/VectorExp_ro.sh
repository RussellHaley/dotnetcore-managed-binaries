...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorExp_ro VectorExp_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorExp_ro VectorExp_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorExp_ro.dasm.il VectorExp_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorExp_ro.dasm.il VectorExp_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorExp_ro.asm.exe  VectorExp_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorExp_ro.asm.exe  VectorExp_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorExp_ro
    if [ ! -f VectorExp_ro.org ]; then
        TakeLock
        if [ ! -f VectorExp_ro.org ]; then
          mkdir IL
          cp VectorExp_ro.exe IL/VectorExp_ro.exe
          mv VectorExp_ro.exe VectorExp_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorExp_ro.org /out VectorExp_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorExp_ro.org /out VectorExp_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorExp_ro.reflect.xml
  if [ -f VectorExp_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorExp_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorExp_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorExp_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorExp_ro.asm.exe $CLRTestExecutionArguments
