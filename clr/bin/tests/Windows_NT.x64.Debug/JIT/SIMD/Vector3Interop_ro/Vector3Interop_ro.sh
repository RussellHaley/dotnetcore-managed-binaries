...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector3Interop_ro Vector3Interop_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector3Interop_ro Vector3Interop_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Vector3Interop_ro.dasm.il Vector3Interop_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Vector3Interop_ro.dasm.il Vector3Interop_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Vector3Interop_ro.asm.exe  Vector3Interop_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Vector3Interop_ro.asm.exe  Vector3Interop_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Vector3Interop_ro
    if [ ! -f Vector3Interop_ro.org ]; then
        TakeLock
        if [ ! -f Vector3Interop_ro.org ]; then
          mkdir IL
          cp Vector3Interop_ro.exe IL/Vector3Interop_ro.exe
          mv Vector3Interop_ro.exe Vector3Interop_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector3Interop_ro.org /out Vector3Interop_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector3Interop_ro.org /out Vector3Interop_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Vector3Interop_ro.reflect.xml
  if [ -f Vector3Interop_ro.reflect.xml ]
then
    ReflectionRoots="-x Vector3Interop_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Vector3Interop_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Vector3Interop_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Vector3Interop_ro.asm.exe $CLRTestExecutionArguments
