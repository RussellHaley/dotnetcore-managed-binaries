...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Vector3Interop_ro Vector3Interop_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Vector3Interop_ro Vector3Interop_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Vector3Interop_ro.dasm.il Vector3Interop_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Vector3Interop_ro.dasm.il Vector3Interop_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Vector3Interop_ro.asm.exe  Vector3Interop_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Vector3Interop_ro.asm.exe  Vector3Interop_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Vector3Interop_ro
    if not exist "Vector3Interop_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Vector3Interop_ro.org" (
            mkdir IL
            copy Vector3Interop_ro.exe IL\Vector3Interop_ro.exe 
            ren Vector3Interop_ro.exe Vector3Interop_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector3Interop_ro.org /out Vector3Interop_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector3Interop_ro.org /out Vector3Interop_ro.exe
...
set ExePath=Vector3Interop_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Vector3Interop_ro.reflect.xml
    IF EXIST Vector3Interop_ro.reflect.xml set ReflectionRoots=-x Vector3Interop_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Vector3Interop_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Vector3Interop_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Vector3Interop_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
