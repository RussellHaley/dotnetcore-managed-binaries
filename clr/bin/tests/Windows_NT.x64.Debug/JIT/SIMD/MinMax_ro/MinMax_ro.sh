...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/MinMax_ro MinMax_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/MinMax_ro MinMax_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MinMax_ro.dasm.il MinMax_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=MinMax_ro.dasm.il MinMax_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MinMax_ro.asm.exe  MinMax_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=MinMax_ro.asm.exe  MinMax_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MinMax_ro
    if [ ! -f MinMax_ro.org ]; then
        TakeLock
        if [ ! -f MinMax_ro.org ]; then
          mkdir IL
          cp MinMax_ro.exe IL/MinMax_ro.exe
          mv MinMax_ro.exe MinMax_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MinMax_ro.org /out MinMax_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MinMax_ro.org /out MinMax_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in MinMax_ro.reflect.xml
  if [ -f MinMax_ro.reflect.xml ]
then
    ReflectionRoots="-x MinMax_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MinMax_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MinMax_ro.asm.exe $CLRTestExecutionArguments
