...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\MinMax_ro MinMax_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\MinMax_ro MinMax_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MinMax_ro.dasm.il MinMax_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MinMax_ro.dasm.il MinMax_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MinMax_ro.asm.exe  MinMax_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MinMax_ro.asm.exe  MinMax_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MinMax_ro
    if not exist "MinMax_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MinMax_ro.org" (
            mkdir IL
            copy MinMax_ro.exe IL\MinMax_ro.exe 
            ren MinMax_ro.exe MinMax_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MinMax_ro.org /out MinMax_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MinMax_ro.org /out MinMax_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MinMax_ro.reflect.xml
    IF EXIST MinMax_ro.reflect.xml set ReflectionRoots=-x MinMax_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\MinMax_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MinMax_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MinMax_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
