...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorUnused_ro VectorUnused_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorUnused_ro VectorUnused_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorUnused_ro.dasm.il VectorUnused_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorUnused_ro.dasm.il VectorUnused_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorUnused_ro.asm.exe  VectorUnused_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorUnused_ro.asm.exe  VectorUnused_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorUnused_ro
    if [ ! -f VectorUnused_ro.org ]; then
        TakeLock
        if [ ! -f VectorUnused_ro.org ]; then
          mkdir IL
          cp VectorUnused_ro.exe IL/VectorUnused_ro.exe
          mv VectorUnused_ro.exe VectorUnused_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorUnused_ro.org /out VectorUnused_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorUnused_ro.org /out VectorUnused_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorUnused_ro.reflect.xml
  if [ -f VectorUnused_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorUnused_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorUnused_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorUnused_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorUnused_ro.asm.exe $CLRTestExecutionArguments
