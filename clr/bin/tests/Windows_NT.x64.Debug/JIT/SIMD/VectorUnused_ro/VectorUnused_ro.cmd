...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorUnused_ro VectorUnused_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorUnused_ro VectorUnused_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorUnused_ro.dasm.il VectorUnused_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorUnused_ro.dasm.il VectorUnused_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorUnused_ro.asm.exe  VectorUnused_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorUnused_ro.asm.exe  VectorUnused_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorUnused_ro
    if not exist "VectorUnused_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorUnused_ro.org" (
            mkdir IL
            copy VectorUnused_ro.exe IL\VectorUnused_ro.exe 
            ren VectorUnused_ro.exe VectorUnused_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorUnused_ro.org /out VectorUnused_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorUnused_ro.org /out VectorUnused_ro.exe
...
set ExePath=VectorUnused_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorUnused_ro.reflect.xml
    IF EXIST VectorUnused_ro.reflect.xml set ReflectionRoots=-x VectorUnused_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorUnused_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorUnused_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorUnused_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
