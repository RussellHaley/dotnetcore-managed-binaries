...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/SqrtGeneric_r SqrtGeneric_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/SqrtGeneric_r SqrtGeneric_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=SqrtGeneric_r.dasm.il SqrtGeneric_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=SqrtGeneric_r.dasm.il SqrtGeneric_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=SqrtGeneric_r.asm.exe  SqrtGeneric_r.dasm.il
    "$CORE_ROOT/ilasm" -output=SqrtGeneric_r.asm.exe  SqrtGeneric_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=SqrtGeneric_r
    if [ ! -f SqrtGeneric_r.org ]; then
        TakeLock
        if [ ! -f SqrtGeneric_r.org ]; then
          mkdir IL
          cp SqrtGeneric_r.exe IL/SqrtGeneric_r.exe
          mv SqrtGeneric_r.exe SqrtGeneric_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SqrtGeneric_r.org /out SqrtGeneric_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SqrtGeneric_r.org /out SqrtGeneric_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in SqrtGeneric_r.reflect.xml
  if [ -f SqrtGeneric_r.reflect.xml ]
then
    ReflectionRoots="-x SqrtGeneric_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/SqrtGeneric_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" SqrtGeneric_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" SqrtGeneric_r.asm.exe $CLRTestExecutionArguments
