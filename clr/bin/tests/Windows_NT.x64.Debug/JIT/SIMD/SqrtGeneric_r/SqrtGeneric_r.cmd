...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\SqrtGeneric_r SqrtGeneric_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\SqrtGeneric_r SqrtGeneric_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=SqrtGeneric_r.dasm.il SqrtGeneric_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=SqrtGeneric_r.dasm.il SqrtGeneric_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=SqrtGeneric_r.asm.exe  SqrtGeneric_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=SqrtGeneric_r.asm.exe  SqrtGeneric_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=SqrtGeneric_r
    if not exist "SqrtGeneric_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "SqrtGeneric_r.org" (
            mkdir IL
            copy SqrtGeneric_r.exe IL\SqrtGeneric_r.exe 
            ren SqrtGeneric_r.exe SqrtGeneric_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SqrtGeneric_r.org /out SqrtGeneric_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SqrtGeneric_r.org /out SqrtGeneric_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in SqrtGeneric_r.reflect.xml
    IF EXIST SqrtGeneric_r.reflect.xml set ReflectionRoots=-x SqrtGeneric_r.reflect.xml
    ...
    set ExePath=%LinkBin%\SqrtGeneric_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% SqrtGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% SqrtGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
