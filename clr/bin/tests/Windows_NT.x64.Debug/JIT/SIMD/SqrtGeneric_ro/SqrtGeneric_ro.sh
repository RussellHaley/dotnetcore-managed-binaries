...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/SqrtGeneric_ro SqrtGeneric_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/SqrtGeneric_ro SqrtGeneric_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=SqrtGeneric_ro.dasm.il SqrtGeneric_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=SqrtGeneric_ro.dasm.il SqrtGeneric_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=SqrtGeneric_ro.asm.exe  SqrtGeneric_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=SqrtGeneric_ro.asm.exe  SqrtGeneric_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=SqrtGeneric_ro
    if [ ! -f SqrtGeneric_ro.org ]; then
        TakeLock
        if [ ! -f SqrtGeneric_ro.org ]; then
          mkdir IL
          cp SqrtGeneric_ro.exe IL/SqrtGeneric_ro.exe
          mv SqrtGeneric_ro.exe SqrtGeneric_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SqrtGeneric_ro.org /out SqrtGeneric_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SqrtGeneric_ro.org /out SqrtGeneric_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in SqrtGeneric_ro.reflect.xml
  if [ -f SqrtGeneric_ro.reflect.xml ]
then
    ReflectionRoots="-x SqrtGeneric_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/SqrtGeneric_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" SqrtGeneric_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" SqrtGeneric_ro.asm.exe $CLRTestExecutionArguments
