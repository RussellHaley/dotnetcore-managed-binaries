...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSqrt_ro VectorSqrt_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSqrt_ro VectorSqrt_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorSqrt_ro.dasm.il VectorSqrt_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorSqrt_ro.dasm.il VectorSqrt_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorSqrt_ro.asm.exe  VectorSqrt_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorSqrt_ro.asm.exe  VectorSqrt_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorSqrt_ro
    if not exist "VectorSqrt_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorSqrt_ro.org" (
            mkdir IL
            copy VectorSqrt_ro.exe IL\VectorSqrt_ro.exe 
            ren VectorSqrt_ro.exe VectorSqrt_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSqrt_ro.org /out VectorSqrt_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSqrt_ro.org /out VectorSqrt_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorSqrt_ro.reflect.xml
    IF EXIST VectorSqrt_ro.reflect.xml set ReflectionRoots=-x VectorSqrt_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorSqrt_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorSqrt_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorSqrt_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
