...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSqrt_ro VectorSqrt_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSqrt_ro VectorSqrt_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorSqrt_ro.dasm.il VectorSqrt_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorSqrt_ro.dasm.il VectorSqrt_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorSqrt_ro.asm.exe  VectorSqrt_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorSqrt_ro.asm.exe  VectorSqrt_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorSqrt_ro
    if [ ! -f VectorSqrt_ro.org ]; then
        TakeLock
        if [ ! -f VectorSqrt_ro.org ]; then
          mkdir IL
          cp VectorSqrt_ro.exe IL/VectorSqrt_ro.exe
          mv VectorSqrt_ro.exe VectorSqrt_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSqrt_ro.org /out VectorSqrt_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSqrt_ro.org /out VectorSqrt_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorSqrt_ro.reflect.xml
  if [ -f VectorSqrt_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorSqrt_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorSqrt_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorSqrt_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorSqrt_ro.asm.exe $CLRTestExecutionArguments
