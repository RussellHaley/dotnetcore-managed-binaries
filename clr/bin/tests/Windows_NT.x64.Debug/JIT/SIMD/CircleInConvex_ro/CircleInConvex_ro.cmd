...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CircleInConvex_ro CircleInConvex_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CircleInConvex_ro CircleInConvex_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CircleInConvex_ro.dasm.il CircleInConvex_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CircleInConvex_ro.dasm.il CircleInConvex_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CircleInConvex_ro.asm.exe  CircleInConvex_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CircleInConvex_ro.asm.exe  CircleInConvex_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CircleInConvex_ro
    if not exist "CircleInConvex_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CircleInConvex_ro.org" (
            mkdir IL
            copy CircleInConvex_ro.exe IL\CircleInConvex_ro.exe 
            ren CircleInConvex_ro.exe CircleInConvex_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CircleInConvex_ro.org /out CircleInConvex_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CircleInConvex_ro.org /out CircleInConvex_ro.exe
...
set ExePath=CircleInConvex_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CircleInConvex_ro.reflect.xml
    IF EXIST CircleInConvex_ro.reflect.xml set ReflectionRoots=-x CircleInConvex_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\CircleInConvex_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CircleInConvex_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CircleInConvex_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
