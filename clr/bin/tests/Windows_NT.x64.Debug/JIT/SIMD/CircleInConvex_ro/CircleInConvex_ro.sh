...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CircleInConvex_ro CircleInConvex_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CircleInConvex_ro CircleInConvex_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CircleInConvex_ro.dasm.il CircleInConvex_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=CircleInConvex_ro.dasm.il CircleInConvex_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CircleInConvex_ro.asm.exe  CircleInConvex_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=CircleInConvex_ro.asm.exe  CircleInConvex_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CircleInConvex_ro
    if [ ! -f CircleInConvex_ro.org ]; then
        TakeLock
        if [ ! -f CircleInConvex_ro.org ]; then
          mkdir IL
          cp CircleInConvex_ro.exe IL/CircleInConvex_ro.exe
          mv CircleInConvex_ro.exe CircleInConvex_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CircleInConvex_ro.org /out CircleInConvex_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CircleInConvex_ro.org /out CircleInConvex_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CircleInConvex_ro.reflect.xml
  if [ -f CircleInConvex_ro.reflect.xml ]
then
    ReflectionRoots="-x CircleInConvex_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CircleInConvex_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CircleInConvex_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CircleInConvex_ro.asm.exe $CLRTestExecutionArguments
