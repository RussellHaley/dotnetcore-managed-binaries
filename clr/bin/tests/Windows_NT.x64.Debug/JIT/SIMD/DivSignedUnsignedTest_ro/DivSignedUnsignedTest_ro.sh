...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/DivSignedUnsignedTest_ro DivSignedUnsignedTest_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/DivSignedUnsignedTest_ro DivSignedUnsignedTest_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DivSignedUnsignedTest_ro.dasm.il DivSignedUnsignedTest_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=DivSignedUnsignedTest_ro.dasm.il DivSignedUnsignedTest_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DivSignedUnsignedTest_ro.asm.exe  DivSignedUnsignedTest_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=DivSignedUnsignedTest_ro.asm.exe  DivSignedUnsignedTest_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DivSignedUnsignedTest_ro
    if [ ! -f DivSignedUnsignedTest_ro.org ]; then
        TakeLock
        if [ ! -f DivSignedUnsignedTest_ro.org ]; then
          mkdir IL
          cp DivSignedUnsignedTest_ro.exe IL/DivSignedUnsignedTest_ro.exe
          mv DivSignedUnsignedTest_ro.exe DivSignedUnsignedTest_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DivSignedUnsignedTest_ro.org /out DivSignedUnsignedTest_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DivSignedUnsignedTest_ro.org /out DivSignedUnsignedTest_ro.exe
          __cgExitCode=$?
...
ExePath=DivSignedUnsignedTest_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in DivSignedUnsignedTest_ro.reflect.xml
  if [ -f DivSignedUnsignedTest_ro.reflect.xml ]
then
    ReflectionRoots="-x DivSignedUnsignedTest_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DivSignedUnsignedTest_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DivSignedUnsignedTest_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DivSignedUnsignedTest_ro.asm.exe $CLRTestExecutionArguments
