...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorUnused_r VectorUnused_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorUnused_r VectorUnused_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorUnused_r.dasm.il VectorUnused_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorUnused_r.dasm.il VectorUnused_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorUnused_r.asm.exe  VectorUnused_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorUnused_r.asm.exe  VectorUnused_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorUnused_r
    if [ ! -f VectorUnused_r.org ]; then
        TakeLock
        if [ ! -f VectorUnused_r.org ]; then
          mkdir IL
          cp VectorUnused_r.exe IL/VectorUnused_r.exe
          mv VectorUnused_r.exe VectorUnused_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorUnused_r.org /out VectorUnused_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorUnused_r.org /out VectorUnused_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorUnused_r.reflect.xml
  if [ -f VectorUnused_r.reflect.xml ]
then
    ReflectionRoots="-x VectorUnused_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorUnused_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorUnused_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorUnused_r.asm.exe $CLRTestExecutionArguments
