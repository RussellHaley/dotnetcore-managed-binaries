...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorUnused_r VectorUnused_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorUnused_r VectorUnused_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorUnused_r.dasm.il VectorUnused_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorUnused_r.dasm.il VectorUnused_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorUnused_r.asm.exe  VectorUnused_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorUnused_r.asm.exe  VectorUnused_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorUnused_r
    if not exist "VectorUnused_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorUnused_r.org" (
            mkdir IL
            copy VectorUnused_r.exe IL\VectorUnused_r.exe 
            ren VectorUnused_r.exe VectorUnused_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorUnused_r.org /out VectorUnused_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorUnused_r.org /out VectorUnused_r.exe
...
set ExePath=VectorUnused_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorUnused_r.reflect.xml
    IF EXIST VectorUnused_r.reflect.xml set ReflectionRoots=-x VectorUnused_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorUnused_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorUnused_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorUnused_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
