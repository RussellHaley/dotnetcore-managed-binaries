...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorHWAccel_r VectorHWAccel_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorHWAccel_r VectorHWAccel_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorHWAccel_r.dasm.il VectorHWAccel_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorHWAccel_r.dasm.il VectorHWAccel_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorHWAccel_r.asm.exe  VectorHWAccel_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorHWAccel_r.asm.exe  VectorHWAccel_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorHWAccel_r
    if not exist "VectorHWAccel_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorHWAccel_r.org" (
            mkdir IL
            copy VectorHWAccel_r.exe IL\VectorHWAccel_r.exe 
            ren VectorHWAccel_r.exe VectorHWAccel_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorHWAccel_r.org /out VectorHWAccel_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorHWAccel_r.org /out VectorHWAccel_r.exe
...
set ExePath=VectorHWAccel_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorHWAccel_r.reflect.xml
    IF EXIST VectorHWAccel_r.reflect.xml set ReflectionRoots=-x VectorHWAccel_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorHWAccel_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorHWAccel_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorHWAccel_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
