...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorHWAccel_r VectorHWAccel_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorHWAccel_r VectorHWAccel_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorHWAccel_r.dasm.il VectorHWAccel_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorHWAccel_r.dasm.il VectorHWAccel_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorHWAccel_r.asm.exe  VectorHWAccel_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorHWAccel_r.asm.exe  VectorHWAccel_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorHWAccel_r
    if [ ! -f VectorHWAccel_r.org ]; then
        TakeLock
        if [ ! -f VectorHWAccel_r.org ]; then
          mkdir IL
          cp VectorHWAccel_r.exe IL/VectorHWAccel_r.exe
          mv VectorHWAccel_r.exe VectorHWAccel_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorHWAccel_r.org /out VectorHWAccel_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorHWAccel_r.org /out VectorHWAccel_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorHWAccel_r.reflect.xml
  if [ -f VectorHWAccel_r.reflect.xml ]
then
    ReflectionRoots="-x VectorHWAccel_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorHWAccel_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorHWAccel_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorHWAccel_r.asm.exe $CLRTestExecutionArguments
