...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorCopyToArray_ro VectorCopyToArray_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorCopyToArray_ro VectorCopyToArray_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorCopyToArray_ro.dasm.il VectorCopyToArray_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorCopyToArray_ro.dasm.il VectorCopyToArray_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorCopyToArray_ro.asm.exe  VectorCopyToArray_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorCopyToArray_ro.asm.exe  VectorCopyToArray_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorCopyToArray_ro
    if [ ! -f VectorCopyToArray_ro.org ]; then
        TakeLock
        if [ ! -f VectorCopyToArray_ro.org ]; then
          mkdir IL
          cp VectorCopyToArray_ro.exe IL/VectorCopyToArray_ro.exe
          mv VectorCopyToArray_ro.exe VectorCopyToArray_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorCopyToArray_ro.org /out VectorCopyToArray_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorCopyToArray_ro.org /out VectorCopyToArray_ro.exe
          __cgExitCode=$?
...
ExePath=VectorCopyToArray_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in VectorCopyToArray_ro.reflect.xml
  if [ -f VectorCopyToArray_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorCopyToArray_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorCopyToArray_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorCopyToArray_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorCopyToArray_ro.asm.exe $CLRTestExecutionArguments
