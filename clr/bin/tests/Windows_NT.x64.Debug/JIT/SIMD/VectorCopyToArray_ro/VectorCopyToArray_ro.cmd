...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorCopyToArray_ro VectorCopyToArray_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorCopyToArray_ro VectorCopyToArray_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorCopyToArray_ro.dasm.il VectorCopyToArray_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorCopyToArray_ro.dasm.il VectorCopyToArray_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorCopyToArray_ro.asm.exe  VectorCopyToArray_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorCopyToArray_ro.asm.exe  VectorCopyToArray_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorCopyToArray_ro
    if not exist "VectorCopyToArray_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorCopyToArray_ro.org" (
            mkdir IL
            copy VectorCopyToArray_ro.exe IL\VectorCopyToArray_ro.exe 
            ren VectorCopyToArray_ro.exe VectorCopyToArray_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorCopyToArray_ro.org /out VectorCopyToArray_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorCopyToArray_ro.org /out VectorCopyToArray_ro.exe
...
set ExePath=VectorCopyToArray_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorCopyToArray_ro.reflect.xml
    IF EXIST VectorCopyToArray_ro.reflect.xml set ReflectionRoots=-x VectorCopyToArray_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorCopyToArray_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorCopyToArray_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorCopyToArray_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
