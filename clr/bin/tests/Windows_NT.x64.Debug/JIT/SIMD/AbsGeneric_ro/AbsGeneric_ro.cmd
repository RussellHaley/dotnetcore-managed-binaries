...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AbsGeneric_ro AbsGeneric_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AbsGeneric_ro AbsGeneric_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbsGeneric_ro.dasm.il AbsGeneric_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbsGeneric_ro.dasm.il AbsGeneric_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbsGeneric_ro.asm.exe  AbsGeneric_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbsGeneric_ro.asm.exe  AbsGeneric_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbsGeneric_ro
    if not exist "AbsGeneric_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbsGeneric_ro.org" (
            mkdir IL
            copy AbsGeneric_ro.exe IL\AbsGeneric_ro.exe 
            ren AbsGeneric_ro.exe AbsGeneric_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbsGeneric_ro.org /out AbsGeneric_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbsGeneric_ro.org /out AbsGeneric_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbsGeneric_ro.reflect.xml
    IF EXIST AbsGeneric_ro.reflect.xml set ReflectionRoots=-x AbsGeneric_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\AbsGeneric_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbsGeneric_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbsGeneric_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
