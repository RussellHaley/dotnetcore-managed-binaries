...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AbsGeneric_ro AbsGeneric_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AbsGeneric_ro AbsGeneric_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbsGeneric_ro.dasm.il AbsGeneric_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbsGeneric_ro.dasm.il AbsGeneric_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbsGeneric_ro.asm.exe  AbsGeneric_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=AbsGeneric_ro.asm.exe  AbsGeneric_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbsGeneric_ro
    if [ ! -f AbsGeneric_ro.org ]; then
        TakeLock
        if [ ! -f AbsGeneric_ro.org ]; then
          mkdir IL
          cp AbsGeneric_ro.exe IL/AbsGeneric_ro.exe
          mv AbsGeneric_ro.exe AbsGeneric_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbsGeneric_ro.org /out AbsGeneric_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbsGeneric_ro.org /out AbsGeneric_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbsGeneric_ro.reflect.xml
  if [ -f AbsGeneric_ro.reflect.xml ]
then
    ReflectionRoots="-x AbsGeneric_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AbsGeneric_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbsGeneric_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbsGeneric_ro.asm.exe $CLRTestExecutionArguments
