...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorHWAccel_ro VectorHWAccel_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorHWAccel_ro VectorHWAccel_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorHWAccel_ro.dasm.il VectorHWAccel_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorHWAccel_ro.dasm.il VectorHWAccel_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorHWAccel_ro.asm.exe  VectorHWAccel_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorHWAccel_ro.asm.exe  VectorHWAccel_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorHWAccel_ro
    if [ ! -f VectorHWAccel_ro.org ]; then
        TakeLock
        if [ ! -f VectorHWAccel_ro.org ]; then
          mkdir IL
          cp VectorHWAccel_ro.exe IL/VectorHWAccel_ro.exe
          mv VectorHWAccel_ro.exe VectorHWAccel_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorHWAccel_ro.org /out VectorHWAccel_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorHWAccel_ro.org /out VectorHWAccel_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorHWAccel_ro.reflect.xml
  if [ -f VectorHWAccel_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorHWAccel_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorHWAccel_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorHWAccel_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorHWAccel_ro.asm.exe $CLRTestExecutionArguments
