...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorHWAccel_ro VectorHWAccel_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorHWAccel_ro VectorHWAccel_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorHWAccel_ro.dasm.il VectorHWAccel_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorHWAccel_ro.dasm.il VectorHWAccel_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorHWAccel_ro.asm.exe  VectorHWAccel_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorHWAccel_ro.asm.exe  VectorHWAccel_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorHWAccel_ro
    if not exist "VectorHWAccel_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorHWAccel_ro.org" (
            mkdir IL
            copy VectorHWAccel_ro.exe IL\VectorHWAccel_ro.exe 
            ren VectorHWAccel_ro.exe VectorHWAccel_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorHWAccel_ro.org /out VectorHWAccel_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorHWAccel_ro.org /out VectorHWAccel_ro.exe
...
set ExePath=VectorHWAccel_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorHWAccel_ro.reflect.xml
    IF EXIST VectorHWAccel_ro.reflect.xml set ReflectionRoots=-x VectorHWAccel_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorHWAccel_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorHWAccel_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorHWAccel_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
