...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Haar-likeFeaturesGeneric_r Haar-likeFeaturesGeneric_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Haar-likeFeaturesGeneric_r Haar-likeFeaturesGeneric_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Haar-likeFeaturesGeneric_r.dasm.il Haar-likeFeaturesGeneric_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Haar-likeFeaturesGeneric_r.dasm.il Haar-likeFeaturesGeneric_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Haar-likeFeaturesGeneric_r.asm.exe  Haar-likeFeaturesGeneric_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Haar-likeFeaturesGeneric_r.asm.exe  Haar-likeFeaturesGeneric_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Haar-likeFeaturesGeneric_r
    if not exist "Haar-likeFeaturesGeneric_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Haar-likeFeaturesGeneric_r.org" (
            mkdir IL
            copy Haar-likeFeaturesGeneric_r.exe IL\Haar-likeFeaturesGeneric_r.exe 
            ren Haar-likeFeaturesGeneric_r.exe Haar-likeFeaturesGeneric_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Haar-likeFeaturesGeneric_r.org /out Haar-likeFeaturesGeneric_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Haar-likeFeaturesGeneric_r.org /out Haar-likeFeaturesGeneric_r.exe
...
set ExePath=Haar-likeFeaturesGeneric_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Haar-likeFeaturesGeneric_r.reflect.xml
    IF EXIST Haar-likeFeaturesGeneric_r.reflect.xml set ReflectionRoots=-x Haar-likeFeaturesGeneric_r.reflect.xml
    ...
    set ExePath=%LinkBin%\Haar-likeFeaturesGeneric_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Haar-likeFeaturesGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Haar-likeFeaturesGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
