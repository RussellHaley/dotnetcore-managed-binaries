...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Haar-likeFeaturesGeneric_r Haar-likeFeaturesGeneric_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Haar-likeFeaturesGeneric_r Haar-likeFeaturesGeneric_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Haar-likeFeaturesGeneric_r.dasm.il Haar-likeFeaturesGeneric_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=Haar-likeFeaturesGeneric_r.dasm.il Haar-likeFeaturesGeneric_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Haar-likeFeaturesGeneric_r.asm.exe  Haar-likeFeaturesGeneric_r.dasm.il
    "$CORE_ROOT/ilasm" -output=Haar-likeFeaturesGeneric_r.asm.exe  Haar-likeFeaturesGeneric_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Haar-likeFeaturesGeneric_r
    if [ ! -f Haar-likeFeaturesGeneric_r.org ]; then
        TakeLock
        if [ ! -f Haar-likeFeaturesGeneric_r.org ]; then
          mkdir IL
          cp Haar-likeFeaturesGeneric_r.exe IL/Haar-likeFeaturesGeneric_r.exe
          mv Haar-likeFeaturesGeneric_r.exe Haar-likeFeaturesGeneric_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Haar-likeFeaturesGeneric_r.org /out Haar-likeFeaturesGeneric_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Haar-likeFeaturesGeneric_r.org /out Haar-likeFeaturesGeneric_r.exe
          __cgExitCode=$?
...
ExePath=Haar-likeFeaturesGeneric_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in Haar-likeFeaturesGeneric_r.reflect.xml
  if [ -f Haar-likeFeaturesGeneric_r.reflect.xml ]
then
    ReflectionRoots="-x Haar-likeFeaturesGeneric_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Haar-likeFeaturesGeneric_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Haar-likeFeaturesGeneric_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Haar-likeFeaturesGeneric_r.asm.exe $CLRTestExecutionArguments
