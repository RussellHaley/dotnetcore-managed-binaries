...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\MinMax_r MinMax_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\MinMax_r MinMax_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MinMax_r.dasm.il MinMax_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MinMax_r.dasm.il MinMax_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MinMax_r.asm.exe  MinMax_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MinMax_r.asm.exe  MinMax_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MinMax_r
    if not exist "MinMax_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MinMax_r.org" (
            mkdir IL
            copy MinMax_r.exe IL\MinMax_r.exe 
            ren MinMax_r.exe MinMax_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MinMax_r.org /out MinMax_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MinMax_r.org /out MinMax_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MinMax_r.reflect.xml
    IF EXIST MinMax_r.reflect.xml set ReflectionRoots=-x MinMax_r.reflect.xml
    ...
    set ExePath=%LinkBin%\MinMax_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MinMax_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MinMax_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
