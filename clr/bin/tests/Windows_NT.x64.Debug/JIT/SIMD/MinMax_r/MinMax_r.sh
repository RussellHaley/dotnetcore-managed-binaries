...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/MinMax_r MinMax_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/MinMax_r MinMax_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MinMax_r.dasm.il MinMax_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=MinMax_r.dasm.il MinMax_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MinMax_r.asm.exe  MinMax_r.dasm.il
    "$CORE_ROOT/ilasm" -output=MinMax_r.asm.exe  MinMax_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MinMax_r
    if [ ! -f MinMax_r.org ]; then
        TakeLock
        if [ ! -f MinMax_r.org ]; then
          mkdir IL
          cp MinMax_r.exe IL/MinMax_r.exe
          mv MinMax_r.exe MinMax_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MinMax_r.org /out MinMax_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MinMax_r.org /out MinMax_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in MinMax_r.reflect.xml
  if [ -f MinMax_r.reflect.xml ]
then
    ReflectionRoots="-x MinMax_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MinMax_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MinMax_r.asm.exe $CLRTestExecutionArguments
