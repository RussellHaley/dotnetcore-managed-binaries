...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CreateGeneric_r CreateGeneric_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CreateGeneric_r CreateGeneric_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CreateGeneric_r.dasm.il CreateGeneric_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=CreateGeneric_r.dasm.il CreateGeneric_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CreateGeneric_r.asm.exe  CreateGeneric_r.dasm.il
    "$CORE_ROOT/ilasm" -output=CreateGeneric_r.asm.exe  CreateGeneric_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CreateGeneric_r
    if [ ! -f CreateGeneric_r.org ]; then
        TakeLock
        if [ ! -f CreateGeneric_r.org ]; then
          mkdir IL
          cp CreateGeneric_r.exe IL/CreateGeneric_r.exe
          mv CreateGeneric_r.exe CreateGeneric_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CreateGeneric_r.org /out CreateGeneric_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CreateGeneric_r.org /out CreateGeneric_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CreateGeneric_r.reflect.xml
  if [ -f CreateGeneric_r.reflect.xml ]
then
    ReflectionRoots="-x CreateGeneric_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CreateGeneric_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CreateGeneric_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CreateGeneric_r.asm.exe $CLRTestExecutionArguments
