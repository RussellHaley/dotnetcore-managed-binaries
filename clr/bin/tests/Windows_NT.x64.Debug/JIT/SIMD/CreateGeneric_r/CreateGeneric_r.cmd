...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CreateGeneric_r CreateGeneric_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CreateGeneric_r CreateGeneric_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CreateGeneric_r.dasm.il CreateGeneric_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CreateGeneric_r.dasm.il CreateGeneric_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CreateGeneric_r.asm.exe  CreateGeneric_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CreateGeneric_r.asm.exe  CreateGeneric_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CreateGeneric_r
    if not exist "CreateGeneric_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CreateGeneric_r.org" (
            mkdir IL
            copy CreateGeneric_r.exe IL\CreateGeneric_r.exe 
            ren CreateGeneric_r.exe CreateGeneric_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CreateGeneric_r.org /out CreateGeneric_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CreateGeneric_r.org /out CreateGeneric_r.exe
...
set ExePath=CreateGeneric_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CreateGeneric_r.reflect.xml
    IF EXIST CreateGeneric_r.reflect.xml set ReflectionRoots=-x CreateGeneric_r.reflect.xml
    ...
    set ExePath=%LinkBin%\CreateGeneric_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CreateGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CreateGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
