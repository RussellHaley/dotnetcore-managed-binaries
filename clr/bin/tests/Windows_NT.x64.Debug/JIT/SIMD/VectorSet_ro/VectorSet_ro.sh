...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSet_ro VectorSet_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSet_ro VectorSet_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorSet_ro.dasm.il VectorSet_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorSet_ro.dasm.il VectorSet_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorSet_ro.asm.exe  VectorSet_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorSet_ro.asm.exe  VectorSet_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorSet_ro
    if [ ! -f VectorSet_ro.org ]; then
        TakeLock
        if [ ! -f VectorSet_ro.org ]; then
          mkdir IL
          cp VectorSet_ro.exe IL/VectorSet_ro.exe
          mv VectorSet_ro.exe VectorSet_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSet_ro.org /out VectorSet_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSet_ro.org /out VectorSet_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorSet_ro.reflect.xml
  if [ -f VectorSet_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorSet_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorSet_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorSet_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorSet_ro.asm.exe $CLRTestExecutionArguments
