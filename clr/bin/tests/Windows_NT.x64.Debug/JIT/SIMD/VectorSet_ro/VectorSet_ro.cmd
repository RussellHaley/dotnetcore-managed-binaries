...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSet_ro VectorSet_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSet_ro VectorSet_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorSet_ro.dasm.il VectorSet_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorSet_ro.dasm.il VectorSet_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorSet_ro.asm.exe  VectorSet_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorSet_ro.asm.exe  VectorSet_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorSet_ro
    if not exist "VectorSet_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorSet_ro.org" (
            mkdir IL
            copy VectorSet_ro.exe IL\VectorSet_ro.exe 
            ren VectorSet_ro.exe VectorSet_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSet_ro.org /out VectorSet_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSet_ro.org /out VectorSet_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorSet_ro.reflect.xml
    IF EXIST VectorSet_ro.reflect.xml set ReflectionRoots=-x VectorSet_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorSet_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorSet_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorSet_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
