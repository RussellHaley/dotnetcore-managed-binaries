...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Sums_ro Sums_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Sums_ro Sums_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Sums_ro.dasm.il Sums_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Sums_ro.dasm.il Sums_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Sums_ro.asm.exe  Sums_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Sums_ro.asm.exe  Sums_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Sums_ro
    if [ ! -f Sums_ro.org ]; then
        TakeLock
        if [ ! -f Sums_ro.org ]; then
          mkdir IL
          cp Sums_ro.exe IL/Sums_ro.exe
          mv Sums_ro.exe Sums_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Sums_ro.org /out Sums_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Sums_ro.org /out Sums_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Sums_ro.reflect.xml
  if [ -f Sums_ro.reflect.xml ]
then
    ReflectionRoots="-x Sums_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Sums_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Sums_ro.asm.exe $CLRTestExecutionArguments
