...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Sums_ro Sums_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Sums_ro Sums_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Sums_ro.dasm.il Sums_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Sums_ro.dasm.il Sums_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Sums_ro.asm.exe  Sums_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Sums_ro.asm.exe  Sums_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Sums_ro
    if not exist "Sums_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Sums_ro.org" (
            mkdir IL
            copy Sums_ro.exe IL\Sums_ro.exe 
            ren Sums_ro.exe Sums_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Sums_ro.org /out Sums_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Sums_ro.org /out Sums_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Sums_ro.reflect.xml
    IF EXIST Sums_ro.reflect.xml set ReflectionRoots=-x Sums_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Sums_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Sums_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Sums_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
