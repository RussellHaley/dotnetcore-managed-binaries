...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorInit_r VectorInit_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorInit_r VectorInit_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorInit_r.dasm.il VectorInit_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorInit_r.dasm.il VectorInit_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorInit_r.asm.exe  VectorInit_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorInit_r.asm.exe  VectorInit_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorInit_r
    if not exist "VectorInit_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorInit_r.org" (
            mkdir IL
            copy VectorInit_r.exe IL\VectorInit_r.exe 
            ren VectorInit_r.exe VectorInit_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorInit_r.org /out VectorInit_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorInit_r.org /out VectorInit_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorInit_r.reflect.xml
    IF EXIST VectorInit_r.reflect.xml set ReflectionRoots=-x VectorInit_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorInit_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorInit_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorInit_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
