...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorInit_r VectorInit_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorInit_r VectorInit_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorInit_r.dasm.il VectorInit_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorInit_r.dasm.il VectorInit_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorInit_r.asm.exe  VectorInit_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorInit_r.asm.exe  VectorInit_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorInit_r
    if [ ! -f VectorInit_r.org ]; then
        TakeLock
        if [ ! -f VectorInit_r.org ]; then
          mkdir IL
          cp VectorInit_r.exe IL/VectorInit_r.exe
          mv VectorInit_r.exe VectorInit_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorInit_r.org /out VectorInit_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorInit_r.org /out VectorInit_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorInit_r.reflect.xml
  if [ -f VectorInit_r.reflect.xml ]
then
    ReflectionRoots="-x VectorInit_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorInit_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorInit_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorInit_r.asm.exe $CLRTestExecutionArguments
