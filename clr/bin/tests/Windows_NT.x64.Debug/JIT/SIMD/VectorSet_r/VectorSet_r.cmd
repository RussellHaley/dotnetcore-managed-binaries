...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSet_r VectorSet_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSet_r VectorSet_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorSet_r.dasm.il VectorSet_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorSet_r.dasm.il VectorSet_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorSet_r.asm.exe  VectorSet_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorSet_r.asm.exe  VectorSet_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorSet_r
    if not exist "VectorSet_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorSet_r.org" (
            mkdir IL
            copy VectorSet_r.exe IL\VectorSet_r.exe 
            ren VectorSet_r.exe VectorSet_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSet_r.org /out VectorSet_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSet_r.org /out VectorSet_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorSet_r.reflect.xml
    IF EXIST VectorSet_r.reflect.xml set ReflectionRoots=-x VectorSet_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorSet_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorSet_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorSet_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
