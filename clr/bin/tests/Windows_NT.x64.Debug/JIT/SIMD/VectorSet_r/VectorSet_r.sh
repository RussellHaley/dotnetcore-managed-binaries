...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSet_r VectorSet_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSet_r VectorSet_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorSet_r.dasm.il VectorSet_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorSet_r.dasm.il VectorSet_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorSet_r.asm.exe  VectorSet_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorSet_r.asm.exe  VectorSet_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorSet_r
    if [ ! -f VectorSet_r.org ]; then
        TakeLock
        if [ ! -f VectorSet_r.org ]; then
          mkdir IL
          cp VectorSet_r.exe IL/VectorSet_r.exe
          mv VectorSet_r.exe VectorSet_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSet_r.org /out VectorSet_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSet_r.org /out VectorSet_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorSet_r.reflect.xml
  if [ -f VectorSet_r.reflect.xml ]
then
    ReflectionRoots="-x VectorSet_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorSet_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorSet_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorSet_r.asm.exe $CLRTestExecutionArguments
