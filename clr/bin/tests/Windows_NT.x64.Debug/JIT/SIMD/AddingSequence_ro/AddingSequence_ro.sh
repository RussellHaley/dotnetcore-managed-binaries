...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AddingSequence_ro AddingSequence_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AddingSequence_ro AddingSequence_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AddingSequence_ro.dasm.il AddingSequence_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=AddingSequence_ro.dasm.il AddingSequence_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AddingSequence_ro.asm.exe  AddingSequence_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=AddingSequence_ro.asm.exe  AddingSequence_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AddingSequence_ro
    if [ ! -f AddingSequence_ro.org ]; then
        TakeLock
        if [ ! -f AddingSequence_ro.org ]; then
          mkdir IL
          cp AddingSequence_ro.exe IL/AddingSequence_ro.exe
          mv AddingSequence_ro.exe AddingSequence_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddingSequence_ro.org /out AddingSequence_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddingSequence_ro.org /out AddingSequence_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AddingSequence_ro.reflect.xml
  if [ -f AddingSequence_ro.reflect.xml ]
then
    ReflectionRoots="-x AddingSequence_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AddingSequence_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AddingSequence_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AddingSequence_ro.asm.exe $CLRTestExecutionArguments
