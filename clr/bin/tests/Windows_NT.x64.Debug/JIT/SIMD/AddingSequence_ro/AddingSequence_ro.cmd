...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AddingSequence_ro AddingSequence_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AddingSequence_ro AddingSequence_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AddingSequence_ro.dasm.il AddingSequence_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AddingSequence_ro.dasm.il AddingSequence_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AddingSequence_ro.asm.exe  AddingSequence_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AddingSequence_ro.asm.exe  AddingSequence_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AddingSequence_ro
    if not exist "AddingSequence_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AddingSequence_ro.org" (
            mkdir IL
            copy AddingSequence_ro.exe IL\AddingSequence_ro.exe 
            ren AddingSequence_ro.exe AddingSequence_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddingSequence_ro.org /out AddingSequence_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddingSequence_ro.org /out AddingSequence_ro.exe
...
set ExePath=AddingSequence_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AddingSequence_ro.reflect.xml
    IF EXIST AddingSequence_ro.reflect.xml set ReflectionRoots=-x AddingSequence_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\AddingSequence_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AddingSequence_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AddingSequence_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
