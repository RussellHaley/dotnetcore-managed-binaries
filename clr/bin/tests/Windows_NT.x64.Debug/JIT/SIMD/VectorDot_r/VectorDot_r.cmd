...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorDot_r VectorDot_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorDot_r VectorDot_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorDot_r.dasm.il VectorDot_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorDot_r.dasm.il VectorDot_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorDot_r.asm.exe  VectorDot_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorDot_r.asm.exe  VectorDot_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorDot_r
    if not exist "VectorDot_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorDot_r.org" (
            mkdir IL
            copy VectorDot_r.exe IL\VectorDot_r.exe 
            ren VectorDot_r.exe VectorDot_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorDot_r.org /out VectorDot_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorDot_r.org /out VectorDot_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorDot_r.reflect.xml
    IF EXIST VectorDot_r.reflect.xml set ReflectionRoots=-x VectorDot_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorDot_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorDot_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorDot_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
