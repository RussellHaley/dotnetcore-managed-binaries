...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorDot_r VectorDot_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorDot_r VectorDot_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorDot_r.dasm.il VectorDot_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorDot_r.dasm.il VectorDot_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorDot_r.asm.exe  VectorDot_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorDot_r.asm.exe  VectorDot_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorDot_r
    if [ ! -f VectorDot_r.org ]; then
        TakeLock
        if [ ! -f VectorDot_r.org ]; then
          mkdir IL
          cp VectorDot_r.exe IL/VectorDot_r.exe
          mv VectorDot_r.exe VectorDot_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorDot_r.org /out VectorDot_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorDot_r.org /out VectorDot_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorDot_r.reflect.xml
  if [ -f VectorDot_r.reflect.xml ]
then
    ReflectionRoots="-x VectorDot_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorDot_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorDot_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorDot_r.asm.exe $CLRTestExecutionArguments
