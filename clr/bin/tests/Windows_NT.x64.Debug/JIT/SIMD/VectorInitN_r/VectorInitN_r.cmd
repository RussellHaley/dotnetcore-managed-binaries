...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorInitN_r VectorInitN_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorInitN_r VectorInitN_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorInitN_r.dasm.il VectorInitN_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorInitN_r.dasm.il VectorInitN_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorInitN_r.asm.exe  VectorInitN_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorInitN_r.asm.exe  VectorInitN_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorInitN_r
    if not exist "VectorInitN_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorInitN_r.org" (
            mkdir IL
            copy VectorInitN_r.exe IL\VectorInitN_r.exe 
            ren VectorInitN_r.exe VectorInitN_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorInitN_r.org /out VectorInitN_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorInitN_r.org /out VectorInitN_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorInitN_r.reflect.xml
    IF EXIST VectorInitN_r.reflect.xml set ReflectionRoots=-x VectorInitN_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorInitN_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorInitN_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorInitN_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
