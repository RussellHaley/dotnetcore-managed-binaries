...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorInitN_r VectorInitN_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorInitN_r VectorInitN_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorInitN_r.dasm.il VectorInitN_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorInitN_r.dasm.il VectorInitN_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorInitN_r.asm.exe  VectorInitN_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorInitN_r.asm.exe  VectorInitN_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorInitN_r
    if [ ! -f VectorInitN_r.org ]; then
        TakeLock
        if [ ! -f VectorInitN_r.org ]; then
          mkdir IL
          cp VectorInitN_r.exe IL/VectorInitN_r.exe
          mv VectorInitN_r.exe VectorInitN_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorInitN_r.org /out VectorInitN_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorInitN_r.org /out VectorInitN_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorInitN_r.reflect.xml
  if [ -f VectorInitN_r.reflect.xml ]
then
    ReflectionRoots="-x VectorInitN_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorInitN_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorInitN_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorInitN_r.asm.exe $CLRTestExecutionArguments
