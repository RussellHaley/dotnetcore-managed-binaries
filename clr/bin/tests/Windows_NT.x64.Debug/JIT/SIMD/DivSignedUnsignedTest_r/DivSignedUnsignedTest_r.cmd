...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\DivSignedUnsignedTest_r DivSignedUnsignedTest_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\DivSignedUnsignedTest_r DivSignedUnsignedTest_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DivSignedUnsignedTest_r.dasm.il DivSignedUnsignedTest_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DivSignedUnsignedTest_r.dasm.il DivSignedUnsignedTest_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DivSignedUnsignedTest_r.asm.exe  DivSignedUnsignedTest_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DivSignedUnsignedTest_r.asm.exe  DivSignedUnsignedTest_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DivSignedUnsignedTest_r
    if not exist "DivSignedUnsignedTest_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DivSignedUnsignedTest_r.org" (
            mkdir IL
            copy DivSignedUnsignedTest_r.exe IL\DivSignedUnsignedTest_r.exe 
            ren DivSignedUnsignedTest_r.exe DivSignedUnsignedTest_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DivSignedUnsignedTest_r.org /out DivSignedUnsignedTest_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DivSignedUnsignedTest_r.org /out DivSignedUnsignedTest_r.exe
...
set ExePath=DivSignedUnsignedTest_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DivSignedUnsignedTest_r.reflect.xml
    IF EXIST DivSignedUnsignedTest_r.reflect.xml set ReflectionRoots=-x DivSignedUnsignedTest_r.reflect.xml
    ...
    set ExePath=%LinkBin%\DivSignedUnsignedTest_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DivSignedUnsignedTest_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DivSignedUnsignedTest_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
