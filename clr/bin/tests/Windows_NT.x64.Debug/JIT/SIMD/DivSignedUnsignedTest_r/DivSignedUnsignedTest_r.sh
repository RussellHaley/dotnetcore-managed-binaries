...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/DivSignedUnsignedTest_r DivSignedUnsignedTest_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/DivSignedUnsignedTest_r DivSignedUnsignedTest_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DivSignedUnsignedTest_r.dasm.il DivSignedUnsignedTest_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=DivSignedUnsignedTest_r.dasm.il DivSignedUnsignedTest_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DivSignedUnsignedTest_r.asm.exe  DivSignedUnsignedTest_r.dasm.il
    "$CORE_ROOT/ilasm" -output=DivSignedUnsignedTest_r.asm.exe  DivSignedUnsignedTest_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DivSignedUnsignedTest_r
    if [ ! -f DivSignedUnsignedTest_r.org ]; then
        TakeLock
        if [ ! -f DivSignedUnsignedTest_r.org ]; then
          mkdir IL
          cp DivSignedUnsignedTest_r.exe IL/DivSignedUnsignedTest_r.exe
          mv DivSignedUnsignedTest_r.exe DivSignedUnsignedTest_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DivSignedUnsignedTest_r.org /out DivSignedUnsignedTest_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DivSignedUnsignedTest_r.org /out DivSignedUnsignedTest_r.exe
          __cgExitCode=$?
...
ExePath=DivSignedUnsignedTest_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in DivSignedUnsignedTest_r.reflect.xml
  if [ -f DivSignedUnsignedTest_r.reflect.xml ]
then
    ReflectionRoots="-x DivSignedUnsignedTest_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DivSignedUnsignedTest_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DivSignedUnsignedTest_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DivSignedUnsignedTest_r.asm.exe $CLRTestExecutionArguments
