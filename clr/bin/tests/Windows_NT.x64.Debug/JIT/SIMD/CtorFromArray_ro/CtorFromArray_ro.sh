...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CtorFromArray_ro CtorFromArray_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CtorFromArray_ro CtorFromArray_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CtorFromArray_ro.dasm.il CtorFromArray_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=CtorFromArray_ro.dasm.il CtorFromArray_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CtorFromArray_ro.asm.exe  CtorFromArray_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=CtorFromArray_ro.asm.exe  CtorFromArray_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CtorFromArray_ro
    if [ ! -f CtorFromArray_ro.org ]; then
        TakeLock
        if [ ! -f CtorFromArray_ro.org ]; then
          mkdir IL
          cp CtorFromArray_ro.exe IL/CtorFromArray_ro.exe
          mv CtorFromArray_ro.exe CtorFromArray_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CtorFromArray_ro.org /out CtorFromArray_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CtorFromArray_ro.org /out CtorFromArray_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CtorFromArray_ro.reflect.xml
  if [ -f CtorFromArray_ro.reflect.xml ]
then
    ReflectionRoots="-x CtorFromArray_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CtorFromArray_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CtorFromArray_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CtorFromArray_ro.asm.exe $CLRTestExecutionArguments
