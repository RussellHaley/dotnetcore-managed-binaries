...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CtorFromArray_ro CtorFromArray_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CtorFromArray_ro CtorFromArray_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CtorFromArray_ro.dasm.il CtorFromArray_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CtorFromArray_ro.dasm.il CtorFromArray_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CtorFromArray_ro.asm.exe  CtorFromArray_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CtorFromArray_ro.asm.exe  CtorFromArray_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CtorFromArray_ro
    if not exist "CtorFromArray_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CtorFromArray_ro.org" (
            mkdir IL
            copy CtorFromArray_ro.exe IL\CtorFromArray_ro.exe 
            ren CtorFromArray_ro.exe CtorFromArray_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CtorFromArray_ro.org /out CtorFromArray_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CtorFromArray_ro.org /out CtorFromArray_ro.exe
...
set ExePath=CtorFromArray_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CtorFromArray_ro.reflect.xml
    IF EXIST CtorFromArray_ro.reflect.xml set ReflectionRoots=-x CtorFromArray_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\CtorFromArray_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CtorFromArray_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CtorFromArray_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
