...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Haar-likeFeaturesGeneric_ro Haar-likeFeaturesGeneric_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Haar-likeFeaturesGeneric_ro Haar-likeFeaturesGeneric_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Haar-likeFeaturesGeneric_ro.dasm.il Haar-likeFeaturesGeneric_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Haar-likeFeaturesGeneric_ro.dasm.il Haar-likeFeaturesGeneric_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Haar-likeFeaturesGeneric_ro.asm.exe  Haar-likeFeaturesGeneric_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Haar-likeFeaturesGeneric_ro.asm.exe  Haar-likeFeaturesGeneric_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Haar-likeFeaturesGeneric_ro
    if [ ! -f Haar-likeFeaturesGeneric_ro.org ]; then
        TakeLock
        if [ ! -f Haar-likeFeaturesGeneric_ro.org ]; then
          mkdir IL
          cp Haar-likeFeaturesGeneric_ro.exe IL/Haar-likeFeaturesGeneric_ro.exe
          mv Haar-likeFeaturesGeneric_ro.exe Haar-likeFeaturesGeneric_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Haar-likeFeaturesGeneric_ro.org /out Haar-likeFeaturesGeneric_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Haar-likeFeaturesGeneric_ro.org /out Haar-likeFeaturesGeneric_ro.exe
          __cgExitCode=$?
...
ExePath=Haar-likeFeaturesGeneric_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in Haar-likeFeaturesGeneric_ro.reflect.xml
  if [ -f Haar-likeFeaturesGeneric_ro.reflect.xml ]
then
    ReflectionRoots="-x Haar-likeFeaturesGeneric_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Haar-likeFeaturesGeneric_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Haar-likeFeaturesGeneric_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Haar-likeFeaturesGeneric_ro.asm.exe $CLRTestExecutionArguments
