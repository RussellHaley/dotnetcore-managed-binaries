...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Haar-likeFeaturesGeneric_ro Haar-likeFeaturesGeneric_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Haar-likeFeaturesGeneric_ro Haar-likeFeaturesGeneric_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Haar-likeFeaturesGeneric_ro.dasm.il Haar-likeFeaturesGeneric_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Haar-likeFeaturesGeneric_ro.dasm.il Haar-likeFeaturesGeneric_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Haar-likeFeaturesGeneric_ro.asm.exe  Haar-likeFeaturesGeneric_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Haar-likeFeaturesGeneric_ro.asm.exe  Haar-likeFeaturesGeneric_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Haar-likeFeaturesGeneric_ro
    if not exist "Haar-likeFeaturesGeneric_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Haar-likeFeaturesGeneric_ro.org" (
            mkdir IL
            copy Haar-likeFeaturesGeneric_ro.exe IL\Haar-likeFeaturesGeneric_ro.exe 
            ren Haar-likeFeaturesGeneric_ro.exe Haar-likeFeaturesGeneric_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Haar-likeFeaturesGeneric_ro.org /out Haar-likeFeaturesGeneric_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Haar-likeFeaturesGeneric_ro.org /out Haar-likeFeaturesGeneric_ro.exe
...
set ExePath=Haar-likeFeaturesGeneric_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Haar-likeFeaturesGeneric_ro.reflect.xml
    IF EXIST Haar-likeFeaturesGeneric_ro.reflect.xml set ReflectionRoots=-x Haar-likeFeaturesGeneric_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Haar-likeFeaturesGeneric_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Haar-likeFeaturesGeneric_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Haar-likeFeaturesGeneric_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
