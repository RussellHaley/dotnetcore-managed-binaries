...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorGet_r VectorGet_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorGet_r VectorGet_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorGet_r.dasm.il VectorGet_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorGet_r.dasm.il VectorGet_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorGet_r.asm.exe  VectorGet_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorGet_r.asm.exe  VectorGet_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorGet_r
    if [ ! -f VectorGet_r.org ]; then
        TakeLock
        if [ ! -f VectorGet_r.org ]; then
          mkdir IL
          cp VectorGet_r.exe IL/VectorGet_r.exe
          mv VectorGet_r.exe VectorGet_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorGet_r.org /out VectorGet_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorGet_r.org /out VectorGet_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorGet_r.reflect.xml
  if [ -f VectorGet_r.reflect.xml ]
then
    ReflectionRoots="-x VectorGet_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorGet_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorGet_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorGet_r.asm.exe $CLRTestExecutionArguments
