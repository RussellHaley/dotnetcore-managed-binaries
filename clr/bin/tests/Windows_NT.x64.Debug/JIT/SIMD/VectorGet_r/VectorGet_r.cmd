...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorGet_r VectorGet_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorGet_r VectorGet_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorGet_r.dasm.il VectorGet_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorGet_r.dasm.il VectorGet_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorGet_r.asm.exe  VectorGet_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorGet_r.asm.exe  VectorGet_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorGet_r
    if not exist "VectorGet_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorGet_r.org" (
            mkdir IL
            copy VectorGet_r.exe IL\VectorGet_r.exe 
            ren VectorGet_r.exe VectorGet_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorGet_r.org /out VectorGet_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorGet_r.org /out VectorGet_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorGet_r.reflect.xml
    IF EXIST VectorGet_r.reflect.xml set ReflectionRoots=-x VectorGet_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorGet_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorGet_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorGet_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
