...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorInitN_ro VectorInitN_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorInitN_ro VectorInitN_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorInitN_ro.dasm.il VectorInitN_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorInitN_ro.dasm.il VectorInitN_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorInitN_ro.asm.exe  VectorInitN_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorInitN_ro.asm.exe  VectorInitN_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorInitN_ro
    if [ ! -f VectorInitN_ro.org ]; then
        TakeLock
        if [ ! -f VectorInitN_ro.org ]; then
          mkdir IL
          cp VectorInitN_ro.exe IL/VectorInitN_ro.exe
          mv VectorInitN_ro.exe VectorInitN_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorInitN_ro.org /out VectorInitN_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorInitN_ro.org /out VectorInitN_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorInitN_ro.reflect.xml
  if [ -f VectorInitN_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorInitN_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorInitN_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorInitN_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorInitN_ro.asm.exe $CLRTestExecutionArguments
