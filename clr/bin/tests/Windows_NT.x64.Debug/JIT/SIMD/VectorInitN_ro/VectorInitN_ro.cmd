...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorInitN_ro VectorInitN_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorInitN_ro VectorInitN_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorInitN_ro.dasm.il VectorInitN_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorInitN_ro.dasm.il VectorInitN_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorInitN_ro.asm.exe  VectorInitN_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorInitN_ro.asm.exe  VectorInitN_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorInitN_ro
    if not exist "VectorInitN_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorInitN_ro.org" (
            mkdir IL
            copy VectorInitN_ro.exe IL\VectorInitN_ro.exe 
            ren VectorInitN_ro.exe VectorInitN_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorInitN_ro.org /out VectorInitN_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorInitN_ro.org /out VectorInitN_ro.exe
...
set ExePath=VectorInitN_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorInitN_ro.reflect.xml
    IF EXIST VectorInitN_ro.reflect.xml set ReflectionRoots=-x VectorInitN_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorInitN_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorInitN_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorInitN_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
