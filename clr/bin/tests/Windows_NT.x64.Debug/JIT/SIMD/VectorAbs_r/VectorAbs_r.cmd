...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorAbs_r VectorAbs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorAbs_r VectorAbs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorAbs_r.dasm.il VectorAbs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorAbs_r.dasm.il VectorAbs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorAbs_r.asm.exe  VectorAbs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorAbs_r.asm.exe  VectorAbs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorAbs_r
    if not exist "VectorAbs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorAbs_r.org" (
            mkdir IL
            copy VectorAbs_r.exe IL\VectorAbs_r.exe 
            ren VectorAbs_r.exe VectorAbs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorAbs_r.org /out VectorAbs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorAbs_r.org /out VectorAbs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorAbs_r.reflect.xml
    IF EXIST VectorAbs_r.reflect.xml set ReflectionRoots=-x VectorAbs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorAbs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorAbs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorAbs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
