...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorAbs_r VectorAbs_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorAbs_r VectorAbs_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorAbs_r.dasm.il VectorAbs_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorAbs_r.dasm.il VectorAbs_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorAbs_r.asm.exe  VectorAbs_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorAbs_r.asm.exe  VectorAbs_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorAbs_r
    if [ ! -f VectorAbs_r.org ]; then
        TakeLock
        if [ ! -f VectorAbs_r.org ]; then
          mkdir IL
          cp VectorAbs_r.exe IL/VectorAbs_r.exe
          mv VectorAbs_r.exe VectorAbs_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorAbs_r.org /out VectorAbs_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorAbs_r.org /out VectorAbs_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorAbs_r.reflect.xml
  if [ -f VectorAbs_r.reflect.xml ]
then
    ReflectionRoots="-x VectorAbs_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorAbs_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorAbs_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorAbs_r.asm.exe $CLRTestExecutionArguments
