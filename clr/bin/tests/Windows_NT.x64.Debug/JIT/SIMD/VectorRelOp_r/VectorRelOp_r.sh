...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorRelOp_r VectorRelOp_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorRelOp_r VectorRelOp_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorRelOp_r.dasm.il VectorRelOp_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorRelOp_r.dasm.il VectorRelOp_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorRelOp_r.asm.exe  VectorRelOp_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorRelOp_r.asm.exe  VectorRelOp_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorRelOp_r
    if [ ! -f VectorRelOp_r.org ]; then
        TakeLock
        if [ ! -f VectorRelOp_r.org ]; then
          mkdir IL
          cp VectorRelOp_r.exe IL/VectorRelOp_r.exe
          mv VectorRelOp_r.exe VectorRelOp_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorRelOp_r.org /out VectorRelOp_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorRelOp_r.org /out VectorRelOp_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorRelOp_r.reflect.xml
  if [ -f VectorRelOp_r.reflect.xml ]
then
    ReflectionRoots="-x VectorRelOp_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorRelOp_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorRelOp_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorRelOp_r.asm.exe $CLRTestExecutionArguments
