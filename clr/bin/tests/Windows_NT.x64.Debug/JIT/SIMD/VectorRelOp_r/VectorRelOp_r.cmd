...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorRelOp_r VectorRelOp_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorRelOp_r VectorRelOp_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorRelOp_r.dasm.il VectorRelOp_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorRelOp_r.dasm.il VectorRelOp_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorRelOp_r.asm.exe  VectorRelOp_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorRelOp_r.asm.exe  VectorRelOp_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorRelOp_r
    if not exist "VectorRelOp_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorRelOp_r.org" (
            mkdir IL
            copy VectorRelOp_r.exe IL\VectorRelOp_r.exe 
            ren VectorRelOp_r.exe VectorRelOp_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorRelOp_r.org /out VectorRelOp_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorRelOp_r.org /out VectorRelOp_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorRelOp_r.reflect.xml
    IF EXIST VectorRelOp_r.reflect.xml set ReflectionRoots=-x VectorRelOp_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorRelOp_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorRelOp_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorRelOp_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
