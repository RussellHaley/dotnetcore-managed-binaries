...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMax_r VectorMax_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMax_r VectorMax_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorMax_r.dasm.il VectorMax_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorMax_r.dasm.il VectorMax_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorMax_r.asm.exe  VectorMax_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorMax_r.asm.exe  VectorMax_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorMax_r
    if not exist "VectorMax_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorMax_r.org" (
            mkdir IL
            copy VectorMax_r.exe IL\VectorMax_r.exe 
            ren VectorMax_r.exe VectorMax_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMax_r.org /out VectorMax_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMax_r.org /out VectorMax_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorMax_r.reflect.xml
    IF EXIST VectorMax_r.reflect.xml set ReflectionRoots=-x VectorMax_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorMax_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorMax_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorMax_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
