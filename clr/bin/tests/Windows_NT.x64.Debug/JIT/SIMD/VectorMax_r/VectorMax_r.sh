...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMax_r VectorMax_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMax_r VectorMax_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorMax_r.dasm.il VectorMax_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorMax_r.dasm.il VectorMax_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorMax_r.asm.exe  VectorMax_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorMax_r.asm.exe  VectorMax_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorMax_r
    if [ ! -f VectorMax_r.org ]; then
        TakeLock
        if [ ! -f VectorMax_r.org ]; then
          mkdir IL
          cp VectorMax_r.exe IL/VectorMax_r.exe
          mv VectorMax_r.exe VectorMax_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMax_r.org /out VectorMax_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMax_r.org /out VectorMax_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorMax_r.reflect.xml
  if [ -f VectorMax_r.reflect.xml ]
then
    ReflectionRoots="-x VectorMax_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorMax_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorMax_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorMax_r.asm.exe $CLRTestExecutionArguments
