...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArrayInit_r VectorArrayInit_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArrayInit_r VectorArrayInit_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorArrayInit_r.dasm.il VectorArrayInit_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorArrayInit_r.dasm.il VectorArrayInit_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorArrayInit_r.asm.exe  VectorArrayInit_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorArrayInit_r.asm.exe  VectorArrayInit_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorArrayInit_r
    if not exist "VectorArrayInit_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorArrayInit_r.org" (
            mkdir IL
            copy VectorArrayInit_r.exe IL\VectorArrayInit_r.exe 
            ren VectorArrayInit_r.exe VectorArrayInit_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArrayInit_r.org /out VectorArrayInit_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArrayInit_r.org /out VectorArrayInit_r.exe
...
set ExePath=VectorArrayInit_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorArrayInit_r.reflect.xml
    IF EXIST VectorArrayInit_r.reflect.xml set ReflectionRoots=-x VectorArrayInit_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorArrayInit_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorArrayInit_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorArrayInit_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
