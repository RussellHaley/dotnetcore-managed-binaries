...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArrayInit_r VectorArrayInit_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArrayInit_r VectorArrayInit_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorArrayInit_r.dasm.il VectorArrayInit_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorArrayInit_r.dasm.il VectorArrayInit_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorArrayInit_r.asm.exe  VectorArrayInit_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorArrayInit_r.asm.exe  VectorArrayInit_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorArrayInit_r
    if [ ! -f VectorArrayInit_r.org ]; then
        TakeLock
        if [ ! -f VectorArrayInit_r.org ]; then
          mkdir IL
          cp VectorArrayInit_r.exe IL/VectorArrayInit_r.exe
          mv VectorArrayInit_r.exe VectorArrayInit_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArrayInit_r.org /out VectorArrayInit_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArrayInit_r.org /out VectorArrayInit_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorArrayInit_r.reflect.xml
  if [ -f VectorArrayInit_r.reflect.xml ]
then
    ReflectionRoots="-x VectorArrayInit_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorArrayInit_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorArrayInit_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorArrayInit_r.asm.exe $CLRTestExecutionArguments
