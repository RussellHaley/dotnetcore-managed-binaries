...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorExp_r VectorExp_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorExp_r VectorExp_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorExp_r.dasm.il VectorExp_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorExp_r.dasm.il VectorExp_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorExp_r.asm.exe  VectorExp_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorExp_r.asm.exe  VectorExp_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorExp_r
    if not exist "VectorExp_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorExp_r.org" (
            mkdir IL
            copy VectorExp_r.exe IL\VectorExp_r.exe 
            ren VectorExp_r.exe VectorExp_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorExp_r.org /out VectorExp_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorExp_r.org /out VectorExp_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorExp_r.reflect.xml
    IF EXIST VectorExp_r.reflect.xml set ReflectionRoots=-x VectorExp_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorExp_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorExp_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorExp_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
