...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorExp_r VectorExp_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorExp_r VectorExp_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorExp_r.dasm.il VectorExp_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorExp_r.dasm.il VectorExp_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorExp_r.asm.exe  VectorExp_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorExp_r.asm.exe  VectorExp_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorExp_r
    if [ ! -f VectorExp_r.org ]; then
        TakeLock
        if [ ! -f VectorExp_r.org ]; then
          mkdir IL
          cp VectorExp_r.exe IL/VectorExp_r.exe
          mv VectorExp_r.exe VectorExp_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorExp_r.org /out VectorExp_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorExp_r.org /out VectorExp_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorExp_r.reflect.xml
  if [ -f VectorExp_r.reflect.xml ]
then
    ReflectionRoots="-x VectorExp_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorExp_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorExp_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorExp_r.asm.exe $CLRTestExecutionArguments
