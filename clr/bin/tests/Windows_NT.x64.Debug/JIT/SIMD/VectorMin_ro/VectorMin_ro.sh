...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMin_ro VectorMin_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMin_ro VectorMin_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorMin_ro.dasm.il VectorMin_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorMin_ro.dasm.il VectorMin_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorMin_ro.asm.exe  VectorMin_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorMin_ro.asm.exe  VectorMin_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorMin_ro
    if [ ! -f VectorMin_ro.org ]; then
        TakeLock
        if [ ! -f VectorMin_ro.org ]; then
          mkdir IL
          cp VectorMin_ro.exe IL/VectorMin_ro.exe
          mv VectorMin_ro.exe VectorMin_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMin_ro.org /out VectorMin_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMin_ro.org /out VectorMin_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorMin_ro.reflect.xml
  if [ -f VectorMin_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorMin_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorMin_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorMin_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorMin_ro.asm.exe $CLRTestExecutionArguments
