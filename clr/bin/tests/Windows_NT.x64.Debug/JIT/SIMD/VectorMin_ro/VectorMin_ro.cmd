...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMin_ro VectorMin_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMin_ro VectorMin_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorMin_ro.dasm.il VectorMin_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorMin_ro.dasm.il VectorMin_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorMin_ro.asm.exe  VectorMin_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorMin_ro.asm.exe  VectorMin_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorMin_ro
    if not exist "VectorMin_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorMin_ro.org" (
            mkdir IL
            copy VectorMin_ro.exe IL\VectorMin_ro.exe 
            ren VectorMin_ro.exe VectorMin_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMin_ro.org /out VectorMin_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMin_ro.org /out VectorMin_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorMin_ro.reflect.xml
    IF EXIST VectorMin_ro.reflect.xml set ReflectionRoots=-x VectorMin_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorMin_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorMin_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorMin_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
