...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Vector4_r Vector4_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Vector4_r Vector4_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Vector4_r.dasm.il Vector4_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Vector4_r.dasm.il Vector4_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Vector4_r.asm.exe  Vector4_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Vector4_r.asm.exe  Vector4_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Vector4_r
    if not exist "Vector4_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Vector4_r.org" (
            mkdir IL
            copy Vector4_r.exe IL\Vector4_r.exe 
            ren Vector4_r.exe Vector4_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector4_r.org /out Vector4_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector4_r.org /out Vector4_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Vector4_r.reflect.xml
    IF EXIST Vector4_r.reflect.xml set ReflectionRoots=-x Vector4_r.reflect.xml
    ...
    set ExePath=%LinkBin%\Vector4_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Vector4_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Vector4_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
