...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector4_r Vector4_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector4_r Vector4_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Vector4_r.dasm.il Vector4_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=Vector4_r.dasm.il Vector4_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Vector4_r.asm.exe  Vector4_r.dasm.il
    "$CORE_ROOT/ilasm" -output=Vector4_r.asm.exe  Vector4_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Vector4_r
    if [ ! -f Vector4_r.org ]; then
        TakeLock
        if [ ! -f Vector4_r.org ]; then
          mkdir IL
          cp Vector4_r.exe IL/Vector4_r.exe
          mv Vector4_r.exe Vector4_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector4_r.org /out Vector4_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector4_r.org /out Vector4_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Vector4_r.reflect.xml
  if [ -f Vector4_r.reflect.xml ]
then
    ReflectionRoots="-x Vector4_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Vector4_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Vector4_r.asm.exe $CLRTestExecutionArguments
