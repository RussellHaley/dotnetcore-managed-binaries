...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorReturn_ro VectorReturn_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorReturn_ro VectorReturn_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorReturn_ro.dasm.il VectorReturn_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorReturn_ro.dasm.il VectorReturn_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorReturn_ro.asm.exe  VectorReturn_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorReturn_ro.asm.exe  VectorReturn_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorReturn_ro
    if not exist "VectorReturn_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorReturn_ro.org" (
            mkdir IL
            copy VectorReturn_ro.exe IL\VectorReturn_ro.exe 
            ren VectorReturn_ro.exe VectorReturn_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorReturn_ro.org /out VectorReturn_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorReturn_ro.org /out VectorReturn_ro.exe
...
set ExePath=VectorReturn_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorReturn_ro.reflect.xml
    IF EXIST VectorReturn_ro.reflect.xml set ReflectionRoots=-x VectorReturn_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorReturn_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorReturn_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorReturn_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
