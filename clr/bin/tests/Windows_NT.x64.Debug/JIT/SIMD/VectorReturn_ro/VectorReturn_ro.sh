...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorReturn_ro VectorReturn_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorReturn_ro VectorReturn_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorReturn_ro.dasm.il VectorReturn_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorReturn_ro.dasm.il VectorReturn_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorReturn_ro.asm.exe  VectorReturn_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorReturn_ro.asm.exe  VectorReturn_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorReturn_ro
    if [ ! -f VectorReturn_ro.org ]; then
        TakeLock
        if [ ! -f VectorReturn_ro.org ]; then
          mkdir IL
          cp VectorReturn_ro.exe IL/VectorReturn_ro.exe
          mv VectorReturn_ro.exe VectorReturn_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorReturn_ro.org /out VectorReturn_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorReturn_ro.org /out VectorReturn_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorReturn_ro.reflect.xml
  if [ -f VectorReturn_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorReturn_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorReturn_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorReturn_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorReturn_ro.asm.exe $CLRTestExecutionArguments
