...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorDiv_r VectorDiv_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorDiv_r VectorDiv_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorDiv_r.dasm.il VectorDiv_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorDiv_r.dasm.il VectorDiv_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorDiv_r.asm.exe  VectorDiv_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorDiv_r.asm.exe  VectorDiv_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorDiv_r
    if [ ! -f VectorDiv_r.org ]; then
        TakeLock
        if [ ! -f VectorDiv_r.org ]; then
          mkdir IL
          cp VectorDiv_r.exe IL/VectorDiv_r.exe
          mv VectorDiv_r.exe VectorDiv_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorDiv_r.org /out VectorDiv_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorDiv_r.org /out VectorDiv_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorDiv_r.reflect.xml
  if [ -f VectorDiv_r.reflect.xml ]
then
    ReflectionRoots="-x VectorDiv_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorDiv_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorDiv_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorDiv_r.asm.exe $CLRTestExecutionArguments
