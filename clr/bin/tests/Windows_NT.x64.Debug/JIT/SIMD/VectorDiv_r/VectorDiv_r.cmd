...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorDiv_r VectorDiv_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorDiv_r VectorDiv_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorDiv_r.dasm.il VectorDiv_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorDiv_r.dasm.il VectorDiv_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorDiv_r.asm.exe  VectorDiv_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorDiv_r.asm.exe  VectorDiv_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorDiv_r
    if not exist "VectorDiv_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorDiv_r.org" (
            mkdir IL
            copy VectorDiv_r.exe IL\VectorDiv_r.exe 
            ren VectorDiv_r.exe VectorDiv_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorDiv_r.org /out VectorDiv_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorDiv_r.org /out VectorDiv_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorDiv_r.reflect.xml
    IF EXIST VectorDiv_r.reflect.xml set ReflectionRoots=-x VectorDiv_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorDiv_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorDiv_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorDiv_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
