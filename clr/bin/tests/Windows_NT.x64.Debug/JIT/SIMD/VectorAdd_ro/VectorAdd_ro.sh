...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorAdd_ro VectorAdd_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorAdd_ro VectorAdd_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorAdd_ro.dasm.il VectorAdd_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorAdd_ro.dasm.il VectorAdd_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorAdd_ro.asm.exe  VectorAdd_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorAdd_ro.asm.exe  VectorAdd_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorAdd_ro
    if [ ! -f VectorAdd_ro.org ]; then
        TakeLock
        if [ ! -f VectorAdd_ro.org ]; then
          mkdir IL
          cp VectorAdd_ro.exe IL/VectorAdd_ro.exe
          mv VectorAdd_ro.exe VectorAdd_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorAdd_ro.org /out VectorAdd_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorAdd_ro.org /out VectorAdd_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorAdd_ro.reflect.xml
  if [ -f VectorAdd_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorAdd_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorAdd_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorAdd_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorAdd_ro.asm.exe $CLRTestExecutionArguments
