...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorAdd_ro VectorAdd_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorAdd_ro VectorAdd_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorAdd_ro.dasm.il VectorAdd_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorAdd_ro.dasm.il VectorAdd_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorAdd_ro.asm.exe  VectorAdd_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorAdd_ro.asm.exe  VectorAdd_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorAdd_ro
    if not exist "VectorAdd_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorAdd_ro.org" (
            mkdir IL
            copy VectorAdd_ro.exe IL\VectorAdd_ro.exe 
            ren VectorAdd_ro.exe VectorAdd_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorAdd_ro.org /out VectorAdd_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorAdd_ro.org /out VectorAdd_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorAdd_ro.reflect.xml
    IF EXIST VectorAdd_ro.reflect.xml set ReflectionRoots=-x VectorAdd_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorAdd_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorAdd_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorAdd_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
