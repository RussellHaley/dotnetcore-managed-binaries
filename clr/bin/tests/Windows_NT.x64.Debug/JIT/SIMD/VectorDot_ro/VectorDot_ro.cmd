...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorDot_ro VectorDot_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorDot_ro VectorDot_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorDot_ro.dasm.il VectorDot_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorDot_ro.dasm.il VectorDot_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorDot_ro.asm.exe  VectorDot_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorDot_ro.asm.exe  VectorDot_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorDot_ro
    if not exist "VectorDot_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorDot_ro.org" (
            mkdir IL
            copy VectorDot_ro.exe IL\VectorDot_ro.exe 
            ren VectorDot_ro.exe VectorDot_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorDot_ro.org /out VectorDot_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorDot_ro.org /out VectorDot_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorDot_ro.reflect.xml
    IF EXIST VectorDot_ro.reflect.xml set ReflectionRoots=-x VectorDot_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorDot_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorDot_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorDot_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
