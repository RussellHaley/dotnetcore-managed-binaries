...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorDot_ro VectorDot_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorDot_ro VectorDot_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorDot_ro.dasm.il VectorDot_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorDot_ro.dasm.il VectorDot_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorDot_ro.asm.exe  VectorDot_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorDot_ro.asm.exe  VectorDot_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorDot_ro
    if [ ! -f VectorDot_ro.org ]; then
        TakeLock
        if [ ! -f VectorDot_ro.org ]; then
          mkdir IL
          cp VectorDot_ro.exe IL/VectorDot_ro.exe
          mv VectorDot_ro.exe VectorDot_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorDot_ro.org /out VectorDot_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorDot_ro.org /out VectorDot_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorDot_ro.reflect.xml
  if [ -f VectorDot_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorDot_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorDot_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorDot_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorDot_ro.asm.exe $CLRTestExecutionArguments
