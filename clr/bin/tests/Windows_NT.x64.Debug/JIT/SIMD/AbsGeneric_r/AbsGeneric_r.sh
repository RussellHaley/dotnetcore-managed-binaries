...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AbsGeneric_r AbsGeneric_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AbsGeneric_r AbsGeneric_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbsGeneric_r.dasm.il AbsGeneric_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbsGeneric_r.dasm.il AbsGeneric_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbsGeneric_r.asm.exe  AbsGeneric_r.dasm.il
    "$CORE_ROOT/ilasm" -output=AbsGeneric_r.asm.exe  AbsGeneric_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbsGeneric_r
    if [ ! -f AbsGeneric_r.org ]; then
        TakeLock
        if [ ! -f AbsGeneric_r.org ]; then
          mkdir IL
          cp AbsGeneric_r.exe IL/AbsGeneric_r.exe
          mv AbsGeneric_r.exe AbsGeneric_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbsGeneric_r.org /out AbsGeneric_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbsGeneric_r.org /out AbsGeneric_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbsGeneric_r.reflect.xml
  if [ -f AbsGeneric_r.reflect.xml ]
then
    ReflectionRoots="-x AbsGeneric_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AbsGeneric_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbsGeneric_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbsGeneric_r.asm.exe $CLRTestExecutionArguments
