...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AbsGeneric_r AbsGeneric_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AbsGeneric_r AbsGeneric_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbsGeneric_r.dasm.il AbsGeneric_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbsGeneric_r.dasm.il AbsGeneric_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbsGeneric_r.asm.exe  AbsGeneric_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbsGeneric_r.asm.exe  AbsGeneric_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbsGeneric_r
    if not exist "AbsGeneric_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbsGeneric_r.org" (
            mkdir IL
            copy AbsGeneric_r.exe IL\AbsGeneric_r.exe 
            ren AbsGeneric_r.exe AbsGeneric_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbsGeneric_r.org /out AbsGeneric_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbsGeneric_r.org /out AbsGeneric_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbsGeneric_r.reflect.xml
    IF EXIST AbsGeneric_r.reflect.xml set ReflectionRoots=-x AbsGeneric_r.reflect.xml
    ...
    set ExePath=%LinkBin%\AbsGeneric_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbsGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbsGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
