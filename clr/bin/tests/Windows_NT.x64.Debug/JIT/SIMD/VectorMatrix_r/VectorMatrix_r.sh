...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMatrix_r VectorMatrix_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMatrix_r VectorMatrix_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorMatrix_r.dasm.il VectorMatrix_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorMatrix_r.dasm.il VectorMatrix_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorMatrix_r.asm.exe  VectorMatrix_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorMatrix_r.asm.exe  VectorMatrix_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorMatrix_r
    if [ ! -f VectorMatrix_r.org ]; then
        TakeLock
        if [ ! -f VectorMatrix_r.org ]; then
          mkdir IL
          cp VectorMatrix_r.exe IL/VectorMatrix_r.exe
          mv VectorMatrix_r.exe VectorMatrix_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMatrix_r.org /out VectorMatrix_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMatrix_r.org /out VectorMatrix_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorMatrix_r.reflect.xml
  if [ -f VectorMatrix_r.reflect.xml ]
then
    ReflectionRoots="-x VectorMatrix_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorMatrix_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorMatrix_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorMatrix_r.asm.exe $CLRTestExecutionArguments
