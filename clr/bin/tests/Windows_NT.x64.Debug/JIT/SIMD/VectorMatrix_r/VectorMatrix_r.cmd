...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMatrix_r VectorMatrix_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMatrix_r VectorMatrix_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorMatrix_r.dasm.il VectorMatrix_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorMatrix_r.dasm.il VectorMatrix_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorMatrix_r.asm.exe  VectorMatrix_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorMatrix_r.asm.exe  VectorMatrix_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorMatrix_r
    if not exist "VectorMatrix_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorMatrix_r.org" (
            mkdir IL
            copy VectorMatrix_r.exe IL\VectorMatrix_r.exe 
            ren VectorMatrix_r.exe VectorMatrix_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMatrix_r.org /out VectorMatrix_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMatrix_r.org /out VectorMatrix_r.exe
...
set ExePath=VectorMatrix_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorMatrix_r.reflect.xml
    IF EXIST VectorMatrix_r.reflect.xml set ReflectionRoots=-x VectorMatrix_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorMatrix_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorMatrix_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorMatrix_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
