...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMax_ro VectorMax_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorMax_ro VectorMax_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorMax_ro.dasm.il VectorMax_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorMax_ro.dasm.il VectorMax_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorMax_ro.asm.exe  VectorMax_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorMax_ro.asm.exe  VectorMax_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorMax_ro
    if [ ! -f VectorMax_ro.org ]; then
        TakeLock
        if [ ! -f VectorMax_ro.org ]; then
          mkdir IL
          cp VectorMax_ro.exe IL/VectorMax_ro.exe
          mv VectorMax_ro.exe VectorMax_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMax_ro.org /out VectorMax_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorMax_ro.org /out VectorMax_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorMax_ro.reflect.xml
  if [ -f VectorMax_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorMax_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorMax_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorMax_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorMax_ro.asm.exe $CLRTestExecutionArguments
