...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMax_ro VectorMax_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorMax_ro VectorMax_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorMax_ro.dasm.il VectorMax_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorMax_ro.dasm.il VectorMax_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorMax_ro.asm.exe  VectorMax_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorMax_ro.asm.exe  VectorMax_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorMax_ro
    if not exist "VectorMax_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorMax_ro.org" (
            mkdir IL
            copy VectorMax_ro.exe IL\VectorMax_ro.exe 
            ren VectorMax_ro.exe VectorMax_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMax_ro.org /out VectorMax_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorMax_ro.org /out VectorMax_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorMax_ro.reflect.xml
    IF EXIST VectorMax_ro.reflect.xml set ReflectionRoots=-x VectorMax_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorMax_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorMax_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorMax_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
