...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Dup_ro Dup_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Dup_ro Dup_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Dup_ro.dasm.il Dup_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Dup_ro.dasm.il Dup_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Dup_ro.asm.exe  Dup_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Dup_ro.asm.exe  Dup_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Dup_ro
    if not exist "Dup_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Dup_ro.org" (
            mkdir IL
            copy Dup_ro.exe IL\Dup_ro.exe 
            ren Dup_ro.exe Dup_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Dup_ro.org /out Dup_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Dup_ro.org /out Dup_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Dup_ro.reflect.xml
    IF EXIST Dup_ro.reflect.xml set ReflectionRoots=-x Dup_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Dup_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Dup_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Dup_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
