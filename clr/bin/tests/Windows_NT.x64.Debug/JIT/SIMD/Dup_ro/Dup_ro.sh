...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Dup_ro Dup_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Dup_ro Dup_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Dup_ro.dasm.il Dup_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Dup_ro.dasm.il Dup_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Dup_ro.asm.exe  Dup_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Dup_ro.asm.exe  Dup_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Dup_ro
    if [ ! -f Dup_ro.org ]; then
        TakeLock
        if [ ! -f Dup_ro.org ]; then
          mkdir IL
          cp Dup_ro.exe IL/Dup_ro.exe
          mv Dup_ro.exe Dup_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Dup_ro.org /out Dup_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Dup_ro.org /out Dup_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Dup_ro.reflect.xml
  if [ -f Dup_ro.reflect.xml ]
then
    ReflectionRoots="-x Dup_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Dup_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Dup_ro.asm.exe $CLRTestExecutionArguments
