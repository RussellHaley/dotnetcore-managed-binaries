...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\BitwiseOperations_r BitwiseOperations_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\BitwiseOperations_r BitwiseOperations_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=BitwiseOperations_r.dasm.il BitwiseOperations_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=BitwiseOperations_r.dasm.il BitwiseOperations_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=BitwiseOperations_r.asm.exe  BitwiseOperations_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=BitwiseOperations_r.asm.exe  BitwiseOperations_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=BitwiseOperations_r
    if not exist "BitwiseOperations_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "BitwiseOperations_r.org" (
            mkdir IL
            copy BitwiseOperations_r.exe IL\BitwiseOperations_r.exe 
            ren BitwiseOperations_r.exe BitwiseOperations_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BitwiseOperations_r.org /out BitwiseOperations_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BitwiseOperations_r.org /out BitwiseOperations_r.exe
...
set ExePath=BitwiseOperations_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in BitwiseOperations_r.reflect.xml
    IF EXIST BitwiseOperations_r.reflect.xml set ReflectionRoots=-x BitwiseOperations_r.reflect.xml
    ...
    set ExePath=%LinkBin%\BitwiseOperations_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% BitwiseOperations_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% BitwiseOperations_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
