...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/BitwiseOperations_r BitwiseOperations_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/BitwiseOperations_r BitwiseOperations_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=BitwiseOperations_r.dasm.il BitwiseOperations_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=BitwiseOperations_r.dasm.il BitwiseOperations_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=BitwiseOperations_r.asm.exe  BitwiseOperations_r.dasm.il
    "$CORE_ROOT/ilasm" -output=BitwiseOperations_r.asm.exe  BitwiseOperations_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=BitwiseOperations_r
    if [ ! -f BitwiseOperations_r.org ]; then
        TakeLock
        if [ ! -f BitwiseOperations_r.org ]; then
          mkdir IL
          cp BitwiseOperations_r.exe IL/BitwiseOperations_r.exe
          mv BitwiseOperations_r.exe BitwiseOperations_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BitwiseOperations_r.org /out BitwiseOperations_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BitwiseOperations_r.org /out BitwiseOperations_r.exe
          __cgExitCode=$?
...
ExePath=BitwiseOperations_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in BitwiseOperations_r.reflect.xml
  if [ -f BitwiseOperations_r.reflect.xml ]
then
    ReflectionRoots="-x BitwiseOperations_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/BitwiseOperations_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" BitwiseOperations_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" BitwiseOperations_r.asm.exe $CLRTestExecutionArguments
