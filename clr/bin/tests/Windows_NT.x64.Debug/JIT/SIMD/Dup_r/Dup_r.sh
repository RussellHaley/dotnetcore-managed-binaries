...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Dup_r Dup_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Dup_r Dup_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Dup_r.dasm.il Dup_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=Dup_r.dasm.il Dup_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Dup_r.asm.exe  Dup_r.dasm.il
    "$CORE_ROOT/ilasm" -output=Dup_r.asm.exe  Dup_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Dup_r
    if [ ! -f Dup_r.org ]; then
        TakeLock
        if [ ! -f Dup_r.org ]; then
          mkdir IL
          cp Dup_r.exe IL/Dup_r.exe
          mv Dup_r.exe Dup_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Dup_r.org /out Dup_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Dup_r.org /out Dup_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Dup_r.reflect.xml
  if [ -f Dup_r.reflect.xml ]
then
    ReflectionRoots="-x Dup_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Dup_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Dup_r.asm.exe $CLRTestExecutionArguments
