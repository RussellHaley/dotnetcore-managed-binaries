...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Dup_r Dup_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Dup_r Dup_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Dup_r.dasm.il Dup_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Dup_r.dasm.il Dup_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Dup_r.asm.exe  Dup_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Dup_r.asm.exe  Dup_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Dup_r
    if not exist "Dup_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Dup_r.org" (
            mkdir IL
            copy Dup_r.exe IL\Dup_r.exe 
            ren Dup_r.exe Dup_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Dup_r.org /out Dup_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Dup_r.org /out Dup_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Dup_r.reflect.xml
    IF EXIST Dup_r.reflect.xml set ReflectionRoots=-x Dup_r.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Dup_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Dup_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
