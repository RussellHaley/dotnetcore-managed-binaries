...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Sums_r Sums_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Sums_r Sums_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Sums_r.dasm.il Sums_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Sums_r.dasm.il Sums_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Sums_r.asm.exe  Sums_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Sums_r.asm.exe  Sums_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Sums_r
    if not exist "Sums_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Sums_r.org" (
            mkdir IL
            copy Sums_r.exe IL\Sums_r.exe 
            ren Sums_r.exe Sums_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Sums_r.org /out Sums_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Sums_r.org /out Sums_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Sums_r.reflect.xml
    IF EXIST Sums_r.reflect.xml set ReflectionRoots=-x Sums_r.reflect.xml
    ...
    set ExePath=%LinkBin%\Sums_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Sums_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Sums_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
