...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Sums_r Sums_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Sums_r Sums_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Sums_r.dasm.il Sums_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=Sums_r.dasm.il Sums_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Sums_r.asm.exe  Sums_r.dasm.il
    "$CORE_ROOT/ilasm" -output=Sums_r.asm.exe  Sums_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Sums_r
    if [ ! -f Sums_r.org ]; then
        TakeLock
        if [ ! -f Sums_r.org ]; then
          mkdir IL
          cp Sums_r.exe IL/Sums_r.exe
          mv Sums_r.exe Sums_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Sums_r.org /out Sums_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Sums_r.org /out Sums_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Sums_r.reflect.xml
  if [ -f Sums_r.reflect.xml ]
then
    ReflectionRoots="-x Sums_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Sums_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Sums_r.asm.exe $CLRTestExecutionArguments
