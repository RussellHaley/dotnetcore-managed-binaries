...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSub_ro VectorSub_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSub_ro VectorSub_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorSub_ro.dasm.il VectorSub_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorSub_ro.dasm.il VectorSub_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorSub_ro.asm.exe  VectorSub_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorSub_ro.asm.exe  VectorSub_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorSub_ro
    if not exist "VectorSub_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorSub_ro.org" (
            mkdir IL
            copy VectorSub_ro.exe IL\VectorSub_ro.exe 
            ren VectorSub_ro.exe VectorSub_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSub_ro.org /out VectorSub_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSub_ro.org /out VectorSub_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorSub_ro.reflect.xml
    IF EXIST VectorSub_ro.reflect.xml set ReflectionRoots=-x VectorSub_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorSub_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorSub_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorSub_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
