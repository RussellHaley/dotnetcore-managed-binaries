...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSub_ro VectorSub_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSub_ro VectorSub_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorSub_ro.dasm.il VectorSub_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorSub_ro.dasm.il VectorSub_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorSub_ro.asm.exe  VectorSub_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorSub_ro.asm.exe  VectorSub_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorSub_ro
    if [ ! -f VectorSub_ro.org ]; then
        TakeLock
        if [ ! -f VectorSub_ro.org ]; then
          mkdir IL
          cp VectorSub_ro.exe IL/VectorSub_ro.exe
          mv VectorSub_ro.exe VectorSub_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSub_ro.org /out VectorSub_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSub_ro.org /out VectorSub_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorSub_ro.reflect.xml
  if [ -f VectorSub_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorSub_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorSub_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorSub_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorSub_ro.asm.exe $CLRTestExecutionArguments
