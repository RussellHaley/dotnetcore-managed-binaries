...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorCast_r VectorCast_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorCast_r VectorCast_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorCast_r.dasm.il VectorCast_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorCast_r.dasm.il VectorCast_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorCast_r.asm.exe  VectorCast_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorCast_r.asm.exe  VectorCast_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorCast_r
    if not exist "VectorCast_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorCast_r.org" (
            mkdir IL
            copy VectorCast_r.exe IL\VectorCast_r.exe 
            ren VectorCast_r.exe VectorCast_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorCast_r.org /out VectorCast_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorCast_r.org /out VectorCast_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorCast_r.reflect.xml
    IF EXIST VectorCast_r.reflect.xml set ReflectionRoots=-x VectorCast_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorCast_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorCast_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorCast_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
