...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorCast_r VectorCast_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorCast_r VectorCast_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorCast_r.dasm.il VectorCast_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorCast_r.dasm.il VectorCast_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorCast_r.asm.exe  VectorCast_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorCast_r.asm.exe  VectorCast_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorCast_r
    if [ ! -f VectorCast_r.org ]; then
        TakeLock
        if [ ! -f VectorCast_r.org ]; then
          mkdir IL
          cp VectorCast_r.exe IL/VectorCast_r.exe
          mv VectorCast_r.exe VectorCast_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorCast_r.org /out VectorCast_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorCast_r.org /out VectorCast_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorCast_r.reflect.xml
  if [ -f VectorCast_r.reflect.xml ]
then
    ReflectionRoots="-x VectorCast_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorCast_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorCast_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorCast_r.asm.exe $CLRTestExecutionArguments
