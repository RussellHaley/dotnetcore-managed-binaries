...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/LdfldGeneric_r LdfldGeneric_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/LdfldGeneric_r LdfldGeneric_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LdfldGeneric_r.dasm.il LdfldGeneric_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=LdfldGeneric_r.dasm.il LdfldGeneric_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LdfldGeneric_r.asm.exe  LdfldGeneric_r.dasm.il
    "$CORE_ROOT/ilasm" -output=LdfldGeneric_r.asm.exe  LdfldGeneric_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LdfldGeneric_r
    if [ ! -f LdfldGeneric_r.org ]; then
        TakeLock
        if [ ! -f LdfldGeneric_r.org ]; then
          mkdir IL
          cp LdfldGeneric_r.exe IL/LdfldGeneric_r.exe
          mv LdfldGeneric_r.exe LdfldGeneric_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LdfldGeneric_r.org /out LdfldGeneric_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LdfldGeneric_r.org /out LdfldGeneric_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LdfldGeneric_r.reflect.xml
  if [ -f LdfldGeneric_r.reflect.xml ]
then
    ReflectionRoots="-x LdfldGeneric_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/LdfldGeneric_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LdfldGeneric_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LdfldGeneric_r.asm.exe $CLRTestExecutionArguments
