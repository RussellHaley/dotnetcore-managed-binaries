...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\LdfldGeneric_r LdfldGeneric_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\LdfldGeneric_r LdfldGeneric_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LdfldGeneric_r.dasm.il LdfldGeneric_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LdfldGeneric_r.dasm.il LdfldGeneric_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LdfldGeneric_r.asm.exe  LdfldGeneric_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LdfldGeneric_r.asm.exe  LdfldGeneric_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LdfldGeneric_r
    if not exist "LdfldGeneric_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LdfldGeneric_r.org" (
            mkdir IL
            copy LdfldGeneric_r.exe IL\LdfldGeneric_r.exe 
            ren LdfldGeneric_r.exe LdfldGeneric_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LdfldGeneric_r.org /out LdfldGeneric_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LdfldGeneric_r.org /out LdfldGeneric_r.exe
...
set ExePath=LdfldGeneric_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LdfldGeneric_r.reflect.xml
    IF EXIST LdfldGeneric_r.reflect.xml set ReflectionRoots=-x LdfldGeneric_r.reflect.xml
    ...
    set ExePath=%LinkBin%\LdfldGeneric_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LdfldGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LdfldGeneric_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
