...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AddingSequence_r AddingSequence_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AddingSequence_r AddingSequence_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AddingSequence_r.dasm.il AddingSequence_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=AddingSequence_r.dasm.il AddingSequence_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AddingSequence_r.asm.exe  AddingSequence_r.dasm.il
    "$CORE_ROOT/ilasm" -output=AddingSequence_r.asm.exe  AddingSequence_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AddingSequence_r
    if [ ! -f AddingSequence_r.org ]; then
        TakeLock
        if [ ! -f AddingSequence_r.org ]; then
          mkdir IL
          cp AddingSequence_r.exe IL/AddingSequence_r.exe
          mv AddingSequence_r.exe AddingSequence_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddingSequence_r.org /out AddingSequence_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddingSequence_r.org /out AddingSequence_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AddingSequence_r.reflect.xml
  if [ -f AddingSequence_r.reflect.xml ]
then
    ReflectionRoots="-x AddingSequence_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AddingSequence_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AddingSequence_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AddingSequence_r.asm.exe $CLRTestExecutionArguments
