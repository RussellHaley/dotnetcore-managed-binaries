...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AddingSequence_r AddingSequence_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AddingSequence_r AddingSequence_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AddingSequence_r.dasm.il AddingSequence_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AddingSequence_r.dasm.il AddingSequence_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AddingSequence_r.asm.exe  AddingSequence_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AddingSequence_r.asm.exe  AddingSequence_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AddingSequence_r
    if not exist "AddingSequence_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AddingSequence_r.org" (
            mkdir IL
            copy AddingSequence_r.exe IL\AddingSequence_r.exe 
            ren AddingSequence_r.exe AddingSequence_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddingSequence_r.org /out AddingSequence_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddingSequence_r.org /out AddingSequence_r.exe
...
set ExePath=AddingSequence_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AddingSequence_r.reflect.xml
    IF EXIST AddingSequence_r.reflect.xml set ReflectionRoots=-x AddingSequence_r.reflect.xml
    ...
    set ExePath=%LinkBin%\AddingSequence_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AddingSequence_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AddingSequence_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
