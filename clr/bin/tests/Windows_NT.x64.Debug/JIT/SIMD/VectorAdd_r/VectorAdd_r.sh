...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorAdd_r VectorAdd_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorAdd_r VectorAdd_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorAdd_r.dasm.il VectorAdd_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorAdd_r.dasm.il VectorAdd_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorAdd_r.asm.exe  VectorAdd_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorAdd_r.asm.exe  VectorAdd_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorAdd_r
    if [ ! -f VectorAdd_r.org ]; then
        TakeLock
        if [ ! -f VectorAdd_r.org ]; then
          mkdir IL
          cp VectorAdd_r.exe IL/VectorAdd_r.exe
          mv VectorAdd_r.exe VectorAdd_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorAdd_r.org /out VectorAdd_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorAdd_r.org /out VectorAdd_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorAdd_r.reflect.xml
  if [ -f VectorAdd_r.reflect.xml ]
then
    ReflectionRoots="-x VectorAdd_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorAdd_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorAdd_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorAdd_r.asm.exe $CLRTestExecutionArguments
