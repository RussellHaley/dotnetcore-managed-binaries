...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorAdd_r VectorAdd_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorAdd_r VectorAdd_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorAdd_r.dasm.il VectorAdd_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorAdd_r.dasm.il VectorAdd_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorAdd_r.asm.exe  VectorAdd_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorAdd_r.asm.exe  VectorAdd_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorAdd_r
    if not exist "VectorAdd_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorAdd_r.org" (
            mkdir IL
            copy VectorAdd_r.exe IL\VectorAdd_r.exe 
            ren VectorAdd_r.exe VectorAdd_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorAdd_r.org /out VectorAdd_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorAdd_r.org /out VectorAdd_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorAdd_r.reflect.xml
    IF EXIST VectorAdd_r.reflect.xml set ReflectionRoots=-x VectorAdd_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorAdd_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorAdd_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorAdd_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
