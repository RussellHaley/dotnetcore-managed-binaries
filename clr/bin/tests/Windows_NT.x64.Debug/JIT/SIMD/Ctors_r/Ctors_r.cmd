...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Ctors_r Ctors_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\Ctors_r Ctors_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Ctors_r.dasm.il Ctors_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Ctors_r.dasm.il Ctors_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Ctors_r.asm.exe  Ctors_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Ctors_r.asm.exe  Ctors_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Ctors_r
    if not exist "Ctors_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Ctors_r.org" (
            mkdir IL
            copy Ctors_r.exe IL\Ctors_r.exe 
            ren Ctors_r.exe Ctors_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ctors_r.org /out Ctors_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ctors_r.org /out Ctors_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Ctors_r.reflect.xml
    IF EXIST Ctors_r.reflect.xml set ReflectionRoots=-x Ctors_r.reflect.xml
    ...
    set ExePath=%LinkBin%\Ctors_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Ctors_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Ctors_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
