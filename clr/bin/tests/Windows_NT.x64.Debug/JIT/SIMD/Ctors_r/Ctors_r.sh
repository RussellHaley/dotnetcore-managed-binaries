...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Ctors_r Ctors_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Ctors_r Ctors_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Ctors_r.dasm.il Ctors_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=Ctors_r.dasm.il Ctors_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Ctors_r.asm.exe  Ctors_r.dasm.il
    "$CORE_ROOT/ilasm" -output=Ctors_r.asm.exe  Ctors_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Ctors_r
    if [ ! -f Ctors_r.org ]; then
        TakeLock
        if [ ! -f Ctors_r.org ]; then
          mkdir IL
          cp Ctors_r.exe IL/Ctors_r.exe
          mv Ctors_r.exe Ctors_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ctors_r.org /out Ctors_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ctors_r.org /out Ctors_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Ctors_r.reflect.xml
  if [ -f Ctors_r.reflect.xml ]
then
    ReflectionRoots="-x Ctors_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Ctors_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Ctors_r.asm.exe $CLRTestExecutionArguments
