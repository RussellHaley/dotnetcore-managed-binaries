...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AbsSqrt_ro AbsSqrt_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/AbsSqrt_ro AbsSqrt_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AbsSqrt_ro.dasm.il AbsSqrt_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=AbsSqrt_ro.dasm.il AbsSqrt_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AbsSqrt_ro.asm.exe  AbsSqrt_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=AbsSqrt_ro.asm.exe  AbsSqrt_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AbsSqrt_ro
    if [ ! -f AbsSqrt_ro.org ]; then
        TakeLock
        if [ ! -f AbsSqrt_ro.org ]; then
          mkdir IL
          cp AbsSqrt_ro.exe IL/AbsSqrt_ro.exe
          mv AbsSqrt_ro.exe AbsSqrt_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbsSqrt_ro.org /out AbsSqrt_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AbsSqrt_ro.org /out AbsSqrt_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AbsSqrt_ro.reflect.xml
  if [ -f AbsSqrt_ro.reflect.xml ]
then
    ReflectionRoots="-x AbsSqrt_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AbsSqrt_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AbsSqrt_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AbsSqrt_ro.asm.exe $CLRTestExecutionArguments
