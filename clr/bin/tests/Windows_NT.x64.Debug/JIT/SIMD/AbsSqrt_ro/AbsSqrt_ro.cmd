...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AbsSqrt_ro AbsSqrt_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\AbsSqrt_ro AbsSqrt_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AbsSqrt_ro.dasm.il AbsSqrt_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AbsSqrt_ro.dasm.il AbsSqrt_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AbsSqrt_ro.asm.exe  AbsSqrt_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AbsSqrt_ro.asm.exe  AbsSqrt_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AbsSqrt_ro
    if not exist "AbsSqrt_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AbsSqrt_ro.org" (
            mkdir IL
            copy AbsSqrt_ro.exe IL\AbsSqrt_ro.exe 
            ren AbsSqrt_ro.exe AbsSqrt_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbsSqrt_ro.org /out AbsSqrt_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AbsSqrt_ro.org /out AbsSqrt_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AbsSqrt_ro.reflect.xml
    IF EXIST AbsSqrt_ro.reflect.xml set ReflectionRoots=-x AbsSqrt_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\AbsSqrt_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AbsSqrt_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AbsSqrt_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
