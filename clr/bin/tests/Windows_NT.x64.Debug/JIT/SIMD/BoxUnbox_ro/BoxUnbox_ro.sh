...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/BoxUnbox_ro BoxUnbox_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/BoxUnbox_ro BoxUnbox_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=BoxUnbox_ro.dasm.il BoxUnbox_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=BoxUnbox_ro.dasm.il BoxUnbox_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=BoxUnbox_ro.asm.exe  BoxUnbox_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=BoxUnbox_ro.asm.exe  BoxUnbox_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=BoxUnbox_ro
    if [ ! -f BoxUnbox_ro.org ]; then
        TakeLock
        if [ ! -f BoxUnbox_ro.org ]; then
          mkdir IL
          cp BoxUnbox_ro.exe IL/BoxUnbox_ro.exe
          mv BoxUnbox_ro.exe BoxUnbox_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BoxUnbox_ro.org /out BoxUnbox_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BoxUnbox_ro.org /out BoxUnbox_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in BoxUnbox_ro.reflect.xml
  if [ -f BoxUnbox_ro.reflect.xml ]
then
    ReflectionRoots="-x BoxUnbox_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/BoxUnbox_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" BoxUnbox_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" BoxUnbox_ro.asm.exe $CLRTestExecutionArguments
