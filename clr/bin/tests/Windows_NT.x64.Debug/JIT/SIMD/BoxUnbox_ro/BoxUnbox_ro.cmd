...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\BoxUnbox_ro BoxUnbox_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\BoxUnbox_ro BoxUnbox_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=BoxUnbox_ro.dasm.il BoxUnbox_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=BoxUnbox_ro.dasm.il BoxUnbox_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=BoxUnbox_ro.asm.exe  BoxUnbox_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=BoxUnbox_ro.asm.exe  BoxUnbox_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=BoxUnbox_ro
    if not exist "BoxUnbox_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "BoxUnbox_ro.org" (
            mkdir IL
            copy BoxUnbox_ro.exe IL\BoxUnbox_ro.exe 
            ren BoxUnbox_ro.exe BoxUnbox_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BoxUnbox_ro.org /out BoxUnbox_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BoxUnbox_ro.org /out BoxUnbox_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in BoxUnbox_ro.reflect.xml
    IF EXIST BoxUnbox_ro.reflect.xml set ReflectionRoots=-x BoxUnbox_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\BoxUnbox_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% BoxUnbox_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% BoxUnbox_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
