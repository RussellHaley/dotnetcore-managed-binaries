...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSub_r VectorSub_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorSub_r VectorSub_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorSub_r.dasm.il VectorSub_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorSub_r.dasm.il VectorSub_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorSub_r.asm.exe  VectorSub_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorSub_r.asm.exe  VectorSub_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorSub_r
    if not exist "VectorSub_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorSub_r.org" (
            mkdir IL
            copy VectorSub_r.exe IL\VectorSub_r.exe 
            ren VectorSub_r.exe VectorSub_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSub_r.org /out VectorSub_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorSub_r.org /out VectorSub_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorSub_r.reflect.xml
    IF EXIST VectorSub_r.reflect.xml set ReflectionRoots=-x VectorSub_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorSub_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorSub_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorSub_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
