...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSub_r VectorSub_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorSub_r VectorSub_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorSub_r.dasm.il VectorSub_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorSub_r.dasm.il VectorSub_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorSub_r.asm.exe  VectorSub_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorSub_r.asm.exe  VectorSub_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorSub_r
    if [ ! -f VectorSub_r.org ]; then
        TakeLock
        if [ ! -f VectorSub_r.org ]; then
          mkdir IL
          cp VectorSub_r.exe IL/VectorSub_r.exe
          mv VectorSub_r.exe VectorSub_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSub_r.org /out VectorSub_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorSub_r.org /out VectorSub_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorSub_r.reflect.xml
  if [ -f VectorSub_r.reflect.xml ]
then
    ReflectionRoots="-x VectorSub_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorSub_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorSub_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorSub_r.asm.exe $CLRTestExecutionArguments
