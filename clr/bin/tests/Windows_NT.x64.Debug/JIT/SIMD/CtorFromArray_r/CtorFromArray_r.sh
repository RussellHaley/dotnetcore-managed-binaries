...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CtorFromArray_r CtorFromArray_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CtorFromArray_r CtorFromArray_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CtorFromArray_r.dasm.il CtorFromArray_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=CtorFromArray_r.dasm.il CtorFromArray_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CtorFromArray_r.asm.exe  CtorFromArray_r.dasm.il
    "$CORE_ROOT/ilasm" -output=CtorFromArray_r.asm.exe  CtorFromArray_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CtorFromArray_r
    if [ ! -f CtorFromArray_r.org ]; then
        TakeLock
        if [ ! -f CtorFromArray_r.org ]; then
          mkdir IL
          cp CtorFromArray_r.exe IL/CtorFromArray_r.exe
          mv CtorFromArray_r.exe CtorFromArray_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CtorFromArray_r.org /out CtorFromArray_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CtorFromArray_r.org /out CtorFromArray_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CtorFromArray_r.reflect.xml
  if [ -f CtorFromArray_r.reflect.xml ]
then
    ReflectionRoots="-x CtorFromArray_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CtorFromArray_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CtorFromArray_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CtorFromArray_r.asm.exe $CLRTestExecutionArguments
