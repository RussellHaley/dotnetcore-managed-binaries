...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CtorFromArray_r CtorFromArray_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CtorFromArray_r CtorFromArray_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CtorFromArray_r.dasm.il CtorFromArray_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CtorFromArray_r.dasm.il CtorFromArray_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CtorFromArray_r.asm.exe  CtorFromArray_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CtorFromArray_r.asm.exe  CtorFromArray_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CtorFromArray_r
    if not exist "CtorFromArray_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CtorFromArray_r.org" (
            mkdir IL
            copy CtorFromArray_r.exe IL\CtorFromArray_r.exe 
            ren CtorFromArray_r.exe CtorFromArray_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CtorFromArray_r.org /out CtorFromArray_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CtorFromArray_r.org /out CtorFromArray_r.exe
...
set ExePath=CtorFromArray_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CtorFromArray_r.reflect.xml
    IF EXIST CtorFromArray_r.reflect.xml set ReflectionRoots=-x CtorFromArray_r.reflect.xml
    ...
    set ExePath=%LinkBin%\CtorFromArray_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CtorFromArray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CtorFromArray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
