...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CircleInConvex_r CircleInConvex_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/CircleInConvex_r CircleInConvex_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CircleInConvex_r.dasm.il CircleInConvex_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=CircleInConvex_r.dasm.il CircleInConvex_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CircleInConvex_r.asm.exe  CircleInConvex_r.dasm.il
    "$CORE_ROOT/ilasm" -output=CircleInConvex_r.asm.exe  CircleInConvex_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CircleInConvex_r
    if [ ! -f CircleInConvex_r.org ]; then
        TakeLock
        if [ ! -f CircleInConvex_r.org ]; then
          mkdir IL
          cp CircleInConvex_r.exe IL/CircleInConvex_r.exe
          mv CircleInConvex_r.exe CircleInConvex_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CircleInConvex_r.org /out CircleInConvex_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CircleInConvex_r.org /out CircleInConvex_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CircleInConvex_r.reflect.xml
  if [ -f CircleInConvex_r.reflect.xml ]
then
    ReflectionRoots="-x CircleInConvex_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CircleInConvex_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CircleInConvex_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CircleInConvex_r.asm.exe $CLRTestExecutionArguments
