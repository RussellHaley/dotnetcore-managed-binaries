...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CircleInConvex_r CircleInConvex_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\CircleInConvex_r CircleInConvex_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CircleInConvex_r.dasm.il CircleInConvex_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CircleInConvex_r.dasm.il CircleInConvex_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CircleInConvex_r.asm.exe  CircleInConvex_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CircleInConvex_r.asm.exe  CircleInConvex_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CircleInConvex_r
    if not exist "CircleInConvex_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CircleInConvex_r.org" (
            mkdir IL
            copy CircleInConvex_r.exe IL\CircleInConvex_r.exe 
            ren CircleInConvex_r.exe CircleInConvex_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CircleInConvex_r.org /out CircleInConvex_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CircleInConvex_r.org /out CircleInConvex_r.exe
...
set ExePath=CircleInConvex_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CircleInConvex_r.reflect.xml
    IF EXIST CircleInConvex_r.reflect.xml set ReflectionRoots=-x CircleInConvex_r.reflect.xml
    ...
    set ExePath=%LinkBin%\CircleInConvex_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CircleInConvex_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CircleInConvex_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
