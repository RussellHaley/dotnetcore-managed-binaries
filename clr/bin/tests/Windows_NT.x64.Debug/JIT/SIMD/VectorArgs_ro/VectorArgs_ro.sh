...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArgs_ro VectorArgs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArgs_ro VectorArgs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorArgs_ro.dasm.il VectorArgs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorArgs_ro.dasm.il VectorArgs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorArgs_ro.asm.exe  VectorArgs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorArgs_ro.asm.exe  VectorArgs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorArgs_ro
    if [ ! -f VectorArgs_ro.org ]; then
        TakeLock
        if [ ! -f VectorArgs_ro.org ]; then
          mkdir IL
          cp VectorArgs_ro.exe IL/VectorArgs_ro.exe
          mv VectorArgs_ro.exe VectorArgs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArgs_ro.org /out VectorArgs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArgs_ro.org /out VectorArgs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorArgs_ro.reflect.xml
  if [ -f VectorArgs_ro.reflect.xml ]
then
    ReflectionRoots="-x VectorArgs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorArgs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorArgs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorArgs_ro.asm.exe $CLRTestExecutionArguments
