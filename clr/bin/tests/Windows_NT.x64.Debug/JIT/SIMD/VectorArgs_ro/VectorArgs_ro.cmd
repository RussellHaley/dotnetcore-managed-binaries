...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArgs_ro VectorArgs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArgs_ro VectorArgs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorArgs_ro.dasm.il VectorArgs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorArgs_ro.dasm.il VectorArgs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorArgs_ro.asm.exe  VectorArgs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorArgs_ro.asm.exe  VectorArgs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorArgs_ro
    if not exist "VectorArgs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorArgs_ro.org" (
            mkdir IL
            copy VectorArgs_ro.exe IL\VectorArgs_ro.exe 
            ren VectorArgs_ro.exe VectorArgs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArgs_ro.org /out VectorArgs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArgs_ro.org /out VectorArgs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorArgs_ro.reflect.xml
    IF EXIST VectorArgs_ro.reflect.xml set ReflectionRoots=-x VectorArgs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorArgs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorArgs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorArgs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
