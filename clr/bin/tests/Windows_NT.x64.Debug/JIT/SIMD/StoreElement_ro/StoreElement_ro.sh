...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/StoreElement_ro StoreElement_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/StoreElement_ro StoreElement_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=StoreElement_ro.dasm.il StoreElement_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=StoreElement_ro.dasm.il StoreElement_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=StoreElement_ro.asm.exe  StoreElement_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=StoreElement_ro.asm.exe  StoreElement_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=StoreElement_ro
    if [ ! -f StoreElement_ro.org ]; then
        TakeLock
        if [ ! -f StoreElement_ro.org ]; then
          mkdir IL
          cp StoreElement_ro.exe IL/StoreElement_ro.exe
          mv StoreElement_ro.exe StoreElement_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in StoreElement_ro.org /out StoreElement_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in StoreElement_ro.org /out StoreElement_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in StoreElement_ro.reflect.xml
  if [ -f StoreElement_ro.reflect.xml ]
then
    ReflectionRoots="-x StoreElement_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/StoreElement_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" StoreElement_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" StoreElement_ro.asm.exe $CLRTestExecutionArguments
