...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\StoreElement_ro StoreElement_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\StoreElement_ro StoreElement_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=StoreElement_ro.dasm.il StoreElement_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=StoreElement_ro.dasm.il StoreElement_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=StoreElement_ro.asm.exe  StoreElement_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=StoreElement_ro.asm.exe  StoreElement_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=StoreElement_ro
    if not exist "StoreElement_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "StoreElement_ro.org" (
            mkdir IL
            copy StoreElement_ro.exe IL\StoreElement_ro.exe 
            ren StoreElement_ro.exe StoreElement_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in StoreElement_ro.org /out StoreElement_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in StoreElement_ro.org /out StoreElement_ro.exe
...
set ExePath=StoreElement_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in StoreElement_ro.reflect.xml
    IF EXIST StoreElement_ro.reflect.xml set ReflectionRoots=-x StoreElement_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\StoreElement_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% StoreElement_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% StoreElement_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
