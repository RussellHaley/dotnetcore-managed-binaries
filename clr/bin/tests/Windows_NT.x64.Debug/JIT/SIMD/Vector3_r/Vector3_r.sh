...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector3_r Vector3_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/Vector3_r Vector3_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Vector3_r.dasm.il Vector3_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=Vector3_r.dasm.il Vector3_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Vector3_r.asm.exe  Vector3_r.dasm.il
    "$CORE_ROOT/ilasm" -output=Vector3_r.asm.exe  Vector3_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Vector3_r
    if [ ! -f Vector3_r.org ]; then
        TakeLock
        if [ ! -f Vector3_r.org ]; then
          mkdir IL
          cp Vector3_r.exe IL/Vector3_r.exe
          mv Vector3_r.exe Vector3_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector3_r.org /out Vector3_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector3_r.org /out Vector3_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Vector3_r.reflect.xml
  if [ -f Vector3_r.reflect.xml ]
then
    ReflectionRoots="-x Vector3_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Vector3_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Vector3_r.asm.exe $CLRTestExecutionArguments
