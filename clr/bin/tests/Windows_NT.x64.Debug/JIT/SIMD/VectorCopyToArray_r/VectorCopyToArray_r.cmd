...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorCopyToArray_r VectorCopyToArray_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorCopyToArray_r VectorCopyToArray_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorCopyToArray_r.dasm.il VectorCopyToArray_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorCopyToArray_r.dasm.il VectorCopyToArray_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorCopyToArray_r.asm.exe  VectorCopyToArray_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorCopyToArray_r.asm.exe  VectorCopyToArray_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorCopyToArray_r
    if not exist "VectorCopyToArray_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorCopyToArray_r.org" (
            mkdir IL
            copy VectorCopyToArray_r.exe IL\VectorCopyToArray_r.exe 
            ren VectorCopyToArray_r.exe VectorCopyToArray_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorCopyToArray_r.org /out VectorCopyToArray_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorCopyToArray_r.org /out VectorCopyToArray_r.exe
...
set ExePath=VectorCopyToArray_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorCopyToArray_r.reflect.xml
    IF EXIST VectorCopyToArray_r.reflect.xml set ReflectionRoots=-x VectorCopyToArray_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorCopyToArray_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorCopyToArray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorCopyToArray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
