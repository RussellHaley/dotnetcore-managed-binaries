...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorCopyToArray_r VectorCopyToArray_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorCopyToArray_r VectorCopyToArray_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorCopyToArray_r.dasm.il VectorCopyToArray_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorCopyToArray_r.dasm.il VectorCopyToArray_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorCopyToArray_r.asm.exe  VectorCopyToArray_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorCopyToArray_r.asm.exe  VectorCopyToArray_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorCopyToArray_r
    if [ ! -f VectorCopyToArray_r.org ]; then
        TakeLock
        if [ ! -f VectorCopyToArray_r.org ]; then
          mkdir IL
          cp VectorCopyToArray_r.exe IL/VectorCopyToArray_r.exe
          mv VectorCopyToArray_r.exe VectorCopyToArray_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorCopyToArray_r.org /out VectorCopyToArray_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorCopyToArray_r.org /out VectorCopyToArray_r.exe
          __cgExitCode=$?
...
ExePath=VectorCopyToArray_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in VectorCopyToArray_r.reflect.xml
  if [ -f VectorCopyToArray_r.reflect.xml ]
then
    ReflectionRoots="-x VectorCopyToArray_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorCopyToArray_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorCopyToArray_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorCopyToArray_r.asm.exe $CLRTestExecutionArguments
