...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArray_r VectorArray_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/VectorArray_r VectorArray_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VectorArray_r.dasm.il VectorArray_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=VectorArray_r.dasm.il VectorArray_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VectorArray_r.asm.exe  VectorArray_r.dasm.il
    "$CORE_ROOT/ilasm" -output=VectorArray_r.asm.exe  VectorArray_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VectorArray_r
    if [ ! -f VectorArray_r.org ]; then
        TakeLock
        if [ ! -f VectorArray_r.org ]; then
          mkdir IL
          cp VectorArray_r.exe IL/VectorArray_r.exe
          mv VectorArray_r.exe VectorArray_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArray_r.org /out VectorArray_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VectorArray_r.org /out VectorArray_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in VectorArray_r.reflect.xml
  if [ -f VectorArray_r.reflect.xml ]
then
    ReflectionRoots="-x VectorArray_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VectorArray_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VectorArray_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VectorArray_r.asm.exe $CLRTestExecutionArguments
