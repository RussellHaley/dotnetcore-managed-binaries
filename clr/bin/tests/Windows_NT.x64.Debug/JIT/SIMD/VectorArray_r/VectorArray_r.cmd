...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArray_r VectorArray_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\VectorArray_r VectorArray_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VectorArray_r.dasm.il VectorArray_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VectorArray_r.dasm.il VectorArray_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VectorArray_r.asm.exe  VectorArray_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VectorArray_r.asm.exe  VectorArray_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VectorArray_r
    if not exist "VectorArray_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VectorArray_r.org" (
            mkdir IL
            copy VectorArray_r.exe IL\VectorArray_r.exe 
            ren VectorArray_r.exe VectorArray_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArray_r.org /out VectorArray_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VectorArray_r.org /out VectorArray_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VectorArray_r.reflect.xml
    IF EXIST VectorArray_r.reflect.xml set ReflectionRoots=-x VectorArray_r.reflect.xml
    ...
    set ExePath=%LinkBin%\VectorArray_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VectorArray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VectorArray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
