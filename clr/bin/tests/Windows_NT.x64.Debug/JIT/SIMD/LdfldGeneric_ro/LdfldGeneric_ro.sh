...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/LdfldGeneric_ro LdfldGeneric_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/SIMD/LdfldGeneric_ro LdfldGeneric_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LdfldGeneric_ro.dasm.il LdfldGeneric_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=LdfldGeneric_ro.dasm.il LdfldGeneric_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LdfldGeneric_ro.asm.exe  LdfldGeneric_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=LdfldGeneric_ro.asm.exe  LdfldGeneric_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LdfldGeneric_ro
    if [ ! -f LdfldGeneric_ro.org ]; then
        TakeLock
        if [ ! -f LdfldGeneric_ro.org ]; then
          mkdir IL
          cp LdfldGeneric_ro.exe IL/LdfldGeneric_ro.exe
          mv LdfldGeneric_ro.exe LdfldGeneric_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LdfldGeneric_ro.org /out LdfldGeneric_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LdfldGeneric_ro.org /out LdfldGeneric_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LdfldGeneric_ro.reflect.xml
  if [ -f LdfldGeneric_ro.reflect.xml ]
then
    ReflectionRoots="-x LdfldGeneric_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/LdfldGeneric_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LdfldGeneric_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LdfldGeneric_ro.asm.exe $CLRTestExecutionArguments
