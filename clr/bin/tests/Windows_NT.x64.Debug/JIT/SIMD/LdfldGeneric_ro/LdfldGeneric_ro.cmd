...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\LdfldGeneric_ro LdfldGeneric_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\SIMD\LdfldGeneric_ro LdfldGeneric_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LdfldGeneric_ro.dasm.il LdfldGeneric_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LdfldGeneric_ro.dasm.il LdfldGeneric_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LdfldGeneric_ro.asm.exe  LdfldGeneric_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LdfldGeneric_ro.asm.exe  LdfldGeneric_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LdfldGeneric_ro
    if not exist "LdfldGeneric_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LdfldGeneric_ro.org" (
            mkdir IL
            copy LdfldGeneric_ro.exe IL\LdfldGeneric_ro.exe 
            ren LdfldGeneric_ro.exe LdfldGeneric_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LdfldGeneric_ro.org /out LdfldGeneric_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LdfldGeneric_ro.org /out LdfldGeneric_ro.exe
...
set ExePath=LdfldGeneric_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LdfldGeneric_ro.reflect.xml
    IF EXIST LdfldGeneric_ro.reflect.xml set ReflectionRoots=-x LdfldGeneric_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\LdfldGeneric_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LdfldGeneric_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LdfldGeneric_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
