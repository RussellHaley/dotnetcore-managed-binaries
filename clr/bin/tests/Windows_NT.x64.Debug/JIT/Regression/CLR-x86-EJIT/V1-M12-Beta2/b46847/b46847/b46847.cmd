...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-EJIT\V1-M12-Beta2\b46847\b46847 b46847.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-EJIT\V1-M12-Beta2\b46847\b46847 b46847.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b46847.dasm.il b46847.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b46847.dasm.il b46847.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b46847.asm.exe  b46847.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b46847.asm.exe  b46847.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b46847
    if not exist "b46847.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b46847.org" (
            mkdir IL
            copy b46847.exe IL\b46847.exe 
            ren b46847.exe b46847.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b46847.org /out b46847.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b46847.org /out b46847.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b46847.reflect.xml
    IF EXIST b46847.reflect.xml set ReflectionRoots=-x b46847.reflect.xml
    ...
    set ExePath=%LinkBin%\b46847.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b46847.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b46847.asm.exe %CLRTestExecutionArguments% %Host_Args%
