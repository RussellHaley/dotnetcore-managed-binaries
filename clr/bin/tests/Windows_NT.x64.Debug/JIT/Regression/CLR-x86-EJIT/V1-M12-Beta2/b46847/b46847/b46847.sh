...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-EJIT/V1-M12-Beta2/b46847/b46847 b46847.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-EJIT/V1-M12-Beta2/b46847/b46847 b46847.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b46847.dasm.il b46847.exe
    "$CORE_ROOT/ildasm" -raweh -out=b46847.dasm.il b46847.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b46847.asm.exe  b46847.dasm.il
    "$CORE_ROOT/ilasm" -output=b46847.asm.exe  b46847.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b46847
    if [ ! -f b46847.org ]; then
        TakeLock
        if [ ! -f b46847.org ]; then
          mkdir IL
          cp b46847.exe IL/b46847.exe
          mv b46847.exe b46847.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b46847.org /out b46847.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b46847.org /out b46847.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b46847.reflect.xml
  if [ -f b46847.reflect.xml ]
then
    ReflectionRoots="-x b46847.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b46847.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b46847.asm.exe $CLRTestExecutionArguments
