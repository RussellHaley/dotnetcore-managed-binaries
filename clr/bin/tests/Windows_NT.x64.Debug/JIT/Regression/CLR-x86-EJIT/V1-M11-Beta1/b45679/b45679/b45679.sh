...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-EJIT/V1-M11-Beta1/b45679/b45679 b45679.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-EJIT/V1-M11-Beta1/b45679/b45679 b45679.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b45679.dasm.il b45679.exe
    "$CORE_ROOT/ildasm" -raweh -out=b45679.dasm.il b45679.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b45679.asm.exe  b45679.dasm.il
    "$CORE_ROOT/ilasm" -output=b45679.asm.exe  b45679.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b45679
    if [ ! -f b45679.org ]; then
        TakeLock
        if [ ! -f b45679.org ]; then
          mkdir IL
          cp b45679.exe IL/b45679.exe
          mv b45679.exe b45679.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b45679.org /out b45679.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b45679.org /out b45679.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b45679.reflect.xml
  if [ -f b45679.reflect.xml ]
then
    ReflectionRoots="-x b45679.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b45679.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b45679.asm.exe $CLRTestExecutionArguments
