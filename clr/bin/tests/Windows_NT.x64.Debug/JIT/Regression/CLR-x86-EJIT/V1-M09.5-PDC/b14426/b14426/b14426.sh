...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-EJIT/V1-M09.5-PDC/b14426/b14426 b14426.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-EJIT/V1-M09.5-PDC/b14426/b14426 b14426.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b14426.dasm.il b14426.exe
    "$CORE_ROOT/ildasm" -raweh -out=b14426.dasm.il b14426.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b14426.asm.exe  b14426.dasm.il
    "$CORE_ROOT/ilasm" -output=b14426.asm.exe  b14426.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b14426
    if [ ! -f b14426.org ]; then
        TakeLock
        if [ ! -f b14426.org ]; then
          mkdir IL
          cp b14426.exe IL/b14426.exe
          mv b14426.exe b14426.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b14426.org /out b14426.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b14426.org /out b14426.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b14426.reflect.xml
  if [ -f b14426.reflect.xml ]
then
    ReflectionRoots="-x b14426.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b14426.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b14426.asm.exe $CLRTestExecutionArguments
