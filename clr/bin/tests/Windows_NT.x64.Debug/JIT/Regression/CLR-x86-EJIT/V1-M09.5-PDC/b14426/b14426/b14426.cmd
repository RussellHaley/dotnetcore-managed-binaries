...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-EJIT\V1-M09.5-PDC\b14426\b14426 b14426.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-EJIT\V1-M09.5-PDC\b14426\b14426 b14426.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b14426.dasm.il b14426.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b14426.dasm.il b14426.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b14426.asm.exe  b14426.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b14426.asm.exe  b14426.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b14426
    if not exist "b14426.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b14426.org" (
            mkdir IL
            copy b14426.exe IL\b14426.exe 
            ren b14426.exe b14426.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b14426.org /out b14426.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b14426.org /out b14426.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b14426.reflect.xml
    IF EXIST b14426.reflect.xml set ReflectionRoots=-x b14426.reflect.xml
    ...
    set ExePath=%LinkBin%\b14426.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b14426.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b14426.asm.exe %CLRTestExecutionArguments% %Host_Args%
