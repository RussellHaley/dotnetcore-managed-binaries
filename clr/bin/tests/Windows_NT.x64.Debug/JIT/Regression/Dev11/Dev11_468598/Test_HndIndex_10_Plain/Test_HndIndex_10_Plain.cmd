...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\Dev11\Dev11_468598\Test_HndIndex_10_Plain Test_HndIndex_10_Plain.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\Dev11\Dev11_468598\Test_HndIndex_10_Plain Test_HndIndex_10_Plain.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Test_HndIndex_10_Plain.dasm.il Test_HndIndex_10_Plain.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Test_HndIndex_10_Plain.dasm.il Test_HndIndex_10_Plain.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Test_HndIndex_10_Plain.asm.exe -QUIET -NOLOGO -DEBUG Test_HndIndex_10_Plain.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Test_HndIndex_10_Plain.asm.exe -QUIET -NOLOGO -DEBUG Test_HndIndex_10_Plain.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Test_HndIndex_10_Plain
    if not exist "Test_HndIndex_10_Plain.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Test_HndIndex_10_Plain.org" (
            mkdir IL
            copy Test_HndIndex_10_Plain.exe IL\Test_HndIndex_10_Plain.exe 
            ren Test_HndIndex_10_Plain.exe Test_HndIndex_10_Plain.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Test_HndIndex_10_Plain.org /out Test_HndIndex_10_Plain.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Test_HndIndex_10_Plain.org /out Test_HndIndex_10_Plain.exe
...
set ExePath=Test_HndIndex_10_Plain.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Test_HndIndex_10_Plain.reflect.xml
    IF EXIST Test_HndIndex_10_Plain.reflect.xml set ReflectionRoots=-x Test_HndIndex_10_Plain.reflect.xml
    ...
    set ExePath=%LinkBin%\Test_HndIndex_10_Plain.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Test_HndIndex_10_Plain.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Test_HndIndex_10_Plain.asm.exe %CLRTestExecutionArguments% %Host_Args%
