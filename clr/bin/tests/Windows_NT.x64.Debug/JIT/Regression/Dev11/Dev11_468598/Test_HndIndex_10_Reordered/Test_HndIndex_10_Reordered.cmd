...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\Dev11\Dev11_468598\Test_HndIndex_10_Reordered Test_HndIndex_10_Reordered.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\Dev11\Dev11_468598\Test_HndIndex_10_Reordered Test_HndIndex_10_Reordered.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Test_HndIndex_10_Reordered.dasm.il Test_HndIndex_10_Reordered.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Test_HndIndex_10_Reordered.dasm.il Test_HndIndex_10_Reordered.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Test_HndIndex_10_Reordered.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Test_HndIndex_10_Reordered.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Test_HndIndex_10_Reordered.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Test_HndIndex_10_Reordered.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Test_HndIndex_10_Reordered
    if not exist "Test_HndIndex_10_Reordered.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Test_HndIndex_10_Reordered.org" (
            mkdir IL
            copy Test_HndIndex_10_Reordered.exe IL\Test_HndIndex_10_Reordered.exe 
            ren Test_HndIndex_10_Reordered.exe Test_HndIndex_10_Reordered.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Test_HndIndex_10_Reordered.org /out Test_HndIndex_10_Reordered.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Test_HndIndex_10_Reordered.org /out Test_HndIndex_10_Reordered.exe
...
set ExePath=Test_HndIndex_10_Reordered.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Test_HndIndex_10_Reordered.reflect.xml
    IF EXIST Test_HndIndex_10_Reordered.reflect.xml set ReflectionRoots=-x Test_HndIndex_10_Reordered.reflect.xml
    ...
    set ExePath=%LinkBin%\Test_HndIndex_10_Reordered.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Test_HndIndex_10_Reordered.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Test_HndIndex_10_Reordered.asm.exe %CLRTestExecutionArguments% %Host_Args%
