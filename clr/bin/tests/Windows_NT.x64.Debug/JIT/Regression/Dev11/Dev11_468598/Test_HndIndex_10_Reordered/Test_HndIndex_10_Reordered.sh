...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/Dev11/Dev11_468598/Test_HndIndex_10_Reordered Test_HndIndex_10_Reordered.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/Dev11/Dev11_468598/Test_HndIndex_10_Reordered Test_HndIndex_10_Reordered.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Test_HndIndex_10_Reordered.dasm.il Test_HndIndex_10_Reordered.exe
    "$CORE_ROOT/ildasm" -raweh -out=Test_HndIndex_10_Reordered.dasm.il Test_HndIndex_10_Reordered.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Test_HndIndex_10_Reordered.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Test_HndIndex_10_Reordered.dasm.il
    "$CORE_ROOT/ilasm" -output=Test_HndIndex_10_Reordered.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Test_HndIndex_10_Reordered.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Test_HndIndex_10_Reordered
    if [ ! -f Test_HndIndex_10_Reordered.org ]; then
        TakeLock
        if [ ! -f Test_HndIndex_10_Reordered.org ]; then
          mkdir IL
          cp Test_HndIndex_10_Reordered.exe IL/Test_HndIndex_10_Reordered.exe
          mv Test_HndIndex_10_Reordered.exe Test_HndIndex_10_Reordered.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Test_HndIndex_10_Reordered.org /out Test_HndIndex_10_Reordered.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Test_HndIndex_10_Reordered.org /out Test_HndIndex_10_Reordered.exe
          __cgExitCode=$?
...
ExePath=Test_HndIndex_10_Reordered.exe
...
then
  ...
  # Use hints for reflection roots, if provided in Test_HndIndex_10_Reordered.reflect.xml
  if [ -f Test_HndIndex_10_Reordered.reflect.xml ]
then
    ReflectionRoots="-x Test_HndIndex_10_Reordered.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Test_HndIndex_10_Reordered.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Test_HndIndex_10_Reordered.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Test_HndIndex_10_Reordered.asm.exe $CLRTestExecutionArguments
