...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/Dev11/dev11_4421/Dev11_4421 Dev11_4421.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/Dev11/dev11_4421/Dev11_4421 Dev11_4421.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Dev11_4421.dasm.il Dev11_4421.exe
    "$CORE_ROOT/ildasm" -raweh -out=Dev11_4421.dasm.il Dev11_4421.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Dev11_4421.asm.exe  Dev11_4421.dasm.il
    "$CORE_ROOT/ilasm" -output=Dev11_4421.asm.exe  Dev11_4421.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Dev11_4421
    if [ ! -f Dev11_4421.org ]; then
        TakeLock
        if [ ! -f Dev11_4421.org ]; then
          mkdir IL
          cp Dev11_4421.exe IL/Dev11_4421.exe
          mv Dev11_4421.exe Dev11_4421.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Dev11_4421.org /out Dev11_4421.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Dev11_4421.org /out Dev11_4421.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Dev11_4421.reflect.xml
  if [ -f Dev11_4421.reflect.xml ]
then
    ReflectionRoots="-x Dev11_4421.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Dev11_4421.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Dev11_4421.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Dev11_4421.asm.exe $CLRTestExecutionArguments
