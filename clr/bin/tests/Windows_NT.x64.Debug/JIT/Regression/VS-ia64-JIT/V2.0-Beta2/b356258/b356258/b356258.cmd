...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V2.0-Beta2\b356258\b356258 b356258.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V2.0-Beta2\b356258\b356258 b356258.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b356258.dasm.il b356258.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b356258.dasm.il b356258.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b356258.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b356258.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b356258.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b356258.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b356258
    if not exist "b356258.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b356258.org" (
            mkdir IL
            copy b356258.exe IL\b356258.exe 
            ren b356258.exe b356258.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b356258.org /out b356258.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b356258.org /out b356258.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b356258.reflect.xml
    IF EXIST b356258.reflect.xml set ReflectionRoots=-x b356258.reflect.xml
    ...
    set ExePath=%LinkBin%\b356258.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b356258.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b356258.asm.exe %CLRTestExecutionArguments% %Host_Args%
