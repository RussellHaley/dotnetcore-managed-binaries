...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-Beta2/b309576/b309576 b309576.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-Beta2/b309576/b309576 b309576.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b309576.dasm.il b309576.exe
    "$CORE_ROOT/ildasm" -raweh -out=b309576.dasm.il b309576.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b309576.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b309576.dasm.il
    "$CORE_ROOT/ilasm" -output=b309576.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b309576.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b309576
    if [ ! -f b309576.org ]; then
        TakeLock
        if [ ! -f b309576.org ]; then
          mkdir IL
          cp b309576.exe IL/b309576.exe
          mv b309576.exe b309576.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b309576.org /out b309576.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b309576.org /out b309576.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b309576.reflect.xml
  if [ -f b309576.reflect.xml ]
then
    ReflectionRoots="-x b309576.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b309576.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b309576.asm.exe $CLRTestExecutionArguments
