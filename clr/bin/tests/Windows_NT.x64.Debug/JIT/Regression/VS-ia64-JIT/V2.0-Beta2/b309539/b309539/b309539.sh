...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-Beta2/b309539/b309539 b309539.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-Beta2/b309539/b309539 b309539.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b309539.dasm.il b309539.exe
    "$CORE_ROOT/ildasm" -raweh -out=b309539.dasm.il b309539.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b309539.asm.exe  b309539.dasm.il
    "$CORE_ROOT/ilasm" -output=b309539.asm.exe  b309539.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b309539
    if [ ! -f b309539.org ]; then
        TakeLock
        if [ ! -f b309539.org ]; then
          mkdir IL
          cp b309539.exe IL/b309539.exe
          mv b309539.exe b309539.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b309539.org /out b309539.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b309539.org /out b309539.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b309539.reflect.xml
  if [ -f b309539.reflect.xml ]
then
    ReflectionRoots="-x b309539.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b309539.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b309539.asm.exe $CLRTestExecutionArguments
