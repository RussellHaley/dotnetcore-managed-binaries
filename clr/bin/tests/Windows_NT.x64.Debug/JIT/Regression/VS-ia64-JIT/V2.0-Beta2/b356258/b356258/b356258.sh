...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-Beta2/b356258/b356258 b356258.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-Beta2/b356258/b356258 b356258.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b356258.dasm.il b356258.exe
    "$CORE_ROOT/ildasm" -raweh -out=b356258.dasm.il b356258.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b356258.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b356258.dasm.il
    "$CORE_ROOT/ilasm" -output=b356258.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b356258.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b356258
    if [ ! -f b356258.org ]; then
        TakeLock
        if [ ! -f b356258.org ]; then
          mkdir IL
          cp b356258.exe IL/b356258.exe
          mv b356258.exe b356258.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b356258.org /out b356258.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b356258.org /out b356258.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b356258.reflect.xml
  if [ -f b356258.reflect.xml ]
then
    ReflectionRoots="-x b356258.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b356258.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b356258.asm.exe $CLRTestExecutionArguments
