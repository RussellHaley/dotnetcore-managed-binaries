...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V2.0-Beta2\b302558\b302558 b302558.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V2.0-Beta2\b302558\b302558 b302558.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b302558.dasm.il b302558.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b302558.dasm.il b302558.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b302558.asm.exe  b302558.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b302558.asm.exe  b302558.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b302558
    if not exist "b302558.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b302558.org" (
            mkdir IL
            copy b302558.exe IL\b302558.exe 
            ren b302558.exe b302558.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b302558.org /out b302558.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b302558.org /out b302558.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b302558.reflect.xml
    IF EXIST b302558.reflect.xml set ReflectionRoots=-x b302558.reflect.xml
    ...
    set ExePath=%LinkBin%\b302558.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b302558.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b302558.asm.exe %CLRTestExecutionArguments% %Host_Args%
