...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-Beta2/b302558/b302558 b302558.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-Beta2/b302558/b302558 b302558.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b302558.dasm.il b302558.exe
    "$CORE_ROOT/ildasm" -raweh -out=b302558.dasm.il b302558.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b302558.asm.exe  b302558.dasm.il
    "$CORE_ROOT/ilasm" -output=b302558.asm.exe  b302558.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b302558
    if [ ! -f b302558.org ]; then
        TakeLock
        if [ ! -f b302558.org ]; then
          mkdir IL
          cp b302558.exe IL/b302558.exe
          mv b302558.exe b302558.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b302558.org /out b302558.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b302558.org /out b302558.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b302558.reflect.xml
  if [ -f b302558.reflect.xml ]
then
    ReflectionRoots="-x b302558.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b302558.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b302558.asm.exe $CLRTestExecutionArguments
