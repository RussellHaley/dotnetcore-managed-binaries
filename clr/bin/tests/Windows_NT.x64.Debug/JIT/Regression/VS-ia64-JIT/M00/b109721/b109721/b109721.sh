...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b109721/b109721 b109721.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b109721/b109721 b109721.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b109721.dasm.il b109721.exe
    "$CORE_ROOT/ildasm" -raweh -out=b109721.dasm.il b109721.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b109721.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b109721.dasm.il
    "$CORE_ROOT/ilasm" -output=b109721.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b109721.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b109721
    if [ ! -f b109721.org ]; then
        TakeLock
        if [ ! -f b109721.org ]; then
          mkdir IL
          cp b109721.exe IL/b109721.exe
          mv b109721.exe b109721.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b109721.org /out b109721.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b109721.org /out b109721.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b109721.reflect.xml
  if [ -f b109721.reflect.xml ]
then
    ReflectionRoots="-x b109721.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b109721.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b109721.asm.exe $CLRTestExecutionArguments
