...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b99219\b99219 b99219.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b99219\b99219 b99219.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b99219.dasm.il b99219.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b99219.dasm.il b99219.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b99219.asm.exe  b99219.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b99219.asm.exe  b99219.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b99219
    if not exist "b99219.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b99219.org" (
            mkdir IL
            copy b99219.exe IL\b99219.exe 
            ren b99219.exe b99219.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b99219.org /out b99219.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b99219.org /out b99219.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b99219.reflect.xml
    IF EXIST b99219.reflect.xml set ReflectionRoots=-x b99219.reflect.xml
    ...
    set ExePath=%LinkBin%\b99219.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b99219.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b99219.asm.exe %CLRTestExecutionArguments% %Host_Args%
