...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b108366\b108366 b108366.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b108366\b108366 b108366.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b108366.dasm.il b108366.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b108366.dasm.il b108366.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b108366.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b108366.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b108366.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b108366.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b108366
    if not exist "b108366.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b108366.org" (
            mkdir IL
            copy b108366.exe IL\b108366.exe 
            ren b108366.exe b108366.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b108366.org /out b108366.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b108366.org /out b108366.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b108366.reflect.xml
    IF EXIST b108366.reflect.xml set ReflectionRoots=-x b108366.reflect.xml
    ...
    set ExePath=%LinkBin%\b108366.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b108366.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b108366.asm.exe %CLRTestExecutionArguments% %Host_Args%
