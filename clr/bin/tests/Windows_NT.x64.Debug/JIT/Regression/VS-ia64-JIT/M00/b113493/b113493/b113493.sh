...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b113493/b113493 b113493.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b113493/b113493 b113493.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b113493.dasm.il b113493.exe
    "$CORE_ROOT/ildasm" -raweh -out=b113493.dasm.il b113493.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b113493.asm.exe  b113493.dasm.il
    "$CORE_ROOT/ilasm" -output=b113493.asm.exe  b113493.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b113493
    if [ ! -f b113493.org ]; then
        TakeLock
        if [ ! -f b113493.org ]; then
          mkdir IL
          cp b113493.exe IL/b113493.exe
          mv b113493.exe b113493.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b113493.org /out b113493.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b113493.org /out b113493.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b113493.reflect.xml
  if [ -f b113493.reflect.xml ]
then
    ReflectionRoots="-x b113493.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b113493.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b113493.asm.exe $CLRTestExecutionArguments
