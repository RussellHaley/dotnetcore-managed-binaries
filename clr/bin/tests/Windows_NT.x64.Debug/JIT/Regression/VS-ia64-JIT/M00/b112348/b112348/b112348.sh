...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b112348/b112348 b112348.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b112348/b112348 b112348.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b112348.dasm.il b112348.exe
    "$CORE_ROOT/ildasm" -raweh -out=b112348.dasm.il b112348.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b112348.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b112348.dasm.il
    "$CORE_ROOT/ilasm" -output=b112348.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b112348.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b112348
    if [ ! -f b112348.org ]; then
        TakeLock
        if [ ! -f b112348.org ]; then
          mkdir IL
          cp b112348.exe IL/b112348.exe
          mv b112348.exe b112348.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b112348.org /out b112348.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b112348.org /out b112348.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b112348.reflect.xml
  if [ -f b112348.reflect.xml ]
then
    ReflectionRoots="-x b112348.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b112348.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b112348.asm.exe $CLRTestExecutionArguments
