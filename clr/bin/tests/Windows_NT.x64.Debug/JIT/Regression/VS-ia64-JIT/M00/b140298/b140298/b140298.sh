...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b140298/b140298 b140298.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b140298/b140298 b140298.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b140298.dasm.il b140298.exe
    "$CORE_ROOT/ildasm" -raweh -out=b140298.dasm.il b140298.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b140298.asm.exe  b140298.dasm.il
    "$CORE_ROOT/ilasm" -output=b140298.asm.exe  b140298.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b140298
    if [ ! -f b140298.org ]; then
        TakeLock
        if [ ! -f b140298.org ]; then
          mkdir IL
          cp b140298.exe IL/b140298.exe
          mv b140298.exe b140298.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b140298.org /out b140298.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b140298.org /out b140298.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b140298.reflect.xml
  if [ -f b140298.reflect.xml ]
then
    ReflectionRoots="-x b140298.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b140298.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b140298.asm.exe $CLRTestExecutionArguments
