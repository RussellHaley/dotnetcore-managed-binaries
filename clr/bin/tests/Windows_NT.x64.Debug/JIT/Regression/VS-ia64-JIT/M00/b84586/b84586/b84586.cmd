...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b84586\b84586 b84586.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b84586\b84586 b84586.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b84586.dasm.il b84586.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b84586.dasm.il b84586.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b84586.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84586.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b84586.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84586.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b84586
    if not exist "b84586.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b84586.org" (
            mkdir IL
            copy b84586.exe IL\b84586.exe 
            ren b84586.exe b84586.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b84586.org /out b84586.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b84586.org /out b84586.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b84586.reflect.xml
    IF EXIST b84586.reflect.xml set ReflectionRoots=-x b84586.reflect.xml
    ...
    set ExePath=%LinkBin%\b84586.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b84586.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b84586.asm.exe %CLRTestExecutionArguments% %Host_Args%
