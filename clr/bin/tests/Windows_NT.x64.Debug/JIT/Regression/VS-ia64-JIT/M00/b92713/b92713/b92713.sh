...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b92713/b92713 b92713.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b92713/b92713 b92713.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b92713.dasm.il b92713.exe
    "$CORE_ROOT/ildasm" -raweh -out=b92713.dasm.il b92713.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b92713.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92713.dasm.il
    "$CORE_ROOT/ilasm" -output=b92713.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92713.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b92713
    if [ ! -f b92713.org ]; then
        TakeLock
        if [ ! -f b92713.org ]; then
          mkdir IL
          cp b92713.exe IL/b92713.exe
          mv b92713.exe b92713.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92713.org /out b92713.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92713.org /out b92713.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b92713.reflect.xml
  if [ -f b92713.reflect.xml ]
then
    ReflectionRoots="-x b92713.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b92713.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b92713.asm.exe $CLRTestExecutionArguments
