...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b99219/b99219 b99219.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b99219/b99219 b99219.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b99219.dasm.il b99219.exe
    "$CORE_ROOT/ildasm" -raweh -out=b99219.dasm.il b99219.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b99219.asm.exe  b99219.dasm.il
    "$CORE_ROOT/ilasm" -output=b99219.asm.exe  b99219.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b99219
    if [ ! -f b99219.org ]; then
        TakeLock
        if [ ! -f b99219.org ]; then
          mkdir IL
          cp b99219.exe IL/b99219.exe
          mv b99219.exe b99219.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b99219.org /out b99219.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b99219.org /out b99219.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b99219.reflect.xml
  if [ -f b99219.reflect.xml ]
then
    ReflectionRoots="-x b99219.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b99219.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b99219.asm.exe $CLRTestExecutionArguments
