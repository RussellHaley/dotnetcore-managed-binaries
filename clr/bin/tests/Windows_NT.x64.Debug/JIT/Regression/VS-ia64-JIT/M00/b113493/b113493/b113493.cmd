...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b113493\b113493 b113493.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b113493\b113493 b113493.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b113493.dasm.il b113493.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b113493.dasm.il b113493.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b113493.asm.exe  b113493.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b113493.asm.exe  b113493.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b113493
    if not exist "b113493.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b113493.org" (
            mkdir IL
            copy b113493.exe IL\b113493.exe 
            ren b113493.exe b113493.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b113493.org /out b113493.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b113493.org /out b113493.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b113493.reflect.xml
    IF EXIST b113493.reflect.xml set ReflectionRoots=-x b113493.reflect.xml
    ...
    set ExePath=%LinkBin%\b113493.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b113493.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b113493.asm.exe %CLRTestExecutionArguments% %Host_Args%
