...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b109878/b109878 b109878.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b109878/b109878 b109878.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b109878.dasm.il b109878.exe
    "$CORE_ROOT/ildasm" -raweh -out=b109878.dasm.il b109878.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b109878.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b109878.dasm.il
    "$CORE_ROOT/ilasm" -output=b109878.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b109878.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b109878
    if [ ! -f b109878.org ]; then
        TakeLock
        if [ ! -f b109878.org ]; then
          mkdir IL
          cp b109878.exe IL/b109878.exe
          mv b109878.exe b109878.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b109878.org /out b109878.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b109878.org /out b109878.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b109878.reflect.xml
  if [ -f b109878.reflect.xml ]
then
    ReflectionRoots="-x b109878.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b109878.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b109878.asm.exe $CLRTestExecutionArguments
