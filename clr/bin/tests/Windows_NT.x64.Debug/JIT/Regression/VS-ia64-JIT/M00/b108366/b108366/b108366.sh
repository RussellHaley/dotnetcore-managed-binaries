...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b108366/b108366 b108366.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b108366/b108366 b108366.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b108366.dasm.il b108366.exe
    "$CORE_ROOT/ildasm" -raweh -out=b108366.dasm.il b108366.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b108366.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b108366.dasm.il
    "$CORE_ROOT/ilasm" -output=b108366.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b108366.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b108366
    if [ ! -f b108366.org ]; then
        TakeLock
        if [ ! -f b108366.org ]; then
          mkdir IL
          cp b108366.exe IL/b108366.exe
          mv b108366.exe b108366.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b108366.org /out b108366.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b108366.org /out b108366.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b108366.reflect.xml
  if [ -f b108366.reflect.xml ]
then
    ReflectionRoots="-x b108366.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b108366.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b108366.asm.exe $CLRTestExecutionArguments
