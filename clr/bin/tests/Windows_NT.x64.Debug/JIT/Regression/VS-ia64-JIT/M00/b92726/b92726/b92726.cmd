...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b92726\b92726 b92726.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b92726\b92726 b92726.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b92726.dasm.il b92726.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b92726.dasm.il b92726.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b92726.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92726.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b92726.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92726.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b92726
    if not exist "b92726.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b92726.org" (
            mkdir IL
            copy b92726.exe IL\b92726.exe 
            ren b92726.exe b92726.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b92726.org /out b92726.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b92726.org /out b92726.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b92726.reflect.xml
    IF EXIST b92726.reflect.xml set ReflectionRoots=-x b92726.reflect.xml
    ...
    set ExePath=%LinkBin%\b92726.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b92726.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b92726.asm.exe %CLRTestExecutionArguments% %Host_Args%
