...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b112982\b112982 b112982.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b112982\b112982 b112982.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b112982.dasm.il b112982.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b112982.dasm.il b112982.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b112982.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b112982.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b112982.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b112982.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b112982
    if not exist "b112982.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b112982.org" (
            mkdir IL
            copy b112982.exe IL\b112982.exe 
            ren b112982.exe b112982.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b112982.org /out b112982.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b112982.org /out b112982.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b112982.reflect.xml
    IF EXIST b112982.reflect.xml set ReflectionRoots=-x b112982.reflect.xml
    ...
    set ExePath=%LinkBin%\b112982.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b112982.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b112982.asm.exe %CLRTestExecutionArguments% %Host_Args%
