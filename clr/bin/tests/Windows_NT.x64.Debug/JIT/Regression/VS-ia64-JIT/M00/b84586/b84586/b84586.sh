...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b84586/b84586 b84586.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/M00/b84586/b84586 b84586.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b84586.dasm.il b84586.exe
    "$CORE_ROOT/ildasm" -raweh -out=b84586.dasm.il b84586.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b84586.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84586.dasm.il
    "$CORE_ROOT/ilasm" -output=b84586.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84586.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b84586
    if [ ! -f b84586.org ]; then
        TakeLock
        if [ ! -f b84586.org ]; then
          mkdir IL
          cp b84586.exe IL/b84586.exe
          mv b84586.exe b84586.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b84586.org /out b84586.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b84586.org /out b84586.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b84586.reflect.xml
  if [ -f b84586.reflect.xml ]
then
    ReflectionRoots="-x b84586.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b84586.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b84586.asm.exe $CLRTestExecutionArguments
