...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b140298\b140298 b140298.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\M00\b140298\b140298 b140298.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b140298.dasm.il b140298.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b140298.dasm.il b140298.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b140298.asm.exe  b140298.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b140298.asm.exe  b140298.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b140298
    if not exist "b140298.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b140298.org" (
            mkdir IL
            copy b140298.exe IL\b140298.exe 
            ren b140298.exe b140298.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b140298.org /out b140298.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b140298.org /out b140298.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b140298.reflect.xml
    IF EXIST b140298.reflect.xml set ReflectionRoots=-x b140298.reflect.xml
    ...
    set ExePath=%LinkBin%\b140298.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b140298.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b140298.asm.exe %CLRTestExecutionArguments% %Host_Args%
