...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b26496\b26496 b26496.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b26496\b26496 b26496.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b26496.dasm.il b26496.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b26496.dasm.il b26496.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b26496.asm.exe  b26496.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b26496.asm.exe  b26496.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b26496
    if not exist "b26496.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b26496.org" (
            mkdir IL
            copy b26496.exe IL\b26496.exe 
            ren b26496.exe b26496.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b26496.org /out b26496.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b26496.org /out b26496.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b26496.reflect.xml
    IF EXIST b26496.reflect.xml set ReflectionRoots=-x b26496.reflect.xml
    ...
    set ExePath=%LinkBin%\b26496.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b26496.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b26496.asm.exe %CLRTestExecutionArguments% %Host_Args%
