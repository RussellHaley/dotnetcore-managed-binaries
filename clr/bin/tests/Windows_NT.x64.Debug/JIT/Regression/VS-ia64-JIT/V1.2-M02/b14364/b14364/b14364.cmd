...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b14364\b14364 b14364.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b14364\b14364 b14364.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b14364.dasm.il b14364.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b14364.dasm.il b14364.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b14364.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b14364.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b14364.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b14364.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b14364
    if not exist "b14364.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b14364.org" (
            mkdir IL
            copy b14364.exe IL\b14364.exe 
            ren b14364.exe b14364.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b14364.org /out b14364.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b14364.org /out b14364.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b14364.reflect.xml
    IF EXIST b14364.reflect.xml set ReflectionRoots=-x b14364.reflect.xml
    ...
    set ExePath=%LinkBin%\b14364.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b14364.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b14364.asm.exe %CLRTestExecutionArguments% %Host_Args%
