...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b14364/b14364 b14364.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b14364/b14364 b14364.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b14364.dasm.il b14364.exe
    "$CORE_ROOT/ildasm" -raweh -out=b14364.dasm.il b14364.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b14364.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b14364.dasm.il
    "$CORE_ROOT/ilasm" -output=b14364.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b14364.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b14364
    if [ ! -f b14364.org ]; then
        TakeLock
        if [ ! -f b14364.org ]; then
          mkdir IL
          cp b14364.exe IL/b14364.exe
          mv b14364.exe b14364.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b14364.org /out b14364.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b14364.org /out b14364.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b14364.reflect.xml
  if [ -f b14364.reflect.xml ]
then
    ReflectionRoots="-x b14364.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b14364.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b14364.asm.exe $CLRTestExecutionArguments
