...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b102844\b102844 b102844.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b102844\b102844 b102844.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b102844.dasm.il b102844.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b102844.dasm.il b102844.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b102844.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b102844.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b102844.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b102844.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b102844
    if not exist "b102844.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b102844.org" (
            mkdir IL
            copy b102844.exe IL\b102844.exe 
            ren b102844.exe b102844.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b102844.org /out b102844.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b102844.org /out b102844.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b102844.reflect.xml
    IF EXIST b102844.reflect.xml set ReflectionRoots=-x b102844.reflect.xml
    ...
    set ExePath=%LinkBin%\b102844.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b102844.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b102844.asm.exe %CLRTestExecutionArguments% %Host_Args%
