...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b17904\b17904 b17904.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b17904\b17904 b17904.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b17904.dasm.il b17904.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b17904.dasm.il b17904.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b17904.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b17904.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b17904.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b17904.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b17904
    if not exist "b17904.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b17904.org" (
            mkdir IL
            copy b17904.exe IL\b17904.exe 
            ren b17904.exe b17904.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b17904.org /out b17904.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b17904.org /out b17904.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b17904.reflect.xml
    IF EXIST b17904.reflect.xml set ReflectionRoots=-x b17904.reflect.xml
    ...
    set ExePath=%LinkBin%\b17904.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b17904.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b17904.asm.exe %CLRTestExecutionArguments% %Host_Args%
