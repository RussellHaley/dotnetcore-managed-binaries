...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b102886/b102886 b102886.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b102886/b102886 b102886.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b102886.dasm.il b102886.exe
    "$CORE_ROOT/ildasm" -raweh -out=b102886.dasm.il b102886.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b102886.asm.exe  b102886.dasm.il
    "$CORE_ROOT/ilasm" -output=b102886.asm.exe  b102886.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b102886
    if [ ! -f b102886.org ]; then
        TakeLock
        if [ ! -f b102886.org ]; then
          mkdir IL
          cp b102886.exe IL/b102886.exe
          mv b102886.exe b102886.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b102886.org /out b102886.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b102886.org /out b102886.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b102886.reflect.xml
  if [ -f b102886.reflect.xml ]
then
    ReflectionRoots="-x b102886.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b102886.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b102886.asm.exe $CLRTestExecutionArguments
