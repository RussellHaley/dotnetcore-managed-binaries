...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b16198\b16198 b16198.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b16198\b16198 b16198.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16198.dasm.il b16198.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16198.dasm.il b16198.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16198.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16198.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16198.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16198.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16198
    if not exist "b16198.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16198.org" (
            mkdir IL
            copy b16198.exe IL\b16198.exe 
            ren b16198.exe b16198.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16198.org /out b16198.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16198.org /out b16198.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16198.reflect.xml
    IF EXIST b16198.reflect.xml set ReflectionRoots=-x b16198.reflect.xml
    ...
    set ExePath=%LinkBin%\b16198.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16198.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16198.asm.exe %CLRTestExecutionArguments% %Host_Args%
