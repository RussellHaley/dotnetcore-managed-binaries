...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b12011\b12011 b12011.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b12011\b12011 b12011.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b12011.dasm.il b12011.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b12011.dasm.il b12011.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b12011.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12011.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b12011.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12011.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b12011
    if not exist "b12011.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b12011.org" (
            mkdir IL
            copy b12011.exe IL\b12011.exe 
            ren b12011.exe b12011.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b12011.org /out b12011.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b12011.org /out b12011.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b12011.reflect.xml
    IF EXIST b12011.reflect.xml set ReflectionRoots=-x b12011.reflect.xml
    ...
    set ExePath=%LinkBin%\b12011.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b12011.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b12011.asm.exe %CLRTestExecutionArguments% %Host_Args%
