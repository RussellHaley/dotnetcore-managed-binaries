...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b26496/b26496 b26496.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b26496/b26496 b26496.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b26496.dasm.il b26496.exe
    "$CORE_ROOT/ildasm" -raweh -out=b26496.dasm.il b26496.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b26496.asm.exe  b26496.dasm.il
    "$CORE_ROOT/ilasm" -output=b26496.asm.exe  b26496.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b26496
    if [ ! -f b26496.org ]; then
        TakeLock
        if [ ! -f b26496.org ]; then
          mkdir IL
          cp b26496.exe IL/b26496.exe
          mv b26496.exe b26496.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26496.org /out b26496.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26496.org /out b26496.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b26496.reflect.xml
  if [ -f b26496.reflect.xml ]
then
    ReflectionRoots="-x b26496.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b26496.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b26496.asm.exe $CLRTestExecutionArguments
