...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b12011/b12011 b12011.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b12011/b12011 b12011.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b12011.dasm.il b12011.exe
    "$CORE_ROOT/ildasm" -raweh -out=b12011.dasm.il b12011.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b12011.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12011.dasm.il
    "$CORE_ROOT/ilasm" -output=b12011.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12011.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b12011
    if [ ! -f b12011.org ]; then
        TakeLock
        if [ ! -f b12011.org ]; then
          mkdir IL
          cp b12011.exe IL/b12011.exe
          mv b12011.exe b12011.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12011.org /out b12011.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12011.org /out b12011.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b12011.reflect.xml
  if [ -f b12011.reflect.xml ]
then
    ReflectionRoots="-x b12011.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b12011.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b12011.asm.exe $CLRTestExecutionArguments
