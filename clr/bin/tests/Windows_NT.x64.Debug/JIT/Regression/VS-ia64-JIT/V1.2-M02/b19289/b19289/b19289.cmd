...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b19289\b19289 b19289.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b19289\b19289 b19289.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b19289.dasm.il b19289.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b19289.dasm.il b19289.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b19289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b19289.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b19289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b19289.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b19289
    if not exist "b19289.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b19289.org" (
            mkdir IL
            copy b19289.exe IL\b19289.exe 
            ren b19289.exe b19289.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b19289.org /out b19289.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b19289.org /out b19289.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b19289.reflect.xml
    IF EXIST b19289.reflect.xml set ReflectionRoots=-x b19289.reflect.xml
    ...
    set ExePath=%LinkBin%\b19289.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b19289.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b19289.asm.exe %CLRTestExecutionArguments% %Host_Args%
