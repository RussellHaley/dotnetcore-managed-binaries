...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b10828/b10828 b10828.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b10828/b10828 b10828.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b10828.dasm.il b10828.exe
    "$CORE_ROOT/ildasm" -raweh -out=b10828.dasm.il b10828.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b10828.asm.exe  b10828.dasm.il
    "$CORE_ROOT/ilasm" -output=b10828.asm.exe  b10828.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b10828
    if [ ! -f b10828.org ]; then
        TakeLock
        if [ ! -f b10828.org ]; then
          mkdir IL
          cp b10828.exe IL/b10828.exe
          mv b10828.exe b10828.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b10828.org /out b10828.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b10828.org /out b10828.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b10828.reflect.xml
  if [ -f b10828.reflect.xml ]
then
    ReflectionRoots="-x b10828.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b10828.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b10828.asm.exe $CLRTestExecutionArguments
