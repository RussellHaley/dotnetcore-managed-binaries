...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b19289/b19289 b19289.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b19289/b19289 b19289.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b19289.dasm.il b19289.exe
    "$CORE_ROOT/ildasm" -raweh -out=b19289.dasm.il b19289.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b19289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b19289.dasm.il
    "$CORE_ROOT/ilasm" -output=b19289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b19289.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b19289
    if [ ! -f b19289.org ]; then
        TakeLock
        if [ ! -f b19289.org ]; then
          mkdir IL
          cp b19289.exe IL/b19289.exe
          mv b19289.exe b19289.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b19289.org /out b19289.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b19289.org /out b19289.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b19289.reflect.xml
  if [ -f b19289.reflect.xml ]
then
    ReflectionRoots="-x b19289.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b19289.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b19289.asm.exe $CLRTestExecutionArguments
