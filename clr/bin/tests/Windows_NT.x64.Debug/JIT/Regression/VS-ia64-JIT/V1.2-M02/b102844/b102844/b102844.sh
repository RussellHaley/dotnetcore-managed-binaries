...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b102844/b102844 b102844.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b102844/b102844 b102844.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b102844.dasm.il b102844.exe
    "$CORE_ROOT/ildasm" -raweh -out=b102844.dasm.il b102844.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b102844.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b102844.dasm.il
    "$CORE_ROOT/ilasm" -output=b102844.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b102844.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b102844
    if [ ! -f b102844.org ]; then
        TakeLock
        if [ ! -f b102844.org ]; then
          mkdir IL
          cp b102844.exe IL/b102844.exe
          mv b102844.exe b102844.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b102844.org /out b102844.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b102844.org /out b102844.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b102844.reflect.xml
  if [ -f b102844.reflect.xml ]
then
    ReflectionRoots="-x b102844.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b102844.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b102844.asm.exe $CLRTestExecutionArguments
