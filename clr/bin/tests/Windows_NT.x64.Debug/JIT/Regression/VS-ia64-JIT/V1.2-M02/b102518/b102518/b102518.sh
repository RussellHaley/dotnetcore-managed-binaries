...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b102518/b102518 b102518.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b102518/b102518 b102518.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b102518.dasm.il b102518.exe
    "$CORE_ROOT/ildasm" -raweh -out=b102518.dasm.il b102518.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b102518.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b102518.dasm.il
    "$CORE_ROOT/ilasm" -output=b102518.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b102518.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b102518
    if [ ! -f b102518.org ]; then
        TakeLock
        if [ ! -f b102518.org ]; then
          mkdir IL
          cp b102518.exe IL/b102518.exe
          mv b102518.exe b102518.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b102518.org /out b102518.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b102518.org /out b102518.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b102518.reflect.xml
  if [ -f b102518.reflect.xml ]
then
    ReflectionRoots="-x b102518.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b102518.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b102518.asm.exe $CLRTestExecutionArguments
