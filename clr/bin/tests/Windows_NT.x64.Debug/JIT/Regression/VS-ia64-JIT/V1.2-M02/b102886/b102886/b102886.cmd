...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b102886\b102886 b102886.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M02\b102886\b102886 b102886.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b102886.dasm.il b102886.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b102886.dasm.il b102886.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b102886.asm.exe  b102886.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b102886.asm.exe  b102886.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b102886
    if not exist "b102886.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b102886.org" (
            mkdir IL
            copy b102886.exe IL\b102886.exe 
            ren b102886.exe b102886.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b102886.org /out b102886.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b102886.org /out b102886.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b102886.reflect.xml
    IF EXIST b102886.reflect.xml set ReflectionRoots=-x b102886.reflect.xml
    ...
    set ExePath=%LinkBin%\b102886.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b102886.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b102886.asm.exe %CLRTestExecutionArguments% %Host_Args%
