...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b17904/b17904 b17904.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b17904/b17904 b17904.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b17904.dasm.il b17904.exe
    "$CORE_ROOT/ildasm" -raweh -out=b17904.dasm.il b17904.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b17904.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b17904.dasm.il
    "$CORE_ROOT/ilasm" -output=b17904.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b17904.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b17904
    if [ ! -f b17904.org ]; then
        TakeLock
        if [ ! -f b17904.org ]; then
          mkdir IL
          cp b17904.exe IL/b17904.exe
          mv b17904.exe b17904.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b17904.org /out b17904.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b17904.org /out b17904.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b17904.reflect.xml
  if [ -f b17904.reflect.xml ]
then
    ReflectionRoots="-x b17904.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b17904.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b17904.asm.exe $CLRTestExecutionArguments
