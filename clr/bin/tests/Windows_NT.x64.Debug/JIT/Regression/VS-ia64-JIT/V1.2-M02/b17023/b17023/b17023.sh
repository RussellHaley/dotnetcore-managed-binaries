...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b17023/b17023 b17023.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b17023/b17023 b17023.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b17023.dasm.il b17023.exe
    "$CORE_ROOT/ildasm" -raweh -out=b17023.dasm.il b17023.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b17023.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b17023.dasm.il
    "$CORE_ROOT/ilasm" -output=b17023.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b17023.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b17023
    if [ ! -f b17023.org ]; then
        TakeLock
        if [ ! -f b17023.org ]; then
          mkdir IL
          cp b17023.exe IL/b17023.exe
          mv b17023.exe b17023.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b17023.org /out b17023.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b17023.org /out b17023.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b17023.reflect.xml
  if [ -f b17023.reflect.xml ]
then
    ReflectionRoots="-x b17023.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b17023.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b17023.asm.exe $CLRTestExecutionArguments
