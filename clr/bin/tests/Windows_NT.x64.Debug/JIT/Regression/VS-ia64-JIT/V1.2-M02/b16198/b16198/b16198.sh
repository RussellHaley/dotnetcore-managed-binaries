...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b16198/b16198 b16198.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M02/b16198/b16198 b16198.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16198.dasm.il b16198.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16198.dasm.il b16198.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16198.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16198.dasm.il
    "$CORE_ROOT/ilasm" -output=b16198.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16198.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16198
    if [ ! -f b16198.org ]; then
        TakeLock
        if [ ! -f b16198.org ]; then
          mkdir IL
          cp b16198.exe IL/b16198.exe
          mv b16198.exe b16198.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16198.org /out b16198.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16198.org /out b16198.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16198.reflect.xml
  if [ -f b16198.reflect.xml ]
then
    ReflectionRoots="-x b16198.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16198.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16198.asm.exe $CLRTestExecutionArguments
