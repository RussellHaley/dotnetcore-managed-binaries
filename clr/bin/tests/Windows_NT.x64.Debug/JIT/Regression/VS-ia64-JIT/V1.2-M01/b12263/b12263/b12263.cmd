...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M01\b12263\b12263 b12263.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M01\b12263\b12263 b12263.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b12263.dasm.il b12263.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b12263.dasm.il b12263.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b12263.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12263.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b12263.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12263.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b12263
    if not exist "b12263.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b12263.org" (
            mkdir IL
            copy b12263.exe IL\b12263.exe 
            ren b12263.exe b12263.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b12263.org /out b12263.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b12263.org /out b12263.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b12263.reflect.xml
    IF EXIST b12263.reflect.xml set ReflectionRoots=-x b12263.reflect.xml
    ...
    set ExePath=%LinkBin%\b12263.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b12263.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b12263.asm.exe %CLRTestExecutionArguments% %Host_Args%
