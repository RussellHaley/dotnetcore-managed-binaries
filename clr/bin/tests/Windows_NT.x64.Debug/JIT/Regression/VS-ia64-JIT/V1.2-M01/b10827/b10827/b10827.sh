...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b10827/b10827 b10827.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b10827/b10827 b10827.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b10827.dasm.il b10827.exe
    "$CORE_ROOT/ildasm" -raweh -out=b10827.dasm.il b10827.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b10827.asm.exe  b10827.dasm.il
    "$CORE_ROOT/ilasm" -output=b10827.asm.exe  b10827.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b10827
    if [ ! -f b10827.org ]; then
        TakeLock
        if [ ! -f b10827.org ]; then
          mkdir IL
          cp b10827.exe IL/b10827.exe
          mv b10827.exe b10827.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b10827.org /out b10827.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b10827.org /out b10827.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b10827.reflect.xml
  if [ -f b10827.reflect.xml ]
then
    ReflectionRoots="-x b10827.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b10827.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b10827.asm.exe $CLRTestExecutionArguments
