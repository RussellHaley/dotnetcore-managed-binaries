...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b10802/b10802 b10802.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b10802/b10802 b10802.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b10802.dasm.il b10802.exe
    "$CORE_ROOT/ildasm" -raweh -out=b10802.dasm.il b10802.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b10802.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b10802.dasm.il
    "$CORE_ROOT/ilasm" -output=b10802.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b10802.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b10802
    if [ ! -f b10802.org ]; then
        TakeLock
        if [ ! -f b10802.org ]; then
          mkdir IL
          cp b10802.exe IL/b10802.exe
          mv b10802.exe b10802.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b10802.org /out b10802.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b10802.org /out b10802.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b10802.reflect.xml
  if [ -f b10802.reflect.xml ]
then
    ReflectionRoots="-x b10802.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b10802.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b10802.asm.exe $CLRTestExecutionArguments
