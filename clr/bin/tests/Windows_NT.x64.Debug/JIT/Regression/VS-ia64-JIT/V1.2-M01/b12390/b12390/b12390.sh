...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b12390/b12390 b12390.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b12390/b12390 b12390.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b12390.dasm.il b12390.exe
    "$CORE_ROOT/ildasm" -raweh -out=b12390.dasm.il b12390.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b12390.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12390.dasm.il
    "$CORE_ROOT/ilasm" -output=b12390.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12390.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b12390
    if [ ! -f b12390.org ]; then
        TakeLock
        if [ ! -f b12390.org ]; then
          mkdir IL
          cp b12390.exe IL/b12390.exe
          mv b12390.exe b12390.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12390.org /out b12390.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12390.org /out b12390.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b12390.reflect.xml
  if [ -f b12390.reflect.xml ]
then
    ReflectionRoots="-x b12390.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b12390.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b12390.asm.exe $CLRTestExecutionArguments
