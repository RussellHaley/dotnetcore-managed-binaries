...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M01\b10802\b10802 b10802.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-M01\b10802\b10802 b10802.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b10802.dasm.il b10802.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b10802.dasm.il b10802.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b10802.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b10802.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b10802.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b10802.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b10802
    if not exist "b10802.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b10802.org" (
            mkdir IL
            copy b10802.exe IL\b10802.exe 
            ren b10802.exe b10802.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b10802.org /out b10802.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b10802.org /out b10802.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b10802.reflect.xml
    IF EXIST b10802.reflect.xml set ReflectionRoots=-x b10802.reflect.xml
    ...
    set ExePath=%LinkBin%\b10802.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b10802.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b10802.asm.exe %CLRTestExecutionArguments% %Host_Args%
