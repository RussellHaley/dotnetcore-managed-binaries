...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b12343/b12343 b12343.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b12343/b12343 b12343.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b12343.dasm.il b12343.exe
    "$CORE_ROOT/ildasm" -raweh -out=b12343.dasm.il b12343.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b12343.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12343.dasm.il
    "$CORE_ROOT/ilasm" -output=b12343.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12343.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b12343
    if [ ! -f b12343.org ]; then
        TakeLock
        if [ ! -f b12343.org ]; then
          mkdir IL
          cp b12343.exe IL/b12343.exe
          mv b12343.exe b12343.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12343.org /out b12343.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12343.org /out b12343.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b12343.reflect.xml
  if [ -f b12343.reflect.xml ]
then
    ReflectionRoots="-x b12343.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b12343.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b12343.asm.exe $CLRTestExecutionArguments
