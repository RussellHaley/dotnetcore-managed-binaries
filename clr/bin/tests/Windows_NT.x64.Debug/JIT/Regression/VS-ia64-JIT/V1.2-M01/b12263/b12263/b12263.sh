...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b12263/b12263 b12263.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-M01/b12263/b12263 b12263.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b12263.dasm.il b12263.exe
    "$CORE_ROOT/ildasm" -raweh -out=b12263.dasm.il b12263.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b12263.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12263.dasm.il
    "$CORE_ROOT/ilasm" -output=b12263.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12263.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b12263
    if [ ! -f b12263.org ]; then
        TakeLock
        if [ ! -f b12263.org ]; then
          mkdir IL
          cp b12263.exe IL/b12263.exe
          mv b12263.exe b12263.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12263.org /out b12263.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12263.org /out b12263.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b12263.reflect.xml
  if [ -f b12263.reflect.xml ]
then
    ReflectionRoots="-x b12263.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b12263.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b12263.asm.exe $CLRTestExecutionArguments
