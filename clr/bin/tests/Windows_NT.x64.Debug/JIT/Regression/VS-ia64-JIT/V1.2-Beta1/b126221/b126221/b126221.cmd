...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-Beta1\b126221\b126221 b126221.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-Beta1\b126221\b126221 b126221.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b126221.dasm.il b126221.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b126221.dasm.il b126221.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b126221.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b126221.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b126221.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b126221.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b126221
    if not exist "b126221.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b126221.org" (
            mkdir IL
            copy b126221.exe IL\b126221.exe 
            ren b126221.exe b126221.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b126221.org /out b126221.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b126221.org /out b126221.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b126221.reflect.xml
    IF EXIST b126221.reflect.xml set ReflectionRoots=-x b126221.reflect.xml
    ...
    set ExePath=%LinkBin%\b126221.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b126221.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b126221.asm.exe %CLRTestExecutionArguments% %Host_Args%
