...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-Beta1/b91944/b91944 b91944.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-Beta1/b91944/b91944 b91944.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b91944.dasm.il b91944.exe
    "$CORE_ROOT/ildasm" -raweh -out=b91944.dasm.il b91944.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b91944.asm.exe  b91944.dasm.il
    "$CORE_ROOT/ilasm" -output=b91944.asm.exe  b91944.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b91944
    if [ ! -f b91944.org ]; then
        TakeLock
        if [ ! -f b91944.org ]; then
          mkdir IL
          cp b91944.exe IL/b91944.exe
          mv b91944.exe b91944.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91944.org /out b91944.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91944.org /out b91944.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b91944.reflect.xml
  if [ -f b91944.reflect.xml ]
then
    ReflectionRoots="-x b91944.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b91944.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b91944.asm.exe $CLRTestExecutionArguments
