...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-Beta1\b91944\b91944 b91944.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V1.2-Beta1\b91944\b91944 b91944.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b91944.dasm.il b91944.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b91944.dasm.il b91944.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b91944.asm.exe  b91944.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b91944.asm.exe  b91944.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b91944
    if not exist "b91944.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b91944.org" (
            mkdir IL
            copy b91944.exe IL\b91944.exe 
            ren b91944.exe b91944.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b91944.org /out b91944.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b91944.org /out b91944.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b91944.reflect.xml
    IF EXIST b91944.reflect.xml set ReflectionRoots=-x b91944.reflect.xml
    ...
    set ExePath=%LinkBin%\b91944.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b91944.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b91944.asm.exe %CLRTestExecutionArguments% %Host_Args%
