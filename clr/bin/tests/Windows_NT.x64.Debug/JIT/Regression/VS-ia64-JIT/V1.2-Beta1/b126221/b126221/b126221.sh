...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-Beta1/b126221/b126221 b126221.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V1.2-Beta1/b126221/b126221 b126221.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b126221.dasm.il b126221.exe
    "$CORE_ROOT/ildasm" -raweh -out=b126221.dasm.il b126221.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b126221.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b126221.dasm.il
    "$CORE_ROOT/ilasm" -output=b126221.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b126221.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b126221
    if [ ! -f b126221.org ]; then
        TakeLock
        if [ ! -f b126221.org ]; then
          mkdir IL
          cp b126221.exe IL/b126221.exe
          mv b126221.exe b126221.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b126221.org /out b126221.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b126221.org /out b126221.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b126221.reflect.xml
  if [ -f b126221.reflect.xml ]
then
    ReflectionRoots="-x b126221.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b126221.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b126221.asm.exe $CLRTestExecutionArguments
