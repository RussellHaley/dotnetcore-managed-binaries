...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-RTM/b539509/b539509 b539509.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/VS-ia64-JIT/V2.0-RTM/b539509/b539509 b539509.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b539509.dasm.il b539509.exe
    "$CORE_ROOT/ildasm" -raweh -out=b539509.dasm.il b539509.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b539509.asm.exe  b539509.dasm.il
    "$CORE_ROOT/ilasm" -output=b539509.asm.exe  b539509.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b539509
    if [ ! -f b539509.org ]; then
        TakeLock
        if [ ! -f b539509.org ]; then
          mkdir IL
          cp b539509.exe IL/b539509.exe
          mv b539509.exe b539509.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b539509.org /out b539509.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b539509.org /out b539509.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b539509.reflect.xml
  if [ -f b539509.reflect.xml ]
then
    ReflectionRoots="-x b539509.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b539509.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b539509.asm.exe $CLRTestExecutionArguments
