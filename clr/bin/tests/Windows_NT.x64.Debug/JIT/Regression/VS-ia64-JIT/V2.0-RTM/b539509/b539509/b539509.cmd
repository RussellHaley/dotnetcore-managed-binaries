...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V2.0-RTM\b539509\b539509 b539509.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\VS-ia64-JIT\V2.0-RTM\b539509\b539509 b539509.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b539509.dasm.il b539509.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b539509.dasm.il b539509.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b539509.asm.exe  b539509.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b539509.asm.exe  b539509.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b539509
    if not exist "b539509.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b539509.org" (
            mkdir IL
            copy b539509.exe IL\b539509.exe 
            ren b539509.exe b539509.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b539509.org /out b539509.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b539509.org /out b539509.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b539509.reflect.xml
    IF EXIST b539509.reflect.xml set ReflectionRoots=-x b539509.reflect.xml
    ...
    set ExePath=%LinkBin%\b539509.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b539509.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b539509.asm.exe %CLRTestExecutionArguments% %Host_Args%
