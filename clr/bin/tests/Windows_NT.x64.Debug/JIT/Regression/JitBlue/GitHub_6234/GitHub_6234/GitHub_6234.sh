...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_6234/GitHub_6234 GitHub_6234.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_6234/GitHub_6234 GitHub_6234.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_6234.dasm.il GitHub_6234.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_6234.dasm.il GitHub_6234.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_6234.asm.exe  GitHub_6234.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_6234.asm.exe  GitHub_6234.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_6234
    if [ ! -f GitHub_6234.org ]; then
        TakeLock
        if [ ! -f GitHub_6234.org ]; then
          mkdir IL
          cp GitHub_6234.exe IL/GitHub_6234.exe
          mv GitHub_6234.exe GitHub_6234.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_6234.org /out GitHub_6234.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_6234.org /out GitHub_6234.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_6234.reflect.xml
  if [ -f GitHub_6234.reflect.xml ]
then
    ReflectionRoots="-x GitHub_6234.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_6234.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_6234.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_6234.asm.exe $CLRTestExecutionArguments
