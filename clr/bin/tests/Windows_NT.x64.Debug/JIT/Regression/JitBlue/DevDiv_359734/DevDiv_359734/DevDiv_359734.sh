...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_359734/DevDiv_359734 DevDiv_359734.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_359734/DevDiv_359734 DevDiv_359734.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_359734.dasm.il DevDiv_359734.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_359734.dasm.il DevDiv_359734.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_359734.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_359734.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_359734.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_359734.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_359734
    if [ ! -f DevDiv_359734.org ]; then
        TakeLock
        if [ ! -f DevDiv_359734.org ]; then
          mkdir IL
          cp DevDiv_359734.exe IL/DevDiv_359734.exe
          mv DevDiv_359734.exe DevDiv_359734.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_359734.org /out DevDiv_359734.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_359734.org /out DevDiv_359734.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_359734.reflect.xml
  if [ -f DevDiv_359734.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_359734.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_359734.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_359734.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_359734.asm.exe $CLRTestExecutionArguments
