...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_199169/DevDiv_199169 DevDiv_199169.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_199169/DevDiv_199169 DevDiv_199169.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_199169.dasm.il DevDiv_199169.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_199169.dasm.il DevDiv_199169.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_199169.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_199169.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_199169.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_199169.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_199169
    if [ ! -f DevDiv_199169.org ]; then
        TakeLock
        if [ ! -f DevDiv_199169.org ]; then
          mkdir IL
          cp DevDiv_199169.exe IL/DevDiv_199169.exe
          mv DevDiv_199169.exe DevDiv_199169.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_199169.org /out DevDiv_199169.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_199169.org /out DevDiv_199169.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_199169.reflect.xml
  if [ -f DevDiv_199169.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_199169.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_199169.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_199169.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_199169.asm.exe $CLRTestExecutionArguments
