...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_13910/GitHub_13910 GitHub_13910.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_13910/GitHub_13910 GitHub_13910.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_13910.dasm.il GitHub_13910.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_13910.dasm.il GitHub_13910.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_13910.asm.exe  GitHub_13910.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_13910.asm.exe  GitHub_13910.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_13910
    if [ ! -f GitHub_13910.org ]; then
        TakeLock
        if [ ! -f GitHub_13910.org ]; then
          mkdir IL
          cp GitHub_13910.exe IL/GitHub_13910.exe
          mv GitHub_13910.exe GitHub_13910.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_13910.org /out GitHub_13910.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_13910.org /out GitHub_13910.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_13910.reflect.xml
  if [ -f GitHub_13910.reflect.xml ]
then
    ReflectionRoots="-x GitHub_13910.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_13910.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_13910.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_13910.asm.exe $CLRTestExecutionArguments
