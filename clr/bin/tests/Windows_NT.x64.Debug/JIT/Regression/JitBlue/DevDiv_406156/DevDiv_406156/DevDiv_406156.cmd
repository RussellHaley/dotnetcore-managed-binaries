...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_406156\DevDiv_406156 DevDiv_406156.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_406156\DevDiv_406156 DevDiv_406156.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_406156.dasm.il DevDiv_406156.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_406156.dasm.il DevDiv_406156.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DevDiv_406156.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_406156.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DevDiv_406156.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_406156.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DevDiv_406156
    if not exist "DevDiv_406156.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DevDiv_406156.org" (
            mkdir IL
            copy DevDiv_406156.exe IL\DevDiv_406156.exe 
            ren DevDiv_406156.exe DevDiv_406156.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_406156.org /out DevDiv_406156.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_406156.org /out DevDiv_406156.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DevDiv_406156.reflect.xml
    IF EXIST DevDiv_406156.reflect.xml set ReflectionRoots=-x DevDiv_406156.reflect.xml
    ...
    set ExePath=%LinkBin%\DevDiv_406156.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DevDiv_406156.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DevDiv_406156.asm.exe %CLRTestExecutionArguments% %Host_Args%
