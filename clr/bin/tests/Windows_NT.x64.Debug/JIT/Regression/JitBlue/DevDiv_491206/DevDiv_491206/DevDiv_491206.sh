...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_491206/DevDiv_491206 DevDiv_491206.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_491206/DevDiv_491206 DevDiv_491206.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_491206.dasm.il DevDiv_491206.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_491206.dasm.il DevDiv_491206.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_491206.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_491206.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_491206.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_491206.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_491206
    if [ ! -f DevDiv_491206.org ]; then
        TakeLock
        if [ ! -f DevDiv_491206.org ]; then
          mkdir IL
          cp DevDiv_491206.exe IL/DevDiv_491206.exe
          mv DevDiv_491206.exe DevDiv_491206.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_491206.org /out DevDiv_491206.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_491206.org /out DevDiv_491206.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_491206.reflect.xml
  if [ -f DevDiv_491206.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_491206.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_491206.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_491206.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_491206.asm.exe $CLRTestExecutionArguments
