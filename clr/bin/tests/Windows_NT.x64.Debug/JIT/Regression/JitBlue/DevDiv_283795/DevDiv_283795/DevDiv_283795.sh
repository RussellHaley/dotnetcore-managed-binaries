...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_283795/DevDiv_283795 DevDiv_283795.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_283795/DevDiv_283795 DevDiv_283795.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_283795.dasm.il DevDiv_283795.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_283795.dasm.il DevDiv_283795.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_283795.asm.exe  DevDiv_283795.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_283795.asm.exe  DevDiv_283795.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_283795
    if [ ! -f DevDiv_283795.org ]; then
        TakeLock
        if [ ! -f DevDiv_283795.org ]; then
          mkdir IL
          cp DevDiv_283795.exe IL/DevDiv_283795.exe
          mv DevDiv_283795.exe DevDiv_283795.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_283795.org /out DevDiv_283795.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_283795.org /out DevDiv_283795.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_283795.reflect.xml
  if [ -f DevDiv_283795.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_283795.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_283795.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_283795.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_283795.asm.exe $CLRTestExecutionArguments
