...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_487699/DevDiv_487699 DevDiv_487699.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_487699/DevDiv_487699 DevDiv_487699.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_487699.dasm.il DevDiv_487699.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_487699.dasm.il DevDiv_487699.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_487699.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_487699.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_487699.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_487699.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_487699
    if [ ! -f DevDiv_487699.org ]; then
        TakeLock
        if [ ! -f DevDiv_487699.org ]; then
          mkdir IL
          cp DevDiv_487699.exe IL/DevDiv_487699.exe
          mv DevDiv_487699.exe DevDiv_487699.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_487699.org /out DevDiv_487699.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_487699.org /out DevDiv_487699.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_487699.reflect.xml
  if [ -f DevDiv_487699.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_487699.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_487699.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_487699.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_487699.asm.exe $CLRTestExecutionArguments
