...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_461649/DevDiv_461649 DevDiv_461649.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_461649/DevDiv_461649 DevDiv_461649.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_461649.dasm.il DevDiv_461649.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_461649.dasm.il DevDiv_461649.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_461649.asm.exe  DevDiv_461649.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_461649.asm.exe  DevDiv_461649.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_461649
    if [ ! -f DevDiv_461649.org ]; then
        TakeLock
        if [ ! -f DevDiv_461649.org ]; then
          mkdir IL
          cp DevDiv_461649.exe IL/DevDiv_461649.exe
          mv DevDiv_461649.exe DevDiv_461649.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_461649.org /out DevDiv_461649.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_461649.org /out DevDiv_461649.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_461649.reflect.xml
  if [ -f DevDiv_461649.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_461649.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_461649.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_461649.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_461649.asm.exe $CLRTestExecutionArguments
