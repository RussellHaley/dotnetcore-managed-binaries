...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_461649\DevDiv_461649 DevDiv_461649.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_461649\DevDiv_461649 DevDiv_461649.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_461649.dasm.il DevDiv_461649.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_461649.dasm.il DevDiv_461649.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DevDiv_461649.asm.exe  DevDiv_461649.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DevDiv_461649.asm.exe  DevDiv_461649.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DevDiv_461649
    if not exist "DevDiv_461649.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DevDiv_461649.org" (
            mkdir IL
            copy DevDiv_461649.exe IL\DevDiv_461649.exe 
            ren DevDiv_461649.exe DevDiv_461649.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_461649.org /out DevDiv_461649.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_461649.org /out DevDiv_461649.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DevDiv_461649.reflect.xml
    IF EXIST DevDiv_461649.reflect.xml set ReflectionRoots=-x DevDiv_461649.reflect.xml
    ...
    set ExePath=%LinkBin%\DevDiv_461649.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DevDiv_461649.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DevDiv_461649.asm.exe %CLRTestExecutionArguments% %Host_Args%
