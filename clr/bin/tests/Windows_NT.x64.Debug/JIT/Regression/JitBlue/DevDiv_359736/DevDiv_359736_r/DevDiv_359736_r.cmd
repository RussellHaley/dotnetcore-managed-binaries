...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_359736\DevDiv_359736_r DevDiv_359736_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_359736\DevDiv_359736_r DevDiv_359736_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_359736_r.dasm.il DevDiv_359736_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_359736_r.dasm.il DevDiv_359736_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DevDiv_359736_r.asm.exe  DevDiv_359736_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DevDiv_359736_r.asm.exe  DevDiv_359736_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DevDiv_359736_r
    if not exist "DevDiv_359736_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DevDiv_359736_r.org" (
            mkdir IL
            copy DevDiv_359736_r.exe IL\DevDiv_359736_r.exe 
            ren DevDiv_359736_r.exe DevDiv_359736_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_359736_r.org /out DevDiv_359736_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_359736_r.org /out DevDiv_359736_r.exe
...
set ExePath=DevDiv_359736_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DevDiv_359736_r.reflect.xml
    IF EXIST DevDiv_359736_r.reflect.xml set ReflectionRoots=-x DevDiv_359736_r.reflect.xml
    ...
    set ExePath=%LinkBin%\DevDiv_359736_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DevDiv_359736_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DevDiv_359736_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
