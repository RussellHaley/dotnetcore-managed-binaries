...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_359736/DevDiv_359736_do DevDiv_359736_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_359736/DevDiv_359736_do DevDiv_359736_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_359736_do.dasm.il DevDiv_359736_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_359736_do.dasm.il DevDiv_359736_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_359736_do.asm.exe  DevDiv_359736_do.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_359736_do.asm.exe  DevDiv_359736_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_359736_do
    if [ ! -f DevDiv_359736_do.org ]; then
        TakeLock
        if [ ! -f DevDiv_359736_do.org ]; then
          mkdir IL
          cp DevDiv_359736_do.exe IL/DevDiv_359736_do.exe
          mv DevDiv_359736_do.exe DevDiv_359736_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_359736_do.org /out DevDiv_359736_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_359736_do.org /out DevDiv_359736_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_359736_do.reflect.xml
  if [ -f DevDiv_359736_do.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_359736_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_359736_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_359736_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_359736_do.asm.exe $CLRTestExecutionArguments
