...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_11343\GitHub_11343 GitHub_11343.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_11343\GitHub_11343 GitHub_11343.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_11343.dasm.il GitHub_11343.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_11343.dasm.il GitHub_11343.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GitHub_11343.asm.exe  GitHub_11343.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GitHub_11343.asm.exe  GitHub_11343.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GitHub_11343
    if not exist "GitHub_11343.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GitHub_11343.org" (
            mkdir IL
            copy GitHub_11343.exe IL\GitHub_11343.exe 
            ren GitHub_11343.exe GitHub_11343.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_11343.org /out GitHub_11343.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_11343.org /out GitHub_11343.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GitHub_11343.reflect.xml
    IF EXIST GitHub_11343.reflect.xml set ReflectionRoots=-x GitHub_11343.reflect.xml
    ...
    set ExePath=%LinkBin%\GitHub_11343.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GitHub_11343.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GitHub_11343.asm.exe %CLRTestExecutionArguments% %Host_Args%
