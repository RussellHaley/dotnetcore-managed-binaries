...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_3449/GitHub_3449 GitHub_3449.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_3449/GitHub_3449 GitHub_3449.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_3449.dasm.il GitHub_3449.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_3449.dasm.il GitHub_3449.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_3449.asm.exe  GitHub_3449.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_3449.asm.exe  GitHub_3449.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_3449
    if [ ! -f GitHub_3449.org ]; then
        TakeLock
        if [ ! -f GitHub_3449.org ]; then
          mkdir IL
          cp GitHub_3449.exe IL/GitHub_3449.exe
          mv GitHub_3449.exe GitHub_3449.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_3449.org /out GitHub_3449.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_3449.org /out GitHub_3449.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_3449.reflect.xml
  if [ -f GitHub_3449.reflect.xml ]
then
    ReflectionRoots="-x GitHub_3449.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_3449.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_3449.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_3449.asm.exe $CLRTestExecutionArguments
