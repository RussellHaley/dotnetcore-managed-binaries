...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_370233/DevDiv_370233 DevDiv_370233.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_370233/DevDiv_370233 DevDiv_370233.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_370233.dasm.il DevDiv_370233.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_370233.dasm.il DevDiv_370233.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_370233.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_370233.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_370233.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_370233.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_370233
    if [ ! -f DevDiv_370233.org ]; then
        TakeLock
        if [ ! -f DevDiv_370233.org ]; then
          mkdir IL
          cp DevDiv_370233.exe IL/DevDiv_370233.exe
          mv DevDiv_370233.exe DevDiv_370233.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_370233.org /out DevDiv_370233.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_370233.org /out DevDiv_370233.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_370233.reflect.xml
  if [ -f DevDiv_370233.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_370233.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_370233.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_370233.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_370233.asm.exe $CLRTestExecutionArguments
