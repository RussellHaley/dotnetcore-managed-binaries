...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_367099/DevDiv_367099 DevDiv_367099.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_367099/DevDiv_367099 DevDiv_367099.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_367099.dasm.il DevDiv_367099.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_367099.dasm.il DevDiv_367099.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_367099.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_367099.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_367099.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_367099.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_367099
    if [ ! -f DevDiv_367099.org ]; then
        TakeLock
        if [ ! -f DevDiv_367099.org ]; then
          mkdir IL
          cp DevDiv_367099.exe IL/DevDiv_367099.exe
          mv DevDiv_367099.exe DevDiv_367099.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_367099.org /out DevDiv_367099.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_367099.org /out DevDiv_367099.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_367099.reflect.xml
  if [ -f DevDiv_367099.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_367099.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_367099.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_367099.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_367099.asm.exe $CLRTestExecutionArguments
