...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_367099\DevDiv_367099 DevDiv_367099.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_367099\DevDiv_367099 DevDiv_367099.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_367099.dasm.il DevDiv_367099.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_367099.dasm.il DevDiv_367099.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DevDiv_367099.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_367099.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DevDiv_367099.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_367099.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DevDiv_367099
    if not exist "DevDiv_367099.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DevDiv_367099.org" (
            mkdir IL
            copy DevDiv_367099.exe IL\DevDiv_367099.exe 
            ren DevDiv_367099.exe DevDiv_367099.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_367099.org /out DevDiv_367099.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_367099.org /out DevDiv_367099.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DevDiv_367099.reflect.xml
    IF EXIST DevDiv_367099.reflect.xml set ReflectionRoots=-x DevDiv_367099.reflect.xml
    ...
    set ExePath=%LinkBin%\DevDiv_367099.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DevDiv_367099.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DevDiv_367099.asm.exe %CLRTestExecutionArguments% %Host_Args%
