...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_406158/DevDiv_406158 DevDiv_406158.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_406158/DevDiv_406158 DevDiv_406158.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_406158.dasm.il DevDiv_406158.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_406158.dasm.il DevDiv_406158.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_406158.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_406158.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_406158.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_406158.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_406158
    if [ ! -f DevDiv_406158.org ]; then
        TakeLock
        if [ ! -f DevDiv_406158.org ]; then
          mkdir IL
          cp DevDiv_406158.exe IL/DevDiv_406158.exe
          mv DevDiv_406158.exe DevDiv_406158.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_406158.org /out DevDiv_406158.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_406158.org /out DevDiv_406158.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_406158.reflect.xml
  if [ -f DevDiv_406158.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_406158.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_406158.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_406158.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_406158.asm.exe $CLRTestExecutionArguments
