...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_255263/DevDiv_255263 DevDiv_255263.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_255263/DevDiv_255263 DevDiv_255263.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_255263.dasm.il DevDiv_255263.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_255263.dasm.il DevDiv_255263.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_255263.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_255263.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_255263.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_255263.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_255263
    if [ ! -f DevDiv_255263.org ]; then
        TakeLock
        if [ ! -f DevDiv_255263.org ]; then
          mkdir IL
          cp DevDiv_255263.exe IL/DevDiv_255263.exe
          mv DevDiv_255263.exe DevDiv_255263.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_255263.org /out DevDiv_255263.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_255263.org /out DevDiv_255263.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_255263.reflect.xml
  if [ -f DevDiv_255263.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_255263.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_255263.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_255263.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_255263.asm.exe $CLRTestExecutionArguments
