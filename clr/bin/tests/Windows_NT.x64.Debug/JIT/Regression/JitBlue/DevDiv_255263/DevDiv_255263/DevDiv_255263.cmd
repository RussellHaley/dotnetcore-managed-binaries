...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_255263\DevDiv_255263 DevDiv_255263.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_255263\DevDiv_255263 DevDiv_255263.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_255263.dasm.il DevDiv_255263.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_255263.dasm.il DevDiv_255263.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DevDiv_255263.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_255263.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DevDiv_255263.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_255263.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DevDiv_255263
    if not exist "DevDiv_255263.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DevDiv_255263.org" (
            mkdir IL
            copy DevDiv_255263.exe IL\DevDiv_255263.exe 
            ren DevDiv_255263.exe DevDiv_255263.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_255263.org /out DevDiv_255263.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_255263.org /out DevDiv_255263.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DevDiv_255263.reflect.xml
    IF EXIST DevDiv_255263.reflect.xml set ReflectionRoots=-x DevDiv_255263.reflect.xml
    ...
    set ExePath=%LinkBin%\DevDiv_255263.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DevDiv_255263.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DevDiv_255263.asm.exe %CLRTestExecutionArguments% %Host_Args%
