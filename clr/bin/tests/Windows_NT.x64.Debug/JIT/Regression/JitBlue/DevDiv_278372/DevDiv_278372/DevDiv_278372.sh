...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_278372/DevDiv_278372 DevDiv_278372.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_278372/DevDiv_278372 DevDiv_278372.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_278372.dasm.il DevDiv_278372.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_278372.dasm.il DevDiv_278372.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_278372.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_278372.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_278372.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_278372.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_278372
    if [ ! -f DevDiv_278372.org ]; then
        TakeLock
        if [ ! -f DevDiv_278372.org ]; then
          mkdir IL
          cp DevDiv_278372.exe IL/DevDiv_278372.exe
          mv DevDiv_278372.exe DevDiv_278372.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_278372.org /out DevDiv_278372.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_278372.org /out DevDiv_278372.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_278372.reflect.xml
  if [ -f DevDiv_278372.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_278372.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_278372.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_278372.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_278372.asm.exe $CLRTestExecutionArguments
