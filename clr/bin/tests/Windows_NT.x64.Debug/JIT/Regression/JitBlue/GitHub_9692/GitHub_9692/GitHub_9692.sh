...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_9692/GitHub_9692 GitHub_9692.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_9692/GitHub_9692 GitHub_9692.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_9692.dasm.il GitHub_9692.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_9692.dasm.il GitHub_9692.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_9692.asm.exe  GitHub_9692.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_9692.asm.exe  GitHub_9692.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_9692
    if [ ! -f GitHub_9692.org ]; then
        TakeLock
        if [ ! -f GitHub_9692.org ]; then
          mkdir IL
          cp GitHub_9692.exe IL/GitHub_9692.exe
          mv GitHub_9692.exe GitHub_9692.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_9692.org /out GitHub_9692.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_9692.org /out GitHub_9692.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_9692.reflect.xml
  if [ -f GitHub_9692.reflect.xml ]
then
    ReflectionRoots="-x GitHub_9692.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_9692.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_9692.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_9692.asm.exe $CLRTestExecutionArguments
