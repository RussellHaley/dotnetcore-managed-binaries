...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/devdiv_902271/DevDiv_902271 DevDiv_902271.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/devdiv_902271/DevDiv_902271 DevDiv_902271.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_902271.dasm.il DevDiv_902271.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_902271.dasm.il DevDiv_902271.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_902271.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_902271.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_902271.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_902271.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_902271
    if [ ! -f DevDiv_902271.org ]; then
        TakeLock
        if [ ! -f DevDiv_902271.org ]; then
          mkdir IL
          cp DevDiv_902271.exe IL/DevDiv_902271.exe
          mv DevDiv_902271.exe DevDiv_902271.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_902271.org /out DevDiv_902271.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_902271.org /out DevDiv_902271.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_902271.reflect.xml
  if [ -f DevDiv_902271.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_902271.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_902271.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_902271.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_902271.asm.exe $CLRTestExecutionArguments
