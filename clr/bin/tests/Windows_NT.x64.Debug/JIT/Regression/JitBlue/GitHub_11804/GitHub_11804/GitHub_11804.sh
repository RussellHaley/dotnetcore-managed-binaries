...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_11804/GitHub_11804 GitHub_11804.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_11804/GitHub_11804 GitHub_11804.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_11804.dasm.il GitHub_11804.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_11804.dasm.il GitHub_11804.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_11804.asm.exe -QUIET -NOLOGO -OPTIMIZE GitHub_11804.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_11804.asm.exe -QUIET -NOLOGO -OPTIMIZE GitHub_11804.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_11804
    if [ ! -f GitHub_11804.org ]; then
        TakeLock
        if [ ! -f GitHub_11804.org ]; then
          mkdir IL
          cp GitHub_11804.exe IL/GitHub_11804.exe
          mv GitHub_11804.exe GitHub_11804.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_11804.org /out GitHub_11804.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_11804.org /out GitHub_11804.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_11804.reflect.xml
  if [ -f GitHub_11804.reflect.xml ]
then
    ReflectionRoots="-x GitHub_11804.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_11804.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_11804.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_11804.asm.exe $CLRTestExecutionArguments
