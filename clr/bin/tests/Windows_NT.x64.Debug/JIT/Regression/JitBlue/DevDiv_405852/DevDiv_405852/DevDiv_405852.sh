...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_405852/DevDiv_405852 DevDiv_405852.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_405852/DevDiv_405852 DevDiv_405852.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_405852.dasm.il DevDiv_405852.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_405852.dasm.il DevDiv_405852.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_405852.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_405852.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_405852.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_405852.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_405852
    if [ ! -f DevDiv_405852.org ]; then
        TakeLock
        if [ ! -f DevDiv_405852.org ]; then
          mkdir IL
          cp DevDiv_405852.exe IL/DevDiv_405852.exe
          mv DevDiv_405852.exe DevDiv_405852.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_405852.org /out DevDiv_405852.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_405852.org /out DevDiv_405852.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_405852.reflect.xml
  if [ -f DevDiv_405852.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_405852.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_405852.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_405852.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_405852.asm.exe $CLRTestExecutionArguments
