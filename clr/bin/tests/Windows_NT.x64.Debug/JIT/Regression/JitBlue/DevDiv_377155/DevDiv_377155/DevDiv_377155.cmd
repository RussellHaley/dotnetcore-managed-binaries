...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_377155\DevDiv_377155 DevDiv_377155.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_377155\DevDiv_377155 DevDiv_377155.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_377155.dasm.il DevDiv_377155.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_377155.dasm.il DevDiv_377155.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DevDiv_377155.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_377155.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DevDiv_377155.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_377155.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DevDiv_377155
    if not exist "DevDiv_377155.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DevDiv_377155.org" (
            mkdir IL
            copy DevDiv_377155.exe IL\DevDiv_377155.exe 
            ren DevDiv_377155.exe DevDiv_377155.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_377155.org /out DevDiv_377155.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_377155.org /out DevDiv_377155.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DevDiv_377155.reflect.xml
    IF EXIST DevDiv_377155.reflect.xml set ReflectionRoots=-x DevDiv_377155.reflect.xml
    ...
    set ExePath=%LinkBin%\DevDiv_377155.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DevDiv_377155.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DevDiv_377155.asm.exe %CLRTestExecutionArguments% %Host_Args%
