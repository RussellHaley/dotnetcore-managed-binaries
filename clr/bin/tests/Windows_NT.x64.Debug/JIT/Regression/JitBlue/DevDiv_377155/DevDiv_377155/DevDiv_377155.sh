...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_377155/DevDiv_377155 DevDiv_377155.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_377155/DevDiv_377155 DevDiv_377155.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_377155.dasm.il DevDiv_377155.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_377155.dasm.il DevDiv_377155.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_377155.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_377155.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_377155.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_377155.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_377155
    if [ ! -f DevDiv_377155.org ]; then
        TakeLock
        if [ ! -f DevDiv_377155.org ]; then
          mkdir IL
          cp DevDiv_377155.exe IL/DevDiv_377155.exe
          mv DevDiv_377155.exe DevDiv_377155.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_377155.org /out DevDiv_377155.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_377155.org /out DevDiv_377155.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_377155.reflect.xml
  if [ -f DevDiv_377155.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_377155.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_377155.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_377155.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_377155.asm.exe $CLRTestExecutionArguments
