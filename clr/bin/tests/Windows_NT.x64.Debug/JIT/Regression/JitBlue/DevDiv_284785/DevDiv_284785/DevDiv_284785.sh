...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_284785/DevDiv_284785 DevDiv_284785.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_284785/DevDiv_284785 DevDiv_284785.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_284785.dasm.il DevDiv_284785.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_284785.dasm.il DevDiv_284785.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_284785.asm.exe  DevDiv_284785.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_284785.asm.exe  DevDiv_284785.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_284785
    if [ ! -f DevDiv_284785.org ]; then
        TakeLock
        if [ ! -f DevDiv_284785.org ]; then
          mkdir IL
          cp DevDiv_284785.exe IL/DevDiv_284785.exe
          mv DevDiv_284785.exe DevDiv_284785.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_284785.org /out DevDiv_284785.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_284785.org /out DevDiv_284785.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_284785.reflect.xml
  if [ -f DevDiv_284785.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_284785.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_284785.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_284785.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_284785.asm.exe $CLRTestExecutionArguments
