...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_8220\GitHub_8220 GitHub_8220.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_8220\GitHub_8220 GitHub_8220.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_8220.dasm.il GitHub_8220.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_8220.dasm.il GitHub_8220.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GitHub_8220.asm.exe  GitHub_8220.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GitHub_8220.asm.exe  GitHub_8220.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GitHub_8220
    if not exist "GitHub_8220.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GitHub_8220.org" (
            mkdir IL
            copy GitHub_8220.exe IL\GitHub_8220.exe 
            ren GitHub_8220.exe GitHub_8220.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_8220.org /out GitHub_8220.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_8220.org /out GitHub_8220.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GitHub_8220.reflect.xml
    IF EXIST GitHub_8220.reflect.xml set ReflectionRoots=-x GitHub_8220.reflect.xml
    ...
    set ExePath=%LinkBin%\GitHub_8220.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GitHub_8220.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GitHub_8220.asm.exe %CLRTestExecutionArguments% %Host_Args%
