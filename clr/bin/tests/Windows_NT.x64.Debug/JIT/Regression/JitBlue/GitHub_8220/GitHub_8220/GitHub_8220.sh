...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_8220/GitHub_8220 GitHub_8220.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_8220/GitHub_8220 GitHub_8220.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_8220.dasm.il GitHub_8220.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_8220.dasm.il GitHub_8220.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_8220.asm.exe  GitHub_8220.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_8220.asm.exe  GitHub_8220.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_8220
    if [ ! -f GitHub_8220.org ]; then
        TakeLock
        if [ ! -f GitHub_8220.org ]; then
          mkdir IL
          cp GitHub_8220.exe IL/GitHub_8220.exe
          mv GitHub_8220.exe GitHub_8220.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_8220.org /out GitHub_8220.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_8220.org /out GitHub_8220.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_8220.reflect.xml
  if [ -f GitHub_8220.reflect.xml ]
then
    ReflectionRoots="-x GitHub_8220.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_8220.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_8220.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_8220.asm.exe $CLRTestExecutionArguments
