...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_13919\GitHub_13919 GitHub_13919.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_13919\GitHub_13919 GitHub_13919.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_13919.dasm.il GitHub_13919.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_13919.dasm.il GitHub_13919.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GitHub_13919.asm.exe  GitHub_13919.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GitHub_13919.asm.exe  GitHub_13919.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GitHub_13919
    if not exist "GitHub_13919.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GitHub_13919.org" (
            mkdir IL
            copy GitHub_13919.exe IL\GitHub_13919.exe 
            ren GitHub_13919.exe GitHub_13919.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_13919.org /out GitHub_13919.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_13919.org /out GitHub_13919.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GitHub_13919.reflect.xml
    IF EXIST GitHub_13919.reflect.xml set ReflectionRoots=-x GitHub_13919.reflect.xml
    ...
    set ExePath=%LinkBin%\GitHub_13919.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GitHub_13919.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GitHub_13919.asm.exe %CLRTestExecutionArguments% %Host_Args%
