...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_362706/DevDiv_362706 DevDiv_362706.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_362706/DevDiv_362706 DevDiv_362706.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_362706.dasm.il DevDiv_362706.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_362706.dasm.il DevDiv_362706.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_362706.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_362706.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_362706.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_362706.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_362706
    if [ ! -f DevDiv_362706.org ]; then
        TakeLock
        if [ ! -f DevDiv_362706.org ]; then
          mkdir IL
          cp DevDiv_362706.exe IL/DevDiv_362706.exe
          mv DevDiv_362706.exe DevDiv_362706.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_362706.org /out DevDiv_362706.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_362706.org /out DevDiv_362706.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_362706.reflect.xml
  if [ -f DevDiv_362706.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_362706.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_362706.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_362706.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_362706.asm.exe $CLRTestExecutionArguments
