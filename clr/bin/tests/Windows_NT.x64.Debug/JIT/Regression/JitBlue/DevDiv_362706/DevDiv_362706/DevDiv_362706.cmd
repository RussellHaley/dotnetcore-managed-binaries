...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_362706\DevDiv_362706 DevDiv_362706.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_362706\DevDiv_362706 DevDiv_362706.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_362706.dasm.il DevDiv_362706.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_362706.dasm.il DevDiv_362706.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DevDiv_362706.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_362706.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DevDiv_362706.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_362706.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DevDiv_362706
    if not exist "DevDiv_362706.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DevDiv_362706.org" (
            mkdir IL
            copy DevDiv_362706.exe IL\DevDiv_362706.exe 
            ren DevDiv_362706.exe DevDiv_362706.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_362706.org /out DevDiv_362706.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_362706.org /out DevDiv_362706.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DevDiv_362706.reflect.xml
    IF EXIST DevDiv_362706.reflect.xml set ReflectionRoots=-x DevDiv_362706.reflect.xml
    ...
    set ExePath=%LinkBin%\DevDiv_362706.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DevDiv_362706.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DevDiv_362706.asm.exe %CLRTestExecutionArguments% %Host_Args%
