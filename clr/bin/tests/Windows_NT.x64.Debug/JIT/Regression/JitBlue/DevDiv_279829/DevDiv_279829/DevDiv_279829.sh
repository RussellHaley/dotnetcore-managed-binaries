...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_279829/DevDiv_279829 DevDiv_279829.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_279829/DevDiv_279829 DevDiv_279829.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_279829.dasm.il DevDiv_279829.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_279829.dasm.il DevDiv_279829.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_279829.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_279829.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_279829.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_279829.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_279829
    if [ ! -f DevDiv_279829.org ]; then
        TakeLock
        if [ ! -f DevDiv_279829.org ]; then
          mkdir IL
          cp DevDiv_279829.exe IL/DevDiv_279829.exe
          mv DevDiv_279829.exe DevDiv_279829.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_279829.org /out DevDiv_279829.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_279829.org /out DevDiv_279829.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_279829.reflect.xml
  if [ -f DevDiv_279829.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_279829.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_279829.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_279829.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_279829.asm.exe $CLRTestExecutionArguments
