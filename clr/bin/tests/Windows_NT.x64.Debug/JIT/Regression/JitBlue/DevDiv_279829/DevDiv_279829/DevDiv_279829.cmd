...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_279829\DevDiv_279829 DevDiv_279829.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\DevDiv_279829\DevDiv_279829 DevDiv_279829.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_279829.dasm.il DevDiv_279829.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DevDiv_279829.dasm.il DevDiv_279829.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DevDiv_279829.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_279829.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DevDiv_279829.asm.exe -QUIET -NOLOGO -OPTIMIZE DevDiv_279829.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DevDiv_279829
    if not exist "DevDiv_279829.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DevDiv_279829.org" (
            mkdir IL
            copy DevDiv_279829.exe IL\DevDiv_279829.exe 
            ren DevDiv_279829.exe DevDiv_279829.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_279829.org /out DevDiv_279829.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DevDiv_279829.org /out DevDiv_279829.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DevDiv_279829.reflect.xml
    IF EXIST DevDiv_279829.reflect.xml set ReflectionRoots=-x DevDiv_279829.reflect.xml
    ...
    set ExePath=%LinkBin%\DevDiv_279829.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DevDiv_279829.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DevDiv_279829.asm.exe %CLRTestExecutionArguments% %Host_Args%
