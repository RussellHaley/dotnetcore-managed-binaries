...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_5164/GitHub_5164 GitHub_5164.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_5164/GitHub_5164 GitHub_5164.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_5164.dasm.il GitHub_5164.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_5164.dasm.il GitHub_5164.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_5164.asm.exe -QUIET -NOLOGO -OPTIMIZE GitHub_5164.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_5164.asm.exe -QUIET -NOLOGO -OPTIMIZE GitHub_5164.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_5164
    if [ ! -f GitHub_5164.org ]; then
        TakeLock
        if [ ! -f GitHub_5164.org ]; then
          mkdir IL
          cp GitHub_5164.exe IL/GitHub_5164.exe
          mv GitHub_5164.exe GitHub_5164.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_5164.org /out GitHub_5164.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_5164.org /out GitHub_5164.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_5164.reflect.xml
  if [ -f GitHub_5164.reflect.xml ]
then
    ReflectionRoots="-x GitHub_5164.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_5164.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_5164.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_5164.asm.exe $CLRTestExecutionArguments
