...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_5164\GitHub_5164 GitHub_5164.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_5164\GitHub_5164 GitHub_5164.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_5164.dasm.il GitHub_5164.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_5164.dasm.il GitHub_5164.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GitHub_5164.asm.exe -QUIET -NOLOGO -OPTIMIZE GitHub_5164.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GitHub_5164.asm.exe -QUIET -NOLOGO -OPTIMIZE GitHub_5164.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GitHub_5164
    if not exist "GitHub_5164.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GitHub_5164.org" (
            mkdir IL
            copy GitHub_5164.exe IL\GitHub_5164.exe 
            ren GitHub_5164.exe GitHub_5164.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_5164.org /out GitHub_5164.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_5164.org /out GitHub_5164.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GitHub_5164.reflect.xml
    IF EXIST GitHub_5164.reflect.xml set ReflectionRoots=-x GitHub_5164.reflect.xml
    ...
    set ExePath=%LinkBin%\GitHub_5164.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GitHub_5164.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GitHub_5164.asm.exe %CLRTestExecutionArguments% %Host_Args%
