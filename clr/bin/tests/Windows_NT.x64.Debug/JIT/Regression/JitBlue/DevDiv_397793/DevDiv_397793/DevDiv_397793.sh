...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_397793/DevDiv_397793 DevDiv_397793.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/DevDiv_397793/DevDiv_397793 DevDiv_397793.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DevDiv_397793.dasm.il DevDiv_397793.exe
    "$CORE_ROOT/ildasm" -raweh -out=DevDiv_397793.dasm.il DevDiv_397793.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DevDiv_397793.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_397793.dasm.il
    "$CORE_ROOT/ilasm" -output=DevDiv_397793.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE DevDiv_397793.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DevDiv_397793
    if [ ! -f DevDiv_397793.org ]; then
        TakeLock
        if [ ! -f DevDiv_397793.org ]; then
          mkdir IL
          cp DevDiv_397793.exe IL/DevDiv_397793.exe
          mv DevDiv_397793.exe DevDiv_397793.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_397793.org /out DevDiv_397793.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DevDiv_397793.org /out DevDiv_397793.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DevDiv_397793.reflect.xml
  if [ -f DevDiv_397793.reflect.xml ]
then
    ReflectionRoots="-x DevDiv_397793.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DevDiv_397793.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DevDiv_397793.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DevDiv_397793.asm.exe $CLRTestExecutionArguments
