...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_4044/GitHub_4044 GitHub_4044.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_4044/GitHub_4044 GitHub_4044.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_4044.dasm.il GitHub_4044.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_4044.dasm.il GitHub_4044.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_4044.asm.exe  GitHub_4044.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_4044.asm.exe  GitHub_4044.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_4044
    if [ ! -f GitHub_4044.org ]; then
        TakeLock
        if [ ! -f GitHub_4044.org ]; then
          mkdir IL
          cp GitHub_4044.exe IL/GitHub_4044.exe
          mv GitHub_4044.exe GitHub_4044.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_4044.org /out GitHub_4044.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_4044.org /out GitHub_4044.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_4044.reflect.xml
  if [ -f GitHub_4044.reflect.xml ]
then
    ReflectionRoots="-x GitHub_4044.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_4044.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_4044.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_4044.asm.exe $CLRTestExecutionArguments
