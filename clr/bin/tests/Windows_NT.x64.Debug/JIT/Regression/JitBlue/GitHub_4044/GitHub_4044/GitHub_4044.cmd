...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_4044\GitHub_4044 GitHub_4044.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_4044\GitHub_4044 GitHub_4044.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_4044.dasm.il GitHub_4044.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GitHub_4044.dasm.il GitHub_4044.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GitHub_4044.asm.exe  GitHub_4044.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GitHub_4044.asm.exe  GitHub_4044.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GitHub_4044
    if not exist "GitHub_4044.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GitHub_4044.org" (
            mkdir IL
            copy GitHub_4044.exe IL\GitHub_4044.exe 
            ren GitHub_4044.exe GitHub_4044.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_4044.org /out GitHub_4044.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHub_4044.org /out GitHub_4044.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GitHub_4044.reflect.xml
    IF EXIST GitHub_4044.reflect.xml set ReflectionRoots=-x GitHub_4044.reflect.xml
    ...
    set ExePath=%LinkBin%\GitHub_4044.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GitHub_4044.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GitHub_4044.asm.exe %CLRTestExecutionArguments% %Host_Args%
