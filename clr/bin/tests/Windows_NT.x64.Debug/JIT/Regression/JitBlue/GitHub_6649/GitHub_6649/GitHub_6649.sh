...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_6649/GitHub_6649 GitHub_6649.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_6649/GitHub_6649 GitHub_6649.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_6649.dasm.il GitHub_6649.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_6649.dasm.il GitHub_6649.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_6649.asm.exe  GitHub_6649.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_6649.asm.exe  GitHub_6649.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_6649
    if [ ! -f GitHub_6649.org ]; then
        TakeLock
        if [ ! -f GitHub_6649.org ]; then
          mkdir IL
          cp GitHub_6649.exe IL/GitHub_6649.exe
          mv GitHub_6649.exe GitHub_6649.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_6649.org /out GitHub_6649.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_6649.org /out GitHub_6649.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_6649.reflect.xml
  if [ -f GitHub_6649.reflect.xml ]
then
    ReflectionRoots="-x GitHub_6649.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_6649.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_6649.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_6649.asm.exe $CLRTestExecutionArguments
