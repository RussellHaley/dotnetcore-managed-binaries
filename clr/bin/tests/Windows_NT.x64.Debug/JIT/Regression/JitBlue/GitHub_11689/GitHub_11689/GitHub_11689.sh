...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_11689/GitHub_11689 GitHub_11689.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_11689/GitHub_11689 GitHub_11689.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHub_11689.dasm.il GitHub_11689.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHub_11689.dasm.il GitHub_11689.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHub_11689.asm.exe  GitHub_11689.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHub_11689.asm.exe  GitHub_11689.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHub_11689
    if [ ! -f GitHub_11689.org ]; then
        TakeLock
        if [ ! -f GitHub_11689.org ]; then
          mkdir IL
          cp GitHub_11689.exe IL/GitHub_11689.exe
          mv GitHub_11689.exe GitHub_11689.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_11689.org /out GitHub_11689.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHub_11689.org /out GitHub_11689.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHub_11689.reflect.xml
  if [ -f GitHub_11689.reflect.xml ]
then
    ReflectionRoots="-x GitHub_11689.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHub_11689.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHub_11689.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHub_11689.asm.exe $CLRTestExecutionArguments
