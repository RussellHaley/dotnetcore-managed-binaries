...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_7508\Vector3Test Vector3Test.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Regression\JitBlue\GitHub_7508\Vector3Test Vector3Test.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Vector3Test.dasm.il Vector3Test.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Vector3Test.dasm.il Vector3Test.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Vector3Test.asm.exe  Vector3Test.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Vector3Test.asm.exe  Vector3Test.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Vector3Test
    if not exist "Vector3Test.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Vector3Test.org" (
            mkdir IL
            copy Vector3Test.exe IL\Vector3Test.exe 
            ren Vector3Test.exe Vector3Test.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector3Test.org /out Vector3Test.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Vector3Test.org /out Vector3Test.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Vector3Test.reflect.xml
    IF EXIST Vector3Test.reflect.xml set ReflectionRoots=-x Vector3Test.reflect.xml
    ...
    set ExePath=%LinkBin%\Vector3Test.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Vector3Test.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Vector3Test.asm.exe %CLRTestExecutionArguments% %Host_Args%
