...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_7508/Vector3Test Vector3Test.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Regression/JitBlue/GitHub_7508/Vector3Test Vector3Test.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Vector3Test.dasm.il Vector3Test.exe
    "$CORE_ROOT/ildasm" -raweh -out=Vector3Test.dasm.il Vector3Test.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Vector3Test.asm.exe  Vector3Test.dasm.il
    "$CORE_ROOT/ilasm" -output=Vector3Test.asm.exe  Vector3Test.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Vector3Test
    if [ ! -f Vector3Test.org ]; then
        TakeLock
        if [ ! -f Vector3Test.org ]; then
          mkdir IL
          cp Vector3Test.exe IL/Vector3Test.exe
          mv Vector3Test.exe Vector3Test.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector3Test.org /out Vector3Test.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Vector3Test.org /out Vector3Test.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Vector3Test.reflect.xml
  if [ -f Vector3Test.reflect.xml ]
then
    ReflectionRoots="-x Vector3Test.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Vector3Test.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Vector3Test.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Vector3Test.asm.exe $CLRTestExecutionArguments
