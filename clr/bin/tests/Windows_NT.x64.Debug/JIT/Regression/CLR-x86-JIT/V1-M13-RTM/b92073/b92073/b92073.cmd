...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b92073\b92073 b92073.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b92073\b92073 b92073.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b92073.dasm.il b92073.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b92073.dasm.il b92073.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b92073.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92073.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b92073.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92073.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b92073
    if not exist "b92073.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b92073.org" (
            mkdir IL
            copy b92073.exe IL\b92073.exe 
            ren b92073.exe b92073.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b92073.org /out b92073.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b92073.org /out b92073.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b92073.reflect.xml
    IF EXIST b92073.reflect.xml set ReflectionRoots=-x b92073.reflect.xml
    ...
    set ExePath=%LinkBin%\b92073.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b92073.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b92073.asm.exe %CLRTestExecutionArguments% %Host_Args%
