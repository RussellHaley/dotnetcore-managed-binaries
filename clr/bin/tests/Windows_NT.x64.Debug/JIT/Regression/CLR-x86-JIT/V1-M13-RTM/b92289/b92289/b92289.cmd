...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b92289\b92289 b92289.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b92289\b92289 b92289.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b92289.dasm.il b92289.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b92289.dasm.il b92289.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b92289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92289.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b92289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92289.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b92289
    if not exist "b92289.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b92289.org" (
            mkdir IL
            copy b92289.exe IL\b92289.exe 
            ren b92289.exe b92289.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b92289.org /out b92289.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b92289.org /out b92289.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b92289.reflect.xml
    IF EXIST b92289.reflect.xml set ReflectionRoots=-x b92289.reflect.xml
    ...
    set ExePath=%LinkBin%\b92289.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b92289.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b92289.asm.exe %CLRTestExecutionArguments% %Host_Args%
