...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b92066\b92066 b92066.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b92066\b92066 b92066.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b92066.dasm.il b92066.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b92066.dasm.il b92066.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b92066.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92066.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b92066.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92066.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b92066
    if not exist "b92066.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b92066.org" (
            mkdir IL
            copy b92066.exe IL\b92066.exe 
            ren b92066.exe b92066.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b92066.org /out b92066.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b92066.org /out b92066.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b92066.reflect.xml
    IF EXIST b92066.reflect.xml set ReflectionRoots=-x b92066.reflect.xml
    ...
    set ExePath=%LinkBin%\b92066.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b92066.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b92066.asm.exe %CLRTestExecutionArguments% %Host_Args%
