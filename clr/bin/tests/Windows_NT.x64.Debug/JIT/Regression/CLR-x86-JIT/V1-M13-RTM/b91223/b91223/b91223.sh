...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b91223/b91223 b91223.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b91223/b91223 b91223.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b91223.dasm.il b91223.exe
    "$CORE_ROOT/ildasm" -raweh -out=b91223.dasm.il b91223.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b91223.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b91223.dasm.il
    "$CORE_ROOT/ilasm" -output=b91223.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b91223.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b91223
    if [ ! -f b91223.org ]; then
        TakeLock
        if [ ! -f b91223.org ]; then
          mkdir IL
          cp b91223.exe IL/b91223.exe
          mv b91223.exe b91223.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91223.org /out b91223.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91223.org /out b91223.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b91223.reflect.xml
  if [ -f b91223.reflect.xml ]
then
    ReflectionRoots="-x b91223.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b91223.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b91223.asm.exe $CLRTestExecutionArguments
