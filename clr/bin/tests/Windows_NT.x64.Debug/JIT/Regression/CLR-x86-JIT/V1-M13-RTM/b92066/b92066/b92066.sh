...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b92066/b92066 b92066.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b92066/b92066 b92066.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b92066.dasm.il b92066.exe
    "$CORE_ROOT/ildasm" -raweh -out=b92066.dasm.il b92066.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b92066.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92066.dasm.il
    "$CORE_ROOT/ilasm" -output=b92066.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92066.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b92066
    if [ ! -f b92066.org ]; then
        TakeLock
        if [ ! -f b92066.org ]; then
          mkdir IL
          cp b92066.exe IL/b92066.exe
          mv b92066.exe b92066.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92066.org /out b92066.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92066.org /out b92066.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b92066.reflect.xml
  if [ -f b92066.reflect.xml ]
then
    ReflectionRoots="-x b92066.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b92066.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b92066.asm.exe $CLRTestExecutionArguments
