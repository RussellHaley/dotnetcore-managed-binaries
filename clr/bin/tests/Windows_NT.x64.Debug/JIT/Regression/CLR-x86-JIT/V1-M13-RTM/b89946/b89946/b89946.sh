...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b89946/b89946 b89946.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b89946/b89946 b89946.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b89946.dasm.il b89946.exe
    "$CORE_ROOT/ildasm" -raweh -out=b89946.dasm.il b89946.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b89946.asm.exe  b89946.dasm.il
    "$CORE_ROOT/ilasm" -output=b89946.asm.exe  b89946.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b89946
    if [ ! -f b89946.org ]; then
        TakeLock
        if [ ! -f b89946.org ]; then
          mkdir IL
          cp b89946.exe IL/b89946.exe
          mv b89946.exe b89946.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b89946.org /out b89946.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b89946.org /out b89946.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b89946.reflect.xml
  if [ -f b89946.reflect.xml ]
then
    ReflectionRoots="-x b89946.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b89946.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b89946.asm.exe $CLRTestExecutionArguments
