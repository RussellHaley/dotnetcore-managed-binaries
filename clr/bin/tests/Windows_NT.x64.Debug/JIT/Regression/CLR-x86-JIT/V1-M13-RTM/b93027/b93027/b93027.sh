...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b93027/b93027 b93027.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b93027/b93027 b93027.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b93027.dasm.il b93027.exe
    "$CORE_ROOT/ildasm" -raweh -out=b93027.dasm.il b93027.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b93027.asm.exe  b93027.dasm.il
    "$CORE_ROOT/ilasm" -output=b93027.asm.exe  b93027.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b93027
    if [ ! -f b93027.org ]; then
        TakeLock
        if [ ! -f b93027.org ]; then
          mkdir IL
          cp b93027.exe IL/b93027.exe
          mv b93027.exe b93027.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b93027.org /out b93027.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b93027.org /out b93027.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b93027.reflect.xml
  if [ -f b93027.reflect.xml ]
then
    ReflectionRoots="-x b93027.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b93027.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b93027.asm.exe $CLRTestExecutionArguments
