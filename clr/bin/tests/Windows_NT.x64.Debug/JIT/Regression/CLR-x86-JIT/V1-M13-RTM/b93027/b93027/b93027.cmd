...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b93027\b93027 b93027.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b93027\b93027 b93027.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b93027.dasm.il b93027.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b93027.dasm.il b93027.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b93027.asm.exe  b93027.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b93027.asm.exe  b93027.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b93027
    if not exist "b93027.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b93027.org" (
            mkdir IL
            copy b93027.exe IL\b93027.exe 
            ren b93027.exe b93027.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b93027.org /out b93027.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b93027.org /out b93027.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b93027.reflect.xml
    IF EXIST b93027.reflect.xml set ReflectionRoots=-x b93027.reflect.xml
    ...
    set ExePath=%LinkBin%\b93027.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b93027.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b93027.asm.exe %CLRTestExecutionArguments% %Host_Args%
