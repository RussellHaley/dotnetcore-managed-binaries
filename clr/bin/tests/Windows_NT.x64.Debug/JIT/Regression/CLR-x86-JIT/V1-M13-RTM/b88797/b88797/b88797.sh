...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b88797/b88797 b88797.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b88797/b88797 b88797.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b88797.dasm.il b88797.exe
    "$CORE_ROOT/ildasm" -raweh -out=b88797.dasm.il b88797.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b88797.asm.exe  b88797.dasm.il
    "$CORE_ROOT/ilasm" -output=b88797.asm.exe  b88797.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b88797
    if [ ! -f b88797.org ]; then
        TakeLock
        if [ ! -f b88797.org ]; then
          mkdir IL
          cp b88797.exe IL/b88797.exe
          mv b88797.exe b88797.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b88797.org /out b88797.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b88797.org /out b88797.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b88797.reflect.xml
  if [ -f b88797.reflect.xml ]
then
    ReflectionRoots="-x b88797.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b88797.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b88797.asm.exe $CLRTestExecutionArguments
