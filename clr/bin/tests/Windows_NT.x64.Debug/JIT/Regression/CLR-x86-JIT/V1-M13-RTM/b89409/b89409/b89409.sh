...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b89409/b89409 b89409.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b89409/b89409 b89409.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b89409.dasm.il b89409.exe
    "$CORE_ROOT/ildasm" -raweh -out=b89409.dasm.il b89409.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b89409.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b89409.dasm.il
    "$CORE_ROOT/ilasm" -output=b89409.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b89409.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b89409
    if [ ! -f b89409.org ]; then
        TakeLock
        if [ ! -f b89409.org ]; then
          mkdir IL
          cp b89409.exe IL/b89409.exe
          mv b89409.exe b89409.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b89409.org /out b89409.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b89409.org /out b89409.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b89409.reflect.xml
  if [ -f b89409.reflect.xml ]
then
    ReflectionRoots="-x b89409.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b89409.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b89409.asm.exe $CLRTestExecutionArguments
