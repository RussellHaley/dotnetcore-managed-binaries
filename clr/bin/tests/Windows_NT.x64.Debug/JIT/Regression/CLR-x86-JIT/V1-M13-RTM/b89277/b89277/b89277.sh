...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b89277/b89277 b89277.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b89277/b89277 b89277.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b89277.dasm.il b89277.exe
    "$CORE_ROOT/ildasm" -raweh -out=b89277.dasm.il b89277.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b89277.asm.exe  b89277.dasm.il
    "$CORE_ROOT/ilasm" -output=b89277.asm.exe  b89277.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b89277
    if [ ! -f b89277.org ]; then
        TakeLock
        if [ ! -f b89277.org ]; then
          mkdir IL
          cp b89277.exe IL/b89277.exe
          mv b89277.exe b89277.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b89277.org /out b89277.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b89277.org /out b89277.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b89277.reflect.xml
  if [ -f b89277.reflect.xml ]
then
    ReflectionRoots="-x b89277.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b89277.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b89277.asm.exe $CLRTestExecutionArguments
