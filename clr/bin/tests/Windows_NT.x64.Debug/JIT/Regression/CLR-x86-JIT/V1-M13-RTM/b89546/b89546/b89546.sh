...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b89546/b89546 b89546.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b89546/b89546 b89546.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b89546.dasm.il b89546.exe
    "$CORE_ROOT/ildasm" -raweh -out=b89546.dasm.il b89546.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b89546.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b89546.dasm.il
    "$CORE_ROOT/ilasm" -output=b89546.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b89546.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b89546
    if [ ! -f b89546.org ]; then
        TakeLock
        if [ ! -f b89546.org ]; then
          mkdir IL
          cp b89546.exe IL/b89546.exe
          mv b89546.exe b89546.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b89546.org /out b89546.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b89546.org /out b89546.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b89546.reflect.xml
  if [ -f b89546.reflect.xml ]
then
    ReflectionRoots="-x b89546.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b89546.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b89546.asm.exe $CLRTestExecutionArguments
