...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b91021/b91021 b91021.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b91021/b91021 b91021.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b91021.dasm.il b91021.exe
    "$CORE_ROOT/ildasm" -raweh -out=b91021.dasm.il b91021.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b91021.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b91021.dasm.il
    "$CORE_ROOT/ilasm" -output=b91021.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b91021.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b91021
    if [ ! -f b91021.org ]; then
        TakeLock
        if [ ! -f b91021.org ]; then
          mkdir IL
          cp b91021.exe IL/b91021.exe
          mv b91021.exe b91021.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91021.org /out b91021.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91021.org /out b91021.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b91021.reflect.xml
  if [ -f b91021.reflect.xml ]
then
    ReflectionRoots="-x b91021.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b91021.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b91021.asm.exe $CLRTestExecutionArguments
