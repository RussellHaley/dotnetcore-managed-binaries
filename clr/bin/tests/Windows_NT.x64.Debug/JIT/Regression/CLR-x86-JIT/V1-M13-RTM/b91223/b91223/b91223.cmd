...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b91223\b91223 b91223.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b91223\b91223 b91223.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b91223.dasm.il b91223.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b91223.dasm.il b91223.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b91223.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b91223.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b91223.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b91223.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b91223
    if not exist "b91223.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b91223.org" (
            mkdir IL
            copy b91223.exe IL\b91223.exe 
            ren b91223.exe b91223.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b91223.org /out b91223.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b91223.org /out b91223.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b91223.reflect.xml
    IF EXIST b91223.reflect.xml set ReflectionRoots=-x b91223.reflect.xml
    ...
    set ExePath=%LinkBin%\b91223.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b91223.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b91223.asm.exe %CLRTestExecutionArguments% %Host_Args%
