...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b99969/b99969 b99969.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b99969/b99969 b99969.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b99969.dasm.il b99969.exe
    "$CORE_ROOT/ildasm" -raweh -out=b99969.dasm.il b99969.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b99969.asm.exe  b99969.dasm.il
    "$CORE_ROOT/ilasm" -output=b99969.asm.exe  b99969.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b99969
    if [ ! -f b99969.org ]; then
        TakeLock
        if [ ! -f b99969.org ]; then
          mkdir IL
          cp b99969.exe IL/b99969.exe
          mv b99969.exe b99969.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b99969.org /out b99969.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b99969.org /out b99969.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b99969.reflect.xml
  if [ -f b99969.reflect.xml ]
then
    ReflectionRoots="-x b99969.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b99969.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b99969.asm.exe $CLRTestExecutionArguments
