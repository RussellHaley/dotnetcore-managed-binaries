...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b98958\b98958 b98958.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b98958\b98958 b98958.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b98958.dasm.il b98958.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b98958.dasm.il b98958.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b98958.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b98958.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b98958.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b98958.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b98958
    if not exist "b98958.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b98958.org" (
            mkdir IL
            copy b98958.exe IL\b98958.exe 
            ren b98958.exe b98958.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b98958.org /out b98958.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b98958.org /out b98958.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b98958.reflect.xml
    IF EXIST b98958.reflect.xml set ReflectionRoots=-x b98958.reflect.xml
    ...
    set ExePath=%LinkBin%\b98958.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b98958.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b98958.asm.exe %CLRTestExecutionArguments% %Host_Args%
