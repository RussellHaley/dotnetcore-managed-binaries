...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b86139\b86139 b86139.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b86139\b86139 b86139.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b86139.dasm.il b86139.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b86139.dasm.il b86139.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b86139.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b86139.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b86139.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b86139.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b86139
    if not exist "b86139.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b86139.org" (
            mkdir IL
            copy b86139.exe IL\b86139.exe 
            ren b86139.exe b86139.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b86139.org /out b86139.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b86139.org /out b86139.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b86139.reflect.xml
    IF EXIST b86139.reflect.xml set ReflectionRoots=-x b86139.reflect.xml
    ...
    set ExePath=%LinkBin%\b86139.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b86139.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b86139.asm.exe %CLRTestExecutionArguments% %Host_Args%
