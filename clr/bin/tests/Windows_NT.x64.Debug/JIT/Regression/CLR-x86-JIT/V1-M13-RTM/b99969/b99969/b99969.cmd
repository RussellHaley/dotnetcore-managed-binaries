...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b99969\b99969 b99969.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b99969\b99969 b99969.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b99969.dasm.il b99969.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b99969.dasm.il b99969.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b99969.asm.exe  b99969.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b99969.asm.exe  b99969.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b99969
    if not exist "b99969.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b99969.org" (
            mkdir IL
            copy b99969.exe IL\b99969.exe 
            ren b99969.exe b99969.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b99969.org /out b99969.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b99969.org /out b99969.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b99969.reflect.xml
    IF EXIST b99969.reflect.xml set ReflectionRoots=-x b99969.reflect.xml
    ...
    set ExePath=%LinkBin%\b99969.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b99969.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b99969.asm.exe %CLRTestExecutionArguments% %Host_Args%
