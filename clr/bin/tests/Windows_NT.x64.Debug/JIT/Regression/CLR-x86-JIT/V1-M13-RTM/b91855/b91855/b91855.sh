...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b91855/b91855 b91855.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b91855/b91855 b91855.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b91855.dasm.il b91855.exe
    "$CORE_ROOT/ildasm" -raweh -out=b91855.dasm.il b91855.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b91855.asm.exe  b91855.dasm.il
    "$CORE_ROOT/ilasm" -output=b91855.asm.exe  b91855.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b91855
    if [ ! -f b91855.org ]; then
        TakeLock
        if [ ! -f b91855.org ]; then
          mkdir IL
          cp b91855.exe IL/b91855.exe
          mv b91855.exe b91855.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91855.org /out b91855.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91855.org /out b91855.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b91855.reflect.xml
  if [ -f b91855.reflect.xml ]
then
    ReflectionRoots="-x b91855.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b91855.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b91855.asm.exe $CLRTestExecutionArguments
