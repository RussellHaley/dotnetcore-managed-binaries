...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b91021\b91021 b91021.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b91021\b91021 b91021.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b91021.dasm.il b91021.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b91021.dasm.il b91021.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b91021.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b91021.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b91021.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b91021.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b91021
    if not exist "b91021.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b91021.org" (
            mkdir IL
            copy b91021.exe IL\b91021.exe 
            ren b91021.exe b91021.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b91021.org /out b91021.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b91021.org /out b91021.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b91021.reflect.xml
    IF EXIST b91021.reflect.xml set ReflectionRoots=-x b91021.reflect.xml
    ...
    set ExePath=%LinkBin%\b91021.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b91021.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b91021.asm.exe %CLRTestExecutionArguments% %Host_Args%
