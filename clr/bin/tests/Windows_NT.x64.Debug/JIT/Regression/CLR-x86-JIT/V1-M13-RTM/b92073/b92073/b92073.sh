...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b92073/b92073 b92073.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b92073/b92073 b92073.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b92073.dasm.il b92073.exe
    "$CORE_ROOT/ildasm" -raweh -out=b92073.dasm.il b92073.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b92073.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92073.dasm.il
    "$CORE_ROOT/ilasm" -output=b92073.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92073.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b92073
    if [ ! -f b92073.org ]; then
        TakeLock
        if [ ! -f b92073.org ]; then
          mkdir IL
          cp b92073.exe IL/b92073.exe
          mv b92073.exe b92073.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92073.org /out b92073.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92073.org /out b92073.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b92073.reflect.xml
  if [ -f b92073.reflect.xml ]
then
    ReflectionRoots="-x b92073.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b92073.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b92073.asm.exe $CLRTestExecutionArguments
