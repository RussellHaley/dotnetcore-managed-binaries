...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b94306\b94306 b94306.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b94306\b94306 b94306.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b94306.dasm.il b94306.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b94306.dasm.il b94306.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b94306.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b94306.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b94306.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b94306.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b94306
    if not exist "b94306.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b94306.org" (
            mkdir IL
            copy b94306.exe IL\b94306.exe 
            ren b94306.exe b94306.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b94306.org /out b94306.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b94306.org /out b94306.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b94306.reflect.xml
    IF EXIST b94306.reflect.xml set ReflectionRoots=-x b94306.reflect.xml
    ...
    set ExePath=%LinkBin%\b94306.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b94306.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b94306.asm.exe %CLRTestExecutionArguments% %Host_Args%
