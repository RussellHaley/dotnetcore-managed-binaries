...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b89409\b89409 b89409.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M13-RTM\b89409\b89409 b89409.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b89409.dasm.il b89409.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b89409.dasm.il b89409.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b89409.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b89409.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b89409.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b89409.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b89409
    if not exist "b89409.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b89409.org" (
            mkdir IL
            copy b89409.exe IL\b89409.exe 
            ren b89409.exe b89409.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b89409.org /out b89409.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b89409.org /out b89409.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b89409.reflect.xml
    IF EXIST b89409.reflect.xml set ReflectionRoots=-x b89409.reflect.xml
    ...
    set ExePath=%LinkBin%\b89409.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b89409.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b89409.asm.exe %CLRTestExecutionArguments% %Host_Args%
