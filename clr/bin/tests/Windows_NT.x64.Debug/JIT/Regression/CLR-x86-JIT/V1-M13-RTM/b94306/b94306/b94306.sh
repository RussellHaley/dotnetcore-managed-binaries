...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b94306/b94306 b94306.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b94306/b94306 b94306.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b94306.dasm.il b94306.exe
    "$CORE_ROOT/ildasm" -raweh -out=b94306.dasm.il b94306.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b94306.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b94306.dasm.il
    "$CORE_ROOT/ilasm" -output=b94306.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b94306.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b94306
    if [ ! -f b94306.org ]; then
        TakeLock
        if [ ! -f b94306.org ]; then
          mkdir IL
          cp b94306.exe IL/b94306.exe
          mv b94306.exe b94306.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b94306.org /out b94306.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b94306.org /out b94306.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b94306.reflect.xml
  if [ -f b94306.reflect.xml ]
then
    ReflectionRoots="-x b94306.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b94306.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b94306.asm.exe $CLRTestExecutionArguments
