...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b92289/b92289 b92289.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b92289/b92289 b92289.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b92289.dasm.il b92289.exe
    "$CORE_ROOT/ildasm" -raweh -out=b92289.dasm.il b92289.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b92289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92289.dasm.il
    "$CORE_ROOT/ilasm" -output=b92289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b92289.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b92289
    if [ ! -f b92289.org ]; then
        TakeLock
        if [ ! -f b92289.org ]; then
          mkdir IL
          cp b92289.exe IL/b92289.exe
          mv b92289.exe b92289.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92289.org /out b92289.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92289.org /out b92289.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b92289.reflect.xml
  if [ -f b92289.reflect.xml ]
then
    ReflectionRoots="-x b92289.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b92289.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b92289.asm.exe $CLRTestExecutionArguments
