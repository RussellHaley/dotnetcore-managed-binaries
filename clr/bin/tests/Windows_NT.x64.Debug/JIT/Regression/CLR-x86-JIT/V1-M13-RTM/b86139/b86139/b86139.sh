...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b86139/b86139 b86139.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b86139/b86139 b86139.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b86139.dasm.il b86139.exe
    "$CORE_ROOT/ildasm" -raweh -out=b86139.dasm.il b86139.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b86139.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b86139.dasm.il
    "$CORE_ROOT/ilasm" -output=b86139.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b86139.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b86139
    if [ ! -f b86139.org ]; then
        TakeLock
        if [ ! -f b86139.org ]; then
          mkdir IL
          cp b86139.exe IL/b86139.exe
          mv b86139.exe b86139.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b86139.org /out b86139.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b86139.org /out b86139.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b86139.reflect.xml
  if [ -f b86139.reflect.xml ]
then
    ReflectionRoots="-x b86139.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b86139.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b86139.asm.exe $CLRTestExecutionArguments
