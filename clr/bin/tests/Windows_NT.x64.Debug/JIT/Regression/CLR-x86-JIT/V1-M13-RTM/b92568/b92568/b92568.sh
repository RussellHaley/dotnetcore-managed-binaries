...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b92568/b92568 b92568.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M13-RTM/b92568/b92568 b92568.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b92568.dasm.il b92568.exe
    "$CORE_ROOT/ildasm" -raweh -out=b92568.dasm.il b92568.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b92568.asm.exe  b92568.dasm.il
    "$CORE_ROOT/ilasm" -output=b92568.asm.exe  b92568.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b92568
    if [ ! -f b92568.org ]; then
        TakeLock
        if [ ! -f b92568.org ]; then
          mkdir IL
          cp b92568.exe IL/b92568.exe
          mv b92568.exe b92568.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92568.org /out b92568.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b92568.org /out b92568.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b92568.reflect.xml
  if [ -f b92568.reflect.xml ]
then
    ReflectionRoots="-x b92568.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b92568.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b92568.asm.exe $CLRTestExecutionArguments
