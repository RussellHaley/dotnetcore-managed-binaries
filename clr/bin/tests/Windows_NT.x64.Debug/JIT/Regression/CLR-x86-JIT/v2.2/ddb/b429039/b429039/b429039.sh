...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.2/ddb/b429039/b429039 b429039.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.2/ddb/b429039/b429039 b429039.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b429039.dasm.il b429039.exe
    "$CORE_ROOT/ildasm" -raweh -out=b429039.dasm.il b429039.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b429039.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b429039.dasm.il
    "$CORE_ROOT/ilasm" -output=b429039.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b429039.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b429039
    if [ ! -f b429039.org ]; then
        TakeLock
        if [ ! -f b429039.org ]; then
          mkdir IL
          cp b429039.exe IL/b429039.exe
          mv b429039.exe b429039.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b429039.org /out b429039.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b429039.org /out b429039.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b429039.reflect.xml
  if [ -f b429039.reflect.xml ]
then
    ReflectionRoots="-x b429039.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b429039.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b429039.asm.exe $CLRTestExecutionArguments
