...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.2\ddb\b429039\b429039 b429039.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.2\ddb\b429039\b429039 b429039.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b429039.dasm.il b429039.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b429039.dasm.il b429039.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b429039.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b429039.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b429039.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b429039.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b429039
    if not exist "b429039.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b429039.org" (
            mkdir IL
            copy b429039.exe IL\b429039.exe 
            ren b429039.exe b429039.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b429039.org /out b429039.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b429039.org /out b429039.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b429039.reflect.xml
    IF EXIST b429039.reflect.xml set ReflectionRoots=-x b429039.reflect.xml
    ...
    set ExePath=%LinkBin%\b429039.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b429039.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b429039.asm.exe %CLRTestExecutionArguments% %Host_Args%
