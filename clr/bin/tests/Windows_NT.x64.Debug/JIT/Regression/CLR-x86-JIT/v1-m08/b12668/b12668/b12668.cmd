...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v1-m08\b12668\b12668 b12668.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v1-m08\b12668\b12668 b12668.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b12668.dasm.il b12668.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b12668.dasm.il b12668.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b12668.asm.exe  b12668.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b12668.asm.exe  b12668.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b12668
    if not exist "b12668.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b12668.org" (
            mkdir IL
            copy b12668.exe IL\b12668.exe 
            ren b12668.exe b12668.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b12668.org /out b12668.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b12668.org /out b12668.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b12668.reflect.xml
    IF EXIST b12668.reflect.xml set ReflectionRoots=-x b12668.reflect.xml
    ...
    set ExePath=%LinkBin%\b12668.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b12668.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b12668.asm.exe %CLRTestExecutionArguments% %Host_Args%
