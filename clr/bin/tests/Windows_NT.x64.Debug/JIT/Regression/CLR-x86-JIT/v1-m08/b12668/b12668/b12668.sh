...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v1-m08/b12668/b12668 b12668.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v1-m08/b12668/b12668 b12668.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b12668.dasm.il b12668.exe
    "$CORE_ROOT/ildasm" -raweh -out=b12668.dasm.il b12668.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b12668.asm.exe  b12668.dasm.il
    "$CORE_ROOT/ilasm" -output=b12668.asm.exe  b12668.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b12668
    if [ ! -f b12668.org ]; then
        TakeLock
        if [ ! -f b12668.org ]; then
          mkdir IL
          cp b12668.exe IL/b12668.exe
          mv b12668.exe b12668.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12668.org /out b12668.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12668.org /out b12668.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b12668.reflect.xml
  if [ -f b12668.reflect.xml ]
then
    ReflectionRoots="-x b12668.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b12668.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b12668.asm.exe $CLRTestExecutionArguments
