...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/b106272/b106272 b106272.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/b106272/b106272 b106272.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b106272.dasm.il b106272.exe
    "$CORE_ROOT/ildasm" -raweh -out=b106272.dasm.il b106272.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b106272.asm.exe  b106272.dasm.il
    "$CORE_ROOT/ilasm" -output=b106272.asm.exe  b106272.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b106272
    if [ ! -f b106272.org ]; then
        TakeLock
        if [ ! -f b106272.org ]; then
          mkdir IL
          cp b106272.exe IL/b106272.exe
          mv b106272.exe b106272.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b106272.org /out b106272.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b106272.org /out b106272.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b106272.reflect.xml
  if [ -f b106272.reflect.xml ]
then
    ReflectionRoots="-x b106272.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b106272.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b106272.asm.exe $CLRTestExecutionArguments
