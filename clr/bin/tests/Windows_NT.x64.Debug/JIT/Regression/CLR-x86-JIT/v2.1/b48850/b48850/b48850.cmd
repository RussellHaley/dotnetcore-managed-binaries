...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\b48850\b48850 b48850.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\b48850\b48850 b48850.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b48850.dasm.il b48850.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b48850.dasm.il b48850.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b48850.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b48850.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b48850.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b48850.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b48850
    if not exist "b48850.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b48850.org" (
            mkdir IL
            copy b48850.exe IL\b48850.exe 
            ren b48850.exe b48850.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b48850.org /out b48850.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b48850.org /out b48850.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b48850.reflect.xml
    IF EXIST b48850.reflect.xml set ReflectionRoots=-x b48850.reflect.xml
    ...
    set ExePath=%LinkBin%\b48850.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b48850.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b48850.asm.exe %CLRTestExecutionArguments% %Host_Args%
