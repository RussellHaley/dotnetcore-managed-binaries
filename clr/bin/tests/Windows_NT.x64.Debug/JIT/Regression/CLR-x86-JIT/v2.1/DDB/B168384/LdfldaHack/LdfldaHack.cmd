...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\DDB\B168384\LdfldaHack LdfldaHack.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\DDB\B168384\LdfldaHack LdfldaHack.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LdfldaHack.dasm.il LdfldaHack.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LdfldaHack.dasm.il LdfldaHack.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LdfldaHack.asm.exe -QUIET -NOLOGO -DEBUG LdfldaHack.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LdfldaHack.asm.exe -QUIET -NOLOGO -DEBUG LdfldaHack.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LdfldaHack
    if not exist "LdfldaHack.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LdfldaHack.org" (
            mkdir IL
            copy LdfldaHack.exe IL\LdfldaHack.exe 
            ren LdfldaHack.exe LdfldaHack.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LdfldaHack.org /out LdfldaHack.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LdfldaHack.org /out LdfldaHack.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LdfldaHack.reflect.xml
    IF EXIST LdfldaHack.reflect.xml set ReflectionRoots=-x LdfldaHack.reflect.xml
    ...
    set ExePath=%LinkBin%\LdfldaHack.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LdfldaHack.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LdfldaHack.asm.exe %CLRTestExecutionArguments% %Host_Args%
