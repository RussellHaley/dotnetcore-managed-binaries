...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/b152292/b152292 b152292.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/b152292/b152292 b152292.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b152292.dasm.il b152292.exe
    "$CORE_ROOT/ildasm" -raweh -out=b152292.dasm.il b152292.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b152292.asm.exe  b152292.dasm.il
    "$CORE_ROOT/ilasm" -output=b152292.asm.exe  b152292.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b152292
    if [ ! -f b152292.org ]; then
        TakeLock
        if [ ! -f b152292.org ]; then
          mkdir IL
          cp b152292.exe IL/b152292.exe
          mv b152292.exe b152292.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b152292.org /out b152292.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b152292.org /out b152292.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b152292.reflect.xml
  if [ -f b152292.reflect.xml ]
then
    ReflectionRoots="-x b152292.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b152292.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b152292.asm.exe $CLRTestExecutionArguments
