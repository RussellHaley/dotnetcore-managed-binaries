...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\DDB\b19679\b19679 b19679.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\DDB\b19679\b19679 b19679.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b19679.dasm.il b19679.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b19679.dasm.il b19679.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b19679.asm.exe  b19679.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b19679.asm.exe  b19679.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b19679
    if not exist "b19679.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b19679.org" (
            mkdir IL
            copy b19679.exe IL\b19679.exe 
            ren b19679.exe b19679.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b19679.org /out b19679.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b19679.org /out b19679.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b19679.reflect.xml
    IF EXIST b19679.reflect.xml set ReflectionRoots=-x b19679.reflect.xml
    ...
    set ExePath=%LinkBin%\b19679.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b19679.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b19679.asm.exe %CLRTestExecutionArguments% %Host_Args%
