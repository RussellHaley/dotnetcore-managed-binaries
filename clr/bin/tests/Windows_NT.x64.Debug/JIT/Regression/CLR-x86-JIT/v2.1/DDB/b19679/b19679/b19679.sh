...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/DDB/b19679/b19679 b19679.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/DDB/b19679/b19679 b19679.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b19679.dasm.il b19679.exe
    "$CORE_ROOT/ildasm" -raweh -out=b19679.dasm.il b19679.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b19679.asm.exe  b19679.dasm.il
    "$CORE_ROOT/ilasm" -output=b19679.asm.exe  b19679.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b19679
    if [ ! -f b19679.org ]; then
        TakeLock
        if [ ! -f b19679.org ]; then
          mkdir IL
          cp b19679.exe IL/b19679.exe
          mv b19679.exe b19679.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b19679.org /out b19679.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b19679.org /out b19679.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b19679.reflect.xml
  if [ -f b19679.reflect.xml ]
then
    ReflectionRoots="-x b19679.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b19679.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b19679.asm.exe $CLRTestExecutionArguments
