...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\b106272\b106272 b106272.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\b106272\b106272 b106272.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b106272.dasm.il b106272.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b106272.dasm.il b106272.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b106272.asm.exe  b106272.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b106272.asm.exe  b106272.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b106272
    if not exist "b106272.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b106272.org" (
            mkdir IL
            copy b106272.exe IL\b106272.exe 
            ren b106272.exe b106272.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b106272.org /out b106272.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b106272.org /out b106272.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b106272.reflect.xml
    IF EXIST b106272.reflect.xml set ReflectionRoots=-x b106272.reflect.xml
    ...
    set ExePath=%LinkBin%\b106272.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b106272.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b106272.asm.exe %CLRTestExecutionArguments% %Host_Args%
