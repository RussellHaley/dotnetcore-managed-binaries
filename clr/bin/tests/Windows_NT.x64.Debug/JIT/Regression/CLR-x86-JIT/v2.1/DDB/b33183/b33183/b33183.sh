...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/DDB/b33183/b33183 b33183.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/DDB/b33183/b33183 b33183.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b33183.dasm.il b33183.exe
    "$CORE_ROOT/ildasm" -raweh -out=b33183.dasm.il b33183.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b33183.asm.exe  b33183.dasm.il
    "$CORE_ROOT/ilasm" -output=b33183.asm.exe  b33183.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b33183
    if [ ! -f b33183.org ]; then
        TakeLock
        if [ ! -f b33183.org ]; then
          mkdir IL
          cp b33183.exe IL/b33183.exe
          mv b33183.exe b33183.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b33183.org /out b33183.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b33183.org /out b33183.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b33183.reflect.xml
  if [ -f b33183.reflect.xml ]
then
    ReflectionRoots="-x b33183.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b33183.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b33183.asm.exe $CLRTestExecutionArguments
