...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/b48850/b48850 b48850.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/b48850/b48850 b48850.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b48850.dasm.il b48850.exe
    "$CORE_ROOT/ildasm" -raweh -out=b48850.dasm.il b48850.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b48850.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b48850.dasm.il
    "$CORE_ROOT/ilasm" -output=b48850.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b48850.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b48850
    if [ ! -f b48850.org ]; then
        TakeLock
        if [ ! -f b48850.org ]; then
          mkdir IL
          cp b48850.exe IL/b48850.exe
          mv b48850.exe b48850.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b48850.org /out b48850.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b48850.org /out b48850.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b48850.reflect.xml
  if [ -f b48850.reflect.xml ]
then
    ReflectionRoots="-x b48850.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b48850.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b48850.asm.exe $CLRTestExecutionArguments
