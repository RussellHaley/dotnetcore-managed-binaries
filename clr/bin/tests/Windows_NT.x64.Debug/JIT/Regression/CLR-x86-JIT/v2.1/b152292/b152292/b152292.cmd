...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\b152292\b152292 b152292.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\v2.1\b152292\b152292 b152292.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b152292.dasm.il b152292.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b152292.dasm.il b152292.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b152292.asm.exe  b152292.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b152292.asm.exe  b152292.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b152292
    if not exist "b152292.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b152292.org" (
            mkdir IL
            copy b152292.exe IL\b152292.exe 
            ren b152292.exe b152292.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b152292.org /out b152292.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b152292.org /out b152292.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b152292.reflect.xml
    IF EXIST b152292.reflect.xml set ReflectionRoots=-x b152292.reflect.xml
    ...
    set ExePath=%LinkBin%\b152292.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b152292.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b152292.asm.exe %CLRTestExecutionArguments% %Host_Args%
