...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/DDB/B168384/LdfldaHack LdfldaHack.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Regression/CLR-x86-JIT/v2.1/DDB/B168384/LdfldaHack LdfldaHack.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LdfldaHack.dasm.il LdfldaHack.exe
    "$CORE_ROOT/ildasm" -raweh -out=LdfldaHack.dasm.il LdfldaHack.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LdfldaHack.asm.exe -QUIET -NOLOGO -DEBUG LdfldaHack.dasm.il
    "$CORE_ROOT/ilasm" -output=LdfldaHack.asm.exe -QUIET -NOLOGO -DEBUG LdfldaHack.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LdfldaHack
    if [ ! -f LdfldaHack.org ]; then
        TakeLock
        if [ ! -f LdfldaHack.org ]; then
          mkdir IL
          cp LdfldaHack.exe IL/LdfldaHack.exe
          mv LdfldaHack.exe LdfldaHack.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LdfldaHack.org /out LdfldaHack.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LdfldaHack.org /out LdfldaHack.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LdfldaHack.reflect.xml
  if [ -f LdfldaHack.reflect.xml ]
then
    ReflectionRoots="-x LdfldaHack.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/LdfldaHack.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LdfldaHack.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LdfldaHack.asm.exe $CLRTestExecutionArguments
