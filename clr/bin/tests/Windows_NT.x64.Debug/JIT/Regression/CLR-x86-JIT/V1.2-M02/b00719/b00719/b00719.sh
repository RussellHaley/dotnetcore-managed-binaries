...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M02/b00719/b00719 b00719.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M02/b00719/b00719 b00719.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b00719.dasm.il b00719.exe
    "$CORE_ROOT/ildasm" -raweh -out=b00719.dasm.il b00719.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b00719.asm.exe  b00719.dasm.il
    "$CORE_ROOT/ilasm" -output=b00719.asm.exe  b00719.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b00719
    if [ ! -f b00719.org ]; then
        TakeLock
        if [ ! -f b00719.org ]; then
          mkdir IL
          cp b00719.exe IL/b00719.exe
          mv b00719.exe b00719.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b00719.org /out b00719.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b00719.org /out b00719.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b00719.reflect.xml
  if [ -f b00719.reflect.xml ]
then
    ReflectionRoots="-x b00719.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b00719.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b00719.asm.exe $CLRTestExecutionArguments
