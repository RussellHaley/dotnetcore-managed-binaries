...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M02\b00719\b00719 b00719.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M02\b00719\b00719 b00719.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b00719.dasm.il b00719.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b00719.dasm.il b00719.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b00719.asm.exe  b00719.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b00719.asm.exe  b00719.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b00719
    if not exist "b00719.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b00719.org" (
            mkdir IL
            copy b00719.exe IL\b00719.exe 
            ren b00719.exe b00719.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b00719.org /out b00719.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b00719.org /out b00719.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b00719.reflect.xml
    IF EXIST b00719.reflect.xml set ReflectionRoots=-x b00719.reflect.xml
    ...
    set ExePath=%LinkBin%\b00719.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b00719.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b00719.asm.exe %CLRTestExecutionArguments% %Host_Args%
