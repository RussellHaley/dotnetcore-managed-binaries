...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b53942\b53942a b53942a.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b53942\b53942a b53942a.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b53942a.dasm.il b53942a.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b53942a.dasm.il b53942a.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b53942a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53942a.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b53942a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53942a.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b53942a
    if not exist "b53942a.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b53942a.org" (
            mkdir IL
            copy b53942a.exe IL\b53942a.exe 
            ren b53942a.exe b53942a.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b53942a.org /out b53942a.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b53942a.org /out b53942a.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b53942a.reflect.xml
    IF EXIST b53942a.reflect.xml set ReflectionRoots=-x b53942a.reflect.xml
    ...
    set ExePath=%LinkBin%\b53942a.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b53942a.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b53942a.asm.exe %CLRTestExecutionArguments% %Host_Args%
