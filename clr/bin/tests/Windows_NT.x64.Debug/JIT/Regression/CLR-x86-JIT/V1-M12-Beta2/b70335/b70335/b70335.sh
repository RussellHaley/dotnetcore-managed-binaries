...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b70335/b70335 b70335.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b70335/b70335 b70335.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b70335.dasm.il b70335.exe
    "$CORE_ROOT/ildasm" -raweh -out=b70335.dasm.il b70335.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b70335.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70335.dasm.il
    "$CORE_ROOT/ilasm" -output=b70335.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70335.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b70335
    if [ ! -f b70335.org ]; then
        TakeLock
        if [ ! -f b70335.org ]; then
          mkdir IL
          cp b70335.exe IL/b70335.exe
          mv b70335.exe b70335.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b70335.org /out b70335.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b70335.org /out b70335.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b70335.reflect.xml
  if [ -f b70335.reflect.xml ]
then
    ReflectionRoots="-x b70335.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b70335.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b70335.asm.exe $CLRTestExecutionArguments
