...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71179/b71179 b71179.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71179/b71179 b71179.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b71179.dasm.il b71179.exe
    "$CORE_ROOT/ildasm" -raweh -out=b71179.dasm.il b71179.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b71179.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b71179.dasm.il
    "$CORE_ROOT/ilasm" -output=b71179.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b71179.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b71179
    if [ ! -f b71179.org ]; then
        TakeLock
        if [ ! -f b71179.org ]; then
          mkdir IL
          cp b71179.exe IL/b71179.exe
          mv b71179.exe b71179.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71179.org /out b71179.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71179.org /out b71179.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b71179.reflect.xml
  if [ -f b71179.reflect.xml ]
then
    ReflectionRoots="-x b71179.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b71179.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b71179.asm.exe $CLRTestExecutionArguments
