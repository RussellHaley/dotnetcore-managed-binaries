...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b50027/b50027 b50027.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b50027/b50027 b50027.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b50027.dasm.il b50027.exe
    "$CORE_ROOT/ildasm" -raweh -out=b50027.dasm.il b50027.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b50027.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b50027.dasm.il
    "$CORE_ROOT/ilasm" -output=b50027.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b50027.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b50027
    if [ ! -f b50027.org ]; then
        TakeLock
        if [ ! -f b50027.org ]; then
          mkdir IL
          cp b50027.exe IL/b50027.exe
          mv b50027.exe b50027.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b50027.org /out b50027.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b50027.org /out b50027.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b50027.reflect.xml
  if [ -f b50027.reflect.xml ]
then
    ReflectionRoots="-x b50027.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b50027.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b50027.asm.exe $CLRTestExecutionArguments
