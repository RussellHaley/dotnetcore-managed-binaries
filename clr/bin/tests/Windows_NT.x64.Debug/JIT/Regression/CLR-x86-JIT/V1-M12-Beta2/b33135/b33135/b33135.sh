...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b33135/b33135 b33135.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b33135/b33135 b33135.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b33135.dasm.il b33135.exe
    "$CORE_ROOT/ildasm" -raweh -out=b33135.dasm.il b33135.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b33135.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33135.dasm.il
    "$CORE_ROOT/ilasm" -output=b33135.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33135.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b33135
    if [ ! -f b33135.org ]; then
        TakeLock
        if [ ! -f b33135.org ]; then
          mkdir IL
          cp b33135.exe IL/b33135.exe
          mv b33135.exe b33135.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b33135.org /out b33135.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b33135.org /out b33135.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b33135.reflect.xml
  if [ -f b33135.reflect.xml ]
then
    ReflectionRoots="-x b33135.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b33135.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b33135.asm.exe $CLRTestExecutionArguments
