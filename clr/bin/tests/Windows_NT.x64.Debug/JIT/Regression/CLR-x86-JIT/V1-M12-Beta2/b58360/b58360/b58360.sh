...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b58360/b58360 b58360.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b58360/b58360 b58360.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b58360.dasm.il b58360.exe
    "$CORE_ROOT/ildasm" -raweh -out=b58360.dasm.il b58360.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b58360.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b58360.dasm.il
    "$CORE_ROOT/ilasm" -output=b58360.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b58360.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b58360
    if [ ! -f b58360.org ]; then
        TakeLock
        if [ ! -f b58360.org ]; then
          mkdir IL
          cp b58360.exe IL/b58360.exe
          mv b58360.exe b58360.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b58360.org /out b58360.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b58360.org /out b58360.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b58360.reflect.xml
  if [ -f b58360.reflect.xml ]
then
    ReflectionRoots="-x b58360.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b58360.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b58360.asm.exe $CLRTestExecutionArguments
