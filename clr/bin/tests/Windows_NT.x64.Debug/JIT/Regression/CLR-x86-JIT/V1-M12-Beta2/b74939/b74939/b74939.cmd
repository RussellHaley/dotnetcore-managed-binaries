...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b74939\b74939 b74939.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b74939\b74939 b74939.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b74939.dasm.il b74939.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b74939.dasm.il b74939.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b74939.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b74939.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b74939.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b74939.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b74939
    if not exist "b74939.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b74939.org" (
            mkdir IL
            copy b74939.exe IL\b74939.exe 
            ren b74939.exe b74939.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b74939.org /out b74939.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b74939.org /out b74939.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b74939.reflect.xml
    IF EXIST b74939.reflect.xml set ReflectionRoots=-x b74939.reflect.xml
    ...
    set ExePath=%LinkBin%\b74939.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b74939.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b74939.asm.exe %CLRTestExecutionArguments% %Host_Args%
