...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b54611\b54611 b54611.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b54611\b54611 b54611.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b54611.dasm.il b54611.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b54611.dasm.il b54611.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b54611.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54611.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b54611.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54611.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b54611
    if not exist "b54611.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b54611.org" (
            mkdir IL
            copy b54611.exe IL\b54611.exe 
            ren b54611.exe b54611.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b54611.org /out b54611.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b54611.org /out b54611.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b54611.reflect.xml
    IF EXIST b54611.reflect.xml set ReflectionRoots=-x b54611.reflect.xml
    ...
    set ExePath=%LinkBin%\b54611.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b54611.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b54611.asm.exe %CLRTestExecutionArguments% %Host_Args%
