...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b51469\b51469 b51469.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b51469\b51469 b51469.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b51469.dasm.il b51469.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b51469.dasm.il b51469.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b51469.asm.exe  b51469.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b51469.asm.exe  b51469.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b51469
    if not exist "b51469.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b51469.org" (
            mkdir IL
            copy b51469.exe IL\b51469.exe 
            ren b51469.exe b51469.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b51469.org /out b51469.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b51469.org /out b51469.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b51469.reflect.xml
    IF EXIST b51469.reflect.xml set ReflectionRoots=-x b51469.reflect.xml
    ...
    set ExePath=%LinkBin%\b51469.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b51469.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b51469.asm.exe %CLRTestExecutionArguments% %Host_Args%
