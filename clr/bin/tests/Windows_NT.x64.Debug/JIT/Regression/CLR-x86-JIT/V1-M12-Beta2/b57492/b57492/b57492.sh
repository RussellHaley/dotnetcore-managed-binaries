...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b57492/b57492 b57492.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b57492/b57492 b57492.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b57492.dasm.il b57492.exe
    "$CORE_ROOT/ildasm" -raweh -out=b57492.dasm.il b57492.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b57492.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57492.dasm.il
    "$CORE_ROOT/ilasm" -output=b57492.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57492.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b57492
    if [ ! -f b57492.org ]; then
        TakeLock
        if [ ! -f b57492.org ]; then
          mkdir IL
          cp b57492.exe IL/b57492.exe
          mv b57492.exe b57492.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b57492.org /out b57492.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b57492.org /out b57492.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b57492.reflect.xml
  if [ -f b57492.reflect.xml ]
then
    ReflectionRoots="-x b57492.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b57492.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b57492.asm.exe $CLRTestExecutionArguments
