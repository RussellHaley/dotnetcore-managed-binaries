...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b65087\b65087 b65087.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b65087\b65087 b65087.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b65087.dasm.il b65087.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b65087.dasm.il b65087.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b65087.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b65087.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b65087.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b65087.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b65087
    if not exist "b65087.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b65087.org" (
            mkdir IL
            copy b65087.exe IL\b65087.exe 
            ren b65087.exe b65087.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b65087.org /out b65087.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b65087.org /out b65087.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b65087.reflect.xml
    IF EXIST b65087.reflect.xml set ReflectionRoots=-x b65087.reflect.xml
    ...
    set ExePath=%LinkBin%\b65087.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b65087.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b65087.asm.exe %CLRTestExecutionArguments% %Host_Args%
