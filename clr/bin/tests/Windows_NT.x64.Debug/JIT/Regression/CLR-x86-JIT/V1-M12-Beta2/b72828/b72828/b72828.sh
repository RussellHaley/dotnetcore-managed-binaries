...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b72828/b72828 b72828.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b72828/b72828 b72828.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b72828.dasm.il b72828.exe
    "$CORE_ROOT/ildasm" -raweh -out=b72828.dasm.il b72828.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b72828.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b72828.dasm.il
    "$CORE_ROOT/ilasm" -output=b72828.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b72828.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b72828
    if [ ! -f b72828.org ]; then
        TakeLock
        if [ ! -f b72828.org ]; then
          mkdir IL
          cp b72828.exe IL/b72828.exe
          mv b72828.exe b72828.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b72828.org /out b72828.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b72828.org /out b72828.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b72828.reflect.xml
  if [ -f b72828.reflect.xml ]
then
    ReflectionRoots="-x b72828.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b72828.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b72828.asm.exe $CLRTestExecutionArguments
