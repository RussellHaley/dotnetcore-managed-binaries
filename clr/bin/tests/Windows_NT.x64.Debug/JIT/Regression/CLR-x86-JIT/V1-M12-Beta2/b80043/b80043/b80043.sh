...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b80043/b80043 b80043.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b80043/b80043 b80043.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b80043.dasm.il b80043.exe
    "$CORE_ROOT/ildasm" -raweh -out=b80043.dasm.il b80043.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b80043.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b80043.dasm.il
    "$CORE_ROOT/ilasm" -output=b80043.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b80043.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b80043
    if [ ! -f b80043.org ]; then
        TakeLock
        if [ ! -f b80043.org ]; then
          mkdir IL
          cp b80043.exe IL/b80043.exe
          mv b80043.exe b80043.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b80043.org /out b80043.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b80043.org /out b80043.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b80043.reflect.xml
  if [ -f b80043.reflect.xml ]
then
    ReflectionRoots="-x b80043.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b80043.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b80043.asm.exe $CLRTestExecutionArguments
