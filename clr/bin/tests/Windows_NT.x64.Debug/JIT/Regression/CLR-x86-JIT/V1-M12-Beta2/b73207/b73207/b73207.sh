...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b73207/b73207 b73207.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b73207/b73207 b73207.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b73207.dasm.il b73207.exe
    "$CORE_ROOT/ildasm" -raweh -out=b73207.dasm.il b73207.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b73207.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b73207.dasm.il
    "$CORE_ROOT/ilasm" -output=b73207.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b73207.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b73207
    if [ ! -f b73207.org ]; then
        TakeLock
        if [ ! -f b73207.org ]; then
          mkdir IL
          cp b73207.exe IL/b73207.exe
          mv b73207.exe b73207.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b73207.org /out b73207.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b73207.org /out b73207.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b73207.reflect.xml
  if [ -f b73207.reflect.xml ]
then
    ReflectionRoots="-x b73207.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b73207.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b73207.asm.exe $CLRTestExecutionArguments
