...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b34953/b34953 b34953.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b34953/b34953 b34953.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b34953.dasm.il b34953.exe
    "$CORE_ROOT/ildasm" -raweh -out=b34953.dasm.il b34953.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b34953.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b34953.dasm.il
    "$CORE_ROOT/ilasm" -output=b34953.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b34953.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b34953
    if [ ! -f b34953.org ]; then
        TakeLock
        if [ ! -f b34953.org ]; then
          mkdir IL
          cp b34953.exe IL/b34953.exe
          mv b34953.exe b34953.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b34953.org /out b34953.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b34953.org /out b34953.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b34953.reflect.xml
  if [ -f b34953.reflect.xml ]
then
    ReflectionRoots="-x b34953.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b34953.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b34953.asm.exe $CLRTestExecutionArguments
