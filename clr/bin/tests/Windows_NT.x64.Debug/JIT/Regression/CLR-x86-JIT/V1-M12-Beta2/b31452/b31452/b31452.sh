...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31452/b31452 b31452.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31452/b31452 b31452.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31452.dasm.il b31452.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31452.dasm.il b31452.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31452.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31452.dasm.il
    "$CORE_ROOT/ilasm" -output=b31452.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31452.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31452
    if [ ! -f b31452.org ]; then
        TakeLock
        if [ ! -f b31452.org ]; then
          mkdir IL
          cp b31452.exe IL/b31452.exe
          mv b31452.exe b31452.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31452.org /out b31452.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31452.org /out b31452.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31452.reflect.xml
  if [ -f b31452.reflect.xml ]
then
    ReflectionRoots="-x b31452.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31452.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31452.asm.exe $CLRTestExecutionArguments
