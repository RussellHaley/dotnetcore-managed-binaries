...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b35351/b35351 b35351.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b35351/b35351 b35351.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b35351.dasm.il b35351.exe
    "$CORE_ROOT/ildasm" -raweh -out=b35351.dasm.il b35351.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b35351.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b35351.dasm.il
    "$CORE_ROOT/ilasm" -output=b35351.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b35351.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b35351
    if [ ! -f b35351.org ]; then
        TakeLock
        if [ ! -f b35351.org ]; then
          mkdir IL
          cp b35351.exe IL/b35351.exe
          mv b35351.exe b35351.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b35351.org /out b35351.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b35351.org /out b35351.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b35351.reflect.xml
  if [ -f b35351.reflect.xml ]
then
    ReflectionRoots="-x b35351.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b35351.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b35351.asm.exe $CLRTestExecutionArguments
