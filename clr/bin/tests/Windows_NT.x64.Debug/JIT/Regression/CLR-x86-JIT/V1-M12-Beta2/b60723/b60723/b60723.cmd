...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b60723\b60723 b60723.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b60723\b60723 b60723.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b60723.dasm.il b60723.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b60723.dasm.il b60723.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b60723.asm.exe  b60723.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b60723.asm.exe  b60723.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b60723
    if not exist "b60723.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b60723.org" (
            mkdir IL
            copy b60723.exe IL\b60723.exe 
            ren b60723.exe b60723.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b60723.org /out b60723.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b60723.org /out b60723.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b60723.reflect.xml
    IF EXIST b60723.reflect.xml set ReflectionRoots=-x b60723.reflect.xml
    ...
    set ExePath=%LinkBin%\b60723.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b60723.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b60723.asm.exe %CLRTestExecutionArguments% %Host_Args%
