...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b69848\b69848 b69848.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b69848\b69848 b69848.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b69848.dasm.il b69848.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b69848.dasm.il b69848.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b69848.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69848.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b69848.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69848.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b69848
    if not exist "b69848.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b69848.org" (
            mkdir IL
            copy b69848.exe IL\b69848.exe 
            ren b69848.exe b69848.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b69848.org /out b69848.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b69848.org /out b69848.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b69848.reflect.xml
    IF EXIST b69848.reflect.xml set ReflectionRoots=-x b69848.reflect.xml
    ...
    set ExePath=%LinkBin%\b69848.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b69848.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b69848.asm.exe %CLRTestExecutionArguments% %Host_Args%
