...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b80045/b80045 b80045.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b80045/b80045 b80045.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b80045.dasm.il b80045.exe
    "$CORE_ROOT/ildasm" -raweh -out=b80045.dasm.il b80045.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b80045.asm.exe  b80045.dasm.il
    "$CORE_ROOT/ilasm" -output=b80045.asm.exe  b80045.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b80045
    if [ ! -f b80045.org ]; then
        TakeLock
        if [ ! -f b80045.org ]; then
          mkdir IL
          cp b80045.exe IL/b80045.exe
          mv b80045.exe b80045.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b80045.org /out b80045.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b80045.org /out b80045.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b80045.reflect.xml
  if [ -f b80045.reflect.xml ]
then
    ReflectionRoots="-x b80045.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b80045.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b80045.asm.exe $CLRTestExecutionArguments
