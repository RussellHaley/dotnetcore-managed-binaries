...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b74939/b74939 b74939.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b74939/b74939 b74939.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b74939.dasm.il b74939.exe
    "$CORE_ROOT/ildasm" -raweh -out=b74939.dasm.il b74939.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b74939.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b74939.dasm.il
    "$CORE_ROOT/ilasm" -output=b74939.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b74939.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b74939
    if [ ! -f b74939.org ]; then
        TakeLock
        if [ ! -f b74939.org ]; then
          mkdir IL
          cp b74939.exe IL/b74939.exe
          mv b74939.exe b74939.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b74939.org /out b74939.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b74939.org /out b74939.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b74939.reflect.xml
  if [ -f b74939.reflect.xml ]
then
    ReflectionRoots="-x b74939.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b74939.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b74939.asm.exe $CLRTestExecutionArguments
