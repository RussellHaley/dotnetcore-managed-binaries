...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b37214/b37214 b37214.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b37214/b37214 b37214.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b37214.dasm.il b37214.exe
    "$CORE_ROOT/ildasm" -raweh -out=b37214.dasm.il b37214.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b37214.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37214.dasm.il
    "$CORE_ROOT/ilasm" -output=b37214.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37214.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b37214
    if [ ! -f b37214.org ]; then
        TakeLock
        if [ ! -f b37214.org ]; then
          mkdir IL
          cp b37214.exe IL/b37214.exe
          mv b37214.exe b37214.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b37214.org /out b37214.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b37214.org /out b37214.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b37214.reflect.xml
  if [ -f b37214.reflect.xml ]
then
    ReflectionRoots="-x b37214.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b37214.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b37214.asm.exe $CLRTestExecutionArguments
