...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31289/b31289 b31289.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31289/b31289 b31289.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31289.dasm.il b31289.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31289.dasm.il b31289.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31289.dasm.il
    "$CORE_ROOT/ilasm" -output=b31289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31289.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31289
    if [ ! -f b31289.org ]; then
        TakeLock
        if [ ! -f b31289.org ]; then
          mkdir IL
          cp b31289.exe IL/b31289.exe
          mv b31289.exe b31289.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31289.org /out b31289.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31289.org /out b31289.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31289.reflect.xml
  if [ -f b31289.reflect.xml ]
then
    ReflectionRoots="-x b31289.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31289.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31289.asm.exe $CLRTestExecutionArguments
