...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b62555\b62555 b62555.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b62555\b62555 b62555.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b62555.dasm.il b62555.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b62555.dasm.il b62555.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b62555.asm.exe  b62555.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b62555.asm.exe  b62555.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b62555
    if not exist "b62555.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b62555.org" (
            mkdir IL
            copy b62555.exe IL\b62555.exe 
            ren b62555.exe b62555.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b62555.org /out b62555.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b62555.org /out b62555.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b62555.reflect.xml
    IF EXIST b62555.reflect.xml set ReflectionRoots=-x b62555.reflect.xml
    ...
    set ExePath=%LinkBin%\b62555.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b62555.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b62555.asm.exe %CLRTestExecutionArguments% %Host_Args%
