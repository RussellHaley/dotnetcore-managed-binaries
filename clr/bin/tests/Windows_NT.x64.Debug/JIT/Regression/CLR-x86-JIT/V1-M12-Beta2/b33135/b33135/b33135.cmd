...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b33135\b33135 b33135.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b33135\b33135 b33135.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b33135.dasm.il b33135.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b33135.dasm.il b33135.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b33135.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33135.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b33135.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33135.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b33135
    if not exist "b33135.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b33135.org" (
            mkdir IL
            copy b33135.exe IL\b33135.exe 
            ren b33135.exe b33135.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b33135.org /out b33135.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b33135.org /out b33135.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b33135.reflect.xml
    IF EXIST b33135.reflect.xml set ReflectionRoots=-x b33135.reflect.xml
    ...
    set ExePath=%LinkBin%\b33135.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b33135.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b33135.asm.exe %CLRTestExecutionArguments% %Host_Args%
