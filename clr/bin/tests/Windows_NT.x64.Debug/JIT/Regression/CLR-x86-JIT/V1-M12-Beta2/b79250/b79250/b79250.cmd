...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b79250\b79250 b79250.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b79250\b79250 b79250.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b79250.dasm.il b79250.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b79250.dasm.il b79250.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b79250.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b79250.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b79250.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b79250.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b79250
    if not exist "b79250.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b79250.org" (
            mkdir IL
            copy b79250.exe IL\b79250.exe 
            ren b79250.exe b79250.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b79250.org /out b79250.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b79250.org /out b79250.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b79250.reflect.xml
    IF EXIST b79250.reflect.xml set ReflectionRoots=-x b79250.reflect.xml
    ...
    set ExePath=%LinkBin%\b79250.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b79250.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b79250.asm.exe %CLRTestExecutionArguments% %Host_Args%
