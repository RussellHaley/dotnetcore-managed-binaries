...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b51420\b51420 b51420.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b51420\b51420 b51420.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b51420.dasm.il b51420.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b51420.dasm.il b51420.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b51420.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b51420.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b51420.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b51420.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b51420
    if not exist "b51420.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b51420.org" (
            mkdir IL
            copy b51420.exe IL\b51420.exe 
            ren b51420.exe b51420.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b51420.org /out b51420.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b51420.org /out b51420.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b51420.reflect.xml
    IF EXIST b51420.reflect.xml set ReflectionRoots=-x b51420.reflect.xml
    ...
    set ExePath=%LinkBin%\b51420.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b51420.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b51420.asm.exe %CLRTestExecutionArguments% %Host_Args%
