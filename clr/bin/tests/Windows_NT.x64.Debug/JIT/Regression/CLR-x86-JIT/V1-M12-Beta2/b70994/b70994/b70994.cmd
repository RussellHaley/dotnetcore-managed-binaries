...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b70994\b70994 b70994.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b70994\b70994 b70994.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b70994.dasm.il b70994.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b70994.dasm.il b70994.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b70994.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70994.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b70994.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70994.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b70994
    if not exist "b70994.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b70994.org" (
            mkdir IL
            copy b70994.exe IL\b70994.exe 
            ren b70994.exe b70994.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b70994.org /out b70994.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b70994.org /out b70994.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b70994.reflect.xml
    IF EXIST b70994.reflect.xml set ReflectionRoots=-x b70994.reflect.xml
    ...
    set ExePath=%LinkBin%\b70994.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b70994.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b70994.asm.exe %CLRTestExecutionArguments% %Host_Args%
