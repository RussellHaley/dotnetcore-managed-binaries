...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b75944\b75944 b75944.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b75944\b75944 b75944.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b75944.dasm.il b75944.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b75944.dasm.il b75944.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b75944.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75944.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b75944.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75944.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b75944
    if not exist "b75944.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b75944.org" (
            mkdir IL
            copy b75944.exe IL\b75944.exe 
            ren b75944.exe b75944.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b75944.org /out b75944.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b75944.org /out b75944.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b75944.reflect.xml
    IF EXIST b75944.reflect.xml set ReflectionRoots=-x b75944.reflect.xml
    ...
    set ExePath=%LinkBin%\b75944.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b75944.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b75944.asm.exe %CLRTestExecutionArguments% %Host_Args%
