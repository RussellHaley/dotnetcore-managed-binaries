...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b68028/b68028 b68028.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b68028/b68028 b68028.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b68028.dasm.il b68028.exe
    "$CORE_ROOT/ildasm" -raweh -out=b68028.dasm.il b68028.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b68028.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b68028.dasm.il
    "$CORE_ROOT/ilasm" -output=b68028.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b68028.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b68028
    if [ ! -f b68028.org ]; then
        TakeLock
        if [ ! -f b68028.org ]; then
          mkdir IL
          cp b68028.exe IL/b68028.exe
          mv b68028.exe b68028.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b68028.org /out b68028.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b68028.org /out b68028.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b68028.reflect.xml
  if [ -f b68028.reflect.xml ]
then
    ReflectionRoots="-x b68028.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b68028.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b68028.asm.exe $CLRTestExecutionArguments
