...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b62145/b62145 b62145.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b62145/b62145 b62145.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b62145.dasm.il b62145.exe
    "$CORE_ROOT/ildasm" -raweh -out=b62145.dasm.il b62145.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b62145.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b62145.dasm.il
    "$CORE_ROOT/ilasm" -output=b62145.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b62145.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b62145
    if [ ! -f b62145.org ]; then
        TakeLock
        if [ ! -f b62145.org ]; then
          mkdir IL
          cp b62145.exe IL/b62145.exe
          mv b62145.exe b62145.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b62145.org /out b62145.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b62145.org /out b62145.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b62145.reflect.xml
  if [ -f b62145.reflect.xml ]
then
    ReflectionRoots="-x b62145.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b62145.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b62145.asm.exe $CLRTestExecutionArguments
