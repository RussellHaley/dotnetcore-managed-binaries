...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b37238\b37238 b37238.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b37238\b37238 b37238.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b37238.dasm.il b37238.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b37238.dasm.il b37238.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b37238.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37238.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b37238.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37238.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b37238
    if not exist "b37238.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b37238.org" (
            mkdir IL
            copy b37238.exe IL\b37238.exe 
            ren b37238.exe b37238.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37238.org /out b37238.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37238.org /out b37238.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b37238.reflect.xml
    IF EXIST b37238.reflect.xml set ReflectionRoots=-x b37238.reflect.xml
    ...
    set ExePath=%LinkBin%\b37238.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b37238.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b37238.asm.exe %CLRTestExecutionArguments% %Host_Args%
