...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b61215\b61215 b61215.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b61215\b61215 b61215.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b61215.dasm.il b61215.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b61215.dasm.il b61215.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b61215.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61215.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b61215.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61215.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b61215
    if not exist "b61215.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b61215.org" (
            mkdir IL
            copy b61215.exe IL\b61215.exe 
            ren b61215.exe b61215.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b61215.org /out b61215.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b61215.org /out b61215.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b61215.reflect.xml
    IF EXIST b61215.reflect.xml set ReflectionRoots=-x b61215.reflect.xml
    ...
    set ExePath=%LinkBin%\b61215.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b61215.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b61215.asm.exe %CLRTestExecutionArguments% %Host_Args%
