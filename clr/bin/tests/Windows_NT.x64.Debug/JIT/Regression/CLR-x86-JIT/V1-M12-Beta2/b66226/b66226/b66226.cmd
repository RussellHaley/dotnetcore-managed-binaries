...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b66226\b66226 b66226.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b66226\b66226 b66226.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b66226.dasm.il b66226.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b66226.dasm.il b66226.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b66226.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b66226.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b66226.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b66226.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b66226
    if not exist "b66226.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b66226.org" (
            mkdir IL
            copy b66226.exe IL\b66226.exe 
            ren b66226.exe b66226.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b66226.org /out b66226.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b66226.org /out b66226.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b66226.reflect.xml
    IF EXIST b66226.reflect.xml set ReflectionRoots=-x b66226.reflect.xml
    ...
    set ExePath=%LinkBin%\b66226.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b66226.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b66226.asm.exe %CLRTestExecutionArguments% %Host_Args%
