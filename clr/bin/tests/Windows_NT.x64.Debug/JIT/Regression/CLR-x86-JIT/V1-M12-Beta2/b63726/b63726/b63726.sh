...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b63726/b63726 b63726.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b63726/b63726 b63726.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b63726.dasm.il b63726.exe
    "$CORE_ROOT/ildasm" -raweh -out=b63726.dasm.il b63726.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b63726.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b63726.dasm.il
    "$CORE_ROOT/ilasm" -output=b63726.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b63726.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b63726
    if [ ! -f b63726.org ]; then
        TakeLock
        if [ ! -f b63726.org ]; then
          mkdir IL
          cp b63726.exe IL/b63726.exe
          mv b63726.exe b63726.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b63726.org /out b63726.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b63726.org /out b63726.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b63726.reflect.xml
  if [ -f b63726.reflect.xml ]
then
    ReflectionRoots="-x b63726.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b63726.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b63726.asm.exe $CLRTestExecutionArguments
