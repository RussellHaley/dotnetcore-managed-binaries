...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b80043\b80043 b80043.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b80043\b80043 b80043.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b80043.dasm.il b80043.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b80043.dasm.il b80043.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b80043.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b80043.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b80043.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b80043.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b80043
    if not exist "b80043.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b80043.org" (
            mkdir IL
            copy b80043.exe IL\b80043.exe 
            ren b80043.exe b80043.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b80043.org /out b80043.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b80043.org /out b80043.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b80043.reflect.xml
    IF EXIST b80043.reflect.xml set ReflectionRoots=-x b80043.reflect.xml
    ...
    set ExePath=%LinkBin%\b80043.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b80043.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b80043.asm.exe %CLRTestExecutionArguments% %Host_Args%
