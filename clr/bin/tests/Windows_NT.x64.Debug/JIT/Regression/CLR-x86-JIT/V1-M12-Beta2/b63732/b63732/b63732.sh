...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b63732/b63732 b63732.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b63732/b63732 b63732.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b63732.dasm.il b63732.exe
    "$CORE_ROOT/ildasm" -raweh -out=b63732.dasm.il b63732.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b63732.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b63732.dasm.il
    "$CORE_ROOT/ilasm" -output=b63732.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b63732.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b63732
    if [ ! -f b63732.org ]; then
        TakeLock
        if [ ! -f b63732.org ]; then
          mkdir IL
          cp b63732.exe IL/b63732.exe
          mv b63732.exe b63732.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b63732.org /out b63732.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b63732.org /out b63732.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b63732.reflect.xml
  if [ -f b63732.reflect.xml ]
then
    ReflectionRoots="-x b63732.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b63732.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b63732.asm.exe $CLRTestExecutionArguments
