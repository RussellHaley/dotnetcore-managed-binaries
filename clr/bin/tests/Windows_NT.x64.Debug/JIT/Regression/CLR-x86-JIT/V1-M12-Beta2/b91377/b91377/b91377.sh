...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b91377/b91377 b91377.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b91377/b91377 b91377.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b91377.dasm.il b91377.exe
    "$CORE_ROOT/ildasm" -raweh -out=b91377.dasm.il b91377.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b91377.asm.exe  b91377.dasm.il
    "$CORE_ROOT/ilasm" -output=b91377.asm.exe  b91377.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b91377
    if [ ! -f b91377.org ]; then
        TakeLock
        if [ ! -f b91377.org ]; then
          mkdir IL
          cp b91377.exe IL/b91377.exe
          mv b91377.exe b91377.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91377.org /out b91377.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b91377.org /out b91377.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b91377.reflect.xml
  if [ -f b91377.reflect.xml ]
then
    ReflectionRoots="-x b91377.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b91377.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b91377.asm.exe $CLRTestExecutionArguments
