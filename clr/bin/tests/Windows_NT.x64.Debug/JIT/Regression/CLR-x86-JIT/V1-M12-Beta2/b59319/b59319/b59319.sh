...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b59319/b59319 b59319.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b59319/b59319 b59319.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b59319.dasm.il b59319.exe
    "$CORE_ROOT/ildasm" -raweh -out=b59319.dasm.il b59319.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b59319.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59319.dasm.il
    "$CORE_ROOT/ilasm" -output=b59319.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59319.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b59319
    if [ ! -f b59319.org ]; then
        TakeLock
        if [ ! -f b59319.org ]; then
          mkdir IL
          cp b59319.exe IL/b59319.exe
          mv b59319.exe b59319.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b59319.org /out b59319.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b59319.org /out b59319.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b59319.reflect.xml
  if [ -f b59319.reflect.xml ]
then
    ReflectionRoots="-x b59319.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b59319.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b59319.asm.exe $CLRTestExecutionArguments
