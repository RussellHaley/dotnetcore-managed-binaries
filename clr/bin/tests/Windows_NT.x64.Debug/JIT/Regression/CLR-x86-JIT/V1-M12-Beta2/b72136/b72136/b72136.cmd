...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b72136\b72136 b72136.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b72136\b72136 b72136.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b72136.dasm.il b72136.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b72136.dasm.il b72136.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b72136.asm.exe  b72136.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b72136.asm.exe  b72136.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b72136
    if not exist "b72136.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b72136.org" (
            mkdir IL
            copy b72136.exe IL\b72136.exe 
            ren b72136.exe b72136.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b72136.org /out b72136.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b72136.org /out b72136.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b72136.reflect.xml
    IF EXIST b72136.reflect.xml set ReflectionRoots=-x b72136.reflect.xml
    ...
    set ExePath=%LinkBin%\b72136.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b72136.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b72136.asm.exe %CLRTestExecutionArguments% %Host_Args%
