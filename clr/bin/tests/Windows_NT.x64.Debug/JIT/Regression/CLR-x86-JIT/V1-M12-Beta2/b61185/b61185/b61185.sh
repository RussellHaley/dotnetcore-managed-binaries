...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b61185/b61185 b61185.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b61185/b61185 b61185.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b61185.dasm.il b61185.exe
    "$CORE_ROOT/ildasm" -raweh -out=b61185.dasm.il b61185.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b61185.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61185.dasm.il
    "$CORE_ROOT/ilasm" -output=b61185.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61185.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b61185
    if [ ! -f b61185.org ]; then
        TakeLock
        if [ ! -f b61185.org ]; then
          mkdir IL
          cp b61185.exe IL/b61185.exe
          mv b61185.exe b61185.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b61185.org /out b61185.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b61185.org /out b61185.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b61185.reflect.xml
  if [ -f b61185.reflect.xml ]
then
    ReflectionRoots="-x b61185.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b61185.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b61185.asm.exe $CLRTestExecutionArguments
