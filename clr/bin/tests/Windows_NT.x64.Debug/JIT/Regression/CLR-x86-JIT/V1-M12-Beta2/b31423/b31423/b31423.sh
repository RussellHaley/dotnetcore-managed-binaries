...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31423/b31423 b31423.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31423/b31423 b31423.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31423.dasm.il b31423.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31423.dasm.il b31423.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31423.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31423.dasm.il
    "$CORE_ROOT/ilasm" -output=b31423.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31423.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31423
    if [ ! -f b31423.org ]; then
        TakeLock
        if [ ! -f b31423.org ]; then
          mkdir IL
          cp b31423.exe IL/b31423.exe
          mv b31423.exe b31423.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31423.org /out b31423.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31423.org /out b31423.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31423.reflect.xml
  if [ -f b31423.reflect.xml ]
then
    ReflectionRoots="-x b31423.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31423.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31423.asm.exe $CLRTestExecutionArguments
