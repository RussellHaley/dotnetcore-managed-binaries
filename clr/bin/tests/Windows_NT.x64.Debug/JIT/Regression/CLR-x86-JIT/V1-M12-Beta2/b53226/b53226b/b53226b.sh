...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53226/b53226b b53226b.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53226/b53226b b53226b.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b53226b.dasm.il b53226b.exe
    "$CORE_ROOT/ildasm" -raweh -out=b53226b.dasm.il b53226b.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b53226b.asm.exe  b53226b.dasm.il
    "$CORE_ROOT/ilasm" -output=b53226b.asm.exe  b53226b.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b53226b
    if [ ! -f b53226b.org ]; then
        TakeLock
        if [ ! -f b53226b.org ]; then
          mkdir IL
          cp b53226b.exe IL/b53226b.exe
          mv b53226b.exe b53226b.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53226b.org /out b53226b.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53226b.org /out b53226b.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b53226b.reflect.xml
  if [ -f b53226b.reflect.xml ]
then
    ReflectionRoots="-x b53226b.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b53226b.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b53226b.asm.exe $CLRTestExecutionArguments
