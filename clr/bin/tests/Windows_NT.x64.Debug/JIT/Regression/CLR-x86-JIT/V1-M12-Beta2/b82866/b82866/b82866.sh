...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b82866/b82866 b82866.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b82866/b82866 b82866.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b82866.dasm.il b82866.exe
    "$CORE_ROOT/ildasm" -raweh -out=b82866.dasm.il b82866.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b82866.asm.exe  b82866.dasm.il
    "$CORE_ROOT/ilasm" -output=b82866.asm.exe  b82866.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b82866
    if [ ! -f b82866.org ]; then
        TakeLock
        if [ ! -f b82866.org ]; then
          mkdir IL
          cp b82866.exe IL/b82866.exe
          mv b82866.exe b82866.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b82866.org /out b82866.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b82866.org /out b82866.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b82866.reflect.xml
  if [ -f b82866.reflect.xml ]
then
    ReflectionRoots="-x b82866.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b82866.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b82866.asm.exe $CLRTestExecutionArguments
