...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b75944/b75944 b75944.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b75944/b75944 b75944.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b75944.dasm.il b75944.exe
    "$CORE_ROOT/ildasm" -raweh -out=b75944.dasm.il b75944.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b75944.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75944.dasm.il
    "$CORE_ROOT/ilasm" -output=b75944.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75944.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b75944
    if [ ! -f b75944.org ]; then
        TakeLock
        if [ ! -f b75944.org ]; then
          mkdir IL
          cp b75944.exe IL/b75944.exe
          mv b75944.exe b75944.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b75944.org /out b75944.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b75944.org /out b75944.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b75944.reflect.xml
  if [ -f b75944.reflect.xml ]
then
    ReflectionRoots="-x b75944.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b75944.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b75944.asm.exe $CLRTestExecutionArguments
