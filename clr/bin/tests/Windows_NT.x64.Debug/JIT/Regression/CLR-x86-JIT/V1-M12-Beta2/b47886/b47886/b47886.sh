...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b47886/b47886 b47886.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b47886/b47886 b47886.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b47886.dasm.il b47886.exe
    "$CORE_ROOT/ildasm" -raweh -out=b47886.dasm.il b47886.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b47886.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47886.dasm.il
    "$CORE_ROOT/ilasm" -output=b47886.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47886.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b47886
    if [ ! -f b47886.org ]; then
        TakeLock
        if [ ! -f b47886.org ]; then
          mkdir IL
          cp b47886.exe IL/b47886.exe
          mv b47886.exe b47886.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b47886.org /out b47886.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b47886.org /out b47886.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b47886.reflect.xml
  if [ -f b47886.reflect.xml ]
then
    ReflectionRoots="-x b47886.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b47886.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b47886.asm.exe $CLRTestExecutionArguments
