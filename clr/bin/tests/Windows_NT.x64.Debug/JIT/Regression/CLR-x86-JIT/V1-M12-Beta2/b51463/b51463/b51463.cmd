...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b51463\b51463 b51463.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b51463\b51463 b51463.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b51463.dasm.il b51463.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b51463.dasm.il b51463.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b51463.asm.exe  b51463.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b51463.asm.exe  b51463.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b51463
    if not exist "b51463.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b51463.org" (
            mkdir IL
            copy b51463.exe IL\b51463.exe 
            ren b51463.exe b51463.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b51463.org /out b51463.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b51463.org /out b51463.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b51463.reflect.xml
    IF EXIST b51463.reflect.xml set ReflectionRoots=-x b51463.reflect.xml
    ...
    set ExePath=%LinkBin%\b51463.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b51463.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b51463.asm.exe %CLRTestExecutionArguments% %Host_Args%
