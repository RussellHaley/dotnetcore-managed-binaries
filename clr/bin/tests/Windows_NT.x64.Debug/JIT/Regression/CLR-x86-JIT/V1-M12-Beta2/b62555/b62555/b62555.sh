...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b62555/b62555 b62555.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b62555/b62555 b62555.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b62555.dasm.il b62555.exe
    "$CORE_ROOT/ildasm" -raweh -out=b62555.dasm.il b62555.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b62555.asm.exe  b62555.dasm.il
    "$CORE_ROOT/ilasm" -output=b62555.asm.exe  b62555.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b62555
    if [ ! -f b62555.org ]; then
        TakeLock
        if [ ! -f b62555.org ]; then
          mkdir IL
          cp b62555.exe IL/b62555.exe
          mv b62555.exe b62555.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b62555.org /out b62555.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b62555.org /out b62555.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b62555.reflect.xml
  if [ -f b62555.reflect.xml ]
then
    ReflectionRoots="-x b62555.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b62555.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b62555.asm.exe $CLRTestExecutionArguments
