...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b72828\b72828 b72828.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b72828\b72828 b72828.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b72828.dasm.il b72828.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b72828.dasm.il b72828.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b72828.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b72828.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b72828.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b72828.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b72828
    if not exist "b72828.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b72828.org" (
            mkdir IL
            copy b72828.exe IL\b72828.exe 
            ren b72828.exe b72828.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b72828.org /out b72828.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b72828.org /out b72828.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b72828.reflect.xml
    IF EXIST b72828.reflect.xml set ReflectionRoots=-x b72828.reflect.xml
    ...
    set ExePath=%LinkBin%\b72828.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b72828.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b72828.asm.exe %CLRTestExecutionArguments% %Host_Args%
