...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31917/b31917 b31917.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31917/b31917 b31917.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31917.dasm.il b31917.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31917.dasm.il b31917.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31917.asm.exe  b31917.dasm.il
    "$CORE_ROOT/ilasm" -output=b31917.asm.exe  b31917.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31917
    if [ ! -f b31917.org ]; then
        TakeLock
        if [ ! -f b31917.org ]; then
          mkdir IL
          cp b31917.exe IL/b31917.exe
          mv b31917.exe b31917.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31917.org /out b31917.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31917.org /out b31917.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31917.reflect.xml
  if [ -f b31917.reflect.xml ]
then
    ReflectionRoots="-x b31917.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31917.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31917.asm.exe $CLRTestExecutionArguments
