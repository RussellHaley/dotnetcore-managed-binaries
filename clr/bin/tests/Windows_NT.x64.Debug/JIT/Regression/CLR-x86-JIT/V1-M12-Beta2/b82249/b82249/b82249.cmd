...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b82249\b82249 b82249.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b82249\b82249 b82249.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b82249.dasm.il b82249.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b82249.dasm.il b82249.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b82249.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b82249.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b82249.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b82249.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b82249
    if not exist "b82249.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b82249.org" (
            mkdir IL
            copy b82249.exe IL\b82249.exe 
            ren b82249.exe b82249.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b82249.org /out b82249.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b82249.org /out b82249.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b82249.reflect.xml
    IF EXIST b82249.reflect.xml set ReflectionRoots=-x b82249.reflect.xml
    ...
    set ExePath=%LinkBin%\b82249.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b82249.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b82249.asm.exe %CLRTestExecutionArguments% %Host_Args%
