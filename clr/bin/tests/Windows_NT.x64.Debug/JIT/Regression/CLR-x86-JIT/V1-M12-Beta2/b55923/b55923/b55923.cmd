...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b55923\b55923 b55923.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b55923\b55923 b55923.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b55923.dasm.il b55923.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b55923.dasm.il b55923.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b55923.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b55923.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b55923.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b55923.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b55923
    if not exist "b55923.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b55923.org" (
            mkdir IL
            copy b55923.exe IL\b55923.exe 
            ren b55923.exe b55923.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b55923.org /out b55923.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b55923.org /out b55923.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b55923.reflect.xml
    IF EXIST b55923.reflect.xml set ReflectionRoots=-x b55923.reflect.xml
    ...
    set ExePath=%LinkBin%\b55923.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b55923.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b55923.asm.exe %CLRTestExecutionArguments% %Host_Args%
