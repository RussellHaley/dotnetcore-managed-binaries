...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b70335\b70335 b70335.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b70335\b70335 b70335.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b70335.dasm.il b70335.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b70335.dasm.il b70335.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b70335.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70335.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b70335.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70335.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b70335
    if not exist "b70335.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b70335.org" (
            mkdir IL
            copy b70335.exe IL\b70335.exe 
            ren b70335.exe b70335.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b70335.org /out b70335.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b70335.org /out b70335.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b70335.reflect.xml
    IF EXIST b70335.reflect.xml set ReflectionRoots=-x b70335.reflect.xml
    ...
    set ExePath=%LinkBin%\b70335.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b70335.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b70335.asm.exe %CLRTestExecutionArguments% %Host_Args%
