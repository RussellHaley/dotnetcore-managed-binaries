...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b31547\b31547 b31547.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b31547\b31547 b31547.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b31547.dasm.il b31547.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b31547.dasm.il b31547.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b31547.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31547.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b31547.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31547.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b31547
    if not exist "b31547.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b31547.org" (
            mkdir IL
            copy b31547.exe IL\b31547.exe 
            ren b31547.exe b31547.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31547.org /out b31547.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31547.org /out b31547.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b31547.reflect.xml
    IF EXIST b31547.reflect.xml set ReflectionRoots=-x b31547.reflect.xml
    ...
    set ExePath=%LinkBin%\b31547.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b31547.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b31547.asm.exe %CLRTestExecutionArguments% %Host_Args%
