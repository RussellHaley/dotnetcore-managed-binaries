...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b83690\b83690 b83690.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b83690\b83690 b83690.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b83690.dasm.il b83690.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b83690.dasm.il b83690.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b83690.asm.exe  b83690.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b83690.asm.exe  b83690.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b83690
    if not exist "b83690.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b83690.org" (
            mkdir IL
            copy b83690.exe IL\b83690.exe 
            ren b83690.exe b83690.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b83690.org /out b83690.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b83690.org /out b83690.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b83690.reflect.xml
    IF EXIST b83690.reflect.xml set ReflectionRoots=-x b83690.reflect.xml
    ...
    set ExePath=%LinkBin%\b83690.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b83690.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b83690.asm.exe %CLRTestExecutionArguments% %Host_Args%
