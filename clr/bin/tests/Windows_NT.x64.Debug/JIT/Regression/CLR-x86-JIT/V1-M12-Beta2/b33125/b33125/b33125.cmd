...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b33125\b33125 b33125.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b33125\b33125 b33125.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b33125.dasm.il b33125.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b33125.dasm.il b33125.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b33125.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33125.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b33125.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33125.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b33125
    if not exist "b33125.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b33125.org" (
            mkdir IL
            copy b33125.exe IL\b33125.exe 
            ren b33125.exe b33125.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b33125.org /out b33125.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b33125.org /out b33125.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b33125.reflect.xml
    IF EXIST b33125.reflect.xml set ReflectionRoots=-x b33125.reflect.xml
    ...
    set ExePath=%LinkBin%\b33125.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b33125.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b33125.asm.exe %CLRTestExecutionArguments% %Host_Args%
