...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b36304/b36304 b36304.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b36304/b36304 b36304.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b36304.dasm.il b36304.exe
    "$CORE_ROOT/ildasm" -raweh -out=b36304.dasm.il b36304.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b36304.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b36304.dasm.il
    "$CORE_ROOT/ilasm" -output=b36304.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b36304.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b36304
    if [ ! -f b36304.org ]; then
        TakeLock
        if [ ! -f b36304.org ]; then
          mkdir IL
          cp b36304.exe IL/b36304.exe
          mv b36304.exe b36304.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b36304.org /out b36304.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b36304.org /out b36304.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b36304.reflect.xml
  if [ -f b36304.reflect.xml ]
then
    ReflectionRoots="-x b36304.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b36304.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b36304.asm.exe $CLRTestExecutionArguments
