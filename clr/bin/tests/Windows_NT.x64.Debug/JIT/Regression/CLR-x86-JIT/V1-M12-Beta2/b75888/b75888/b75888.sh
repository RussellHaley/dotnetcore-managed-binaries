...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b75888/b75888 b75888.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b75888/b75888 b75888.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b75888.dasm.il b75888.exe
    "$CORE_ROOT/ildasm" -raweh -out=b75888.dasm.il b75888.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b75888.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75888.dasm.il
    "$CORE_ROOT/ilasm" -output=b75888.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75888.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b75888
    if [ ! -f b75888.org ]; then
        TakeLock
        if [ ! -f b75888.org ]; then
          mkdir IL
          cp b75888.exe IL/b75888.exe
          mv b75888.exe b75888.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b75888.org /out b75888.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b75888.org /out b75888.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b75888.reflect.xml
  if [ -f b75888.reflect.xml ]
then
    ReflectionRoots="-x b75888.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b75888.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b75888.asm.exe $CLRTestExecutionArguments
