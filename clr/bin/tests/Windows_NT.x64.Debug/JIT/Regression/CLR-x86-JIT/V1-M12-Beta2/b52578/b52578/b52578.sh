...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b52578/b52578 b52578.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b52578/b52578 b52578.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b52578.dasm.il b52578.exe
    "$CORE_ROOT/ildasm" -raweh -out=b52578.dasm.il b52578.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b52578.asm.exe  b52578.dasm.il
    "$CORE_ROOT/ilasm" -output=b52578.asm.exe  b52578.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b52578
    if [ ! -f b52578.org ]; then
        TakeLock
        if [ ! -f b52578.org ]; then
          mkdir IL
          cp b52578.exe IL/b52578.exe
          mv b52578.exe b52578.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b52578.org /out b52578.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b52578.org /out b52578.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b52578.reflect.xml
  if [ -f b52578.reflect.xml ]
then
    ReflectionRoots="-x b52578.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b52578.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b52578.asm.exe $CLRTestExecutionArguments
