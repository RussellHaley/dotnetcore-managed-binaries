...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b83690/b83690 b83690.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b83690/b83690 b83690.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b83690.dasm.il b83690.exe
    "$CORE_ROOT/ildasm" -raweh -out=b83690.dasm.il b83690.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b83690.asm.exe  b83690.dasm.il
    "$CORE_ROOT/ilasm" -output=b83690.asm.exe  b83690.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b83690
    if [ ! -f b83690.org ]; then
        TakeLock
        if [ ! -f b83690.org ]; then
          mkdir IL
          cp b83690.exe IL/b83690.exe
          mv b83690.exe b83690.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b83690.org /out b83690.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b83690.org /out b83690.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b83690.reflect.xml
  if [ -f b83690.reflect.xml ]
then
    ReflectionRoots="-x b83690.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b83690.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b83690.asm.exe $CLRTestExecutionArguments
