...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b82160/b82160 b82160.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b82160/b82160 b82160.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b82160.dasm.il b82160.exe
    "$CORE_ROOT/ildasm" -raweh -out=b82160.dasm.il b82160.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b82160.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b82160.dasm.il
    "$CORE_ROOT/ilasm" -output=b82160.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b82160.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b82160
    if [ ! -f b82160.org ]; then
        TakeLock
        if [ ! -f b82160.org ]; then
          mkdir IL
          cp b82160.exe IL/b82160.exe
          mv b82160.exe b82160.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b82160.org /out b82160.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b82160.org /out b82160.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b82160.reflect.xml
  if [ -f b82160.reflect.xml ]
then
    ReflectionRoots="-x b82160.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b82160.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b82160.asm.exe $CLRTestExecutionArguments
