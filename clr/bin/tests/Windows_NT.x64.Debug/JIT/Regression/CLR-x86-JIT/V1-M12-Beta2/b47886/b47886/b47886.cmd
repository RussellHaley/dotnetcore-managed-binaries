...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b47886\b47886 b47886.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b47886\b47886 b47886.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b47886.dasm.il b47886.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b47886.dasm.il b47886.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b47886.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47886.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b47886.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47886.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b47886
    if not exist "b47886.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b47886.org" (
            mkdir IL
            copy b47886.exe IL\b47886.exe 
            ren b47886.exe b47886.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47886.org /out b47886.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47886.org /out b47886.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b47886.reflect.xml
    IF EXIST b47886.reflect.xml set ReflectionRoots=-x b47886.reflect.xml
    ...
    set ExePath=%LinkBin%\b47886.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b47886.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b47886.asm.exe %CLRTestExecutionArguments% %Host_Args%
