...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b59952\b59952 b59952.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b59952\b59952 b59952.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b59952.dasm.il b59952.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b59952.dasm.il b59952.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b59952.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59952.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b59952.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59952.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b59952
    if not exist "b59952.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b59952.org" (
            mkdir IL
            copy b59952.exe IL\b59952.exe 
            ren b59952.exe b59952.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b59952.org /out b59952.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b59952.org /out b59952.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b59952.reflect.xml
    IF EXIST b59952.reflect.xml set ReflectionRoots=-x b59952.reflect.xml
    ...
    set ExePath=%LinkBin%\b59952.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b59952.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b59952.asm.exe %CLRTestExecutionArguments% %Host_Args%
