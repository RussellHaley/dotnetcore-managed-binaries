...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71120/b71120 b71120.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71120/b71120 b71120.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b71120.dasm.il b71120.exe
    "$CORE_ROOT/ildasm" -raweh -out=b71120.dasm.il b71120.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b71120.asm.exe  b71120.dasm.il
    "$CORE_ROOT/ilasm" -output=b71120.asm.exe  b71120.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b71120
    if [ ! -f b71120.org ]; then
        TakeLock
        if [ ! -f b71120.org ]; then
          mkdir IL
          cp b71120.exe IL/b71120.exe
          mv b71120.exe b71120.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71120.org /out b71120.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71120.org /out b71120.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b71120.reflect.xml
  if [ -f b71120.reflect.xml ]
then
    ReflectionRoots="-x b71120.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b71120.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b71120.asm.exe $CLRTestExecutionArguments
