...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b73207\b73207 b73207.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b73207\b73207 b73207.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b73207.dasm.il b73207.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b73207.dasm.il b73207.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b73207.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b73207.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b73207.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b73207.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b73207
    if not exist "b73207.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b73207.org" (
            mkdir IL
            copy b73207.exe IL\b73207.exe 
            ren b73207.exe b73207.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b73207.org /out b73207.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b73207.org /out b73207.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b73207.reflect.xml
    IF EXIST b73207.reflect.xml set ReflectionRoots=-x b73207.reflect.xml
    ...
    set ExePath=%LinkBin%\b73207.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b73207.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b73207.asm.exe %CLRTestExecutionArguments% %Host_Args%
