...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b51565/b51565 b51565.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b51565/b51565 b51565.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b51565.dasm.il b51565.exe
    "$CORE_ROOT/ildasm" -raweh -out=b51565.dasm.il b51565.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b51565.asm.exe  b51565.dasm.il
    "$CORE_ROOT/ilasm" -output=b51565.asm.exe  b51565.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b51565
    if [ ! -f b51565.org ]; then
        TakeLock
        if [ ! -f b51565.org ]; then
          mkdir IL
          cp b51565.exe IL/b51565.exe
          mv b51565.exe b51565.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b51565.org /out b51565.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b51565.org /out b51565.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b51565.reflect.xml
  if [ -f b51565.reflect.xml ]
then
    ReflectionRoots="-x b51565.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b51565.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b51565.asm.exe $CLRTestExecutionArguments
