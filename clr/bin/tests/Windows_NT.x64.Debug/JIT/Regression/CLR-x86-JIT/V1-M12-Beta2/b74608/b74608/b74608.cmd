...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b74608\b74608 b74608.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b74608\b74608 b74608.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b74608.dasm.il b74608.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b74608.dasm.il b74608.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b74608.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b74608.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b74608.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b74608.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b74608
    if not exist "b74608.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b74608.org" (
            mkdir IL
            copy b74608.exe IL\b74608.exe 
            ren b74608.exe b74608.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b74608.org /out b74608.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b74608.org /out b74608.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b74608.reflect.xml
    IF EXIST b74608.reflect.xml set ReflectionRoots=-x b74608.reflect.xml
    ...
    set ExePath=%LinkBin%\b74608.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b74608.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b74608.asm.exe %CLRTestExecutionArguments% %Host_Args%
