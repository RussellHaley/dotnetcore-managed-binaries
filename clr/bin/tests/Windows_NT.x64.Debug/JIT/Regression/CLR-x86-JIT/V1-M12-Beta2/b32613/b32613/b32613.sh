...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b32613/b32613 b32613.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b32613/b32613 b32613.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b32613.dasm.il b32613.exe
    "$CORE_ROOT/ildasm" -raweh -out=b32613.dasm.il b32613.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b32613.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32613.dasm.il
    "$CORE_ROOT/ilasm" -output=b32613.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32613.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b32613
    if [ ! -f b32613.org ]; then
        TakeLock
        if [ ! -f b32613.org ]; then
          mkdir IL
          cp b32613.exe IL/b32613.exe
          mv b32613.exe b32613.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32613.org /out b32613.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32613.org /out b32613.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b32613.reflect.xml
  if [ -f b32613.reflect.xml ]
then
    ReflectionRoots="-x b32613.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b32613.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b32613.asm.exe $CLRTestExecutionArguments
