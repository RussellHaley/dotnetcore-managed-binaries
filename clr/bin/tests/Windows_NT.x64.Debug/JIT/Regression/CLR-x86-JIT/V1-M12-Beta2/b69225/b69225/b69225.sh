...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b69225/b69225 b69225.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b69225/b69225 b69225.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b69225.dasm.il b69225.exe
    "$CORE_ROOT/ildasm" -raweh -out=b69225.dasm.il b69225.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b69225.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69225.dasm.il
    "$CORE_ROOT/ilasm" -output=b69225.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69225.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b69225
    if [ ! -f b69225.org ]; then
        TakeLock
        if [ ! -f b69225.org ]; then
          mkdir IL
          cp b69225.exe IL/b69225.exe
          mv b69225.exe b69225.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b69225.org /out b69225.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b69225.org /out b69225.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b69225.reflect.xml
  if [ -f b69225.reflect.xml ]
then
    ReflectionRoots="-x b69225.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b69225.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b69225.asm.exe $CLRTestExecutionArguments
