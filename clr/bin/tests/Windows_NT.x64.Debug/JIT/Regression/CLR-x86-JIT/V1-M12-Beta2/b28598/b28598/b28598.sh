...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b28598/b28598 b28598.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b28598/b28598 b28598.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b28598.dasm.il b28598.exe
    "$CORE_ROOT/ildasm" -raweh -out=b28598.dasm.il b28598.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b28598.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28598.dasm.il
    "$CORE_ROOT/ilasm" -output=b28598.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28598.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b28598
    if [ ! -f b28598.org ]; then
        TakeLock
        if [ ! -f b28598.org ]; then
          mkdir IL
          cp b28598.exe IL/b28598.exe
          mv b28598.exe b28598.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28598.org /out b28598.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28598.org /out b28598.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b28598.reflect.xml
  if [ -f b28598.reflect.xml ]
then
    ReflectionRoots="-x b28598.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b28598.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b28598.asm.exe $CLRTestExecutionArguments
