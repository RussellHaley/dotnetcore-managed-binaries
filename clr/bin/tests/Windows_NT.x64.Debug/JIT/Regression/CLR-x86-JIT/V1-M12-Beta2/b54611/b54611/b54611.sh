...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b54611/b54611 b54611.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b54611/b54611 b54611.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b54611.dasm.il b54611.exe
    "$CORE_ROOT/ildasm" -raweh -out=b54611.dasm.il b54611.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b54611.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54611.dasm.il
    "$CORE_ROOT/ilasm" -output=b54611.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54611.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b54611
    if [ ! -f b54611.org ]; then
        TakeLock
        if [ ! -f b54611.org ]; then
          mkdir IL
          cp b54611.exe IL/b54611.exe
          mv b54611.exe b54611.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b54611.org /out b54611.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b54611.org /out b54611.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b54611.reflect.xml
  if [ -f b54611.reflect.xml ]
then
    ReflectionRoots="-x b54611.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b54611.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b54611.asm.exe $CLRTestExecutionArguments
