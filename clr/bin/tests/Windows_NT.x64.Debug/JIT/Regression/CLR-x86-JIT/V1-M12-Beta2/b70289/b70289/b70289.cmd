...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b70289\b70289 b70289.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b70289\b70289 b70289.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b70289.dasm.il b70289.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b70289.dasm.il b70289.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b70289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70289.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b70289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70289.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b70289
    if not exist "b70289.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b70289.org" (
            mkdir IL
            copy b70289.exe IL\b70289.exe 
            ren b70289.exe b70289.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b70289.org /out b70289.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b70289.org /out b70289.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b70289.reflect.xml
    IF EXIST b70289.reflect.xml set ReflectionRoots=-x b70289.reflect.xml
    ...
    set ExePath=%LinkBin%\b70289.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b70289.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b70289.asm.exe %CLRTestExecutionArguments% %Host_Args%
