...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b28598\b28598 b28598.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b28598\b28598 b28598.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b28598.dasm.il b28598.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b28598.dasm.il b28598.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b28598.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28598.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b28598.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28598.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b28598
    if not exist "b28598.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b28598.org" (
            mkdir IL
            copy b28598.exe IL\b28598.exe 
            ren b28598.exe b28598.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b28598.org /out b28598.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b28598.org /out b28598.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b28598.reflect.xml
    IF EXIST b28598.reflect.xml set ReflectionRoots=-x b28598.reflect.xml
    ...
    set ExePath=%LinkBin%\b28598.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b28598.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b28598.asm.exe %CLRTestExecutionArguments% %Host_Args%
