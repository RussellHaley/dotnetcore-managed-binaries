...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71869/b71869 b71869.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71869/b71869 b71869.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b71869.dasm.il b71869.exe
    "$CORE_ROOT/ildasm" -raweh -out=b71869.dasm.il b71869.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b71869.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b71869.dasm.il
    "$CORE_ROOT/ilasm" -output=b71869.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b71869.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b71869
    if [ ! -f b71869.org ]; then
        TakeLock
        if [ ! -f b71869.org ]; then
          mkdir IL
          cp b71869.exe IL/b71869.exe
          mv b71869.exe b71869.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71869.org /out b71869.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71869.org /out b71869.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b71869.reflect.xml
  if [ -f b71869.reflect.xml ]
then
    ReflectionRoots="-x b71869.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b71869.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b71869.asm.exe $CLRTestExecutionArguments
