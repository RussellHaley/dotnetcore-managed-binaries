...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b75888\b75888 b75888.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b75888\b75888 b75888.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b75888.dasm.il b75888.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b75888.dasm.il b75888.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b75888.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75888.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b75888.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75888.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b75888
    if not exist "b75888.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b75888.org" (
            mkdir IL
            copy b75888.exe IL\b75888.exe 
            ren b75888.exe b75888.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b75888.org /out b75888.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b75888.org /out b75888.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b75888.reflect.xml
    IF EXIST b75888.reflect.xml set ReflectionRoots=-x b75888.reflect.xml
    ...
    set ExePath=%LinkBin%\b75888.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b75888.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b75888.asm.exe %CLRTestExecutionArguments% %Host_Args%
