...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b70289/b70289 b70289.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b70289/b70289 b70289.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b70289.dasm.il b70289.exe
    "$CORE_ROOT/ildasm" -raweh -out=b70289.dasm.il b70289.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b70289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70289.dasm.il
    "$CORE_ROOT/ilasm" -output=b70289.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70289.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b70289
    if [ ! -f b70289.org ]; then
        TakeLock
        if [ ! -f b70289.org ]; then
          mkdir IL
          cp b70289.exe IL/b70289.exe
          mv b70289.exe b70289.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b70289.org /out b70289.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b70289.org /out b70289.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b70289.reflect.xml
  if [ -f b70289.reflect.xml ]
then
    ReflectionRoots="-x b70289.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b70289.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b70289.asm.exe $CLRTestExecutionArguments
