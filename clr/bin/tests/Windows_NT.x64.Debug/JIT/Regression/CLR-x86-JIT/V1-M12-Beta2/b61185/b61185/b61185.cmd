...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b61185\b61185 b61185.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b61185\b61185 b61185.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b61185.dasm.il b61185.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b61185.dasm.il b61185.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b61185.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61185.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b61185.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61185.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b61185
    if not exist "b61185.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b61185.org" (
            mkdir IL
            copy b61185.exe IL\b61185.exe 
            ren b61185.exe b61185.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b61185.org /out b61185.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b61185.org /out b61185.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b61185.reflect.xml
    IF EXIST b61185.reflect.xml set ReflectionRoots=-x b61185.reflect.xml
    ...
    set ExePath=%LinkBin%\b61185.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b61185.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b61185.asm.exe %CLRTestExecutionArguments% %Host_Args%
