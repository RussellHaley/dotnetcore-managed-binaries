...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b67819\b67819 b67819.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b67819\b67819 b67819.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b67819.dasm.il b67819.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b67819.dasm.il b67819.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b67819.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b67819.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b67819.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b67819.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b67819
    if not exist "b67819.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b67819.org" (
            mkdir IL
            copy b67819.exe IL\b67819.exe 
            ren b67819.exe b67819.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b67819.org /out b67819.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b67819.org /out b67819.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b67819.reflect.xml
    IF EXIST b67819.reflect.xml set ReflectionRoots=-x b67819.reflect.xml
    ...
    set ExePath=%LinkBin%\b67819.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b67819.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b67819.asm.exe %CLRTestExecutionArguments% %Host_Args%
