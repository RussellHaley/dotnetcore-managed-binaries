...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b61025\b61025 b61025.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b61025\b61025 b61025.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b61025.dasm.il b61025.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b61025.dasm.il b61025.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b61025.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61025.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b61025.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61025.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b61025
    if not exist "b61025.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b61025.org" (
            mkdir IL
            copy b61025.exe IL\b61025.exe 
            ren b61025.exe b61025.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b61025.org /out b61025.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b61025.org /out b61025.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b61025.reflect.xml
    IF EXIST b61025.reflect.xml set ReflectionRoots=-x b61025.reflect.xml
    ...
    set ExePath=%LinkBin%\b61025.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b61025.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b61025.asm.exe %CLRTestExecutionArguments% %Host_Args%
