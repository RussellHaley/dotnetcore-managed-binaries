...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b84909/b84909 b84909.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b84909/b84909 b84909.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b84909.dasm.il b84909.exe
    "$CORE_ROOT/ildasm" -raweh -out=b84909.dasm.il b84909.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b84909.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84909.dasm.il
    "$CORE_ROOT/ilasm" -output=b84909.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84909.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b84909
    if [ ! -f b84909.org ]; then
        TakeLock
        if [ ! -f b84909.org ]; then
          mkdir IL
          cp b84909.exe IL/b84909.exe
          mv b84909.exe b84909.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b84909.org /out b84909.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b84909.org /out b84909.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b84909.reflect.xml
  if [ -f b84909.reflect.xml ]
then
    ReflectionRoots="-x b84909.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b84909.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b84909.asm.exe $CLRTestExecutionArguments
