...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b59822/b59822 b59822.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b59822/b59822 b59822.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b59822.dasm.il b59822.exe
    "$CORE_ROOT/ildasm" -raweh -out=b59822.dasm.il b59822.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b59822.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59822.dasm.il
    "$CORE_ROOT/ilasm" -output=b59822.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59822.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b59822
    if [ ! -f b59822.org ]; then
        TakeLock
        if [ ! -f b59822.org ]; then
          mkdir IL
          cp b59822.exe IL/b59822.exe
          mv b59822.exe b59822.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b59822.org /out b59822.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b59822.org /out b59822.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b59822.reflect.xml
  if [ -f b59822.reflect.xml ]
then
    ReflectionRoots="-x b59822.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b59822.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b59822.asm.exe $CLRTestExecutionArguments
