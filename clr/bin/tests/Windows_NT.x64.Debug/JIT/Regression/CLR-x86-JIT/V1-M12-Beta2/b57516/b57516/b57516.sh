...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b57516/b57516 b57516.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b57516/b57516 b57516.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b57516.dasm.il b57516.exe
    "$CORE_ROOT/ildasm" -raweh -out=b57516.dasm.il b57516.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b57516.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57516.dasm.il
    "$CORE_ROOT/ilasm" -output=b57516.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57516.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b57516
    if [ ! -f b57516.org ]; then
        TakeLock
        if [ ! -f b57516.org ]; then
          mkdir IL
          cp b57516.exe IL/b57516.exe
          mv b57516.exe b57516.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b57516.org /out b57516.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b57516.org /out b57516.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b57516.reflect.xml
  if [ -f b57516.reflect.xml ]
then
    ReflectionRoots="-x b57516.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b57516.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b57516.asm.exe $CLRTestExecutionArguments
