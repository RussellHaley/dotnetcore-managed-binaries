...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b68872/b68872 b68872.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b68872/b68872 b68872.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b68872.dasm.il b68872.exe
    "$CORE_ROOT/ildasm" -raweh -out=b68872.dasm.il b68872.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b68872.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b68872.dasm.il
    "$CORE_ROOT/ilasm" -output=b68872.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b68872.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b68872
    if [ ! -f b68872.org ]; then
        TakeLock
        if [ ! -f b68872.org ]; then
          mkdir IL
          cp b68872.exe IL/b68872.exe
          mv b68872.exe b68872.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b68872.org /out b68872.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b68872.org /out b68872.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b68872.reflect.xml
  if [ -f b68872.reflect.xml ]
then
    ReflectionRoots="-x b68872.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b68872.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b68872.asm.exe $CLRTestExecutionArguments
