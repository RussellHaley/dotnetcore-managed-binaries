...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b82247/b82247 b82247.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b82247/b82247 b82247.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b82247.dasm.il b82247.exe
    "$CORE_ROOT/ildasm" -raweh -out=b82247.dasm.il b82247.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b82247.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b82247.dasm.il
    "$CORE_ROOT/ilasm" -output=b82247.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b82247.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b82247
    if [ ! -f b82247.org ]; then
        TakeLock
        if [ ! -f b82247.org ]; then
          mkdir IL
          cp b82247.exe IL/b82247.exe
          mv b82247.exe b82247.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b82247.org /out b82247.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b82247.org /out b82247.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b82247.reflect.xml
  if [ -f b82247.reflect.xml ]
then
    ReflectionRoots="-x b82247.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b82247.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b82247.asm.exe $CLRTestExecutionArguments
