...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b37238/b37238 b37238.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b37238/b37238 b37238.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b37238.dasm.il b37238.exe
    "$CORE_ROOT/ildasm" -raweh -out=b37238.dasm.il b37238.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b37238.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37238.dasm.il
    "$CORE_ROOT/ilasm" -output=b37238.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37238.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b37238
    if [ ! -f b37238.org ]; then
        TakeLock
        if [ ! -f b37238.org ]; then
          mkdir IL
          cp b37238.exe IL/b37238.exe
          mv b37238.exe b37238.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b37238.org /out b37238.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b37238.org /out b37238.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b37238.reflect.xml
  if [ -f b37238.reflect.xml ]
then
    ReflectionRoots="-x b37238.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b37238.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b37238.asm.exe $CLRTestExecutionArguments
