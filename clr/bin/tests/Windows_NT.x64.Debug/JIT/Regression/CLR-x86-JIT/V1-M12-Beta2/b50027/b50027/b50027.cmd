...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b50027\b50027 b50027.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b50027\b50027 b50027.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b50027.dasm.il b50027.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b50027.dasm.il b50027.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b50027.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b50027.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b50027.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b50027.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b50027
    if not exist "b50027.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b50027.org" (
            mkdir IL
            copy b50027.exe IL\b50027.exe 
            ren b50027.exe b50027.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b50027.org /out b50027.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b50027.org /out b50027.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b50027.reflect.xml
    IF EXIST b50027.reflect.xml set ReflectionRoots=-x b50027.reflect.xml
    ...
    set ExePath=%LinkBin%\b50027.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b50027.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b50027.asm.exe %CLRTestExecutionArguments% %Host_Args%
