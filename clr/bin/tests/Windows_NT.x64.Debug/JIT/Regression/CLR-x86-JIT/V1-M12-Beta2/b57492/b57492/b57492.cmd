...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b57492\b57492 b57492.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b57492\b57492 b57492.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b57492.dasm.il b57492.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b57492.dasm.il b57492.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b57492.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57492.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b57492.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57492.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b57492
    if not exist "b57492.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b57492.org" (
            mkdir IL
            copy b57492.exe IL\b57492.exe 
            ren b57492.exe b57492.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b57492.org /out b57492.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b57492.org /out b57492.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b57492.reflect.xml
    IF EXIST b57492.reflect.xml set ReflectionRoots=-x b57492.reflect.xml
    ...
    set ExePath=%LinkBin%\b57492.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b57492.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b57492.asm.exe %CLRTestExecutionArguments% %Host_Args%
