...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b37214\b37214 b37214.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b37214\b37214 b37214.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b37214.dasm.il b37214.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b37214.dasm.il b37214.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b37214.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37214.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b37214.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37214.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b37214
    if not exist "b37214.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b37214.org" (
            mkdir IL
            copy b37214.exe IL\b37214.exe 
            ren b37214.exe b37214.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37214.org /out b37214.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37214.org /out b37214.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b37214.reflect.xml
    IF EXIST b37214.reflect.xml set ReflectionRoots=-x b37214.reflect.xml
    ...
    set ExePath=%LinkBin%\b37214.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b37214.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b37214.asm.exe %CLRTestExecutionArguments% %Host_Args%
