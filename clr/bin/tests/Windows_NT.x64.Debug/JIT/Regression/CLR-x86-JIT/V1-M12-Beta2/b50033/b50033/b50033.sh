...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b50033/b50033 b50033.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b50033/b50033 b50033.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b50033.dasm.il b50033.exe
    "$CORE_ROOT/ildasm" -raweh -out=b50033.dasm.il b50033.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b50033.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b50033.dasm.il
    "$CORE_ROOT/ilasm" -output=b50033.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b50033.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b50033
    if [ ! -f b50033.org ]; then
        TakeLock
        if [ ! -f b50033.org ]; then
          mkdir IL
          cp b50033.exe IL/b50033.exe
          mv b50033.exe b50033.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b50033.org /out b50033.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b50033.org /out b50033.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b50033.reflect.xml
  if [ -f b50033.reflect.xml ]
then
    ReflectionRoots="-x b50033.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b50033.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b50033.asm.exe $CLRTestExecutionArguments
