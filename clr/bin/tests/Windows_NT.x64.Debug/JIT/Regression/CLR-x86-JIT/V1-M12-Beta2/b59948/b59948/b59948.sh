...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b59948/b59948 b59948.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b59948/b59948 b59948.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b59948.dasm.il b59948.exe
    "$CORE_ROOT/ildasm" -raweh -out=b59948.dasm.il b59948.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b59948.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59948.dasm.il
    "$CORE_ROOT/ilasm" -output=b59948.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59948.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b59948
    if [ ! -f b59948.org ]; then
        TakeLock
        if [ ! -f b59948.org ]; then
          mkdir IL
          cp b59948.exe IL/b59948.exe
          mv b59948.exe b59948.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b59948.org /out b59948.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b59948.org /out b59948.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b59948.reflect.xml
  if [ -f b59948.reflect.xml ]
then
    ReflectionRoots="-x b59948.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b59948.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b59948.asm.exe $CLRTestExecutionArguments
