...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b63183/b63183 b63183.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b63183/b63183 b63183.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b63183.dasm.il b63183.exe
    "$CORE_ROOT/ildasm" -raweh -out=b63183.dasm.il b63183.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b63183.asm.exe  b63183.dasm.il
    "$CORE_ROOT/ilasm" -output=b63183.asm.exe  b63183.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b63183
    if [ ! -f b63183.org ]; then
        TakeLock
        if [ ! -f b63183.org ]; then
          mkdir IL
          cp b63183.exe IL/b63183.exe
          mv b63183.exe b63183.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b63183.org /out b63183.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b63183.org /out b63183.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b63183.reflect.xml
  if [ -f b63183.reflect.xml ]
then
    ReflectionRoots="-x b63183.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b63183.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b63183.asm.exe $CLRTestExecutionArguments
