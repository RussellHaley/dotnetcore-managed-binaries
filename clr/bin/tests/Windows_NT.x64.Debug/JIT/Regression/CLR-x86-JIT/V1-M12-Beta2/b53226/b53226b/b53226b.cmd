...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b53226\b53226b b53226b.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b53226\b53226b b53226b.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b53226b.dasm.il b53226b.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b53226b.dasm.il b53226b.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b53226b.asm.exe  b53226b.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b53226b.asm.exe  b53226b.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b53226b
    if not exist "b53226b.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b53226b.org" (
            mkdir IL
            copy b53226b.exe IL\b53226b.exe 
            ren b53226b.exe b53226b.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b53226b.org /out b53226b.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b53226b.org /out b53226b.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b53226b.reflect.xml
    IF EXIST b53226b.reflect.xml set ReflectionRoots=-x b53226b.reflect.xml
    ...
    set ExePath=%LinkBin%\b53226b.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b53226b.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b53226b.asm.exe %CLRTestExecutionArguments% %Host_Args%
