...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b67987/b67987 b67987.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b67987/b67987 b67987.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b67987.dasm.il b67987.exe
    "$CORE_ROOT/ildasm" -raweh -out=b67987.dasm.il b67987.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b67987.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b67987.dasm.il
    "$CORE_ROOT/ilasm" -output=b67987.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b67987.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b67987
    if [ ! -f b67987.org ]; then
        TakeLock
        if [ ! -f b67987.org ]; then
          mkdir IL
          cp b67987.exe IL/b67987.exe
          mv b67987.exe b67987.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b67987.org /out b67987.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b67987.org /out b67987.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b67987.reflect.xml
  if [ -f b67987.reflect.xml ]
then
    ReflectionRoots="-x b67987.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b67987.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b67987.asm.exe $CLRTestExecutionArguments
