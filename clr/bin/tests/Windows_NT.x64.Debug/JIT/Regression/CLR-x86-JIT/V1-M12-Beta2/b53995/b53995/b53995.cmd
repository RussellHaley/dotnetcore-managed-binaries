...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b53995\b53995 b53995.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b53995\b53995 b53995.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b53995.dasm.il b53995.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b53995.dasm.il b53995.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b53995.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53995.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b53995.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53995.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b53995
    if not exist "b53995.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b53995.org" (
            mkdir IL
            copy b53995.exe IL\b53995.exe 
            ren b53995.exe b53995.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b53995.org /out b53995.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b53995.org /out b53995.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b53995.reflect.xml
    IF EXIST b53995.reflect.xml set ReflectionRoots=-x b53995.reflect.xml
    ...
    set ExePath=%LinkBin%\b53995.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b53995.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b53995.asm.exe %CLRTestExecutionArguments% %Host_Args%
