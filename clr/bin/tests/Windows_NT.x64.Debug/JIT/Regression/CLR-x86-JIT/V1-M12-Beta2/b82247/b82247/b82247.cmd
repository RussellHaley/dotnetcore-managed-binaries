...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b82247\b82247 b82247.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b82247\b82247 b82247.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b82247.dasm.il b82247.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b82247.dasm.il b82247.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b82247.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b82247.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b82247.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b82247.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b82247
    if not exist "b82247.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b82247.org" (
            mkdir IL
            copy b82247.exe IL\b82247.exe 
            ren b82247.exe b82247.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b82247.org /out b82247.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b82247.org /out b82247.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b82247.reflect.xml
    IF EXIST b82247.reflect.xml set ReflectionRoots=-x b82247.reflect.xml
    ...
    set ExePath=%LinkBin%\b82247.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b82247.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b82247.asm.exe %CLRTestExecutionArguments% %Host_Args%
