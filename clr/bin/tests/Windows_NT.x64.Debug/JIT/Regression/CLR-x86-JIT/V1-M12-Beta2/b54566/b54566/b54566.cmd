...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b54566\b54566 b54566.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b54566\b54566 b54566.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b54566.dasm.il b54566.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b54566.dasm.il b54566.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b54566.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54566.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b54566.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54566.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b54566
    if not exist "b54566.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b54566.org" (
            mkdir IL
            copy b54566.exe IL\b54566.exe 
            ren b54566.exe b54566.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b54566.org /out b54566.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b54566.org /out b54566.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b54566.reflect.xml
    IF EXIST b54566.reflect.xml set ReflectionRoots=-x b54566.reflect.xml
    ...
    set ExePath=%LinkBin%\b54566.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b54566.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b54566.asm.exe %CLRTestExecutionArguments% %Host_Args%
