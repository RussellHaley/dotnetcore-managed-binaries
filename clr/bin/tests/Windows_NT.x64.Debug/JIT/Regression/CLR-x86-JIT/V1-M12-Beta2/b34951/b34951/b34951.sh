...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b34951/b34951 b34951.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b34951/b34951 b34951.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b34951.dasm.il b34951.exe
    "$CORE_ROOT/ildasm" -raweh -out=b34951.dasm.il b34951.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b34951.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b34951.dasm.il
    "$CORE_ROOT/ilasm" -output=b34951.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b34951.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b34951
    if [ ! -f b34951.org ]; then
        TakeLock
        if [ ! -f b34951.org ]; then
          mkdir IL
          cp b34951.exe IL/b34951.exe
          mv b34951.exe b34951.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b34951.org /out b34951.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b34951.org /out b34951.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b34951.reflect.xml
  if [ -f b34951.reflect.xml ]
then
    ReflectionRoots="-x b34951.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b34951.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b34951.asm.exe $CLRTestExecutionArguments
