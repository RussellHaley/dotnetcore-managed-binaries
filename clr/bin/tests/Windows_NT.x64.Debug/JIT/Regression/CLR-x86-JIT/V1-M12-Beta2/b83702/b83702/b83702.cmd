...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b83702\b83702 b83702.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b83702\b83702 b83702.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b83702.dasm.il b83702.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b83702.dasm.il b83702.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b83702.asm.exe  b83702.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b83702.asm.exe  b83702.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b83702
    if not exist "b83702.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b83702.org" (
            mkdir IL
            copy b83702.exe IL\b83702.exe 
            ren b83702.exe b83702.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b83702.org /out b83702.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b83702.org /out b83702.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b83702.reflect.xml
    IF EXIST b83702.reflect.xml set ReflectionRoots=-x b83702.reflect.xml
    ...
    set ExePath=%LinkBin%\b83702.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b83702.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b83702.asm.exe %CLRTestExecutionArguments% %Host_Args%
