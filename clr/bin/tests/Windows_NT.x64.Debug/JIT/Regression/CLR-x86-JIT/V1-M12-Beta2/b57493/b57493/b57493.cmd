...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b57493\b57493 b57493.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b57493\b57493 b57493.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b57493.dasm.il b57493.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b57493.dasm.il b57493.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b57493.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57493.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b57493.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57493.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b57493
    if not exist "b57493.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b57493.org" (
            mkdir IL
            copy b57493.exe IL\b57493.exe 
            ren b57493.exe b57493.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b57493.org /out b57493.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b57493.org /out b57493.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b57493.reflect.xml
    IF EXIST b57493.reflect.xml set ReflectionRoots=-x b57493.reflect.xml
    ...
    set ExePath=%LinkBin%\b57493.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b57493.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b57493.asm.exe %CLRTestExecutionArguments% %Host_Args%
