...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b34952\b34952 b34952.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b34952\b34952 b34952.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b34952.dasm.il b34952.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b34952.dasm.il b34952.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b34952.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b34952.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b34952.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b34952.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b34952
    if not exist "b34952.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b34952.org" (
            mkdir IL
            copy b34952.exe IL\b34952.exe 
            ren b34952.exe b34952.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b34952.org /out b34952.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b34952.org /out b34952.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b34952.reflect.xml
    IF EXIST b34952.reflect.xml set ReflectionRoots=-x b34952.reflect.xml
    ...
    set ExePath=%LinkBin%\b34952.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b34952.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b34952.asm.exe %CLRTestExecutionArguments% %Host_Args%
