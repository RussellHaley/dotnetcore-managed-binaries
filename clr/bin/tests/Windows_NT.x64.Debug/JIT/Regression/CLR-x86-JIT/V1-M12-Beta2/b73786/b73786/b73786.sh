...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b73786/b73786 b73786.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b73786/b73786 b73786.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b73786.dasm.il b73786.exe
    "$CORE_ROOT/ildasm" -raweh -out=b73786.dasm.il b73786.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b73786.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b73786.dasm.il
    "$CORE_ROOT/ilasm" -output=b73786.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b73786.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b73786
    if [ ! -f b73786.org ]; then
        TakeLock
        if [ ! -f b73786.org ]; then
          mkdir IL
          cp b73786.exe IL/b73786.exe
          mv b73786.exe b73786.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b73786.org /out b73786.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b73786.org /out b73786.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b73786.reflect.xml
  if [ -f b73786.reflect.xml ]
then
    ReflectionRoots="-x b73786.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b73786.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b73786.asm.exe $CLRTestExecutionArguments
