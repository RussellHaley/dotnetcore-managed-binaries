...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b66679/b66679 b66679.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b66679/b66679 b66679.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b66679.dasm.il b66679.exe
    "$CORE_ROOT/ildasm" -raweh -out=b66679.dasm.il b66679.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b66679.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b66679.dasm.il
    "$CORE_ROOT/ilasm" -output=b66679.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b66679.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b66679
    if [ ! -f b66679.org ]; then
        TakeLock
        if [ ! -f b66679.org ]; then
          mkdir IL
          cp b66679.exe IL/b66679.exe
          mv b66679.exe b66679.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b66679.org /out b66679.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b66679.org /out b66679.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b66679.reflect.xml
  if [ -f b66679.reflect.xml ]
then
    ReflectionRoots="-x b66679.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b66679.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b66679.asm.exe $CLRTestExecutionArguments
