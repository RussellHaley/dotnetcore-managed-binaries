...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b60142/b60142 b60142.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b60142/b60142 b60142.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b60142.dasm.il b60142.exe
    "$CORE_ROOT/ildasm" -raweh -out=b60142.dasm.il b60142.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b60142.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b60142.dasm.il
    "$CORE_ROOT/ilasm" -output=b60142.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b60142.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b60142
    if [ ! -f b60142.org ]; then
        TakeLock
        if [ ! -f b60142.org ]; then
          mkdir IL
          cp b60142.exe IL/b60142.exe
          mv b60142.exe b60142.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b60142.org /out b60142.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b60142.org /out b60142.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b60142.reflect.xml
  if [ -f b60142.reflect.xml ]
then
    ReflectionRoots="-x b60142.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b60142.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b60142.asm.exe $CLRTestExecutionArguments
