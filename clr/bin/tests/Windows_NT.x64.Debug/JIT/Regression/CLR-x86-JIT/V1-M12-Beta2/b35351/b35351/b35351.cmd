...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b35351\b35351 b35351.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b35351\b35351 b35351.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b35351.dasm.il b35351.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b35351.dasm.il b35351.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b35351.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b35351.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b35351.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b35351.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b35351
    if not exist "b35351.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b35351.org" (
            mkdir IL
            copy b35351.exe IL\b35351.exe 
            ren b35351.exe b35351.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b35351.org /out b35351.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b35351.org /out b35351.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b35351.reflect.xml
    IF EXIST b35351.reflect.xml set ReflectionRoots=-x b35351.reflect.xml
    ...
    set ExePath=%LinkBin%\b35351.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b35351.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b35351.asm.exe %CLRTestExecutionArguments% %Host_Args%
