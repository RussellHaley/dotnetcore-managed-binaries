...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b47975/b47975 b47975.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b47975/b47975 b47975.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b47975.dasm.il b47975.exe
    "$CORE_ROOT/ildasm" -raweh -out=b47975.dasm.il b47975.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b47975.asm.exe  b47975.dasm.il
    "$CORE_ROOT/ilasm" -output=b47975.asm.exe  b47975.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b47975
    if [ ! -f b47975.org ]; then
        TakeLock
        if [ ! -f b47975.org ]; then
          mkdir IL
          cp b47975.exe IL/b47975.exe
          mv b47975.exe b47975.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b47975.org /out b47975.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b47975.org /out b47975.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b47975.reflect.xml
  if [ -f b47975.reflect.xml ]
then
    ReflectionRoots="-x b47975.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b47975.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b47975.asm.exe $CLRTestExecutionArguments
