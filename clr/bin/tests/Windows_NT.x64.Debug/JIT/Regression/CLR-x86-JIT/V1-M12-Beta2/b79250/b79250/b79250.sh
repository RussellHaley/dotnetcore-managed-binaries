...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b79250/b79250 b79250.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b79250/b79250 b79250.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b79250.dasm.il b79250.exe
    "$CORE_ROOT/ildasm" -raweh -out=b79250.dasm.il b79250.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b79250.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b79250.dasm.il
    "$CORE_ROOT/ilasm" -output=b79250.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b79250.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b79250
    if [ ! -f b79250.org ]; then
        TakeLock
        if [ ! -f b79250.org ]; then
          mkdir IL
          cp b79250.exe IL/b79250.exe
          mv b79250.exe b79250.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b79250.org /out b79250.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b79250.org /out b79250.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b79250.reflect.xml
  if [ -f b79250.reflect.xml ]
then
    ReflectionRoots="-x b79250.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b79250.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b79250.asm.exe $CLRTestExecutionArguments
