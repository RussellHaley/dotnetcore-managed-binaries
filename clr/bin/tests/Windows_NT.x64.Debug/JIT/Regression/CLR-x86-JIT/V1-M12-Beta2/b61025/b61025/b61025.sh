...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b61025/b61025 b61025.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b61025/b61025 b61025.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b61025.dasm.il b61025.exe
    "$CORE_ROOT/ildasm" -raweh -out=b61025.dasm.il b61025.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b61025.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61025.dasm.il
    "$CORE_ROOT/ilasm" -output=b61025.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b61025.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b61025
    if [ ! -f b61025.org ]; then
        TakeLock
        if [ ! -f b61025.org ]; then
          mkdir IL
          cp b61025.exe IL/b61025.exe
          mv b61025.exe b61025.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b61025.org /out b61025.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b61025.org /out b61025.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b61025.reflect.xml
  if [ -f b61025.reflect.xml ]
then
    ReflectionRoots="-x b61025.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b61025.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b61025.asm.exe $CLRTestExecutionArguments
