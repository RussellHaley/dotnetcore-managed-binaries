...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b84971/b84971 b84971.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b84971/b84971 b84971.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b84971.dasm.il b84971.exe
    "$CORE_ROOT/ildasm" -raweh -out=b84971.dasm.il b84971.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b84971.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84971.dasm.il
    "$CORE_ROOT/ilasm" -output=b84971.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84971.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b84971
    if [ ! -f b84971.org ]; then
        TakeLock
        if [ ! -f b84971.org ]; then
          mkdir IL
          cp b84971.exe IL/b84971.exe
          mv b84971.exe b84971.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b84971.org /out b84971.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b84971.org /out b84971.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b84971.reflect.xml
  if [ -f b84971.reflect.xml ]
then
    ReflectionRoots="-x b84971.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b84971.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b84971.asm.exe $CLRTestExecutionArguments
