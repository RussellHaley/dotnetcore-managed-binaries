...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b75890/b75890 b75890.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b75890/b75890 b75890.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b75890.dasm.il b75890.exe
    "$CORE_ROOT/ildasm" -raweh -out=b75890.dasm.il b75890.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b75890.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75890.dasm.il
    "$CORE_ROOT/ilasm" -output=b75890.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75890.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b75890
    if [ ! -f b75890.org ]; then
        TakeLock
        if [ ! -f b75890.org ]; then
          mkdir IL
          cp b75890.exe IL/b75890.exe
          mv b75890.exe b75890.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b75890.org /out b75890.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b75890.org /out b75890.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b75890.reflect.xml
  if [ -f b75890.reflect.xml ]
then
    ReflectionRoots="-x b75890.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b75890.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b75890.asm.exe $CLRTestExecutionArguments
