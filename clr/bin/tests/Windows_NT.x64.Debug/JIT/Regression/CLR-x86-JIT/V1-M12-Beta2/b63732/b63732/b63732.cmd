...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b63732\b63732 b63732.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b63732\b63732 b63732.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b63732.dasm.il b63732.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b63732.dasm.il b63732.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b63732.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b63732.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b63732.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b63732.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b63732
    if not exist "b63732.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b63732.org" (
            mkdir IL
            copy b63732.exe IL\b63732.exe 
            ren b63732.exe b63732.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b63732.org /out b63732.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b63732.org /out b63732.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b63732.reflect.xml
    IF EXIST b63732.reflect.xml set ReflectionRoots=-x b63732.reflect.xml
    ...
    set ExePath=%LinkBin%\b63732.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b63732.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b63732.asm.exe %CLRTestExecutionArguments% %Host_Args%
