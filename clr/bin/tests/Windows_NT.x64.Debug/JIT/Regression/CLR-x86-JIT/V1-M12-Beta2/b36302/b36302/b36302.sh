...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b36302/b36302 b36302.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b36302/b36302 b36302.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b36302.dasm.il b36302.exe
    "$CORE_ROOT/ildasm" -raweh -out=b36302.dasm.il b36302.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b36302.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b36302.dasm.il
    "$CORE_ROOT/ilasm" -output=b36302.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b36302.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b36302
    if [ ! -f b36302.org ]; then
        TakeLock
        if [ ! -f b36302.org ]; then
          mkdir IL
          cp b36302.exe IL/b36302.exe
          mv b36302.exe b36302.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b36302.org /out b36302.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b36302.org /out b36302.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b36302.reflect.xml
  if [ -f b36302.reflect.xml ]
then
    ReflectionRoots="-x b36302.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b36302.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b36302.asm.exe $CLRTestExecutionArguments
