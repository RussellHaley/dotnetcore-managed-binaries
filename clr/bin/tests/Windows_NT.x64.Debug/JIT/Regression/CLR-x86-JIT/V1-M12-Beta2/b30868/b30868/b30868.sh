...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b30868/b30868 b30868.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b30868/b30868 b30868.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b30868.dasm.il b30868.exe
    "$CORE_ROOT/ildasm" -raweh -out=b30868.dasm.il b30868.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b30868.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30868.dasm.il
    "$CORE_ROOT/ilasm" -output=b30868.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30868.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b30868
    if [ ! -f b30868.org ]; then
        TakeLock
        if [ ! -f b30868.org ]; then
          mkdir IL
          cp b30868.exe IL/b30868.exe
          mv b30868.exe b30868.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30868.org /out b30868.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30868.org /out b30868.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b30868.reflect.xml
  if [ -f b30868.reflect.xml ]
then
    ReflectionRoots="-x b30868.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b30868.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b30868.asm.exe $CLRTestExecutionArguments
