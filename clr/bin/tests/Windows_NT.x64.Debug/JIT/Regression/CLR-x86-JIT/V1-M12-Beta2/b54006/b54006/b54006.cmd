...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b54006\b54006 b54006.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b54006\b54006 b54006.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b54006.dasm.il b54006.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b54006.dasm.il b54006.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b54006.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54006.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b54006.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54006.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b54006
    if not exist "b54006.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b54006.org" (
            mkdir IL
            copy b54006.exe IL\b54006.exe 
            ren b54006.exe b54006.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b54006.org /out b54006.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b54006.org /out b54006.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b54006.reflect.xml
    IF EXIST b54006.reflect.xml set ReflectionRoots=-x b54006.reflect.xml
    ...
    set ExePath=%LinkBin%\b54006.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b54006.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b54006.asm.exe %CLRTestExecutionArguments% %Host_Args%
