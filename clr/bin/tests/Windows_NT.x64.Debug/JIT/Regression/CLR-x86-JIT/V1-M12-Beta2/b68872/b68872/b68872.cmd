...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b68872\b68872 b68872.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b68872\b68872 b68872.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b68872.dasm.il b68872.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b68872.dasm.il b68872.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b68872.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b68872.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b68872.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b68872.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b68872
    if not exist "b68872.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b68872.org" (
            mkdir IL
            copy b68872.exe IL\b68872.exe 
            ren b68872.exe b68872.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b68872.org /out b68872.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b68872.org /out b68872.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b68872.reflect.xml
    IF EXIST b68872.reflect.xml set ReflectionRoots=-x b68872.reflect.xml
    ...
    set ExePath=%LinkBin%\b68872.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b68872.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b68872.asm.exe %CLRTestExecutionArguments% %Host_Args%
