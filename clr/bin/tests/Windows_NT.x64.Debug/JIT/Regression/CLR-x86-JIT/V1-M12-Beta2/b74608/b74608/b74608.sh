...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b74608/b74608 b74608.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b74608/b74608 b74608.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b74608.dasm.il b74608.exe
    "$CORE_ROOT/ildasm" -raweh -out=b74608.dasm.il b74608.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b74608.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b74608.dasm.il
    "$CORE_ROOT/ilasm" -output=b74608.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b74608.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b74608
    if [ ! -f b74608.org ]; then
        TakeLock
        if [ ! -f b74608.org ]; then
          mkdir IL
          cp b74608.exe IL/b74608.exe
          mv b74608.exe b74608.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b74608.org /out b74608.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b74608.org /out b74608.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b74608.reflect.xml
  if [ -f b74608.reflect.xml ]
then
    ReflectionRoots="-x b74608.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b74608.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b74608.asm.exe $CLRTestExecutionArguments
