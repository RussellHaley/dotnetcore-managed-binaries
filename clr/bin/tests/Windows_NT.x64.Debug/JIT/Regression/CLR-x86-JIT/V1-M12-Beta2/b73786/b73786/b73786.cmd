...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b73786\b73786 b73786.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b73786\b73786 b73786.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b73786.dasm.il b73786.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b73786.dasm.il b73786.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b73786.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b73786.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b73786.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b73786.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b73786
    if not exist "b73786.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b73786.org" (
            mkdir IL
            copy b73786.exe IL\b73786.exe 
            ren b73786.exe b73786.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b73786.org /out b73786.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b73786.org /out b73786.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b73786.reflect.xml
    IF EXIST b73786.reflect.xml set ReflectionRoots=-x b73786.reflect.xml
    ...
    set ExePath=%LinkBin%\b73786.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b73786.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b73786.asm.exe %CLRTestExecutionArguments% %Host_Args%
