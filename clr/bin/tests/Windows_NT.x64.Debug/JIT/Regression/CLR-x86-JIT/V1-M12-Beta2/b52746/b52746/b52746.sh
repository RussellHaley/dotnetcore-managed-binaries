...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b52746/b52746 b52746.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b52746/b52746 b52746.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b52746.dasm.il b52746.exe
    "$CORE_ROOT/ildasm" -raweh -out=b52746.dasm.il b52746.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b52746.asm.exe  b52746.dasm.il
    "$CORE_ROOT/ilasm" -output=b52746.asm.exe  b52746.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b52746
    if [ ! -f b52746.org ]; then
        TakeLock
        if [ ! -f b52746.org ]; then
          mkdir IL
          cp b52746.exe IL/b52746.exe
          mv b52746.exe b52746.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b52746.org /out b52746.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b52746.org /out b52746.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b52746.reflect.xml
  if [ -f b52746.reflect.xml ]
then
    ReflectionRoots="-x b52746.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b52746.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b52746.asm.exe $CLRTestExecutionArguments
