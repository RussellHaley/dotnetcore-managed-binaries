...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b37578\b37578 b37578.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b37578\b37578 b37578.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b37578.dasm.il b37578.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b37578.dasm.il b37578.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b37578.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37578.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b37578.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b37578.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b37578
    if not exist "b37578.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b37578.org" (
            mkdir IL
            copy b37578.exe IL\b37578.exe 
            ren b37578.exe b37578.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37578.org /out b37578.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37578.org /out b37578.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b37578.reflect.xml
    IF EXIST b37578.reflect.xml set ReflectionRoots=-x b37578.reflect.xml
    ...
    set ExePath=%LinkBin%\b37578.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b37578.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b37578.asm.exe %CLRTestExecutionArguments% %Host_Args%
