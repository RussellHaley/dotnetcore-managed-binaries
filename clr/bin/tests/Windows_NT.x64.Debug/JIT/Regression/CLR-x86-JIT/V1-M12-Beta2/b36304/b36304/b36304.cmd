...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b36304\b36304 b36304.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b36304\b36304 b36304.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b36304.dasm.il b36304.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b36304.dasm.il b36304.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b36304.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b36304.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b36304.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b36304.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b36304
    if not exist "b36304.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b36304.org" (
            mkdir IL
            copy b36304.exe IL\b36304.exe 
            ren b36304.exe b36304.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b36304.org /out b36304.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b36304.org /out b36304.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b36304.reflect.xml
    IF EXIST b36304.reflect.xml set ReflectionRoots=-x b36304.reflect.xml
    ...
    set ExePath=%LinkBin%\b36304.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b36304.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b36304.asm.exe %CLRTestExecutionArguments% %Host_Args%
