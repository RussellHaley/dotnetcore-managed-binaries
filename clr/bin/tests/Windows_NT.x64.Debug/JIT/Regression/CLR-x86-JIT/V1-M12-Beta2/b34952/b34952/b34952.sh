...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b34952/b34952 b34952.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b34952/b34952 b34952.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b34952.dasm.il b34952.exe
    "$CORE_ROOT/ildasm" -raweh -out=b34952.dasm.il b34952.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b34952.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b34952.dasm.il
    "$CORE_ROOT/ilasm" -output=b34952.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b34952.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b34952
    if [ ! -f b34952.org ]; then
        TakeLock
        if [ ! -f b34952.org ]; then
          mkdir IL
          cp b34952.exe IL/b34952.exe
          mv b34952.exe b34952.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b34952.org /out b34952.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b34952.org /out b34952.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b34952.reflect.xml
  if [ -f b34952.reflect.xml ]
then
    ReflectionRoots="-x b34952.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b34952.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b34952.asm.exe $CLRTestExecutionArguments
