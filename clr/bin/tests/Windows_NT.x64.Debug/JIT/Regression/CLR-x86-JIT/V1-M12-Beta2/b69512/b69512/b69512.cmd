...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b69512\b69512 b69512.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b69512\b69512 b69512.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b69512.dasm.il b69512.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b69512.dasm.il b69512.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b69512.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69512.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b69512.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69512.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b69512
    if not exist "b69512.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b69512.org" (
            mkdir IL
            copy b69512.exe IL\b69512.exe 
            ren b69512.exe b69512.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b69512.org /out b69512.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b69512.org /out b69512.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b69512.reflect.xml
    IF EXIST b69512.reflect.xml set ReflectionRoots=-x b69512.reflect.xml
    ...
    set ExePath=%LinkBin%\b69512.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b69512.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b69512.asm.exe %CLRTestExecutionArguments% %Host_Args%
