...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b67819/b67819 b67819.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b67819/b67819 b67819.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b67819.dasm.il b67819.exe
    "$CORE_ROOT/ildasm" -raweh -out=b67819.dasm.il b67819.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b67819.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b67819.dasm.il
    "$CORE_ROOT/ilasm" -output=b67819.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b67819.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b67819
    if [ ! -f b67819.org ]; then
        TakeLock
        if [ ! -f b67819.org ]; then
          mkdir IL
          cp b67819.exe IL/b67819.exe
          mv b67819.exe b67819.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b67819.org /out b67819.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b67819.org /out b67819.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b67819.reflect.xml
  if [ -f b67819.reflect.xml ]
then
    ReflectionRoots="-x b67819.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b67819.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b67819.asm.exe $CLRTestExecutionArguments
