...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b71003\b71003 b71003.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b71003\b71003 b71003.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b71003.dasm.il b71003.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b71003.dasm.il b71003.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b71003.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b71003.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b71003.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b71003.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b71003
    if not exist "b71003.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b71003.org" (
            mkdir IL
            copy b71003.exe IL\b71003.exe 
            ren b71003.exe b71003.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b71003.org /out b71003.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b71003.org /out b71003.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b71003.reflect.xml
    IF EXIST b71003.reflect.xml set ReflectionRoots=-x b71003.reflect.xml
    ...
    set ExePath=%LinkBin%\b71003.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b71003.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b71003.asm.exe %CLRTestExecutionArguments% %Host_Args%
