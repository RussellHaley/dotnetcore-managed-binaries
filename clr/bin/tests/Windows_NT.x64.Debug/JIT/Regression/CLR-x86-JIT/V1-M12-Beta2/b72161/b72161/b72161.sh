...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b72161/b72161 b72161.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b72161/b72161 b72161.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b72161.dasm.il b72161.exe
    "$CORE_ROOT/ildasm" -raweh -out=b72161.dasm.il b72161.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b72161.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b72161.dasm.il
    "$CORE_ROOT/ilasm" -output=b72161.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b72161.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b72161
    if [ ! -f b72161.org ]; then
        TakeLock
        if [ ! -f b72161.org ]; then
          mkdir IL
          cp b72161.exe IL/b72161.exe
          mv b72161.exe b72161.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b72161.org /out b72161.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b72161.org /out b72161.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b72161.reflect.xml
  if [ -f b72161.reflect.xml ]
then
    ReflectionRoots="-x b72161.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b72161.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b72161.asm.exe $CLRTestExecutionArguments
