...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b52840\b52840 b52840.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b52840\b52840 b52840.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b52840.dasm.il b52840.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b52840.dasm.il b52840.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b52840.asm.exe  b52840.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b52840.asm.exe  b52840.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b52840
    if not exist "b52840.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b52840.org" (
            mkdir IL
            copy b52840.exe IL\b52840.exe 
            ren b52840.exe b52840.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b52840.org /out b52840.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b52840.org /out b52840.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b52840.reflect.xml
    IF EXIST b52840.reflect.xml set ReflectionRoots=-x b52840.reflect.xml
    ...
    set ExePath=%LinkBin%\b52840.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b52840.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b52840.asm.exe %CLRTestExecutionArguments% %Host_Args%
