...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b65087/b65087 b65087.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b65087/b65087 b65087.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b65087.dasm.il b65087.exe
    "$CORE_ROOT/ildasm" -raweh -out=b65087.dasm.il b65087.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b65087.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b65087.dasm.il
    "$CORE_ROOT/ilasm" -output=b65087.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b65087.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b65087
    if [ ! -f b65087.org ]; then
        TakeLock
        if [ ! -f b65087.org ]; then
          mkdir IL
          cp b65087.exe IL/b65087.exe
          mv b65087.exe b65087.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b65087.org /out b65087.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b65087.org /out b65087.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b65087.reflect.xml
  if [ -f b65087.reflect.xml ]
then
    ReflectionRoots="-x b65087.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b65087.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b65087.asm.exe $CLRTestExecutionArguments
