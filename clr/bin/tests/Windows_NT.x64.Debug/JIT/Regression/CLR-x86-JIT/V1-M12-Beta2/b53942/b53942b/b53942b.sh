...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53942/b53942b b53942b.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53942/b53942b b53942b.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b53942b.dasm.il b53942b.exe
    "$CORE_ROOT/ildasm" -raweh -out=b53942b.dasm.il b53942b.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b53942b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53942b.dasm.il
    "$CORE_ROOT/ilasm" -output=b53942b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53942b.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b53942b
    if [ ! -f b53942b.org ]; then
        TakeLock
        if [ ! -f b53942b.org ]; then
          mkdir IL
          cp b53942b.exe IL/b53942b.exe
          mv b53942b.exe b53942b.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53942b.org /out b53942b.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53942b.org /out b53942b.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b53942b.reflect.xml
  if [ -f b53942b.reflect.xml ]
then
    ReflectionRoots="-x b53942b.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b53942b.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b53942b.asm.exe $CLRTestExecutionArguments
