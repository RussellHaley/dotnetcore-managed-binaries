...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b72136/b72136 b72136.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b72136/b72136 b72136.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b72136.dasm.il b72136.exe
    "$CORE_ROOT/ildasm" -raweh -out=b72136.dasm.il b72136.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b72136.asm.exe  b72136.dasm.il
    "$CORE_ROOT/ilasm" -output=b72136.asm.exe  b72136.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b72136
    if [ ! -f b72136.org ]; then
        TakeLock
        if [ ! -f b72136.org ]; then
          mkdir IL
          cp b72136.exe IL/b72136.exe
          mv b72136.exe b72136.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b72136.org /out b72136.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b72136.org /out b72136.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b72136.reflect.xml
  if [ -f b72136.reflect.xml ]
then
    ReflectionRoots="-x b72136.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b72136.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b72136.asm.exe $CLRTestExecutionArguments
