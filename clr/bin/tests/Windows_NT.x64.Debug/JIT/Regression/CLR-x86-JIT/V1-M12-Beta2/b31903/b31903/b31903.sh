...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31903/b31903 b31903.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31903/b31903 b31903.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31903.dasm.il b31903.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31903.dasm.il b31903.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31903.asm.exe  b31903.dasm.il
    "$CORE_ROOT/ilasm" -output=b31903.asm.exe  b31903.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31903
    if [ ! -f b31903.org ]; then
        TakeLock
        if [ ! -f b31903.org ]; then
          mkdir IL
          cp b31903.exe IL/b31903.exe
          mv b31903.exe b31903.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31903.org /out b31903.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31903.org /out b31903.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31903.reflect.xml
  if [ -f b31903.reflect.xml ]
then
    ReflectionRoots="-x b31903.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31903.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31903.asm.exe $CLRTestExecutionArguments
