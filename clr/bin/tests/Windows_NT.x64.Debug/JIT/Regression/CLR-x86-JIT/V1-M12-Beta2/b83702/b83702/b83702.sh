...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b83702/b83702 b83702.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b83702/b83702 b83702.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b83702.dasm.il b83702.exe
    "$CORE_ROOT/ildasm" -raweh -out=b83702.dasm.il b83702.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b83702.asm.exe  b83702.dasm.il
    "$CORE_ROOT/ilasm" -output=b83702.asm.exe  b83702.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b83702
    if [ ! -f b83702.org ]; then
        TakeLock
        if [ ! -f b83702.org ]; then
          mkdir IL
          cp b83702.exe IL/b83702.exe
          mv b83702.exe b83702.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b83702.org /out b83702.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b83702.org /out b83702.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b83702.reflect.xml
  if [ -f b83702.reflect.xml ]
then
    ReflectionRoots="-x b83702.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b83702.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b83702.asm.exe $CLRTestExecutionArguments
