...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b57518/b57518 b57518.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b57518/b57518 b57518.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b57518.dasm.il b57518.exe
    "$CORE_ROOT/ildasm" -raweh -out=b57518.dasm.il b57518.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b57518.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57518.dasm.il
    "$CORE_ROOT/ilasm" -output=b57518.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b57518.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b57518
    if [ ! -f b57518.org ]; then
        TakeLock
        if [ ! -f b57518.org ]; then
          mkdir IL
          cp b57518.exe IL/b57518.exe
          mv b57518.exe b57518.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b57518.org /out b57518.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b57518.org /out b57518.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b57518.reflect.xml
  if [ -f b57518.reflect.xml ]
then
    ReflectionRoots="-x b57518.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b57518.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b57518.asm.exe $CLRTestExecutionArguments
