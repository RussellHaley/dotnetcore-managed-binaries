...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b59949/b59949 b59949.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b59949/b59949 b59949.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b59949.dasm.il b59949.exe
    "$CORE_ROOT/ildasm" -raweh -out=b59949.dasm.il b59949.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b59949.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59949.dasm.il
    "$CORE_ROOT/ilasm" -output=b59949.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b59949.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b59949
    if [ ! -f b59949.org ]; then
        TakeLock
        if [ ! -f b59949.org ]; then
          mkdir IL
          cp b59949.exe IL/b59949.exe
          mv b59949.exe b59949.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b59949.org /out b59949.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b59949.org /out b59949.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b59949.reflect.xml
  if [ -f b59949.reflect.xml ]
then
    ReflectionRoots="-x b59949.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b59949.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b59949.asm.exe $CLRTestExecutionArguments
