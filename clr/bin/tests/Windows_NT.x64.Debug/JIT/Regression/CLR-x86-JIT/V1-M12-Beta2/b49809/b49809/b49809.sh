...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b49809/b49809 b49809.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b49809/b49809 b49809.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b49809.dasm.il b49809.exe
    "$CORE_ROOT/ildasm" -raweh -out=b49809.dasm.il b49809.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b49809.asm.exe  b49809.dasm.il
    "$CORE_ROOT/ilasm" -output=b49809.asm.exe  b49809.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b49809
    if [ ! -f b49809.org ]; then
        TakeLock
        if [ ! -f b49809.org ]; then
          mkdir IL
          cp b49809.exe IL/b49809.exe
          mv b49809.exe b49809.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b49809.org /out b49809.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b49809.org /out b49809.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b49809.reflect.xml
  if [ -f b49809.reflect.xml ]
then
    ReflectionRoots="-x b49809.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b49809.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b49809.asm.exe $CLRTestExecutionArguments
