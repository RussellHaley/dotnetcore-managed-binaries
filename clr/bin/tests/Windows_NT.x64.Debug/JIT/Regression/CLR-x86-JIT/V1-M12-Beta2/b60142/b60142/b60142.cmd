...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b60142\b60142 b60142.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b60142\b60142 b60142.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b60142.dasm.il b60142.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b60142.dasm.il b60142.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b60142.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b60142.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b60142.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b60142.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b60142
    if not exist "b60142.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b60142.org" (
            mkdir IL
            copy b60142.exe IL\b60142.exe 
            ren b60142.exe b60142.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b60142.org /out b60142.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b60142.org /out b60142.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b60142.reflect.xml
    IF EXIST b60142.reflect.xml set ReflectionRoots=-x b60142.reflect.xml
    ...
    set ExePath=%LinkBin%\b60142.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b60142.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b60142.asm.exe %CLRTestExecutionArguments% %Host_Args%
