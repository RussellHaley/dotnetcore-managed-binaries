...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53942/b53942a b53942a.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53942/b53942a b53942a.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b53942a.dasm.il b53942a.exe
    "$CORE_ROOT/ildasm" -raweh -out=b53942a.dasm.il b53942a.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b53942a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53942a.dasm.il
    "$CORE_ROOT/ilasm" -output=b53942a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53942a.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b53942a
    if [ ! -f b53942a.org ]; then
        TakeLock
        if [ ! -f b53942a.org ]; then
          mkdir IL
          cp b53942a.exe IL/b53942a.exe
          mv b53942a.exe b53942a.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53942a.org /out b53942a.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53942a.org /out b53942a.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b53942a.reflect.xml
  if [ -f b53942a.reflect.xml ]
then
    ReflectionRoots="-x b53942a.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b53942a.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b53942a.asm.exe $CLRTestExecutionArguments
