...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b55923/b55923 b55923.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b55923/b55923 b55923.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b55923.dasm.il b55923.exe
    "$CORE_ROOT/ildasm" -raweh -out=b55923.dasm.il b55923.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b55923.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b55923.dasm.il
    "$CORE_ROOT/ilasm" -output=b55923.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b55923.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b55923
    if [ ! -f b55923.org ]; then
        TakeLock
        if [ ! -f b55923.org ]; then
          mkdir IL
          cp b55923.exe IL/b55923.exe
          mv b55923.exe b55923.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b55923.org /out b55923.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b55923.org /out b55923.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b55923.reflect.xml
  if [ -f b55923.reflect.xml ]
then
    ReflectionRoots="-x b55923.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b55923.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b55923.asm.exe $CLRTestExecutionArguments
