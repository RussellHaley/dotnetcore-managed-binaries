...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b50033\b50033 b50033.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b50033\b50033 b50033.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b50033.dasm.il b50033.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b50033.dasm.il b50033.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b50033.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b50033.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b50033.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b50033.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b50033
    if not exist "b50033.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b50033.org" (
            mkdir IL
            copy b50033.exe IL\b50033.exe 
            ren b50033.exe b50033.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b50033.org /out b50033.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b50033.org /out b50033.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b50033.reflect.xml
    IF EXIST b50033.reflect.xml set ReflectionRoots=-x b50033.reflect.xml
    ...
    set ExePath=%LinkBin%\b50033.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b50033.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b50033.asm.exe %CLRTestExecutionArguments% %Host_Args%
