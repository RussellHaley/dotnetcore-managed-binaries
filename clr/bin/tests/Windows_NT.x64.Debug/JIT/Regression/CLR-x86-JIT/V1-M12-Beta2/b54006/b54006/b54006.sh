...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b54006/b54006 b54006.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b54006/b54006 b54006.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b54006.dasm.il b54006.exe
    "$CORE_ROOT/ildasm" -raweh -out=b54006.dasm.il b54006.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b54006.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54006.dasm.il
    "$CORE_ROOT/ilasm" -output=b54006.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54006.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b54006
    if [ ! -f b54006.org ]; then
        TakeLock
        if [ ! -f b54006.org ]; then
          mkdir IL
          cp b54006.exe IL/b54006.exe
          mv b54006.exe b54006.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b54006.org /out b54006.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b54006.org /out b54006.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b54006.reflect.xml
  if [ -f b54006.reflect.xml ]
then
    ReflectionRoots="-x b54006.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b54006.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b54006.asm.exe $CLRTestExecutionArguments
