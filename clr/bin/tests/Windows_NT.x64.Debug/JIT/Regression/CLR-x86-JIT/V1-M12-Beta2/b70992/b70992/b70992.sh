...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b70992/b70992 b70992.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b70992/b70992 b70992.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b70992.dasm.il b70992.exe
    "$CORE_ROOT/ildasm" -raweh -out=b70992.dasm.il b70992.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b70992.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70992.dasm.il
    "$CORE_ROOT/ilasm" -output=b70992.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70992.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b70992
    if [ ! -f b70992.org ]; then
        TakeLock
        if [ ! -f b70992.org ]; then
          mkdir IL
          cp b70992.exe IL/b70992.exe
          mv b70992.exe b70992.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b70992.org /out b70992.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b70992.org /out b70992.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b70992.reflect.xml
  if [ -f b70992.reflect.xml ]
then
    ReflectionRoots="-x b70992.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b70992.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b70992.asm.exe $CLRTestExecutionArguments
