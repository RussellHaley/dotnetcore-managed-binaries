...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b31292\b31292 b31292.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b31292\b31292 b31292.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b31292.dasm.il b31292.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b31292.dasm.il b31292.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b31292.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31292.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b31292.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31292.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b31292
    if not exist "b31292.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b31292.org" (
            mkdir IL
            copy b31292.exe IL\b31292.exe 
            ren b31292.exe b31292.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31292.org /out b31292.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31292.org /out b31292.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b31292.reflect.xml
    IF EXIST b31292.reflect.xml set ReflectionRoots=-x b31292.reflect.xml
    ...
    set ExePath=%LinkBin%\b31292.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b31292.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b31292.asm.exe %CLRTestExecutionArguments% %Host_Args%
