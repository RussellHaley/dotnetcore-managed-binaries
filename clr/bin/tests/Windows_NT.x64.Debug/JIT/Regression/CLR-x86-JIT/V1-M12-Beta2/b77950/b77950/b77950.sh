...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b77950/b77950 b77950.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b77950/b77950 b77950.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b77950.dasm.il b77950.exe
    "$CORE_ROOT/ildasm" -raweh -out=b77950.dasm.il b77950.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b77950.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b77950.dasm.il
    "$CORE_ROOT/ilasm" -output=b77950.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b77950.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b77950
    if [ ! -f b77950.org ]; then
        TakeLock
        if [ ! -f b77950.org ]; then
          mkdir IL
          cp b77950.exe IL/b77950.exe
          mv b77950.exe b77950.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b77950.org /out b77950.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b77950.org /out b77950.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b77950.reflect.xml
  if [ -f b77950.reflect.xml ]
then
    ReflectionRoots="-x b77950.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b77950.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b77950.asm.exe $CLRTestExecutionArguments
