...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b69227/b69227 b69227.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b69227/b69227 b69227.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b69227.dasm.il b69227.exe
    "$CORE_ROOT/ildasm" -raweh -out=b69227.dasm.il b69227.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b69227.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69227.dasm.il
    "$CORE_ROOT/ilasm" -output=b69227.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69227.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b69227
    if [ ! -f b69227.org ]; then
        TakeLock
        if [ ! -f b69227.org ]; then
          mkdir IL
          cp b69227.exe IL/b69227.exe
          mv b69227.exe b69227.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b69227.org /out b69227.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b69227.org /out b69227.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b69227.reflect.xml
  if [ -f b69227.reflect.xml ]
then
    ReflectionRoots="-x b69227.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b69227.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b69227.asm.exe $CLRTestExecutionArguments
