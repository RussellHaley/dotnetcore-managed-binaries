...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b51420/b51420 b51420.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b51420/b51420 b51420.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b51420.dasm.il b51420.exe
    "$CORE_ROOT/ildasm" -raweh -out=b51420.dasm.il b51420.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b51420.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b51420.dasm.il
    "$CORE_ROOT/ilasm" -output=b51420.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b51420.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b51420
    if [ ! -f b51420.org ]; then
        TakeLock
        if [ ! -f b51420.org ]; then
          mkdir IL
          cp b51420.exe IL/b51420.exe
          mv b51420.exe b51420.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b51420.org /out b51420.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b51420.org /out b51420.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b51420.reflect.xml
  if [ -f b51420.reflect.xml ]
then
    ReflectionRoots="-x b51420.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b51420.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b51420.asm.exe $CLRTestExecutionArguments
