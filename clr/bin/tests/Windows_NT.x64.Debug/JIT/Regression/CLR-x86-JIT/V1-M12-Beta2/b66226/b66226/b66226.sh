...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b66226/b66226 b66226.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b66226/b66226 b66226.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b66226.dasm.il b66226.exe
    "$CORE_ROOT/ildasm" -raweh -out=b66226.dasm.il b66226.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b66226.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b66226.dasm.il
    "$CORE_ROOT/ilasm" -output=b66226.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b66226.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b66226
    if [ ! -f b66226.org ]; then
        TakeLock
        if [ ! -f b66226.org ]; then
          mkdir IL
          cp b66226.exe IL/b66226.exe
          mv b66226.exe b66226.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b66226.org /out b66226.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b66226.org /out b66226.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b66226.reflect.xml
  if [ -f b66226.reflect.xml ]
then
    ReflectionRoots="-x b66226.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b66226.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b66226.asm.exe $CLRTestExecutionArguments
