...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b65176\b65176 b65176.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b65176\b65176 b65176.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b65176.dasm.il b65176.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b65176.dasm.il b65176.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b65176.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b65176.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b65176.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b65176.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b65176
    if not exist "b65176.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b65176.org" (
            mkdir IL
            copy b65176.exe IL\b65176.exe 
            ren b65176.exe b65176.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b65176.org /out b65176.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b65176.org /out b65176.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b65176.reflect.xml
    IF EXIST b65176.reflect.xml set ReflectionRoots=-x b65176.reflect.xml
    ...
    set ExePath=%LinkBin%\b65176.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b65176.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b65176.asm.exe %CLRTestExecutionArguments% %Host_Args%
