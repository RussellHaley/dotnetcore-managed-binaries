...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b70992\b70992 b70992.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b70992\b70992 b70992.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b70992.dasm.il b70992.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b70992.dasm.il b70992.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b70992.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70992.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b70992.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b70992.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b70992
    if not exist "b70992.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b70992.org" (
            mkdir IL
            copy b70992.exe IL\b70992.exe 
            ren b70992.exe b70992.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b70992.org /out b70992.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b70992.org /out b70992.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b70992.reflect.xml
    IF EXIST b70992.reflect.xml set ReflectionRoots=-x b70992.reflect.xml
    ...
    set ExePath=%LinkBin%\b70992.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b70992.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b70992.asm.exe %CLRTestExecutionArguments% %Host_Args%
