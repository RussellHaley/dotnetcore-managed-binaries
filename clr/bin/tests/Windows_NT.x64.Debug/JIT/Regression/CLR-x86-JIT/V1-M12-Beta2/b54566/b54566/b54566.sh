...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b54566/b54566 b54566.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b54566/b54566 b54566.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b54566.dasm.il b54566.exe
    "$CORE_ROOT/ildasm" -raweh -out=b54566.dasm.il b54566.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b54566.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54566.dasm.il
    "$CORE_ROOT/ilasm" -output=b54566.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b54566.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b54566
    if [ ! -f b54566.org ]; then
        TakeLock
        if [ ! -f b54566.org ]; then
          mkdir IL
          cp b54566.exe IL/b54566.exe
          mv b54566.exe b54566.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b54566.org /out b54566.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b54566.org /out b54566.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b54566.reflect.xml
  if [ -f b54566.reflect.xml ]
then
    ReflectionRoots="-x b54566.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b54566.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b54566.asm.exe $CLRTestExecutionArguments
