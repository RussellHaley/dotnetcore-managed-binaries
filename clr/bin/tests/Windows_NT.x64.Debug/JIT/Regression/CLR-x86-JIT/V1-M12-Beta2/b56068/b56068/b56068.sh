...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b56068/b56068 b56068.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b56068/b56068 b56068.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b56068.dasm.il b56068.exe
    "$CORE_ROOT/ildasm" -raweh -out=b56068.dasm.il b56068.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b56068.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b56068.dasm.il
    "$CORE_ROOT/ilasm" -output=b56068.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b56068.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b56068
    if [ ! -f b56068.org ]; then
        TakeLock
        if [ ! -f b56068.org ]; then
          mkdir IL
          cp b56068.exe IL/b56068.exe
          mv b56068.exe b56068.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b56068.org /out b56068.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b56068.org /out b56068.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b56068.reflect.xml
  if [ -f b56068.reflect.xml ]
then
    ReflectionRoots="-x b56068.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b56068.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b56068.asm.exe $CLRTestExecutionArguments
