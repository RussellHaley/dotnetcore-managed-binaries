...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71099/b71099 b71099.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71099/b71099 b71099.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b71099.dasm.il b71099.exe
    "$CORE_ROOT/ildasm" -raweh -out=b71099.dasm.il b71099.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b71099.asm.exe  b71099.dasm.il
    "$CORE_ROOT/ilasm" -output=b71099.asm.exe  b71099.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b71099
    if [ ! -f b71099.org ]; then
        TakeLock
        if [ ! -f b71099.org ]; then
          mkdir IL
          cp b71099.exe IL/b71099.exe
          mv b71099.exe b71099.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71099.org /out b71099.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71099.org /out b71099.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b71099.reflect.xml
  if [ -f b71099.reflect.xml ]
then
    ReflectionRoots="-x b71099.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b71099.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b71099.asm.exe $CLRTestExecutionArguments
