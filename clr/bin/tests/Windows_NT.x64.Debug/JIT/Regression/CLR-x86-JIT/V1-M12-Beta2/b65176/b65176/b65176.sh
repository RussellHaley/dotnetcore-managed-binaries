...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b65176/b65176 b65176.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b65176/b65176 b65176.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b65176.dasm.il b65176.exe
    "$CORE_ROOT/ildasm" -raweh -out=b65176.dasm.il b65176.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b65176.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b65176.dasm.il
    "$CORE_ROOT/ilasm" -output=b65176.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b65176.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b65176
    if [ ! -f b65176.org ]; then
        TakeLock
        if [ ! -f b65176.org ]; then
          mkdir IL
          cp b65176.exe IL/b65176.exe
          mv b65176.exe b65176.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b65176.org /out b65176.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b65176.org /out b65176.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b65176.reflect.xml
  if [ -f b65176.reflect.xml ]
then
    ReflectionRoots="-x b65176.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b65176.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b65176.asm.exe $CLRTestExecutionArguments
