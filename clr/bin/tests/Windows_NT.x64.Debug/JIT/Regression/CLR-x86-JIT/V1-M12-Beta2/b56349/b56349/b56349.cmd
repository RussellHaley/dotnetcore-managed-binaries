...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b56349\b56349 b56349.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b56349\b56349 b56349.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b56349.dasm.il b56349.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b56349.dasm.il b56349.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b56349.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b56349.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b56349.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b56349.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b56349
    if not exist "b56349.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b56349.org" (
            mkdir IL
            copy b56349.exe IL\b56349.exe 
            ren b56349.exe b56349.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b56349.org /out b56349.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b56349.org /out b56349.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b56349.reflect.xml
    IF EXIST b56349.reflect.xml set ReflectionRoots=-x b56349.reflect.xml
    ...
    set ExePath=%LinkBin%\b56349.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b56349.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b56349.asm.exe %CLRTestExecutionArguments% %Host_Args%
