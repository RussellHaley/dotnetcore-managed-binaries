...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b32614\b32614 b32614.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b32614\b32614 b32614.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b32614.dasm.il b32614.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b32614.dasm.il b32614.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b32614.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32614.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b32614.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32614.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b32614
    if not exist "b32614.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b32614.org" (
            mkdir IL
            copy b32614.exe IL\b32614.exe 
            ren b32614.exe b32614.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b32614.org /out b32614.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b32614.org /out b32614.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b32614.reflect.xml
    IF EXIST b32614.reflect.xml set ReflectionRoots=-x b32614.reflect.xml
    ...
    set ExePath=%LinkBin%\b32614.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b32614.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b32614.asm.exe %CLRTestExecutionArguments% %Host_Args%
