...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53958/b53958 b53958.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53958/b53958 b53958.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b53958.dasm.il b53958.exe
    "$CORE_ROOT/ildasm" -raweh -out=b53958.dasm.il b53958.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b53958.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53958.dasm.il
    "$CORE_ROOT/ilasm" -output=b53958.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53958.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b53958
    if [ ! -f b53958.org ]; then
        TakeLock
        if [ ! -f b53958.org ]; then
          mkdir IL
          cp b53958.exe IL/b53958.exe
          mv b53958.exe b53958.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53958.org /out b53958.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53958.org /out b53958.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b53958.reflect.xml
  if [ -f b53958.reflect.xml ]
then
    ReflectionRoots="-x b53958.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b53958.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b53958.asm.exe $CLRTestExecutionArguments
