...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b63183\b63183 b63183.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b63183\b63183 b63183.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b63183.dasm.il b63183.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b63183.dasm.il b63183.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b63183.asm.exe  b63183.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b63183.asm.exe  b63183.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b63183
    if not exist "b63183.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b63183.org" (
            mkdir IL
            copy b63183.exe IL\b63183.exe 
            ren b63183.exe b63183.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b63183.org /out b63183.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b63183.org /out b63183.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b63183.reflect.xml
    IF EXIST b63183.reflect.xml set ReflectionRoots=-x b63183.reflect.xml
    ...
    set ExePath=%LinkBin%\b63183.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b63183.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b63183.asm.exe %CLRTestExecutionArguments% %Host_Args%
