...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b69848/b69848 b69848.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b69848/b69848 b69848.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b69848.dasm.il b69848.exe
    "$CORE_ROOT/ildasm" -raweh -out=b69848.dasm.il b69848.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b69848.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69848.dasm.il
    "$CORE_ROOT/ilasm" -output=b69848.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69848.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b69848
    if [ ! -f b69848.org ]; then
        TakeLock
        if [ ! -f b69848.org ]; then
          mkdir IL
          cp b69848.exe IL/b69848.exe
          mv b69848.exe b69848.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b69848.org /out b69848.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b69848.org /out b69848.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b69848.reflect.xml
  if [ -f b69848.reflect.xml ]
then
    ReflectionRoots="-x b69848.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b69848.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b69848.asm.exe $CLRTestExecutionArguments
