...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b46566\b46566 b46566.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b46566\b46566 b46566.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b46566.dasm.il b46566.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b46566.dasm.il b46566.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b46566.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b46566.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b46566.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b46566.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b46566
    if not exist "b46566.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b46566.org" (
            mkdir IL
            copy b46566.exe IL\b46566.exe 
            ren b46566.exe b46566.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b46566.org /out b46566.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b46566.org /out b46566.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b46566.reflect.xml
    IF EXIST b46566.reflect.xml set ReflectionRoots=-x b46566.reflect.xml
    ...
    set ExePath=%LinkBin%\b46566.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b46566.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b46566.asm.exe %CLRTestExecutionArguments% %Host_Args%
