...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b80045\b80045 b80045.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b80045\b80045 b80045.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b80045.dasm.il b80045.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b80045.dasm.il b80045.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b80045.asm.exe  b80045.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b80045.asm.exe  b80045.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b80045
    if not exist "b80045.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b80045.org" (
            mkdir IL
            copy b80045.exe IL\b80045.exe 
            ren b80045.exe b80045.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b80045.org /out b80045.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b80045.org /out b80045.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b80045.reflect.xml
    IF EXIST b80045.reflect.xml set ReflectionRoots=-x b80045.reflect.xml
    ...
    set ExePath=%LinkBin%\b80045.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b80045.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b80045.asm.exe %CLRTestExecutionArguments% %Host_Args%
