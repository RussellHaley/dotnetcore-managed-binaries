...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b52840/b52840 b52840.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b52840/b52840 b52840.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b52840.dasm.il b52840.exe
    "$CORE_ROOT/ildasm" -raweh -out=b52840.dasm.il b52840.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b52840.asm.exe  b52840.dasm.il
    "$CORE_ROOT/ilasm" -output=b52840.asm.exe  b52840.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b52840
    if [ ! -f b52840.org ]; then
        TakeLock
        if [ ! -f b52840.org ]; then
          mkdir IL
          cp b52840.exe IL/b52840.exe
          mv b52840.exe b52840.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b52840.org /out b52840.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b52840.org /out b52840.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b52840.reflect.xml
  if [ -f b52840.reflect.xml ]
then
    ReflectionRoots="-x b52840.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b52840.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b52840.asm.exe $CLRTestExecutionArguments
