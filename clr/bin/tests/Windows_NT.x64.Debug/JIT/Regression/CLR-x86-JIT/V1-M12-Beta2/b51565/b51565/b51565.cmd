...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b51565\b51565 b51565.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b51565\b51565 b51565.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b51565.dasm.il b51565.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b51565.dasm.il b51565.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b51565.asm.exe  b51565.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b51565.asm.exe  b51565.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b51565
    if not exist "b51565.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b51565.org" (
            mkdir IL
            copy b51565.exe IL\b51565.exe 
            ren b51565.exe b51565.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b51565.org /out b51565.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b51565.org /out b51565.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b51565.reflect.xml
    IF EXIST b51565.reflect.xml set ReflectionRoots=-x b51565.reflect.xml
    ...
    set ExePath=%LinkBin%\b51565.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b51565.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b51565.asm.exe %CLRTestExecutionArguments% %Host_Args%
