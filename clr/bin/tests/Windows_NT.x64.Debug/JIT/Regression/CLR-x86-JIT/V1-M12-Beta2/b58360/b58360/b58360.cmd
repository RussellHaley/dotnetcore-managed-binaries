...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b58360\b58360 b58360.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b58360\b58360 b58360.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b58360.dasm.il b58360.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b58360.dasm.il b58360.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b58360.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b58360.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b58360.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b58360.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b58360
    if not exist "b58360.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b58360.org" (
            mkdir IL
            copy b58360.exe IL\b58360.exe 
            ren b58360.exe b58360.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b58360.org /out b58360.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b58360.org /out b58360.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b58360.reflect.xml
    IF EXIST b58360.reflect.xml set ReflectionRoots=-x b58360.reflect.xml
    ...
    set ExePath=%LinkBin%\b58360.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b58360.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b58360.asm.exe %CLRTestExecutionArguments% %Host_Args%
