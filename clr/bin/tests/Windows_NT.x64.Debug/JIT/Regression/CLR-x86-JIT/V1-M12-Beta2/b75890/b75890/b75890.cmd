...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b75890\b75890 b75890.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b75890\b75890 b75890.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b75890.dasm.il b75890.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b75890.dasm.il b75890.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b75890.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75890.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b75890.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b75890.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b75890
    if not exist "b75890.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b75890.org" (
            mkdir IL
            copy b75890.exe IL\b75890.exe 
            ren b75890.exe b75890.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b75890.org /out b75890.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b75890.org /out b75890.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b75890.reflect.xml
    IF EXIST b75890.reflect.xml set ReflectionRoots=-x b75890.reflect.xml
    ...
    set ExePath=%LinkBin%\b75890.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b75890.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b75890.asm.exe %CLRTestExecutionArguments% %Host_Args%
