...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b47975\b47975 b47975.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b47975\b47975 b47975.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b47975.dasm.il b47975.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b47975.dasm.il b47975.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b47975.asm.exe  b47975.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b47975.asm.exe  b47975.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b47975
    if not exist "b47975.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b47975.org" (
            mkdir IL
            copy b47975.exe IL\b47975.exe 
            ren b47975.exe b47975.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47975.org /out b47975.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47975.org /out b47975.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b47975.reflect.xml
    IF EXIST b47975.reflect.xml set ReflectionRoots=-x b47975.reflect.xml
    ...
    set ExePath=%LinkBin%\b47975.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b47975.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b47975.asm.exe %CLRTestExecutionArguments% %Host_Args%
