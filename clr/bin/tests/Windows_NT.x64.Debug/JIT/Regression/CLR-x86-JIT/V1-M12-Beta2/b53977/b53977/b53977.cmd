...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b53977\b53977 b53977.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b53977\b53977 b53977.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b53977.dasm.il b53977.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b53977.dasm.il b53977.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b53977.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53977.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b53977.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53977.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b53977
    if not exist "b53977.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b53977.org" (
            mkdir IL
            copy b53977.exe IL\b53977.exe 
            ren b53977.exe b53977.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b53977.org /out b53977.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b53977.org /out b53977.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b53977.reflect.xml
    IF EXIST b53977.reflect.xml set ReflectionRoots=-x b53977.reflect.xml
    ...
    set ExePath=%LinkBin%\b53977.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b53977.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b53977.asm.exe %CLRTestExecutionArguments% %Host_Args%
