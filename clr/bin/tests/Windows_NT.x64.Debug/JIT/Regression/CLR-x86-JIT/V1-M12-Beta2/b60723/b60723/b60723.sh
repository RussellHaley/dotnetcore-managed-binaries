...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b60723/b60723 b60723.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b60723/b60723 b60723.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b60723.dasm.il b60723.exe
    "$CORE_ROOT/ildasm" -raweh -out=b60723.dasm.il b60723.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b60723.asm.exe  b60723.dasm.il
    "$CORE_ROOT/ilasm" -output=b60723.asm.exe  b60723.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b60723
    if [ ! -f b60723.org ]; then
        TakeLock
        if [ ! -f b60723.org ]; then
          mkdir IL
          cp b60723.exe IL/b60723.exe
          mv b60723.exe b60723.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b60723.org /out b60723.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b60723.org /out b60723.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b60723.reflect.xml
  if [ -f b60723.reflect.xml ]
then
    ReflectionRoots="-x b60723.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b60723.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b60723.asm.exe $CLRTestExecutionArguments
