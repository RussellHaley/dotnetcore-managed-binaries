...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b51469/b51469 b51469.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b51469/b51469 b51469.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b51469.dasm.il b51469.exe
    "$CORE_ROOT/ildasm" -raweh -out=b51469.dasm.il b51469.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b51469.asm.exe  b51469.dasm.il
    "$CORE_ROOT/ilasm" -output=b51469.asm.exe  b51469.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b51469
    if [ ! -f b51469.org ]; then
        TakeLock
        if [ ! -f b51469.org ]; then
          mkdir IL
          cp b51469.exe IL/b51469.exe
          mv b51469.exe b51469.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b51469.org /out b51469.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b51469.org /out b51469.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b51469.reflect.xml
  if [ -f b51469.reflect.xml ]
then
    ReflectionRoots="-x b51469.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b51469.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b51469.asm.exe $CLRTestExecutionArguments
