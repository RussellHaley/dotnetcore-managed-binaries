...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53977/b53977 b53977.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53977/b53977 b53977.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b53977.dasm.il b53977.exe
    "$CORE_ROOT/ildasm" -raweh -out=b53977.dasm.il b53977.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b53977.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53977.dasm.il
    "$CORE_ROOT/ilasm" -output=b53977.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53977.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b53977
    if [ ! -f b53977.org ]; then
        TakeLock
        if [ ! -f b53977.org ]; then
          mkdir IL
          cp b53977.exe IL/b53977.exe
          mv b53977.exe b53977.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53977.org /out b53977.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53977.org /out b53977.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b53977.reflect.xml
  if [ -f b53977.reflect.xml ]
then
    ReflectionRoots="-x b53977.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b53977.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b53977.asm.exe $CLRTestExecutionArguments
