...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31547/b31547 b31547.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31547/b31547 b31547.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31547.dasm.il b31547.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31547.dasm.il b31547.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31547.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31547.dasm.il
    "$CORE_ROOT/ilasm" -output=b31547.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31547.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31547
    if [ ! -f b31547.org ]; then
        TakeLock
        if [ ! -f b31547.org ]; then
          mkdir IL
          cp b31547.exe IL/b31547.exe
          mv b31547.exe b31547.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31547.org /out b31547.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31547.org /out b31547.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31547.reflect.xml
  if [ -f b31547.reflect.xml ]
then
    ReflectionRoots="-x b31547.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31547.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31547.asm.exe $CLRTestExecutionArguments
