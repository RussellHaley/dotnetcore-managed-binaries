...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b77304/b77304 b77304.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b77304/b77304 b77304.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b77304.dasm.il b77304.exe
    "$CORE_ROOT/ildasm" -raweh -out=b77304.dasm.il b77304.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b77304.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b77304.dasm.il
    "$CORE_ROOT/ilasm" -output=b77304.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b77304.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b77304
    if [ ! -f b77304.org ]; then
        TakeLock
        if [ ! -f b77304.org ]; then
          mkdir IL
          cp b77304.exe IL/b77304.exe
          mv b77304.exe b77304.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b77304.org /out b77304.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b77304.org /out b77304.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b77304.reflect.xml
  if [ -f b77304.reflect.xml ]
then
    ReflectionRoots="-x b77304.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b77304.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b77304.asm.exe $CLRTestExecutionArguments
