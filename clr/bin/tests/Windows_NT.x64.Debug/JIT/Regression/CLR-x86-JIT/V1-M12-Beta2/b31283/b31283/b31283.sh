...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31283/b31283 b31283.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31283/b31283 b31283.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31283.dasm.il b31283.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31283.dasm.il b31283.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31283.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31283.dasm.il
    "$CORE_ROOT/ilasm" -output=b31283.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31283.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31283
    if [ ! -f b31283.org ]; then
        TakeLock
        if [ ! -f b31283.org ]; then
          mkdir IL
          cp b31283.exe IL/b31283.exe
          mv b31283.exe b31283.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31283.org /out b31283.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31283.org /out b31283.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31283.reflect.xml
  if [ -f b31283.reflect.xml ]
then
    ReflectionRoots="-x b31283.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31283.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31283.asm.exe $CLRTestExecutionArguments
