...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b35354/b35354 b35354.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b35354/b35354 b35354.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b35354.dasm.il b35354.exe
    "$CORE_ROOT/ildasm" -raweh -out=b35354.dasm.il b35354.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b35354.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b35354.dasm.il
    "$CORE_ROOT/ilasm" -output=b35354.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b35354.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b35354
    if [ ! -f b35354.org ]; then
        TakeLock
        if [ ! -f b35354.org ]; then
          mkdir IL
          cp b35354.exe IL/b35354.exe
          mv b35354.exe b35354.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b35354.org /out b35354.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b35354.org /out b35354.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b35354.reflect.xml
  if [ -f b35354.reflect.xml ]
then
    ReflectionRoots="-x b35354.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b35354.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b35354.asm.exe $CLRTestExecutionArguments
