...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b77950\b77950 b77950.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b77950\b77950 b77950.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b77950.dasm.il b77950.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b77950.dasm.il b77950.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b77950.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b77950.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b77950.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b77950.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b77950
    if not exist "b77950.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b77950.org" (
            mkdir IL
            copy b77950.exe IL\b77950.exe 
            ren b77950.exe b77950.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b77950.org /out b77950.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b77950.org /out b77950.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b77950.reflect.xml
    IF EXIST b77950.reflect.xml set ReflectionRoots=-x b77950.reflect.xml
    ...
    set ExePath=%LinkBin%\b77950.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b77950.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b77950.asm.exe %CLRTestExecutionArguments% %Host_Args%
