...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b62498\b62498 b62498.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b62498\b62498 b62498.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b62498.dasm.il b62498.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b62498.dasm.il b62498.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b62498.asm.exe  b62498.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b62498.asm.exe  b62498.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b62498
    if not exist "b62498.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b62498.org" (
            mkdir IL
            copy b62498.exe IL\b62498.exe 
            ren b62498.exe b62498.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b62498.org /out b62498.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b62498.org /out b62498.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b62498.reflect.xml
    IF EXIST b62498.reflect.xml set ReflectionRoots=-x b62498.reflect.xml
    ...
    set ExePath=%LinkBin%\b62498.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b62498.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b62498.asm.exe %CLRTestExecutionArguments% %Host_Args%
