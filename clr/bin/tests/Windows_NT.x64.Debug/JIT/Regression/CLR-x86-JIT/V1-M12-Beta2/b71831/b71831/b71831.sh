...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71831/b71831 b71831.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b71831/b71831 b71831.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b71831.dasm.il b71831.exe
    "$CORE_ROOT/ildasm" -raweh -out=b71831.dasm.il b71831.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b71831.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b71831.dasm.il
    "$CORE_ROOT/ilasm" -output=b71831.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b71831.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b71831
    if [ ! -f b71831.org ]; then
        TakeLock
        if [ ! -f b71831.org ]; then
          mkdir IL
          cp b71831.exe IL/b71831.exe
          mv b71831.exe b71831.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71831.org /out b71831.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b71831.org /out b71831.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b71831.reflect.xml
  if [ -f b71831.reflect.xml ]
then
    ReflectionRoots="-x b71831.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b71831.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b71831.asm.exe $CLRTestExecutionArguments
