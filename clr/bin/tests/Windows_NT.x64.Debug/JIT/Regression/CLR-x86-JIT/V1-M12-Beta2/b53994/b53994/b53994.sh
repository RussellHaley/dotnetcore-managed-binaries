...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53994/b53994 b53994.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53994/b53994 b53994.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b53994.dasm.il b53994.exe
    "$CORE_ROOT/ildasm" -raweh -out=b53994.dasm.il b53994.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b53994.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53994.dasm.il
    "$CORE_ROOT/ilasm" -output=b53994.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53994.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b53994
    if [ ! -f b53994.org ]; then
        TakeLock
        if [ ! -f b53994.org ]; then
          mkdir IL
          cp b53994.exe IL/b53994.exe
          mv b53994.exe b53994.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53994.org /out b53994.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53994.org /out b53994.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b53994.reflect.xml
  if [ -f b53994.reflect.xml ]
then
    ReflectionRoots="-x b53994.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b53994.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b53994.asm.exe $CLRTestExecutionArguments
