...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53980/b53980 b53980.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53980/b53980 b53980.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b53980.dasm.il b53980.exe
    "$CORE_ROOT/ildasm" -raweh -out=b53980.dasm.il b53980.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b53980.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53980.dasm.il
    "$CORE_ROOT/ilasm" -output=b53980.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53980.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b53980
    if [ ! -f b53980.org ]; then
        TakeLock
        if [ ! -f b53980.org ]; then
          mkdir IL
          cp b53980.exe IL/b53980.exe
          mv b53980.exe b53980.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53980.org /out b53980.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53980.org /out b53980.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b53980.reflect.xml
  if [ -f b53980.reflect.xml ]
then
    ReflectionRoots="-x b53980.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b53980.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b53980.asm.exe $CLRTestExecutionArguments
