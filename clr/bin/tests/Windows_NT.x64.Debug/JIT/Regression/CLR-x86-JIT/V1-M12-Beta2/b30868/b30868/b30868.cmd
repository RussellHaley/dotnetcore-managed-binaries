...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b30868\b30868 b30868.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b30868\b30868 b30868.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b30868.dasm.il b30868.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b30868.dasm.il b30868.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b30868.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30868.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b30868.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30868.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b30868
    if not exist "b30868.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b30868.org" (
            mkdir IL
            copy b30868.exe IL\b30868.exe 
            ren b30868.exe b30868.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30868.org /out b30868.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30868.org /out b30868.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b30868.reflect.xml
    IF EXIST b30868.reflect.xml set ReflectionRoots=-x b30868.reflect.xml
    ...
    set ExePath=%LinkBin%\b30868.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b30868.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b30868.asm.exe %CLRTestExecutionArguments% %Host_Args%
