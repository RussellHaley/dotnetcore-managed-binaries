...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b62498/b62498 b62498.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b62498/b62498 b62498.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b62498.dasm.il b62498.exe
    "$CORE_ROOT/ildasm" -raweh -out=b62498.dasm.il b62498.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b62498.asm.exe  b62498.dasm.il
    "$CORE_ROOT/ilasm" -output=b62498.asm.exe  b62498.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b62498
    if [ ! -f b62498.org ]; then
        TakeLock
        if [ ! -f b62498.org ]; then
          mkdir IL
          cp b62498.exe IL/b62498.exe
          mv b62498.exe b62498.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b62498.org /out b62498.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b62498.org /out b62498.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b62498.reflect.xml
  if [ -f b62498.reflect.xml ]
then
    ReflectionRoots="-x b62498.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b62498.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b62498.asm.exe $CLRTestExecutionArguments
