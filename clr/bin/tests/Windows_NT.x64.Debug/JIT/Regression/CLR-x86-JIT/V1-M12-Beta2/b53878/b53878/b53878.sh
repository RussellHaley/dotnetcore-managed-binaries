...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53878/b53878 b53878.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53878/b53878 b53878.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b53878.dasm.il b53878.exe
    "$CORE_ROOT/ildasm" -raweh -out=b53878.dasm.il b53878.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b53878.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53878.dasm.il
    "$CORE_ROOT/ilasm" -output=b53878.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53878.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b53878
    if [ ! -f b53878.org ]; then
        TakeLock
        if [ ! -f b53878.org ]; then
          mkdir IL
          cp b53878.exe IL/b53878.exe
          mv b53878.exe b53878.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53878.org /out b53878.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53878.org /out b53878.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b53878.reflect.xml
  if [ -f b53878.reflect.xml ]
then
    ReflectionRoots="-x b53878.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b53878.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b53878.asm.exe $CLRTestExecutionArguments
