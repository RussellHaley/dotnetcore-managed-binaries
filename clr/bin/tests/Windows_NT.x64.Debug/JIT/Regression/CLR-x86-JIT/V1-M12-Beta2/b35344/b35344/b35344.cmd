...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b35344\b35344 b35344.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b35344\b35344 b35344.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b35344.dasm.il b35344.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b35344.dasm.il b35344.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b35344.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b35344.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b35344.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b35344.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b35344
    if not exist "b35344.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b35344.org" (
            mkdir IL
            copy b35344.exe IL\b35344.exe 
            ren b35344.exe b35344.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b35344.org /out b35344.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b35344.org /out b35344.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b35344.reflect.xml
    IF EXIST b35344.reflect.xml set ReflectionRoots=-x b35344.reflect.xml
    ...
    set ExePath=%LinkBin%\b35344.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b35344.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b35344.asm.exe %CLRTestExecutionArguments% %Host_Args%
