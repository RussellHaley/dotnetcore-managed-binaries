...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b84916/b84916 b84916.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b84916/b84916 b84916.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b84916.dasm.il b84916.exe
    "$CORE_ROOT/ildasm" -raweh -out=b84916.dasm.il b84916.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b84916.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84916.dasm.il
    "$CORE_ROOT/ilasm" -output=b84916.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b84916.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b84916
    if [ ! -f b84916.org ]; then
        TakeLock
        if [ ! -f b84916.org ]; then
          mkdir IL
          cp b84916.exe IL/b84916.exe
          mv b84916.exe b84916.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b84916.org /out b84916.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b84916.org /out b84916.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b84916.reflect.xml
  if [ -f b84916.reflect.xml ]
then
    ReflectionRoots="-x b84916.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b84916.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b84916.asm.exe $CLRTestExecutionArguments
