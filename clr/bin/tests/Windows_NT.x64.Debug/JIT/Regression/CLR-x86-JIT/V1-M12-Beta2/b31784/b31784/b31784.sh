...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31784/b31784 b31784.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b31784/b31784 b31784.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31784.dasm.il b31784.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31784.dasm.il b31784.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31784.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31784.dasm.il
    "$CORE_ROOT/ilasm" -output=b31784.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31784.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31784
    if [ ! -f b31784.org ]; then
        TakeLock
        if [ ! -f b31784.org ]; then
          mkdir IL
          cp b31784.exe IL/b31784.exe
          mv b31784.exe b31784.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31784.org /out b31784.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31784.org /out b31784.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31784.reflect.xml
  if [ -f b31784.reflect.xml ]
then
    ReflectionRoots="-x b31784.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31784.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31784.asm.exe $CLRTestExecutionArguments
