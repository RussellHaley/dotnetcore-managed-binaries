...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53884/b53884 b53884.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b53884/b53884 b53884.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b53884.dasm.il b53884.exe
    "$CORE_ROOT/ildasm" -raweh -out=b53884.dasm.il b53884.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b53884.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53884.dasm.il
    "$CORE_ROOT/ilasm" -output=b53884.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b53884.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b53884
    if [ ! -f b53884.org ]; then
        TakeLock
        if [ ! -f b53884.org ]; then
          mkdir IL
          cp b53884.exe IL/b53884.exe
          mv b53884.exe b53884.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53884.org /out b53884.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b53884.org /out b53884.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b53884.reflect.xml
  if [ -f b53884.reflect.xml ]
then
    ReflectionRoots="-x b53884.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b53884.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b53884.asm.exe $CLRTestExecutionArguments
