...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b69227\b69227 b69227.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M12-Beta2\b69227\b69227 b69227.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b69227.dasm.il b69227.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b69227.dasm.il b69227.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b69227.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69227.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b69227.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b69227.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b69227
    if not exist "b69227.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b69227.org" (
            mkdir IL
            copy b69227.exe IL\b69227.exe 
            ren b69227.exe b69227.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b69227.org /out b69227.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b69227.org /out b69227.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b69227.reflect.xml
    IF EXIST b69227.reflect.xml set ReflectionRoots=-x b69227.reflect.xml
    ...
    set ExePath=%LinkBin%\b69227.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b69227.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b69227.asm.exe %CLRTestExecutionArguments% %Host_Args%
