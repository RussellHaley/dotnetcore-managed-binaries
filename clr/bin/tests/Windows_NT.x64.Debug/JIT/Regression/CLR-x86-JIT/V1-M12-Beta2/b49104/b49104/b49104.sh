...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b49104/b49104 b49104.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M12-Beta2/b49104/b49104 b49104.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b49104.dasm.il b49104.exe
    "$CORE_ROOT/ildasm" -raweh -out=b49104.dasm.il b49104.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b49104.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b49104.dasm.il
    "$CORE_ROOT/ilasm" -output=b49104.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b49104.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b49104
    if [ ! -f b49104.org ]; then
        TakeLock
        if [ ! -f b49104.org ]; then
          mkdir IL
          cp b49104.exe IL/b49104.exe
          mv b49104.exe b49104.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b49104.org /out b49104.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b49104.org /out b49104.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b49104.reflect.xml
  if [ -f b49104.reflect.xml ]
then
    ReflectionRoots="-x b49104.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b49104.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b49104.asm.exe $CLRTestExecutionArguments
