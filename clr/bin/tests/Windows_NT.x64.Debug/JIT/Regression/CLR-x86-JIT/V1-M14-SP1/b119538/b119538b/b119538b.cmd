...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M14-SP1\b119538\b119538b b119538b.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M14-SP1\b119538\b119538b b119538b.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b119538b.dasm.il b119538b.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b119538b.dasm.il b119538b.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b119538b.asm.exe  b119538b.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b119538b.asm.exe  b119538b.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b119538b
    if not exist "b119538b.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b119538b.org" (
            mkdir IL
            copy b119538b.exe IL\b119538b.exe 
            ren b119538b.exe b119538b.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b119538b.org /out b119538b.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b119538b.org /out b119538b.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b119538b.reflect.xml
    IF EXIST b119538b.reflect.xml set ReflectionRoots=-x b119538b.reflect.xml
    ...
    set ExePath=%LinkBin%\b119538b.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b119538b.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b119538b.asm.exe %CLRTestExecutionArguments% %Host_Args%
