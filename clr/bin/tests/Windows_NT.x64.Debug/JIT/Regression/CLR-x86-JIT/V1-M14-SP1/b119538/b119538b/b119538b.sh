...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M14-SP1/b119538/b119538b b119538b.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M14-SP1/b119538/b119538b b119538b.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b119538b.dasm.il b119538b.exe
    "$CORE_ROOT/ildasm" -raweh -out=b119538b.dasm.il b119538b.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b119538b.asm.exe  b119538b.dasm.il
    "$CORE_ROOT/ilasm" -output=b119538b.asm.exe  b119538b.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b119538b
    if [ ! -f b119538b.org ]; then
        TakeLock
        if [ ! -f b119538b.org ]; then
          mkdir IL
          cp b119538b.exe IL/b119538b.exe
          mv b119538b.exe b119538b.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b119538b.org /out b119538b.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b119538b.org /out b119538b.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b119538b.reflect.xml
  if [ -f b119538b.reflect.xml ]
then
    ReflectionRoots="-x b119538b.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b119538b.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b119538b.asm.exe $CLRTestExecutionArguments
