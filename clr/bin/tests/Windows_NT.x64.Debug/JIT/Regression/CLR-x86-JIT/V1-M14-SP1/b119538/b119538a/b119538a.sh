...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M14-SP1/b119538/b119538a b119538a.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M14-SP1/b119538/b119538a b119538a.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b119538a.dasm.il b119538a.exe
    "$CORE_ROOT/ildasm" -raweh -out=b119538a.dasm.il b119538a.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b119538a.asm.exe  b119538a.dasm.il
    "$CORE_ROOT/ilasm" -output=b119538a.asm.exe  b119538a.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b119538a
    if [ ! -f b119538a.org ]; then
        TakeLock
        if [ ! -f b119538a.org ]; then
          mkdir IL
          cp b119538a.exe IL/b119538a.exe
          mv b119538a.exe b119538a.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b119538a.org /out b119538a.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b119538a.org /out b119538a.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b119538a.reflect.xml
  if [ -f b119538a.reflect.xml ]
then
    ReflectionRoots="-x b119538a.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b119538a.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b119538a.asm.exe $CLRTestExecutionArguments
