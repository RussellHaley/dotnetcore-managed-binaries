...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09\b13621\b13621 b13621.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09\b13621\b13621 b13621.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b13621.dasm.il b13621.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b13621.dasm.il b13621.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b13621.asm.exe  b13621.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b13621.asm.exe  b13621.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b13621
    if not exist "b13621.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b13621.org" (
            mkdir IL
            copy b13621.exe IL\b13621.exe 
            ren b13621.exe b13621.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b13621.org /out b13621.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b13621.org /out b13621.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b13621.reflect.xml
    IF EXIST b13621.reflect.xml set ReflectionRoots=-x b13621.reflect.xml
    ...
    set ExePath=%LinkBin%\b13621.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b13621.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b13621.asm.exe %CLRTestExecutionArguments% %Host_Args%
