...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09/b16294/b16294 b16294.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09/b16294/b16294 b16294.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16294.dasm.il b16294.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16294.dasm.il b16294.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16294.asm.exe  b16294.dasm.il
    "$CORE_ROOT/ilasm" -output=b16294.asm.exe  b16294.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16294
    if [ ! -f b16294.org ]; then
        TakeLock
        if [ ! -f b16294.org ]; then
          mkdir IL
          cp b16294.exe IL/b16294.exe
          mv b16294.exe b16294.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16294.org /out b16294.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16294.org /out b16294.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16294.reflect.xml
  if [ -f b16294.reflect.xml ]
then
    ReflectionRoots="-x b16294.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16294.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16294.asm.exe $CLRTestExecutionArguments
