...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09\b13647\b13647 b13647.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09\b13647\b13647 b13647.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b13647.dasm.il b13647.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b13647.dasm.il b13647.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b13647.asm.exe  b13647.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b13647.asm.exe  b13647.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b13647
    if not exist "b13647.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b13647.org" (
            mkdir IL
            copy b13647.exe IL\b13647.exe 
            ren b13647.exe b13647.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b13647.org /out b13647.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b13647.org /out b13647.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b13647.reflect.xml
    IF EXIST b13647.reflect.xml set ReflectionRoots=-x b13647.reflect.xml
    ...
    set ExePath=%LinkBin%\b13647.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b13647.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b13647.asm.exe %CLRTestExecutionArguments% %Host_Args%
