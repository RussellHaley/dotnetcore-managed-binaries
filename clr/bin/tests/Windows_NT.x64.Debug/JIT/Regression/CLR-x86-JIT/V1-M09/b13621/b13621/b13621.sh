...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09/b13621/b13621 b13621.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09/b13621/b13621 b13621.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b13621.dasm.il b13621.exe
    "$CORE_ROOT/ildasm" -raweh -out=b13621.dasm.il b13621.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b13621.asm.exe  b13621.dasm.il
    "$CORE_ROOT/ilasm" -output=b13621.asm.exe  b13621.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b13621
    if [ ! -f b13621.org ]; then
        TakeLock
        if [ ! -f b13621.org ]; then
          mkdir IL
          cp b13621.exe IL/b13621.exe
          mv b13621.exe b13621.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b13621.org /out b13621.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b13621.org /out b13621.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b13621.reflect.xml
  if [ -f b13621.reflect.xml ]
then
    ReflectionRoots="-x b13621.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b13621.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b13621.asm.exe $CLRTestExecutionArguments
