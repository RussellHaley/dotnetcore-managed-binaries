...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09/b15864/b15864 b15864.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09/b15864/b15864 b15864.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b15864.dasm.il b15864.exe
    "$CORE_ROOT/ildasm" -raweh -out=b15864.dasm.il b15864.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b15864.asm.exe  b15864.dasm.il
    "$CORE_ROOT/ilasm" -output=b15864.asm.exe  b15864.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b15864
    if [ ! -f b15864.org ]; then
        TakeLock
        if [ ! -f b15864.org ]; then
          mkdir IL
          cp b15864.exe IL/b15864.exe
          mv b15864.exe b15864.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b15864.org /out b15864.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b15864.org /out b15864.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b15864.reflect.xml
  if [ -f b15864.reflect.xml ]
then
    ReflectionRoots="-x b15864.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b15864.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b15864.asm.exe $CLRTestExecutionArguments
