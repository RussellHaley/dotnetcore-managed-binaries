...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09\b15864\b15864 b15864.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09\b15864\b15864 b15864.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b15864.dasm.il b15864.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b15864.dasm.il b15864.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b15864.asm.exe  b15864.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b15864.asm.exe  b15864.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b15864
    if not exist "b15864.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b15864.org" (
            mkdir IL
            copy b15864.exe IL\b15864.exe 
            ren b15864.exe b15864.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b15864.org /out b15864.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b15864.org /out b15864.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b15864.reflect.xml
    IF EXIST b15864.reflect.xml set ReflectionRoots=-x b15864.reflect.xml
    ...
    set ExePath=%LinkBin%\b15864.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b15864.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b15864.asm.exe %CLRTestExecutionArguments% %Host_Args%
