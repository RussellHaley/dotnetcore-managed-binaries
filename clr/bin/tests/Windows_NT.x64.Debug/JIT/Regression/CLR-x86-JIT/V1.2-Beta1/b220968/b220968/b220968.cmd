...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b220968\b220968 b220968.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b220968\b220968 b220968.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b220968.dasm.il b220968.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b220968.dasm.il b220968.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b220968.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b220968.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b220968.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b220968.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b220968
    if not exist "b220968.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b220968.org" (
            mkdir IL
            copy b220968.exe IL\b220968.exe 
            ren b220968.exe b220968.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b220968.org /out b220968.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b220968.org /out b220968.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b220968.reflect.xml
    IF EXIST b220968.reflect.xml set ReflectionRoots=-x b220968.reflect.xml
    ...
    set ExePath=%LinkBin%\b220968.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b220968.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b220968.asm.exe %CLRTestExecutionArguments% %Host_Args%
