...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b223936/b223936 b223936.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b223936/b223936 b223936.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b223936.dasm.il b223936.exe
    "$CORE_ROOT/ildasm" -raweh -out=b223936.dasm.il b223936.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b223936.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b223936.dasm.il
    "$CORE_ROOT/ilasm" -output=b223936.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b223936.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b223936
    if [ ! -f b223936.org ]; then
        TakeLock
        if [ ! -f b223936.org ]; then
          mkdir IL
          cp b223936.exe IL/b223936.exe
          mv b223936.exe b223936.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b223936.org /out b223936.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b223936.org /out b223936.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b223936.reflect.xml
  if [ -f b223936.reflect.xml ]
then
    ReflectionRoots="-x b223936.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b223936.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b223936.asm.exe $CLRTestExecutionArguments
