...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b223924/b223924 b223924.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b223924/b223924 b223924.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b223924.dasm.il b223924.exe
    "$CORE_ROOT/ildasm" -raweh -out=b223924.dasm.il b223924.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b223924.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b223924.dasm.il
    "$CORE_ROOT/ilasm" -output=b223924.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b223924.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b223924
    if [ ! -f b223924.org ]; then
        TakeLock
        if [ ! -f b223924.org ]; then
          mkdir IL
          cp b223924.exe IL/b223924.exe
          mv b223924.exe b223924.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b223924.org /out b223924.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b223924.org /out b223924.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b223924.reflect.xml
  if [ -f b223924.reflect.xml ]
then
    ReflectionRoots="-x b223924.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b223924.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b223924.asm.exe $CLRTestExecutionArguments
