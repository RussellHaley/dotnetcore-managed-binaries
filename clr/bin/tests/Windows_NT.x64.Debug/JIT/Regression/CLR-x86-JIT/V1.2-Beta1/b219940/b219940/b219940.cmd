...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b219940\b219940 b219940.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b219940\b219940 b219940.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b219940.dasm.il b219940.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b219940.dasm.il b219940.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b219940.asm.exe  b219940.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b219940.asm.exe  b219940.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b219940
    if not exist "b219940.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b219940.org" (
            mkdir IL
            copy b219940.exe IL\b219940.exe 
            ren b219940.exe b219940.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b219940.org /out b219940.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b219940.org /out b219940.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b219940.reflect.xml
    IF EXIST b219940.reflect.xml set ReflectionRoots=-x b219940.reflect.xml
    ...
    set ExePath=%LinkBin%\b219940.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b219940.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b219940.asm.exe %CLRTestExecutionArguments% %Host_Args%
