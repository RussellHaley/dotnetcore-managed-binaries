...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b219940/b219940 b219940.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b219940/b219940 b219940.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b219940.dasm.il b219940.exe
    "$CORE_ROOT/ildasm" -raweh -out=b219940.dasm.il b219940.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b219940.asm.exe  b219940.dasm.il
    "$CORE_ROOT/ilasm" -output=b219940.asm.exe  b219940.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b219940
    if [ ! -f b219940.org ]; then
        TakeLock
        if [ ! -f b219940.org ]; then
          mkdir IL
          cp b219940.exe IL/b219940.exe
          mv b219940.exe b219940.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b219940.org /out b219940.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b219940.org /out b219940.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b219940.reflect.xml
  if [ -f b219940.reflect.xml ]
then
    ReflectionRoots="-x b219940.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b219940.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b219940.asm.exe $CLRTestExecutionArguments
