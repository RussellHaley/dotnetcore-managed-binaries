...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b223936\b223936 b223936.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b223936\b223936 b223936.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b223936.dasm.il b223936.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b223936.dasm.il b223936.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b223936.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b223936.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b223936.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b223936.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b223936
    if not exist "b223936.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b223936.org" (
            mkdir IL
            copy b223936.exe IL\b223936.exe 
            ren b223936.exe b223936.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b223936.org /out b223936.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b223936.org /out b223936.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b223936.reflect.xml
    IF EXIST b223936.reflect.xml set ReflectionRoots=-x b223936.reflect.xml
    ...
    set ExePath=%LinkBin%\b223936.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b223936.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b223936.asm.exe %CLRTestExecutionArguments% %Host_Args%
