...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b220968/b220968 b220968.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b220968/b220968 b220968.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b220968.dasm.il b220968.exe
    "$CORE_ROOT/ildasm" -raweh -out=b220968.dasm.il b220968.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b220968.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b220968.dasm.il
    "$CORE_ROOT/ilasm" -output=b220968.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b220968.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b220968
    if [ ! -f b220968.org ]; then
        TakeLock
        if [ ! -f b220968.org ]; then
          mkdir IL
          cp b220968.exe IL/b220968.exe
          mv b220968.exe b220968.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b220968.org /out b220968.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b220968.org /out b220968.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b220968.reflect.xml
  if [ -f b220968.reflect.xml ]
then
    ReflectionRoots="-x b220968.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b220968.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b220968.asm.exe $CLRTestExecutionArguments
