...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b147147/b147147 b147147.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b147147/b147147 b147147.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b147147.dasm.il b147147.exe
    "$CORE_ROOT/ildasm" -raweh -out=b147147.dasm.il b147147.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b147147.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b147147.dasm.il
    "$CORE_ROOT/ilasm" -output=b147147.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b147147.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b147147
    if [ ! -f b147147.org ]; then
        TakeLock
        if [ ! -f b147147.org ]; then
          mkdir IL
          cp b147147.exe IL/b147147.exe
          mv b147147.exe b147147.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b147147.org /out b147147.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b147147.org /out b147147.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b147147.reflect.xml
  if [ -f b147147.reflect.xml ]
then
    ReflectionRoots="-x b147147.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b147147.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b147147.asm.exe $CLRTestExecutionArguments
