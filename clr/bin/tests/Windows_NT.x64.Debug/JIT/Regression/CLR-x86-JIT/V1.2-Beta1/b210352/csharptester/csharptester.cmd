...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b210352\csharptester csharptester.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b210352\csharptester csharptester.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=csharptester.dasm.il csharptester.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=csharptester.dasm.il csharptester.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=csharptester.asm.exe  csharptester.dasm.il
    %CORE_ROOT%\ilasm.exe /output=csharptester.asm.exe  csharptester.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=csharptester
    if not exist "csharptester.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "csharptester.org" (
            mkdir IL
            copy csharptester.exe IL\csharptester.exe 
            ren csharptester.exe csharptester.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in csharptester.org /out csharptester.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in csharptester.org /out csharptester.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in csharptester.reflect.xml
    IF EXIST csharptester.reflect.xml set ReflectionRoots=-x csharptester.reflect.xml
    ...
    set ExePath=%LinkBin%\csharptester.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% csharptester.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% csharptester.asm.exe %CLRTestExecutionArguments% %Host_Args%
