...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b223924\b223924 b223924.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-Beta1\b223924\b223924 b223924.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b223924.dasm.il b223924.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b223924.dasm.il b223924.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b223924.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b223924.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b223924.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b223924.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b223924
    if not exist "b223924.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b223924.org" (
            mkdir IL
            copy b223924.exe IL\b223924.exe 
            ren b223924.exe b223924.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b223924.org /out b223924.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b223924.org /out b223924.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b223924.reflect.xml
    IF EXIST b223924.reflect.xml set ReflectionRoots=-x b223924.reflect.xml
    ...
    set ExePath=%LinkBin%\b223924.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b223924.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b223924.asm.exe %CLRTestExecutionArguments% %Host_Args%
