...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b210352/csharptester csharptester.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-Beta1/b210352/csharptester csharptester.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=csharptester.dasm.il csharptester.exe
    "$CORE_ROOT/ildasm" -raweh -out=csharptester.dasm.il csharptester.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=csharptester.asm.exe  csharptester.dasm.il
    "$CORE_ROOT/ilasm" -output=csharptester.asm.exe  csharptester.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=csharptester
    if [ ! -f csharptester.org ]; then
        TakeLock
        if [ ! -f csharptester.org ]; then
          mkdir IL
          cp csharptester.exe IL/csharptester.exe
          mv csharptester.exe csharptester.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in csharptester.org /out csharptester.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in csharptester.org /out csharptester.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in csharptester.reflect.xml
  if [ -f csharptester.reflect.xml ]
then
    ReflectionRoots="-x csharptester.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/csharptester.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" csharptester.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" csharptester.asm.exe $CLRTestExecutionArguments
