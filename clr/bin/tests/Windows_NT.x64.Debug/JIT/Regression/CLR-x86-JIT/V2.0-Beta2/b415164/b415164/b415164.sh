...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b415164/b415164 b415164.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b415164/b415164 b415164.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b415164.dasm.il b415164.exe
    "$CORE_ROOT/ildasm" -raweh -out=b415164.dasm.il b415164.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b415164.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b415164.dasm.il
    "$CORE_ROOT/ilasm" -output=b415164.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b415164.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b415164
    if [ ! -f b415164.org ]; then
        TakeLock
        if [ ! -f b415164.org ]; then
          mkdir IL
          cp b415164.exe IL/b415164.exe
          mv b415164.exe b415164.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b415164.org /out b415164.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b415164.org /out b415164.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b415164.reflect.xml
  if [ -f b415164.reflect.xml ]
then
    ReflectionRoots="-x b415164.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b415164.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b415164.asm.exe $CLRTestExecutionArguments
