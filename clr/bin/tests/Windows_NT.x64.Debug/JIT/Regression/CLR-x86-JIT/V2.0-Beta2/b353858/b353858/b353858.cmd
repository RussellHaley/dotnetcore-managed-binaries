...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b353858\b353858 b353858.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b353858\b353858 b353858.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b353858.dasm.il b353858.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b353858.dasm.il b353858.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b353858.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b353858.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b353858.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b353858.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b353858
    if not exist "b353858.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b353858.org" (
            mkdir IL
            copy b353858.exe IL\b353858.exe 
            ren b353858.exe b353858.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b353858.org /out b353858.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b353858.org /out b353858.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b353858.reflect.xml
    IF EXIST b353858.reflect.xml set ReflectionRoots=-x b353858.reflect.xml
    ...
    set ExePath=%LinkBin%\b353858.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b353858.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b353858.asm.exe %CLRTestExecutionArguments% %Host_Args%
