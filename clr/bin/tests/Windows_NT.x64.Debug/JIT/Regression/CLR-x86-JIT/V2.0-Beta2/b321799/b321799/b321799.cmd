...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b321799\b321799 b321799.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b321799\b321799 b321799.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b321799.dasm.il b321799.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b321799.dasm.il b321799.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b321799.asm.exe  b321799.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b321799.asm.exe  b321799.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b321799
    if not exist "b321799.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b321799.org" (
            mkdir IL
            copy b321799.exe IL\b321799.exe 
            ren b321799.exe b321799.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b321799.org /out b321799.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b321799.org /out b321799.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b321799.reflect.xml
    IF EXIST b321799.reflect.xml set ReflectionRoots=-x b321799.reflect.xml
    ...
    set ExePath=%LinkBin%\b321799.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b321799.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b321799.asm.exe %CLRTestExecutionArguments% %Host_Args%
