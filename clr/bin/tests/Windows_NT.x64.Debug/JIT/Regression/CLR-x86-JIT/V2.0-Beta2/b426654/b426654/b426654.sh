...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b426654/b426654 b426654.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b426654/b426654 b426654.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b426654.dasm.il b426654.exe
    "$CORE_ROOT/ildasm" -raweh -out=b426654.dasm.il b426654.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b426654.asm.exe -QUIET -NOLOGO -DEBUG b426654.dasm.il
    "$CORE_ROOT/ilasm" -output=b426654.asm.exe -QUIET -NOLOGO -DEBUG b426654.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b426654
    if [ ! -f b426654.org ]; then
        TakeLock
        if [ ! -f b426654.org ]; then
          mkdir IL
          cp b426654.exe IL/b426654.exe
          mv b426654.exe b426654.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b426654.org /out b426654.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b426654.org /out b426654.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b426654.reflect.xml
  if [ -f b426654.reflect.xml ]
then
    ReflectionRoots="-x b426654.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b426654.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b426654.asm.exe $CLRTestExecutionArguments
