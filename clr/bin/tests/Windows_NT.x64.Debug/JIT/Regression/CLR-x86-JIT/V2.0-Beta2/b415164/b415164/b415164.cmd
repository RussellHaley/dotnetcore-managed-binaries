...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b415164\b415164 b415164.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b415164\b415164 b415164.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b415164.dasm.il b415164.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b415164.dasm.il b415164.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b415164.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b415164.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b415164.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b415164.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b415164
    if not exist "b415164.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b415164.org" (
            mkdir IL
            copy b415164.exe IL\b415164.exe 
            ren b415164.exe b415164.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b415164.org /out b415164.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b415164.org /out b415164.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b415164.reflect.xml
    IF EXIST b415164.reflect.xml set ReflectionRoots=-x b415164.reflect.xml
    ...
    set ExePath=%LinkBin%\b415164.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b415164.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b415164.asm.exe %CLRTestExecutionArguments% %Host_Args%
