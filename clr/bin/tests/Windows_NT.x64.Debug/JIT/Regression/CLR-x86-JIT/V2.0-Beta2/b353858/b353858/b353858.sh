...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b353858/b353858 b353858.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b353858/b353858 b353858.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b353858.dasm.il b353858.exe
    "$CORE_ROOT/ildasm" -raweh -out=b353858.dasm.il b353858.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b353858.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b353858.dasm.il
    "$CORE_ROOT/ilasm" -output=b353858.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b353858.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b353858
    if [ ! -f b353858.org ]; then
        TakeLock
        if [ ! -f b353858.org ]; then
          mkdir IL
          cp b353858.exe IL/b353858.exe
          mv b353858.exe b353858.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b353858.org /out b353858.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b353858.org /out b353858.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b353858.reflect.xml
  if [ -f b353858.reflect.xml ]
then
    ReflectionRoots="-x b353858.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b353858.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b353858.asm.exe $CLRTestExecutionArguments
