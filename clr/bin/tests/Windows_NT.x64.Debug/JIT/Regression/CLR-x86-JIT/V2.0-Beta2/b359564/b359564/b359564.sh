...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b359564/b359564 b359564.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b359564/b359564 b359564.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b359564.dasm.il b359564.exe
    "$CORE_ROOT/ildasm" -raweh -out=b359564.dasm.il b359564.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b359564.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b359564.dasm.il
    "$CORE_ROOT/ilasm" -output=b359564.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b359564.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b359564
    if [ ! -f b359564.org ]; then
        TakeLock
        if [ ! -f b359564.org ]; then
          mkdir IL
          cp b359564.exe IL/b359564.exe
          mv b359564.exe b359564.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b359564.org /out b359564.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b359564.org /out b359564.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b359564.reflect.xml
  if [ -f b359564.reflect.xml ]
then
    ReflectionRoots="-x b359564.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b359564.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b359564.asm.exe $CLRTestExecutionArguments
