...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b374944/b374944 b374944.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b374944/b374944 b374944.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b374944.dasm.il b374944.exe
    "$CORE_ROOT/ildasm" -raweh -out=b374944.dasm.il b374944.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b374944.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b374944.dasm.il
    "$CORE_ROOT/ilasm" -output=b374944.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b374944.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b374944
    if [ ! -f b374944.org ]; then
        TakeLock
        if [ ! -f b374944.org ]; then
          mkdir IL
          cp b374944.exe IL/b374944.exe
          mv b374944.exe b374944.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b374944.org /out b374944.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b374944.org /out b374944.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b374944.reflect.xml
  if [ -f b374944.reflect.xml ]
then
    ReflectionRoots="-x b374944.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b374944.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b374944.asm.exe $CLRTestExecutionArguments
