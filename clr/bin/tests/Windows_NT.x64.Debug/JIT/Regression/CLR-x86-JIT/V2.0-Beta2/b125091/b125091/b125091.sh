...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b125091/b125091 b125091.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b125091/b125091 b125091.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b125091.dasm.il b125091.exe
    "$CORE_ROOT/ildasm" -raweh -out=b125091.dasm.il b125091.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b125091.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b125091.dasm.il
    "$CORE_ROOT/ilasm" -output=b125091.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b125091.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b125091
    if [ ! -f b125091.org ]; then
        TakeLock
        if [ ! -f b125091.org ]; then
          mkdir IL
          cp b125091.exe IL/b125091.exe
          mv b125091.exe b125091.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b125091.org /out b125091.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b125091.org /out b125091.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b125091.reflect.xml
  if [ -f b125091.reflect.xml ]
then
    ReflectionRoots="-x b125091.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b125091.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b125091.asm.exe $CLRTestExecutionArguments
