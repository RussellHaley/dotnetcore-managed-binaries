...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b441487/b441487 b441487.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b441487/b441487 b441487.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b441487.dasm.il b441487.exe
    "$CORE_ROOT/ildasm" -raweh -out=b441487.dasm.il b441487.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b441487.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b441487.dasm.il
    "$CORE_ROOT/ilasm" -output=b441487.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b441487.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b441487
    if [ ! -f b441487.org ]; then
        TakeLock
        if [ ! -f b441487.org ]; then
          mkdir IL
          cp b441487.exe IL/b441487.exe
          mv b441487.exe b441487.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b441487.org /out b441487.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b441487.org /out b441487.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b441487.reflect.xml
  if [ -f b441487.reflect.xml ]
then
    ReflectionRoots="-x b441487.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b441487.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b441487.asm.exe $CLRTestExecutionArguments
