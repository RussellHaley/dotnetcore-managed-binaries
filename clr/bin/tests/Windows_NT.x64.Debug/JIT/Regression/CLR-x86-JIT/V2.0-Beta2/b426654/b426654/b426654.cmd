...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b426654\b426654 b426654.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b426654\b426654 b426654.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b426654.dasm.il b426654.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b426654.dasm.il b426654.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b426654.asm.exe -QUIET -NOLOGO -DEBUG b426654.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b426654.asm.exe -QUIET -NOLOGO -DEBUG b426654.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b426654
    if not exist "b426654.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b426654.org" (
            mkdir IL
            copy b426654.exe IL\b426654.exe 
            ren b426654.exe b426654.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b426654.org /out b426654.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b426654.org /out b426654.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b426654.reflect.xml
    IF EXIST b426654.reflect.xml set ReflectionRoots=-x b426654.reflect.xml
    ...
    set ExePath=%LinkBin%\b426654.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b426654.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b426654.asm.exe %CLRTestExecutionArguments% %Host_Args%
