...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b091942\b091942 b091942.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b091942\b091942 b091942.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b091942.dasm.il b091942.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b091942.dasm.il b091942.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b091942.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b091942.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b091942.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b091942.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b091942
    if not exist "b091942.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b091942.org" (
            mkdir IL
            copy b091942.exe IL\b091942.exe 
            ren b091942.exe b091942.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b091942.org /out b091942.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b091942.org /out b091942.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b091942.reflect.xml
    IF EXIST b091942.reflect.xml set ReflectionRoots=-x b091942.reflect.xml
    ...
    set ExePath=%LinkBin%\b091942.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b091942.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b091942.asm.exe %CLRTestExecutionArguments% %Host_Args%
