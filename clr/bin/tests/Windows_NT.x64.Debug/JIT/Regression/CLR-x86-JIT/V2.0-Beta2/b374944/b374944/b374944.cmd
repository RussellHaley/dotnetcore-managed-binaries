...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b374944\b374944 b374944.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b374944\b374944 b374944.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b374944.dasm.il b374944.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b374944.dasm.il b374944.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b374944.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b374944.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b374944.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b374944.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b374944
    if not exist "b374944.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b374944.org" (
            mkdir IL
            copy b374944.exe IL\b374944.exe 
            ren b374944.exe b374944.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b374944.org /out b374944.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b374944.org /out b374944.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b374944.reflect.xml
    IF EXIST b374944.reflect.xml set ReflectionRoots=-x b374944.reflect.xml
    ...
    set ExePath=%LinkBin%\b374944.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b374944.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b374944.asm.exe %CLRTestExecutionArguments% %Host_Args%
