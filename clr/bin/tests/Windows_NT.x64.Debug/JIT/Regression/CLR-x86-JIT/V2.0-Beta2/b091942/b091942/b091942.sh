...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b091942/b091942 b091942.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b091942/b091942 b091942.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b091942.dasm.il b091942.exe
    "$CORE_ROOT/ildasm" -raweh -out=b091942.dasm.il b091942.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b091942.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b091942.dasm.il
    "$CORE_ROOT/ilasm" -output=b091942.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b091942.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b091942
    if [ ! -f b091942.org ]; then
        TakeLock
        if [ ! -f b091942.org ]; then
          mkdir IL
          cp b091942.exe IL/b091942.exe
          mv b091942.exe b091942.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b091942.org /out b091942.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b091942.org /out b091942.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b091942.reflect.xml
  if [ -f b091942.reflect.xml ]
then
    ReflectionRoots="-x b091942.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b091942.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b091942.asm.exe $CLRTestExecutionArguments
