...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b425314\b425314 b425314.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b425314\b425314 b425314.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b425314.dasm.il b425314.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b425314.dasm.il b425314.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b425314.asm.exe  b425314.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b425314.asm.exe  b425314.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b425314
    if not exist "b425314.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b425314.org" (
            mkdir IL
            copy b425314.exe IL\b425314.exe 
            ren b425314.exe b425314.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b425314.org /out b425314.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b425314.org /out b425314.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b425314.reflect.xml
    IF EXIST b425314.reflect.xml set ReflectionRoots=-x b425314.reflect.xml
    ...
    set ExePath=%LinkBin%\b425314.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b425314.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b425314.asm.exe %CLRTestExecutionArguments% %Host_Args%
