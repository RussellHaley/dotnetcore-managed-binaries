...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b425314/b425314 b425314.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b425314/b425314 b425314.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b425314.dasm.il b425314.exe
    "$CORE_ROOT/ildasm" -raweh -out=b425314.dasm.il b425314.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b425314.asm.exe  b425314.dasm.il
    "$CORE_ROOT/ilasm" -output=b425314.asm.exe  b425314.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b425314
    if [ ! -f b425314.org ]; then
        TakeLock
        if [ ! -f b425314.org ]; then
          mkdir IL
          cp b425314.exe IL/b425314.exe
          mv b425314.exe b425314.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b425314.org /out b425314.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b425314.org /out b425314.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b425314.reflect.xml
  if [ -f b425314.reflect.xml ]
then
    ReflectionRoots="-x b425314.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b425314.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b425314.asm.exe $CLRTestExecutionArguments
