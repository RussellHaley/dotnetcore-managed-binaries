...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b321799/b321799 b321799.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-Beta2/b321799/b321799 b321799.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b321799.dasm.il b321799.exe
    "$CORE_ROOT/ildasm" -raweh -out=b321799.dasm.il b321799.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b321799.asm.exe  b321799.dasm.il
    "$CORE_ROOT/ilasm" -output=b321799.asm.exe  b321799.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b321799
    if [ ! -f b321799.org ]; then
        TakeLock
        if [ ! -f b321799.org ]; then
          mkdir IL
          cp b321799.exe IL/b321799.exe
          mv b321799.exe b321799.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b321799.org /out b321799.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b321799.org /out b321799.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b321799.reflect.xml
  if [ -f b321799.reflect.xml ]
then
    ReflectionRoots="-x b321799.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b321799.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b321799.asm.exe $CLRTestExecutionArguments
