...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b125091\b125091 b125091.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b125091\b125091 b125091.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b125091.dasm.il b125091.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b125091.dasm.il b125091.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b125091.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b125091.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b125091.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b125091.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b125091
    if not exist "b125091.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b125091.org" (
            mkdir IL
            copy b125091.exe IL\b125091.exe 
            ren b125091.exe b125091.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b125091.org /out b125091.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b125091.org /out b125091.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b125091.reflect.xml
    IF EXIST b125091.reflect.xml set ReflectionRoots=-x b125091.reflect.xml
    ...
    set ExePath=%LinkBin%\b125091.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b125091.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b125091.asm.exe %CLRTestExecutionArguments% %Host_Args%
