...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b441487\b441487 b441487.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-Beta2\b441487\b441487 b441487.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b441487.dasm.il b441487.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b441487.dasm.il b441487.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b441487.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b441487.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b441487.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b441487.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b441487
    if not exist "b441487.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b441487.org" (
            mkdir IL
            copy b441487.exe IL\b441487.exe 
            ren b441487.exe b441487.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b441487.org /out b441487.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b441487.org /out b441487.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b441487.reflect.xml
    IF EXIST b441487.reflect.xml set ReflectionRoots=-x b441487.reflect.xml
    ...
    set ExePath=%LinkBin%\b441487.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b441487.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b441487.asm.exe %CLRTestExecutionArguments% %Host_Args%
