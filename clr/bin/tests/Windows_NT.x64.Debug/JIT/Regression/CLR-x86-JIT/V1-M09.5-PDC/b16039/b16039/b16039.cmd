...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16039\b16039 b16039.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16039\b16039 b16039.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16039.dasm.il b16039.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16039.dasm.il b16039.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16039.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16039.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16039.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16039.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16039
    if not exist "b16039.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16039.org" (
            mkdir IL
            copy b16039.exe IL\b16039.exe 
            ren b16039.exe b16039.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16039.org /out b16039.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16039.org /out b16039.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16039.reflect.xml
    IF EXIST b16039.reflect.xml set ReflectionRoots=-x b16039.reflect.xml
    ...
    set ExePath=%LinkBin%\b16039.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16039.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16039.asm.exe %CLRTestExecutionArguments% %Host_Args%
