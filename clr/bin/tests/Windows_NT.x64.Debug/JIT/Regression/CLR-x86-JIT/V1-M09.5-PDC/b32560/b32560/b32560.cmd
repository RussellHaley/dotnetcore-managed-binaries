...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b32560\b32560 b32560.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b32560\b32560 b32560.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b32560.dasm.il b32560.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b32560.dasm.il b32560.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b32560.asm.exe  b32560.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b32560.asm.exe  b32560.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b32560
    if not exist "b32560.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b32560.org" (
            mkdir IL
            copy b32560.exe IL\b32560.exe 
            ren b32560.exe b32560.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b32560.org /out b32560.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b32560.org /out b32560.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b32560.reflect.xml
    IF EXIST b32560.reflect.xml set ReflectionRoots=-x b32560.reflect.xml
    ...
    set ExePath=%LinkBin%\b32560.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b32560.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b32560.asm.exe %CLRTestExecutionArguments% %Host_Args%
