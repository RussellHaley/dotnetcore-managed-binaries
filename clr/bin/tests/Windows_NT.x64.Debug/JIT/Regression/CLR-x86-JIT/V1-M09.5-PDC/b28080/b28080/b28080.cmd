...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b28080\b28080 b28080.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b28080\b28080 b28080.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b28080.dasm.il b28080.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b28080.dasm.il b28080.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b28080.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28080.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b28080.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28080.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b28080
    if not exist "b28080.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b28080.org" (
            mkdir IL
            copy b28080.exe IL\b28080.exe 
            ren b28080.exe b28080.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b28080.org /out b28080.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b28080.org /out b28080.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b28080.reflect.xml
    IF EXIST b28080.reflect.xml set ReflectionRoots=-x b28080.reflect.xml
    ...
    set ExePath=%LinkBin%\b28080.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b28080.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b28080.asm.exe %CLRTestExecutionArguments% %Host_Args%
