...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b31102/b31102 b31102.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b31102/b31102 b31102.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31102.dasm.il b31102.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31102.dasm.il b31102.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31102.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31102.dasm.il
    "$CORE_ROOT/ilasm" -output=b31102.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31102.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31102
    if [ ! -f b31102.org ]; then
        TakeLock
        if [ ! -f b31102.org ]; then
          mkdir IL
          cp b31102.exe IL/b31102.exe
          mv b31102.exe b31102.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31102.org /out b31102.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31102.org /out b31102.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31102.reflect.xml
  if [ -f b31102.reflect.xml ]
then
    ReflectionRoots="-x b31102.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31102.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31102.asm.exe $CLRTestExecutionArguments
