...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25813/b25813 b25813.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25813/b25813 b25813.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b25813.dasm.il b25813.exe
    "$CORE_ROOT/ildasm" -raweh -out=b25813.dasm.il b25813.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b25813.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25813.dasm.il
    "$CORE_ROOT/ilasm" -output=b25813.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25813.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b25813
    if [ ! -f b25813.org ]; then
        TakeLock
        if [ ! -f b25813.org ]; then
          mkdir IL
          cp b25813.exe IL/b25813.exe
          mv b25813.exe b25813.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25813.org /out b25813.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25813.org /out b25813.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b25813.reflect.xml
  if [ -f b25813.reflect.xml ]
then
    ReflectionRoots="-x b25813.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b25813.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b25813.asm.exe $CLRTestExecutionArguments
