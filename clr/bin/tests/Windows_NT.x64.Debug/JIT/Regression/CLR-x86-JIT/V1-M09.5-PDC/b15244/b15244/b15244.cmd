...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b15244\b15244 b15244.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b15244\b15244 b15244.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b15244.dasm.il b15244.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b15244.dasm.il b15244.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b15244.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b15244.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b15244.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b15244.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b15244
    if not exist "b15244.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b15244.org" (
            mkdir IL
            copy b15244.exe IL\b15244.exe 
            ren b15244.exe b15244.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b15244.org /out b15244.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b15244.org /out b15244.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b15244.reflect.xml
    IF EXIST b15244.reflect.xml set ReflectionRoots=-x b15244.reflect.xml
    ...
    set ExePath=%LinkBin%\b15244.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b15244.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b15244.asm.exe %CLRTestExecutionArguments% %Host_Args%
