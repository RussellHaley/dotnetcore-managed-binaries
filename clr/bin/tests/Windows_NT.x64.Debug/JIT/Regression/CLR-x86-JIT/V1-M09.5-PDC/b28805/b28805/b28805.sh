...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28805/b28805 b28805.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28805/b28805 b28805.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b28805.dasm.il b28805.exe
    "$CORE_ROOT/ildasm" -raweh -out=b28805.dasm.il b28805.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b28805.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28805.dasm.il
    "$CORE_ROOT/ilasm" -output=b28805.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28805.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b28805
    if [ ! -f b28805.org ]; then
        TakeLock
        if [ ! -f b28805.org ]; then
          mkdir IL
          cp b28805.exe IL/b28805.exe
          mv b28805.exe b28805.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28805.org /out b28805.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28805.org /out b28805.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b28805.reflect.xml
  if [ -f b28805.reflect.xml ]
then
    ReflectionRoots="-x b28805.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b28805.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b28805.asm.exe $CLRTestExecutionArguments
