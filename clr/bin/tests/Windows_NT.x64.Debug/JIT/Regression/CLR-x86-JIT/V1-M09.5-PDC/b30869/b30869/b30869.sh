...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b30869/b30869 b30869.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b30869/b30869 b30869.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b30869.dasm.il b30869.exe
    "$CORE_ROOT/ildasm" -raweh -out=b30869.dasm.il b30869.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b30869.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30869.dasm.il
    "$CORE_ROOT/ilasm" -output=b30869.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30869.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b30869
    if [ ! -f b30869.org ]; then
        TakeLock
        if [ ! -f b30869.org ]; then
          mkdir IL
          cp b30869.exe IL/b30869.exe
          mv b30869.exe b30869.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30869.org /out b30869.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30869.org /out b30869.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b30869.reflect.xml
  if [ -f b30869.reflect.xml ]
then
    ReflectionRoots="-x b30869.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b30869.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b30869.asm.exe $CLRTestExecutionArguments
