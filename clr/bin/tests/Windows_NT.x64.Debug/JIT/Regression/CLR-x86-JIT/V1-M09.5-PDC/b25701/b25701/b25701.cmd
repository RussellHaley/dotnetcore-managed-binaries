...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b25701\b25701 b25701.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b25701\b25701 b25701.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b25701.dasm.il b25701.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b25701.dasm.il b25701.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b25701.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25701.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b25701.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25701.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b25701
    if not exist "b25701.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b25701.org" (
            mkdir IL
            copy b25701.exe IL\b25701.exe 
            ren b25701.exe b25701.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b25701.org /out b25701.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b25701.org /out b25701.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b25701.reflect.xml
    IF EXIST b25701.reflect.xml set ReflectionRoots=-x b25701.reflect.xml
    ...
    set ExePath=%LinkBin%\b25701.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b25701.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b25701.asm.exe %CLRTestExecutionArguments% %Host_Args%
