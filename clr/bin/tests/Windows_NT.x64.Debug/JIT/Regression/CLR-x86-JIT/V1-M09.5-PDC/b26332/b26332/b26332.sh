...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26332/b26332 b26332.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26332/b26332 b26332.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b26332.dasm.il b26332.exe
    "$CORE_ROOT/ildasm" -raweh -out=b26332.dasm.il b26332.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b26332.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26332.dasm.il
    "$CORE_ROOT/ilasm" -output=b26332.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26332.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b26332
    if [ ! -f b26332.org ]; then
        TakeLock
        if [ ! -f b26332.org ]; then
          mkdir IL
          cp b26332.exe IL/b26332.exe
          mv b26332.exe b26332.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26332.org /out b26332.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26332.org /out b26332.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b26332.reflect.xml
  if [ -f b26332.reflect.xml ]
then
    ReflectionRoots="-x b26332.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b26332.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b26332.asm.exe $CLRTestExecutionArguments
