...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b18852/b18852 b18852.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b18852/b18852 b18852.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b18852.dasm.il b18852.exe
    "$CORE_ROOT/ildasm" -raweh -out=b18852.dasm.il b18852.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b18852.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b18852.dasm.il
    "$CORE_ROOT/ilasm" -output=b18852.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b18852.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b18852
    if [ ! -f b18852.org ]; then
        TakeLock
        if [ ! -f b18852.org ]; then
          mkdir IL
          cp b18852.exe IL/b18852.exe
          mv b18852.exe b18852.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b18852.org /out b18852.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b18852.org /out b18852.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b18852.reflect.xml
  if [ -f b18852.reflect.xml ]
then
    ReflectionRoots="-x b18852.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b18852.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b18852.asm.exe $CLRTestExecutionArguments
