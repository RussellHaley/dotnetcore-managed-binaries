...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28776/b28776 b28776.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28776/b28776 b28776.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b28776.dasm.il b28776.exe
    "$CORE_ROOT/ildasm" -raweh -out=b28776.dasm.il b28776.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b28776.asm.exe  b28776.dasm.il
    "$CORE_ROOT/ilasm" -output=b28776.asm.exe  b28776.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b28776
    if [ ! -f b28776.org ]; then
        TakeLock
        if [ ! -f b28776.org ]; then
          mkdir IL
          cp b28776.exe IL/b28776.exe
          mv b28776.exe b28776.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28776.org /out b28776.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28776.org /out b28776.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b28776.reflect.xml
  if [ -f b28776.reflect.xml ]
then
    ReflectionRoots="-x b28776.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b28776.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b28776.asm.exe $CLRTestExecutionArguments
