...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b30128/b30128 b30128.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b30128/b30128 b30128.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b30128.dasm.il b30128.exe
    "$CORE_ROOT/ildasm" -raweh -out=b30128.dasm.il b30128.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b30128.asm.exe  b30128.dasm.il
    "$CORE_ROOT/ilasm" -output=b30128.asm.exe  b30128.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b30128
    if [ ! -f b30128.org ]; then
        TakeLock
        if [ ! -f b30128.org ]; then
          mkdir IL
          cp b30128.exe IL/b30128.exe
          mv b30128.exe b30128.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30128.org /out b30128.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30128.org /out b30128.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b30128.reflect.xml
  if [ -f b30128.reflect.xml ]
then
    ReflectionRoots="-x b30128.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b30128.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b30128.asm.exe $CLRTestExecutionArguments
