...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b27883/b27883 b27883.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b27883/b27883 b27883.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b27883.dasm.il b27883.exe
    "$CORE_ROOT/ildasm" -raweh -out=b27883.dasm.il b27883.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b27883.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27883.dasm.il
    "$CORE_ROOT/ilasm" -output=b27883.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27883.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b27883
    if [ ! -f b27883.org ]; then
        TakeLock
        if [ ! -f b27883.org ]; then
          mkdir IL
          cp b27883.exe IL/b27883.exe
          mv b27883.exe b27883.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b27883.org /out b27883.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b27883.org /out b27883.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b27883.reflect.xml
  if [ -f b27883.reflect.xml ]
then
    ReflectionRoots="-x b27883.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b27883.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b27883.asm.exe $CLRTestExecutionArguments
