...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b32551/b32551a b32551a.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b32551/b32551a b32551a.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b32551a.dasm.il b32551a.exe
    "$CORE_ROOT/ildasm" -raweh -out=b32551a.dasm.il b32551a.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b32551a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32551a.dasm.il
    "$CORE_ROOT/ilasm" -output=b32551a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32551a.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b32551a
    if [ ! -f b32551a.org ]; then
        TakeLock
        if [ ! -f b32551a.org ]; then
          mkdir IL
          cp b32551a.exe IL/b32551a.exe
          mv b32551a.exe b32551a.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32551a.org /out b32551a.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32551a.org /out b32551a.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b32551a.reflect.xml
  if [ -f b32551a.reflect.xml ]
then
    ReflectionRoots="-x b32551a.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b32551a.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b32551a.asm.exe $CLRTestExecutionArguments
