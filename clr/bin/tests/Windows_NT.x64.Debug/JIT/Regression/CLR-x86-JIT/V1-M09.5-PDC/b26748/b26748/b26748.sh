...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26748/b26748 b26748.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26748/b26748 b26748.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b26748.dasm.il b26748.exe
    "$CORE_ROOT/ildasm" -raweh -out=b26748.dasm.il b26748.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b26748.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26748.dasm.il
    "$CORE_ROOT/ilasm" -output=b26748.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26748.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b26748
    if [ ! -f b26748.org ]; then
        TakeLock
        if [ ! -f b26748.org ]; then
          mkdir IL
          cp b26748.exe IL/b26748.exe
          mv b26748.exe b26748.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26748.org /out b26748.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26748.org /out b26748.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b26748.reflect.xml
  if [ -f b26748.reflect.xml ]
then
    ReflectionRoots="-x b26748.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b26748.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b26748.asm.exe $CLRTestExecutionArguments
