...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b12274/b12274 b12274.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b12274/b12274 b12274.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b12274.dasm.il b12274.exe
    "$CORE_ROOT/ildasm" -raweh -out=b12274.dasm.il b12274.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b12274.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12274.dasm.il
    "$CORE_ROOT/ilasm" -output=b12274.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b12274.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b12274
    if [ ! -f b12274.org ]; then
        TakeLock
        if [ ! -f b12274.org ]; then
          mkdir IL
          cp b12274.exe IL/b12274.exe
          mv b12274.exe b12274.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12274.org /out b12274.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12274.org /out b12274.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b12274.reflect.xml
  if [ -f b12274.reflect.xml ]
then
    ReflectionRoots="-x b12274.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b12274.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b12274.asm.exe $CLRTestExecutionArguments
