...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28806/b28806 b28806.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28806/b28806 b28806.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b28806.dasm.il b28806.exe
    "$CORE_ROOT/ildasm" -raweh -out=b28806.dasm.il b28806.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b28806.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28806.dasm.il
    "$CORE_ROOT/ilasm" -output=b28806.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28806.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b28806
    if [ ! -f b28806.org ]; then
        TakeLock
        if [ ! -f b28806.org ]; then
          mkdir IL
          cp b28806.exe IL/b28806.exe
          mv b28806.exe b28806.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28806.org /out b28806.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28806.org /out b28806.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b28806.reflect.xml
  if [ -f b28806.reflect.xml ]
then
    ReflectionRoots="-x b28806.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b28806.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b28806.asm.exe $CLRTestExecutionArguments
