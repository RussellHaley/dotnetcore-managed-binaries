...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28037/b28037 b28037.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28037/b28037 b28037.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b28037.dasm.il b28037.exe
    "$CORE_ROOT/ildasm" -raweh -out=b28037.dasm.il b28037.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b28037.asm.exe  b28037.dasm.il
    "$CORE_ROOT/ilasm" -output=b28037.asm.exe  b28037.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b28037
    if [ ! -f b28037.org ]; then
        TakeLock
        if [ ! -f b28037.org ]; then
          mkdir IL
          cp b28037.exe IL/b28037.exe
          mv b28037.exe b28037.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28037.org /out b28037.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28037.org /out b28037.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b28037.reflect.xml
  if [ -f b28037.reflect.xml ]
then
    ReflectionRoots="-x b28037.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b28037.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b28037.asm.exe $CLRTestExecutionArguments
