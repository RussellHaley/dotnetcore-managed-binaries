...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26888/b26888 b26888.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26888/b26888 b26888.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b26888.dasm.il b26888.exe
    "$CORE_ROOT/ildasm" -raweh -out=b26888.dasm.il b26888.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b26888.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26888.dasm.il
    "$CORE_ROOT/ilasm" -output=b26888.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26888.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b26888
    if [ ! -f b26888.org ]; then
        TakeLock
        if [ ! -f b26888.org ]; then
          mkdir IL
          cp b26888.exe IL/b26888.exe
          mv b26888.exe b26888.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26888.org /out b26888.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26888.org /out b26888.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b26888.reflect.xml
  if [ -f b26888.reflect.xml ]
then
    ReflectionRoots="-x b26888.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b26888.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b26888.asm.exe $CLRTestExecutionArguments
