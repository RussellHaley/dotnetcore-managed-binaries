...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b14068\b14068 b14068.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b14068\b14068 b14068.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b14068.dasm.il b14068.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b14068.dasm.il b14068.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b14068.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b14068.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b14068.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b14068.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b14068
    if not exist "b14068.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b14068.org" (
            mkdir IL
            copy b14068.exe IL\b14068.exe 
            ren b14068.exe b14068.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b14068.org /out b14068.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b14068.org /out b14068.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b14068.reflect.xml
    IF EXIST b14068.reflect.xml set ReflectionRoots=-x b14068.reflect.xml
    ...
    set ExePath=%LinkBin%\b14068.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b14068.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b14068.asm.exe %CLRTestExecutionArguments% %Host_Args%
