...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16499/b16499b b16499b.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16499/b16499b b16499b.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16499b.dasm.il b16499b.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16499b.dasm.il b16499b.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16499b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16499b.dasm.il
    "$CORE_ROOT/ilasm" -output=b16499b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16499b.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16499b
    if [ ! -f b16499b.org ]; then
        TakeLock
        if [ ! -f b16499b.org ]; then
          mkdir IL
          cp b16499b.exe IL/b16499b.exe
          mv b16499b.exe b16499b.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16499b.org /out b16499b.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16499b.org /out b16499b.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16499b.reflect.xml
  if [ -f b16499b.reflect.xml ]
then
    ReflectionRoots="-x b16499b.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16499b.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16499b.asm.exe $CLRTestExecutionArguments
