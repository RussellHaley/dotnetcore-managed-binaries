...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26155/b26155 b26155.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26155/b26155 b26155.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b26155.dasm.il b26155.exe
    "$CORE_ROOT/ildasm" -raweh -out=b26155.dasm.il b26155.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b26155.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26155.dasm.il
    "$CORE_ROOT/ilasm" -output=b26155.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26155.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b26155
    if [ ! -f b26155.org ]; then
        TakeLock
        if [ ! -f b26155.org ]; then
          mkdir IL
          cp b26155.exe IL/b26155.exe
          mv b26155.exe b26155.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26155.org /out b26155.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26155.org /out b26155.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b26155.reflect.xml
  if [ -f b26155.reflect.xml ]
then
    ReflectionRoots="-x b26155.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b26155.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b26155.asm.exe $CLRTestExecutionArguments
