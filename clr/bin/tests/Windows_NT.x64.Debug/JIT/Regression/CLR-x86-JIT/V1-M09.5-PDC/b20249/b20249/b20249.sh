...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b20249/b20249 b20249.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b20249/b20249 b20249.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b20249.dasm.il b20249.exe
    "$CORE_ROOT/ildasm" -raweh -out=b20249.dasm.il b20249.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b20249.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20249.dasm.il
    "$CORE_ROOT/ilasm" -output=b20249.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20249.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b20249
    if [ ! -f b20249.org ]; then
        TakeLock
        if [ ! -f b20249.org ]; then
          mkdir IL
          cp b20249.exe IL/b20249.exe
          mv b20249.exe b20249.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b20249.org /out b20249.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b20249.org /out b20249.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b20249.reflect.xml
  if [ -f b20249.reflect.xml ]
then
    ReflectionRoots="-x b20249.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b20249.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b20249.asm.exe $CLRTestExecutionArguments
