...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b29068\b29068 b29068.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b29068\b29068 b29068.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b29068.dasm.il b29068.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b29068.dasm.il b29068.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b29068.asm.exe  b29068.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b29068.asm.exe  b29068.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b29068
    if not exist "b29068.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b29068.org" (
            mkdir IL
            copy b29068.exe IL\b29068.exe 
            ren b29068.exe b29068.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b29068.org /out b29068.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b29068.org /out b29068.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b29068.reflect.xml
    IF EXIST b29068.reflect.xml set ReflectionRoots=-x b29068.reflect.xml
    ...
    set ExePath=%LinkBin%\b29068.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b29068.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b29068.asm.exe %CLRTestExecutionArguments% %Host_Args%
