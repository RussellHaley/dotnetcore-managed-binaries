...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16881/b16881a b16881a.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16881/b16881a b16881a.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16881a.dasm.il b16881a.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16881a.dasm.il b16881a.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16881a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16881a.dasm.il
    "$CORE_ROOT/ilasm" -output=b16881a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16881a.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16881a
    if [ ! -f b16881a.org ]; then
        TakeLock
        if [ ! -f b16881a.org ]; then
          mkdir IL
          cp b16881a.exe IL/b16881a.exe
          mv b16881a.exe b16881a.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16881a.org /out b16881a.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16881a.org /out b16881a.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16881a.reflect.xml
  if [ -f b16881a.reflect.xml ]
then
    ReflectionRoots="-x b16881a.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16881a.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16881a.asm.exe $CLRTestExecutionArguments
