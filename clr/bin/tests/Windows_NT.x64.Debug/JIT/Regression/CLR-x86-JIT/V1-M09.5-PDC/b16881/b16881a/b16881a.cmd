...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16881\b16881a b16881a.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16881\b16881a b16881a.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16881a.dasm.il b16881a.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16881a.dasm.il b16881a.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16881a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16881a.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16881a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16881a.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16881a
    if not exist "b16881a.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16881a.org" (
            mkdir IL
            copy b16881a.exe IL\b16881a.exe 
            ren b16881a.exe b16881a.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16881a.org /out b16881a.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16881a.org /out b16881a.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16881a.reflect.xml
    IF EXIST b16881a.reflect.xml set ReflectionRoots=-x b16881a.reflect.xml
    ...
    set ExePath=%LinkBin%\b16881a.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16881a.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16881a.asm.exe %CLRTestExecutionArguments% %Host_Args%
