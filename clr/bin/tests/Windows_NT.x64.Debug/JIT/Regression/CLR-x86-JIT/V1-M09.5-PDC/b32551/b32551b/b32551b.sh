...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b32551/b32551b b32551b.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b32551/b32551b b32551b.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b32551b.dasm.il b32551b.exe
    "$CORE_ROOT/ildasm" -raweh -out=b32551b.dasm.il b32551b.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b32551b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32551b.dasm.il
    "$CORE_ROOT/ilasm" -output=b32551b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32551b.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b32551b
    if [ ! -f b32551b.org ]; then
        TakeLock
        if [ ! -f b32551b.org ]; then
          mkdir IL
          cp b32551b.exe IL/b32551b.exe
          mv b32551b.exe b32551b.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32551b.org /out b32551b.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32551b.org /out b32551b.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b32551b.reflect.xml
  if [ -f b32551b.reflect.xml ]
then
    ReflectionRoots="-x b32551b.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b32551b.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b32551b.asm.exe $CLRTestExecutionArguments
