...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b15244/b15244 b15244.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b15244/b15244 b15244.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b15244.dasm.il b15244.exe
    "$CORE_ROOT/ildasm" -raweh -out=b15244.dasm.il b15244.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b15244.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b15244.dasm.il
    "$CORE_ROOT/ilasm" -output=b15244.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b15244.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b15244
    if [ ! -f b15244.org ]; then
        TakeLock
        if [ ! -f b15244.org ]; then
          mkdir IL
          cp b15244.exe IL/b15244.exe
          mv b15244.exe b15244.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b15244.org /out b15244.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b15244.org /out b15244.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b15244.reflect.xml
  if [ -f b15244.reflect.xml ]
then
    ReflectionRoots="-x b15244.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b15244.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b15244.asm.exe $CLRTestExecutionArguments
