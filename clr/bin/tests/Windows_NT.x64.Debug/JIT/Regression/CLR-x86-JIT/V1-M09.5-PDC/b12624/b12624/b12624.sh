...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b12624/b12624 b12624.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b12624/b12624 b12624.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b12624.dasm.il b12624.exe
    "$CORE_ROOT/ildasm" -raweh -out=b12624.dasm.il b12624.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b12624.asm.exe  b12624.dasm.il
    "$CORE_ROOT/ilasm" -output=b12624.asm.exe  b12624.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b12624
    if [ ! -f b12624.org ]; then
        TakeLock
        if [ ! -f b12624.org ]; then
          mkdir IL
          cp b12624.exe IL/b12624.exe
          mv b12624.exe b12624.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12624.org /out b12624.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b12624.org /out b12624.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b12624.reflect.xml
  if [ -f b12624.reflect.xml ]
then
    ReflectionRoots="-x b12624.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b12624.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b12624.asm.exe $CLRTestExecutionArguments
