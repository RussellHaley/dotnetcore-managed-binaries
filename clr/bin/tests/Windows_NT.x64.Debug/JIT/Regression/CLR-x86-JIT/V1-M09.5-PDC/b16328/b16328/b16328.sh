...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16328/b16328 b16328.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16328/b16328 b16328.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16328.dasm.il b16328.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16328.dasm.il b16328.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16328.asm.exe  b16328.dasm.il
    "$CORE_ROOT/ilasm" -output=b16328.asm.exe  b16328.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16328
    if [ ! -f b16328.org ]; then
        TakeLock
        if [ ! -f b16328.org ]; then
          mkdir IL
          cp b16328.exe IL/b16328.exe
          mv b16328.exe b16328.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16328.org /out b16328.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16328.org /out b16328.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16328.reflect.xml
  if [ -f b16328.reflect.xml ]
then
    ReflectionRoots="-x b16328.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16328.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16328.asm.exe $CLRTestExecutionArguments
