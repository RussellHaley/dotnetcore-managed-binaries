...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b26020\b26020 b26020.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b26020\b26020 b26020.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b26020.dasm.il b26020.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b26020.dasm.il b26020.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b26020.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26020.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b26020.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26020.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b26020
    if not exist "b26020.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b26020.org" (
            mkdir IL
            copy b26020.exe IL\b26020.exe 
            ren b26020.exe b26020.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b26020.org /out b26020.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b26020.org /out b26020.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b26020.reflect.xml
    IF EXIST b26020.reflect.xml set ReflectionRoots=-x b26020.reflect.xml
    ...
    set ExePath=%LinkBin%\b26020.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b26020.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b26020.asm.exe %CLRTestExecutionArguments% %Host_Args%
