...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b13569/b13569 b13569.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b13569/b13569 b13569.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b13569.dasm.il b13569.exe
    "$CORE_ROOT/ildasm" -raweh -out=b13569.dasm.il b13569.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b13569.asm.exe  b13569.dasm.il
    "$CORE_ROOT/ilasm" -output=b13569.asm.exe  b13569.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b13569
    if [ ! -f b13569.org ]; then
        TakeLock
        if [ ! -f b13569.org ]; then
          mkdir IL
          cp b13569.exe IL/b13569.exe
          mv b13569.exe b13569.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b13569.org /out b13569.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b13569.org /out b13569.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b13569.reflect.xml
  if [ -f b13569.reflect.xml ]
then
    ReflectionRoots="-x b13569.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b13569.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b13569.asm.exe $CLRTestExecutionArguments
