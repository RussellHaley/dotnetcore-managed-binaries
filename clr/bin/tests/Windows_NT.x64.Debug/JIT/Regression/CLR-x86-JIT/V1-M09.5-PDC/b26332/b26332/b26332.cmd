...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b26332\b26332 b26332.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b26332\b26332 b26332.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b26332.dasm.il b26332.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b26332.dasm.il b26332.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b26332.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26332.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b26332.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26332.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b26332
    if not exist "b26332.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b26332.org" (
            mkdir IL
            copy b26332.exe IL\b26332.exe 
            ren b26332.exe b26332.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b26332.org /out b26332.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b26332.org /out b26332.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b26332.reflect.xml
    IF EXIST b26332.reflect.xml set ReflectionRoots=-x b26332.reflect.xml
    ...
    set ExePath=%LinkBin%\b26332.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b26332.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b26332.asm.exe %CLRTestExecutionArguments% %Host_Args%
