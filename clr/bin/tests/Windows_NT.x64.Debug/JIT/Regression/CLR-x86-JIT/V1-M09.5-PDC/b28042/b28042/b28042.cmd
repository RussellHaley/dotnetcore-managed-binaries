...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b28042\b28042 b28042.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b28042\b28042 b28042.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b28042.dasm.il b28042.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b28042.dasm.il b28042.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b28042.asm.exe  b28042.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b28042.asm.exe  b28042.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b28042
    if not exist "b28042.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b28042.org" (
            mkdir IL
            copy b28042.exe IL\b28042.exe 
            ren b28042.exe b28042.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b28042.org /out b28042.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b28042.org /out b28042.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b28042.reflect.xml
    IF EXIST b28042.reflect.xml set ReflectionRoots=-x b28042.reflect.xml
    ...
    set ExePath=%LinkBin%\b28042.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b28042.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b28042.asm.exe %CLRTestExecutionArguments% %Host_Args%
