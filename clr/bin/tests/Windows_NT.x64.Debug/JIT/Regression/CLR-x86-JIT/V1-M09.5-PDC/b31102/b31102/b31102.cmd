...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b31102\b31102 b31102.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b31102\b31102 b31102.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b31102.dasm.il b31102.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b31102.dasm.il b31102.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b31102.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31102.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b31102.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31102.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b31102
    if not exist "b31102.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b31102.org" (
            mkdir IL
            copy b31102.exe IL\b31102.exe 
            ren b31102.exe b31102.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31102.org /out b31102.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31102.org /out b31102.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b31102.reflect.xml
    IF EXIST b31102.reflect.xml set ReflectionRoots=-x b31102.reflect.xml
    ...
    set ExePath=%LinkBin%\b31102.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b31102.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b31102.asm.exe %CLRTestExecutionArguments% %Host_Args%
