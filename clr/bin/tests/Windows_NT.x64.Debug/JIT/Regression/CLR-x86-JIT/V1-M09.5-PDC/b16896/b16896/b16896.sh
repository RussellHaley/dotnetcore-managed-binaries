...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16896/b16896 b16896.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16896/b16896 b16896.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16896.dasm.il b16896.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16896.dasm.il b16896.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16896.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16896.dasm.il
    "$CORE_ROOT/ilasm" -output=b16896.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16896.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16896
    if [ ! -f b16896.org ]; then
        TakeLock
        if [ ! -f b16896.org ]; then
          mkdir IL
          cp b16896.exe IL/b16896.exe
          mv b16896.exe b16896.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16896.org /out b16896.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16896.org /out b16896.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16896.reflect.xml
  if [ -f b16896.reflect.xml ]
then
    ReflectionRoots="-x b16896.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16896.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16896.asm.exe $CLRTestExecutionArguments
