...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b31763/b31763 b31763.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b31763/b31763 b31763.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31763.dasm.il b31763.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31763.dasm.il b31763.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31763.asm.exe  b31763.dasm.il
    "$CORE_ROOT/ilasm" -output=b31763.asm.exe  b31763.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31763
    if [ ! -f b31763.org ]; then
        TakeLock
        if [ ! -f b31763.org ]; then
          mkdir IL
          cp b31763.exe IL/b31763.exe
          mv b31763.exe b31763.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31763.org /out b31763.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31763.org /out b31763.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31763.reflect.xml
  if [ -f b31763.reflect.xml ]
then
    ReflectionRoots="-x b31763.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31763.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31763.asm.exe $CLRTestExecutionArguments
