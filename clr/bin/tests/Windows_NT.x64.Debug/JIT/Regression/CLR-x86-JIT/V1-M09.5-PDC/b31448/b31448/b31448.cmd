...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b31448\b31448 b31448.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b31448\b31448 b31448.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b31448.dasm.il b31448.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b31448.dasm.il b31448.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b31448.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31448.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b31448.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31448.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b31448
    if not exist "b31448.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b31448.org" (
            mkdir IL
            copy b31448.exe IL\b31448.exe 
            ren b31448.exe b31448.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31448.org /out b31448.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31448.org /out b31448.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b31448.reflect.xml
    IF EXIST b31448.reflect.xml set ReflectionRoots=-x b31448.reflect.xml
    ...
    set ExePath=%LinkBin%\b31448.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b31448.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b31448.asm.exe %CLRTestExecutionArguments% %Host_Args%
