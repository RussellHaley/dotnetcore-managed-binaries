...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16499\b16499b b16499b.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16499\b16499b b16499b.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16499b.dasm.il b16499b.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16499b.dasm.il b16499b.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16499b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16499b.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16499b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16499b.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16499b
    if not exist "b16499b.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16499b.org" (
            mkdir IL
            copy b16499b.exe IL\b16499b.exe 
            ren b16499b.exe b16499b.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16499b.org /out b16499b.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16499b.org /out b16499b.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16499b.reflect.xml
    IF EXIST b16499b.reflect.xml set ReflectionRoots=-x b16499b.reflect.xml
    ...
    set ExePath=%LinkBin%\b16499b.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16499b.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16499b.asm.exe %CLRTestExecutionArguments% %Host_Args%
