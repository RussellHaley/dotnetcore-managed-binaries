...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b10894\b10894 b10894.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b10894\b10894 b10894.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b10894.dasm.il b10894.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b10894.dasm.il b10894.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b10894.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b10894.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b10894.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b10894.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b10894
    if not exist "b10894.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b10894.org" (
            mkdir IL
            copy b10894.exe IL\b10894.exe 
            ren b10894.exe b10894.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b10894.org /out b10894.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b10894.org /out b10894.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b10894.reflect.xml
    IF EXIST b10894.reflect.xml set ReflectionRoots=-x b10894.reflect.xml
    ...
    set ExePath=%LinkBin%\b10894.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b10894.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b10894.asm.exe %CLRTestExecutionArguments% %Host_Args%
