...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16881\b16881b b16881b.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16881\b16881b b16881b.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16881b.dasm.il b16881b.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16881b.dasm.il b16881b.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16881b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16881b.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16881b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16881b.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16881b
    if not exist "b16881b.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16881b.org" (
            mkdir IL
            copy b16881b.exe IL\b16881b.exe 
            ren b16881b.exe b16881b.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16881b.org /out b16881b.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16881b.org /out b16881b.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16881b.reflect.xml
    IF EXIST b16881b.reflect.xml set ReflectionRoots=-x b16881b.reflect.xml
    ...
    set ExePath=%LinkBin%\b16881b.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16881b.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16881b.asm.exe %CLRTestExecutionArguments% %Host_Args%
