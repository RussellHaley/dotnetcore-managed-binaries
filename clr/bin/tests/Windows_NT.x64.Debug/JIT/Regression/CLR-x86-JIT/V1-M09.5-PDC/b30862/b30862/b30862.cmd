...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30862\b30862 b30862.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30862\b30862 b30862.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b30862.dasm.il b30862.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b30862.dasm.il b30862.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b30862.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30862.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b30862.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30862.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b30862
    if not exist "b30862.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b30862.org" (
            mkdir IL
            copy b30862.exe IL\b30862.exe 
            ren b30862.exe b30862.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30862.org /out b30862.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30862.org /out b30862.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b30862.reflect.xml
    IF EXIST b30862.reflect.xml set ReflectionRoots=-x b30862.reflect.xml
    ...
    set ExePath=%LinkBin%\b30862.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b30862.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b30862.asm.exe %CLRTestExecutionArguments% %Host_Args%
