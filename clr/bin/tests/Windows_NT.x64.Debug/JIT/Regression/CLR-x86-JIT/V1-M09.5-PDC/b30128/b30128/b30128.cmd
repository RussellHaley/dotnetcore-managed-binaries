...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30128\b30128 b30128.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30128\b30128 b30128.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b30128.dasm.il b30128.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b30128.dasm.il b30128.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b30128.asm.exe  b30128.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b30128.asm.exe  b30128.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b30128
    if not exist "b30128.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b30128.org" (
            mkdir IL
            copy b30128.exe IL\b30128.exe 
            ren b30128.exe b30128.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30128.org /out b30128.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30128.org /out b30128.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b30128.reflect.xml
    IF EXIST b30128.reflect.xml set ReflectionRoots=-x b30128.reflect.xml
    ...
    set ExePath=%LinkBin%\b30128.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b30128.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b30128.asm.exe %CLRTestExecutionArguments% %Host_Args%
