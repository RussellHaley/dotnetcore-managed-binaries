...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25739/b25739 b25739.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25739/b25739 b25739.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b25739.dasm.il b25739.exe
    "$CORE_ROOT/ildasm" -raweh -out=b25739.dasm.il b25739.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b25739.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25739.dasm.il
    "$CORE_ROOT/ilasm" -output=b25739.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25739.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b25739
    if [ ! -f b25739.org ]; then
        TakeLock
        if [ ! -f b25739.org ]; then
          mkdir IL
          cp b25739.exe IL/b25739.exe
          mv b25739.exe b25739.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25739.org /out b25739.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25739.org /out b25739.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b25739.reflect.xml
  if [ -f b25739.reflect.xml ]
then
    ReflectionRoots="-x b25739.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b25739.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b25739.asm.exe $CLRTestExecutionArguments
