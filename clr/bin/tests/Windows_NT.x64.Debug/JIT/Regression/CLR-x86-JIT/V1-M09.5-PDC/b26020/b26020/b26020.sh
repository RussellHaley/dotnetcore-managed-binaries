...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26020/b26020 b26020.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26020/b26020 b26020.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b26020.dasm.il b26020.exe
    "$CORE_ROOT/ildasm" -raweh -out=b26020.dasm.il b26020.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b26020.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26020.dasm.il
    "$CORE_ROOT/ilasm" -output=b26020.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26020.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b26020
    if [ ! -f b26020.org ]; then
        TakeLock
        if [ ! -f b26020.org ]; then
          mkdir IL
          cp b26020.exe IL/b26020.exe
          mv b26020.exe b26020.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26020.org /out b26020.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26020.org /out b26020.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b26020.reflect.xml
  if [ -f b26020.reflect.xml ]
then
    ReflectionRoots="-x b26020.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b26020.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b26020.asm.exe $CLRTestExecutionArguments
