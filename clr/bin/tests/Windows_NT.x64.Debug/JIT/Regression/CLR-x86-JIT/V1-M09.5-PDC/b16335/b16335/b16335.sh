...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16335/b16335 b16335.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16335/b16335 b16335.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16335.dasm.il b16335.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16335.dasm.il b16335.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16335.asm.exe  b16335.dasm.il
    "$CORE_ROOT/ilasm" -output=b16335.asm.exe  b16335.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16335
    if [ ! -f b16335.org ]; then
        TakeLock
        if [ ! -f b16335.org ]; then
          mkdir IL
          cp b16335.exe IL/b16335.exe
          mv b16335.exe b16335.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16335.org /out b16335.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16335.org /out b16335.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16335.reflect.xml
  if [ -f b16335.reflect.xml ]
then
    ReflectionRoots="-x b16335.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16335.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16335.asm.exe $CLRTestExecutionArguments
