...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b31273\b31273 b31273.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b31273\b31273 b31273.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b31273.dasm.il b31273.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b31273.dasm.il b31273.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b31273.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31273.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b31273.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31273.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b31273
    if not exist "b31273.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b31273.org" (
            mkdir IL
            copy b31273.exe IL\b31273.exe 
            ren b31273.exe b31273.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31273.org /out b31273.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b31273.org /out b31273.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b31273.reflect.xml
    IF EXIST b31273.reflect.xml set ReflectionRoots=-x b31273.reflect.xml
    ...
    set ExePath=%LinkBin%\b31273.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b31273.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b31273.asm.exe %CLRTestExecutionArguments% %Host_Args%
