...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b10894/b10894 b10894.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b10894/b10894 b10894.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b10894.dasm.il b10894.exe
    "$CORE_ROOT/ildasm" -raweh -out=b10894.dasm.il b10894.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b10894.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b10894.dasm.il
    "$CORE_ROOT/ilasm" -output=b10894.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b10894.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b10894
    if [ ! -f b10894.org ]; then
        TakeLock
        if [ ! -f b10894.org ]; then
          mkdir IL
          cp b10894.exe IL/b10894.exe
          mv b10894.exe b10894.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b10894.org /out b10894.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b10894.org /out b10894.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b10894.reflect.xml
  if [ -f b10894.reflect.xml ]
then
    ReflectionRoots="-x b10894.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b10894.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b10894.asm.exe $CLRTestExecutionArguments
