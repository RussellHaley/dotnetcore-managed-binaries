...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25468/b25468 b25468.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25468/b25468 b25468.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b25468.dasm.il b25468.exe
    "$CORE_ROOT/ildasm" -raweh -out=b25468.dasm.il b25468.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b25468.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25468.dasm.il
    "$CORE_ROOT/ilasm" -output=b25468.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25468.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b25468
    if [ ! -f b25468.org ]; then
        TakeLock
        if [ ! -f b25468.org ]; then
          mkdir IL
          cp b25468.exe IL/b25468.exe
          mv b25468.exe b25468.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25468.org /out b25468.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25468.org /out b25468.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b25468.reflect.xml
  if [ -f b25468.reflect.xml ]
then
    ReflectionRoots="-x b25468.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b25468.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b25468.asm.exe $CLRTestExecutionArguments
