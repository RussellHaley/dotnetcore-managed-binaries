...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25833/b25833 b25833.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25833/b25833 b25833.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b25833.dasm.il b25833.exe
    "$CORE_ROOT/ildasm" -raweh -out=b25833.dasm.il b25833.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b25833.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25833.dasm.il
    "$CORE_ROOT/ilasm" -output=b25833.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25833.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b25833
    if [ ! -f b25833.org ]; then
        TakeLock
        if [ ! -f b25833.org ]; then
          mkdir IL
          cp b25833.exe IL/b25833.exe
          mv b25833.exe b25833.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25833.org /out b25833.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25833.org /out b25833.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b25833.reflect.xml
  if [ -f b25833.reflect.xml ]
then
    ReflectionRoots="-x b25833.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b25833.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b25833.asm.exe $CLRTestExecutionArguments
