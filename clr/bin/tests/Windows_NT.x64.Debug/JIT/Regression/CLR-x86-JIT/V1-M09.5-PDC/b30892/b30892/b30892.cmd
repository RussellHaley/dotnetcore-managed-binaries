...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30892\b30892 b30892.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30892\b30892 b30892.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b30892.dasm.il b30892.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b30892.dasm.il b30892.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b30892.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30892.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b30892.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30892.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b30892
    if not exist "b30892.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b30892.org" (
            mkdir IL
            copy b30892.exe IL\b30892.exe 
            ren b30892.exe b30892.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30892.org /out b30892.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30892.org /out b30892.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b30892.reflect.xml
    IF EXIST b30892.reflect.xml set ReflectionRoots=-x b30892.reflect.xml
    ...
    set ExePath=%LinkBin%\b30892.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b30892.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b30892.asm.exe %CLRTestExecutionArguments% %Host_Args%
