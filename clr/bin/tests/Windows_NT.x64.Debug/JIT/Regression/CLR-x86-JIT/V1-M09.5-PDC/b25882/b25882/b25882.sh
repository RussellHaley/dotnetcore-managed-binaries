...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25882/b25882 b25882.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25882/b25882 b25882.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b25882.dasm.il b25882.exe
    "$CORE_ROOT/ildasm" -raweh -out=b25882.dasm.il b25882.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b25882.asm.exe  b25882.dasm.il
    "$CORE_ROOT/ilasm" -output=b25882.asm.exe  b25882.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b25882
    if [ ! -f b25882.org ]; then
        TakeLock
        if [ ! -f b25882.org ]; then
          mkdir IL
          cp b25882.exe IL/b25882.exe
          mv b25882.exe b25882.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25882.org /out b25882.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25882.org /out b25882.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b25882.reflect.xml
  if [ -f b25882.reflect.xml ]
then
    ReflectionRoots="-x b25882.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b25882.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b25882.asm.exe $CLRTestExecutionArguments
