...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25459/b25459 b25459.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25459/b25459 b25459.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b25459.dasm.il b25459.exe
    "$CORE_ROOT/ildasm" -raweh -out=b25459.dasm.il b25459.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b25459.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25459.dasm.il
    "$CORE_ROOT/ilasm" -output=b25459.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25459.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b25459
    if [ ! -f b25459.org ]; then
        TakeLock
        if [ ! -f b25459.org ]; then
          mkdir IL
          cp b25459.exe IL/b25459.exe
          mv b25459.exe b25459.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25459.org /out b25459.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25459.org /out b25459.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b25459.reflect.xml
  if [ -f b25459.reflect.xml ]
then
    ReflectionRoots="-x b25459.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b25459.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b25459.asm.exe $CLRTestExecutionArguments
