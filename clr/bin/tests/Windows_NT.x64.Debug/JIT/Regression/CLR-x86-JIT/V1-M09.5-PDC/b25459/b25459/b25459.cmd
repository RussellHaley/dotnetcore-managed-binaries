...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b25459\b25459 b25459.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b25459\b25459 b25459.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b25459.dasm.il b25459.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b25459.dasm.il b25459.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b25459.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25459.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b25459.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25459.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b25459
    if not exist "b25459.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b25459.org" (
            mkdir IL
            copy b25459.exe IL\b25459.exe 
            ren b25459.exe b25459.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b25459.org /out b25459.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b25459.org /out b25459.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b25459.reflect.xml
    IF EXIST b25459.reflect.xml set ReflectionRoots=-x b25459.reflect.xml
    ...
    set ExePath=%LinkBin%\b25459.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b25459.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b25459.asm.exe %CLRTestExecutionArguments% %Host_Args%
