...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28596/b28596 b28596.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28596/b28596 b28596.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b28596.dasm.il b28596.exe
    "$CORE_ROOT/ildasm" -raweh -out=b28596.dasm.il b28596.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b28596.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28596.dasm.il
    "$CORE_ROOT/ilasm" -output=b28596.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28596.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b28596
    if [ ! -f b28596.org ]; then
        TakeLock
        if [ ! -f b28596.org ]; then
          mkdir IL
          cp b28596.exe IL/b28596.exe
          mv b28596.exe b28596.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28596.org /out b28596.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28596.org /out b28596.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b28596.reflect.xml
  if [ -f b28596.reflect.xml ]
then
    ReflectionRoots="-x b28596.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b28596.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b28596.asm.exe $CLRTestExecutionArguments
