...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28597/b28597 b28597.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28597/b28597 b28597.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b28597.dasm.il b28597.exe
    "$CORE_ROOT/ildasm" -raweh -out=b28597.dasm.il b28597.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b28597.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28597.dasm.il
    "$CORE_ROOT/ilasm" -output=b28597.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28597.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b28597
    if [ ! -f b28597.org ]; then
        TakeLock
        if [ ! -f b28597.org ]; then
          mkdir IL
          cp b28597.exe IL/b28597.exe
          mv b28597.exe b28597.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28597.org /out b28597.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28597.org /out b28597.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b28597.reflect.xml
  if [ -f b28597.reflect.xml ]
then
    ReflectionRoots="-x b28597.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b28597.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b28597.asm.exe $CLRTestExecutionArguments
