...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16328\b16328 b16328.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16328\b16328 b16328.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16328.dasm.il b16328.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16328.dasm.il b16328.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16328.asm.exe  b16328.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16328.asm.exe  b16328.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16328
    if not exist "b16328.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16328.org" (
            mkdir IL
            copy b16328.exe IL\b16328.exe 
            ren b16328.exe b16328.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16328.org /out b16328.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16328.org /out b16328.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16328.reflect.xml
    IF EXIST b16328.reflect.xml set ReflectionRoots=-x b16328.reflect.xml
    ...
    set ExePath=%LinkBin%\b16328.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16328.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16328.asm.exe %CLRTestExecutionArguments% %Host_Args%
