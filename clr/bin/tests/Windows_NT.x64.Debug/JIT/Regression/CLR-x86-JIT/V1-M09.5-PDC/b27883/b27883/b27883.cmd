...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b27883\b27883 b27883.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b27883\b27883 b27883.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b27883.dasm.il b27883.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b27883.dasm.il b27883.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b27883.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27883.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b27883.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27883.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b27883
    if not exist "b27883.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b27883.org" (
            mkdir IL
            copy b27883.exe IL\b27883.exe 
            ren b27883.exe b27883.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b27883.org /out b27883.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b27883.org /out b27883.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b27883.reflect.xml
    IF EXIST b27883.reflect.xml set ReflectionRoots=-x b27883.reflect.xml
    ...
    set ExePath=%LinkBin%\b27883.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b27883.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b27883.asm.exe %CLRTestExecutionArguments% %Host_Args%
