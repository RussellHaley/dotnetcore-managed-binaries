...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25458/b25458 b25458.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25458/b25458 b25458.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b25458.dasm.il b25458.exe
    "$CORE_ROOT/ildasm" -raweh -out=b25458.dasm.il b25458.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b25458.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25458.dasm.il
    "$CORE_ROOT/ilasm" -output=b25458.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25458.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b25458
    if [ ! -f b25458.org ]; then
        TakeLock
        if [ ! -f b25458.org ]; then
          mkdir IL
          cp b25458.exe IL/b25458.exe
          mv b25458.exe b25458.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25458.org /out b25458.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25458.org /out b25458.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b25458.reflect.xml
  if [ -f b25458.reflect.xml ]
then
    ReflectionRoots="-x b25458.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b25458.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b25458.asm.exe $CLRTestExecutionArguments
