...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26863/b26863 b26863.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26863/b26863 b26863.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b26863.dasm.il b26863.exe
    "$CORE_ROOT/ildasm" -raweh -out=b26863.dasm.il b26863.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b26863.asm.exe  b26863.dasm.il
    "$CORE_ROOT/ilasm" -output=b26863.asm.exe  b26863.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b26863
    if [ ! -f b26863.org ]; then
        TakeLock
        if [ ! -f b26863.org ]; then
          mkdir IL
          cp b26863.exe IL/b26863.exe
          mv b26863.exe b26863.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26863.org /out b26863.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26863.org /out b26863.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b26863.reflect.xml
  if [ -f b26863.reflect.xml ]
then
    ReflectionRoots="-x b26863.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b26863.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b26863.asm.exe $CLRTestExecutionArguments
