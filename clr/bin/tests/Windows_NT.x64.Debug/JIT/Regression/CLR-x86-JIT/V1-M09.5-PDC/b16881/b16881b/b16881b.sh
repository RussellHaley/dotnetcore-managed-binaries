...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16881/b16881b b16881b.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b16881/b16881b b16881b.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16881b.dasm.il b16881b.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16881b.dasm.il b16881b.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16881b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16881b.dasm.il
    "$CORE_ROOT/ilasm" -output=b16881b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16881b.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16881b
    if [ ! -f b16881b.org ]; then
        TakeLock
        if [ ! -f b16881b.org ]; then
          mkdir IL
          cp b16881b.exe IL/b16881b.exe
          mv b16881b.exe b16881b.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16881b.org /out b16881b.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16881b.org /out b16881b.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16881b.reflect.xml
  if [ -f b16881b.reflect.xml ]
then
    ReflectionRoots="-x b16881b.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16881b.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16881b.asm.exe $CLRTestExecutionArguments
