...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b13522/b13522 b13522.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b13522/b13522 b13522.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b13522.dasm.il b13522.exe
    "$CORE_ROOT/ildasm" -raweh -out=b13522.dasm.il b13522.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b13522.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b13522.dasm.il
    "$CORE_ROOT/ilasm" -output=b13522.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b13522.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b13522
    if [ ! -f b13522.org ]; then
        TakeLock
        if [ ! -f b13522.org ]; then
          mkdir IL
          cp b13522.exe IL/b13522.exe
          mv b13522.exe b13522.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b13522.org /out b13522.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b13522.org /out b13522.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b13522.reflect.xml
  if [ -f b13522.reflect.xml ]
then
    ReflectionRoots="-x b13522.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b13522.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b13522.asm.exe $CLRTestExecutionArguments
