...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30792\b30792 b30792.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30792\b30792 b30792.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b30792.dasm.il b30792.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b30792.dasm.il b30792.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b30792.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30792.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b30792.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30792.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b30792
    if not exist "b30792.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b30792.org" (
            mkdir IL
            copy b30792.exe IL\b30792.exe 
            ren b30792.exe b30792.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30792.org /out b30792.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30792.org /out b30792.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b30792.reflect.xml
    IF EXIST b30792.reflect.xml set ReflectionRoots=-x b30792.reflect.xml
    ...
    set ExePath=%LinkBin%\b30792.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b30792.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b30792.asm.exe %CLRTestExecutionArguments% %Host_Args%
