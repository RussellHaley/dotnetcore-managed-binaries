...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16922\b16922 b16922.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16922\b16922 b16922.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16922.dasm.il b16922.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16922.dasm.il b16922.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16922.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16922.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16922.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16922.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16922
    if not exist "b16922.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16922.org" (
            mkdir IL
            copy b16922.exe IL\b16922.exe 
            ren b16922.exe b16922.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16922.org /out b16922.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16922.org /out b16922.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16922.reflect.xml
    IF EXIST b16922.reflect.xml set ReflectionRoots=-x b16922.reflect.xml
    ...
    set ExePath=%LinkBin%\b16922.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16922.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16922.asm.exe %CLRTestExecutionArguments% %Host_Args%
