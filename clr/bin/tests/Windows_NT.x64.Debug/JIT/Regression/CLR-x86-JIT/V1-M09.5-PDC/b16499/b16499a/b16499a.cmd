...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16499\b16499a b16499a.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b16499\b16499a b16499a.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16499a.dasm.il b16499a.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16499a.dasm.il b16499a.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16499a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16499a.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16499a.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16499a.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16499a
    if not exist "b16499a.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16499a.org" (
            mkdir IL
            copy b16499a.exe IL\b16499a.exe 
            ren b16499a.exe b16499a.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16499a.org /out b16499a.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16499a.org /out b16499a.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16499a.reflect.xml
    IF EXIST b16499a.reflect.xml set ReflectionRoots=-x b16499a.reflect.xml
    ...
    set ExePath=%LinkBin%\b16499a.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16499a.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16499a.asm.exe %CLRTestExecutionArguments% %Host_Args%
