...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b14068/b14068 b14068.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b14068/b14068 b14068.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b14068.dasm.il b14068.exe
    "$CORE_ROOT/ildasm" -raweh -out=b14068.dasm.il b14068.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b14068.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b14068.dasm.il
    "$CORE_ROOT/ilasm" -output=b14068.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b14068.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b14068
    if [ ! -f b14068.org ]; then
        TakeLock
        if [ ! -f b14068.org ]; then
          mkdir IL
          cp b14068.exe IL/b14068.exe
          mv b14068.exe b14068.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b14068.org /out b14068.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b14068.org /out b14068.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b14068.reflect.xml
  if [ -f b14068.reflect.xml ]
then
    ReflectionRoots="-x b14068.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b14068.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b14068.asm.exe $CLRTestExecutionArguments
