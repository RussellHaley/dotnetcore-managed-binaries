...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b25491\b25491 b25491.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b25491\b25491 b25491.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b25491.dasm.il b25491.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b25491.dasm.il b25491.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b25491.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25491.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b25491.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25491.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b25491
    if not exist "b25491.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b25491.org" (
            mkdir IL
            copy b25491.exe IL\b25491.exe 
            ren b25491.exe b25491.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b25491.org /out b25491.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b25491.org /out b25491.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b25491.reflect.xml
    IF EXIST b25491.reflect.xml set ReflectionRoots=-x b25491.reflect.xml
    ...
    set ExePath=%LinkBin%\b25491.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b25491.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b25491.asm.exe %CLRTestExecutionArguments% %Host_Args%
