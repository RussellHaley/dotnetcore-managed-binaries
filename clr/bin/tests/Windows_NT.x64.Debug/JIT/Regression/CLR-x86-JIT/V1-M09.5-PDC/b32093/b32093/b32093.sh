...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b32093/b32093 b32093.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b32093/b32093 b32093.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b32093.dasm.il b32093.exe
    "$CORE_ROOT/ildasm" -raweh -out=b32093.dasm.il b32093.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b32093.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32093.dasm.il
    "$CORE_ROOT/ilasm" -output=b32093.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32093.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b32093
    if [ ! -f b32093.org ]; then
        TakeLock
        if [ ! -f b32093.org ]; then
          mkdir IL
          cp b32093.exe IL/b32093.exe
          mv b32093.exe b32093.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32093.org /out b32093.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32093.org /out b32093.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b32093.reflect.xml
  if [ -f b32093.reflect.xml ]
then
    ReflectionRoots="-x b32093.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b32093.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b32093.asm.exe $CLRTestExecutionArguments
