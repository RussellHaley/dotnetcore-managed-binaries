...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b30792/b30792 b30792.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b30792/b30792 b30792.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b30792.dasm.il b30792.exe
    "$CORE_ROOT/ildasm" -raweh -out=b30792.dasm.il b30792.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b30792.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30792.dasm.il
    "$CORE_ROOT/ilasm" -output=b30792.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30792.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b30792
    if [ ! -f b30792.org ]; then
        TakeLock
        if [ ! -f b30792.org ]; then
          mkdir IL
          cp b30792.exe IL/b30792.exe
          mv b30792.exe b30792.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30792.org /out b30792.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30792.org /out b30792.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b30792.reflect.xml
  if [ -f b30792.reflect.xml ]
then
    ReflectionRoots="-x b30792.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b30792.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b30792.asm.exe $CLRTestExecutionArguments
