...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26957/b26957 b26957.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b26957/b26957 b26957.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b26957.dasm.il b26957.exe
    "$CORE_ROOT/ildasm" -raweh -out=b26957.dasm.il b26957.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b26957.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26957.dasm.il
    "$CORE_ROOT/ilasm" -output=b26957.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b26957.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b26957
    if [ ! -f b26957.org ]; then
        TakeLock
        if [ ! -f b26957.org ]; then
          mkdir IL
          cp b26957.exe IL/b26957.exe
          mv b26957.exe b26957.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26957.org /out b26957.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b26957.org /out b26957.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b26957.reflect.xml
  if [ -f b26957.reflect.xml ]
then
    ReflectionRoots="-x b26957.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b26957.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b26957.asm.exe $CLRTestExecutionArguments
