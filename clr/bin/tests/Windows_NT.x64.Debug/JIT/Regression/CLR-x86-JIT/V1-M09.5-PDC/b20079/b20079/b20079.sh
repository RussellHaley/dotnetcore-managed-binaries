...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b20079/b20079 b20079.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b20079/b20079 b20079.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b20079.dasm.il b20079.exe
    "$CORE_ROOT/ildasm" -raweh -out=b20079.dasm.il b20079.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b20079.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20079.dasm.il
    "$CORE_ROOT/ilasm" -output=b20079.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20079.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b20079
    if [ ! -f b20079.org ]; then
        TakeLock
        if [ ! -f b20079.org ]; then
          mkdir IL
          cp b20079.exe IL/b20079.exe
          mv b20079.exe b20079.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b20079.org /out b20079.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b20079.org /out b20079.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b20079.reflect.xml
  if [ -f b20079.reflect.xml ]
then
    ReflectionRoots="-x b20079.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b20079.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b20079.asm.exe $CLRTestExecutionArguments
