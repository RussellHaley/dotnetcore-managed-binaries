...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28042/b28042 b28042.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28042/b28042 b28042.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b28042.dasm.il b28042.exe
    "$CORE_ROOT/ildasm" -raweh -out=b28042.dasm.il b28042.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b28042.asm.exe  b28042.dasm.il
    "$CORE_ROOT/ilasm" -output=b28042.asm.exe  b28042.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b28042
    if [ ! -f b28042.org ]; then
        TakeLock
        if [ ! -f b28042.org ]; then
          mkdir IL
          cp b28042.exe IL/b28042.exe
          mv b28042.exe b28042.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28042.org /out b28042.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28042.org /out b28042.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b28042.reflect.xml
  if [ -f b28042.reflect.xml ]
then
    ReflectionRoots="-x b28042.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b28042.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b28042.asm.exe $CLRTestExecutionArguments
