...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b32560/b32560 b32560.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b32560/b32560 b32560.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b32560.dasm.il b32560.exe
    "$CORE_ROOT/ildasm" -raweh -out=b32560.dasm.il b32560.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b32560.asm.exe  b32560.dasm.il
    "$CORE_ROOT/ilasm" -output=b32560.asm.exe  b32560.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b32560
    if [ ! -f b32560.org ]; then
        TakeLock
        if [ ! -f b32560.org ]; then
          mkdir IL
          cp b32560.exe IL/b32560.exe
          mv b32560.exe b32560.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32560.org /out b32560.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b32560.org /out b32560.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b32560.reflect.xml
  if [ -f b32560.reflect.xml ]
then
    ReflectionRoots="-x b32560.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b32560.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b32560.asm.exe $CLRTestExecutionArguments
