...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b32551\b32551b b32551b.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b32551\b32551b b32551b.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b32551b.dasm.il b32551b.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b32551b.dasm.il b32551b.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b32551b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32551b.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b32551b.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b32551b.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b32551b
    if not exist "b32551b.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b32551b.org" (
            mkdir IL
            copy b32551b.exe IL\b32551b.exe 
            ren b32551b.exe b32551b.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b32551b.org /out b32551b.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b32551b.org /out b32551b.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b32551b.reflect.xml
    IF EXIST b32551b.reflect.xml set ReflectionRoots=-x b32551b.reflect.xml
    ...
    set ExePath=%LinkBin%\b32551b.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b32551b.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b32551b.asm.exe %CLRTestExecutionArguments% %Host_Args%
