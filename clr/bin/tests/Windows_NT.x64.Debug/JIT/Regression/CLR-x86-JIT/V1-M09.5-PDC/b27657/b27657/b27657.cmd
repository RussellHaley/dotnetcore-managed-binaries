...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b27657\b27657 b27657.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b27657\b27657 b27657.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b27657.dasm.il b27657.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b27657.dasm.il b27657.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b27657.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27657.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b27657.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27657.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b27657
    if not exist "b27657.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b27657.org" (
            mkdir IL
            copy b27657.exe IL\b27657.exe 
            ren b27657.exe b27657.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b27657.org /out b27657.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b27657.org /out b27657.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b27657.reflect.xml
    IF EXIST b27657.reflect.xml set ReflectionRoots=-x b27657.reflect.xml
    ...
    set ExePath=%LinkBin%\b27657.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b27657.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b27657.asm.exe %CLRTestExecutionArguments% %Host_Args%
