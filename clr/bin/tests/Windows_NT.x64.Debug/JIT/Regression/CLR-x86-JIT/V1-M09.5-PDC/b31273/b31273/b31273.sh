...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b31273/b31273 b31273.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b31273/b31273 b31273.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31273.dasm.il b31273.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31273.dasm.il b31273.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31273.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31273.dasm.il
    "$CORE_ROOT/ilasm" -output=b31273.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b31273.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31273
    if [ ! -f b31273.org ]; then
        TakeLock
        if [ ! -f b31273.org ]; then
          mkdir IL
          cp b31273.exe IL/b31273.exe
          mv b31273.exe b31273.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31273.org /out b31273.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31273.org /out b31273.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31273.reflect.xml
  if [ -f b31273.reflect.xml ]
then
    ReflectionRoots="-x b31273.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31273.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31273.asm.exe $CLRTestExecutionArguments
