...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b20217/b20217 b20217.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b20217/b20217 b20217.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b20217.dasm.il b20217.exe
    "$CORE_ROOT/ildasm" -raweh -out=b20217.dasm.il b20217.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b20217.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20217.dasm.il
    "$CORE_ROOT/ilasm" -output=b20217.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20217.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b20217
    if [ ! -f b20217.org ]; then
        TakeLock
        if [ ! -f b20217.org ]; then
          mkdir IL
          cp b20217.exe IL/b20217.exe
          mv b20217.exe b20217.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b20217.org /out b20217.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b20217.org /out b20217.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b20217.reflect.xml
  if [ -f b20217.reflect.xml ]
then
    ReflectionRoots="-x b20217.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b20217.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b20217.asm.exe $CLRTestExecutionArguments
