...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b20217\b20217 b20217.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b20217\b20217 b20217.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b20217.dasm.il b20217.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b20217.dasm.il b20217.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b20217.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20217.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b20217.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20217.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b20217
    if not exist "b20217.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b20217.org" (
            mkdir IL
            copy b20217.exe IL\b20217.exe 
            ren b20217.exe b20217.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b20217.org /out b20217.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b20217.org /out b20217.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b20217.reflect.xml
    IF EXIST b20217.reflect.xml set ReflectionRoots=-x b20217.reflect.xml
    ...
    set ExePath=%LinkBin%\b20217.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b20217.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b20217.asm.exe %CLRTestExecutionArguments% %Host_Args%
