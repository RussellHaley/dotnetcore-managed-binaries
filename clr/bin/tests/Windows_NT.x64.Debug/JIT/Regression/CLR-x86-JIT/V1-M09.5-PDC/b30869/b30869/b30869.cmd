...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30869\b30869 b30869.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b30869\b30869 b30869.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b30869.dasm.il b30869.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b30869.dasm.il b30869.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b30869.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30869.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b30869.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30869.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b30869
    if not exist "b30869.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b30869.org" (
            mkdir IL
            copy b30869.exe IL\b30869.exe 
            ren b30869.exe b30869.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30869.org /out b30869.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30869.org /out b30869.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b30869.reflect.xml
    IF EXIST b30869.reflect.xml set ReflectionRoots=-x b30869.reflect.xml
    ...
    set ExePath=%LinkBin%\b30869.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b30869.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b30869.asm.exe %CLRTestExecutionArguments% %Host_Args%
