...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b30862/b30862 b30862.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b30862/b30862 b30862.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b30862.dasm.il b30862.exe
    "$CORE_ROOT/ildasm" -raweh -out=b30862.dasm.il b30862.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b30862.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30862.dasm.il
    "$CORE_ROOT/ilasm" -output=b30862.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b30862.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b30862
    if [ ! -f b30862.org ]; then
        TakeLock
        if [ ! -f b30862.org ]; then
          mkdir IL
          cp b30862.exe IL/b30862.exe
          mv b30862.exe b30862.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30862.org /out b30862.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30862.org /out b30862.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b30862.reflect.xml
  if [ -f b30862.reflect.xml ]
then
    ReflectionRoots="-x b30862.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b30862.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b30862.asm.exe $CLRTestExecutionArguments
