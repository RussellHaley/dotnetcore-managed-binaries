...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28595/b28595 b28595.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b28595/b28595 b28595.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b28595.dasm.il b28595.exe
    "$CORE_ROOT/ildasm" -raweh -out=b28595.dasm.il b28595.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b28595.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28595.dasm.il
    "$CORE_ROOT/ilasm" -output=b28595.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b28595.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b28595
    if [ ! -f b28595.org ]; then
        TakeLock
        if [ ! -f b28595.org ]; then
          mkdir IL
          cp b28595.exe IL/b28595.exe
          mv b28595.exe b28595.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28595.org /out b28595.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b28595.org /out b28595.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b28595.reflect.xml
  if [ -f b28595.reflect.xml ]
then
    ReflectionRoots="-x b28595.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b28595.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b28595.asm.exe $CLRTestExecutionArguments
