...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b20249\b20249 b20249.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b20249\b20249 b20249.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b20249.dasm.il b20249.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b20249.dasm.il b20249.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b20249.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20249.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b20249.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b20249.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b20249
    if not exist "b20249.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b20249.org" (
            mkdir IL
            copy b20249.exe IL\b20249.exe 
            ren b20249.exe b20249.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b20249.org /out b20249.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b20249.org /out b20249.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b20249.reflect.xml
    IF EXIST b20249.reflect.xml set ReflectionRoots=-x b20249.reflect.xml
    ...
    set ExePath=%LinkBin%\b20249.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b20249.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b20249.asm.exe %CLRTestExecutionArguments% %Host_Args%
