...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b29068/b29068 b29068.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b29068/b29068 b29068.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b29068.dasm.il b29068.exe
    "$CORE_ROOT/ildasm" -raweh -out=b29068.dasm.il b29068.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b29068.asm.exe  b29068.dasm.il
    "$CORE_ROOT/ilasm" -output=b29068.asm.exe  b29068.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b29068
    if [ ! -f b29068.org ]; then
        TakeLock
        if [ ! -f b29068.org ]; then
          mkdir IL
          cp b29068.exe IL/b29068.exe
          mv b29068.exe b29068.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b29068.org /out b29068.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b29068.org /out b29068.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b29068.reflect.xml
  if [ -f b29068.reflect.xml ]
then
    ReflectionRoots="-x b29068.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b29068.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b29068.asm.exe $CLRTestExecutionArguments
