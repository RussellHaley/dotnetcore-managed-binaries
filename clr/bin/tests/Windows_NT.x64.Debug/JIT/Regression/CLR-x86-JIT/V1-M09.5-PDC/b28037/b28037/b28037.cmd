...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b28037\b28037 b28037.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M09.5-PDC\b28037\b28037 b28037.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b28037.dasm.il b28037.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b28037.dasm.il b28037.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b28037.asm.exe  b28037.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b28037.asm.exe  b28037.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b28037
    if not exist "b28037.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b28037.org" (
            mkdir IL
            copy b28037.exe IL\b28037.exe 
            ren b28037.exe b28037.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b28037.org /out b28037.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b28037.org /out b28037.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b28037.reflect.xml
    IF EXIST b28037.reflect.xml set ReflectionRoots=-x b28037.reflect.xml
    ...
    set ExePath=%LinkBin%\b28037.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b28037.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b28037.asm.exe %CLRTestExecutionArguments% %Host_Args%
