...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25815/b25815 b25815.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M09.5-PDC/b25815/b25815 b25815.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b25815.dasm.il b25815.exe
    "$CORE_ROOT/ildasm" -raweh -out=b25815.dasm.il b25815.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b25815.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25815.dasm.il
    "$CORE_ROOT/ilasm" -output=b25815.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b25815.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b25815
    if [ ! -f b25815.org ]; then
        TakeLock
        if [ ! -f b25815.org ]; then
          mkdir IL
          cp b25815.exe IL/b25815.exe
          mv b25815.exe b25815.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25815.org /out b25815.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b25815.org /out b25815.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b25815.reflect.xml
  if [ -f b25815.reflect.xml ]
then
    ReflectionRoots="-x b25815.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b25815.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b25815.asm.exe $CLRTestExecutionArguments
