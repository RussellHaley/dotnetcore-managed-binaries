...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b08944/b08944b b08944b.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b08944/b08944b b08944b.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b08944b.dasm.il b08944b.exe
    "$CORE_ROOT/ildasm" -raweh -out=b08944b.dasm.il b08944b.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b08944b.asm.exe  b08944b.dasm.il
    "$CORE_ROOT/ilasm" -output=b08944b.asm.exe  b08944b.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b08944b
    if [ ! -f b08944b.org ]; then
        TakeLock
        if [ ! -f b08944b.org ]; then
          mkdir IL
          cp b08944b.exe IL/b08944b.exe
          mv b08944b.exe b08944b.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b08944b.org /out b08944b.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b08944b.org /out b08944b.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b08944b.reflect.xml
  if [ -f b08944b.reflect.xml ]
then
    ReflectionRoots="-x b08944b.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b08944b.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b08944b.asm.exe $CLRTestExecutionArguments
