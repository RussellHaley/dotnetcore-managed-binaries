...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b02352\b02352 b02352.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b02352\b02352 b02352.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b02352.dasm.il b02352.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b02352.dasm.il b02352.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b02352.asm.exe  b02352.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b02352.asm.exe  b02352.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b02352
    if not exist "b02352.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b02352.org" (
            mkdir IL
            copy b02352.exe IL\b02352.exe 
            ren b02352.exe b02352.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b02352.org /out b02352.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b02352.org /out b02352.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b02352.reflect.xml
    IF EXIST b02352.reflect.xml set ReflectionRoots=-x b02352.reflect.xml
    ...
    set ExePath=%LinkBin%\b02352.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b02352.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b02352.asm.exe %CLRTestExecutionArguments% %Host_Args%
