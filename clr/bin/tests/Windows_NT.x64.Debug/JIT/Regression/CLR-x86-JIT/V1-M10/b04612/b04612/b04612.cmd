...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b04612\b04612 b04612.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b04612\b04612 b04612.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b04612.dasm.il b04612.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b04612.dasm.il b04612.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b04612.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b04612.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b04612.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b04612.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b04612
    if not exist "b04612.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b04612.org" (
            mkdir IL
            copy b04612.exe IL\b04612.exe 
            ren b04612.exe b04612.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b04612.org /out b04612.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b04612.org /out b04612.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b04612.reflect.xml
    IF EXIST b04612.reflect.xml set ReflectionRoots=-x b04612.reflect.xml
    ...
    set ExePath=%LinkBin%\b04612.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b04612.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b04612.asm.exe %CLRTestExecutionArguments% %Host_Args%
