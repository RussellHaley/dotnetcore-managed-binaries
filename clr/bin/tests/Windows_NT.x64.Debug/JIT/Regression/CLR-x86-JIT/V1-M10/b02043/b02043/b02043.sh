...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b02043/b02043 b02043.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b02043/b02043 b02043.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b02043.dasm.il b02043.exe
    "$CORE_ROOT/ildasm" -raweh -out=b02043.dasm.il b02043.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b02043.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b02043.dasm.il
    "$CORE_ROOT/ilasm" -output=b02043.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b02043.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b02043
    if [ ! -f b02043.org ]; then
        TakeLock
        if [ ! -f b02043.org ]; then
          mkdir IL
          cp b02043.exe IL/b02043.exe
          mv b02043.exe b02043.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b02043.org /out b02043.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b02043.org /out b02043.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b02043.reflect.xml
  if [ -f b02043.reflect.xml ]
then
    ReflectionRoots="-x b02043.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b02043.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b02043.asm.exe $CLRTestExecutionArguments
