...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b08944\b08944b b08944b.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b08944\b08944b b08944b.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b08944b.dasm.il b08944b.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b08944b.dasm.il b08944b.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b08944b.asm.exe  b08944b.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b08944b.asm.exe  b08944b.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b08944b
    if not exist "b08944b.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b08944b.org" (
            mkdir IL
            copy b08944b.exe IL\b08944b.exe 
            ren b08944b.exe b08944b.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b08944b.org /out b08944b.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b08944b.org /out b08944b.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b08944b.reflect.xml
    IF EXIST b08944b.reflect.xml set ReflectionRoots=-x b08944b.reflect.xml
    ...
    set ExePath=%LinkBin%\b08944b.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b08944b.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b08944b.asm.exe %CLRTestExecutionArguments% %Host_Args%
