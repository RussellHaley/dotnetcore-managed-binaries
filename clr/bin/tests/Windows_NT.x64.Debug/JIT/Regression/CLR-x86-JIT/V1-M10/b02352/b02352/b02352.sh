...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b02352/b02352 b02352.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b02352/b02352 b02352.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b02352.dasm.il b02352.exe
    "$CORE_ROOT/ildasm" -raweh -out=b02352.dasm.il b02352.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b02352.asm.exe  b02352.dasm.il
    "$CORE_ROOT/ilasm" -output=b02352.asm.exe  b02352.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b02352
    if [ ! -f b02352.org ]; then
        TakeLock
        if [ ! -f b02352.org ]; then
          mkdir IL
          cp b02352.exe IL/b02352.exe
          mv b02352.exe b02352.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b02352.org /out b02352.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b02352.org /out b02352.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b02352.reflect.xml
  if [ -f b02352.reflect.xml ]
then
    ReflectionRoots="-x b02352.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b02352.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b02352.asm.exe $CLRTestExecutionArguments
