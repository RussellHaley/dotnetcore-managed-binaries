...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b08944/b08944a b08944a.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b08944/b08944a b08944a.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b08944a.dasm.il b08944a.exe
    "$CORE_ROOT/ildasm" -raweh -out=b08944a.dasm.il b08944a.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b08944a.asm.exe  b08944a.dasm.il
    "$CORE_ROOT/ilasm" -output=b08944a.asm.exe  b08944a.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b08944a
    if [ ! -f b08944a.org ]; then
        TakeLock
        if [ ! -f b08944a.org ]; then
          mkdir IL
          cp b08944a.exe IL/b08944a.exe
          mv b08944a.exe b08944a.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b08944a.org /out b08944a.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b08944a.org /out b08944a.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b08944a.reflect.xml
  if [ -f b08944a.reflect.xml ]
then
    ReflectionRoots="-x b08944a.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b08944a.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b08944a.asm.exe $CLRTestExecutionArguments
