...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b06812/b06812 b06812.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b06812/b06812 b06812.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b06812.dasm.il b06812.exe
    "$CORE_ROOT/ildasm" -raweh -out=b06812.dasm.il b06812.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b06812.asm.exe  b06812.dasm.il
    "$CORE_ROOT/ilasm" -output=b06812.asm.exe  b06812.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b06812
    if [ ! -f b06812.org ]; then
        TakeLock
        if [ ! -f b06812.org ]; then
          mkdir IL
          cp b06812.exe IL/b06812.exe
          mv b06812.exe b06812.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b06812.org /out b06812.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b06812.org /out b06812.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b06812.reflect.xml
  if [ -f b06812.reflect.xml ]
then
    ReflectionRoots="-x b06812.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b06812.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b06812.asm.exe $CLRTestExecutionArguments
