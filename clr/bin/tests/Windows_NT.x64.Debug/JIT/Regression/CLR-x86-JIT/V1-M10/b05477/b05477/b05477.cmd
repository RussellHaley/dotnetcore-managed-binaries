...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b05477\b05477 b05477.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b05477\b05477 b05477.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b05477.dasm.il b05477.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b05477.dasm.il b05477.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b05477.asm.exe  b05477.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b05477.asm.exe  b05477.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b05477
    if not exist "b05477.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b05477.org" (
            mkdir IL
            copy b05477.exe IL\b05477.exe 
            ren b05477.exe b05477.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b05477.org /out b05477.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b05477.org /out b05477.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b05477.reflect.xml
    IF EXIST b05477.reflect.xml set ReflectionRoots=-x b05477.reflect.xml
    ...
    set ExePath=%LinkBin%\b05477.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b05477.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b05477.asm.exe %CLRTestExecutionArguments% %Host_Args%
