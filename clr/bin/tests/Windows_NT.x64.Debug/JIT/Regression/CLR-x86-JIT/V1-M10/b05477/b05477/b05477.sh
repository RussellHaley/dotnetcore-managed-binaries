...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b05477/b05477 b05477.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b05477/b05477 b05477.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b05477.dasm.il b05477.exe
    "$CORE_ROOT/ildasm" -raweh -out=b05477.dasm.il b05477.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b05477.asm.exe  b05477.dasm.il
    "$CORE_ROOT/ilasm" -output=b05477.asm.exe  b05477.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b05477
    if [ ! -f b05477.org ]; then
        TakeLock
        if [ ! -f b05477.org ]; then
          mkdir IL
          cp b05477.exe IL/b05477.exe
          mv b05477.exe b05477.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b05477.org /out b05477.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b05477.org /out b05477.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b05477.reflect.xml
  if [ -f b05477.reflect.xml ]
then
    ReflectionRoots="-x b05477.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b05477.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b05477.asm.exe $CLRTestExecutionArguments
