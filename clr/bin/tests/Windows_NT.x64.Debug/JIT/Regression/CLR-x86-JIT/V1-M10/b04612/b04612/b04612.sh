...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b04612/b04612 b04612.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M10/b04612/b04612 b04612.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b04612.dasm.il b04612.exe
    "$CORE_ROOT/ildasm" -raweh -out=b04612.dasm.il b04612.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b04612.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b04612.dasm.il
    "$CORE_ROOT/ilasm" -output=b04612.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b04612.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b04612
    if [ ! -f b04612.org ]; then
        TakeLock
        if [ ! -f b04612.org ]; then
          mkdir IL
          cp b04612.exe IL/b04612.exe
          mv b04612.exe b04612.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b04612.org /out b04612.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b04612.org /out b04612.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b04612.reflect.xml
  if [ -f b04612.reflect.xml ]
then
    ReflectionRoots="-x b04612.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b04612.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b04612.asm.exe $CLRTestExecutionArguments
