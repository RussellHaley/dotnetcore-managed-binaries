...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b06812\b06812 b06812.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M10\b06812\b06812 b06812.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b06812.dasm.il b06812.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b06812.dasm.il b06812.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b06812.asm.exe  b06812.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b06812.asm.exe  b06812.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b06812
    if not exist "b06812.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b06812.org" (
            mkdir IL
            copy b06812.exe IL\b06812.exe 
            ren b06812.exe b06812.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b06812.org /out b06812.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b06812.org /out b06812.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b06812.reflect.xml
    IF EXIST b06812.reflect.xml set ReflectionRoots=-x b06812.reflect.xml
    ...
    set ExePath=%LinkBin%\b06812.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b06812.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b06812.asm.exe %CLRTestExecutionArguments% %Host_Args%
