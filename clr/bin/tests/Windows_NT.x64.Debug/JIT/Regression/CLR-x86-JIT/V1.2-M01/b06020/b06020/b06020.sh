...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b06020/b06020 b06020.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b06020/b06020 b06020.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b06020.dasm.il b06020.exe
    "$CORE_ROOT/ildasm" -raweh -out=b06020.dasm.il b06020.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b06020.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b06020.dasm.il
    "$CORE_ROOT/ilasm" -output=b06020.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b06020.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b06020
    if [ ! -f b06020.org ]; then
        TakeLock
        if [ ! -f b06020.org ]; then
          mkdir IL
          cp b06020.exe IL/b06020.exe
          mv b06020.exe b06020.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b06020.org /out b06020.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b06020.org /out b06020.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b06020.reflect.xml
  if [ -f b06020.reflect.xml ]
then
    ReflectionRoots="-x b06020.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b06020.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b06020.asm.exe $CLRTestExecutionArguments
