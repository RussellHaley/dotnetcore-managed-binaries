...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b07900/b07900 b07900.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b07900/b07900 b07900.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b07900.dasm.il b07900.exe
    "$CORE_ROOT/ildasm" -raweh -out=b07900.dasm.il b07900.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b07900.asm.exe  b07900.dasm.il
    "$CORE_ROOT/ilasm" -output=b07900.asm.exe  b07900.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b07900
    if [ ! -f b07900.org ]; then
        TakeLock
        if [ ! -f b07900.org ]; then
          mkdir IL
          cp b07900.exe IL/b07900.exe
          mv b07900.exe b07900.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b07900.org /out b07900.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b07900.org /out b07900.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b07900.reflect.xml
  if [ -f b07900.reflect.xml ]
then
    ReflectionRoots="-x b07900.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b07900.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b07900.asm.exe $CLRTestExecutionArguments
