...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b07947\b07947 b07947.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b07947\b07947 b07947.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b07947.dasm.il b07947.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b07947.dasm.il b07947.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b07947.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b07947.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b07947.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b07947.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b07947
    if not exist "b07947.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b07947.org" (
            mkdir IL
            copy b07947.exe IL\b07947.exe 
            ren b07947.exe b07947.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b07947.org /out b07947.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b07947.org /out b07947.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b07947.reflect.xml
    IF EXIST b07947.reflect.xml set ReflectionRoots=-x b07947.reflect.xml
    ...
    set ExePath=%LinkBin%\b07947.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b07947.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b07947.asm.exe %CLRTestExecutionArguments% %Host_Args%
