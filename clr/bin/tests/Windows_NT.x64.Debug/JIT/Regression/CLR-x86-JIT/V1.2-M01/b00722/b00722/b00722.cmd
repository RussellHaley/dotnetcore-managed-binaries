...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b00722\b00722 b00722.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b00722\b00722 b00722.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b00722.dasm.il b00722.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b00722.dasm.il b00722.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b00722.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b00722.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b00722.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b00722.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b00722
    if not exist "b00722.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b00722.org" (
            mkdir IL
            copy b00722.exe IL\b00722.exe 
            ren b00722.exe b00722.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b00722.org /out b00722.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b00722.org /out b00722.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b00722.reflect.xml
    IF EXIST b00722.reflect.xml set ReflectionRoots=-x b00722.reflect.xml
    ...
    set ExePath=%LinkBin%\b00722.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b00722.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b00722.asm.exe %CLRTestExecutionArguments% %Host_Args%
