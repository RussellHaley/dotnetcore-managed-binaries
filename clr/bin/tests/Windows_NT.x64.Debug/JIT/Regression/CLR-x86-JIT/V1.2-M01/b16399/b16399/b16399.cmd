...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b16399\b16399 b16399.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b16399\b16399 b16399.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16399.dasm.il b16399.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16399.dasm.il b16399.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16399.asm.exe  b16399.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16399.asm.exe  b16399.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16399
    if not exist "b16399.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16399.org" (
            mkdir IL
            copy b16399.exe IL\b16399.exe 
            ren b16399.exe b16399.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16399.org /out b16399.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16399.org /out b16399.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16399.reflect.xml
    IF EXIST b16399.reflect.xml set ReflectionRoots=-x b16399.reflect.xml
    ...
    set ExePath=%LinkBin%\b16399.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16399.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16399.asm.exe %CLRTestExecutionArguments% %Host_Args%
