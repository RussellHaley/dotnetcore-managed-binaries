...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b16473/b16473 b16473.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b16473/b16473 b16473.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16473.dasm.il b16473.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16473.dasm.il b16473.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16473.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16473.dasm.il
    "$CORE_ROOT/ilasm" -output=b16473.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16473.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16473
    if [ ! -f b16473.org ]; then
        TakeLock
        if [ ! -f b16473.org ]; then
          mkdir IL
          cp b16473.exe IL/b16473.exe
          mv b16473.exe b16473.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16473.org /out b16473.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16473.org /out b16473.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16473.reflect.xml
  if [ -f b16473.reflect.xml ]
then
    ReflectionRoots="-x b16473.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16473.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16473.asm.exe $CLRTestExecutionArguments
