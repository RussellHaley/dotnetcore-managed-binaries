...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b00722/b00722 b00722.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b00722/b00722 b00722.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b00722.dasm.il b00722.exe
    "$CORE_ROOT/ildasm" -raweh -out=b00722.dasm.il b00722.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b00722.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b00722.dasm.il
    "$CORE_ROOT/ilasm" -output=b00722.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b00722.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b00722
    if [ ! -f b00722.org ]; then
        TakeLock
        if [ ! -f b00722.org ]; then
          mkdir IL
          cp b00722.exe IL/b00722.exe
          mv b00722.exe b00722.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b00722.org /out b00722.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b00722.org /out b00722.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b00722.reflect.xml
  if [ -f b00722.reflect.xml ]
then
    ReflectionRoots="-x b00722.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b00722.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b00722.asm.exe $CLRTestExecutionArguments
