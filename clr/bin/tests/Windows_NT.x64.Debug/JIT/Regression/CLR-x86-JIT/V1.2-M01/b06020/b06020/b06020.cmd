...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b06020\b06020 b06020.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b06020\b06020 b06020.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b06020.dasm.il b06020.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b06020.dasm.il b06020.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b06020.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b06020.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b06020.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b06020.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b06020
    if not exist "b06020.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b06020.org" (
            mkdir IL
            copy b06020.exe IL\b06020.exe 
            ren b06020.exe b06020.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b06020.org /out b06020.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b06020.org /out b06020.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b06020.reflect.xml
    IF EXIST b06020.reflect.xml set ReflectionRoots=-x b06020.reflect.xml
    ...
    set ExePath=%LinkBin%\b06020.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b06020.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b06020.asm.exe %CLRTestExecutionArguments% %Host_Args%
