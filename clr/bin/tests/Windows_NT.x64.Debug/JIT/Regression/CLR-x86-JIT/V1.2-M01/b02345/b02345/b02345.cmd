...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b02345\b02345 b02345.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b02345\b02345 b02345.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b02345.dasm.il b02345.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b02345.dasm.il b02345.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b02345.asm.exe  b02345.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b02345.asm.exe  b02345.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b02345
    if not exist "b02345.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b02345.org" (
            mkdir IL
            copy b02345.exe IL\b02345.exe 
            ren b02345.exe b02345.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b02345.org /out b02345.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b02345.org /out b02345.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b02345.reflect.xml
    IF EXIST b02345.reflect.xml set ReflectionRoots=-x b02345.reflect.xml
    ...
    set ExePath=%LinkBin%\b02345.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b02345.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b02345.asm.exe %CLRTestExecutionArguments% %Host_Args%
