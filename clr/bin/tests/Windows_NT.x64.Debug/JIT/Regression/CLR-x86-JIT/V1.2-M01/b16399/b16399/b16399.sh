...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b16399/b16399 b16399.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b16399/b16399 b16399.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b16399.dasm.il b16399.exe
    "$CORE_ROOT/ildasm" -raweh -out=b16399.dasm.il b16399.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b16399.asm.exe  b16399.dasm.il
    "$CORE_ROOT/ilasm" -output=b16399.asm.exe  b16399.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b16399
    if [ ! -f b16399.org ]; then
        TakeLock
        if [ ! -f b16399.org ]; then
          mkdir IL
          cp b16399.exe IL/b16399.exe
          mv b16399.exe b16399.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16399.org /out b16399.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b16399.org /out b16399.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b16399.reflect.xml
  if [ -f b16399.reflect.xml ]
then
    ReflectionRoots="-x b16399.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b16399.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b16399.asm.exe $CLRTestExecutionArguments
