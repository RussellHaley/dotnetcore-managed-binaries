...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b07369\b07369 b07369.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b07369\b07369 b07369.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b07369.dasm.il b07369.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b07369.dasm.il b07369.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b07369.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b07369.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b07369.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b07369.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b07369
    if not exist "b07369.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b07369.org" (
            mkdir IL
            copy b07369.exe IL\b07369.exe 
            ren b07369.exe b07369.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b07369.org /out b07369.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b07369.org /out b07369.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b07369.reflect.xml
    IF EXIST b07369.reflect.xml set ReflectionRoots=-x b07369.reflect.xml
    ...
    set ExePath=%LinkBin%\b07369.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b07369.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b07369.asm.exe %CLRTestExecutionArguments% %Host_Args%
