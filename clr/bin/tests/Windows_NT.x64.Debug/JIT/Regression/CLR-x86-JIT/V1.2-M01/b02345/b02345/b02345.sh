...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b02345/b02345 b02345.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b02345/b02345 b02345.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b02345.dasm.il b02345.exe
    "$CORE_ROOT/ildasm" -raweh -out=b02345.dasm.il b02345.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b02345.asm.exe  b02345.dasm.il
    "$CORE_ROOT/ilasm" -output=b02345.asm.exe  b02345.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b02345
    if [ ! -f b02345.org ]; then
        TakeLock
        if [ ! -f b02345.org ]; then
          mkdir IL
          cp b02345.exe IL/b02345.exe
          mv b02345.exe b02345.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b02345.org /out b02345.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b02345.org /out b02345.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b02345.reflect.xml
  if [ -f b02345.reflect.xml ]
then
    ReflectionRoots="-x b02345.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b02345.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b02345.asm.exe $CLRTestExecutionArguments
