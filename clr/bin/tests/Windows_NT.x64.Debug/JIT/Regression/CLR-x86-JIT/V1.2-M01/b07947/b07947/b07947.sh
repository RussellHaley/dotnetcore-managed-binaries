...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b07947/b07947 b07947.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b07947/b07947 b07947.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b07947.dasm.il b07947.exe
    "$CORE_ROOT/ildasm" -raweh -out=b07947.dasm.il b07947.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b07947.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b07947.dasm.il
    "$CORE_ROOT/ilasm" -output=b07947.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b07947.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b07947
    if [ ! -f b07947.org ]; then
        TakeLock
        if [ ! -f b07947.org ]; then
          mkdir IL
          cp b07947.exe IL/b07947.exe
          mv b07947.exe b07947.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b07947.org /out b07947.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b07947.org /out b07947.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b07947.reflect.xml
  if [ -f b07947.reflect.xml ]
then
    ReflectionRoots="-x b07947.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b07947.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b07947.asm.exe $CLRTestExecutionArguments
