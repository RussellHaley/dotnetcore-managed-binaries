...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b16473\b16473 b16473.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1.2-M01\b16473\b16473 b16473.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b16473.dasm.il b16473.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b16473.dasm.il b16473.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b16473.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16473.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b16473.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b16473.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b16473
    if not exist "b16473.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b16473.org" (
            mkdir IL
            copy b16473.exe IL\b16473.exe 
            ren b16473.exe b16473.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16473.org /out b16473.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b16473.org /out b16473.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b16473.reflect.xml
    IF EXIST b16473.reflect.xml set ReflectionRoots=-x b16473.reflect.xml
    ...
    set ExePath=%LinkBin%\b16473.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b16473.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b16473.asm.exe %CLRTestExecutionArguments% %Host_Args%
