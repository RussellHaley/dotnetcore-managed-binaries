...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b04319/b04319 b04319.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.2-M01/b04319/b04319 b04319.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b04319.dasm.il b04319.exe
    "$CORE_ROOT/ildasm" -raweh -out=b04319.dasm.il b04319.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b04319.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b04319.dasm.il
    "$CORE_ROOT/ilasm" -output=b04319.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b04319.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b04319
    if [ ! -f b04319.org ]; then
        TakeLock
        if [ ! -f b04319.org ]; then
          mkdir IL
          cp b04319.exe IL/b04319.exe
          mv b04319.exe b04319.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b04319.org /out b04319.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b04319.org /out b04319.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b04319.reflect.xml
  if [ -f b04319.reflect.xml ]
then
    ReflectionRoots="-x b04319.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b04319.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b04319.asm.exe $CLRTestExecutionArguments
