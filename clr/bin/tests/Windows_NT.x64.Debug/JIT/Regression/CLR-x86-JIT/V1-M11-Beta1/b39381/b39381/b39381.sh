...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b39381/b39381 b39381.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b39381/b39381 b39381.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b39381.dasm.il b39381.exe
    "$CORE_ROOT/ildasm" -raweh -out=b39381.dasm.il b39381.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b39381.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b39381.dasm.il
    "$CORE_ROOT/ilasm" -output=b39381.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b39381.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b39381
    if [ ! -f b39381.org ]; then
        TakeLock
        if [ ! -f b39381.org ]; then
          mkdir IL
          cp b39381.exe IL/b39381.exe
          mv b39381.exe b39381.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b39381.org /out b39381.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b39381.org /out b39381.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b39381.reflect.xml
  if [ -f b39381.reflect.xml ]
then
    ReflectionRoots="-x b39381.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b39381.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b39381.asm.exe $CLRTestExecutionArguments
