...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b48797/b48797 b48797.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b48797/b48797 b48797.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b48797.dasm.il b48797.exe
    "$CORE_ROOT/ildasm" -raweh -out=b48797.dasm.il b48797.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b48797.asm.exe  b48797.dasm.il
    "$CORE_ROOT/ilasm" -output=b48797.asm.exe  b48797.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b48797
    if [ ! -f b48797.org ]; then
        TakeLock
        if [ ! -f b48797.org ]; then
          mkdir IL
          cp b48797.exe IL/b48797.exe
          mv b48797.exe b48797.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b48797.org /out b48797.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b48797.org /out b48797.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b48797.reflect.xml
  if [ -f b48797.reflect.xml ]
then
    ReflectionRoots="-x b48797.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b48797.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b48797.asm.exe $CLRTestExecutionArguments
