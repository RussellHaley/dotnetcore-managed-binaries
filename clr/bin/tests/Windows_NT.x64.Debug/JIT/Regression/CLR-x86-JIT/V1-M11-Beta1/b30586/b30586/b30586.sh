...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b30586/b30586 b30586.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b30586/b30586 b30586.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b30586.dasm.il b30586.exe
    "$CORE_ROOT/ildasm" -raweh -out=b30586.dasm.il b30586.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b30586.asm.exe  b30586.dasm.il
    "$CORE_ROOT/ilasm" -output=b30586.asm.exe  b30586.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b30586
    if [ ! -f b30586.org ]; then
        TakeLock
        if [ ! -f b30586.org ]; then
          mkdir IL
          cp b30586.exe IL/b30586.exe
          mv b30586.exe b30586.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30586.org /out b30586.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b30586.org /out b30586.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b30586.reflect.xml
  if [ -f b30586.reflect.xml ]
then
    ReflectionRoots="-x b30586.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b30586.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b30586.asm.exe $CLRTestExecutionArguments
