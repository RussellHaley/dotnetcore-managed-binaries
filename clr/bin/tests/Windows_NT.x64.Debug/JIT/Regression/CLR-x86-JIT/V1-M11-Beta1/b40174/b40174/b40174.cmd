...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b40174\b40174 b40174.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b40174\b40174 b40174.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b40174.dasm.il b40174.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b40174.dasm.il b40174.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b40174.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b40174.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b40174.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b40174.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b40174
    if not exist "b40174.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b40174.org" (
            mkdir IL
            copy b40174.exe IL\b40174.exe 
            ren b40174.exe b40174.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b40174.org /out b40174.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b40174.org /out b40174.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b40174.reflect.xml
    IF EXIST b40174.reflect.xml set ReflectionRoots=-x b40174.reflect.xml
    ...
    set ExePath=%LinkBin%\b40174.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b40174.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b40174.asm.exe %CLRTestExecutionArguments% %Host_Args%
