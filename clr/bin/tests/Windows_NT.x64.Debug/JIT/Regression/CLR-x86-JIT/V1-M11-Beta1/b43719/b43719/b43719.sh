...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43719/b43719 b43719.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43719/b43719 b43719.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b43719.dasm.il b43719.exe
    "$CORE_ROOT/ildasm" -raweh -out=b43719.dasm.il b43719.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b43719.asm.exe  b43719.dasm.il
    "$CORE_ROOT/ilasm" -output=b43719.asm.exe  b43719.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b43719
    if [ ! -f b43719.org ]; then
        TakeLock
        if [ ! -f b43719.org ]; then
          mkdir IL
          cp b43719.exe IL/b43719.exe
          mv b43719.exe b43719.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43719.org /out b43719.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43719.org /out b43719.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b43719.reflect.xml
  if [ -f b43719.reflect.xml ]
then
    ReflectionRoots="-x b43719.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b43719.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b43719.asm.exe $CLRTestExecutionArguments
