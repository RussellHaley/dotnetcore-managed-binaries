...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b39417/b39417 b39417.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b39417/b39417 b39417.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b39417.dasm.il b39417.exe
    "$CORE_ROOT/ildasm" -raweh -out=b39417.dasm.il b39417.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b39417.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b39417.dasm.il
    "$CORE_ROOT/ilasm" -output=b39417.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b39417.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b39417
    if [ ! -f b39417.org ]; then
        TakeLock
        if [ ! -f b39417.org ]; then
          mkdir IL
          cp b39417.exe IL/b39417.exe
          mv b39417.exe b39417.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b39417.org /out b39417.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b39417.org /out b39417.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b39417.reflect.xml
  if [ -f b39417.reflect.xml ]
then
    ReflectionRoots="-x b39417.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b39417.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b39417.asm.exe $CLRTestExecutionArguments
