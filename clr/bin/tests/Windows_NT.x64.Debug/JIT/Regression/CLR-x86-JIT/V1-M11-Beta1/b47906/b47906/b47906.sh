...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b47906/b47906 b47906.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b47906/b47906 b47906.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b47906.dasm.il b47906.exe
    "$CORE_ROOT/ildasm" -raweh -out=b47906.dasm.il b47906.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b47906.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47906.dasm.il
    "$CORE_ROOT/ilasm" -output=b47906.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47906.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b47906
    if [ ! -f b47906.org ]; then
        TakeLock
        if [ ! -f b47906.org ]; then
          mkdir IL
          cp b47906.exe IL/b47906.exe
          mv b47906.exe b47906.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b47906.org /out b47906.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b47906.org /out b47906.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b47906.reflect.xml
  if [ -f b47906.reflect.xml ]
then
    ReflectionRoots="-x b47906.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b47906.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b47906.asm.exe $CLRTestExecutionArguments
