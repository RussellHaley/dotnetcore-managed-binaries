...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b45956/b45956 b45956.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b45956/b45956 b45956.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b45956.dasm.il b45956.exe
    "$CORE_ROOT/ildasm" -raweh -out=b45956.dasm.il b45956.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b45956.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45956.dasm.il
    "$CORE_ROOT/ilasm" -output=b45956.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45956.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b45956
    if [ ! -f b45956.org ]; then
        TakeLock
        if [ ! -f b45956.org ]; then
          mkdir IL
          cp b45956.exe IL/b45956.exe
          mv b45956.exe b45956.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b45956.org /out b45956.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b45956.org /out b45956.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b45956.reflect.xml
  if [ -f b45956.reflect.xml ]
then
    ReflectionRoots="-x b45956.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b45956.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b45956.asm.exe $CLRTestExecutionArguments
