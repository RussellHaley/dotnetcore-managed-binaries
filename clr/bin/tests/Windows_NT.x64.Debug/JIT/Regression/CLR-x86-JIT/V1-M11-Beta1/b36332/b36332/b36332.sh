...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b36332/b36332 b36332.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b36332/b36332 b36332.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b36332.dasm.il b36332.exe
    "$CORE_ROOT/ildasm" -raweh -out=b36332.dasm.il b36332.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b36332.asm.exe  b36332.dasm.il
    "$CORE_ROOT/ilasm" -output=b36332.asm.exe  b36332.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b36332
    if [ ! -f b36332.org ]; then
        TakeLock
        if [ ! -f b36332.org ]; then
          mkdir IL
          cp b36332.exe IL/b36332.exe
          mv b36332.exe b36332.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b36332.org /out b36332.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b36332.org /out b36332.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b36332.reflect.xml
  if [ -f b36332.reflect.xml ]
then
    ReflectionRoots="-x b36332.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b36332.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b36332.asm.exe $CLRTestExecutionArguments
