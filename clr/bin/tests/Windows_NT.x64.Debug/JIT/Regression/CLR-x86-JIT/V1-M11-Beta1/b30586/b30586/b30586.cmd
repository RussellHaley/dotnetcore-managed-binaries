...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b30586\b30586 b30586.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b30586\b30586 b30586.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b30586.dasm.il b30586.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b30586.dasm.il b30586.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b30586.asm.exe  b30586.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b30586.asm.exe  b30586.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b30586
    if not exist "b30586.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b30586.org" (
            mkdir IL
            copy b30586.exe IL\b30586.exe 
            ren b30586.exe b30586.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30586.org /out b30586.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b30586.org /out b30586.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b30586.reflect.xml
    IF EXIST b30586.reflect.xml set ReflectionRoots=-x b30586.reflect.xml
    ...
    set ExePath=%LinkBin%\b30586.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b30586.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b30586.asm.exe %CLRTestExecutionArguments% %Host_Args%
