...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b36332\b36332 b36332.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b36332\b36332 b36332.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b36332.dasm.il b36332.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b36332.dasm.il b36332.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b36332.asm.exe  b36332.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b36332.asm.exe  b36332.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b36332
    if not exist "b36332.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b36332.org" (
            mkdir IL
            copy b36332.exe IL\b36332.exe 
            ren b36332.exe b36332.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b36332.org /out b36332.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b36332.org /out b36332.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b36332.reflect.xml
    IF EXIST b36332.reflect.xml set ReflectionRoots=-x b36332.reflect.xml
    ...
    set ExePath=%LinkBin%\b36332.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b36332.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b36332.asm.exe %CLRTestExecutionArguments% %Host_Args%
