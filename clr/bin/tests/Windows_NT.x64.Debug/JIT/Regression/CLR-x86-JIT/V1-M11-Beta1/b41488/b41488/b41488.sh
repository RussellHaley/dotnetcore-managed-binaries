...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41488/b41488 b41488.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41488/b41488 b41488.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b41488.dasm.il b41488.exe
    "$CORE_ROOT/ildasm" -raweh -out=b41488.dasm.il b41488.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b41488.asm.exe  b41488.dasm.il
    "$CORE_ROOT/ilasm" -output=b41488.asm.exe  b41488.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b41488
    if [ ! -f b41488.org ]; then
        TakeLock
        if [ ! -f b41488.org ]; then
          mkdir IL
          cp b41488.exe IL/b41488.exe
          mv b41488.exe b41488.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41488.org /out b41488.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41488.org /out b41488.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b41488.reflect.xml
  if [ -f b41488.reflect.xml ]
then
    ReflectionRoots="-x b41488.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b41488.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b41488.asm.exe $CLRTestExecutionArguments
