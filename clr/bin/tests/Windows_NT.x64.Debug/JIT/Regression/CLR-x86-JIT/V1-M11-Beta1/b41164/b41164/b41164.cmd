...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b41164\b41164 b41164.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b41164\b41164 b41164.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b41164.dasm.il b41164.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b41164.dasm.il b41164.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b41164.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b41164.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b41164.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b41164.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b41164
    if not exist "b41164.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b41164.org" (
            mkdir IL
            copy b41164.exe IL\b41164.exe 
            ren b41164.exe b41164.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b41164.org /out b41164.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b41164.org /out b41164.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b41164.reflect.xml
    IF EXIST b41164.reflect.xml set ReflectionRoots=-x b41164.reflect.xml
    ...
    set ExePath=%LinkBin%\b41164.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b41164.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b41164.asm.exe %CLRTestExecutionArguments% %Host_Args%
