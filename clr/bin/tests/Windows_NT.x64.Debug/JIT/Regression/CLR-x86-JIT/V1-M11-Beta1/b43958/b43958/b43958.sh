...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43958/b43958 b43958.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43958/b43958 b43958.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b43958.dasm.il b43958.exe
    "$CORE_ROOT/ildasm" -raweh -out=b43958.dasm.il b43958.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b43958.asm.exe  b43958.dasm.il
    "$CORE_ROOT/ilasm" -output=b43958.asm.exe  b43958.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b43958
    if [ ! -f b43958.org ]; then
        TakeLock
        if [ ! -f b43958.org ]; then
          mkdir IL
          cp b43958.exe IL/b43958.exe
          mv b43958.exe b43958.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43958.org /out b43958.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43958.org /out b43958.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b43958.reflect.xml
  if [ -f b43958.reflect.xml ]
then
    ReflectionRoots="-x b43958.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b43958.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b43958.asm.exe $CLRTestExecutionArguments
