...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b46641/b46641 b46641.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b46641/b46641 b46641.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b46641.dasm.il b46641.exe
    "$CORE_ROOT/ildasm" -raweh -out=b46641.dasm.il b46641.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b46641.asm.exe  b46641.dasm.il
    "$CORE_ROOT/ilasm" -output=b46641.asm.exe  b46641.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b46641
    if [ ! -f b46641.org ]; then
        TakeLock
        if [ ! -f b46641.org ]; then
          mkdir IL
          cp b46641.exe IL/b46641.exe
          mv b46641.exe b46641.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b46641.org /out b46641.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b46641.org /out b46641.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b46641.reflect.xml
  if [ -f b46641.reflect.xml ]
then
    ReflectionRoots="-x b46641.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b46641.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b46641.asm.exe $CLRTestExecutionArguments
