...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b45439/b45439 b45439.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b45439/b45439 b45439.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b45439.dasm.il b45439.exe
    "$CORE_ROOT/ildasm" -raweh -out=b45439.dasm.il b45439.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b45439.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45439.dasm.il
    "$CORE_ROOT/ilasm" -output=b45439.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45439.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b45439
    if [ ! -f b45439.org ]; then
        TakeLock
        if [ ! -f b45439.org ]; then
          mkdir IL
          cp b45439.exe IL/b45439.exe
          mv b45439.exe b45439.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b45439.org /out b45439.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b45439.org /out b45439.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b45439.reflect.xml
  if [ -f b45439.reflect.xml ]
then
    ReflectionRoots="-x b45439.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b45439.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b45439.asm.exe $CLRTestExecutionArguments
