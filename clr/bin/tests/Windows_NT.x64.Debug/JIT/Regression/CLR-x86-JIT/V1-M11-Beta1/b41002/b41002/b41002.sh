...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41002/b41002 b41002.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41002/b41002 b41002.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b41002.dasm.il b41002.exe
    "$CORE_ROOT/ildasm" -raweh -out=b41002.dasm.il b41002.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b41002.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b41002.dasm.il
    "$CORE_ROOT/ilasm" -output=b41002.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b41002.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b41002
    if [ ! -f b41002.org ]; then
        TakeLock
        if [ ! -f b41002.org ]; then
          mkdir IL
          cp b41002.exe IL/b41002.exe
          mv b41002.exe b41002.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41002.org /out b41002.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41002.org /out b41002.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b41002.reflect.xml
  if [ -f b41002.reflect.xml ]
then
    ReflectionRoots="-x b41002.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b41002.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b41002.asm.exe $CLRTestExecutionArguments
