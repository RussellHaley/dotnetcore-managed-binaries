...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b34945/b34945 b34945.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b34945/b34945 b34945.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b34945.dasm.il b34945.exe
    "$CORE_ROOT/ildasm" -raweh -out=b34945.dasm.il b34945.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b34945.asm.exe  b34945.dasm.il
    "$CORE_ROOT/ilasm" -output=b34945.asm.exe  b34945.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b34945
    if [ ! -f b34945.org ]; then
        TakeLock
        if [ ! -f b34945.org ]; then
          mkdir IL
          cp b34945.exe IL/b34945.exe
          mv b34945.exe b34945.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b34945.org /out b34945.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b34945.org /out b34945.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b34945.reflect.xml
  if [ -f b34945.reflect.xml ]
then
    ReflectionRoots="-x b34945.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b34945.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b34945.asm.exe $CLRTestExecutionArguments
