...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b34945\b34945 b34945.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b34945\b34945 b34945.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b34945.dasm.il b34945.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b34945.dasm.il b34945.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b34945.asm.exe  b34945.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b34945.asm.exe  b34945.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b34945
    if not exist "b34945.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b34945.org" (
            mkdir IL
            copy b34945.exe IL\b34945.exe 
            ren b34945.exe b34945.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b34945.org /out b34945.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b34945.org /out b34945.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b34945.reflect.xml
    IF EXIST b34945.reflect.xml set ReflectionRoots=-x b34945.reflect.xml
    ...
    set ExePath=%LinkBin%\b34945.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b34945.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b34945.asm.exe %CLRTestExecutionArguments% %Host_Args%
