...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43115/b43115 b43115.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43115/b43115 b43115.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b43115.dasm.il b43115.exe
    "$CORE_ROOT/ildasm" -raweh -out=b43115.dasm.il b43115.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b43115.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43115.dasm.il
    "$CORE_ROOT/ilasm" -output=b43115.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43115.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b43115
    if [ ! -f b43115.org ]; then
        TakeLock
        if [ ! -f b43115.org ]; then
          mkdir IL
          cp b43115.exe IL/b43115.exe
          mv b43115.exe b43115.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43115.org /out b43115.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43115.org /out b43115.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b43115.reflect.xml
  if [ -f b43115.reflect.xml ]
then
    ReflectionRoots="-x b43115.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b43115.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b43115.asm.exe $CLRTestExecutionArguments
