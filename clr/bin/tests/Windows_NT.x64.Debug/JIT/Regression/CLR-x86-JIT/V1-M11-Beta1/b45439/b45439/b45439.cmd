...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b45439\b45439 b45439.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b45439\b45439 b45439.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b45439.dasm.il b45439.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b45439.dasm.il b45439.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b45439.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45439.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b45439.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45439.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b45439
    if not exist "b45439.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b45439.org" (
            mkdir IL
            copy b45439.exe IL\b45439.exe 
            ren b45439.exe b45439.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b45439.org /out b45439.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b45439.org /out b45439.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b45439.reflect.xml
    IF EXIST b45439.reflect.xml set ReflectionRoots=-x b45439.reflect.xml
    ...
    set ExePath=%LinkBin%\b45439.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b45439.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b45439.asm.exe %CLRTestExecutionArguments% %Host_Args%
