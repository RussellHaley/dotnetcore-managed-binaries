...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b33888/b33888 b33888.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b33888/b33888 b33888.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b33888.dasm.il b33888.exe
    "$CORE_ROOT/ildasm" -raweh -out=b33888.dasm.il b33888.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b33888.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33888.dasm.il
    "$CORE_ROOT/ilasm" -output=b33888.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33888.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b33888
    if [ ! -f b33888.org ]; then
        TakeLock
        if [ ! -f b33888.org ]; then
          mkdir IL
          cp b33888.exe IL/b33888.exe
          mv b33888.exe b33888.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b33888.org /out b33888.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b33888.org /out b33888.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b33888.reflect.xml
  if [ -f b33888.reflect.xml ]
then
    ReflectionRoots="-x b33888.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b33888.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b33888.asm.exe $CLRTestExecutionArguments
