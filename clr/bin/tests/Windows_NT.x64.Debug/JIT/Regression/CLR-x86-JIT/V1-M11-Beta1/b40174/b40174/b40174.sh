...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40174/b40174 b40174.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40174/b40174 b40174.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b40174.dasm.il b40174.exe
    "$CORE_ROOT/ildasm" -raweh -out=b40174.dasm.il b40174.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b40174.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b40174.dasm.il
    "$CORE_ROOT/ilasm" -output=b40174.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b40174.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b40174
    if [ ! -f b40174.org ]; then
        TakeLock
        if [ ! -f b40174.org ]; then
          mkdir IL
          cp b40174.exe IL/b40174.exe
          mv b40174.exe b40174.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40174.org /out b40174.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40174.org /out b40174.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b40174.reflect.xml
  if [ -f b40174.reflect.xml ]
then
    ReflectionRoots="-x b40174.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b40174.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b40174.asm.exe $CLRTestExecutionArguments
