...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b44204/b44204 b44204.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b44204/b44204 b44204.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b44204.dasm.il b44204.exe
    "$CORE_ROOT/ildasm" -raweh -out=b44204.dasm.il b44204.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b44204.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44204.dasm.il
    "$CORE_ROOT/ilasm" -output=b44204.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44204.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b44204
    if [ ! -f b44204.org ]; then
        TakeLock
        if [ ! -f b44204.org ]; then
          mkdir IL
          cp b44204.exe IL/b44204.exe
          mv b44204.exe b44204.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b44204.org /out b44204.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b44204.org /out b44204.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b44204.reflect.xml
  if [ -f b44204.reflect.xml ]
then
    ReflectionRoots="-x b44204.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b44204.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b44204.asm.exe $CLRTestExecutionArguments
