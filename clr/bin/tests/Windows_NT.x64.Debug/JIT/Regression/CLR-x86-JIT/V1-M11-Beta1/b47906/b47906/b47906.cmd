...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b47906\b47906 b47906.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b47906\b47906 b47906.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b47906.dasm.il b47906.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b47906.dasm.il b47906.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b47906.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47906.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b47906.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47906.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b47906
    if not exist "b47906.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b47906.org" (
            mkdir IL
            copy b47906.exe IL\b47906.exe 
            ren b47906.exe b47906.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47906.org /out b47906.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47906.org /out b47906.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b47906.reflect.xml
    IF EXIST b47906.reflect.xml set ReflectionRoots=-x b47906.reflect.xml
    ...
    set ExePath=%LinkBin%\b47906.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b47906.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b47906.asm.exe %CLRTestExecutionArguments% %Host_Args%
