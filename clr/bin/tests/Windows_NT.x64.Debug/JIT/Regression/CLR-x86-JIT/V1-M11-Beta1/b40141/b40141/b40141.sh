...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40141/b40141 b40141.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40141/b40141 b40141.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b40141.dasm.il b40141.exe
    "$CORE_ROOT/ildasm" -raweh -out=b40141.dasm.il b40141.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b40141.asm.exe  b40141.dasm.il
    "$CORE_ROOT/ilasm" -output=b40141.asm.exe  b40141.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b40141
    if [ ! -f b40141.org ]; then
        TakeLock
        if [ ! -f b40141.org ]; then
          mkdir IL
          cp b40141.exe IL/b40141.exe
          mv b40141.exe b40141.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40141.org /out b40141.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40141.org /out b40141.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b40141.reflect.xml
  if [ -f b40141.reflect.xml ]
then
    ReflectionRoots="-x b40141.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b40141.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b40141.asm.exe $CLRTestExecutionArguments
