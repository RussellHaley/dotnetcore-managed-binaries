...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b31878/b31878 b31878.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b31878/b31878 b31878.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b31878.dasm.il b31878.exe
    "$CORE_ROOT/ildasm" -raweh -out=b31878.dasm.il b31878.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b31878.asm.exe  b31878.dasm.il
    "$CORE_ROOT/ilasm" -output=b31878.asm.exe  b31878.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b31878
    if [ ! -f b31878.org ]; then
        TakeLock
        if [ ! -f b31878.org ]; then
          mkdir IL
          cp b31878.exe IL/b31878.exe
          mv b31878.exe b31878.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31878.org /out b31878.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b31878.org /out b31878.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b31878.reflect.xml
  if [ -f b31878.reflect.xml ]
then
    ReflectionRoots="-x b31878.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b31878.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b31878.asm.exe $CLRTestExecutionArguments
