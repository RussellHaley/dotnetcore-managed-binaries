...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41126/b41126 b41126.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41126/b41126 b41126.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b41126.dasm.il b41126.exe
    "$CORE_ROOT/ildasm" -raweh -out=b41126.dasm.il b41126.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b41126.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b41126.dasm.il
    "$CORE_ROOT/ilasm" -output=b41126.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b41126.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b41126
    if [ ! -f b41126.org ]; then
        TakeLock
        if [ ! -f b41126.org ]; then
          mkdir IL
          cp b41126.exe IL/b41126.exe
          mv b41126.exe b41126.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41126.org /out b41126.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41126.org /out b41126.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b41126.reflect.xml
  if [ -f b41126.reflect.xml ]
then
    ReflectionRoots="-x b41126.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b41126.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b41126.asm.exe $CLRTestExecutionArguments
