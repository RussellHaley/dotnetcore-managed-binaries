...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40496/b40496 b40496.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40496/b40496 b40496.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b40496.dasm.il b40496.exe
    "$CORE_ROOT/ildasm" -raweh -out=b40496.dasm.il b40496.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b40496.asm.exe  b40496.dasm.il
    "$CORE_ROOT/ilasm" -output=b40496.asm.exe  b40496.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b40496
    if [ ! -f b40496.org ]; then
        TakeLock
        if [ ! -f b40496.org ]; then
          mkdir IL
          cp b40496.exe IL/b40496.exe
          mv b40496.exe b40496.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40496.org /out b40496.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40496.org /out b40496.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b40496.reflect.xml
  if [ -f b40496.reflect.xml ]
then
    ReflectionRoots="-x b40496.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b40496.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b40496.asm.exe $CLRTestExecutionArguments
