...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b43714\b43714 b43714.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b43714\b43714 b43714.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b43714.dasm.il b43714.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b43714.dasm.il b43714.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b43714.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43714.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b43714.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43714.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b43714
    if not exist "b43714.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b43714.org" (
            mkdir IL
            copy b43714.exe IL\b43714.exe 
            ren b43714.exe b43714.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b43714.org /out b43714.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b43714.org /out b43714.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b43714.reflect.xml
    IF EXIST b43714.reflect.xml set ReflectionRoots=-x b43714.reflect.xml
    ...
    set ExePath=%LinkBin%\b43714.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b43714.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b43714.asm.exe %CLRTestExecutionArguments% %Host_Args%
