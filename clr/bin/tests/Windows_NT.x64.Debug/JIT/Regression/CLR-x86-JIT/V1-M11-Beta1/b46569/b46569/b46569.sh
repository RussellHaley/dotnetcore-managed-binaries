...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b46569/b46569 b46569.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b46569/b46569 b46569.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b46569.dasm.il b46569.exe
    "$CORE_ROOT/ildasm" -raweh -out=b46569.dasm.il b46569.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b46569.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b46569.dasm.il
    "$CORE_ROOT/ilasm" -output=b46569.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b46569.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b46569
    if [ ! -f b46569.org ]; then
        TakeLock
        if [ ! -f b46569.org ]; then
          mkdir IL
          cp b46569.exe IL/b46569.exe
          mv b46569.exe b46569.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b46569.org /out b46569.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b46569.org /out b46569.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b46569.reflect.xml
  if [ -f b46569.reflect.xml ]
then
    ReflectionRoots="-x b46569.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b46569.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b46569.asm.exe $CLRTestExecutionArguments
