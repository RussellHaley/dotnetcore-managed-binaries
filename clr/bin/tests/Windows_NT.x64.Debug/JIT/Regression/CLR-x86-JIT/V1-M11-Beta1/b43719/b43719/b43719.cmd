...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b43719\b43719 b43719.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b43719\b43719 b43719.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b43719.dasm.il b43719.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b43719.dasm.il b43719.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b43719.asm.exe  b43719.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b43719.asm.exe  b43719.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b43719
    if not exist "b43719.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b43719.org" (
            mkdir IL
            copy b43719.exe IL\b43719.exe 
            ren b43719.exe b43719.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b43719.org /out b43719.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b43719.org /out b43719.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b43719.reflect.xml
    IF EXIST b43719.reflect.xml set ReflectionRoots=-x b43719.reflect.xml
    ...
    set ExePath=%LinkBin%\b43719.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b43719.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b43719.asm.exe %CLRTestExecutionArguments% %Host_Args%
