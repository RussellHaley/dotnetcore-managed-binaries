...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b47093/b47093 b47093.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b47093/b47093 b47093.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b47093.dasm.il b47093.exe
    "$CORE_ROOT/ildasm" -raweh -out=b47093.dasm.il b47093.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b47093.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47093.dasm.il
    "$CORE_ROOT/ilasm" -output=b47093.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47093.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b47093
    if [ ! -f b47093.org ]; then
        TakeLock
        if [ ! -f b47093.org ]; then
          mkdir IL
          cp b47093.exe IL/b47093.exe
          mv b47093.exe b47093.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b47093.org /out b47093.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b47093.org /out b47093.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b47093.reflect.xml
  if [ -f b47093.reflect.xml ]
then
    ReflectionRoots="-x b47093.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b47093.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b47093.asm.exe $CLRTestExecutionArguments
