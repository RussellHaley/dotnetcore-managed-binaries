...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b42013\b42013 b42013.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b42013\b42013 b42013.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b42013.dasm.il b42013.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b42013.dasm.il b42013.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b42013.asm.exe  b42013.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b42013.asm.exe  b42013.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b42013
    if not exist "b42013.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b42013.org" (
            mkdir IL
            copy b42013.exe IL\b42013.exe 
            ren b42013.exe b42013.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b42013.org /out b42013.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b42013.org /out b42013.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b42013.reflect.xml
    IF EXIST b42013.reflect.xml set ReflectionRoots=-x b42013.reflect.xml
    ...
    set ExePath=%LinkBin%\b42013.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b42013.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b42013.asm.exe %CLRTestExecutionArguments% %Host_Args%
