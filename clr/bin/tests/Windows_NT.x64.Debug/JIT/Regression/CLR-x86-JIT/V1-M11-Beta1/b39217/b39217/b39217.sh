...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b39217/b39217 b39217.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b39217/b39217 b39217.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b39217.dasm.il b39217.exe
    "$CORE_ROOT/ildasm" -raweh -out=b39217.dasm.il b39217.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b39217.asm.exe  b39217.dasm.il
    "$CORE_ROOT/ilasm" -output=b39217.asm.exe  b39217.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b39217
    if [ ! -f b39217.org ]; then
        TakeLock
        if [ ! -f b39217.org ]; then
          mkdir IL
          cp b39217.exe IL/b39217.exe
          mv b39217.exe b39217.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b39217.org /out b39217.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b39217.org /out b39217.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b39217.reflect.xml
  if [ -f b39217.reflect.xml ]
then
    ReflectionRoots="-x b39217.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b39217.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b39217.asm.exe $CLRTestExecutionArguments
