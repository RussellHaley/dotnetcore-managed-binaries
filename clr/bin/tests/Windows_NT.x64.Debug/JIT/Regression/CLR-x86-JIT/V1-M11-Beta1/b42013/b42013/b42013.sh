...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b42013/b42013 b42013.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b42013/b42013 b42013.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b42013.dasm.il b42013.exe
    "$CORE_ROOT/ildasm" -raweh -out=b42013.dasm.il b42013.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b42013.asm.exe  b42013.dasm.il
    "$CORE_ROOT/ilasm" -output=b42013.asm.exe  b42013.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b42013
    if [ ! -f b42013.org ]; then
        TakeLock
        if [ ! -f b42013.org ]; then
          mkdir IL
          cp b42013.exe IL/b42013.exe
          mv b42013.exe b42013.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b42013.org /out b42013.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b42013.org /out b42013.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b42013.reflect.xml
  if [ -f b42013.reflect.xml ]
then
    ReflectionRoots="-x b42013.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b42013.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b42013.asm.exe $CLRTestExecutionArguments
