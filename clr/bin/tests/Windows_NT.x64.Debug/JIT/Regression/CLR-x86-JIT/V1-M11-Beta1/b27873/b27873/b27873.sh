...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b27873/b27873 b27873.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b27873/b27873 b27873.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b27873.dasm.il b27873.exe
    "$CORE_ROOT/ildasm" -raweh -out=b27873.dasm.il b27873.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b27873.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27873.dasm.il
    "$CORE_ROOT/ilasm" -output=b27873.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27873.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b27873
    if [ ! -f b27873.org ]; then
        TakeLock
        if [ ! -f b27873.org ]; then
          mkdir IL
          cp b27873.exe IL/b27873.exe
          mv b27873.exe b27873.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b27873.org /out b27873.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b27873.org /out b27873.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b27873.reflect.xml
  if [ -f b27873.reflect.xml ]
then
    ReflectionRoots="-x b27873.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b27873.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b27873.asm.exe $CLRTestExecutionArguments
