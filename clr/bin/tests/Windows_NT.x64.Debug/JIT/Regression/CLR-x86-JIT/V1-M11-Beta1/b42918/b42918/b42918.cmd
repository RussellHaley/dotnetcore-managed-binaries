...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b42918\b42918 b42918.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b42918\b42918 b42918.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b42918.dasm.il b42918.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b42918.dasm.il b42918.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b42918.asm.exe  b42918.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b42918.asm.exe  b42918.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b42918
    if not exist "b42918.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b42918.org" (
            mkdir IL
            copy b42918.exe IL\b42918.exe 
            ren b42918.exe b42918.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b42918.org /out b42918.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b42918.org /out b42918.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b42918.reflect.xml
    IF EXIST b42918.reflect.xml set ReflectionRoots=-x b42918.reflect.xml
    ...
    set ExePath=%LinkBin%\b42918.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b42918.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b42918.asm.exe %CLRTestExecutionArguments% %Host_Args%
