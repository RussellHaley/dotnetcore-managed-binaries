...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b43313\Desktop\b43313 b43313.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b43313\Desktop\b43313 b43313.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b43313.dasm.il b43313.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b43313.dasm.il b43313.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b43313.asm.exe  b43313.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b43313.asm.exe  b43313.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b43313
    if not exist "b43313.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b43313.org" (
            mkdir IL
            copy b43313.exe IL\b43313.exe 
            ren b43313.exe b43313.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b43313.org /out b43313.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b43313.org /out b43313.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b43313.reflect.xml
    IF EXIST b43313.reflect.xml set ReflectionRoots=-x b43313.reflect.xml
    ...
    set ExePath=%LinkBin%\b43313.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b43313.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b43313.asm.exe %CLRTestExecutionArguments% %Host_Args%
