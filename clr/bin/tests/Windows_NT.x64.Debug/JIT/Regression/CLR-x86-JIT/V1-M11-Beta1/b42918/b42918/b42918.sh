...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b42918/b42918 b42918.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b42918/b42918 b42918.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b42918.dasm.il b42918.exe
    "$CORE_ROOT/ildasm" -raweh -out=b42918.dasm.il b42918.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b42918.asm.exe  b42918.dasm.il
    "$CORE_ROOT/ilasm" -output=b42918.asm.exe  b42918.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b42918
    if [ ! -f b42918.org ]; then
        TakeLock
        if [ ! -f b42918.org ]; then
          mkdir IL
          cp b42918.exe IL/b42918.exe
          mv b42918.exe b42918.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b42918.org /out b42918.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b42918.org /out b42918.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b42918.reflect.xml
  if [ -f b42918.reflect.xml ]
then
    ReflectionRoots="-x b42918.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b42918.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b42918.asm.exe $CLRTestExecutionArguments
