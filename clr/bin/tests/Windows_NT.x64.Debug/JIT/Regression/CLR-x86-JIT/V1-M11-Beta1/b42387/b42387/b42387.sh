...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b42387/b42387 b42387.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b42387/b42387 b42387.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b42387.dasm.il b42387.exe
    "$CORE_ROOT/ildasm" -raweh -out=b42387.dasm.il b42387.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b42387.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b42387.dasm.il
    "$CORE_ROOT/ilasm" -output=b42387.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b42387.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b42387
    if [ ! -f b42387.org ]; then
        TakeLock
        if [ ! -f b42387.org ]; then
          mkdir IL
          cp b42387.exe IL/b42387.exe
          mv b42387.exe b42387.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b42387.org /out b42387.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b42387.org /out b42387.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b42387.reflect.xml
  if [ -f b42387.reflect.xml ]
then
    ReflectionRoots="-x b42387.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b42387.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b42387.asm.exe $CLRTestExecutionArguments
