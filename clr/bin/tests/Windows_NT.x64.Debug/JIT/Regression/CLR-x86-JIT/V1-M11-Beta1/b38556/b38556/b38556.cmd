...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b38556\b38556 b38556.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b38556\b38556 b38556.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b38556.dasm.il b38556.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b38556.dasm.il b38556.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b38556.asm.exe  b38556.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b38556.asm.exe  b38556.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b38556
    if not exist "b38556.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b38556.org" (
            mkdir IL
            copy b38556.exe IL\b38556.exe 
            ren b38556.exe b38556.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b38556.org /out b38556.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b38556.org /out b38556.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b38556.reflect.xml
    IF EXIST b38556.reflect.xml set ReflectionRoots=-x b38556.reflect.xml
    ...
    set ExePath=%LinkBin%\b38556.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b38556.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b38556.asm.exe %CLRTestExecutionArguments% %Host_Args%
