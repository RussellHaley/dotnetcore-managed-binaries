...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43033/b43033 b43033.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43033/b43033 b43033.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b43033.dasm.il b43033.exe
    "$CORE_ROOT/ildasm" -raweh -out=b43033.dasm.il b43033.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b43033.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43033.dasm.il
    "$CORE_ROOT/ilasm" -output=b43033.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43033.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b43033
    if [ ! -f b43033.org ]; then
        TakeLock
        if [ ! -f b43033.org ]; then
          mkdir IL
          cp b43033.exe IL/b43033.exe
          mv b43033.exe b43033.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43033.org /out b43033.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43033.org /out b43033.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b43033.reflect.xml
  if [ -f b43033.reflect.xml ]
then
    ReflectionRoots="-x b43033.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b43033.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b43033.asm.exe $CLRTestExecutionArguments
