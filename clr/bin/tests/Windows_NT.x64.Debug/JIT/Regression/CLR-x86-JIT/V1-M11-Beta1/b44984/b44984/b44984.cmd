...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b44984\b44984 b44984.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b44984\b44984 b44984.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b44984.dasm.il b44984.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b44984.dasm.il b44984.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b44984.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44984.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b44984.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44984.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b44984
    if not exist "b44984.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b44984.org" (
            mkdir IL
            copy b44984.exe IL\b44984.exe 
            ren b44984.exe b44984.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b44984.org /out b44984.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b44984.org /out b44984.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b44984.reflect.xml
    IF EXIST b44984.reflect.xml set ReflectionRoots=-x b44984.reflect.xml
    ...
    set ExePath=%LinkBin%\b44984.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b44984.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b44984.asm.exe %CLRTestExecutionArguments% %Host_Args%
