...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b47093\b47093 b47093.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b47093\b47093 b47093.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b47093.dasm.il b47093.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b47093.dasm.il b47093.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b47093.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47093.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b47093.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47093.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b47093
    if not exist "b47093.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b47093.org" (
            mkdir IL
            copy b47093.exe IL\b47093.exe 
            ren b47093.exe b47093.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47093.org /out b47093.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47093.org /out b47093.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b47093.reflect.xml
    IF EXIST b47093.reflect.xml set ReflectionRoots=-x b47093.reflect.xml
    ...
    set ExePath=%LinkBin%\b47093.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b47093.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b47093.asm.exe %CLRTestExecutionArguments% %Host_Args%
