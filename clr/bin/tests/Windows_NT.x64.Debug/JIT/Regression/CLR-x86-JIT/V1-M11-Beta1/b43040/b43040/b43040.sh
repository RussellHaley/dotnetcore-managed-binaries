...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43040/b43040 b43040.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43040/b43040 b43040.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b43040.dasm.il b43040.exe
    "$CORE_ROOT/ildasm" -raweh -out=b43040.dasm.il b43040.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b43040.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43040.dasm.il
    "$CORE_ROOT/ilasm" -output=b43040.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43040.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b43040
    if [ ! -f b43040.org ]; then
        TakeLock
        if [ ! -f b43040.org ]; then
          mkdir IL
          cp b43040.exe IL/b43040.exe
          mv b43040.exe b43040.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43040.org /out b43040.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43040.org /out b43040.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b43040.reflect.xml
  if [ -f b43040.reflect.xml ]
then
    ReflectionRoots="-x b43040.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b43040.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b43040.asm.exe $CLRTestExecutionArguments
