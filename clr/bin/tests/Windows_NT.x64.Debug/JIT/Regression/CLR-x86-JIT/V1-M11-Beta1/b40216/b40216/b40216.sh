...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40216/b40216 b40216.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40216/b40216 b40216.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b40216.dasm.il b40216.exe
    "$CORE_ROOT/ildasm" -raweh -out=b40216.dasm.il b40216.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b40216.asm.exe  b40216.dasm.il
    "$CORE_ROOT/ilasm" -output=b40216.asm.exe  b40216.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b40216
    if [ ! -f b40216.org ]; then
        TakeLock
        if [ ! -f b40216.org ]; then
          mkdir IL
          cp b40216.exe IL/b40216.exe
          mv b40216.exe b40216.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40216.org /out b40216.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40216.org /out b40216.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b40216.reflect.xml
  if [ -f b40216.reflect.xml ]
then
    ReflectionRoots="-x b40216.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b40216.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b40216.asm.exe $CLRTestExecutionArguments
