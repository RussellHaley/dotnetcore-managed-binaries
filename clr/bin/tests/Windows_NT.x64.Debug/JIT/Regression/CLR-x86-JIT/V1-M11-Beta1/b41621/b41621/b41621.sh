...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41621/b41621 b41621.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41621/b41621 b41621.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b41621.dasm.il b41621.exe
    "$CORE_ROOT/ildasm" -raweh -out=b41621.dasm.il b41621.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b41621.asm.exe  b41621.dasm.il
    "$CORE_ROOT/ilasm" -output=b41621.asm.exe  b41621.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b41621
    if [ ! -f b41621.org ]; then
        TakeLock
        if [ ! -f b41621.org ]; then
          mkdir IL
          cp b41621.exe IL/b41621.exe
          mv b41621.exe b41621.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41621.org /out b41621.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41621.org /out b41621.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b41621.reflect.xml
  if [ -f b41621.reflect.xml ]
then
    ReflectionRoots="-x b41621.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b41621.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b41621.asm.exe $CLRTestExecutionArguments
