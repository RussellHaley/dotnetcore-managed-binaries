...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b44724\b44724 b44724.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b44724\b44724 b44724.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b44724.dasm.il b44724.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b44724.dasm.il b44724.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b44724.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44724.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b44724.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44724.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b44724
    if not exist "b44724.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b44724.org" (
            mkdir IL
            copy b44724.exe IL\b44724.exe 
            ren b44724.exe b44724.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b44724.org /out b44724.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b44724.org /out b44724.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b44724.reflect.xml
    IF EXIST b44724.reflect.xml set ReflectionRoots=-x b44724.reflect.xml
    ...
    set ExePath=%LinkBin%\b44724.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b44724.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b44724.asm.exe %CLRTestExecutionArguments% %Host_Args%
