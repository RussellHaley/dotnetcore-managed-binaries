...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b44724/b44724 b44724.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b44724/b44724 b44724.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b44724.dasm.il b44724.exe
    "$CORE_ROOT/ildasm" -raweh -out=b44724.dasm.il b44724.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b44724.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44724.dasm.il
    "$CORE_ROOT/ilasm" -output=b44724.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44724.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b44724
    if [ ! -f b44724.org ]; then
        TakeLock
        if [ ! -f b44724.org ]; then
          mkdir IL
          cp b44724.exe IL/b44724.exe
          mv b44724.exe b44724.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b44724.org /out b44724.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b44724.org /out b44724.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b44724.reflect.xml
  if [ -f b44724.reflect.xml ]
then
    ReflectionRoots="-x b44724.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b44724.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b44724.asm.exe $CLRTestExecutionArguments
