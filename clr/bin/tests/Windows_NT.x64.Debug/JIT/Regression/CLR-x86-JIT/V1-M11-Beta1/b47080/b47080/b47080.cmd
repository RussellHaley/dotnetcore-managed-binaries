...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b47080\b47080 b47080.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b47080\b47080 b47080.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b47080.dasm.il b47080.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b47080.dasm.il b47080.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b47080.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47080.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b47080.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b47080.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b47080
    if not exist "b47080.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b47080.org" (
            mkdir IL
            copy b47080.exe IL\b47080.exe 
            ren b47080.exe b47080.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47080.org /out b47080.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b47080.org /out b47080.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b47080.reflect.xml
    IF EXIST b47080.reflect.xml set ReflectionRoots=-x b47080.reflect.xml
    ...
    set ExePath=%LinkBin%\b47080.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b47080.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b47080.asm.exe %CLRTestExecutionArguments% %Host_Args%
