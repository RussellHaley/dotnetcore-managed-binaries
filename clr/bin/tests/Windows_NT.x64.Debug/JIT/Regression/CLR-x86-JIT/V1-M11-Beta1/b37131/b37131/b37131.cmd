...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b37131\b37131 b37131.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b37131\b37131 b37131.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b37131.dasm.il b37131.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b37131.dasm.il b37131.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b37131.asm.exe  b37131.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b37131.asm.exe  b37131.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b37131
    if not exist "b37131.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b37131.org" (
            mkdir IL
            copy b37131.exe IL\b37131.exe 
            ren b37131.exe b37131.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37131.org /out b37131.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37131.org /out b37131.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b37131.reflect.xml
    IF EXIST b37131.reflect.xml set ReflectionRoots=-x b37131.reflect.xml
    ...
    set ExePath=%LinkBin%\b37131.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b37131.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b37131.asm.exe %CLRTestExecutionArguments% %Host_Args%
