...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b48614\b48614 b48614.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b48614\b48614 b48614.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b48614.dasm.il b48614.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b48614.dasm.il b48614.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b48614.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b48614.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b48614.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b48614.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b48614
    if not exist "b48614.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b48614.org" (
            mkdir IL
            copy b48614.exe IL\b48614.exe 
            ren b48614.exe b48614.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b48614.org /out b48614.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b48614.org /out b48614.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b48614.reflect.xml
    IF EXIST b48614.reflect.xml set ReflectionRoots=-x b48614.reflect.xml
    ...
    set ExePath=%LinkBin%\b48614.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b48614.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b48614.asm.exe %CLRTestExecutionArguments% %Host_Args%
