...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b41470\b41470 b41470.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b41470\b41470 b41470.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b41470.dasm.il b41470.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b41470.dasm.il b41470.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b41470.asm.exe  b41470.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b41470.asm.exe  b41470.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b41470
    if not exist "b41470.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b41470.org" (
            mkdir IL
            copy b41470.exe IL\b41470.exe 
            ren b41470.exe b41470.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b41470.org /out b41470.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b41470.org /out b41470.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b41470.reflect.xml
    IF EXIST b41470.reflect.xml set ReflectionRoots=-x b41470.reflect.xml
    ...
    set ExePath=%LinkBin%\b41470.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b41470.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b41470.asm.exe %CLRTestExecutionArguments% %Host_Args%
