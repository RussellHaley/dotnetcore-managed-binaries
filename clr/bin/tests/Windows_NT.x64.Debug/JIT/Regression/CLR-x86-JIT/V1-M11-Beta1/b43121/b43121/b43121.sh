...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43121/b43121 b43121.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43121/b43121 b43121.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b43121.dasm.il b43121.exe
    "$CORE_ROOT/ildasm" -raweh -out=b43121.dasm.il b43121.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b43121.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43121.dasm.il
    "$CORE_ROOT/ilasm" -output=b43121.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43121.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b43121
    if [ ! -f b43121.org ]; then
        TakeLock
        if [ ! -f b43121.org ]; then
          mkdir IL
          cp b43121.exe IL/b43121.exe
          mv b43121.exe b43121.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43121.org /out b43121.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43121.org /out b43121.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b43121.reflect.xml
  if [ -f b43121.reflect.xml ]
then
    ReflectionRoots="-x b43121.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b43121.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b43121.asm.exe $CLRTestExecutionArguments
