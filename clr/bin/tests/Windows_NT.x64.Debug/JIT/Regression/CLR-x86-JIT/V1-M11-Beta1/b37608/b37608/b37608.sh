...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b37608/b37608 b37608.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b37608/b37608 b37608.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b37608.dasm.il b37608.exe
    "$CORE_ROOT/ildasm" -raweh -out=b37608.dasm.il b37608.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b37608.asm.exe  b37608.dasm.il
    "$CORE_ROOT/ilasm" -output=b37608.asm.exe  b37608.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b37608
    if [ ! -f b37608.org ]; then
        TakeLock
        if [ ! -f b37608.org ]; then
          mkdir IL
          cp b37608.exe IL/b37608.exe
          mv b37608.exe b37608.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b37608.org /out b37608.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b37608.org /out b37608.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b37608.reflect.xml
  if [ -f b37608.reflect.xml ]
then
    ReflectionRoots="-x b37608.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b37608.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b37608.asm.exe $CLRTestExecutionArguments
