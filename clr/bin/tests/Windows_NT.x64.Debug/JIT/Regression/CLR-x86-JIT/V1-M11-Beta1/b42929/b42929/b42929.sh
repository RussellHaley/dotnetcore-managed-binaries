...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b42929/b42929 b42929.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b42929/b42929 b42929.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b42929.dasm.il b42929.exe
    "$CORE_ROOT/ildasm" -raweh -out=b42929.dasm.il b42929.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b42929.asm.exe  b42929.dasm.il
    "$CORE_ROOT/ilasm" -output=b42929.asm.exe  b42929.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b42929
    if [ ! -f b42929.org ]; then
        TakeLock
        if [ ! -f b42929.org ]; then
          mkdir IL
          cp b42929.exe IL/b42929.exe
          mv b42929.exe b42929.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b42929.org /out b42929.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b42929.org /out b42929.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b42929.reflect.xml
  if [ -f b42929.reflect.xml ]
then
    ReflectionRoots="-x b42929.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b42929.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b42929.asm.exe $CLRTestExecutionArguments
