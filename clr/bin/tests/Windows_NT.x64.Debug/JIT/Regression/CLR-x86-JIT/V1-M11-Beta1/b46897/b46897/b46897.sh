...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b46897/b46897 b46897.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b46897/b46897 b46897.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b46897.dasm.il b46897.exe
    "$CORE_ROOT/ildasm" -raweh -out=b46897.dasm.il b46897.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b46897.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b46897.dasm.il
    "$CORE_ROOT/ilasm" -output=b46897.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b46897.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b46897
    if [ ! -f b46897.org ]; then
        TakeLock
        if [ ! -f b46897.org ]; then
          mkdir IL
          cp b46897.exe IL/b46897.exe
          mv b46897.exe b46897.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b46897.org /out b46897.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b46897.org /out b46897.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b46897.reflect.xml
  if [ -f b46897.reflect.xml ]
then
    ReflectionRoots="-x b46897.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b46897.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b46897.asm.exe $CLRTestExecutionArguments
