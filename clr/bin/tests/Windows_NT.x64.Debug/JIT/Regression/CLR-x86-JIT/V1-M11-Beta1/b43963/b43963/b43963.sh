...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43963/b43963 b43963.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43963/b43963 b43963.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b43963.dasm.il b43963.exe
    "$CORE_ROOT/ildasm" -raweh -out=b43963.dasm.il b43963.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b43963.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43963.dasm.il
    "$CORE_ROOT/ilasm" -output=b43963.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43963.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b43963
    if [ ! -f b43963.org ]; then
        TakeLock
        if [ ! -f b43963.org ]; then
          mkdir IL
          cp b43963.exe IL/b43963.exe
          mv b43963.exe b43963.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43963.org /out b43963.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43963.org /out b43963.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b43963.reflect.xml
  if [ -f b43963.reflect.xml ]
then
    ReflectionRoots="-x b43963.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b43963.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b43963.asm.exe $CLRTestExecutionArguments
