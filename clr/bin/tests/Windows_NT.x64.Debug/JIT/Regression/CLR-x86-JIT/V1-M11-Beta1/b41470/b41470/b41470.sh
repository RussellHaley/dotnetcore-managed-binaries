...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41470/b41470 b41470.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41470/b41470 b41470.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b41470.dasm.il b41470.exe
    "$CORE_ROOT/ildasm" -raweh -out=b41470.dasm.il b41470.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b41470.asm.exe  b41470.dasm.il
    "$CORE_ROOT/ilasm" -output=b41470.asm.exe  b41470.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b41470
    if [ ! -f b41470.org ]; then
        TakeLock
        if [ ! -f b41470.org ]; then
          mkdir IL
          cp b41470.exe IL/b41470.exe
          mv b41470.exe b41470.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41470.org /out b41470.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41470.org /out b41470.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b41470.reflect.xml
  if [ -f b41470.reflect.xml ]
then
    ReflectionRoots="-x b41470.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b41470.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b41470.asm.exe $CLRTestExecutionArguments
