...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b39946/b39946 b39946.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b39946/b39946 b39946.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b39946.dasm.il b39946.exe
    "$CORE_ROOT/ildasm" -raweh -out=b39946.dasm.il b39946.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b39946.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b39946.dasm.il
    "$CORE_ROOT/ilasm" -output=b39946.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b39946.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b39946
    if [ ! -f b39946.org ]; then
        TakeLock
        if [ ! -f b39946.org ]; then
          mkdir IL
          cp b39946.exe IL/b39946.exe
          mv b39946.exe b39946.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b39946.org /out b39946.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b39946.org /out b39946.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b39946.reflect.xml
  if [ -f b39946.reflect.xml ]
then
    ReflectionRoots="-x b39946.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b39946.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b39946.asm.exe $CLRTestExecutionArguments
