...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b37131/b37131 b37131.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b37131/b37131 b37131.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b37131.dasm.il b37131.exe
    "$CORE_ROOT/ildasm" -raweh -out=b37131.dasm.il b37131.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b37131.asm.exe  b37131.dasm.il
    "$CORE_ROOT/ilasm" -output=b37131.asm.exe  b37131.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b37131
    if [ ! -f b37131.org ]; then
        TakeLock
        if [ ! -f b37131.org ]; then
          mkdir IL
          cp b37131.exe IL/b37131.exe
          mv b37131.exe b37131.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b37131.org /out b37131.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b37131.org /out b37131.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b37131.reflect.xml
  if [ -f b37131.reflect.xml ]
then
    ReflectionRoots="-x b37131.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b37131.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b37131.asm.exe $CLRTestExecutionArguments
