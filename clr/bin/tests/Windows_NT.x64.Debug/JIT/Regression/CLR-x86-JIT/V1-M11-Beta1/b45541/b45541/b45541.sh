...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b45541/b45541 b45541.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b45541/b45541 b45541.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b45541.dasm.il b45541.exe
    "$CORE_ROOT/ildasm" -raweh -out=b45541.dasm.il b45541.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b45541.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45541.dasm.il
    "$CORE_ROOT/ilasm" -output=b45541.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45541.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b45541
    if [ ! -f b45541.org ]; then
        TakeLock
        if [ ! -f b45541.org ]; then
          mkdir IL
          cp b45541.exe IL/b45541.exe
          mv b45541.exe b45541.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b45541.org /out b45541.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b45541.org /out b45541.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b45541.reflect.xml
  if [ -f b45541.reflect.xml ]
then
    ReflectionRoots="-x b45541.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b45541.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b45541.asm.exe $CLRTestExecutionArguments
