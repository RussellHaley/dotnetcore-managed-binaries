...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b33922/b33922 b33922.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b33922/b33922 b33922.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b33922.dasm.il b33922.exe
    "$CORE_ROOT/ildasm" -raweh -out=b33922.dasm.il b33922.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b33922.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33922.dasm.il
    "$CORE_ROOT/ilasm" -output=b33922.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b33922.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b33922
    if [ ! -f b33922.org ]; then
        TakeLock
        if [ ! -f b33922.org ]; then
          mkdir IL
          cp b33922.exe IL/b33922.exe
          mv b33922.exe b33922.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b33922.org /out b33922.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b33922.org /out b33922.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b33922.reflect.xml
  if [ -f b33922.reflect.xml ]
then
    ReflectionRoots="-x b33922.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b33922.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b33922.asm.exe $CLRTestExecutionArguments
