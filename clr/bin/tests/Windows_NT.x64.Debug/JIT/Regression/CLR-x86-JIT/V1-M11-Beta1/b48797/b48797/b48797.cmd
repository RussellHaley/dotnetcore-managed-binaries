...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b48797\b48797 b48797.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b48797\b48797 b48797.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b48797.dasm.il b48797.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b48797.dasm.il b48797.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b48797.asm.exe  b48797.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b48797.asm.exe  b48797.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b48797
    if not exist "b48797.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b48797.org" (
            mkdir IL
            copy b48797.exe IL\b48797.exe 
            ren b48797.exe b48797.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b48797.org /out b48797.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b48797.org /out b48797.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b48797.reflect.xml
    IF EXIST b48797.reflect.xml set ReflectionRoots=-x b48797.reflect.xml
    ...
    set ExePath=%LinkBin%\b48797.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b48797.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b48797.asm.exe %CLRTestExecutionArguments% %Host_Args%
