...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b46641\b46641 b46641.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b46641\b46641 b46641.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b46641.dasm.il b46641.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b46641.dasm.il b46641.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b46641.asm.exe  b46641.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b46641.asm.exe  b46641.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b46641
    if not exist "b46641.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b46641.org" (
            mkdir IL
            copy b46641.exe IL\b46641.exe 
            ren b46641.exe b46641.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b46641.org /out b46641.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b46641.org /out b46641.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b46641.reflect.xml
    IF EXIST b46641.reflect.xml set ReflectionRoots=-x b46641.reflect.xml
    ...
    set ExePath=%LinkBin%\b46641.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b46641.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b46641.asm.exe %CLRTestExecutionArguments% %Host_Args%
