...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b46576\b46576 b46576.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b46576\b46576 b46576.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b46576.dasm.il b46576.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b46576.dasm.il b46576.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b46576.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b46576.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b46576.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b46576.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b46576
    if not exist "b46576.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b46576.org" (
            mkdir IL
            copy b46576.exe IL\b46576.exe 
            ren b46576.exe b46576.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b46576.org /out b46576.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b46576.org /out b46576.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b46576.reflect.xml
    IF EXIST b46576.reflect.xml set ReflectionRoots=-x b46576.reflect.xml
    ...
    set ExePath=%LinkBin%\b46576.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b46576.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b46576.asm.exe %CLRTestExecutionArguments% %Host_Args%
