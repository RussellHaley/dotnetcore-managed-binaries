...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b48614/b48614 b48614.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b48614/b48614 b48614.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b48614.dasm.il b48614.exe
    "$CORE_ROOT/ildasm" -raweh -out=b48614.dasm.il b48614.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b48614.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b48614.dasm.il
    "$CORE_ROOT/ilasm" -output=b48614.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b48614.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b48614
    if [ ! -f b48614.org ]; then
        TakeLock
        if [ ! -f b48614.org ]; then
          mkdir IL
          cp b48614.exe IL/b48614.exe
          mv b48614.exe b48614.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b48614.org /out b48614.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b48614.org /out b48614.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b48614.reflect.xml
  if [ -f b48614.reflect.xml ]
then
    ReflectionRoots="-x b48614.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b48614.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b48614.asm.exe $CLRTestExecutionArguments
