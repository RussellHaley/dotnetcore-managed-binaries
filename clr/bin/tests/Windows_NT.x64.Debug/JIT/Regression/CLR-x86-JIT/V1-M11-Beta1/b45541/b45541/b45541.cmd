...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b45541\b45541 b45541.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b45541\b45541 b45541.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b45541.dasm.il b45541.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b45541.dasm.il b45541.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b45541.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45541.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b45541.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b45541.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b45541
    if not exist "b45541.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b45541.org" (
            mkdir IL
            copy b45541.exe IL\b45541.exe 
            ren b45541.exe b45541.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b45541.org /out b45541.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b45541.org /out b45541.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b45541.reflect.xml
    IF EXIST b45541.reflect.xml set ReflectionRoots=-x b45541.reflect.xml
    ...
    set ExePath=%LinkBin%\b45541.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b45541.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b45541.asm.exe %CLRTestExecutionArguments% %Host_Args%
