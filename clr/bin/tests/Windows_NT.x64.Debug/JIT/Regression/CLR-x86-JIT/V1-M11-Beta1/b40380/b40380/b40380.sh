...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40380/b40380 b40380.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b40380/b40380 b40380.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b40380.dasm.il b40380.exe
    "$CORE_ROOT/ildasm" -raweh -out=b40380.dasm.il b40380.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b40380.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b40380.dasm.il
    "$CORE_ROOT/ilasm" -output=b40380.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b40380.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b40380
    if [ ! -f b40380.org ]; then
        TakeLock
        if [ ! -f b40380.org ]; then
          mkdir IL
          cp b40380.exe IL/b40380.exe
          mv b40380.exe b40380.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40380.org /out b40380.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b40380.org /out b40380.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b40380.reflect.xml
  if [ -f b40380.reflect.xml ]
then
    ReflectionRoots="-x b40380.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b40380.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b40380.asm.exe $CLRTestExecutionArguments
