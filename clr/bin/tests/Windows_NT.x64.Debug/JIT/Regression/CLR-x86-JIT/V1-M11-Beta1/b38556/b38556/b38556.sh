...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b38556/b38556 b38556.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b38556/b38556 b38556.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b38556.dasm.il b38556.exe
    "$CORE_ROOT/ildasm" -raweh -out=b38556.dasm.il b38556.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b38556.asm.exe  b38556.dasm.il
    "$CORE_ROOT/ilasm" -output=b38556.asm.exe  b38556.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b38556
    if [ ! -f b38556.org ]; then
        TakeLock
        if [ ! -f b38556.org ]; then
          mkdir IL
          cp b38556.exe IL/b38556.exe
          mv b38556.exe b38556.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b38556.org /out b38556.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b38556.org /out b38556.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b38556.reflect.xml
  if [ -f b38556.reflect.xml ]
then
    ReflectionRoots="-x b38556.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b38556.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b38556.asm.exe $CLRTestExecutionArguments
