...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b40216\b40216 b40216.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b40216\b40216 b40216.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b40216.dasm.il b40216.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b40216.dasm.il b40216.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b40216.asm.exe  b40216.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b40216.asm.exe  b40216.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b40216
    if not exist "b40216.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b40216.org" (
            mkdir IL
            copy b40216.exe IL\b40216.exe 
            ren b40216.exe b40216.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b40216.org /out b40216.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b40216.org /out b40216.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b40216.reflect.xml
    IF EXIST b40216.reflect.xml set ReflectionRoots=-x b40216.reflect.xml
    ...
    set ExePath=%LinkBin%\b40216.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b40216.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b40216.asm.exe %CLRTestExecutionArguments% %Host_Args%
