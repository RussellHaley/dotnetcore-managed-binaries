...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b36274\b36274 b36274.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b36274\b36274 b36274.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b36274.dasm.il b36274.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b36274.dasm.il b36274.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b36274.asm.exe  b36274.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b36274.asm.exe  b36274.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b36274
    if not exist "b36274.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b36274.org" (
            mkdir IL
            copy b36274.exe IL\b36274.exe 
            ren b36274.exe b36274.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b36274.org /out b36274.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b36274.org /out b36274.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b36274.reflect.xml
    IF EXIST b36274.reflect.xml set ReflectionRoots=-x b36274.reflect.xml
    ...
    set ExePath=%LinkBin%\b36274.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b36274.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b36274.asm.exe %CLRTestExecutionArguments% %Host_Args%
