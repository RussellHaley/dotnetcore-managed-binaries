...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b37608\b37608 b37608.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b37608\b37608 b37608.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b37608.dasm.il b37608.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b37608.dasm.il b37608.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b37608.asm.exe  b37608.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b37608.asm.exe  b37608.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b37608
    if not exist "b37608.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b37608.org" (
            mkdir IL
            copy b37608.exe IL\b37608.exe 
            ren b37608.exe b37608.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37608.org /out b37608.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b37608.org /out b37608.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b37608.reflect.xml
    IF EXIST b37608.reflect.xml set ReflectionRoots=-x b37608.reflect.xml
    ...
    set ExePath=%LinkBin%\b37608.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b37608.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b37608.asm.exe %CLRTestExecutionArguments% %Host_Args%
