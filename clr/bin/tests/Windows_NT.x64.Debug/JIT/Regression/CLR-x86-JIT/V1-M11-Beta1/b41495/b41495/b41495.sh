...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41495/b41495 b41495.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b41495/b41495 b41495.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b41495.dasm.il b41495.exe
    "$CORE_ROOT/ildasm" -raweh -out=b41495.dasm.il b41495.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b41495.asm.exe  b41495.dasm.il
    "$CORE_ROOT/ilasm" -output=b41495.asm.exe  b41495.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b41495
    if [ ! -f b41495.org ]; then
        TakeLock
        if [ ! -f b41495.org ]; then
          mkdir IL
          cp b41495.exe IL/b41495.exe
          mv b41495.exe b41495.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41495.org /out b41495.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b41495.org /out b41495.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b41495.reflect.xml
  if [ -f b41495.reflect.xml ]
then
    ReflectionRoots="-x b41495.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b41495.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b41495.asm.exe $CLRTestExecutionArguments
