...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43714/b43714 b43714.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b43714/b43714 b43714.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b43714.dasm.il b43714.exe
    "$CORE_ROOT/ildasm" -raweh -out=b43714.dasm.il b43714.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b43714.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43714.dasm.il
    "$CORE_ROOT/ilasm" -output=b43714.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b43714.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b43714
    if [ ! -f b43714.org ]; then
        TakeLock
        if [ ! -f b43714.org ]; then
          mkdir IL
          cp b43714.exe IL/b43714.exe
          mv b43714.exe b43714.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43714.org /out b43714.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b43714.org /out b43714.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b43714.reflect.xml
  if [ -f b43714.reflect.xml ]
then
    ReflectionRoots="-x b43714.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b43714.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b43714.asm.exe $CLRTestExecutionArguments
