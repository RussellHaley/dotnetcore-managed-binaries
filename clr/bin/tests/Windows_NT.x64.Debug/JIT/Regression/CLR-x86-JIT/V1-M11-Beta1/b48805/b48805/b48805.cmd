...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b48805\b48805 b48805.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b48805\b48805 b48805.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b48805.dasm.il b48805.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b48805.dasm.il b48805.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b48805.asm.exe  b48805.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b48805.asm.exe  b48805.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b48805
    if not exist "b48805.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b48805.org" (
            mkdir IL
            copy b48805.exe IL\b48805.exe 
            ren b48805.exe b48805.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b48805.org /out b48805.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b48805.org /out b48805.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b48805.reflect.xml
    IF EXIST b48805.reflect.xml set ReflectionRoots=-x b48805.reflect.xml
    ...
    set ExePath=%LinkBin%\b48805.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b48805.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b48805.asm.exe %CLRTestExecutionArguments% %Host_Args%
