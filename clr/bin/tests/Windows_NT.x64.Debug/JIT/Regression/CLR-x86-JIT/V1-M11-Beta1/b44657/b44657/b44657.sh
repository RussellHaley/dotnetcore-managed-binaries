...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b44657/b44657 b44657.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1-M11-Beta1/b44657/b44657 b44657.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b44657.dasm.il b44657.exe
    "$CORE_ROOT/ildasm" -raweh -out=b44657.dasm.il b44657.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b44657.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44657.dasm.il
    "$CORE_ROOT/ilasm" -output=b44657.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b44657.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b44657
    if [ ! -f b44657.org ]; then
        TakeLock
        if [ ! -f b44657.org ]; then
          mkdir IL
          cp b44657.exe IL/b44657.exe
          mv b44657.exe b44657.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b44657.org /out b44657.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b44657.org /out b44657.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b44657.reflect.xml
  if [ -f b44657.reflect.xml ]
then
    ReflectionRoots="-x b44657.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b44657.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b44657.asm.exe $CLRTestExecutionArguments
