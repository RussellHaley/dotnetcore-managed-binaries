...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b27873\b27873 b27873.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b27873\b27873 b27873.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b27873.dasm.il b27873.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b27873.dasm.il b27873.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b27873.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27873.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b27873.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b27873.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b27873
    if not exist "b27873.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b27873.org" (
            mkdir IL
            copy b27873.exe IL\b27873.exe 
            ren b27873.exe b27873.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b27873.org /out b27873.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b27873.org /out b27873.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b27873.reflect.xml
    IF EXIST b27873.reflect.xml set ReflectionRoots=-x b27873.reflect.xml
    ...
    set ExePath=%LinkBin%\b27873.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b27873.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b27873.asm.exe %CLRTestExecutionArguments% %Host_Args%
