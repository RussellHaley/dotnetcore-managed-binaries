...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b40521\b40521 b40521.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V1-M11-Beta1\b40521\b40521 b40521.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b40521.dasm.il b40521.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b40521.dasm.il b40521.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b40521.asm.exe  b40521.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b40521.asm.exe  b40521.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b40521
    if not exist "b40521.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b40521.org" (
            mkdir IL
            copy b40521.exe IL\b40521.exe 
            ren b40521.exe b40521.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b40521.org /out b40521.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b40521.org /out b40521.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b40521.reflect.xml
    IF EXIST b40521.reflect.xml set ReflectionRoots=-x b40521.reflect.xml
    ...
    set ExePath=%LinkBin%\b40521.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b40521.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b40521.asm.exe %CLRTestExecutionArguments% %Host_Args%
