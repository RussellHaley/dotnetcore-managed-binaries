...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.1-M1-Beta1/b143840/b143840 b143840.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V1.1-M1-Beta1/b143840/b143840 b143840.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b143840.dasm.il b143840.exe
    "$CORE_ROOT/ildasm" -raweh -out=b143840.dasm.il b143840.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b143840.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b143840.dasm.il
    "$CORE_ROOT/ilasm" -output=b143840.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b143840.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b143840
    if [ ! -f b143840.org ]; then
        TakeLock
        if [ ! -f b143840.org ]; then
          mkdir IL
          cp b143840.exe IL/b143840.exe
          mv b143840.exe b143840.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b143840.org /out b143840.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b143840.org /out b143840.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b143840.reflect.xml
  if [ -f b143840.reflect.xml ]
then
    ReflectionRoots="-x b143840.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b143840.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b143840.asm.exe $CLRTestExecutionArguments
