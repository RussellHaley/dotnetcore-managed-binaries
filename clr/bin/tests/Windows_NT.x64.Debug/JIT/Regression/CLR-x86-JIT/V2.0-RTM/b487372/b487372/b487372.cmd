...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-RTM\b487372\b487372 b487372.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-RTM\b487372\b487372 b487372.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b487372.dasm.il b487372.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b487372.dasm.il b487372.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b487372.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b487372.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b487372.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b487372.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b487372
    if not exist "b487372.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b487372.org" (
            mkdir IL
            copy b487372.exe IL\b487372.exe 
            ren b487372.exe b487372.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b487372.org /out b487372.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b487372.org /out b487372.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b487372.reflect.xml
    IF EXIST b487372.reflect.xml set ReflectionRoots=-x b487372.reflect.xml
    ...
    set ExePath=%LinkBin%\b487372.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b487372.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b487372.asm.exe %CLRTestExecutionArguments% %Host_Args%
