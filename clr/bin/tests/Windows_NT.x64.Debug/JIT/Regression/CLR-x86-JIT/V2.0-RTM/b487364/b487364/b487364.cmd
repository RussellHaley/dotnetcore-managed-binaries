...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-RTM\b487364\b487364 b487364.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-RTM\b487364\b487364 b487364.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b487364.dasm.il b487364.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b487364.dasm.il b487364.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b487364.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b487364.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b487364.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b487364.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b487364
    if not exist "b487364.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b487364.org" (
            mkdir IL
            copy b487364.exe IL\b487364.exe 
            ren b487364.exe b487364.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b487364.org /out b487364.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b487364.org /out b487364.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b487364.reflect.xml
    IF EXIST b487364.reflect.xml set ReflectionRoots=-x b487364.reflect.xml
    ...
    set ExePath=%LinkBin%\b487364.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b487364.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b487364.asm.exe %CLRTestExecutionArguments% %Host_Args%
