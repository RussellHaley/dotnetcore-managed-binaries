...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-RTM/b530694/b530694 b530694.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-RTM/b530694/b530694 b530694.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b530694.dasm.il b530694.exe
    "$CORE_ROOT/ildasm" -raweh -out=b530694.dasm.il b530694.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b530694.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b530694.dasm.il
    "$CORE_ROOT/ilasm" -output=b530694.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b530694.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b530694
    if [ ! -f b530694.org ]; then
        TakeLock
        if [ ! -f b530694.org ]; then
          mkdir IL
          cp b530694.exe IL/b530694.exe
          mv b530694.exe b530694.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b530694.org /out b530694.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b530694.org /out b530694.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b530694.reflect.xml
  if [ -f b530694.reflect.xml ]
then
    ReflectionRoots="-x b530694.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b530694.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b530694.asm.exe $CLRTestExecutionArguments
