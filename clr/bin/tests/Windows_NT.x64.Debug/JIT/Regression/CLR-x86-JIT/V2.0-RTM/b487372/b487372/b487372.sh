...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-RTM/b487372/b487372 b487372.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-RTM/b487372/b487372 b487372.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b487372.dasm.il b487372.exe
    "$CORE_ROOT/ildasm" -raweh -out=b487372.dasm.il b487372.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b487372.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b487372.dasm.il
    "$CORE_ROOT/ilasm" -output=b487372.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b487372.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b487372
    if [ ! -f b487372.org ]; then
        TakeLock
        if [ ! -f b487372.org ]; then
          mkdir IL
          cp b487372.exe IL/b487372.exe
          mv b487372.exe b487372.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b487372.org /out b487372.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b487372.org /out b487372.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b487372.reflect.xml
  if [ -f b487372.reflect.xml ]
then
    ReflectionRoots="-x b487372.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b487372.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b487372.asm.exe $CLRTestExecutionArguments
