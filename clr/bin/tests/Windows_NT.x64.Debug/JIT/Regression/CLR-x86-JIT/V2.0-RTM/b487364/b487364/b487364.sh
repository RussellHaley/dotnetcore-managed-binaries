...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-RTM/b487364/b487364 b487364.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-RTM/b487364/b487364 b487364.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b487364.dasm.il b487364.exe
    "$CORE_ROOT/ildasm" -raweh -out=b487364.dasm.il b487364.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b487364.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b487364.dasm.il
    "$CORE_ROOT/ilasm" -output=b487364.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b487364.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b487364
    if [ ! -f b487364.org ]; then
        TakeLock
        if [ ! -f b487364.org ]; then
          mkdir IL
          cp b487364.exe IL/b487364.exe
          mv b487364.exe b487364.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b487364.org /out b487364.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b487364.org /out b487364.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b487364.reflect.xml
  if [ -f b487364.reflect.xml ]
then
    ReflectionRoots="-x b487364.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b487364.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b487364.asm.exe $CLRTestExecutionArguments
