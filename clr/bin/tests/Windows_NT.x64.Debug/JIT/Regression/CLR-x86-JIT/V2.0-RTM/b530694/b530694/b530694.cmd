...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-RTM\b530694\b530694 b530694.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-RTM\b530694\b530694 b530694.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b530694.dasm.il b530694.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b530694.dasm.il b530694.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b530694.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b530694.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b530694.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE b530694.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b530694
    if not exist "b530694.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b530694.org" (
            mkdir IL
            copy b530694.exe IL\b530694.exe 
            ren b530694.exe b530694.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b530694.org /out b530694.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b530694.org /out b530694.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b530694.reflect.xml
    IF EXIST b530694.reflect.xml set ReflectionRoots=-x b530694.reflect.xml
    ...
    set ExePath=%LinkBin%\b530694.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b530694.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b530694.asm.exe %CLRTestExecutionArguments% %Host_Args%
