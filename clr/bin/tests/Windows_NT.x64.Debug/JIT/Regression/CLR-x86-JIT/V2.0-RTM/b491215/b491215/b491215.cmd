...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-RTM\b491215\b491215 b491215.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Regression\CLR-x86-JIT\V2.0-RTM\b491215\b491215 b491215.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=b491215.dasm.il b491215.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=b491215.dasm.il b491215.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=b491215.asm.exe  b491215.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b491215.asm.exe  b491215.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=b491215
    if not exist "b491215.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "b491215.org" (
            mkdir IL
            copy b491215.exe IL\b491215.exe 
            ren b491215.exe b491215.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b491215.org /out b491215.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in b491215.org /out b491215.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in b491215.reflect.xml
    IF EXIST b491215.reflect.xml set ReflectionRoots=-x b491215.reflect.xml
    ...
    set ExePath=%LinkBin%\b491215.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% b491215.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% b491215.asm.exe %CLRTestExecutionArguments% %Host_Args%
