...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-RTM/b491215/b491215 b491215.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Regression/CLR-x86-JIT/V2.0-RTM/b491215/b491215 b491215.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=b491215.dasm.il b491215.exe
    "$CORE_ROOT/ildasm" -raweh -out=b491215.dasm.il b491215.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=b491215.asm.exe  b491215.dasm.il
    "$CORE_ROOT/ilasm" -output=b491215.asm.exe  b491215.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=b491215
    if [ ! -f b491215.org ]; then
        TakeLock
        if [ ! -f b491215.org ]; then
          mkdir IL
          cp b491215.exe IL/b491215.exe
          mv b491215.exe b491215.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b491215.org /out b491215.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in b491215.org /out b491215.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in b491215.reflect.xml
  if [ -f b491215.reflect.xml ]
then
    ReflectionRoots="-x b491215.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" b491215.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" b491215.asm.exe $CLRTestExecutionArguments
