...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\superpmi\superpmicollect superpmicollect.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\superpmi\superpmicollect superpmicollect.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=superpmicollect.dasm.il superpmicollect.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=superpmicollect.dasm.il superpmicollect.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=superpmicollect.asm.exe  superpmicollect.dasm.il
    %CORE_ROOT%\ilasm.exe /output=superpmicollect.asm.exe  superpmicollect.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=superpmicollect
    if not exist "superpmicollect.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "superpmicollect.org" (
            mkdir IL
            copy superpmicollect.exe IL\superpmicollect.exe 
            ren superpmicollect.exe superpmicollect.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in superpmicollect.org /out superpmicollect.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in superpmicollect.org /out superpmicollect.exe
...
set ExePath=superpmicollect.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in superpmicollect.reflect.xml
    IF EXIST superpmicollect.reflect.xml set ReflectionRoots=-x superpmicollect.reflect.xml
    ...
    set ExePath=%LinkBin%\superpmicollect.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% superpmicollect.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% superpmicollect.asm.exe %CLRTestExecutionArguments% %Host_Args%
