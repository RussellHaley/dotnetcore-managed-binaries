...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/superpmi/superpmicollect superpmicollect.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/superpmi/superpmicollect superpmicollect.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=superpmicollect.dasm.il superpmicollect.exe
    "$CORE_ROOT/ildasm" -raweh -out=superpmicollect.dasm.il superpmicollect.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=superpmicollect.asm.exe  superpmicollect.dasm.il
    "$CORE_ROOT/ilasm" -output=superpmicollect.asm.exe  superpmicollect.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=superpmicollect
    if [ ! -f superpmicollect.org ]; then
        TakeLock
        if [ ! -f superpmicollect.org ]; then
          mkdir IL
          cp superpmicollect.exe IL/superpmicollect.exe
          mv superpmicollect.exe superpmicollect.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in superpmicollect.org /out superpmicollect.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in superpmicollect.org /out superpmicollect.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in superpmicollect.reflect.xml
  if [ -f superpmicollect.reflect.xml ]
then
    ReflectionRoots="-x superpmicollect.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/superpmicollect.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" superpmicollect.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" superpmicollect.asm.exe $CLRTestExecutionArguments
