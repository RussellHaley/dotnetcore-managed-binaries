...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/UDivConst UDivConst.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/UDivConst UDivConst.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=UDivConst.dasm.il UDivConst.exe
    "$CORE_ROOT/ildasm" -raweh -out=UDivConst.dasm.il UDivConst.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=UDivConst.asm.exe  UDivConst.dasm.il
    "$CORE_ROOT/ilasm" -output=UDivConst.asm.exe  UDivConst.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=UDivConst
    if [ ! -f UDivConst.org ]; then
        TakeLock
        if [ ! -f UDivConst.org ]; then
          mkdir IL
          cp UDivConst.exe IL/UDivConst.exe
          mv UDivConst.exe UDivConst.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in UDivConst.org /out UDivConst.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in UDivConst.org /out UDivConst.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in UDivConst.reflect.xml
  if [ -f UDivConst.reflect.xml ]
then
    ReflectionRoots="-x UDivConst.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" UDivConst.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" UDivConst.asm.exe $CLRTestExecutionArguments
