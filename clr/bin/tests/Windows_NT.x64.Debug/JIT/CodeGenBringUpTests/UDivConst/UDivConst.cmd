...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\UDivConst UDivConst.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\UDivConst UDivConst.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=UDivConst.dasm.il UDivConst.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=UDivConst.dasm.il UDivConst.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=UDivConst.asm.exe  UDivConst.dasm.il
    %CORE_ROOT%\ilasm.exe /output=UDivConst.asm.exe  UDivConst.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=UDivConst
    if not exist "UDivConst.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "UDivConst.org" (
            mkdir IL
            copy UDivConst.exe IL\UDivConst.exe 
            ren UDivConst.exe UDivConst.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in UDivConst.org /out UDivConst.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in UDivConst.org /out UDivConst.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in UDivConst.reflect.xml
    IF EXIST UDivConst.reflect.xml set ReflectionRoots=-x UDivConst.reflect.xml
    ...
    set ExePath=%LinkBin%\UDivConst.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% UDivConst.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% UDivConst.asm.exe %CLRTestExecutionArguments% %Host_Args%
