...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/UModConst UModConst.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/UModConst UModConst.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=UModConst.dasm.il UModConst.exe
    "$CORE_ROOT/ildasm" -raweh -out=UModConst.dasm.il UModConst.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=UModConst.asm.exe  UModConst.dasm.il
    "$CORE_ROOT/ilasm" -output=UModConst.asm.exe  UModConst.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=UModConst
    if [ ! -f UModConst.org ]; then
        TakeLock
        if [ ! -f UModConst.org ]; then
          mkdir IL
          cp UModConst.exe IL/UModConst.exe
          mv UModConst.exe UModConst.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in UModConst.org /out UModConst.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in UModConst.org /out UModConst.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in UModConst.reflect.xml
  if [ -f UModConst.reflect.xml ]
then
    ReflectionRoots="-x UModConst.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" UModConst.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" UModConst.asm.exe $CLRTestExecutionArguments
