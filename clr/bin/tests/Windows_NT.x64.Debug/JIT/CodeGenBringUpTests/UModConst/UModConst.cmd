...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\UModConst UModConst.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\UModConst UModConst.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=UModConst.dasm.il UModConst.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=UModConst.dasm.il UModConst.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=UModConst.asm.exe  UModConst.dasm.il
    %CORE_ROOT%\ilasm.exe /output=UModConst.asm.exe  UModConst.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=UModConst
    if not exist "UModConst.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "UModConst.org" (
            mkdir IL
            copy UModConst.exe IL\UModConst.exe 
            ren UModConst.exe UModConst.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in UModConst.org /out UModConst.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in UModConst.org /out UModConst.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in UModConst.reflect.xml
    IF EXIST UModConst.reflect.xml set ReflectionRoots=-x UModConst.reflect.xml
    ...
    set ExePath=%LinkBin%\UModConst.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% UModConst.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% UModConst.asm.exe %CLRTestExecutionArguments% %Host_Args%
