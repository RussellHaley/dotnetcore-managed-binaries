...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\ArrayExc ArrayExc.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\ArrayExc ArrayExc.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ArrayExc.dasm.il ArrayExc.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ArrayExc.dasm.il ArrayExc.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ArrayExc.asm.exe  ArrayExc.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ArrayExc.asm.exe  ArrayExc.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ArrayExc
    if not exist "ArrayExc.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ArrayExc.org" (
            mkdir IL
            copy ArrayExc.exe IL\ArrayExc.exe 
            ren ArrayExc.exe ArrayExc.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayExc.org /out ArrayExc.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayExc.org /out ArrayExc.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ArrayExc.reflect.xml
    IF EXIST ArrayExc.reflect.xml set ReflectionRoots=-x ArrayExc.reflect.xml
    ...
    set ExePath=%LinkBin%\ArrayExc.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ArrayExc.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ArrayExc.asm.exe %CLRTestExecutionArguments% %Host_Args%
