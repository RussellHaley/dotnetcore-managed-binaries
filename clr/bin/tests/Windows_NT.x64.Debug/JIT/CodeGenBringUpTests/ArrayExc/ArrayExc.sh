...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/ArrayExc ArrayExc.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/ArrayExc ArrayExc.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ArrayExc.dasm.il ArrayExc.exe
    "$CORE_ROOT/ildasm" -raweh -out=ArrayExc.dasm.il ArrayExc.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ArrayExc.asm.exe  ArrayExc.dasm.il
    "$CORE_ROOT/ilasm" -output=ArrayExc.asm.exe  ArrayExc.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ArrayExc
    if [ ! -f ArrayExc.org ]; then
        TakeLock
        if [ ! -f ArrayExc.org ]; then
          mkdir IL
          cp ArrayExc.exe IL/ArrayExc.exe
          mv ArrayExc.exe ArrayExc.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayExc.org /out ArrayExc.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayExc.org /out ArrayExc.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ArrayExc.reflect.xml
  if [ -f ArrayExc.reflect.xml ]
then
    ReflectionRoots="-x ArrayExc.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ArrayExc.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ArrayExc.asm.exe $CLRTestExecutionArguments
