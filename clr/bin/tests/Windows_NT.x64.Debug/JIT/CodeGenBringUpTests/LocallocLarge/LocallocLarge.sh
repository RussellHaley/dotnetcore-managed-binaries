...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/LocallocLarge LocallocLarge.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/LocallocLarge LocallocLarge.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LocallocLarge.dasm.il LocallocLarge.exe
    "$CORE_ROOT/ildasm" -raweh -out=LocallocLarge.dasm.il LocallocLarge.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LocallocLarge.asm.exe  LocallocLarge.dasm.il
    "$CORE_ROOT/ilasm" -output=LocallocLarge.asm.exe  LocallocLarge.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LocallocLarge
    if [ ! -f LocallocLarge.org ]; then
        TakeLock
        if [ ! -f LocallocLarge.org ]; then
          mkdir IL
          cp LocallocLarge.exe IL/LocallocLarge.exe
          mv LocallocLarge.exe LocallocLarge.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LocallocLarge.org /out LocallocLarge.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LocallocLarge.org /out LocallocLarge.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LocallocLarge.reflect.xml
  if [ -f LocallocLarge.reflect.xml ]
then
    ReflectionRoots="-x LocallocLarge.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/LocallocLarge.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LocallocLarge.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LocallocLarge.asm.exe $CLRTestExecutionArguments
