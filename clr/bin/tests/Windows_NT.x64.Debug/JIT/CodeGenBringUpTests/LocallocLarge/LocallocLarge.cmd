...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\LocallocLarge LocallocLarge.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\LocallocLarge LocallocLarge.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LocallocLarge.dasm.il LocallocLarge.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LocallocLarge.dasm.il LocallocLarge.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LocallocLarge.asm.exe  LocallocLarge.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LocallocLarge.asm.exe  LocallocLarge.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LocallocLarge
    if not exist "LocallocLarge.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LocallocLarge.org" (
            mkdir IL
            copy LocallocLarge.exe IL\LocallocLarge.exe 
            ren LocallocLarge.exe LocallocLarge.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LocallocLarge.org /out LocallocLarge.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LocallocLarge.org /out LocallocLarge.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LocallocLarge.reflect.xml
    IF EXIST LocallocLarge.reflect.xml set ReflectionRoots=-x LocallocLarge.reflect.xml
    ...
    set ExePath=%LinkBin%\LocallocLarge.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LocallocLarge.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LocallocLarge.asm.exe %CLRTestExecutionArguments% %Host_Args%
