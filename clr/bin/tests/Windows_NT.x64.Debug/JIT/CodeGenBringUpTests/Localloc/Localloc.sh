...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/Localloc Localloc.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/Localloc Localloc.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Localloc.dasm.il Localloc.exe
    "$CORE_ROOT/ildasm" -raweh -out=Localloc.dasm.il Localloc.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Localloc.asm.exe  Localloc.dasm.il
    "$CORE_ROOT/ilasm" -output=Localloc.asm.exe  Localloc.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Localloc
    if [ ! -f Localloc.org ]; then
        TakeLock
        if [ ! -f Localloc.org ]; then
          mkdir IL
          cp Localloc.exe IL/Localloc.exe
          mv Localloc.exe Localloc.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Localloc.org /out Localloc.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Localloc.org /out Localloc.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Localloc.reflect.xml
  if [ -f Localloc.reflect.xml ]
then
    ReflectionRoots="-x Localloc.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Localloc.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Localloc.asm.exe $CLRTestExecutionArguments
