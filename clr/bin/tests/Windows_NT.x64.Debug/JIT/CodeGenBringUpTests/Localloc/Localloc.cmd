...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\Localloc Localloc.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\Localloc Localloc.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Localloc.dasm.il Localloc.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Localloc.dasm.il Localloc.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Localloc.asm.exe  Localloc.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Localloc.asm.exe  Localloc.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Localloc
    if not exist "Localloc.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Localloc.org" (
            mkdir IL
            copy Localloc.exe IL\Localloc.exe 
            ren Localloc.exe Localloc.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Localloc.org /out Localloc.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Localloc.org /out Localloc.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Localloc.reflect.xml
    IF EXIST Localloc.reflect.xml set ReflectionRoots=-x Localloc.reflect.xml
    ...
    set ExePath=%LinkBin%\Localloc.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Localloc.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Localloc.asm.exe %CLRTestExecutionArguments% %Host_Args%
