...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/ModConst ModConst.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/ModConst ModConst.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ModConst.dasm.il ModConst.exe
    "$CORE_ROOT/ildasm" -raweh -out=ModConst.dasm.il ModConst.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ModConst.asm.exe  ModConst.dasm.il
    "$CORE_ROOT/ilasm" -output=ModConst.asm.exe  ModConst.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ModConst
    if [ ! -f ModConst.org ]; then
        TakeLock
        if [ ! -f ModConst.org ]; then
          mkdir IL
          cp ModConst.exe IL/ModConst.exe
          mv ModConst.exe ModConst.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ModConst.org /out ModConst.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ModConst.org /out ModConst.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ModConst.reflect.xml
  if [ -f ModConst.reflect.xml ]
then
    ReflectionRoots="-x ModConst.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ModConst.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ModConst.asm.exe $CLRTestExecutionArguments
