...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\ModConst ModConst.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\ModConst ModConst.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ModConst.dasm.il ModConst.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ModConst.dasm.il ModConst.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ModConst.asm.exe  ModConst.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ModConst.asm.exe  ModConst.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ModConst
    if not exist "ModConst.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ModConst.org" (
            mkdir IL
            copy ModConst.exe IL\ModConst.exe 
            ren ModConst.exe ModConst.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ModConst.org /out ModConst.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ModConst.org /out ModConst.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ModConst.reflect.xml
    IF EXIST ModConst.reflect.xml set ReflectionRoots=-x ModConst.reflect.xml
    ...
    set ExePath=%LinkBin%\ModConst.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ModConst.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ModConst.asm.exe %CLRTestExecutionArguments% %Host_Args%
