...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\div2 div2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\div2 div2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=div2.dasm.il div2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=div2.dasm.il div2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=div2.asm.exe  div2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=div2.asm.exe  div2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=div2
    if not exist "div2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "div2.org" (
            mkdir IL
            copy div2.exe IL\div2.exe 
            ren div2.exe div2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in div2.org /out div2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in div2.org /out div2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in div2.reflect.xml
    IF EXIST div2.reflect.xml set ReflectionRoots=-x div2.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% div2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% div2.asm.exe %CLRTestExecutionArguments% %Host_Args%
