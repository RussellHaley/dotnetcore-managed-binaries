...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/div2 div2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/div2 div2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=div2.dasm.il div2.exe
    "$CORE_ROOT/ildasm" -raweh -out=div2.dasm.il div2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=div2.asm.exe  div2.dasm.il
    "$CORE_ROOT/ilasm" -output=div2.asm.exe  div2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=div2
    if [ ! -f div2.org ]; then
        TakeLock
        if [ ! -f div2.org ]; then
          mkdir IL
          cp div2.exe IL/div2.exe
          mv div2.exe div2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in div2.org /out div2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in div2.org /out div2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in div2.reflect.xml
  if [ -f div2.reflect.xml ]
then
    ReflectionRoots="-x div2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" div2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" div2.asm.exe $CLRTestExecutionArguments
