...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\DivConst DivConst.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CodeGenBringUpTests\DivConst DivConst.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DivConst.dasm.il DivConst.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DivConst.dasm.il DivConst.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DivConst.asm.exe  DivConst.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DivConst.asm.exe  DivConst.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DivConst
    if not exist "DivConst.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DivConst.org" (
            mkdir IL
            copy DivConst.exe IL\DivConst.exe 
            ren DivConst.exe DivConst.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DivConst.org /out DivConst.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DivConst.org /out DivConst.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DivConst.reflect.xml
    IF EXIST DivConst.reflect.xml set ReflectionRoots=-x DivConst.reflect.xml
    ...
    set ExePath=%LinkBin%\DivConst.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DivConst.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DivConst.asm.exe %CLRTestExecutionArguments% %Host_Args%
