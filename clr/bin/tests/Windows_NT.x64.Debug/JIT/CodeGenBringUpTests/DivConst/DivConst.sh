...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/DivConst DivConst.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CodeGenBringUpTests/DivConst DivConst.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DivConst.dasm.il DivConst.exe
    "$CORE_ROOT/ildasm" -raweh -out=DivConst.dasm.il DivConst.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DivConst.asm.exe  DivConst.dasm.il
    "$CORE_ROOT/ilasm" -output=DivConst.asm.exe  DivConst.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DivConst
    if [ ! -f DivConst.org ]; then
        TakeLock
        if [ ! -f DivConst.org ]; then
          mkdir IL
          cp DivConst.exe IL/DivConst.exe
          mv DivConst.exe DivConst.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DivConst.org /out DivConst.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DivConst.org /out DivConst.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DivConst.reflect.xml
  if [ -f DivConst.reflect.xml ]
then
    ReflectionRoots="-x DivConst.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DivConst.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DivConst.asm.exe $CLRTestExecutionArguments
