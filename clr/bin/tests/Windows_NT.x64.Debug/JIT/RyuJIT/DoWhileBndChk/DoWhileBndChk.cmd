...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\RyuJIT\DoWhileBndChk DoWhileBndChk.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\RyuJIT\DoWhileBndChk DoWhileBndChk.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DoWhileBndChk.dasm.il DoWhileBndChk.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DoWhileBndChk.dasm.il DoWhileBndChk.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DoWhileBndChk.asm.exe  DoWhileBndChk.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DoWhileBndChk.asm.exe  DoWhileBndChk.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DoWhileBndChk
    if not exist "DoWhileBndChk.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DoWhileBndChk.org" (
            mkdir IL
            copy DoWhileBndChk.exe IL\DoWhileBndChk.exe 
            ren DoWhileBndChk.exe DoWhileBndChk.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DoWhileBndChk.org /out DoWhileBndChk.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DoWhileBndChk.org /out DoWhileBndChk.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DoWhileBndChk.reflect.xml
    IF EXIST DoWhileBndChk.reflect.xml set ReflectionRoots=-x DoWhileBndChk.reflect.xml
    ...
    set ExePath=%LinkBin%\DoWhileBndChk.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DoWhileBndChk.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DoWhileBndChk.asm.exe %CLRTestExecutionArguments% %Host_Args%
