...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/RyuJIT/DoWhileBndChk DoWhileBndChk.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/RyuJIT/DoWhileBndChk DoWhileBndChk.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DoWhileBndChk.dasm.il DoWhileBndChk.exe
    "$CORE_ROOT/ildasm" -raweh -out=DoWhileBndChk.dasm.il DoWhileBndChk.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DoWhileBndChk.asm.exe  DoWhileBndChk.dasm.il
    "$CORE_ROOT/ilasm" -output=DoWhileBndChk.asm.exe  DoWhileBndChk.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DoWhileBndChk
    if [ ! -f DoWhileBndChk.org ]; then
        TakeLock
        if [ ! -f DoWhileBndChk.org ]; then
          mkdir IL
          cp DoWhileBndChk.exe IL/DoWhileBndChk.exe
          mv DoWhileBndChk.exe DoWhileBndChk.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DoWhileBndChk.org /out DoWhileBndChk.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DoWhileBndChk.org /out DoWhileBndChk.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DoWhileBndChk.reflect.xml
  if [ -f DoWhileBndChk.reflect.xml ]
then
    ReflectionRoots="-x DoWhileBndChk.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DoWhileBndChk.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DoWhileBndChk.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DoWhileBndChk.asm.exe $CLRTestExecutionArguments
