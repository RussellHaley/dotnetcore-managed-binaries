...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Performance\RunBenchmarks\RunBenchmarks RunBenchmarks.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Performance\RunBenchmarks\RunBenchmarks RunBenchmarks.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=RunBenchmarks.dasm.il RunBenchmarks.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=RunBenchmarks.dasm.il RunBenchmarks.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=RunBenchmarks.asm.exe  RunBenchmarks.dasm.il
    %CORE_ROOT%\ilasm.exe /output=RunBenchmarks.asm.exe  RunBenchmarks.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=RunBenchmarks
    if not exist "RunBenchmarks.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "RunBenchmarks.org" (
            mkdir IL
            copy RunBenchmarks.exe IL\RunBenchmarks.exe 
            ren RunBenchmarks.exe RunBenchmarks.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RunBenchmarks.org /out RunBenchmarks.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RunBenchmarks.org /out RunBenchmarks.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in RunBenchmarks.reflect.xml
    IF EXIST RunBenchmarks.reflect.xml set ReflectionRoots=-x RunBenchmarks.reflect.xml
    ...
    set ExePath=%LinkBin%\RunBenchmarks.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% RunBenchmarks.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% RunBenchmarks.asm.exe %CLRTestExecutionArguments% %Host_Args%
