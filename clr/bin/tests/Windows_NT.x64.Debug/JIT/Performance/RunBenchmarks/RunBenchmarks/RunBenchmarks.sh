...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Performance/RunBenchmarks/RunBenchmarks RunBenchmarks.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Performance/RunBenchmarks/RunBenchmarks RunBenchmarks.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=RunBenchmarks.dasm.il RunBenchmarks.exe
    "$CORE_ROOT/ildasm" -raweh -out=RunBenchmarks.dasm.il RunBenchmarks.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=RunBenchmarks.asm.exe  RunBenchmarks.dasm.il
    "$CORE_ROOT/ilasm" -output=RunBenchmarks.asm.exe  RunBenchmarks.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=RunBenchmarks
    if [ ! -f RunBenchmarks.org ]; then
        TakeLock
        if [ ! -f RunBenchmarks.org ]; then
          mkdir IL
          cp RunBenchmarks.exe IL/RunBenchmarks.exe
          mv RunBenchmarks.exe RunBenchmarks.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RunBenchmarks.org /out RunBenchmarks.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RunBenchmarks.org /out RunBenchmarks.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in RunBenchmarks.reflect.xml
  if [ -f RunBenchmarks.reflect.xml ]
then
    ReflectionRoots="-x RunBenchmarks.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/RunBenchmarks.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" RunBenchmarks.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" RunBenchmarks.asm.exe $CLRTestExecutionArguments
