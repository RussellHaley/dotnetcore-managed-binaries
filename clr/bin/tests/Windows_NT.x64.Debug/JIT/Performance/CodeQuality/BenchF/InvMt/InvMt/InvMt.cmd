...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\InvMt\InvMt InvMt.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\InvMt\InvMt InvMt.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=InvMt.dasm.il InvMt.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=InvMt.dasm.il InvMt.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=InvMt.asm.exe  InvMt.dasm.il
    %CORE_ROOT%\ilasm.exe /output=InvMt.asm.exe  InvMt.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=InvMt
    if not exist "InvMt.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "InvMt.org" (
            mkdir IL
            copy InvMt.exe IL\InvMt.exe 
            ren InvMt.exe InvMt.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InvMt.org /out InvMt.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InvMt.org /out InvMt.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in InvMt.reflect.xml
    IF EXIST InvMt.reflect.xml set ReflectionRoots=-x InvMt.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% InvMt.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% InvMt.asm.exe %CLRTestExecutionArguments% %Host_Args%
