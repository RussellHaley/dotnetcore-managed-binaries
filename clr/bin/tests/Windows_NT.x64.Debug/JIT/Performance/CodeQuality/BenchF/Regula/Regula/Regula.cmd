...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Regula\Regula Regula.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Regula\Regula Regula.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Regula.dasm.il Regula.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Regula.dasm.il Regula.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Regula.asm.exe  Regula.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Regula.asm.exe  Regula.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Regula
    if not exist "Regula.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Regula.org" (
            mkdir IL
            copy Regula.exe IL\Regula.exe 
            ren Regula.exe Regula.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Regula.org /out Regula.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Regula.org /out Regula.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Regula.reflect.xml
    IF EXIST Regula.reflect.xml set ReflectionRoots=-x Regula.reflect.xml
    ...
    set ExePath=%LinkBin%\Regula.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Regula.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Regula.asm.exe %CLRTestExecutionArguments% %Host_Args%
