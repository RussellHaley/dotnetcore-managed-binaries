...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Romber\Romber Romber.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Romber\Romber Romber.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Romber.dasm.il Romber.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Romber.dasm.il Romber.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Romber.asm.exe  Romber.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Romber.asm.exe  Romber.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Romber
    if not exist "Romber.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Romber.org" (
            mkdir IL
            copy Romber.exe IL\Romber.exe 
            ren Romber.exe Romber.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Romber.org /out Romber.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Romber.org /out Romber.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Romber.reflect.xml
    IF EXIST Romber.reflect.xml set ReflectionRoots=-x Romber.reflect.xml
    ...
    set ExePath=%LinkBin%\Romber.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Romber.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Romber.asm.exe %CLRTestExecutionArguments% %Host_Args%
