...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\NewtR\NewtR NewtR.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\NewtR\NewtR NewtR.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NewtR.dasm.il NewtR.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NewtR.dasm.il NewtR.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NewtR.asm.exe  NewtR.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NewtR.asm.exe  NewtR.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NewtR
    if not exist "NewtR.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NewtR.org" (
            mkdir IL
            copy NewtR.exe IL\NewtR.exe 
            ren NewtR.exe NewtR.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NewtR.org /out NewtR.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NewtR.org /out NewtR.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NewtR.reflect.xml
    IF EXIST NewtR.reflect.xml set ReflectionRoots=-x NewtR.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NewtR.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NewtR.asm.exe %CLRTestExecutionArguments% %Host_Args%
