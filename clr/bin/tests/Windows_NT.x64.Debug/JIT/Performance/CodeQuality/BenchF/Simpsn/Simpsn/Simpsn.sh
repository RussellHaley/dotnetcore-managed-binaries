...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Simpsn/Simpsn Simpsn.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Simpsn/Simpsn Simpsn.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Simpsn.dasm.il Simpsn.exe
    "$CORE_ROOT/ildasm" -raweh -out=Simpsn.dasm.il Simpsn.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Simpsn.asm.exe  Simpsn.dasm.il
    "$CORE_ROOT/ilasm" -output=Simpsn.asm.exe  Simpsn.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Simpsn
    if [ ! -f Simpsn.org ]; then
        TakeLock
        if [ ! -f Simpsn.org ]; then
          mkdir IL
          cp Simpsn.exe IL/Simpsn.exe
          mv Simpsn.exe Simpsn.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Simpsn.org /out Simpsn.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Simpsn.org /out Simpsn.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Simpsn.reflect.xml
  if [ -f Simpsn.reflect.xml ]
then
    ReflectionRoots="-x Simpsn.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Simpsn.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Simpsn.asm.exe $CLRTestExecutionArguments
