...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Secant\Secant Secant.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Secant\Secant Secant.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Secant.dasm.il Secant.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Secant.dasm.il Secant.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Secant.asm.exe  Secant.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Secant.asm.exe  Secant.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Secant
    if not exist "Secant.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Secant.org" (
            mkdir IL
            copy Secant.exe IL\Secant.exe 
            ren Secant.exe Secant.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Secant.org /out Secant.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Secant.org /out Secant.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Secant.reflect.xml
    IF EXIST Secant.reflect.xml set ReflectionRoots=-x Secant.reflect.xml
    ...
    set ExePath=%LinkBin%\Secant.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Secant.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Secant.asm.exe %CLRTestExecutionArguments% %Host_Args%
