...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Adams\Adams Adams.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Adams\Adams Adams.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Adams.dasm.il Adams.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Adams.dasm.il Adams.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Adams.asm.exe  Adams.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Adams.asm.exe  Adams.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Adams
    if not exist "Adams.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Adams.org" (
            mkdir IL
            copy Adams.exe IL\Adams.exe 
            ren Adams.exe Adams.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Adams.org /out Adams.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Adams.org /out Adams.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Adams.reflect.xml
    IF EXIST Adams.reflect.xml set ReflectionRoots=-x Adams.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Adams.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Adams.asm.exe %CLRTestExecutionArguments% %Host_Args%
