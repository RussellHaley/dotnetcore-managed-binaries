...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/NewtR/NewtR NewtR.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/NewtR/NewtR NewtR.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NewtR.dasm.il NewtR.exe
    "$CORE_ROOT/ildasm" -raweh -out=NewtR.dasm.il NewtR.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NewtR.asm.exe  NewtR.dasm.il
    "$CORE_ROOT/ilasm" -output=NewtR.asm.exe  NewtR.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NewtR
    if [ ! -f NewtR.org ]; then
        TakeLock
        if [ ! -f NewtR.org ]; then
          mkdir IL
          cp NewtR.exe IL/NewtR.exe
          mv NewtR.exe NewtR.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NewtR.org /out NewtR.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NewtR.org /out NewtR.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NewtR.reflect.xml
  if [ -f NewtR.reflect.xml ]
then
    ReflectionRoots="-x NewtR.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NewtR.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NewtR.asm.exe $CLRTestExecutionArguments
