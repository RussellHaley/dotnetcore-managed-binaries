...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Adams/Adams Adams.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Adams/Adams Adams.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Adams.dasm.il Adams.exe
    "$CORE_ROOT/ildasm" -raweh -out=Adams.dasm.il Adams.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Adams.asm.exe  Adams.dasm.il
    "$CORE_ROOT/ilasm" -output=Adams.asm.exe  Adams.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Adams
    if [ ! -f Adams.org ]; then
        TakeLock
        if [ ! -f Adams.org ]; then
          mkdir IL
          cp Adams.exe IL/Adams.exe
          mv Adams.exe Adams.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Adams.org /out Adams.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Adams.org /out Adams.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Adams.reflect.xml
  if [ -f Adams.reflect.xml ]
then
    ReflectionRoots="-x Adams.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Adams.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Adams.asm.exe $CLRTestExecutionArguments
