...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/InProd/InProd InProd.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/InProd/InProd InProd.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=InProd.dasm.il InProd.exe
    "$CORE_ROOT/ildasm" -raweh -out=InProd.dasm.il InProd.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=InProd.asm.exe  InProd.dasm.il
    "$CORE_ROOT/ilasm" -output=InProd.asm.exe  InProd.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=InProd
    if [ ! -f InProd.org ]; then
        TakeLock
        if [ ! -f InProd.org ]; then
          mkdir IL
          cp InProd.exe IL/InProd.exe
          mv InProd.exe InProd.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InProd.org /out InProd.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InProd.org /out InProd.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in InProd.reflect.xml
  if [ -f InProd.reflect.xml ]
then
    ReflectionRoots="-x InProd.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" InProd.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" InProd.asm.exe $CLRTestExecutionArguments
