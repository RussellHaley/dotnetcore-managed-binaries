...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\FFT\FFT FFT.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\FFT\FFT FFT.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=FFT.dasm.il FFT.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=FFT.dasm.il FFT.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=FFT.asm.exe  FFT.dasm.il
    %CORE_ROOT%\ilasm.exe /output=FFT.asm.exe  FFT.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=FFT
    if not exist "FFT.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "FFT.org" (
            mkdir IL
            copy FFT.exe IL\FFT.exe 
            ren FFT.exe FFT.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FFT.org /out FFT.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FFT.org /out FFT.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in FFT.reflect.xml
    IF EXIST FFT.reflect.xml set ReflectionRoots=-x FFT.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% FFT.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% FFT.asm.exe %CLRTestExecutionArguments% %Host_Args%
