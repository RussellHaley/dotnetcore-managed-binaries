...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/DMath/DMath DMath.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/DMath/DMath DMath.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DMath.dasm.il DMath.exe
    "$CORE_ROOT/ildasm" -raweh -out=DMath.dasm.il DMath.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DMath.asm.exe  DMath.dasm.il
    "$CORE_ROOT/ilasm" -output=DMath.asm.exe  DMath.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DMath
    if [ ! -f DMath.org ]; then
        TakeLock
        if [ ! -f DMath.org ]; then
          mkdir IL
          cp DMath.exe IL/DMath.exe
          mv DMath.exe DMath.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DMath.org /out DMath.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DMath.org /out DMath.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in DMath.reflect.xml
  if [ -f DMath.reflect.xml ]
then
    ReflectionRoots="-x DMath.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DMath.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DMath.asm.exe $CLRTestExecutionArguments
