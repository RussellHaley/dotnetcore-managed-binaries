...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Simpsn\Simpsn Simpsn.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Simpsn\Simpsn Simpsn.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Simpsn.dasm.il Simpsn.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Simpsn.dasm.il Simpsn.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Simpsn.asm.exe  Simpsn.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Simpsn.asm.exe  Simpsn.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Simpsn
    if not exist "Simpsn.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Simpsn.org" (
            mkdir IL
            copy Simpsn.exe IL\Simpsn.exe 
            ren Simpsn.exe Simpsn.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Simpsn.org /out Simpsn.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Simpsn.org /out Simpsn.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Simpsn.reflect.xml
    IF EXIST Simpsn.reflect.xml set ReflectionRoots=-x Simpsn.reflect.xml
    ...
    set ExePath=%LinkBin%\Simpsn.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Simpsn.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Simpsn.asm.exe %CLRTestExecutionArguments% %Host_Args%
