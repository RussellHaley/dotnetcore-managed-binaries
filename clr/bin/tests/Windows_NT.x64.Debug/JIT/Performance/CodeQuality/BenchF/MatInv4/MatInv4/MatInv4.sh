...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/MatInv4/MatInv4 MatInv4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/MatInv4/MatInv4 MatInv4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MatInv4.dasm.il MatInv4.exe
    "$CORE_ROOT/ildasm" -raweh -out=MatInv4.dasm.il MatInv4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MatInv4.asm.exe  MatInv4.dasm.il
    "$CORE_ROOT/ilasm" -output=MatInv4.asm.exe  MatInv4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MatInv4
    if [ ! -f MatInv4.org ]; then
        TakeLock
        if [ ! -f MatInv4.org ]; then
          mkdir IL
          cp MatInv4.exe IL/MatInv4.exe
          mv MatInv4.exe MatInv4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MatInv4.org /out MatInv4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MatInv4.org /out MatInv4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in MatInv4.reflect.xml
  if [ -f MatInv4.reflect.xml ]
then
    ReflectionRoots="-x MatInv4.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MatInv4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MatInv4.asm.exe $CLRTestExecutionArguments
