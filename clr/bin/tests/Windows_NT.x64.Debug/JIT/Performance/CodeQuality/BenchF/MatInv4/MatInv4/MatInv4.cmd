...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\MatInv4\MatInv4 MatInv4.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\MatInv4\MatInv4 MatInv4.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MatInv4.dasm.il MatInv4.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MatInv4.dasm.il MatInv4.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MatInv4.asm.exe  MatInv4.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MatInv4.asm.exe  MatInv4.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MatInv4
    if not exist "MatInv4.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MatInv4.org" (
            mkdir IL
            copy MatInv4.exe IL\MatInv4.exe 
            ren MatInv4.exe MatInv4.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MatInv4.org /out MatInv4.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MatInv4.org /out MatInv4.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MatInv4.reflect.xml
    IF EXIST MatInv4.reflect.xml set ReflectionRoots=-x MatInv4.reflect.xml
    ...
    set ExePath=%LinkBin%\MatInv4.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MatInv4.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MatInv4.asm.exe %CLRTestExecutionArguments% %Host_Args%
