...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Trap\Trap Trap.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Trap\Trap Trap.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Trap.dasm.il Trap.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Trap.dasm.il Trap.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Trap.asm.exe  Trap.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Trap.asm.exe  Trap.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Trap
    if not exist "Trap.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Trap.org" (
            mkdir IL
            copy Trap.exe IL\Trap.exe 
            ren Trap.exe Trap.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Trap.org /out Trap.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Trap.org /out Trap.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Trap.reflect.xml
    IF EXIST Trap.reflect.xml set ReflectionRoots=-x Trap.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Trap.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Trap.asm.exe %CLRTestExecutionArguments% %Host_Args%
