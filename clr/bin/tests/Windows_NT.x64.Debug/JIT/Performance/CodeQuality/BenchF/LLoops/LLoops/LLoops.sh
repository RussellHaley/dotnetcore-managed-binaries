...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/LLoops/LLoops LLoops.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/LLoops/LLoops LLoops.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LLoops.dasm.il LLoops.exe
    "$CORE_ROOT/ildasm" -raweh -out=LLoops.dasm.il LLoops.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LLoops.asm.exe  LLoops.dasm.il
    "$CORE_ROOT/ilasm" -output=LLoops.asm.exe  LLoops.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LLoops
    if [ ! -f LLoops.org ]; then
        TakeLock
        if [ ! -f LLoops.org ]; then
          mkdir IL
          cp LLoops.exe IL/LLoops.exe
          mv LLoops.exe LLoops.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LLoops.org /out LLoops.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LLoops.org /out LLoops.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LLoops.reflect.xml
  if [ -f LLoops.reflect.xml ]
then
    ReflectionRoots="-x LLoops.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LLoops.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LLoops.asm.exe $CLRTestExecutionArguments
