...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Bisect\Bisect Bisect.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Bisect\Bisect Bisect.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Bisect.dasm.il Bisect.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Bisect.dasm.il Bisect.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Bisect.asm.exe  Bisect.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Bisect.asm.exe  Bisect.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Bisect
    if not exist "Bisect.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Bisect.org" (
            mkdir IL
            copy Bisect.exe IL\Bisect.exe 
            ren Bisect.exe Bisect.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Bisect.org /out Bisect.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Bisect.org /out Bisect.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Bisect.reflect.xml
    IF EXIST Bisect.reflect.xml set ReflectionRoots=-x Bisect.reflect.xml
    ...
    set ExePath=%LinkBin%\Bisect.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Bisect.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Bisect.asm.exe %CLRTestExecutionArguments% %Host_Args%
