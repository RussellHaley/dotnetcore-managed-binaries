...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\DMath\DMath DMath.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\DMath\DMath DMath.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DMath.dasm.il DMath.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DMath.dasm.il DMath.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DMath.asm.exe  DMath.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DMath.asm.exe  DMath.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DMath
    if not exist "DMath.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DMath.org" (
            mkdir IL
            copy DMath.exe IL\DMath.exe 
            ren DMath.exe DMath.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DMath.org /out DMath.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DMath.org /out DMath.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DMath.reflect.xml
    IF EXIST DMath.reflect.xml set ReflectionRoots=-x DMath.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DMath.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DMath.asm.exe %CLRTestExecutionArguments% %Host_Args%
