...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Lorenz\Lorenz Lorenz.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Lorenz\Lorenz Lorenz.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Lorenz.dasm.il Lorenz.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Lorenz.dasm.il Lorenz.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Lorenz.asm.exe  Lorenz.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Lorenz.asm.exe  Lorenz.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Lorenz
    if not exist "Lorenz.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Lorenz.org" (
            mkdir IL
            copy Lorenz.exe IL\Lorenz.exe 
            ren Lorenz.exe Lorenz.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Lorenz.org /out Lorenz.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Lorenz.org /out Lorenz.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Lorenz.reflect.xml
    IF EXIST Lorenz.reflect.xml set ReflectionRoots=-x Lorenz.reflect.xml
    ...
    set ExePath=%LinkBin%\Lorenz.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Lorenz.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Lorenz.asm.exe %CLRTestExecutionArguments% %Host_Args%
