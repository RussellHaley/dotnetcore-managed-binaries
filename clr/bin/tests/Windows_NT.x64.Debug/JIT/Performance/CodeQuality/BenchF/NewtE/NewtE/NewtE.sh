...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/NewtE/NewtE NewtE.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/NewtE/NewtE NewtE.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NewtE.dasm.il NewtE.exe
    "$CORE_ROOT/ildasm" -raweh -out=NewtE.dasm.il NewtE.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NewtE.asm.exe  NewtE.dasm.il
    "$CORE_ROOT/ilasm" -output=NewtE.asm.exe  NewtE.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NewtE
    if [ ! -f NewtE.org ]; then
        TakeLock
        if [ ! -f NewtE.org ]; then
          mkdir IL
          cp NewtE.exe IL/NewtE.exe
          mv NewtE.exe NewtE.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NewtE.org /out NewtE.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NewtE.org /out NewtE.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NewtE.reflect.xml
  if [ -f NewtE.reflect.xml ]
then
    ReflectionRoots="-x NewtE.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NewtE.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NewtE.asm.exe $CLRTestExecutionArguments
