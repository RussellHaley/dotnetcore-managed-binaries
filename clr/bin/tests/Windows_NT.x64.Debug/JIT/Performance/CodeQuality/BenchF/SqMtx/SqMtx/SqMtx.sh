...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/SqMtx/SqMtx SqMtx.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/SqMtx/SqMtx SqMtx.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=SqMtx.dasm.il SqMtx.exe
    "$CORE_ROOT/ildasm" -raweh -out=SqMtx.dasm.il SqMtx.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=SqMtx.asm.exe  SqMtx.dasm.il
    "$CORE_ROOT/ilasm" -output=SqMtx.asm.exe  SqMtx.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=SqMtx
    if [ ! -f SqMtx.org ]; then
        TakeLock
        if [ ! -f SqMtx.org ]; then
          mkdir IL
          cp SqMtx.exe IL/SqMtx.exe
          mv SqMtx.exe SqMtx.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SqMtx.org /out SqMtx.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SqMtx.org /out SqMtx.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in SqMtx.reflect.xml
  if [ -f SqMtx.reflect.xml ]
then
    ReflectionRoots="-x SqMtx.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" SqMtx.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" SqMtx.asm.exe $CLRTestExecutionArguments
