...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/BenchMk2/BenchMk2 BenchMk2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/BenchMk2/BenchMk2 BenchMk2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=BenchMk2.dasm.il BenchMk2.exe
    "$CORE_ROOT/ildasm" -raweh -out=BenchMk2.dasm.il BenchMk2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=BenchMk2.asm.exe  BenchMk2.dasm.il
    "$CORE_ROOT/ilasm" -output=BenchMk2.asm.exe  BenchMk2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=BenchMk2
    if [ ! -f BenchMk2.org ]; then
        TakeLock
        if [ ! -f BenchMk2.org ]; then
          mkdir IL
          cp BenchMk2.exe IL/BenchMk2.exe
          mv BenchMk2.exe BenchMk2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BenchMk2.org /out BenchMk2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BenchMk2.org /out BenchMk2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in BenchMk2.reflect.xml
  if [ -f BenchMk2.reflect.xml ]
then
    ReflectionRoots="-x BenchMk2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" BenchMk2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" BenchMk2.asm.exe $CLRTestExecutionArguments
