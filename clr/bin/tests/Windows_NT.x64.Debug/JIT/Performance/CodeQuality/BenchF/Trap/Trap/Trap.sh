...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Trap/Trap Trap.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Trap/Trap Trap.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Trap.dasm.il Trap.exe
    "$CORE_ROOT/ildasm" -raweh -out=Trap.dasm.il Trap.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Trap.asm.exe  Trap.dasm.il
    "$CORE_ROOT/ilasm" -output=Trap.asm.exe  Trap.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Trap
    if [ ! -f Trap.org ]; then
        TakeLock
        if [ ! -f Trap.org ]; then
          mkdir IL
          cp Trap.exe IL/Trap.exe
          mv Trap.exe Trap.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Trap.org /out Trap.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Trap.org /out Trap.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Trap.reflect.xml
  if [ -f Trap.reflect.xml ]
then
    ReflectionRoots="-x Trap.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Trap.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Trap.asm.exe $CLRTestExecutionArguments
