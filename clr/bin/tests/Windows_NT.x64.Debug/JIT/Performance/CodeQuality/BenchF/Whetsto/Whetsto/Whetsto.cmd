...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Whetsto\Whetsto Whetsto.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\Whetsto\Whetsto Whetsto.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Whetsto.dasm.il Whetsto.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Whetsto.dasm.il Whetsto.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Whetsto.asm.exe  Whetsto.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Whetsto.asm.exe  Whetsto.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Whetsto
    if not exist "Whetsto.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Whetsto.org" (
            mkdir IL
            copy Whetsto.exe IL\Whetsto.exe 
            ren Whetsto.exe Whetsto.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Whetsto.org /out Whetsto.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Whetsto.org /out Whetsto.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Whetsto.reflect.xml
    IF EXIST Whetsto.reflect.xml set ReflectionRoots=-x Whetsto.reflect.xml
    ...
    set ExePath=%LinkBin%\Whetsto.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Whetsto.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Whetsto.asm.exe %CLRTestExecutionArguments% %Host_Args%
