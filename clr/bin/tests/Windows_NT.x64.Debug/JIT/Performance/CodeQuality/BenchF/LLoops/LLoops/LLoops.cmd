...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\LLoops\LLoops LLoops.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\LLoops\LLoops LLoops.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LLoops.dasm.il LLoops.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LLoops.dasm.il LLoops.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LLoops.asm.exe  LLoops.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LLoops.asm.exe  LLoops.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LLoops
    if not exist "LLoops.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LLoops.org" (
            mkdir IL
            copy LLoops.exe IL\LLoops.exe 
            ren LLoops.exe LLoops.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LLoops.org /out LLoops.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LLoops.org /out LLoops.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LLoops.reflect.xml
    IF EXIST LLoops.reflect.xml set ReflectionRoots=-x LLoops.reflect.xml
    ...
    set ExePath=%LinkBin%\LLoops.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LLoops.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LLoops.asm.exe %CLRTestExecutionArguments% %Host_Args%
