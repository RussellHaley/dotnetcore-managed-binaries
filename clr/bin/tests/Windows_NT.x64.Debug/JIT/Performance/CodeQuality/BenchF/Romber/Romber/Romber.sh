...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Romber/Romber Romber.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Romber/Romber Romber.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Romber.dasm.il Romber.exe
    "$CORE_ROOT/ildasm" -raweh -out=Romber.dasm.il Romber.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Romber.asm.exe  Romber.dasm.il
    "$CORE_ROOT/ilasm" -output=Romber.asm.exe  Romber.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Romber
    if [ ! -f Romber.org ]; then
        TakeLock
        if [ ! -f Romber.org ]; then
          mkdir IL
          cp Romber.exe IL/Romber.exe
          mv Romber.exe Romber.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Romber.org /out Romber.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Romber.org /out Romber.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Romber.reflect.xml
  if [ -f Romber.reflect.xml ]
then
    ReflectionRoots="-x Romber.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Romber.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Romber.asm.exe $CLRTestExecutionArguments
