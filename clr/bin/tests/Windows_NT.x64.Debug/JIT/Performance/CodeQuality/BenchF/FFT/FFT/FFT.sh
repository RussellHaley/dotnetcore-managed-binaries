...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/FFT/FFT FFT.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/FFT/FFT FFT.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=FFT.dasm.il FFT.exe
    "$CORE_ROOT/ildasm" -raweh -out=FFT.dasm.il FFT.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=FFT.asm.exe  FFT.dasm.il
    "$CORE_ROOT/ilasm" -output=FFT.asm.exe  FFT.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=FFT
    if [ ! -f FFT.org ]; then
        TakeLock
        if [ ! -f FFT.org ]; then
          mkdir IL
          cp FFT.exe IL/FFT.exe
          mv FFT.exe FFT.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FFT.org /out FFT.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FFT.org /out FFT.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in FFT.reflect.xml
  if [ -f FFT.reflect.xml ]
then
    ReflectionRoots="-x FFT.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" FFT.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" FFT.asm.exe $CLRTestExecutionArguments
