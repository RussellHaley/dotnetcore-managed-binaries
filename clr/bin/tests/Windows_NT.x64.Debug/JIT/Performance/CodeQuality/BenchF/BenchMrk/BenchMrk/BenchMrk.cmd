...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\BenchMrk\BenchMrk BenchMrk.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\BenchMrk\BenchMrk BenchMrk.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=BenchMrk.dasm.il BenchMrk.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=BenchMrk.dasm.il BenchMrk.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=BenchMrk.asm.exe  BenchMrk.dasm.il
    %CORE_ROOT%\ilasm.exe /output=BenchMrk.asm.exe  BenchMrk.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=BenchMrk
    if not exist "BenchMrk.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "BenchMrk.org" (
            mkdir IL
            copy BenchMrk.exe IL\BenchMrk.exe 
            ren BenchMrk.exe BenchMrk.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BenchMrk.org /out BenchMrk.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BenchMrk.org /out BenchMrk.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in BenchMrk.reflect.xml
    IF EXIST BenchMrk.reflect.xml set ReflectionRoots=-x BenchMrk.reflect.xml
    ...
    set ExePath=%LinkBin%\BenchMrk.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% BenchMrk.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% BenchMrk.asm.exe %CLRTestExecutionArguments% %Host_Args%
