...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Bisect/Bisect Bisect.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Bisect/Bisect Bisect.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Bisect.dasm.il Bisect.exe
    "$CORE_ROOT/ildasm" -raweh -out=Bisect.dasm.il Bisect.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Bisect.asm.exe  Bisect.dasm.il
    "$CORE_ROOT/ilasm" -output=Bisect.asm.exe  Bisect.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Bisect
    if [ ! -f Bisect.org ]; then
        TakeLock
        if [ ! -f Bisect.org ]; then
          mkdir IL
          cp Bisect.exe IL/Bisect.exe
          mv Bisect.exe Bisect.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Bisect.org /out Bisect.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Bisect.org /out Bisect.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Bisect.reflect.xml
  if [ -f Bisect.reflect.xml ]
then
    ReflectionRoots="-x Bisect.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Bisect.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Bisect.asm.exe $CLRTestExecutionArguments
