...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\InProd\InProd InProd.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\InProd\InProd InProd.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=InProd.dasm.il InProd.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=InProd.dasm.il InProd.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=InProd.asm.exe  InProd.dasm.il
    %CORE_ROOT%\ilasm.exe /output=InProd.asm.exe  InProd.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=InProd
    if not exist "InProd.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "InProd.org" (
            mkdir IL
            copy InProd.exe IL\InProd.exe 
            ren InProd.exe InProd.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InProd.org /out InProd.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InProd.org /out InProd.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in InProd.reflect.xml
    IF EXIST InProd.reflect.xml set ReflectionRoots=-x InProd.reflect.xml
    ...
    set ExePath=%LinkBin%\InProd.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% InProd.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% InProd.asm.exe %CLRTestExecutionArguments% %Host_Args%
