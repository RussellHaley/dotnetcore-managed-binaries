...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Whetsto/Whetsto Whetsto.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Whetsto/Whetsto Whetsto.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Whetsto.dasm.il Whetsto.exe
    "$CORE_ROOT/ildasm" -raweh -out=Whetsto.dasm.il Whetsto.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Whetsto.asm.exe  Whetsto.dasm.il
    "$CORE_ROOT/ilasm" -output=Whetsto.asm.exe  Whetsto.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Whetsto
    if [ ! -f Whetsto.org ]; then
        TakeLock
        if [ ! -f Whetsto.org ]; then
          mkdir IL
          cp Whetsto.exe IL/Whetsto.exe
          mv Whetsto.exe Whetsto.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Whetsto.org /out Whetsto.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Whetsto.org /out Whetsto.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Whetsto.reflect.xml
  if [ -f Whetsto.reflect.xml ]
then
    ReflectionRoots="-x Whetsto.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Whetsto.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Whetsto.asm.exe $CLRTestExecutionArguments
