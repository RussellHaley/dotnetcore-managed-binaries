...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Secant/Secant Secant.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Secant/Secant Secant.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Secant.dasm.il Secant.exe
    "$CORE_ROOT/ildasm" -raweh -out=Secant.dasm.il Secant.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Secant.asm.exe  Secant.dasm.il
    "$CORE_ROOT/ilasm" -output=Secant.asm.exe  Secant.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Secant
    if [ ! -f Secant.org ]; then
        TakeLock
        if [ ! -f Secant.org ]; then
          mkdir IL
          cp Secant.exe IL/Secant.exe
          mv Secant.exe Secant.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Secant.org /out Secant.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Secant.org /out Secant.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Secant.reflect.xml
  if [ -f Secant.reflect.xml ]
then
    ReflectionRoots="-x Secant.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Secant.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Secant.asm.exe $CLRTestExecutionArguments
