...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/BenchMrk/BenchMrk BenchMrk.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/BenchMrk/BenchMrk BenchMrk.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=BenchMrk.dasm.il BenchMrk.exe
    "$CORE_ROOT/ildasm" -raweh -out=BenchMrk.dasm.il BenchMrk.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=BenchMrk.asm.exe  BenchMrk.dasm.il
    "$CORE_ROOT/ilasm" -output=BenchMrk.asm.exe  BenchMrk.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=BenchMrk
    if [ ! -f BenchMrk.org ]; then
        TakeLock
        if [ ! -f BenchMrk.org ]; then
          mkdir IL
          cp BenchMrk.exe IL/BenchMrk.exe
          mv BenchMrk.exe BenchMrk.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BenchMrk.org /out BenchMrk.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BenchMrk.org /out BenchMrk.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in BenchMrk.reflect.xml
  if [ -f BenchMrk.reflect.xml ]
then
    ReflectionRoots="-x BenchMrk.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" BenchMrk.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" BenchMrk.asm.exe $CLRTestExecutionArguments
