...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\BenchMk2\BenchMk2 BenchMk2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\BenchMk2\BenchMk2 BenchMk2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=BenchMk2.dasm.il BenchMk2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=BenchMk2.dasm.il BenchMk2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=BenchMk2.asm.exe  BenchMk2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=BenchMk2.asm.exe  BenchMk2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=BenchMk2
    if not exist "BenchMk2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "BenchMk2.org" (
            mkdir IL
            copy BenchMk2.exe IL\BenchMk2.exe 
            ren BenchMk2.exe BenchMk2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BenchMk2.org /out BenchMk2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BenchMk2.org /out BenchMk2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in BenchMk2.reflect.xml
    IF EXIST BenchMk2.reflect.xml set ReflectionRoots=-x BenchMk2.reflect.xml
    ...
    set ExePath=%LinkBin%\BenchMk2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% BenchMk2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% BenchMk2.asm.exe %CLRTestExecutionArguments% %Host_Args%
