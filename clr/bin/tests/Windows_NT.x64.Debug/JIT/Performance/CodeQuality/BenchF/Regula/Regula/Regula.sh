...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Regula/Regula Regula.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Regula/Regula Regula.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Regula.dasm.il Regula.exe
    "$CORE_ROOT/ildasm" -raweh -out=Regula.dasm.il Regula.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Regula.asm.exe  Regula.dasm.il
    "$CORE_ROOT/ilasm" -output=Regula.asm.exe  Regula.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Regula
    if [ ! -f Regula.org ]; then
        TakeLock
        if [ ! -f Regula.org ]; then
          mkdir IL
          cp Regula.exe IL/Regula.exe
          mv Regula.exe Regula.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Regula.org /out Regula.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Regula.org /out Regula.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Regula.reflect.xml
  if [ -f Regula.reflect.xml ]
then
    ReflectionRoots="-x Regula.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Regula.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Regula.asm.exe $CLRTestExecutionArguments
