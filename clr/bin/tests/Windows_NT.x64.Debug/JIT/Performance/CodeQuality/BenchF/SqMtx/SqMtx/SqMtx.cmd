...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\SqMtx\SqMtx SqMtx.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchF\SqMtx\SqMtx SqMtx.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=SqMtx.dasm.il SqMtx.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=SqMtx.dasm.il SqMtx.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=SqMtx.asm.exe  SqMtx.dasm.il
    %CORE_ROOT%\ilasm.exe /output=SqMtx.asm.exe  SqMtx.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=SqMtx
    if not exist "SqMtx.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "SqMtx.org" (
            mkdir IL
            copy SqMtx.exe IL\SqMtx.exe 
            ren SqMtx.exe SqMtx.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SqMtx.org /out SqMtx.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SqMtx.org /out SqMtx.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in SqMtx.reflect.xml
    IF EXIST SqMtx.reflect.xml set ReflectionRoots=-x SqMtx.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% SqMtx.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% SqMtx.asm.exe %CLRTestExecutionArguments% %Host_Args%
