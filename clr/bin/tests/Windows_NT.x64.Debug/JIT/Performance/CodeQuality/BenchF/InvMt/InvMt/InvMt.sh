...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/InvMt/InvMt InvMt.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/InvMt/InvMt InvMt.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=InvMt.dasm.il InvMt.exe
    "$CORE_ROOT/ildasm" -raweh -out=InvMt.dasm.il InvMt.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=InvMt.asm.exe  InvMt.dasm.il
    "$CORE_ROOT/ilasm" -output=InvMt.asm.exe  InvMt.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=InvMt
    if [ ! -f InvMt.org ]; then
        TakeLock
        if [ ! -f InvMt.org ]; then
          mkdir IL
          cp InvMt.exe IL/InvMt.exe
          mv InvMt.exe InvMt.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InvMt.org /out InvMt.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InvMt.org /out InvMt.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in InvMt.reflect.xml
  if [ -f InvMt.reflect.xml ]
then
    ReflectionRoots="-x InvMt.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" InvMt.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" InvMt.asm.exe $CLRTestExecutionArguments
