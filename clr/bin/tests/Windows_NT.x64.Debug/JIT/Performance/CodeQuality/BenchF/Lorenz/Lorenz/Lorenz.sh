...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Lorenz/Lorenz Lorenz.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchF/Lorenz/Lorenz Lorenz.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Lorenz.dasm.il Lorenz.exe
    "$CORE_ROOT/ildasm" -raweh -out=Lorenz.dasm.il Lorenz.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Lorenz.asm.exe  Lorenz.dasm.il
    "$CORE_ROOT/ilasm" -output=Lorenz.asm.exe  Lorenz.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Lorenz
    if [ ! -f Lorenz.org ]; then
        TakeLock
        if [ ! -f Lorenz.org ]; then
          mkdir IL
          cp Lorenz.exe IL/Lorenz.exe
          mv Lorenz.exe Lorenz.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Lorenz.org /out Lorenz.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Lorenz.org /out Lorenz.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Lorenz.reflect.xml
  if [ -f Lorenz.reflect.xml ]
then
    ReflectionRoots="-x Lorenz.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Lorenz.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Lorenz.asm.exe $CLRTestExecutionArguments
