...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Serialization\Deserialize Deserialize.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Serialization\Deserialize Deserialize.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Deserialize.dasm.il Deserialize.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Deserialize.dasm.il Deserialize.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Deserialize.asm.exe  Deserialize.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Deserialize.asm.exe  Deserialize.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Deserialize
    if not exist "Deserialize.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Deserialize.org" (
            mkdir IL
            copy Deserialize.exe IL\Deserialize.exe 
            ren Deserialize.exe Deserialize.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Deserialize.org /out Deserialize.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Deserialize.org /out Deserialize.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Deserialize.reflect.xml
    IF EXIST Deserialize.reflect.xml set ReflectionRoots=-x Deserialize.reflect.xml
    ...
    set ExePath=%LinkBin%\Deserialize.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Deserialize.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Deserialize.asm.exe %CLRTestExecutionArguments% %Host_Args%
