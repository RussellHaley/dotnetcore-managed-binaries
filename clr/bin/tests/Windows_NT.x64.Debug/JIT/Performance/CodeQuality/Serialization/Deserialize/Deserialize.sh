...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Serialization/Deserialize Deserialize.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Serialization/Deserialize Deserialize.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Deserialize.dasm.il Deserialize.exe
    "$CORE_ROOT/ildasm" -raweh -out=Deserialize.dasm.il Deserialize.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Deserialize.asm.exe  Deserialize.dasm.il
    "$CORE_ROOT/ilasm" -output=Deserialize.asm.exe  Deserialize.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Deserialize
    if [ ! -f Deserialize.org ]; then
        TakeLock
        if [ ! -f Deserialize.org ]; then
          mkdir IL
          cp Deserialize.exe IL/Deserialize.exe
          mv Deserialize.exe Deserialize.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Deserialize.org /out Deserialize.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Deserialize.org /out Deserialize.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Deserialize.reflect.xml
  if [ -f Deserialize.reflect.xml ]
then
    ReflectionRoots="-x Deserialize.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Deserialize.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Deserialize.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Deserialize.asm.exe $CLRTestExecutionArguments
