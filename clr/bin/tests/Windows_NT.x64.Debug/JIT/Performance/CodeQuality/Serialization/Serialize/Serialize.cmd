...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Serialization\Serialize Serialize.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Serialization\Serialize Serialize.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Serialize.dasm.il Serialize.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Serialize.dasm.il Serialize.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Serialize.asm.exe  Serialize.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Serialize.asm.exe  Serialize.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Serialize
    if not exist "Serialize.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Serialize.org" (
            mkdir IL
            copy Serialize.exe IL\Serialize.exe 
            ren Serialize.exe Serialize.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Serialize.org /out Serialize.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Serialize.org /out Serialize.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Serialize.reflect.xml
    IF EXIST Serialize.reflect.xml set ReflectionRoots=-x Serialize.reflect.xml
    ...
    set ExePath=%LinkBin%\Serialize.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Serialize.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Serialize.asm.exe %CLRTestExecutionArguments% %Host_Args%
