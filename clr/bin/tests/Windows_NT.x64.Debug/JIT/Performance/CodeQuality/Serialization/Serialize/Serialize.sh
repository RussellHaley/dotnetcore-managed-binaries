...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Serialization/Serialize Serialize.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Serialization/Serialize Serialize.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Serialize.dasm.il Serialize.exe
    "$CORE_ROOT/ildasm" -raweh -out=Serialize.dasm.il Serialize.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Serialize.asm.exe  Serialize.dasm.il
    "$CORE_ROOT/ilasm" -output=Serialize.asm.exe  Serialize.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Serialize
    if [ ! -f Serialize.org ]; then
        TakeLock
        if [ ! -f Serialize.org ]; then
          mkdir IL
          cp Serialize.exe IL/Serialize.exe
          mv Serialize.exe Serialize.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Serialize.org /out Serialize.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Serialize.org /out Serialize.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Serialize.reflect.xml
  if [ -f Serialize.reflect.xml ]
then
    ReflectionRoots="-x Serialize.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Serialize.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Serialize.asm.exe $CLRTestExecutionArguments
