...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Linq\Linq Linq.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Linq\Linq Linq.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Linq.dasm.il Linq.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Linq.dasm.il Linq.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Linq.asm.exe  Linq.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Linq.asm.exe  Linq.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Linq
    if not exist "Linq.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Linq.org" (
            mkdir IL
            copy Linq.exe IL\Linq.exe 
            ren Linq.exe Linq.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Linq.org /out Linq.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Linq.org /out Linq.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Linq.reflect.xml
    IF EXIST Linq.reflect.xml set ReflectionRoots=-x Linq.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Linq.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Linq.asm.exe %CLRTestExecutionArguments% %Host_Args%
