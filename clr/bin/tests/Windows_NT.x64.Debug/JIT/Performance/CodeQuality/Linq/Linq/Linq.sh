...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Linq/Linq Linq.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Linq/Linq Linq.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Linq.dasm.il Linq.exe
    "$CORE_ROOT/ildasm" -raweh -out=Linq.dasm.il Linq.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Linq.asm.exe  Linq.dasm.il
    "$CORE_ROOT/ilasm" -output=Linq.asm.exe  Linq.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Linq
    if [ ! -f Linq.org ]; then
        TakeLock
        if [ ! -f Linq.org ]; then
          mkdir IL
          cp Linq.exe IL/Linq.exe
          mv Linq.exe Linq.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Linq.org /out Linq.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Linq.org /out Linq.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Linq.reflect.xml
  if [ -f Linq.reflect.xml ]
then
    ReflectionRoots="-x Linq.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Linq.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Linq.asm.exe $CLRTestExecutionArguments
