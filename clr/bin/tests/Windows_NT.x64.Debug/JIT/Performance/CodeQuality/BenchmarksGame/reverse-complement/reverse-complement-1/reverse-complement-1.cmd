...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\reverse-complement\reverse-complement-1 reverse-complement-1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\reverse-complement\reverse-complement-1 reverse-complement-1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=reverse-complement-1.dasm.il reverse-complement-1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=reverse-complement-1.dasm.il reverse-complement-1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=reverse-complement-1.asm.exe  reverse-complement-1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=reverse-complement-1.asm.exe  reverse-complement-1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=reverse-complement-1
    if not exist "reverse-complement-1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "reverse-complement-1.org" (
            mkdir IL
            copy reverse-complement-1.exe IL\reverse-complement-1.exe 
            ren reverse-complement-1.exe reverse-complement-1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reverse-complement-1.org /out reverse-complement-1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reverse-complement-1.org /out reverse-complement-1.exe
...
set ExePath=reverse-complement-1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in reverse-complement-1.reflect.xml
    IF EXIST reverse-complement-1.reflect.xml set ReflectionRoots=-x reverse-complement-1.reflect.xml
    ...
    set ExePath=%LinkBin%\reverse-complement-1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% reverse-complement-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% reverse-complement-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
