...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\k-nucleotide\k-nucleotide-1 k-nucleotide-1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\k-nucleotide\k-nucleotide-1 k-nucleotide-1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=k-nucleotide-1.dasm.il k-nucleotide-1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=k-nucleotide-1.dasm.il k-nucleotide-1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=k-nucleotide-1.asm.exe  k-nucleotide-1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=k-nucleotide-1.asm.exe  k-nucleotide-1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=k-nucleotide-1
    if not exist "k-nucleotide-1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "k-nucleotide-1.org" (
            mkdir IL
            copy k-nucleotide-1.exe IL\k-nucleotide-1.exe 
            ren k-nucleotide-1.exe k-nucleotide-1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in k-nucleotide-1.org /out k-nucleotide-1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in k-nucleotide-1.org /out k-nucleotide-1.exe
...
set ExePath=k-nucleotide-1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in k-nucleotide-1.reflect.xml
    IF EXIST k-nucleotide-1.reflect.xml set ReflectionRoots=-x k-nucleotide-1.reflect.xml
    ...
    set ExePath=%LinkBin%\k-nucleotide-1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% k-nucleotide-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% k-nucleotide-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
