...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/regex-redux/regex-redux-5 regex-redux-5.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/regex-redux/regex-redux-5 regex-redux-5.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=regex-redux-5.dasm.il regex-redux-5.exe
    "$CORE_ROOT/ildasm" -raweh -out=regex-redux-5.dasm.il regex-redux-5.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=regex-redux-5.asm.exe  regex-redux-5.dasm.il
    "$CORE_ROOT/ilasm" -output=regex-redux-5.asm.exe  regex-redux-5.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=regex-redux-5
    if [ ! -f regex-redux-5.org ]; then
        TakeLock
        if [ ! -f regex-redux-5.org ]; then
          mkdir IL
          cp regex-redux-5.exe IL/regex-redux-5.exe
          mv regex-redux-5.exe regex-redux-5.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in regex-redux-5.org /out regex-redux-5.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in regex-redux-5.org /out regex-redux-5.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in regex-redux-5.reflect.xml
  if [ -f regex-redux-5.reflect.xml ]
then
    ReflectionRoots="-x regex-redux-5.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/regex-redux-5.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" regex-redux-5.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" regex-redux-5.asm.exe $CLRTestExecutionArguments
