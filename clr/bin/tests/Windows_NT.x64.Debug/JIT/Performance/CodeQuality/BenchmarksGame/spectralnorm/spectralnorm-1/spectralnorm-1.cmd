...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\spectralnorm\spectralnorm-1 spectralnorm-1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\spectralnorm\spectralnorm-1 spectralnorm-1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=spectralnorm-1.dasm.il spectralnorm-1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=spectralnorm-1.dasm.il spectralnorm-1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=spectralnorm-1.asm.exe  spectralnorm-1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=spectralnorm-1.asm.exe  spectralnorm-1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=spectralnorm-1
    if not exist "spectralnorm-1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "spectralnorm-1.org" (
            mkdir IL
            copy spectralnorm-1.exe IL\spectralnorm-1.exe 
            ren spectralnorm-1.exe spectralnorm-1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in spectralnorm-1.org /out spectralnorm-1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in spectralnorm-1.org /out spectralnorm-1.exe
...
set ExePath=spectralnorm-1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in spectralnorm-1.reflect.xml
    IF EXIST spectralnorm-1.reflect.xml set ReflectionRoots=-x spectralnorm-1.reflect.xml
    ...
    set ExePath=%LinkBin%\spectralnorm-1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% spectralnorm-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% spectralnorm-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
