...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/k-nucleotide/k-nucleotide-9 k-nucleotide-9.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/k-nucleotide/k-nucleotide-9 k-nucleotide-9.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=k-nucleotide-9.dasm.il k-nucleotide-9.exe
    "$CORE_ROOT/ildasm" -raweh -out=k-nucleotide-9.dasm.il k-nucleotide-9.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=k-nucleotide-9.asm.exe  k-nucleotide-9.dasm.il
    "$CORE_ROOT/ilasm" -output=k-nucleotide-9.asm.exe  k-nucleotide-9.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=k-nucleotide-9
    if [ ! -f k-nucleotide-9.org ]; then
        TakeLock
        if [ ! -f k-nucleotide-9.org ]; then
          mkdir IL
          cp k-nucleotide-9.exe IL/k-nucleotide-9.exe
          mv k-nucleotide-9.exe k-nucleotide-9.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in k-nucleotide-9.org /out k-nucleotide-9.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in k-nucleotide-9.org /out k-nucleotide-9.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in k-nucleotide-9.reflect.xml
  if [ -f k-nucleotide-9.reflect.xml ]
then
    ReflectionRoots="-x k-nucleotide-9.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/k-nucleotide-9.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" k-nucleotide-9.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" k-nucleotide-9.asm.exe $CLRTestExecutionArguments
