...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\pidigits\pidigits-3 pidigits-3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\pidigits\pidigits-3 pidigits-3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=pidigits-3.dasm.il pidigits-3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=pidigits-3.dasm.il pidigits-3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=pidigits-3.asm.exe  pidigits-3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=pidigits-3.asm.exe  pidigits-3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=pidigits-3
    if not exist "pidigits-3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "pidigits-3.org" (
            mkdir IL
            copy pidigits-3.exe IL\pidigits-3.exe 
            ren pidigits-3.exe pidigits-3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in pidigits-3.org /out pidigits-3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in pidigits-3.org /out pidigits-3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in pidigits-3.reflect.xml
    IF EXIST pidigits-3.reflect.xml set ReflectionRoots=-x pidigits-3.reflect.xml
    ...
    set ExePath=%LinkBin%\pidigits-3.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% pidigits-3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% pidigits-3.asm.exe %CLRTestExecutionArguments% %Host_Args%
