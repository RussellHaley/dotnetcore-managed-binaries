...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\binarytrees\binarytrees-2 binarytrees-2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\binarytrees\binarytrees-2 binarytrees-2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=binarytrees-2.dasm.il binarytrees-2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=binarytrees-2.dasm.il binarytrees-2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=binarytrees-2.asm.exe  binarytrees-2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=binarytrees-2.asm.exe  binarytrees-2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=binarytrees-2
    if not exist "binarytrees-2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "binarytrees-2.org" (
            mkdir IL
            copy binarytrees-2.exe IL\binarytrees-2.exe 
            ren binarytrees-2.exe binarytrees-2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in binarytrees-2.org /out binarytrees-2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in binarytrees-2.org /out binarytrees-2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in binarytrees-2.reflect.xml
    IF EXIST binarytrees-2.reflect.xml set ReflectionRoots=-x binarytrees-2.reflect.xml
    ...
    set ExePath=%LinkBin%\binarytrees-2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% binarytrees-2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% binarytrees-2.asm.exe %CLRTestExecutionArguments% %Host_Args%
