...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/n-body/n-body-3 n-body-3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/n-body/n-body-3 n-body-3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=n-body-3.dasm.il n-body-3.exe
    "$CORE_ROOT/ildasm" -raweh -out=n-body-3.dasm.il n-body-3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=n-body-3.asm.exe  n-body-3.dasm.il
    "$CORE_ROOT/ilasm" -output=n-body-3.asm.exe  n-body-3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=n-body-3
    if [ ! -f n-body-3.org ]; then
        TakeLock
        if [ ! -f n-body-3.org ]; then
          mkdir IL
          cp n-body-3.exe IL/n-body-3.exe
          mv n-body-3.exe n-body-3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in n-body-3.org /out n-body-3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in n-body-3.org /out n-body-3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in n-body-3.reflect.xml
  if [ -f n-body-3.reflect.xml ]
then
    ReflectionRoots="-x n-body-3.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" n-body-3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" n-body-3.asm.exe $CLRTestExecutionArguments
