...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\fannkuch-redux\fannkuch-redux-2 fannkuch-redux-2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\fannkuch-redux\fannkuch-redux-2 fannkuch-redux-2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=fannkuch-redux-2.dasm.il fannkuch-redux-2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=fannkuch-redux-2.dasm.il fannkuch-redux-2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=fannkuch-redux-2.asm.exe  fannkuch-redux-2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=fannkuch-redux-2.asm.exe  fannkuch-redux-2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=fannkuch-redux-2
    if not exist "fannkuch-redux-2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "fannkuch-redux-2.org" (
            mkdir IL
            copy fannkuch-redux-2.exe IL\fannkuch-redux-2.exe 
            ren fannkuch-redux-2.exe fannkuch-redux-2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fannkuch-redux-2.org /out fannkuch-redux-2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fannkuch-redux-2.org /out fannkuch-redux-2.exe
...
set ExePath=fannkuch-redux-2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in fannkuch-redux-2.reflect.xml
    IF EXIST fannkuch-redux-2.reflect.xml set ReflectionRoots=-x fannkuch-redux-2.reflect.xml
    ...
    set ExePath=%LinkBin%\fannkuch-redux-2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% fannkuch-redux-2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% fannkuch-redux-2.asm.exe %CLRTestExecutionArguments% %Host_Args%
