...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/mandelbrot/mandelbrot-2 mandelbrot-2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/mandelbrot/mandelbrot-2 mandelbrot-2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mandelbrot-2.dasm.il mandelbrot-2.exe
    "$CORE_ROOT/ildasm" -raweh -out=mandelbrot-2.dasm.il mandelbrot-2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mandelbrot-2.asm.exe  mandelbrot-2.dasm.il
    "$CORE_ROOT/ilasm" -output=mandelbrot-2.asm.exe  mandelbrot-2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mandelbrot-2
    if [ ! -f mandelbrot-2.org ]; then
        TakeLock
        if [ ! -f mandelbrot-2.org ]; then
          mkdir IL
          cp mandelbrot-2.exe IL/mandelbrot-2.exe
          mv mandelbrot-2.exe mandelbrot-2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mandelbrot-2.org /out mandelbrot-2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mandelbrot-2.org /out mandelbrot-2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mandelbrot-2.reflect.xml
  if [ -f mandelbrot-2.reflect.xml ]
then
    ReflectionRoots="-x mandelbrot-2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/mandelbrot-2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mandelbrot-2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mandelbrot-2.asm.exe $CLRTestExecutionArguments
