...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/fannkuch-redux/fannkuch-redux-5 fannkuch-redux-5.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/fannkuch-redux/fannkuch-redux-5 fannkuch-redux-5.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=fannkuch-redux-5.dasm.il fannkuch-redux-5.exe
    "$CORE_ROOT/ildasm" -raweh -out=fannkuch-redux-5.dasm.il fannkuch-redux-5.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=fannkuch-redux-5.asm.exe  fannkuch-redux-5.dasm.il
    "$CORE_ROOT/ilasm" -output=fannkuch-redux-5.asm.exe  fannkuch-redux-5.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=fannkuch-redux-5
    if [ ! -f fannkuch-redux-5.org ]; then
        TakeLock
        if [ ! -f fannkuch-redux-5.org ]; then
          mkdir IL
          cp fannkuch-redux-5.exe IL/fannkuch-redux-5.exe
          mv fannkuch-redux-5.exe fannkuch-redux-5.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fannkuch-redux-5.org /out fannkuch-redux-5.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fannkuch-redux-5.org /out fannkuch-redux-5.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in fannkuch-redux-5.reflect.xml
  if [ -f fannkuch-redux-5.reflect.xml ]
then
    ReflectionRoots="-x fannkuch-redux-5.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/fannkuch-redux-5.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" fannkuch-redux-5.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" fannkuch-redux-5.asm.exe $CLRTestExecutionArguments
