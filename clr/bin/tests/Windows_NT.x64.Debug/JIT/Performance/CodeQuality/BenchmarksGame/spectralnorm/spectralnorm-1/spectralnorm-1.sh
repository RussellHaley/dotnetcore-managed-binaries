...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/spectralnorm/spectralnorm-1 spectralnorm-1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/spectralnorm/spectralnorm-1 spectralnorm-1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=spectralnorm-1.dasm.il spectralnorm-1.exe
    "$CORE_ROOT/ildasm" -raweh -out=spectralnorm-1.dasm.il spectralnorm-1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=spectralnorm-1.asm.exe  spectralnorm-1.dasm.il
    "$CORE_ROOT/ilasm" -output=spectralnorm-1.asm.exe  spectralnorm-1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=spectralnorm-1
    if [ ! -f spectralnorm-1.org ]; then
        TakeLock
        if [ ! -f spectralnorm-1.org ]; then
          mkdir IL
          cp spectralnorm-1.exe IL/spectralnorm-1.exe
          mv spectralnorm-1.exe spectralnorm-1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in spectralnorm-1.org /out spectralnorm-1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in spectralnorm-1.org /out spectralnorm-1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in spectralnorm-1.reflect.xml
  if [ -f spectralnorm-1.reflect.xml ]
then
    ReflectionRoots="-x spectralnorm-1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/spectralnorm-1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" spectralnorm-1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" spectralnorm-1.asm.exe $CLRTestExecutionArguments
