...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/regex-redux/regex-redux-1 regex-redux-1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/regex-redux/regex-redux-1 regex-redux-1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=regex-redux-1.dasm.il regex-redux-1.exe
    "$CORE_ROOT/ildasm" -raweh -out=regex-redux-1.dasm.il regex-redux-1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=regex-redux-1.asm.exe  regex-redux-1.dasm.il
    "$CORE_ROOT/ilasm" -output=regex-redux-1.asm.exe  regex-redux-1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=regex-redux-1
    if [ ! -f regex-redux-1.org ]; then
        TakeLock
        if [ ! -f regex-redux-1.org ]; then
          mkdir IL
          cp regex-redux-1.exe IL/regex-redux-1.exe
          mv regex-redux-1.exe regex-redux-1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in regex-redux-1.org /out regex-redux-1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in regex-redux-1.org /out regex-redux-1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in regex-redux-1.reflect.xml
  if [ -f regex-redux-1.reflect.xml ]
then
    ReflectionRoots="-x regex-redux-1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/regex-redux-1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" regex-redux-1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" regex-redux-1.asm.exe $CLRTestExecutionArguments
