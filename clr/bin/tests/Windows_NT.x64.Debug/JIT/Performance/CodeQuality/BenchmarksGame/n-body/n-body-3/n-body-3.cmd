...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\n-body\n-body-3 n-body-3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\n-body\n-body-3 n-body-3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=n-body-3.dasm.il n-body-3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=n-body-3.dasm.il n-body-3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=n-body-3.asm.exe  n-body-3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=n-body-3.asm.exe  n-body-3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=n-body-3
    if not exist "n-body-3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "n-body-3.org" (
            mkdir IL
            copy n-body-3.exe IL\n-body-3.exe 
            ren n-body-3.exe n-body-3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in n-body-3.org /out n-body-3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in n-body-3.org /out n-body-3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in n-body-3.reflect.xml
    IF EXIST n-body-3.reflect.xml set ReflectionRoots=-x n-body-3.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% n-body-3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% n-body-3.asm.exe %CLRTestExecutionArguments% %Host_Args%
