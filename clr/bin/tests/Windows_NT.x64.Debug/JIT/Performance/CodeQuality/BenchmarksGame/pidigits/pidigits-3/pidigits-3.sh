...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/pidigits/pidigits-3 pidigits-3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/pidigits/pidigits-3 pidigits-3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=pidigits-3.dasm.il pidigits-3.exe
    "$CORE_ROOT/ildasm" -raweh -out=pidigits-3.dasm.il pidigits-3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=pidigits-3.asm.exe  pidigits-3.dasm.il
    "$CORE_ROOT/ilasm" -output=pidigits-3.asm.exe  pidigits-3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=pidigits-3
    if [ ! -f pidigits-3.org ]; then
        TakeLock
        if [ ! -f pidigits-3.org ]; then
          mkdir IL
          cp pidigits-3.exe IL/pidigits-3.exe
          mv pidigits-3.exe pidigits-3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in pidigits-3.org /out pidigits-3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in pidigits-3.org /out pidigits-3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in pidigits-3.reflect.xml
  if [ -f pidigits-3.reflect.xml ]
then
    ReflectionRoots="-x pidigits-3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/pidigits-3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" pidigits-3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" pidigits-3.asm.exe $CLRTestExecutionArguments
