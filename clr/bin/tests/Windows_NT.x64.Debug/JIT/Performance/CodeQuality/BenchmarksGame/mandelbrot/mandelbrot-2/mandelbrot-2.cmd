...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\mandelbrot\mandelbrot-2 mandelbrot-2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\mandelbrot\mandelbrot-2 mandelbrot-2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mandelbrot-2.dasm.il mandelbrot-2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mandelbrot-2.dasm.il mandelbrot-2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mandelbrot-2.asm.exe  mandelbrot-2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mandelbrot-2.asm.exe  mandelbrot-2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mandelbrot-2
    if not exist "mandelbrot-2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mandelbrot-2.org" (
            mkdir IL
            copy mandelbrot-2.exe IL\mandelbrot-2.exe 
            ren mandelbrot-2.exe mandelbrot-2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mandelbrot-2.org /out mandelbrot-2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mandelbrot-2.org /out mandelbrot-2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mandelbrot-2.reflect.xml
    IF EXIST mandelbrot-2.reflect.xml set ReflectionRoots=-x mandelbrot-2.reflect.xml
    ...
    set ExePath=%LinkBin%\mandelbrot-2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mandelbrot-2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mandelbrot-2.asm.exe %CLRTestExecutionArguments% %Host_Args%
