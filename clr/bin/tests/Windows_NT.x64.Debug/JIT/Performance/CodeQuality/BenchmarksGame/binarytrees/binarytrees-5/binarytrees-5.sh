...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/binarytrees/binarytrees-5 binarytrees-5.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/binarytrees/binarytrees-5 binarytrees-5.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=binarytrees-5.dasm.il binarytrees-5.exe
    "$CORE_ROOT/ildasm" -raweh -out=binarytrees-5.dasm.il binarytrees-5.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=binarytrees-5.asm.exe  binarytrees-5.dasm.il
    "$CORE_ROOT/ilasm" -output=binarytrees-5.asm.exe  binarytrees-5.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=binarytrees-5
    if [ ! -f binarytrees-5.org ]; then
        TakeLock
        if [ ! -f binarytrees-5.org ]; then
          mkdir IL
          cp binarytrees-5.exe IL/binarytrees-5.exe
          mv binarytrees-5.exe binarytrees-5.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in binarytrees-5.org /out binarytrees-5.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in binarytrees-5.org /out binarytrees-5.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in binarytrees-5.reflect.xml
  if [ -f binarytrees-5.reflect.xml ]
then
    ReflectionRoots="-x binarytrees-5.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/binarytrees-5.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" binarytrees-5.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" binarytrees-5.asm.exe $CLRTestExecutionArguments
