...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\regex-redux\regex-redux-1 regex-redux-1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\regex-redux\regex-redux-1 regex-redux-1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=regex-redux-1.dasm.il regex-redux-1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=regex-redux-1.dasm.il regex-redux-1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=regex-redux-1.asm.exe  regex-redux-1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=regex-redux-1.asm.exe  regex-redux-1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=regex-redux-1
    if not exist "regex-redux-1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "regex-redux-1.org" (
            mkdir IL
            copy regex-redux-1.exe IL\regex-redux-1.exe 
            ren regex-redux-1.exe regex-redux-1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in regex-redux-1.org /out regex-redux-1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in regex-redux-1.org /out regex-redux-1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in regex-redux-1.reflect.xml
    IF EXIST regex-redux-1.reflect.xml set ReflectionRoots=-x regex-redux-1.reflect.xml
    ...
    set ExePath=%LinkBin%\regex-redux-1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% regex-redux-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% regex-redux-1.asm.exe %CLRTestExecutionArguments% %Host_Args%
