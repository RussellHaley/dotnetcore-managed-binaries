...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/mandelbrot/mandelbrot-4 mandelbrot-4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/mandelbrot/mandelbrot-4 mandelbrot-4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mandelbrot-4.dasm.il mandelbrot-4.exe
    "$CORE_ROOT/ildasm" -raweh -out=mandelbrot-4.dasm.il mandelbrot-4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mandelbrot-4.asm.exe  mandelbrot-4.dasm.il
    "$CORE_ROOT/ilasm" -output=mandelbrot-4.asm.exe  mandelbrot-4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mandelbrot-4
    if [ ! -f mandelbrot-4.org ]; then
        TakeLock
        if [ ! -f mandelbrot-4.org ]; then
          mkdir IL
          cp mandelbrot-4.exe IL/mandelbrot-4.exe
          mv mandelbrot-4.exe mandelbrot-4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mandelbrot-4.org /out mandelbrot-4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mandelbrot-4.org /out mandelbrot-4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mandelbrot-4.reflect.xml
  if [ -f mandelbrot-4.reflect.xml ]
then
    ReflectionRoots="-x mandelbrot-4.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/mandelbrot-4.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mandelbrot-4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mandelbrot-4.asm.exe $CLRTestExecutionArguments
