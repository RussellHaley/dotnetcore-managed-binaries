...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/reverse-complement/reverse-complement-6 reverse-complement-6.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/reverse-complement/reverse-complement-6 reverse-complement-6.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=reverse-complement-6.dasm.il reverse-complement-6.exe
    "$CORE_ROOT/ildasm" -raweh -out=reverse-complement-6.dasm.il reverse-complement-6.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=reverse-complement-6.asm.exe  reverse-complement-6.dasm.il
    "$CORE_ROOT/ilasm" -output=reverse-complement-6.asm.exe  reverse-complement-6.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=reverse-complement-6
    if [ ! -f reverse-complement-6.org ]; then
        TakeLock
        if [ ! -f reverse-complement-6.org ]; then
          mkdir IL
          cp reverse-complement-6.exe IL/reverse-complement-6.exe
          mv reverse-complement-6.exe reverse-complement-6.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reverse-complement-6.org /out reverse-complement-6.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reverse-complement-6.org /out reverse-complement-6.exe
          __cgExitCode=$?
...
ExePath=reverse-complement-6.exe
...
then
  ...
  # Use hints for reflection roots, if provided in reverse-complement-6.reflect.xml
  if [ -f reverse-complement-6.reflect.xml ]
then
    ReflectionRoots="-x reverse-complement-6.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/reverse-complement-6.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" reverse-complement-6.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" reverse-complement-6.asm.exe $CLRTestExecutionArguments
