...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\fannkuch-redux\fannkuch-redux-5 fannkuch-redux-5.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchmarksGame\fannkuch-redux\fannkuch-redux-5 fannkuch-redux-5.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=fannkuch-redux-5.dasm.il fannkuch-redux-5.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=fannkuch-redux-5.dasm.il fannkuch-redux-5.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=fannkuch-redux-5.asm.exe  fannkuch-redux-5.dasm.il
    %CORE_ROOT%\ilasm.exe /output=fannkuch-redux-5.asm.exe  fannkuch-redux-5.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=fannkuch-redux-5
    if not exist "fannkuch-redux-5.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "fannkuch-redux-5.org" (
            mkdir IL
            copy fannkuch-redux-5.exe IL\fannkuch-redux-5.exe 
            ren fannkuch-redux-5.exe fannkuch-redux-5.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fannkuch-redux-5.org /out fannkuch-redux-5.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fannkuch-redux-5.org /out fannkuch-redux-5.exe
...
set ExePath=fannkuch-redux-5.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in fannkuch-redux-5.reflect.xml
    IF EXIST fannkuch-redux-5.reflect.xml set ReflectionRoots=-x fannkuch-redux-5.reflect.xml
    ...
    set ExePath=%LinkBin%\fannkuch-redux-5.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% fannkuch-redux-5.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% fannkuch-redux-5.asm.exe %CLRTestExecutionArguments% %Host_Args%
