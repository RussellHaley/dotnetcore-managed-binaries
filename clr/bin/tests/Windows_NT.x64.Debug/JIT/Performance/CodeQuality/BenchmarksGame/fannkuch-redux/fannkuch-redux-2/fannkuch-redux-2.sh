...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/fannkuch-redux/fannkuch-redux-2 fannkuch-redux-2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/fannkuch-redux/fannkuch-redux-2 fannkuch-redux-2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=fannkuch-redux-2.dasm.il fannkuch-redux-2.exe
    "$CORE_ROOT/ildasm" -raweh -out=fannkuch-redux-2.dasm.il fannkuch-redux-2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=fannkuch-redux-2.asm.exe  fannkuch-redux-2.dasm.il
    "$CORE_ROOT/ilasm" -output=fannkuch-redux-2.asm.exe  fannkuch-redux-2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=fannkuch-redux-2
    if [ ! -f fannkuch-redux-2.org ]; then
        TakeLock
        if [ ! -f fannkuch-redux-2.org ]; then
          mkdir IL
          cp fannkuch-redux-2.exe IL/fannkuch-redux-2.exe
          mv fannkuch-redux-2.exe fannkuch-redux-2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fannkuch-redux-2.org /out fannkuch-redux-2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fannkuch-redux-2.org /out fannkuch-redux-2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in fannkuch-redux-2.reflect.xml
  if [ -f fannkuch-redux-2.reflect.xml ]
then
    ReflectionRoots="-x fannkuch-redux-2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/fannkuch-redux-2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" fannkuch-redux-2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" fannkuch-redux-2.asm.exe $CLRTestExecutionArguments
