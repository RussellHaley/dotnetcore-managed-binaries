...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/binarytrees/binarytrees-2 binarytrees-2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/binarytrees/binarytrees-2 binarytrees-2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=binarytrees-2.dasm.il binarytrees-2.exe
    "$CORE_ROOT/ildasm" -raweh -out=binarytrees-2.dasm.il binarytrees-2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=binarytrees-2.asm.exe  binarytrees-2.dasm.il
    "$CORE_ROOT/ilasm" -output=binarytrees-2.asm.exe  binarytrees-2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=binarytrees-2
    if [ ! -f binarytrees-2.org ]; then
        TakeLock
        if [ ! -f binarytrees-2.org ]; then
          mkdir IL
          cp binarytrees-2.exe IL/binarytrees-2.exe
          mv binarytrees-2.exe binarytrees-2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in binarytrees-2.org /out binarytrees-2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in binarytrees-2.org /out binarytrees-2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in binarytrees-2.reflect.xml
  if [ -f binarytrees-2.reflect.xml ]
then
    ReflectionRoots="-x binarytrees-2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/binarytrees-2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" binarytrees-2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" binarytrees-2.asm.exe $CLRTestExecutionArguments
