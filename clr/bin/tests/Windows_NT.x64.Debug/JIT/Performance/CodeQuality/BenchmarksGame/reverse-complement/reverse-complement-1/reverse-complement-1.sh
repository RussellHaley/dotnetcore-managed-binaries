...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/reverse-complement/reverse-complement-1 reverse-complement-1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/reverse-complement/reverse-complement-1 reverse-complement-1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=reverse-complement-1.dasm.il reverse-complement-1.exe
    "$CORE_ROOT/ildasm" -raweh -out=reverse-complement-1.dasm.il reverse-complement-1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=reverse-complement-1.asm.exe  reverse-complement-1.dasm.il
    "$CORE_ROOT/ilasm" -output=reverse-complement-1.asm.exe  reverse-complement-1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=reverse-complement-1
    if [ ! -f reverse-complement-1.org ]; then
        TakeLock
        if [ ! -f reverse-complement-1.org ]; then
          mkdir IL
          cp reverse-complement-1.exe IL/reverse-complement-1.exe
          mv reverse-complement-1.exe reverse-complement-1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reverse-complement-1.org /out reverse-complement-1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reverse-complement-1.org /out reverse-complement-1.exe
          __cgExitCode=$?
...
ExePath=reverse-complement-1.exe
...
then
  ...
  # Use hints for reflection roots, if provided in reverse-complement-1.reflect.xml
  if [ -f reverse-complement-1.reflect.xml ]
then
    ReflectionRoots="-x reverse-complement-1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/reverse-complement-1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" reverse-complement-1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" reverse-complement-1.asm.exe $CLRTestExecutionArguments
