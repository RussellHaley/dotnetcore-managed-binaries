...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/spectralnorm/spectralnorm-3 spectralnorm-3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchmarksGame/spectralnorm/spectralnorm-3 spectralnorm-3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=spectralnorm-3.dasm.il spectralnorm-3.exe
    "$CORE_ROOT/ildasm" -raweh -out=spectralnorm-3.dasm.il spectralnorm-3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=spectralnorm-3.asm.exe  spectralnorm-3.dasm.il
    "$CORE_ROOT/ilasm" -output=spectralnorm-3.asm.exe  spectralnorm-3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=spectralnorm-3
    if [ ! -f spectralnorm-3.org ]; then
        TakeLock
        if [ ! -f spectralnorm-3.org ]; then
          mkdir IL
          cp spectralnorm-3.exe IL/spectralnorm-3.exe
          mv spectralnorm-3.exe spectralnorm-3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in spectralnorm-3.org /out spectralnorm-3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in spectralnorm-3.org /out spectralnorm-3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in spectralnorm-3.reflect.xml
  if [ -f spectralnorm-3.reflect.xml ]
then
    ReflectionRoots="-x spectralnorm-3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/spectralnorm-3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" spectralnorm-3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" spectralnorm-3.asm.exe $CLRTestExecutionArguments
