...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Bytemark\Bytemark Bytemark.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Bytemark\Bytemark Bytemark.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Bytemark.dasm.il Bytemark.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Bytemark.dasm.il Bytemark.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Bytemark.asm.exe  Bytemark.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Bytemark.asm.exe  Bytemark.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Bytemark
    if not exist "Bytemark.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Bytemark.org" (
            mkdir IL
            copy Bytemark.exe IL\Bytemark.exe 
            ren Bytemark.exe Bytemark.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Bytemark.org /out Bytemark.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Bytemark.org /out Bytemark.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Bytemark.reflect.xml
    IF EXIST Bytemark.reflect.xml set ReflectionRoots=-x Bytemark.reflect.xml
    ...
    set ExePath=%LinkBin%\Bytemark.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Bytemark.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Bytemark.asm.exe %CLRTestExecutionArguments% %Host_Args%
