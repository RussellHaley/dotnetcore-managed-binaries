...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Bytemark/Bytemark Bytemark.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Bytemark/Bytemark Bytemark.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Bytemark.dasm.il Bytemark.exe
    "$CORE_ROOT/ildasm" -raweh -out=Bytemark.dasm.il Bytemark.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Bytemark.asm.exe  Bytemark.dasm.il
    "$CORE_ROOT/ilasm" -output=Bytemark.asm.exe  Bytemark.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Bytemark
    if [ ! -f Bytemark.org ]; then
        TakeLock
        if [ ! -f Bytemark.org ]; then
          mkdir IL
          cp Bytemark.exe IL/Bytemark.exe
          mv Bytemark.exe Bytemark.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Bytemark.org /out Bytemark.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Bytemark.org /out Bytemark.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Bytemark.reflect.xml
  if [ -f Bytemark.reflect.xml ]
then
    ReflectionRoots="-x Bytemark.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Bytemark.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Bytemark.asm.exe $CLRTestExecutionArguments
