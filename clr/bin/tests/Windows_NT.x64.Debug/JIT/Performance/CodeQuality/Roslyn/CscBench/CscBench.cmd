...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Roslyn\CscBench CscBench.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Roslyn\CscBench CscBench.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CscBench.dasm.il CscBench.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CscBench.dasm.il CscBench.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CscBench.asm.exe  CscBench.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CscBench.asm.exe  CscBench.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CscBench
    if not exist "CscBench.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CscBench.org" (
            mkdir IL
            copy CscBench.exe IL\CscBench.exe 
            ren CscBench.exe CscBench.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CscBench.org /out CscBench.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CscBench.org /out CscBench.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CscBench.reflect.xml
    IF EXIST CscBench.reflect.xml set ReflectionRoots=-x CscBench.reflect.xml
    ...
    set ExePath=%LinkBin%\CscBench.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CscBench.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CscBench.asm.exe %CLRTestExecutionArguments% %Host_Args%
