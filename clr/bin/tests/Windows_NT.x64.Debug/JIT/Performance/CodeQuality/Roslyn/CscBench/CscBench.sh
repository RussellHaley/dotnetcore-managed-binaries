...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Roslyn/CscBench CscBench.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Roslyn/CscBench CscBench.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CscBench.dasm.il CscBench.exe
    "$CORE_ROOT/ildasm" -raweh -out=CscBench.dasm.il CscBench.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CscBench.asm.exe  CscBench.dasm.il
    "$CORE_ROOT/ilasm" -output=CscBench.asm.exe  CscBench.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CscBench
    if [ ! -f CscBench.org ]; then
        TakeLock
        if [ ! -f CscBench.org ]; then
          mkdir IL
          cp CscBench.exe IL/CscBench.exe
          mv CscBench.exe CscBench.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CscBench.org /out CscBench.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CscBench.org /out CscBench.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CscBench.reflect.xml
  if [ -f CscBench.reflect.xml ]
then
    ReflectionRoots="-x CscBench.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CscBench.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CscBench.asm.exe $CLRTestExecutionArguments
