...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Span\SpanBench SpanBench.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Span\SpanBench SpanBench.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=SpanBench.dasm.il SpanBench.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=SpanBench.dasm.il SpanBench.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=SpanBench.asm.exe  SpanBench.dasm.il
    %CORE_ROOT%\ilasm.exe /output=SpanBench.asm.exe  SpanBench.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=SpanBench
    if not exist "SpanBench.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "SpanBench.org" (
            mkdir IL
            copy SpanBench.exe IL\SpanBench.exe 
            ren SpanBench.exe SpanBench.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SpanBench.org /out SpanBench.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SpanBench.org /out SpanBench.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in SpanBench.reflect.xml
    IF EXIST SpanBench.reflect.xml set ReflectionRoots=-x SpanBench.reflect.xml
    ...
    set ExePath=%LinkBin%\SpanBench.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% SpanBench.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% SpanBench.asm.exe %CLRTestExecutionArguments% %Host_Args%
