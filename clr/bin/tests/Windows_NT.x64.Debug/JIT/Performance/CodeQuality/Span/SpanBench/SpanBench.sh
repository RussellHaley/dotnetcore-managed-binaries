...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Span/SpanBench SpanBench.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Span/SpanBench SpanBench.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=SpanBench.dasm.il SpanBench.exe
    "$CORE_ROOT/ildasm" -raweh -out=SpanBench.dasm.il SpanBench.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=SpanBench.asm.exe  SpanBench.dasm.il
    "$CORE_ROOT/ilasm" -output=SpanBench.asm.exe  SpanBench.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=SpanBench
    if [ ! -f SpanBench.org ]; then
        TakeLock
        if [ ! -f SpanBench.org ]; then
          mkdir IL
          cp SpanBench.exe IL/SpanBench.exe
          mv SpanBench.exe SpanBench.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SpanBench.org /out SpanBench.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SpanBench.org /out SpanBench.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in SpanBench.reflect.xml
  if [ -f SpanBench.reflect.xml ]
then
    ReflectionRoots="-x SpanBench.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" SpanBench.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" SpanBench.asm.exe $CLRTestExecutionArguments
