...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Span/Indexer Indexer.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Span/Indexer Indexer.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Indexer.dasm.il Indexer.exe
    "$CORE_ROOT/ildasm" -raweh -out=Indexer.dasm.il Indexer.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Indexer.asm.exe  Indexer.dasm.il
    "$CORE_ROOT/ilasm" -output=Indexer.asm.exe  Indexer.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Indexer
    if [ ! -f Indexer.org ]; then
        TakeLock
        if [ ! -f Indexer.org ]; then
          mkdir IL
          cp Indexer.exe IL/Indexer.exe
          mv Indexer.exe Indexer.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Indexer.org /out Indexer.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Indexer.org /out Indexer.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Indexer.reflect.xml
  if [ -f Indexer.reflect.xml ]
then
    ReflectionRoots="-x Indexer.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Indexer.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Indexer.asm.exe $CLRTestExecutionArguments
