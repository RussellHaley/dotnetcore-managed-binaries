...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Span\Indexer Indexer.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Span\Indexer Indexer.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Indexer.dasm.il Indexer.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Indexer.dasm.il Indexer.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Indexer.asm.exe  Indexer.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Indexer.asm.exe  Indexer.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Indexer
    if not exist "Indexer.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Indexer.org" (
            mkdir IL
            copy Indexer.exe IL\Indexer.exe 
            ren Indexer.exe Indexer.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Indexer.org /out Indexer.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Indexer.org /out Indexer.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Indexer.reflect.xml
    IF EXIST Indexer.reflect.xml set ReflectionRoots=-x Indexer.reflect.xml
    ...
    set ExePath=%LinkBin%\Indexer.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Indexer.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Indexer.asm.exe %CLRTestExecutionArguments% %Host_Args%
