...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/Math/Functions/Functions Functions.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/Math/Functions/Functions Functions.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Functions.dasm.il Functions.exe
    "$CORE_ROOT/ildasm" -raweh -out=Functions.dasm.il Functions.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Functions.asm.exe  Functions.dasm.il
    "$CORE_ROOT/ilasm" -output=Functions.asm.exe  Functions.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Functions
    if [ ! -f Functions.org ]; then
        TakeLock
        if [ ! -f Functions.org ]; then
          mkdir IL
          cp Functions.exe IL/Functions.exe
          mv Functions.exe Functions.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Functions.org /out Functions.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Functions.org /out Functions.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Functions.reflect.xml
  if [ -f Functions.reflect.xml ]
then
    ReflectionRoots="-x Functions.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Functions.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Functions.asm.exe $CLRTestExecutionArguments
