...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Math\Functions\Functions Functions.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Math\Functions\Functions Functions.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Functions.dasm.il Functions.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Functions.dasm.il Functions.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Functions.asm.exe  Functions.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Functions.asm.exe  Functions.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Functions
    if not exist "Functions.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Functions.org" (
            mkdir IL
            copy Functions.exe IL\Functions.exe 
            ren Functions.exe Functions.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Functions.org /out Functions.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Functions.org /out Functions.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Functions.reflect.xml
    IF EXIST Functions.reflect.xml set ReflectionRoots=-x Functions.reflect.xml
    ...
    set ExePath=%LinkBin%\Functions.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Functions.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Functions.asm.exe %CLRTestExecutionArguments% %Host_Args%
