...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Layout/SearchLoops SearchLoops.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Layout/SearchLoops SearchLoops.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=SearchLoops.dasm.il SearchLoops.exe
    "$CORE_ROOT/ildasm" -raweh -out=SearchLoops.dasm.il SearchLoops.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=SearchLoops.asm.exe  SearchLoops.dasm.il
    "$CORE_ROOT/ilasm" -output=SearchLoops.asm.exe  SearchLoops.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=SearchLoops
    if [ ! -f SearchLoops.org ]; then
        TakeLock
        if [ ! -f SearchLoops.org ]; then
          mkdir IL
          cp SearchLoops.exe IL/SearchLoops.exe
          mv SearchLoops.exe SearchLoops.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SearchLoops.org /out SearchLoops.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SearchLoops.org /out SearchLoops.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in SearchLoops.reflect.xml
  if [ -f SearchLoops.reflect.xml ]
then
    ReflectionRoots="-x SearchLoops.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/SearchLoops.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" SearchLoops.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" SearchLoops.asm.exe $CLRTestExecutionArguments
