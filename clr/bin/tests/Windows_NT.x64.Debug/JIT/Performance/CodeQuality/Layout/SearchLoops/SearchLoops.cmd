...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Layout\SearchLoops SearchLoops.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Layout\SearchLoops SearchLoops.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=SearchLoops.dasm.il SearchLoops.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=SearchLoops.dasm.il SearchLoops.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=SearchLoops.asm.exe  SearchLoops.dasm.il
    %CORE_ROOT%\ilasm.exe /output=SearchLoops.asm.exe  SearchLoops.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=SearchLoops
    if not exist "SearchLoops.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "SearchLoops.org" (
            mkdir IL
            copy SearchLoops.exe IL\SearchLoops.exe 
            ren SearchLoops.exe SearchLoops.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SearchLoops.org /out SearchLoops.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SearchLoops.org /out SearchLoops.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in SearchLoops.reflect.xml
    IF EXIST SearchLoops.reflect.xml set ReflectionRoots=-x SearchLoops.reflect.xml
    ...
    set ExePath=%LinkBin%\SearchLoops.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% SearchLoops.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% SearchLoops.asm.exe %CLRTestExecutionArguments% %Host_Args%
