...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\V8\Crypto\Crypto Crypto.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\V8\Crypto\Crypto Crypto.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Crypto.dasm.il Crypto.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Crypto.dasm.il Crypto.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Crypto.asm.exe  Crypto.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Crypto.asm.exe  Crypto.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Crypto
    if not exist "Crypto.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Crypto.org" (
            mkdir IL
            copy Crypto.exe IL\Crypto.exe 
            ren Crypto.exe Crypto.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Crypto.org /out Crypto.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Crypto.org /out Crypto.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Crypto.reflect.xml
    IF EXIST Crypto.reflect.xml set ReflectionRoots=-x Crypto.reflect.xml
    ...
    set ExePath=%LinkBin%\Crypto.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Crypto.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Crypto.asm.exe %CLRTestExecutionArguments% %Host_Args%
