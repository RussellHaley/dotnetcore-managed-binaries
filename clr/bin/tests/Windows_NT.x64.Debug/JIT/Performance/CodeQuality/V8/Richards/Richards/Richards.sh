...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/V8/Richards/Richards Richards.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/V8/Richards/Richards Richards.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Richards.dasm.il Richards.exe
    "$CORE_ROOT/ildasm" -raweh -out=Richards.dasm.il Richards.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Richards.asm.exe  Richards.dasm.il
    "$CORE_ROOT/ilasm" -output=Richards.asm.exe  Richards.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Richards
    if [ ! -f Richards.org ]; then
        TakeLock
        if [ ! -f Richards.org ]; then
          mkdir IL
          cp Richards.exe IL/Richards.exe
          mv Richards.exe Richards.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Richards.org /out Richards.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Richards.org /out Richards.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Richards.reflect.xml
  if [ -f Richards.reflect.xml ]
then
    ReflectionRoots="-x Richards.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Richards.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Richards.asm.exe $CLRTestExecutionArguments
