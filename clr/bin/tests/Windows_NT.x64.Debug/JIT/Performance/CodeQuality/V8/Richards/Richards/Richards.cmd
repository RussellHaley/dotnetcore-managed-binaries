...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\V8\Richards\Richards Richards.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\V8\Richards\Richards Richards.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Richards.dasm.il Richards.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Richards.dasm.il Richards.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Richards.asm.exe  Richards.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Richards.asm.exe  Richards.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Richards
    if not exist "Richards.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Richards.org" (
            mkdir IL
            copy Richards.exe IL\Richards.exe 
            ren Richards.exe Richards.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Richards.org /out Richards.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Richards.org /out Richards.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Richards.reflect.xml
    IF EXIST Richards.reflect.xml set ReflectionRoots=-x Richards.reflect.xml
    ...
    set ExePath=%LinkBin%\Richards.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Richards.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Richards.asm.exe %CLRTestExecutionArguments% %Host_Args%
