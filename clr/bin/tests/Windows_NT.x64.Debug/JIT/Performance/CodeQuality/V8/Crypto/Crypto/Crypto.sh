...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/V8/Crypto/Crypto Crypto.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/V8/Crypto/Crypto Crypto.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Crypto.dasm.il Crypto.exe
    "$CORE_ROOT/ildasm" -raweh -out=Crypto.dasm.il Crypto.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Crypto.asm.exe  Crypto.dasm.il
    "$CORE_ROOT/ilasm" -output=Crypto.asm.exe  Crypto.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Crypto
    if [ ! -f Crypto.org ]; then
        TakeLock
        if [ ! -f Crypto.org ]; then
          mkdir IL
          cp Crypto.exe IL/Crypto.exe
          mv Crypto.exe Crypto.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Crypto.org /out Crypto.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Crypto.org /out Crypto.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Crypto.reflect.xml
  if [ -f Crypto.reflect.xml ]
then
    ReflectionRoots="-x Crypto.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Crypto.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Crypto.asm.exe $CLRTestExecutionArguments
