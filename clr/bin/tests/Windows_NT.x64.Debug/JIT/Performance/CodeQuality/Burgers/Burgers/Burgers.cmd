...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Burgers\Burgers Burgers.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Burgers\Burgers Burgers.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Burgers.dasm.il Burgers.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Burgers.dasm.il Burgers.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Burgers.asm.exe  Burgers.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Burgers.asm.exe  Burgers.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Burgers
    if not exist "Burgers.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Burgers.org" (
            mkdir IL
            copy Burgers.exe IL\Burgers.exe 
            ren Burgers.exe Burgers.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Burgers.org /out Burgers.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Burgers.org /out Burgers.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Burgers.reflect.xml
    IF EXIST Burgers.reflect.xml set ReflectionRoots=-x Burgers.reflect.xml
    ...
    set ExePath=%LinkBin%\Burgers.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Burgers.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Burgers.asm.exe %CLRTestExecutionArguments% %Host_Args%
