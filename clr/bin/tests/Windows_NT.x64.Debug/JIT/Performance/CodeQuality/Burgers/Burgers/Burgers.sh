...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Burgers/Burgers Burgers.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Burgers/Burgers Burgers.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Burgers.dasm.il Burgers.exe
    "$CORE_ROOT/ildasm" -raweh -out=Burgers.dasm.il Burgers.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Burgers.asm.exe  Burgers.dasm.il
    "$CORE_ROOT/ilasm" -output=Burgers.asm.exe  Burgers.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Burgers
    if [ ! -f Burgers.org ]; then
        TakeLock
        if [ ! -f Burgers.org ]; then
          mkdir IL
          cp Burgers.exe IL/Burgers.exe
          mv Burgers.exe Burgers.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Burgers.org /out Burgers.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Burgers.org /out Burgers.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Burgers.reflect.xml
  if [ -f Burgers.reflect.xml ]
then
    ReflectionRoots="-x Burgers.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Burgers.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Burgers.asm.exe $CLRTestExecutionArguments
