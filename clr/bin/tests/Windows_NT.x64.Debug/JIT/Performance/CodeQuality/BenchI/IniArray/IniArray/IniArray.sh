...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/IniArray/IniArray IniArray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/IniArray/IniArray IniArray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=IniArray.dasm.il IniArray.exe
    "$CORE_ROOT/ildasm" -raweh -out=IniArray.dasm.il IniArray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=IniArray.asm.exe  IniArray.dasm.il
    "$CORE_ROOT/ilasm" -output=IniArray.asm.exe  IniArray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=IniArray
    if [ ! -f IniArray.org ]; then
        TakeLock
        if [ ! -f IniArray.org ]; then
          mkdir IL
          cp IniArray.exe IL/IniArray.exe
          mv IniArray.exe IniArray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in IniArray.org /out IniArray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in IniArray.org /out IniArray.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in IniArray.reflect.xml
  if [ -f IniArray.reflect.xml ]
then
    ReflectionRoots="-x IniArray.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" IniArray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" IniArray.asm.exe $CLRTestExecutionArguments
