...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\AddArray2\AddArray2 AddArray2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\AddArray2\AddArray2 AddArray2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AddArray2.dasm.il AddArray2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AddArray2.dasm.il AddArray2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AddArray2.asm.exe  AddArray2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AddArray2.asm.exe  AddArray2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AddArray2
    if not exist "AddArray2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AddArray2.org" (
            mkdir IL
            copy AddArray2.exe IL\AddArray2.exe 
            ren AddArray2.exe AddArray2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddArray2.org /out AddArray2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddArray2.org /out AddArray2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AddArray2.reflect.xml
    IF EXIST AddArray2.reflect.xml set ReflectionRoots=-x AddArray2.reflect.xml
    ...
    set ExePath=%LinkBin%\AddArray2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AddArray2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AddArray2.asm.exe %CLRTestExecutionArguments% %Host_Args%
