...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/NDhrystone/NDhrystone NDhrystone.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/NDhrystone/NDhrystone NDhrystone.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NDhrystone.dasm.il NDhrystone.exe
    "$CORE_ROOT/ildasm" -raweh -out=NDhrystone.dasm.il NDhrystone.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NDhrystone.asm.exe  NDhrystone.dasm.il
    "$CORE_ROOT/ilasm" -output=NDhrystone.asm.exe  NDhrystone.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NDhrystone
    if [ ! -f NDhrystone.org ]; then
        TakeLock
        if [ ! -f NDhrystone.org ]; then
          mkdir IL
          cp NDhrystone.exe IL/NDhrystone.exe
          mv NDhrystone.exe NDhrystone.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NDhrystone.org /out NDhrystone.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NDhrystone.org /out NDhrystone.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NDhrystone.reflect.xml
  if [ -f NDhrystone.reflect.xml ]
then
    ReflectionRoots="-x NDhrystone.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NDhrystone.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NDhrystone.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NDhrystone.asm.exe $CLRTestExecutionArguments
