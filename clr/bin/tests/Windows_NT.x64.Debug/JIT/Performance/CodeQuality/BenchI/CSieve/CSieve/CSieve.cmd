...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\CSieve\CSieve CSieve.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\CSieve\CSieve CSieve.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CSieve.dasm.il CSieve.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CSieve.dasm.il CSieve.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CSieve.asm.exe  CSieve.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CSieve.asm.exe  CSieve.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CSieve
    if not exist "CSieve.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CSieve.org" (
            mkdir IL
            copy CSieve.exe IL\CSieve.exe 
            ren CSieve.exe CSieve.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CSieve.org /out CSieve.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CSieve.org /out CSieve.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CSieve.reflect.xml
    IF EXIST CSieve.reflect.xml set ReflectionRoots=-x CSieve.reflect.xml
    ...
    set ExePath=%LinkBin%\CSieve.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CSieve.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CSieve.asm.exe %CLRTestExecutionArguments% %Host_Args%
