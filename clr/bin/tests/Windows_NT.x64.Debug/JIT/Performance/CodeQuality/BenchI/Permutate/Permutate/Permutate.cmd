...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Permutate\Permutate Permutate.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Permutate\Permutate Permutate.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Permutate.dasm.il Permutate.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Permutate.dasm.il Permutate.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Permutate.asm.exe  Permutate.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Permutate.asm.exe  Permutate.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Permutate
    if not exist "Permutate.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Permutate.org" (
            mkdir IL
            copy Permutate.exe IL\Permutate.exe 
            ren Permutate.exe Permutate.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Permutate.org /out Permutate.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Permutate.org /out Permutate.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Permutate.reflect.xml
    IF EXIST Permutate.reflect.xml set ReflectionRoots=-x Permutate.reflect.xml
    ...
    set ExePath=%LinkBin%\Permutate.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Permutate.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Permutate.asm.exe %CLRTestExecutionArguments% %Host_Args%
