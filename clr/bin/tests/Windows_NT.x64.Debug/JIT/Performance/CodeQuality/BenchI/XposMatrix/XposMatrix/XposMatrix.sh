...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/XposMatrix/XposMatrix XposMatrix.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/XposMatrix/XposMatrix XposMatrix.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=XposMatrix.dasm.il XposMatrix.exe
    "$CORE_ROOT/ildasm" -raweh -out=XposMatrix.dasm.il XposMatrix.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=XposMatrix.asm.exe  XposMatrix.dasm.il
    "$CORE_ROOT/ilasm" -output=XposMatrix.asm.exe  XposMatrix.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=XposMatrix
    if [ ! -f XposMatrix.org ]; then
        TakeLock
        if [ ! -f XposMatrix.org ]; then
          mkdir IL
          cp XposMatrix.exe IL/XposMatrix.exe
          mv XposMatrix.exe XposMatrix.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in XposMatrix.org /out XposMatrix.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in XposMatrix.org /out XposMatrix.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in XposMatrix.reflect.xml
  if [ -f XposMatrix.reflect.xml ]
then
    ReflectionRoots="-x XposMatrix.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/XposMatrix.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" XposMatrix.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" XposMatrix.asm.exe $CLRTestExecutionArguments
