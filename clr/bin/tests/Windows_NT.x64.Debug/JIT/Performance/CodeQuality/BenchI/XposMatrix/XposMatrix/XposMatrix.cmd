...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\XposMatrix\XposMatrix XposMatrix.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\XposMatrix\XposMatrix XposMatrix.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=XposMatrix.dasm.il XposMatrix.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=XposMatrix.dasm.il XposMatrix.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=XposMatrix.asm.exe  XposMatrix.dasm.il
    %CORE_ROOT%\ilasm.exe /output=XposMatrix.asm.exe  XposMatrix.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=XposMatrix
    if not exist "XposMatrix.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "XposMatrix.org" (
            mkdir IL
            copy XposMatrix.exe IL\XposMatrix.exe 
            ren XposMatrix.exe XposMatrix.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in XposMatrix.org /out XposMatrix.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in XposMatrix.org /out XposMatrix.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in XposMatrix.reflect.xml
    IF EXIST XposMatrix.reflect.xml set ReflectionRoots=-x XposMatrix.reflect.xml
    ...
    set ExePath=%LinkBin%\XposMatrix.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% XposMatrix.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% XposMatrix.asm.exe %CLRTestExecutionArguments% %Host_Args%
