...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\NDhrystone\NDhrystone NDhrystone.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\NDhrystone\NDhrystone NDhrystone.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NDhrystone.dasm.il NDhrystone.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NDhrystone.dasm.il NDhrystone.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NDhrystone.asm.exe  NDhrystone.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NDhrystone.asm.exe  NDhrystone.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NDhrystone
    if not exist "NDhrystone.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NDhrystone.org" (
            mkdir IL
            copy NDhrystone.exe IL\NDhrystone.exe 
            ren NDhrystone.exe NDhrystone.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NDhrystone.org /out NDhrystone.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NDhrystone.org /out NDhrystone.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NDhrystone.reflect.xml
    IF EXIST NDhrystone.reflect.xml set ReflectionRoots=-x NDhrystone.reflect.xml
    ...
    set ExePath=%LinkBin%\NDhrystone.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NDhrystone.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NDhrystone.asm.exe %CLRTestExecutionArguments% %Host_Args%
