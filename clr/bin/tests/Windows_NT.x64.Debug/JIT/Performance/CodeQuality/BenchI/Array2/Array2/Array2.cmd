...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Array2\Array2 Array2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Array2\Array2 Array2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Array2.dasm.il Array2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Array2.dasm.il Array2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Array2.asm.exe  Array2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Array2.asm.exe  Array2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Array2
    if not exist "Array2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Array2.org" (
            mkdir IL
            copy Array2.exe IL\Array2.exe 
            ren Array2.exe Array2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Array2.org /out Array2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Array2.org /out Array2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Array2.reflect.xml
    IF EXIST Array2.reflect.xml set ReflectionRoots=-x Array2.reflect.xml
    ...
    set ExePath=%LinkBin%\Array2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Array2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Array2.asm.exe %CLRTestExecutionArguments% %Host_Args%
