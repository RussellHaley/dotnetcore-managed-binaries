...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Fib/Fib Fib.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Fib/Fib Fib.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Fib.dasm.il Fib.exe
    "$CORE_ROOT/ildasm" -raweh -out=Fib.dasm.il Fib.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Fib.asm.exe  Fib.dasm.il
    "$CORE_ROOT/ilasm" -output=Fib.asm.exe  Fib.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Fib
    if [ ! -f Fib.org ]; then
        TakeLock
        if [ ! -f Fib.org ]; then
          mkdir IL
          cp Fib.exe IL/Fib.exe
          mv Fib.exe Fib.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Fib.org /out Fib.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Fib.org /out Fib.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Fib.reflect.xml
  if [ -f Fib.reflect.xml ]
then
    ReflectionRoots="-x Fib.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Fib.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Fib.asm.exe $CLRTestExecutionArguments
