...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\BubbleSort\BubbleSort BubbleSort.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\BubbleSort\BubbleSort BubbleSort.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=BubbleSort.dasm.il BubbleSort.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=BubbleSort.dasm.il BubbleSort.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=BubbleSort.asm.exe  BubbleSort.dasm.il
    %CORE_ROOT%\ilasm.exe /output=BubbleSort.asm.exe  BubbleSort.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=BubbleSort
    if not exist "BubbleSort.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "BubbleSort.org" (
            mkdir IL
            copy BubbleSort.exe IL\BubbleSort.exe 
            ren BubbleSort.exe BubbleSort.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BubbleSort.org /out BubbleSort.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BubbleSort.org /out BubbleSort.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in BubbleSort.reflect.xml
    IF EXIST BubbleSort.reflect.xml set ReflectionRoots=-x BubbleSort.reflect.xml
    ...
    set ExePath=%LinkBin%\BubbleSort.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% BubbleSort.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% BubbleSort.asm.exe %CLRTestExecutionArguments% %Host_Args%
