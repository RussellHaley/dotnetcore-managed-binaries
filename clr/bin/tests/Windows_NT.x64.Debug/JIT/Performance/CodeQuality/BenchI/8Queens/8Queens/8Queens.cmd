...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\8Queens\8Queens 8Queens.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\8Queens\8Queens 8Queens.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=8Queens.dasm.il 8Queens.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=8Queens.dasm.il 8Queens.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=8Queens.asm.exe  8Queens.dasm.il
    %CORE_ROOT%\ilasm.exe /output=8Queens.asm.exe  8Queens.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=8Queens
    if not exist "8Queens.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "8Queens.org" (
            mkdir IL
            copy 8Queens.exe IL\8Queens.exe 
            ren 8Queens.exe 8Queens.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 8Queens.org /out 8Queens.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 8Queens.org /out 8Queens.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 8Queens.reflect.xml
    IF EXIST 8Queens.reflect.xml set ReflectionRoots=-x 8Queens.reflect.xml
    ...
    set ExePath=%LinkBin%\8Queens.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 8Queens.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 8Queens.asm.exe %CLRTestExecutionArguments% %Host_Args%
