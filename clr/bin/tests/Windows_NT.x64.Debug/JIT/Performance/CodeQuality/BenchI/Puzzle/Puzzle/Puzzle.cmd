...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Puzzle\Puzzle Puzzle.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Puzzle\Puzzle Puzzle.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Puzzle.dasm.il Puzzle.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Puzzle.dasm.il Puzzle.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Puzzle.asm.exe  Puzzle.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Puzzle.asm.exe  Puzzle.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Puzzle
    if not exist "Puzzle.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Puzzle.org" (
            mkdir IL
            copy Puzzle.exe IL\Puzzle.exe 
            ren Puzzle.exe Puzzle.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Puzzle.org /out Puzzle.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Puzzle.org /out Puzzle.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Puzzle.reflect.xml
    IF EXIST Puzzle.reflect.xml set ReflectionRoots=-x Puzzle.reflect.xml
    ...
    set ExePath=%LinkBin%\Puzzle.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Puzzle.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Puzzle.asm.exe %CLRTestExecutionArguments% %Host_Args%
