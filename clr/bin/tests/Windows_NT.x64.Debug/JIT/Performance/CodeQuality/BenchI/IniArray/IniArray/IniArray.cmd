...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\IniArray\IniArray IniArray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\IniArray\IniArray IniArray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=IniArray.dasm.il IniArray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=IniArray.dasm.il IniArray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=IniArray.asm.exe  IniArray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=IniArray.asm.exe  IniArray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=IniArray
    if not exist "IniArray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "IniArray.org" (
            mkdir IL
            copy IniArray.exe IL\IniArray.exe 
            ren IniArray.exe IniArray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in IniArray.org /out IniArray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in IniArray.org /out IniArray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in IniArray.reflect.xml
    IF EXIST IniArray.reflect.xml set ReflectionRoots=-x IniArray.reflect.xml
    ...
    set ExePath=%LinkBin%\IniArray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% IniArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% IniArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
