...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/TreeSort/TreeSort TreeSort.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/TreeSort/TreeSort TreeSort.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=TreeSort.dasm.il TreeSort.exe
    "$CORE_ROOT/ildasm" -raweh -out=TreeSort.dasm.il TreeSort.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=TreeSort.asm.exe  TreeSort.dasm.il
    "$CORE_ROOT/ilasm" -output=TreeSort.asm.exe  TreeSort.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=TreeSort
    if [ ! -f TreeSort.org ]; then
        TakeLock
        if [ ! -f TreeSort.org ]; then
          mkdir IL
          cp TreeSort.exe IL/TreeSort.exe
          mv TreeSort.exe TreeSort.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TreeSort.org /out TreeSort.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TreeSort.org /out TreeSort.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in TreeSort.reflect.xml
  if [ -f TreeSort.reflect.xml ]
then
    ReflectionRoots="-x TreeSort.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" TreeSort.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" TreeSort.asm.exe $CLRTestExecutionArguments
