...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Permutate/Permutate Permutate.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Permutate/Permutate Permutate.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Permutate.dasm.il Permutate.exe
    "$CORE_ROOT/ildasm" -raweh -out=Permutate.dasm.il Permutate.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Permutate.asm.exe  Permutate.dasm.il
    "$CORE_ROOT/ilasm" -output=Permutate.asm.exe  Permutate.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Permutate
    if [ ! -f Permutate.org ]; then
        TakeLock
        if [ ! -f Permutate.org ]; then
          mkdir IL
          cp Permutate.exe IL/Permutate.exe
          mv Permutate.exe Permutate.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Permutate.org /out Permutate.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Permutate.org /out Permutate.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Permutate.reflect.xml
  if [ -f Permutate.reflect.xml ]
then
    ReflectionRoots="-x Permutate.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Permutate.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Permutate.asm.exe $CLRTestExecutionArguments
