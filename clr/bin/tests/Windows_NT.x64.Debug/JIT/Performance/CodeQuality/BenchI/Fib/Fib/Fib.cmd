...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Fib\Fib Fib.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Fib\Fib Fib.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Fib.dasm.il Fib.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Fib.dasm.il Fib.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Fib.asm.exe  Fib.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Fib.asm.exe  Fib.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Fib
    if not exist "Fib.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Fib.org" (
            mkdir IL
            copy Fib.exe IL\Fib.exe 
            ren Fib.exe Fib.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Fib.org /out Fib.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Fib.org /out Fib.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Fib.reflect.xml
    IF EXIST Fib.reflect.xml set ReflectionRoots=-x Fib.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Fib.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Fib.asm.exe %CLRTestExecutionArguments% %Host_Args%
