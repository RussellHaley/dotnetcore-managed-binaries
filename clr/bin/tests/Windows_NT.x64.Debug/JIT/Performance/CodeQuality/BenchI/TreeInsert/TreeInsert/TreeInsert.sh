...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/TreeInsert/TreeInsert TreeInsert.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/TreeInsert/TreeInsert TreeInsert.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=TreeInsert.dasm.il TreeInsert.exe
    "$CORE_ROOT/ildasm" -raweh -out=TreeInsert.dasm.il TreeInsert.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=TreeInsert.asm.exe  TreeInsert.dasm.il
    "$CORE_ROOT/ilasm" -output=TreeInsert.asm.exe  TreeInsert.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=TreeInsert
    if [ ! -f TreeInsert.org ]; then
        TakeLock
        if [ ! -f TreeInsert.org ]; then
          mkdir IL
          cp TreeInsert.exe IL/TreeInsert.exe
          mv TreeInsert.exe TreeInsert.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TreeInsert.org /out TreeInsert.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TreeInsert.org /out TreeInsert.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in TreeInsert.reflect.xml
  if [ -f TreeInsert.reflect.xml ]
then
    ReflectionRoots="-x TreeInsert.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/TreeInsert.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" TreeInsert.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" TreeInsert.asm.exe $CLRTestExecutionArguments
