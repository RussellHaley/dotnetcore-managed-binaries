...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Puzzle/Puzzle Puzzle.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Puzzle/Puzzle Puzzle.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Puzzle.dasm.il Puzzle.exe
    "$CORE_ROOT/ildasm" -raweh -out=Puzzle.dasm.il Puzzle.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Puzzle.asm.exe  Puzzle.dasm.il
    "$CORE_ROOT/ilasm" -output=Puzzle.asm.exe  Puzzle.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Puzzle
    if [ ! -f Puzzle.org ]; then
        TakeLock
        if [ ! -f Puzzle.org ]; then
          mkdir IL
          cp Puzzle.exe IL/Puzzle.exe
          mv Puzzle.exe Puzzle.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Puzzle.org /out Puzzle.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Puzzle.org /out Puzzle.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Puzzle.reflect.xml
  if [ -f Puzzle.reflect.xml ]
then
    ReflectionRoots="-x Puzzle.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Puzzle.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Puzzle.asm.exe $CLRTestExecutionArguments
