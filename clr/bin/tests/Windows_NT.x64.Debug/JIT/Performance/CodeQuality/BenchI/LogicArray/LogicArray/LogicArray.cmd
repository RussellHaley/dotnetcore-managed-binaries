...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\LogicArray\LogicArray LogicArray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\LogicArray\LogicArray LogicArray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LogicArray.dasm.il LogicArray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LogicArray.dasm.il LogicArray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LogicArray.asm.exe  LogicArray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LogicArray.asm.exe  LogicArray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LogicArray
    if not exist "LogicArray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LogicArray.org" (
            mkdir IL
            copy LogicArray.exe IL\LogicArray.exe 
            ren LogicArray.exe LogicArray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LogicArray.org /out LogicArray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LogicArray.org /out LogicArray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LogicArray.reflect.xml
    IF EXIST LogicArray.reflect.xml set ReflectionRoots=-x LogicArray.reflect.xml
    ...
    set ExePath=%LinkBin%\LogicArray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LogicArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LogicArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
