...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\TreeSort\TreeSort TreeSort.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\TreeSort\TreeSort TreeSort.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=TreeSort.dasm.il TreeSort.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=TreeSort.dasm.il TreeSort.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=TreeSort.asm.exe  TreeSort.dasm.il
    %CORE_ROOT%\ilasm.exe /output=TreeSort.asm.exe  TreeSort.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=TreeSort
    if not exist "TreeSort.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "TreeSort.org" (
            mkdir IL
            copy TreeSort.exe IL\TreeSort.exe 
            ren TreeSort.exe TreeSort.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TreeSort.org /out TreeSort.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TreeSort.org /out TreeSort.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in TreeSort.reflect.xml
    IF EXIST TreeSort.reflect.xml set ReflectionRoots=-x TreeSort.reflect.xml
    ...
    set ExePath=%LinkBin%\TreeSort.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% TreeSort.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% TreeSort.asm.exe %CLRTestExecutionArguments% %Host_Args%
