...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Pi\Pi Pi.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Pi\Pi Pi.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Pi.dasm.il Pi.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Pi.dasm.il Pi.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Pi.asm.exe  Pi.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Pi.asm.exe  Pi.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Pi
    if not exist "Pi.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Pi.org" (
            mkdir IL
            copy Pi.exe IL\Pi.exe 
            ren Pi.exe Pi.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Pi.org /out Pi.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Pi.org /out Pi.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Pi.reflect.xml
    IF EXIST Pi.reflect.xml set ReflectionRoots=-x Pi.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Pi.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Pi.asm.exe %CLRTestExecutionArguments% %Host_Args%
