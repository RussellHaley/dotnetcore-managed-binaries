...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Midpoint/Midpoint Midpoint.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Midpoint/Midpoint Midpoint.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Midpoint.dasm.il Midpoint.exe
    "$CORE_ROOT/ildasm" -raweh -out=Midpoint.dasm.il Midpoint.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Midpoint.asm.exe  Midpoint.dasm.il
    "$CORE_ROOT/ilasm" -output=Midpoint.asm.exe  Midpoint.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Midpoint
    if [ ! -f Midpoint.org ]; then
        TakeLock
        if [ ! -f Midpoint.org ]; then
          mkdir IL
          cp Midpoint.exe IL/Midpoint.exe
          mv Midpoint.exe Midpoint.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Midpoint.org /out Midpoint.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Midpoint.org /out Midpoint.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Midpoint.reflect.xml
  if [ -f Midpoint.reflect.xml ]
then
    ReflectionRoots="-x Midpoint.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Midpoint.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Midpoint.asm.exe $CLRTestExecutionArguments
