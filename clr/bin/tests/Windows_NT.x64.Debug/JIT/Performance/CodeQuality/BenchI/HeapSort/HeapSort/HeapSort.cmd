...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\HeapSort\HeapSort HeapSort.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\HeapSort\HeapSort HeapSort.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=HeapSort.dasm.il HeapSort.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=HeapSort.dasm.il HeapSort.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=HeapSort.asm.exe  HeapSort.dasm.il
    %CORE_ROOT%\ilasm.exe /output=HeapSort.asm.exe  HeapSort.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=HeapSort
    if not exist "HeapSort.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "HeapSort.org" (
            mkdir IL
            copy HeapSort.exe IL\HeapSort.exe 
            ren HeapSort.exe HeapSort.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in HeapSort.org /out HeapSort.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in HeapSort.org /out HeapSort.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in HeapSort.reflect.xml
    IF EXIST HeapSort.reflect.xml set ReflectionRoots=-x HeapSort.reflect.xml
    ...
    set ExePath=%LinkBin%\HeapSort.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% HeapSort.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% HeapSort.asm.exe %CLRTestExecutionArguments% %Host_Args%
