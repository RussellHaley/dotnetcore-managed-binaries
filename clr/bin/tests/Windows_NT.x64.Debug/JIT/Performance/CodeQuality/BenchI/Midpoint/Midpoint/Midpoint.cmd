...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Midpoint\Midpoint Midpoint.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Midpoint\Midpoint Midpoint.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Midpoint.dasm.il Midpoint.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Midpoint.dasm.il Midpoint.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Midpoint.asm.exe  Midpoint.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Midpoint.asm.exe  Midpoint.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Midpoint
    if not exist "Midpoint.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Midpoint.org" (
            mkdir IL
            copy Midpoint.exe IL\Midpoint.exe 
            ren Midpoint.exe Midpoint.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Midpoint.org /out Midpoint.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Midpoint.org /out Midpoint.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Midpoint.reflect.xml
    IF EXIST Midpoint.reflect.xml set ReflectionRoots=-x Midpoint.reflect.xml
    ...
    set ExePath=%LinkBin%\Midpoint.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Midpoint.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Midpoint.asm.exe %CLRTestExecutionArguments% %Host_Args%
