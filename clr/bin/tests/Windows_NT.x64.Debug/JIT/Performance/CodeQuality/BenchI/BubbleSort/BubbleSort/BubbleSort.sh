...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/BubbleSort/BubbleSort BubbleSort.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/BubbleSort/BubbleSort BubbleSort.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=BubbleSort.dasm.il BubbleSort.exe
    "$CORE_ROOT/ildasm" -raweh -out=BubbleSort.dasm.il BubbleSort.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=BubbleSort.asm.exe  BubbleSort.dasm.il
    "$CORE_ROOT/ilasm" -output=BubbleSort.asm.exe  BubbleSort.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=BubbleSort
    if [ ! -f BubbleSort.org ]; then
        TakeLock
        if [ ! -f BubbleSort.org ]; then
          mkdir IL
          cp BubbleSort.exe IL/BubbleSort.exe
          mv BubbleSort.exe BubbleSort.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BubbleSort.org /out BubbleSort.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BubbleSort.org /out BubbleSort.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in BubbleSort.reflect.xml
  if [ -f BubbleSort.reflect.xml ]
then
    ReflectionRoots="-x BubbleSort.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/BubbleSort.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" BubbleSort.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" BubbleSort.asm.exe $CLRTestExecutionArguments
