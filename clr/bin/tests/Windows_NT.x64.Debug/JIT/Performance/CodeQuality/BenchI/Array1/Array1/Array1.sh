...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Array1/Array1 Array1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Array1/Array1 Array1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Array1.dasm.il Array1.exe
    "$CORE_ROOT/ildasm" -raweh -out=Array1.dasm.il Array1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Array1.asm.exe  Array1.dasm.il
    "$CORE_ROOT/ilasm" -output=Array1.asm.exe  Array1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Array1
    if [ ! -f Array1.org ]; then
        TakeLock
        if [ ! -f Array1.org ]; then
          mkdir IL
          cp Array1.exe IL/Array1.exe
          mv Array1.exe Array1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Array1.org /out Array1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Array1.org /out Array1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Array1.reflect.xml
  if [ -f Array1.reflect.xml ]
then
    ReflectionRoots="-x Array1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Array1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Array1.asm.exe $CLRTestExecutionArguments
