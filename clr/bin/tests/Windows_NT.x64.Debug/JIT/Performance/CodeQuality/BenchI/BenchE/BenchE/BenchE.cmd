...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\BenchE\BenchE BenchE.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\BenchE\BenchE BenchE.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=BenchE.dasm.il BenchE.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=BenchE.dasm.il BenchE.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=BenchE.asm.exe  BenchE.dasm.il
    %CORE_ROOT%\ilasm.exe /output=BenchE.asm.exe  BenchE.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=BenchE
    if not exist "BenchE.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "BenchE.org" (
            mkdir IL
            copy BenchE.exe IL\BenchE.exe 
            ren BenchE.exe BenchE.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BenchE.org /out BenchE.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BenchE.org /out BenchE.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in BenchE.reflect.xml
    IF EXIST BenchE.reflect.xml set ReflectionRoots=-x BenchE.reflect.xml
    ...
    set ExePath=%LinkBin%\BenchE.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% BenchE.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% BenchE.asm.exe %CLRTestExecutionArguments% %Host_Args%
