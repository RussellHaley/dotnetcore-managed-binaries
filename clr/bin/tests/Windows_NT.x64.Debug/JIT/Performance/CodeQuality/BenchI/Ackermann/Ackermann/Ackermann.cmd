...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Ackermann\Ackermann Ackermann.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Ackermann\Ackermann Ackermann.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Ackermann.dasm.il Ackermann.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Ackermann.dasm.il Ackermann.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Ackermann.asm.exe  Ackermann.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Ackermann.asm.exe  Ackermann.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Ackermann
    if not exist "Ackermann.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Ackermann.org" (
            mkdir IL
            copy Ackermann.exe IL\Ackermann.exe 
            ren Ackermann.exe Ackermann.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ackermann.org /out Ackermann.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Ackermann.org /out Ackermann.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Ackermann.reflect.xml
    IF EXIST Ackermann.reflect.xml set ReflectionRoots=-x Ackermann.reflect.xml
    ...
    set ExePath=%LinkBin%\Ackermann.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Ackermann.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Ackermann.asm.exe %CLRTestExecutionArguments% %Host_Args%
