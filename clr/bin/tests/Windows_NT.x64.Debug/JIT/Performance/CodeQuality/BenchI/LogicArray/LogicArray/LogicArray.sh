...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/LogicArray/LogicArray LogicArray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/LogicArray/LogicArray LogicArray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LogicArray.dasm.il LogicArray.exe
    "$CORE_ROOT/ildasm" -raweh -out=LogicArray.dasm.il LogicArray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LogicArray.asm.exe  LogicArray.dasm.il
    "$CORE_ROOT/ilasm" -output=LogicArray.asm.exe  LogicArray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LogicArray
    if [ ! -f LogicArray.org ]; then
        TakeLock
        if [ ! -f LogicArray.org ]; then
          mkdir IL
          cp LogicArray.exe IL/LogicArray.exe
          mv LogicArray.exe LogicArray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LogicArray.org /out LogicArray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LogicArray.org /out LogicArray.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LogicArray.reflect.xml
  if [ -f LogicArray.reflect.xml ]
then
    ReflectionRoots="-x LogicArray.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/LogicArray.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LogicArray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LogicArray.asm.exe $CLRTestExecutionArguments
