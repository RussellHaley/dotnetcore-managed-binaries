...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\AddArray\AddArray AddArray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\AddArray\AddArray AddArray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AddArray.dasm.il AddArray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AddArray.dasm.il AddArray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AddArray.asm.exe  AddArray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AddArray.asm.exe  AddArray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AddArray
    if not exist "AddArray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AddArray.org" (
            mkdir IL
            copy AddArray.exe IL\AddArray.exe 
            ren AddArray.exe AddArray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddArray.org /out AddArray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddArray.org /out AddArray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AddArray.reflect.xml
    IF EXIST AddArray.reflect.xml set ReflectionRoots=-x AddArray.reflect.xml
    ...
    set ExePath=%LinkBin%\AddArray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AddArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AddArray.asm.exe %CLRTestExecutionArguments% %Host_Args%
