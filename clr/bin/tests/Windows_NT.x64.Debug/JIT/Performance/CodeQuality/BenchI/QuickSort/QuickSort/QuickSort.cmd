...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\QuickSort\QuickSort QuickSort.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\QuickSort\QuickSort QuickSort.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=QuickSort.dasm.il QuickSort.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=QuickSort.dasm.il QuickSort.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=QuickSort.asm.exe  QuickSort.dasm.il
    %CORE_ROOT%\ilasm.exe /output=QuickSort.asm.exe  QuickSort.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=QuickSort
    if not exist "QuickSort.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "QuickSort.org" (
            mkdir IL
            copy QuickSort.exe IL\QuickSort.exe 
            ren QuickSort.exe QuickSort.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in QuickSort.org /out QuickSort.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in QuickSort.org /out QuickSort.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in QuickSort.reflect.xml
    IF EXIST QuickSort.reflect.xml set ReflectionRoots=-x QuickSort.reflect.xml
    ...
    set ExePath=%LinkBin%\QuickSort.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% QuickSort.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% QuickSort.asm.exe %CLRTestExecutionArguments% %Host_Args%
