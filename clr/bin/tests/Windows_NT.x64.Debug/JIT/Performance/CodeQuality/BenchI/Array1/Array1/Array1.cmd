...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Array1\Array1 Array1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\Array1\Array1 Array1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Array1.dasm.il Array1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Array1.dasm.il Array1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Array1.asm.exe  Array1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Array1.asm.exe  Array1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Array1
    if not exist "Array1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Array1.org" (
            mkdir IL
            copy Array1.exe IL\Array1.exe 
            ren Array1.exe Array1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Array1.org /out Array1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Array1.org /out Array1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Array1.reflect.xml
    IF EXIST Array1.reflect.xml set ReflectionRoots=-x Array1.reflect.xml
    ...
    set ExePath=%LinkBin%\Array1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Array1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Array1.asm.exe %CLRTestExecutionArguments% %Host_Args%
