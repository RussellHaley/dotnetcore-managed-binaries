...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Array2/Array2 Array2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Array2/Array2 Array2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Array2.dasm.il Array2.exe
    "$CORE_ROOT/ildasm" -raweh -out=Array2.dasm.il Array2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Array2.asm.exe  Array2.dasm.il
    "$CORE_ROOT/ilasm" -output=Array2.asm.exe  Array2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Array2
    if [ ! -f Array2.org ]; then
        TakeLock
        if [ ! -f Array2.org ]; then
          mkdir IL
          cp Array2.exe IL/Array2.exe
          mv Array2.exe Array2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Array2.org /out Array2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Array2.org /out Array2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Array2.reflect.xml
  if [ -f Array2.reflect.xml ]
then
    ReflectionRoots="-x Array2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Array2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Array2.asm.exe $CLRTestExecutionArguments
