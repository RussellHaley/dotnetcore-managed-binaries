...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/QuickSort/QuickSort QuickSort.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/QuickSort/QuickSort QuickSort.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=QuickSort.dasm.il QuickSort.exe
    "$CORE_ROOT/ildasm" -raweh -out=QuickSort.dasm.il QuickSort.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=QuickSort.asm.exe  QuickSort.dasm.il
    "$CORE_ROOT/ilasm" -output=QuickSort.asm.exe  QuickSort.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=QuickSort
    if [ ! -f QuickSort.org ]; then
        TakeLock
        if [ ! -f QuickSort.org ]; then
          mkdir IL
          cp QuickSort.exe IL/QuickSort.exe
          mv QuickSort.exe QuickSort.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in QuickSort.org /out QuickSort.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in QuickSort.org /out QuickSort.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in QuickSort.reflect.xml
  if [ -f QuickSort.reflect.xml ]
then
    ReflectionRoots="-x QuickSort.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" QuickSort.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" QuickSort.asm.exe $CLRTestExecutionArguments
