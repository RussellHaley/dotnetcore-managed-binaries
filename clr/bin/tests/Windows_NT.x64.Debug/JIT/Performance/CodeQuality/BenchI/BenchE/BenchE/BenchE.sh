...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/BenchE/BenchE BenchE.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/BenchE/BenchE BenchE.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=BenchE.dasm.il BenchE.exe
    "$CORE_ROOT/ildasm" -raweh -out=BenchE.dasm.il BenchE.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=BenchE.asm.exe  BenchE.dasm.il
    "$CORE_ROOT/ilasm" -output=BenchE.asm.exe  BenchE.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=BenchE
    if [ ! -f BenchE.org ]; then
        TakeLock
        if [ ! -f BenchE.org ]; then
          mkdir IL
          cp BenchE.exe IL/BenchE.exe
          mv BenchE.exe BenchE.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BenchE.org /out BenchE.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BenchE.org /out BenchE.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in BenchE.reflect.xml
  if [ -f BenchE.reflect.xml ]
then
    ReflectionRoots="-x BenchE.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" BenchE.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" BenchE.asm.exe $CLRTestExecutionArguments
