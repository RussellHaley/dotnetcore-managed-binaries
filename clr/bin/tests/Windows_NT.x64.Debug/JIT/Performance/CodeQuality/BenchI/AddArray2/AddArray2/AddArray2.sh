...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/AddArray2/AddArray2 AddArray2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/AddArray2/AddArray2 AddArray2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AddArray2.dasm.il AddArray2.exe
    "$CORE_ROOT/ildasm" -raweh -out=AddArray2.dasm.il AddArray2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AddArray2.asm.exe  AddArray2.dasm.il
    "$CORE_ROOT/ilasm" -output=AddArray2.asm.exe  AddArray2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AddArray2
    if [ ! -f AddArray2.org ]; then
        TakeLock
        if [ ! -f AddArray2.org ]; then
          mkdir IL
          cp AddArray2.exe IL/AddArray2.exe
          mv AddArray2.exe AddArray2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddArray2.org /out AddArray2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddArray2.org /out AddArray2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AddArray2.reflect.xml
  if [ -f AddArray2.reflect.xml ]
then
    ReflectionRoots="-x AddArray2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AddArray2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AddArray2.asm.exe $CLRTestExecutionArguments
