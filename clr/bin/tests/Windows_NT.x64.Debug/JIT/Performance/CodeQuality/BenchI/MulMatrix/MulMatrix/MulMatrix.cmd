...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\MulMatrix\MulMatrix MulMatrix.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\MulMatrix\MulMatrix MulMatrix.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MulMatrix.dasm.il MulMatrix.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MulMatrix.dasm.il MulMatrix.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MulMatrix.asm.exe  MulMatrix.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MulMatrix.asm.exe  MulMatrix.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MulMatrix
    if not exist "MulMatrix.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MulMatrix.org" (
            mkdir IL
            copy MulMatrix.exe IL\MulMatrix.exe 
            ren MulMatrix.exe MulMatrix.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MulMatrix.org /out MulMatrix.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MulMatrix.org /out MulMatrix.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MulMatrix.reflect.xml
    IF EXIST MulMatrix.reflect.xml set ReflectionRoots=-x MulMatrix.reflect.xml
    ...
    set ExePath=%LinkBin%\MulMatrix.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MulMatrix.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MulMatrix.asm.exe %CLRTestExecutionArguments% %Host_Args%
