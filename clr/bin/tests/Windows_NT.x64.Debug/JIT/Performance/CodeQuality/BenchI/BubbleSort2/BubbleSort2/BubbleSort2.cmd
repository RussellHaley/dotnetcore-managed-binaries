...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\BubbleSort2\BubbleSort2 BubbleSort2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\BubbleSort2\BubbleSort2 BubbleSort2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=BubbleSort2.dasm.il BubbleSort2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=BubbleSort2.dasm.il BubbleSort2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=BubbleSort2.asm.exe  BubbleSort2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=BubbleSort2.asm.exe  BubbleSort2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=BubbleSort2
    if not exist "BubbleSort2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "BubbleSort2.org" (
            mkdir IL
            copy BubbleSort2.exe IL\BubbleSort2.exe 
            ren BubbleSort2.exe BubbleSort2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BubbleSort2.org /out BubbleSort2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BubbleSort2.org /out BubbleSort2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in BubbleSort2.reflect.xml
    IF EXIST BubbleSort2.reflect.xml set ReflectionRoots=-x BubbleSort2.reflect.xml
    ...
    set ExePath=%LinkBin%\BubbleSort2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% BubbleSort2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% BubbleSort2.asm.exe %CLRTestExecutionArguments% %Host_Args%
