...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/HeapSort/HeapSort HeapSort.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/HeapSort/HeapSort HeapSort.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=HeapSort.dasm.il HeapSort.exe
    "$CORE_ROOT/ildasm" -raweh -out=HeapSort.dasm.il HeapSort.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=HeapSort.asm.exe  HeapSort.dasm.il
    "$CORE_ROOT/ilasm" -output=HeapSort.asm.exe  HeapSort.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=HeapSort
    if [ ! -f HeapSort.org ]; then
        TakeLock
        if [ ! -f HeapSort.org ]; then
          mkdir IL
          cp HeapSort.exe IL/HeapSort.exe
          mv HeapSort.exe HeapSort.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in HeapSort.org /out HeapSort.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in HeapSort.org /out HeapSort.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in HeapSort.reflect.xml
  if [ -f HeapSort.reflect.xml ]
then
    ReflectionRoots="-x HeapSort.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" HeapSort.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" HeapSort.asm.exe $CLRTestExecutionArguments
