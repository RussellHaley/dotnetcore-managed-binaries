...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Pi/Pi Pi.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Pi/Pi Pi.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Pi.dasm.il Pi.exe
    "$CORE_ROOT/ildasm" -raweh -out=Pi.dasm.il Pi.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Pi.asm.exe  Pi.dasm.il
    "$CORE_ROOT/ilasm" -output=Pi.asm.exe  Pi.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Pi
    if [ ! -f Pi.org ]; then
        TakeLock
        if [ ! -f Pi.org ]; then
          mkdir IL
          cp Pi.exe IL/Pi.exe
          mv Pi.exe Pi.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Pi.org /out Pi.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Pi.org /out Pi.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Pi.reflect.xml
  if [ -f Pi.reflect.xml ]
then
    ReflectionRoots="-x Pi.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Pi.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Pi.asm.exe $CLRTestExecutionArguments
