...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Ackermann/Ackermann Ackermann.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/Ackermann/Ackermann Ackermann.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Ackermann.dasm.il Ackermann.exe
    "$CORE_ROOT/ildasm" -raweh -out=Ackermann.dasm.il Ackermann.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Ackermann.asm.exe  Ackermann.dasm.il
    "$CORE_ROOT/ilasm" -output=Ackermann.asm.exe  Ackermann.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Ackermann
    if [ ! -f Ackermann.org ]; then
        TakeLock
        if [ ! -f Ackermann.org ]; then
          mkdir IL
          cp Ackermann.exe IL/Ackermann.exe
          mv Ackermann.exe Ackermann.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ackermann.org /out Ackermann.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Ackermann.org /out Ackermann.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Ackermann.reflect.xml
  if [ -f Ackermann.reflect.xml ]
then
    ReflectionRoots="-x Ackermann.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Ackermann.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Ackermann.asm.exe $CLRTestExecutionArguments
