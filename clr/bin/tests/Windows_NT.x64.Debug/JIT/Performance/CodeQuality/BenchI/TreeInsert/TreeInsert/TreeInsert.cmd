...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\TreeInsert\TreeInsert TreeInsert.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\BenchI\TreeInsert\TreeInsert TreeInsert.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=TreeInsert.dasm.il TreeInsert.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=TreeInsert.dasm.il TreeInsert.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=TreeInsert.asm.exe  TreeInsert.dasm.il
    %CORE_ROOT%\ilasm.exe /output=TreeInsert.asm.exe  TreeInsert.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=TreeInsert
    if not exist "TreeInsert.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "TreeInsert.org" (
            mkdir IL
            copy TreeInsert.exe IL\TreeInsert.exe 
            ren TreeInsert.exe TreeInsert.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TreeInsert.org /out TreeInsert.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TreeInsert.org /out TreeInsert.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in TreeInsert.reflect.xml
    IF EXIST TreeInsert.reflect.xml set ReflectionRoots=-x TreeInsert.reflect.xml
    ...
    set ExePath=%LinkBin%\TreeInsert.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% TreeInsert.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% TreeInsert.asm.exe %CLRTestExecutionArguments% %Host_Args%
