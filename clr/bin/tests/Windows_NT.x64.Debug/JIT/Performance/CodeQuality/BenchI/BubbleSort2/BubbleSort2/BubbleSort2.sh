...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/BubbleSort2/BubbleSort2 BubbleSort2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/BubbleSort2/BubbleSort2 BubbleSort2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=BubbleSort2.dasm.il BubbleSort2.exe
    "$CORE_ROOT/ildasm" -raweh -out=BubbleSort2.dasm.il BubbleSort2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=BubbleSort2.asm.exe  BubbleSort2.dasm.il
    "$CORE_ROOT/ilasm" -output=BubbleSort2.asm.exe  BubbleSort2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=BubbleSort2
    if [ ! -f BubbleSort2.org ]; then
        TakeLock
        if [ ! -f BubbleSort2.org ]; then
          mkdir IL
          cp BubbleSort2.exe IL/BubbleSort2.exe
          mv BubbleSort2.exe BubbleSort2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BubbleSort2.org /out BubbleSort2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BubbleSort2.org /out BubbleSort2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in BubbleSort2.reflect.xml
  if [ -f BubbleSort2.reflect.xml ]
then
    ReflectionRoots="-x BubbleSort2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/BubbleSort2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" BubbleSort2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" BubbleSort2.asm.exe $CLRTestExecutionArguments
