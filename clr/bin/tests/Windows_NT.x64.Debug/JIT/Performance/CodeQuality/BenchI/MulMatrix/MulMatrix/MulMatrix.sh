...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/MulMatrix/MulMatrix MulMatrix.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/MulMatrix/MulMatrix MulMatrix.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MulMatrix.dasm.il MulMatrix.exe
    "$CORE_ROOT/ildasm" -raweh -out=MulMatrix.dasm.il MulMatrix.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MulMatrix.asm.exe  MulMatrix.dasm.il
    "$CORE_ROOT/ilasm" -output=MulMatrix.asm.exe  MulMatrix.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MulMatrix
    if [ ! -f MulMatrix.org ]; then
        TakeLock
        if [ ! -f MulMatrix.org ]; then
          mkdir IL
          cp MulMatrix.exe IL/MulMatrix.exe
          mv MulMatrix.exe MulMatrix.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MulMatrix.org /out MulMatrix.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MulMatrix.org /out MulMatrix.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in MulMatrix.reflect.xml
  if [ -f MulMatrix.reflect.xml ]
then
    ReflectionRoots="-x MulMatrix.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MulMatrix.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MulMatrix.asm.exe $CLRTestExecutionArguments
