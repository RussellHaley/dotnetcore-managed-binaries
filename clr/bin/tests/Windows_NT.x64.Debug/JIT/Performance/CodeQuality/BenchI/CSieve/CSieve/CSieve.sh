...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/CSieve/CSieve CSieve.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/CSieve/CSieve CSieve.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CSieve.dasm.il CSieve.exe
    "$CORE_ROOT/ildasm" -raweh -out=CSieve.dasm.il CSieve.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CSieve.asm.exe  CSieve.dasm.il
    "$CORE_ROOT/ilasm" -output=CSieve.asm.exe  CSieve.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CSieve
    if [ ! -f CSieve.org ]; then
        TakeLock
        if [ ! -f CSieve.org ]; then
          mkdir IL
          cp CSieve.exe IL/CSieve.exe
          mv CSieve.exe CSieve.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CSieve.org /out CSieve.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CSieve.org /out CSieve.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CSieve.reflect.xml
  if [ -f CSieve.reflect.xml ]
then
    ReflectionRoots="-x CSieve.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CSieve.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CSieve.asm.exe $CLRTestExecutionArguments
