...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/8Queens/8Queens 8Queens.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/8Queens/8Queens 8Queens.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=8Queens.dasm.il 8Queens.exe
    "$CORE_ROOT/ildasm" -raweh -out=8Queens.dasm.il 8Queens.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=8Queens.asm.exe  8Queens.dasm.il
    "$CORE_ROOT/ilasm" -output=8Queens.asm.exe  8Queens.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=8Queens
    if [ ! -f 8Queens.org ]; then
        TakeLock
        if [ ! -f 8Queens.org ]; then
          mkdir IL
          cp 8Queens.exe IL/8Queens.exe
          mv 8Queens.exe 8Queens.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 8Queens.org /out 8Queens.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 8Queens.org /out 8Queens.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 8Queens.reflect.xml
  if [ -f 8Queens.reflect.xml ]
then
    ReflectionRoots="-x 8Queens.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 8Queens.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 8Queens.asm.exe $CLRTestExecutionArguments
