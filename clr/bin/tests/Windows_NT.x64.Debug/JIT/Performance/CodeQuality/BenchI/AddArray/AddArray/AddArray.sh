...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/AddArray/AddArray AddArray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/BenchI/AddArray/AddArray AddArray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AddArray.dasm.il AddArray.exe
    "$CORE_ROOT/ildasm" -raweh -out=AddArray.dasm.il AddArray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AddArray.asm.exe  AddArray.dasm.il
    "$CORE_ROOT/ilasm" -output=AddArray.asm.exe  AddArray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AddArray
    if [ ! -f AddArray.org ]; then
        TakeLock
        if [ ! -f AddArray.org ]; then
          mkdir IL
          cp AddArray.exe IL/AddArray.exe
          mv AddArray.exe AddArray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddArray.org /out AddArray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddArray.org /out AddArray.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AddArray.reflect.xml
  if [ -f AddArray.reflect.xml ]
then
    ReflectionRoots="-x AddArray.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AddArray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AddArray.asm.exe $CLRTestExecutionArguments
