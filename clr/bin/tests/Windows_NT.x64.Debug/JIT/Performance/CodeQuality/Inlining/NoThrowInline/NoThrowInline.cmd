...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Inlining\NoThrowInline NoThrowInline.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Inlining\NoThrowInline NoThrowInline.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NoThrowInline.dasm.il NoThrowInline.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NoThrowInline.dasm.il NoThrowInline.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NoThrowInline.asm.exe  NoThrowInline.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NoThrowInline.asm.exe  NoThrowInline.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NoThrowInline
    if not exist "NoThrowInline.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NoThrowInline.org" (
            mkdir IL
            copy NoThrowInline.exe IL\NoThrowInline.exe 
            ren NoThrowInline.exe NoThrowInline.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NoThrowInline.org /out NoThrowInline.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NoThrowInline.org /out NoThrowInline.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NoThrowInline.reflect.xml
    IF EXIST NoThrowInline.reflect.xml set ReflectionRoots=-x NoThrowInline.reflect.xml
    ...
    set ExePath=%LinkBin%\NoThrowInline.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NoThrowInline.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NoThrowInline.asm.exe %CLRTestExecutionArguments% %Host_Args%
