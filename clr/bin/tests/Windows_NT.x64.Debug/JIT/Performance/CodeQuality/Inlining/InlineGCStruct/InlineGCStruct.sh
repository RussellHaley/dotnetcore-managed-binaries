...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Inlining/InlineGCStruct InlineGCStruct.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Inlining/InlineGCStruct InlineGCStruct.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=InlineGCStruct.dasm.il InlineGCStruct.exe
    "$CORE_ROOT/ildasm" -raweh -out=InlineGCStruct.dasm.il InlineGCStruct.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=InlineGCStruct.asm.exe  InlineGCStruct.dasm.il
    "$CORE_ROOT/ilasm" -output=InlineGCStruct.asm.exe  InlineGCStruct.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=InlineGCStruct
    if [ ! -f InlineGCStruct.org ]; then
        TakeLock
        if [ ! -f InlineGCStruct.org ]; then
          mkdir IL
          cp InlineGCStruct.exe IL/InlineGCStruct.exe
          mv InlineGCStruct.exe InlineGCStruct.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InlineGCStruct.org /out InlineGCStruct.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InlineGCStruct.org /out InlineGCStruct.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in InlineGCStruct.reflect.xml
  if [ -f InlineGCStruct.reflect.xml ]
then
    ReflectionRoots="-x InlineGCStruct.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/InlineGCStruct.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" InlineGCStruct.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" InlineGCStruct.asm.exe $CLRTestExecutionArguments
