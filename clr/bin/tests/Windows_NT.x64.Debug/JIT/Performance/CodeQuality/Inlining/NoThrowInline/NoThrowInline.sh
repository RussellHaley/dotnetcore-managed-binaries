...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Inlining/NoThrowInline NoThrowInline.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Inlining/NoThrowInline NoThrowInline.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NoThrowInline.dasm.il NoThrowInline.exe
    "$CORE_ROOT/ildasm" -raweh -out=NoThrowInline.dasm.il NoThrowInline.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NoThrowInline.asm.exe  NoThrowInline.dasm.il
    "$CORE_ROOT/ilasm" -output=NoThrowInline.asm.exe  NoThrowInline.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NoThrowInline
    if [ ! -f NoThrowInline.org ]; then
        TakeLock
        if [ ! -f NoThrowInline.org ]; then
          mkdir IL
          cp NoThrowInline.exe IL/NoThrowInline.exe
          mv NoThrowInline.exe NoThrowInline.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NoThrowInline.org /out NoThrowInline.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NoThrowInline.org /out NoThrowInline.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NoThrowInline.reflect.xml
  if [ -f NoThrowInline.reflect.xml ]
then
    ReflectionRoots="-x NoThrowInline.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NoThrowInline.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NoThrowInline.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NoThrowInline.asm.exe $CLRTestExecutionArguments
