...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Inlining\InlineGCStruct InlineGCStruct.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Inlining\InlineGCStruct InlineGCStruct.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=InlineGCStruct.dasm.il InlineGCStruct.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=InlineGCStruct.dasm.il InlineGCStruct.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=InlineGCStruct.asm.exe  InlineGCStruct.dasm.il
    %CORE_ROOT%\ilasm.exe /output=InlineGCStruct.asm.exe  InlineGCStruct.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=InlineGCStruct
    if not exist "InlineGCStruct.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "InlineGCStruct.org" (
            mkdir IL
            copy InlineGCStruct.exe IL\InlineGCStruct.exe 
            ren InlineGCStruct.exe InlineGCStruct.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InlineGCStruct.org /out InlineGCStruct.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InlineGCStruct.org /out InlineGCStruct.exe
...
set ExePath=InlineGCStruct.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in InlineGCStruct.reflect.xml
    IF EXIST InlineGCStruct.reflect.xml set ReflectionRoots=-x InlineGCStruct.reflect.xml
    ...
    set ExePath=%LinkBin%\InlineGCStruct.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% InlineGCStruct.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% InlineGCStruct.asm.exe %CLRTestExecutionArguments% %Host_Args%
