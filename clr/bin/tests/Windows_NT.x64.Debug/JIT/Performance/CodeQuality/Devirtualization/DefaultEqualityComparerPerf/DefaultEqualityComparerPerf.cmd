...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Devirtualization\DefaultEqualityComparerPerf DefaultEqualityComparerPerf.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\Devirtualization\DefaultEqualityComparerPerf DefaultEqualityComparerPerf.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=DefaultEqualityComparerPerf.dasm.il DefaultEqualityComparerPerf.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=DefaultEqualityComparerPerf.dasm.il DefaultEqualityComparerPerf.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=DefaultEqualityComparerPerf.asm.exe  DefaultEqualityComparerPerf.dasm.il
    %CORE_ROOT%\ilasm.exe /output=DefaultEqualityComparerPerf.asm.exe  DefaultEqualityComparerPerf.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=DefaultEqualityComparerPerf
    if not exist "DefaultEqualityComparerPerf.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "DefaultEqualityComparerPerf.org" (
            mkdir IL
            copy DefaultEqualityComparerPerf.exe IL\DefaultEqualityComparerPerf.exe 
            ren DefaultEqualityComparerPerf.exe DefaultEqualityComparerPerf.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DefaultEqualityComparerPerf.org /out DefaultEqualityComparerPerf.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in DefaultEqualityComparerPerf.org /out DefaultEqualityComparerPerf.exe
...
set ExePath=DefaultEqualityComparerPerf.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in DefaultEqualityComparerPerf.reflect.xml
    IF EXIST DefaultEqualityComparerPerf.reflect.xml set ReflectionRoots=-x DefaultEqualityComparerPerf.reflect.xml
    ...
    set ExePath=%LinkBin%\DefaultEqualityComparerPerf.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% DefaultEqualityComparerPerf.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% DefaultEqualityComparerPerf.asm.exe %CLRTestExecutionArguments% %Host_Args%
