...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Devirtualization/DefaultEqualityComparerPerf DefaultEqualityComparerPerf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/Devirtualization/DefaultEqualityComparerPerf DefaultEqualityComparerPerf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=DefaultEqualityComparerPerf.dasm.il DefaultEqualityComparerPerf.exe
    "$CORE_ROOT/ildasm" -raweh -out=DefaultEqualityComparerPerf.dasm.il DefaultEqualityComparerPerf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=DefaultEqualityComparerPerf.asm.exe  DefaultEqualityComparerPerf.dasm.il
    "$CORE_ROOT/ilasm" -output=DefaultEqualityComparerPerf.asm.exe  DefaultEqualityComparerPerf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=DefaultEqualityComparerPerf
    if [ ! -f DefaultEqualityComparerPerf.org ]; then
        TakeLock
        if [ ! -f DefaultEqualityComparerPerf.org ]; then
          mkdir IL
          cp DefaultEqualityComparerPerf.exe IL/DefaultEqualityComparerPerf.exe
          mv DefaultEqualityComparerPerf.exe DefaultEqualityComparerPerf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DefaultEqualityComparerPerf.org /out DefaultEqualityComparerPerf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in DefaultEqualityComparerPerf.org /out DefaultEqualityComparerPerf.exe
          __cgExitCode=$?
...
ExePath=DefaultEqualityComparerPerf.exe
...
then
  ...
  # Use hints for reflection roots, if provided in DefaultEqualityComparerPerf.reflect.xml
  if [ -f DefaultEqualityComparerPerf.reflect.xml ]
then
    ReflectionRoots="-x DefaultEqualityComparerPerf.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/DefaultEqualityComparerPerf.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" DefaultEqualityComparerPerf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" DefaultEqualityComparerPerf.asm.exe $CLRTestExecutionArguments
