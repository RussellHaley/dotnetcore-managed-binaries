...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/SciMark/SciMark SciMark.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/SciMark/SciMark SciMark.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=SciMark.dasm.il SciMark.exe
    "$CORE_ROOT/ildasm" -raweh -out=SciMark.dasm.il SciMark.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=SciMark.asm.exe  SciMark.dasm.il
    "$CORE_ROOT/ilasm" -output=SciMark.asm.exe  SciMark.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=SciMark
    if [ ! -f SciMark.org ]; then
        TakeLock
        if [ ! -f SciMark.org ]; then
          mkdir IL
          cp SciMark.exe IL/SciMark.exe
          mv SciMark.exe SciMark.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SciMark.org /out SciMark.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SciMark.org /out SciMark.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in SciMark.reflect.xml
  if [ -f SciMark.reflect.xml ]
then
    ReflectionRoots="-x SciMark.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" SciMark.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" SciMark.asm.exe $CLRTestExecutionArguments
