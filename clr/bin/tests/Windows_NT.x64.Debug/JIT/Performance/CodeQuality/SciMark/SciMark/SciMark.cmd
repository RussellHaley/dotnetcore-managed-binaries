...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\SciMark\SciMark SciMark.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\SciMark\SciMark SciMark.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=SciMark.dasm.il SciMark.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=SciMark.dasm.il SciMark.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=SciMark.asm.exe  SciMark.dasm.il
    %CORE_ROOT%\ilasm.exe /output=SciMark.asm.exe  SciMark.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=SciMark
    if not exist "SciMark.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "SciMark.org" (
            mkdir IL
            copy SciMark.exe IL\SciMark.exe 
            ren SciMark.exe SciMark.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SciMark.org /out SciMark.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SciMark.org /out SciMark.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in SciMark.reflect.xml
    IF EXIST SciMark.reflect.xml set ReflectionRoots=-x SciMark.reflect.xml
    ...
    set ExePath=%LinkBin%\SciMark.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% SciMark.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% SciMark.asm.exe %CLRTestExecutionArguments% %Host_Args%
