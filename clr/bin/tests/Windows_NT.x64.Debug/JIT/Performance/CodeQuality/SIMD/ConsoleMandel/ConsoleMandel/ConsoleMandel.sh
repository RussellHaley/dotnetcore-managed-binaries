...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/SIMD/ConsoleMandel/ConsoleMandel ConsoleMandel.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/SIMD/ConsoleMandel/ConsoleMandel ConsoleMandel.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ConsoleMandel.dasm.il ConsoleMandel.exe
    "$CORE_ROOT/ildasm" -raweh -out=ConsoleMandel.dasm.il ConsoleMandel.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ConsoleMandel.asm.exe  ConsoleMandel.dasm.il
    "$CORE_ROOT/ilasm" -output=ConsoleMandel.asm.exe  ConsoleMandel.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ConsoleMandel
    if [ ! -f ConsoleMandel.org ]; then
        TakeLock
        if [ ! -f ConsoleMandel.org ]; then
          mkdir IL
          cp ConsoleMandel.exe IL/ConsoleMandel.exe
          mv ConsoleMandel.exe ConsoleMandel.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ConsoleMandel.org /out ConsoleMandel.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ConsoleMandel.org /out ConsoleMandel.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ConsoleMandel.reflect.xml
  if [ -f ConsoleMandel.reflect.xml ]
then
    ReflectionRoots="-x ConsoleMandel.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ConsoleMandel.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ConsoleMandel.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ConsoleMandel.asm.exe $CLRTestExecutionArguments
