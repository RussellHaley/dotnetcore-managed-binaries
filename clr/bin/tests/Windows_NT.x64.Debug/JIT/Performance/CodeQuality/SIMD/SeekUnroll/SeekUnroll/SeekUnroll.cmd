...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\SIMD\SeekUnroll\SeekUnroll SeekUnroll.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\SIMD\SeekUnroll\SeekUnroll SeekUnroll.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=SeekUnroll.dasm.il SeekUnroll.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=SeekUnroll.dasm.il SeekUnroll.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=SeekUnroll.asm.exe  SeekUnroll.dasm.il
    %CORE_ROOT%\ilasm.exe /output=SeekUnroll.asm.exe  SeekUnroll.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=SeekUnroll
    if not exist "SeekUnroll.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "SeekUnroll.org" (
            mkdir IL
            copy SeekUnroll.exe IL\SeekUnroll.exe 
            ren SeekUnroll.exe SeekUnroll.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SeekUnroll.org /out SeekUnroll.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SeekUnroll.org /out SeekUnroll.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in SeekUnroll.reflect.xml
    IF EXIST SeekUnroll.reflect.xml set ReflectionRoots=-x SeekUnroll.reflect.xml
    ...
    set ExePath=%LinkBin%\SeekUnroll.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% SeekUnroll.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% SeekUnroll.asm.exe %CLRTestExecutionArguments% %Host_Args%
