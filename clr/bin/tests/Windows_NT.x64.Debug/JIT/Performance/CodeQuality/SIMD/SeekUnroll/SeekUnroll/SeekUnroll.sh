...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/SIMD/SeekUnroll/SeekUnroll SeekUnroll.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/SIMD/SeekUnroll/SeekUnroll SeekUnroll.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=SeekUnroll.dasm.il SeekUnroll.exe
    "$CORE_ROOT/ildasm" -raweh -out=SeekUnroll.dasm.il SeekUnroll.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=SeekUnroll.asm.exe  SeekUnroll.dasm.il
    "$CORE_ROOT/ilasm" -output=SeekUnroll.asm.exe  SeekUnroll.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=SeekUnroll
    if [ ! -f SeekUnroll.org ]; then
        TakeLock
        if [ ! -f SeekUnroll.org ]; then
          mkdir IL
          cp SeekUnroll.exe IL/SeekUnroll.exe
          mv SeekUnroll.exe SeekUnroll.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SeekUnroll.org /out SeekUnroll.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SeekUnroll.org /out SeekUnroll.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in SeekUnroll.reflect.xml
  if [ -f SeekUnroll.reflect.xml ]
then
    ReflectionRoots="-x SeekUnroll.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/SeekUnroll.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" SeekUnroll.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" SeekUnroll.asm.exe $CLRTestExecutionArguments
