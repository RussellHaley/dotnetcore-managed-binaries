...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\SIMD\RayTracer\RayTracer RayTracer.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\SIMD\RayTracer\RayTracer RayTracer.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=RayTracer.dasm.il RayTracer.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=RayTracer.dasm.il RayTracer.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=RayTracer.asm.exe  RayTracer.dasm.il
    %CORE_ROOT%\ilasm.exe /output=RayTracer.asm.exe  RayTracer.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=RayTracer
    if not exist "RayTracer.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "RayTracer.org" (
            mkdir IL
            copy RayTracer.exe IL\RayTracer.exe 
            ren RayTracer.exe RayTracer.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RayTracer.org /out RayTracer.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RayTracer.org /out RayTracer.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in RayTracer.reflect.xml
    IF EXIST RayTracer.reflect.xml set ReflectionRoots=-x RayTracer.reflect.xml
    ...
    set ExePath=%LinkBin%\RayTracer.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% RayTracer.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% RayTracer.asm.exe %CLRTestExecutionArguments% %Host_Args%
