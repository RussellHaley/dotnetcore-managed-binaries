...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\SIMD\ConsoleMandel\ConsoleMandel ConsoleMandel.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Performance\CodeQuality\SIMD\ConsoleMandel\ConsoleMandel ConsoleMandel.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ConsoleMandel.dasm.il ConsoleMandel.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ConsoleMandel.dasm.il ConsoleMandel.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ConsoleMandel.asm.exe  ConsoleMandel.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ConsoleMandel.asm.exe  ConsoleMandel.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ConsoleMandel
    if not exist "ConsoleMandel.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ConsoleMandel.org" (
            mkdir IL
            copy ConsoleMandel.exe IL\ConsoleMandel.exe 
            ren ConsoleMandel.exe ConsoleMandel.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ConsoleMandel.org /out ConsoleMandel.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ConsoleMandel.org /out ConsoleMandel.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ConsoleMandel.reflect.xml
    IF EXIST ConsoleMandel.reflect.xml set ReflectionRoots=-x ConsoleMandel.reflect.xml
    ...
    set ExePath=%LinkBin%\ConsoleMandel.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ConsoleMandel.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ConsoleMandel.asm.exe %CLRTestExecutionArguments% %Host_Args%
