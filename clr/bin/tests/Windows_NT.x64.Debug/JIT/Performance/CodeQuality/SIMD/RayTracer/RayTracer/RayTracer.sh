...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/SIMD/RayTracer/RayTracer RayTracer.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Performance/CodeQuality/SIMD/RayTracer/RayTracer RayTracer.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=RayTracer.dasm.il RayTracer.exe
    "$CORE_ROOT/ildasm" -raweh -out=RayTracer.dasm.il RayTracer.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=RayTracer.asm.exe  RayTracer.dasm.il
    "$CORE_ROOT/ilasm" -output=RayTracer.asm.exe  RayTracer.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=RayTracer
    if [ ! -f RayTracer.org ]; then
        TakeLock
        if [ ! -f RayTracer.org ]; then
          mkdir IL
          cp RayTracer.exe IL/RayTracer.exe
          mv RayTracer.exe RayTracer.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RayTracer.org /out RayTracer.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RayTracer.org /out RayTracer.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in RayTracer.reflect.xml
  if [ -f RayTracer.reflect.xml ]
then
    ReflectionRoots="-x RayTracer.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" RayTracer.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" RayTracer.asm.exe $CLRTestExecutionArguments
