...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\FractalPerf\FractalPerf FractalPerf.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Performance\CodeQuality\FractalPerf\FractalPerf FractalPerf.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=FractalPerf.dasm.il FractalPerf.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=FractalPerf.dasm.il FractalPerf.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=FractalPerf.asm.exe  FractalPerf.dasm.il
    %CORE_ROOT%\ilasm.exe /output=FractalPerf.asm.exe  FractalPerf.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=FractalPerf
    if not exist "FractalPerf.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "FractalPerf.org" (
            mkdir IL
            copy FractalPerf.exe IL\FractalPerf.exe 
            ren FractalPerf.exe FractalPerf.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FractalPerf.org /out FractalPerf.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FractalPerf.org /out FractalPerf.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in FractalPerf.reflect.xml
    IF EXIST FractalPerf.reflect.xml set ReflectionRoots=-x FractalPerf.reflect.xml
    ...
    set ExePath=%LinkBin%\FractalPerf.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% FractalPerf.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% FractalPerf.asm.exe %CLRTestExecutionArguments% %Host_Args%
