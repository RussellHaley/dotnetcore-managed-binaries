...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/FractalPerf/FractalPerf FractalPerf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Performance/CodeQuality/FractalPerf/FractalPerf FractalPerf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=FractalPerf.dasm.il FractalPerf.exe
    "$CORE_ROOT/ildasm" -raweh -out=FractalPerf.dasm.il FractalPerf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=FractalPerf.asm.exe  FractalPerf.dasm.il
    "$CORE_ROOT/ilasm" -output=FractalPerf.asm.exe  FractalPerf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=FractalPerf
    if [ ! -f FractalPerf.org ]; then
        TakeLock
        if [ ! -f FractalPerf.org ]; then
          mkdir IL
          cp FractalPerf.exe IL/FractalPerf.exe
          mv FractalPerf.exe FractalPerf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FractalPerf.org /out FractalPerf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FractalPerf.org /out FractalPerf.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in FractalPerf.reflect.xml
  if [ -f FractalPerf.reflect.xml ]
then
    ReflectionRoots="-x FractalPerf.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/FractalPerf.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" FractalPerf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" FractalPerf.asm.exe $CLRTestExecutionArguments
