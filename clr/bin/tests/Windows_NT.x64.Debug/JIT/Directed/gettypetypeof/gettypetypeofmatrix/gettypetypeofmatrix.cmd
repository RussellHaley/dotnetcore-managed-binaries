...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\gettypetypeof\gettypetypeofmatrix gettypetypeofmatrix.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\gettypetypeof\gettypetypeofmatrix gettypetypeofmatrix.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gettypetypeofmatrix.dasm.il gettypetypeofmatrix.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gettypetypeofmatrix.dasm.il gettypetypeofmatrix.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gettypetypeofmatrix.asm.exe  gettypetypeofmatrix.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gettypetypeofmatrix.asm.exe  gettypetypeofmatrix.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gettypetypeofmatrix
    if not exist "gettypetypeofmatrix.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gettypetypeofmatrix.org" (
            mkdir IL
            copy gettypetypeofmatrix.exe IL\gettypetypeofmatrix.exe 
            ren gettypetypeofmatrix.exe gettypetypeofmatrix.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gettypetypeofmatrix.org /out gettypetypeofmatrix.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gettypetypeofmatrix.org /out gettypetypeofmatrix.exe
...
set ExePath=gettypetypeofmatrix.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gettypetypeofmatrix.reflect.xml
    IF EXIST gettypetypeofmatrix.reflect.xml set ReflectionRoots=-x gettypetypeofmatrix.reflect.xml
    ...
    set ExePath=%LinkBin%\gettypetypeofmatrix.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gettypetypeofmatrix.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gettypetypeofmatrix.asm.exe %CLRTestExecutionArguments% %Host_Args%
