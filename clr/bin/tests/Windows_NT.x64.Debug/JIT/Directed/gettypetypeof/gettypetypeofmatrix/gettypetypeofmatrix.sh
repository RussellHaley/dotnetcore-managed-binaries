...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/gettypetypeof/gettypetypeofmatrix gettypetypeofmatrix.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/gettypetypeof/gettypetypeofmatrix gettypetypeofmatrix.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gettypetypeofmatrix.dasm.il gettypetypeofmatrix.exe
    "$CORE_ROOT/ildasm" -raweh -out=gettypetypeofmatrix.dasm.il gettypetypeofmatrix.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gettypetypeofmatrix.asm.exe  gettypetypeofmatrix.dasm.il
    "$CORE_ROOT/ilasm" -output=gettypetypeofmatrix.asm.exe  gettypetypeofmatrix.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gettypetypeofmatrix
    if [ ! -f gettypetypeofmatrix.org ]; then
        TakeLock
        if [ ! -f gettypetypeofmatrix.org ]; then
          mkdir IL
          cp gettypetypeofmatrix.exe IL/gettypetypeofmatrix.exe
          mv gettypetypeofmatrix.exe gettypetypeofmatrix.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gettypetypeofmatrix.org /out gettypetypeofmatrix.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gettypetypeofmatrix.org /out gettypetypeofmatrix.exe
          __cgExitCode=$?
...
ExePath=gettypetypeofmatrix.exe
...
then
  ...
  # Use hints for reflection roots, if provided in gettypetypeofmatrix.reflect.xml
  if [ -f gettypetypeofmatrix.reflect.xml ]
then
    ReflectionRoots="-x gettypetypeofmatrix.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/gettypetypeofmatrix.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gettypetypeofmatrix.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gettypetypeofmatrix.asm.exe $CLRTestExecutionArguments
