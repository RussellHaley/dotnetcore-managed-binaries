...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=newarr.dasm.il newarr.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=newarr.dasm.il newarr.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=newarr.asm.exe  newarr.dasm.il
    %CORE_ROOT%\ilasm.exe /output=newarr.asm.exe  newarr.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=newarr
    if not exist "newarr.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "newarr.org" (
            mkdir IL
            copy newarr.exe IL\newarr.exe 
            ren newarr.exe newarr.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in newarr.org /out newarr.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in newarr.org /out newarr.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in newarr.reflect.xml
    IF EXIST newarr.reflect.xml set ReflectionRoots=-x newarr.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% newarr.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% newarr.asm.exe %CLRTestExecutionArguments% %Host_Args%
