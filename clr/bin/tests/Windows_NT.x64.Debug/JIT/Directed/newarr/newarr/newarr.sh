...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/newarr/newarr newarr.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/newarr/newarr newarr.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=newarr.dasm.il newarr.exe
    "$CORE_ROOT/ildasm" -raweh -out=newarr.dasm.il newarr.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=newarr.asm.exe  newarr.dasm.il
    "$CORE_ROOT/ilasm" -output=newarr.asm.exe  newarr.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=newarr
    if [ ! -f newarr.org ]; then
        TakeLock
        if [ ! -f newarr.org ]; then
          mkdir IL
          cp newarr.exe IL/newarr.exe
          mv newarr.exe newarr.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in newarr.org /out newarr.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in newarr.org /out newarr.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in newarr.reflect.xml
  if [ -f newarr.reflect.xml ]
then
    ReflectionRoots="-x newarr.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" newarr.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" newarr.asm.exe $CLRTestExecutionArguments
