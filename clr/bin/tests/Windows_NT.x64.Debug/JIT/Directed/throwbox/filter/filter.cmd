...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\throwbox\filter filter.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\throwbox\filter filter.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=filter.dasm.il filter.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=filter.dasm.il filter.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=filter.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE filter.dasm.il
    %CORE_ROOT%\ilasm.exe /output=filter.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE filter.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=filter
    if not exist "filter.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "filter.org" (
            mkdir IL
            copy filter.exe IL\filter.exe 
            ren filter.exe filter.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in filter.org /out filter.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in filter.org /out filter.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in filter.reflect.xml
    IF EXIST filter.reflect.xml set ReflectionRoots=-x filter.reflect.xml
    ...
    set ExePath=%LinkBin%\filter.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% filter.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% filter.asm.exe %CLRTestExecutionArguments% %Host_Args%
