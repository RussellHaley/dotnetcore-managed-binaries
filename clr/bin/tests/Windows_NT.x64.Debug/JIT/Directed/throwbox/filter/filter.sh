...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/throwbox/filter filter.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/throwbox/filter filter.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=filter.dasm.il filter.exe
    "$CORE_ROOT/ildasm" -raweh -out=filter.dasm.il filter.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=filter.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE filter.dasm.il
    "$CORE_ROOT/ilasm" -output=filter.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE filter.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=filter
    if [ ! -f filter.org ]; then
        TakeLock
        if [ ! -f filter.org ]; then
          mkdir IL
          cp filter.exe IL/filter.exe
          mv filter.exe filter.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in filter.org /out filter.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in filter.org /out filter.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in filter.reflect.xml
  if [ -f filter.reflect.xml ]
then
    ReflectionRoots="-x filter.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" filter.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" filter.asm.exe $CLRTestExecutionArguments
