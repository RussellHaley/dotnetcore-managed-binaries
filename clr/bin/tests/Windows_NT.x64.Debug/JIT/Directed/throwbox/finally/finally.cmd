...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\throwbox\finally finally.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\throwbox\finally finally.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=finally.dasm.il finally.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=finally.dasm.il finally.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=finally.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE finally.dasm.il
    %CORE_ROOT%\ilasm.exe /output=finally.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE finally.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=finally
    if not exist "finally.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "finally.org" (
            mkdir IL
            copy finally.exe IL\finally.exe 
            ren finally.exe finally.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in finally.org /out finally.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in finally.org /out finally.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in finally.reflect.xml
    IF EXIST finally.reflect.xml set ReflectionRoots=-x finally.reflect.xml
    ...
    set ExePath=%LinkBin%\finally.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% finally.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% finally.asm.exe %CLRTestExecutionArguments% %Host_Args%
