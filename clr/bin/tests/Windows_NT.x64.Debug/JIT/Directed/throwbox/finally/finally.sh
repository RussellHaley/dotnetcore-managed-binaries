...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/throwbox/finally finally.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/throwbox/finally finally.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=finally.dasm.il finally.exe
    "$CORE_ROOT/ildasm" -raweh -out=finally.dasm.il finally.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=finally.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE finally.dasm.il
    "$CORE_ROOT/ilasm" -output=finally.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE finally.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=finally
    if [ ! -f finally.org ]; then
        TakeLock
        if [ ! -f finally.org ]; then
          mkdir IL
          cp finally.exe IL/finally.exe
          mv finally.exe finally.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in finally.org /out finally.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in finally.org /out finally.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in finally.reflect.xml
  if [ -f finally.reflect.xml ]
then
    ReflectionRoots="-x finally.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" finally.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" finally.asm.exe $CLRTestExecutionArguments
