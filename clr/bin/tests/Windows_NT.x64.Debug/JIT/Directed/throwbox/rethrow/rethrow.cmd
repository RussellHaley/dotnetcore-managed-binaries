...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\throwbox\rethrow rethrow.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\throwbox\rethrow rethrow.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=rethrow.dasm.il rethrow.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=rethrow.dasm.il rethrow.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=rethrow.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rethrow.dasm.il
    %CORE_ROOT%\ilasm.exe /output=rethrow.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rethrow.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=rethrow
    if not exist "rethrow.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "rethrow.org" (
            mkdir IL
            copy rethrow.exe IL\rethrow.exe 
            ren rethrow.exe rethrow.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrow.org /out rethrow.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrow.org /out rethrow.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in rethrow.reflect.xml
    IF EXIST rethrow.reflect.xml set ReflectionRoots=-x rethrow.reflect.xml
    ...
    set ExePath=%LinkBin%\rethrow.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% rethrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% rethrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
