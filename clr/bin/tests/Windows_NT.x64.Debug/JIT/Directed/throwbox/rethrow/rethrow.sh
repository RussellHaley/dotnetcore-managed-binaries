...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/throwbox/rethrow rethrow.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/throwbox/rethrow rethrow.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rethrow.dasm.il rethrow.exe
    "$CORE_ROOT/ildasm" -raweh -out=rethrow.dasm.il rethrow.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rethrow.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rethrow.dasm.il
    "$CORE_ROOT/ilasm" -output=rethrow.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rethrow.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rethrow
    if [ ! -f rethrow.org ]; then
        TakeLock
        if [ ! -f rethrow.org ]; then
          mkdir IL
          cp rethrow.exe IL/rethrow.exe
          mv rethrow.exe rethrow.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrow.org /out rethrow.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrow.org /out rethrow.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in rethrow.reflect.xml
  if [ -f rethrow.reflect.xml ]
then
    ReflectionRoots="-x rethrow.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rethrow.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rethrow.asm.exe $CLRTestExecutionArguments
