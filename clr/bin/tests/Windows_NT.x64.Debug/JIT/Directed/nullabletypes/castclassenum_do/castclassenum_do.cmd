...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassenum_do.dasm.il castclassenum_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassenum_do.dasm.il castclassenum_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassenum_do.asm.exe  castclassenum_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassenum_do.asm.exe  castclassenum_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassenum_do
    if not exist "castclassenum_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassenum_do.org" (
            mkdir IL
            copy castclassenum_do.exe IL\castclassenum_do.exe 
            ren castclassenum_do.exe castclassenum_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassenum_do.org /out castclassenum_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassenum_do.org /out castclassenum_do.exe
...
set ExePath=castclassenum_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassenum_do.reflect.xml
    IF EXIST castclassenum_do.reflect.xml set ReflectionRoots=-x castclassenum_do.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassenum_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassenum_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassenum_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
