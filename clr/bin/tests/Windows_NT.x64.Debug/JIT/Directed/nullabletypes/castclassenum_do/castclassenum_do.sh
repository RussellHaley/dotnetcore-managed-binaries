...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassenum_do castclassenum_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassenum_do castclassenum_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassenum_do.dasm.il castclassenum_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassenum_do.dasm.il castclassenum_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassenum_do.asm.exe  castclassenum_do.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassenum_do.asm.exe  castclassenum_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassenum_do
    if [ ! -f castclassenum_do.org ]; then
        TakeLock
        if [ ! -f castclassenum_do.org ]; then
          mkdir IL
          cp castclassenum_do.exe IL/castclassenum_do.exe
          mv castclassenum_do.exe castclassenum_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassenum_do.org /out castclassenum_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassenum_do.org /out castclassenum_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in castclassenum_do.reflect.xml
  if [ -f castclassenum_do.reflect.xml ]
then
    ReflectionRoots="-x castclassenum_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassenum_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassenum_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassenum_do.asm.exe $CLRTestExecutionArguments
