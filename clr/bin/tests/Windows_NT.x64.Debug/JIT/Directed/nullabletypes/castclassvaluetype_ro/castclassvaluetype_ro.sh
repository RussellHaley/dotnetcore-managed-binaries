...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassvaluetype_ro castclassvaluetype_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassvaluetype_ro castclassvaluetype_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassvaluetype_ro.dasm.il castclassvaluetype_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassvaluetype_ro.dasm.il castclassvaluetype_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassvaluetype_ro.asm.exe  castclassvaluetype_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassvaluetype_ro.asm.exe  castclassvaluetype_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassvaluetype_ro
    if [ ! -f castclassvaluetype_ro.org ]; then
        TakeLock
        if [ ! -f castclassvaluetype_ro.org ]; then
          mkdir IL
          cp castclassvaluetype_ro.exe IL/castclassvaluetype_ro.exe
          mv castclassvaluetype_ro.exe castclassvaluetype_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassvaluetype_ro.org /out castclassvaluetype_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassvaluetype_ro.org /out castclassvaluetype_ro.exe
          __cgExitCode=$?
...
ExePath=castclassvaluetype_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in castclassvaluetype_ro.reflect.xml
  if [ -f castclassvaluetype_ro.reflect.xml ]
then
    ReflectionRoots="-x castclassvaluetype_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassvaluetype_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassvaluetype_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassvaluetype_ro.asm.exe $CLRTestExecutionArguments
