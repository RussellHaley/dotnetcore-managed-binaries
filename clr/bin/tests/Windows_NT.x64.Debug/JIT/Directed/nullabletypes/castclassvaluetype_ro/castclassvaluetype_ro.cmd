...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassvaluetype_ro.dasm.il castclassvaluetype_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassvaluetype_ro.dasm.il castclassvaluetype_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassvaluetype_ro.asm.exe  castclassvaluetype_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassvaluetype_ro.asm.exe  castclassvaluetype_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassvaluetype_ro
    if not exist "castclassvaluetype_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassvaluetype_ro.org" (
            mkdir IL
            copy castclassvaluetype_ro.exe IL\castclassvaluetype_ro.exe 
            ren castclassvaluetype_ro.exe castclassvaluetype_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassvaluetype_ro.org /out castclassvaluetype_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassvaluetype_ro.org /out castclassvaluetype_ro.exe
...
set ExePath=castclassvaluetype_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassvaluetype_ro.reflect.xml
    IF EXIST castclassvaluetype_ro.reflect.xml set ReflectionRoots=-x castclassvaluetype_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassvaluetype_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassvaluetype_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassvaluetype_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
