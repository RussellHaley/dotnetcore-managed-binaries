...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassvaluetype_do castclassvaluetype_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassvaluetype_do castclassvaluetype_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassvaluetype_do.dasm.il castclassvaluetype_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassvaluetype_do.dasm.il castclassvaluetype_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassvaluetype_do.asm.exe  castclassvaluetype_do.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassvaluetype_do.asm.exe  castclassvaluetype_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassvaluetype_do
    if [ ! -f castclassvaluetype_do.org ]; then
        TakeLock
        if [ ! -f castclassvaluetype_do.org ]; then
          mkdir IL
          cp castclassvaluetype_do.exe IL/castclassvaluetype_do.exe
          mv castclassvaluetype_do.exe castclassvaluetype_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassvaluetype_do.org /out castclassvaluetype_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassvaluetype_do.org /out castclassvaluetype_do.exe
          __cgExitCode=$?
...
ExePath=castclassvaluetype_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in castclassvaluetype_do.reflect.xml
  if [ -f castclassvaluetype_do.reflect.xml ]
then
    ReflectionRoots="-x castclassvaluetype_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassvaluetype_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassvaluetype_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassvaluetype_do.asm.exe $CLRTestExecutionArguments
