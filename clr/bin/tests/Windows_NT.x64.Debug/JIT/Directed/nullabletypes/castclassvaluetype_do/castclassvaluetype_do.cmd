...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\nullabletypes\castclassvaluetype_do castclassvaluetype_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\nullabletypes\castclassvaluetype_do castclassvaluetype_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassvaluetype_do.dasm.il castclassvaluetype_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassvaluetype_do.dasm.il castclassvaluetype_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassvaluetype_do.asm.exe  castclassvaluetype_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassvaluetype_do.asm.exe  castclassvaluetype_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassvaluetype_do
    if not exist "castclassvaluetype_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassvaluetype_do.org" (
            mkdir IL
            copy castclassvaluetype_do.exe IL\castclassvaluetype_do.exe 
            ren castclassvaluetype_do.exe castclassvaluetype_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassvaluetype_do.org /out castclassvaluetype_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassvaluetype_do.org /out castclassvaluetype_do.exe
...
set ExePath=castclassvaluetype_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassvaluetype_do.reflect.xml
    IF EXIST castclassvaluetype_do.reflect.xml set ReflectionRoots=-x castclassvaluetype_do.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassvaluetype_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassvaluetype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassvaluetype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
