...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassenum_ro castclassenum_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassenum_ro castclassenum_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassenum_ro.dasm.il castclassenum_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassenum_ro.dasm.il castclassenum_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassenum_ro.asm.exe  castclassenum_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassenum_ro.asm.exe  castclassenum_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassenum_ro
    if [ ! -f castclassenum_ro.org ]; then
        TakeLock
        if [ ! -f castclassenum_ro.org ]; then
          mkdir IL
          cp castclassenum_ro.exe IL/castclassenum_ro.exe
          mv castclassenum_ro.exe castclassenum_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassenum_ro.org /out castclassenum_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassenum_ro.org /out castclassenum_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in castclassenum_ro.reflect.xml
  if [ -f castclassenum_ro.reflect.xml ]
then
    ReflectionRoots="-x castclassenum_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassenum_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassenum_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassenum_ro.asm.exe $CLRTestExecutionArguments
