...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassenum_ro.dasm.il castclassenum_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassenum_ro.dasm.il castclassenum_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassenum_ro.asm.exe  castclassenum_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassenum_ro.asm.exe  castclassenum_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassenum_ro
    if not exist "castclassenum_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassenum_ro.org" (
            mkdir IL
            copy castclassenum_ro.exe IL\castclassenum_ro.exe 
            ren castclassenum_ro.exe castclassenum_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassenum_ro.org /out castclassenum_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassenum_ro.org /out castclassenum_ro.exe
...
set ExePath=castclassenum_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassenum_ro.reflect.xml
    IF EXIST castclassenum_ro.reflect.xml set ReflectionRoots=-x castclassenum_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassenum_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassenum_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassenum_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
