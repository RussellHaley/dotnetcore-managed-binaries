...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/hashcode_do hashcode_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/hashcode_do hashcode_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=hashcode_do.dasm.il hashcode_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=hashcode_do.dasm.il hashcode_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=hashcode_do.asm.exe  hashcode_do.dasm.il
    "$CORE_ROOT/ilasm" -output=hashcode_do.asm.exe  hashcode_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=hashcode_do
    if [ ! -f hashcode_do.org ]; then
        TakeLock
        if [ ! -f hashcode_do.org ]; then
          mkdir IL
          cp hashcode_do.exe IL/hashcode_do.exe
          mv hashcode_do.exe hashcode_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hashcode_do.org /out hashcode_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hashcode_do.org /out hashcode_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in hashcode_do.reflect.xml
  if [ -f hashcode_do.reflect.xml ]
then
    ReflectionRoots="-x hashcode_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/hashcode_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" hashcode_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" hashcode_do.asm.exe $CLRTestExecutionArguments
