...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=hashcode_do.dasm.il hashcode_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=hashcode_do.dasm.il hashcode_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=hashcode_do.asm.exe  hashcode_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=hashcode_do.asm.exe  hashcode_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=hashcode_do
    if not exist "hashcode_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "hashcode_do.org" (
            mkdir IL
            copy hashcode_do.exe IL\hashcode_do.exe 
            ren hashcode_do.exe hashcode_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hashcode_do.org /out hashcode_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hashcode_do.org /out hashcode_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in hashcode_do.reflect.xml
    IF EXIST hashcode_do.reflect.xml set ReflectionRoots=-x hashcode_do.reflect.xml
    ...
    set ExePath=%LinkBin%\hashcode_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% hashcode_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% hashcode_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
