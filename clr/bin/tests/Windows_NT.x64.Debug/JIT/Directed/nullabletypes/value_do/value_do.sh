...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/value_do value_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/value_do value_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=value_do.dasm.il value_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=value_do.dasm.il value_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=value_do.asm.exe  value_do.dasm.il
    "$CORE_ROOT/ilasm" -output=value_do.asm.exe  value_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=value_do
    if [ ! -f value_do.org ]; then
        TakeLock
        if [ ! -f value_do.org ]; then
          mkdir IL
          cp value_do.exe IL/value_do.exe
          mv value_do.exe value_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in value_do.org /out value_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in value_do.org /out value_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in value_do.reflect.xml
  if [ -f value_do.reflect.xml ]
then
    ReflectionRoots="-x value_do.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" value_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" value_do.asm.exe $CLRTestExecutionArguments
