...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=value_do.dasm.il value_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=value_do.dasm.il value_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=value_do.asm.exe  value_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=value_do.asm.exe  value_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=value_do
    if not exist "value_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "value_do.org" (
            mkdir IL
            copy value_do.exe IL\value_do.exe 
            ren value_do.exe value_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in value_do.org /out value_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in value_do.org /out value_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in value_do.reflect.xml
    IF EXIST value_do.reflect.xml set ReflectionRoots=-x value_do.reflect.xml
    ...
    set ExePath=%LinkBin%\value_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% value_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% value_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
