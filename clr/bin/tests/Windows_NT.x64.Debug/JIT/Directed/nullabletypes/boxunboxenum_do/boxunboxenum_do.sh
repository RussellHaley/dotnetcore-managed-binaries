...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxenum_do boxunboxenum_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxenum_do boxunboxenum_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxenum_do.dasm.il boxunboxenum_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxenum_do.dasm.il boxunboxenum_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxenum_do.asm.exe  boxunboxenum_do.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxenum_do.asm.exe  boxunboxenum_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxenum_do
    if [ ! -f boxunboxenum_do.org ]; then
        TakeLock
        if [ ! -f boxunboxenum_do.org ]; then
          mkdir IL
          cp boxunboxenum_do.exe IL/boxunboxenum_do.exe
          mv boxunboxenum_do.exe boxunboxenum_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxenum_do.org /out boxunboxenum_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxenum_do.org /out boxunboxenum_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxenum_do.reflect.xml
  if [ -f boxunboxenum_do.reflect.xml ]
then
    ReflectionRoots="-x boxunboxenum_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxenum_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxenum_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxenum_do.asm.exe $CLRTestExecutionArguments
