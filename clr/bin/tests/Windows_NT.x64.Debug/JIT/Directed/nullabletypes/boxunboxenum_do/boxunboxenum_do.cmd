...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxenum_do.dasm.il boxunboxenum_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxenum_do.dasm.il boxunboxenum_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxenum_do.asm.exe  boxunboxenum_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxenum_do.asm.exe  boxunboxenum_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxenum_do
    if not exist "boxunboxenum_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxenum_do.org" (
            mkdir IL
            copy boxunboxenum_do.exe IL\boxunboxenum_do.exe 
            ren boxunboxenum_do.exe boxunboxenum_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxenum_do.org /out boxunboxenum_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxenum_do.org /out boxunboxenum_do.exe
...
set ExePath=boxunboxenum_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxenum_do.reflect.xml
    IF EXIST boxunboxenum_do.reflect.xml set ReflectionRoots=-x boxunboxenum_do.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxenum_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxenum_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxenum_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
