...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/value_d value_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/value_d value_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=value_d.dasm.il value_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=value_d.dasm.il value_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=value_d.asm.exe  value_d.dasm.il
    "$CORE_ROOT/ilasm" -output=value_d.asm.exe  value_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=value_d
    if [ ! -f value_d.org ]; then
        TakeLock
        if [ ! -f value_d.org ]; then
          mkdir IL
          cp value_d.exe IL/value_d.exe
          mv value_d.exe value_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in value_d.org /out value_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in value_d.org /out value_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in value_d.reflect.xml
  if [ -f value_d.reflect.xml ]
then
    ReflectionRoots="-x value_d.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" value_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" value_d.asm.exe $CLRTestExecutionArguments
