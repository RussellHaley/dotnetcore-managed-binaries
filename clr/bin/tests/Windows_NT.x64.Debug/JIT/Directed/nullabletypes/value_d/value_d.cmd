...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=value_d.dasm.il value_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=value_d.dasm.il value_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=value_d.asm.exe  value_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=value_d.asm.exe  value_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=value_d
    if not exist "value_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "value_d.org" (
            mkdir IL
            copy value_d.exe IL\value_d.exe 
            ren value_d.exe value_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in value_d.org /out value_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in value_d.org /out value_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in value_d.reflect.xml
    IF EXIST value_d.reflect.xml set ReflectionRoots=-x value_d.reflect.xml
    ...
    set ExePath=%LinkBin%\value_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% value_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% value_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
