...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=hashcode_ro.dasm.il hashcode_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=hashcode_ro.dasm.il hashcode_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=hashcode_ro.asm.exe  hashcode_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=hashcode_ro.asm.exe  hashcode_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=hashcode_ro
    if not exist "hashcode_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "hashcode_ro.org" (
            mkdir IL
            copy hashcode_ro.exe IL\hashcode_ro.exe 
            ren hashcode_ro.exe hashcode_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hashcode_ro.org /out hashcode_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hashcode_ro.org /out hashcode_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in hashcode_ro.reflect.xml
    IF EXIST hashcode_ro.reflect.xml set ReflectionRoots=-x hashcode_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\hashcode_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% hashcode_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% hashcode_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
