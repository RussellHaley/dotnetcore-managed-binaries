...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/hashcode_ro hashcode_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/hashcode_ro hashcode_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=hashcode_ro.dasm.il hashcode_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=hashcode_ro.dasm.il hashcode_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=hashcode_ro.asm.exe  hashcode_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=hashcode_ro.asm.exe  hashcode_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=hashcode_ro
    if [ ! -f hashcode_ro.org ]; then
        TakeLock
        if [ ! -f hashcode_ro.org ]; then
          mkdir IL
          cp hashcode_ro.exe IL/hashcode_ro.exe
          mv hashcode_ro.exe hashcode_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hashcode_ro.org /out hashcode_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hashcode_ro.org /out hashcode_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in hashcode_ro.reflect.xml
  if [ -f hashcode_ro.reflect.xml ]
then
    ReflectionRoots="-x hashcode_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/hashcode_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" hashcode_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" hashcode_ro.asm.exe $CLRTestExecutionArguments
