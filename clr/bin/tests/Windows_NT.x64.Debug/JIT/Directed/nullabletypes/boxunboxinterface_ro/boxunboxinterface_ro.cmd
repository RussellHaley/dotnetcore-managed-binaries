...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxinterface_ro.dasm.il boxunboxinterface_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxinterface_ro.dasm.il boxunboxinterface_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxinterface_ro.asm.exe  boxunboxinterface_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxinterface_ro.asm.exe  boxunboxinterface_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxinterface_ro
    if not exist "boxunboxinterface_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxinterface_ro.org" (
            mkdir IL
            copy boxunboxinterface_ro.exe IL\boxunboxinterface_ro.exe 
            ren boxunboxinterface_ro.exe boxunboxinterface_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxinterface_ro.org /out boxunboxinterface_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxinterface_ro.org /out boxunboxinterface_ro.exe
...
set ExePath=boxunboxinterface_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxinterface_ro.reflect.xml
    IF EXIST boxunboxinterface_ro.reflect.xml set ReflectionRoots=-x boxunboxinterface_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxinterface_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxinterface_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxinterface_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
