...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxinterface_ro boxunboxinterface_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxinterface_ro boxunboxinterface_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxinterface_ro.dasm.il boxunboxinterface_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxinterface_ro.dasm.il boxunboxinterface_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxinterface_ro.asm.exe  boxunboxinterface_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxinterface_ro.asm.exe  boxunboxinterface_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxinterface_ro
    if [ ! -f boxunboxinterface_ro.org ]; then
        TakeLock
        if [ ! -f boxunboxinterface_ro.org ]; then
          mkdir IL
          cp boxunboxinterface_ro.exe IL/boxunboxinterface_ro.exe
          mv boxunboxinterface_ro.exe boxunboxinterface_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxinterface_ro.org /out boxunboxinterface_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxinterface_ro.org /out boxunboxinterface_ro.exe
          __cgExitCode=$?
...
ExePath=boxunboxinterface_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxinterface_ro.reflect.xml
  if [ -f boxunboxinterface_ro.reflect.xml ]
then
    ReflectionRoots="-x boxunboxinterface_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxinterface_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxinterface_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxinterface_ro.asm.exe $CLRTestExecutionArguments
