...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxenum_d.dasm.il boxunboxenum_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxenum_d.dasm.il boxunboxenum_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxenum_d.asm.exe  boxunboxenum_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxenum_d.asm.exe  boxunboxenum_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxenum_d
    if not exist "boxunboxenum_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxenum_d.org" (
            mkdir IL
            copy boxunboxenum_d.exe IL\boxunboxenum_d.exe 
            ren boxunboxenum_d.exe boxunboxenum_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxenum_d.org /out boxunboxenum_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxenum_d.org /out boxunboxenum_d.exe
...
set ExePath=boxunboxenum_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxenum_d.reflect.xml
    IF EXIST boxunboxenum_d.reflect.xml set ReflectionRoots=-x boxunboxenum_d.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxenum_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxenum_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxenum_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
