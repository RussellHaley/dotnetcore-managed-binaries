...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxenum_d boxunboxenum_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxenum_d boxunboxenum_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxenum_d.dasm.il boxunboxenum_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxenum_d.dasm.il boxunboxenum_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxenum_d.asm.exe  boxunboxenum_d.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxenum_d.asm.exe  boxunboxenum_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxenum_d
    if [ ! -f boxunboxenum_d.org ]; then
        TakeLock
        if [ ! -f boxunboxenum_d.org ]; then
          mkdir IL
          cp boxunboxenum_d.exe IL/boxunboxenum_d.exe
          mv boxunboxenum_d.exe boxunboxenum_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxenum_d.org /out boxunboxenum_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxenum_d.org /out boxunboxenum_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxenum_d.reflect.xml
  if [ -f boxunboxenum_d.reflect.xml ]
then
    ReflectionRoots="-x boxunboxenum_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxenum_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxenum_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxenum_d.asm.exe $CLRTestExecutionArguments
