...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxinterface_r boxunboxinterface_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxinterface_r boxunboxinterface_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxinterface_r.dasm.il boxunboxinterface_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxinterface_r.dasm.il boxunboxinterface_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxinterface_r.asm.exe  boxunboxinterface_r.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxinterface_r.asm.exe  boxunboxinterface_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxinterface_r
    if [ ! -f boxunboxinterface_r.org ]; then
        TakeLock
        if [ ! -f boxunboxinterface_r.org ]; then
          mkdir IL
          cp boxunboxinterface_r.exe IL/boxunboxinterface_r.exe
          mv boxunboxinterface_r.exe boxunboxinterface_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxinterface_r.org /out boxunboxinterface_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxinterface_r.org /out boxunboxinterface_r.exe
          __cgExitCode=$?
...
ExePath=boxunboxinterface_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxinterface_r.reflect.xml
  if [ -f boxunboxinterface_r.reflect.xml ]
then
    ReflectionRoots="-x boxunboxinterface_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxinterface_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxinterface_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxinterface_r.asm.exe $CLRTestExecutionArguments
