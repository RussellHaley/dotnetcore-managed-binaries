...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxinterface_r.dasm.il boxunboxinterface_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxinterface_r.dasm.il boxunboxinterface_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxinterface_r.asm.exe  boxunboxinterface_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxinterface_r.asm.exe  boxunboxinterface_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxinterface_r
    if not exist "boxunboxinterface_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxinterface_r.org" (
            mkdir IL
            copy boxunboxinterface_r.exe IL\boxunboxinterface_r.exe 
            ren boxunboxinterface_r.exe boxunboxinterface_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxinterface_r.org /out boxunboxinterface_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxinterface_r.org /out boxunboxinterface_r.exe
...
set ExePath=boxunboxinterface_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxinterface_r.reflect.xml
    IF EXIST boxunboxinterface_r.reflect.xml set ReflectionRoots=-x boxunboxinterface_r.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxinterface_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxinterface_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxinterface_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
