...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/unboxnullable_ro unboxnullable_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/unboxnullable_ro unboxnullable_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=unboxnullable_ro.dasm.il unboxnullable_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=unboxnullable_ro.dasm.il unboxnullable_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=unboxnullable_ro.asm.exe  unboxnullable_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=unboxnullable_ro.asm.exe  unboxnullable_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=unboxnullable_ro
    if [ ! -f unboxnullable_ro.org ]; then
        TakeLock
        if [ ! -f unboxnullable_ro.org ]; then
          mkdir IL
          cp unboxnullable_ro.exe IL/unboxnullable_ro.exe
          mv unboxnullable_ro.exe unboxnullable_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unboxnullable_ro.org /out unboxnullable_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unboxnullable_ro.org /out unboxnullable_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in unboxnullable_ro.reflect.xml
  if [ -f unboxnullable_ro.reflect.xml ]
then
    ReflectionRoots="-x unboxnullable_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/unboxnullable_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" unboxnullable_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" unboxnullable_ro.asm.exe $CLRTestExecutionArguments
