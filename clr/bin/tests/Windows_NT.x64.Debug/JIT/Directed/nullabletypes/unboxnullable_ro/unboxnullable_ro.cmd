...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=unboxnullable_ro.dasm.il unboxnullable_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=unboxnullable_ro.dasm.il unboxnullable_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=unboxnullable_ro.asm.exe  unboxnullable_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=unboxnullable_ro.asm.exe  unboxnullable_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=unboxnullable_ro
    if not exist "unboxnullable_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "unboxnullable_ro.org" (
            mkdir IL
            copy unboxnullable_ro.exe IL\unboxnullable_ro.exe 
            ren unboxnullable_ro.exe unboxnullable_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unboxnullable_ro.org /out unboxnullable_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unboxnullable_ro.org /out unboxnullable_ro.exe
...
set ExePath=unboxnullable_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in unboxnullable_ro.reflect.xml
    IF EXIST unboxnullable_ro.reflect.xml set ReflectionRoots=-x unboxnullable_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\unboxnullable_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% unboxnullable_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% unboxnullable_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
