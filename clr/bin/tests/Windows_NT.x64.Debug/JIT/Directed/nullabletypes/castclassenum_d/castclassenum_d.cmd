...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassenum_d.dasm.il castclassenum_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassenum_d.dasm.il castclassenum_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassenum_d.asm.exe  castclassenum_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassenum_d.asm.exe  castclassenum_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassenum_d
    if not exist "castclassenum_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassenum_d.org" (
            mkdir IL
            copy castclassenum_d.exe IL\castclassenum_d.exe 
            ren castclassenum_d.exe castclassenum_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassenum_d.org /out castclassenum_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassenum_d.org /out castclassenum_d.exe
...
set ExePath=castclassenum_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassenum_d.reflect.xml
    IF EXIST castclassenum_d.reflect.xml set ReflectionRoots=-x castclassenum_d.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassenum_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassenum_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassenum_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
