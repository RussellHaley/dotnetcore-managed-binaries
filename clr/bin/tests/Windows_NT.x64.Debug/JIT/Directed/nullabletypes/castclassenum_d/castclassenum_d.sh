...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassenum_d castclassenum_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassenum_d castclassenum_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassenum_d.dasm.il castclassenum_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassenum_d.dasm.il castclassenum_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassenum_d.asm.exe  castclassenum_d.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassenum_d.asm.exe  castclassenum_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassenum_d
    if [ ! -f castclassenum_d.org ]; then
        TakeLock
        if [ ! -f castclassenum_d.org ]; then
          mkdir IL
          cp castclassenum_d.exe IL/castclassenum_d.exe
          mv castclassenum_d.exe castclassenum_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassenum_d.org /out castclassenum_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassenum_d.org /out castclassenum_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in castclassenum_d.reflect.xml
  if [ -f castclassenum_d.reflect.xml ]
then
    ReflectionRoots="-x castclassenum_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassenum_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassenum_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassenum_d.asm.exe $CLRTestExecutionArguments
