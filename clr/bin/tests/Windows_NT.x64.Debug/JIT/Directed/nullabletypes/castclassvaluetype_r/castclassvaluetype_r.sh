...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassvaluetype_r castclassvaluetype_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassvaluetype_r castclassvaluetype_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassvaluetype_r.dasm.il castclassvaluetype_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassvaluetype_r.dasm.il castclassvaluetype_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassvaluetype_r.asm.exe  castclassvaluetype_r.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassvaluetype_r.asm.exe  castclassvaluetype_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassvaluetype_r
    if [ ! -f castclassvaluetype_r.org ]; then
        TakeLock
        if [ ! -f castclassvaluetype_r.org ]; then
          mkdir IL
          cp castclassvaluetype_r.exe IL/castclassvaluetype_r.exe
          mv castclassvaluetype_r.exe castclassvaluetype_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassvaluetype_r.org /out castclassvaluetype_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassvaluetype_r.org /out castclassvaluetype_r.exe
          __cgExitCode=$?
...
ExePath=castclassvaluetype_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in castclassvaluetype_r.reflect.xml
  if [ -f castclassvaluetype_r.reflect.xml ]
then
    ReflectionRoots="-x castclassvaluetype_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassvaluetype_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassvaluetype_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassvaluetype_r.asm.exe $CLRTestExecutionArguments
