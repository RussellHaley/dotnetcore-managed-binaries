...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassvaluetype_r.dasm.il castclassvaluetype_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassvaluetype_r.dasm.il castclassvaluetype_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassvaluetype_r.asm.exe  castclassvaluetype_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassvaluetype_r.asm.exe  castclassvaluetype_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassvaluetype_r
    if not exist "castclassvaluetype_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassvaluetype_r.org" (
            mkdir IL
            copy castclassvaluetype_r.exe IL\castclassvaluetype_r.exe 
            ren castclassvaluetype_r.exe castclassvaluetype_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassvaluetype_r.org /out castclassvaluetype_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassvaluetype_r.org /out castclassvaluetype_r.exe
...
set ExePath=castclassvaluetype_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassvaluetype_r.reflect.xml
    IF EXIST castclassvaluetype_r.reflect.xml set ReflectionRoots=-x castclassvaluetype_r.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassvaluetype_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassvaluetype_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassvaluetype_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
