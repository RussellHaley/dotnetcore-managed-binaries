...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassvaluetype_d castclassvaluetype_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassvaluetype_d castclassvaluetype_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassvaluetype_d.dasm.il castclassvaluetype_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassvaluetype_d.dasm.il castclassvaluetype_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassvaluetype_d.asm.exe  castclassvaluetype_d.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassvaluetype_d.asm.exe  castclassvaluetype_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassvaluetype_d
    if [ ! -f castclassvaluetype_d.org ]; then
        TakeLock
        if [ ! -f castclassvaluetype_d.org ]; then
          mkdir IL
          cp castclassvaluetype_d.exe IL/castclassvaluetype_d.exe
          mv castclassvaluetype_d.exe castclassvaluetype_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassvaluetype_d.org /out castclassvaluetype_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassvaluetype_d.org /out castclassvaluetype_d.exe
          __cgExitCode=$?
...
ExePath=castclassvaluetype_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in castclassvaluetype_d.reflect.xml
  if [ -f castclassvaluetype_d.reflect.xml ]
then
    ReflectionRoots="-x castclassvaluetype_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassvaluetype_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassvaluetype_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassvaluetype_d.asm.exe $CLRTestExecutionArguments
