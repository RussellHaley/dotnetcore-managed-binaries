...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassvaluetype_d.dasm.il castclassvaluetype_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassvaluetype_d.dasm.il castclassvaluetype_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassvaluetype_d.asm.exe  castclassvaluetype_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassvaluetype_d.asm.exe  castclassvaluetype_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassvaluetype_d
    if not exist "castclassvaluetype_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassvaluetype_d.org" (
            mkdir IL
            copy castclassvaluetype_d.exe IL\castclassvaluetype_d.exe 
            ren castclassvaluetype_d.exe castclassvaluetype_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassvaluetype_d.org /out castclassvaluetype_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassvaluetype_d.org /out castclassvaluetype_d.exe
...
set ExePath=castclassvaluetype_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassvaluetype_d.reflect.xml
    IF EXIST castclassvaluetype_d.reflect.xml set ReflectionRoots=-x castclassvaluetype_d.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassvaluetype_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassvaluetype_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassvaluetype_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
