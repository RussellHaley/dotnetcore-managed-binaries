...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/value_r value_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/value_r value_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=value_r.dasm.il value_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=value_r.dasm.il value_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=value_r.asm.exe  value_r.dasm.il
    "$CORE_ROOT/ilasm" -output=value_r.asm.exe  value_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=value_r
    if [ ! -f value_r.org ]; then
        TakeLock
        if [ ! -f value_r.org ]; then
          mkdir IL
          cp value_r.exe IL/value_r.exe
          mv value_r.exe value_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in value_r.org /out value_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in value_r.org /out value_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in value_r.reflect.xml
  if [ -f value_r.reflect.xml ]
then
    ReflectionRoots="-x value_r.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" value_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" value_r.asm.exe $CLRTestExecutionArguments
