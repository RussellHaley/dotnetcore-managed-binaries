...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=value_r.dasm.il value_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=value_r.dasm.il value_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=value_r.asm.exe  value_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=value_r.asm.exe  value_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=value_r
    if not exist "value_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "value_r.org" (
            mkdir IL
            copy value_r.exe IL\value_r.exe 
            ren value_r.exe value_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in value_r.org /out value_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in value_r.org /out value_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in value_r.reflect.xml
    IF EXIST value_r.reflect.xml set ReflectionRoots=-x value_r.reflect.xml
    ...
    set ExePath=%LinkBin%\value_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% value_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% value_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
