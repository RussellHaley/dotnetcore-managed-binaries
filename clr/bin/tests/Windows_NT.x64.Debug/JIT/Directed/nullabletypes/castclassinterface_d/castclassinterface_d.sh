...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassinterface_d castclassinterface_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassinterface_d castclassinterface_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassinterface_d.dasm.il castclassinterface_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassinterface_d.dasm.il castclassinterface_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassinterface_d.asm.exe  castclassinterface_d.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassinterface_d.asm.exe  castclassinterface_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassinterface_d
    if [ ! -f castclassinterface_d.org ]; then
        TakeLock
        if [ ! -f castclassinterface_d.org ]; then
          mkdir IL
          cp castclassinterface_d.exe IL/castclassinterface_d.exe
          mv castclassinterface_d.exe castclassinterface_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassinterface_d.org /out castclassinterface_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassinterface_d.org /out castclassinterface_d.exe
          __cgExitCode=$?
...
ExePath=castclassinterface_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in castclassinterface_d.reflect.xml
  if [ -f castclassinterface_d.reflect.xml ]
then
    ReflectionRoots="-x castclassinterface_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassinterface_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassinterface_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassinterface_d.asm.exe $CLRTestExecutionArguments
