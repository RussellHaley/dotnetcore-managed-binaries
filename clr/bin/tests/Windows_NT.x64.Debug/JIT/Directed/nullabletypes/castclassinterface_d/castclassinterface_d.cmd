...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassinterface_d.dasm.il castclassinterface_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassinterface_d.dasm.il castclassinterface_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassinterface_d.asm.exe  castclassinterface_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassinterface_d.asm.exe  castclassinterface_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassinterface_d
    if not exist "castclassinterface_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassinterface_d.org" (
            mkdir IL
            copy castclassinterface_d.exe IL\castclassinterface_d.exe 
            ren castclassinterface_d.exe castclassinterface_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassinterface_d.org /out castclassinterface_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassinterface_d.org /out castclassinterface_d.exe
...
set ExePath=castclassinterface_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassinterface_d.reflect.xml
    IF EXIST castclassinterface_d.reflect.xml set ReflectionRoots=-x castclassinterface_d.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassinterface_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassinterface_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassinterface_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
