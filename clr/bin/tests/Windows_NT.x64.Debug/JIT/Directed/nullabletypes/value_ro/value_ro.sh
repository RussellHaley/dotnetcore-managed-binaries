...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/value_ro value_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/value_ro value_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=value_ro.dasm.il value_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=value_ro.dasm.il value_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=value_ro.asm.exe  value_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=value_ro.asm.exe  value_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=value_ro
    if [ ! -f value_ro.org ]; then
        TakeLock
        if [ ! -f value_ro.org ]; then
          mkdir IL
          cp value_ro.exe IL/value_ro.exe
          mv value_ro.exe value_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in value_ro.org /out value_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in value_ro.org /out value_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in value_ro.reflect.xml
  if [ -f value_ro.reflect.xml ]
then
    ReflectionRoots="-x value_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" value_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" value_ro.asm.exe $CLRTestExecutionArguments
