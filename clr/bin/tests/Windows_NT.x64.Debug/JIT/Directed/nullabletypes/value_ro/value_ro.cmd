...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=value_ro.dasm.il value_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=value_ro.dasm.il value_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=value_ro.asm.exe  value_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=value_ro.asm.exe  value_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=value_ro
    if not exist "value_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "value_ro.org" (
            mkdir IL
            copy value_ro.exe IL\value_ro.exe 
            ren value_ro.exe value_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in value_ro.org /out value_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in value_ro.org /out value_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in value_ro.reflect.xml
    IF EXIST value_ro.reflect.xml set ReflectionRoots=-x value_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\value_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% value_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% value_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
