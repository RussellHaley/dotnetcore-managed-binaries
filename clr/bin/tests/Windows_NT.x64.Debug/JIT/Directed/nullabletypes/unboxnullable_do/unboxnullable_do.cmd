...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=unboxnullable_do.dasm.il unboxnullable_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=unboxnullable_do.dasm.il unboxnullable_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=unboxnullable_do.asm.exe  unboxnullable_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=unboxnullable_do.asm.exe  unboxnullable_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=unboxnullable_do
    if not exist "unboxnullable_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "unboxnullable_do.org" (
            mkdir IL
            copy unboxnullable_do.exe IL\unboxnullable_do.exe 
            ren unboxnullable_do.exe unboxnullable_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unboxnullable_do.org /out unboxnullable_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unboxnullable_do.org /out unboxnullable_do.exe
...
set ExePath=unboxnullable_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in unboxnullable_do.reflect.xml
    IF EXIST unboxnullable_do.reflect.xml set ReflectionRoots=-x unboxnullable_do.reflect.xml
    ...
    set ExePath=%LinkBin%\unboxnullable_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% unboxnullable_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% unboxnullable_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
