...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/unboxnullable_do unboxnullable_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/unboxnullable_do unboxnullable_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=unboxnullable_do.dasm.il unboxnullable_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=unboxnullable_do.dasm.il unboxnullable_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=unboxnullable_do.asm.exe  unboxnullable_do.dasm.il
    "$CORE_ROOT/ilasm" -output=unboxnullable_do.asm.exe  unboxnullable_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=unboxnullable_do
    if [ ! -f unboxnullable_do.org ]; then
        TakeLock
        if [ ! -f unboxnullable_do.org ]; then
          mkdir IL
          cp unboxnullable_do.exe IL/unboxnullable_do.exe
          mv unboxnullable_do.exe unboxnullable_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unboxnullable_do.org /out unboxnullable_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unboxnullable_do.org /out unboxnullable_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in unboxnullable_do.reflect.xml
  if [ -f unboxnullable_do.reflect.xml ]
then
    ReflectionRoots="-x unboxnullable_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/unboxnullable_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" unboxnullable_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" unboxnullable_do.asm.exe $CLRTestExecutionArguments
