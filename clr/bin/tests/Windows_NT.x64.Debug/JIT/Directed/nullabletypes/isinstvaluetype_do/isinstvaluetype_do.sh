...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/isinstvaluetype_do isinstvaluetype_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/isinstvaluetype_do isinstvaluetype_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=isinstvaluetype_do.dasm.il isinstvaluetype_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=isinstvaluetype_do.dasm.il isinstvaluetype_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=isinstvaluetype_do.asm.exe  isinstvaluetype_do.dasm.il
    "$CORE_ROOT/ilasm" -output=isinstvaluetype_do.asm.exe  isinstvaluetype_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=isinstvaluetype_do
    if [ ! -f isinstvaluetype_do.org ]; then
        TakeLock
        if [ ! -f isinstvaluetype_do.org ]; then
          mkdir IL
          cp isinstvaluetype_do.exe IL/isinstvaluetype_do.exe
          mv isinstvaluetype_do.exe isinstvaluetype_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in isinstvaluetype_do.org /out isinstvaluetype_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in isinstvaluetype_do.org /out isinstvaluetype_do.exe
          __cgExitCode=$?
...
ExePath=isinstvaluetype_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in isinstvaluetype_do.reflect.xml
  if [ -f isinstvaluetype_do.reflect.xml ]
then
    ReflectionRoots="-x isinstvaluetype_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/isinstvaluetype_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" isinstvaluetype_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" isinstvaluetype_do.asm.exe $CLRTestExecutionArguments
