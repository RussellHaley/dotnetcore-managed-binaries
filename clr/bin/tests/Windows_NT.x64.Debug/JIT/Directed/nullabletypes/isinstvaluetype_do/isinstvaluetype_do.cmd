...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=isinstvaluetype_do.dasm.il isinstvaluetype_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=isinstvaluetype_do.dasm.il isinstvaluetype_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=isinstvaluetype_do.asm.exe  isinstvaluetype_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=isinstvaluetype_do.asm.exe  isinstvaluetype_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=isinstvaluetype_do
    if not exist "isinstvaluetype_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "isinstvaluetype_do.org" (
            mkdir IL
            copy isinstvaluetype_do.exe IL\isinstvaluetype_do.exe 
            ren isinstvaluetype_do.exe isinstvaluetype_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in isinstvaluetype_do.org /out isinstvaluetype_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in isinstvaluetype_do.org /out isinstvaluetype_do.exe
...
set ExePath=isinstvaluetype_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in isinstvaluetype_do.reflect.xml
    IF EXIST isinstvaluetype_do.reflect.xml set ReflectionRoots=-x isinstvaluetype_do.reflect.xml
    ...
    set ExePath=%LinkBin%\isinstvaluetype_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% isinstvaluetype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% isinstvaluetype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
