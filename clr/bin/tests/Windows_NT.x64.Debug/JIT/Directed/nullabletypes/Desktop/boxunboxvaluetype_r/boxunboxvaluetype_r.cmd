...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxvaluetype_r.dasm.il boxunboxvaluetype_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxvaluetype_r.dasm.il boxunboxvaluetype_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxvaluetype_r.asm.exe  boxunboxvaluetype_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxvaluetype_r.asm.exe  boxunboxvaluetype_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxvaluetype_r
    if not exist "boxunboxvaluetype_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxvaluetype_r.org" (
            mkdir IL
            copy boxunboxvaluetype_r.exe IL\boxunboxvaluetype_r.exe 
            ren boxunboxvaluetype_r.exe boxunboxvaluetype_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxvaluetype_r.org /out boxunboxvaluetype_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxvaluetype_r.org /out boxunboxvaluetype_r.exe
...
set ExePath=boxunboxvaluetype_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxvaluetype_r.reflect.xml
    IF EXIST boxunboxvaluetype_r.reflect.xml set ReflectionRoots=-x boxunboxvaluetype_r.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxvaluetype_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxvaluetype_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxvaluetype_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
