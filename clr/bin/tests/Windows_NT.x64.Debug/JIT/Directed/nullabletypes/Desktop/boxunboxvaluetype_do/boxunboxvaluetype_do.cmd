...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxvaluetype_do.dasm.il boxunboxvaluetype_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxvaluetype_do.dasm.il boxunboxvaluetype_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxvaluetype_do.asm.exe  boxunboxvaluetype_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxvaluetype_do.asm.exe  boxunboxvaluetype_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxvaluetype_do
    if not exist "boxunboxvaluetype_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxvaluetype_do.org" (
            mkdir IL
            copy boxunboxvaluetype_do.exe IL\boxunboxvaluetype_do.exe 
            ren boxunboxvaluetype_do.exe boxunboxvaluetype_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxvaluetype_do.org /out boxunboxvaluetype_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxvaluetype_do.org /out boxunboxvaluetype_do.exe
...
set ExePath=boxunboxvaluetype_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxvaluetype_do.reflect.xml
    IF EXIST boxunboxvaluetype_do.reflect.xml set ReflectionRoots=-x boxunboxvaluetype_do.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxvaluetype_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxvaluetype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxvaluetype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
