...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/nullabletypes/Desktop/boxunboxvaluetype_r boxunboxvaluetype_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/nullabletypes/Desktop/boxunboxvaluetype_r boxunboxvaluetype_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxvaluetype_r.dasm.il boxunboxvaluetype_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxvaluetype_r.dasm.il boxunboxvaluetype_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxvaluetype_r.asm.exe  boxunboxvaluetype_r.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxvaluetype_r.asm.exe  boxunboxvaluetype_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxvaluetype_r
    if [ ! -f boxunboxvaluetype_r.org ]; then
        TakeLock
        if [ ! -f boxunboxvaluetype_r.org ]; then
          mkdir IL
          cp boxunboxvaluetype_r.exe IL/boxunboxvaluetype_r.exe
          mv boxunboxvaluetype_r.exe boxunboxvaluetype_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxvaluetype_r.org /out boxunboxvaluetype_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxvaluetype_r.org /out boxunboxvaluetype_r.exe
          __cgExitCode=$?
...
ExePath=boxunboxvaluetype_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxvaluetype_r.reflect.xml
  if [ -f boxunboxvaluetype_r.reflect.xml ]
then
    ReflectionRoots="-x boxunboxvaluetype_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxvaluetype_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxvaluetype_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxvaluetype_r.asm.exe $CLRTestExecutionArguments
