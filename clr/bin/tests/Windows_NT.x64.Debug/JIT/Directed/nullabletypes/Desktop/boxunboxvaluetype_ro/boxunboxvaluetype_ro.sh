...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/nullabletypes/Desktop/boxunboxvaluetype_ro boxunboxvaluetype_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/nullabletypes/Desktop/boxunboxvaluetype_ro boxunboxvaluetype_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxvaluetype_ro.dasm.il boxunboxvaluetype_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxvaluetype_ro.dasm.il boxunboxvaluetype_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxvaluetype_ro.asm.exe  boxunboxvaluetype_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxvaluetype_ro.asm.exe  boxunboxvaluetype_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxvaluetype_ro
    if [ ! -f boxunboxvaluetype_ro.org ]; then
        TakeLock
        if [ ! -f boxunboxvaluetype_ro.org ]; then
          mkdir IL
          cp boxunboxvaluetype_ro.exe IL/boxunboxvaluetype_ro.exe
          mv boxunboxvaluetype_ro.exe boxunboxvaluetype_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxvaluetype_ro.org /out boxunboxvaluetype_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxvaluetype_ro.org /out boxunboxvaluetype_ro.exe
          __cgExitCode=$?
...
ExePath=boxunboxvaluetype_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxvaluetype_ro.reflect.xml
  if [ -f boxunboxvaluetype_ro.reflect.xml ]
then
    ReflectionRoots="-x boxunboxvaluetype_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxvaluetype_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxvaluetype_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxvaluetype_ro.asm.exe $CLRTestExecutionArguments
