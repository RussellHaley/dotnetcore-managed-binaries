...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/nullabletypes/Desktop/boxunboxvaluetype_do boxunboxvaluetype_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/nullabletypes/Desktop/boxunboxvaluetype_do boxunboxvaluetype_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxvaluetype_do.dasm.il boxunboxvaluetype_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxvaluetype_do.dasm.il boxunboxvaluetype_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxvaluetype_do.asm.exe  boxunboxvaluetype_do.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxvaluetype_do.asm.exe  boxunboxvaluetype_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxvaluetype_do
    if [ ! -f boxunboxvaluetype_do.org ]; then
        TakeLock
        if [ ! -f boxunboxvaluetype_do.org ]; then
          mkdir IL
          cp boxunboxvaluetype_do.exe IL/boxunboxvaluetype_do.exe
          mv boxunboxvaluetype_do.exe boxunboxvaluetype_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxvaluetype_do.org /out boxunboxvaluetype_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxvaluetype_do.org /out boxunboxvaluetype_do.exe
          __cgExitCode=$?
...
ExePath=boxunboxvaluetype_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxvaluetype_do.reflect.xml
  if [ -f boxunboxvaluetype_do.reflect.xml ]
then
    ReflectionRoots="-x boxunboxvaluetype_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxvaluetype_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxvaluetype_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxvaluetype_do.asm.exe $CLRTestExecutionArguments
