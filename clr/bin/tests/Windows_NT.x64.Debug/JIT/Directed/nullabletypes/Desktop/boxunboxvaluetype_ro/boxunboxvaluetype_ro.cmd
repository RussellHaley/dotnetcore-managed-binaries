...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxvaluetype_ro.dasm.il boxunboxvaluetype_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxvaluetype_ro.dasm.il boxunboxvaluetype_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxvaluetype_ro.asm.exe  boxunboxvaluetype_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxvaluetype_ro.asm.exe  boxunboxvaluetype_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxvaluetype_ro
    if not exist "boxunboxvaluetype_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxvaluetype_ro.org" (
            mkdir IL
            copy boxunboxvaluetype_ro.exe IL\boxunboxvaluetype_ro.exe 
            ren boxunboxvaluetype_ro.exe boxunboxvaluetype_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxvaluetype_ro.org /out boxunboxvaluetype_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxvaluetype_ro.org /out boxunboxvaluetype_ro.exe
...
set ExePath=boxunboxvaluetype_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxvaluetype_ro.reflect.xml
    IF EXIST boxunboxvaluetype_ro.reflect.xml set ReflectionRoots=-x boxunboxvaluetype_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxvaluetype_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxvaluetype_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxvaluetype_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
