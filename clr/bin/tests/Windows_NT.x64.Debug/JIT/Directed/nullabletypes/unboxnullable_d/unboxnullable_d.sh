...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/unboxnullable_d unboxnullable_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/unboxnullable_d unboxnullable_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=unboxnullable_d.dasm.il unboxnullable_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=unboxnullable_d.dasm.il unboxnullable_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=unboxnullable_d.asm.exe  unboxnullable_d.dasm.il
    "$CORE_ROOT/ilasm" -output=unboxnullable_d.asm.exe  unboxnullable_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=unboxnullable_d
    if [ ! -f unboxnullable_d.org ]; then
        TakeLock
        if [ ! -f unboxnullable_d.org ]; then
          mkdir IL
          cp unboxnullable_d.exe IL/unboxnullable_d.exe
          mv unboxnullable_d.exe unboxnullable_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unboxnullable_d.org /out unboxnullable_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unboxnullable_d.org /out unboxnullable_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in unboxnullable_d.reflect.xml
  if [ -f unboxnullable_d.reflect.xml ]
then
    ReflectionRoots="-x unboxnullable_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/unboxnullable_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" unboxnullable_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" unboxnullable_d.asm.exe $CLRTestExecutionArguments
