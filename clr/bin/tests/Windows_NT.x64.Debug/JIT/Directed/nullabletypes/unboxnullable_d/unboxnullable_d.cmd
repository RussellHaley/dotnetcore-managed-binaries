...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=unboxnullable_d.dasm.il unboxnullable_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=unboxnullable_d.dasm.il unboxnullable_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=unboxnullable_d.asm.exe  unboxnullable_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=unboxnullable_d.asm.exe  unboxnullable_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=unboxnullable_d
    if not exist "unboxnullable_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "unboxnullable_d.org" (
            mkdir IL
            copy unboxnullable_d.exe IL\unboxnullable_d.exe 
            ren unboxnullable_d.exe unboxnullable_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unboxnullable_d.org /out unboxnullable_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unboxnullable_d.org /out unboxnullable_d.exe
...
set ExePath=unboxnullable_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in unboxnullable_d.reflect.xml
    IF EXIST unboxnullable_d.reflect.xml set ReflectionRoots=-x unboxnullable_d.reflect.xml
    ...
    set ExePath=%LinkBin%\unboxnullable_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% unboxnullable_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% unboxnullable_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
