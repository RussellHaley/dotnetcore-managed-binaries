...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxenum_ro boxunboxenum_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxenum_ro boxunboxenum_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxenum_ro.dasm.il boxunboxenum_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxenum_ro.dasm.il boxunboxenum_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxenum_ro.asm.exe  boxunboxenum_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxenum_ro.asm.exe  boxunboxenum_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxenum_ro
    if [ ! -f boxunboxenum_ro.org ]; then
        TakeLock
        if [ ! -f boxunboxenum_ro.org ]; then
          mkdir IL
          cp boxunboxenum_ro.exe IL/boxunboxenum_ro.exe
          mv boxunboxenum_ro.exe boxunboxenum_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxenum_ro.org /out boxunboxenum_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxenum_ro.org /out boxunboxenum_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxenum_ro.reflect.xml
  if [ -f boxunboxenum_ro.reflect.xml ]
then
    ReflectionRoots="-x boxunboxenum_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxenum_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxenum_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxenum_ro.asm.exe $CLRTestExecutionArguments
