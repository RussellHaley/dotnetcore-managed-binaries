...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxenum_ro.dasm.il boxunboxenum_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxenum_ro.dasm.il boxunboxenum_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxenum_ro.asm.exe  boxunboxenum_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxenum_ro.asm.exe  boxunboxenum_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxenum_ro
    if not exist "boxunboxenum_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxenum_ro.org" (
            mkdir IL
            copy boxunboxenum_ro.exe IL\boxunboxenum_ro.exe 
            ren boxunboxenum_ro.exe boxunboxenum_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxenum_ro.org /out boxunboxenum_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxenum_ro.org /out boxunboxenum_ro.exe
...
set ExePath=boxunboxenum_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxenum_ro.reflect.xml
    IF EXIST boxunboxenum_ro.reflect.xml set ReflectionRoots=-x boxunboxenum_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxenum_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxenum_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxenum_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
