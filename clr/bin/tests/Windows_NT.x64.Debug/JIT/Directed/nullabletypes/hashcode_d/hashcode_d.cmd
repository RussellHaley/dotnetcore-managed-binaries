...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=hashcode_d.dasm.il hashcode_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=hashcode_d.dasm.il hashcode_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=hashcode_d.asm.exe  hashcode_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=hashcode_d.asm.exe  hashcode_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=hashcode_d
    if not exist "hashcode_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "hashcode_d.org" (
            mkdir IL
            copy hashcode_d.exe IL\hashcode_d.exe 
            ren hashcode_d.exe hashcode_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hashcode_d.org /out hashcode_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hashcode_d.org /out hashcode_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in hashcode_d.reflect.xml
    IF EXIST hashcode_d.reflect.xml set ReflectionRoots=-x hashcode_d.reflect.xml
    ...
    set ExePath=%LinkBin%\hashcode_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% hashcode_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% hashcode_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
