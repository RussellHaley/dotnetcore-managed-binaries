...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/hashcode_d hashcode_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/hashcode_d hashcode_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=hashcode_d.dasm.il hashcode_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=hashcode_d.dasm.il hashcode_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=hashcode_d.asm.exe  hashcode_d.dasm.il
    "$CORE_ROOT/ilasm" -output=hashcode_d.asm.exe  hashcode_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=hashcode_d
    if [ ! -f hashcode_d.org ]; then
        TakeLock
        if [ ! -f hashcode_d.org ]; then
          mkdir IL
          cp hashcode_d.exe IL/hashcode_d.exe
          mv hashcode_d.exe hashcode_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hashcode_d.org /out hashcode_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hashcode_d.org /out hashcode_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in hashcode_d.reflect.xml
  if [ -f hashcode_d.reflect.xml ]
then
    ReflectionRoots="-x hashcode_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/hashcode_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" hashcode_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" hashcode_d.asm.exe $CLRTestExecutionArguments
