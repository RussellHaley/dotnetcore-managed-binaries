...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=unboxnullable_r.dasm.il unboxnullable_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=unboxnullable_r.dasm.il unboxnullable_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=unboxnullable_r.asm.exe  unboxnullable_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=unboxnullable_r.asm.exe  unboxnullable_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=unboxnullable_r
    if not exist "unboxnullable_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "unboxnullable_r.org" (
            mkdir IL
            copy unboxnullable_r.exe IL\unboxnullable_r.exe 
            ren unboxnullable_r.exe unboxnullable_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unboxnullable_r.org /out unboxnullable_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unboxnullable_r.org /out unboxnullable_r.exe
...
set ExePath=unboxnullable_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in unboxnullable_r.reflect.xml
    IF EXIST unboxnullable_r.reflect.xml set ReflectionRoots=-x unboxnullable_r.reflect.xml
    ...
    set ExePath=%LinkBin%\unboxnullable_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% unboxnullable_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% unboxnullable_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
