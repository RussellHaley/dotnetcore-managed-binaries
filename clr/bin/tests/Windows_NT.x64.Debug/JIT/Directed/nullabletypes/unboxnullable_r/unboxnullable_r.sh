...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/unboxnullable_r unboxnullable_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/unboxnullable_r unboxnullable_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=unboxnullable_r.dasm.il unboxnullable_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=unboxnullable_r.dasm.il unboxnullable_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=unboxnullable_r.asm.exe  unboxnullable_r.dasm.il
    "$CORE_ROOT/ilasm" -output=unboxnullable_r.asm.exe  unboxnullable_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=unboxnullable_r
    if [ ! -f unboxnullable_r.org ]; then
        TakeLock
        if [ ! -f unboxnullable_r.org ]; then
          mkdir IL
          cp unboxnullable_r.exe IL/unboxnullable_r.exe
          mv unboxnullable_r.exe unboxnullable_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unboxnullable_r.org /out unboxnullable_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unboxnullable_r.org /out unboxnullable_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in unboxnullable_r.reflect.xml
  if [ -f unboxnullable_r.reflect.xml ]
then
    ReflectionRoots="-x unboxnullable_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/unboxnullable_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" unboxnullable_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" unboxnullable_r.asm.exe $CLRTestExecutionArguments
