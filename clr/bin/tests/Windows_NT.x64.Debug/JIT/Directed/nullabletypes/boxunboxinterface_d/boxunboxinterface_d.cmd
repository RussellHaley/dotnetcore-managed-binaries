...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxinterface_d.dasm.il boxunboxinterface_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxinterface_d.dasm.il boxunboxinterface_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxinterface_d.asm.exe  boxunboxinterface_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxinterface_d.asm.exe  boxunboxinterface_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxinterface_d
    if not exist "boxunboxinterface_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxinterface_d.org" (
            mkdir IL
            copy boxunboxinterface_d.exe IL\boxunboxinterface_d.exe 
            ren boxunboxinterface_d.exe boxunboxinterface_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxinterface_d.org /out boxunboxinterface_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxinterface_d.org /out boxunboxinterface_d.exe
...
set ExePath=boxunboxinterface_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxinterface_d.reflect.xml
    IF EXIST boxunboxinterface_d.reflect.xml set ReflectionRoots=-x boxunboxinterface_d.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxinterface_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxinterface_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxinterface_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
