...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxinterface_d boxunboxinterface_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxinterface_d boxunboxinterface_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxinterface_d.dasm.il boxunboxinterface_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxinterface_d.dasm.il boxunboxinterface_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxinterface_d.asm.exe  boxunboxinterface_d.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxinterface_d.asm.exe  boxunboxinterface_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxinterface_d
    if [ ! -f boxunboxinterface_d.org ]; then
        TakeLock
        if [ ! -f boxunboxinterface_d.org ]; then
          mkdir IL
          cp boxunboxinterface_d.exe IL/boxunboxinterface_d.exe
          mv boxunboxinterface_d.exe boxunboxinterface_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxinterface_d.org /out boxunboxinterface_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxinterface_d.org /out boxunboxinterface_d.exe
          __cgExitCode=$?
...
ExePath=boxunboxinterface_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxinterface_d.reflect.xml
  if [ -f boxunboxinterface_d.reflect.xml ]
then
    ReflectionRoots="-x boxunboxinterface_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxinterface_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxinterface_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxinterface_d.asm.exe $CLRTestExecutionArguments
