...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassinterface_do.dasm.il castclassinterface_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassinterface_do.dasm.il castclassinterface_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassinterface_do.asm.exe  castclassinterface_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassinterface_do.asm.exe  castclassinterface_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassinterface_do
    if not exist "castclassinterface_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassinterface_do.org" (
            mkdir IL
            copy castclassinterface_do.exe IL\castclassinterface_do.exe 
            ren castclassinterface_do.exe castclassinterface_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassinterface_do.org /out castclassinterface_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassinterface_do.org /out castclassinterface_do.exe
...
set ExePath=castclassinterface_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassinterface_do.reflect.xml
    IF EXIST castclassinterface_do.reflect.xml set ReflectionRoots=-x castclassinterface_do.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassinterface_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassinterface_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassinterface_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
