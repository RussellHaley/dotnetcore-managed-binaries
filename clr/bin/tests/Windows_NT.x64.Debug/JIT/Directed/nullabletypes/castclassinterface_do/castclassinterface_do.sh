...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassinterface_do castclassinterface_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassinterface_do castclassinterface_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassinterface_do.dasm.il castclassinterface_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassinterface_do.dasm.il castclassinterface_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassinterface_do.asm.exe  castclassinterface_do.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassinterface_do.asm.exe  castclassinterface_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassinterface_do
    if [ ! -f castclassinterface_do.org ]; then
        TakeLock
        if [ ! -f castclassinterface_do.org ]; then
          mkdir IL
          cp castclassinterface_do.exe IL/castclassinterface_do.exe
          mv castclassinterface_do.exe castclassinterface_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassinterface_do.org /out castclassinterface_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassinterface_do.org /out castclassinterface_do.exe
          __cgExitCode=$?
...
ExePath=castclassinterface_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in castclassinterface_do.reflect.xml
  if [ -f castclassinterface_do.reflect.xml ]
then
    ReflectionRoots="-x castclassinterface_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassinterface_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassinterface_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassinterface_do.asm.exe $CLRTestExecutionArguments
