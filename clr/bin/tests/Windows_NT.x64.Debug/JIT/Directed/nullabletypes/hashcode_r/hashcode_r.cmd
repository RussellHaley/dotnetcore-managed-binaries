...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=hashcode_r.dasm.il hashcode_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=hashcode_r.dasm.il hashcode_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=hashcode_r.asm.exe  hashcode_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=hashcode_r.asm.exe  hashcode_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=hashcode_r
    if not exist "hashcode_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "hashcode_r.org" (
            mkdir IL
            copy hashcode_r.exe IL\hashcode_r.exe 
            ren hashcode_r.exe hashcode_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hashcode_r.org /out hashcode_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hashcode_r.org /out hashcode_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in hashcode_r.reflect.xml
    IF EXIST hashcode_r.reflect.xml set ReflectionRoots=-x hashcode_r.reflect.xml
    ...
    set ExePath=%LinkBin%\hashcode_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% hashcode_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% hashcode_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
