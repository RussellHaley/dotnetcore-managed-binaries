...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/hashcode_r hashcode_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/hashcode_r hashcode_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=hashcode_r.dasm.il hashcode_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=hashcode_r.dasm.il hashcode_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=hashcode_r.asm.exe  hashcode_r.dasm.il
    "$CORE_ROOT/ilasm" -output=hashcode_r.asm.exe  hashcode_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=hashcode_r
    if [ ! -f hashcode_r.org ]; then
        TakeLock
        if [ ! -f hashcode_r.org ]; then
          mkdir IL
          cp hashcode_r.exe IL/hashcode_r.exe
          mv hashcode_r.exe hashcode_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hashcode_r.org /out hashcode_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hashcode_r.org /out hashcode_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in hashcode_r.reflect.xml
  if [ -f hashcode_r.reflect.xml ]
then
    ReflectionRoots="-x hashcode_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/hashcode_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" hashcode_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" hashcode_r.asm.exe $CLRTestExecutionArguments
