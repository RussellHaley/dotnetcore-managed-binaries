...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=isinst_do.dasm.il isinst_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=isinst_do.dasm.il isinst_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=isinst_do.asm.exe  isinst_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=isinst_do.asm.exe  isinst_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=isinst_do
    if not exist "isinst_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "isinst_do.org" (
            mkdir IL
            copy isinst_do.exe IL\isinst_do.exe 
            ren isinst_do.exe isinst_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in isinst_do.org /out isinst_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in isinst_do.org /out isinst_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in isinst_do.reflect.xml
    IF EXIST isinst_do.reflect.xml set ReflectionRoots=-x isinst_do.reflect.xml
    ...
    set ExePath=%LinkBin%\isinst_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% isinst_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% isinst_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
