...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/isinst_do isinst_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/isinst_do isinst_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=isinst_do.dasm.il isinst_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=isinst_do.dasm.il isinst_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=isinst_do.asm.exe  isinst_do.dasm.il
    "$CORE_ROOT/ilasm" -output=isinst_do.asm.exe  isinst_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=isinst_do
    if [ ! -f isinst_do.org ]; then
        TakeLock
        if [ ! -f isinst_do.org ]; then
          mkdir IL
          cp isinst_do.exe IL/isinst_do.exe
          mv isinst_do.exe isinst_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in isinst_do.org /out isinst_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in isinst_do.org /out isinst_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in isinst_do.reflect.xml
  if [ -f isinst_do.reflect.xml ]
then
    ReflectionRoots="-x isinst_do.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" isinst_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" isinst_do.asm.exe $CLRTestExecutionArguments
