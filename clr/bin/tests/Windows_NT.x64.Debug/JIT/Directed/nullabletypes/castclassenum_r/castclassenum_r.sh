...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassenum_r castclassenum_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassenum_r castclassenum_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassenum_r.dasm.il castclassenum_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassenum_r.dasm.il castclassenum_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassenum_r.asm.exe  castclassenum_r.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassenum_r.asm.exe  castclassenum_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassenum_r
    if [ ! -f castclassenum_r.org ]; then
        TakeLock
        if [ ! -f castclassenum_r.org ]; then
          mkdir IL
          cp castclassenum_r.exe IL/castclassenum_r.exe
          mv castclassenum_r.exe castclassenum_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassenum_r.org /out castclassenum_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassenum_r.org /out castclassenum_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in castclassenum_r.reflect.xml
  if [ -f castclassenum_r.reflect.xml ]
then
    ReflectionRoots="-x castclassenum_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassenum_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassenum_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassenum_r.asm.exe $CLRTestExecutionArguments
