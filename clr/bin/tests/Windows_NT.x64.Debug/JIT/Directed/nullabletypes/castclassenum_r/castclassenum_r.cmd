...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassenum_r.dasm.il castclassenum_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassenum_r.dasm.il castclassenum_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassenum_r.asm.exe  castclassenum_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassenum_r.asm.exe  castclassenum_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassenum_r
    if not exist "castclassenum_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassenum_r.org" (
            mkdir IL
            copy castclassenum_r.exe IL\castclassenum_r.exe 
            ren castclassenum_r.exe castclassenum_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassenum_r.org /out castclassenum_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassenum_r.org /out castclassenum_r.exe
...
set ExePath=castclassenum_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassenum_r.reflect.xml
    IF EXIST castclassenum_r.reflect.xml set ReflectionRoots=-x castclassenum_r.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassenum_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassenum_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassenum_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
