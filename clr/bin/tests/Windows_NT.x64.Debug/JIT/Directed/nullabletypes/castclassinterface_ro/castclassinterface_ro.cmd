...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassinterface_ro.dasm.il castclassinterface_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassinterface_ro.dasm.il castclassinterface_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassinterface_ro.asm.exe  castclassinterface_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassinterface_ro.asm.exe  castclassinterface_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassinterface_ro
    if not exist "castclassinterface_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassinterface_ro.org" (
            mkdir IL
            copy castclassinterface_ro.exe IL\castclassinterface_ro.exe 
            ren castclassinterface_ro.exe castclassinterface_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassinterface_ro.org /out castclassinterface_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassinterface_ro.org /out castclassinterface_ro.exe
...
set ExePath=castclassinterface_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassinterface_ro.reflect.xml
    IF EXIST castclassinterface_ro.reflect.xml set ReflectionRoots=-x castclassinterface_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassinterface_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassinterface_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassinterface_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
