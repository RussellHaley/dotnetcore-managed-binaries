...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassinterface_ro castclassinterface_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassinterface_ro castclassinterface_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassinterface_ro.dasm.il castclassinterface_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassinterface_ro.dasm.il castclassinterface_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassinterface_ro.asm.exe  castclassinterface_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassinterface_ro.asm.exe  castclassinterface_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassinterface_ro
    if [ ! -f castclassinterface_ro.org ]; then
        TakeLock
        if [ ! -f castclassinterface_ro.org ]; then
          mkdir IL
          cp castclassinterface_ro.exe IL/castclassinterface_ro.exe
          mv castclassinterface_ro.exe castclassinterface_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassinterface_ro.org /out castclassinterface_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassinterface_ro.org /out castclassinterface_ro.exe
          __cgExitCode=$?
...
ExePath=castclassinterface_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in castclassinterface_ro.reflect.xml
  if [ -f castclassinterface_ro.reflect.xml ]
then
    ReflectionRoots="-x castclassinterface_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassinterface_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassinterface_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassinterface_ro.asm.exe $CLRTestExecutionArguments
