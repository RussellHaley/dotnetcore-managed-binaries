...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassinterface_r castclassinterface_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/castclassinterface_r castclassinterface_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclassinterface_r.dasm.il castclassinterface_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclassinterface_r.dasm.il castclassinterface_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclassinterface_r.asm.exe  castclassinterface_r.dasm.il
    "$CORE_ROOT/ilasm" -output=castclassinterface_r.asm.exe  castclassinterface_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclassinterface_r
    if [ ! -f castclassinterface_r.org ]; then
        TakeLock
        if [ ! -f castclassinterface_r.org ]; then
          mkdir IL
          cp castclassinterface_r.exe IL/castclassinterface_r.exe
          mv castclassinterface_r.exe castclassinterface_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassinterface_r.org /out castclassinterface_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclassinterface_r.org /out castclassinterface_r.exe
          __cgExitCode=$?
...
ExePath=castclassinterface_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in castclassinterface_r.reflect.xml
  if [ -f castclassinterface_r.reflect.xml ]
then
    ReflectionRoots="-x castclassinterface_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclassinterface_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclassinterface_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclassinterface_r.asm.exe $CLRTestExecutionArguments
