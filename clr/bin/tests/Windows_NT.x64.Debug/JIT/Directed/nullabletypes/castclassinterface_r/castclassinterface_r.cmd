...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclassinterface_r.dasm.il castclassinterface_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclassinterface_r.dasm.il castclassinterface_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclassinterface_r.asm.exe  castclassinterface_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclassinterface_r.asm.exe  castclassinterface_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclassinterface_r
    if not exist "castclassinterface_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclassinterface_r.org" (
            mkdir IL
            copy castclassinterface_r.exe IL\castclassinterface_r.exe 
            ren castclassinterface_r.exe castclassinterface_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassinterface_r.org /out castclassinterface_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclassinterface_r.org /out castclassinterface_r.exe
...
set ExePath=castclassinterface_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclassinterface_r.reflect.xml
    IF EXIST castclassinterface_r.reflect.xml set ReflectionRoots=-x castclassinterface_r.reflect.xml
    ...
    set ExePath=%LinkBin%\castclassinterface_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclassinterface_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclassinterface_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
