...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxinterface_do.dasm.il boxunboxinterface_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxinterface_do.dasm.il boxunboxinterface_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxinterface_do.asm.exe  boxunboxinterface_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxinterface_do.asm.exe  boxunboxinterface_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxinterface_do
    if not exist "boxunboxinterface_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxinterface_do.org" (
            mkdir IL
            copy boxunboxinterface_do.exe IL\boxunboxinterface_do.exe 
            ren boxunboxinterface_do.exe boxunboxinterface_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxinterface_do.org /out boxunboxinterface_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxinterface_do.org /out boxunboxinterface_do.exe
...
set ExePath=boxunboxinterface_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxinterface_do.reflect.xml
    IF EXIST boxunboxinterface_do.reflect.xml set ReflectionRoots=-x boxunboxinterface_do.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxinterface_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxinterface_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxinterface_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
