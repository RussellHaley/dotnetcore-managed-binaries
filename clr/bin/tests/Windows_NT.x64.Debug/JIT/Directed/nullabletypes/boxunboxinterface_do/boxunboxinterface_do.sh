...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxinterface_do boxunboxinterface_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxinterface_do boxunboxinterface_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxinterface_do.dasm.il boxunboxinterface_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxinterface_do.dasm.il boxunboxinterface_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxinterface_do.asm.exe  boxunboxinterface_do.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxinterface_do.asm.exe  boxunboxinterface_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxinterface_do
    if [ ! -f boxunboxinterface_do.org ]; then
        TakeLock
        if [ ! -f boxunboxinterface_do.org ]; then
          mkdir IL
          cp boxunboxinterface_do.exe IL/boxunboxinterface_do.exe
          mv boxunboxinterface_do.exe boxunboxinterface_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxinterface_do.org /out boxunboxinterface_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxinterface_do.org /out boxunboxinterface_do.exe
          __cgExitCode=$?
...
ExePath=boxunboxinterface_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxinterface_do.reflect.xml
  if [ -f boxunboxinterface_do.reflect.xml ]
then
    ReflectionRoots="-x boxunboxinterface_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxinterface_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxinterface_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxinterface_do.asm.exe $CLRTestExecutionArguments
