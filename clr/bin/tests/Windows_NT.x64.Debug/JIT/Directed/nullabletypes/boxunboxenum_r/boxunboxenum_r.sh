...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxenum_r boxunboxenum_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/nullabletypes/boxunboxenum_r boxunboxenum_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=boxunboxenum_r.dasm.il boxunboxenum_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=boxunboxenum_r.dasm.il boxunboxenum_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=boxunboxenum_r.asm.exe  boxunboxenum_r.dasm.il
    "$CORE_ROOT/ilasm" -output=boxunboxenum_r.asm.exe  boxunboxenum_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=boxunboxenum_r
    if [ ! -f boxunboxenum_r.org ]; then
        TakeLock
        if [ ! -f boxunboxenum_r.org ]; then
          mkdir IL
          cp boxunboxenum_r.exe IL/boxunboxenum_r.exe
          mv boxunboxenum_r.exe boxunboxenum_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxenum_r.org /out boxunboxenum_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in boxunboxenum_r.org /out boxunboxenum_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in boxunboxenum_r.reflect.xml
  if [ -f boxunboxenum_r.reflect.xml ]
then
    ReflectionRoots="-x boxunboxenum_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/boxunboxenum_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" boxunboxenum_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" boxunboxenum_r.asm.exe $CLRTestExecutionArguments
