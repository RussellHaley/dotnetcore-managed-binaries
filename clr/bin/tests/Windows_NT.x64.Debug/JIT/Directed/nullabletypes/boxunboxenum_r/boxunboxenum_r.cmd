...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxenum_r.dasm.il boxunboxenum_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=boxunboxenum_r.dasm.il boxunboxenum_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=boxunboxenum_r.asm.exe  boxunboxenum_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=boxunboxenum_r.asm.exe  boxunboxenum_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=boxunboxenum_r
    if not exist "boxunboxenum_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "boxunboxenum_r.org" (
            mkdir IL
            copy boxunboxenum_r.exe IL\boxunboxenum_r.exe 
            ren boxunboxenum_r.exe boxunboxenum_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxenum_r.org /out boxunboxenum_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in boxunboxenum_r.org /out boxunboxenum_r.exe
...
set ExePath=boxunboxenum_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in boxunboxenum_r.reflect.xml
    IF EXIST boxunboxenum_r.reflect.xml set ReflectionRoots=-x boxunboxenum_r.reflect.xml
    ...
    set ExePath=%LinkBin%\boxunboxenum_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% boxunboxenum_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% boxunboxenum_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
