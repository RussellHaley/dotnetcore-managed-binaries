...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/zeroinit/tail tail.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/zeroinit/tail tail.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tail.dasm.il tail.exe
    "$CORE_ROOT/ildasm" -raweh -out=tail.dasm.il tail.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tail.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tail.dasm.il
    "$CORE_ROOT/ilasm" -output=tail.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tail.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tail
    if [ ! -f tail.org ]; then
        TakeLock
        if [ ! -f tail.org ]; then
          mkdir IL
          cp tail.exe IL/tail.exe
          mv tail.exe tail.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tail.org /out tail.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tail.org /out tail.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in tail.reflect.xml
  if [ -f tail.reflect.xml ]
then
    ReflectionRoots="-x tail.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tail.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tail.asm.exe $CLRTestExecutionArguments
