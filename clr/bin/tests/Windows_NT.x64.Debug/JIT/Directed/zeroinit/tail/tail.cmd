...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\zeroinit\tail tail.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\zeroinit\tail tail.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tail.dasm.il tail.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tail.dasm.il tail.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tail.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tail.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tail.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tail.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tail
    if not exist "tail.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tail.org" (
            mkdir IL
            copy tail.exe IL\tail.exe 
            ren tail.exe tail.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tail.org /out tail.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tail.org /out tail.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tail.reflect.xml
    IF EXIST tail.reflect.xml set ReflectionRoots=-x tail.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tail.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tail.asm.exe %CLRTestExecutionArguments% %Host_Args%
