...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/zeroinit/Dev10_863995 Dev10_863995.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/zeroinit/Dev10_863995 Dev10_863995.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Dev10_863995.dasm.il Dev10_863995.exe
    "$CORE_ROOT/ildasm" -raweh -out=Dev10_863995.dasm.il Dev10_863995.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Dev10_863995.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Dev10_863995.dasm.il
    "$CORE_ROOT/ilasm" -output=Dev10_863995.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Dev10_863995.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Dev10_863995
    if [ ! -f Dev10_863995.org ]; then
        TakeLock
        if [ ! -f Dev10_863995.org ]; then
          mkdir IL
          cp Dev10_863995.exe IL/Dev10_863995.exe
          mv Dev10_863995.exe Dev10_863995.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Dev10_863995.org /out Dev10_863995.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Dev10_863995.org /out Dev10_863995.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Dev10_863995.reflect.xml
  if [ -f Dev10_863995.reflect.xml ]
then
    ReflectionRoots="-x Dev10_863995.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Dev10_863995.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Dev10_863995.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Dev10_863995.asm.exe $CLRTestExecutionArguments
