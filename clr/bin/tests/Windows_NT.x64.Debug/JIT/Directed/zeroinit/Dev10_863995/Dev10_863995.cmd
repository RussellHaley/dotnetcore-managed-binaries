...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\zeroinit\Dev10_863995 Dev10_863995.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\zeroinit\Dev10_863995 Dev10_863995.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Dev10_863995.dasm.il Dev10_863995.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Dev10_863995.dasm.il Dev10_863995.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Dev10_863995.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Dev10_863995.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Dev10_863995.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Dev10_863995.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Dev10_863995
    if not exist "Dev10_863995.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Dev10_863995.org" (
            mkdir IL
            copy Dev10_863995.exe IL\Dev10_863995.exe 
            ren Dev10_863995.exe Dev10_863995.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Dev10_863995.org /out Dev10_863995.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Dev10_863995.org /out Dev10_863995.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Dev10_863995.reflect.xml
    IF EXIST Dev10_863995.reflect.xml set ReflectionRoots=-x Dev10_863995.reflect.xml
    ...
    set ExePath=%LinkBin%\Dev10_863995.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Dev10_863995.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Dev10_863995.asm.exe %CLRTestExecutionArguments% %Host_Args%
