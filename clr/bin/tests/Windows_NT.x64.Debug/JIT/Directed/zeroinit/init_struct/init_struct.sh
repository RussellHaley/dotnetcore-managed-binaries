...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/zeroinit/init_struct init_struct.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/zeroinit/init_struct init_struct.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=init_struct.dasm.il init_struct.exe
    "$CORE_ROOT/ildasm" -raweh -out=init_struct.dasm.il init_struct.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=init_struct.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE init_struct.dasm.il
    "$CORE_ROOT/ilasm" -output=init_struct.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE init_struct.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=init_struct
    if [ ! -f init_struct.org ]; then
        TakeLock
        if [ ! -f init_struct.org ]; then
          mkdir IL
          cp init_struct.exe IL/init_struct.exe
          mv init_struct.exe init_struct.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in init_struct.org /out init_struct.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in init_struct.org /out init_struct.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in init_struct.reflect.xml
  if [ -f init_struct.reflect.xml ]
then
    ReflectionRoots="-x init_struct.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/init_struct.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" init_struct.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" init_struct.asm.exe $CLRTestExecutionArguments
