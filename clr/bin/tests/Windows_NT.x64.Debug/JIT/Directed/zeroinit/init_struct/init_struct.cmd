...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\zeroinit\init_struct init_struct.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\zeroinit\init_struct init_struct.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=init_struct.dasm.il init_struct.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=init_struct.dasm.il init_struct.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=init_struct.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE init_struct.dasm.il
    %CORE_ROOT%\ilasm.exe /output=init_struct.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE init_struct.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=init_struct
    if not exist "init_struct.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "init_struct.org" (
            mkdir IL
            copy init_struct.exe IL\init_struct.exe 
            ren init_struct.exe init_struct.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in init_struct.org /out init_struct.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in init_struct.org /out init_struct.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in init_struct.reflect.xml
    IF EXIST init_struct.reflect.xml set ReflectionRoots=-x init_struct.reflect.xml
    ...
    set ExePath=%LinkBin%\init_struct.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% init_struct.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% init_struct.asm.exe %CLRTestExecutionArguments% %Host_Args%
