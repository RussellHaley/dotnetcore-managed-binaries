...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/forceinlining/NegativeCases NegativeCases.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/forceinlining/NegativeCases NegativeCases.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NegativeCases.dasm.il NegativeCases.exe
    "$CORE_ROOT/ildasm" -raweh -out=NegativeCases.dasm.il NegativeCases.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NegativeCases.asm.exe -QUIET -NOLOGO -DEBUG=OPT NegativeCases.dasm.il
    "$CORE_ROOT/ilasm" -output=NegativeCases.asm.exe -QUIET -NOLOGO -DEBUG=OPT NegativeCases.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NegativeCases
    if [ ! -f NegativeCases.org ]; then
        TakeLock
        if [ ! -f NegativeCases.org ]; then
          mkdir IL
          cp NegativeCases.exe IL/NegativeCases.exe
          mv NegativeCases.exe NegativeCases.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NegativeCases.org /out NegativeCases.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NegativeCases.org /out NegativeCases.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NegativeCases.reflect.xml
  if [ -f NegativeCases.reflect.xml ]
then
    ReflectionRoots="-x NegativeCases.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NegativeCases.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NegativeCases.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NegativeCases.asm.exe $CLRTestExecutionArguments
