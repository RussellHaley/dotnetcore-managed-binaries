...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\forceinlining\NegativeCases NegativeCases.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\forceinlining\NegativeCases NegativeCases.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NegativeCases.dasm.il NegativeCases.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NegativeCases.dasm.il NegativeCases.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NegativeCases.asm.exe -QUIET -NOLOGO -DEBUG=OPT NegativeCases.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NegativeCases.asm.exe -QUIET -NOLOGO -DEBUG=OPT NegativeCases.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NegativeCases
    if not exist "NegativeCases.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NegativeCases.org" (
            mkdir IL
            copy NegativeCases.exe IL\NegativeCases.exe 
            ren NegativeCases.exe NegativeCases.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NegativeCases.org /out NegativeCases.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NegativeCases.org /out NegativeCases.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NegativeCases.reflect.xml
    IF EXIST NegativeCases.reflect.xml set ReflectionRoots=-x NegativeCases.reflect.xml
    ...
    set ExePath=%LinkBin%\NegativeCases.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NegativeCases.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NegativeCases.asm.exe %CLRTestExecutionArguments% %Host_Args%
