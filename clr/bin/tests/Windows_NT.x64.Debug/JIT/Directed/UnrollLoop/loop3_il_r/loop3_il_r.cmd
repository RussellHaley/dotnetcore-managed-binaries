...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\UnrollLoop\loop3_il_r loop3_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\UnrollLoop\loop3_il_r loop3_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=loop3_il_r.dasm.il loop3_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=loop3_il_r.dasm.il loop3_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=loop3_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT loop3_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=loop3_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT loop3_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=loop3_il_r
    if not exist "loop3_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "loop3_il_r.org" (
            mkdir IL
            copy loop3_il_r.exe IL\loop3_il_r.exe 
            ren loop3_il_r.exe loop3_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loop3_il_r.org /out loop3_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loop3_il_r.org /out loop3_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in loop3_il_r.reflect.xml
    IF EXIST loop3_il_r.reflect.xml set ReflectionRoots=-x loop3_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\loop3_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% loop3_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% loop3_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
