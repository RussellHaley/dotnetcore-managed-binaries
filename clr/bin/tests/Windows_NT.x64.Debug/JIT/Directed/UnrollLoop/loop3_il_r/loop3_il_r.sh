...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/UnrollLoop/loop3_il_r loop3_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/UnrollLoop/loop3_il_r loop3_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=loop3_il_r.dasm.il loop3_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=loop3_il_r.dasm.il loop3_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=loop3_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT loop3_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=loop3_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT loop3_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=loop3_il_r
    if [ ! -f loop3_il_r.org ]; then
        TakeLock
        if [ ! -f loop3_il_r.org ]; then
          mkdir IL
          cp loop3_il_r.exe IL/loop3_il_r.exe
          mv loop3_il_r.exe loop3_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loop3_il_r.org /out loop3_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loop3_il_r.org /out loop3_il_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in loop3_il_r.reflect.xml
  if [ -f loop3_il_r.reflect.xml ]
then
    ReflectionRoots="-x loop3_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/loop3_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" loop3_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" loop3_il_r.asm.exe $CLRTestExecutionArguments
