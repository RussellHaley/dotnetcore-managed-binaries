...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/UnrollLoop/loop4_cs_ro loop4_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/UnrollLoop/loop4_cs_ro loop4_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=loop4_cs_ro.dasm.il loop4_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=loop4_cs_ro.dasm.il loop4_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=loop4_cs_ro.asm.exe  loop4_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=loop4_cs_ro.asm.exe  loop4_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=loop4_cs_ro
    if [ ! -f loop4_cs_ro.org ]; then
        TakeLock
        if [ ! -f loop4_cs_ro.org ]; then
          mkdir IL
          cp loop4_cs_ro.exe IL/loop4_cs_ro.exe
          mv loop4_cs_ro.exe loop4_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loop4_cs_ro.org /out loop4_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loop4_cs_ro.org /out loop4_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in loop4_cs_ro.reflect.xml
  if [ -f loop4_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x loop4_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/loop4_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" loop4_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" loop4_cs_ro.asm.exe $CLRTestExecutionArguments
