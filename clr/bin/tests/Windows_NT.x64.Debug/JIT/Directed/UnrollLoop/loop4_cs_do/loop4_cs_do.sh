...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/UnrollLoop/loop4_cs_do loop4_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/UnrollLoop/loop4_cs_do loop4_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=loop4_cs_do.dasm.il loop4_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=loop4_cs_do.dasm.il loop4_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=loop4_cs_do.asm.exe  loop4_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=loop4_cs_do.asm.exe  loop4_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=loop4_cs_do
    if [ ! -f loop4_cs_do.org ]; then
        TakeLock
        if [ ! -f loop4_cs_do.org ]; then
          mkdir IL
          cp loop4_cs_do.exe IL/loop4_cs_do.exe
          mv loop4_cs_do.exe loop4_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loop4_cs_do.org /out loop4_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loop4_cs_do.org /out loop4_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in loop4_cs_do.reflect.xml
  if [ -f loop4_cs_do.reflect.xml ]
then
    ReflectionRoots="-x loop4_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/loop4_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" loop4_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" loop4_cs_do.asm.exe $CLRTestExecutionArguments
