...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\UnrollLoop\loop4_cs_do loop4_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\UnrollLoop\loop4_cs_do loop4_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=loop4_cs_do.dasm.il loop4_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=loop4_cs_do.dasm.il loop4_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=loop4_cs_do.asm.exe  loop4_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=loop4_cs_do.asm.exe  loop4_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=loop4_cs_do
    if not exist "loop4_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "loop4_cs_do.org" (
            mkdir IL
            copy loop4_cs_do.exe IL\loop4_cs_do.exe 
            ren loop4_cs_do.exe loop4_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loop4_cs_do.org /out loop4_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loop4_cs_do.org /out loop4_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in loop4_cs_do.reflect.xml
    IF EXIST loop4_cs_do.reflect.xml set ReflectionRoots=-x loop4_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\loop4_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% loop4_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% loop4_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
