...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\cmov\Int_Or_Op_cs_ro Int_Or_Op_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\cmov\Int_Or_Op_cs_ro Int_Or_Op_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Int_Or_Op_cs_ro.dasm.il Int_Or_Op_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Int_Or_Op_cs_ro.dasm.il Int_Or_Op_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Int_Or_Op_cs_ro.asm.exe  Int_Or_Op_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Int_Or_Op_cs_ro.asm.exe  Int_Or_Op_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Int_Or_Op_cs_ro
    if not exist "Int_Or_Op_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Int_Or_Op_cs_ro.org" (
            mkdir IL
            copy Int_Or_Op_cs_ro.exe IL\Int_Or_Op_cs_ro.exe 
            ren Int_Or_Op_cs_ro.exe Int_Or_Op_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Int_Or_Op_cs_ro.org /out Int_Or_Op_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Int_Or_Op_cs_ro.org /out Int_Or_Op_cs_ro.exe
...
set ExePath=Int_Or_Op_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Int_Or_Op_cs_ro.reflect.xml
    IF EXIST Int_Or_Op_cs_ro.reflect.xml set ReflectionRoots=-x Int_Or_Op_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Int_Or_Op_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Int_Or_Op_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Int_Or_Op_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
