...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/cmov/Int_Or_Op_cs_ro Int_Or_Op_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/cmov/Int_Or_Op_cs_ro Int_Or_Op_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Int_Or_Op_cs_ro.dasm.il Int_Or_Op_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Int_Or_Op_cs_ro.dasm.il Int_Or_Op_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Int_Or_Op_cs_ro.asm.exe  Int_Or_Op_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Int_Or_Op_cs_ro.asm.exe  Int_Or_Op_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Int_Or_Op_cs_ro
    if [ ! -f Int_Or_Op_cs_ro.org ]; then
        TakeLock
        if [ ! -f Int_Or_Op_cs_ro.org ]; then
          mkdir IL
          cp Int_Or_Op_cs_ro.exe IL/Int_Or_Op_cs_ro.exe
          mv Int_Or_Op_cs_ro.exe Int_Or_Op_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Int_Or_Op_cs_ro.org /out Int_Or_Op_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Int_Or_Op_cs_ro.org /out Int_Or_Op_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Int_Or_Op_cs_ro.reflect.xml
  if [ -f Int_Or_Op_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x Int_Or_Op_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Int_Or_Op_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Int_Or_Op_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Int_Or_Op_cs_ro.asm.exe $CLRTestExecutionArguments
