...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\cmov\Float_Xor_Op_cs_ro Float_Xor_Op_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\cmov\Float_Xor_Op_cs_ro Float_Xor_Op_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Float_Xor_Op_cs_ro.dasm.il Float_Xor_Op_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Float_Xor_Op_cs_ro.dasm.il Float_Xor_Op_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Float_Xor_Op_cs_ro.asm.exe  Float_Xor_Op_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Float_Xor_Op_cs_ro.asm.exe  Float_Xor_Op_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Float_Xor_Op_cs_ro
    if not exist "Float_Xor_Op_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Float_Xor_Op_cs_ro.org" (
            mkdir IL
            copy Float_Xor_Op_cs_ro.exe IL\Float_Xor_Op_cs_ro.exe 
            ren Float_Xor_Op_cs_ro.exe Float_Xor_Op_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Float_Xor_Op_cs_ro.org /out Float_Xor_Op_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Float_Xor_Op_cs_ro.org /out Float_Xor_Op_cs_ro.exe
...
set ExePath=Float_Xor_Op_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Float_Xor_Op_cs_ro.reflect.xml
    IF EXIST Float_Xor_Op_cs_ro.reflect.xml set ReflectionRoots=-x Float_Xor_Op_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\Float_Xor_Op_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Float_Xor_Op_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Float_Xor_Op_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
