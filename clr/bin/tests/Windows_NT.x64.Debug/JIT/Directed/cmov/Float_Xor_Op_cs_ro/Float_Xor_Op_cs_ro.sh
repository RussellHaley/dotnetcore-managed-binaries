...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/cmov/Float_Xor_Op_cs_ro Float_Xor_Op_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/cmov/Float_Xor_Op_cs_ro Float_Xor_Op_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Float_Xor_Op_cs_ro.dasm.il Float_Xor_Op_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=Float_Xor_Op_cs_ro.dasm.il Float_Xor_Op_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Float_Xor_Op_cs_ro.asm.exe  Float_Xor_Op_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=Float_Xor_Op_cs_ro.asm.exe  Float_Xor_Op_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Float_Xor_Op_cs_ro
    if [ ! -f Float_Xor_Op_cs_ro.org ]; then
        TakeLock
        if [ ! -f Float_Xor_Op_cs_ro.org ]; then
          mkdir IL
          cp Float_Xor_Op_cs_ro.exe IL/Float_Xor_Op_cs_ro.exe
          mv Float_Xor_Op_cs_ro.exe Float_Xor_Op_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Float_Xor_Op_cs_ro.org /out Float_Xor_Op_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Float_Xor_Op_cs_ro.org /out Float_Xor_Op_cs_ro.exe
          __cgExitCode=$?
...
ExePath=Float_Xor_Op_cs_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in Float_Xor_Op_cs_ro.reflect.xml
  if [ -f Float_Xor_Op_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x Float_Xor_Op_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Float_Xor_Op_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Float_Xor_Op_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Float_Xor_Op_cs_ro.asm.exe $CLRTestExecutionArguments
