...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/cmov/Int_Or_Op_cs_do Int_Or_Op_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/cmov/Int_Or_Op_cs_do Int_Or_Op_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Int_Or_Op_cs_do.dasm.il Int_Or_Op_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=Int_Or_Op_cs_do.dasm.il Int_Or_Op_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Int_Or_Op_cs_do.asm.exe  Int_Or_Op_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=Int_Or_Op_cs_do.asm.exe  Int_Or_Op_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Int_Or_Op_cs_do
    if [ ! -f Int_Or_Op_cs_do.org ]; then
        TakeLock
        if [ ! -f Int_Or_Op_cs_do.org ]; then
          mkdir IL
          cp Int_Or_Op_cs_do.exe IL/Int_Or_Op_cs_do.exe
          mv Int_Or_Op_cs_do.exe Int_Or_Op_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Int_Or_Op_cs_do.org /out Int_Or_Op_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Int_Or_Op_cs_do.org /out Int_Or_Op_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Int_Or_Op_cs_do.reflect.xml
  if [ -f Int_Or_Op_cs_do.reflect.xml ]
then
    ReflectionRoots="-x Int_Or_Op_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Int_Or_Op_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Int_Or_Op_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Int_Or_Op_cs_do.asm.exe $CLRTestExecutionArguments
