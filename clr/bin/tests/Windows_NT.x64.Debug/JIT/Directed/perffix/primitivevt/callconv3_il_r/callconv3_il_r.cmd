...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\perffix\primitivevt\callconv3_il_r callconv3_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\perffix\primitivevt\callconv3_il_r callconv3_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=callconv3_il_r.dasm.il callconv3_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=callconv3_il_r.dasm.il callconv3_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=callconv3_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT callconv3_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=callconv3_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT callconv3_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=callconv3_il_r
    if not exist "callconv3_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "callconv3_il_r.org" (
            mkdir IL
            copy callconv3_il_r.exe IL\callconv3_il_r.exe 
            ren callconv3_il_r.exe callconv3_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in callconv3_il_r.org /out callconv3_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in callconv3_il_r.org /out callconv3_il_r.exe
...
set ExePath=callconv3_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in callconv3_il_r.reflect.xml
    IF EXIST callconv3_il_r.reflect.xml set ReflectionRoots=-x callconv3_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\callconv3_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% callconv3_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% callconv3_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
