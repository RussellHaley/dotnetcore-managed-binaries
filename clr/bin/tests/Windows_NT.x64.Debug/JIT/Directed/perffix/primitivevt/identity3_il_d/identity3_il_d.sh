...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/perffix/primitivevt/identity3_il_d identity3_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/perffix/primitivevt/identity3_il_d identity3_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=identity3_il_d.dasm.il identity3_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=identity3_il_d.dasm.il identity3_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=identity3_il_d.asm.exe -QUIET -NOLOGO -DEBUG identity3_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=identity3_il_d.asm.exe -QUIET -NOLOGO -DEBUG identity3_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=identity3_il_d
    if [ ! -f identity3_il_d.org ]; then
        TakeLock
        if [ ! -f identity3_il_d.org ]; then
          mkdir IL
          cp identity3_il_d.exe IL/identity3_il_d.exe
          mv identity3_il_d.exe identity3_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in identity3_il_d.org /out identity3_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in identity3_il_d.org /out identity3_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in identity3_il_d.reflect.xml
  if [ -f identity3_il_d.reflect.xml ]
then
    ReflectionRoots="-x identity3_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/identity3_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" identity3_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" identity3_il_d.asm.exe $CLRTestExecutionArguments
