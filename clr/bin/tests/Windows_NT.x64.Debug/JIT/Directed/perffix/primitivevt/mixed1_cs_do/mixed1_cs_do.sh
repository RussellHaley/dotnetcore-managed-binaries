...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/perffix/primitivevt/mixed1_cs_do mixed1_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/perffix/primitivevt/mixed1_cs_do mixed1_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mixed1_cs_do.dasm.il mixed1_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=mixed1_cs_do.dasm.il mixed1_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mixed1_cs_do.asm.exe  mixed1_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=mixed1_cs_do.asm.exe  mixed1_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mixed1_cs_do
    if [ ! -f mixed1_cs_do.org ]; then
        TakeLock
        if [ ! -f mixed1_cs_do.org ]; then
          mkdir IL
          cp mixed1_cs_do.exe IL/mixed1_cs_do.exe
          mv mixed1_cs_do.exe mixed1_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mixed1_cs_do.org /out mixed1_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mixed1_cs_do.org /out mixed1_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mixed1_cs_do.reflect.xml
  if [ -f mixed1_cs_do.reflect.xml ]
then
    ReflectionRoots="-x mixed1_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/mixed1_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mixed1_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mixed1_cs_do.asm.exe $CLRTestExecutionArguments
