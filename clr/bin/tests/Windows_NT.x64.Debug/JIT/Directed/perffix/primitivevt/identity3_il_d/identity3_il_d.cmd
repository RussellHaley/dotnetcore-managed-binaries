...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\perffix\primitivevt\identity3_il_d identity3_il_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\perffix\primitivevt\identity3_il_d identity3_il_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=identity3_il_d.dasm.il identity3_il_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=identity3_il_d.dasm.il identity3_il_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=identity3_il_d.asm.exe -QUIET -NOLOGO -DEBUG identity3_il_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=identity3_il_d.asm.exe -QUIET -NOLOGO -DEBUG identity3_il_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=identity3_il_d
    if not exist "identity3_il_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "identity3_il_d.org" (
            mkdir IL
            copy identity3_il_d.exe IL\identity3_il_d.exe 
            ren identity3_il_d.exe identity3_il_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in identity3_il_d.org /out identity3_il_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in identity3_il_d.org /out identity3_il_d.exe
...
set ExePath=identity3_il_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in identity3_il_d.reflect.xml
    IF EXIST identity3_il_d.reflect.xml set ReflectionRoots=-x identity3_il_d.reflect.xml
    ...
    set ExePath=%LinkBin%\identity3_il_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% identity3_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% identity3_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
