...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/perffix/primitivevt/mixed1_cs_ro mixed1_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/perffix/primitivevt/mixed1_cs_ro mixed1_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mixed1_cs_ro.dasm.il mixed1_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=mixed1_cs_ro.dasm.il mixed1_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mixed1_cs_ro.asm.exe  mixed1_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=mixed1_cs_ro.asm.exe  mixed1_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mixed1_cs_ro
    if [ ! -f mixed1_cs_ro.org ]; then
        TakeLock
        if [ ! -f mixed1_cs_ro.org ]; then
          mkdir IL
          cp mixed1_cs_ro.exe IL/mixed1_cs_ro.exe
          mv mixed1_cs_ro.exe mixed1_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mixed1_cs_ro.org /out mixed1_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mixed1_cs_ro.org /out mixed1_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mixed1_cs_ro.reflect.xml
  if [ -f mixed1_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x mixed1_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/mixed1_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mixed1_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mixed1_cs_ro.asm.exe $CLRTestExecutionArguments
