...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\perffix\primitivevt\mixed1_cs_do mixed1_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\perffix\primitivevt\mixed1_cs_do mixed1_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mixed1_cs_do.dasm.il mixed1_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mixed1_cs_do.dasm.il mixed1_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mixed1_cs_do.asm.exe  mixed1_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mixed1_cs_do.asm.exe  mixed1_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mixed1_cs_do
    if not exist "mixed1_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mixed1_cs_do.org" (
            mkdir IL
            copy mixed1_cs_do.exe IL\mixed1_cs_do.exe 
            ren mixed1_cs_do.exe mixed1_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mixed1_cs_do.org /out mixed1_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mixed1_cs_do.org /out mixed1_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mixed1_cs_do.reflect.xml
    IF EXIST mixed1_cs_do.reflect.xml set ReflectionRoots=-x mixed1_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\mixed1_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mixed1_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mixed1_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
