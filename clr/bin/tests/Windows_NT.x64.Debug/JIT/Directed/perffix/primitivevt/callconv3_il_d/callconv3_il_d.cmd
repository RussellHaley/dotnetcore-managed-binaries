...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\perffix\primitivevt\callconv3_il_d callconv3_il_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\perffix\primitivevt\callconv3_il_d callconv3_il_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=callconv3_il_d.dasm.il callconv3_il_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=callconv3_il_d.dasm.il callconv3_il_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=callconv3_il_d.asm.exe -QUIET -NOLOGO -DEBUG callconv3_il_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=callconv3_il_d.asm.exe -QUIET -NOLOGO -DEBUG callconv3_il_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=callconv3_il_d
    if not exist "callconv3_il_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "callconv3_il_d.org" (
            mkdir IL
            copy callconv3_il_d.exe IL\callconv3_il_d.exe 
            ren callconv3_il_d.exe callconv3_il_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in callconv3_il_d.org /out callconv3_il_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in callconv3_il_d.org /out callconv3_il_d.exe
...
set ExePath=callconv3_il_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in callconv3_il_d.reflect.xml
    IF EXIST callconv3_il_d.reflect.xml set ReflectionRoots=-x callconv3_il_d.reflect.xml
    ...
    set ExePath=%LinkBin%\callconv3_il_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% callconv3_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% callconv3_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
