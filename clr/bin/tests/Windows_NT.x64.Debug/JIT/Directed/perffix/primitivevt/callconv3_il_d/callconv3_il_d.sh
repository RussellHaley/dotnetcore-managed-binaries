...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/perffix/primitivevt/callconv3_il_d callconv3_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/perffix/primitivevt/callconv3_il_d callconv3_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=callconv3_il_d.dasm.il callconv3_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=callconv3_il_d.dasm.il callconv3_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=callconv3_il_d.asm.exe -QUIET -NOLOGO -DEBUG callconv3_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=callconv3_il_d.asm.exe -QUIET -NOLOGO -DEBUG callconv3_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=callconv3_il_d
    if [ ! -f callconv3_il_d.org ]; then
        TakeLock
        if [ ! -f callconv3_il_d.org ]; then
          mkdir IL
          cp callconv3_il_d.exe IL/callconv3_il_d.exe
          mv callconv3_il_d.exe callconv3_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in callconv3_il_d.org /out callconv3_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in callconv3_il_d.org /out callconv3_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in callconv3_il_d.reflect.xml
  if [ -f callconv3_il_d.reflect.xml ]
then
    ReflectionRoots="-x callconv3_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/callconv3_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" callconv3_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" callconv3_il_d.asm.exe $CLRTestExecutionArguments
