...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/pinvoke/calli_excep calli_excep.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/pinvoke/calli_excep calli_excep.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=calli_excep.dasm.il calli_excep.exe
    "$CORE_ROOT/ildasm" -raweh -out=calli_excep.dasm.il calli_excep.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=calli_excep.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE calli_excep.dasm.il
    "$CORE_ROOT/ilasm" -output=calli_excep.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE calli_excep.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=calli_excep
    if [ ! -f calli_excep.org ]; then
        TakeLock
        if [ ! -f calli_excep.org ]; then
          mkdir IL
          cp calli_excep.exe IL/calli_excep.exe
          mv calli_excep.exe calli_excep.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in calli_excep.org /out calli_excep.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in calli_excep.org /out calli_excep.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in calli_excep.reflect.xml
  if [ -f calli_excep.reflect.xml ]
then
    ReflectionRoots="-x calli_excep.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/calli_excep.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" calli_excep.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" calli_excep.asm.exe $CLRTestExecutionArguments
