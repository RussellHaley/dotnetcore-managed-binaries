...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\pinvoke\calli_excep calli_excep.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\pinvoke\calli_excep calli_excep.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=calli_excep.dasm.il calli_excep.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=calli_excep.dasm.il calli_excep.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=calli_excep.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE calli_excep.dasm.il
    %CORE_ROOT%\ilasm.exe /output=calli_excep.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE calli_excep.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=calli_excep
    if not exist "calli_excep.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "calli_excep.org" (
            mkdir IL
            copy calli_excep.exe IL\calli_excep.exe 
            ren calli_excep.exe calli_excep.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in calli_excep.org /out calli_excep.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in calli_excep.org /out calli_excep.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in calli_excep.reflect.xml
    IF EXIST calli_excep.reflect.xml set ReflectionRoots=-x calli_excep.reflect.xml
    ...
    set ExePath=%LinkBin%\calli_excep.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% calli_excep.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% calli_excep.asm.exe %CLRTestExecutionArguments% %Host_Args%
