...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\ExcepFilters\excepobj\excepobj excepobj.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\ExcepFilters\excepobj\excepobj excepobj.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=excepobj.dasm.il excepobj.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=excepobj.dasm.il excepobj.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=excepobj.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE excepobj.dasm.il
    %CORE_ROOT%\ilasm.exe /output=excepobj.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE excepobj.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=excepobj
    if not exist "excepobj.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "excepobj.org" (
            mkdir IL
            copy excepobj.exe IL\excepobj.exe 
            ren excepobj.exe excepobj.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in excepobj.org /out excepobj.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in excepobj.org /out excepobj.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in excepobj.reflect.xml
    IF EXIST excepobj.reflect.xml set ReflectionRoots=-x excepobj.reflect.xml
    ...
    set ExePath=%LinkBin%\excepobj.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% excepobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% excepobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
