...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/ExcepFilters/excepobj/excepobj excepobj.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/ExcepFilters/excepobj/excepobj excepobj.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=excepobj.dasm.il excepobj.exe
    "$CORE_ROOT/ildasm" -raweh -out=excepobj.dasm.il excepobj.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=excepobj.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE excepobj.dasm.il
    "$CORE_ROOT/ilasm" -output=excepobj.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE excepobj.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=excepobj
    if [ ! -f excepobj.org ]; then
        TakeLock
        if [ ! -f excepobj.org ]; then
          mkdir IL
          cp excepobj.exe IL/excepobj.exe
          mv excepobj.exe excepobj.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in excepobj.org /out excepobj.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in excepobj.org /out excepobj.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in excepobj.reflect.xml
  if [ -f excepobj.reflect.xml ]
then
    ReflectionRoots="-x excepobj.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" excepobj.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" excepobj.asm.exe $CLRTestExecutionArguments
