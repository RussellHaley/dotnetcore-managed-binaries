...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\ExcepFilters\mixed3\mixed3 mixed3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\ExcepFilters\mixed3\mixed3 mixed3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mixed3.dasm.il mixed3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mixed3.dasm.il mixed3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mixed3.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mixed3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mixed3.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mixed3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mixed3
    if not exist "mixed3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mixed3.org" (
            mkdir IL
            copy mixed3.exe IL\mixed3.exe 
            ren mixed3.exe mixed3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mixed3.org /out mixed3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mixed3.org /out mixed3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mixed3.reflect.xml
    IF EXIST mixed3.reflect.xml set ReflectionRoots=-x mixed3.reflect.xml
    ...
    set ExePath=%LinkBin%\mixed3.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mixed3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mixed3.asm.exe %CLRTestExecutionArguments% %Host_Args%
