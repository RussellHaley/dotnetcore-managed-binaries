...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/ExcepFilters/mixed3/mixed3 mixed3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/ExcepFilters/mixed3/mixed3 mixed3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mixed3.dasm.il mixed3.exe
    "$CORE_ROOT/ildasm" -raweh -out=mixed3.dasm.il mixed3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mixed3.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mixed3.dasm.il
    "$CORE_ROOT/ilasm" -output=mixed3.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mixed3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mixed3
    if [ ! -f mixed3.org ]; then
        TakeLock
        if [ ! -f mixed3.org ]; then
          mkdir IL
          cp mixed3.exe IL/mixed3.exe
          mv mixed3.exe mixed3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mixed3.org /out mixed3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mixed3.org /out mixed3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mixed3.reflect.xml
  if [ -f mixed3.reflect.xml ]
then
    ReflectionRoots="-x mixed3.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mixed3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mixed3.asm.exe $CLRTestExecutionArguments
