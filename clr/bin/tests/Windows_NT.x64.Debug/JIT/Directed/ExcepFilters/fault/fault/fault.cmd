...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\ExcepFilters\fault\fault fault.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\ExcepFilters\fault\fault fault.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=fault.dasm.il fault.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=fault.dasm.il fault.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=fault.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE fault.dasm.il
    %CORE_ROOT%\ilasm.exe /output=fault.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE fault.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=fault
    if not exist "fault.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "fault.org" (
            mkdir IL
            copy fault.exe IL\fault.exe 
            ren fault.exe fault.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fault.org /out fault.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fault.org /out fault.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in fault.reflect.xml
    IF EXIST fault.reflect.xml set ReflectionRoots=-x fault.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% fault.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% fault.asm.exe %CLRTestExecutionArguments% %Host_Args%
