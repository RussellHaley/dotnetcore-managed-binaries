...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/ExcepFilters/fault/fault fault.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/ExcepFilters/fault/fault fault.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=fault.dasm.il fault.exe
    "$CORE_ROOT/ildasm" -raweh -out=fault.dasm.il fault.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=fault.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE fault.dasm.il
    "$CORE_ROOT/ilasm" -output=fault.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE fault.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=fault
    if [ ! -f fault.org ]; then
        TakeLock
        if [ ! -f fault.org ]; then
          mkdir IL
          cp fault.exe IL/fault.exe
          mv fault.exe fault.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fault.org /out fault.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fault.org /out fault.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in fault.reflect.xml
  if [ -f fault.reflect.xml ]
then
    ReflectionRoots="-x fault.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" fault.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" fault.asm.exe $CLRTestExecutionArguments
