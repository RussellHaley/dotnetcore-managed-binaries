...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/callipinvoke callipinvoke.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/callipinvoke callipinvoke.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=callipinvoke.dasm.il callipinvoke.exe
    "$CORE_ROOT/ildasm" -raweh -out=callipinvoke.dasm.il callipinvoke.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=callipinvoke.asm.exe -QUIET -NOLOGO -DEBUG callipinvoke.dasm.il
    "$CORE_ROOT/ilasm" -output=callipinvoke.asm.exe -QUIET -NOLOGO -DEBUG callipinvoke.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=callipinvoke
    if [ ! -f callipinvoke.org ]; then
        TakeLock
        if [ ! -f callipinvoke.org ]; then
          mkdir IL
          cp callipinvoke.exe IL/callipinvoke.exe
          mv callipinvoke.exe callipinvoke.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in callipinvoke.org /out callipinvoke.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in callipinvoke.org /out callipinvoke.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in callipinvoke.reflect.xml
  if [ -f callipinvoke.reflect.xml ]
then
    ReflectionRoots="-x callipinvoke.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/callipinvoke.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" callipinvoke.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" callipinvoke.asm.exe $CLRTestExecutionArguments
