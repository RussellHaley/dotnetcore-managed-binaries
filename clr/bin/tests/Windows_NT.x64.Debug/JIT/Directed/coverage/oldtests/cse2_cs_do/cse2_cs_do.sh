...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/cse2_cs_do cse2_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/cse2_cs_do cse2_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=cse2_cs_do.dasm.il cse2_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=cse2_cs_do.dasm.il cse2_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=cse2_cs_do.asm.exe  cse2_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=cse2_cs_do.asm.exe  cse2_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=cse2_cs_do
    if [ ! -f cse2_cs_do.org ]; then
        TakeLock
        if [ ! -f cse2_cs_do.org ]; then
          mkdir IL
          cp cse2_cs_do.exe IL/cse2_cs_do.exe
          mv cse2_cs_do.exe cse2_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cse2_cs_do.org /out cse2_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cse2_cs_do.org /out cse2_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in cse2_cs_do.reflect.xml
  if [ -f cse2_cs_do.reflect.xml ]
then
    ReflectionRoots="-x cse2_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/cse2_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" cse2_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" cse2_cs_do.asm.exe $CLRTestExecutionArguments
