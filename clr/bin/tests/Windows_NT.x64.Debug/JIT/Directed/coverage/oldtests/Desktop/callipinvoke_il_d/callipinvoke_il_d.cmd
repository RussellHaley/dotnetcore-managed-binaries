...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\Desktop\callipinvoke_il_d callipinvoke_il_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\Desktop\callipinvoke_il_d callipinvoke_il_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=callipinvoke_il_d.dasm.il callipinvoke_il_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=callipinvoke_il_d.dasm.il callipinvoke_il_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=callipinvoke_il_d.asm.exe -QUIET -NOLOGO -DEBUG callipinvoke_il_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=callipinvoke_il_d.asm.exe -QUIET -NOLOGO -DEBUG callipinvoke_il_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=callipinvoke_il_d
    if not exist "callipinvoke_il_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "callipinvoke_il_d.org" (
            mkdir IL
            copy callipinvoke_il_d.exe IL\callipinvoke_il_d.exe 
            ren callipinvoke_il_d.exe callipinvoke_il_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in callipinvoke_il_d.org /out callipinvoke_il_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in callipinvoke_il_d.org /out callipinvoke_il_d.exe
...
set ExePath=callipinvoke_il_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in callipinvoke_il_d.reflect.xml
    IF EXIST callipinvoke_il_d.reflect.xml set ReflectionRoots=-x callipinvoke_il_d.reflect.xml
    ...
    set ExePath=%LinkBin%\callipinvoke_il_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% callipinvoke_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% callipinvoke_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
