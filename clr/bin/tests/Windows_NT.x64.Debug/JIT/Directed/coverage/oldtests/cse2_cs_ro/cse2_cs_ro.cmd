...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\cse2_cs_ro cse2_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\cse2_cs_ro cse2_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=cse2_cs_ro.dasm.il cse2_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=cse2_cs_ro.dasm.il cse2_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=cse2_cs_ro.asm.exe  cse2_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=cse2_cs_ro.asm.exe  cse2_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=cse2_cs_ro
    if not exist "cse2_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "cse2_cs_ro.org" (
            mkdir IL
            copy cse2_cs_ro.exe IL\cse2_cs_ro.exe 
            ren cse2_cs_ro.exe cse2_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cse2_cs_ro.org /out cse2_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cse2_cs_ro.org /out cse2_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in cse2_cs_ro.reflect.xml
    IF EXIST cse2_cs_ro.reflect.xml set ReflectionRoots=-x cse2_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\cse2_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% cse2_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% cse2_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
