...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/oldtests/Desktop/callipinvoke_il_r callipinvoke_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/oldtests/Desktop/callipinvoke_il_r callipinvoke_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=callipinvoke_il_r.dasm.il callipinvoke_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=callipinvoke_il_r.dasm.il callipinvoke_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=callipinvoke_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT callipinvoke_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=callipinvoke_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT callipinvoke_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=callipinvoke_il_r
    if [ ! -f callipinvoke_il_r.org ]; then
        TakeLock
        if [ ! -f callipinvoke_il_r.org ]; then
          mkdir IL
          cp callipinvoke_il_r.exe IL/callipinvoke_il_r.exe
          mv callipinvoke_il_r.exe callipinvoke_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in callipinvoke_il_r.org /out callipinvoke_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in callipinvoke_il_r.org /out callipinvoke_il_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in callipinvoke_il_r.reflect.xml
  if [ -f callipinvoke_il_r.reflect.xml ]
then
    ReflectionRoots="-x callipinvoke_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/callipinvoke_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" callipinvoke_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" callipinvoke_il_r.asm.exe $CLRTestExecutionArguments
