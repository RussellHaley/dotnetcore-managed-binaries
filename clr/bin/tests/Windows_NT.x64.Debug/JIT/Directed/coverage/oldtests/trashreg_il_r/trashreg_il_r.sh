...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/trashreg_il_r trashreg_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/trashreg_il_r trashreg_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trashreg_il_r.dasm.il trashreg_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=trashreg_il_r.dasm.il trashreg_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trashreg_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT trashreg_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=trashreg_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT trashreg_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trashreg_il_r
    if [ ! -f trashreg_il_r.org ]; then
        TakeLock
        if [ ! -f trashreg_il_r.org ]; then
          mkdir IL
          cp trashreg_il_r.exe IL/trashreg_il_r.exe
          mv trashreg_il_r.exe trashreg_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trashreg_il_r.org /out trashreg_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trashreg_il_r.org /out trashreg_il_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in trashreg_il_r.reflect.xml
  if [ -f trashreg_il_r.reflect.xml ]
then
    ReflectionRoots="-x trashreg_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trashreg_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trashreg_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trashreg_il_r.asm.exe $CLRTestExecutionArguments
