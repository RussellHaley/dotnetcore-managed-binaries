...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\lclfldadd_cs_r lclfldadd_cs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\lclfldadd_cs_r lclfldadd_cs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=lclfldadd_cs_r.dasm.il lclfldadd_cs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=lclfldadd_cs_r.dasm.il lclfldadd_cs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=lclfldadd_cs_r.asm.exe  lclfldadd_cs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=lclfldadd_cs_r.asm.exe  lclfldadd_cs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=lclfldadd_cs_r
    if not exist "lclfldadd_cs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "lclfldadd_cs_r.org" (
            mkdir IL
            copy lclfldadd_cs_r.exe IL\lclfldadd_cs_r.exe 
            ren lclfldadd_cs_r.exe lclfldadd_cs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in lclfldadd_cs_r.org /out lclfldadd_cs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in lclfldadd_cs_r.org /out lclfldadd_cs_r.exe
...
set ExePath=lclfldadd_cs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in lclfldadd_cs_r.reflect.xml
    IF EXIST lclfldadd_cs_r.reflect.xml set ReflectionRoots=-x lclfldadd_cs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\lclfldadd_cs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% lclfldadd_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% lclfldadd_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
