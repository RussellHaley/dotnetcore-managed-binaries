...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/trashreg_il_d trashreg_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/trashreg_il_d trashreg_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trashreg_il_d.dasm.il trashreg_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=trashreg_il_d.dasm.il trashreg_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trashreg_il_d.asm.exe -QUIET -NOLOGO -DEBUG trashreg_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=trashreg_il_d.asm.exe -QUIET -NOLOGO -DEBUG trashreg_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trashreg_il_d
    if [ ! -f trashreg_il_d.org ]; then
        TakeLock
        if [ ! -f trashreg_il_d.org ]; then
          mkdir IL
          cp trashreg_il_d.exe IL/trashreg_il_d.exe
          mv trashreg_il_d.exe trashreg_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trashreg_il_d.org /out trashreg_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trashreg_il_d.org /out trashreg_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in trashreg_il_d.reflect.xml
  if [ -f trashreg_il_d.reflect.xml ]
then
    ReflectionRoots="-x trashreg_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trashreg_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trashreg_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trashreg_il_d.asm.exe $CLRTestExecutionArguments
