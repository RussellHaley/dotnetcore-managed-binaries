...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\callipinvoke callipinvoke.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\callipinvoke callipinvoke.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=callipinvoke.dasm.il callipinvoke.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=callipinvoke.dasm.il callipinvoke.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=callipinvoke.asm.exe -QUIET -NOLOGO -DEBUG callipinvoke.dasm.il
    %CORE_ROOT%\ilasm.exe /output=callipinvoke.asm.exe -QUIET -NOLOGO -DEBUG callipinvoke.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=callipinvoke
    if not exist "callipinvoke.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "callipinvoke.org" (
            mkdir IL
            copy callipinvoke.exe IL\callipinvoke.exe 
            ren callipinvoke.exe callipinvoke.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in callipinvoke.org /out callipinvoke.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in callipinvoke.org /out callipinvoke.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in callipinvoke.reflect.xml
    IF EXIST callipinvoke.reflect.xml set ReflectionRoots=-x callipinvoke.reflect.xml
    ...
    set ExePath=%LinkBin%\callipinvoke.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% callipinvoke.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% callipinvoke.asm.exe %CLRTestExecutionArguments% %Host_Args%
