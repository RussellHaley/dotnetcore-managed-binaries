...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/lclfldadd_cs_r lclfldadd_cs_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/lclfldadd_cs_r lclfldadd_cs_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=lclfldadd_cs_r.dasm.il lclfldadd_cs_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=lclfldadd_cs_r.dasm.il lclfldadd_cs_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=lclfldadd_cs_r.asm.exe  lclfldadd_cs_r.dasm.il
    "$CORE_ROOT/ilasm" -output=lclfldadd_cs_r.asm.exe  lclfldadd_cs_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=lclfldadd_cs_r
    if [ ! -f lclfldadd_cs_r.org ]; then
        TakeLock
        if [ ! -f lclfldadd_cs_r.org ]; then
          mkdir IL
          cp lclfldadd_cs_r.exe IL/lclfldadd_cs_r.exe
          mv lclfldadd_cs_r.exe lclfldadd_cs_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in lclfldadd_cs_r.org /out lclfldadd_cs_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in lclfldadd_cs_r.org /out lclfldadd_cs_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in lclfldadd_cs_r.reflect.xml
  if [ -f lclfldadd_cs_r.reflect.xml ]
then
    ReflectionRoots="-x lclfldadd_cs_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/lclfldadd_cs_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" lclfldadd_cs_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" lclfldadd_cs_r.asm.exe $CLRTestExecutionArguments
