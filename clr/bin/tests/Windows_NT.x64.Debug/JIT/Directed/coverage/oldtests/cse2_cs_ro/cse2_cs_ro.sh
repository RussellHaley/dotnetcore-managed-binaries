...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/cse2_cs_ro cse2_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/oldtests/cse2_cs_ro cse2_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=cse2_cs_ro.dasm.il cse2_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=cse2_cs_ro.dasm.il cse2_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=cse2_cs_ro.asm.exe  cse2_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=cse2_cs_ro.asm.exe  cse2_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=cse2_cs_ro
    if [ ! -f cse2_cs_ro.org ]; then
        TakeLock
        if [ ! -f cse2_cs_ro.org ]; then
          mkdir IL
          cp cse2_cs_ro.exe IL/cse2_cs_ro.exe
          mv cse2_cs_ro.exe cse2_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cse2_cs_ro.org /out cse2_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cse2_cs_ro.org /out cse2_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in cse2_cs_ro.reflect.xml
  if [ -f cse2_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x cse2_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/cse2_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" cse2_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" cse2_cs_ro.asm.exe $CLRTestExecutionArguments
