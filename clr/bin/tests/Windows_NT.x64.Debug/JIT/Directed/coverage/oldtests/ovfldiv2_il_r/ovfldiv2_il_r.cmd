...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\ovfldiv2_il_r ovfldiv2_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\oldtests\ovfldiv2_il_r ovfldiv2_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ovfldiv2_il_r.dasm.il ovfldiv2_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ovfldiv2_il_r.dasm.il ovfldiv2_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ovfldiv2_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT ovfldiv2_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ovfldiv2_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT ovfldiv2_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ovfldiv2_il_r
    if not exist "ovfldiv2_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ovfldiv2_il_r.org" (
            mkdir IL
            copy ovfldiv2_il_r.exe IL\ovfldiv2_il_r.exe 
            ren ovfldiv2_il_r.exe ovfldiv2_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ovfldiv2_il_r.org /out ovfldiv2_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ovfldiv2_il_r.org /out ovfldiv2_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ovfldiv2_il_r.reflect.xml
    IF EXIST ovfldiv2_il_r.reflect.xml set ReflectionRoots=-x ovfldiv2_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\ovfldiv2_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ovfldiv2_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ovfldiv2_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
