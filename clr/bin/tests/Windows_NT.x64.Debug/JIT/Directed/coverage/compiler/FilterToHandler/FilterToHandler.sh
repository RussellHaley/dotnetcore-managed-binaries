...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/compiler/FilterToHandler FilterToHandler.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/compiler/FilterToHandler FilterToHandler.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=FilterToHandler.dasm.il FilterToHandler.exe
    "$CORE_ROOT/ildasm" -raweh -out=FilterToHandler.dasm.il FilterToHandler.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=FilterToHandler.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE FilterToHandler.dasm.il
    "$CORE_ROOT/ilasm" -output=FilterToHandler.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE FilterToHandler.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=FilterToHandler
    if [ ! -f FilterToHandler.org ]; then
        TakeLock
        if [ ! -f FilterToHandler.org ]; then
          mkdir IL
          cp FilterToHandler.exe IL/FilterToHandler.exe
          mv FilterToHandler.exe FilterToHandler.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FilterToHandler.org /out FilterToHandler.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FilterToHandler.org /out FilterToHandler.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in FilterToHandler.reflect.xml
  if [ -f FilterToHandler.reflect.xml ]
then
    ReflectionRoots="-x FilterToHandler.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/FilterToHandler.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" FilterToHandler.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" FilterToHandler.asm.exe $CLRTestExecutionArguments
