...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\compiler\FilterToHandler FilterToHandler.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\compiler\FilterToHandler FilterToHandler.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=FilterToHandler.dasm.il FilterToHandler.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=FilterToHandler.dasm.il FilterToHandler.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=FilterToHandler.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE FilterToHandler.dasm.il
    %CORE_ROOT%\ilasm.exe /output=FilterToHandler.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE FilterToHandler.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=FilterToHandler
    if not exist "FilterToHandler.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "FilterToHandler.org" (
            mkdir IL
            copy FilterToHandler.exe IL\FilterToHandler.exe 
            ren FilterToHandler.exe FilterToHandler.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FilterToHandler.org /out FilterToHandler.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FilterToHandler.org /out FilterToHandler.exe
...
set ExePath=FilterToHandler.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in FilterToHandler.reflect.xml
    IF EXIST FilterToHandler.reflect.xml set ReflectionRoots=-x FilterToHandler.reflect.xml
    ...
    set ExePath=%LinkBin%\FilterToHandler.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% FilterToHandler.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% FilterToHandler.asm.exe %CLRTestExecutionArguments% %Host_Args%
