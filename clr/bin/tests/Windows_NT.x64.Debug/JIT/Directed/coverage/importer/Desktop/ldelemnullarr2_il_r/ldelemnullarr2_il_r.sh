...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/ldelemnullarr2_il_r ldelemnullarr2_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/ldelemnullarr2_il_r ldelemnullarr2_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ldelemnullarr2_il_r.dasm.il ldelemnullarr2_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=ldelemnullarr2_il_r.dasm.il ldelemnullarr2_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ldelemnullarr2_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT ldelemnullarr2_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=ldelemnullarr2_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT ldelemnullarr2_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ldelemnullarr2_il_r
    if [ ! -f ldelemnullarr2_il_r.org ]; then
        TakeLock
        if [ ! -f ldelemnullarr2_il_r.org ]; then
          mkdir IL
          cp ldelemnullarr2_il_r.exe IL/ldelemnullarr2_il_r.exe
          mv ldelemnullarr2_il_r.exe ldelemnullarr2_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldelemnullarr2_il_r.org /out ldelemnullarr2_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldelemnullarr2_il_r.org /out ldelemnullarr2_il_r.exe
          __cgExitCode=$?
...
ExePath=ldelemnullarr2_il_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in ldelemnullarr2_il_r.reflect.xml
  if [ -f ldelemnullarr2_il_r.reflect.xml ]
then
    ReflectionRoots="-x ldelemnullarr2_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ldelemnullarr2_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ldelemnullarr2_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ldelemnullarr2_il_r.asm.exe $CLRTestExecutionArguments
