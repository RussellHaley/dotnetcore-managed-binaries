...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/ceeillegal ceeillegal.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/ceeillegal ceeillegal.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ceeillegal.dasm.il ceeillegal.exe
    "$CORE_ROOT/ildasm" -raweh -out=ceeillegal.dasm.il ceeillegal.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ceeillegal.asm.exe -QUIET -NOLOGO -DEBUG ceeillegal.dasm.il
    "$CORE_ROOT/ilasm" -output=ceeillegal.asm.exe -QUIET -NOLOGO -DEBUG ceeillegal.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ceeillegal
    if [ ! -f ceeillegal.org ]; then
        TakeLock
        if [ ! -f ceeillegal.org ]; then
          mkdir IL
          cp ceeillegal.exe IL/ceeillegal.exe
          mv ceeillegal.exe ceeillegal.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ceeillegal.org /out ceeillegal.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ceeillegal.org /out ceeillegal.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ceeillegal.reflect.xml
  if [ -f ceeillegal.reflect.xml ]
then
    ReflectionRoots="-x ceeillegal.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ceeillegal.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ceeillegal.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ceeillegal.asm.exe $CLRTestExecutionArguments
