...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\badendfinally_il_r badendfinally_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\badendfinally_il_r badendfinally_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=badendfinally_il_r.dasm.il badendfinally_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=badendfinally_il_r.dasm.il badendfinally_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=badendfinally_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badendfinally_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=badendfinally_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badendfinally_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=badendfinally_il_r
    if not exist "badendfinally_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "badendfinally_il_r.org" (
            mkdir IL
            copy badendfinally_il_r.exe IL\badendfinally_il_r.exe 
            ren badendfinally_il_r.exe badendfinally_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badendfinally_il_r.org /out badendfinally_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badendfinally_il_r.org /out badendfinally_il_r.exe
...
set ExePath=badendfinally_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in badendfinally_il_r.reflect.xml
    IF EXIST badendfinally_il_r.reflect.xml set ReflectionRoots=-x badendfinally_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\badendfinally_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% badendfinally_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% badendfinally_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
