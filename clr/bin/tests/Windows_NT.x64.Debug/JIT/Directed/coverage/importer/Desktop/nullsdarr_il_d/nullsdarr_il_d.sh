...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/nullsdarr_il_d nullsdarr_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/nullsdarr_il_d nullsdarr_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nullsdarr_il_d.dasm.il nullsdarr_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=nullsdarr_il_d.dasm.il nullsdarr_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nullsdarr_il_d.asm.exe -QUIET -NOLOGO -DEBUG nullsdarr_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=nullsdarr_il_d.asm.exe -QUIET -NOLOGO -DEBUG nullsdarr_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nullsdarr_il_d
    if [ ! -f nullsdarr_il_d.org ]; then
        TakeLock
        if [ ! -f nullsdarr_il_d.org ]; then
          mkdir IL
          cp nullsdarr_il_d.exe IL/nullsdarr_il_d.exe
          mv nullsdarr_il_d.exe nullsdarr_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nullsdarr_il_d.org /out nullsdarr_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nullsdarr_il_d.org /out nullsdarr_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in nullsdarr_il_d.reflect.xml
  if [ -f nullsdarr_il_d.reflect.xml ]
then
    ReflectionRoots="-x nullsdarr_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nullsdarr_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nullsdarr_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nullsdarr_il_d.asm.exe $CLRTestExecutionArguments
