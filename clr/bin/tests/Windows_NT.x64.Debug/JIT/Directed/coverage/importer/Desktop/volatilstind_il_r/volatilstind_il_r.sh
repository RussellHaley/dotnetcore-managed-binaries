...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/volatilstind_il_r volatilstind_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/volatilstind_il_r volatilstind_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=volatilstind_il_r.dasm.il volatilstind_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=volatilstind_il_r.dasm.il volatilstind_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=volatilstind_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatilstind_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=volatilstind_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatilstind_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=volatilstind_il_r
    if [ ! -f volatilstind_il_r.org ]; then
        TakeLock
        if [ ! -f volatilstind_il_r.org ]; then
          mkdir IL
          cp volatilstind_il_r.exe IL/volatilstind_il_r.exe
          mv volatilstind_il_r.exe volatilstind_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilstind_il_r.org /out volatilstind_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilstind_il_r.org /out volatilstind_il_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in volatilstind_il_r.reflect.xml
  if [ -f volatilstind_il_r.reflect.xml ]
then
    ReflectionRoots="-x volatilstind_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/volatilstind_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" volatilstind_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" volatilstind_il_r.asm.exe $CLRTestExecutionArguments
