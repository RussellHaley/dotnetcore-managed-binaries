...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/nullsdarr_il_r nullsdarr_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/nullsdarr_il_r nullsdarr_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nullsdarr_il_r.dasm.il nullsdarr_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=nullsdarr_il_r.dasm.il nullsdarr_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nullsdarr_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nullsdarr_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=nullsdarr_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nullsdarr_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nullsdarr_il_r
    if [ ! -f nullsdarr_il_r.org ]; then
        TakeLock
        if [ ! -f nullsdarr_il_r.org ]; then
          mkdir IL
          cp nullsdarr_il_r.exe IL/nullsdarr_il_r.exe
          mv nullsdarr_il_r.exe nullsdarr_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nullsdarr_il_r.org /out nullsdarr_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nullsdarr_il_r.org /out nullsdarr_il_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in nullsdarr_il_r.reflect.xml
  if [ -f nullsdarr_il_r.reflect.xml ]
then
    ReflectionRoots="-x nullsdarr_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nullsdarr_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nullsdarr_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nullsdarr_il_r.asm.exe $CLRTestExecutionArguments
