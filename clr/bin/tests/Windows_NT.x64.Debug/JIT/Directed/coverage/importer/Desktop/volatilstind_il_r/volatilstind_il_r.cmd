...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\volatilstind_il_r volatilstind_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\volatilstind_il_r volatilstind_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=volatilstind_il_r.dasm.il volatilstind_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=volatilstind_il_r.dasm.il volatilstind_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=volatilstind_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatilstind_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=volatilstind_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatilstind_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=volatilstind_il_r
    if not exist "volatilstind_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "volatilstind_il_r.org" (
            mkdir IL
            copy volatilstind_il_r.exe IL\volatilstind_il_r.exe 
            ren volatilstind_il_r.exe volatilstind_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilstind_il_r.org /out volatilstind_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilstind_il_r.org /out volatilstind_il_r.exe
...
set ExePath=volatilstind_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in volatilstind_il_r.reflect.xml
    IF EXIST volatilstind_il_r.reflect.xml set ReflectionRoots=-x volatilstind_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\volatilstind_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% volatilstind_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% volatilstind_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
