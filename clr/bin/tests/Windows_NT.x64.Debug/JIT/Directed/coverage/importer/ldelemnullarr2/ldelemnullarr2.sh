...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/ldelemnullarr2 ldelemnullarr2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/ldelemnullarr2 ldelemnullarr2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ldelemnullarr2.dasm.il ldelemnullarr2.exe
    "$CORE_ROOT/ildasm" -raweh -out=ldelemnullarr2.dasm.il ldelemnullarr2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ldelemnullarr2.asm.exe -QUIET -NOLOGO -DEBUG ldelemnullarr2.dasm.il
    "$CORE_ROOT/ilasm" -output=ldelemnullarr2.asm.exe -QUIET -NOLOGO -DEBUG ldelemnullarr2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ldelemnullarr2
    if [ ! -f ldelemnullarr2.org ]; then
        TakeLock
        if [ ! -f ldelemnullarr2.org ]; then
          mkdir IL
          cp ldelemnullarr2.exe IL/ldelemnullarr2.exe
          mv ldelemnullarr2.exe ldelemnullarr2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldelemnullarr2.org /out ldelemnullarr2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldelemnullarr2.org /out ldelemnullarr2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ldelemnullarr2.reflect.xml
  if [ -f ldelemnullarr2.reflect.xml ]
then
    ReflectionRoots="-x ldelemnullarr2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ldelemnullarr2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ldelemnullarr2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ldelemnullarr2.asm.exe $CLRTestExecutionArguments
