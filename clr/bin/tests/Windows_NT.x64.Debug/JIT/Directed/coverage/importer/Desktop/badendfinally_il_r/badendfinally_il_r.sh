...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/badendfinally_il_r badendfinally_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/badendfinally_il_r badendfinally_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=badendfinally_il_r.dasm.il badendfinally_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=badendfinally_il_r.dasm.il badendfinally_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=badendfinally_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badendfinally_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=badendfinally_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badendfinally_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=badendfinally_il_r
    if [ ! -f badendfinally_il_r.org ]; then
        TakeLock
        if [ ! -f badendfinally_il_r.org ]; then
          mkdir IL
          cp badendfinally_il_r.exe IL/badendfinally_il_r.exe
          mv badendfinally_il_r.exe badendfinally_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badendfinally_il_r.org /out badendfinally_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badendfinally_il_r.org /out badendfinally_il_r.exe
          __cgExitCode=$?
...
ExePath=badendfinally_il_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in badendfinally_il_r.reflect.xml
  if [ -f badendfinally_il_r.reflect.xml ]
then
    ReflectionRoots="-x badendfinally_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/badendfinally_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" badendfinally_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" badendfinally_il_r.asm.exe $CLRTestExecutionArguments
