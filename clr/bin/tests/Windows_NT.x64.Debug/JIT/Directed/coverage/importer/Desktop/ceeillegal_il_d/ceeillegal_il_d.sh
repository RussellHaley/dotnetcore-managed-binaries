...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/ceeillegal_il_d ceeillegal_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/ceeillegal_il_d ceeillegal_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ceeillegal_il_d.dasm.il ceeillegal_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=ceeillegal_il_d.dasm.il ceeillegal_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ceeillegal_il_d.asm.exe -QUIET -NOLOGO -DEBUG ceeillegal_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=ceeillegal_il_d.asm.exe -QUIET -NOLOGO -DEBUG ceeillegal_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ceeillegal_il_d
    if [ ! -f ceeillegal_il_d.org ]; then
        TakeLock
        if [ ! -f ceeillegal_il_d.org ]; then
          mkdir IL
          cp ceeillegal_il_d.exe IL/ceeillegal_il_d.exe
          mv ceeillegal_il_d.exe ceeillegal_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ceeillegal_il_d.org /out ceeillegal_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ceeillegal_il_d.org /out ceeillegal_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ceeillegal_il_d.reflect.xml
  if [ -f ceeillegal_il_d.reflect.xml ]
then
    ReflectionRoots="-x ceeillegal_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ceeillegal_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ceeillegal_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ceeillegal_il_d.asm.exe $CLRTestExecutionArguments
