...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\volatilldind_il_d volatilldind_il_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\volatilldind_il_d volatilldind_il_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=volatilldind_il_d.dasm.il volatilldind_il_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=volatilldind_il_d.dasm.il volatilldind_il_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=volatilldind_il_d.asm.exe -QUIET -NOLOGO -DEBUG volatilldind_il_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=volatilldind_il_d.asm.exe -QUIET -NOLOGO -DEBUG volatilldind_il_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=volatilldind_il_d
    if not exist "volatilldind_il_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "volatilldind_il_d.org" (
            mkdir IL
            copy volatilldind_il_d.exe IL\volatilldind_il_d.exe 
            ren volatilldind_il_d.exe volatilldind_il_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilldind_il_d.org /out volatilldind_il_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilldind_il_d.org /out volatilldind_il_d.exe
...
set ExePath=volatilldind_il_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in volatilldind_il_d.reflect.xml
    IF EXIST volatilldind_il_d.reflect.xml set ReflectionRoots=-x volatilldind_il_d.reflect.xml
    ...
    set ExePath=%LinkBin%\volatilldind_il_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% volatilldind_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% volatilldind_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
