...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/volatilstind_il_d volatilstind_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/volatilstind_il_d volatilstind_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=volatilstind_il_d.dasm.il volatilstind_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=volatilstind_il_d.dasm.il volatilstind_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=volatilstind_il_d.asm.exe -QUIET -NOLOGO -DEBUG volatilstind_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=volatilstind_il_d.asm.exe -QUIET -NOLOGO -DEBUG volatilstind_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=volatilstind_il_d
    if [ ! -f volatilstind_il_d.org ]; then
        TakeLock
        if [ ! -f volatilstind_il_d.org ]; then
          mkdir IL
          cp volatilstind_il_d.exe IL/volatilstind_il_d.exe
          mv volatilstind_il_d.exe volatilstind_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilstind_il_d.org /out volatilstind_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilstind_il_d.org /out volatilstind_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in volatilstind_il_d.reflect.xml
  if [ -f volatilstind_il_d.reflect.xml ]
then
    ReflectionRoots="-x volatilstind_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/volatilstind_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" volatilstind_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" volatilstind_il_d.asm.exe $CLRTestExecutionArguments
