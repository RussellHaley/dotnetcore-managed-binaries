...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\volatilldind_il_r volatilldind_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\volatilldind_il_r volatilldind_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=volatilldind_il_r.dasm.il volatilldind_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=volatilldind_il_r.dasm.il volatilldind_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=volatilldind_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatilldind_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=volatilldind_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatilldind_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=volatilldind_il_r
    if not exist "volatilldind_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "volatilldind_il_r.org" (
            mkdir IL
            copy volatilldind_il_r.exe IL\volatilldind_il_r.exe 
            ren volatilldind_il_r.exe volatilldind_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilldind_il_r.org /out volatilldind_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilldind_il_r.org /out volatilldind_il_r.exe
...
set ExePath=volatilldind_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in volatilldind_il_r.reflect.xml
    IF EXIST volatilldind_il_r.reflect.xml set ReflectionRoots=-x volatilldind_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\volatilldind_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% volatilldind_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% volatilldind_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
