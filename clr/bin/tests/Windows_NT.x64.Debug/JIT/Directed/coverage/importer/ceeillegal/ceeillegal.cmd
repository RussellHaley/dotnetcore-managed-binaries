...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\ceeillegal ceeillegal.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\ceeillegal ceeillegal.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ceeillegal.dasm.il ceeillegal.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ceeillegal.dasm.il ceeillegal.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ceeillegal.asm.exe -QUIET -NOLOGO -DEBUG ceeillegal.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ceeillegal.asm.exe -QUIET -NOLOGO -DEBUG ceeillegal.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ceeillegal
    if not exist "ceeillegal.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ceeillegal.org" (
            mkdir IL
            copy ceeillegal.exe IL\ceeillegal.exe 
            ren ceeillegal.exe ceeillegal.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ceeillegal.org /out ceeillegal.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ceeillegal.org /out ceeillegal.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ceeillegal.reflect.xml
    IF EXIST ceeillegal.reflect.xml set ReflectionRoots=-x ceeillegal.reflect.xml
    ...
    set ExePath=%LinkBin%\ceeillegal.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ceeillegal.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ceeillegal.asm.exe %CLRTestExecutionArguments% %Host_Args%
