...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\nullsdarr nullsdarr.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\nullsdarr nullsdarr.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nullsdarr.dasm.il nullsdarr.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nullsdarr.dasm.il nullsdarr.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nullsdarr.asm.exe -QUIET -NOLOGO -DEBUG nullsdarr.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nullsdarr.asm.exe -QUIET -NOLOGO -DEBUG nullsdarr.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nullsdarr
    if not exist "nullsdarr.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nullsdarr.org" (
            mkdir IL
            copy nullsdarr.exe IL\nullsdarr.exe 
            ren nullsdarr.exe nullsdarr.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nullsdarr.org /out nullsdarr.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nullsdarr.org /out nullsdarr.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nullsdarr.reflect.xml
    IF EXIST nullsdarr.reflect.xml set ReflectionRoots=-x nullsdarr.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nullsdarr.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nullsdarr.asm.exe %CLRTestExecutionArguments% %Host_Args%
