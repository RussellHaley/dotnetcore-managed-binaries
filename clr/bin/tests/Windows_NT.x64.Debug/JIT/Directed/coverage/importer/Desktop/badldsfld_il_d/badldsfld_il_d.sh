...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/badldsfld_il_d badldsfld_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/badldsfld_il_d badldsfld_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=badldsfld_il_d.dasm.il badldsfld_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=badldsfld_il_d.dasm.il badldsfld_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=badldsfld_il_d.asm.exe -QUIET -NOLOGO -DEBUG badldsfld_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=badldsfld_il_d.asm.exe -QUIET -NOLOGO -DEBUG badldsfld_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=badldsfld_il_d
    if [ ! -f badldsfld_il_d.org ]; then
        TakeLock
        if [ ! -f badldsfld_il_d.org ]; then
          mkdir IL
          cp badldsfld_il_d.exe IL/badldsfld_il_d.exe
          mv badldsfld_il_d.exe badldsfld_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badldsfld_il_d.org /out badldsfld_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badldsfld_il_d.org /out badldsfld_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in badldsfld_il_d.reflect.xml
  if [ -f badldsfld_il_d.reflect.xml ]
then
    ReflectionRoots="-x badldsfld_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/badldsfld_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" badldsfld_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" badldsfld_il_d.asm.exe $CLRTestExecutionArguments
