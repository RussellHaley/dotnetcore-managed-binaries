...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/volatilldind volatilldind.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/volatilldind volatilldind.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=volatilldind.dasm.il volatilldind.exe
    "$CORE_ROOT/ildasm" -raweh -out=volatilldind.dasm.il volatilldind.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=volatilldind.asm.exe -QUIET -NOLOGO -DEBUG volatilldind.dasm.il
    "$CORE_ROOT/ilasm" -output=volatilldind.asm.exe -QUIET -NOLOGO -DEBUG volatilldind.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=volatilldind
    if [ ! -f volatilldind.org ]; then
        TakeLock
        if [ ! -f volatilldind.org ]; then
          mkdir IL
          cp volatilldind.exe IL/volatilldind.exe
          mv volatilldind.exe volatilldind.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilldind.org /out volatilldind.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilldind.org /out volatilldind.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in volatilldind.reflect.xml
  if [ -f volatilldind.reflect.xml ]
then
    ReflectionRoots="-x volatilldind.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/volatilldind.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" volatilldind.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" volatilldind.asm.exe $CLRTestExecutionArguments
