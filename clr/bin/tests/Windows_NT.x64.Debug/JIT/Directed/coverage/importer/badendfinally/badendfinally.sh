...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/badendfinally badendfinally.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/badendfinally badendfinally.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=badendfinally.dasm.il badendfinally.exe
    "$CORE_ROOT/ildasm" -raweh -out=badendfinally.dasm.il badendfinally.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=badendfinally.asm.exe -QUIET -NOLOGO -DEBUG badendfinally.dasm.il
    "$CORE_ROOT/ilasm" -output=badendfinally.asm.exe -QUIET -NOLOGO -DEBUG badendfinally.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=badendfinally
    if [ ! -f badendfinally.org ]; then
        TakeLock
        if [ ! -f badendfinally.org ]; then
          mkdir IL
          cp badendfinally.exe IL/badendfinally.exe
          mv badendfinally.exe badendfinally.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badendfinally.org /out badendfinally.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badendfinally.org /out badendfinally.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in badendfinally.reflect.xml
  if [ -f badendfinally.reflect.xml ]
then
    ReflectionRoots="-x badendfinally.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/badendfinally.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" badendfinally.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" badendfinally.asm.exe $CLRTestExecutionArguments
