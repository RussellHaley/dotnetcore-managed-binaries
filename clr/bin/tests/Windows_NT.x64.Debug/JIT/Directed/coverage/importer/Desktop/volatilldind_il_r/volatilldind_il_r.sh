...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/volatilldind_il_r volatilldind_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/coverage/importer/Desktop/volatilldind_il_r volatilldind_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=volatilldind_il_r.dasm.il volatilldind_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=volatilldind_il_r.dasm.il volatilldind_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=volatilldind_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatilldind_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=volatilldind_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatilldind_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=volatilldind_il_r
    if [ ! -f volatilldind_il_r.org ]; then
        TakeLock
        if [ ! -f volatilldind_il_r.org ]; then
          mkdir IL
          cp volatilldind_il_r.exe IL/volatilldind_il_r.exe
          mv volatilldind_il_r.exe volatilldind_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilldind_il_r.org /out volatilldind_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilldind_il_r.org /out volatilldind_il_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in volatilldind_il_r.reflect.xml
  if [ -f volatilldind_il_r.reflect.xml ]
then
    ReflectionRoots="-x volatilldind_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/volatilldind_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" volatilldind_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" volatilldind_il_r.asm.exe $CLRTestExecutionArguments
