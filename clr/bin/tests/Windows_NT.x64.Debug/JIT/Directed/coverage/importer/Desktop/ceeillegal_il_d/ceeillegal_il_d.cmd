...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\ceeillegal_il_d ceeillegal_il_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\ceeillegal_il_d ceeillegal_il_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ceeillegal_il_d.dasm.il ceeillegal_il_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ceeillegal_il_d.dasm.il ceeillegal_il_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ceeillegal_il_d.asm.exe -QUIET -NOLOGO -DEBUG ceeillegal_il_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ceeillegal_il_d.asm.exe -QUIET -NOLOGO -DEBUG ceeillegal_il_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ceeillegal_il_d
    if not exist "ceeillegal_il_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ceeillegal_il_d.org" (
            mkdir IL
            copy ceeillegal_il_d.exe IL\ceeillegal_il_d.exe 
            ren ceeillegal_il_d.exe ceeillegal_il_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ceeillegal_il_d.org /out ceeillegal_il_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ceeillegal_il_d.org /out ceeillegal_il_d.exe
...
set ExePath=ceeillegal_il_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ceeillegal_il_d.reflect.xml
    IF EXIST ceeillegal_il_d.reflect.xml set ReflectionRoots=-x ceeillegal_il_d.reflect.xml
    ...
    set ExePath=%LinkBin%\ceeillegal_il_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ceeillegal_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ceeillegal_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
