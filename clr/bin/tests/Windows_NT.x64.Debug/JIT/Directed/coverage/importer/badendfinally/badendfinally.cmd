...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\badendfinally badendfinally.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\badendfinally badendfinally.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=badendfinally.dasm.il badendfinally.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=badendfinally.dasm.il badendfinally.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=badendfinally.asm.exe -QUIET -NOLOGO -DEBUG badendfinally.dasm.il
    %CORE_ROOT%\ilasm.exe /output=badendfinally.asm.exe -QUIET -NOLOGO -DEBUG badendfinally.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=badendfinally
    if not exist "badendfinally.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "badendfinally.org" (
            mkdir IL
            copy badendfinally.exe IL\badendfinally.exe 
            ren badendfinally.exe badendfinally.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badendfinally.org /out badendfinally.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badendfinally.org /out badendfinally.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in badendfinally.reflect.xml
    IF EXIST badendfinally.reflect.xml set ReflectionRoots=-x badendfinally.reflect.xml
    ...
    set ExePath=%LinkBin%\badendfinally.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% badendfinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% badendfinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
