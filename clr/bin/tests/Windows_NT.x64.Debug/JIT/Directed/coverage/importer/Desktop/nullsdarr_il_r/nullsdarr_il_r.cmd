...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\nullsdarr_il_r nullsdarr_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\nullsdarr_il_r nullsdarr_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nullsdarr_il_r.dasm.il nullsdarr_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nullsdarr_il_r.dasm.il nullsdarr_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nullsdarr_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nullsdarr_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nullsdarr_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nullsdarr_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nullsdarr_il_r
    if not exist "nullsdarr_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nullsdarr_il_r.org" (
            mkdir IL
            copy nullsdarr_il_r.exe IL\nullsdarr_il_r.exe 
            ren nullsdarr_il_r.exe nullsdarr_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nullsdarr_il_r.org /out nullsdarr_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nullsdarr_il_r.org /out nullsdarr_il_r.exe
...
set ExePath=nullsdarr_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nullsdarr_il_r.reflect.xml
    IF EXIST nullsdarr_il_r.reflect.xml set ReflectionRoots=-x nullsdarr_il_r.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nullsdarr_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nullsdarr_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
