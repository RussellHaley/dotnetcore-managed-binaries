...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\volatilldind volatilldind.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\volatilldind volatilldind.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=volatilldind.dasm.il volatilldind.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=volatilldind.dasm.il volatilldind.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=volatilldind.asm.exe -QUIET -NOLOGO -DEBUG volatilldind.dasm.il
    %CORE_ROOT%\ilasm.exe /output=volatilldind.asm.exe -QUIET -NOLOGO -DEBUG volatilldind.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=volatilldind
    if not exist "volatilldind.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "volatilldind.org" (
            mkdir IL
            copy volatilldind.exe IL\volatilldind.exe 
            ren volatilldind.exe volatilldind.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilldind.org /out volatilldind.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilldind.org /out volatilldind.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in volatilldind.reflect.xml
    IF EXIST volatilldind.reflect.xml set ReflectionRoots=-x volatilldind.reflect.xml
    ...
    set ExePath=%LinkBin%\volatilldind.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% volatilldind.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% volatilldind.asm.exe %CLRTestExecutionArguments% %Host_Args%
