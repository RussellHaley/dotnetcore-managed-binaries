...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/volatilstind volatilstind.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/volatilstind volatilstind.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=volatilstind.dasm.il volatilstind.exe
    "$CORE_ROOT/ildasm" -raweh -out=volatilstind.dasm.il volatilstind.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=volatilstind.asm.exe -QUIET -NOLOGO -DEBUG volatilstind.dasm.il
    "$CORE_ROOT/ilasm" -output=volatilstind.asm.exe -QUIET -NOLOGO -DEBUG volatilstind.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=volatilstind
    if [ ! -f volatilstind.org ]; then
        TakeLock
        if [ ! -f volatilstind.org ]; then
          mkdir IL
          cp volatilstind.exe IL/volatilstind.exe
          mv volatilstind.exe volatilstind.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilstind.org /out volatilstind.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatilstind.org /out volatilstind.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in volatilstind.reflect.xml
  if [ -f volatilstind.reflect.xml ]
then
    ReflectionRoots="-x volatilstind.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/volatilstind.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" volatilstind.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" volatilstind.asm.exe $CLRTestExecutionArguments
