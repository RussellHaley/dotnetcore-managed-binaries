...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/nullsdarr nullsdarr.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/coverage/importer/nullsdarr nullsdarr.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nullsdarr.dasm.il nullsdarr.exe
    "$CORE_ROOT/ildasm" -raweh -out=nullsdarr.dasm.il nullsdarr.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nullsdarr.asm.exe -QUIET -NOLOGO -DEBUG nullsdarr.dasm.il
    "$CORE_ROOT/ilasm" -output=nullsdarr.asm.exe -QUIET -NOLOGO -DEBUG nullsdarr.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nullsdarr
    if [ ! -f nullsdarr.org ]; then
        TakeLock
        if [ ! -f nullsdarr.org ]; then
          mkdir IL
          cp nullsdarr.exe IL/nullsdarr.exe
          mv nullsdarr.exe nullsdarr.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nullsdarr.org /out nullsdarr.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nullsdarr.org /out nullsdarr.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in nullsdarr.reflect.xml
  if [ -f nullsdarr.reflect.xml ]
then
    ReflectionRoots="-x nullsdarr.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nullsdarr.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nullsdarr.asm.exe $CLRTestExecutionArguments
