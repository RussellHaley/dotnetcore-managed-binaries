...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nullsdarr_il_d.dasm.il nullsdarr_il_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nullsdarr_il_d.dasm.il nullsdarr_il_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nullsdarr_il_d.asm.exe -QUIET -NOLOGO -DEBUG nullsdarr_il_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nullsdarr_il_d.asm.exe -QUIET -NOLOGO -DEBUG nullsdarr_il_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nullsdarr_il_d
    if not exist "nullsdarr_il_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nullsdarr_il_d.org" (
            mkdir IL
            copy nullsdarr_il_d.exe IL\nullsdarr_il_d.exe 
            ren nullsdarr_il_d.exe nullsdarr_il_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nullsdarr_il_d.org /out nullsdarr_il_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nullsdarr_il_d.org /out nullsdarr_il_d.exe
...
set ExePath=nullsdarr_il_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nullsdarr_il_d.reflect.xml
    IF EXIST nullsdarr_il_d.reflect.xml set ReflectionRoots=-x nullsdarr_il_d.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nullsdarr_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nullsdarr_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
