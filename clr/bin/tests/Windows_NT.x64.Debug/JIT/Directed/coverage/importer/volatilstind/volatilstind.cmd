...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\volatilstind volatilstind.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\coverage\importer\volatilstind volatilstind.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=volatilstind.dasm.il volatilstind.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=volatilstind.dasm.il volatilstind.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=volatilstind.asm.exe -QUIET -NOLOGO -DEBUG volatilstind.dasm.il
    %CORE_ROOT%\ilasm.exe /output=volatilstind.asm.exe -QUIET -NOLOGO -DEBUG volatilstind.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=volatilstind
    if not exist "volatilstind.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "volatilstind.org" (
            mkdir IL
            copy volatilstind.exe IL\volatilstind.exe 
            ren volatilstind.exe volatilstind.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilstind.org /out volatilstind.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatilstind.org /out volatilstind.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in volatilstind.reflect.xml
    IF EXIST volatilstind.reflect.xml set ReflectionRoots=-x volatilstind.reflect.xml
    ...
    set ExePath=%LinkBin%\volatilstind.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% volatilstind.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% volatilstind.asm.exe %CLRTestExecutionArguments% %Host_Args%
