...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\badldsfld_il_r badldsfld_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\coverage\importer\Desktop\badldsfld_il_r badldsfld_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=badldsfld_il_r.dasm.il badldsfld_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=badldsfld_il_r.dasm.il badldsfld_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=badldsfld_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badldsfld_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=badldsfld_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badldsfld_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=badldsfld_il_r
    if not exist "badldsfld_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "badldsfld_il_r.org" (
            mkdir IL
            copy badldsfld_il_r.exe IL\badldsfld_il_r.exe 
            ren badldsfld_il_r.exe badldsfld_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badldsfld_il_r.org /out badldsfld_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badldsfld_il_r.org /out badldsfld_il_r.exe
...
set ExePath=badldsfld_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in badldsfld_il_r.reflect.xml
    IF EXIST badldsfld_il_r.reflect.xml set ReflectionRoots=-x badldsfld_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\badldsfld_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% badldsfld_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% badldsfld_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
