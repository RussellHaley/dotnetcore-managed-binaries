...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\shift\uint16_ro uint16_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\shift\uint16_ro uint16_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=uint16_ro.dasm.il uint16_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=uint16_ro.dasm.il uint16_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=uint16_ro.asm.exe  uint16_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=uint16_ro.asm.exe  uint16_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=uint16_ro
    if not exist "uint16_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "uint16_ro.org" (
            mkdir IL
            copy uint16_ro.exe IL\uint16_ro.exe 
            ren uint16_ro.exe uint16_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in uint16_ro.org /out uint16_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in uint16_ro.org /out uint16_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in uint16_ro.reflect.xml
    IF EXIST uint16_ro.reflect.xml set ReflectionRoots=-x uint16_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\uint16_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% uint16_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% uint16_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
