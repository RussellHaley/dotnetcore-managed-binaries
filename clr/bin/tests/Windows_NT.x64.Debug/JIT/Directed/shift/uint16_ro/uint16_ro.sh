...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/shift/uint16_ro uint16_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/shift/uint16_ro uint16_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=uint16_ro.dasm.il uint16_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=uint16_ro.dasm.il uint16_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=uint16_ro.asm.exe  uint16_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=uint16_ro.asm.exe  uint16_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=uint16_ro
    if [ ! -f uint16_ro.org ]; then
        TakeLock
        if [ ! -f uint16_ro.org ]; then
          mkdir IL
          cp uint16_ro.exe IL/uint16_ro.exe
          mv uint16_ro.exe uint16_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in uint16_ro.org /out uint16_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in uint16_ro.org /out uint16_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in uint16_ro.reflect.xml
  if [ -f uint16_ro.reflect.xml ]
then
    ReflectionRoots="-x uint16_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" uint16_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" uint16_ro.asm.exe $CLRTestExecutionArguments
