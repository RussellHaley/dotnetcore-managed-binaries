...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\PREFIX\volatile\1\cpblk cpblk.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Directed\PREFIX\volatile\1\cpblk cpblk.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=cpblk.dasm.il cpblk.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=cpblk.dasm.il cpblk.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=cpblk.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE cpblk.dasm.il
    %CORE_ROOT%\ilasm.exe /output=cpblk.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE cpblk.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=cpblk
    if not exist "cpblk.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "cpblk.org" (
            mkdir IL
            copy cpblk.exe IL\cpblk.exe 
            ren cpblk.exe cpblk.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cpblk.org /out cpblk.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cpblk.org /out cpblk.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in cpblk.reflect.xml
    IF EXIST cpblk.reflect.xml set ReflectionRoots=-x cpblk.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% cpblk.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% cpblk.asm.exe %CLRTestExecutionArguments% %Host_Args%
