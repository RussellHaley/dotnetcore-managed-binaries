...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/PREFIX/volatile/1/cpblk cpblk.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Directed/PREFIX/volatile/1/cpblk cpblk.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=cpblk.dasm.il cpblk.exe
    "$CORE_ROOT/ildasm" -raweh -out=cpblk.dasm.il cpblk.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=cpblk.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE cpblk.dasm.il
    "$CORE_ROOT/ilasm" -output=cpblk.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE cpblk.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=cpblk
    if [ ! -f cpblk.org ]; then
        TakeLock
        if [ ! -f cpblk.org ]; then
          mkdir IL
          cp cpblk.exe IL/cpblk.exe
          mv cpblk.exe cpblk.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cpblk.org /out cpblk.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cpblk.org /out cpblk.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in cpblk.reflect.xml
  if [ -f cpblk.reflect.xml ]
then
    ReflectionRoots="-x cpblk.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" cpblk.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" cpblk.asm.exe $CLRTestExecutionArguments
