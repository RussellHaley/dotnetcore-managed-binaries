...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\array-il\_Arrayscomplex3 _Arrayscomplex3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\array-il\_Arrayscomplex3 _Arrayscomplex3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_Arrayscomplex3.dasm.il _Arrayscomplex3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_Arrayscomplex3.dasm.il _Arrayscomplex3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_Arrayscomplex3.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE _Arrayscomplex3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_Arrayscomplex3.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE _Arrayscomplex3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_Arrayscomplex3
    if not exist "_Arrayscomplex3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_Arrayscomplex3.org" (
            mkdir IL
            copy _Arrayscomplex3.exe IL\_Arrayscomplex3.exe 
            ren _Arrayscomplex3.exe _Arrayscomplex3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _Arrayscomplex3.org /out _Arrayscomplex3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _Arrayscomplex3.org /out _Arrayscomplex3.exe
...
set ExePath=_Arrayscomplex3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _Arrayscomplex3.reflect.xml
    IF EXIST _Arrayscomplex3.reflect.xml set ReflectionRoots=-x _Arrayscomplex3.reflect.xml
    ...
    set ExePath=%LinkBin%\_Arrayscomplex3.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _Arrayscomplex3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _Arrayscomplex3.asm.exe %CLRTestExecutionArguments% %Host_Args%
