...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/array-il/_Arrayscomplex3 _Arrayscomplex3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/array-il/_Arrayscomplex3 _Arrayscomplex3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_Arrayscomplex3.dasm.il _Arrayscomplex3.exe
    "$CORE_ROOT/ildasm" -raweh -out=_Arrayscomplex3.dasm.il _Arrayscomplex3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_Arrayscomplex3.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE _Arrayscomplex3.dasm.il
    "$CORE_ROOT/ilasm" -output=_Arrayscomplex3.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE _Arrayscomplex3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_Arrayscomplex3
    if [ ! -f _Arrayscomplex3.org ]; then
        TakeLock
        if [ ! -f _Arrayscomplex3.org ]; then
          mkdir IL
          cp _Arrayscomplex3.exe IL/_Arrayscomplex3.exe
          mv _Arrayscomplex3.exe _Arrayscomplex3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _Arrayscomplex3.org /out _Arrayscomplex3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _Arrayscomplex3.org /out _Arrayscomplex3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _Arrayscomplex3.reflect.xml
  if [ -f _Arrayscomplex3.reflect.xml ]
then
    ReflectionRoots="-x _Arrayscomplex3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_Arrayscomplex3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _Arrayscomplex3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _Arrayscomplex3.asm.exe $CLRTestExecutionArguments
