...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\Misc\function_pointer\MutualThdRecur-fptr MutualThdRecur-fptr.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\Misc\function_pointer\MutualThdRecur-fptr MutualThdRecur-fptr.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MutualThdRecur-fptr.dasm.il MutualThdRecur-fptr.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MutualThdRecur-fptr.dasm.il MutualThdRecur-fptr.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MutualThdRecur-fptr.asm.exe -QUIET -NOLOGO -DEBUG=OPT MutualThdRecur-fptr.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MutualThdRecur-fptr.asm.exe -QUIET -NOLOGO -DEBUG=OPT MutualThdRecur-fptr.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MutualThdRecur-fptr
    if not exist "MutualThdRecur-fptr.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MutualThdRecur-fptr.org" (
            mkdir IL
            copy MutualThdRecur-fptr.exe IL\MutualThdRecur-fptr.exe 
            ren MutualThdRecur-fptr.exe MutualThdRecur-fptr.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MutualThdRecur-fptr.org /out MutualThdRecur-fptr.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MutualThdRecur-fptr.org /out MutualThdRecur-fptr.exe
...
set ExePath=MutualThdRecur-fptr.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MutualThdRecur-fptr.reflect.xml
    IF EXIST MutualThdRecur-fptr.reflect.xml set ReflectionRoots=-x MutualThdRecur-fptr.reflect.xml
    ...
    set ExePath=%LinkBin%\MutualThdRecur-fptr.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MutualThdRecur-fptr.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MutualThdRecur-fptr.asm.exe %CLRTestExecutionArguments% %Host_Args%
