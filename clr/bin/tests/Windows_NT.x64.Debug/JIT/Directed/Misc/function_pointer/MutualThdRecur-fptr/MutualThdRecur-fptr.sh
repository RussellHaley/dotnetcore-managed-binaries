...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/Misc/function_pointer/MutualThdRecur-fptr MutualThdRecur-fptr.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/Misc/function_pointer/MutualThdRecur-fptr MutualThdRecur-fptr.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MutualThdRecur-fptr.dasm.il MutualThdRecur-fptr.exe
    "$CORE_ROOT/ildasm" -raweh -out=MutualThdRecur-fptr.dasm.il MutualThdRecur-fptr.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MutualThdRecur-fptr.asm.exe -QUIET -NOLOGO -DEBUG=OPT MutualThdRecur-fptr.dasm.il
    "$CORE_ROOT/ilasm" -output=MutualThdRecur-fptr.asm.exe -QUIET -NOLOGO -DEBUG=OPT MutualThdRecur-fptr.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MutualThdRecur-fptr
    if [ ! -f MutualThdRecur-fptr.org ]; then
        TakeLock
        if [ ! -f MutualThdRecur-fptr.org ]; then
          mkdir IL
          cp MutualThdRecur-fptr.exe IL/MutualThdRecur-fptr.exe
          mv MutualThdRecur-fptr.exe MutualThdRecur-fptr.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MutualThdRecur-fptr.org /out MutualThdRecur-fptr.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MutualThdRecur-fptr.org /out MutualThdRecur-fptr.exe
          __cgExitCode=$?
...
ExePath=MutualThdRecur-fptr.exe
...
then
  ...
  # Use hints for reflection roots, if provided in MutualThdRecur-fptr.reflect.xml
  if [ -f MutualThdRecur-fptr.reflect.xml ]
then
    ReflectionRoots="-x MutualThdRecur-fptr.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/MutualThdRecur-fptr.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MutualThdRecur-fptr.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MutualThdRecur-fptr.asm.exe $CLRTestExecutionArguments
