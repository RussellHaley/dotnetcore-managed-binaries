...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\Tailcall\JitTailcall2 JitTailcall2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\Tailcall\JitTailcall2 JitTailcall2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=JitTailcall2.dasm.il JitTailcall2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=JitTailcall2.dasm.il JitTailcall2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=JitTailcall2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE JitTailcall2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=JitTailcall2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE JitTailcall2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=JitTailcall2
    if not exist "JitTailcall2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "JitTailcall2.org" (
            mkdir IL
            copy JitTailcall2.exe IL\JitTailcall2.exe 
            ren JitTailcall2.exe JitTailcall2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in JitTailcall2.org /out JitTailcall2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in JitTailcall2.org /out JitTailcall2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in JitTailcall2.reflect.xml
    IF EXIST JitTailcall2.reflect.xml set ReflectionRoots=-x JitTailcall2.reflect.xml
    ...
    set ExePath=%LinkBin%\JitTailcall2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% JitTailcall2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% JitTailcall2.asm.exe %CLRTestExecutionArguments% %Host_Args%
