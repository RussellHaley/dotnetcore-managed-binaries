...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\Tailcall\jitTailcall1 jitTailcall1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\Tailcall\jitTailcall1 jitTailcall1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=jitTailcall1.dasm.il jitTailcall1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=jitTailcall1.dasm.il jitTailcall1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=jitTailcall1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE jitTailcall1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=jitTailcall1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE jitTailcall1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=jitTailcall1
    if not exist "jitTailcall1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "jitTailcall1.org" (
            mkdir IL
            copy jitTailcall1.exe IL\jitTailcall1.exe 
            ren jitTailcall1.exe jitTailcall1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in jitTailcall1.org /out jitTailcall1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in jitTailcall1.org /out jitTailcall1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in jitTailcall1.reflect.xml
    IF EXIST jitTailcall1.reflect.xml set ReflectionRoots=-x jitTailcall1.reflect.xml
    ...
    set ExePath=%LinkBin%\jitTailcall1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% jitTailcall1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% jitTailcall1.asm.exe %CLRTestExecutionArguments% %Host_Args%
