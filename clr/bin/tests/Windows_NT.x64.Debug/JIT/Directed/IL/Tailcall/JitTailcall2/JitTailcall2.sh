...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/Tailcall/JitTailcall2 JitTailcall2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/Tailcall/JitTailcall2 JitTailcall2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=JitTailcall2.dasm.il JitTailcall2.exe
    "$CORE_ROOT/ildasm" -raweh -out=JitTailcall2.dasm.il JitTailcall2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=JitTailcall2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE JitTailcall2.dasm.il
    "$CORE_ROOT/ilasm" -output=JitTailcall2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE JitTailcall2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=JitTailcall2
    if [ ! -f JitTailcall2.org ]; then
        TakeLock
        if [ ! -f JitTailcall2.org ]; then
          mkdir IL
          cp JitTailcall2.exe IL/JitTailcall2.exe
          mv JitTailcall2.exe JitTailcall2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in JitTailcall2.org /out JitTailcall2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in JitTailcall2.org /out JitTailcall2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in JitTailcall2.reflect.xml
  if [ -f JitTailcall2.reflect.xml ]
then
    ReflectionRoots="-x JitTailcall2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/JitTailcall2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" JitTailcall2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" JitTailcall2.asm.exe $CLRTestExecutionArguments
