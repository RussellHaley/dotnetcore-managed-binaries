...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/Tailcall/jitTailcall1 jitTailcall1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/Tailcall/jitTailcall1 jitTailcall1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=jitTailcall1.dasm.il jitTailcall1.exe
    "$CORE_ROOT/ildasm" -raweh -out=jitTailcall1.dasm.il jitTailcall1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=jitTailcall1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE jitTailcall1.dasm.il
    "$CORE_ROOT/ilasm" -output=jitTailcall1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE jitTailcall1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=jitTailcall1
    if [ ! -f jitTailcall1.org ]; then
        TakeLock
        if [ ! -f jitTailcall1.org ]; then
          mkdir IL
          cp jitTailcall1.exe IL/jitTailcall1.exe
          mv jitTailcall1.exe jitTailcall1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in jitTailcall1.org /out jitTailcall1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in jitTailcall1.org /out jitTailcall1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in jitTailcall1.reflect.xml
  if [ -f jitTailcall1.reflect.xml ]
then
    ReflectionRoots="-x jitTailcall1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/jitTailcall1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" jitTailcall1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" jitTailcall1.asm.exe $CLRTestExecutionArguments
