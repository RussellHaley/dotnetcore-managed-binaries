...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/rethrow/Rethrow1 Rethrow1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/rethrow/Rethrow1 Rethrow1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Rethrow1.dasm.il Rethrow1.exe
    "$CORE_ROOT/ildasm" -raweh -out=Rethrow1.dasm.il Rethrow1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Rethrow1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Rethrow1.dasm.il
    "$CORE_ROOT/ilasm" -output=Rethrow1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Rethrow1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Rethrow1
    if [ ! -f Rethrow1.org ]; then
        TakeLock
        if [ ! -f Rethrow1.org ]; then
          mkdir IL
          cp Rethrow1.exe IL/Rethrow1.exe
          mv Rethrow1.exe Rethrow1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Rethrow1.org /out Rethrow1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Rethrow1.org /out Rethrow1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Rethrow1.reflect.xml
  if [ -f Rethrow1.reflect.xml ]
then
    ReflectionRoots="-x Rethrow1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Rethrow1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Rethrow1.asm.exe $CLRTestExecutionArguments
