...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\rethrow\Rethrow2 Rethrow2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\rethrow\Rethrow2 Rethrow2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Rethrow2.dasm.il Rethrow2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Rethrow2.dasm.il Rethrow2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Rethrow2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Rethrow2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Rethrow2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Rethrow2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Rethrow2
    if not exist "Rethrow2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Rethrow2.org" (
            mkdir IL
            copy Rethrow2.exe IL\Rethrow2.exe 
            ren Rethrow2.exe Rethrow2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Rethrow2.org /out Rethrow2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Rethrow2.org /out Rethrow2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Rethrow2.reflect.xml
    IF EXIST Rethrow2.reflect.xml set ReflectionRoots=-x Rethrow2.reflect.xml
    ...
    set ExePath=%LinkBin%\Rethrow2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Rethrow2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Rethrow2.asm.exe %CLRTestExecutionArguments% %Host_Args%
