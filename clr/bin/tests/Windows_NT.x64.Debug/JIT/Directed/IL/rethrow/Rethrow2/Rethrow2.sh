...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/rethrow/Rethrow2 Rethrow2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/rethrow/Rethrow2 Rethrow2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Rethrow2.dasm.il Rethrow2.exe
    "$CORE_ROOT/ildasm" -raweh -out=Rethrow2.dasm.il Rethrow2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Rethrow2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Rethrow2.dasm.il
    "$CORE_ROOT/ilasm" -output=Rethrow2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Rethrow2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Rethrow2
    if [ ! -f Rethrow2.org ]; then
        TakeLock
        if [ ! -f Rethrow2.org ]; then
          mkdir IL
          cp Rethrow2.exe IL/Rethrow2.exe
          mv Rethrow2.exe Rethrow2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Rethrow2.org /out Rethrow2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Rethrow2.org /out Rethrow2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Rethrow2.reflect.xml
  if [ -f Rethrow2.reflect.xml ]
then
    ReflectionRoots="-x Rethrow2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Rethrow2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Rethrow2.asm.exe $CLRTestExecutionArguments
