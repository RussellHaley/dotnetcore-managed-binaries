...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\rethrow\Rethrow1 Rethrow1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\rethrow\Rethrow1 Rethrow1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Rethrow1.dasm.il Rethrow1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Rethrow1.dasm.il Rethrow1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Rethrow1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Rethrow1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Rethrow1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE Rethrow1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Rethrow1
    if not exist "Rethrow1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Rethrow1.org" (
            mkdir IL
            copy Rethrow1.exe IL\Rethrow1.exe 
            ren Rethrow1.exe Rethrow1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Rethrow1.org /out Rethrow1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Rethrow1.org /out Rethrow1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Rethrow1.reflect.xml
    IF EXIST Rethrow1.reflect.xml set ReflectionRoots=-x Rethrow1.reflect.xml
    ...
    set ExePath=%LinkBin%\Rethrow1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Rethrow1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Rethrow1.asm.exe %CLRTestExecutionArguments% %Host_Args%
