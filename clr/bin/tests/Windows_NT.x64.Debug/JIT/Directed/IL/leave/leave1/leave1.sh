...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/leave/leave1 leave1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/leave/leave1 leave1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=leave1.dasm.il leave1.exe
    "$CORE_ROOT/ildasm" -raweh -out=leave1.dasm.il leave1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=leave1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE leave1.dasm.il
    "$CORE_ROOT/ilasm" -output=leave1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE leave1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=leave1
    if [ ! -f leave1.org ]; then
        TakeLock
        if [ ! -f leave1.org ]; then
          mkdir IL
          cp leave1.exe IL/leave1.exe
          mv leave1.exe leave1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in leave1.org /out leave1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in leave1.org /out leave1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in leave1.reflect.xml
  if [ -f leave1.reflect.xml ]
then
    ReflectionRoots="-x leave1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" leave1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" leave1.asm.exe $CLRTestExecutionArguments
