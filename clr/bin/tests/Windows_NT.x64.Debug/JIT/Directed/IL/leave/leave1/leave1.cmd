...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\leave\leave1 leave1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\leave\leave1 leave1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=leave1.dasm.il leave1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=leave1.dasm.il leave1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=leave1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE leave1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=leave1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE leave1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=leave1
    if not exist "leave1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "leave1.org" (
            mkdir IL
            copy leave1.exe IL\leave1.exe 
            ren leave1.exe leave1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in leave1.org /out leave1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in leave1.org /out leave1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in leave1.reflect.xml
    IF EXIST leave1.reflect.xml set ReflectionRoots=-x leave1.reflect.xml
    ...
    set ExePath=%LinkBin%\leave1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% leave1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% leave1.asm.exe %CLRTestExecutionArguments% %Host_Args%
