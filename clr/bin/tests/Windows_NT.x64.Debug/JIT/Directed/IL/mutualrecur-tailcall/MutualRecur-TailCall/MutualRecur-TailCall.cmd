...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\mutualrecur-tailcall\MutualRecur-TailCall MutualRecur-TailCall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\mutualrecur-tailcall\MutualRecur-TailCall MutualRecur-TailCall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MutualRecur-TailCall.dasm.il MutualRecur-TailCall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MutualRecur-TailCall.dasm.il MutualRecur-TailCall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MutualRecur-TailCall.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE MutualRecur-TailCall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MutualRecur-TailCall.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE MutualRecur-TailCall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MutualRecur-TailCall
    if not exist "MutualRecur-TailCall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MutualRecur-TailCall.org" (
            mkdir IL
            copy MutualRecur-TailCall.exe IL\MutualRecur-TailCall.exe 
            ren MutualRecur-TailCall.exe MutualRecur-TailCall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MutualRecur-TailCall.org /out MutualRecur-TailCall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MutualRecur-TailCall.org /out MutualRecur-TailCall.exe
...
set ExePath=MutualRecur-TailCall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MutualRecur-TailCall.reflect.xml
    IF EXIST MutualRecur-TailCall.reflect.xml set ReflectionRoots=-x MutualRecur-TailCall.reflect.xml
    ...
    set ExePath=%LinkBin%\MutualRecur-TailCall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MutualRecur-TailCall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MutualRecur-TailCall.asm.exe %CLRTestExecutionArguments% %Host_Args%
