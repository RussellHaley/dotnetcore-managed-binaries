...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/mutualrecur-tailcall/MutualRecur-TailCall MutualRecur-TailCall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/mutualrecur-tailcall/MutualRecur-TailCall MutualRecur-TailCall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MutualRecur-TailCall.dasm.il MutualRecur-TailCall.exe
    "$CORE_ROOT/ildasm" -raweh -out=MutualRecur-TailCall.dasm.il MutualRecur-TailCall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MutualRecur-TailCall.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE MutualRecur-TailCall.dasm.il
    "$CORE_ROOT/ilasm" -output=MutualRecur-TailCall.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE MutualRecur-TailCall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MutualRecur-TailCall
    if [ ! -f MutualRecur-TailCall.org ]; then
        TakeLock
        if [ ! -f MutualRecur-TailCall.org ]; then
          mkdir IL
          cp MutualRecur-TailCall.exe IL/MutualRecur-TailCall.exe
          mv MutualRecur-TailCall.exe MutualRecur-TailCall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MutualRecur-TailCall.org /out MutualRecur-TailCall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MutualRecur-TailCall.org /out MutualRecur-TailCall.exe
          __cgExitCode=$?
...
ExePath=MutualRecur-TailCall.exe
...
then
  ...
  # Use hints for reflection roots, if provided in MutualRecur-TailCall.reflect.xml
  if [ -f MutualRecur-TailCall.reflect.xml ]
then
    ReflectionRoots="-x MutualRecur-TailCall.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/MutualRecur-TailCall.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MutualRecur-TailCall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MutualRecur-TailCall.asm.exe $CLRTestExecutionArguments
