...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/PInvokeTail/TailWinApi TailWinApi.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Directed/IL/PInvokeTail/TailWinApi TailWinApi.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=TailWinApi.dasm.il TailWinApi.exe
    "$CORE_ROOT/ildasm" -raweh -out=TailWinApi.dasm.il TailWinApi.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=TailWinApi.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE TailWinApi.dasm.il
    "$CORE_ROOT/ilasm" -output=TailWinApi.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE TailWinApi.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=TailWinApi
    if [ ! -f TailWinApi.org ]; then
        TakeLock
        if [ ! -f TailWinApi.org ]; then
          mkdir IL
          cp TailWinApi.exe IL/TailWinApi.exe
          mv TailWinApi.exe TailWinApi.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TailWinApi.org /out TailWinApi.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TailWinApi.org /out TailWinApi.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in TailWinApi.reflect.xml
  if [ -f TailWinApi.reflect.xml ]
then
    ReflectionRoots="-x TailWinApi.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/TailWinApi.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" TailWinApi.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" TailWinApi.asm.exe $CLRTestExecutionArguments
