...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\PInvokeTail\TailWinApi TailWinApi.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Directed\IL\PInvokeTail\TailWinApi TailWinApi.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=TailWinApi.dasm.il TailWinApi.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=TailWinApi.dasm.il TailWinApi.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=TailWinApi.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE TailWinApi.dasm.il
    %CORE_ROOT%\ilasm.exe /output=TailWinApi.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE TailWinApi.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=TailWinApi
    if not exist "TailWinApi.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "TailWinApi.org" (
            mkdir IL
            copy TailWinApi.exe IL\TailWinApi.exe 
            ren TailWinApi.exe TailWinApi.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TailWinApi.org /out TailWinApi.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TailWinApi.org /out TailWinApi.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in TailWinApi.reflect.xml
    IF EXIST TailWinApi.reflect.xml set ReflectionRoots=-x TailWinApi.reflect.xml
    ...
    set ExePath=%LinkBin%\TailWinApi.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% TailWinApi.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% TailWinApi.asm.exe %CLRTestExecutionArguments% %Host_Args%
