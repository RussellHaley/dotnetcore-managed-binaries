...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\StrAccess\straccess2_cs_do straccess2_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\StrAccess\straccess2_cs_do straccess2_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=straccess2_cs_do.dasm.il straccess2_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=straccess2_cs_do.dasm.il straccess2_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=straccess2_cs_do.asm.exe  straccess2_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=straccess2_cs_do.asm.exe  straccess2_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=straccess2_cs_do
    if not exist "straccess2_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "straccess2_cs_do.org" (
            mkdir IL
            copy straccess2_cs_do.exe IL\straccess2_cs_do.exe 
            ren straccess2_cs_do.exe straccess2_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in straccess2_cs_do.org /out straccess2_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in straccess2_cs_do.org /out straccess2_cs_do.exe
...
set ExePath=straccess2_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in straccess2_cs_do.reflect.xml
    IF EXIST straccess2_cs_do.reflect.xml set ReflectionRoots=-x straccess2_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\straccess2_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% straccess2_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% straccess2_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
