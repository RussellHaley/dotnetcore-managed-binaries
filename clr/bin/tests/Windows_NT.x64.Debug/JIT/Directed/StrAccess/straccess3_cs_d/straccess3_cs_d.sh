...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess3_cs_d straccess3_cs_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess3_cs_d straccess3_cs_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=straccess3_cs_d.dasm.il straccess3_cs_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=straccess3_cs_d.dasm.il straccess3_cs_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=straccess3_cs_d.asm.exe  straccess3_cs_d.dasm.il
    "$CORE_ROOT/ilasm" -output=straccess3_cs_d.asm.exe  straccess3_cs_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=straccess3_cs_d
    if [ ! -f straccess3_cs_d.org ]; then
        TakeLock
        if [ ! -f straccess3_cs_d.org ]; then
          mkdir IL
          cp straccess3_cs_d.exe IL/straccess3_cs_d.exe
          mv straccess3_cs_d.exe straccess3_cs_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess3_cs_d.org /out straccess3_cs_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess3_cs_d.org /out straccess3_cs_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in straccess3_cs_d.reflect.xml
  if [ -f straccess3_cs_d.reflect.xml ]
then
    ReflectionRoots="-x straccess3_cs_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/straccess3_cs_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" straccess3_cs_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" straccess3_cs_d.asm.exe $CLRTestExecutionArguments
