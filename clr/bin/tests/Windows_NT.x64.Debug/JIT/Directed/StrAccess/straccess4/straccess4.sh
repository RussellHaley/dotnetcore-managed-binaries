...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess4 straccess4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess4 straccess4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=straccess4.dasm.il straccess4.exe
    "$CORE_ROOT/ildasm" -raweh -out=straccess4.dasm.il straccess4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=straccess4.asm.exe  straccess4.dasm.il
    "$CORE_ROOT/ilasm" -output=straccess4.asm.exe  straccess4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=straccess4
    if [ ! -f straccess4.org ]; then
        TakeLock
        if [ ! -f straccess4.org ]; then
          mkdir IL
          cp straccess4.exe IL/straccess4.exe
          mv straccess4.exe straccess4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess4.org /out straccess4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess4.org /out straccess4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in straccess4.reflect.xml
  if [ -f straccess4.reflect.xml ]
then
    ReflectionRoots="-x straccess4.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/straccess4.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" straccess4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" straccess4.asm.exe $CLRTestExecutionArguments
