...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\StrAccess\straccess4 straccess4.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\StrAccess\straccess4 straccess4.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=straccess4.dasm.il straccess4.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=straccess4.dasm.il straccess4.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=straccess4.asm.exe  straccess4.dasm.il
    %CORE_ROOT%\ilasm.exe /output=straccess4.asm.exe  straccess4.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=straccess4
    if not exist "straccess4.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "straccess4.org" (
            mkdir IL
            copy straccess4.exe IL\straccess4.exe 
            ren straccess4.exe straccess4.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in straccess4.org /out straccess4.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in straccess4.org /out straccess4.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in straccess4.reflect.xml
    IF EXIST straccess4.reflect.xml set ReflectionRoots=-x straccess4.reflect.xml
    ...
    set ExePath=%LinkBin%\straccess4.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% straccess4.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% straccess4.asm.exe %CLRTestExecutionArguments% %Host_Args%
