...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess1_cs_ro straccess1_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess1_cs_ro straccess1_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=straccess1_cs_ro.dasm.il straccess1_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=straccess1_cs_ro.dasm.il straccess1_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=straccess1_cs_ro.asm.exe  straccess1_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=straccess1_cs_ro.asm.exe  straccess1_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=straccess1_cs_ro
    if [ ! -f straccess1_cs_ro.org ]; then
        TakeLock
        if [ ! -f straccess1_cs_ro.org ]; then
          mkdir IL
          cp straccess1_cs_ro.exe IL/straccess1_cs_ro.exe
          mv straccess1_cs_ro.exe straccess1_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess1_cs_ro.org /out straccess1_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess1_cs_ro.org /out straccess1_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in straccess1_cs_ro.reflect.xml
  if [ -f straccess1_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x straccess1_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/straccess1_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" straccess1_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" straccess1_cs_ro.asm.exe $CLRTestExecutionArguments
