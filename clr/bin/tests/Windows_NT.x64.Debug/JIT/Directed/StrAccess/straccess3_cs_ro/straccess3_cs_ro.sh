...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess3_cs_ro straccess3_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess3_cs_ro straccess3_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=straccess3_cs_ro.dasm.il straccess3_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=straccess3_cs_ro.dasm.il straccess3_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=straccess3_cs_ro.asm.exe  straccess3_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=straccess3_cs_ro.asm.exe  straccess3_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=straccess3_cs_ro
    if [ ! -f straccess3_cs_ro.org ]; then
        TakeLock
        if [ ! -f straccess3_cs_ro.org ]; then
          mkdir IL
          cp straccess3_cs_ro.exe IL/straccess3_cs_ro.exe
          mv straccess3_cs_ro.exe straccess3_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess3_cs_ro.org /out straccess3_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess3_cs_ro.org /out straccess3_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in straccess3_cs_ro.reflect.xml
  if [ -f straccess3_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x straccess3_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/straccess3_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" straccess3_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" straccess3_cs_ro.asm.exe $CLRTestExecutionArguments
