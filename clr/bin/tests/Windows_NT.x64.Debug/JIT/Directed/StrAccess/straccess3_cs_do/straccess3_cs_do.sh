...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess3_cs_do straccess3_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess3_cs_do straccess3_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=straccess3_cs_do.dasm.il straccess3_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=straccess3_cs_do.dasm.il straccess3_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=straccess3_cs_do.asm.exe  straccess3_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=straccess3_cs_do.asm.exe  straccess3_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=straccess3_cs_do
    if [ ! -f straccess3_cs_do.org ]; then
        TakeLock
        if [ ! -f straccess3_cs_do.org ]; then
          mkdir IL
          cp straccess3_cs_do.exe IL/straccess3_cs_do.exe
          mv straccess3_cs_do.exe straccess3_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess3_cs_do.org /out straccess3_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess3_cs_do.org /out straccess3_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in straccess3_cs_do.reflect.xml
  if [ -f straccess3_cs_do.reflect.xml ]
then
    ReflectionRoots="-x straccess3_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/straccess3_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" straccess3_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" straccess3_cs_do.asm.exe $CLRTestExecutionArguments
