...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess3_cs_r straccess3_cs_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Directed/StrAccess/straccess3_cs_r straccess3_cs_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=straccess3_cs_r.dasm.il straccess3_cs_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=straccess3_cs_r.dasm.il straccess3_cs_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=straccess3_cs_r.asm.exe  straccess3_cs_r.dasm.il
    "$CORE_ROOT/ilasm" -output=straccess3_cs_r.asm.exe  straccess3_cs_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=straccess3_cs_r
    if [ ! -f straccess3_cs_r.org ]; then
        TakeLock
        if [ ! -f straccess3_cs_r.org ]; then
          mkdir IL
          cp straccess3_cs_r.exe IL/straccess3_cs_r.exe
          mv straccess3_cs_r.exe straccess3_cs_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess3_cs_r.org /out straccess3_cs_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in straccess3_cs_r.org /out straccess3_cs_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in straccess3_cs_r.reflect.xml
  if [ -f straccess3_cs_r.reflect.xml ]
then
    ReflectionRoots="-x straccess3_cs_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/straccess3_cs_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" straccess3_cs_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" straccess3_cs_r.asm.exe $CLRTestExecutionArguments
