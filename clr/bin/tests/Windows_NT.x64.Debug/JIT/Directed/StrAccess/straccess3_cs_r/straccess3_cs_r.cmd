...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\StrAccess\straccess3_cs_r straccess3_cs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Directed\StrAccess\straccess3_cs_r straccess3_cs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=straccess3_cs_r.dasm.il straccess3_cs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=straccess3_cs_r.dasm.il straccess3_cs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=straccess3_cs_r.asm.exe  straccess3_cs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=straccess3_cs_r.asm.exe  straccess3_cs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=straccess3_cs_r
    if not exist "straccess3_cs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "straccess3_cs_r.org" (
            mkdir IL
            copy straccess3_cs_r.exe IL\straccess3_cs_r.exe 
            ren straccess3_cs_r.exe straccess3_cs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in straccess3_cs_r.org /out straccess3_cs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in straccess3_cs_r.org /out straccess3_cs_r.exe
...
set ExePath=straccess3_cs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in straccess3_cs_r.reflect.xml
    IF EXIST straccess3_cs_r.reflect.xml set ReflectionRoots=-x straccess3_cs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\straccess3_cs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% straccess3_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% straccess3_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
