...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CheckProjects/CheckProjects CheckProjects.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/CheckProjects/CheckProjects CheckProjects.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CheckProjects.dasm.il CheckProjects.exe
    "$CORE_ROOT/ildasm" -raweh -out=CheckProjects.dasm.il CheckProjects.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CheckProjects.asm.exe  CheckProjects.dasm.il
    "$CORE_ROOT/ilasm" -output=CheckProjects.asm.exe  CheckProjects.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CheckProjects
    if [ ! -f CheckProjects.org ]; then
        TakeLock
        if [ ! -f CheckProjects.org ]; then
          mkdir IL
          cp CheckProjects.exe IL/CheckProjects.exe
          mv CheckProjects.exe CheckProjects.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CheckProjects.org /out CheckProjects.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CheckProjects.org /out CheckProjects.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CheckProjects.reflect.xml
  if [ -f CheckProjects.reflect.xml ]
then
    ReflectionRoots="-x CheckProjects.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CheckProjects.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CheckProjects.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CheckProjects.asm.exe $CLRTestExecutionArguments
