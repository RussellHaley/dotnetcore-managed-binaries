...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CheckProjects\CheckProjects CheckProjects.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\CheckProjects\CheckProjects CheckProjects.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CheckProjects.dasm.il CheckProjects.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CheckProjects.dasm.il CheckProjects.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CheckProjects.asm.exe  CheckProjects.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CheckProjects.asm.exe  CheckProjects.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CheckProjects
    if not exist "CheckProjects.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CheckProjects.org" (
            mkdir IL
            copy CheckProjects.exe IL\CheckProjects.exe 
            ren CheckProjects.exe CheckProjects.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CheckProjects.org /out CheckProjects.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CheckProjects.org /out CheckProjects.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CheckProjects.reflect.xml
    IF EXIST CheckProjects.reflect.xml set ReflectionRoots=-x CheckProjects.reflect.xml
    ...
    set ExePath=%LinkBin%\CheckProjects.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CheckProjects.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CheckProjects.asm.exe %CLRTestExecutionArguments% %Host_Args%
