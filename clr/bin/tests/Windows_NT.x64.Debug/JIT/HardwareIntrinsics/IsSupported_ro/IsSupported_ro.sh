...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/HardwareIntrinsics/IsSupported_ro IsSupported_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/HardwareIntrinsics/IsSupported_ro IsSupported_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=IsSupported_ro.dasm.il IsSupported_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=IsSupported_ro.dasm.il IsSupported_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=IsSupported_ro.asm.exe  IsSupported_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=IsSupported_ro.asm.exe  IsSupported_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=IsSupported_ro
    if [ ! -f IsSupported_ro.org ]; then
        TakeLock
        if [ ! -f IsSupported_ro.org ]; then
          mkdir IL
          cp IsSupported_ro.exe IL/IsSupported_ro.exe
          mv IsSupported_ro.exe IsSupported_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in IsSupported_ro.org /out IsSupported_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in IsSupported_ro.org /out IsSupported_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in IsSupported_ro.reflect.xml
  if [ -f IsSupported_ro.reflect.xml ]
then
    ReflectionRoots="-x IsSupported_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/IsSupported_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" IsSupported_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" IsSupported_ro.asm.exe $CLRTestExecutionArguments
