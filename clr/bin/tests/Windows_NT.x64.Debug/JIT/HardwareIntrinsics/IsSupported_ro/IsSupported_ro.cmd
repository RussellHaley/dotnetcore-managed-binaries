...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\HardwareIntrinsics\IsSupported_ro IsSupported_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\HardwareIntrinsics\IsSupported_ro IsSupported_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=IsSupported_ro.dasm.il IsSupported_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=IsSupported_ro.dasm.il IsSupported_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=IsSupported_ro.asm.exe  IsSupported_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=IsSupported_ro.asm.exe  IsSupported_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=IsSupported_ro
    if not exist "IsSupported_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "IsSupported_ro.org" (
            mkdir IL
            copy IsSupported_ro.exe IL\IsSupported_ro.exe 
            ren IsSupported_ro.exe IsSupported_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in IsSupported_ro.org /out IsSupported_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in IsSupported_ro.org /out IsSupported_ro.exe
...
set ExePath=IsSupported_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in IsSupported_ro.reflect.xml
    IF EXIST IsSupported_ro.reflect.xml set ReflectionRoots=-x IsSupported_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\IsSupported_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% IsSupported_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% IsSupported_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
