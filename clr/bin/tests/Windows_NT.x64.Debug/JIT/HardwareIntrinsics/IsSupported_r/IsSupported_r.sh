...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/HardwareIntrinsics/IsSupported_r IsSupported_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/JIT/HardwareIntrinsics/IsSupported_r IsSupported_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=IsSupported_r.dasm.il IsSupported_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=IsSupported_r.dasm.il IsSupported_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=IsSupported_r.asm.exe  IsSupported_r.dasm.il
    "$CORE_ROOT/ilasm" -output=IsSupported_r.asm.exe  IsSupported_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=IsSupported_r
    if [ ! -f IsSupported_r.org ]; then
        TakeLock
        if [ ! -f IsSupported_r.org ]; then
          mkdir IL
          cp IsSupported_r.exe IL/IsSupported_r.exe
          mv IsSupported_r.exe IsSupported_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in IsSupported_r.org /out IsSupported_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in IsSupported_r.org /out IsSupported_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in IsSupported_r.reflect.xml
  if [ -f IsSupported_r.reflect.xml ]
then
    ReflectionRoots="-x IsSupported_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/IsSupported_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" IsSupported_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" IsSupported_r.asm.exe $CLRTestExecutionArguments
