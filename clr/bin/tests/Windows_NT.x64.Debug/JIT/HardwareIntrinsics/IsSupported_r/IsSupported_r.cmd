...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\HardwareIntrinsics\IsSupported_r IsSupported_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\JIT\HardwareIntrinsics\IsSupported_r IsSupported_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=IsSupported_r.dasm.il IsSupported_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=IsSupported_r.dasm.il IsSupported_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=IsSupported_r.asm.exe  IsSupported_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=IsSupported_r.asm.exe  IsSupported_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=IsSupported_r
    if not exist "IsSupported_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "IsSupported_r.org" (
            mkdir IL
            copy IsSupported_r.exe IL\IsSupported_r.exe 
            ren IsSupported_r.exe IsSupported_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in IsSupported_r.org /out IsSupported_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in IsSupported_r.org /out IsSupported_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in IsSupported_r.reflect.xml
    IF EXIST IsSupported_r.reflect.xml set ReflectionRoots=-x IsSupported_r.reflect.xml
    ...
    set ExePath=%LinkBin%\IsSupported_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% IsSupported_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% IsSupported_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
