...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679955\volatileLocal2 volatileLocal2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679955\volatileLocal2 volatileLocal2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=volatileLocal2.dasm.il volatileLocal2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=volatileLocal2.dasm.il volatileLocal2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=volatileLocal2.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatileLocal2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=volatileLocal2.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatileLocal2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=volatileLocal2
    if not exist "volatileLocal2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "volatileLocal2.org" (
            mkdir IL
            copy volatileLocal2.exe IL\volatileLocal2.exe 
            ren volatileLocal2.exe volatileLocal2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatileLocal2.org /out volatileLocal2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in volatileLocal2.org /out volatileLocal2.exe
...
set ExePath=volatileLocal2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in volatileLocal2.reflect.xml
    IF EXIST volatileLocal2.reflect.xml set ReflectionRoots=-x volatileLocal2.reflect.xml
    ...
    set ExePath=%LinkBin%\volatileLocal2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% volatileLocal2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% volatileLocal2.asm.exe %CLRTestExecutionArguments% %Host_Args%
