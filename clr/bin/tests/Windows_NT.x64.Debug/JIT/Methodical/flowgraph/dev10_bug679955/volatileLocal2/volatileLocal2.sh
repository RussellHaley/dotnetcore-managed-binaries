...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679955/volatileLocal2 volatileLocal2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679955/volatileLocal2 volatileLocal2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=volatileLocal2.dasm.il volatileLocal2.exe
    "$CORE_ROOT/ildasm" -raweh -out=volatileLocal2.dasm.il volatileLocal2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=volatileLocal2.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatileLocal2.dasm.il
    "$CORE_ROOT/ilasm" -output=volatileLocal2.asm.exe -QUIET -NOLOGO -DEBUG=OPT volatileLocal2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=volatileLocal2
    if [ ! -f volatileLocal2.org ]; then
        TakeLock
        if [ ! -f volatileLocal2.org ]; then
          mkdir IL
          cp volatileLocal2.exe IL/volatileLocal2.exe
          mv volatileLocal2.exe volatileLocal2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatileLocal2.org /out volatileLocal2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in volatileLocal2.org /out volatileLocal2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in volatileLocal2.reflect.xml
  if [ -f volatileLocal2.reflect.xml ]
then
    ReflectionRoots="-x volatileLocal2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/volatileLocal2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" volatileLocal2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" volatileLocal2.asm.exe $CLRTestExecutionArguments
