...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679955\indexMinusOne indexMinusOne.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679955\indexMinusOne indexMinusOne.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=indexMinusOne.dasm.il indexMinusOne.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=indexMinusOne.dasm.il indexMinusOne.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=indexMinusOne.asm.exe -QUIET -NOLOGO -DEBUG=OPT indexMinusOne.dasm.il
    %CORE_ROOT%\ilasm.exe /output=indexMinusOne.asm.exe -QUIET -NOLOGO -DEBUG=OPT indexMinusOne.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=indexMinusOne
    if not exist "indexMinusOne.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "indexMinusOne.org" (
            mkdir IL
            copy indexMinusOne.exe IL\indexMinusOne.exe 
            ren indexMinusOne.exe indexMinusOne.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in indexMinusOne.org /out indexMinusOne.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in indexMinusOne.org /out indexMinusOne.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in indexMinusOne.reflect.xml
    IF EXIST indexMinusOne.reflect.xml set ReflectionRoots=-x indexMinusOne.reflect.xml
    ...
    set ExePath=%LinkBin%\indexMinusOne.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% indexMinusOne.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% indexMinusOne.asm.exe %CLRTestExecutionArguments% %Host_Args%
