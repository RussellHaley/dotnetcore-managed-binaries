...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679955/indexMinusOne indexMinusOne.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679955/indexMinusOne indexMinusOne.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=indexMinusOne.dasm.il indexMinusOne.exe
    "$CORE_ROOT/ildasm" -raweh -out=indexMinusOne.dasm.il indexMinusOne.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=indexMinusOne.asm.exe -QUIET -NOLOGO -DEBUG=OPT indexMinusOne.dasm.il
    "$CORE_ROOT/ilasm" -output=indexMinusOne.asm.exe -QUIET -NOLOGO -DEBUG=OPT indexMinusOne.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=indexMinusOne
    if [ ! -f indexMinusOne.org ]; then
        TakeLock
        if [ ! -f indexMinusOne.org ]; then
          mkdir IL
          cp indexMinusOne.exe IL/indexMinusOne.exe
          mv indexMinusOne.exe indexMinusOne.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in indexMinusOne.org /out indexMinusOne.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in indexMinusOne.org /out indexMinusOne.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in indexMinusOne.reflect.xml
  if [ -f indexMinusOne.reflect.xml ]
then
    ReflectionRoots="-x indexMinusOne.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/indexMinusOne.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" indexMinusOne.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" indexMinusOne.asm.exe $CLRTestExecutionArguments
