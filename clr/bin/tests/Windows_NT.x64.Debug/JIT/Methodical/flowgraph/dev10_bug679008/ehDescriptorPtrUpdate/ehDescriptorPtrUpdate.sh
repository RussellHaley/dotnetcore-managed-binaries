...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/ehDescriptorPtrUpdate ehDescriptorPtrUpdate.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/ehDescriptorPtrUpdate ehDescriptorPtrUpdate.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ehDescriptorPtrUpdate.dasm.il ehDescriptorPtrUpdate.exe
    "$CORE_ROOT/ildasm" -raweh -out=ehDescriptorPtrUpdate.dasm.il ehDescriptorPtrUpdate.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ehDescriptorPtrUpdate.asm.exe -QUIET -NOLOGO -DEBUG=OPT ehDescriptorPtrUpdate.dasm.il
    "$CORE_ROOT/ilasm" -output=ehDescriptorPtrUpdate.asm.exe -QUIET -NOLOGO -DEBUG=OPT ehDescriptorPtrUpdate.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ehDescriptorPtrUpdate
    if [ ! -f ehDescriptorPtrUpdate.org ]; then
        TakeLock
        if [ ! -f ehDescriptorPtrUpdate.org ]; then
          mkdir IL
          cp ehDescriptorPtrUpdate.exe IL/ehDescriptorPtrUpdate.exe
          mv ehDescriptorPtrUpdate.exe ehDescriptorPtrUpdate.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ehDescriptorPtrUpdate.org /out ehDescriptorPtrUpdate.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ehDescriptorPtrUpdate.org /out ehDescriptorPtrUpdate.exe
          __cgExitCode=$?
...
ExePath=ehDescriptorPtrUpdate.exe
...
then
  ...
  # Use hints for reflection roots, if provided in ehDescriptorPtrUpdate.reflect.xml
  if [ -f ehDescriptorPtrUpdate.reflect.xml ]
then
    ReflectionRoots="-x ehDescriptorPtrUpdate.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ehDescriptorPtrUpdate.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ehDescriptorPtrUpdate.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ehDescriptorPtrUpdate.asm.exe $CLRTestExecutionArguments
