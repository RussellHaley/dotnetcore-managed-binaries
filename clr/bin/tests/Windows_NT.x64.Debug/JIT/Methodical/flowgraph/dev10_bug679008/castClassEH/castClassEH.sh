...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/castClassEH castClassEH.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/castClassEH castClassEH.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castClassEH.dasm.il castClassEH.exe
    "$CORE_ROOT/ildasm" -raweh -out=castClassEH.dasm.il castClassEH.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castClassEH.asm.exe  castClassEH.dasm.il
    "$CORE_ROOT/ilasm" -output=castClassEH.asm.exe  castClassEH.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castClassEH
    if [ ! -f castClassEH.org ]; then
        TakeLock
        if [ ! -f castClassEH.org ]; then
          mkdir IL
          cp castClassEH.exe IL/castClassEH.exe
          mv castClassEH.exe castClassEH.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castClassEH.org /out castClassEH.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castClassEH.org /out castClassEH.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in castClassEH.reflect.xml
  if [ -f castClassEH.reflect.xml ]
then
    ReflectionRoots="-x castClassEH.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castClassEH.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castClassEH.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castClassEH.asm.exe $CLRTestExecutionArguments
