...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\fgloop2 fgloop2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\fgloop2 fgloop2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=fgloop2.dasm.il fgloop2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=fgloop2.dasm.il fgloop2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=fgloop2.asm.exe -QUIET -NOLOGO -DEBUG=OPT fgloop2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=fgloop2.asm.exe -QUIET -NOLOGO -DEBUG=OPT fgloop2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=fgloop2
    if not exist "fgloop2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "fgloop2.org" (
            mkdir IL
            copy fgloop2.exe IL\fgloop2.exe 
            ren fgloop2.exe fgloop2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fgloop2.org /out fgloop2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fgloop2.org /out fgloop2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in fgloop2.reflect.xml
    IF EXIST fgloop2.reflect.xml set ReflectionRoots=-x fgloop2.reflect.xml
    ...
    set ExePath=%LinkBin%\fgloop2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% fgloop2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% fgloop2.asm.exe %CLRTestExecutionArguments% %Host_Args%
