...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\fgloop fgloop.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\fgloop fgloop.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=fgloop.dasm.il fgloop.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=fgloop.dasm.il fgloop.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=fgloop.asm.exe -QUIET -NOLOGO -DEBUG=OPT fgloop.dasm.il
    %CORE_ROOT%\ilasm.exe /output=fgloop.asm.exe -QUIET -NOLOGO -DEBUG=OPT fgloop.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=fgloop
    if not exist "fgloop.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "fgloop.org" (
            mkdir IL
            copy fgloop.exe IL\fgloop.exe 
            ren fgloop.exe fgloop.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fgloop.org /out fgloop.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fgloop.org /out fgloop.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in fgloop.reflect.xml
    IF EXIST fgloop.reflect.xml set ReflectionRoots=-x fgloop.reflect.xml
    ...
    set ExePath=%LinkBin%\fgloop.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% fgloop.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% fgloop.asm.exe %CLRTestExecutionArguments% %Host_Args%
