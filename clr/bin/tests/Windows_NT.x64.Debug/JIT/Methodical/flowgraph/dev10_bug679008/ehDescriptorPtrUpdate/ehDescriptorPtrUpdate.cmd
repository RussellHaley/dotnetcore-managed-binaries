...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\ehDescriptorPtrUpdate ehDescriptorPtrUpdate.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\ehDescriptorPtrUpdate ehDescriptorPtrUpdate.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ehDescriptorPtrUpdate.dasm.il ehDescriptorPtrUpdate.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ehDescriptorPtrUpdate.dasm.il ehDescriptorPtrUpdate.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ehDescriptorPtrUpdate.asm.exe -QUIET -NOLOGO -DEBUG=OPT ehDescriptorPtrUpdate.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ehDescriptorPtrUpdate.asm.exe -QUIET -NOLOGO -DEBUG=OPT ehDescriptorPtrUpdate.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ehDescriptorPtrUpdate
    if not exist "ehDescriptorPtrUpdate.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ehDescriptorPtrUpdate.org" (
            mkdir IL
            copy ehDescriptorPtrUpdate.exe IL\ehDescriptorPtrUpdate.exe 
            ren ehDescriptorPtrUpdate.exe ehDescriptorPtrUpdate.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ehDescriptorPtrUpdate.org /out ehDescriptorPtrUpdate.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ehDescriptorPtrUpdate.org /out ehDescriptorPtrUpdate.exe
...
set ExePath=ehDescriptorPtrUpdate.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ehDescriptorPtrUpdate.reflect.xml
    IF EXIST ehDescriptorPtrUpdate.reflect.xml set ReflectionRoots=-x ehDescriptorPtrUpdate.reflect.xml
    ...
    set ExePath=%LinkBin%\ehDescriptorPtrUpdate.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ehDescriptorPtrUpdate.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ehDescriptorPtrUpdate.asm.exe %CLRTestExecutionArguments% %Host_Args%
