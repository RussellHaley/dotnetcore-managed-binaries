...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\castClassEH castClassEH.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\castClassEH castClassEH.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castClassEH.dasm.il castClassEH.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castClassEH.dasm.il castClassEH.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castClassEH.asm.exe  castClassEH.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castClassEH.asm.exe  castClassEH.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castClassEH
    if not exist "castClassEH.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castClassEH.org" (
            mkdir IL
            copy castClassEH.exe IL\castClassEH.exe 
            ren castClassEH.exe castClassEH.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castClassEH.org /out castClassEH.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castClassEH.org /out castClassEH.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castClassEH.reflect.xml
    IF EXIST castClassEH.reflect.xml set ReflectionRoots=-x castClassEH.reflect.xml
    ...
    set ExePath=%LinkBin%\castClassEH.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castClassEH.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castClassEH.asm.exe %CLRTestExecutionArguments% %Host_Args%
