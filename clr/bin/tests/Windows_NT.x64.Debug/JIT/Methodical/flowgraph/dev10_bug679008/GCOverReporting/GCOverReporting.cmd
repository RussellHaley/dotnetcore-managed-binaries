...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\GCOverReporting GCOverReporting.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679008\GCOverReporting GCOverReporting.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GCOverReporting.dasm.il GCOverReporting.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GCOverReporting.dasm.il GCOverReporting.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GCOverReporting.asm.exe  GCOverReporting.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GCOverReporting.asm.exe  GCOverReporting.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GCOverReporting
    if not exist "GCOverReporting.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GCOverReporting.org" (
            mkdir IL
            copy GCOverReporting.exe IL\GCOverReporting.exe 
            ren GCOverReporting.exe GCOverReporting.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GCOverReporting.org /out GCOverReporting.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GCOverReporting.org /out GCOverReporting.exe
...
set ExePath=GCOverReporting.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GCOverReporting.reflect.xml
    IF EXIST GCOverReporting.reflect.xml set ReflectionRoots=-x GCOverReporting.reflect.xml
    ...
    set ExePath=%LinkBin%\GCOverReporting.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GCOverReporting.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GCOverReporting.asm.exe %CLRTestExecutionArguments% %Host_Args%
