...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/fgloop fgloop.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/fgloop fgloop.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=fgloop.dasm.il fgloop.exe
    "$CORE_ROOT/ildasm" -raweh -out=fgloop.dasm.il fgloop.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=fgloop.asm.exe -QUIET -NOLOGO -DEBUG=OPT fgloop.dasm.il
    "$CORE_ROOT/ilasm" -output=fgloop.asm.exe -QUIET -NOLOGO -DEBUG=OPT fgloop.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=fgloop
    if [ ! -f fgloop.org ]; then
        TakeLock
        if [ ! -f fgloop.org ]; then
          mkdir IL
          cp fgloop.exe IL/fgloop.exe
          mv fgloop.exe fgloop.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fgloop.org /out fgloop.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fgloop.org /out fgloop.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in fgloop.reflect.xml
  if [ -f fgloop.reflect.xml ]
then
    ReflectionRoots="-x fgloop.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" fgloop.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" fgloop.asm.exe $CLRTestExecutionArguments
