...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/fgloop2 fgloop2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/fgloop2 fgloop2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=fgloop2.dasm.il fgloop2.exe
    "$CORE_ROOT/ildasm" -raweh -out=fgloop2.dasm.il fgloop2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=fgloop2.asm.exe -QUIET -NOLOGO -DEBUG=OPT fgloop2.dasm.il
    "$CORE_ROOT/ilasm" -output=fgloop2.asm.exe -QUIET -NOLOGO -DEBUG=OPT fgloop2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=fgloop2
    if [ ! -f fgloop2.org ]; then
        TakeLock
        if [ ! -f fgloop2.org ]; then
          mkdir IL
          cp fgloop2.exe IL/fgloop2.exe
          mv fgloop2.exe fgloop2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fgloop2.org /out fgloop2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fgloop2.org /out fgloop2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in fgloop2.reflect.xml
  if [ -f fgloop2.reflect.xml ]
then
    ReflectionRoots="-x fgloop2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" fgloop2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" fgloop2.asm.exe $CLRTestExecutionArguments
