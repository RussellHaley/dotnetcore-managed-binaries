...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/GCOverReporting GCOverReporting.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679008/GCOverReporting GCOverReporting.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GCOverReporting.dasm.il GCOverReporting.exe
    "$CORE_ROOT/ildasm" -raweh -out=GCOverReporting.dasm.il GCOverReporting.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GCOverReporting.asm.exe  GCOverReporting.dasm.il
    "$CORE_ROOT/ilasm" -output=GCOverReporting.asm.exe  GCOverReporting.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GCOverReporting
    if [ ! -f GCOverReporting.org ]; then
        TakeLock
        if [ ! -f GCOverReporting.org ]; then
          mkdir IL
          cp GCOverReporting.exe IL/GCOverReporting.exe
          mv GCOverReporting.exe GCOverReporting.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GCOverReporting.org /out GCOverReporting.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GCOverReporting.org /out GCOverReporting.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GCOverReporting.reflect.xml
  if [ -f GCOverReporting.reflect.xml ]
then
    ReflectionRoots="-x GCOverReporting.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GCOverReporting.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GCOverReporting.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GCOverReporting.asm.exe $CLRTestExecutionArguments
