...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\bug619534\moduleHandleCache moduleHandleCache.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\bug619534\moduleHandleCache moduleHandleCache.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=moduleHandleCache.dasm.il moduleHandleCache.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=moduleHandleCache.dasm.il moduleHandleCache.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=moduleHandleCache.asm.exe  moduleHandleCache.dasm.il
    %CORE_ROOT%\ilasm.exe /output=moduleHandleCache.asm.exe  moduleHandleCache.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=moduleHandleCache
    if not exist "moduleHandleCache.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "moduleHandleCache.org" (
            mkdir IL
            copy moduleHandleCache.exe IL\moduleHandleCache.exe 
            ren moduleHandleCache.exe moduleHandleCache.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in moduleHandleCache.org /out moduleHandleCache.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in moduleHandleCache.org /out moduleHandleCache.exe
...
set ExePath=moduleHandleCache.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in moduleHandleCache.reflect.xml
    IF EXIST moduleHandleCache.reflect.xml set ReflectionRoots=-x moduleHandleCache.reflect.xml
    ...
    set ExePath=%LinkBin%\moduleHandleCache.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% moduleHandleCache.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% moduleHandleCache.asm.exe %CLRTestExecutionArguments% %Host_Args%
