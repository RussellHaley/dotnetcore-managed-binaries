...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/bug619534/ehCodeMotion ehCodeMotion.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/bug619534/ehCodeMotion ehCodeMotion.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ehCodeMotion.dasm.il ehCodeMotion.exe
    "$CORE_ROOT/ildasm" -raweh -out=ehCodeMotion.dasm.il ehCodeMotion.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ehCodeMotion.asm.exe  ehCodeMotion.dasm.il
    "$CORE_ROOT/ilasm" -output=ehCodeMotion.asm.exe  ehCodeMotion.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ehCodeMotion
    if [ ! -f ehCodeMotion.org ]; then
        TakeLock
        if [ ! -f ehCodeMotion.org ]; then
          mkdir IL
          cp ehCodeMotion.exe IL/ehCodeMotion.exe
          mv ehCodeMotion.exe ehCodeMotion.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ehCodeMotion.org /out ehCodeMotion.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ehCodeMotion.org /out ehCodeMotion.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ehCodeMotion.reflect.xml
  if [ -f ehCodeMotion.reflect.xml ]
then
    ReflectionRoots="-x ehCodeMotion.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ehCodeMotion.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ehCodeMotion.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ehCodeMotion.asm.exe $CLRTestExecutionArguments
