...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\bug619534\ehCodeMotion ehCodeMotion.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\bug619534\ehCodeMotion ehCodeMotion.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ehCodeMotion.dasm.il ehCodeMotion.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ehCodeMotion.dasm.il ehCodeMotion.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ehCodeMotion.asm.exe  ehCodeMotion.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ehCodeMotion.asm.exe  ehCodeMotion.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ehCodeMotion
    if not exist "ehCodeMotion.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ehCodeMotion.org" (
            mkdir IL
            copy ehCodeMotion.exe IL\ehCodeMotion.exe 
            ren ehCodeMotion.exe ehCodeMotion.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ehCodeMotion.org /out ehCodeMotion.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ehCodeMotion.org /out ehCodeMotion.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ehCodeMotion.reflect.xml
    IF EXIST ehCodeMotion.reflect.xml set ReflectionRoots=-x ehCodeMotion.reflect.xml
    ...
    set ExePath=%LinkBin%\ehCodeMotion.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ehCodeMotion.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ehCodeMotion.asm.exe %CLRTestExecutionArguments% %Host_Args%
