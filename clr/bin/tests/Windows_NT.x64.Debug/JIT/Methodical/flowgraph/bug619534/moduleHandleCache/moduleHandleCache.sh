...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/bug619534/moduleHandleCache moduleHandleCache.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/bug619534/moduleHandleCache moduleHandleCache.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=moduleHandleCache.dasm.il moduleHandleCache.exe
    "$CORE_ROOT/ildasm" -raweh -out=moduleHandleCache.dasm.il moduleHandleCache.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=moduleHandleCache.asm.exe  moduleHandleCache.dasm.il
    "$CORE_ROOT/ilasm" -output=moduleHandleCache.asm.exe  moduleHandleCache.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=moduleHandleCache
    if [ ! -f moduleHandleCache.org ]; then
        TakeLock
        if [ ! -f moduleHandleCache.org ]; then
          mkdir IL
          cp moduleHandleCache.exe IL/moduleHandleCache.exe
          mv moduleHandleCache.exe moduleHandleCache.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in moduleHandleCache.org /out moduleHandleCache.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in moduleHandleCache.org /out moduleHandleCache.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in moduleHandleCache.reflect.xml
  if [ -f moduleHandleCache.reflect.xml ]
then
    ReflectionRoots="-x moduleHandleCache.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/moduleHandleCache.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" moduleHandleCache.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" moduleHandleCache.asm.exe $CLRTestExecutionArguments
