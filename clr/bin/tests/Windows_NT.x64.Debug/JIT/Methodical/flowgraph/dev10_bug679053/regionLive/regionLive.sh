...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679053/regionLive regionLive.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug679053/regionLive regionLive.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=regionLive.dasm.il regionLive.exe
    "$CORE_ROOT/ildasm" -raweh -out=regionLive.dasm.il regionLive.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=regionLive.asm.exe -QUIET -NOLOGO -DEBUG=OPT regionLive.dasm.il
    "$CORE_ROOT/ilasm" -output=regionLive.asm.exe -QUIET -NOLOGO -DEBUG=OPT regionLive.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=regionLive
    if [ ! -f regionLive.org ]; then
        TakeLock
        if [ ! -f regionLive.org ]; then
          mkdir IL
          cp regionLive.exe IL/regionLive.exe
          mv regionLive.exe regionLive.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in regionLive.org /out regionLive.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in regionLive.org /out regionLive.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in regionLive.reflect.xml
  if [ -f regionLive.reflect.xml ]
then
    ReflectionRoots="-x regionLive.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/regionLive.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" regionLive.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" regionLive.asm.exe $CLRTestExecutionArguments
