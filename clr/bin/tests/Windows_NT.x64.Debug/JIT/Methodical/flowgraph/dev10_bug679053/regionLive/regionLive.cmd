...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679053\regionLive regionLive.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug679053\regionLive regionLive.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=regionLive.dasm.il regionLive.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=regionLive.dasm.il regionLive.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=regionLive.asm.exe -QUIET -NOLOGO -DEBUG=OPT regionLive.dasm.il
    %CORE_ROOT%\ilasm.exe /output=regionLive.asm.exe -QUIET -NOLOGO -DEBUG=OPT regionLive.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=regionLive
    if not exist "regionLive.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "regionLive.org" (
            mkdir IL
            copy regionLive.exe IL\regionLive.exe 
            ren regionLive.exe regionLive.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in regionLive.org /out regionLive.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in regionLive.org /out regionLive.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in regionLive.reflect.xml
    IF EXIST regionLive.reflect.xml set ReflectionRoots=-x regionLive.reflect.xml
    ...
    set ExePath=%LinkBin%\regionLive.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% regionLive.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% regionLive.asm.exe %CLRTestExecutionArguments% %Host_Args%
