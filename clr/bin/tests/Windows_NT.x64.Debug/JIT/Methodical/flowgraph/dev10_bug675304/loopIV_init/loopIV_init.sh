...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug675304/loopIV_init loopIV_init.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/flowgraph/dev10_bug675304/loopIV_init loopIV_init.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=loopIV_init.dasm.il loopIV_init.exe
    "$CORE_ROOT/ildasm" -raweh -out=loopIV_init.dasm.il loopIV_init.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=loopIV_init.asm.exe -QUIET -NOLOGO -DEBUG=OPT loopIV_init.dasm.il
    "$CORE_ROOT/ilasm" -output=loopIV_init.asm.exe -QUIET -NOLOGO -DEBUG=OPT loopIV_init.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=loopIV_init
    if [ ! -f loopIV_init.org ]; then
        TakeLock
        if [ ! -f loopIV_init.org ]; then
          mkdir IL
          cp loopIV_init.exe IL/loopIV_init.exe
          mv loopIV_init.exe loopIV_init.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopIV_init.org /out loopIV_init.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopIV_init.org /out loopIV_init.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in loopIV_init.reflect.xml
  if [ -f loopIV_init.reflect.xml ]
then
    ReflectionRoots="-x loopIV_init.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/loopIV_init.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" loopIV_init.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" loopIV_init.asm.exe $CLRTestExecutionArguments
