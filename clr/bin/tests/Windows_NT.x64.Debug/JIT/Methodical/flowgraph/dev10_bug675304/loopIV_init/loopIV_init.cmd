...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug675304\loopIV_init loopIV_init.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\flowgraph\dev10_bug675304\loopIV_init loopIV_init.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=loopIV_init.dasm.il loopIV_init.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=loopIV_init.dasm.il loopIV_init.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=loopIV_init.asm.exe -QUIET -NOLOGO -DEBUG=OPT loopIV_init.dasm.il
    %CORE_ROOT%\ilasm.exe /output=loopIV_init.asm.exe -QUIET -NOLOGO -DEBUG=OPT loopIV_init.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=loopIV_init
    if not exist "loopIV_init.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "loopIV_init.org" (
            mkdir IL
            copy loopIV_init.exe IL\loopIV_init.exe 
            ren loopIV_init.exe loopIV_init.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopIV_init.org /out loopIV_init.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopIV_init.org /out loopIV_init.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in loopIV_init.reflect.xml
    IF EXIST loopIV_init.reflect.xml set ReflectionRoots=-x loopIV_init.reflect.xml
    ...
    set ExePath=%LinkBin%\loopIV_init.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% loopIV_init.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% loopIV_init.asm.exe %CLRTestExecutionArguments% %Host_Args%
