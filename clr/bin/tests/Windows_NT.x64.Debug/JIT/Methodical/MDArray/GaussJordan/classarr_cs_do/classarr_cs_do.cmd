...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\GaussJordan\classarr_cs_do classarr_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\GaussJordan\classarr_cs_do classarr_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=classarr_cs_do.dasm.il classarr_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=classarr_cs_do.dasm.il classarr_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=classarr_cs_do.asm.exe  classarr_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=classarr_cs_do.asm.exe  classarr_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=classarr_cs_do
    if not exist "classarr_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "classarr_cs_do.org" (
            mkdir IL
            copy classarr_cs_do.exe IL\classarr_cs_do.exe 
            ren classarr_cs_do.exe classarr_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in classarr_cs_do.org /out classarr_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in classarr_cs_do.org /out classarr_cs_do.exe
...
set ExePath=classarr_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in classarr_cs_do.reflect.xml
    IF EXIST classarr_cs_do.reflect.xml set ReflectionRoots=-x classarr_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\classarr_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% classarr_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% classarr_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
