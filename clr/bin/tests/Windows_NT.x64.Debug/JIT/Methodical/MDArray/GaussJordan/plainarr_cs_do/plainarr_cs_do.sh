...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/GaussJordan/plainarr_cs_do plainarr_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/GaussJordan/plainarr_cs_do plainarr_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=plainarr_cs_do.dasm.il plainarr_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=plainarr_cs_do.dasm.il plainarr_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=plainarr_cs_do.asm.exe  plainarr_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=plainarr_cs_do.asm.exe  plainarr_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=plainarr_cs_do
    if [ ! -f plainarr_cs_do.org ]; then
        TakeLock
        if [ ! -f plainarr_cs_do.org ]; then
          mkdir IL
          cp plainarr_cs_do.exe IL/plainarr_cs_do.exe
          mv plainarr_cs_do.exe plainarr_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in plainarr_cs_do.org /out plainarr_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in plainarr_cs_do.org /out plainarr_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in plainarr_cs_do.reflect.xml
  if [ -f plainarr_cs_do.reflect.xml ]
then
    ReflectionRoots="-x plainarr_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/plainarr_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" plainarr_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" plainarr_cs_do.asm.exe $CLRTestExecutionArguments
