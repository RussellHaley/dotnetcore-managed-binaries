...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\GaussJordan\structarr_cs_ro structarr_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\GaussJordan\structarr_cs_ro structarr_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=structarr_cs_ro.dasm.il structarr_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=structarr_cs_ro.dasm.il structarr_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=structarr_cs_ro.asm.exe  structarr_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=structarr_cs_ro.asm.exe  structarr_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=structarr_cs_ro
    if not exist "structarr_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "structarr_cs_ro.org" (
            mkdir IL
            copy structarr_cs_ro.exe IL\structarr_cs_ro.exe 
            ren structarr_cs_ro.exe structarr_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in structarr_cs_ro.org /out structarr_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in structarr_cs_ro.org /out structarr_cs_ro.exe
...
set ExePath=structarr_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in structarr_cs_ro.reflect.xml
    IF EXIST structarr_cs_ro.reflect.xml set ReflectionRoots=-x structarr_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\structarr_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% structarr_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% structarr_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
