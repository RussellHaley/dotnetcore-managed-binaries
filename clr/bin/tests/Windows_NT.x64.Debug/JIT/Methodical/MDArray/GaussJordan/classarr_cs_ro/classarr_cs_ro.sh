...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/GaussJordan/classarr_cs_ro classarr_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/GaussJordan/classarr_cs_ro classarr_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=classarr_cs_ro.dasm.il classarr_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=classarr_cs_ro.dasm.il classarr_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=classarr_cs_ro.asm.exe  classarr_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=classarr_cs_ro.asm.exe  classarr_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=classarr_cs_ro
    if [ ! -f classarr_cs_ro.org ]; then
        TakeLock
        if [ ! -f classarr_cs_ro.org ]; then
          mkdir IL
          cp classarr_cs_ro.exe IL/classarr_cs_ro.exe
          mv classarr_cs_ro.exe classarr_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in classarr_cs_ro.org /out classarr_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in classarr_cs_ro.org /out classarr_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in classarr_cs_ro.reflect.xml
  if [ -f classarr_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x classarr_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/classarr_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" classarr_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" classarr_cs_ro.asm.exe $CLRTestExecutionArguments
