...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/GaussJordan/jaggedarr_cs_do jaggedarr_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/GaussJordan/jaggedarr_cs_do jaggedarr_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=jaggedarr_cs_do.dasm.il jaggedarr_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=jaggedarr_cs_do.dasm.il jaggedarr_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=jaggedarr_cs_do.asm.exe  jaggedarr_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=jaggedarr_cs_do.asm.exe  jaggedarr_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=jaggedarr_cs_do
    if [ ! -f jaggedarr_cs_do.org ]; then
        TakeLock
        if [ ! -f jaggedarr_cs_do.org ]; then
          mkdir IL
          cp jaggedarr_cs_do.exe IL/jaggedarr_cs_do.exe
          mv jaggedarr_cs_do.exe jaggedarr_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in jaggedarr_cs_do.org /out jaggedarr_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in jaggedarr_cs_do.org /out jaggedarr_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in jaggedarr_cs_do.reflect.xml
  if [ -f jaggedarr_cs_do.reflect.xml ]
then
    ReflectionRoots="-x jaggedarr_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/jaggedarr_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" jaggedarr_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" jaggedarr_cs_do.asm.exe $CLRTestExecutionArguments
