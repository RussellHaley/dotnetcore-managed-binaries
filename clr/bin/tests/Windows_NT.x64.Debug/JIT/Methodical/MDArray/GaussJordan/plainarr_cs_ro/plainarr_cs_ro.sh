...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/GaussJordan/plainarr_cs_ro plainarr_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/GaussJordan/plainarr_cs_ro plainarr_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=plainarr_cs_ro.dasm.il plainarr_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=plainarr_cs_ro.dasm.il plainarr_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=plainarr_cs_ro.asm.exe  plainarr_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=plainarr_cs_ro.asm.exe  plainarr_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=plainarr_cs_ro
    if [ ! -f plainarr_cs_ro.org ]; then
        TakeLock
        if [ ! -f plainarr_cs_ro.org ]; then
          mkdir IL
          cp plainarr_cs_ro.exe IL/plainarr_cs_ro.exe
          mv plainarr_cs_ro.exe plainarr_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in plainarr_cs_ro.org /out plainarr_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in plainarr_cs_ro.org /out plainarr_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in plainarr_cs_ro.reflect.xml
  if [ -f plainarr_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x plainarr_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/plainarr_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" plainarr_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" plainarr_cs_ro.asm.exe $CLRTestExecutionArguments
