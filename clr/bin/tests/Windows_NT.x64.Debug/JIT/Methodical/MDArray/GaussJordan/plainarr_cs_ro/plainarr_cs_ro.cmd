...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\GaussJordan\plainarr_cs_ro plainarr_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\GaussJordan\plainarr_cs_ro plainarr_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=plainarr_cs_ro.dasm.il plainarr_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=plainarr_cs_ro.dasm.il plainarr_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=plainarr_cs_ro.asm.exe  plainarr_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=plainarr_cs_ro.asm.exe  plainarr_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=plainarr_cs_ro
    if not exist "plainarr_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "plainarr_cs_ro.org" (
            mkdir IL
            copy plainarr_cs_ro.exe IL\plainarr_cs_ro.exe 
            ren plainarr_cs_ro.exe plainarr_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in plainarr_cs_ro.org /out plainarr_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in plainarr_cs_ro.org /out plainarr_cs_ro.exe
...
set ExePath=plainarr_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in plainarr_cs_ro.reflect.xml
    IF EXIST plainarr_cs_ro.reflect.xml set ReflectionRoots=-x plainarr_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\plainarr_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% plainarr_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% plainarr_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
