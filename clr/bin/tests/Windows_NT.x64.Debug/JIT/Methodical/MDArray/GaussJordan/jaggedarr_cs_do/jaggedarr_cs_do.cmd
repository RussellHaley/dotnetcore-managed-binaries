...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\GaussJordan\jaggedarr_cs_do jaggedarr_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\GaussJordan\jaggedarr_cs_do jaggedarr_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=jaggedarr_cs_do.dasm.il jaggedarr_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=jaggedarr_cs_do.dasm.il jaggedarr_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=jaggedarr_cs_do.asm.exe  jaggedarr_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=jaggedarr_cs_do.asm.exe  jaggedarr_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=jaggedarr_cs_do
    if not exist "jaggedarr_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "jaggedarr_cs_do.org" (
            mkdir IL
            copy jaggedarr_cs_do.exe IL\jaggedarr_cs_do.exe 
            ren jaggedarr_cs_do.exe jaggedarr_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in jaggedarr_cs_do.org /out jaggedarr_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in jaggedarr_cs_do.org /out jaggedarr_cs_do.exe
...
set ExePath=jaggedarr_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in jaggedarr_cs_do.reflect.xml
    IF EXIST jaggedarr_cs_do.reflect.xml set ReflectionRoots=-x jaggedarr_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\jaggedarr_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% jaggedarr_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% jaggedarr_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
