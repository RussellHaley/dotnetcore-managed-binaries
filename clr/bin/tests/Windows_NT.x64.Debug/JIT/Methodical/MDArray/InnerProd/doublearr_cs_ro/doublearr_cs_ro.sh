...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/InnerProd/doublearr_cs_ro doublearr_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/InnerProd/doublearr_cs_ro doublearr_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=doublearr_cs_ro.dasm.il doublearr_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=doublearr_cs_ro.dasm.il doublearr_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=doublearr_cs_ro.asm.exe  doublearr_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=doublearr_cs_ro.asm.exe  doublearr_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=doublearr_cs_ro
    if [ ! -f doublearr_cs_ro.org ]; then
        TakeLock
        if [ ! -f doublearr_cs_ro.org ]; then
          mkdir IL
          cp doublearr_cs_ro.exe IL/doublearr_cs_ro.exe
          mv doublearr_cs_ro.exe doublearr_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublearr_cs_ro.org /out doublearr_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublearr_cs_ro.org /out doublearr_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in doublearr_cs_ro.reflect.xml
  if [ -f doublearr_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x doublearr_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/doublearr_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" doublearr_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" doublearr_cs_ro.asm.exe $CLRTestExecutionArguments
