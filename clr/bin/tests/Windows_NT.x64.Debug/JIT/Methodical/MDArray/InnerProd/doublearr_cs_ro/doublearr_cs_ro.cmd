...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\InnerProd\doublearr_cs_ro doublearr_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\InnerProd\doublearr_cs_ro doublearr_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=doublearr_cs_ro.dasm.il doublearr_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=doublearr_cs_ro.dasm.il doublearr_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=doublearr_cs_ro.asm.exe  doublearr_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=doublearr_cs_ro.asm.exe  doublearr_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=doublearr_cs_ro
    if not exist "doublearr_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "doublearr_cs_ro.org" (
            mkdir IL
            copy doublearr_cs_ro.exe IL\doublearr_cs_ro.exe 
            ren doublearr_cs_ro.exe doublearr_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublearr_cs_ro.org /out doublearr_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublearr_cs_ro.org /out doublearr_cs_ro.exe
...
set ExePath=doublearr_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in doublearr_cs_ro.reflect.xml
    IF EXIST doublearr_cs_ro.reflect.xml set ReflectionRoots=-x doublearr_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\doublearr_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% doublearr_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% doublearr_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
