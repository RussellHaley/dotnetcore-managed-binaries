...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/InnerProd/intarr_cs_do intarr_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/InnerProd/intarr_cs_do intarr_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=intarr_cs_do.dasm.il intarr_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=intarr_cs_do.dasm.il intarr_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=intarr_cs_do.asm.exe  intarr_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=intarr_cs_do.asm.exe  intarr_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=intarr_cs_do
    if [ ! -f intarr_cs_do.org ]; then
        TakeLock
        if [ ! -f intarr_cs_do.org ]; then
          mkdir IL
          cp intarr_cs_do.exe IL/intarr_cs_do.exe
          mv intarr_cs_do.exe intarr_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in intarr_cs_do.org /out intarr_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in intarr_cs_do.org /out intarr_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in intarr_cs_do.reflect.xml
  if [ -f intarr_cs_do.reflect.xml ]
then
    ReflectionRoots="-x intarr_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/intarr_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" intarr_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" intarr_cs_do.asm.exe $CLRTestExecutionArguments
