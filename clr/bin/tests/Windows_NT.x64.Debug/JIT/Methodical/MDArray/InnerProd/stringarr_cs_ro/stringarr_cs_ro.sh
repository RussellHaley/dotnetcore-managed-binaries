...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/InnerProd/stringarr_cs_ro stringarr_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/InnerProd/stringarr_cs_ro stringarr_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=stringarr_cs_ro.dasm.il stringarr_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=stringarr_cs_ro.dasm.il stringarr_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=stringarr_cs_ro.asm.exe  stringarr_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=stringarr_cs_ro.asm.exe  stringarr_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=stringarr_cs_ro
    if [ ! -f stringarr_cs_ro.org ]; then
        TakeLock
        if [ ! -f stringarr_cs_ro.org ]; then
          mkdir IL
          cp stringarr_cs_ro.exe IL/stringarr_cs_ro.exe
          mv stringarr_cs_ro.exe stringarr_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in stringarr_cs_ro.org /out stringarr_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in stringarr_cs_ro.org /out stringarr_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in stringarr_cs_ro.reflect.xml
  if [ -f stringarr_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x stringarr_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/stringarr_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" stringarr_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" stringarr_cs_ro.asm.exe $CLRTestExecutionArguments
