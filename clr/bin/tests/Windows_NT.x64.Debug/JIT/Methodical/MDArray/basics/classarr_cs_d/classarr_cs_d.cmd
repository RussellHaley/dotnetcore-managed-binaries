...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\basics\classarr_cs_d classarr_cs_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\basics\classarr_cs_d classarr_cs_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=classarr_cs_d.dasm.il classarr_cs_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=classarr_cs_d.dasm.il classarr_cs_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=classarr_cs_d.asm.exe  classarr_cs_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=classarr_cs_d.asm.exe  classarr_cs_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=classarr_cs_d
    if not exist "classarr_cs_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "classarr_cs_d.org" (
            mkdir IL
            copy classarr_cs_d.exe IL\classarr_cs_d.exe 
            ren classarr_cs_d.exe classarr_cs_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in classarr_cs_d.org /out classarr_cs_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in classarr_cs_d.org /out classarr_cs_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in classarr_cs_d.reflect.xml
    IF EXIST classarr_cs_d.reflect.xml set ReflectionRoots=-x classarr_cs_d.reflect.xml
    ...
    set ExePath=%LinkBin%\classarr_cs_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% classarr_cs_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% classarr_cs_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
