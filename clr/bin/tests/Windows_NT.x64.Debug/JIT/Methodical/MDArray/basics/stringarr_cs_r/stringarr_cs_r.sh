...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/stringarr_cs_r stringarr_cs_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/stringarr_cs_r stringarr_cs_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=stringarr_cs_r.dasm.il stringarr_cs_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=stringarr_cs_r.dasm.il stringarr_cs_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=stringarr_cs_r.asm.exe  stringarr_cs_r.dasm.il
    "$CORE_ROOT/ilasm" -output=stringarr_cs_r.asm.exe  stringarr_cs_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=stringarr_cs_r
    if [ ! -f stringarr_cs_r.org ]; then
        TakeLock
        if [ ! -f stringarr_cs_r.org ]; then
          mkdir IL
          cp stringarr_cs_r.exe IL/stringarr_cs_r.exe
          mv stringarr_cs_r.exe stringarr_cs_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in stringarr_cs_r.org /out stringarr_cs_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in stringarr_cs_r.org /out stringarr_cs_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in stringarr_cs_r.reflect.xml
  if [ -f stringarr_cs_r.reflect.xml ]
then
    ReflectionRoots="-x stringarr_cs_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/stringarr_cs_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" stringarr_cs_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" stringarr_cs_r.asm.exe $CLRTestExecutionArguments
