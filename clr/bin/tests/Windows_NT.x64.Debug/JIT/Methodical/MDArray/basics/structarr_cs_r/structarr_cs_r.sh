...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/structarr_cs_r structarr_cs_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/structarr_cs_r structarr_cs_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=structarr_cs_r.dasm.il structarr_cs_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=structarr_cs_r.dasm.il structarr_cs_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=structarr_cs_r.asm.exe  structarr_cs_r.dasm.il
    "$CORE_ROOT/ilasm" -output=structarr_cs_r.asm.exe  structarr_cs_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=structarr_cs_r
    if [ ! -f structarr_cs_r.org ]; then
        TakeLock
        if [ ! -f structarr_cs_r.org ]; then
          mkdir IL
          cp structarr_cs_r.exe IL/structarr_cs_r.exe
          mv structarr_cs_r.exe structarr_cs_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in structarr_cs_r.org /out structarr_cs_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in structarr_cs_r.org /out structarr_cs_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in structarr_cs_r.reflect.xml
  if [ -f structarr_cs_r.reflect.xml ]
then
    ReflectionRoots="-x structarr_cs_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/structarr_cs_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" structarr_cs_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" structarr_cs_r.asm.exe $CLRTestExecutionArguments
