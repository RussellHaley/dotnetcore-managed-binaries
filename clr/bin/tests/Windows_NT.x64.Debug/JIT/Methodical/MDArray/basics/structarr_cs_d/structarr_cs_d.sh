...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/structarr_cs_d structarr_cs_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/structarr_cs_d structarr_cs_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=structarr_cs_d.dasm.il structarr_cs_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=structarr_cs_d.dasm.il structarr_cs_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=structarr_cs_d.asm.exe  structarr_cs_d.dasm.il
    "$CORE_ROOT/ilasm" -output=structarr_cs_d.asm.exe  structarr_cs_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=structarr_cs_d
    if [ ! -f structarr_cs_d.org ]; then
        TakeLock
        if [ ! -f structarr_cs_d.org ]; then
          mkdir IL
          cp structarr_cs_d.exe IL/structarr_cs_d.exe
          mv structarr_cs_d.exe structarr_cs_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in structarr_cs_d.org /out structarr_cs_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in structarr_cs_d.org /out structarr_cs_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in structarr_cs_d.reflect.xml
  if [ -f structarr_cs_d.reflect.xml ]
then
    ReflectionRoots="-x structarr_cs_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/structarr_cs_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" structarr_cs_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" structarr_cs_d.asm.exe $CLRTestExecutionArguments
