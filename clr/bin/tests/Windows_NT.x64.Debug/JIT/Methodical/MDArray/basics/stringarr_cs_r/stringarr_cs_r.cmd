...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\basics\stringarr_cs_r stringarr_cs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\basics\stringarr_cs_r stringarr_cs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=stringarr_cs_r.dasm.il stringarr_cs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=stringarr_cs_r.dasm.il stringarr_cs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=stringarr_cs_r.asm.exe  stringarr_cs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=stringarr_cs_r.asm.exe  stringarr_cs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=stringarr_cs_r
    if not exist "stringarr_cs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "stringarr_cs_r.org" (
            mkdir IL
            copy stringarr_cs_r.exe IL\stringarr_cs_r.exe 
            ren stringarr_cs_r.exe stringarr_cs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in stringarr_cs_r.org /out stringarr_cs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in stringarr_cs_r.org /out stringarr_cs_r.exe
...
set ExePath=stringarr_cs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in stringarr_cs_r.reflect.xml
    IF EXIST stringarr_cs_r.reflect.xml set ReflectionRoots=-x stringarr_cs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\stringarr_cs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% stringarr_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% stringarr_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
