...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\basics\jaggedarr_cs_d jaggedarr_cs_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\basics\jaggedarr_cs_d jaggedarr_cs_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=jaggedarr_cs_d.dasm.il jaggedarr_cs_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=jaggedarr_cs_d.dasm.il jaggedarr_cs_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=jaggedarr_cs_d.asm.exe  jaggedarr_cs_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=jaggedarr_cs_d.asm.exe  jaggedarr_cs_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=jaggedarr_cs_d
    if not exist "jaggedarr_cs_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "jaggedarr_cs_d.org" (
            mkdir IL
            copy jaggedarr_cs_d.exe IL\jaggedarr_cs_d.exe 
            ren jaggedarr_cs_d.exe jaggedarr_cs_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in jaggedarr_cs_d.org /out jaggedarr_cs_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in jaggedarr_cs_d.org /out jaggedarr_cs_d.exe
...
set ExePath=jaggedarr_cs_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in jaggedarr_cs_d.reflect.xml
    IF EXIST jaggedarr_cs_d.reflect.xml set ReflectionRoots=-x jaggedarr_cs_d.reflect.xml
    ...
    set ExePath=%LinkBin%\jaggedarr_cs_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% jaggedarr_cs_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% jaggedarr_cs_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
