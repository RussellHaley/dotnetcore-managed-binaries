...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/jaggedarr_cs_d jaggedarr_cs_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/jaggedarr_cs_d jaggedarr_cs_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=jaggedarr_cs_d.dasm.il jaggedarr_cs_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=jaggedarr_cs_d.dasm.il jaggedarr_cs_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=jaggedarr_cs_d.asm.exe  jaggedarr_cs_d.dasm.il
    "$CORE_ROOT/ilasm" -output=jaggedarr_cs_d.asm.exe  jaggedarr_cs_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=jaggedarr_cs_d
    if [ ! -f jaggedarr_cs_d.org ]; then
        TakeLock
        if [ ! -f jaggedarr_cs_d.org ]; then
          mkdir IL
          cp jaggedarr_cs_d.exe IL/jaggedarr_cs_d.exe
          mv jaggedarr_cs_d.exe jaggedarr_cs_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in jaggedarr_cs_d.org /out jaggedarr_cs_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in jaggedarr_cs_d.org /out jaggedarr_cs_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in jaggedarr_cs_d.reflect.xml
  if [ -f jaggedarr_cs_d.reflect.xml ]
then
    ReflectionRoots="-x jaggedarr_cs_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/jaggedarr_cs_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" jaggedarr_cs_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" jaggedarr_cs_d.asm.exe $CLRTestExecutionArguments
