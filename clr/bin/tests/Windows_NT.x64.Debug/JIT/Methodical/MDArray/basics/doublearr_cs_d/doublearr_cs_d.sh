...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/doublearr_cs_d doublearr_cs_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/doublearr_cs_d doublearr_cs_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=doublearr_cs_d.dasm.il doublearr_cs_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=doublearr_cs_d.dasm.il doublearr_cs_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=doublearr_cs_d.asm.exe  doublearr_cs_d.dasm.il
    "$CORE_ROOT/ilasm" -output=doublearr_cs_d.asm.exe  doublearr_cs_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=doublearr_cs_d
    if [ ! -f doublearr_cs_d.org ]; then
        TakeLock
        if [ ! -f doublearr_cs_d.org ]; then
          mkdir IL
          cp doublearr_cs_d.exe IL/doublearr_cs_d.exe
          mv doublearr_cs_d.exe doublearr_cs_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublearr_cs_d.org /out doublearr_cs_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublearr_cs_d.org /out doublearr_cs_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in doublearr_cs_d.reflect.xml
  if [ -f doublearr_cs_d.reflect.xml ]
then
    ReflectionRoots="-x doublearr_cs_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/doublearr_cs_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" doublearr_cs_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" doublearr_cs_d.asm.exe $CLRTestExecutionArguments
