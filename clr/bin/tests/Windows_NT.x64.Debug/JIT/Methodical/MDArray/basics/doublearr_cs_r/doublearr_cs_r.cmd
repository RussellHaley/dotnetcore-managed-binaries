...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\basics\doublearr_cs_r doublearr_cs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\MDArray\basics\doublearr_cs_r doublearr_cs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=doublearr_cs_r.dasm.il doublearr_cs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=doublearr_cs_r.dasm.il doublearr_cs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=doublearr_cs_r.asm.exe  doublearr_cs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=doublearr_cs_r.asm.exe  doublearr_cs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=doublearr_cs_r
    if not exist "doublearr_cs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "doublearr_cs_r.org" (
            mkdir IL
            copy doublearr_cs_r.exe IL\doublearr_cs_r.exe 
            ren doublearr_cs_r.exe doublearr_cs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublearr_cs_r.org /out doublearr_cs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublearr_cs_r.org /out doublearr_cs_r.exe
...
set ExePath=doublearr_cs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in doublearr_cs_r.reflect.xml
    IF EXIST doublearr_cs_r.reflect.xml set ReflectionRoots=-x doublearr_cs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\doublearr_cs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% doublearr_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% doublearr_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
