...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/stringarr_cs_d stringarr_cs_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/MDArray/basics/stringarr_cs_d stringarr_cs_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=stringarr_cs_d.dasm.il stringarr_cs_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=stringarr_cs_d.dasm.il stringarr_cs_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=stringarr_cs_d.asm.exe  stringarr_cs_d.dasm.il
    "$CORE_ROOT/ilasm" -output=stringarr_cs_d.asm.exe  stringarr_cs_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=stringarr_cs_d
    if [ ! -f stringarr_cs_d.org ]; then
        TakeLock
        if [ ! -f stringarr_cs_d.org ]; then
          mkdir IL
          cp stringarr_cs_d.exe IL/stringarr_cs_d.exe
          mv stringarr_cs_d.exe stringarr_cs_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in stringarr_cs_d.org /out stringarr_cs_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in stringarr_cs_d.org /out stringarr_cs_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in stringarr_cs_d.reflect.xml
  if [ -f stringarr_cs_d.reflect.xml ]
then
    ReflectionRoots="-x stringarr_cs_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/stringarr_cs_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" stringarr_cs_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" stringarr_cs_d.asm.exe $CLRTestExecutionArguments
