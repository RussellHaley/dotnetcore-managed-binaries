...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/cctor/misc/Desktop/throw_cs_r throw_cs_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/cctor/misc/Desktop/throw_cs_r throw_cs_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throw_cs_r.dasm.il throw_cs_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throw_cs_r.dasm.il throw_cs_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throw_cs_r.asm.exe  throw_cs_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throw_cs_r.asm.exe  throw_cs_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throw_cs_r
    if [ ! -f throw_cs_r.org ]; then
        TakeLock
        if [ ! -f throw_cs_r.org ]; then
          mkdir IL
          cp throw_cs_r.exe IL/throw_cs_r.exe
          mv throw_cs_r.exe throw_cs_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw_cs_r.org /out throw_cs_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw_cs_r.org /out throw_cs_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throw_cs_r.reflect.xml
  if [ -f throw_cs_r.reflect.xml ]
then
    ReflectionRoots="-x throw_cs_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throw_cs_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throw_cs_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throw_cs_r.asm.exe $CLRTestExecutionArguments
