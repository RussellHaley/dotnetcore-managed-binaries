...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/cctor/misc/Desktop/throw_cs_do throw_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/cctor/misc/Desktop/throw_cs_do throw_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throw_cs_do.dasm.il throw_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throw_cs_do.dasm.il throw_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throw_cs_do.asm.exe  throw_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throw_cs_do.asm.exe  throw_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throw_cs_do
    if [ ! -f throw_cs_do.org ]; then
        TakeLock
        if [ ! -f throw_cs_do.org ]; then
          mkdir IL
          cp throw_cs_do.exe IL/throw_cs_do.exe
          mv throw_cs_do.exe throw_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw_cs_do.org /out throw_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw_cs_do.org /out throw_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throw_cs_do.reflect.xml
  if [ -f throw_cs_do.reflect.xml ]
then
    ReflectionRoots="-x throw_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throw_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throw_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throw_cs_do.asm.exe $CLRTestExecutionArguments
