...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/cctor/misc/Desktop/throw_cs_d throw_cs_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/cctor/misc/Desktop/throw_cs_d throw_cs_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throw_cs_d.dasm.il throw_cs_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throw_cs_d.dasm.il throw_cs_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throw_cs_d.asm.exe  throw_cs_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throw_cs_d.asm.exe  throw_cs_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throw_cs_d
    if [ ! -f throw_cs_d.org ]; then
        TakeLock
        if [ ! -f throw_cs_d.org ]; then
          mkdir IL
          cp throw_cs_d.exe IL/throw_cs_d.exe
          mv throw_cs_d.exe throw_cs_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw_cs_d.org /out throw_cs_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw_cs_d.org /out throw_cs_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throw_cs_d.reflect.xml
  if [ -f throw_cs_d.reflect.xml ]
then
    ReflectionRoots="-x throw_cs_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throw_cs_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throw_cs_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throw_cs_d.asm.exe $CLRTestExecutionArguments
