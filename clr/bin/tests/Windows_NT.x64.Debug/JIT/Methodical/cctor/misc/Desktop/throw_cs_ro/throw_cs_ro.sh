...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/cctor/misc/Desktop/throw_cs_ro throw_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/cctor/misc/Desktop/throw_cs_ro throw_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throw_cs_ro.dasm.il throw_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=throw_cs_ro.dasm.il throw_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throw_cs_ro.asm.exe  throw_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=throw_cs_ro.asm.exe  throw_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throw_cs_ro
    if [ ! -f throw_cs_ro.org ]; then
        TakeLock
        if [ ! -f throw_cs_ro.org ]; then
          mkdir IL
          cp throw_cs_ro.exe IL/throw_cs_ro.exe
          mv throw_cs_ro.exe throw_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw_cs_ro.org /out throw_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw_cs_ro.org /out throw_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throw_cs_ro.reflect.xml
  if [ -f throw_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x throw_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throw_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throw_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throw_cs_ro.asm.exe $CLRTestExecutionArguments
