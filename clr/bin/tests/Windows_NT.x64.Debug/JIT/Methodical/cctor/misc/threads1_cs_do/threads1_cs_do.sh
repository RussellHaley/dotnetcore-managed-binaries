...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/cctor/misc/threads1_cs_do threads1_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/cctor/misc/threads1_cs_do threads1_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=threads1_cs_do.dasm.il threads1_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=threads1_cs_do.dasm.il threads1_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=threads1_cs_do.asm.exe  threads1_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=threads1_cs_do.asm.exe  threads1_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=threads1_cs_do
    if [ ! -f threads1_cs_do.org ]; then
        TakeLock
        if [ ! -f threads1_cs_do.org ]; then
          mkdir IL
          cp threads1_cs_do.exe IL/threads1_cs_do.exe
          mv threads1_cs_do.exe threads1_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in threads1_cs_do.org /out threads1_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in threads1_cs_do.org /out threads1_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in threads1_cs_do.reflect.xml
  if [ -f threads1_cs_do.reflect.xml ]
then
    ReflectionRoots="-x threads1_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/threads1_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" threads1_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" threads1_cs_do.asm.exe $CLRTestExecutionArguments
