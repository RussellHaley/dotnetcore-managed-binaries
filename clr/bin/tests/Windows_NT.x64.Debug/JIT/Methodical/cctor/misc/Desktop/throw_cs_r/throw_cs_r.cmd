...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Methodical\cctor\misc\Desktop\throw_cs_r throw_cs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Methodical\cctor\misc\Desktop\throw_cs_r throw_cs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throw_cs_r.dasm.il throw_cs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throw_cs_r.dasm.il throw_cs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throw_cs_r.asm.exe  throw_cs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throw_cs_r.asm.exe  throw_cs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throw_cs_r
    if not exist "throw_cs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throw_cs_r.org" (
            mkdir IL
            copy throw_cs_r.exe IL\throw_cs_r.exe 
            ren throw_cs_r.exe throw_cs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw_cs_r.org /out throw_cs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw_cs_r.org /out throw_cs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throw_cs_r.reflect.xml
    IF EXIST throw_cs_r.reflect.xml set ReflectionRoots=-x throw_cs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throw_cs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throw_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throw_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
