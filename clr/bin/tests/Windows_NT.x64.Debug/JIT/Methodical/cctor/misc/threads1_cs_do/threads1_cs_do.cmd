...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\cctor\misc\threads1_cs_do threads1_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\cctor\misc\threads1_cs_do threads1_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=threads1_cs_do.dasm.il threads1_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=threads1_cs_do.dasm.il threads1_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=threads1_cs_do.asm.exe  threads1_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=threads1_cs_do.asm.exe  threads1_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=threads1_cs_do
    if not exist "threads1_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "threads1_cs_do.org" (
            mkdir IL
            copy threads1_cs_do.exe IL\threads1_cs_do.exe 
            ren threads1_cs_do.exe threads1_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in threads1_cs_do.org /out threads1_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in threads1_cs_do.org /out threads1_cs_do.exe
...
set ExePath=threads1_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in threads1_cs_do.reflect.xml
    IF EXIST threads1_cs_do.reflect.xml set ReflectionRoots=-x threads1_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\threads1_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% threads1_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% threads1_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
