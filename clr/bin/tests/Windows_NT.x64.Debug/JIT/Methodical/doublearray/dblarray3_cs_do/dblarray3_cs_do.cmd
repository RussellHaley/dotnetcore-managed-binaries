...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\doublearray\dblarray3_cs_do dblarray3_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\doublearray\dblarray3_cs_do dblarray3_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=dblarray3_cs_do.dasm.il dblarray3_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=dblarray3_cs_do.dasm.il dblarray3_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=dblarray3_cs_do.asm.exe  dblarray3_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=dblarray3_cs_do.asm.exe  dblarray3_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=dblarray3_cs_do
    if not exist "dblarray3_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "dblarray3_cs_do.org" (
            mkdir IL
            copy dblarray3_cs_do.exe IL\dblarray3_cs_do.exe 
            ren dblarray3_cs_do.exe dblarray3_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dblarray3_cs_do.org /out dblarray3_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dblarray3_cs_do.org /out dblarray3_cs_do.exe
...
set ExePath=dblarray3_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in dblarray3_cs_do.reflect.xml
    IF EXIST dblarray3_cs_do.reflect.xml set ReflectionRoots=-x dblarray3_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\dblarray3_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% dblarray3_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% dblarray3_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
