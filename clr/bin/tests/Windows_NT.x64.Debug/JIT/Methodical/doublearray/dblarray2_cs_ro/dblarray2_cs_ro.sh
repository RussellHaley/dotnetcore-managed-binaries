...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/doublearray/dblarray2_cs_ro dblarray2_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/doublearray/dblarray2_cs_ro dblarray2_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=dblarray2_cs_ro.dasm.il dblarray2_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=dblarray2_cs_ro.dasm.il dblarray2_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=dblarray2_cs_ro.asm.exe  dblarray2_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=dblarray2_cs_ro.asm.exe  dblarray2_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=dblarray2_cs_ro
    if [ ! -f dblarray2_cs_ro.org ]; then
        TakeLock
        if [ ! -f dblarray2_cs_ro.org ]; then
          mkdir IL
          cp dblarray2_cs_ro.exe IL/dblarray2_cs_ro.exe
          mv dblarray2_cs_ro.exe dblarray2_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dblarray2_cs_ro.org /out dblarray2_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dblarray2_cs_ro.org /out dblarray2_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in dblarray2_cs_ro.reflect.xml
  if [ -f dblarray2_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x dblarray2_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/dblarray2_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" dblarray2_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" dblarray2_cs_ro.asm.exe $CLRTestExecutionArguments
