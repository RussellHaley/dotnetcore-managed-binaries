...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/doublearray/dblarray2_cs_do dblarray2_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/doublearray/dblarray2_cs_do dblarray2_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=dblarray2_cs_do.dasm.il dblarray2_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=dblarray2_cs_do.dasm.il dblarray2_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=dblarray2_cs_do.asm.exe  dblarray2_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=dblarray2_cs_do.asm.exe  dblarray2_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=dblarray2_cs_do
    if [ ! -f dblarray2_cs_do.org ]; then
        TakeLock
        if [ ! -f dblarray2_cs_do.org ]; then
          mkdir IL
          cp dblarray2_cs_do.exe IL/dblarray2_cs_do.exe
          mv dblarray2_cs_do.exe dblarray2_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dblarray2_cs_do.org /out dblarray2_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dblarray2_cs_do.org /out dblarray2_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in dblarray2_cs_do.reflect.xml
  if [ -f dblarray2_cs_do.reflect.xml ]
then
    ReflectionRoots="-x dblarray2_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/dblarray2_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" dblarray2_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" dblarray2_cs_do.asm.exe $CLRTestExecutionArguments
