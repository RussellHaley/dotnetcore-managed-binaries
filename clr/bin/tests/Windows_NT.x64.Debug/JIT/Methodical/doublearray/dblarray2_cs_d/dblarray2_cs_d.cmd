...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\doublearray\dblarray2_cs_d dblarray2_cs_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\doublearray\dblarray2_cs_d dblarray2_cs_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=dblarray2_cs_d.dasm.il dblarray2_cs_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=dblarray2_cs_d.dasm.il dblarray2_cs_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=dblarray2_cs_d.asm.exe  dblarray2_cs_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=dblarray2_cs_d.asm.exe  dblarray2_cs_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=dblarray2_cs_d
    if not exist "dblarray2_cs_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "dblarray2_cs_d.org" (
            mkdir IL
            copy dblarray2_cs_d.exe IL\dblarray2_cs_d.exe 
            ren dblarray2_cs_d.exe dblarray2_cs_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dblarray2_cs_d.org /out dblarray2_cs_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dblarray2_cs_d.org /out dblarray2_cs_d.exe
...
set ExePath=dblarray2_cs_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in dblarray2_cs_d.reflect.xml
    IF EXIST dblarray2_cs_d.reflect.xml set ReflectionRoots=-x dblarray2_cs_d.reflect.xml
    ...
    set ExePath=%LinkBin%\dblarray2_cs_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% dblarray2_cs_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% dblarray2_cs_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
