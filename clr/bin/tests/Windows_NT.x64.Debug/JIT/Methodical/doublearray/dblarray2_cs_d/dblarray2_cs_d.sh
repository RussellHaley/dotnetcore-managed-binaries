...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/doublearray/dblarray2_cs_d dblarray2_cs_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/doublearray/dblarray2_cs_d dblarray2_cs_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=dblarray2_cs_d.dasm.il dblarray2_cs_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=dblarray2_cs_d.dasm.il dblarray2_cs_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=dblarray2_cs_d.asm.exe  dblarray2_cs_d.dasm.il
    "$CORE_ROOT/ilasm" -output=dblarray2_cs_d.asm.exe  dblarray2_cs_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=dblarray2_cs_d
    if [ ! -f dblarray2_cs_d.org ]; then
        TakeLock
        if [ ! -f dblarray2_cs_d.org ]; then
          mkdir IL
          cp dblarray2_cs_d.exe IL/dblarray2_cs_d.exe
          mv dblarray2_cs_d.exe dblarray2_cs_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dblarray2_cs_d.org /out dblarray2_cs_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dblarray2_cs_d.org /out dblarray2_cs_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in dblarray2_cs_d.reflect.xml
  if [ -f dblarray2_cs_d.reflect.xml ]
then
    ReflectionRoots="-x dblarray2_cs_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/dblarray2_cs_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" dblarray2_cs_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" dblarray2_cs_d.asm.exe $CLRTestExecutionArguments
