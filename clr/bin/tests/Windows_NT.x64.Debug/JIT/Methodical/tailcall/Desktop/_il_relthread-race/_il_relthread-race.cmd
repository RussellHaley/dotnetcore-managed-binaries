...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\tailcall\Desktop\_il_relthread-race _il_relthread-race.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\tailcall\Desktop\_il_relthread-race _il_relthread-race.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relthread-race.dasm.il _il_relthread-race.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relthread-race.dasm.il _il_relthread-race.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relthread-race.asm.exe  _il_relthread-race.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relthread-race.asm.exe  _il_relthread-race.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relthread-race
    if not exist "_il_relthread-race.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relthread-race.org" (
            mkdir IL
            copy _il_relthread-race.exe IL\_il_relthread-race.exe 
            ren _il_relthread-race.exe _il_relthread-race.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relthread-race.org /out _il_relthread-race.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relthread-race.org /out _il_relthread-race.exe
...
set ExePath=_il_relthread-race.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relthread-race.reflect.xml
    IF EXIST _il_relthread-race.reflect.xml set ReflectionRoots=-x _il_relthread-race.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relthread-race.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relthread-race.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relthread-race.asm.exe %CLRTestExecutionArguments% %Host_Args%
