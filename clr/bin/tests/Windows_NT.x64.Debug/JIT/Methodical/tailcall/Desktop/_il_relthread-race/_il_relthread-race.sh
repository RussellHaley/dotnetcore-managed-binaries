...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/tailcall/Desktop/_il_relthread-race _il_relthread-race.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/tailcall/Desktop/_il_relthread-race _il_relthread-race.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relthread-race.dasm.il _il_relthread-race.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relthread-race.dasm.il _il_relthread-race.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relthread-race.asm.exe  _il_relthread-race.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relthread-race.asm.exe  _il_relthread-race.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relthread-race
    if [ ! -f _il_relthread-race.org ]; then
        TakeLock
        if [ ! -f _il_relthread-race.org ]; then
          mkdir IL
          cp _il_relthread-race.exe IL/_il_relthread-race.exe
          mv _il_relthread-race.exe _il_relthread-race.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relthread-race.org /out _il_relthread-race.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relthread-race.org /out _il_relthread-race.exe
          __cgExitCode=$?
...
ExePath=_il_relthread-race.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relthread-race.reflect.xml
  if [ -f _il_relthread-race.reflect.xml ]
then
    ReflectionRoots="-x _il_relthread-race.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relthread-race.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relthread-race.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relthread-race.asm.exe $CLRTestExecutionArguments
