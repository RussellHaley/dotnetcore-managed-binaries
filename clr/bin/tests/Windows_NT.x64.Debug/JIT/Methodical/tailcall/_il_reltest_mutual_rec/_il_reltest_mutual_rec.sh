...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_reltest_mutual_rec _il_reltest_mutual_rec.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_reltest_mutual_rec _il_reltest_mutual_rec.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_reltest_mutual_rec.dasm.il _il_reltest_mutual_rec.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_reltest_mutual_rec.dasm.il _il_reltest_mutual_rec.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_reltest_mutual_rec.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltest_mutual_rec.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_reltest_mutual_rec.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltest_mutual_rec.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_reltest_mutual_rec
    if [ ! -f _il_reltest_mutual_rec.org ]; then
        TakeLock
        if [ ! -f _il_reltest_mutual_rec.org ]; then
          mkdir IL
          cp _il_reltest_mutual_rec.exe IL/_il_reltest_mutual_rec.exe
          mv _il_reltest_mutual_rec.exe _il_reltest_mutual_rec.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reltest_mutual_rec.org /out _il_reltest_mutual_rec.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reltest_mutual_rec.org /out _il_reltest_mutual_rec.exe
          __cgExitCode=$?
...
ExePath=_il_reltest_mutual_rec.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_reltest_mutual_rec.reflect.xml
  if [ -f _il_reltest_mutual_rec.reflect.xml ]
then
    ReflectionRoots="-x _il_reltest_mutual_rec.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_reltest_mutual_rec.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_reltest_mutual_rec.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_reltest_mutual_rec.asm.exe $CLRTestExecutionArguments
