...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_reldeep_array_nz _il_reldeep_array_nz.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_reldeep_array_nz _il_reldeep_array_nz.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_reldeep_array_nz.dasm.il _il_reldeep_array_nz.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_reldeep_array_nz.dasm.il _il_reldeep_array_nz.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_reldeep_array_nz.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reldeep_array_nz.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_reldeep_array_nz.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reldeep_array_nz.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_reldeep_array_nz
    if [ ! -f _il_reldeep_array_nz.org ]; then
        TakeLock
        if [ ! -f _il_reldeep_array_nz.org ]; then
          mkdir IL
          cp _il_reldeep_array_nz.exe IL/_il_reldeep_array_nz.exe
          mv _il_reldeep_array_nz.exe _il_reldeep_array_nz.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reldeep_array_nz.org /out _il_reldeep_array_nz.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reldeep_array_nz.org /out _il_reldeep_array_nz.exe
          __cgExitCode=$?
...
ExePath=_il_reldeep_array_nz.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_reldeep_array_nz.reflect.xml
  if [ -f _il_reldeep_array_nz.reflect.xml ]
then
    ReflectionRoots="-x _il_reldeep_array_nz.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_reldeep_array_nz.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_reldeep_array_nz.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_reldeep_array_nz.asm.exe $CLRTestExecutionArguments
