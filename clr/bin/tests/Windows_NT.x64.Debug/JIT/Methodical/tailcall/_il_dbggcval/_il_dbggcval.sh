...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbggcval _il_dbggcval.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbggcval _il_dbggcval.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbggcval.dasm.il _il_dbggcval.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbggcval.dasm.il _il_dbggcval.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbggcval.asm.exe -QUIET -NOLOGO -DEBUG _il_dbggcval.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbggcval.asm.exe -QUIET -NOLOGO -DEBUG _il_dbggcval.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbggcval
    if [ ! -f _il_dbggcval.org ]; then
        TakeLock
        if [ ! -f _il_dbggcval.org ]; then
          mkdir IL
          cp _il_dbggcval.exe IL/_il_dbggcval.exe
          mv _il_dbggcval.exe _il_dbggcval.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbggcval.org /out _il_dbggcval.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbggcval.org /out _il_dbggcval.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbggcval.reflect.xml
  if [ -f _il_dbggcval.reflect.xml ]
then
    ReflectionRoots="-x _il_dbggcval.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbggcval.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbggcval.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbggcval.asm.exe $CLRTestExecutionArguments
