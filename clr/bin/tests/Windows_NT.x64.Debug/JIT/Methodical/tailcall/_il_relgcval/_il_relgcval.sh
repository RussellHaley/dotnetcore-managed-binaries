...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_relgcval _il_relgcval.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_relgcval _il_relgcval.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relgcval.dasm.il _il_relgcval.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relgcval.dasm.il _il_relgcval.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relgcval.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcval.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relgcval.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcval.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relgcval
    if [ ! -f _il_relgcval.org ]; then
        TakeLock
        if [ ! -f _il_relgcval.org ]; then
          mkdir IL
          cp _il_relgcval.exe IL/_il_relgcval.exe
          mv _il_relgcval.exe _il_relgcval.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relgcval.org /out _il_relgcval.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relgcval.org /out _il_relgcval.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relgcval.reflect.xml
  if [ -f _il_relgcval.reflect.xml ]
then
    ReflectionRoots="-x _il_relgcval.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relgcval.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relgcval.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relgcval.asm.exe $CLRTestExecutionArguments
