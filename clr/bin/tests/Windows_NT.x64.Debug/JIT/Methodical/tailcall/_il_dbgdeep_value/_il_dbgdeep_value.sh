...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgdeep_value _il_dbgdeep_value.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgdeep_value _il_dbgdeep_value.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgdeep_value.dasm.il _il_dbgdeep_value.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgdeep_value.dasm.il _il_dbgdeep_value.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgdeep_value.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgdeep_value.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgdeep_value.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgdeep_value.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgdeep_value
    if [ ! -f _il_dbgdeep_value.org ]; then
        TakeLock
        if [ ! -f _il_dbgdeep_value.org ]; then
          mkdir IL
          cp _il_dbgdeep_value.exe IL/_il_dbgdeep_value.exe
          mv _il_dbgdeep_value.exe _il_dbgdeep_value.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgdeep_value.org /out _il_dbgdeep_value.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgdeep_value.org /out _il_dbgdeep_value.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgdeep_value.reflect.xml
  if [ -f _il_dbgdeep_value.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgdeep_value.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgdeep_value.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgdeep_value.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgdeep_value.asm.exe $CLRTestExecutionArguments
