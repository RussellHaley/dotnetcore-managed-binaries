...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_reltest_switch _il_reltest_switch.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_reltest_switch _il_reltest_switch.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_reltest_switch.dasm.il _il_reltest_switch.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_reltest_switch.dasm.il _il_reltest_switch.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_reltest_switch.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltest_switch.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_reltest_switch.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltest_switch.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_reltest_switch
    if not exist "_il_reltest_switch.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_reltest_switch.org" (
            mkdir IL
            copy _il_reltest_switch.exe IL\_il_reltest_switch.exe 
            ren _il_reltest_switch.exe _il_reltest_switch.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reltest_switch.org /out _il_reltest_switch.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reltest_switch.org /out _il_reltest_switch.exe
...
set ExePath=_il_reltest_switch.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_reltest_switch.reflect.xml
    IF EXIST _il_reltest_switch.reflect.xml set ReflectionRoots=-x _il_reltest_switch.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_reltest_switch.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_reltest_switch.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_reltest_switch.asm.exe %CLRTestExecutionArguments% %Host_Args%
