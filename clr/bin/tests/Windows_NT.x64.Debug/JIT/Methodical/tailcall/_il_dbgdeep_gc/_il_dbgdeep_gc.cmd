...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgdeep_gc _il_dbgdeep_gc.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgdeep_gc _il_dbgdeep_gc.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgdeep_gc.dasm.il _il_dbgdeep_gc.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgdeep_gc.dasm.il _il_dbgdeep_gc.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgdeep_gc.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgdeep_gc.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgdeep_gc.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgdeep_gc.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgdeep_gc
    if not exist "_il_dbgdeep_gc.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgdeep_gc.org" (
            mkdir IL
            copy _il_dbgdeep_gc.exe IL\_il_dbgdeep_gc.exe 
            ren _il_dbgdeep_gc.exe _il_dbgdeep_gc.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgdeep_gc.org /out _il_dbgdeep_gc.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgdeep_gc.org /out _il_dbgdeep_gc.exe
...
set ExePath=_il_dbgdeep_gc.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgdeep_gc.reflect.xml
    IF EXIST _il_dbgdeep_gc.reflect.xml set ReflectionRoots=-x _il_dbgdeep_gc.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgdeep_gc.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgdeep_gc.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgdeep_gc.asm.exe %CLRTestExecutionArguments% %Host_Args%
