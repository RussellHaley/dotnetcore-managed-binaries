...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgreference_i _il_dbgreference_i.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgreference_i _il_dbgreference_i.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgreference_i.dasm.il _il_dbgreference_i.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgreference_i.dasm.il _il_dbgreference_i.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgreference_i.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgreference_i.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgreference_i.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgreference_i.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgreference_i
    if not exist "_il_dbgreference_i.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgreference_i.org" (
            mkdir IL
            copy _il_dbgreference_i.exe IL\_il_dbgreference_i.exe 
            ren _il_dbgreference_i.exe _il_dbgreference_i.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgreference_i.org /out _il_dbgreference_i.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgreference_i.org /out _il_dbgreference_i.exe
...
set ExePath=_il_dbgreference_i.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgreference_i.reflect.xml
    IF EXIST _il_dbgreference_i.reflect.xml set ReflectionRoots=-x _il_dbgreference_i.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgreference_i.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgreference_i.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgreference_i.asm.exe %CLRTestExecutionArguments% %Host_Args%
