...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgrecurse_ep_void _il_dbgrecurse_ep_void.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgrecurse_ep_void _il_dbgrecurse_ep_void.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrecurse_ep_void.dasm.il _il_dbgrecurse_ep_void.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrecurse_ep_void.dasm.il _il_dbgrecurse_ep_void.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgrecurse_ep_void.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_ep_void.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgrecurse_ep_void.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_ep_void.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgrecurse_ep_void
    if not exist "_il_dbgrecurse_ep_void.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgrecurse_ep_void.org" (
            mkdir IL
            copy _il_dbgrecurse_ep_void.exe IL\_il_dbgrecurse_ep_void.exe 
            ren _il_dbgrecurse_ep_void.exe _il_dbgrecurse_ep_void.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrecurse_ep_void.org /out _il_dbgrecurse_ep_void.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrecurse_ep_void.org /out _il_dbgrecurse_ep_void.exe
...
set ExePath=_il_dbgrecurse_ep_void.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgrecurse_ep_void.reflect.xml
    IF EXIST _il_dbgrecurse_ep_void.reflect.xml set ReflectionRoots=-x _il_dbgrecurse_ep_void.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgrecurse_ep_void.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgrecurse_ep_void.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgrecurse_ep_void.asm.exe %CLRTestExecutionArguments% %Host_Args%
