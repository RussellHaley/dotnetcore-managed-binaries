...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgrecurse_ep_void _il_dbgrecurse_ep_void.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgrecurse_ep_void _il_dbgrecurse_ep_void.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrecurse_ep_void.dasm.il _il_dbgrecurse_ep_void.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrecurse_ep_void.dasm.il _il_dbgrecurse_ep_void.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgrecurse_ep_void.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_ep_void.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgrecurse_ep_void.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_ep_void.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgrecurse_ep_void
    if [ ! -f _il_dbgrecurse_ep_void.org ]; then
        TakeLock
        if [ ! -f _il_dbgrecurse_ep_void.org ]; then
          mkdir IL
          cp _il_dbgrecurse_ep_void.exe IL/_il_dbgrecurse_ep_void.exe
          mv _il_dbgrecurse_ep_void.exe _il_dbgrecurse_ep_void.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrecurse_ep_void.org /out _il_dbgrecurse_ep_void.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrecurse_ep_void.org /out _il_dbgrecurse_ep_void.exe
          __cgExitCode=$?
...
ExePath=_il_dbgrecurse_ep_void.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgrecurse_ep_void.reflect.xml
  if [ -f _il_dbgrecurse_ep_void.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgrecurse_ep_void.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgrecurse_ep_void.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgrecurse_ep_void.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgrecurse_ep_void.asm.exe $CLRTestExecutionArguments
