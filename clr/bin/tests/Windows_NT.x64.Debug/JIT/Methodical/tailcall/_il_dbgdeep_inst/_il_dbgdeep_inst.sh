...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgdeep_inst _il_dbgdeep_inst.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgdeep_inst _il_dbgdeep_inst.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgdeep_inst.dasm.il _il_dbgdeep_inst.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgdeep_inst.dasm.il _il_dbgdeep_inst.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgdeep_inst.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgdeep_inst.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgdeep_inst.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgdeep_inst.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgdeep_inst
    if [ ! -f _il_dbgdeep_inst.org ]; then
        TakeLock
        if [ ! -f _il_dbgdeep_inst.org ]; then
          mkdir IL
          cp _il_dbgdeep_inst.exe IL/_il_dbgdeep_inst.exe
          mv _il_dbgdeep_inst.exe _il_dbgdeep_inst.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgdeep_inst.org /out _il_dbgdeep_inst.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgdeep_inst.org /out _il_dbgdeep_inst.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgdeep_inst.reflect.xml
  if [ -f _il_dbgdeep_inst.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgdeep_inst.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgdeep_inst.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgdeep_inst.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgdeep_inst.asm.exe $CLRTestExecutionArguments
