...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgtest_switch _il_dbgtest_switch.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgtest_switch _il_dbgtest_switch.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtest_switch.dasm.il _il_dbgtest_switch.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtest_switch.dasm.il _il_dbgtest_switch.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgtest_switch.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_switch.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgtest_switch.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_switch.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgtest_switch
    if [ ! -f _il_dbgtest_switch.org ]; then
        TakeLock
        if [ ! -f _il_dbgtest_switch.org ]; then
          mkdir IL
          cp _il_dbgtest_switch.exe IL/_il_dbgtest_switch.exe
          mv _il_dbgtest_switch.exe _il_dbgtest_switch.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtest_switch.org /out _il_dbgtest_switch.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtest_switch.org /out _il_dbgtest_switch.exe
          __cgExitCode=$?
...
ExePath=_il_dbgtest_switch.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgtest_switch.reflect.xml
  if [ -f _il_dbgtest_switch.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgtest_switch.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgtest_switch.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgtest_switch.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgtest_switch.asm.exe $CLRTestExecutionArguments
