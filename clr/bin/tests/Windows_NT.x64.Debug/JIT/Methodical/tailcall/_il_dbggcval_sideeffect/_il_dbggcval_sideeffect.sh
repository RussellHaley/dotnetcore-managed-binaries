...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbggcval_sideeffect _il_dbggcval_sideeffect.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbggcval_sideeffect _il_dbggcval_sideeffect.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbggcval_sideeffect.dasm.il _il_dbggcval_sideeffect.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbggcval_sideeffect.dasm.il _il_dbggcval_sideeffect.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbggcval_sideeffect.asm.exe -QUIET -NOLOGO -DEBUG _il_dbggcval_sideeffect.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbggcval_sideeffect.asm.exe -QUIET -NOLOGO -DEBUG _il_dbggcval_sideeffect.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbggcval_sideeffect
    if [ ! -f _il_dbggcval_sideeffect.org ]; then
        TakeLock
        if [ ! -f _il_dbggcval_sideeffect.org ]; then
          mkdir IL
          cp _il_dbggcval_sideeffect.exe IL/_il_dbggcval_sideeffect.exe
          mv _il_dbggcval_sideeffect.exe _il_dbggcval_sideeffect.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbggcval_sideeffect.org /out _il_dbggcval_sideeffect.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbggcval_sideeffect.org /out _il_dbggcval_sideeffect.exe
          __cgExitCode=$?
...
ExePath=_il_dbggcval_sideeffect.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbggcval_sideeffect.reflect.xml
  if [ -f _il_dbggcval_sideeffect.reflect.xml ]
then
    ReflectionRoots="-x _il_dbggcval_sideeffect.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbggcval_sideeffect.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbggcval_sideeffect.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbggcval_sideeffect.asm.exe $CLRTestExecutionArguments
