...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbggcval_sideeffect _il_dbggcval_sideeffect.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbggcval_sideeffect _il_dbggcval_sideeffect.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbggcval_sideeffect.dasm.il _il_dbggcval_sideeffect.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbggcval_sideeffect.dasm.il _il_dbggcval_sideeffect.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbggcval_sideeffect.asm.exe -QUIET -NOLOGO -DEBUG _il_dbggcval_sideeffect.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbggcval_sideeffect.asm.exe -QUIET -NOLOGO -DEBUG _il_dbggcval_sideeffect.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbggcval_sideeffect
    if not exist "_il_dbggcval_sideeffect.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbggcval_sideeffect.org" (
            mkdir IL
            copy _il_dbggcval_sideeffect.exe IL\_il_dbggcval_sideeffect.exe 
            ren _il_dbggcval_sideeffect.exe _il_dbggcval_sideeffect.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbggcval_sideeffect.org /out _il_dbggcval_sideeffect.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbggcval_sideeffect.org /out _il_dbggcval_sideeffect.exe
...
set ExePath=_il_dbggcval_sideeffect.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbggcval_sideeffect.reflect.xml
    IF EXIST _il_dbggcval_sideeffect.reflect.xml set ReflectionRoots=-x _il_dbggcval_sideeffect.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbggcval_sideeffect.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbggcval_sideeffect.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbggcval_sideeffect.asm.exe %CLRTestExecutionArguments% %Host_Args%
