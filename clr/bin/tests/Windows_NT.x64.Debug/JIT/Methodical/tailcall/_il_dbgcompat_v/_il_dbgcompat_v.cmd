...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgcompat_v _il_dbgcompat_v.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgcompat_v _il_dbgcompat_v.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcompat_v.dasm.il _il_dbgcompat_v.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcompat_v.dasm.il _il_dbgcompat_v.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgcompat_v.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcompat_v.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgcompat_v.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcompat_v.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgcompat_v
    if not exist "_il_dbgcompat_v.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgcompat_v.org" (
            mkdir IL
            copy _il_dbgcompat_v.exe IL\_il_dbgcompat_v.exe 
            ren _il_dbgcompat_v.exe _il_dbgcompat_v.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcompat_v.org /out _il_dbgcompat_v.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcompat_v.org /out _il_dbgcompat_v.exe
...
set ExePath=_il_dbgcompat_v.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgcompat_v.reflect.xml
    IF EXIST _il_dbgcompat_v.reflect.xml set ReflectionRoots=-x _il_dbgcompat_v.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgcompat_v.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgcompat_v.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgcompat_v.asm.exe %CLRTestExecutionArguments% %Host_Args%
