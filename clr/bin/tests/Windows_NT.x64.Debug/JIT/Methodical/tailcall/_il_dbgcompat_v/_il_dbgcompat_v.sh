...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgcompat_v _il_dbgcompat_v.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgcompat_v _il_dbgcompat_v.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcompat_v.dasm.il _il_dbgcompat_v.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcompat_v.dasm.il _il_dbgcompat_v.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgcompat_v.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcompat_v.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgcompat_v.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcompat_v.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgcompat_v
    if [ ! -f _il_dbgcompat_v.org ]; then
        TakeLock
        if [ ! -f _il_dbgcompat_v.org ]; then
          mkdir IL
          cp _il_dbgcompat_v.exe IL/_il_dbgcompat_v.exe
          mv _il_dbgcompat_v.exe _il_dbgcompat_v.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcompat_v.org /out _il_dbgcompat_v.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcompat_v.org /out _il_dbgcompat_v.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgcompat_v.reflect.xml
  if [ -f _il_dbgcompat_v.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgcompat_v.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgcompat_v.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgcompat_v.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgcompat_v.asm.exe $CLRTestExecutionArguments
