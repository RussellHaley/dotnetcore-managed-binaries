...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgtest_implicit _il_dbgtest_implicit.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgtest_implicit _il_dbgtest_implicit.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtest_implicit.dasm.il _il_dbgtest_implicit.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtest_implicit.dasm.il _il_dbgtest_implicit.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgtest_implicit.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_implicit.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgtest_implicit.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_implicit.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgtest_implicit
    if [ ! -f _il_dbgtest_implicit.org ]; then
        TakeLock
        if [ ! -f _il_dbgtest_implicit.org ]; then
          mkdir IL
          cp _il_dbgtest_implicit.exe IL/_il_dbgtest_implicit.exe
          mv _il_dbgtest_implicit.exe _il_dbgtest_implicit.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtest_implicit.org /out _il_dbgtest_implicit.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtest_implicit.org /out _il_dbgtest_implicit.exe
          __cgExitCode=$?
...
ExePath=_il_dbgtest_implicit.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgtest_implicit.reflect.xml
  if [ -f _il_dbgtest_implicit.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgtest_implicit.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgtest_implicit.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgtest_implicit.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgtest_implicit.asm.exe $CLRTestExecutionArguments
