...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgtest_implicit _il_dbgtest_implicit.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgtest_implicit _il_dbgtest_implicit.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgtest_implicit.dasm.il _il_dbgtest_implicit.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgtest_implicit.dasm.il _il_dbgtest_implicit.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgtest_implicit.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_implicit.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgtest_implicit.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_implicit.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgtest_implicit
    if not exist "_il_dbgtest_implicit.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgtest_implicit.org" (
            mkdir IL
            copy _il_dbgtest_implicit.exe IL\_il_dbgtest_implicit.exe 
            ren _il_dbgtest_implicit.exe _il_dbgtest_implicit.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgtest_implicit.org /out _il_dbgtest_implicit.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgtest_implicit.org /out _il_dbgtest_implicit.exe
...
set ExePath=_il_dbgtest_implicit.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgtest_implicit.reflect.xml
    IF EXIST _il_dbgtest_implicit.reflect.xml set ReflectionRoots=-x _il_dbgtest_implicit.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgtest_implicit.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgtest_implicit.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgtest_implicit.asm.exe %CLRTestExecutionArguments% %Host_Args%
