...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_relrecurse_ep _il_relrecurse_ep.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_relrecurse_ep _il_relrecurse_ep.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relrecurse_ep.dasm.il _il_relrecurse_ep.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relrecurse_ep.dasm.il _il_relrecurse_ep.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relrecurse_ep.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrecurse_ep.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relrecurse_ep.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrecurse_ep.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relrecurse_ep
    if not exist "_il_relrecurse_ep.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relrecurse_ep.org" (
            mkdir IL
            copy _il_relrecurse_ep.exe IL\_il_relrecurse_ep.exe 
            ren _il_relrecurse_ep.exe _il_relrecurse_ep.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relrecurse_ep.org /out _il_relrecurse_ep.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relrecurse_ep.org /out _il_relrecurse_ep.exe
...
set ExePath=_il_relrecurse_ep.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relrecurse_ep.reflect.xml
    IF EXIST _il_relrecurse_ep.reflect.xml set ReflectionRoots=-x _il_relrecurse_ep.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relrecurse_ep.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relrecurse_ep.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relrecurse_ep.asm.exe %CLRTestExecutionArguments% %Host_Args%
