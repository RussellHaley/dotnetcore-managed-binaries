...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgcompat_i4_u _il_dbgcompat_i4_u.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgcompat_i4_u _il_dbgcompat_i4_u.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcompat_i4_u.dasm.il _il_dbgcompat_i4_u.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcompat_i4_u.dasm.il _il_dbgcompat_i4_u.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgcompat_i4_u.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcompat_i4_u.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgcompat_i4_u.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcompat_i4_u.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgcompat_i4_u
    if [ ! -f _il_dbgcompat_i4_u.org ]; then
        TakeLock
        if [ ! -f _il_dbgcompat_i4_u.org ]; then
          mkdir IL
          cp _il_dbgcompat_i4_u.exe IL/_il_dbgcompat_i4_u.exe
          mv _il_dbgcompat_i4_u.exe _il_dbgcompat_i4_u.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcompat_i4_u.org /out _il_dbgcompat_i4_u.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcompat_i4_u.org /out _il_dbgcompat_i4_u.exe
          __cgExitCode=$?
...
ExePath=_il_dbgcompat_i4_u.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgcompat_i4_u.reflect.xml
  if [ -f _il_dbgcompat_i4_u.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgcompat_i4_u.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgcompat_i4_u.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgcompat_i4_u.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgcompat_i4_u.asm.exe $CLRTestExecutionArguments
