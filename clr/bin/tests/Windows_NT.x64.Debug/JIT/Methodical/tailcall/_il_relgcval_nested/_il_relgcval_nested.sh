...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_relgcval_nested _il_relgcval_nested.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_relgcval_nested _il_relgcval_nested.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relgcval_nested.dasm.il _il_relgcval_nested.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relgcval_nested.dasm.il _il_relgcval_nested.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relgcval_nested.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcval_nested.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relgcval_nested.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcval_nested.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relgcval_nested
    if [ ! -f _il_relgcval_nested.org ]; then
        TakeLock
        if [ ! -f _il_relgcval_nested.org ]; then
          mkdir IL
          cp _il_relgcval_nested.exe IL/_il_relgcval_nested.exe
          mv _il_relgcval_nested.exe _il_relgcval_nested.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relgcval_nested.org /out _il_relgcval_nested.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relgcval_nested.org /out _il_relgcval_nested.exe
          __cgExitCode=$?
...
ExePath=_il_relgcval_nested.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relgcval_nested.reflect.xml
  if [ -f _il_relgcval_nested.reflect.xml ]
then
    ReflectionRoots="-x _il_relgcval_nested.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relgcval_nested.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relgcval_nested.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relgcval_nested.asm.exe $CLRTestExecutionArguments
