...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_relgcval_nested _il_relgcval_nested.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_relgcval_nested _il_relgcval_nested.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relgcval_nested.dasm.il _il_relgcval_nested.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relgcval_nested.dasm.il _il_relgcval_nested.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relgcval_nested.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcval_nested.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relgcval_nested.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcval_nested.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relgcval_nested
    if not exist "_il_relgcval_nested.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relgcval_nested.org" (
            mkdir IL
            copy _il_relgcval_nested.exe IL\_il_relgcval_nested.exe 
            ren _il_relgcval_nested.exe _il_relgcval_nested.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relgcval_nested.org /out _il_relgcval_nested.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relgcval_nested.org /out _il_relgcval_nested.exe
...
set ExePath=_il_relgcval_nested.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relgcval_nested.reflect.xml
    IF EXIST _il_relgcval_nested.reflect.xml set ReflectionRoots=-x _il_relgcval_nested.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relgcval_nested.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relgcval_nested.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relgcval_nested.asm.exe %CLRTestExecutionArguments% %Host_Args%
