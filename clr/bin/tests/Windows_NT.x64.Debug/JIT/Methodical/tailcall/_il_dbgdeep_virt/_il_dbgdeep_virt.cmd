...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgdeep_virt _il_dbgdeep_virt.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgdeep_virt _il_dbgdeep_virt.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgdeep_virt.dasm.il _il_dbgdeep_virt.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgdeep_virt.dasm.il _il_dbgdeep_virt.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgdeep_virt.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgdeep_virt.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgdeep_virt.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgdeep_virt.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgdeep_virt
    if not exist "_il_dbgdeep_virt.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgdeep_virt.org" (
            mkdir IL
            copy _il_dbgdeep_virt.exe IL\_il_dbgdeep_virt.exe 
            ren _il_dbgdeep_virt.exe _il_dbgdeep_virt.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgdeep_virt.org /out _il_dbgdeep_virt.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgdeep_virt.org /out _il_dbgdeep_virt.exe
...
set ExePath=_il_dbgdeep_virt.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgdeep_virt.reflect.xml
    IF EXIST _il_dbgdeep_virt.reflect.xml set ReflectionRoots=-x _il_dbgdeep_virt.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgdeep_virt.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgdeep_virt.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgdeep_virt.asm.exe %CLRTestExecutionArguments% %Host_Args%
