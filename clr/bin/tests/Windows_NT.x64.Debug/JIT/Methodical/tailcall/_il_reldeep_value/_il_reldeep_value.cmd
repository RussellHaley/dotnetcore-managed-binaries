...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_reldeep_value _il_reldeep_value.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_reldeep_value _il_reldeep_value.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_reldeep_value.dasm.il _il_reldeep_value.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_reldeep_value.dasm.il _il_reldeep_value.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_reldeep_value.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reldeep_value.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_reldeep_value.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reldeep_value.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_reldeep_value
    if not exist "_il_reldeep_value.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_reldeep_value.org" (
            mkdir IL
            copy _il_reldeep_value.exe IL\_il_reldeep_value.exe 
            ren _il_reldeep_value.exe _il_reldeep_value.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reldeep_value.org /out _il_reldeep_value.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reldeep_value.org /out _il_reldeep_value.exe
...
set ExePath=_il_reldeep_value.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_reldeep_value.reflect.xml
    IF EXIST _il_reldeep_value.reflect.xml set ReflectionRoots=-x _il_reldeep_value.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_reldeep_value.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_reldeep_value.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_reldeep_value.asm.exe %CLRTestExecutionArguments% %Host_Args%
