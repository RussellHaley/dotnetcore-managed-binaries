...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgtest_mutual_rec _il_dbgtest_mutual_rec.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall\_il_dbgtest_mutual_rec _il_dbgtest_mutual_rec.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgtest_mutual_rec.dasm.il _il_dbgtest_mutual_rec.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgtest_mutual_rec.dasm.il _il_dbgtest_mutual_rec.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgtest_mutual_rec.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_mutual_rec.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgtest_mutual_rec.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_mutual_rec.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgtest_mutual_rec
    if not exist "_il_dbgtest_mutual_rec.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgtest_mutual_rec.org" (
            mkdir IL
            copy _il_dbgtest_mutual_rec.exe IL\_il_dbgtest_mutual_rec.exe 
            ren _il_dbgtest_mutual_rec.exe _il_dbgtest_mutual_rec.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgtest_mutual_rec.org /out _il_dbgtest_mutual_rec.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgtest_mutual_rec.org /out _il_dbgtest_mutual_rec.exe
...
set ExePath=_il_dbgtest_mutual_rec.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgtest_mutual_rec.reflect.xml
    IF EXIST _il_dbgtest_mutual_rec.reflect.xml set ReflectionRoots=-x _il_dbgtest_mutual_rec.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgtest_mutual_rec.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgtest_mutual_rec.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgtest_mutual_rec.asm.exe %CLRTestExecutionArguments% %Host_Args%
