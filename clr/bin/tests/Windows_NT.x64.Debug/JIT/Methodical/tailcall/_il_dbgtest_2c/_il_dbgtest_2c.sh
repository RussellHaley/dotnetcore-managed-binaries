...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgtest_2c _il_dbgtest_2c.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall/_il_dbgtest_2c _il_dbgtest_2c.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtest_2c.dasm.il _il_dbgtest_2c.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtest_2c.dasm.il _il_dbgtest_2c.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgtest_2c.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_2c.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgtest_2c.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtest_2c.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgtest_2c
    if [ ! -f _il_dbgtest_2c.org ]; then
        TakeLock
        if [ ! -f _il_dbgtest_2c.org ]; then
          mkdir IL
          cp _il_dbgtest_2c.exe IL/_il_dbgtest_2c.exe
          mv _il_dbgtest_2c.exe _il_dbgtest_2c.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtest_2c.org /out _il_dbgtest_2c.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtest_2c.org /out _il_dbgtest_2c.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgtest_2c.reflect.xml
  if [ -f _il_dbgtest_2c.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgtest_2c.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgtest_2c.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgtest_2c.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgtest_2c.asm.exe $CLRTestExecutionArguments
