...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\VT\callconv\_speed_reljumper _speed_reljumper.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\VT\callconv\_speed_reljumper _speed_reljumper.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_reljumper.dasm.il _speed_reljumper.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_reljumper.dasm.il _speed_reljumper.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_reljumper.asm.exe  _speed_reljumper.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_reljumper.asm.exe  _speed_reljumper.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_reljumper
    if not exist "_speed_reljumper.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_reljumper.org" (
            mkdir IL
            copy _speed_reljumper.exe IL\_speed_reljumper.exe 
            ren _speed_reljumper.exe _speed_reljumper.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_reljumper.org /out _speed_reljumper.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_reljumper.org /out _speed_reljumper.exe
...
set ExePath=_speed_reljumper.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_reljumper.reflect.xml
    IF EXIST _speed_reljumper.reflect.xml set ReflectionRoots=-x _speed_reljumper.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_reljumper.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_reljumper.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_reljumper.asm.exe %CLRTestExecutionArguments% %Host_Args%
