...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_il_relvtret _il_relvtret.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_il_relvtret _il_relvtret.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relvtret.dasm.il _il_relvtret.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relvtret.dasm.il _il_relvtret.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relvtret.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relvtret.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relvtret.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relvtret.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relvtret
    if [ ! -f _il_relvtret.org ]; then
        TakeLock
        if [ ! -f _il_relvtret.org ]; then
          mkdir IL
          cp _il_relvtret.exe IL/_il_relvtret.exe
          mv _il_relvtret.exe _il_relvtret.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relvtret.org /out _il_relvtret.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relvtret.org /out _il_relvtret.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relvtret.reflect.xml
  if [ -f _il_relvtret.reflect.xml ]
then
    ReflectionRoots="-x _il_relvtret.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relvtret.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relvtret.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relvtret.asm.exe $CLRTestExecutionArguments
