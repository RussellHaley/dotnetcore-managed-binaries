...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\VT\callconv\_il_reljumper5 _il_reljumper5.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\VT\callconv\_il_reljumper5 _il_reljumper5.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_reljumper5.dasm.il _il_reljumper5.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_reljumper5.dasm.il _il_reljumper5.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_reljumper5.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reljumper5.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_reljumper5.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reljumper5.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_reljumper5
    if not exist "_il_reljumper5.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_reljumper5.org" (
            mkdir IL
            copy _il_reljumper5.exe IL\_il_reljumper5.exe 
            ren _il_reljumper5.exe _il_reljumper5.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reljumper5.org /out _il_reljumper5.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reljumper5.org /out _il_reljumper5.exe
...
set ExePath=_il_reljumper5.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_reljumper5.reflect.xml
    IF EXIST _il_reljumper5.reflect.xml set ReflectionRoots=-x _il_reljumper5.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_reljumper5.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_reljumper5.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_reljumper5.asm.exe %CLRTestExecutionArguments% %Host_Args%
