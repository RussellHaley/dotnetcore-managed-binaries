...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_il_dbgjumps3 _il_dbgjumps3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_il_dbgjumps3 _il_dbgjumps3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgjumps3.dasm.il _il_dbgjumps3.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgjumps3.dasm.il _il_dbgjumps3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgjumps3.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgjumps3.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgjumps3.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgjumps3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgjumps3
    if [ ! -f _il_dbgjumps3.org ]; then
        TakeLock
        if [ ! -f _il_dbgjumps3.org ]; then
          mkdir IL
          cp _il_dbgjumps3.exe IL/_il_dbgjumps3.exe
          mv _il_dbgjumps3.exe _il_dbgjumps3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgjumps3.org /out _il_dbgjumps3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgjumps3.org /out _il_dbgjumps3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgjumps3.reflect.xml
  if [ -f _il_dbgjumps3.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgjumps3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgjumps3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgjumps3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgjumps3.asm.exe $CLRTestExecutionArguments
