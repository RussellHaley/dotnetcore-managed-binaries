...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_speed_reljumper _speed_reljumper.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_speed_reljumper _speed_reljumper.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_reljumper.dasm.il _speed_reljumper.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_reljumper.dasm.il _speed_reljumper.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_reljumper.asm.exe  _speed_reljumper.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_reljumper.asm.exe  _speed_reljumper.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_reljumper
    if [ ! -f _speed_reljumper.org ]; then
        TakeLock
        if [ ! -f _speed_reljumper.org ]; then
          mkdir IL
          cp _speed_reljumper.exe IL/_speed_reljumper.exe
          mv _speed_reljumper.exe _speed_reljumper.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_reljumper.org /out _speed_reljumper.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_reljumper.org /out _speed_reljumper.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_reljumper.reflect.xml
  if [ -f _speed_reljumper.reflect.xml ]
then
    ReflectionRoots="-x _speed_reljumper.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_reljumper.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_reljumper.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_reljumper.asm.exe $CLRTestExecutionArguments
