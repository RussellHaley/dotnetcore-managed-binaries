...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\VT\callconv\_il_dbgvtret _il_dbgvtret.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\VT\callconv\_il_dbgvtret _il_dbgvtret.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgvtret.dasm.il _il_dbgvtret.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgvtret.dasm.il _il_dbgvtret.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgvtret.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgvtret.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgvtret.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgvtret.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgvtret
    if not exist "_il_dbgvtret.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgvtret.org" (
            mkdir IL
            copy _il_dbgvtret.exe IL\_il_dbgvtret.exe 
            ren _il_dbgvtret.exe _il_dbgvtret.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgvtret.org /out _il_dbgvtret.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgvtret.org /out _il_dbgvtret.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgvtret.reflect.xml
    IF EXIST _il_dbgvtret.reflect.xml set ReflectionRoots=-x _il_dbgvtret.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgvtret.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgvtret.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgvtret.asm.exe %CLRTestExecutionArguments% %Host_Args%
