...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_il_dbgvtret _il_dbgvtret.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_il_dbgvtret _il_dbgvtret.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgvtret.dasm.il _il_dbgvtret.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgvtret.dasm.il _il_dbgvtret.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgvtret.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgvtret.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgvtret.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgvtret.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgvtret
    if [ ! -f _il_dbgvtret.org ]; then
        TakeLock
        if [ ! -f _il_dbgvtret.org ]; then
          mkdir IL
          cp _il_dbgvtret.exe IL/_il_dbgvtret.exe
          mv _il_dbgvtret.exe _il_dbgvtret.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgvtret.org /out _il_dbgvtret.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgvtret.org /out _il_dbgvtret.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgvtret.reflect.xml
  if [ -f _il_dbgvtret.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgvtret.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgvtret.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgvtret.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgvtret.asm.exe $CLRTestExecutionArguments
