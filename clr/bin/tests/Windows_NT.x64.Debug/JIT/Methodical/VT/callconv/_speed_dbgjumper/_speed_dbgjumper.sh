...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_speed_dbgjumper _speed_dbgjumper.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/callconv/_speed_dbgjumper _speed_dbgjumper.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgjumper.dasm.il _speed_dbgjumper.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgjumper.dasm.il _speed_dbgjumper.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgjumper.asm.exe  _speed_dbgjumper.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgjumper.asm.exe  _speed_dbgjumper.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgjumper
    if [ ! -f _speed_dbgjumper.org ]; then
        TakeLock
        if [ ! -f _speed_dbgjumper.org ]; then
          mkdir IL
          cp _speed_dbgjumper.exe IL/_speed_dbgjumper.exe
          mv _speed_dbgjumper.exe _speed_dbgjumper.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgjumper.org /out _speed_dbgjumper.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgjumper.org /out _speed_dbgjumper.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgjumper.reflect.xml
  if [ -f _speed_dbgjumper.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgjumper.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgjumper.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgjumper.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgjumper.asm.exe $CLRTestExecutionArguments
