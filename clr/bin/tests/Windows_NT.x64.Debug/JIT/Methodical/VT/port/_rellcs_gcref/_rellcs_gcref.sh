...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/port/_rellcs_gcref _rellcs_gcref.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/port/_rellcs_gcref _rellcs_gcref.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_rellcs_gcref.dasm.il _rellcs_gcref.exe
    "$CORE_ROOT/ildasm" -raweh -out=_rellcs_gcref.dasm.il _rellcs_gcref.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_rellcs_gcref.asm.exe  _rellcs_gcref.dasm.il
    "$CORE_ROOT/ilasm" -output=_rellcs_gcref.asm.exe  _rellcs_gcref.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_rellcs_gcref
    if [ ! -f _rellcs_gcref.org ]; then
        TakeLock
        if [ ! -f _rellcs_gcref.org ]; then
          mkdir IL
          cp _rellcs_gcref.exe IL/_rellcs_gcref.exe
          mv _rellcs_gcref.exe _rellcs_gcref.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _rellcs_gcref.org /out _rellcs_gcref.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _rellcs_gcref.org /out _rellcs_gcref.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _rellcs_gcref.reflect.xml
  if [ -f _rellcs_gcref.reflect.xml ]
then
    ReflectionRoots="-x _rellcs_gcref.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_rellcs_gcref.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _rellcs_gcref.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _rellcs_gcref.asm.exe $CLRTestExecutionArguments
