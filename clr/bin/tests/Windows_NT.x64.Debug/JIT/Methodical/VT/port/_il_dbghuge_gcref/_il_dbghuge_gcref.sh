...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/port/_il_dbghuge_gcref _il_dbghuge_gcref.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/port/_il_dbghuge_gcref _il_dbghuge_gcref.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbghuge_gcref.dasm.il _il_dbghuge_gcref.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbghuge_gcref.dasm.il _il_dbghuge_gcref.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbghuge_gcref.asm.exe -QUIET -NOLOGO -DEBUG _il_dbghuge_gcref.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbghuge_gcref.asm.exe -QUIET -NOLOGO -DEBUG _il_dbghuge_gcref.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbghuge_gcref
    if [ ! -f _il_dbghuge_gcref.org ]; then
        TakeLock
        if [ ! -f _il_dbghuge_gcref.org ]; then
          mkdir IL
          cp _il_dbghuge_gcref.exe IL/_il_dbghuge_gcref.exe
          mv _il_dbghuge_gcref.exe _il_dbghuge_gcref.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbghuge_gcref.org /out _il_dbghuge_gcref.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbghuge_gcref.org /out _il_dbghuge_gcref.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbghuge_gcref.reflect.xml
  if [ -f _il_dbghuge_gcref.reflect.xml ]
then
    ReflectionRoots="-x _il_dbghuge_gcref.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbghuge_gcref.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbghuge_gcref.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbghuge_gcref.asm.exe $CLRTestExecutionArguments
