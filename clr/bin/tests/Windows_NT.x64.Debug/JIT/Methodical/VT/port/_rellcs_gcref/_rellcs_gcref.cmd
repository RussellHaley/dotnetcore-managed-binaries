...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\VT\port\_rellcs_gcref _rellcs_gcref.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\VT\port\_rellcs_gcref _rellcs_gcref.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_rellcs_gcref.dasm.il _rellcs_gcref.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_rellcs_gcref.dasm.il _rellcs_gcref.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_rellcs_gcref.asm.exe  _rellcs_gcref.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_rellcs_gcref.asm.exe  _rellcs_gcref.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_rellcs_gcref
    if not exist "_rellcs_gcref.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_rellcs_gcref.org" (
            mkdir IL
            copy _rellcs_gcref.exe IL\_rellcs_gcref.exe 
            ren _rellcs_gcref.exe _rellcs_gcref.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcs_gcref.org /out _rellcs_gcref.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcs_gcref.org /out _rellcs_gcref.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _rellcs_gcref.reflect.xml
    IF EXIST _rellcs_gcref.reflect.xml set ReflectionRoots=-x _rellcs_gcref.reflect.xml
    ...
    set ExePath=%LinkBin%\_rellcs_gcref.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _rellcs_gcref.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _rellcs_gcref.asm.exe %CLRTestExecutionArguments% %Host_Args%
