...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/port/_dbglcs_gcref _dbglcs_gcref.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/VT/port/_dbglcs_gcref _dbglcs_gcref.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbglcs_gcref.dasm.il _dbglcs_gcref.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbglcs_gcref.dasm.il _dbglcs_gcref.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbglcs_gcref.asm.exe  _dbglcs_gcref.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbglcs_gcref.asm.exe  _dbglcs_gcref.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbglcs_gcref
    if [ ! -f _dbglcs_gcref.org ]; then
        TakeLock
        if [ ! -f _dbglcs_gcref.org ]; then
          mkdir IL
          cp _dbglcs_gcref.exe IL/_dbglcs_gcref.exe
          mv _dbglcs_gcref.exe _dbglcs_gcref.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcs_gcref.org /out _dbglcs_gcref.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcs_gcref.org /out _dbglcs_gcref.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbglcs_gcref.reflect.xml
  if [ -f _dbglcs_gcref.reflect.xml ]
then
    ReflectionRoots="-x _dbglcs_gcref.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbglcs_gcref.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbglcs_gcref.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbglcs_gcref.asm.exe $CLRTestExecutionArguments
