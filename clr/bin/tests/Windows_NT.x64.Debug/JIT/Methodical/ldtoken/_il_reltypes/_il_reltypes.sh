...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ldtoken/_il_reltypes _il_reltypes.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ldtoken/_il_reltypes _il_reltypes.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_reltypes.dasm.il _il_reltypes.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_reltypes.dasm.il _il_reltypes.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_reltypes.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltypes.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_reltypes.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltypes.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_reltypes
    if [ ! -f _il_reltypes.org ]; then
        TakeLock
        if [ ! -f _il_reltypes.org ]; then
          mkdir IL
          cp _il_reltypes.exe IL/_il_reltypes.exe
          mv _il_reltypes.exe _il_reltypes.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reltypes.org /out _il_reltypes.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reltypes.org /out _il_reltypes.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_reltypes.reflect.xml
  if [ -f _il_reltypes.reflect.xml ]
then
    ReflectionRoots="-x _il_reltypes.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_reltypes.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_reltypes.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_reltypes.asm.exe $CLRTestExecutionArguments
