...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\ldtoken\_il_reltypes _il_reltypes.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\ldtoken\_il_reltypes _il_reltypes.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_reltypes.dasm.il _il_reltypes.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_reltypes.dasm.il _il_reltypes.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_reltypes.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltypes.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_reltypes.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltypes.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_reltypes
    if not exist "_il_reltypes.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_reltypes.org" (
            mkdir IL
            copy _il_reltypes.exe IL\_il_reltypes.exe 
            ren _il_reltypes.exe _il_reltypes.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reltypes.org /out _il_reltypes.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reltypes.org /out _il_reltypes.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_reltypes.reflect.xml
    IF EXIST _il_reltypes.reflect.xml set ReflectionRoots=-x _il_reltypes.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_reltypes.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_reltypes.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_reltypes.asm.exe %CLRTestExecutionArguments% %Host_Args%
