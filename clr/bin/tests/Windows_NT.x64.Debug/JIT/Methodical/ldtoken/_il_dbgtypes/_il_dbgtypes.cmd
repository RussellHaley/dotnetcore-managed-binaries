...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\ldtoken\_il_dbgtypes _il_dbgtypes.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\ldtoken\_il_dbgtypes _il_dbgtypes.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgtypes.dasm.il _il_dbgtypes.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgtypes.dasm.il _il_dbgtypes.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgtypes.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtypes.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgtypes.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtypes.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgtypes
    if not exist "_il_dbgtypes.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgtypes.org" (
            mkdir IL
            copy _il_dbgtypes.exe IL\_il_dbgtypes.exe 
            ren _il_dbgtypes.exe _il_dbgtypes.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgtypes.org /out _il_dbgtypes.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgtypes.org /out _il_dbgtypes.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgtypes.reflect.xml
    IF EXIST _il_dbgtypes.reflect.xml set ReflectionRoots=-x _il_dbgtypes.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgtypes.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgtypes.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgtypes.asm.exe %CLRTestExecutionArguments% %Host_Args%
