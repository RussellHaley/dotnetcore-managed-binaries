...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ldtoken/_il_dbgtypes _il_dbgtypes.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ldtoken/_il_dbgtypes _il_dbgtypes.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtypes.dasm.il _il_dbgtypes.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtypes.dasm.il _il_dbgtypes.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgtypes.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtypes.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgtypes.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtypes.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgtypes
    if [ ! -f _il_dbgtypes.org ]; then
        TakeLock
        if [ ! -f _il_dbgtypes.org ]; then
          mkdir IL
          cp _il_dbgtypes.exe IL/_il_dbgtypes.exe
          mv _il_dbgtypes.exe _il_dbgtypes.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtypes.org /out _il_dbgtypes.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtypes.org /out _il_dbgtypes.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgtypes.reflect.xml
  if [ -f _il_dbgtypes.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgtypes.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgtypes.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgtypes.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgtypes.asm.exe $CLRTestExecutionArguments
