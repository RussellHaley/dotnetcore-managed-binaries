...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\fp\exgen\10w5d_cs_d 10w5d_cs_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\fp\exgen\10w5d_cs_d 10w5d_cs_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=10w5d_cs_d.dasm.il 10w5d_cs_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=10w5d_cs_d.dasm.il 10w5d_cs_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=10w5d_cs_d.asm.exe  10w5d_cs_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=10w5d_cs_d.asm.exe  10w5d_cs_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=10w5d_cs_d
    if not exist "10w5d_cs_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "10w5d_cs_d.org" (
            mkdir IL
            copy 10w5d_cs_d.exe IL\10w5d_cs_d.exe 
            ren 10w5d_cs_d.exe 10w5d_cs_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 10w5d_cs_d.org /out 10w5d_cs_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 10w5d_cs_d.org /out 10w5d_cs_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 10w5d_cs_d.reflect.xml
    IF EXIST 10w5d_cs_d.reflect.xml set ReflectionRoots=-x 10w5d_cs_d.reflect.xml
    ...
    set ExePath=%LinkBin%\10w5d_cs_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 10w5d_cs_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 10w5d_cs_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
