...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/fp/exgen/1000w1d_cs_do 1000w1d_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/fp/exgen/1000w1d_cs_do 1000w1d_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=1000w1d_cs_do.dasm.il 1000w1d_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=1000w1d_cs_do.dasm.il 1000w1d_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=1000w1d_cs_do.asm.exe  1000w1d_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=1000w1d_cs_do.asm.exe  1000w1d_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=1000w1d_cs_do
    if [ ! -f 1000w1d_cs_do.org ]; then
        TakeLock
        if [ ! -f 1000w1d_cs_do.org ]; then
          mkdir IL
          cp 1000w1d_cs_do.exe IL/1000w1d_cs_do.exe
          mv 1000w1d_cs_do.exe 1000w1d_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 1000w1d_cs_do.org /out 1000w1d_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 1000w1d_cs_do.org /out 1000w1d_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 1000w1d_cs_do.reflect.xml
  if [ -f 1000w1d_cs_do.reflect.xml ]
then
    ReflectionRoots="-x 1000w1d_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/1000w1d_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 1000w1d_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 1000w1d_cs_do.asm.exe $CLRTestExecutionArguments
