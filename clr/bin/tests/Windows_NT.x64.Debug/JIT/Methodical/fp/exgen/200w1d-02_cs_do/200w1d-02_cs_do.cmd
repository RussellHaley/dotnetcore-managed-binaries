...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\fp\exgen\200w1d-02_cs_do 200w1d-02_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\fp\exgen\200w1d-02_cs_do 200w1d-02_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=200w1d-02_cs_do.dasm.il 200w1d-02_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=200w1d-02_cs_do.dasm.il 200w1d-02_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=200w1d-02_cs_do.asm.exe  200w1d-02_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=200w1d-02_cs_do.asm.exe  200w1d-02_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=200w1d-02_cs_do
    if not exist "200w1d-02_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "200w1d-02_cs_do.org" (
            mkdir IL
            copy 200w1d-02_cs_do.exe IL\200w1d-02_cs_do.exe 
            ren 200w1d-02_cs_do.exe 200w1d-02_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 200w1d-02_cs_do.org /out 200w1d-02_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 200w1d-02_cs_do.org /out 200w1d-02_cs_do.exe
...
set ExePath=200w1d-02_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 200w1d-02_cs_do.reflect.xml
    IF EXIST 200w1d-02_cs_do.reflect.xml set ReflectionRoots=-x 200w1d-02_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\200w1d-02_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 200w1d-02_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 200w1d-02_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
