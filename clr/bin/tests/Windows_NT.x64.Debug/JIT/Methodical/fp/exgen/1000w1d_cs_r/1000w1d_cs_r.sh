...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/fp/exgen/1000w1d_cs_r 1000w1d_cs_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/fp/exgen/1000w1d_cs_r 1000w1d_cs_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=1000w1d_cs_r.dasm.il 1000w1d_cs_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=1000w1d_cs_r.dasm.il 1000w1d_cs_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=1000w1d_cs_r.asm.exe  1000w1d_cs_r.dasm.il
    "$CORE_ROOT/ilasm" -output=1000w1d_cs_r.asm.exe  1000w1d_cs_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=1000w1d_cs_r
    if [ ! -f 1000w1d_cs_r.org ]; then
        TakeLock
        if [ ! -f 1000w1d_cs_r.org ]; then
          mkdir IL
          cp 1000w1d_cs_r.exe IL/1000w1d_cs_r.exe
          mv 1000w1d_cs_r.exe 1000w1d_cs_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 1000w1d_cs_r.org /out 1000w1d_cs_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 1000w1d_cs_r.org /out 1000w1d_cs_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 1000w1d_cs_r.reflect.xml
  if [ -f 1000w1d_cs_r.reflect.xml ]
then
    ReflectionRoots="-x 1000w1d_cs_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/1000w1d_cs_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 1000w1d_cs_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 1000w1d_cs_r.asm.exe $CLRTestExecutionArguments
