...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/fp/exgen/200w1d-02_cs_ro 200w1d-02_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/fp/exgen/200w1d-02_cs_ro 200w1d-02_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=200w1d-02_cs_ro.dasm.il 200w1d-02_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=200w1d-02_cs_ro.dasm.il 200w1d-02_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=200w1d-02_cs_ro.asm.exe  200w1d-02_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=200w1d-02_cs_ro.asm.exe  200w1d-02_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=200w1d-02_cs_ro
    if [ ! -f 200w1d-02_cs_ro.org ]; then
        TakeLock
        if [ ! -f 200w1d-02_cs_ro.org ]; then
          mkdir IL
          cp 200w1d-02_cs_ro.exe IL/200w1d-02_cs_ro.exe
          mv 200w1d-02_cs_ro.exe 200w1d-02_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 200w1d-02_cs_ro.org /out 200w1d-02_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 200w1d-02_cs_ro.org /out 200w1d-02_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 200w1d-02_cs_ro.reflect.xml
  if [ -f 200w1d-02_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x 200w1d-02_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/200w1d-02_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 200w1d-02_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 200w1d-02_cs_ro.asm.exe $CLRTestExecutionArguments
