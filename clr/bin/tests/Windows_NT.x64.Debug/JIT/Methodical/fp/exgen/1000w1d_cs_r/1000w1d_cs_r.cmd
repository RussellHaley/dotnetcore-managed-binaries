...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\fp\exgen\1000w1d_cs_r 1000w1d_cs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\fp\exgen\1000w1d_cs_r 1000w1d_cs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=1000w1d_cs_r.dasm.il 1000w1d_cs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=1000w1d_cs_r.dasm.il 1000w1d_cs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=1000w1d_cs_r.asm.exe  1000w1d_cs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=1000w1d_cs_r.asm.exe  1000w1d_cs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=1000w1d_cs_r
    if not exist "1000w1d_cs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "1000w1d_cs_r.org" (
            mkdir IL
            copy 1000w1d_cs_r.exe IL\1000w1d_cs_r.exe 
            ren 1000w1d_cs_r.exe 1000w1d_cs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 1000w1d_cs_r.org /out 1000w1d_cs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 1000w1d_cs_r.org /out 1000w1d_cs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 1000w1d_cs_r.reflect.xml
    IF EXIST 1000w1d_cs_r.reflect.xml set ReflectionRoots=-x 1000w1d_cs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\1000w1d_cs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 1000w1d_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 1000w1d_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
