...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\fp\exgen\200w1d-02_cs_ro 200w1d-02_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\fp\exgen\200w1d-02_cs_ro 200w1d-02_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=200w1d-02_cs_ro.dasm.il 200w1d-02_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=200w1d-02_cs_ro.dasm.il 200w1d-02_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=200w1d-02_cs_ro.asm.exe  200w1d-02_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=200w1d-02_cs_ro.asm.exe  200w1d-02_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=200w1d-02_cs_ro
    if not exist "200w1d-02_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "200w1d-02_cs_ro.org" (
            mkdir IL
            copy 200w1d-02_cs_ro.exe IL\200w1d-02_cs_ro.exe 
            ren 200w1d-02_cs_ro.exe 200w1d-02_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 200w1d-02_cs_ro.org /out 200w1d-02_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 200w1d-02_cs_ro.org /out 200w1d-02_cs_ro.exe
...
set ExePath=200w1d-02_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 200w1d-02_cs_ro.reflect.xml
    IF EXIST 200w1d-02_cs_ro.reflect.xml set ReflectionRoots=-x 200w1d-02_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\200w1d-02_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 200w1d-02_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 200w1d-02_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
