...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/switch/switch5 switch5.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/switch/switch5 switch5.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=switch5.dasm.il switch5.exe
    "$CORE_ROOT/ildasm" -raweh -out=switch5.dasm.il switch5.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=switch5.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE switch5.dasm.il
    "$CORE_ROOT/ilasm" -output=switch5.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE switch5.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=switch5
    if [ ! -f switch5.org ]; then
        TakeLock
        if [ ! -f switch5.org ]; then
          mkdir IL
          cp switch5.exe IL/switch5.exe
          mv switch5.exe switch5.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switch5.org /out switch5.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switch5.org /out switch5.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in switch5.reflect.xml
  if [ -f switch5.reflect.xml ]
then
    ReflectionRoots="-x switch5.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" switch5.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" switch5.asm.exe $CLRTestExecutionArguments
