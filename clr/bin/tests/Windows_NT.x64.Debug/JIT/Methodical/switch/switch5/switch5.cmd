...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\switch\switch5 switch5.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\switch\switch5 switch5.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=switch5.dasm.il switch5.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=switch5.dasm.il switch5.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=switch5.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE switch5.dasm.il
    %CORE_ROOT%\ilasm.exe /output=switch5.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE switch5.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=switch5
    if not exist "switch5.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "switch5.org" (
            mkdir IL
            copy switch5.exe IL\switch5.exe 
            ren switch5.exe switch5.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switch5.org /out switch5.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switch5.org /out switch5.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in switch5.reflect.xml
    IF EXIST switch5.reflect.xml set ReflectionRoots=-x switch5.reflect.xml
    ...
    set ExePath=%LinkBin%\switch5.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% switch5.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% switch5.asm.exe %CLRTestExecutionArguments% %Host_Args%
