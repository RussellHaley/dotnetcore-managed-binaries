...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/switch/switch6 switch6.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/switch/switch6 switch6.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=switch6.dasm.il switch6.exe
    "$CORE_ROOT/ildasm" -raweh -out=switch6.dasm.il switch6.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=switch6.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE switch6.dasm.il
    "$CORE_ROOT/ilasm" -output=switch6.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE switch6.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=switch6
    if [ ! -f switch6.org ]; then
        TakeLock
        if [ ! -f switch6.org ]; then
          mkdir IL
          cp switch6.exe IL/switch6.exe
          mv switch6.exe switch6.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switch6.org /out switch6.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switch6.org /out switch6.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in switch6.reflect.xml
  if [ -f switch6.reflect.xml ]
then
    ReflectionRoots="-x switch6.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" switch6.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" switch6.asm.exe $CLRTestExecutionArguments
