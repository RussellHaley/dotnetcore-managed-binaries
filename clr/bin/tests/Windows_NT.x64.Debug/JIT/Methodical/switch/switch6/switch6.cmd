...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\switch\switch6 switch6.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\switch\switch6 switch6.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=switch6.dasm.il switch6.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=switch6.dasm.il switch6.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=switch6.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE switch6.dasm.il
    %CORE_ROOT%\ilasm.exe /output=switch6.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE switch6.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=switch6
    if not exist "switch6.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "switch6.org" (
            mkdir IL
            copy switch6.exe IL\switch6.exe 
            ren switch6.exe switch6.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switch6.org /out switch6.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switch6.org /out switch6.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in switch6.reflect.xml
    IF EXIST switch6.reflect.xml set ReflectionRoots=-x switch6.reflect.xml
    ...
    set ExePath=%LinkBin%\switch6.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% switch6.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% switch6.asm.exe %CLRTestExecutionArguments% %Host_Args%
