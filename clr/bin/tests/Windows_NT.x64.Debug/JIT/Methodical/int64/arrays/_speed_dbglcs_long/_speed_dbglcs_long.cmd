...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\arrays\_speed_dbglcs_long _speed_dbglcs_long.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\arrays\_speed_dbglcs_long _speed_dbglcs_long.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbglcs_long.dasm.il _speed_dbglcs_long.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbglcs_long.dasm.il _speed_dbglcs_long.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbglcs_long.asm.exe  _speed_dbglcs_long.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbglcs_long.asm.exe  _speed_dbglcs_long.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbglcs_long
    if not exist "_speed_dbglcs_long.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbglcs_long.org" (
            mkdir IL
            copy _speed_dbglcs_long.exe IL\_speed_dbglcs_long.exe 
            ren _speed_dbglcs_long.exe _speed_dbglcs_long.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbglcs_long.org /out _speed_dbglcs_long.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbglcs_long.org /out _speed_dbglcs_long.exe
...
set ExePath=_speed_dbglcs_long.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbglcs_long.reflect.xml
    IF EXIST _speed_dbglcs_long.reflect.xml set ReflectionRoots=-x _speed_dbglcs_long.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbglcs_long.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbglcs_long.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbglcs_long.asm.exe %CLRTestExecutionArguments% %Host_Args%
