...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/arrays/_il_relhugedim _il_relhugedim.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/arrays/_il_relhugedim _il_relhugedim.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relhugedim.dasm.il _il_relhugedim.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relhugedim.dasm.il _il_relhugedim.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relhugedim.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relhugedim.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relhugedim.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relhugedim.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relhugedim
    if [ ! -f _il_relhugedim.org ]; then
        TakeLock
        if [ ! -f _il_relhugedim.org ]; then
          mkdir IL
          cp _il_relhugedim.exe IL/_il_relhugedim.exe
          mv _il_relhugedim.exe _il_relhugedim.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relhugedim.org /out _il_relhugedim.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relhugedim.org /out _il_relhugedim.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relhugedim.reflect.xml
  if [ -f _il_relhugedim.reflect.xml ]
then
    ReflectionRoots="-x _il_relhugedim.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relhugedim.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relhugedim.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relhugedim.asm.exe $CLRTestExecutionArguments
