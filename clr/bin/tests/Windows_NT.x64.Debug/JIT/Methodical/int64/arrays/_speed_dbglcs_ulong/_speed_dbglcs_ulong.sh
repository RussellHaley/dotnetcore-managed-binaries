...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/arrays/_speed_dbglcs_ulong _speed_dbglcs_ulong.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/arrays/_speed_dbglcs_ulong _speed_dbglcs_ulong.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcs_ulong.dasm.il _speed_dbglcs_ulong.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcs_ulong.dasm.il _speed_dbglcs_ulong.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbglcs_ulong.asm.exe  _speed_dbglcs_ulong.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbglcs_ulong.asm.exe  _speed_dbglcs_ulong.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbglcs_ulong
    if [ ! -f _speed_dbglcs_ulong.org ]; then
        TakeLock
        if [ ! -f _speed_dbglcs_ulong.org ]; then
          mkdir IL
          cp _speed_dbglcs_ulong.exe IL/_speed_dbglcs_ulong.exe
          mv _speed_dbglcs_ulong.exe _speed_dbglcs_ulong.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcs_ulong.org /out _speed_dbglcs_ulong.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcs_ulong.org /out _speed_dbglcs_ulong.exe
          __cgExitCode=$?
...
ExePath=_speed_dbglcs_ulong.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbglcs_ulong.reflect.xml
  if [ -f _speed_dbglcs_ulong.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbglcs_ulong.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbglcs_ulong.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbglcs_ulong.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbglcs_ulong.asm.exe $CLRTestExecutionArguments
