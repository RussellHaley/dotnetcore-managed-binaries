...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/arrays/_speed_rellcs_long _speed_rellcs_long.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/arrays/_speed_rellcs_long _speed_rellcs_long.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcs_long.dasm.il _speed_rellcs_long.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcs_long.dasm.il _speed_rellcs_long.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_rellcs_long.asm.exe  _speed_rellcs_long.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_rellcs_long.asm.exe  _speed_rellcs_long.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_rellcs_long
    if [ ! -f _speed_rellcs_long.org ]; then
        TakeLock
        if [ ! -f _speed_rellcs_long.org ]; then
          mkdir IL
          cp _speed_rellcs_long.exe IL/_speed_rellcs_long.exe
          mv _speed_rellcs_long.exe _speed_rellcs_long.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcs_long.org /out _speed_rellcs_long.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcs_long.org /out _speed_rellcs_long.exe
          __cgExitCode=$?
...
ExePath=_speed_rellcs_long.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_rellcs_long.reflect.xml
  if [ -f _speed_rellcs_long.reflect.xml ]
then
    ReflectionRoots="-x _speed_rellcs_long.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_rellcs_long.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_rellcs_long.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_rellcs_long.asm.exe $CLRTestExecutionArguments
