...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\arrays\_speed_rellcs_ulong _speed_rellcs_ulong.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\arrays\_speed_rellcs_ulong _speed_rellcs_ulong.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_rellcs_ulong.dasm.il _speed_rellcs_ulong.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_rellcs_ulong.dasm.il _speed_rellcs_ulong.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_rellcs_ulong.asm.exe  _speed_rellcs_ulong.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_rellcs_ulong.asm.exe  _speed_rellcs_ulong.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_rellcs_ulong
    if not exist "_speed_rellcs_ulong.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_rellcs_ulong.org" (
            mkdir IL
            copy _speed_rellcs_ulong.exe IL\_speed_rellcs_ulong.exe 
            ren _speed_rellcs_ulong.exe _speed_rellcs_ulong.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_rellcs_ulong.org /out _speed_rellcs_ulong.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_rellcs_ulong.org /out _speed_rellcs_ulong.exe
...
set ExePath=_speed_rellcs_ulong.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_rellcs_ulong.reflect.xml
    IF EXIST _speed_rellcs_ulong.reflect.xml set ReflectionRoots=-x _speed_rellcs_ulong.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_rellcs_ulong.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_rellcs_ulong.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_rellcs_ulong.asm.exe %CLRTestExecutionArguments% %Host_Args%
