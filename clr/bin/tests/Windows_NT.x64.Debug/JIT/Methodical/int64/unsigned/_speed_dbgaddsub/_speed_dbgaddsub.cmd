...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_speed_dbgaddsub _speed_dbgaddsub.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_speed_dbgaddsub _speed_dbgaddsub.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgaddsub.dasm.il _speed_dbgaddsub.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgaddsub.dasm.il _speed_dbgaddsub.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgaddsub.asm.exe  _speed_dbgaddsub.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgaddsub.asm.exe  _speed_dbgaddsub.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgaddsub
    if not exist "_speed_dbgaddsub.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgaddsub.org" (
            mkdir IL
            copy _speed_dbgaddsub.exe IL\_speed_dbgaddsub.exe 
            ren _speed_dbgaddsub.exe _speed_dbgaddsub.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgaddsub.org /out _speed_dbgaddsub.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgaddsub.org /out _speed_dbgaddsub.exe
...
set ExePath=_speed_dbgaddsub.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgaddsub.reflect.xml
    IF EXIST _speed_dbgaddsub.reflect.xml set ReflectionRoots=-x _speed_dbgaddsub.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgaddsub.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgaddsub.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgaddsub.asm.exe %CLRTestExecutionArguments% %Host_Args%
