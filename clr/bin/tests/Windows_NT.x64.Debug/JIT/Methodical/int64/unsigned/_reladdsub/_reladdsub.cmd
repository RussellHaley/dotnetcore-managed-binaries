...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_reladdsub _reladdsub.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_reladdsub _reladdsub.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_reladdsub.dasm.il _reladdsub.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_reladdsub.dasm.il _reladdsub.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_reladdsub.asm.exe  _reladdsub.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_reladdsub.asm.exe  _reladdsub.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_reladdsub
    if not exist "_reladdsub.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_reladdsub.org" (
            mkdir IL
            copy _reladdsub.exe IL\_reladdsub.exe 
            ren _reladdsub.exe _reladdsub.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _reladdsub.org /out _reladdsub.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _reladdsub.org /out _reladdsub.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _reladdsub.reflect.xml
    IF EXIST _reladdsub.reflect.xml set ReflectionRoots=-x _reladdsub.reflect.xml
    ...
    set ExePath=%LinkBin%\_reladdsub.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _reladdsub.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _reladdsub.asm.exe %CLRTestExecutionArguments% %Host_Args%
