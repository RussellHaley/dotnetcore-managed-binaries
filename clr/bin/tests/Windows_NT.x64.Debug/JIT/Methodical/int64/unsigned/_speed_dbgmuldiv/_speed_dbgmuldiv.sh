...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_dbgmuldiv _speed_dbgmuldiv.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_dbgmuldiv _speed_dbgmuldiv.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgmuldiv.dasm.il _speed_dbgmuldiv.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgmuldiv.dasm.il _speed_dbgmuldiv.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgmuldiv.asm.exe  _speed_dbgmuldiv.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgmuldiv.asm.exe  _speed_dbgmuldiv.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgmuldiv
    if [ ! -f _speed_dbgmuldiv.org ]; then
        TakeLock
        if [ ! -f _speed_dbgmuldiv.org ]; then
          mkdir IL
          cp _speed_dbgmuldiv.exe IL/_speed_dbgmuldiv.exe
          mv _speed_dbgmuldiv.exe _speed_dbgmuldiv.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgmuldiv.org /out _speed_dbgmuldiv.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgmuldiv.org /out _speed_dbgmuldiv.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgmuldiv.reflect.xml
  if [ -f _speed_dbgmuldiv.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgmuldiv.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgmuldiv.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgmuldiv.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgmuldiv.asm.exe $CLRTestExecutionArguments
