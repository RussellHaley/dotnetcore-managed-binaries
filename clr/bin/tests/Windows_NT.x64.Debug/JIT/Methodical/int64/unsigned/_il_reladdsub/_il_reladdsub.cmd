...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_il_reladdsub _il_reladdsub.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_il_reladdsub _il_reladdsub.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_reladdsub.dasm.il _il_reladdsub.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_reladdsub.dasm.il _il_reladdsub.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_reladdsub.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reladdsub.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_reladdsub.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reladdsub.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_reladdsub
    if not exist "_il_reladdsub.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_reladdsub.org" (
            mkdir IL
            copy _il_reladdsub.exe IL\_il_reladdsub.exe 
            ren _il_reladdsub.exe _il_reladdsub.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reladdsub.org /out _il_reladdsub.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reladdsub.org /out _il_reladdsub.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_reladdsub.reflect.xml
    IF EXIST _il_reladdsub.reflect.xml set ReflectionRoots=-x _il_reladdsub.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_reladdsub.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_reladdsub.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_reladdsub.asm.exe %CLRTestExecutionArguments% %Host_Args%
