...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_reladdsub _speed_reladdsub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_reladdsub _speed_reladdsub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_reladdsub.dasm.il _speed_reladdsub.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_reladdsub.dasm.il _speed_reladdsub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_reladdsub.asm.exe  _speed_reladdsub.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_reladdsub.asm.exe  _speed_reladdsub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_reladdsub
    if [ ! -f _speed_reladdsub.org ]; then
        TakeLock
        if [ ! -f _speed_reladdsub.org ]; then
          mkdir IL
          cp _speed_reladdsub.exe IL/_speed_reladdsub.exe
          mv _speed_reladdsub.exe _speed_reladdsub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_reladdsub.org /out _speed_reladdsub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_reladdsub.org /out _speed_reladdsub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_reladdsub.reflect.xml
  if [ -f _speed_reladdsub.reflect.xml ]
then
    ReflectionRoots="-x _speed_reladdsub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_reladdsub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_reladdsub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_reladdsub.asm.exe $CLRTestExecutionArguments
