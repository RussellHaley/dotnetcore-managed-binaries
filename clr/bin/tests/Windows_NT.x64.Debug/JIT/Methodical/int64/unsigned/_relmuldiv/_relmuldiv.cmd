...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_relmuldiv _relmuldiv.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_relmuldiv _relmuldiv.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relmuldiv.dasm.il _relmuldiv.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relmuldiv.dasm.il _relmuldiv.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relmuldiv.asm.exe  _relmuldiv.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relmuldiv.asm.exe  _relmuldiv.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relmuldiv
    if not exist "_relmuldiv.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relmuldiv.org" (
            mkdir IL
            copy _relmuldiv.exe IL\_relmuldiv.exe 
            ren _relmuldiv.exe _relmuldiv.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relmuldiv.org /out _relmuldiv.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relmuldiv.org /out _relmuldiv.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relmuldiv.reflect.xml
    IF EXIST _relmuldiv.reflect.xml set ReflectionRoots=-x _relmuldiv.reflect.xml
    ...
    set ExePath=%LinkBin%\_relmuldiv.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relmuldiv.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relmuldiv.asm.exe %CLRTestExecutionArguments% %Host_Args%
