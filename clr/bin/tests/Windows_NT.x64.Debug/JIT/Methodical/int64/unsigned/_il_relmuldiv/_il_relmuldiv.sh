...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_relmuldiv _il_relmuldiv.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_relmuldiv _il_relmuldiv.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relmuldiv.dasm.il _il_relmuldiv.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relmuldiv.dasm.il _il_relmuldiv.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relmuldiv.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relmuldiv.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relmuldiv.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relmuldiv.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relmuldiv
    if [ ! -f _il_relmuldiv.org ]; then
        TakeLock
        if [ ! -f _il_relmuldiv.org ]; then
          mkdir IL
          cp _il_relmuldiv.exe IL/_il_relmuldiv.exe
          mv _il_relmuldiv.exe _il_relmuldiv.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relmuldiv.org /out _il_relmuldiv.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relmuldiv.org /out _il_relmuldiv.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relmuldiv.reflect.xml
  if [ -f _il_relmuldiv.reflect.xml ]
then
    ReflectionRoots="-x _il_relmuldiv.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relmuldiv.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relmuldiv.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relmuldiv.asm.exe $CLRTestExecutionArguments
