...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_reladdsub _reladdsub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_reladdsub _reladdsub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_reladdsub.dasm.il _reladdsub.exe
    "$CORE_ROOT/ildasm" -raweh -out=_reladdsub.dasm.il _reladdsub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_reladdsub.asm.exe  _reladdsub.dasm.il
    "$CORE_ROOT/ilasm" -output=_reladdsub.asm.exe  _reladdsub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_reladdsub
    if [ ! -f _reladdsub.org ]; then
        TakeLock
        if [ ! -f _reladdsub.org ]; then
          mkdir IL
          cp _reladdsub.exe IL/_reladdsub.exe
          mv _reladdsub.exe _reladdsub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reladdsub.org /out _reladdsub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reladdsub.org /out _reladdsub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _reladdsub.reflect.xml
  if [ -f _reladdsub.reflect.xml ]
then
    ReflectionRoots="-x _reladdsub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_reladdsub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _reladdsub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _reladdsub.asm.exe $CLRTestExecutionArguments
