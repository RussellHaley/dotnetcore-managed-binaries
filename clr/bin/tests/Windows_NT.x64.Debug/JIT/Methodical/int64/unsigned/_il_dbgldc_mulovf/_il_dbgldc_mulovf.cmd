...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_il_dbgldc_mulovf _il_dbgldc_mulovf.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_il_dbgldc_mulovf _il_dbgldc_mulovf.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgldc_mulovf.dasm.il _il_dbgldc_mulovf.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgldc_mulovf.dasm.il _il_dbgldc_mulovf.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgldc_mulovf.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgldc_mulovf.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgldc_mulovf.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgldc_mulovf.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgldc_mulovf
    if not exist "_il_dbgldc_mulovf.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgldc_mulovf.org" (
            mkdir IL
            copy _il_dbgldc_mulovf.exe IL\_il_dbgldc_mulovf.exe 
            ren _il_dbgldc_mulovf.exe _il_dbgldc_mulovf.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgldc_mulovf.org /out _il_dbgldc_mulovf.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgldc_mulovf.org /out _il_dbgldc_mulovf.exe
...
set ExePath=_il_dbgldc_mulovf.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgldc_mulovf.reflect.xml
    IF EXIST _il_dbgldc_mulovf.reflect.xml set ReflectionRoots=-x _il_dbgldc_mulovf.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgldc_mulovf.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgldc_mulovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgldc_mulovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
