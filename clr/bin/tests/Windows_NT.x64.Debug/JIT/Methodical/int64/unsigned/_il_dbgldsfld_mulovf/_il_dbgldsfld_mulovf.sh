...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_dbgldsfld_mulovf _il_dbgldsfld_mulovf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_dbgldsfld_mulovf _il_dbgldsfld_mulovf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgldsfld_mulovf.dasm.il _il_dbgldsfld_mulovf.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgldsfld_mulovf.dasm.il _il_dbgldsfld_mulovf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgldsfld_mulovf.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgldsfld_mulovf.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgldsfld_mulovf.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgldsfld_mulovf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgldsfld_mulovf
    if [ ! -f _il_dbgldsfld_mulovf.org ]; then
        TakeLock
        if [ ! -f _il_dbgldsfld_mulovf.org ]; then
          mkdir IL
          cp _il_dbgldsfld_mulovf.exe IL/_il_dbgldsfld_mulovf.exe
          mv _il_dbgldsfld_mulovf.exe _il_dbgldsfld_mulovf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgldsfld_mulovf.org /out _il_dbgldsfld_mulovf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgldsfld_mulovf.org /out _il_dbgldsfld_mulovf.exe
          __cgExitCode=$?
...
ExePath=_il_dbgldsfld_mulovf.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgldsfld_mulovf.reflect.xml
  if [ -f _il_dbgldsfld_mulovf.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgldsfld_mulovf.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgldsfld_mulovf.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgldsfld_mulovf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgldsfld_mulovf.asm.exe $CLRTestExecutionArguments
