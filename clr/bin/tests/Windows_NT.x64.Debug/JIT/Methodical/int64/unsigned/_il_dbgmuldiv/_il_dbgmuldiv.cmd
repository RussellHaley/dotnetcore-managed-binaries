...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_il_dbgmuldiv _il_dbgmuldiv.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_il_dbgmuldiv _il_dbgmuldiv.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgmuldiv.dasm.il _il_dbgmuldiv.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgmuldiv.dasm.il _il_dbgmuldiv.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgmuldiv.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgmuldiv.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgmuldiv.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgmuldiv.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgmuldiv
    if not exist "_il_dbgmuldiv.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgmuldiv.org" (
            mkdir IL
            copy _il_dbgmuldiv.exe IL\_il_dbgmuldiv.exe 
            ren _il_dbgmuldiv.exe _il_dbgmuldiv.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgmuldiv.org /out _il_dbgmuldiv.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgmuldiv.org /out _il_dbgmuldiv.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgmuldiv.reflect.xml
    IF EXIST _il_dbgmuldiv.reflect.xml set ReflectionRoots=-x _il_dbgmuldiv.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgmuldiv.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgmuldiv.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgmuldiv.asm.exe %CLRTestExecutionArguments% %Host_Args%
