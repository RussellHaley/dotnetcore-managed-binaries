...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_relldc_mulovf _speed_relldc_mulovf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_relldc_mulovf _speed_relldc_mulovf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relldc_mulovf.dasm.il _speed_relldc_mulovf.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relldc_mulovf.dasm.il _speed_relldc_mulovf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relldc_mulovf.asm.exe  _speed_relldc_mulovf.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relldc_mulovf.asm.exe  _speed_relldc_mulovf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relldc_mulovf
    if [ ! -f _speed_relldc_mulovf.org ]; then
        TakeLock
        if [ ! -f _speed_relldc_mulovf.org ]; then
          mkdir IL
          cp _speed_relldc_mulovf.exe IL/_speed_relldc_mulovf.exe
          mv _speed_relldc_mulovf.exe _speed_relldc_mulovf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relldc_mulovf.org /out _speed_relldc_mulovf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relldc_mulovf.org /out _speed_relldc_mulovf.exe
          __cgExitCode=$?
...
ExePath=_speed_relldc_mulovf.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relldc_mulovf.reflect.xml
  if [ -f _speed_relldc_mulovf.reflect.xml ]
then
    ReflectionRoots="-x _speed_relldc_mulovf.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relldc_mulovf.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relldc_mulovf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relldc_mulovf.asm.exe $CLRTestExecutionArguments
