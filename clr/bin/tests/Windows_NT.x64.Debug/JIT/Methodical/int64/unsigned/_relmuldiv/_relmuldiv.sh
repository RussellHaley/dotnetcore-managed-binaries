...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_relmuldiv _relmuldiv.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_relmuldiv _relmuldiv.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relmuldiv.dasm.il _relmuldiv.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relmuldiv.dasm.il _relmuldiv.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relmuldiv.asm.exe  _relmuldiv.dasm.il
    "$CORE_ROOT/ilasm" -output=_relmuldiv.asm.exe  _relmuldiv.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relmuldiv
    if [ ! -f _relmuldiv.org ]; then
        TakeLock
        if [ ! -f _relmuldiv.org ]; then
          mkdir IL
          cp _relmuldiv.exe IL/_relmuldiv.exe
          mv _relmuldiv.exe _relmuldiv.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relmuldiv.org /out _relmuldiv.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relmuldiv.org /out _relmuldiv.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relmuldiv.reflect.xml
  if [ -f _relmuldiv.reflect.xml ]
then
    ReflectionRoots="-x _relmuldiv.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relmuldiv.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relmuldiv.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relmuldiv.asm.exe $CLRTestExecutionArguments
