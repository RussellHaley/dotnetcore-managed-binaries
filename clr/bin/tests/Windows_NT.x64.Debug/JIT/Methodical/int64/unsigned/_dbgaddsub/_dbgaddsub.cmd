...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_dbgaddsub _dbgaddsub.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_dbgaddsub _dbgaddsub.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgaddsub.dasm.il _dbgaddsub.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgaddsub.dasm.il _dbgaddsub.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgaddsub.asm.exe  _dbgaddsub.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgaddsub.asm.exe  _dbgaddsub.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgaddsub
    if not exist "_dbgaddsub.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgaddsub.org" (
            mkdir IL
            copy _dbgaddsub.exe IL\_dbgaddsub.exe 
            ren _dbgaddsub.exe _dbgaddsub.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgaddsub.org /out _dbgaddsub.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgaddsub.org /out _dbgaddsub.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgaddsub.reflect.xml
    IF EXIST _dbgaddsub.reflect.xml set ReflectionRoots=-x _dbgaddsub.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgaddsub.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgaddsub.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgaddsub.asm.exe %CLRTestExecutionArguments% %Host_Args%
