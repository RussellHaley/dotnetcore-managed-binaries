...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_dbgaddsub _speed_dbgaddsub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_dbgaddsub _speed_dbgaddsub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgaddsub.dasm.il _speed_dbgaddsub.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgaddsub.dasm.il _speed_dbgaddsub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgaddsub.asm.exe  _speed_dbgaddsub.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgaddsub.asm.exe  _speed_dbgaddsub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgaddsub
    if [ ! -f _speed_dbgaddsub.org ]; then
        TakeLock
        if [ ! -f _speed_dbgaddsub.org ]; then
          mkdir IL
          cp _speed_dbgaddsub.exe IL/_speed_dbgaddsub.exe
          mv _speed_dbgaddsub.exe _speed_dbgaddsub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgaddsub.org /out _speed_dbgaddsub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgaddsub.org /out _speed_dbgaddsub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgaddsub.reflect.xml
  if [ -f _speed_dbgaddsub.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgaddsub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgaddsub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgaddsub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgaddsub.asm.exe $CLRTestExecutionArguments
