...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_dbgaddsub _dbgaddsub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_dbgaddsub _dbgaddsub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgaddsub.dasm.il _dbgaddsub.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgaddsub.dasm.il _dbgaddsub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgaddsub.asm.exe  _dbgaddsub.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgaddsub.asm.exe  _dbgaddsub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgaddsub
    if [ ! -f _dbgaddsub.org ]; then
        TakeLock
        if [ ! -f _dbgaddsub.org ]; then
          mkdir IL
          cp _dbgaddsub.exe IL/_dbgaddsub.exe
          mv _dbgaddsub.exe _dbgaddsub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgaddsub.org /out _dbgaddsub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgaddsub.org /out _dbgaddsub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgaddsub.reflect.xml
  if [ -f _dbgaddsub.reflect.xml ]
then
    ReflectionRoots="-x _dbgaddsub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgaddsub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgaddsub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgaddsub.asm.exe $CLRTestExecutionArguments
