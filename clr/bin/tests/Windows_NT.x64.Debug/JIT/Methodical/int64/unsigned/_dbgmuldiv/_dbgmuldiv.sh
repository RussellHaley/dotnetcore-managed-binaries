...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_dbgmuldiv _dbgmuldiv.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_dbgmuldiv _dbgmuldiv.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgmuldiv.dasm.il _dbgmuldiv.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgmuldiv.dasm.il _dbgmuldiv.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgmuldiv.asm.exe  _dbgmuldiv.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgmuldiv.asm.exe  _dbgmuldiv.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgmuldiv
    if [ ! -f _dbgmuldiv.org ]; then
        TakeLock
        if [ ! -f _dbgmuldiv.org ]; then
          mkdir IL
          cp _dbgmuldiv.exe IL/_dbgmuldiv.exe
          mv _dbgmuldiv.exe _dbgmuldiv.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgmuldiv.org /out _dbgmuldiv.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgmuldiv.org /out _dbgmuldiv.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgmuldiv.reflect.xml
  if [ -f _dbgmuldiv.reflect.xml ]
then
    ReflectionRoots="-x _dbgmuldiv.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgmuldiv.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgmuldiv.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgmuldiv.asm.exe $CLRTestExecutionArguments
