...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_dbgmuldiv _dbgmuldiv.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_dbgmuldiv _dbgmuldiv.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgmuldiv.dasm.il _dbgmuldiv.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgmuldiv.dasm.il _dbgmuldiv.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgmuldiv.asm.exe  _dbgmuldiv.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgmuldiv.asm.exe  _dbgmuldiv.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgmuldiv
    if not exist "_dbgmuldiv.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgmuldiv.org" (
            mkdir IL
            copy _dbgmuldiv.exe IL\_dbgmuldiv.exe 
            ren _dbgmuldiv.exe _dbgmuldiv.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgmuldiv.org /out _dbgmuldiv.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgmuldiv.org /out _dbgmuldiv.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgmuldiv.reflect.xml
    IF EXIST _dbgmuldiv.reflect.xml set ReflectionRoots=-x _dbgmuldiv.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgmuldiv.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgmuldiv.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgmuldiv.asm.exe %CLRTestExecutionArguments% %Host_Args%
