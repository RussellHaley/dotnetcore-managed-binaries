...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_relmuldiv _speed_relmuldiv.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_speed_relmuldiv _speed_relmuldiv.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relmuldiv.dasm.il _speed_relmuldiv.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relmuldiv.dasm.il _speed_relmuldiv.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relmuldiv.asm.exe  _speed_relmuldiv.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relmuldiv.asm.exe  _speed_relmuldiv.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relmuldiv
    if [ ! -f _speed_relmuldiv.org ]; then
        TakeLock
        if [ ! -f _speed_relmuldiv.org ]; then
          mkdir IL
          cp _speed_relmuldiv.exe IL/_speed_relmuldiv.exe
          mv _speed_relmuldiv.exe _speed_relmuldiv.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relmuldiv.org /out _speed_relmuldiv.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relmuldiv.org /out _speed_relmuldiv.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relmuldiv.reflect.xml
  if [ -f _speed_relmuldiv.reflect.xml ]
then
    ReflectionRoots="-x _speed_relmuldiv.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relmuldiv.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relmuldiv.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relmuldiv.asm.exe $CLRTestExecutionArguments
