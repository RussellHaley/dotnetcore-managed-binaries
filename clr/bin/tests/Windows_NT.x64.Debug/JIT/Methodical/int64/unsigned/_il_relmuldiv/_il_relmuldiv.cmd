...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_il_relmuldiv _il_relmuldiv.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_il_relmuldiv _il_relmuldiv.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relmuldiv.dasm.il _il_relmuldiv.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relmuldiv.dasm.il _il_relmuldiv.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relmuldiv.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relmuldiv.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relmuldiv.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relmuldiv.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relmuldiv
    if not exist "_il_relmuldiv.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relmuldiv.org" (
            mkdir IL
            copy _il_relmuldiv.exe IL\_il_relmuldiv.exe 
            ren _il_relmuldiv.exe _il_relmuldiv.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relmuldiv.org /out _il_relmuldiv.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relmuldiv.org /out _il_relmuldiv.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relmuldiv.reflect.xml
    IF EXIST _il_relmuldiv.reflect.xml set ReflectionRoots=-x _il_relmuldiv.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relmuldiv.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relmuldiv.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relmuldiv.asm.exe %CLRTestExecutionArguments% %Host_Args%
