...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_dbgmuldiv _il_dbgmuldiv.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_dbgmuldiv _il_dbgmuldiv.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgmuldiv.dasm.il _il_dbgmuldiv.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgmuldiv.dasm.il _il_dbgmuldiv.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgmuldiv.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgmuldiv.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgmuldiv.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgmuldiv.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgmuldiv
    if [ ! -f _il_dbgmuldiv.org ]; then
        TakeLock
        if [ ! -f _il_dbgmuldiv.org ]; then
          mkdir IL
          cp _il_dbgmuldiv.exe IL/_il_dbgmuldiv.exe
          mv _il_dbgmuldiv.exe _il_dbgmuldiv.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgmuldiv.org /out _il_dbgmuldiv.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgmuldiv.org /out _il_dbgmuldiv.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgmuldiv.reflect.xml
  if [ -f _il_dbgmuldiv.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgmuldiv.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgmuldiv.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgmuldiv.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgmuldiv.asm.exe $CLRTestExecutionArguments
