...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_dbgldc_mulovf _dbgldc_mulovf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_dbgldc_mulovf _dbgldc_mulovf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgldc_mulovf.dasm.il _dbgldc_mulovf.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgldc_mulovf.dasm.il _dbgldc_mulovf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgldc_mulovf.asm.exe  _dbgldc_mulovf.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgldc_mulovf.asm.exe  _dbgldc_mulovf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgldc_mulovf
    if [ ! -f _dbgldc_mulovf.org ]; then
        TakeLock
        if [ ! -f _dbgldc_mulovf.org ]; then
          mkdir IL
          cp _dbgldc_mulovf.exe IL/_dbgldc_mulovf.exe
          mv _dbgldc_mulovf.exe _dbgldc_mulovf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgldc_mulovf.org /out _dbgldc_mulovf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgldc_mulovf.org /out _dbgldc_mulovf.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgldc_mulovf.reflect.xml
  if [ -f _dbgldc_mulovf.reflect.xml ]
then
    ReflectionRoots="-x _dbgldc_mulovf.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgldc_mulovf.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgldc_mulovf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgldc_mulovf.asm.exe $CLRTestExecutionArguments
