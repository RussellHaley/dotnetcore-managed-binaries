...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_speed_relldc_mulovf _speed_relldc_mulovf.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\unsigned\_speed_relldc_mulovf _speed_relldc_mulovf.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relldc_mulovf.dasm.il _speed_relldc_mulovf.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relldc_mulovf.dasm.il _speed_relldc_mulovf.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_relldc_mulovf.asm.exe  _speed_relldc_mulovf.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_relldc_mulovf.asm.exe  _speed_relldc_mulovf.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_relldc_mulovf
    if not exist "_speed_relldc_mulovf.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_relldc_mulovf.org" (
            mkdir IL
            copy _speed_relldc_mulovf.exe IL\_speed_relldc_mulovf.exe 
            ren _speed_relldc_mulovf.exe _speed_relldc_mulovf.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relldc_mulovf.org /out _speed_relldc_mulovf.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relldc_mulovf.org /out _speed_relldc_mulovf.exe
...
set ExePath=_speed_relldc_mulovf.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_relldc_mulovf.reflect.xml
    IF EXIST _speed_relldc_mulovf.reflect.xml set ReflectionRoots=-x _speed_relldc_mulovf.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_relldc_mulovf.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_relldc_mulovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_relldc_mulovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
