...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_reladdsub _il_reladdsub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_reladdsub _il_reladdsub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_reladdsub.dasm.il _il_reladdsub.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_reladdsub.dasm.il _il_reladdsub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_reladdsub.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reladdsub.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_reladdsub.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reladdsub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_reladdsub
    if [ ! -f _il_reladdsub.org ]; then
        TakeLock
        if [ ! -f _il_reladdsub.org ]; then
          mkdir IL
          cp _il_reladdsub.exe IL/_il_reladdsub.exe
          mv _il_reladdsub.exe _il_reladdsub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reladdsub.org /out _il_reladdsub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reladdsub.org /out _il_reladdsub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_reladdsub.reflect.xml
  if [ -f _il_reladdsub.reflect.xml ]
then
    ReflectionRoots="-x _il_reladdsub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_reladdsub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_reladdsub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_reladdsub.asm.exe $CLRTestExecutionArguments
