...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_dbgaddsub _il_dbgaddsub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/unsigned/_il_dbgaddsub _il_dbgaddsub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgaddsub.dasm.il _il_dbgaddsub.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgaddsub.dasm.il _il_dbgaddsub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgaddsub.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgaddsub.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgaddsub.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgaddsub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgaddsub
    if [ ! -f _il_dbgaddsub.org ]; then
        TakeLock
        if [ ! -f _il_dbgaddsub.org ]; then
          mkdir IL
          cp _il_dbgaddsub.exe IL/_il_dbgaddsub.exe
          mv _il_dbgaddsub.exe _il_dbgaddsub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgaddsub.org /out _il_dbgaddsub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgaddsub.org /out _il_dbgaddsub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgaddsub.reflect.xml
  if [ -f _il_dbgaddsub.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgaddsub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgaddsub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgaddsub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgaddsub.asm.exe $CLRTestExecutionArguments
