...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_speed_dbgsuperlong _speed_dbgsuperlong.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_speed_dbgsuperlong _speed_dbgsuperlong.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgsuperlong.dasm.il _speed_dbgsuperlong.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgsuperlong.dasm.il _speed_dbgsuperlong.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgsuperlong.asm.exe  _speed_dbgsuperlong.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgsuperlong.asm.exe  _speed_dbgsuperlong.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgsuperlong
    if not exist "_speed_dbgsuperlong.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgsuperlong.org" (
            mkdir IL
            copy _speed_dbgsuperlong.exe IL\_speed_dbgsuperlong.exe 
            ren _speed_dbgsuperlong.exe _speed_dbgsuperlong.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgsuperlong.org /out _speed_dbgsuperlong.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgsuperlong.org /out _speed_dbgsuperlong.exe
...
set ExePath=_speed_dbgsuperlong.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgsuperlong.reflect.xml
    IF EXIST _speed_dbgsuperlong.reflect.xml set ReflectionRoots=-x _speed_dbgsuperlong.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgsuperlong.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
