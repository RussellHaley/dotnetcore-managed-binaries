...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_il_dbgsuperlong _il_dbgsuperlong.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_il_dbgsuperlong _il_dbgsuperlong.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgsuperlong.dasm.il _il_dbgsuperlong.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgsuperlong.dasm.il _il_dbgsuperlong.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgsuperlong.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgsuperlong.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgsuperlong.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgsuperlong.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgsuperlong
    if [ ! -f _il_dbgsuperlong.org ]; then
        TakeLock
        if [ ! -f _il_dbgsuperlong.org ]; then
          mkdir IL
          cp _il_dbgsuperlong.exe IL/_il_dbgsuperlong.exe
          mv _il_dbgsuperlong.exe _il_dbgsuperlong.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgsuperlong.org /out _il_dbgsuperlong.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgsuperlong.org /out _il_dbgsuperlong.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgsuperlong.reflect.xml
  if [ -f _il_dbgsuperlong.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgsuperlong.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgsuperlong.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgsuperlong.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgsuperlong.asm.exe $CLRTestExecutionArguments
