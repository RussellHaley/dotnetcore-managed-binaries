...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_speed_relsuperlong _speed_relsuperlong.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_speed_relsuperlong _speed_relsuperlong.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relsuperlong.dasm.il _speed_relsuperlong.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relsuperlong.dasm.il _speed_relsuperlong.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relsuperlong.asm.exe  _speed_relsuperlong.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relsuperlong.asm.exe  _speed_relsuperlong.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relsuperlong
    if [ ! -f _speed_relsuperlong.org ]; then
        TakeLock
        if [ ! -f _speed_relsuperlong.org ]; then
          mkdir IL
          cp _speed_relsuperlong.exe IL/_speed_relsuperlong.exe
          mv _speed_relsuperlong.exe _speed_relsuperlong.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relsuperlong.org /out _speed_relsuperlong.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relsuperlong.org /out _speed_relsuperlong.exe
          __cgExitCode=$?
...
ExePath=_speed_relsuperlong.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relsuperlong.reflect.xml
  if [ -f _speed_relsuperlong.reflect.xml ]
then
    ReflectionRoots="-x _speed_relsuperlong.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relsuperlong.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relsuperlong.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relsuperlong.asm.exe $CLRTestExecutionArguments
