...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_speed_dbgsuperlong _speed_dbgsuperlong.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_speed_dbgsuperlong _speed_dbgsuperlong.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgsuperlong.dasm.il _speed_dbgsuperlong.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgsuperlong.dasm.il _speed_dbgsuperlong.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgsuperlong.asm.exe  _speed_dbgsuperlong.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgsuperlong.asm.exe  _speed_dbgsuperlong.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgsuperlong
    if [ ! -f _speed_dbgsuperlong.org ]; then
        TakeLock
        if [ ! -f _speed_dbgsuperlong.org ]; then
          mkdir IL
          cp _speed_dbgsuperlong.exe IL/_speed_dbgsuperlong.exe
          mv _speed_dbgsuperlong.exe _speed_dbgsuperlong.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgsuperlong.org /out _speed_dbgsuperlong.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgsuperlong.org /out _speed_dbgsuperlong.exe
          __cgExitCode=$?
...
ExePath=_speed_dbgsuperlong.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgsuperlong.reflect.xml
  if [ -f _speed_dbgsuperlong.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgsuperlong.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgsuperlong.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgsuperlong.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgsuperlong.asm.exe $CLRTestExecutionArguments
