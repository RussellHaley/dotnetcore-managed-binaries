...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_il_relsuperlong _il_relsuperlong.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_il_relsuperlong _il_relsuperlong.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relsuperlong.dasm.il _il_relsuperlong.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relsuperlong.dasm.il _il_relsuperlong.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relsuperlong.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relsuperlong.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relsuperlong.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relsuperlong.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relsuperlong
    if not exist "_il_relsuperlong.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relsuperlong.org" (
            mkdir IL
            copy _il_relsuperlong.exe IL\_il_relsuperlong.exe 
            ren _il_relsuperlong.exe _il_relsuperlong.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relsuperlong.org /out _il_relsuperlong.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relsuperlong.org /out _il_relsuperlong.exe
...
set ExePath=_il_relsuperlong.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relsuperlong.reflect.xml
    IF EXIST _il_relsuperlong.reflect.xml set ReflectionRoots=-x _il_relsuperlong.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relsuperlong.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
