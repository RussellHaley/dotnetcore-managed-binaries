...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_il_relsuperlong _il_relsuperlong.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_il_relsuperlong _il_relsuperlong.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relsuperlong.dasm.il _il_relsuperlong.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relsuperlong.dasm.il _il_relsuperlong.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relsuperlong.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relsuperlong.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relsuperlong.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relsuperlong.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relsuperlong
    if [ ! -f _il_relsuperlong.org ]; then
        TakeLock
        if [ ! -f _il_relsuperlong.org ]; then
          mkdir IL
          cp _il_relsuperlong.exe IL/_il_relsuperlong.exe
          mv _il_relsuperlong.exe _il_relsuperlong.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relsuperlong.org /out _il_relsuperlong.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relsuperlong.org /out _il_relsuperlong.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relsuperlong.reflect.xml
  if [ -f _il_relsuperlong.reflect.xml ]
then
    ReflectionRoots="-x _il_relsuperlong.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relsuperlong.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relsuperlong.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relsuperlong.asm.exe $CLRTestExecutionArguments
