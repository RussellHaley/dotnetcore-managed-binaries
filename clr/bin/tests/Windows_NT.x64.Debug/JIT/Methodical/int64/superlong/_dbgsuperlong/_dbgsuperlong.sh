...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_dbgsuperlong _dbgsuperlong.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_dbgsuperlong _dbgsuperlong.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgsuperlong.dasm.il _dbgsuperlong.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgsuperlong.dasm.il _dbgsuperlong.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgsuperlong.asm.exe  _dbgsuperlong.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgsuperlong.asm.exe  _dbgsuperlong.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgsuperlong
    if [ ! -f _dbgsuperlong.org ]; then
        TakeLock
        if [ ! -f _dbgsuperlong.org ]; then
          mkdir IL
          cp _dbgsuperlong.exe IL/_dbgsuperlong.exe
          mv _dbgsuperlong.exe _dbgsuperlong.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgsuperlong.org /out _dbgsuperlong.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgsuperlong.org /out _dbgsuperlong.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgsuperlong.reflect.xml
  if [ -f _dbgsuperlong.reflect.xml ]
then
    ReflectionRoots="-x _dbgsuperlong.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgsuperlong.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgsuperlong.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgsuperlong.asm.exe $CLRTestExecutionArguments
