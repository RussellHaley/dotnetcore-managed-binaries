...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_il_dbgsuperlong _il_dbgsuperlong.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_il_dbgsuperlong _il_dbgsuperlong.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgsuperlong.dasm.il _il_dbgsuperlong.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgsuperlong.dasm.il _il_dbgsuperlong.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgsuperlong.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgsuperlong.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgsuperlong.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgsuperlong.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgsuperlong
    if not exist "_il_dbgsuperlong.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgsuperlong.org" (
            mkdir IL
            copy _il_dbgsuperlong.exe IL\_il_dbgsuperlong.exe 
            ren _il_dbgsuperlong.exe _il_dbgsuperlong.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgsuperlong.org /out _il_dbgsuperlong.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgsuperlong.org /out _il_dbgsuperlong.exe
...
set ExePath=_il_dbgsuperlong.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgsuperlong.reflect.xml
    IF EXIST _il_dbgsuperlong.reflect.xml set ReflectionRoots=-x _il_dbgsuperlong.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgsuperlong.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
