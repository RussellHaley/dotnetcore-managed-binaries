...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_relsuperlong _relsuperlong.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_relsuperlong _relsuperlong.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relsuperlong.dasm.il _relsuperlong.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relsuperlong.dasm.il _relsuperlong.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relsuperlong.asm.exe  _relsuperlong.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relsuperlong.asm.exe  _relsuperlong.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relsuperlong
    if not exist "_relsuperlong.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relsuperlong.org" (
            mkdir IL
            copy _relsuperlong.exe IL\_relsuperlong.exe 
            ren _relsuperlong.exe _relsuperlong.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relsuperlong.org /out _relsuperlong.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relsuperlong.org /out _relsuperlong.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relsuperlong.reflect.xml
    IF EXIST _relsuperlong.reflect.xml set ReflectionRoots=-x _relsuperlong.reflect.xml
    ...
    set ExePath=%LinkBin%\_relsuperlong.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
