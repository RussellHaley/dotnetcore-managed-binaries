...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_dbgsuperlong _dbgsuperlong.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\superlong\_dbgsuperlong _dbgsuperlong.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgsuperlong.dasm.il _dbgsuperlong.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgsuperlong.dasm.il _dbgsuperlong.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgsuperlong.asm.exe  _dbgsuperlong.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgsuperlong.asm.exe  _dbgsuperlong.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgsuperlong
    if not exist "_dbgsuperlong.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgsuperlong.org" (
            mkdir IL
            copy _dbgsuperlong.exe IL\_dbgsuperlong.exe 
            ren _dbgsuperlong.exe _dbgsuperlong.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgsuperlong.org /out _dbgsuperlong.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgsuperlong.org /out _dbgsuperlong.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgsuperlong.reflect.xml
    IF EXIST _dbgsuperlong.reflect.xml set ReflectionRoots=-x _dbgsuperlong.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgsuperlong.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgsuperlong.asm.exe %CLRTestExecutionArguments% %Host_Args%
