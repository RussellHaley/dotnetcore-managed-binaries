...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_relsuperlong _relsuperlong.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/superlong/_relsuperlong _relsuperlong.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relsuperlong.dasm.il _relsuperlong.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relsuperlong.dasm.il _relsuperlong.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relsuperlong.asm.exe  _relsuperlong.dasm.il
    "$CORE_ROOT/ilasm" -output=_relsuperlong.asm.exe  _relsuperlong.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relsuperlong
    if [ ! -f _relsuperlong.org ]; then
        TakeLock
        if [ ! -f _relsuperlong.org ]; then
          mkdir IL
          cp _relsuperlong.exe IL/_relsuperlong.exe
          mv _relsuperlong.exe _relsuperlong.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relsuperlong.org /out _relsuperlong.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relsuperlong.org /out _relsuperlong.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relsuperlong.reflect.xml
  if [ -f _relsuperlong.reflect.xml ]
then
    ReflectionRoots="-x _relsuperlong.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relsuperlong.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relsuperlong.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relsuperlong.asm.exe $CLRTestExecutionArguments
