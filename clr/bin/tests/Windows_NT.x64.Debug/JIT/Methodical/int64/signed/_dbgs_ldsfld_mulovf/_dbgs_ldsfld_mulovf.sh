...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/signed/_dbgs_ldsfld_mulovf _dbgs_ldsfld_mulovf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/signed/_dbgs_ldsfld_mulovf _dbgs_ldsfld_mulovf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgs_ldsfld_mulovf.dasm.il _dbgs_ldsfld_mulovf.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgs_ldsfld_mulovf.dasm.il _dbgs_ldsfld_mulovf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgs_ldsfld_mulovf.asm.exe  _dbgs_ldsfld_mulovf.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgs_ldsfld_mulovf.asm.exe  _dbgs_ldsfld_mulovf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgs_ldsfld_mulovf
    if [ ! -f _dbgs_ldsfld_mulovf.org ]; then
        TakeLock
        if [ ! -f _dbgs_ldsfld_mulovf.org ]; then
          mkdir IL
          cp _dbgs_ldsfld_mulovf.exe IL/_dbgs_ldsfld_mulovf.exe
          mv _dbgs_ldsfld_mulovf.exe _dbgs_ldsfld_mulovf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgs_ldsfld_mulovf.org /out _dbgs_ldsfld_mulovf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgs_ldsfld_mulovf.org /out _dbgs_ldsfld_mulovf.exe
          __cgExitCode=$?
...
ExePath=_dbgs_ldsfld_mulovf.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgs_ldsfld_mulovf.reflect.xml
  if [ -f _dbgs_ldsfld_mulovf.reflect.xml ]
then
    ReflectionRoots="-x _dbgs_ldsfld_mulovf.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgs_ldsfld_mulovf.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgs_ldsfld_mulovf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgs_ldsfld_mulovf.asm.exe $CLRTestExecutionArguments
