...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\signed\_speed_dbgs_ldc_div _speed_dbgs_ldc_div.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\signed\_speed_dbgs_ldc_div _speed_dbgs_ldc_div.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgs_ldc_div.dasm.il _speed_dbgs_ldc_div.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgs_ldc_div.dasm.il _speed_dbgs_ldc_div.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgs_ldc_div.asm.exe  _speed_dbgs_ldc_div.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgs_ldc_div.asm.exe  _speed_dbgs_ldc_div.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgs_ldc_div
    if not exist "_speed_dbgs_ldc_div.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgs_ldc_div.org" (
            mkdir IL
            copy _speed_dbgs_ldc_div.exe IL\_speed_dbgs_ldc_div.exe 
            ren _speed_dbgs_ldc_div.exe _speed_dbgs_ldc_div.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgs_ldc_div.org /out _speed_dbgs_ldc_div.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgs_ldc_div.org /out _speed_dbgs_ldc_div.exe
...
set ExePath=_speed_dbgs_ldc_div.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgs_ldc_div.reflect.xml
    IF EXIST _speed_dbgs_ldc_div.reflect.xml set ReflectionRoots=-x _speed_dbgs_ldc_div.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgs_ldc_div.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgs_ldc_div.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgs_ldc_div.asm.exe %CLRTestExecutionArguments% %Host_Args%
