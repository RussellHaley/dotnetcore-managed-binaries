...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/signed/_il_dbgs_muldiv _il_dbgs_muldiv.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/signed/_il_dbgs_muldiv _il_dbgs_muldiv.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgs_muldiv.dasm.il _il_dbgs_muldiv.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgs_muldiv.dasm.il _il_dbgs_muldiv.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgs_muldiv.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgs_muldiv.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgs_muldiv.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgs_muldiv.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgs_muldiv
    if [ ! -f _il_dbgs_muldiv.org ]; then
        TakeLock
        if [ ! -f _il_dbgs_muldiv.org ]; then
          mkdir IL
          cp _il_dbgs_muldiv.exe IL/_il_dbgs_muldiv.exe
          mv _il_dbgs_muldiv.exe _il_dbgs_muldiv.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgs_muldiv.org /out _il_dbgs_muldiv.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgs_muldiv.org /out _il_dbgs_muldiv.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgs_muldiv.reflect.xml
  if [ -f _il_dbgs_muldiv.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgs_muldiv.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgs_muldiv.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgs_muldiv.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgs_muldiv.asm.exe $CLRTestExecutionArguments
