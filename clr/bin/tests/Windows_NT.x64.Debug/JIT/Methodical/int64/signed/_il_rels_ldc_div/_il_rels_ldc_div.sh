...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/signed/_il_rels_ldc_div _il_rels_ldc_div.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/signed/_il_rels_ldc_div _il_rels_ldc_div.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_rels_ldc_div.dasm.il _il_rels_ldc_div.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_rels_ldc_div.dasm.il _il_rels_ldc_div.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_rels_ldc_div.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_rels_ldc_div.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_rels_ldc_div.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_rels_ldc_div.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_rels_ldc_div
    if [ ! -f _il_rels_ldc_div.org ]; then
        TakeLock
        if [ ! -f _il_rels_ldc_div.org ]; then
          mkdir IL
          cp _il_rels_ldc_div.exe IL/_il_rels_ldc_div.exe
          mv _il_rels_ldc_div.exe _il_rels_ldc_div.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_rels_ldc_div.org /out _il_rels_ldc_div.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_rels_ldc_div.org /out _il_rels_ldc_div.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_rels_ldc_div.reflect.xml
  if [ -f _il_rels_ldc_div.reflect.xml ]
then
    ReflectionRoots="-x _il_rels_ldc_div.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_rels_ldc_div.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_rels_ldc_div.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_rels_ldc_div.asm.exe $CLRTestExecutionArguments
