...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/signed/_dbgs_muldiv _dbgs_muldiv.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/int64/signed/_dbgs_muldiv _dbgs_muldiv.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgs_muldiv.dasm.il _dbgs_muldiv.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgs_muldiv.dasm.il _dbgs_muldiv.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgs_muldiv.asm.exe  _dbgs_muldiv.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgs_muldiv.asm.exe  _dbgs_muldiv.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgs_muldiv
    if [ ! -f _dbgs_muldiv.org ]; then
        TakeLock
        if [ ! -f _dbgs_muldiv.org ]; then
          mkdir IL
          cp _dbgs_muldiv.exe IL/_dbgs_muldiv.exe
          mv _dbgs_muldiv.exe _dbgs_muldiv.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgs_muldiv.org /out _dbgs_muldiv.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgs_muldiv.org /out _dbgs_muldiv.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgs_muldiv.reflect.xml
  if [ -f _dbgs_muldiv.reflect.xml ]
then
    ReflectionRoots="-x _dbgs_muldiv.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgs_muldiv.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgs_muldiv.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgs_muldiv.asm.exe $CLRTestExecutionArguments
