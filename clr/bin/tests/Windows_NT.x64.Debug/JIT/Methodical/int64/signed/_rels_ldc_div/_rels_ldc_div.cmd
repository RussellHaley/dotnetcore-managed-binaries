...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\signed\_rels_ldc_div _rels_ldc_div.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\int64\signed\_rels_ldc_div _rels_ldc_div.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_rels_ldc_div.dasm.il _rels_ldc_div.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_rels_ldc_div.dasm.il _rels_ldc_div.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_rels_ldc_div.asm.exe  _rels_ldc_div.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_rels_ldc_div.asm.exe  _rels_ldc_div.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_rels_ldc_div
    if not exist "_rels_ldc_div.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_rels_ldc_div.org" (
            mkdir IL
            copy _rels_ldc_div.exe IL\_rels_ldc_div.exe 
            ren _rels_ldc_div.exe _rels_ldc_div.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rels_ldc_div.org /out _rels_ldc_div.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rels_ldc_div.org /out _rels_ldc_div.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _rels_ldc_div.reflect.xml
    IF EXIST _rels_ldc_div.reflect.xml set ReflectionRoots=-x _rels_ldc_div.reflect.xml
    ...
    set ExePath=%LinkBin%\_rels_ldc_div.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _rels_ldc_div.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _rels_ldc_div.asm.exe %CLRTestExecutionArguments% %Host_Args%
