...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\iface\_speed_dbgiface1 _speed_dbgiface1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\iface\_speed_dbgiface1 _speed_dbgiface1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgiface1.dasm.il _speed_dbgiface1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgiface1.dasm.il _speed_dbgiface1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgiface1.asm.exe  _speed_dbgiface1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgiface1.asm.exe  _speed_dbgiface1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgiface1
    if not exist "_speed_dbgiface1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgiface1.org" (
            mkdir IL
            copy _speed_dbgiface1.exe IL\_speed_dbgiface1.exe 
            ren _speed_dbgiface1.exe _speed_dbgiface1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgiface1.org /out _speed_dbgiface1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgiface1.org /out _speed_dbgiface1.exe
...
set ExePath=_speed_dbgiface1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgiface1.reflect.xml
    IF EXIST _speed_dbgiface1.reflect.xml set ReflectionRoots=-x _speed_dbgiface1.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgiface1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgiface1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgiface1.asm.exe %CLRTestExecutionArguments% %Host_Args%
