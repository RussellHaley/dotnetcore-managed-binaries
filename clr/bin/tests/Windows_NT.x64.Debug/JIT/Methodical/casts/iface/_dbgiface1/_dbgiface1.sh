...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/iface/_dbgiface1 _dbgiface1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/iface/_dbgiface1 _dbgiface1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgiface1.dasm.il _dbgiface1.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgiface1.dasm.il _dbgiface1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgiface1.asm.exe  _dbgiface1.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgiface1.asm.exe  _dbgiface1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgiface1
    if [ ! -f _dbgiface1.org ]; then
        TakeLock
        if [ ! -f _dbgiface1.org ]; then
          mkdir IL
          cp _dbgiface1.exe IL/_dbgiface1.exe
          mv _dbgiface1.exe _dbgiface1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgiface1.org /out _dbgiface1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgiface1.org /out _dbgiface1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgiface1.reflect.xml
  if [ -f _dbgiface1.reflect.xml ]
then
    ReflectionRoots="-x _dbgiface1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgiface1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgiface1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgiface1.asm.exe $CLRTestExecutionArguments
