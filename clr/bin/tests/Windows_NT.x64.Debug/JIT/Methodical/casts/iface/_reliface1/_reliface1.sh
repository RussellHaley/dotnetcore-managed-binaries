...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/iface/_reliface1 _reliface1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/iface/_reliface1 _reliface1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_reliface1.dasm.il _reliface1.exe
    "$CORE_ROOT/ildasm" -raweh -out=_reliface1.dasm.il _reliface1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_reliface1.asm.exe  _reliface1.dasm.il
    "$CORE_ROOT/ilasm" -output=_reliface1.asm.exe  _reliface1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_reliface1
    if [ ! -f _reliface1.org ]; then
        TakeLock
        if [ ! -f _reliface1.org ]; then
          mkdir IL
          cp _reliface1.exe IL/_reliface1.exe
          mv _reliface1.exe _reliface1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reliface1.org /out _reliface1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reliface1.org /out _reliface1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _reliface1.reflect.xml
  if [ -f _reliface1.reflect.xml ]
then
    ReflectionRoots="-x _reliface1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_reliface1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _reliface1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _reliface1.asm.exe $CLRTestExecutionArguments
