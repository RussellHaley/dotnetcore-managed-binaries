...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/iface/_speed_reliface1 _speed_reliface1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/iface/_speed_reliface1 _speed_reliface1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_reliface1.dasm.il _speed_reliface1.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_reliface1.dasm.il _speed_reliface1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_reliface1.asm.exe  _speed_reliface1.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_reliface1.asm.exe  _speed_reliface1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_reliface1
    if [ ! -f _speed_reliface1.org ]; then
        TakeLock
        if [ ! -f _speed_reliface1.org ]; then
          mkdir IL
          cp _speed_reliface1.exe IL/_speed_reliface1.exe
          mv _speed_reliface1.exe _speed_reliface1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_reliface1.org /out _speed_reliface1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_reliface1.org /out _speed_reliface1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_reliface1.reflect.xml
  if [ -f _speed_reliface1.reflect.xml ]
then
    ReflectionRoots="-x _speed_reliface1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_reliface1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_reliface1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_reliface1.asm.exe $CLRTestExecutionArguments
