...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_il_dbgisinst_catch_neg _il_dbgisinst_catch_neg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_il_dbgisinst_catch_neg _il_dbgisinst_catch_neg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgisinst_catch_neg.dasm.il _il_dbgisinst_catch_neg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgisinst_catch_neg.dasm.il _il_dbgisinst_catch_neg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgisinst_catch_neg.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgisinst_catch_neg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgisinst_catch_neg.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgisinst_catch_neg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgisinst_catch_neg
    if not exist "_il_dbgisinst_catch_neg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgisinst_catch_neg.org" (
            mkdir IL
            copy _il_dbgisinst_catch_neg.exe IL\_il_dbgisinst_catch_neg.exe 
            ren _il_dbgisinst_catch_neg.exe _il_dbgisinst_catch_neg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgisinst_catch_neg.org /out _il_dbgisinst_catch_neg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgisinst_catch_neg.org /out _il_dbgisinst_catch_neg.exe
...
set ExePath=_il_dbgisinst_catch_neg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgisinst_catch_neg.reflect.xml
    IF EXIST _il_dbgisinst_catch_neg.reflect.xml set ReflectionRoots=-x _il_dbgisinst_catch_neg.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgisinst_catch_neg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgisinst_catch_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgisinst_catch_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
