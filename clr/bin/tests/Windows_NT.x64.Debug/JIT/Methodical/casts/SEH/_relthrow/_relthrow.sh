...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_relthrow _relthrow.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_relthrow _relthrow.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relthrow.dasm.il _relthrow.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relthrow.dasm.il _relthrow.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relthrow.asm.exe  _relthrow.dasm.il
    "$CORE_ROOT/ilasm" -output=_relthrow.asm.exe  _relthrow.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relthrow
    if [ ! -f _relthrow.org ]; then
        TakeLock
        if [ ! -f _relthrow.org ]; then
          mkdir IL
          cp _relthrow.exe IL/_relthrow.exe
          mv _relthrow.exe _relthrow.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relthrow.org /out _relthrow.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relthrow.org /out _relthrow.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relthrow.reflect.xml
  if [ -f _relthrow.reflect.xml ]
then
    ReflectionRoots="-x _relthrow.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relthrow.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relthrow.asm.exe $CLRTestExecutionArguments
