...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_il_relisinst_catch_neg _il_relisinst_catch_neg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_il_relisinst_catch_neg _il_relisinst_catch_neg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relisinst_catch_neg.dasm.il _il_relisinst_catch_neg.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relisinst_catch_neg.dasm.il _il_relisinst_catch_neg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relisinst_catch_neg.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relisinst_catch_neg.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relisinst_catch_neg.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relisinst_catch_neg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relisinst_catch_neg
    if [ ! -f _il_relisinst_catch_neg.org ]; then
        TakeLock
        if [ ! -f _il_relisinst_catch_neg.org ]; then
          mkdir IL
          cp _il_relisinst_catch_neg.exe IL/_il_relisinst_catch_neg.exe
          mv _il_relisinst_catch_neg.exe _il_relisinst_catch_neg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relisinst_catch_neg.org /out _il_relisinst_catch_neg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relisinst_catch_neg.org /out _il_relisinst_catch_neg.exe
          __cgExitCode=$?
...
ExePath=_il_relisinst_catch_neg.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relisinst_catch_neg.reflect.xml
  if [ -f _il_relisinst_catch_neg.reflect.xml ]
then
    ReflectionRoots="-x _il_relisinst_catch_neg.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relisinst_catch_neg.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relisinst_catch_neg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relisinst_catch_neg.asm.exe $CLRTestExecutionArguments
