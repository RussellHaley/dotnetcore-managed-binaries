...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_il_relfilter _il_relfilter.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_il_relfilter _il_relfilter.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relfilter.dasm.il _il_relfilter.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relfilter.dasm.il _il_relfilter.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relfilter.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relfilter.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relfilter.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relfilter.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relfilter
    if not exist "_il_relfilter.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relfilter.org" (
            mkdir IL
            copy _il_relfilter.exe IL\_il_relfilter.exe 
            ren _il_relfilter.exe _il_relfilter.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relfilter.org /out _il_relfilter.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relfilter.org /out _il_relfilter.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relfilter.reflect.xml
    IF EXIST _il_relfilter.reflect.xml set ReflectionRoots=-x _il_relfilter.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relfilter.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relfilter.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relfilter.asm.exe %CLRTestExecutionArguments% %Host_Args%
