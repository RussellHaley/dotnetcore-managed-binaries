...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_speed_relthrow _speed_relthrow.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_speed_relthrow _speed_relthrow.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relthrow.dasm.il _speed_relthrow.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relthrow.dasm.il _speed_relthrow.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_relthrow.asm.exe  _speed_relthrow.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_relthrow.asm.exe  _speed_relthrow.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_relthrow
    if not exist "_speed_relthrow.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_relthrow.org" (
            mkdir IL
            copy _speed_relthrow.exe IL\_speed_relthrow.exe 
            ren _speed_relthrow.exe _speed_relthrow.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relthrow.org /out _speed_relthrow.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relthrow.org /out _speed_relthrow.exe
...
set ExePath=_speed_relthrow.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_relthrow.reflect.xml
    IF EXIST _speed_relthrow.reflect.xml set ReflectionRoots=-x _speed_relthrow.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_relthrow.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_relthrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_relthrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
