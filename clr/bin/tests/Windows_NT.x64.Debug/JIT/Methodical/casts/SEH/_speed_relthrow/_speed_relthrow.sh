...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_speed_relthrow _speed_relthrow.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_speed_relthrow _speed_relthrow.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relthrow.dasm.il _speed_relthrow.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relthrow.dasm.il _speed_relthrow.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relthrow.asm.exe  _speed_relthrow.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relthrow.asm.exe  _speed_relthrow.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relthrow
    if [ ! -f _speed_relthrow.org ]; then
        TakeLock
        if [ ! -f _speed_relthrow.org ]; then
          mkdir IL
          cp _speed_relthrow.exe IL/_speed_relthrow.exe
          mv _speed_relthrow.exe _speed_relthrow.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relthrow.org /out _speed_relthrow.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relthrow.org /out _speed_relthrow.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relthrow.reflect.xml
  if [ -f _speed_relthrow.reflect.xml ]
then
    ReflectionRoots="-x _speed_relthrow.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relthrow.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relthrow.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relthrow.asm.exe $CLRTestExecutionArguments
