...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_speed_dbgthrow _speed_dbgthrow.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_speed_dbgthrow _speed_dbgthrow.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgthrow.dasm.il _speed_dbgthrow.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgthrow.dasm.il _speed_dbgthrow.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgthrow.asm.exe  _speed_dbgthrow.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgthrow.asm.exe  _speed_dbgthrow.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgthrow
    if [ ! -f _speed_dbgthrow.org ]; then
        TakeLock
        if [ ! -f _speed_dbgthrow.org ]; then
          mkdir IL
          cp _speed_dbgthrow.exe IL/_speed_dbgthrow.exe
          mv _speed_dbgthrow.exe _speed_dbgthrow.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgthrow.org /out _speed_dbgthrow.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgthrow.org /out _speed_dbgthrow.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgthrow.reflect.xml
  if [ -f _speed_dbgthrow.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgthrow.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgthrow.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgthrow.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgthrow.asm.exe $CLRTestExecutionArguments
