...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_dbgthrow _dbgthrow.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_dbgthrow _dbgthrow.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgthrow.dasm.il _dbgthrow.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgthrow.dasm.il _dbgthrow.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgthrow.asm.exe  _dbgthrow.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgthrow.asm.exe  _dbgthrow.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgthrow
    if [ ! -f _dbgthrow.org ]; then
        TakeLock
        if [ ! -f _dbgthrow.org ]; then
          mkdir IL
          cp _dbgthrow.exe IL/_dbgthrow.exe
          mv _dbgthrow.exe _dbgthrow.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgthrow.org /out _dbgthrow.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgthrow.org /out _dbgthrow.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgthrow.reflect.xml
  if [ -f _dbgthrow.reflect.xml ]
then
    ReflectionRoots="-x _dbgthrow.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgthrow.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgthrow.asm.exe $CLRTestExecutionArguments
