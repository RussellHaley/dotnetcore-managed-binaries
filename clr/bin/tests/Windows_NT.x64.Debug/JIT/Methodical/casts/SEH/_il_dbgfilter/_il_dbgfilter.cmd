...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_il_dbgfilter _il_dbgfilter.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_il_dbgfilter _il_dbgfilter.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgfilter.dasm.il _il_dbgfilter.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgfilter.dasm.il _il_dbgfilter.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgfilter.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgfilter.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgfilter.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgfilter.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgfilter
    if not exist "_il_dbgfilter.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgfilter.org" (
            mkdir IL
            copy _il_dbgfilter.exe IL\_il_dbgfilter.exe 
            ren _il_dbgfilter.exe _il_dbgfilter.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgfilter.org /out _il_dbgfilter.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgfilter.org /out _il_dbgfilter.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgfilter.reflect.xml
    IF EXIST _il_dbgfilter.reflect.xml set ReflectionRoots=-x _il_dbgfilter.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgfilter.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgfilter.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgfilter.asm.exe %CLRTestExecutionArguments% %Host_Args%
