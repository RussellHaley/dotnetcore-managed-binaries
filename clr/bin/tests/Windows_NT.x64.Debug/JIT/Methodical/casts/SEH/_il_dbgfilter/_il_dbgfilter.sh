...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_il_dbgfilter _il_dbgfilter.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_il_dbgfilter _il_dbgfilter.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgfilter.dasm.il _il_dbgfilter.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgfilter.dasm.il _il_dbgfilter.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgfilter.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgfilter.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgfilter.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgfilter.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgfilter
    if [ ! -f _il_dbgfilter.org ]; then
        TakeLock
        if [ ! -f _il_dbgfilter.org ]; then
          mkdir IL
          cp _il_dbgfilter.exe IL/_il_dbgfilter.exe
          mv _il_dbgfilter.exe _il_dbgfilter.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgfilter.org /out _il_dbgfilter.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgfilter.org /out _il_dbgfilter.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgfilter.reflect.xml
  if [ -f _il_dbgfilter.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgfilter.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgfilter.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgfilter.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgfilter.asm.exe $CLRTestExecutionArguments
