...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_speed_dbgthrow _speed_dbgthrow.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_speed_dbgthrow _speed_dbgthrow.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgthrow.dasm.il _speed_dbgthrow.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgthrow.dasm.il _speed_dbgthrow.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgthrow.asm.exe  _speed_dbgthrow.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgthrow.asm.exe  _speed_dbgthrow.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgthrow
    if not exist "_speed_dbgthrow.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgthrow.org" (
            mkdir IL
            copy _speed_dbgthrow.exe IL\_speed_dbgthrow.exe 
            ren _speed_dbgthrow.exe _speed_dbgthrow.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgthrow.org /out _speed_dbgthrow.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgthrow.org /out _speed_dbgthrow.exe
...
set ExePath=_speed_dbgthrow.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgthrow.reflect.xml
    IF EXIST _speed_dbgthrow.reflect.xml set ReflectionRoots=-x _speed_dbgthrow.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgthrow.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgthrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgthrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
