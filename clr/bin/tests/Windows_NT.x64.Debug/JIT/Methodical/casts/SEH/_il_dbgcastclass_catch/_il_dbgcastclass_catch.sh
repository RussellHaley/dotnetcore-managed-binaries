...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_il_dbgcastclass_catch _il_dbgcastclass_catch.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_il_dbgcastclass_catch _il_dbgcastclass_catch.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcastclass_catch.dasm.il _il_dbgcastclass_catch.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcastclass_catch.dasm.il _il_dbgcastclass_catch.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgcastclass_catch.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcastclass_catch.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgcastclass_catch.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcastclass_catch.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgcastclass_catch
    if [ ! -f _il_dbgcastclass_catch.org ]; then
        TakeLock
        if [ ! -f _il_dbgcastclass_catch.org ]; then
          mkdir IL
          cp _il_dbgcastclass_catch.exe IL/_il_dbgcastclass_catch.exe
          mv _il_dbgcastclass_catch.exe _il_dbgcastclass_catch.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcastclass_catch.org /out _il_dbgcastclass_catch.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcastclass_catch.org /out _il_dbgcastclass_catch.exe
          __cgExitCode=$?
...
ExePath=_il_dbgcastclass_catch.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgcastclass_catch.reflect.xml
  if [ -f _il_dbgcastclass_catch.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgcastclass_catch.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgcastclass_catch.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgcastclass_catch.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgcastclass_catch.asm.exe $CLRTestExecutionArguments
