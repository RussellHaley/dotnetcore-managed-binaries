...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_il_relfilter _il_relfilter.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/SEH/_il_relfilter _il_relfilter.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relfilter.dasm.il _il_relfilter.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relfilter.dasm.il _il_relfilter.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relfilter.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relfilter.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relfilter.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relfilter.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relfilter
    if [ ! -f _il_relfilter.org ]; then
        TakeLock
        if [ ! -f _il_relfilter.org ]; then
          mkdir IL
          cp _il_relfilter.exe IL/_il_relfilter.exe
          mv _il_relfilter.exe _il_relfilter.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relfilter.org /out _il_relfilter.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relfilter.org /out _il_relfilter.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relfilter.reflect.xml
  if [ -f _il_relfilter.reflect.xml ]
then
    ReflectionRoots="-x _il_relfilter.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relfilter.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relfilter.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relfilter.asm.exe $CLRTestExecutionArguments
