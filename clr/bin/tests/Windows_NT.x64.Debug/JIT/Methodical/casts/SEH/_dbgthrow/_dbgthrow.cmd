...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_dbgthrow _dbgthrow.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_dbgthrow _dbgthrow.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgthrow.dasm.il _dbgthrow.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgthrow.dasm.il _dbgthrow.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgthrow.asm.exe  _dbgthrow.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgthrow.asm.exe  _dbgthrow.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgthrow
    if not exist "_dbgthrow.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgthrow.org" (
            mkdir IL
            copy _dbgthrow.exe IL\_dbgthrow.exe 
            ren _dbgthrow.exe _dbgthrow.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgthrow.org /out _dbgthrow.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgthrow.org /out _dbgthrow.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgthrow.reflect.xml
    IF EXIST _dbgthrow.reflect.xml set ReflectionRoots=-x _dbgthrow.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgthrow.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgthrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgthrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
