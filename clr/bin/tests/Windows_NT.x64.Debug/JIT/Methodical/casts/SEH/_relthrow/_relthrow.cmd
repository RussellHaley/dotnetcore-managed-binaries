...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_relthrow _relthrow.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_relthrow _relthrow.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relthrow.dasm.il _relthrow.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relthrow.dasm.il _relthrow.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relthrow.asm.exe  _relthrow.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relthrow.asm.exe  _relthrow.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relthrow
    if not exist "_relthrow.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relthrow.org" (
            mkdir IL
            copy _relthrow.exe IL\_relthrow.exe 
            ren _relthrow.exe _relthrow.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relthrow.org /out _relthrow.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relthrow.org /out _relthrow.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relthrow.reflect.xml
    IF EXIST _relthrow.reflect.xml set ReflectionRoots=-x _relthrow.reflect.xml
    ...
    set ExePath=%LinkBin%\_relthrow.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relthrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relthrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
