...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_il_relisinst_catch _il_relisinst_catch.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\SEH\_il_relisinst_catch _il_relisinst_catch.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relisinst_catch.dasm.il _il_relisinst_catch.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relisinst_catch.dasm.il _il_relisinst_catch.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relisinst_catch.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relisinst_catch.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relisinst_catch.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relisinst_catch.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relisinst_catch
    if not exist "_il_relisinst_catch.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relisinst_catch.org" (
            mkdir IL
            copy _il_relisinst_catch.exe IL\_il_relisinst_catch.exe 
            ren _il_relisinst_catch.exe _il_relisinst_catch.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relisinst_catch.org /out _il_relisinst_catch.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relisinst_catch.org /out _il_relisinst_catch.exe
...
set ExePath=_il_relisinst_catch.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relisinst_catch.reflect.xml
    IF EXIST _il_relisinst_catch.reflect.xml set ReflectionRoots=-x _il_relisinst_catch.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relisinst_catch.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relisinst_catch.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relisinst_catch.asm.exe %CLRTestExecutionArguments% %Host_Args%
