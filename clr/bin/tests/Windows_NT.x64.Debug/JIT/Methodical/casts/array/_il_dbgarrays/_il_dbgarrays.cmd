...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\array\_il_dbgarrays _il_dbgarrays.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\array\_il_dbgarrays _il_dbgarrays.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgarrays.dasm.il _il_dbgarrays.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgarrays.dasm.il _il_dbgarrays.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgarrays.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgarrays.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgarrays.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgarrays.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgarrays
    if not exist "_il_dbgarrays.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgarrays.org" (
            mkdir IL
            copy _il_dbgarrays.exe IL\_il_dbgarrays.exe 
            ren _il_dbgarrays.exe _il_dbgarrays.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgarrays.org /out _il_dbgarrays.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgarrays.org /out _il_dbgarrays.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgarrays.reflect.xml
    IF EXIST _il_dbgarrays.reflect.xml set ReflectionRoots=-x _il_dbgarrays.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgarrays.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgarrays.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgarrays.asm.exe %CLRTestExecutionArguments% %Host_Args%
