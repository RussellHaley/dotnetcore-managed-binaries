...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/array/_il_relarrays _il_relarrays.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/array/_il_relarrays _il_relarrays.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relarrays.dasm.il _il_relarrays.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relarrays.dasm.il _il_relarrays.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relarrays.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relarrays.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relarrays.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relarrays.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relarrays
    if [ ! -f _il_relarrays.org ]; then
        TakeLock
        if [ ! -f _il_relarrays.org ]; then
          mkdir IL
          cp _il_relarrays.exe IL/_il_relarrays.exe
          mv _il_relarrays.exe _il_relarrays.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relarrays.org /out _il_relarrays.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relarrays.org /out _il_relarrays.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relarrays.reflect.xml
  if [ -f _il_relarrays.reflect.xml ]
then
    ReflectionRoots="-x _il_relarrays.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relarrays.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relarrays.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relarrays.asm.exe $CLRTestExecutionArguments
