...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/array/_il_dbgarrays _il_dbgarrays.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/array/_il_dbgarrays _il_dbgarrays.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgarrays.dasm.il _il_dbgarrays.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgarrays.dasm.il _il_dbgarrays.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgarrays.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgarrays.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgarrays.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgarrays.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgarrays
    if [ ! -f _il_dbgarrays.org ]; then
        TakeLock
        if [ ! -f _il_dbgarrays.org ]; then
          mkdir IL
          cp _il_dbgarrays.exe IL/_il_dbgarrays.exe
          mv _il_dbgarrays.exe _il_dbgarrays.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgarrays.org /out _il_dbgarrays.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgarrays.org /out _il_dbgarrays.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgarrays.reflect.xml
  if [ -f _il_dbgarrays.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgarrays.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgarrays.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgarrays.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgarrays.asm.exe $CLRTestExecutionArguments
