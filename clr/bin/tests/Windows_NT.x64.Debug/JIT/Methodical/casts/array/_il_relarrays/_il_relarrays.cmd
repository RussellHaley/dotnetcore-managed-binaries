...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\array\_il_relarrays _il_relarrays.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\array\_il_relarrays _il_relarrays.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relarrays.dasm.il _il_relarrays.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relarrays.dasm.il _il_relarrays.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relarrays.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relarrays.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relarrays.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relarrays.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relarrays
    if not exist "_il_relarrays.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relarrays.org" (
            mkdir IL
            copy _il_relarrays.exe IL\_il_relarrays.exe 
            ren _il_relarrays.exe _il_relarrays.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relarrays.org /out _il_relarrays.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relarrays.org /out _il_relarrays.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relarrays.reflect.xml
    IF EXIST _il_relarrays.reflect.xml set ReflectionRoots=-x _il_relarrays.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relarrays.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relarrays.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relarrays.asm.exe %CLRTestExecutionArguments% %Host_Args%
