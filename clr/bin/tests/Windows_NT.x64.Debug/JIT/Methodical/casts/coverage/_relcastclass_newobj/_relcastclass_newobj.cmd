...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_relcastclass_newobj _relcastclass_newobj.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_relcastclass_newobj _relcastclass_newobj.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relcastclass_newobj.dasm.il _relcastclass_newobj.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relcastclass_newobj.dasm.il _relcastclass_newobj.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relcastclass_newobj.asm.exe  _relcastclass_newobj.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relcastclass_newobj.asm.exe  _relcastclass_newobj.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relcastclass_newobj
    if not exist "_relcastclass_newobj.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relcastclass_newobj.org" (
            mkdir IL
            copy _relcastclass_newobj.exe IL\_relcastclass_newobj.exe 
            ren _relcastclass_newobj.exe _relcastclass_newobj.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relcastclass_newobj.org /out _relcastclass_newobj.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relcastclass_newobj.org /out _relcastclass_newobj.exe
...
set ExePath=_relcastclass_newobj.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relcastclass_newobj.reflect.xml
    IF EXIST _relcastclass_newobj.reflect.xml set ReflectionRoots=-x _relcastclass_newobj.reflect.xml
    ...
    set ExePath=%LinkBin%\_relcastclass_newobj.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relcastclass_newobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relcastclass_newobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
