...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_relcastclass_newobj _relcastclass_newobj.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_relcastclass_newobj _relcastclass_newobj.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relcastclass_newobj.dasm.il _relcastclass_newobj.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relcastclass_newobj.dasm.il _relcastclass_newobj.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relcastclass_newobj.asm.exe  _relcastclass_newobj.dasm.il
    "$CORE_ROOT/ilasm" -output=_relcastclass_newobj.asm.exe  _relcastclass_newobj.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relcastclass_newobj
    if [ ! -f _relcastclass_newobj.org ]; then
        TakeLock
        if [ ! -f _relcastclass_newobj.org ]; then
          mkdir IL
          cp _relcastclass_newobj.exe IL/_relcastclass_newobj.exe
          mv _relcastclass_newobj.exe _relcastclass_newobj.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relcastclass_newobj.org /out _relcastclass_newobj.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relcastclass_newobj.org /out _relcastclass_newobj.exe
          __cgExitCode=$?
...
ExePath=_relcastclass_newobj.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _relcastclass_newobj.reflect.xml
  if [ -f _relcastclass_newobj.reflect.xml ]
then
    ReflectionRoots="-x _relcastclass_newobj.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relcastclass_newobj.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relcastclass_newobj.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relcastclass_newobj.asm.exe $CLRTestExecutionArguments
