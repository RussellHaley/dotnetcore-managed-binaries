...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_speed_dbgcastclass_newobj _speed_dbgcastclass_newobj.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_speed_dbgcastclass_newobj _speed_dbgcastclass_newobj.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgcastclass_newobj.dasm.il _speed_dbgcastclass_newobj.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgcastclass_newobj.dasm.il _speed_dbgcastclass_newobj.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgcastclass_newobj.asm.exe  _speed_dbgcastclass_newobj.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgcastclass_newobj.asm.exe  _speed_dbgcastclass_newobj.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgcastclass_newobj
    if [ ! -f _speed_dbgcastclass_newobj.org ]; then
        TakeLock
        if [ ! -f _speed_dbgcastclass_newobj.org ]; then
          mkdir IL
          cp _speed_dbgcastclass_newobj.exe IL/_speed_dbgcastclass_newobj.exe
          mv _speed_dbgcastclass_newobj.exe _speed_dbgcastclass_newobj.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgcastclass_newobj.org /out _speed_dbgcastclass_newobj.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgcastclass_newobj.org /out _speed_dbgcastclass_newobj.exe
          __cgExitCode=$?
...
ExePath=_speed_dbgcastclass_newobj.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgcastclass_newobj.reflect.xml
  if [ -f _speed_dbgcastclass_newobj.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgcastclass_newobj.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgcastclass_newobj.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgcastclass_newobj.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgcastclass_newobj.asm.exe $CLRTestExecutionArguments
