...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_il_dbgcastclass_ldloc _il_dbgcastclass_ldloc.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_il_dbgcastclass_ldloc _il_dbgcastclass_ldloc.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcastclass_ldloc.dasm.il _il_dbgcastclass_ldloc.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcastclass_ldloc.dasm.il _il_dbgcastclass_ldloc.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgcastclass_ldloc.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcastclass_ldloc.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgcastclass_ldloc.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcastclass_ldloc.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgcastclass_ldloc
    if [ ! -f _il_dbgcastclass_ldloc.org ]; then
        TakeLock
        if [ ! -f _il_dbgcastclass_ldloc.org ]; then
          mkdir IL
          cp _il_dbgcastclass_ldloc.exe IL/_il_dbgcastclass_ldloc.exe
          mv _il_dbgcastclass_ldloc.exe _il_dbgcastclass_ldloc.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcastclass_ldloc.org /out _il_dbgcastclass_ldloc.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcastclass_ldloc.org /out _il_dbgcastclass_ldloc.exe
          __cgExitCode=$?
...
ExePath=_il_dbgcastclass_ldloc.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgcastclass_ldloc.reflect.xml
  if [ -f _il_dbgcastclass_ldloc.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgcastclass_ldloc.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgcastclass_ldloc.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgcastclass_ldloc.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgcastclass_ldloc.asm.exe $CLRTestExecutionArguments
