...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_dbgcastclass_ldarg _dbgcastclass_ldarg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_dbgcastclass_ldarg _dbgcastclass_ldarg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgcastclass_ldarg.dasm.il _dbgcastclass_ldarg.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgcastclass_ldarg.dasm.il _dbgcastclass_ldarg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgcastclass_ldarg.asm.exe  _dbgcastclass_ldarg.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgcastclass_ldarg.asm.exe  _dbgcastclass_ldarg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgcastclass_ldarg
    if [ ! -f _dbgcastclass_ldarg.org ]; then
        TakeLock
        if [ ! -f _dbgcastclass_ldarg.org ]; then
          mkdir IL
          cp _dbgcastclass_ldarg.exe IL/_dbgcastclass_ldarg.exe
          mv _dbgcastclass_ldarg.exe _dbgcastclass_ldarg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcastclass_ldarg.org /out _dbgcastclass_ldarg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcastclass_ldarg.org /out _dbgcastclass_ldarg.exe
          __cgExitCode=$?
...
ExePath=_dbgcastclass_ldarg.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgcastclass_ldarg.reflect.xml
  if [ -f _dbgcastclass_ldarg.reflect.xml ]
then
    ReflectionRoots="-x _dbgcastclass_ldarg.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgcastclass_ldarg.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgcastclass_ldarg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgcastclass_ldarg.asm.exe $CLRTestExecutionArguments
