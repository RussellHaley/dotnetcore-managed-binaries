...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_speed_relcastclass_newobj _speed_relcastclass_newobj.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_speed_relcastclass_newobj _speed_relcastclass_newobj.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relcastclass_newobj.dasm.il _speed_relcastclass_newobj.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relcastclass_newobj.dasm.il _speed_relcastclass_newobj.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_relcastclass_newobj.asm.exe  _speed_relcastclass_newobj.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_relcastclass_newobj.asm.exe  _speed_relcastclass_newobj.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_relcastclass_newobj
    if not exist "_speed_relcastclass_newobj.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_relcastclass_newobj.org" (
            mkdir IL
            copy _speed_relcastclass_newobj.exe IL\_speed_relcastclass_newobj.exe 
            ren _speed_relcastclass_newobj.exe _speed_relcastclass_newobj.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relcastclass_newobj.org /out _speed_relcastclass_newobj.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relcastclass_newobj.org /out _speed_relcastclass_newobj.exe
...
set ExePath=_speed_relcastclass_newobj.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_relcastclass_newobj.reflect.xml
    IF EXIST _speed_relcastclass_newobj.reflect.xml set ReflectionRoots=-x _speed_relcastclass_newobj.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_relcastclass_newobj.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_relcastclass_newobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_relcastclass_newobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
