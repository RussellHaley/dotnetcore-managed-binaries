...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_speed_relcastclass_ldarg _speed_relcastclass_ldarg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_speed_relcastclass_ldarg _speed_relcastclass_ldarg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relcastclass_ldarg.dasm.il _speed_relcastclass_ldarg.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relcastclass_ldarg.dasm.il _speed_relcastclass_ldarg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relcastclass_ldarg.asm.exe  _speed_relcastclass_ldarg.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relcastclass_ldarg.asm.exe  _speed_relcastclass_ldarg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relcastclass_ldarg
    if [ ! -f _speed_relcastclass_ldarg.org ]; then
        TakeLock
        if [ ! -f _speed_relcastclass_ldarg.org ]; then
          mkdir IL
          cp _speed_relcastclass_ldarg.exe IL/_speed_relcastclass_ldarg.exe
          mv _speed_relcastclass_ldarg.exe _speed_relcastclass_ldarg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relcastclass_ldarg.org /out _speed_relcastclass_ldarg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relcastclass_ldarg.org /out _speed_relcastclass_ldarg.exe
          __cgExitCode=$?
...
ExePath=_speed_relcastclass_ldarg.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relcastclass_ldarg.reflect.xml
  if [ -f _speed_relcastclass_ldarg.reflect.xml ]
then
    ReflectionRoots="-x _speed_relcastclass_ldarg.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relcastclass_ldarg.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relcastclass_ldarg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relcastclass_ldarg.asm.exe $CLRTestExecutionArguments
