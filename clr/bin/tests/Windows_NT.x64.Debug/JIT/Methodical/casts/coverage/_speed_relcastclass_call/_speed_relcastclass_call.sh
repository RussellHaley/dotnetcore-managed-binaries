...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_speed_relcastclass_call _speed_relcastclass_call.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_speed_relcastclass_call _speed_relcastclass_call.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relcastclass_call.dasm.il _speed_relcastclass_call.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relcastclass_call.dasm.il _speed_relcastclass_call.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relcastclass_call.asm.exe  _speed_relcastclass_call.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relcastclass_call.asm.exe  _speed_relcastclass_call.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relcastclass_call
    if [ ! -f _speed_relcastclass_call.org ]; then
        TakeLock
        if [ ! -f _speed_relcastclass_call.org ]; then
          mkdir IL
          cp _speed_relcastclass_call.exe IL/_speed_relcastclass_call.exe
          mv _speed_relcastclass_call.exe _speed_relcastclass_call.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relcastclass_call.org /out _speed_relcastclass_call.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relcastclass_call.org /out _speed_relcastclass_call.exe
          __cgExitCode=$?
...
ExePath=_speed_relcastclass_call.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relcastclass_call.reflect.xml
  if [ -f _speed_relcastclass_call.reflect.xml ]
then
    ReflectionRoots="-x _speed_relcastclass_call.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relcastclass_call.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relcastclass_call.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relcastclass_call.asm.exe $CLRTestExecutionArguments
