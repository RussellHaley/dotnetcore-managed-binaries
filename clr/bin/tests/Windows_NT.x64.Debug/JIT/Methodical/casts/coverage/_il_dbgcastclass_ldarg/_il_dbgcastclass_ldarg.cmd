...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_il_dbgcastclass_ldarg _il_dbgcastclass_ldarg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_il_dbgcastclass_ldarg _il_dbgcastclass_ldarg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcastclass_ldarg.dasm.il _il_dbgcastclass_ldarg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcastclass_ldarg.dasm.il _il_dbgcastclass_ldarg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgcastclass_ldarg.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcastclass_ldarg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgcastclass_ldarg.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcastclass_ldarg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgcastclass_ldarg
    if not exist "_il_dbgcastclass_ldarg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgcastclass_ldarg.org" (
            mkdir IL
            copy _il_dbgcastclass_ldarg.exe IL\_il_dbgcastclass_ldarg.exe 
            ren _il_dbgcastclass_ldarg.exe _il_dbgcastclass_ldarg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcastclass_ldarg.org /out _il_dbgcastclass_ldarg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcastclass_ldarg.org /out _il_dbgcastclass_ldarg.exe
...
set ExePath=_il_dbgcastclass_ldarg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgcastclass_ldarg.reflect.xml
    IF EXIST _il_dbgcastclass_ldarg.reflect.xml set ReflectionRoots=-x _il_dbgcastclass_ldarg.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgcastclass_ldarg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgcastclass_ldarg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgcastclass_ldarg.asm.exe %CLRTestExecutionArguments% %Host_Args%
