...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_dbgcastclass_ldloc _dbgcastclass_ldloc.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_dbgcastclass_ldloc _dbgcastclass_ldloc.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgcastclass_ldloc.dasm.il _dbgcastclass_ldloc.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgcastclass_ldloc.dasm.il _dbgcastclass_ldloc.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgcastclass_ldloc.asm.exe  _dbgcastclass_ldloc.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgcastclass_ldloc.asm.exe  _dbgcastclass_ldloc.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgcastclass_ldloc
    if not exist "_dbgcastclass_ldloc.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgcastclass_ldloc.org" (
            mkdir IL
            copy _dbgcastclass_ldloc.exe IL\_dbgcastclass_ldloc.exe 
            ren _dbgcastclass_ldloc.exe _dbgcastclass_ldloc.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgcastclass_ldloc.org /out _dbgcastclass_ldloc.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgcastclass_ldloc.org /out _dbgcastclass_ldloc.exe
...
set ExePath=_dbgcastclass_ldloc.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgcastclass_ldloc.reflect.xml
    IF EXIST _dbgcastclass_ldloc.reflect.xml set ReflectionRoots=-x _dbgcastclass_ldloc.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgcastclass_ldloc.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgcastclass_ldloc.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgcastclass_ldloc.asm.exe %CLRTestExecutionArguments% %Host_Args%
