...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_dbgcastclass_call _dbgcastclass_call.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_dbgcastclass_call _dbgcastclass_call.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgcastclass_call.dasm.il _dbgcastclass_call.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgcastclass_call.dasm.il _dbgcastclass_call.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgcastclass_call.asm.exe  _dbgcastclass_call.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgcastclass_call.asm.exe  _dbgcastclass_call.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgcastclass_call
    if [ ! -f _dbgcastclass_call.org ]; then
        TakeLock
        if [ ! -f _dbgcastclass_call.org ]; then
          mkdir IL
          cp _dbgcastclass_call.exe IL/_dbgcastclass_call.exe
          mv _dbgcastclass_call.exe _dbgcastclass_call.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcastclass_call.org /out _dbgcastclass_call.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcastclass_call.org /out _dbgcastclass_call.exe
          __cgExitCode=$?
...
ExePath=_dbgcastclass_call.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgcastclass_call.reflect.xml
  if [ -f _dbgcastclass_call.reflect.xml ]
then
    ReflectionRoots="-x _dbgcastclass_call.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgcastclass_call.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgcastclass_call.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgcastclass_call.asm.exe $CLRTestExecutionArguments
