...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_dbgcastclass_newobj _dbgcastclass_newobj.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_dbgcastclass_newobj _dbgcastclass_newobj.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgcastclass_newobj.dasm.il _dbgcastclass_newobj.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgcastclass_newobj.dasm.il _dbgcastclass_newobj.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgcastclass_newobj.asm.exe  _dbgcastclass_newobj.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgcastclass_newobj.asm.exe  _dbgcastclass_newobj.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgcastclass_newobj
    if not exist "_dbgcastclass_newobj.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgcastclass_newobj.org" (
            mkdir IL
            copy _dbgcastclass_newobj.exe IL\_dbgcastclass_newobj.exe 
            ren _dbgcastclass_newobj.exe _dbgcastclass_newobj.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgcastclass_newobj.org /out _dbgcastclass_newobj.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgcastclass_newobj.org /out _dbgcastclass_newobj.exe
...
set ExePath=_dbgcastclass_newobj.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgcastclass_newobj.reflect.xml
    IF EXIST _dbgcastclass_newobj.reflect.xml set ReflectionRoots=-x _dbgcastclass_newobj.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgcastclass_newobj.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgcastclass_newobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgcastclass_newobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
