...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_relcastclass_ldarg _relcastclass_ldarg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_relcastclass_ldarg _relcastclass_ldarg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relcastclass_ldarg.dasm.il _relcastclass_ldarg.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relcastclass_ldarg.dasm.il _relcastclass_ldarg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relcastclass_ldarg.asm.exe  _relcastclass_ldarg.dasm.il
    "$CORE_ROOT/ilasm" -output=_relcastclass_ldarg.asm.exe  _relcastclass_ldarg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relcastclass_ldarg
    if [ ! -f _relcastclass_ldarg.org ]; then
        TakeLock
        if [ ! -f _relcastclass_ldarg.org ]; then
          mkdir IL
          cp _relcastclass_ldarg.exe IL/_relcastclass_ldarg.exe
          mv _relcastclass_ldarg.exe _relcastclass_ldarg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relcastclass_ldarg.org /out _relcastclass_ldarg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relcastclass_ldarg.org /out _relcastclass_ldarg.exe
          __cgExitCode=$?
...
ExePath=_relcastclass_ldarg.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _relcastclass_ldarg.reflect.xml
  if [ -f _relcastclass_ldarg.reflect.xml ]
then
    ReflectionRoots="-x _relcastclass_ldarg.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relcastclass_ldarg.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relcastclass_ldarg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relcastclass_ldarg.asm.exe $CLRTestExecutionArguments
