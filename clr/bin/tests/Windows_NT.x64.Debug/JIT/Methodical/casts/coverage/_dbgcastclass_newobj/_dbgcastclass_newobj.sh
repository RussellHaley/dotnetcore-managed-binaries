...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_dbgcastclass_newobj _dbgcastclass_newobj.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_dbgcastclass_newobj _dbgcastclass_newobj.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgcastclass_newobj.dasm.il _dbgcastclass_newobj.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgcastclass_newobj.dasm.il _dbgcastclass_newobj.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgcastclass_newobj.asm.exe  _dbgcastclass_newobj.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgcastclass_newobj.asm.exe  _dbgcastclass_newobj.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgcastclass_newobj
    if [ ! -f _dbgcastclass_newobj.org ]; then
        TakeLock
        if [ ! -f _dbgcastclass_newobj.org ]; then
          mkdir IL
          cp _dbgcastclass_newobj.exe IL/_dbgcastclass_newobj.exe
          mv _dbgcastclass_newobj.exe _dbgcastclass_newobj.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcastclass_newobj.org /out _dbgcastclass_newobj.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcastclass_newobj.org /out _dbgcastclass_newobj.exe
          __cgExitCode=$?
...
ExePath=_dbgcastclass_newobj.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgcastclass_newobj.reflect.xml
  if [ -f _dbgcastclass_newobj.reflect.xml ]
then
    ReflectionRoots="-x _dbgcastclass_newobj.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgcastclass_newobj.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgcastclass_newobj.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgcastclass_newobj.asm.exe $CLRTestExecutionArguments
