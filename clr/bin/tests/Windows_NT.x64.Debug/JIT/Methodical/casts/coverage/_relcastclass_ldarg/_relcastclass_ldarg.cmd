...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_relcastclass_ldarg _relcastclass_ldarg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_relcastclass_ldarg _relcastclass_ldarg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relcastclass_ldarg.dasm.il _relcastclass_ldarg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relcastclass_ldarg.dasm.il _relcastclass_ldarg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relcastclass_ldarg.asm.exe  _relcastclass_ldarg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relcastclass_ldarg.asm.exe  _relcastclass_ldarg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relcastclass_ldarg
    if not exist "_relcastclass_ldarg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relcastclass_ldarg.org" (
            mkdir IL
            copy _relcastclass_ldarg.exe IL\_relcastclass_ldarg.exe 
            ren _relcastclass_ldarg.exe _relcastclass_ldarg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relcastclass_ldarg.org /out _relcastclass_ldarg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relcastclass_ldarg.org /out _relcastclass_ldarg.exe
...
set ExePath=_relcastclass_ldarg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relcastclass_ldarg.reflect.xml
    IF EXIST _relcastclass_ldarg.reflect.xml set ReflectionRoots=-x _relcastclass_ldarg.reflect.xml
    ...
    set ExePath=%LinkBin%\_relcastclass_ldarg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relcastclass_ldarg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relcastclass_ldarg.asm.exe %CLRTestExecutionArguments% %Host_Args%
