...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_speed_dbgcastclass_call _speed_dbgcastclass_call.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\casts\coverage\_speed_dbgcastclass_call _speed_dbgcastclass_call.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgcastclass_call.dasm.il _speed_dbgcastclass_call.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgcastclass_call.dasm.il _speed_dbgcastclass_call.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgcastclass_call.asm.exe  _speed_dbgcastclass_call.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgcastclass_call.asm.exe  _speed_dbgcastclass_call.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgcastclass_call
    if not exist "_speed_dbgcastclass_call.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgcastclass_call.org" (
            mkdir IL
            copy _speed_dbgcastclass_call.exe IL\_speed_dbgcastclass_call.exe 
            ren _speed_dbgcastclass_call.exe _speed_dbgcastclass_call.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgcastclass_call.org /out _speed_dbgcastclass_call.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgcastclass_call.org /out _speed_dbgcastclass_call.exe
...
set ExePath=_speed_dbgcastclass_call.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgcastclass_call.reflect.xml
    IF EXIST _speed_dbgcastclass_call.reflect.xml set ReflectionRoots=-x _speed_dbgcastclass_call.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgcastclass_call.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgcastclass_call.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgcastclass_call.asm.exe %CLRTestExecutionArguments% %Host_Args%
