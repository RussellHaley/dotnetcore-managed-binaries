...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_relcastclass_call _relcastclass_call.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/casts/coverage/_relcastclass_call _relcastclass_call.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relcastclass_call.dasm.il _relcastclass_call.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relcastclass_call.dasm.il _relcastclass_call.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relcastclass_call.asm.exe  _relcastclass_call.dasm.il
    "$CORE_ROOT/ilasm" -output=_relcastclass_call.asm.exe  _relcastclass_call.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relcastclass_call
    if [ ! -f _relcastclass_call.org ]; then
        TakeLock
        if [ ! -f _relcastclass_call.org ]; then
          mkdir IL
          cp _relcastclass_call.exe IL/_relcastclass_call.exe
          mv _relcastclass_call.exe _relcastclass_call.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relcastclass_call.org /out _relcastclass_call.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relcastclass_call.org /out _relcastclass_call.exe
          __cgExitCode=$?
...
ExePath=_relcastclass_call.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _relcastclass_call.reflect.xml
  if [ -f _relcastclass_call.reflect.xml ]
then
    ReflectionRoots="-x _relcastclass_call.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relcastclass_call.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relcastclass_call.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relcastclass_call.asm.exe $CLRTestExecutionArguments
