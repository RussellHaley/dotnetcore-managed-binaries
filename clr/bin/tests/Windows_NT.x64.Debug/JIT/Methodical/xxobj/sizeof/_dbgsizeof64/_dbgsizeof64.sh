...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/xxobj/sizeof/_dbgsizeof64 _dbgsizeof64.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/xxobj/sizeof/_dbgsizeof64 _dbgsizeof64.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgsizeof64.dasm.il _dbgsizeof64.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgsizeof64.dasm.il _dbgsizeof64.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgsizeof64.asm.exe  _dbgsizeof64.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgsizeof64.asm.exe  _dbgsizeof64.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgsizeof64
    if [ ! -f _dbgsizeof64.org ]; then
        TakeLock
        if [ ! -f _dbgsizeof64.org ]; then
          mkdir IL
          cp _dbgsizeof64.exe IL/_dbgsizeof64.exe
          mv _dbgsizeof64.exe _dbgsizeof64.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgsizeof64.org /out _dbgsizeof64.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgsizeof64.org /out _dbgsizeof64.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgsizeof64.reflect.xml
  if [ -f _dbgsizeof64.reflect.xml ]
then
    ReflectionRoots="-x _dbgsizeof64.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgsizeof64.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgsizeof64.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgsizeof64.asm.exe $CLRTestExecutionArguments
