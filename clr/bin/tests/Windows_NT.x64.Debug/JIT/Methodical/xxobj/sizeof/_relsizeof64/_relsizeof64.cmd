...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\xxobj\sizeof\_relsizeof64 _relsizeof64.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\xxobj\sizeof\_relsizeof64 _relsizeof64.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relsizeof64.dasm.il _relsizeof64.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relsizeof64.dasm.il _relsizeof64.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relsizeof64.asm.exe  _relsizeof64.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relsizeof64.asm.exe  _relsizeof64.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relsizeof64
    if not exist "_relsizeof64.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relsizeof64.org" (
            mkdir IL
            copy _relsizeof64.exe IL\_relsizeof64.exe 
            ren _relsizeof64.exe _relsizeof64.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relsizeof64.org /out _relsizeof64.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relsizeof64.org /out _relsizeof64.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relsizeof64.reflect.xml
    IF EXIST _relsizeof64.reflect.xml set ReflectionRoots=-x _relsizeof64.reflect.xml
    ...
    set ExePath=%LinkBin%\_relsizeof64.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relsizeof64.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relsizeof64.asm.exe %CLRTestExecutionArguments% %Host_Args%
