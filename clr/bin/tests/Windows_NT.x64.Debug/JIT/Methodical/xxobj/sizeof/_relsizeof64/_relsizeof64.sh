...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/xxobj/sizeof/_relsizeof64 _relsizeof64.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/xxobj/sizeof/_relsizeof64 _relsizeof64.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relsizeof64.dasm.il _relsizeof64.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relsizeof64.dasm.il _relsizeof64.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relsizeof64.asm.exe  _relsizeof64.dasm.il
    "$CORE_ROOT/ilasm" -output=_relsizeof64.asm.exe  _relsizeof64.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relsizeof64
    if [ ! -f _relsizeof64.org ]; then
        TakeLock
        if [ ! -f _relsizeof64.org ]; then
          mkdir IL
          cp _relsizeof64.exe IL/_relsizeof64.exe
          mv _relsizeof64.exe _relsizeof64.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relsizeof64.org /out _relsizeof64.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relsizeof64.org /out _relsizeof64.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relsizeof64.reflect.xml
  if [ -f _relsizeof64.reflect.xml ]
then
    ReflectionRoots="-x _relsizeof64.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relsizeof64.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relsizeof64.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relsizeof64.asm.exe $CLRTestExecutionArguments
