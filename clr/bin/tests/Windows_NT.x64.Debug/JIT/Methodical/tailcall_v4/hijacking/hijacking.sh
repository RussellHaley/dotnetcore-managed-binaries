...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall_v4/hijacking hijacking.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall_v4/hijacking hijacking.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=hijacking.dasm.il hijacking.exe
    "$CORE_ROOT/ildasm" -raweh -out=hijacking.dasm.il hijacking.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=hijacking.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE hijacking.dasm.il
    "$CORE_ROOT/ilasm" -output=hijacking.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE hijacking.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=hijacking
    if [ ! -f hijacking.org ]; then
        TakeLock
        if [ ! -f hijacking.org ]; then
          mkdir IL
          cp hijacking.exe IL/hijacking.exe
          mv hijacking.exe hijacking.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hijacking.org /out hijacking.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hijacking.org /out hijacking.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in hijacking.reflect.xml
  if [ -f hijacking.reflect.xml ]
then
    ReflectionRoots="-x hijacking.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" hijacking.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" hijacking.asm.exe $CLRTestExecutionArguments
