...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall_v4\hijacking hijacking.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall_v4\hijacking hijacking.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=hijacking.dasm.il hijacking.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=hijacking.dasm.il hijacking.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=hijacking.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE hijacking.dasm.il
    %CORE_ROOT%\ilasm.exe /output=hijacking.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE hijacking.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=hijacking
    if not exist "hijacking.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "hijacking.org" (
            mkdir IL
            copy hijacking.exe IL\hijacking.exe 
            ren hijacking.exe hijacking.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hijacking.org /out hijacking.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hijacking.org /out hijacking.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in hijacking.reflect.xml
    IF EXIST hijacking.reflect.xml set ReflectionRoots=-x hijacking.reflect.xml
    ...
    set ExePath=%LinkBin%\hijacking.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% hijacking.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% hijacking.asm.exe %CLRTestExecutionArguments% %Host_Args%
