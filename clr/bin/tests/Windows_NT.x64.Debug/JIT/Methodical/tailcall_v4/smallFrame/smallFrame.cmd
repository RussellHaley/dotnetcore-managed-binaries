...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall_v4\smallFrame smallFrame.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall_v4\smallFrame smallFrame.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=smallFrame.dasm.il smallFrame.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=smallFrame.dasm.il smallFrame.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=smallFrame.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE smallFrame.dasm.il
    %CORE_ROOT%\ilasm.exe /output=smallFrame.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE smallFrame.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=smallFrame
    if not exist "smallFrame.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "smallFrame.org" (
            mkdir IL
            copy smallFrame.exe IL\smallFrame.exe 
            ren smallFrame.exe smallFrame.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in smallFrame.org /out smallFrame.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in smallFrame.org /out smallFrame.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in smallFrame.reflect.xml
    IF EXIST smallFrame.reflect.xml set ReflectionRoots=-x smallFrame.reflect.xml
    ...
    set ExePath=%LinkBin%\smallFrame.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% smallFrame.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% smallFrame.asm.exe %CLRTestExecutionArguments% %Host_Args%
