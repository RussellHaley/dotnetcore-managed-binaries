...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall_v4/smallFrame smallFrame.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall_v4/smallFrame smallFrame.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=smallFrame.dasm.il smallFrame.exe
    "$CORE_ROOT/ildasm" -raweh -out=smallFrame.dasm.il smallFrame.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=smallFrame.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE smallFrame.dasm.il
    "$CORE_ROOT/ilasm" -output=smallFrame.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE smallFrame.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=smallFrame
    if [ ! -f smallFrame.org ]; then
        TakeLock
        if [ ! -f smallFrame.org ]; then
          mkdir IL
          cp smallFrame.exe IL/smallFrame.exe
          mv smallFrame.exe smallFrame.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in smallFrame.org /out smallFrame.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in smallFrame.org /out smallFrame.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in smallFrame.reflect.xml
  if [ -f smallFrame.reflect.xml ]
then
    ReflectionRoots="-x smallFrame.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/smallFrame.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" smallFrame.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" smallFrame.asm.exe $CLRTestExecutionArguments
