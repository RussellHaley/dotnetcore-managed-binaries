...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall_v4\tailcall_AV tailcall_AV.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\tailcall_v4\tailcall_AV tailcall_AV.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tailcall_AV.dasm.il tailcall_AV.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tailcall_AV.dasm.il tailcall_AV.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tailcall_AV.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall_AV.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tailcall_AV.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall_AV.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tailcall_AV
    if not exist "tailcall_AV.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tailcall_AV.org" (
            mkdir IL
            copy tailcall_AV.exe IL\tailcall_AV.exe 
            ren tailcall_AV.exe tailcall_AV.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tailcall_AV.org /out tailcall_AV.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tailcall_AV.org /out tailcall_AV.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tailcall_AV.reflect.xml
    IF EXIST tailcall_AV.reflect.xml set ReflectionRoots=-x tailcall_AV.reflect.xml
    ...
    set ExePath=%LinkBin%\tailcall_AV.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tailcall_AV.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tailcall_AV.asm.exe %CLRTestExecutionArguments% %Host_Args%
