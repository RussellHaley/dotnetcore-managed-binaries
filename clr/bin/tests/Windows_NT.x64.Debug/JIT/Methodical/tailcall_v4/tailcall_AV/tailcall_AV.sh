...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall_v4/tailcall_AV tailcall_AV.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/tailcall_v4/tailcall_AV tailcall_AV.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tailcall_AV.dasm.il tailcall_AV.exe
    "$CORE_ROOT/ildasm" -raweh -out=tailcall_AV.dasm.il tailcall_AV.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tailcall_AV.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall_AV.dasm.il
    "$CORE_ROOT/ilasm" -output=tailcall_AV.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall_AV.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tailcall_AV
    if [ ! -f tailcall_AV.org ]; then
        TakeLock
        if [ ! -f tailcall_AV.org ]; then
          mkdir IL
          cp tailcall_AV.exe IL/tailcall_AV.exe
          mv tailcall_AV.exe tailcall_AV.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tailcall_AV.org /out tailcall_AV.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tailcall_AV.org /out tailcall_AV.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in tailcall_AV.reflect.xml
  if [ -f tailcall_AV.reflect.xml ]
then
    ReflectionRoots="-x tailcall_AV.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/tailcall_AV.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tailcall_AV.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tailcall_AV.asm.exe $CLRTestExecutionArguments
