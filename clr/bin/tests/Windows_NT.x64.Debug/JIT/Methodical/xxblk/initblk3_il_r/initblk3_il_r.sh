...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/xxblk/initblk3_il_r initblk3_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/xxblk/initblk3_il_r initblk3_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=initblk3_il_r.dasm.il initblk3_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=initblk3_il_r.dasm.il initblk3_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=initblk3_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT initblk3_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=initblk3_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT initblk3_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=initblk3_il_r
    if [ ! -f initblk3_il_r.org ]; then
        TakeLock
        if [ ! -f initblk3_il_r.org ]; then
          mkdir IL
          cp initblk3_il_r.exe IL/initblk3_il_r.exe
          mv initblk3_il_r.exe initblk3_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in initblk3_il_r.org /out initblk3_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in initblk3_il_r.org /out initblk3_il_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in initblk3_il_r.reflect.xml
  if [ -f initblk3_il_r.reflect.xml ]
then
    ReflectionRoots="-x initblk3_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/initblk3_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" initblk3_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" initblk3_il_r.asm.exe $CLRTestExecutionArguments
