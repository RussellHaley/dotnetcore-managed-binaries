...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/xxblk/initblk3_il_d initblk3_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/xxblk/initblk3_il_d initblk3_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=initblk3_il_d.dasm.il initblk3_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=initblk3_il_d.dasm.il initblk3_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=initblk3_il_d.asm.exe -QUIET -NOLOGO -DEBUG initblk3_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=initblk3_il_d.asm.exe -QUIET -NOLOGO -DEBUG initblk3_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=initblk3_il_d
    if [ ! -f initblk3_il_d.org ]; then
        TakeLock
        if [ ! -f initblk3_il_d.org ]; then
          mkdir IL
          cp initblk3_il_d.exe IL/initblk3_il_d.exe
          mv initblk3_il_d.exe initblk3_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in initblk3_il_d.org /out initblk3_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in initblk3_il_d.org /out initblk3_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in initblk3_il_d.reflect.xml
  if [ -f initblk3_il_d.reflect.xml ]
then
    ReflectionRoots="-x initblk3_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/initblk3_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" initblk3_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" initblk3_il_d.asm.exe $CLRTestExecutionArguments
