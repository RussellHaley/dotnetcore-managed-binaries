...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/localloc/verify/verify01_small verify01_small.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/localloc/verify/verify01_small verify01_small.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=verify01_small.dasm.il verify01_small.exe
    "$CORE_ROOT/ildasm" -raweh -out=verify01_small.dasm.il verify01_small.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=verify01_small.asm.exe -QUIET -NOLOGO -DEBUG verify01_small.dasm.il
    "$CORE_ROOT/ilasm" -output=verify01_small.asm.exe -QUIET -NOLOGO -DEBUG verify01_small.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=verify01_small
    if [ ! -f verify01_small.org ]; then
        TakeLock
        if [ ! -f verify01_small.org ]; then
          mkdir IL
          cp verify01_small.exe IL/verify01_small.exe
          mv verify01_small.exe verify01_small.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in verify01_small.org /out verify01_small.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in verify01_small.org /out verify01_small.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in verify01_small.reflect.xml
  if [ -f verify01_small.reflect.xml ]
then
    ReflectionRoots="-x verify01_small.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/verify01_small.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" verify01_small.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" verify01_small.asm.exe $CLRTestExecutionArguments
