...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\localloc\verify\verify01_small verify01_small.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\localloc\verify\verify01_small verify01_small.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=verify01_small.dasm.il verify01_small.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=verify01_small.dasm.il verify01_small.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=verify01_small.asm.exe -QUIET -NOLOGO -DEBUG verify01_small.dasm.il
    %CORE_ROOT%\ilasm.exe /output=verify01_small.asm.exe -QUIET -NOLOGO -DEBUG verify01_small.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=verify01_small
    if not exist "verify01_small.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "verify01_small.org" (
            mkdir IL
            copy verify01_small.exe IL\verify01_small.exe 
            ren verify01_small.exe verify01_small.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in verify01_small.org /out verify01_small.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in verify01_small.org /out verify01_small.exe
...
set ExePath=verify01_small.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in verify01_small.reflect.xml
    IF EXIST verify01_small.reflect.xml set ReflectionRoots=-x verify01_small.reflect.xml
    ...
    set ExePath=%LinkBin%\verify01_small.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% verify01_small.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% verify01_small.asm.exe %CLRTestExecutionArguments% %Host_Args%
