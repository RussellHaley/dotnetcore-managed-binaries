...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/localloc/verify/verify01_large verify01_large.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/localloc/verify/verify01_large verify01_large.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=verify01_large.dasm.il verify01_large.exe
    "$CORE_ROOT/ildasm" -raweh -out=verify01_large.dasm.il verify01_large.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=verify01_large.asm.exe -QUIET -NOLOGO -DEBUG verify01_large.dasm.il
    "$CORE_ROOT/ilasm" -output=verify01_large.asm.exe -QUIET -NOLOGO -DEBUG verify01_large.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=verify01_large
    if [ ! -f verify01_large.org ]; then
        TakeLock
        if [ ! -f verify01_large.org ]; then
          mkdir IL
          cp verify01_large.exe IL/verify01_large.exe
          mv verify01_large.exe verify01_large.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in verify01_large.org /out verify01_large.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in verify01_large.org /out verify01_large.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in verify01_large.reflect.xml
  if [ -f verify01_large.reflect.xml ]
then
    ReflectionRoots="-x verify01_large.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/verify01_large.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" verify01_large.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" verify01_large.asm.exe $CLRTestExecutionArguments
