...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/localloc/verify/verify01_dynamic verify01_dynamic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/localloc/verify/verify01_dynamic verify01_dynamic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=verify01_dynamic.dasm.il verify01_dynamic.exe
    "$CORE_ROOT/ildasm" -raweh -out=verify01_dynamic.dasm.il verify01_dynamic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=verify01_dynamic.asm.exe -QUIET -NOLOGO -DEBUG verify01_dynamic.dasm.il
    "$CORE_ROOT/ilasm" -output=verify01_dynamic.asm.exe -QUIET -NOLOGO -DEBUG verify01_dynamic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=verify01_dynamic
    if [ ! -f verify01_dynamic.org ]; then
        TakeLock
        if [ ! -f verify01_dynamic.org ]; then
          mkdir IL
          cp verify01_dynamic.exe IL/verify01_dynamic.exe
          mv verify01_dynamic.exe verify01_dynamic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in verify01_dynamic.org /out verify01_dynamic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in verify01_dynamic.org /out verify01_dynamic.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in verify01_dynamic.reflect.xml
  if [ -f verify01_dynamic.reflect.xml ]
then
    ReflectionRoots="-x verify01_dynamic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/verify01_dynamic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" verify01_dynamic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" verify01_dynamic.asm.exe $CLRTestExecutionArguments
