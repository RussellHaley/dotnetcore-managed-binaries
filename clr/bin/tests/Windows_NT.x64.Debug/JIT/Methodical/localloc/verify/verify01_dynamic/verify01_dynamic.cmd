...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\localloc\verify\verify01_dynamic verify01_dynamic.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\localloc\verify\verify01_dynamic verify01_dynamic.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=verify01_dynamic.dasm.il verify01_dynamic.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=verify01_dynamic.dasm.il verify01_dynamic.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=verify01_dynamic.asm.exe -QUIET -NOLOGO -DEBUG verify01_dynamic.dasm.il
    %CORE_ROOT%\ilasm.exe /output=verify01_dynamic.asm.exe -QUIET -NOLOGO -DEBUG verify01_dynamic.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=verify01_dynamic
    if not exist "verify01_dynamic.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "verify01_dynamic.org" (
            mkdir IL
            copy verify01_dynamic.exe IL\verify01_dynamic.exe 
            ren verify01_dynamic.exe verify01_dynamic.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in verify01_dynamic.org /out verify01_dynamic.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in verify01_dynamic.org /out verify01_dynamic.exe
...
set ExePath=verify01_dynamic.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in verify01_dynamic.reflect.xml
    IF EXIST verify01_dynamic.reflect.xml set ReflectionRoots=-x verify01_dynamic.reflect.xml
    ...
    set ExePath=%LinkBin%\verify01_dynamic.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% verify01_dynamic.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% verify01_dynamic.asm.exe %CLRTestExecutionArguments% %Host_Args%
