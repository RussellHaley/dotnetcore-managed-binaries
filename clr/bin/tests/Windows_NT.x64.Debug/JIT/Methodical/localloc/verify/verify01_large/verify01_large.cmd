...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\localloc\verify\verify01_large verify01_large.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\localloc\verify\verify01_large verify01_large.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=verify01_large.dasm.il verify01_large.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=verify01_large.dasm.il verify01_large.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=verify01_large.asm.exe -QUIET -NOLOGO -DEBUG verify01_large.dasm.il
    %CORE_ROOT%\ilasm.exe /output=verify01_large.asm.exe -QUIET -NOLOGO -DEBUG verify01_large.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=verify01_large
    if not exist "verify01_large.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "verify01_large.org" (
            mkdir IL
            copy verify01_large.exe IL\verify01_large.exe 
            ren verify01_large.exe verify01_large.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in verify01_large.org /out verify01_large.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in verify01_large.org /out verify01_large.exe
...
set ExePath=verify01_large.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in verify01_large.reflect.xml
    IF EXIST verify01_large.reflect.xml set ReflectionRoots=-x verify01_large.reflect.xml
    ...
    set ExePath=%LinkBin%\verify01_large.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% verify01_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% verify01_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
