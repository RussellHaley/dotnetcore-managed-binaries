...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_dbgi_array_merge _il_dbgi_array_merge.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_dbgi_array_merge _il_dbgi_array_merge.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgi_array_merge.dasm.il _il_dbgi_array_merge.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgi_array_merge.dasm.il _il_dbgi_array_merge.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgi_array_merge.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgi_array_merge.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgi_array_merge.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgi_array_merge.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgi_array_merge
    if [ ! -f _il_dbgi_array_merge.org ]; then
        TakeLock
        if [ ! -f _il_dbgi_array_merge.org ]; then
          mkdir IL
          cp _il_dbgi_array_merge.exe IL/_il_dbgi_array_merge.exe
          mv _il_dbgi_array_merge.exe _il_dbgi_array_merge.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgi_array_merge.org /out _il_dbgi_array_merge.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgi_array_merge.org /out _il_dbgi_array_merge.exe
          __cgExitCode=$?
...
ExePath=_il_dbgi_array_merge.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgi_array_merge.reflect.xml
  if [ -f _il_dbgi_array_merge.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgi_array_merge.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgi_array_merge.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgi_array_merge.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgi_array_merge.asm.exe $CLRTestExecutionArguments
