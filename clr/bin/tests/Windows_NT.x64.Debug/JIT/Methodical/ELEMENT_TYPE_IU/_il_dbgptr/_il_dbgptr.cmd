...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgptr.dasm.il _il_dbgptr.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgptr.dasm.il _il_dbgptr.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgptr.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgptr.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgptr.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgptr.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgptr
    if not exist "_il_dbgptr.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgptr.org" (
            mkdir IL
            copy _il_dbgptr.exe IL\_il_dbgptr.exe 
            ren _il_dbgptr.exe _il_dbgptr.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgptr.org /out _il_dbgptr.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgptr.org /out _il_dbgptr.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgptr.reflect.xml
    IF EXIST _il_dbgptr.reflect.xml set ReflectionRoots=-x _il_dbgptr.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgptr.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgptr.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgptr.asm.exe %CLRTestExecutionArguments% %Host_Args%
