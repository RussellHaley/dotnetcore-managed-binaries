...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_dbgptr _il_dbgptr.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_dbgptr _il_dbgptr.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgptr.dasm.il _il_dbgptr.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgptr.dasm.il _il_dbgptr.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgptr.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgptr.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgptr.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgptr.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgptr
    if [ ! -f _il_dbgptr.org ]; then
        TakeLock
        if [ ! -f _il_dbgptr.org ]; then
          mkdir IL
          cp _il_dbgptr.exe IL/_il_dbgptr.exe
          mv _il_dbgptr.exe _il_dbgptr.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgptr.org /out _il_dbgptr.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgptr.org /out _il_dbgptr.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgptr.reflect.xml
  if [ -f _il_dbgptr.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgptr.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgptr.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgptr.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgptr.asm.exe $CLRTestExecutionArguments
