...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_dbgu_fld _il_dbgu_fld.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_dbgu_fld _il_dbgu_fld.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgu_fld.dasm.il _il_dbgu_fld.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgu_fld.dasm.il _il_dbgu_fld.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgu_fld.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgu_fld.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgu_fld.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgu_fld.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgu_fld
    if [ ! -f _il_dbgu_fld.org ]; then
        TakeLock
        if [ ! -f _il_dbgu_fld.org ]; then
          mkdir IL
          cp _il_dbgu_fld.exe IL/_il_dbgu_fld.exe
          mv _il_dbgu_fld.exe _il_dbgu_fld.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgu_fld.org /out _il_dbgu_fld.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgu_fld.org /out _il_dbgu_fld.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgu_fld.reflect.xml
  if [ -f _il_dbgu_fld.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgu_fld.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgu_fld.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgu_fld.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgu_fld.asm.exe $CLRTestExecutionArguments
