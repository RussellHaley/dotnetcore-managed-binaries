...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_dbgconv_i8_i _il_dbgconv_i8_i.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_dbgconv_i8_i _il_dbgconv_i8_i.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgconv_i8_i.dasm.il _il_dbgconv_i8_i.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgconv_i8_i.dasm.il _il_dbgconv_i8_i.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgconv_i8_i.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgconv_i8_i.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgconv_i8_i.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgconv_i8_i.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgconv_i8_i
    if [ ! -f _il_dbgconv_i8_i.org ]; then
        TakeLock
        if [ ! -f _il_dbgconv_i8_i.org ]; then
          mkdir IL
          cp _il_dbgconv_i8_i.exe IL/_il_dbgconv_i8_i.exe
          mv _il_dbgconv_i8_i.exe _il_dbgconv_i8_i.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgconv_i8_i.org /out _il_dbgconv_i8_i.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgconv_i8_i.org /out _il_dbgconv_i8_i.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgconv_i8_i.reflect.xml
  if [ -f _il_dbgconv_i8_i.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgconv_i8_i.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgconv_i8_i.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgconv_i8_i.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgconv_i8_i.asm.exe $CLRTestExecutionArguments
