...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_relptr _il_relptr.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/ELEMENT_TYPE_IU/_il_relptr _il_relptr.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relptr.dasm.il _il_relptr.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relptr.dasm.il _il_relptr.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relptr.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relptr.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relptr.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relptr.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relptr
    if [ ! -f _il_relptr.org ]; then
        TakeLock
        if [ ! -f _il_relptr.org ]; then
          mkdir IL
          cp _il_relptr.exe IL/_il_relptr.exe
          mv _il_relptr.exe _il_relptr.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relptr.org /out _il_relptr.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relptr.org /out _il_relptr.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relptr.reflect.xml
  if [ -f _il_relptr.reflect.xml ]
then
    ReflectionRoots="-x _il_relptr.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relptr.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relptr.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relptr.asm.exe $CLRTestExecutionArguments
