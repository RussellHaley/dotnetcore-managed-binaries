...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\ELEMENT_TYPE_IU\_il_relptr _il_relptr.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\ELEMENT_TYPE_IU\_il_relptr _il_relptr.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relptr.dasm.il _il_relptr.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relptr.dasm.il _il_relptr.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relptr.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relptr.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relptr.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relptr.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relptr
    if not exist "_il_relptr.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relptr.org" (
            mkdir IL
            copy _il_relptr.exe IL\_il_relptr.exe 
            ren _il_relptr.exe _il_relptr.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relptr.org /out _il_relptr.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relptr.org /out _il_relptr.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relptr.reflect.xml
    IF EXIST _il_relptr.reflect.xml set ReflectionRoots=-x _il_relptr.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relptr.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relptr.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relptr.asm.exe %CLRTestExecutionArguments% %Host_Args%
