...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_relrotarg_valref _relrotarg_valref.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_relrotarg_valref _relrotarg_valref.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relrotarg_valref.dasm.il _relrotarg_valref.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relrotarg_valref.dasm.il _relrotarg_valref.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relrotarg_valref.asm.exe  _relrotarg_valref.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relrotarg_valref.asm.exe  _relrotarg_valref.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relrotarg_valref
    if not exist "_relrotarg_valref.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relrotarg_valref.org" (
            mkdir IL
            copy _relrotarg_valref.exe IL\_relrotarg_valref.exe 
            ren _relrotarg_valref.exe _relrotarg_valref.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relrotarg_valref.org /out _relrotarg_valref.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relrotarg_valref.org /out _relrotarg_valref.exe
...
set ExePath=_relrotarg_valref.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relrotarg_valref.reflect.xml
    IF EXIST _relrotarg_valref.reflect.xml set ReflectionRoots=-x _relrotarg_valref.reflect.xml
    ...
    set ExePath=%LinkBin%\_relrotarg_valref.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relrotarg_valref.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relrotarg_valref.asm.exe %CLRTestExecutionArguments% %Host_Args%
