...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_relrotarg_float _relrotarg_float.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_relrotarg_float _relrotarg_float.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relrotarg_float.dasm.il _relrotarg_float.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relrotarg_float.dasm.il _relrotarg_float.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relrotarg_float.asm.exe  _relrotarg_float.dasm.il
    "$CORE_ROOT/ilasm" -output=_relrotarg_float.asm.exe  _relrotarg_float.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relrotarg_float
    if [ ! -f _relrotarg_float.org ]; then
        TakeLock
        if [ ! -f _relrotarg_float.org ]; then
          mkdir IL
          cp _relrotarg_float.exe IL/_relrotarg_float.exe
          mv _relrotarg_float.exe _relrotarg_float.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrotarg_float.org /out _relrotarg_float.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrotarg_float.org /out _relrotarg_float.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relrotarg_float.reflect.xml
  if [ -f _relrotarg_float.reflect.xml ]
then
    ReflectionRoots="-x _relrotarg_float.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relrotarg_float.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relrotarg_float.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relrotarg_float.asm.exe $CLRTestExecutionArguments
