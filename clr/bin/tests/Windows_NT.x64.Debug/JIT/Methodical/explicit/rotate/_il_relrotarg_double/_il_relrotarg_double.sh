...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_il_relrotarg_double _il_relrotarg_double.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_il_relrotarg_double _il_relrotarg_double.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relrotarg_double.dasm.il _il_relrotarg_double.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relrotarg_double.dasm.il _il_relrotarg_double.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relrotarg_double.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrotarg_double.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relrotarg_double.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrotarg_double.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relrotarg_double
    if [ ! -f _il_relrotarg_double.org ]; then
        TakeLock
        if [ ! -f _il_relrotarg_double.org ]; then
          mkdir IL
          cp _il_relrotarg_double.exe IL/_il_relrotarg_double.exe
          mv _il_relrotarg_double.exe _il_relrotarg_double.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relrotarg_double.org /out _il_relrotarg_double.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relrotarg_double.org /out _il_relrotarg_double.exe
          __cgExitCode=$?
...
ExePath=_il_relrotarg_double.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relrotarg_double.reflect.xml
  if [ -f _il_relrotarg_double.reflect.xml ]
then
    ReflectionRoots="-x _il_relrotarg_double.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relrotarg_double.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relrotarg_double.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relrotarg_double.asm.exe $CLRTestExecutionArguments
