...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_il_dbgrotarg_double _il_dbgrotarg_double.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_il_dbgrotarg_double _il_dbgrotarg_double.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrotarg_double.dasm.il _il_dbgrotarg_double.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrotarg_double.dasm.il _il_dbgrotarg_double.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgrotarg_double.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrotarg_double.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgrotarg_double.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrotarg_double.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgrotarg_double
    if not exist "_il_dbgrotarg_double.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgrotarg_double.org" (
            mkdir IL
            copy _il_dbgrotarg_double.exe IL\_il_dbgrotarg_double.exe 
            ren _il_dbgrotarg_double.exe _il_dbgrotarg_double.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrotarg_double.org /out _il_dbgrotarg_double.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrotarg_double.org /out _il_dbgrotarg_double.exe
...
set ExePath=_il_dbgrotarg_double.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgrotarg_double.reflect.xml
    IF EXIST _il_dbgrotarg_double.reflect.xml set ReflectionRoots=-x _il_dbgrotarg_double.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgrotarg_double.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgrotarg_double.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgrotarg_double.asm.exe %CLRTestExecutionArguments% %Host_Args%
