...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_il_relrotarg_objref _il_relrotarg_objref.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_il_relrotarg_objref _il_relrotarg_objref.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relrotarg_objref.dasm.il _il_relrotarg_objref.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relrotarg_objref.dasm.il _il_relrotarg_objref.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relrotarg_objref.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrotarg_objref.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relrotarg_objref.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrotarg_objref.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relrotarg_objref
    if not exist "_il_relrotarg_objref.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relrotarg_objref.org" (
            mkdir IL
            copy _il_relrotarg_objref.exe IL\_il_relrotarg_objref.exe 
            ren _il_relrotarg_objref.exe _il_relrotarg_objref.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relrotarg_objref.org /out _il_relrotarg_objref.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relrotarg_objref.org /out _il_relrotarg_objref.exe
...
set ExePath=_il_relrotarg_objref.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relrotarg_objref.reflect.xml
    IF EXIST _il_relrotarg_objref.reflect.xml set ReflectionRoots=-x _il_relrotarg_objref.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relrotarg_objref.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relrotarg_objref.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relrotarg_objref.asm.exe %CLRTestExecutionArguments% %Host_Args%
