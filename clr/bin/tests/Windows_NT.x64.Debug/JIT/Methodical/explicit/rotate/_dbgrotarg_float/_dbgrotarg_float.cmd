...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_dbgrotarg_float _dbgrotarg_float.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_dbgrotarg_float _dbgrotarg_float.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgrotarg_float.dasm.il _dbgrotarg_float.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgrotarg_float.dasm.il _dbgrotarg_float.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgrotarg_float.asm.exe  _dbgrotarg_float.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgrotarg_float.asm.exe  _dbgrotarg_float.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgrotarg_float
    if not exist "_dbgrotarg_float.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgrotarg_float.org" (
            mkdir IL
            copy _dbgrotarg_float.exe IL\_dbgrotarg_float.exe 
            ren _dbgrotarg_float.exe _dbgrotarg_float.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgrotarg_float.org /out _dbgrotarg_float.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgrotarg_float.org /out _dbgrotarg_float.exe
...
set ExePath=_dbgrotarg_float.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgrotarg_float.reflect.xml
    IF EXIST _dbgrotarg_float.reflect.xml set ReflectionRoots=-x _dbgrotarg_float.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgrotarg_float.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgrotarg_float.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgrotarg_float.asm.exe %CLRTestExecutionArguments% %Host_Args%
