...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_dbgrotarg_double _dbgrotarg_double.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_dbgrotarg_double _dbgrotarg_double.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgrotarg_double.dasm.il _dbgrotarg_double.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgrotarg_double.dasm.il _dbgrotarg_double.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgrotarg_double.asm.exe  _dbgrotarg_double.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgrotarg_double.asm.exe  _dbgrotarg_double.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgrotarg_double
    if [ ! -f _dbgrotarg_double.org ]; then
        TakeLock
        if [ ! -f _dbgrotarg_double.org ]; then
          mkdir IL
          cp _dbgrotarg_double.exe IL/_dbgrotarg_double.exe
          mv _dbgrotarg_double.exe _dbgrotarg_double.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgrotarg_double.org /out _dbgrotarg_double.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgrotarg_double.org /out _dbgrotarg_double.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgrotarg_double.reflect.xml
  if [ -f _dbgrotarg_double.reflect.xml ]
then
    ReflectionRoots="-x _dbgrotarg_double.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgrotarg_double.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgrotarg_double.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgrotarg_double.asm.exe $CLRTestExecutionArguments
