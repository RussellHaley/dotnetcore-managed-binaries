...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_opt_relrotarg_valref _opt_relrotarg_valref.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_opt_relrotarg_valref _opt_relrotarg_valref.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_opt_relrotarg_valref.dasm.il _opt_relrotarg_valref.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_opt_relrotarg_valref.dasm.il _opt_relrotarg_valref.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_opt_relrotarg_valref.asm.exe  _opt_relrotarg_valref.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_opt_relrotarg_valref.asm.exe  _opt_relrotarg_valref.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_opt_relrotarg_valref
    if not exist "_opt_relrotarg_valref.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_opt_relrotarg_valref.org" (
            mkdir IL
            copy _opt_relrotarg_valref.exe IL\_opt_relrotarg_valref.exe 
            ren _opt_relrotarg_valref.exe _opt_relrotarg_valref.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _opt_relrotarg_valref.org /out _opt_relrotarg_valref.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _opt_relrotarg_valref.org /out _opt_relrotarg_valref.exe
...
set ExePath=_opt_relrotarg_valref.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _opt_relrotarg_valref.reflect.xml
    IF EXIST _opt_relrotarg_valref.reflect.xml set ReflectionRoots=-x _opt_relrotarg_valref.reflect.xml
    ...
    set ExePath=%LinkBin%\_opt_relrotarg_valref.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _opt_relrotarg_valref.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _opt_relrotarg_valref.asm.exe %CLRTestExecutionArguments% %Host_Args%
