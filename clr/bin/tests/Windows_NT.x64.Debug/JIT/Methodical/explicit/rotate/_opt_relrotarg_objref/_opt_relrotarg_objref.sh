...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_opt_relrotarg_objref _opt_relrotarg_objref.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_opt_relrotarg_objref _opt_relrotarg_objref.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_opt_relrotarg_objref.dasm.il _opt_relrotarg_objref.exe
    "$CORE_ROOT/ildasm" -raweh -out=_opt_relrotarg_objref.dasm.il _opt_relrotarg_objref.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_opt_relrotarg_objref.asm.exe  _opt_relrotarg_objref.dasm.il
    "$CORE_ROOT/ilasm" -output=_opt_relrotarg_objref.asm.exe  _opt_relrotarg_objref.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_opt_relrotarg_objref
    if [ ! -f _opt_relrotarg_objref.org ]; then
        TakeLock
        if [ ! -f _opt_relrotarg_objref.org ]; then
          mkdir IL
          cp _opt_relrotarg_objref.exe IL/_opt_relrotarg_objref.exe
          mv _opt_relrotarg_objref.exe _opt_relrotarg_objref.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _opt_relrotarg_objref.org /out _opt_relrotarg_objref.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _opt_relrotarg_objref.org /out _opt_relrotarg_objref.exe
          __cgExitCode=$?
...
ExePath=_opt_relrotarg_objref.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _opt_relrotarg_objref.reflect.xml
  if [ -f _opt_relrotarg_objref.reflect.xml ]
then
    ReflectionRoots="-x _opt_relrotarg_objref.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_opt_relrotarg_objref.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _opt_relrotarg_objref.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _opt_relrotarg_objref.asm.exe $CLRTestExecutionArguments
