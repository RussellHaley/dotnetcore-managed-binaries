...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_relrotarg_valref _relrotarg_valref.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_relrotarg_valref _relrotarg_valref.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relrotarg_valref.dasm.il _relrotarg_valref.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relrotarg_valref.dasm.il _relrotarg_valref.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relrotarg_valref.asm.exe  _relrotarg_valref.dasm.il
    "$CORE_ROOT/ilasm" -output=_relrotarg_valref.asm.exe  _relrotarg_valref.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relrotarg_valref
    if [ ! -f _relrotarg_valref.org ]; then
        TakeLock
        if [ ! -f _relrotarg_valref.org ]; then
          mkdir IL
          cp _relrotarg_valref.exe IL/_relrotarg_valref.exe
          mv _relrotarg_valref.exe _relrotarg_valref.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrotarg_valref.org /out _relrotarg_valref.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrotarg_valref.org /out _relrotarg_valref.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relrotarg_valref.reflect.xml
  if [ -f _relrotarg_valref.reflect.xml ]
then
    ReflectionRoots="-x _relrotarg_valref.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relrotarg_valref.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relrotarg_valref.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relrotarg_valref.asm.exe $CLRTestExecutionArguments
