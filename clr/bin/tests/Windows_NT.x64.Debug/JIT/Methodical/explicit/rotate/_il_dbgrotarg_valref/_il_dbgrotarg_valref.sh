...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_il_dbgrotarg_valref _il_dbgrotarg_valref.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_il_dbgrotarg_valref _il_dbgrotarg_valref.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrotarg_valref.dasm.il _il_dbgrotarg_valref.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrotarg_valref.dasm.il _il_dbgrotarg_valref.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgrotarg_valref.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrotarg_valref.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgrotarg_valref.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrotarg_valref.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgrotarg_valref
    if [ ! -f _il_dbgrotarg_valref.org ]; then
        TakeLock
        if [ ! -f _il_dbgrotarg_valref.org ]; then
          mkdir IL
          cp _il_dbgrotarg_valref.exe IL/_il_dbgrotarg_valref.exe
          mv _il_dbgrotarg_valref.exe _il_dbgrotarg_valref.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrotarg_valref.org /out _il_dbgrotarg_valref.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrotarg_valref.org /out _il_dbgrotarg_valref.exe
          __cgExitCode=$?
...
ExePath=_il_dbgrotarg_valref.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgrotarg_valref.reflect.xml
  if [ -f _il_dbgrotarg_valref.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgrotarg_valref.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgrotarg_valref.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgrotarg_valref.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgrotarg_valref.asm.exe $CLRTestExecutionArguments
