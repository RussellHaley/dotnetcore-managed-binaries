...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_il_dbgrotarg_valref _il_dbgrotarg_valref.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_il_dbgrotarg_valref _il_dbgrotarg_valref.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrotarg_valref.dasm.il _il_dbgrotarg_valref.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrotarg_valref.dasm.il _il_dbgrotarg_valref.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgrotarg_valref.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrotarg_valref.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgrotarg_valref.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrotarg_valref.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgrotarg_valref
    if not exist "_il_dbgrotarg_valref.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgrotarg_valref.org" (
            mkdir IL
            copy _il_dbgrotarg_valref.exe IL\_il_dbgrotarg_valref.exe 
            ren _il_dbgrotarg_valref.exe _il_dbgrotarg_valref.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrotarg_valref.org /out _il_dbgrotarg_valref.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrotarg_valref.org /out _il_dbgrotarg_valref.exe
...
set ExePath=_il_dbgrotarg_valref.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgrotarg_valref.reflect.xml
    IF EXIST _il_dbgrotarg_valref.reflect.xml set ReflectionRoots=-x _il_dbgrotarg_valref.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgrotarg_valref.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgrotarg_valref.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgrotarg_valref.asm.exe %CLRTestExecutionArguments% %Host_Args%
