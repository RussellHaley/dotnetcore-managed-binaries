...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_opt_dbgrotarg_float _opt_dbgrotarg_float.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\rotate\_opt_dbgrotarg_float _opt_dbgrotarg_float.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_opt_dbgrotarg_float.dasm.il _opt_dbgrotarg_float.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_opt_dbgrotarg_float.dasm.il _opt_dbgrotarg_float.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_opt_dbgrotarg_float.asm.exe  _opt_dbgrotarg_float.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_opt_dbgrotarg_float.asm.exe  _opt_dbgrotarg_float.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_opt_dbgrotarg_float
    if not exist "_opt_dbgrotarg_float.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_opt_dbgrotarg_float.org" (
            mkdir IL
            copy _opt_dbgrotarg_float.exe IL\_opt_dbgrotarg_float.exe 
            ren _opt_dbgrotarg_float.exe _opt_dbgrotarg_float.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _opt_dbgrotarg_float.org /out _opt_dbgrotarg_float.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _opt_dbgrotarg_float.org /out _opt_dbgrotarg_float.exe
...
set ExePath=_opt_dbgrotarg_float.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _opt_dbgrotarg_float.reflect.xml
    IF EXIST _opt_dbgrotarg_float.reflect.xml set ReflectionRoots=-x _opt_dbgrotarg_float.reflect.xml
    ...
    set ExePath=%LinkBin%\_opt_dbgrotarg_float.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _opt_dbgrotarg_float.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _opt_dbgrotarg_float.asm.exe %CLRTestExecutionArguments% %Host_Args%
