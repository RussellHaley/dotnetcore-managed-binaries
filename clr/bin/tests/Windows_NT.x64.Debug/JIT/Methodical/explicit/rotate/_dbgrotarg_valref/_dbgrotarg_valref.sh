...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_dbgrotarg_valref _dbgrotarg_valref.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/rotate/_dbgrotarg_valref _dbgrotarg_valref.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgrotarg_valref.dasm.il _dbgrotarg_valref.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgrotarg_valref.dasm.il _dbgrotarg_valref.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgrotarg_valref.asm.exe  _dbgrotarg_valref.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgrotarg_valref.asm.exe  _dbgrotarg_valref.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgrotarg_valref
    if [ ! -f _dbgrotarg_valref.org ]; then
        TakeLock
        if [ ! -f _dbgrotarg_valref.org ]; then
          mkdir IL
          cp _dbgrotarg_valref.exe IL/_dbgrotarg_valref.exe
          mv _dbgrotarg_valref.exe _dbgrotarg_valref.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgrotarg_valref.org /out _dbgrotarg_valref.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgrotarg_valref.org /out _dbgrotarg_valref.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgrotarg_valref.reflect.xml
  if [ -f _dbgrotarg_valref.reflect.xml ]
then
    ReflectionRoots="-x _dbgrotarg_valref.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgrotarg_valref.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgrotarg_valref.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgrotarg_valref.asm.exe $CLRTestExecutionArguments
