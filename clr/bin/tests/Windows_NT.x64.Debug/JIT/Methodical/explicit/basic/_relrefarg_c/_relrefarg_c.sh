...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_relrefarg_c _relrefarg_c.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_relrefarg_c _relrefarg_c.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relrefarg_c.dasm.il _relrefarg_c.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relrefarg_c.dasm.il _relrefarg_c.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relrefarg_c.asm.exe  _relrefarg_c.dasm.il
    "$CORE_ROOT/ilasm" -output=_relrefarg_c.asm.exe  _relrefarg_c.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relrefarg_c
    if [ ! -f _relrefarg_c.org ]; then
        TakeLock
        if [ ! -f _relrefarg_c.org ]; then
          mkdir IL
          cp _relrefarg_c.exe IL/_relrefarg_c.exe
          mv _relrefarg_c.exe _relrefarg_c.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrefarg_c.org /out _relrefarg_c.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrefarg_c.org /out _relrefarg_c.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relrefarg_c.reflect.xml
  if [ -f _relrefarg_c.reflect.xml ]
then
    ReflectionRoots="-x _relrefarg_c.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relrefarg_c.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relrefarg_c.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relrefarg_c.asm.exe $CLRTestExecutionArguments
