...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_il_dbgrefarg_c _il_dbgrefarg_c.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_il_dbgrefarg_c _il_dbgrefarg_c.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrefarg_c.dasm.il _il_dbgrefarg_c.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrefarg_c.dasm.il _il_dbgrefarg_c.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgrefarg_c.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefarg_c.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgrefarg_c.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefarg_c.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgrefarg_c
    if not exist "_il_dbgrefarg_c.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgrefarg_c.org" (
            mkdir IL
            copy _il_dbgrefarg_c.exe IL\_il_dbgrefarg_c.exe 
            ren _il_dbgrefarg_c.exe _il_dbgrefarg_c.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrefarg_c.org /out _il_dbgrefarg_c.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrefarg_c.org /out _il_dbgrefarg_c.exe
...
set ExePath=_il_dbgrefarg_c.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgrefarg_c.reflect.xml
    IF EXIST _il_dbgrefarg_c.reflect.xml set ReflectionRoots=-x _il_dbgrefarg_c.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgrefarg_c.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgrefarg_c.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgrefarg_c.asm.exe %CLRTestExecutionArguments% %Host_Args%
