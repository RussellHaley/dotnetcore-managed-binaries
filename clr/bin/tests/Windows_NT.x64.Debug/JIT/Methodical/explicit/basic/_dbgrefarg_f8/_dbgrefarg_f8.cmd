...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_dbgrefarg_f8 _dbgrefarg_f8.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_dbgrefarg_f8 _dbgrefarg_f8.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgrefarg_f8.dasm.il _dbgrefarg_f8.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgrefarg_f8.dasm.il _dbgrefarg_f8.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgrefarg_f8.asm.exe  _dbgrefarg_f8.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgrefarg_f8.asm.exe  _dbgrefarg_f8.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgrefarg_f8
    if not exist "_dbgrefarg_f8.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgrefarg_f8.org" (
            mkdir IL
            copy _dbgrefarg_f8.exe IL\_dbgrefarg_f8.exe 
            ren _dbgrefarg_f8.exe _dbgrefarg_f8.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgrefarg_f8.org /out _dbgrefarg_f8.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgrefarg_f8.org /out _dbgrefarg_f8.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgrefarg_f8.reflect.xml
    IF EXIST _dbgrefarg_f8.reflect.xml set ReflectionRoots=-x _dbgrefarg_f8.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgrefarg_f8.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgrefarg_f8.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgrefarg_f8.asm.exe %CLRTestExecutionArguments% %Host_Args%
