...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_dbgrefarg_s _dbgrefarg_s.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_dbgrefarg_s _dbgrefarg_s.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgrefarg_s.dasm.il _dbgrefarg_s.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgrefarg_s.dasm.il _dbgrefarg_s.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgrefarg_s.asm.exe  _dbgrefarg_s.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgrefarg_s.asm.exe  _dbgrefarg_s.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgrefarg_s
    if [ ! -f _dbgrefarg_s.org ]; then
        TakeLock
        if [ ! -f _dbgrefarg_s.org ]; then
          mkdir IL
          cp _dbgrefarg_s.exe IL/_dbgrefarg_s.exe
          mv _dbgrefarg_s.exe _dbgrefarg_s.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgrefarg_s.org /out _dbgrefarg_s.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgrefarg_s.org /out _dbgrefarg_s.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgrefarg_s.reflect.xml
  if [ -f _dbgrefarg_s.reflect.xml ]
then
    ReflectionRoots="-x _dbgrefarg_s.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgrefarg_s.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgrefarg_s.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgrefarg_s.asm.exe $CLRTestExecutionArguments
