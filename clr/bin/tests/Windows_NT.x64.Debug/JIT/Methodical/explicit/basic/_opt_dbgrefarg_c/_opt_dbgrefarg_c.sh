...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_opt_dbgrefarg_c _opt_dbgrefarg_c.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_opt_dbgrefarg_c _opt_dbgrefarg_c.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_opt_dbgrefarg_c.dasm.il _opt_dbgrefarg_c.exe
    "$CORE_ROOT/ildasm" -raweh -out=_opt_dbgrefarg_c.dasm.il _opt_dbgrefarg_c.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_opt_dbgrefarg_c.asm.exe  _opt_dbgrefarg_c.dasm.il
    "$CORE_ROOT/ilasm" -output=_opt_dbgrefarg_c.asm.exe  _opt_dbgrefarg_c.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_opt_dbgrefarg_c
    if [ ! -f _opt_dbgrefarg_c.org ]; then
        TakeLock
        if [ ! -f _opt_dbgrefarg_c.org ]; then
          mkdir IL
          cp _opt_dbgrefarg_c.exe IL/_opt_dbgrefarg_c.exe
          mv _opt_dbgrefarg_c.exe _opt_dbgrefarg_c.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _opt_dbgrefarg_c.org /out _opt_dbgrefarg_c.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _opt_dbgrefarg_c.org /out _opt_dbgrefarg_c.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _opt_dbgrefarg_c.reflect.xml
  if [ -f _opt_dbgrefarg_c.reflect.xml ]
then
    ReflectionRoots="-x _opt_dbgrefarg_c.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_opt_dbgrefarg_c.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _opt_dbgrefarg_c.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _opt_dbgrefarg_c.asm.exe $CLRTestExecutionArguments
