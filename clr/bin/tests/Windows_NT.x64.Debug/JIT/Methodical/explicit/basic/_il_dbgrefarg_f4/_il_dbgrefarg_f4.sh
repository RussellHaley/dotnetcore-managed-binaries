...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_il_dbgrefarg_f4 _il_dbgrefarg_f4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_il_dbgrefarg_f4 _il_dbgrefarg_f4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrefarg_f4.dasm.il _il_dbgrefarg_f4.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrefarg_f4.dasm.il _il_dbgrefarg_f4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgrefarg_f4.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefarg_f4.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgrefarg_f4.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefarg_f4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgrefarg_f4
    if [ ! -f _il_dbgrefarg_f4.org ]; then
        TakeLock
        if [ ! -f _il_dbgrefarg_f4.org ]; then
          mkdir IL
          cp _il_dbgrefarg_f4.exe IL/_il_dbgrefarg_f4.exe
          mv _il_dbgrefarg_f4.exe _il_dbgrefarg_f4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrefarg_f4.org /out _il_dbgrefarg_f4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrefarg_f4.org /out _il_dbgrefarg_f4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgrefarg_f4.reflect.xml
  if [ -f _il_dbgrefarg_f4.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgrefarg_f4.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgrefarg_f4.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgrefarg_f4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgrefarg_f4.asm.exe $CLRTestExecutionArguments
