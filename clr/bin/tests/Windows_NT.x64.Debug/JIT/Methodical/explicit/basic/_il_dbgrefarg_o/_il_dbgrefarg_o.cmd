...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_il_dbgrefarg_o _il_dbgrefarg_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_il_dbgrefarg_o _il_dbgrefarg_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrefarg_o.dasm.il _il_dbgrefarg_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrefarg_o.dasm.il _il_dbgrefarg_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgrefarg_o.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefarg_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgrefarg_o.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefarg_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgrefarg_o
    if not exist "_il_dbgrefarg_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgrefarg_o.org" (
            mkdir IL
            copy _il_dbgrefarg_o.exe IL\_il_dbgrefarg_o.exe 
            ren _il_dbgrefarg_o.exe _il_dbgrefarg_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrefarg_o.org /out _il_dbgrefarg_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrefarg_o.org /out _il_dbgrefarg_o.exe
...
set ExePath=_il_dbgrefarg_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgrefarg_o.reflect.xml
    IF EXIST _il_dbgrefarg_o.reflect.xml set ReflectionRoots=-x _il_dbgrefarg_o.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgrefarg_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgrefarg_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgrefarg_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
