...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_il_dbgrefarg_s _il_dbgrefarg_s.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_il_dbgrefarg_s _il_dbgrefarg_s.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrefarg_s.dasm.il _il_dbgrefarg_s.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrefarg_s.dasm.il _il_dbgrefarg_s.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgrefarg_s.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefarg_s.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgrefarg_s.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefarg_s.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgrefarg_s
    if not exist "_il_dbgrefarg_s.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgrefarg_s.org" (
            mkdir IL
            copy _il_dbgrefarg_s.exe IL\_il_dbgrefarg_s.exe 
            ren _il_dbgrefarg_s.exe _il_dbgrefarg_s.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrefarg_s.org /out _il_dbgrefarg_s.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrefarg_s.org /out _il_dbgrefarg_s.exe
...
set ExePath=_il_dbgrefarg_s.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgrefarg_s.reflect.xml
    IF EXIST _il_dbgrefarg_s.reflect.xml set ReflectionRoots=-x _il_dbgrefarg_s.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgrefarg_s.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgrefarg_s.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgrefarg_s.asm.exe %CLRTestExecutionArguments% %Host_Args%
