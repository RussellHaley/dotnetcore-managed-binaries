...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_relrefarg_i4 _relrefarg_i4.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_relrefarg_i4 _relrefarg_i4.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relrefarg_i4.dasm.il _relrefarg_i4.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relrefarg_i4.dasm.il _relrefarg_i4.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relrefarg_i4.asm.exe  _relrefarg_i4.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relrefarg_i4.asm.exe  _relrefarg_i4.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relrefarg_i4
    if not exist "_relrefarg_i4.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relrefarg_i4.org" (
            mkdir IL
            copy _relrefarg_i4.exe IL\_relrefarg_i4.exe 
            ren _relrefarg_i4.exe _relrefarg_i4.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relrefarg_i4.org /out _relrefarg_i4.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relrefarg_i4.org /out _relrefarg_i4.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relrefarg_i4.reflect.xml
    IF EXIST _relrefarg_i4.reflect.xml set ReflectionRoots=-x _relrefarg_i4.reflect.xml
    ...
    set ExePath=%LinkBin%\_relrefarg_i4.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relrefarg_i4.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relrefarg_i4.asm.exe %CLRTestExecutionArguments% %Host_Args%
