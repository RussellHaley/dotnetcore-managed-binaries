...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_il_dbgrefloc_o _il_dbgrefloc_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_il_dbgrefloc_o _il_dbgrefloc_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrefloc_o.dasm.il _il_dbgrefloc_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrefloc_o.dasm.il _il_dbgrefloc_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgrefloc_o.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefloc_o.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgrefloc_o.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrefloc_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgrefloc_o
    if [ ! -f _il_dbgrefloc_o.org ]; then
        TakeLock
        if [ ! -f _il_dbgrefloc_o.org ]; then
          mkdir IL
          cp _il_dbgrefloc_o.exe IL/_il_dbgrefloc_o.exe
          mv _il_dbgrefloc_o.exe _il_dbgrefloc_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrefloc_o.org /out _il_dbgrefloc_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrefloc_o.org /out _il_dbgrefloc_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgrefloc_o.reflect.xml
  if [ -f _il_dbgrefloc_o.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgrefloc_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgrefloc_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgrefloc_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgrefloc_o.asm.exe $CLRTestExecutionArguments
