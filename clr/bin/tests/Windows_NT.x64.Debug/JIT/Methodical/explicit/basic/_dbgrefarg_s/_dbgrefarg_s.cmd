...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_dbgrefarg_s _dbgrefarg_s.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\basic\_dbgrefarg_s _dbgrefarg_s.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgrefarg_s.dasm.il _dbgrefarg_s.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgrefarg_s.dasm.il _dbgrefarg_s.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgrefarg_s.asm.exe  _dbgrefarg_s.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgrefarg_s.asm.exe  _dbgrefarg_s.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgrefarg_s
    if not exist "_dbgrefarg_s.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgrefarg_s.org" (
            mkdir IL
            copy _dbgrefarg_s.exe IL\_dbgrefarg_s.exe 
            ren _dbgrefarg_s.exe _dbgrefarg_s.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgrefarg_s.org /out _dbgrefarg_s.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgrefarg_s.org /out _dbgrefarg_s.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgrefarg_s.reflect.xml
    IF EXIST _dbgrefarg_s.reflect.xml set ReflectionRoots=-x _dbgrefarg_s.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgrefarg_s.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgrefarg_s.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgrefarg_s.asm.exe %CLRTestExecutionArguments% %Host_Args%
