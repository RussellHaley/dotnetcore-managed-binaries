...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_relrefarg_o _relrefarg_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_relrefarg_o _relrefarg_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relrefarg_o.dasm.il _relrefarg_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relrefarg_o.dasm.il _relrefarg_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relrefarg_o.asm.exe  _relrefarg_o.dasm.il
    "$CORE_ROOT/ilasm" -output=_relrefarg_o.asm.exe  _relrefarg_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relrefarg_o
    if [ ! -f _relrefarg_o.org ]; then
        TakeLock
        if [ ! -f _relrefarg_o.org ]; then
          mkdir IL
          cp _relrefarg_o.exe IL/_relrefarg_o.exe
          mv _relrefarg_o.exe _relrefarg_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrefarg_o.org /out _relrefarg_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrefarg_o.org /out _relrefarg_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relrefarg_o.reflect.xml
  if [ -f _relrefarg_o.reflect.xml ]
then
    ReflectionRoots="-x _relrefarg_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relrefarg_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relrefarg_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relrefarg_o.asm.exe $CLRTestExecutionArguments
