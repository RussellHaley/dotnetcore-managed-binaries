...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_relrefarg_i2 _relrefarg_i2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_relrefarg_i2 _relrefarg_i2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relrefarg_i2.dasm.il _relrefarg_i2.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relrefarg_i2.dasm.il _relrefarg_i2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relrefarg_i2.asm.exe  _relrefarg_i2.dasm.il
    "$CORE_ROOT/ilasm" -output=_relrefarg_i2.asm.exe  _relrefarg_i2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relrefarg_i2
    if [ ! -f _relrefarg_i2.org ]; then
        TakeLock
        if [ ! -f _relrefarg_i2.org ]; then
          mkdir IL
          cp _relrefarg_i2.exe IL/_relrefarg_i2.exe
          mv _relrefarg_i2.exe _relrefarg_i2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrefarg_i2.org /out _relrefarg_i2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relrefarg_i2.org /out _relrefarg_i2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relrefarg_i2.reflect.xml
  if [ -f _relrefarg_i2.reflect.xml ]
then
    ReflectionRoots="-x _relrefarg_i2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relrefarg_i2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relrefarg_i2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relrefarg_i2.asm.exe $CLRTestExecutionArguments
