...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_opt_dbgrefarg_o _opt_dbgrefarg_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_opt_dbgrefarg_o _opt_dbgrefarg_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_opt_dbgrefarg_o.dasm.il _opt_dbgrefarg_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=_opt_dbgrefarg_o.dasm.il _opt_dbgrefarg_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_opt_dbgrefarg_o.asm.exe  _opt_dbgrefarg_o.dasm.il
    "$CORE_ROOT/ilasm" -output=_opt_dbgrefarg_o.asm.exe  _opt_dbgrefarg_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_opt_dbgrefarg_o
    if [ ! -f _opt_dbgrefarg_o.org ]; then
        TakeLock
        if [ ! -f _opt_dbgrefarg_o.org ]; then
          mkdir IL
          cp _opt_dbgrefarg_o.exe IL/_opt_dbgrefarg_o.exe
          mv _opt_dbgrefarg_o.exe _opt_dbgrefarg_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _opt_dbgrefarg_o.org /out _opt_dbgrefarg_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _opt_dbgrefarg_o.org /out _opt_dbgrefarg_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _opt_dbgrefarg_o.reflect.xml
  if [ -f _opt_dbgrefarg_o.reflect.xml ]
then
    ReflectionRoots="-x _opt_dbgrefarg_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_opt_dbgrefarg_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _opt_dbgrefarg_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _opt_dbgrefarg_o.asm.exe $CLRTestExecutionArguments
