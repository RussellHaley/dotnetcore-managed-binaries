...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_il_relrefarg_c _il_relrefarg_c.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_il_relrefarg_c _il_relrefarg_c.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relrefarg_c.dasm.il _il_relrefarg_c.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relrefarg_c.dasm.il _il_relrefarg_c.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relrefarg_c.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrefarg_c.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relrefarg_c.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrefarg_c.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relrefarg_c
    if [ ! -f _il_relrefarg_c.org ]; then
        TakeLock
        if [ ! -f _il_relrefarg_c.org ]; then
          mkdir IL
          cp _il_relrefarg_c.exe IL/_il_relrefarg_c.exe
          mv _il_relrefarg_c.exe _il_relrefarg_c.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relrefarg_c.org /out _il_relrefarg_c.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relrefarg_c.org /out _il_relrefarg_c.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relrefarg_c.reflect.xml
  if [ -f _il_relrefarg_c.reflect.xml ]
then
    ReflectionRoots="-x _il_relrefarg_c.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relrefarg_c.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relrefarg_c.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relrefarg_c.asm.exe $CLRTestExecutionArguments
