...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_dbgrefarg_f8 _dbgrefarg_f8.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/basic/_dbgrefarg_f8 _dbgrefarg_f8.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgrefarg_f8.dasm.il _dbgrefarg_f8.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgrefarg_f8.dasm.il _dbgrefarg_f8.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgrefarg_f8.asm.exe  _dbgrefarg_f8.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgrefarg_f8.asm.exe  _dbgrefarg_f8.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgrefarg_f8
    if [ ! -f _dbgrefarg_f8.org ]; then
        TakeLock
        if [ ! -f _dbgrefarg_f8.org ]; then
          mkdir IL
          cp _dbgrefarg_f8.exe IL/_dbgrefarg_f8.exe
          mv _dbgrefarg_f8.exe _dbgrefarg_f8.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgrefarg_f8.org /out _dbgrefarg_f8.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgrefarg_f8.org /out _dbgrefarg_f8.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgrefarg_f8.reflect.xml
  if [ -f _dbgrefarg_f8.reflect.xml ]
then
    ReflectionRoots="-x _dbgrefarg_f8.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgrefarg_f8.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgrefarg_f8.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgrefarg_f8.asm.exe $CLRTestExecutionArguments
