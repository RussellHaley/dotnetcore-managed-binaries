...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\coverage\seq_byte_1_d seq_byte_1_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\coverage\seq_byte_1_d seq_byte_1_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=seq_byte_1_d.dasm.il seq_byte_1_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=seq_byte_1_d.dasm.il seq_byte_1_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=seq_byte_1_d.asm.exe  seq_byte_1_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=seq_byte_1_d.asm.exe  seq_byte_1_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=seq_byte_1_d
    if not exist "seq_byte_1_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "seq_byte_1_d.org" (
            mkdir IL
            copy seq_byte_1_d.exe IL\seq_byte_1_d.exe 
            ren seq_byte_1_d.exe seq_byte_1_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in seq_byte_1_d.org /out seq_byte_1_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in seq_byte_1_d.org /out seq_byte_1_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in seq_byte_1_d.reflect.xml
    IF EXIST seq_byte_1_d.reflect.xml set ReflectionRoots=-x seq_byte_1_d.reflect.xml
    ...
    set ExePath=%LinkBin%\seq_byte_1_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% seq_byte_1_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% seq_byte_1_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
