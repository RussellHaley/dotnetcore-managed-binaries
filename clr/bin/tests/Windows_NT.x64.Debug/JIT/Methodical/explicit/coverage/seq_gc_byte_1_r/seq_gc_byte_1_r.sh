...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/coverage/seq_gc_byte_1_r seq_gc_byte_1_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/coverage/seq_gc_byte_1_r seq_gc_byte_1_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=seq_gc_byte_1_r.dasm.il seq_gc_byte_1_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=seq_gc_byte_1_r.dasm.il seq_gc_byte_1_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=seq_gc_byte_1_r.asm.exe  seq_gc_byte_1_r.dasm.il
    "$CORE_ROOT/ilasm" -output=seq_gc_byte_1_r.asm.exe  seq_gc_byte_1_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=seq_gc_byte_1_r
    if [ ! -f seq_gc_byte_1_r.org ]; then
        TakeLock
        if [ ! -f seq_gc_byte_1_r.org ]; then
          mkdir IL
          cp seq_gc_byte_1_r.exe IL/seq_gc_byte_1_r.exe
          mv seq_gc_byte_1_r.exe seq_gc_byte_1_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in seq_gc_byte_1_r.org /out seq_gc_byte_1_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in seq_gc_byte_1_r.org /out seq_gc_byte_1_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in seq_gc_byte_1_r.reflect.xml
  if [ -f seq_gc_byte_1_r.reflect.xml ]
then
    ReflectionRoots="-x seq_gc_byte_1_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/seq_gc_byte_1_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" seq_gc_byte_1_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" seq_gc_byte_1_r.asm.exe $CLRTestExecutionArguments
