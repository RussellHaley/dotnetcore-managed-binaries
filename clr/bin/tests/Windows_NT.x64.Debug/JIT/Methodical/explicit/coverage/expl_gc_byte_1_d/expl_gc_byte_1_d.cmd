...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\coverage\expl_gc_byte_1_d expl_gc_byte_1_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\coverage\expl_gc_byte_1_d expl_gc_byte_1_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=expl_gc_byte_1_d.dasm.il expl_gc_byte_1_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=expl_gc_byte_1_d.dasm.il expl_gc_byte_1_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=expl_gc_byte_1_d.asm.exe  expl_gc_byte_1_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=expl_gc_byte_1_d.asm.exe  expl_gc_byte_1_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=expl_gc_byte_1_d
    if not exist "expl_gc_byte_1_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "expl_gc_byte_1_d.org" (
            mkdir IL
            copy expl_gc_byte_1_d.exe IL\expl_gc_byte_1_d.exe 
            ren expl_gc_byte_1_d.exe expl_gc_byte_1_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in expl_gc_byte_1_d.org /out expl_gc_byte_1_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in expl_gc_byte_1_d.org /out expl_gc_byte_1_d.exe
...
set ExePath=expl_gc_byte_1_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in expl_gc_byte_1_d.reflect.xml
    IF EXIST expl_gc_byte_1_d.reflect.xml set ReflectionRoots=-x expl_gc_byte_1_d.reflect.xml
    ...
    set ExePath=%LinkBin%\expl_gc_byte_1_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% expl_gc_byte_1_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% expl_gc_byte_1_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
