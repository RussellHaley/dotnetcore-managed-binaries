...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\coverage\expl_gc_byte_1_r expl_gc_byte_1_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\coverage\expl_gc_byte_1_r expl_gc_byte_1_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=expl_gc_byte_1_r.dasm.il expl_gc_byte_1_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=expl_gc_byte_1_r.dasm.il expl_gc_byte_1_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=expl_gc_byte_1_r.asm.exe  expl_gc_byte_1_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=expl_gc_byte_1_r.asm.exe  expl_gc_byte_1_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=expl_gc_byte_1_r
    if not exist "expl_gc_byte_1_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "expl_gc_byte_1_r.org" (
            mkdir IL
            copy expl_gc_byte_1_r.exe IL\expl_gc_byte_1_r.exe 
            ren expl_gc_byte_1_r.exe expl_gc_byte_1_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in expl_gc_byte_1_r.org /out expl_gc_byte_1_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in expl_gc_byte_1_r.org /out expl_gc_byte_1_r.exe
...
set ExePath=expl_gc_byte_1_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in expl_gc_byte_1_r.reflect.xml
    IF EXIST expl_gc_byte_1_r.reflect.xml set ReflectionRoots=-x expl_gc_byte_1_r.reflect.xml
    ...
    set ExePath=%LinkBin%\expl_gc_byte_1_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% expl_gc_byte_1_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% expl_gc_byte_1_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
