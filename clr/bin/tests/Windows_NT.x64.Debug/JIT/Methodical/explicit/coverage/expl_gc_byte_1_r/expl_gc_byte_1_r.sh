...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/coverage/expl_gc_byte_1_r expl_gc_byte_1_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/coverage/expl_gc_byte_1_r expl_gc_byte_1_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=expl_gc_byte_1_r.dasm.il expl_gc_byte_1_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=expl_gc_byte_1_r.dasm.il expl_gc_byte_1_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=expl_gc_byte_1_r.asm.exe  expl_gc_byte_1_r.dasm.il
    "$CORE_ROOT/ilasm" -output=expl_gc_byte_1_r.asm.exe  expl_gc_byte_1_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=expl_gc_byte_1_r
    if [ ! -f expl_gc_byte_1_r.org ]; then
        TakeLock
        if [ ! -f expl_gc_byte_1_r.org ]; then
          mkdir IL
          cp expl_gc_byte_1_r.exe IL/expl_gc_byte_1_r.exe
          mv expl_gc_byte_1_r.exe expl_gc_byte_1_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in expl_gc_byte_1_r.org /out expl_gc_byte_1_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in expl_gc_byte_1_r.org /out expl_gc_byte_1_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in expl_gc_byte_1_r.reflect.xml
  if [ -f expl_gc_byte_1_r.reflect.xml ]
then
    ReflectionRoots="-x expl_gc_byte_1_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/expl_gc_byte_1_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" expl_gc_byte_1_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" expl_gc_byte_1_r.asm.exe $CLRTestExecutionArguments
