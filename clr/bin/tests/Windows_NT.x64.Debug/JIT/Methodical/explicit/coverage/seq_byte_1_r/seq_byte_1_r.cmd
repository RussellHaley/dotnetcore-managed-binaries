...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\coverage\seq_byte_1_r seq_byte_1_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\coverage\seq_byte_1_r seq_byte_1_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=seq_byte_1_r.dasm.il seq_byte_1_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=seq_byte_1_r.dasm.il seq_byte_1_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=seq_byte_1_r.asm.exe  seq_byte_1_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=seq_byte_1_r.asm.exe  seq_byte_1_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=seq_byte_1_r
    if not exist "seq_byte_1_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "seq_byte_1_r.org" (
            mkdir IL
            copy seq_byte_1_r.exe IL\seq_byte_1_r.exe 
            ren seq_byte_1_r.exe seq_byte_1_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in seq_byte_1_r.org /out seq_byte_1_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in seq_byte_1_r.org /out seq_byte_1_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in seq_byte_1_r.reflect.xml
    IF EXIST seq_byte_1_r.reflect.xml set ReflectionRoots=-x seq_byte_1_r.reflect.xml
    ...
    set ExePath=%LinkBin%\seq_byte_1_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% seq_byte_1_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% seq_byte_1_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
