...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_dbgexplicit1 _dbgexplicit1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_dbgexplicit1 _dbgexplicit1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgexplicit1.dasm.il _dbgexplicit1.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgexplicit1.dasm.il _dbgexplicit1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgexplicit1.asm.exe  _dbgexplicit1.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgexplicit1.asm.exe  _dbgexplicit1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgexplicit1
    if [ ! -f _dbgexplicit1.org ]; then
        TakeLock
        if [ ! -f _dbgexplicit1.org ]; then
          mkdir IL
          cp _dbgexplicit1.exe IL/_dbgexplicit1.exe
          mv _dbgexplicit1.exe _dbgexplicit1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgexplicit1.org /out _dbgexplicit1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgexplicit1.org /out _dbgexplicit1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgexplicit1.reflect.xml
  if [ -f _dbgexplicit1.reflect.xml ]
then
    ReflectionRoots="-x _dbgexplicit1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgexplicit1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgexplicit1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgexplicit1.asm.exe $CLRTestExecutionArguments
