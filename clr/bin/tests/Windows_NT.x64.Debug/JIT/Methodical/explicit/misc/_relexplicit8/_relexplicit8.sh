...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit8 _relexplicit8.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit8 _relexplicit8.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relexplicit8.dasm.il _relexplicit8.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relexplicit8.dasm.il _relexplicit8.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relexplicit8.asm.exe  _relexplicit8.dasm.il
    "$CORE_ROOT/ilasm" -output=_relexplicit8.asm.exe  _relexplicit8.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relexplicit8
    if [ ! -f _relexplicit8.org ]; then
        TakeLock
        if [ ! -f _relexplicit8.org ]; then
          mkdir IL
          cp _relexplicit8.exe IL/_relexplicit8.exe
          mv _relexplicit8.exe _relexplicit8.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit8.org /out _relexplicit8.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit8.org /out _relexplicit8.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relexplicit8.reflect.xml
  if [ -f _relexplicit8.reflect.xml ]
then
    ReflectionRoots="-x _relexplicit8.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relexplicit8.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relexplicit8.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relexplicit8.asm.exe $CLRTestExecutionArguments
