...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_relexplicit1 _relexplicit1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_relexplicit1 _relexplicit1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relexplicit1.dasm.il _relexplicit1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relexplicit1.dasm.il _relexplicit1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relexplicit1.asm.exe  _relexplicit1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relexplicit1.asm.exe  _relexplicit1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relexplicit1
    if not exist "_relexplicit1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relexplicit1.org" (
            mkdir IL
            copy _relexplicit1.exe IL\_relexplicit1.exe 
            ren _relexplicit1.exe _relexplicit1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relexplicit1.org /out _relexplicit1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relexplicit1.org /out _relexplicit1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relexplicit1.reflect.xml
    IF EXIST _relexplicit1.reflect.xml set ReflectionRoots=-x _relexplicit1.reflect.xml
    ...
    set ExePath=%LinkBin%\_relexplicit1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relexplicit1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relexplicit1.asm.exe %CLRTestExecutionArguments% %Host_Args%
