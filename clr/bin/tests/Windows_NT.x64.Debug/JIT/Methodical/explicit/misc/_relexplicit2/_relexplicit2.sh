...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit2 _relexplicit2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit2 _relexplicit2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relexplicit2.dasm.il _relexplicit2.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relexplicit2.dasm.il _relexplicit2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relexplicit2.asm.exe  _relexplicit2.dasm.il
    "$CORE_ROOT/ilasm" -output=_relexplicit2.asm.exe  _relexplicit2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relexplicit2
    if [ ! -f _relexplicit2.org ]; then
        TakeLock
        if [ ! -f _relexplicit2.org ]; then
          mkdir IL
          cp _relexplicit2.exe IL/_relexplicit2.exe
          mv _relexplicit2.exe _relexplicit2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit2.org /out _relexplicit2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit2.org /out _relexplicit2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relexplicit2.reflect.xml
  if [ -f _relexplicit2.reflect.xml ]
then
    ReflectionRoots="-x _relexplicit2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relexplicit2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relexplicit2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relexplicit2.asm.exe $CLRTestExecutionArguments
