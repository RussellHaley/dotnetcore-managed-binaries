...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit4 _relexplicit4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit4 _relexplicit4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relexplicit4.dasm.il _relexplicit4.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relexplicit4.dasm.il _relexplicit4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relexplicit4.asm.exe  _relexplicit4.dasm.il
    "$CORE_ROOT/ilasm" -output=_relexplicit4.asm.exe  _relexplicit4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relexplicit4
    if [ ! -f _relexplicit4.org ]; then
        TakeLock
        if [ ! -f _relexplicit4.org ]; then
          mkdir IL
          cp _relexplicit4.exe IL/_relexplicit4.exe
          mv _relexplicit4.exe _relexplicit4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit4.org /out _relexplicit4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit4.org /out _relexplicit4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relexplicit4.reflect.xml
  if [ -f _relexplicit4.reflect.xml ]
then
    ReflectionRoots="-x _relexplicit4.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relexplicit4.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relexplicit4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relexplicit4.asm.exe $CLRTestExecutionArguments
