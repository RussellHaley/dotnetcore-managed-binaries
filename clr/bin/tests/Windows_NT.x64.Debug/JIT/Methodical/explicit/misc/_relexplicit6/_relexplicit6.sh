...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit6 _relexplicit6.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit6 _relexplicit6.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relexplicit6.dasm.il _relexplicit6.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relexplicit6.dasm.il _relexplicit6.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relexplicit6.asm.exe  _relexplicit6.dasm.il
    "$CORE_ROOT/ilasm" -output=_relexplicit6.asm.exe  _relexplicit6.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relexplicit6
    if [ ! -f _relexplicit6.org ]; then
        TakeLock
        if [ ! -f _relexplicit6.org ]; then
          mkdir IL
          cp _relexplicit6.exe IL/_relexplicit6.exe
          mv _relexplicit6.exe _relexplicit6.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit6.org /out _relexplicit6.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit6.org /out _relexplicit6.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relexplicit6.reflect.xml
  if [ -f _relexplicit6.reflect.xml ]
then
    ReflectionRoots="-x _relexplicit6.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relexplicit6.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relexplicit6.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relexplicit6.asm.exe $CLRTestExecutionArguments
