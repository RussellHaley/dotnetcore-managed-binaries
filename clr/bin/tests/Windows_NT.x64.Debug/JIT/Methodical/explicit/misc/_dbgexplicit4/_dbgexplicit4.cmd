...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_dbgexplicit4 _dbgexplicit4.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_dbgexplicit4 _dbgexplicit4.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgexplicit4.dasm.il _dbgexplicit4.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgexplicit4.dasm.il _dbgexplicit4.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgexplicit4.asm.exe  _dbgexplicit4.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgexplicit4.asm.exe  _dbgexplicit4.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgexplicit4
    if not exist "_dbgexplicit4.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgexplicit4.org" (
            mkdir IL
            copy _dbgexplicit4.exe IL\_dbgexplicit4.exe 
            ren _dbgexplicit4.exe _dbgexplicit4.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgexplicit4.org /out _dbgexplicit4.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgexplicit4.org /out _dbgexplicit4.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgexplicit4.reflect.xml
    IF EXIST _dbgexplicit4.reflect.xml set ReflectionRoots=-x _dbgexplicit4.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgexplicit4.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgexplicit4.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgexplicit4.asm.exe %CLRTestExecutionArguments% %Host_Args%
