...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_dbgexplicit6 _dbgexplicit6.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_dbgexplicit6 _dbgexplicit6.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgexplicit6.dasm.il _dbgexplicit6.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgexplicit6.dasm.il _dbgexplicit6.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgexplicit6.asm.exe  _dbgexplicit6.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgexplicit6.asm.exe  _dbgexplicit6.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgexplicit6
    if not exist "_dbgexplicit6.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgexplicit6.org" (
            mkdir IL
            copy _dbgexplicit6.exe IL\_dbgexplicit6.exe 
            ren _dbgexplicit6.exe _dbgexplicit6.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgexplicit6.org /out _dbgexplicit6.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgexplicit6.org /out _dbgexplicit6.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgexplicit6.reflect.xml
    IF EXIST _dbgexplicit6.reflect.xml set ReflectionRoots=-x _dbgexplicit6.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgexplicit6.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgexplicit6.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgexplicit6.asm.exe %CLRTestExecutionArguments% %Host_Args%
