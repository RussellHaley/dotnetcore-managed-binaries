...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_dbgexplicit8 _dbgexplicit8.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_dbgexplicit8 _dbgexplicit8.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgexplicit8.dasm.il _dbgexplicit8.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgexplicit8.dasm.il _dbgexplicit8.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgexplicit8.asm.exe  _dbgexplicit8.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgexplicit8.asm.exe  _dbgexplicit8.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgexplicit8
    if not exist "_dbgexplicit8.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgexplicit8.org" (
            mkdir IL
            copy _dbgexplicit8.exe IL\_dbgexplicit8.exe 
            ren _dbgexplicit8.exe _dbgexplicit8.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgexplicit8.org /out _dbgexplicit8.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgexplicit8.org /out _dbgexplicit8.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgexplicit8.reflect.xml
    IF EXIST _dbgexplicit8.reflect.xml set ReflectionRoots=-x _dbgexplicit8.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgexplicit8.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgexplicit8.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgexplicit8.asm.exe %CLRTestExecutionArguments% %Host_Args%
