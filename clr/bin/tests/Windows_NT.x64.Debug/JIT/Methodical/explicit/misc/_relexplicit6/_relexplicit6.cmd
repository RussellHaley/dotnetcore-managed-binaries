...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_relexplicit6 _relexplicit6.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_relexplicit6 _relexplicit6.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relexplicit6.dasm.il _relexplicit6.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relexplicit6.dasm.il _relexplicit6.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relexplicit6.asm.exe  _relexplicit6.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relexplicit6.asm.exe  _relexplicit6.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relexplicit6
    if not exist "_relexplicit6.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relexplicit6.org" (
            mkdir IL
            copy _relexplicit6.exe IL\_relexplicit6.exe 
            ren _relexplicit6.exe _relexplicit6.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relexplicit6.org /out _relexplicit6.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relexplicit6.org /out _relexplicit6.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relexplicit6.reflect.xml
    IF EXIST _relexplicit6.reflect.xml set ReflectionRoots=-x _relexplicit6.reflect.xml
    ...
    set ExePath=%LinkBin%\_relexplicit6.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relexplicit6.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relexplicit6.asm.exe %CLRTestExecutionArguments% %Host_Args%
