...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit3 _relexplicit3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit3 _relexplicit3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relexplicit3.dasm.il _relexplicit3.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relexplicit3.dasm.il _relexplicit3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relexplicit3.asm.exe  _relexplicit3.dasm.il
    "$CORE_ROOT/ilasm" -output=_relexplicit3.asm.exe  _relexplicit3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relexplicit3
    if [ ! -f _relexplicit3.org ]; then
        TakeLock
        if [ ! -f _relexplicit3.org ]; then
          mkdir IL
          cp _relexplicit3.exe IL/_relexplicit3.exe
          mv _relexplicit3.exe _relexplicit3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit3.org /out _relexplicit3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit3.org /out _relexplicit3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relexplicit3.reflect.xml
  if [ -f _relexplicit3.reflect.xml ]
then
    ReflectionRoots="-x _relexplicit3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relexplicit3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relexplicit3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relexplicit3.asm.exe $CLRTestExecutionArguments
