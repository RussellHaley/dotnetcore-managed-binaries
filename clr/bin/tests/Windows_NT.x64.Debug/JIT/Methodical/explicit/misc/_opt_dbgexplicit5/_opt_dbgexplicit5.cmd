...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_opt_dbgexplicit5 _opt_dbgexplicit5.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_opt_dbgexplicit5 _opt_dbgexplicit5.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_opt_dbgexplicit5.dasm.il _opt_dbgexplicit5.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_opt_dbgexplicit5.dasm.il _opt_dbgexplicit5.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_opt_dbgexplicit5.asm.exe  _opt_dbgexplicit5.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_opt_dbgexplicit5.asm.exe  _opt_dbgexplicit5.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_opt_dbgexplicit5
    if not exist "_opt_dbgexplicit5.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_opt_dbgexplicit5.org" (
            mkdir IL
            copy _opt_dbgexplicit5.exe IL\_opt_dbgexplicit5.exe 
            ren _opt_dbgexplicit5.exe _opt_dbgexplicit5.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _opt_dbgexplicit5.org /out _opt_dbgexplicit5.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _opt_dbgexplicit5.org /out _opt_dbgexplicit5.exe
...
set ExePath=_opt_dbgexplicit5.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _opt_dbgexplicit5.reflect.xml
    IF EXIST _opt_dbgexplicit5.reflect.xml set ReflectionRoots=-x _opt_dbgexplicit5.reflect.xml
    ...
    set ExePath=%LinkBin%\_opt_dbgexplicit5.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _opt_dbgexplicit5.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _opt_dbgexplicit5.asm.exe %CLRTestExecutionArguments% %Host_Args%
