...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_relexplicit2 _relexplicit2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_relexplicit2 _relexplicit2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relexplicit2.dasm.il _relexplicit2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relexplicit2.dasm.il _relexplicit2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relexplicit2.asm.exe  _relexplicit2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relexplicit2.asm.exe  _relexplicit2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relexplicit2
    if not exist "_relexplicit2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relexplicit2.org" (
            mkdir IL
            copy _relexplicit2.exe IL\_relexplicit2.exe 
            ren _relexplicit2.exe _relexplicit2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relexplicit2.org /out _relexplicit2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relexplicit2.org /out _relexplicit2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relexplicit2.reflect.xml
    IF EXIST _relexplicit2.reflect.xml set ReflectionRoots=-x _relexplicit2.reflect.xml
    ...
    set ExePath=%LinkBin%\_relexplicit2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relexplicit2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relexplicit2.asm.exe %CLRTestExecutionArguments% %Host_Args%
