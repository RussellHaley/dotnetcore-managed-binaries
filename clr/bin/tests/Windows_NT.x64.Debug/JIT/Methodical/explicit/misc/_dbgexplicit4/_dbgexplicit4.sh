...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_dbgexplicit4 _dbgexplicit4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_dbgexplicit4 _dbgexplicit4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgexplicit4.dasm.il _dbgexplicit4.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgexplicit4.dasm.il _dbgexplicit4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgexplicit4.asm.exe  _dbgexplicit4.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgexplicit4.asm.exe  _dbgexplicit4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgexplicit4
    if [ ! -f _dbgexplicit4.org ]; then
        TakeLock
        if [ ! -f _dbgexplicit4.org ]; then
          mkdir IL
          cp _dbgexplicit4.exe IL/_dbgexplicit4.exe
          mv _dbgexplicit4.exe _dbgexplicit4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgexplicit4.org /out _dbgexplicit4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgexplicit4.org /out _dbgexplicit4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgexplicit4.reflect.xml
  if [ -f _dbgexplicit4.reflect.xml ]
then
    ReflectionRoots="-x _dbgexplicit4.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgexplicit4.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgexplicit4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgexplicit4.asm.exe $CLRTestExecutionArguments
