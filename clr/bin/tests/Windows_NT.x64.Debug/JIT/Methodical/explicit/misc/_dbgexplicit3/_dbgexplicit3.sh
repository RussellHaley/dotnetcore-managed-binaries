...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_dbgexplicit3 _dbgexplicit3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_dbgexplicit3 _dbgexplicit3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgexplicit3.dasm.il _dbgexplicit3.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgexplicit3.dasm.il _dbgexplicit3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgexplicit3.asm.exe  _dbgexplicit3.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgexplicit3.asm.exe  _dbgexplicit3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgexplicit3
    if [ ! -f _dbgexplicit3.org ]; then
        TakeLock
        if [ ! -f _dbgexplicit3.org ]; then
          mkdir IL
          cp _dbgexplicit3.exe IL/_dbgexplicit3.exe
          mv _dbgexplicit3.exe _dbgexplicit3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgexplicit3.org /out _dbgexplicit3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgexplicit3.org /out _dbgexplicit3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgexplicit3.reflect.xml
  if [ -f _dbgexplicit3.reflect.xml ]
then
    ReflectionRoots="-x _dbgexplicit3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgexplicit3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgexplicit3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgexplicit3.asm.exe $CLRTestExecutionArguments
