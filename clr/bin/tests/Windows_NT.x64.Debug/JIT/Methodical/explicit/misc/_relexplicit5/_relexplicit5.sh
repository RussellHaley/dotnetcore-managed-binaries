...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit5 _relexplicit5.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/explicit/misc/_relexplicit5 _relexplicit5.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relexplicit5.dasm.il _relexplicit5.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relexplicit5.dasm.il _relexplicit5.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relexplicit5.asm.exe  _relexplicit5.dasm.il
    "$CORE_ROOT/ilasm" -output=_relexplicit5.asm.exe  _relexplicit5.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relexplicit5
    if [ ! -f _relexplicit5.org ]; then
        TakeLock
        if [ ! -f _relexplicit5.org ]; then
          mkdir IL
          cp _relexplicit5.exe IL/_relexplicit5.exe
          mv _relexplicit5.exe _relexplicit5.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit5.org /out _relexplicit5.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relexplicit5.org /out _relexplicit5.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relexplicit5.reflect.xml
  if [ -f _relexplicit5.reflect.xml ]
then
    ReflectionRoots="-x _relexplicit5.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relexplicit5.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relexplicit5.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relexplicit5.asm.exe $CLRTestExecutionArguments
