...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_dbgexplicit2 _dbgexplicit2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\explicit\misc\_dbgexplicit2 _dbgexplicit2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgexplicit2.dasm.il _dbgexplicit2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgexplicit2.dasm.il _dbgexplicit2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgexplicit2.asm.exe  _dbgexplicit2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgexplicit2.asm.exe  _dbgexplicit2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgexplicit2
    if not exist "_dbgexplicit2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgexplicit2.org" (
            mkdir IL
            copy _dbgexplicit2.exe IL\_dbgexplicit2.exe 
            ren _dbgexplicit2.exe _dbgexplicit2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgexplicit2.org /out _dbgexplicit2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgexplicit2.org /out _dbgexplicit2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgexplicit2.reflect.xml
    IF EXIST _dbgexplicit2.reflect.xml set ReflectionRoots=-x _dbgexplicit2.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgexplicit2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgexplicit2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgexplicit2.asm.exe %CLRTestExecutionArguments% %Host_Args%
