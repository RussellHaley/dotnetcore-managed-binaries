...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\nonvirtualcall\tailcall_r tailcall_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\nonvirtualcall\tailcall_r tailcall_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tailcall_r.dasm.il tailcall_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tailcall_r.dasm.il tailcall_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tailcall_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tailcall_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tailcall_r
    if not exist "tailcall_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tailcall_r.org" (
            mkdir IL
            copy tailcall_r.exe IL\tailcall_r.exe 
            ren tailcall_r.exe tailcall_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tailcall_r.org /out tailcall_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tailcall_r.org /out tailcall_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tailcall_r.reflect.xml
    IF EXIST tailcall_r.reflect.xml set ReflectionRoots=-x tailcall_r.reflect.xml
    ...
    set ExePath=%LinkBin%\tailcall_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tailcall_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tailcall_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
