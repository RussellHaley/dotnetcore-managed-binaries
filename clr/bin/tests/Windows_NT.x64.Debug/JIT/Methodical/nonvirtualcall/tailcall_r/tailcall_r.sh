...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/nonvirtualcall/tailcall_r tailcall_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/nonvirtualcall/tailcall_r tailcall_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tailcall_r.dasm.il tailcall_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=tailcall_r.dasm.il tailcall_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tailcall_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall_r.dasm.il
    "$CORE_ROOT/ilasm" -output=tailcall_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tailcall_r
    if [ ! -f tailcall_r.org ]; then
        TakeLock
        if [ ! -f tailcall_r.org ]; then
          mkdir IL
          cp tailcall_r.exe IL/tailcall_r.exe
          mv tailcall_r.exe tailcall_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tailcall_r.org /out tailcall_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tailcall_r.org /out tailcall_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in tailcall_r.reflect.xml
  if [ -f tailcall_r.reflect.xml ]
then
    ReflectionRoots="-x tailcall_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/tailcall_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tailcall_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tailcall_r.asm.exe $CLRTestExecutionArguments
