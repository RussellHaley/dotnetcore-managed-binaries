...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tailcall_d.dasm.il tailcall_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tailcall_d.dasm.il tailcall_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tailcall_d.asm.exe -QUIET -NOLOGO -DEBUG tailcall_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tailcall_d.asm.exe -QUIET -NOLOGO -DEBUG tailcall_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tailcall_d
    if not exist "tailcall_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tailcall_d.org" (
            mkdir IL
            copy tailcall_d.exe IL\tailcall_d.exe 
            ren tailcall_d.exe tailcall_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tailcall_d.org /out tailcall_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tailcall_d.org /out tailcall_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tailcall_d.reflect.xml
    IF EXIST tailcall_d.reflect.xml set ReflectionRoots=-x tailcall_d.reflect.xml
    ...
    set ExePath=%LinkBin%\tailcall_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tailcall_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tailcall_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
