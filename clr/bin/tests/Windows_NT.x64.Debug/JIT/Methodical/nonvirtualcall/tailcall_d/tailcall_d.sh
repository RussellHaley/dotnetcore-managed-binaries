...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/nonvirtualcall/tailcall_d tailcall_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/nonvirtualcall/tailcall_d tailcall_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tailcall_d.dasm.il tailcall_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=tailcall_d.dasm.il tailcall_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tailcall_d.asm.exe -QUIET -NOLOGO -DEBUG tailcall_d.dasm.il
    "$CORE_ROOT/ilasm" -output=tailcall_d.asm.exe -QUIET -NOLOGO -DEBUG tailcall_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tailcall_d
    if [ ! -f tailcall_d.org ]; then
        TakeLock
        if [ ! -f tailcall_d.org ]; then
          mkdir IL
          cp tailcall_d.exe IL/tailcall_d.exe
          mv tailcall_d.exe tailcall_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tailcall_d.org /out tailcall_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tailcall_d.org /out tailcall_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in tailcall_d.reflect.xml
  if [ -f tailcall_d.reflect.xml ]
then
    ReflectionRoots="-x tailcall_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/tailcall_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tailcall_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tailcall_d.asm.exe $CLRTestExecutionArguments
