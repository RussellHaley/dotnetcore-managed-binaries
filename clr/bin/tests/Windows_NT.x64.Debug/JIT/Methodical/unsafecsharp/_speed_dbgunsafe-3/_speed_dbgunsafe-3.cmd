...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\unsafecsharp\_speed_dbgunsafe-3 _speed_dbgunsafe-3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\unsafecsharp\_speed_dbgunsafe-3 _speed_dbgunsafe-3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgunsafe-3.dasm.il _speed_dbgunsafe-3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgunsafe-3.dasm.il _speed_dbgunsafe-3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgunsafe-3.asm.exe  _speed_dbgunsafe-3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgunsafe-3.asm.exe  _speed_dbgunsafe-3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgunsafe-3
    if not exist "_speed_dbgunsafe-3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgunsafe-3.org" (
            mkdir IL
            copy _speed_dbgunsafe-3.exe IL\_speed_dbgunsafe-3.exe 
            ren _speed_dbgunsafe-3.exe _speed_dbgunsafe-3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgunsafe-3.org /out _speed_dbgunsafe-3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgunsafe-3.org /out _speed_dbgunsafe-3.exe
...
set ExePath=_speed_dbgunsafe-3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgunsafe-3.reflect.xml
    IF EXIST _speed_dbgunsafe-3.reflect.xml set ReflectionRoots=-x _speed_dbgunsafe-3.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgunsafe-3.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgunsafe-3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgunsafe-3.asm.exe %CLRTestExecutionArguments% %Host_Args%
