...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/unsafecsharp/_speed_relunsafe-3 _speed_relunsafe-3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/unsafecsharp/_speed_relunsafe-3 _speed_relunsafe-3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relunsafe-3.dasm.il _speed_relunsafe-3.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relunsafe-3.dasm.il _speed_relunsafe-3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relunsafe-3.asm.exe  _speed_relunsafe-3.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relunsafe-3.asm.exe  _speed_relunsafe-3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relunsafe-3
    if [ ! -f _speed_relunsafe-3.org ]; then
        TakeLock
        if [ ! -f _speed_relunsafe-3.org ]; then
          mkdir IL
          cp _speed_relunsafe-3.exe IL/_speed_relunsafe-3.exe
          mv _speed_relunsafe-3.exe _speed_relunsafe-3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relunsafe-3.org /out _speed_relunsafe-3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relunsafe-3.org /out _speed_relunsafe-3.exe
          __cgExitCode=$?
...
ExePath=_speed_relunsafe-3.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relunsafe-3.reflect.xml
  if [ -f _speed_relunsafe-3.reflect.xml ]
then
    ReflectionRoots="-x _speed_relunsafe-3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relunsafe-3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relunsafe-3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relunsafe-3.asm.exe $CLRTestExecutionArguments
