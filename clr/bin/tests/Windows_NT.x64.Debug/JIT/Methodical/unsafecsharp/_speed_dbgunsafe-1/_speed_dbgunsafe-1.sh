...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/unsafecsharp/_speed_dbgunsafe-1 _speed_dbgunsafe-1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/unsafecsharp/_speed_dbgunsafe-1 _speed_dbgunsafe-1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgunsafe-1.dasm.il _speed_dbgunsafe-1.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgunsafe-1.dasm.il _speed_dbgunsafe-1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgunsafe-1.asm.exe  _speed_dbgunsafe-1.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgunsafe-1.asm.exe  _speed_dbgunsafe-1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgunsafe-1
    if [ ! -f _speed_dbgunsafe-1.org ]; then
        TakeLock
        if [ ! -f _speed_dbgunsafe-1.org ]; then
          mkdir IL
          cp _speed_dbgunsafe-1.exe IL/_speed_dbgunsafe-1.exe
          mv _speed_dbgunsafe-1.exe _speed_dbgunsafe-1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgunsafe-1.org /out _speed_dbgunsafe-1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgunsafe-1.org /out _speed_dbgunsafe-1.exe
          __cgExitCode=$?
...
ExePath=_speed_dbgunsafe-1.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgunsafe-1.reflect.xml
  if [ -f _speed_dbgunsafe-1.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgunsafe-1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgunsafe-1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgunsafe-1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgunsafe-1.asm.exe $CLRTestExecutionArguments
