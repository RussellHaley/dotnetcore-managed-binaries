...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\divrem\div\u8div_cs_ro u8div_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\divrem\div\u8div_cs_ro u8div_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=u8div_cs_ro.dasm.il u8div_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=u8div_cs_ro.dasm.il u8div_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=u8div_cs_ro.asm.exe  u8div_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=u8div_cs_ro.asm.exe  u8div_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=u8div_cs_ro
    if not exist "u8div_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "u8div_cs_ro.org" (
            mkdir IL
            copy u8div_cs_ro.exe IL\u8div_cs_ro.exe 
            ren u8div_cs_ro.exe u8div_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in u8div_cs_ro.org /out u8div_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in u8div_cs_ro.org /out u8div_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in u8div_cs_ro.reflect.xml
    IF EXIST u8div_cs_ro.reflect.xml set ReflectionRoots=-x u8div_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\u8div_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% u8div_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% u8div_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
