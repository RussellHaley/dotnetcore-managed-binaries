...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/divrem/div/i8div_cs_do i8div_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/divrem/div/i8div_cs_do i8div_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=i8div_cs_do.dasm.il i8div_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=i8div_cs_do.dasm.il i8div_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=i8div_cs_do.asm.exe  i8div_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=i8div_cs_do.asm.exe  i8div_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=i8div_cs_do
    if [ ! -f i8div_cs_do.org ]; then
        TakeLock
        if [ ! -f i8div_cs_do.org ]; then
          mkdir IL
          cp i8div_cs_do.exe IL/i8div_cs_do.exe
          mv i8div_cs_do.exe i8div_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in i8div_cs_do.org /out i8div_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in i8div_cs_do.org /out i8div_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in i8div_cs_do.reflect.xml
  if [ -f i8div_cs_do.reflect.xml ]
then
    ReflectionRoots="-x i8div_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/i8div_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" i8div_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" i8div_cs_do.asm.exe $CLRTestExecutionArguments
