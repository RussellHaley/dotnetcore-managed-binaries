...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\divrem\div\i8div_cs_ro i8div_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\divrem\div\i8div_cs_ro i8div_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=i8div_cs_ro.dasm.il i8div_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=i8div_cs_ro.dasm.il i8div_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=i8div_cs_ro.asm.exe  i8div_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=i8div_cs_ro.asm.exe  i8div_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=i8div_cs_ro
    if not exist "i8div_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "i8div_cs_ro.org" (
            mkdir IL
            copy i8div_cs_ro.exe IL\i8div_cs_ro.exe 
            ren i8div_cs_ro.exe i8div_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in i8div_cs_ro.org /out i8div_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in i8div_cs_ro.org /out i8div_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in i8div_cs_ro.reflect.xml
    IF EXIST i8div_cs_ro.reflect.xml set ReflectionRoots=-x i8div_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\i8div_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% i8div_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% i8div_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
