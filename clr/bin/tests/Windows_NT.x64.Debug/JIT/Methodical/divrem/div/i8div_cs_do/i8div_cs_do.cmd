...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\divrem\div\i8div_cs_do i8div_cs_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\divrem\div\i8div_cs_do i8div_cs_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=i8div_cs_do.dasm.il i8div_cs_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=i8div_cs_do.dasm.il i8div_cs_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=i8div_cs_do.asm.exe  i8div_cs_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=i8div_cs_do.asm.exe  i8div_cs_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=i8div_cs_do
    if not exist "i8div_cs_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "i8div_cs_do.org" (
            mkdir IL
            copy i8div_cs_do.exe IL\i8div_cs_do.exe 
            ren i8div_cs_do.exe i8div_cs_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in i8div_cs_do.org /out i8div_cs_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in i8div_cs_do.org /out i8div_cs_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in i8div_cs_do.reflect.xml
    IF EXIST i8div_cs_do.reflect.xml set ReflectionRoots=-x i8div_cs_do.reflect.xml
    ...
    set ExePath=%LinkBin%\i8div_cs_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% i8div_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% i8div_cs_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
