...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/divrem/rem/u4rem_cs_do u4rem_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/divrem/rem/u4rem_cs_do u4rem_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=u4rem_cs_do.dasm.il u4rem_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=u4rem_cs_do.dasm.il u4rem_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=u4rem_cs_do.asm.exe  u4rem_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=u4rem_cs_do.asm.exe  u4rem_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=u4rem_cs_do
    if [ ! -f u4rem_cs_do.org ]; then
        TakeLock
        if [ ! -f u4rem_cs_do.org ]; then
          mkdir IL
          cp u4rem_cs_do.exe IL/u4rem_cs_do.exe
          mv u4rem_cs_do.exe u4rem_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in u4rem_cs_do.org /out u4rem_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in u4rem_cs_do.org /out u4rem_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in u4rem_cs_do.reflect.xml
  if [ -f u4rem_cs_do.reflect.xml ]
then
    ReflectionRoots="-x u4rem_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/u4rem_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" u4rem_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" u4rem_cs_do.asm.exe $CLRTestExecutionArguments
