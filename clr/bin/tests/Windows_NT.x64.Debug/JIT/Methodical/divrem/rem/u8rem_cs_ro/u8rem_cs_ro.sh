...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/divrem/rem/u8rem_cs_ro u8rem_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/divrem/rem/u8rem_cs_ro u8rem_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=u8rem_cs_ro.dasm.il u8rem_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=u8rem_cs_ro.dasm.il u8rem_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=u8rem_cs_ro.asm.exe  u8rem_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=u8rem_cs_ro.asm.exe  u8rem_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=u8rem_cs_ro
    if [ ! -f u8rem_cs_ro.org ]; then
        TakeLock
        if [ ! -f u8rem_cs_ro.org ]; then
          mkdir IL
          cp u8rem_cs_ro.exe IL/u8rem_cs_ro.exe
          mv u8rem_cs_ro.exe u8rem_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in u8rem_cs_ro.org /out u8rem_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in u8rem_cs_ro.org /out u8rem_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in u8rem_cs_ro.reflect.xml
  if [ -f u8rem_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x u8rem_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/u8rem_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" u8rem_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" u8rem_cs_ro.asm.exe $CLRTestExecutionArguments
