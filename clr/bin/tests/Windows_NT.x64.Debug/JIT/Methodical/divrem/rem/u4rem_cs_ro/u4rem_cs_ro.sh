...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/divrem/rem/u4rem_cs_ro u4rem_cs_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/divrem/rem/u4rem_cs_ro u4rem_cs_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=u4rem_cs_ro.dasm.il u4rem_cs_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=u4rem_cs_ro.dasm.il u4rem_cs_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=u4rem_cs_ro.asm.exe  u4rem_cs_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=u4rem_cs_ro.asm.exe  u4rem_cs_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=u4rem_cs_ro
    if [ ! -f u4rem_cs_ro.org ]; then
        TakeLock
        if [ ! -f u4rem_cs_ro.org ]; then
          mkdir IL
          cp u4rem_cs_ro.exe IL/u4rem_cs_ro.exe
          mv u4rem_cs_ro.exe u4rem_cs_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in u4rem_cs_ro.org /out u4rem_cs_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in u4rem_cs_ro.org /out u4rem_cs_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in u4rem_cs_ro.reflect.xml
  if [ -f u4rem_cs_ro.reflect.xml ]
then
    ReflectionRoots="-x u4rem_cs_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/u4rem_cs_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" u4rem_cs_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" u4rem_cs_ro.asm.exe $CLRTestExecutionArguments
