...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\divrem\rem\i4rem_cs_ro i4rem_cs_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\divrem\rem\i4rem_cs_ro i4rem_cs_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=i4rem_cs_ro.dasm.il i4rem_cs_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=i4rem_cs_ro.dasm.il i4rem_cs_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=i4rem_cs_ro.asm.exe  i4rem_cs_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=i4rem_cs_ro.asm.exe  i4rem_cs_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=i4rem_cs_ro
    if not exist "i4rem_cs_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "i4rem_cs_ro.org" (
            mkdir IL
            copy i4rem_cs_ro.exe IL\i4rem_cs_ro.exe 
            ren i4rem_cs_ro.exe i4rem_cs_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in i4rem_cs_ro.org /out i4rem_cs_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in i4rem_cs_ro.org /out i4rem_cs_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in i4rem_cs_ro.reflect.xml
    IF EXIST i4rem_cs_ro.reflect.xml set ReflectionRoots=-x i4rem_cs_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\i4rem_cs_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% i4rem_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% i4rem_cs_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
