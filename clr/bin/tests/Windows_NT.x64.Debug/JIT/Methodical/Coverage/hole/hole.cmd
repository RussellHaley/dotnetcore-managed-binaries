...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\Coverage\hole hole.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\Coverage\hole hole.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=hole.dasm.il hole.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=hole.dasm.il hole.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=hole.asm.exe -QUIET -NOLOGO hole.dasm.il
    %CORE_ROOT%\ilasm.exe /output=hole.asm.exe -QUIET -NOLOGO hole.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=hole
    if not exist "hole.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "hole.org" (
            mkdir IL
            copy hole.exe IL\hole.exe 
            ren hole.exe hole.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hole.org /out hole.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hole.org /out hole.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in hole.reflect.xml
    IF EXIST hole.reflect.xml set ReflectionRoots=-x hole.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% hole.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% hole.asm.exe %CLRTestExecutionArguments% %Host_Args%
