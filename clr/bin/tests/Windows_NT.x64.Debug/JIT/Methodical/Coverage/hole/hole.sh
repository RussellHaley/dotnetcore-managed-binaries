...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/Coverage/hole hole.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/Coverage/hole hole.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=hole.dasm.il hole.exe
    "$CORE_ROOT/ildasm" -raweh -out=hole.dasm.il hole.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=hole.asm.exe -QUIET -NOLOGO hole.dasm.il
    "$CORE_ROOT/ilasm" -output=hole.asm.exe -QUIET -NOLOGO hole.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=hole
    if [ ! -f hole.org ]; then
        TakeLock
        if [ ! -f hole.org ]; then
          mkdir IL
          cp hole.exe IL/hole.exe
          mv hole.exe hole.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hole.org /out hole.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hole.org /out hole.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in hole.reflect.xml
  if [ -f hole.reflect.xml ]
then
    ReflectionRoots="-x hole.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" hole.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" hole.asm.exe $CLRTestExecutionArguments
