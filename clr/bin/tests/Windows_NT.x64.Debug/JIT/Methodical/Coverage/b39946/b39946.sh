...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/Coverage/b39946 b39946.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/Coverage/b39946 b39946.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    ...
    echo "$CORE_ROOT/ilasm" -output=b39946.asm.exe -QUIET -NOLOGO b39946.dasm.il
    "$CORE_ROOT/ilasm" -output=b39946.asm.exe -QUIET -NOLOGO b39946.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
