...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\Coverage\b39946 b39946.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\Coverage\b39946 b39946.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    ...
    ECHO %CORE_ROOT%\ilasm.exe /output=b39946.asm.exe -QUIET -NOLOGO b39946.dasm.il
    %CORE_ROOT%\ilasm.exe /output=b39946.asm.exe -QUIET -NOLOGO b39946.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
