...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\NaN\r8nanconv_il_d r8nanconv_il_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\NaN\r8nanconv_il_d r8nanconv_il_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=r8nanconv_il_d.dasm.il r8nanconv_il_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=r8nanconv_il_d.dasm.il r8nanconv_il_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=r8nanconv_il_d.asm.exe -QUIET -NOLOGO -DEBUG r8nanconv_il_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=r8nanconv_il_d.asm.exe -QUIET -NOLOGO -DEBUG r8nanconv_il_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=r8nanconv_il_d
    if not exist "r8nanconv_il_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "r8nanconv_il_d.org" (
            mkdir IL
            copy r8nanconv_il_d.exe IL\r8nanconv_il_d.exe 
            ren r8nanconv_il_d.exe r8nanconv_il_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in r8nanconv_il_d.org /out r8nanconv_il_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in r8nanconv_il_d.org /out r8nanconv_il_d.exe
...
set ExePath=r8nanconv_il_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in r8nanconv_il_d.reflect.xml
    IF EXIST r8nanconv_il_d.reflect.xml set ReflectionRoots=-x r8nanconv_il_d.reflect.xml
    ...
    set ExePath=%LinkBin%\r8nanconv_il_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% r8nanconv_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% r8nanconv_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
