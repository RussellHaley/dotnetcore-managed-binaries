...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/NaN/r4nanconv_il_d r4nanconv_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/NaN/r4nanconv_il_d r4nanconv_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=r4nanconv_il_d.dasm.il r4nanconv_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=r4nanconv_il_d.dasm.il r4nanconv_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=r4nanconv_il_d.asm.exe -QUIET -NOLOGO -DEBUG r4nanconv_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=r4nanconv_il_d.asm.exe -QUIET -NOLOGO -DEBUG r4nanconv_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=r4nanconv_il_d
    if [ ! -f r4nanconv_il_d.org ]; then
        TakeLock
        if [ ! -f r4nanconv_il_d.org ]; then
          mkdir IL
          cp r4nanconv_il_d.exe IL/r4nanconv_il_d.exe
          mv r4nanconv_il_d.exe r4nanconv_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in r4nanconv_il_d.org /out r4nanconv_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in r4nanconv_il_d.org /out r4nanconv_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in r4nanconv_il_d.reflect.xml
  if [ -f r4nanconv_il_d.reflect.xml ]
then
    ReflectionRoots="-x r4nanconv_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/r4nanconv_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" r4nanconv_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" r4nanconv_il_d.asm.exe $CLRTestExecutionArguments
