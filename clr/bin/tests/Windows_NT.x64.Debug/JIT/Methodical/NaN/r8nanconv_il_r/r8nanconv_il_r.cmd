...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\NaN\r8nanconv_il_r r8nanconv_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\NaN\r8nanconv_il_r r8nanconv_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=r8nanconv_il_r.dasm.il r8nanconv_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=r8nanconv_il_r.dasm.il r8nanconv_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=r8nanconv_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT r8nanconv_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=r8nanconv_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT r8nanconv_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=r8nanconv_il_r
    if not exist "r8nanconv_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "r8nanconv_il_r.org" (
            mkdir IL
            copy r8nanconv_il_r.exe IL\r8nanconv_il_r.exe 
            ren r8nanconv_il_r.exe r8nanconv_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in r8nanconv_il_r.org /out r8nanconv_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in r8nanconv_il_r.org /out r8nanconv_il_r.exe
...
set ExePath=r8nanconv_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in r8nanconv_il_r.reflect.xml
    IF EXIST r8nanconv_il_r.reflect.xml set ReflectionRoots=-x r8nanconv_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\r8nanconv_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% r8nanconv_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% r8nanconv_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
