...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\NaN\r4nanconv_il_r r4nanconv_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\NaN\r4nanconv_il_r r4nanconv_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=r4nanconv_il_r.dasm.il r4nanconv_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=r4nanconv_il_r.dasm.il r4nanconv_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=r4nanconv_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT r4nanconv_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=r4nanconv_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT r4nanconv_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=r4nanconv_il_r
    if not exist "r4nanconv_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "r4nanconv_il_r.org" (
            mkdir IL
            copy r4nanconv_il_r.exe IL\r4nanconv_il_r.exe 
            ren r4nanconv_il_r.exe r4nanconv_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in r4nanconv_il_r.org /out r4nanconv_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in r4nanconv_il_r.org /out r4nanconv_il_r.exe
...
set ExePath=r4nanconv_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in r4nanconv_il_r.reflect.xml
    IF EXIST r4nanconv_il_r.reflect.xml set ReflectionRoots=-x r4nanconv_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\r4nanconv_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% r4nanconv_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% r4nanconv_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
