...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_dbglcsvalbox _dbglcsvalbox.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_dbglcsvalbox _dbglcsvalbox.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbglcsvalbox.dasm.il _dbglcsvalbox.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbglcsvalbox.dasm.il _dbglcsvalbox.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbglcsvalbox.asm.exe  _dbglcsvalbox.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbglcsvalbox.asm.exe  _dbglcsvalbox.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbglcsvalbox
    if [ ! -f _dbglcsvalbox.org ]; then
        TakeLock
        if [ ! -f _dbglcsvalbox.org ]; then
          mkdir IL
          cp _dbglcsvalbox.exe IL/_dbglcsvalbox.exe
          mv _dbglcsvalbox.exe _dbglcsvalbox.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcsvalbox.org /out _dbglcsvalbox.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcsvalbox.org /out _dbglcsvalbox.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbglcsvalbox.reflect.xml
  if [ -f _dbglcsvalbox.reflect.xml ]
then
    ReflectionRoots="-x _dbglcsvalbox.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbglcsvalbox.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbglcsvalbox.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbglcsvalbox.asm.exe $CLRTestExecutionArguments
