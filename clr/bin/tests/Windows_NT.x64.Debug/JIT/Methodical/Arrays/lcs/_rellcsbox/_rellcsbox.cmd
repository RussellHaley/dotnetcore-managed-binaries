...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_rellcsbox _rellcsbox.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_rellcsbox _rellcsbox.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_rellcsbox.dasm.il _rellcsbox.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_rellcsbox.dasm.il _rellcsbox.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_rellcsbox.asm.exe  _rellcsbox.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_rellcsbox.asm.exe  _rellcsbox.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_rellcsbox
    if not exist "_rellcsbox.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_rellcsbox.org" (
            mkdir IL
            copy _rellcsbox.exe IL\_rellcsbox.exe 
            ren _rellcsbox.exe _rellcsbox.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcsbox.org /out _rellcsbox.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcsbox.org /out _rellcsbox.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _rellcsbox.reflect.xml
    IF EXIST _rellcsbox.reflect.xml set ReflectionRoots=-x _rellcsbox.reflect.xml
    ...
    set ExePath=%LinkBin%\_rellcsbox.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _rellcsbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _rellcsbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
