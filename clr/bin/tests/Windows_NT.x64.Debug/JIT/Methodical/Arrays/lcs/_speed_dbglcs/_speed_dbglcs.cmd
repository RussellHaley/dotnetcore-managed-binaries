...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_speed_dbglcs _speed_dbglcs.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_speed_dbglcs _speed_dbglcs.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbglcs.dasm.il _speed_dbglcs.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbglcs.dasm.il _speed_dbglcs.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbglcs.asm.exe  _speed_dbglcs.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbglcs.asm.exe  _speed_dbglcs.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbglcs
    if not exist "_speed_dbglcs.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbglcs.org" (
            mkdir IL
            copy _speed_dbglcs.exe IL\_speed_dbglcs.exe 
            ren _speed_dbglcs.exe _speed_dbglcs.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbglcs.org /out _speed_dbglcs.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbglcs.org /out _speed_dbglcs.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbglcs.reflect.xml
    IF EXIST _speed_dbglcs.reflect.xml set ReflectionRoots=-x _speed_dbglcs.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbglcs.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbglcs.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbglcs.asm.exe %CLRTestExecutionArguments% %Host_Args%
