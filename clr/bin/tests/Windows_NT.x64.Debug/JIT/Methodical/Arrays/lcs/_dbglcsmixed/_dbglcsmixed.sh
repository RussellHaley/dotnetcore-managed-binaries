...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_dbglcsmixed _dbglcsmixed.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_dbglcsmixed _dbglcsmixed.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbglcsmixed.dasm.il _dbglcsmixed.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbglcsmixed.dasm.il _dbglcsmixed.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbglcsmixed.asm.exe  _dbglcsmixed.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbglcsmixed.asm.exe  _dbglcsmixed.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbglcsmixed
    if [ ! -f _dbglcsmixed.org ]; then
        TakeLock
        if [ ! -f _dbglcsmixed.org ]; then
          mkdir IL
          cp _dbglcsmixed.exe IL/_dbglcsmixed.exe
          mv _dbglcsmixed.exe _dbglcsmixed.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcsmixed.org /out _dbglcsmixed.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcsmixed.org /out _dbglcsmixed.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbglcsmixed.reflect.xml
  if [ -f _dbglcsmixed.reflect.xml ]
then
    ReflectionRoots="-x _dbglcsmixed.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbglcsmixed.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbglcsmixed.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbglcsmixed.asm.exe $CLRTestExecutionArguments
