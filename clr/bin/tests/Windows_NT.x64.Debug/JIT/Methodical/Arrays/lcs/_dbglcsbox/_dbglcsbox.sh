...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_dbglcsbox _dbglcsbox.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_dbglcsbox _dbglcsbox.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbglcsbox.dasm.il _dbglcsbox.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbglcsbox.dasm.il _dbglcsbox.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbglcsbox.asm.exe  _dbglcsbox.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbglcsbox.asm.exe  _dbglcsbox.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbglcsbox
    if [ ! -f _dbglcsbox.org ]; then
        TakeLock
        if [ ! -f _dbglcsbox.org ]; then
          mkdir IL
          cp _dbglcsbox.exe IL/_dbglcsbox.exe
          mv _dbglcsbox.exe _dbglcsbox.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcsbox.org /out _dbglcsbox.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcsbox.org /out _dbglcsbox.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbglcsbox.reflect.xml
  if [ -f _dbglcsbox.reflect.xml ]
then
    ReflectionRoots="-x _dbglcsbox.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbglcsbox.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbglcsbox.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbglcsbox.asm.exe $CLRTestExecutionArguments
