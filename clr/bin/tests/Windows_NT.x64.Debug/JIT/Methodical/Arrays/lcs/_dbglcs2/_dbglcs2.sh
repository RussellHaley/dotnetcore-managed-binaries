...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_dbglcs2 _dbglcs2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_dbglcs2 _dbglcs2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbglcs2.dasm.il _dbglcs2.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbglcs2.dasm.il _dbglcs2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbglcs2.asm.exe  _dbglcs2.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbglcs2.asm.exe  _dbglcs2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbglcs2
    if [ ! -f _dbglcs2.org ]; then
        TakeLock
        if [ ! -f _dbglcs2.org ]; then
          mkdir IL
          cp _dbglcs2.exe IL/_dbglcs2.exe
          mv _dbglcs2.exe _dbglcs2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcs2.org /out _dbglcs2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbglcs2.org /out _dbglcs2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbglcs2.reflect.xml
  if [ -f _dbglcs2.reflect.xml ]
then
    ReflectionRoots="-x _dbglcs2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbglcs2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbglcs2.asm.exe $CLRTestExecutionArguments
