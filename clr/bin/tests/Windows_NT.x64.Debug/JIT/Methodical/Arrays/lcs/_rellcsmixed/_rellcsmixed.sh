...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_rellcsmixed _rellcsmixed.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_rellcsmixed _rellcsmixed.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_rellcsmixed.dasm.il _rellcsmixed.exe
    "$CORE_ROOT/ildasm" -raweh -out=_rellcsmixed.dasm.il _rellcsmixed.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_rellcsmixed.asm.exe  _rellcsmixed.dasm.il
    "$CORE_ROOT/ilasm" -output=_rellcsmixed.asm.exe  _rellcsmixed.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_rellcsmixed
    if [ ! -f _rellcsmixed.org ]; then
        TakeLock
        if [ ! -f _rellcsmixed.org ]; then
          mkdir IL
          cp _rellcsmixed.exe IL/_rellcsmixed.exe
          mv _rellcsmixed.exe _rellcsmixed.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _rellcsmixed.org /out _rellcsmixed.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _rellcsmixed.org /out _rellcsmixed.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _rellcsmixed.reflect.xml
  if [ -f _rellcsmixed.reflect.xml ]
then
    ReflectionRoots="-x _rellcsmixed.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_rellcsmixed.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _rellcsmixed.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _rellcsmixed.asm.exe $CLRTestExecutionArguments
