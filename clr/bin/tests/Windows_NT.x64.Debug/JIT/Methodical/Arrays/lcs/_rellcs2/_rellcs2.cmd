...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_rellcs2 _rellcs2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_rellcs2 _rellcs2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_rellcs2.dasm.il _rellcs2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_rellcs2.dasm.il _rellcs2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_rellcs2.asm.exe  _rellcs2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_rellcs2.asm.exe  _rellcs2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_rellcs2
    if not exist "_rellcs2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_rellcs2.org" (
            mkdir IL
            copy _rellcs2.exe IL\_rellcs2.exe 
            ren _rellcs2.exe _rellcs2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcs2.org /out _rellcs2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcs2.org /out _rellcs2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _rellcs2.reflect.xml
    IF EXIST _rellcs2.reflect.xml set ReflectionRoots=-x _rellcs2.reflect.xml
    ...
    set ExePath=%LinkBin%\_rellcs2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _rellcs2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _rellcs2.asm.exe %CLRTestExecutionArguments% %Host_Args%
