...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsbas _speed_dbglcsbas.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsbas _speed_dbglcsbas.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsbas.dasm.il _speed_dbglcsbas.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsbas.dasm.il _speed_dbglcsbas.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbglcsbas.asm.exe  _speed_dbglcsbas.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbglcsbas.asm.exe  _speed_dbglcsbas.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbglcsbas
    if [ ! -f _speed_dbglcsbas.org ]; then
        TakeLock
        if [ ! -f _speed_dbglcsbas.org ]; then
          mkdir IL
          cp _speed_dbglcsbas.exe IL/_speed_dbglcsbas.exe
          mv _speed_dbglcsbas.exe _speed_dbglcsbas.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsbas.org /out _speed_dbglcsbas.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsbas.org /out _speed_dbglcsbas.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbglcsbas.reflect.xml
  if [ -f _speed_dbglcsbas.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbglcsbas.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbglcsbas.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbglcsbas.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbglcsbas.asm.exe $CLRTestExecutionArguments
