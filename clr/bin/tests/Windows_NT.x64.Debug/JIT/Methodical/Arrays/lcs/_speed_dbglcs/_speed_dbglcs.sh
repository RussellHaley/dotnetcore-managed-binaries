...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcs _speed_dbglcs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcs _speed_dbglcs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcs.dasm.il _speed_dbglcs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcs.dasm.il _speed_dbglcs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbglcs.asm.exe  _speed_dbglcs.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbglcs.asm.exe  _speed_dbglcs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbglcs
    if [ ! -f _speed_dbglcs.org ]; then
        TakeLock
        if [ ! -f _speed_dbglcs.org ]; then
          mkdir IL
          cp _speed_dbglcs.exe IL/_speed_dbglcs.exe
          mv _speed_dbglcs.exe _speed_dbglcs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcs.org /out _speed_dbglcs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcs.org /out _speed_dbglcs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbglcs.reflect.xml
  if [ -f _speed_dbglcs.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbglcs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbglcs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbglcs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbglcs.asm.exe $CLRTestExecutionArguments
