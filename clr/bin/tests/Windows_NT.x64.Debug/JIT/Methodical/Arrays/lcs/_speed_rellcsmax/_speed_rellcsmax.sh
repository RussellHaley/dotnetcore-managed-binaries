...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsmax _speed_rellcsmax.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsmax _speed_rellcsmax.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsmax.dasm.il _speed_rellcsmax.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsmax.dasm.il _speed_rellcsmax.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_rellcsmax.asm.exe  _speed_rellcsmax.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_rellcsmax.asm.exe  _speed_rellcsmax.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_rellcsmax
    if [ ! -f _speed_rellcsmax.org ]; then
        TakeLock
        if [ ! -f _speed_rellcsmax.org ]; then
          mkdir IL
          cp _speed_rellcsmax.exe IL/_speed_rellcsmax.exe
          mv _speed_rellcsmax.exe _speed_rellcsmax.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsmax.org /out _speed_rellcsmax.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsmax.org /out _speed_rellcsmax.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_rellcsmax.reflect.xml
  if [ -f _speed_rellcsmax.reflect.xml ]
then
    ReflectionRoots="-x _speed_rellcsmax.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_rellcsmax.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_rellcsmax.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_rellcsmax.asm.exe $CLRTestExecutionArguments
