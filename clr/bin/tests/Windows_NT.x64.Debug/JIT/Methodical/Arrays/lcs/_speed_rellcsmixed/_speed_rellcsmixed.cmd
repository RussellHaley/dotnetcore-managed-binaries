...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_speed_rellcsmixed _speed_rellcsmixed.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_speed_rellcsmixed _speed_rellcsmixed.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_rellcsmixed.dasm.il _speed_rellcsmixed.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_rellcsmixed.dasm.il _speed_rellcsmixed.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_rellcsmixed.asm.exe  _speed_rellcsmixed.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_rellcsmixed.asm.exe  _speed_rellcsmixed.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_rellcsmixed
    if not exist "_speed_rellcsmixed.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_rellcsmixed.org" (
            mkdir IL
            copy _speed_rellcsmixed.exe IL\_speed_rellcsmixed.exe 
            ren _speed_rellcsmixed.exe _speed_rellcsmixed.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_rellcsmixed.org /out _speed_rellcsmixed.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_rellcsmixed.org /out _speed_rellcsmixed.exe
...
set ExePath=_speed_rellcsmixed.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_rellcsmixed.reflect.xml
    IF EXIST _speed_rellcsmixed.reflect.xml set ReflectionRoots=-x _speed_rellcsmixed.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_rellcsmixed.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_rellcsmixed.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_rellcsmixed.asm.exe %CLRTestExecutionArguments% %Host_Args%
