...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsmixed _speed_rellcsmixed.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsmixed _speed_rellcsmixed.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsmixed.dasm.il _speed_rellcsmixed.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsmixed.dasm.il _speed_rellcsmixed.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_rellcsmixed.asm.exe  _speed_rellcsmixed.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_rellcsmixed.asm.exe  _speed_rellcsmixed.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_rellcsmixed
    if [ ! -f _speed_rellcsmixed.org ]; then
        TakeLock
        if [ ! -f _speed_rellcsmixed.org ]; then
          mkdir IL
          cp _speed_rellcsmixed.exe IL/_speed_rellcsmixed.exe
          mv _speed_rellcsmixed.exe _speed_rellcsmixed.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsmixed.org /out _speed_rellcsmixed.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsmixed.org /out _speed_rellcsmixed.exe
          __cgExitCode=$?
...
ExePath=_speed_rellcsmixed.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_rellcsmixed.reflect.xml
  if [ -f _speed_rellcsmixed.reflect.xml ]
then
    ReflectionRoots="-x _speed_rellcsmixed.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_rellcsmixed.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_rellcsmixed.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_rellcsmixed.asm.exe $CLRTestExecutionArguments
