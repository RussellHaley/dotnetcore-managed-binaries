...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_rellcsvalbox _rellcsvalbox.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_rellcsvalbox _rellcsvalbox.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_rellcsvalbox.dasm.il _rellcsvalbox.exe
    "$CORE_ROOT/ildasm" -raweh -out=_rellcsvalbox.dasm.il _rellcsvalbox.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_rellcsvalbox.asm.exe  _rellcsvalbox.dasm.il
    "$CORE_ROOT/ilasm" -output=_rellcsvalbox.asm.exe  _rellcsvalbox.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_rellcsvalbox
    if [ ! -f _rellcsvalbox.org ]; then
        TakeLock
        if [ ! -f _rellcsvalbox.org ]; then
          mkdir IL
          cp _rellcsvalbox.exe IL/_rellcsvalbox.exe
          mv _rellcsvalbox.exe _rellcsvalbox.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _rellcsvalbox.org /out _rellcsvalbox.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _rellcsvalbox.org /out _rellcsvalbox.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _rellcsvalbox.reflect.xml
  if [ -f _rellcsvalbox.reflect.xml ]
then
    ReflectionRoots="-x _rellcsvalbox.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_rellcsvalbox.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _rellcsvalbox.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _rellcsvalbox.asm.exe $CLRTestExecutionArguments
