...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsvalbox _speed_dbglcsvalbox.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsvalbox _speed_dbglcsvalbox.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsvalbox.dasm.il _speed_dbglcsvalbox.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsvalbox.dasm.il _speed_dbglcsvalbox.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbglcsvalbox.asm.exe  _speed_dbglcsvalbox.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbglcsvalbox.asm.exe  _speed_dbglcsvalbox.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbglcsvalbox
    if [ ! -f _speed_dbglcsvalbox.org ]; then
        TakeLock
        if [ ! -f _speed_dbglcsvalbox.org ]; then
          mkdir IL
          cp _speed_dbglcsvalbox.exe IL/_speed_dbglcsvalbox.exe
          mv _speed_dbglcsvalbox.exe _speed_dbglcsvalbox.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsvalbox.org /out _speed_dbglcsvalbox.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsvalbox.org /out _speed_dbglcsvalbox.exe
          __cgExitCode=$?
...
ExePath=_speed_dbglcsvalbox.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbglcsvalbox.reflect.xml
  if [ -f _speed_dbglcsvalbox.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbglcsvalbox.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbglcsvalbox.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbglcsvalbox.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbglcsvalbox.asm.exe $CLRTestExecutionArguments
