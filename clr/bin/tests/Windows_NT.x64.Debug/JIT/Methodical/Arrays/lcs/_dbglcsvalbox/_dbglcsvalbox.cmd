...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_dbglcsvalbox _dbglcsvalbox.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_dbglcsvalbox _dbglcsvalbox.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbglcsvalbox.dasm.il _dbglcsvalbox.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbglcsvalbox.dasm.il _dbglcsvalbox.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbglcsvalbox.asm.exe  _dbglcsvalbox.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbglcsvalbox.asm.exe  _dbglcsvalbox.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbglcsvalbox
    if not exist "_dbglcsvalbox.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbglcsvalbox.org" (
            mkdir IL
            copy _dbglcsvalbox.exe IL\_dbglcsvalbox.exe 
            ren _dbglcsvalbox.exe _dbglcsvalbox.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbglcsvalbox.org /out _dbglcsvalbox.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbglcsvalbox.org /out _dbglcsvalbox.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbglcsvalbox.reflect.xml
    IF EXIST _dbglcsvalbox.reflect.xml set ReflectionRoots=-x _dbglcsvalbox.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbglcsvalbox.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbglcsvalbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbglcsvalbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
