...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsmax _speed_dbglcsmax.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsmax _speed_dbglcsmax.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsmax.dasm.il _speed_dbglcsmax.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsmax.dasm.il _speed_dbglcsmax.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbglcsmax.asm.exe  _speed_dbglcsmax.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbglcsmax.asm.exe  _speed_dbglcsmax.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbglcsmax
    if [ ! -f _speed_dbglcsmax.org ]; then
        TakeLock
        if [ ! -f _speed_dbglcsmax.org ]; then
          mkdir IL
          cp _speed_dbglcsmax.exe IL/_speed_dbglcsmax.exe
          mv _speed_dbglcsmax.exe _speed_dbglcsmax.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsmax.org /out _speed_dbglcsmax.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsmax.org /out _speed_dbglcsmax.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbglcsmax.reflect.xml
  if [ -f _speed_dbglcsmax.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbglcsmax.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbglcsmax.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbglcsmax.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbglcsmax.asm.exe $CLRTestExecutionArguments
