...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_rellcsmixed _rellcsmixed.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_rellcsmixed _rellcsmixed.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_rellcsmixed.dasm.il _rellcsmixed.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_rellcsmixed.dasm.il _rellcsmixed.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_rellcsmixed.asm.exe  _rellcsmixed.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_rellcsmixed.asm.exe  _rellcsmixed.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_rellcsmixed
    if not exist "_rellcsmixed.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_rellcsmixed.org" (
            mkdir IL
            copy _rellcsmixed.exe IL\_rellcsmixed.exe 
            ren _rellcsmixed.exe _rellcsmixed.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcsmixed.org /out _rellcsmixed.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcsmixed.org /out _rellcsmixed.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _rellcsmixed.reflect.xml
    IF EXIST _rellcsmixed.reflect.xml set ReflectionRoots=-x _rellcsmixed.reflect.xml
    ...
    set ExePath=%LinkBin%\_rellcsmixed.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _rellcsmixed.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _rellcsmixed.asm.exe %CLRTestExecutionArguments% %Host_Args%
