...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsvalbox _speed_rellcsvalbox.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsvalbox _speed_rellcsvalbox.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsvalbox.dasm.il _speed_rellcsvalbox.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsvalbox.dasm.il _speed_rellcsvalbox.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_rellcsvalbox.asm.exe  _speed_rellcsvalbox.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_rellcsvalbox.asm.exe  _speed_rellcsvalbox.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_rellcsvalbox
    if [ ! -f _speed_rellcsvalbox.org ]; then
        TakeLock
        if [ ! -f _speed_rellcsvalbox.org ]; then
          mkdir IL
          cp _speed_rellcsvalbox.exe IL/_speed_rellcsvalbox.exe
          mv _speed_rellcsvalbox.exe _speed_rellcsvalbox.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsvalbox.org /out _speed_rellcsvalbox.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsvalbox.org /out _speed_rellcsvalbox.exe
          __cgExitCode=$?
...
ExePath=_speed_rellcsvalbox.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_rellcsvalbox.reflect.xml
  if [ -f _speed_rellcsvalbox.reflect.xml ]
then
    ReflectionRoots="-x _speed_rellcsvalbox.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_rellcsvalbox.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_rellcsvalbox.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_rellcsvalbox.asm.exe $CLRTestExecutionArguments
