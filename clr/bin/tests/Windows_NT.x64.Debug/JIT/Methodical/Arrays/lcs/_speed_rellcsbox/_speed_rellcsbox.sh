...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsbox _speed_rellcsbox.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsbox _speed_rellcsbox.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsbox.dasm.il _speed_rellcsbox.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsbox.dasm.il _speed_rellcsbox.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_rellcsbox.asm.exe  _speed_rellcsbox.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_rellcsbox.asm.exe  _speed_rellcsbox.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_rellcsbox
    if [ ! -f _speed_rellcsbox.org ]; then
        TakeLock
        if [ ! -f _speed_rellcsbox.org ]; then
          mkdir IL
          cp _speed_rellcsbox.exe IL/_speed_rellcsbox.exe
          mv _speed_rellcsbox.exe _speed_rellcsbox.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsbox.org /out _speed_rellcsbox.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsbox.org /out _speed_rellcsbox.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_rellcsbox.reflect.xml
  if [ -f _speed_rellcsbox.reflect.xml ]
then
    ReflectionRoots="-x _speed_rellcsbox.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_rellcsbox.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_rellcsbox.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_rellcsbox.asm.exe $CLRTestExecutionArguments
