...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsbas _speed_rellcsbas.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_rellcsbas _speed_rellcsbas.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsbas.dasm.il _speed_rellcsbas.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_rellcsbas.dasm.il _speed_rellcsbas.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_rellcsbas.asm.exe  _speed_rellcsbas.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_rellcsbas.asm.exe  _speed_rellcsbas.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_rellcsbas
    if [ ! -f _speed_rellcsbas.org ]; then
        TakeLock
        if [ ! -f _speed_rellcsbas.org ]; then
          mkdir IL
          cp _speed_rellcsbas.exe IL/_speed_rellcsbas.exe
          mv _speed_rellcsbas.exe _speed_rellcsbas.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsbas.org /out _speed_rellcsbas.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_rellcsbas.org /out _speed_rellcsbas.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_rellcsbas.reflect.xml
  if [ -f _speed_rellcsbas.reflect.xml ]
then
    ReflectionRoots="-x _speed_rellcsbas.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_rellcsbas.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_rellcsbas.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_rellcsbas.asm.exe $CLRTestExecutionArguments
