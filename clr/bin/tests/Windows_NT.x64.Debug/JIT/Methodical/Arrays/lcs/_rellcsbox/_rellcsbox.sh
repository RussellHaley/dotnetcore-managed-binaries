...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_rellcsbox _rellcsbox.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_rellcsbox _rellcsbox.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_rellcsbox.dasm.il _rellcsbox.exe
    "$CORE_ROOT/ildasm" -raweh -out=_rellcsbox.dasm.il _rellcsbox.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_rellcsbox.asm.exe  _rellcsbox.dasm.il
    "$CORE_ROOT/ilasm" -output=_rellcsbox.asm.exe  _rellcsbox.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_rellcsbox
    if [ ! -f _rellcsbox.org ]; then
        TakeLock
        if [ ! -f _rellcsbox.org ]; then
          mkdir IL
          cp _rellcsbox.exe IL/_rellcsbox.exe
          mv _rellcsbox.exe _rellcsbox.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _rellcsbox.org /out _rellcsbox.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _rellcsbox.org /out _rellcsbox.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _rellcsbox.reflect.xml
  if [ -f _rellcsbox.reflect.xml ]
then
    ReflectionRoots="-x _rellcsbox.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_rellcsbox.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _rellcsbox.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _rellcsbox.asm.exe $CLRTestExecutionArguments
