...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_speed_rellcsbox _speed_rellcsbox.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_speed_rellcsbox _speed_rellcsbox.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_rellcsbox.dasm.il _speed_rellcsbox.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_rellcsbox.dasm.il _speed_rellcsbox.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_rellcsbox.asm.exe  _speed_rellcsbox.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_rellcsbox.asm.exe  _speed_rellcsbox.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_rellcsbox
    if not exist "_speed_rellcsbox.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_rellcsbox.org" (
            mkdir IL
            copy _speed_rellcsbox.exe IL\_speed_rellcsbox.exe 
            ren _speed_rellcsbox.exe _speed_rellcsbox.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_rellcsbox.org /out _speed_rellcsbox.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_rellcsbox.org /out _speed_rellcsbox.exe
...
set ExePath=_speed_rellcsbox.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_rellcsbox.reflect.xml
    IF EXIST _speed_rellcsbox.reflect.xml set ReflectionRoots=-x _speed_rellcsbox.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_rellcsbox.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_rellcsbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_rellcsbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
