...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_dbglcsbox _dbglcsbox.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_dbglcsbox _dbglcsbox.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbglcsbox.dasm.il _dbglcsbox.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbglcsbox.dasm.il _dbglcsbox.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbglcsbox.asm.exe  _dbglcsbox.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbglcsbox.asm.exe  _dbglcsbox.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbglcsbox
    if not exist "_dbglcsbox.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbglcsbox.org" (
            mkdir IL
            copy _dbglcsbox.exe IL\_dbglcsbox.exe 
            ren _dbglcsbox.exe _dbglcsbox.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbglcsbox.org /out _dbglcsbox.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbglcsbox.org /out _dbglcsbox.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbglcsbox.reflect.xml
    IF EXIST _dbglcsbox.reflect.xml set ReflectionRoots=-x _dbglcsbox.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbglcsbox.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbglcsbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbglcsbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
