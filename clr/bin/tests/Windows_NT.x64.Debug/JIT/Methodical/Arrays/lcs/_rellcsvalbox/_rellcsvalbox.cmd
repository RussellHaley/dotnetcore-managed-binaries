...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_rellcsvalbox _rellcsvalbox.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\lcs\_rellcsvalbox _rellcsvalbox.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_rellcsvalbox.dasm.il _rellcsvalbox.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_rellcsvalbox.dasm.il _rellcsvalbox.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_rellcsvalbox.asm.exe  _rellcsvalbox.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_rellcsvalbox.asm.exe  _rellcsvalbox.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_rellcsvalbox
    if not exist "_rellcsvalbox.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_rellcsvalbox.org" (
            mkdir IL
            copy _rellcsvalbox.exe IL\_rellcsvalbox.exe 
            ren _rellcsvalbox.exe _rellcsvalbox.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcsvalbox.org /out _rellcsvalbox.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _rellcsvalbox.org /out _rellcsvalbox.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _rellcsvalbox.reflect.xml
    IF EXIST _rellcsvalbox.reflect.xml set ReflectionRoots=-x _rellcsvalbox.reflect.xml
    ...
    set ExePath=%LinkBin%\_rellcsvalbox.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _rellcsvalbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _rellcsvalbox.asm.exe %CLRTestExecutionArguments% %Host_Args%
