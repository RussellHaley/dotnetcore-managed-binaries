...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsbox _speed_dbglcsbox.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsbox _speed_dbglcsbox.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsbox.dasm.il _speed_dbglcsbox.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsbox.dasm.il _speed_dbglcsbox.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbglcsbox.asm.exe  _speed_dbglcsbox.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbglcsbox.asm.exe  _speed_dbglcsbox.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbglcsbox
    if [ ! -f _speed_dbglcsbox.org ]; then
        TakeLock
        if [ ! -f _speed_dbglcsbox.org ]; then
          mkdir IL
          cp _speed_dbglcsbox.exe IL/_speed_dbglcsbox.exe
          mv _speed_dbglcsbox.exe _speed_dbglcsbox.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsbox.org /out _speed_dbglcsbox.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsbox.org /out _speed_dbglcsbox.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbglcsbox.reflect.xml
  if [ -f _speed_dbglcsbox.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbglcsbox.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbglcsbox.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbglcsbox.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbglcsbox.asm.exe $CLRTestExecutionArguments
