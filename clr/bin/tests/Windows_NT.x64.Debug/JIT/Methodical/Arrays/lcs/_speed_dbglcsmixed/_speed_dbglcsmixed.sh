...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsmixed _speed_dbglcsmixed.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/lcs/_speed_dbglcsmixed _speed_dbglcsmixed.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsmixed.dasm.il _speed_dbglcsmixed.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbglcsmixed.dasm.il _speed_dbglcsmixed.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbglcsmixed.asm.exe  _speed_dbglcsmixed.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbglcsmixed.asm.exe  _speed_dbglcsmixed.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbglcsmixed
    if [ ! -f _speed_dbglcsmixed.org ]; then
        TakeLock
        if [ ! -f _speed_dbglcsmixed.org ]; then
          mkdir IL
          cp _speed_dbglcsmixed.exe IL/_speed_dbglcsmixed.exe
          mv _speed_dbglcsmixed.exe _speed_dbglcsmixed.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsmixed.org /out _speed_dbglcsmixed.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbglcsmixed.org /out _speed_dbglcsmixed.exe
          __cgExitCode=$?
...
ExePath=_speed_dbglcsmixed.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbglcsmixed.reflect.xml
  if [ -f _speed_dbglcsmixed.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbglcsmixed.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbglcsmixed.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbglcsmixed.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbglcsmixed.asm.exe $CLRTestExecutionArguments
