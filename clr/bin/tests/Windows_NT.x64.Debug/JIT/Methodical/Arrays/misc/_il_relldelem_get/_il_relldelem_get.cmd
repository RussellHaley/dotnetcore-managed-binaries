...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_relldelem_get _il_relldelem_get.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_relldelem_get _il_relldelem_get.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relldelem_get.dasm.il _il_relldelem_get.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relldelem_get.dasm.il _il_relldelem_get.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relldelem_get.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relldelem_get.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relldelem_get.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relldelem_get.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relldelem_get
    if not exist "_il_relldelem_get.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relldelem_get.org" (
            mkdir IL
            copy _il_relldelem_get.exe IL\_il_relldelem_get.exe 
            ren _il_relldelem_get.exe _il_relldelem_get.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relldelem_get.org /out _il_relldelem_get.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relldelem_get.org /out _il_relldelem_get.exe
...
set ExePath=_il_relldelem_get.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relldelem_get.reflect.xml
    IF EXIST _il_relldelem_get.reflect.xml set ReflectionRoots=-x _il_relldelem_get.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relldelem_get.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relldelem_get.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relldelem_get.asm.exe %CLRTestExecutionArguments% %Host_Args%
