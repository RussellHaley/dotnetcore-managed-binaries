...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_relgcarr _il_relgcarr.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_relgcarr _il_relgcarr.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relgcarr.dasm.il _il_relgcarr.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relgcarr.dasm.il _il_relgcarr.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relgcarr.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcarr.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relgcarr.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcarr.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relgcarr
    if [ ! -f _il_relgcarr.org ]; then
        TakeLock
        if [ ! -f _il_relgcarr.org ]; then
          mkdir IL
          cp _il_relgcarr.exe IL/_il_relgcarr.exe
          mv _il_relgcarr.exe _il_relgcarr.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relgcarr.org /out _il_relgcarr.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relgcarr.org /out _il_relgcarr.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relgcarr.reflect.xml
  if [ -f _il_relgcarr.reflect.xml ]
then
    ReflectionRoots="-x _il_relgcarr.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relgcarr.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relgcarr.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relgcarr.asm.exe $CLRTestExecutionArguments
