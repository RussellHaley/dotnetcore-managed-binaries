...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_reladdress _il_reladdress.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_reladdress _il_reladdress.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_reladdress.dasm.il _il_reladdress.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_reladdress.dasm.il _il_reladdress.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_reladdress.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reladdress.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_reladdress.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reladdress.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_reladdress
    if not exist "_il_reladdress.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_reladdress.org" (
            mkdir IL
            copy _il_reladdress.exe IL\_il_reladdress.exe 
            ren _il_reladdress.exe _il_reladdress.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reladdress.org /out _il_reladdress.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reladdress.org /out _il_reladdress.exe
...
set ExePath=_il_reladdress.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_reladdress.reflect.xml
    IF EXIST _il_reladdress.reflect.xml set ReflectionRoots=-x _il_reladdress.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_reladdress.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_reladdress.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_reladdress.asm.exe %CLRTestExecutionArguments% %Host_Args%
