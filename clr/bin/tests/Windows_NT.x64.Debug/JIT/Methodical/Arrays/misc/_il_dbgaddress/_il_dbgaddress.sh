...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_dbgaddress _il_dbgaddress.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_dbgaddress _il_dbgaddress.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgaddress.dasm.il _il_dbgaddress.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgaddress.dasm.il _il_dbgaddress.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgaddress.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgaddress.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgaddress.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgaddress.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgaddress
    if [ ! -f _il_dbgaddress.org ]; then
        TakeLock
        if [ ! -f _il_dbgaddress.org ]; then
          mkdir IL
          cp _il_dbgaddress.exe IL/_il_dbgaddress.exe
          mv _il_dbgaddress.exe _il_dbgaddress.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgaddress.org /out _il_dbgaddress.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgaddress.org /out _il_dbgaddress.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgaddress.reflect.xml
  if [ -f _il_dbgaddress.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgaddress.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgaddress.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgaddress.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgaddress.asm.exe $CLRTestExecutionArguments
