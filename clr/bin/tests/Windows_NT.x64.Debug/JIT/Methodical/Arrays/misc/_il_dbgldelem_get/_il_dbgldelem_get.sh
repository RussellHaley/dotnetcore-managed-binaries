...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_dbgldelem_get _il_dbgldelem_get.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_dbgldelem_get _il_dbgldelem_get.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgldelem_get.dasm.il _il_dbgldelem_get.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgldelem_get.dasm.il _il_dbgldelem_get.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgldelem_get.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgldelem_get.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgldelem_get.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgldelem_get.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgldelem_get
    if [ ! -f _il_dbgldelem_get.org ]; then
        TakeLock
        if [ ! -f _il_dbgldelem_get.org ]; then
          mkdir IL
          cp _il_dbgldelem_get.exe IL/_il_dbgldelem_get.exe
          mv _il_dbgldelem_get.exe _il_dbgldelem_get.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgldelem_get.org /out _il_dbgldelem_get.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgldelem_get.org /out _il_dbgldelem_get.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgldelem_get.reflect.xml
  if [ -f _il_dbgldelem_get.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgldelem_get.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgldelem_get.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgldelem_get.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgldelem_get.asm.exe $CLRTestExecutionArguments
