...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_dbginitializearray _il_dbginitializearray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_dbginitializearray _il_dbginitializearray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbginitializearray.dasm.il _il_dbginitializearray.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbginitializearray.dasm.il _il_dbginitializearray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbginitializearray.asm.exe -QUIET -NOLOGO -DEBUG _il_dbginitializearray.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbginitializearray.asm.exe -QUIET -NOLOGO -DEBUG _il_dbginitializearray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbginitializearray
    if [ ! -f _il_dbginitializearray.org ]; then
        TakeLock
        if [ ! -f _il_dbginitializearray.org ]; then
          mkdir IL
          cp _il_dbginitializearray.exe IL/_il_dbginitializearray.exe
          mv _il_dbginitializearray.exe _il_dbginitializearray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbginitializearray.org /out _il_dbginitializearray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbginitializearray.org /out _il_dbginitializearray.exe
          __cgExitCode=$?
...
ExePath=_il_dbginitializearray.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbginitializearray.reflect.xml
  if [ -f _il_dbginitializearray.reflect.xml ]
then
    ReflectionRoots="-x _il_dbginitializearray.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbginitializearray.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbginitializearray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbginitializearray.asm.exe $CLRTestExecutionArguments
