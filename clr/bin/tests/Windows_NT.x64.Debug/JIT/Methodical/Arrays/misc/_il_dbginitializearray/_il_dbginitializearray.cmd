...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_dbginitializearray _il_dbginitializearray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_dbginitializearray _il_dbginitializearray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbginitializearray.dasm.il _il_dbginitializearray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbginitializearray.dasm.il _il_dbginitializearray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbginitializearray.asm.exe -QUIET -NOLOGO -DEBUG _il_dbginitializearray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbginitializearray.asm.exe -QUIET -NOLOGO -DEBUG _il_dbginitializearray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbginitializearray
    if not exist "_il_dbginitializearray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbginitializearray.org" (
            mkdir IL
            copy _il_dbginitializearray.exe IL\_il_dbginitializearray.exe 
            ren _il_dbginitializearray.exe _il_dbginitializearray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbginitializearray.org /out _il_dbginitializearray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbginitializearray.org /out _il_dbginitializearray.exe
...
set ExePath=_il_dbginitializearray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbginitializearray.reflect.xml
    IF EXIST _il_dbginitializearray.reflect.xml set ReflectionRoots=-x _il_dbginitializearray.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbginitializearray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbginitializearray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbginitializearray.asm.exe %CLRTestExecutionArguments% %Host_Args%
