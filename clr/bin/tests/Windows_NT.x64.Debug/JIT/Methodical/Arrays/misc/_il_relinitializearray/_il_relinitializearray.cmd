...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_relinitializearray _il_relinitializearray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_relinitializearray _il_relinitializearray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relinitializearray.dasm.il _il_relinitializearray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relinitializearray.dasm.il _il_relinitializearray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relinitializearray.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE _il_relinitializearray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relinitializearray.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE _il_relinitializearray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relinitializearray
    if not exist "_il_relinitializearray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relinitializearray.org" (
            mkdir IL
            copy _il_relinitializearray.exe IL\_il_relinitializearray.exe 
            ren _il_relinitializearray.exe _il_relinitializearray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relinitializearray.org /out _il_relinitializearray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relinitializearray.org /out _il_relinitializearray.exe
...
set ExePath=_il_relinitializearray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relinitializearray.reflect.xml
    IF EXIST _il_relinitializearray.reflect.xml set ReflectionRoots=-x _il_relinitializearray.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relinitializearray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relinitializearray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relinitializearray.asm.exe %CLRTestExecutionArguments% %Host_Args%
