...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_reladdress _il_reladdress.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_reladdress _il_reladdress.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_reladdress.dasm.il _il_reladdress.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_reladdress.dasm.il _il_reladdress.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_reladdress.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reladdress.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_reladdress.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reladdress.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_reladdress
    if [ ! -f _il_reladdress.org ]; then
        TakeLock
        if [ ! -f _il_reladdress.org ]; then
          mkdir IL
          cp _il_reladdress.exe IL/_il_reladdress.exe
          mv _il_reladdress.exe _il_reladdress.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reladdress.org /out _il_reladdress.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reladdress.org /out _il_reladdress.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_reladdress.reflect.xml
  if [ -f _il_reladdress.reflect.xml ]
then
    ReflectionRoots="-x _il_reladdress.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_reladdress.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_reladdress.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_reladdress.asm.exe $CLRTestExecutionArguments
