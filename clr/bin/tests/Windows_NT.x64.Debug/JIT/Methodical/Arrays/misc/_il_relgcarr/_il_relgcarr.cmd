...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_relgcarr _il_relgcarr.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_relgcarr _il_relgcarr.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relgcarr.dasm.il _il_relgcarr.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relgcarr.dasm.il _il_relgcarr.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relgcarr.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcarr.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relgcarr.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relgcarr.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relgcarr
    if not exist "_il_relgcarr.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relgcarr.org" (
            mkdir IL
            copy _il_relgcarr.exe IL\_il_relgcarr.exe 
            ren _il_relgcarr.exe _il_relgcarr.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relgcarr.org /out _il_relgcarr.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relgcarr.org /out _il_relgcarr.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relgcarr.reflect.xml
    IF EXIST _il_relgcarr.reflect.xml set ReflectionRoots=-x _il_relgcarr.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relgcarr.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relgcarr.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relgcarr.asm.exe %CLRTestExecutionArguments% %Host_Args%
