...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_dbgaddress _il_dbgaddress.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\misc\_il_dbgaddress _il_dbgaddress.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgaddress.dasm.il _il_dbgaddress.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgaddress.dasm.il _il_dbgaddress.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgaddress.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgaddress.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgaddress.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgaddress.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgaddress
    if not exist "_il_dbgaddress.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgaddress.org" (
            mkdir IL
            copy _il_dbgaddress.exe IL\_il_dbgaddress.exe 
            ren _il_dbgaddress.exe _il_dbgaddress.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgaddress.org /out _il_dbgaddress.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgaddress.org /out _il_dbgaddress.exe
...
set ExePath=_il_dbgaddress.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgaddress.reflect.xml
    IF EXIST _il_dbgaddress.reflect.xml set ReflectionRoots=-x _il_dbgaddress.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgaddress.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgaddress.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgaddress.asm.exe %CLRTestExecutionArguments% %Host_Args%
