...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_relinitializearray _il_relinitializearray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/misc/_il_relinitializearray _il_relinitializearray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relinitializearray.dasm.il _il_relinitializearray.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relinitializearray.dasm.il _il_relinitializearray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relinitializearray.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE _il_relinitializearray.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relinitializearray.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE _il_relinitializearray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relinitializearray
    if [ ! -f _il_relinitializearray.org ]; then
        TakeLock
        if [ ! -f _il_relinitializearray.org ]; then
          mkdir IL
          cp _il_relinitializearray.exe IL/_il_relinitializearray.exe
          mv _il_relinitializearray.exe _il_relinitializearray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relinitializearray.org /out _il_relinitializearray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relinitializearray.org /out _il_relinitializearray.exe
          __cgExitCode=$?
...
ExePath=_il_relinitializearray.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relinitializearray.reflect.xml
  if [ -f _il_relinitializearray.reflect.xml ]
then
    ReflectionRoots="-x _il_relinitializearray.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relinitializearray.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relinitializearray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relinitializearray.asm.exe $CLRTestExecutionArguments
