...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/range/_il_relnegIndexRngChkElim _il_relnegIndexRngChkElim.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/range/_il_relnegIndexRngChkElim _il_relnegIndexRngChkElim.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relnegIndexRngChkElim.dasm.il _il_relnegIndexRngChkElim.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relnegIndexRngChkElim.dasm.il _il_relnegIndexRngChkElim.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relnegIndexRngChkElim.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relnegIndexRngChkElim.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relnegIndexRngChkElim.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relnegIndexRngChkElim.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relnegIndexRngChkElim
    if [ ! -f _il_relnegIndexRngChkElim.org ]; then
        TakeLock
        if [ ! -f _il_relnegIndexRngChkElim.org ]; then
          mkdir IL
          cp _il_relnegIndexRngChkElim.exe IL/_il_relnegIndexRngChkElim.exe
          mv _il_relnegIndexRngChkElim.exe _il_relnegIndexRngChkElim.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relnegIndexRngChkElim.org /out _il_relnegIndexRngChkElim.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relnegIndexRngChkElim.org /out _il_relnegIndexRngChkElim.exe
          __cgExitCode=$?
...
ExePath=_il_relnegIndexRngChkElim.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relnegIndexRngChkElim.reflect.xml
  if [ -f _il_relnegIndexRngChkElim.reflect.xml ]
then
    ReflectionRoots="-x _il_relnegIndexRngChkElim.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relnegIndexRngChkElim.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relnegIndexRngChkElim.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relnegIndexRngChkElim.asm.exe $CLRTestExecutionArguments
