...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\range\_il_relnegIndexRngChkElim _il_relnegIndexRngChkElim.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\range\_il_relnegIndexRngChkElim _il_relnegIndexRngChkElim.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relnegIndexRngChkElim.dasm.il _il_relnegIndexRngChkElim.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relnegIndexRngChkElim.dasm.il _il_relnegIndexRngChkElim.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relnegIndexRngChkElim.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relnegIndexRngChkElim.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relnegIndexRngChkElim.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relnegIndexRngChkElim.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relnegIndexRngChkElim
    if not exist "_il_relnegIndexRngChkElim.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relnegIndexRngChkElim.org" (
            mkdir IL
            copy _il_relnegIndexRngChkElim.exe IL\_il_relnegIndexRngChkElim.exe 
            ren _il_relnegIndexRngChkElim.exe _il_relnegIndexRngChkElim.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relnegIndexRngChkElim.org /out _il_relnegIndexRngChkElim.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relnegIndexRngChkElim.org /out _il_relnegIndexRngChkElim.exe
...
set ExePath=_il_relnegIndexRngChkElim.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relnegIndexRngChkElim.reflect.xml
    IF EXIST _il_relnegIndexRngChkElim.reflect.xml set ReflectionRoots=-x _il_relnegIndexRngChkElim.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relnegIndexRngChkElim.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relnegIndexRngChkElim.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relnegIndexRngChkElim.asm.exe %CLRTestExecutionArguments% %Host_Args%
