...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/range/_il_dbgint32_0_5a _il_dbgint32_0_5a.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/range/_il_dbgint32_0_5a _il_dbgint32_0_5a.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgint32_0_5a.dasm.il _il_dbgint32_0_5a.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgint32_0_5a.dasm.il _il_dbgint32_0_5a.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgint32_0_5a.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgint32_0_5a.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgint32_0_5a.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgint32_0_5a.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgint32_0_5a
    if [ ! -f _il_dbgint32_0_5a.org ]; then
        TakeLock
        if [ ! -f _il_dbgint32_0_5a.org ]; then
          mkdir IL
          cp _il_dbgint32_0_5a.exe IL/_il_dbgint32_0_5a.exe
          mv _il_dbgint32_0_5a.exe _il_dbgint32_0_5a.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgint32_0_5a.org /out _il_dbgint32_0_5a.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgint32_0_5a.org /out _il_dbgint32_0_5a.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgint32_0_5a.reflect.xml
  if [ -f _il_dbgint32_0_5a.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgint32_0_5a.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgint32_0_5a.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgint32_0_5a.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgint32_0_5a.asm.exe $CLRTestExecutionArguments
