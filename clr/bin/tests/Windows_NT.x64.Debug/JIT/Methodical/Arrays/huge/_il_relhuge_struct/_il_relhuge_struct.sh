...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/huge/_il_relhuge_struct _il_relhuge_struct.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Arrays/huge/_il_relhuge_struct _il_relhuge_struct.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relhuge_struct.dasm.il _il_relhuge_struct.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relhuge_struct.dasm.il _il_relhuge_struct.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relhuge_struct.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relhuge_struct.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relhuge_struct.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relhuge_struct.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relhuge_struct
    if [ ! -f _il_relhuge_struct.org ]; then
        TakeLock
        if [ ! -f _il_relhuge_struct.org ]; then
          mkdir IL
          cp _il_relhuge_struct.exe IL/_il_relhuge_struct.exe
          mv _il_relhuge_struct.exe _il_relhuge_struct.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relhuge_struct.org /out _il_relhuge_struct.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relhuge_struct.org /out _il_relhuge_struct.exe
          __cgExitCode=$?
...
ExePath=_il_relhuge_struct.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relhuge_struct.reflect.xml
  if [ -f _il_relhuge_struct.reflect.xml ]
then
    ReflectionRoots="-x _il_relhuge_struct.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relhuge_struct.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relhuge_struct.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relhuge_struct.asm.exe $CLRTestExecutionArguments
