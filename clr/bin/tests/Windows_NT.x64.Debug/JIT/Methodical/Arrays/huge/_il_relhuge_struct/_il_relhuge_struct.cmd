...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\huge\_il_relhuge_struct _il_relhuge_struct.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Arrays\huge\_il_relhuge_struct _il_relhuge_struct.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relhuge_struct.dasm.il _il_relhuge_struct.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relhuge_struct.dasm.il _il_relhuge_struct.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relhuge_struct.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relhuge_struct.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relhuge_struct.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relhuge_struct.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relhuge_struct
    if not exist "_il_relhuge_struct.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relhuge_struct.org" (
            mkdir IL
            copy _il_relhuge_struct.exe IL\_il_relhuge_struct.exe 
            ren _il_relhuge_struct.exe _il_relhuge_struct.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relhuge_struct.org /out _il_relhuge_struct.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relhuge_struct.org /out _il_relhuge_struct.exe
...
set ExePath=_il_relhuge_struct.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relhuge_struct.reflect.xml
    IF EXIST _il_relhuge_struct.reflect.xml set ReflectionRoots=-x _il_relhuge_struct.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relhuge_struct.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relhuge_struct.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relhuge_struct.asm.exe %CLRTestExecutionArguments% %Host_Args%
