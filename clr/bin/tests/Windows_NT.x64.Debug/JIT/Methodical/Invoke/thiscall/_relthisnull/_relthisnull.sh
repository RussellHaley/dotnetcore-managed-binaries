...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/thiscall/_relthisnull _relthisnull.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/thiscall/_relthisnull _relthisnull.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relthisnull.dasm.il _relthisnull.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relthisnull.dasm.il _relthisnull.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relthisnull.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relthisnull.dasm.il
    "$CORE_ROOT/ilasm" -output=_relthisnull.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relthisnull.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relthisnull
    if [ ! -f _relthisnull.org ]; then
        TakeLock
        if [ ! -f _relthisnull.org ]; then
          mkdir IL
          cp _relthisnull.exe IL/_relthisnull.exe
          mv _relthisnull.exe _relthisnull.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relthisnull.org /out _relthisnull.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relthisnull.org /out _relthisnull.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relthisnull.reflect.xml
  if [ -f _relthisnull.reflect.xml ]
then
    ReflectionRoots="-x _relthisnull.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relthisnull.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relthisnull.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relthisnull.asm.exe $CLRTestExecutionArguments
