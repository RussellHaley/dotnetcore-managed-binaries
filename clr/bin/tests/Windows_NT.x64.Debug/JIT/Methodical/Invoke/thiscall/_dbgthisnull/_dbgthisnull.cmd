...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\thiscall\_dbgthisnull _dbgthisnull.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\thiscall\_dbgthisnull _dbgthisnull.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgthisnull.dasm.il _dbgthisnull.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgthisnull.dasm.il _dbgthisnull.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgthisnull.asm.exe -QUIET -NOLOGO -DEBUG _dbgthisnull.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgthisnull.asm.exe -QUIET -NOLOGO -DEBUG _dbgthisnull.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgthisnull
    if not exist "_dbgthisnull.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgthisnull.org" (
            mkdir IL
            copy _dbgthisnull.exe IL\_dbgthisnull.exe 
            ren _dbgthisnull.exe _dbgthisnull.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgthisnull.org /out _dbgthisnull.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgthisnull.org /out _dbgthisnull.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgthisnull.reflect.xml
    IF EXIST _dbgthisnull.reflect.xml set ReflectionRoots=-x _dbgthisnull.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgthisnull.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgthisnull.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgthisnull.asm.exe %CLRTestExecutionArguments% %Host_Args%
