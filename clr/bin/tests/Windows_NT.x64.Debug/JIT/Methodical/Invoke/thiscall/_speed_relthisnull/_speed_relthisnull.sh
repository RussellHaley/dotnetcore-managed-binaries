...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/thiscall/_speed_relthisnull _speed_relthisnull.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/thiscall/_speed_relthisnull _speed_relthisnull.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relthisnull.dasm.il _speed_relthisnull.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relthisnull.dasm.il _speed_relthisnull.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relthisnull.asm.exe -QUIET -NOLOGO -DEBUG=OPT _speed_relthisnull.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relthisnull.asm.exe -QUIET -NOLOGO -DEBUG=OPT _speed_relthisnull.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relthisnull
    if [ ! -f _speed_relthisnull.org ]; then
        TakeLock
        if [ ! -f _speed_relthisnull.org ]; then
          mkdir IL
          cp _speed_relthisnull.exe IL/_speed_relthisnull.exe
          mv _speed_relthisnull.exe _speed_relthisnull.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relthisnull.org /out _speed_relthisnull.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relthisnull.org /out _speed_relthisnull.exe
          __cgExitCode=$?
...
ExePath=_speed_relthisnull.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relthisnull.reflect.xml
  if [ -f _speed_relthisnull.reflect.xml ]
then
    ReflectionRoots="-x _speed_relthisnull.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relthisnull.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relthisnull.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relthisnull.asm.exe $CLRTestExecutionArguments
