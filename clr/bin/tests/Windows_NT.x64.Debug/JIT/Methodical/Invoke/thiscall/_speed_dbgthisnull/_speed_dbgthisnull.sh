...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/thiscall/_speed_dbgthisnull _speed_dbgthisnull.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/thiscall/_speed_dbgthisnull _speed_dbgthisnull.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgthisnull.dasm.il _speed_dbgthisnull.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgthisnull.dasm.il _speed_dbgthisnull.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgthisnull.asm.exe -QUIET -NOLOGO -DEBUG _speed_dbgthisnull.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgthisnull.asm.exe -QUIET -NOLOGO -DEBUG _speed_dbgthisnull.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgthisnull
    if [ ! -f _speed_dbgthisnull.org ]; then
        TakeLock
        if [ ! -f _speed_dbgthisnull.org ]; then
          mkdir IL
          cp _speed_dbgthisnull.exe IL/_speed_dbgthisnull.exe
          mv _speed_dbgthisnull.exe _speed_dbgthisnull.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgthisnull.org /out _speed_dbgthisnull.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgthisnull.org /out _speed_dbgthisnull.exe
          __cgExitCode=$?
...
ExePath=_speed_dbgthisnull.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgthisnull.reflect.xml
  if [ -f _speed_dbgthisnull.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgthisnull.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgthisnull.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgthisnull.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgthisnull.asm.exe $CLRTestExecutionArguments
