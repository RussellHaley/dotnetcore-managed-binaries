...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\thiscall\_relthisnull _relthisnull.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\thiscall\_relthisnull _relthisnull.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relthisnull.dasm.il _relthisnull.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relthisnull.dasm.il _relthisnull.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relthisnull.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relthisnull.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relthisnull.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relthisnull.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relthisnull
    if not exist "_relthisnull.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relthisnull.org" (
            mkdir IL
            copy _relthisnull.exe IL\_relthisnull.exe 
            ren _relthisnull.exe _relthisnull.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relthisnull.org /out _relthisnull.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relthisnull.org /out _relthisnull.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relthisnull.reflect.xml
    IF EXIST _relthisnull.reflect.xml set ReflectionRoots=-x _relthisnull.reflect.xml
    ...
    set ExePath=%LinkBin%\_relthisnull.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relthisnull.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relthisnull.asm.exe %CLRTestExecutionArguments% %Host_Args%
