...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/thiscall/_dbgthisnull _dbgthisnull.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/thiscall/_dbgthisnull _dbgthisnull.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgthisnull.dasm.il _dbgthisnull.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgthisnull.dasm.il _dbgthisnull.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgthisnull.asm.exe -QUIET -NOLOGO -DEBUG _dbgthisnull.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgthisnull.asm.exe -QUIET -NOLOGO -DEBUG _dbgthisnull.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgthisnull
    if [ ! -f _dbgthisnull.org ]; then
        TakeLock
        if [ ! -f _dbgthisnull.org ]; then
          mkdir IL
          cp _dbgthisnull.exe IL/_dbgthisnull.exe
          mv _dbgthisnull.exe _dbgthisnull.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgthisnull.org /out _dbgthisnull.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgthisnull.org /out _dbgthisnull.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgthisnull.reflect.xml
  if [ -f _dbgthisnull.reflect.xml ]
then
    ReflectionRoots="-x _dbgthisnull.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgthisnull.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgthisnull.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgthisnull.asm.exe $CLRTestExecutionArguments
