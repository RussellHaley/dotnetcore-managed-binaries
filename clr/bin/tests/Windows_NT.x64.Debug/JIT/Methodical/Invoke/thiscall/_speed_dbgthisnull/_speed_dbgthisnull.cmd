...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\thiscall\_speed_dbgthisnull _speed_dbgthisnull.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\thiscall\_speed_dbgthisnull _speed_dbgthisnull.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgthisnull.dasm.il _speed_dbgthisnull.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgthisnull.dasm.il _speed_dbgthisnull.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgthisnull.asm.exe -QUIET -NOLOGO -DEBUG _speed_dbgthisnull.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgthisnull.asm.exe -QUIET -NOLOGO -DEBUG _speed_dbgthisnull.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgthisnull
    if not exist "_speed_dbgthisnull.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgthisnull.org" (
            mkdir IL
            copy _speed_dbgthisnull.exe IL\_speed_dbgthisnull.exe 
            ren _speed_dbgthisnull.exe _speed_dbgthisnull.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgthisnull.org /out _speed_dbgthisnull.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgthisnull.org /out _speed_dbgthisnull.exe
...
set ExePath=_speed_dbgthisnull.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgthisnull.reflect.xml
    IF EXIST _speed_dbgthisnull.reflect.xml set ReflectionRoots=-x _speed_dbgthisnull.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgthisnull.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgthisnull.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgthisnull.asm.exe %CLRTestExecutionArguments% %Host_Args%
