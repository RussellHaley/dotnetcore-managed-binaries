...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\thiscall\_speed_relthisnull _speed_relthisnull.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\thiscall\_speed_relthisnull _speed_relthisnull.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relthisnull.dasm.il _speed_relthisnull.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relthisnull.dasm.il _speed_relthisnull.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_relthisnull.asm.exe -QUIET -NOLOGO -DEBUG=OPT _speed_relthisnull.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_relthisnull.asm.exe -QUIET -NOLOGO -DEBUG=OPT _speed_relthisnull.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_relthisnull
    if not exist "_speed_relthisnull.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_relthisnull.org" (
            mkdir IL
            copy _speed_relthisnull.exe IL\_speed_relthisnull.exe 
            ren _speed_relthisnull.exe _speed_relthisnull.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relthisnull.org /out _speed_relthisnull.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relthisnull.org /out _speed_relthisnull.exe
...
set ExePath=_speed_relthisnull.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_relthisnull.reflect.xml
    IF EXIST _speed_relthisnull.reflect.xml set ReflectionRoots=-x _speed_relthisnull.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_relthisnull.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_relthisnull.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_relthisnull.asm.exe %CLRTestExecutionArguments% %Host_Args%
