...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/deep/_speed_reldeep _speed_reldeep.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/deep/_speed_reldeep _speed_reldeep.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_reldeep.dasm.il _speed_reldeep.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_reldeep.dasm.il _speed_reldeep.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_reldeep.asm.exe  _speed_reldeep.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_reldeep.asm.exe  _speed_reldeep.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_reldeep
    if [ ! -f _speed_reldeep.org ]; then
        TakeLock
        if [ ! -f _speed_reldeep.org ]; then
          mkdir IL
          cp _speed_reldeep.exe IL/_speed_reldeep.exe
          mv _speed_reldeep.exe _speed_reldeep.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_reldeep.org /out _speed_reldeep.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_reldeep.org /out _speed_reldeep.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_reldeep.reflect.xml
  if [ -f _speed_reldeep.reflect.xml ]
then
    ReflectionRoots="-x _speed_reldeep.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_reldeep.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_reldeep.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_reldeep.asm.exe $CLRTestExecutionArguments
