...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\deep\_speed_dbgdeep _speed_dbgdeep.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\deep\_speed_dbgdeep _speed_dbgdeep.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgdeep.dasm.il _speed_dbgdeep.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgdeep.dasm.il _speed_dbgdeep.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgdeep.asm.exe  _speed_dbgdeep.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgdeep.asm.exe  _speed_dbgdeep.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgdeep
    if not exist "_speed_dbgdeep.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgdeep.org" (
            mkdir IL
            copy _speed_dbgdeep.exe IL\_speed_dbgdeep.exe 
            ren _speed_dbgdeep.exe _speed_dbgdeep.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgdeep.org /out _speed_dbgdeep.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgdeep.org /out _speed_dbgdeep.exe
...
set ExePath=_speed_dbgdeep.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgdeep.reflect.xml
    IF EXIST _speed_dbgdeep.reflect.xml set ReflectionRoots=-x _speed_dbgdeep.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgdeep.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgdeep.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgdeep.asm.exe %CLRTestExecutionArguments% %Host_Args%
