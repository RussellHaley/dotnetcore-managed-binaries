...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\deep\_dbgdeep _dbgdeep.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\deep\_dbgdeep _dbgdeep.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgdeep.dasm.il _dbgdeep.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgdeep.dasm.il _dbgdeep.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgdeep.asm.exe  _dbgdeep.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgdeep.asm.exe  _dbgdeep.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgdeep
    if not exist "_dbgdeep.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgdeep.org" (
            mkdir IL
            copy _dbgdeep.exe IL\_dbgdeep.exe 
            ren _dbgdeep.exe _dbgdeep.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgdeep.org /out _dbgdeep.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgdeep.org /out _dbgdeep.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgdeep.reflect.xml
    IF EXIST _dbgdeep.reflect.xml set ReflectionRoots=-x _dbgdeep.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgdeep.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgdeep.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgdeep.asm.exe %CLRTestExecutionArguments% %Host_Args%
