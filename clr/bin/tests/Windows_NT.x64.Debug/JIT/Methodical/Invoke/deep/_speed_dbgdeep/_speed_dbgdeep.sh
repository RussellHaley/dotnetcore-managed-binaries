...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/deep/_speed_dbgdeep _speed_dbgdeep.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/deep/_speed_dbgdeep _speed_dbgdeep.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgdeep.dasm.il _speed_dbgdeep.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgdeep.dasm.il _speed_dbgdeep.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgdeep.asm.exe  _speed_dbgdeep.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgdeep.asm.exe  _speed_dbgdeep.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgdeep
    if [ ! -f _speed_dbgdeep.org ]; then
        TakeLock
        if [ ! -f _speed_dbgdeep.org ]; then
          mkdir IL
          cp _speed_dbgdeep.exe IL/_speed_dbgdeep.exe
          mv _speed_dbgdeep.exe _speed_dbgdeep.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgdeep.org /out _speed_dbgdeep.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgdeep.org /out _speed_dbgdeep.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgdeep.reflect.xml
  if [ -f _speed_dbgdeep.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgdeep.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgdeep.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgdeep.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgdeep.asm.exe $CLRTestExecutionArguments
