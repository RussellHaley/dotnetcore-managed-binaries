...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\deep\_speed_reldeep _speed_reldeep.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\deep\_speed_reldeep _speed_reldeep.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_reldeep.dasm.il _speed_reldeep.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_reldeep.dasm.il _speed_reldeep.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_reldeep.asm.exe  _speed_reldeep.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_reldeep.asm.exe  _speed_reldeep.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_reldeep
    if not exist "_speed_reldeep.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_reldeep.org" (
            mkdir IL
            copy _speed_reldeep.exe IL\_speed_reldeep.exe 
            ren _speed_reldeep.exe _speed_reldeep.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_reldeep.org /out _speed_reldeep.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_reldeep.org /out _speed_reldeep.exe
...
set ExePath=_speed_reldeep.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_reldeep.reflect.xml
    IF EXIST _speed_reldeep.reflect.xml set ReflectionRoots=-x _speed_reldeep.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_reldeep.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_reldeep.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_reldeep.asm.exe %CLRTestExecutionArguments% %Host_Args%
