...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/deep/_reldeep _reldeep.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/deep/_reldeep _reldeep.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_reldeep.dasm.il _reldeep.exe
    "$CORE_ROOT/ildasm" -raweh -out=_reldeep.dasm.il _reldeep.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_reldeep.asm.exe  _reldeep.dasm.il
    "$CORE_ROOT/ilasm" -output=_reldeep.asm.exe  _reldeep.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_reldeep
    if [ ! -f _reldeep.org ]; then
        TakeLock
        if [ ! -f _reldeep.org ]; then
          mkdir IL
          cp _reldeep.exe IL/_reldeep.exe
          mv _reldeep.exe _reldeep.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reldeep.org /out _reldeep.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reldeep.org /out _reldeep.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _reldeep.reflect.xml
  if [ -f _reldeep.reflect.xml ]
then
    ReflectionRoots="-x _reldeep.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _reldeep.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _reldeep.asm.exe $CLRTestExecutionArguments
