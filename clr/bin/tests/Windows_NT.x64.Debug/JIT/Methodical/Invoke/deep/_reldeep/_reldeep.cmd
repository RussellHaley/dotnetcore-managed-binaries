...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\deep\_reldeep _reldeep.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\deep\_reldeep _reldeep.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_reldeep.dasm.il _reldeep.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_reldeep.dasm.il _reldeep.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_reldeep.asm.exe  _reldeep.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_reldeep.asm.exe  _reldeep.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_reldeep
    if not exist "_reldeep.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_reldeep.org" (
            mkdir IL
            copy _reldeep.exe IL\_reldeep.exe 
            ren _reldeep.exe _reldeep.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _reldeep.org /out _reldeep.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _reldeep.org /out _reldeep.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _reldeep.reflect.xml
    IF EXIST _reldeep.reflect.xml set ReflectionRoots=-x _reldeep.reflect.xml
    ...
    set ExePath=%LinkBin%\_reldeep.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _reldeep.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _reldeep.asm.exe %CLRTestExecutionArguments% %Host_Args%
