...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/deep/_dbgdeep _dbgdeep.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/deep/_dbgdeep _dbgdeep.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgdeep.dasm.il _dbgdeep.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgdeep.dasm.il _dbgdeep.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgdeep.asm.exe  _dbgdeep.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgdeep.asm.exe  _dbgdeep.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgdeep
    if [ ! -f _dbgdeep.org ]; then
        TakeLock
        if [ ! -f _dbgdeep.org ]; then
          mkdir IL
          cp _dbgdeep.exe IL/_dbgdeep.exe
          mv _dbgdeep.exe _dbgdeep.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgdeep.org /out _dbgdeep.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgdeep.org /out _dbgdeep.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgdeep.reflect.xml
  if [ -f _dbgdeep.reflect.xml ]
then
    ReflectionRoots="-x _dbgdeep.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgdeep.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgdeep.asm.exe $CLRTestExecutionArguments
