...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/fptr/_il_dbgrecurse_tail_calli _il_dbgrecurse_tail_calli.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/fptr/_il_dbgrecurse_tail_calli _il_dbgrecurse_tail_calli.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrecurse_tail_calli.dasm.il _il_dbgrecurse_tail_calli.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgrecurse_tail_calli.dasm.il _il_dbgrecurse_tail_calli.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgrecurse_tail_calli.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_tail_calli.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgrecurse_tail_calli.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_tail_calli.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgrecurse_tail_calli
    if [ ! -f _il_dbgrecurse_tail_calli.org ]; then
        TakeLock
        if [ ! -f _il_dbgrecurse_tail_calli.org ]; then
          mkdir IL
          cp _il_dbgrecurse_tail_calli.exe IL/_il_dbgrecurse_tail_calli.exe
          mv _il_dbgrecurse_tail_calli.exe _il_dbgrecurse_tail_calli.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrecurse_tail_calli.org /out _il_dbgrecurse_tail_calli.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgrecurse_tail_calli.org /out _il_dbgrecurse_tail_calli.exe
          __cgExitCode=$?
...
ExePath=_il_dbgrecurse_tail_calli.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgrecurse_tail_calli.reflect.xml
  if [ -f _il_dbgrecurse_tail_calli.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgrecurse_tail_calli.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgrecurse_tail_calli.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgrecurse_tail_calli.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgrecurse_tail_calli.asm.exe $CLRTestExecutionArguments
