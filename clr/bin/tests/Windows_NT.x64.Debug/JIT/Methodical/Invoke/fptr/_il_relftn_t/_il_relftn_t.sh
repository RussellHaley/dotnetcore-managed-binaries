...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/fptr/_il_relftn_t _il_relftn_t.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/fptr/_il_relftn_t _il_relftn_t.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relftn_t.dasm.il _il_relftn_t.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relftn_t.dasm.il _il_relftn_t.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relftn_t.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relftn_t.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relftn_t.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relftn_t.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relftn_t
    if [ ! -f _il_relftn_t.org ]; then
        TakeLock
        if [ ! -f _il_relftn_t.org ]; then
          mkdir IL
          cp _il_relftn_t.exe IL/_il_relftn_t.exe
          mv _il_relftn_t.exe _il_relftn_t.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relftn_t.org /out _il_relftn_t.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relftn_t.org /out _il_relftn_t.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relftn_t.reflect.xml
  if [ -f _il_relftn_t.reflect.xml ]
then
    ReflectionRoots="-x _il_relftn_t.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relftn_t.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relftn_t.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relftn_t.asm.exe $CLRTestExecutionArguments
