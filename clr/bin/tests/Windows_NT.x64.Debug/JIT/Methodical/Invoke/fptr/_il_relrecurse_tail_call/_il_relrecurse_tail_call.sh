...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/fptr/_il_relrecurse_tail_call _il_relrecurse_tail_call.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/fptr/_il_relrecurse_tail_call _il_relrecurse_tail_call.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relrecurse_tail_call.dasm.il _il_relrecurse_tail_call.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relrecurse_tail_call.dasm.il _il_relrecurse_tail_call.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relrecurse_tail_call.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrecurse_tail_call.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relrecurse_tail_call.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relrecurse_tail_call.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relrecurse_tail_call
    if [ ! -f _il_relrecurse_tail_call.org ]; then
        TakeLock
        if [ ! -f _il_relrecurse_tail_call.org ]; then
          mkdir IL
          cp _il_relrecurse_tail_call.exe IL/_il_relrecurse_tail_call.exe
          mv _il_relrecurse_tail_call.exe _il_relrecurse_tail_call.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relrecurse_tail_call.org /out _il_relrecurse_tail_call.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relrecurse_tail_call.org /out _il_relrecurse_tail_call.exe
          __cgExitCode=$?
...
ExePath=_il_relrecurse_tail_call.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relrecurse_tail_call.reflect.xml
  if [ -f _il_relrecurse_tail_call.reflect.xml ]
then
    ReflectionRoots="-x _il_relrecurse_tail_call.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relrecurse_tail_call.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relrecurse_tail_call.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relrecurse_tail_call.asm.exe $CLRTestExecutionArguments
