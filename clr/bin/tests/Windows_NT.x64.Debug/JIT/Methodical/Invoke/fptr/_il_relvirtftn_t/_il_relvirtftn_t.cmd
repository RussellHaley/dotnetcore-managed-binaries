...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\fptr\_il_relvirtftn_t _il_relvirtftn_t.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\fptr\_il_relvirtftn_t _il_relvirtftn_t.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relvirtftn_t.dasm.il _il_relvirtftn_t.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relvirtftn_t.dasm.il _il_relvirtftn_t.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relvirtftn_t.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relvirtftn_t.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relvirtftn_t.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relvirtftn_t.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relvirtftn_t
    if not exist "_il_relvirtftn_t.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relvirtftn_t.org" (
            mkdir IL
            copy _il_relvirtftn_t.exe IL\_il_relvirtftn_t.exe 
            ren _il_relvirtftn_t.exe _il_relvirtftn_t.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relvirtftn_t.org /out _il_relvirtftn_t.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relvirtftn_t.org /out _il_relvirtftn_t.exe
...
set ExePath=_il_relvirtftn_t.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relvirtftn_t.reflect.xml
    IF EXIST _il_relvirtftn_t.reflect.xml set ReflectionRoots=-x _il_relvirtftn_t.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relvirtftn_t.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relvirtftn_t.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relvirtftn_t.asm.exe %CLRTestExecutionArguments% %Host_Args%
