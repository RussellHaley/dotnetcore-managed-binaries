...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/fptr/_il_dbgvalftn_t _il_dbgvalftn_t.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/fptr/_il_dbgvalftn_t _il_dbgvalftn_t.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgvalftn_t.dasm.il _il_dbgvalftn_t.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgvalftn_t.dasm.il _il_dbgvalftn_t.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgvalftn_t.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgvalftn_t.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgvalftn_t.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgvalftn_t.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgvalftn_t
    if [ ! -f _il_dbgvalftn_t.org ]; then
        TakeLock
        if [ ! -f _il_dbgvalftn_t.org ]; then
          mkdir IL
          cp _il_dbgvalftn_t.exe IL/_il_dbgvalftn_t.exe
          mv _il_dbgvalftn_t.exe _il_dbgvalftn_t.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgvalftn_t.org /out _il_dbgvalftn_t.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgvalftn_t.org /out _il_dbgvalftn_t.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgvalftn_t.reflect.xml
  if [ -f _il_dbgvalftn_t.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgvalftn_t.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgvalftn_t.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgvalftn_t.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgvalftn_t.asm.exe $CLRTestExecutionArguments
