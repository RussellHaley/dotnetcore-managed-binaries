...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\fptr\_il_dbgrecurse_tail_calli _il_dbgrecurse_tail_calli.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\fptr\_il_dbgrecurse_tail_calli _il_dbgrecurse_tail_calli.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrecurse_tail_calli.dasm.il _il_dbgrecurse_tail_calli.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrecurse_tail_calli.dasm.il _il_dbgrecurse_tail_calli.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgrecurse_tail_calli.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_tail_calli.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgrecurse_tail_calli.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_tail_calli.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgrecurse_tail_calli
    if not exist "_il_dbgrecurse_tail_calli.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgrecurse_tail_calli.org" (
            mkdir IL
            copy _il_dbgrecurse_tail_calli.exe IL\_il_dbgrecurse_tail_calli.exe 
            ren _il_dbgrecurse_tail_calli.exe _il_dbgrecurse_tail_calli.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrecurse_tail_calli.org /out _il_dbgrecurse_tail_calli.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrecurse_tail_calli.org /out _il_dbgrecurse_tail_calli.exe
...
set ExePath=_il_dbgrecurse_tail_calli.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgrecurse_tail_calli.reflect.xml
    IF EXIST _il_dbgrecurse_tail_calli.reflect.xml set ReflectionRoots=-x _il_dbgrecurse_tail_calli.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgrecurse_tail_calli.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgrecurse_tail_calli.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgrecurse_tail_calli.asm.exe %CLRTestExecutionArguments% %Host_Args%
