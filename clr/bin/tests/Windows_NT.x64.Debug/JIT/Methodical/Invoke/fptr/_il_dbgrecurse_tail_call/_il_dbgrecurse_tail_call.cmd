...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\fptr\_il_dbgrecurse_tail_call _il_dbgrecurse_tail_call.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\fptr\_il_dbgrecurse_tail_call _il_dbgrecurse_tail_call.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrecurse_tail_call.dasm.il _il_dbgrecurse_tail_call.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgrecurse_tail_call.dasm.il _il_dbgrecurse_tail_call.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgrecurse_tail_call.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_tail_call.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgrecurse_tail_call.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgrecurse_tail_call.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgrecurse_tail_call
    if not exist "_il_dbgrecurse_tail_call.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgrecurse_tail_call.org" (
            mkdir IL
            copy _il_dbgrecurse_tail_call.exe IL\_il_dbgrecurse_tail_call.exe 
            ren _il_dbgrecurse_tail_call.exe _il_dbgrecurse_tail_call.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrecurse_tail_call.org /out _il_dbgrecurse_tail_call.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgrecurse_tail_call.org /out _il_dbgrecurse_tail_call.exe
...
set ExePath=_il_dbgrecurse_tail_call.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgrecurse_tail_call.reflect.xml
    IF EXIST _il_dbgrecurse_tail_call.reflect.xml set ReflectionRoots=-x _il_dbgrecurse_tail_call.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgrecurse_tail_call.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgrecurse_tail_call.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgrecurse_tail_call.asm.exe %CLRTestExecutionArguments% %Host_Args%
