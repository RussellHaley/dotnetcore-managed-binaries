...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\25params\25paramMixed_il_r 25paramMixed_il_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\25params\25paramMixed_il_r 25paramMixed_il_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=25paramMixed_il_r.dasm.il 25paramMixed_il_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=25paramMixed_il_r.dasm.il 25paramMixed_il_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=25paramMixed_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT 25paramMixed_il_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=25paramMixed_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT 25paramMixed_il_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=25paramMixed_il_r
    if not exist "25paramMixed_il_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "25paramMixed_il_r.org" (
            mkdir IL
            copy 25paramMixed_il_r.exe IL\25paramMixed_il_r.exe 
            ren 25paramMixed_il_r.exe 25paramMixed_il_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 25paramMixed_il_r.org /out 25paramMixed_il_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 25paramMixed_il_r.org /out 25paramMixed_il_r.exe
...
set ExePath=25paramMixed_il_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 25paramMixed_il_r.reflect.xml
    IF EXIST 25paramMixed_il_r.reflect.xml set ReflectionRoots=-x 25paramMixed_il_r.reflect.xml
    ...
    set ExePath=%LinkBin%\25paramMixed_il_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 25paramMixed_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 25paramMixed_il_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
