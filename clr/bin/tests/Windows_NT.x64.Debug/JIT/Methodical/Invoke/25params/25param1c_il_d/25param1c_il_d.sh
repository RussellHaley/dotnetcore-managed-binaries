...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/25params/25param1c_il_d 25param1c_il_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/25params/25param1c_il_d 25param1c_il_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=25param1c_il_d.dasm.il 25param1c_il_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=25param1c_il_d.dasm.il 25param1c_il_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=25param1c_il_d.asm.exe -QUIET -NOLOGO -DEBUG 25param1c_il_d.dasm.il
    "$CORE_ROOT/ilasm" -output=25param1c_il_d.asm.exe -QUIET -NOLOGO -DEBUG 25param1c_il_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=25param1c_il_d
    if [ ! -f 25param1c_il_d.org ]; then
        TakeLock
        if [ ! -f 25param1c_il_d.org ]; then
          mkdir IL
          cp 25param1c_il_d.exe IL/25param1c_il_d.exe
          mv 25param1c_il_d.exe 25param1c_il_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 25param1c_il_d.org /out 25param1c_il_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 25param1c_il_d.org /out 25param1c_il_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 25param1c_il_d.reflect.xml
  if [ -f 25param1c_il_d.reflect.xml ]
then
    ReflectionRoots="-x 25param1c_il_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/25param1c_il_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 25param1c_il_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 25param1c_il_d.asm.exe $CLRTestExecutionArguments
