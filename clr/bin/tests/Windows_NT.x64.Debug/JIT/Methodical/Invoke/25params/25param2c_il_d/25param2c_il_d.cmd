...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\25params\25param2c_il_d 25param2c_il_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\25params\25param2c_il_d 25param2c_il_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=25param2c_il_d.dasm.il 25param2c_il_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=25param2c_il_d.dasm.il 25param2c_il_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=25param2c_il_d.asm.exe -QUIET -NOLOGO -DEBUG 25param2c_il_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=25param2c_il_d.asm.exe -QUIET -NOLOGO -DEBUG 25param2c_il_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=25param2c_il_d
    if not exist "25param2c_il_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "25param2c_il_d.org" (
            mkdir IL
            copy 25param2c_il_d.exe IL\25param2c_il_d.exe 
            ren 25param2c_il_d.exe 25param2c_il_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 25param2c_il_d.org /out 25param2c_il_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 25param2c_il_d.org /out 25param2c_il_d.exe
...
set ExePath=25param2c_il_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 25param2c_il_d.reflect.xml
    IF EXIST 25param2c_il_d.reflect.xml set ReflectionRoots=-x 25param2c_il_d.reflect.xml
    ...
    set ExePath=%LinkBin%\25param2c_il_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 25param2c_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 25param2c_il_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
