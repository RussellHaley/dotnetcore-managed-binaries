...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/25params/25param3c_il_r 25param3c_il_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/25params/25param3c_il_r 25param3c_il_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=25param3c_il_r.dasm.il 25param3c_il_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=25param3c_il_r.dasm.il 25param3c_il_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=25param3c_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT 25param3c_il_r.dasm.il
    "$CORE_ROOT/ilasm" -output=25param3c_il_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT 25param3c_il_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=25param3c_il_r
    if [ ! -f 25param3c_il_r.org ]; then
        TakeLock
        if [ ! -f 25param3c_il_r.org ]; then
          mkdir IL
          cp 25param3c_il_r.exe IL/25param3c_il_r.exe
          mv 25param3c_il_r.exe 25param3c_il_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 25param3c_il_r.org /out 25param3c_il_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 25param3c_il_r.org /out 25param3c_il_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 25param3c_il_r.reflect.xml
  if [ -f 25param3c_il_r.reflect.xml ]
then
    ReflectionRoots="-x 25param3c_il_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/25param3c_il_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 25param3c_il_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 25param3c_il_r.asm.exe $CLRTestExecutionArguments
