...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_relcatchfinally _relcatchfinally.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_relcatchfinally _relcatchfinally.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relcatchfinally.dasm.il _relcatchfinally.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relcatchfinally.dasm.il _relcatchfinally.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relcatchfinally.asm.exe  _relcatchfinally.dasm.il
    "$CORE_ROOT/ilasm" -output=_relcatchfinally.asm.exe  _relcatchfinally.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relcatchfinally
    if [ ! -f _relcatchfinally.org ]; then
        TakeLock
        if [ ! -f _relcatchfinally.org ]; then
          mkdir IL
          cp _relcatchfinally.exe IL/_relcatchfinally.exe
          mv _relcatchfinally.exe _relcatchfinally.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relcatchfinally.org /out _relcatchfinally.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relcatchfinally.org /out _relcatchfinally.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relcatchfinally.reflect.xml
  if [ -f _relcatchfinally.reflect.xml ]
then
    ReflectionRoots="-x _relcatchfinally.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relcatchfinally.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relcatchfinally.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relcatchfinally.asm.exe $CLRTestExecutionArguments
