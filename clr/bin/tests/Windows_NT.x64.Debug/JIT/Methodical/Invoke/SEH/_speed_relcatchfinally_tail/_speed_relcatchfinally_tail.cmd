...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_speed_relcatchfinally_tail _speed_relcatchfinally_tail.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_speed_relcatchfinally_tail _speed_relcatchfinally_tail.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relcatchfinally_tail.dasm.il _speed_relcatchfinally_tail.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relcatchfinally_tail.dasm.il _speed_relcatchfinally_tail.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_relcatchfinally_tail.asm.exe  _speed_relcatchfinally_tail.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_relcatchfinally_tail.asm.exe  _speed_relcatchfinally_tail.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_relcatchfinally_tail
    if not exist "_speed_relcatchfinally_tail.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_relcatchfinally_tail.org" (
            mkdir IL
            copy _speed_relcatchfinally_tail.exe IL\_speed_relcatchfinally_tail.exe 
            ren _speed_relcatchfinally_tail.exe _speed_relcatchfinally_tail.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relcatchfinally_tail.org /out _speed_relcatchfinally_tail.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relcatchfinally_tail.org /out _speed_relcatchfinally_tail.exe
...
set ExePath=_speed_relcatchfinally_tail.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_relcatchfinally_tail.reflect.xml
    IF EXIST _speed_relcatchfinally_tail.reflect.xml set ReflectionRoots=-x _speed_relcatchfinally_tail.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_relcatchfinally_tail.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_relcatchfinally_tail.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_relcatchfinally_tail.asm.exe %CLRTestExecutionArguments% %Host_Args%
