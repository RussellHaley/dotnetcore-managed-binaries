...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_dbgcatchfault_tail _il_dbgcatchfault_tail.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_dbgcatchfault_tail _il_dbgcatchfault_tail.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcatchfault_tail.dasm.il _il_dbgcatchfault_tail.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcatchfault_tail.dasm.il _il_dbgcatchfault_tail.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgcatchfault_tail.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfault_tail.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgcatchfault_tail.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfault_tail.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgcatchfault_tail
    if not exist "_il_dbgcatchfault_tail.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgcatchfault_tail.org" (
            mkdir IL
            copy _il_dbgcatchfault_tail.exe IL\_il_dbgcatchfault_tail.exe 
            ren _il_dbgcatchfault_tail.exe _il_dbgcatchfault_tail.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcatchfault_tail.org /out _il_dbgcatchfault_tail.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcatchfault_tail.org /out _il_dbgcatchfault_tail.exe
...
set ExePath=_il_dbgcatchfault_tail.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgcatchfault_tail.reflect.xml
    IF EXIST _il_dbgcatchfault_tail.reflect.xml set ReflectionRoots=-x _il_dbgcatchfault_tail.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgcatchfault_tail.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgcatchfault_tail.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgcatchfault_tail.asm.exe %CLRTestExecutionArguments% %Host_Args%
