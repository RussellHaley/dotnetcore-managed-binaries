...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_dbgcatchfinally_tail _dbgcatchfinally_tail.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_dbgcatchfinally_tail _dbgcatchfinally_tail.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgcatchfinally_tail.dasm.il _dbgcatchfinally_tail.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgcatchfinally_tail.dasm.il _dbgcatchfinally_tail.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgcatchfinally_tail.asm.exe  _dbgcatchfinally_tail.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgcatchfinally_tail.asm.exe  _dbgcatchfinally_tail.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgcatchfinally_tail
    if [ ! -f _dbgcatchfinally_tail.org ]; then
        TakeLock
        if [ ! -f _dbgcatchfinally_tail.org ]; then
          mkdir IL
          cp _dbgcatchfinally_tail.exe IL/_dbgcatchfinally_tail.exe
          mv _dbgcatchfinally_tail.exe _dbgcatchfinally_tail.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcatchfinally_tail.org /out _dbgcatchfinally_tail.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcatchfinally_tail.org /out _dbgcatchfinally_tail.exe
          __cgExitCode=$?
...
ExePath=_dbgcatchfinally_tail.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgcatchfinally_tail.reflect.xml
  if [ -f _dbgcatchfinally_tail.reflect.xml ]
then
    ReflectionRoots="-x _dbgcatchfinally_tail.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgcatchfinally_tail.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgcatchfinally_tail.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgcatchfinally_tail.asm.exe $CLRTestExecutionArguments
