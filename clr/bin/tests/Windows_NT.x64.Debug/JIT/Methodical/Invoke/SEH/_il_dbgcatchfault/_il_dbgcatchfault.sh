...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_dbgcatchfault _il_dbgcatchfault.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_dbgcatchfault _il_dbgcatchfault.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcatchfault.dasm.il _il_dbgcatchfault.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcatchfault.dasm.il _il_dbgcatchfault.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgcatchfault.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfault.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgcatchfault.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfault.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgcatchfault
    if [ ! -f _il_dbgcatchfault.org ]; then
        TakeLock
        if [ ! -f _il_dbgcatchfault.org ]; then
          mkdir IL
          cp _il_dbgcatchfault.exe IL/_il_dbgcatchfault.exe
          mv _il_dbgcatchfault.exe _il_dbgcatchfault.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcatchfault.org /out _il_dbgcatchfault.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcatchfault.org /out _il_dbgcatchfault.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgcatchfault.reflect.xml
  if [ -f _il_dbgcatchfault.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgcatchfault.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgcatchfault.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgcatchfault.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgcatchfault.asm.exe $CLRTestExecutionArguments
