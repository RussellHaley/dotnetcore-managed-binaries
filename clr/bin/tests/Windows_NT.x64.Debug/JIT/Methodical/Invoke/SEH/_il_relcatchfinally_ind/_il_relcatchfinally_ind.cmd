...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_relcatchfinally_ind _il_relcatchfinally_ind.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_relcatchfinally_ind _il_relcatchfinally_ind.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relcatchfinally_ind.dasm.il _il_relcatchfinally_ind.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relcatchfinally_ind.dasm.il _il_relcatchfinally_ind.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relcatchfinally_ind.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relcatchfinally_ind.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relcatchfinally_ind.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relcatchfinally_ind.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relcatchfinally_ind
    if not exist "_il_relcatchfinally_ind.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relcatchfinally_ind.org" (
            mkdir IL
            copy _il_relcatchfinally_ind.exe IL\_il_relcatchfinally_ind.exe 
            ren _il_relcatchfinally_ind.exe _il_relcatchfinally_ind.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relcatchfinally_ind.org /out _il_relcatchfinally_ind.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relcatchfinally_ind.org /out _il_relcatchfinally_ind.exe
...
set ExePath=_il_relcatchfinally_ind.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relcatchfinally_ind.reflect.xml
    IF EXIST _il_relcatchfinally_ind.reflect.xml set ReflectionRoots=-x _il_relcatchfinally_ind.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relcatchfinally_ind.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relcatchfinally_ind.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relcatchfinally_ind.asm.exe %CLRTestExecutionArguments% %Host_Args%
