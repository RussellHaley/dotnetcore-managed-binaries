...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_dbgcatchfinally_tail _dbgcatchfinally_tail.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_dbgcatchfinally_tail _dbgcatchfinally_tail.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgcatchfinally_tail.dasm.il _dbgcatchfinally_tail.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgcatchfinally_tail.dasm.il _dbgcatchfinally_tail.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgcatchfinally_tail.asm.exe  _dbgcatchfinally_tail.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgcatchfinally_tail.asm.exe  _dbgcatchfinally_tail.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgcatchfinally_tail
    if not exist "_dbgcatchfinally_tail.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgcatchfinally_tail.org" (
            mkdir IL
            copy _dbgcatchfinally_tail.exe IL\_dbgcatchfinally_tail.exe 
            ren _dbgcatchfinally_tail.exe _dbgcatchfinally_tail.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgcatchfinally_tail.org /out _dbgcatchfinally_tail.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgcatchfinally_tail.org /out _dbgcatchfinally_tail.exe
...
set ExePath=_dbgcatchfinally_tail.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgcatchfinally_tail.reflect.xml
    IF EXIST _dbgcatchfinally_tail.reflect.xml set ReflectionRoots=-x _dbgcatchfinally_tail.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgcatchfinally_tail.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgcatchfinally_tail.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgcatchfinally_tail.asm.exe %CLRTestExecutionArguments% %Host_Args%
