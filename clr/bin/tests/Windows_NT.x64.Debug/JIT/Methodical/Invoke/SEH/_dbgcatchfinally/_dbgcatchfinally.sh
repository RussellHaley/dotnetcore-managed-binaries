...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_dbgcatchfinally _dbgcatchfinally.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_dbgcatchfinally _dbgcatchfinally.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgcatchfinally.dasm.il _dbgcatchfinally.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgcatchfinally.dasm.il _dbgcatchfinally.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgcatchfinally.asm.exe  _dbgcatchfinally.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgcatchfinally.asm.exe  _dbgcatchfinally.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgcatchfinally
    if [ ! -f _dbgcatchfinally.org ]; then
        TakeLock
        if [ ! -f _dbgcatchfinally.org ]; then
          mkdir IL
          cp _dbgcatchfinally.exe IL/_dbgcatchfinally.exe
          mv _dbgcatchfinally.exe _dbgcatchfinally.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcatchfinally.org /out _dbgcatchfinally.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgcatchfinally.org /out _dbgcatchfinally.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgcatchfinally.reflect.xml
  if [ -f _dbgcatchfinally.reflect.xml ]
then
    ReflectionRoots="-x _dbgcatchfinally.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgcatchfinally.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgcatchfinally.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgcatchfinally.asm.exe $CLRTestExecutionArguments
