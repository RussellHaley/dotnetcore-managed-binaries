...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_relcatchfinally _relcatchfinally.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_relcatchfinally _relcatchfinally.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relcatchfinally.dasm.il _relcatchfinally.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relcatchfinally.dasm.il _relcatchfinally.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relcatchfinally.asm.exe  _relcatchfinally.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relcatchfinally.asm.exe  _relcatchfinally.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relcatchfinally
    if not exist "_relcatchfinally.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relcatchfinally.org" (
            mkdir IL
            copy _relcatchfinally.exe IL\_relcatchfinally.exe 
            ren _relcatchfinally.exe _relcatchfinally.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relcatchfinally.org /out _relcatchfinally.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relcatchfinally.org /out _relcatchfinally.exe
...
set ExePath=_relcatchfinally.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relcatchfinally.reflect.xml
    IF EXIST _relcatchfinally.reflect.xml set ReflectionRoots=-x _relcatchfinally.reflect.xml
    ...
    set ExePath=%LinkBin%\_relcatchfinally.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relcatchfinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relcatchfinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
