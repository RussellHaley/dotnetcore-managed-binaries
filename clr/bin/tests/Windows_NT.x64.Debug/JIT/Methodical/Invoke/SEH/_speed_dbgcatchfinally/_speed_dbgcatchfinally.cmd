...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_speed_dbgcatchfinally _speed_dbgcatchfinally.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_speed_dbgcatchfinally _speed_dbgcatchfinally.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgcatchfinally.dasm.il _speed_dbgcatchfinally.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgcatchfinally.dasm.il _speed_dbgcatchfinally.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgcatchfinally.asm.exe  _speed_dbgcatchfinally.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgcatchfinally.asm.exe  _speed_dbgcatchfinally.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgcatchfinally
    if not exist "_speed_dbgcatchfinally.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgcatchfinally.org" (
            mkdir IL
            copy _speed_dbgcatchfinally.exe IL\_speed_dbgcatchfinally.exe 
            ren _speed_dbgcatchfinally.exe _speed_dbgcatchfinally.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgcatchfinally.org /out _speed_dbgcatchfinally.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgcatchfinally.org /out _speed_dbgcatchfinally.exe
...
set ExePath=_speed_dbgcatchfinally.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgcatchfinally.reflect.xml
    IF EXIST _speed_dbgcatchfinally.reflect.xml set ReflectionRoots=-x _speed_dbgcatchfinally.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgcatchfinally.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgcatchfinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgcatchfinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
