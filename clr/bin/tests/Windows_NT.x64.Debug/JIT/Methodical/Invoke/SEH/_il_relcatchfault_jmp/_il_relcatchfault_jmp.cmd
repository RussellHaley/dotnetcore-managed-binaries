...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_relcatchfault_jmp _il_relcatchfault_jmp.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_relcatchfault_jmp _il_relcatchfault_jmp.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_relcatchfault_jmp.dasm.il _il_relcatchfault_jmp.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_relcatchfault_jmp.dasm.il _il_relcatchfault_jmp.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_relcatchfault_jmp.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relcatchfault_jmp.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_relcatchfault_jmp.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relcatchfault_jmp.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_relcatchfault_jmp
    if not exist "_il_relcatchfault_jmp.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_relcatchfault_jmp.org" (
            mkdir IL
            copy _il_relcatchfault_jmp.exe IL\_il_relcatchfault_jmp.exe 
            ren _il_relcatchfault_jmp.exe _il_relcatchfault_jmp.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relcatchfault_jmp.org /out _il_relcatchfault_jmp.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_relcatchfault_jmp.org /out _il_relcatchfault_jmp.exe
...
set ExePath=_il_relcatchfault_jmp.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_relcatchfault_jmp.reflect.xml
    IF EXIST _il_relcatchfault_jmp.reflect.xml set ReflectionRoots=-x _il_relcatchfault_jmp.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_relcatchfault_jmp.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_relcatchfault_jmp.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_relcatchfault_jmp.asm.exe %CLRTestExecutionArguments% %Host_Args%
