...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_relcatchfault _il_relcatchfault.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_relcatchfault _il_relcatchfault.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relcatchfault.dasm.il _il_relcatchfault.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relcatchfault.dasm.il _il_relcatchfault.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relcatchfault.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relcatchfault.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relcatchfault.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relcatchfault.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relcatchfault
    if [ ! -f _il_relcatchfault.org ]; then
        TakeLock
        if [ ! -f _il_relcatchfault.org ]; then
          mkdir IL
          cp _il_relcatchfault.exe IL/_il_relcatchfault.exe
          mv _il_relcatchfault.exe _il_relcatchfault.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relcatchfault.org /out _il_relcatchfault.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relcatchfault.org /out _il_relcatchfault.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relcatchfault.reflect.xml
  if [ -f _il_relcatchfault.reflect.xml ]
then
    ReflectionRoots="-x _il_relcatchfault.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relcatchfault.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relcatchfault.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relcatchfault.asm.exe $CLRTestExecutionArguments
