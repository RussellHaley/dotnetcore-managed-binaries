...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_dbgcatchfinally_jmp _il_dbgcatchfinally_jmp.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_dbgcatchfinally_jmp _il_dbgcatchfinally_jmp.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcatchfinally_jmp.dasm.il _il_dbgcatchfinally_jmp.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcatchfinally_jmp.dasm.il _il_dbgcatchfinally_jmp.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgcatchfinally_jmp.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfinally_jmp.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgcatchfinally_jmp.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfinally_jmp.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgcatchfinally_jmp
    if [ ! -f _il_dbgcatchfinally_jmp.org ]; then
        TakeLock
        if [ ! -f _il_dbgcatchfinally_jmp.org ]; then
          mkdir IL
          cp _il_dbgcatchfinally_jmp.exe IL/_il_dbgcatchfinally_jmp.exe
          mv _il_dbgcatchfinally_jmp.exe _il_dbgcatchfinally_jmp.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcatchfinally_jmp.org /out _il_dbgcatchfinally_jmp.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcatchfinally_jmp.org /out _il_dbgcatchfinally_jmp.exe
          __cgExitCode=$?
...
ExePath=_il_dbgcatchfinally_jmp.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgcatchfinally_jmp.reflect.xml
  if [ -f _il_dbgcatchfinally_jmp.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgcatchfinally_jmp.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgcatchfinally_jmp.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgcatchfinally_jmp.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgcatchfinally_jmp.asm.exe $CLRTestExecutionArguments
