...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_speed_dbgcatchfinally _speed_dbgcatchfinally.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_speed_dbgcatchfinally _speed_dbgcatchfinally.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgcatchfinally.dasm.il _speed_dbgcatchfinally.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgcatchfinally.dasm.il _speed_dbgcatchfinally.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgcatchfinally.asm.exe  _speed_dbgcatchfinally.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgcatchfinally.asm.exe  _speed_dbgcatchfinally.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgcatchfinally
    if [ ! -f _speed_dbgcatchfinally.org ]; then
        TakeLock
        if [ ! -f _speed_dbgcatchfinally.org ]; then
          mkdir IL
          cp _speed_dbgcatchfinally.exe IL/_speed_dbgcatchfinally.exe
          mv _speed_dbgcatchfinally.exe _speed_dbgcatchfinally.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgcatchfinally.org /out _speed_dbgcatchfinally.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgcatchfinally.org /out _speed_dbgcatchfinally.exe
          __cgExitCode=$?
...
ExePath=_speed_dbgcatchfinally.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgcatchfinally.reflect.xml
  if [ -f _speed_dbgcatchfinally.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgcatchfinally.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgcatchfinally.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgcatchfinally.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgcatchfinally.asm.exe $CLRTestExecutionArguments
