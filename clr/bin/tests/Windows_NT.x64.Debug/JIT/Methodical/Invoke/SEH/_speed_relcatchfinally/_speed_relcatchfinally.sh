...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_speed_relcatchfinally _speed_relcatchfinally.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_speed_relcatchfinally _speed_relcatchfinally.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relcatchfinally.dasm.il _speed_relcatchfinally.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relcatchfinally.dasm.il _speed_relcatchfinally.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relcatchfinally.asm.exe  _speed_relcatchfinally.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relcatchfinally.asm.exe  _speed_relcatchfinally.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relcatchfinally
    if [ ! -f _speed_relcatchfinally.org ]; then
        TakeLock
        if [ ! -f _speed_relcatchfinally.org ]; then
          mkdir IL
          cp _speed_relcatchfinally.exe IL/_speed_relcatchfinally.exe
          mv _speed_relcatchfinally.exe _speed_relcatchfinally.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relcatchfinally.org /out _speed_relcatchfinally.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relcatchfinally.org /out _speed_relcatchfinally.exe
          __cgExitCode=$?
...
ExePath=_speed_relcatchfinally.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relcatchfinally.reflect.xml
  if [ -f _speed_relcatchfinally.reflect.xml ]
then
    ReflectionRoots="-x _speed_relcatchfinally.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relcatchfinally.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relcatchfinally.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relcatchfinally.asm.exe $CLRTestExecutionArguments
