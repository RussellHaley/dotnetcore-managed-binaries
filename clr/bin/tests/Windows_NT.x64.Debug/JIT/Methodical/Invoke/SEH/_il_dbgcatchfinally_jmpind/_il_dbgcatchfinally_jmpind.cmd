...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_dbgcatchfinally_jmpind _il_dbgcatchfinally_jmpind.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_dbgcatchfinally_jmpind _il_dbgcatchfinally_jmpind.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcatchfinally_jmpind.dasm.il _il_dbgcatchfinally_jmpind.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcatchfinally_jmpind.dasm.il _il_dbgcatchfinally_jmpind.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgcatchfinally_jmpind.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfinally_jmpind.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgcatchfinally_jmpind.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfinally_jmpind.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgcatchfinally_jmpind
    if not exist "_il_dbgcatchfinally_jmpind.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgcatchfinally_jmpind.org" (
            mkdir IL
            copy _il_dbgcatchfinally_jmpind.exe IL\_il_dbgcatchfinally_jmpind.exe 
            ren _il_dbgcatchfinally_jmpind.exe _il_dbgcatchfinally_jmpind.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcatchfinally_jmpind.org /out _il_dbgcatchfinally_jmpind.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcatchfinally_jmpind.org /out _il_dbgcatchfinally_jmpind.exe
...
set ExePath=_il_dbgcatchfinally_jmpind.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgcatchfinally_jmpind.reflect.xml
    IF EXIST _il_dbgcatchfinally_jmpind.reflect.xml set ReflectionRoots=-x _il_dbgcatchfinally_jmpind.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgcatchfinally_jmpind.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgcatchfinally_jmpind.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgcatchfinally_jmpind.asm.exe %CLRTestExecutionArguments% %Host_Args%
