...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_speed_relcatchfinally_tail _speed_relcatchfinally_tail.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_speed_relcatchfinally_tail _speed_relcatchfinally_tail.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relcatchfinally_tail.dasm.il _speed_relcatchfinally_tail.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relcatchfinally_tail.dasm.il _speed_relcatchfinally_tail.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relcatchfinally_tail.asm.exe  _speed_relcatchfinally_tail.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relcatchfinally_tail.asm.exe  _speed_relcatchfinally_tail.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relcatchfinally_tail
    if [ ! -f _speed_relcatchfinally_tail.org ]; then
        TakeLock
        if [ ! -f _speed_relcatchfinally_tail.org ]; then
          mkdir IL
          cp _speed_relcatchfinally_tail.exe IL/_speed_relcatchfinally_tail.exe
          mv _speed_relcatchfinally_tail.exe _speed_relcatchfinally_tail.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relcatchfinally_tail.org /out _speed_relcatchfinally_tail.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relcatchfinally_tail.org /out _speed_relcatchfinally_tail.exe
          __cgExitCode=$?
...
ExePath=_speed_relcatchfinally_tail.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relcatchfinally_tail.reflect.xml
  if [ -f _speed_relcatchfinally_tail.reflect.xml ]
then
    ReflectionRoots="-x _speed_relcatchfinally_tail.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relcatchfinally_tail.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relcatchfinally_tail.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relcatchfinally_tail.asm.exe $CLRTestExecutionArguments
