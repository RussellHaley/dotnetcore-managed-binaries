...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_relcatchfinally_jmpind _il_relcatchfinally_jmpind.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_relcatchfinally_jmpind _il_relcatchfinally_jmpind.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_relcatchfinally_jmpind.dasm.il _il_relcatchfinally_jmpind.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_relcatchfinally_jmpind.dasm.il _il_relcatchfinally_jmpind.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_relcatchfinally_jmpind.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relcatchfinally_jmpind.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_relcatchfinally_jmpind.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_relcatchfinally_jmpind.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_relcatchfinally_jmpind
    if [ ! -f _il_relcatchfinally_jmpind.org ]; then
        TakeLock
        if [ ! -f _il_relcatchfinally_jmpind.org ]; then
          mkdir IL
          cp _il_relcatchfinally_jmpind.exe IL/_il_relcatchfinally_jmpind.exe
          mv _il_relcatchfinally_jmpind.exe _il_relcatchfinally_jmpind.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relcatchfinally_jmpind.org /out _il_relcatchfinally_jmpind.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_relcatchfinally_jmpind.org /out _il_relcatchfinally_jmpind.exe
          __cgExitCode=$?
...
ExePath=_il_relcatchfinally_jmpind.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_relcatchfinally_jmpind.reflect.xml
  if [ -f _il_relcatchfinally_jmpind.reflect.xml ]
then
    ReflectionRoots="-x _il_relcatchfinally_jmpind.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_relcatchfinally_jmpind.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_relcatchfinally_jmpind.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_relcatchfinally_jmpind.asm.exe $CLRTestExecutionArguments
