...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_dbgcatchfault _il_dbgcatchfault.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_il_dbgcatchfault _il_dbgcatchfault.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcatchfault.dasm.il _il_dbgcatchfault.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgcatchfault.dasm.il _il_dbgcatchfault.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgcatchfault.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfault.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgcatchfault.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfault.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgcatchfault
    if not exist "_il_dbgcatchfault.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgcatchfault.org" (
            mkdir IL
            copy _il_dbgcatchfault.exe IL\_il_dbgcatchfault.exe 
            ren _il_dbgcatchfault.exe _il_dbgcatchfault.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcatchfault.org /out _il_dbgcatchfault.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgcatchfault.org /out _il_dbgcatchfault.exe
...
set ExePath=_il_dbgcatchfault.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgcatchfault.reflect.xml
    IF EXIST _il_dbgcatchfault.reflect.xml set ReflectionRoots=-x _il_dbgcatchfault.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgcatchfault.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgcatchfault.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgcatchfault.asm.exe %CLRTestExecutionArguments% %Host_Args%
