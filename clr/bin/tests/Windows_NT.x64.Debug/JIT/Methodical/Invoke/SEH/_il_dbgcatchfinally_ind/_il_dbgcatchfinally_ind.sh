...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_dbgcatchfinally_ind _il_dbgcatchfinally_ind.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Invoke/SEH/_il_dbgcatchfinally_ind _il_dbgcatchfinally_ind.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcatchfinally_ind.dasm.il _il_dbgcatchfinally_ind.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgcatchfinally_ind.dasm.il _il_dbgcatchfinally_ind.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgcatchfinally_ind.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfinally_ind.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgcatchfinally_ind.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgcatchfinally_ind.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgcatchfinally_ind
    if [ ! -f _il_dbgcatchfinally_ind.org ]; then
        TakeLock
        if [ ! -f _il_dbgcatchfinally_ind.org ]; then
          mkdir IL
          cp _il_dbgcatchfinally_ind.exe IL/_il_dbgcatchfinally_ind.exe
          mv _il_dbgcatchfinally_ind.exe _il_dbgcatchfinally_ind.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcatchfinally_ind.org /out _il_dbgcatchfinally_ind.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgcatchfinally_ind.org /out _il_dbgcatchfinally_ind.exe
          __cgExitCode=$?
...
ExePath=_il_dbgcatchfinally_ind.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgcatchfinally_ind.reflect.xml
  if [ -f _il_dbgcatchfinally_ind.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgcatchfinally_ind.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgcatchfinally_ind.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgcatchfinally_ind.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgcatchfinally_ind.asm.exe $CLRTestExecutionArguments
