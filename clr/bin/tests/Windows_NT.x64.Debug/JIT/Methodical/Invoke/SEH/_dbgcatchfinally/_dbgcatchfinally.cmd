...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_dbgcatchfinally _dbgcatchfinally.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Invoke\SEH\_dbgcatchfinally _dbgcatchfinally.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgcatchfinally.dasm.il _dbgcatchfinally.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgcatchfinally.dasm.il _dbgcatchfinally.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgcatchfinally.asm.exe  _dbgcatchfinally.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgcatchfinally.asm.exe  _dbgcatchfinally.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgcatchfinally
    if not exist "_dbgcatchfinally.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgcatchfinally.org" (
            mkdir IL
            copy _dbgcatchfinally.exe IL\_dbgcatchfinally.exe 
            ren _dbgcatchfinally.exe _dbgcatchfinally.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgcatchfinally.org /out _dbgcatchfinally.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgcatchfinally.org /out _dbgcatchfinally.exe
...
set ExePath=_dbgcatchfinally.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgcatchfinally.reflect.xml
    IF EXIST _dbgcatchfinally.reflect.xml set ReflectionRoots=-x _dbgcatchfinally.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgcatchfinally.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgcatchfinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgcatchfinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
