...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/morph/sin3double sin3double.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/morph/sin3double sin3double.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=sin3double.dasm.il sin3double.exe
    "$CORE_ROOT/ildasm" -raweh -out=sin3double.dasm.il sin3double.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=sin3double.asm.exe -QUIET -NOLOGO -OPTIMIZE sin3double.dasm.il
    "$CORE_ROOT/ilasm" -output=sin3double.asm.exe -QUIET -NOLOGO -OPTIMIZE sin3double.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=sin3double
    if [ ! -f sin3double.org ]; then
        TakeLock
        if [ ! -f sin3double.org ]; then
          mkdir IL
          cp sin3double.exe IL/sin3double.exe
          mv sin3double.exe sin3double.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in sin3double.org /out sin3double.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in sin3double.org /out sin3double.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in sin3double.reflect.xml
  if [ -f sin3double.reflect.xml ]
then
    ReflectionRoots="-x sin3double.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/sin3double.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" sin3double.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" sin3double.asm.exe $CLRTestExecutionArguments
