...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\morph\sin3double sin3double.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\morph\sin3double sin3double.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=sin3double.dasm.il sin3double.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=sin3double.dasm.il sin3double.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=sin3double.asm.exe -QUIET -NOLOGO -OPTIMIZE sin3double.dasm.il
    %CORE_ROOT%\ilasm.exe /output=sin3double.asm.exe -QUIET -NOLOGO -OPTIMIZE sin3double.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=sin3double
    if not exist "sin3double.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "sin3double.org" (
            mkdir IL
            copy sin3double.exe IL\sin3double.exe 
            ren sin3double.exe sin3double.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in sin3double.org /out sin3double.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in sin3double.org /out sin3double.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in sin3double.reflect.xml
    IF EXIST sin3double.reflect.xml set ReflectionRoots=-x sin3double.reflect.xml
    ...
    set ExePath=%LinkBin%\sin3double.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% sin3double.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% sin3double.asm.exe %CLRTestExecutionArguments% %Host_Args%
