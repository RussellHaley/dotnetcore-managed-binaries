...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/morph/_odbgsin_cs _odbgsin_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/morph/_odbgsin_cs _odbgsin_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_odbgsin_cs.dasm.il _odbgsin_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_odbgsin_cs.dasm.il _odbgsin_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_odbgsin_cs.asm.exe  _odbgsin_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_odbgsin_cs.asm.exe  _odbgsin_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_odbgsin_cs
    if [ ! -f _odbgsin_cs.org ]; then
        TakeLock
        if [ ! -f _odbgsin_cs.org ]; then
          mkdir IL
          cp _odbgsin_cs.exe IL/_odbgsin_cs.exe
          mv _odbgsin_cs.exe _odbgsin_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _odbgsin_cs.org /out _odbgsin_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _odbgsin_cs.org /out _odbgsin_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _odbgsin_cs.reflect.xml
  if [ -f _odbgsin_cs.reflect.xml ]
then
    ReflectionRoots="-x _odbgsin_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_odbgsin_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _odbgsin_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _odbgsin_cs.asm.exe $CLRTestExecutionArguments
