...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/xlang/_relsin_cs_cs _relsin_cs_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/xlang/_relsin_cs_cs _relsin_cs_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relsin_cs_cs.dasm.il _relsin_cs_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relsin_cs_cs.dasm.il _relsin_cs_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relsin_cs_cs.asm.exe  _relsin_cs_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_relsin_cs_cs.asm.exe  _relsin_cs_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relsin_cs_cs
    if [ ! -f _relsin_cs_cs.org ]; then
        TakeLock
        if [ ! -f _relsin_cs_cs.org ]; then
          mkdir IL
          cp _relsin_cs_cs.exe IL/_relsin_cs_cs.exe
          mv _relsin_cs_cs.exe _relsin_cs_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relsin_cs_cs.org /out _relsin_cs_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relsin_cs_cs.org /out _relsin_cs_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relsin_cs_cs.reflect.xml
  if [ -f _relsin_cs_cs.reflect.xml ]
then
    ReflectionRoots="-x _relsin_cs_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relsin_cs_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relsin_cs_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relsin_cs_cs.asm.exe $CLRTestExecutionArguments
