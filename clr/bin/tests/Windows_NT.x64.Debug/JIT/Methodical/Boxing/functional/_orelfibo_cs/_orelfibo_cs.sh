...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/functional/_orelfibo_cs _orelfibo_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/functional/_orelfibo_cs _orelfibo_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_orelfibo_cs.dasm.il _orelfibo_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_orelfibo_cs.dasm.il _orelfibo_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_orelfibo_cs.asm.exe  _orelfibo_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_orelfibo_cs.asm.exe  _orelfibo_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_orelfibo_cs
    if [ ! -f _orelfibo_cs.org ]; then
        TakeLock
        if [ ! -f _orelfibo_cs.org ]; then
          mkdir IL
          cp _orelfibo_cs.exe IL/_orelfibo_cs.exe
          mv _orelfibo_cs.exe _orelfibo_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _orelfibo_cs.org /out _orelfibo_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _orelfibo_cs.org /out _orelfibo_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _orelfibo_cs.reflect.xml
  if [ -f _orelfibo_cs.reflect.xml ]
then
    ReflectionRoots="-x _orelfibo_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_orelfibo_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _orelfibo_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _orelfibo_cs.asm.exe $CLRTestExecutionArguments
