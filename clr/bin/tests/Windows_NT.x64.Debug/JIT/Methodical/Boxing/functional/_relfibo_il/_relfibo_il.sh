...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/functional/_relfibo_il _relfibo_il.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/functional/_relfibo_il _relfibo_il.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relfibo_il.dasm.il _relfibo_il.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relfibo_il.dasm.il _relfibo_il.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relfibo_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relfibo_il.dasm.il
    "$CORE_ROOT/ilasm" -output=_relfibo_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relfibo_il.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relfibo_il
    if [ ! -f _relfibo_il.org ]; then
        TakeLock
        if [ ! -f _relfibo_il.org ]; then
          mkdir IL
          cp _relfibo_il.exe IL/_relfibo_il.exe
          mv _relfibo_il.exe _relfibo_il.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relfibo_il.org /out _relfibo_il.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relfibo_il.org /out _relfibo_il.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relfibo_il.reflect.xml
  if [ -f _relfibo_il.reflect.xml ]
then
    ReflectionRoots="-x _relfibo_il.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relfibo_il.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relfibo_il.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relfibo_il.asm.exe $CLRTestExecutionArguments
