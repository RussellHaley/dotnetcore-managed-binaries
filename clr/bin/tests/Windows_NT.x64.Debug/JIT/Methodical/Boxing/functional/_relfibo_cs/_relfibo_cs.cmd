...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\functional\_relfibo_cs _relfibo_cs.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\functional\_relfibo_cs _relfibo_cs.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relfibo_cs.dasm.il _relfibo_cs.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relfibo_cs.dasm.il _relfibo_cs.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relfibo_cs.asm.exe  _relfibo_cs.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relfibo_cs.asm.exe  _relfibo_cs.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relfibo_cs
    if not exist "_relfibo_cs.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relfibo_cs.org" (
            mkdir IL
            copy _relfibo_cs.exe IL\_relfibo_cs.exe 
            ren _relfibo_cs.exe _relfibo_cs.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relfibo_cs.org /out _relfibo_cs.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relfibo_cs.org /out _relfibo_cs.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relfibo_cs.reflect.xml
    IF EXIST _relfibo_cs.reflect.xml set ReflectionRoots=-x _relfibo_cs.reflect.xml
    ...
    set ExePath=%LinkBin%\_relfibo_cs.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relfibo_cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relfibo_cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
