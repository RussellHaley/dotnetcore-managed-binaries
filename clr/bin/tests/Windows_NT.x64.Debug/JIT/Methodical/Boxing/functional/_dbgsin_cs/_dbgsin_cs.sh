...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/functional/_dbgsin_cs _dbgsin_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/functional/_dbgsin_cs _dbgsin_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgsin_cs.dasm.il _dbgsin_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgsin_cs.dasm.il _dbgsin_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgsin_cs.asm.exe  _dbgsin_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgsin_cs.asm.exe  _dbgsin_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgsin_cs
    if [ ! -f _dbgsin_cs.org ]; then
        TakeLock
        if [ ! -f _dbgsin_cs.org ]; then
          mkdir IL
          cp _dbgsin_cs.exe IL/_dbgsin_cs.exe
          mv _dbgsin_cs.exe _dbgsin_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgsin_cs.org /out _dbgsin_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgsin_cs.org /out _dbgsin_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgsin_cs.reflect.xml
  if [ -f _dbgsin_cs.reflect.xml ]
then
    ReflectionRoots="-x _dbgsin_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgsin_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgsin_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgsin_cs.asm.exe $CLRTestExecutionArguments
