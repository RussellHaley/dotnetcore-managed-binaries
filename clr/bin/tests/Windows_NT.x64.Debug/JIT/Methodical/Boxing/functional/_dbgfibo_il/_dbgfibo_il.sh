...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/functional/_dbgfibo_il _dbgfibo_il.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/functional/_dbgfibo_il _dbgfibo_il.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgfibo_il.dasm.il _dbgfibo_il.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgfibo_il.dasm.il _dbgfibo_il.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgfibo_il.asm.exe -QUIET -NOLOGO -DEBUG _dbgfibo_il.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgfibo_il.asm.exe -QUIET -NOLOGO -DEBUG _dbgfibo_il.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgfibo_il
    if [ ! -f _dbgfibo_il.org ]; then
        TakeLock
        if [ ! -f _dbgfibo_il.org ]; then
          mkdir IL
          cp _dbgfibo_il.exe IL/_dbgfibo_il.exe
          mv _dbgfibo_il.exe _dbgfibo_il.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgfibo_il.org /out _dbgfibo_il.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgfibo_il.org /out _dbgfibo_il.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgfibo_il.reflect.xml
  if [ -f _dbgfibo_il.reflect.xml ]
then
    ReflectionRoots="-x _dbgfibo_il.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgfibo_il.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgfibo_il.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgfibo_il.asm.exe $CLRTestExecutionArguments
