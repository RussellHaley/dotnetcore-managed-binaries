...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\functional\_relfibo_il _relfibo_il.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\functional\_relfibo_il _relfibo_il.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relfibo_il.dasm.il _relfibo_il.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relfibo_il.dasm.il _relfibo_il.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relfibo_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relfibo_il.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relfibo_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relfibo_il.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relfibo_il
    if not exist "_relfibo_il.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relfibo_il.org" (
            mkdir IL
            copy _relfibo_il.exe IL\_relfibo_il.exe 
            ren _relfibo_il.exe _relfibo_il.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relfibo_il.org /out _relfibo_il.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relfibo_il.org /out _relfibo_il.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relfibo_il.reflect.xml
    IF EXIST _relfibo_il.reflect.xml set ReflectionRoots=-x _relfibo_il.reflect.xml
    ...
    set ExePath=%LinkBin%\_relfibo_il.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relfibo_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relfibo_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
