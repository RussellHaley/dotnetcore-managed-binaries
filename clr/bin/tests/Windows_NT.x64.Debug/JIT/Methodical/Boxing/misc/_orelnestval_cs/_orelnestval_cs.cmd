...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_orelnestval_cs _orelnestval_cs.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_orelnestval_cs _orelnestval_cs.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_orelnestval_cs.dasm.il _orelnestval_cs.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_orelnestval_cs.dasm.il _orelnestval_cs.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_orelnestval_cs.asm.exe  _orelnestval_cs.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_orelnestval_cs.asm.exe  _orelnestval_cs.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_orelnestval_cs
    if not exist "_orelnestval_cs.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_orelnestval_cs.org" (
            mkdir IL
            copy _orelnestval_cs.exe IL\_orelnestval_cs.exe 
            ren _orelnestval_cs.exe _orelnestval_cs.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _orelnestval_cs.org /out _orelnestval_cs.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _orelnestval_cs.org /out _orelnestval_cs.exe
...
set ExePath=_orelnestval_cs.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _orelnestval_cs.reflect.xml
    IF EXIST _orelnestval_cs.reflect.xml set ReflectionRoots=-x _orelnestval_cs.reflect.xml
    ...
    set ExePath=%LinkBin%\_orelnestval_cs.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _orelnestval_cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _orelnestval_cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
