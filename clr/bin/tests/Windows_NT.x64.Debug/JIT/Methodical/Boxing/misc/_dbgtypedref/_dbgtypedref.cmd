...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_dbgtypedref _dbgtypedref.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_dbgtypedref _dbgtypedref.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgtypedref.dasm.il _dbgtypedref.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgtypedref.dasm.il _dbgtypedref.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgtypedref.asm.exe -QUIET -NOLOGO -DEBUG _dbgtypedref.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgtypedref.asm.exe -QUIET -NOLOGO -DEBUG _dbgtypedref.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgtypedref
    if not exist "_dbgtypedref.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgtypedref.org" (
            mkdir IL
            copy _dbgtypedref.exe IL\_dbgtypedref.exe 
            ren _dbgtypedref.exe _dbgtypedref.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgtypedref.org /out _dbgtypedref.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgtypedref.org /out _dbgtypedref.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgtypedref.reflect.xml
    IF EXIST _dbgtypedref.reflect.xml set ReflectionRoots=-x _dbgtypedref.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgtypedref.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgtypedref.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgtypedref.asm.exe %CLRTestExecutionArguments% %Host_Args%
