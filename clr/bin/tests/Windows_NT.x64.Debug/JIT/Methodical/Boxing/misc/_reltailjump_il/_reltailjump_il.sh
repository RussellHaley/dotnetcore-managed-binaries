...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_reltailjump_il _reltailjump_il.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_reltailjump_il _reltailjump_il.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_reltailjump_il.dasm.il _reltailjump_il.exe
    "$CORE_ROOT/ildasm" -raweh -out=_reltailjump_il.dasm.il _reltailjump_il.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_reltailjump_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _reltailjump_il.dasm.il
    "$CORE_ROOT/ilasm" -output=_reltailjump_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _reltailjump_il.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_reltailjump_il
    if [ ! -f _reltailjump_il.org ]; then
        TakeLock
        if [ ! -f _reltailjump_il.org ]; then
          mkdir IL
          cp _reltailjump_il.exe IL/_reltailjump_il.exe
          mv _reltailjump_il.exe _reltailjump_il.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reltailjump_il.org /out _reltailjump_il.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reltailjump_il.org /out _reltailjump_il.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _reltailjump_il.reflect.xml
  if [ -f _reltailjump_il.reflect.xml ]
then
    ReflectionRoots="-x _reltailjump_il.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_reltailjump_il.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _reltailjump_il.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _reltailjump_il.asm.exe $CLRTestExecutionArguments
