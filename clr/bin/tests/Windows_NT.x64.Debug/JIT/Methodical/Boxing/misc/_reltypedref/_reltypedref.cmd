...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_reltypedref _reltypedref.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_reltypedref _reltypedref.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_reltypedref.dasm.il _reltypedref.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_reltypedref.dasm.il _reltypedref.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_reltypedref.asm.exe -QUIET -NOLOGO -DEBUG=OPT _reltypedref.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_reltypedref.asm.exe -QUIET -NOLOGO -DEBUG=OPT _reltypedref.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_reltypedref
    if not exist "_reltypedref.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_reltypedref.org" (
            mkdir IL
            copy _reltypedref.exe IL\_reltypedref.exe 
            ren _reltypedref.exe _reltypedref.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _reltypedref.org /out _reltypedref.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _reltypedref.org /out _reltypedref.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _reltypedref.reflect.xml
    IF EXIST _reltypedref.reflect.xml set ReflectionRoots=-x _reltypedref.reflect.xml
    ...
    set ExePath=%LinkBin%\_reltypedref.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _reltypedref.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _reltypedref.asm.exe %CLRTestExecutionArguments% %Host_Args%
