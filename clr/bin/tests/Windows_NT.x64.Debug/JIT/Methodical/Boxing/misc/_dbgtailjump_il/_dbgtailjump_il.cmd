...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_dbgtailjump_il _dbgtailjump_il.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_dbgtailjump_il _dbgtailjump_il.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgtailjump_il.dasm.il _dbgtailjump_il.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgtailjump_il.dasm.il _dbgtailjump_il.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgtailjump_il.asm.exe -QUIET -NOLOGO -DEBUG _dbgtailjump_il.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgtailjump_il.asm.exe -QUIET -NOLOGO -DEBUG _dbgtailjump_il.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgtailjump_il
    if not exist "_dbgtailjump_il.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgtailjump_il.org" (
            mkdir IL
            copy _dbgtailjump_il.exe IL\_dbgtailjump_il.exe 
            ren _dbgtailjump_il.exe _dbgtailjump_il.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgtailjump_il.org /out _dbgtailjump_il.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgtailjump_il.org /out _dbgtailjump_il.exe
...
set ExePath=_dbgtailjump_il.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgtailjump_il.reflect.xml
    IF EXIST _dbgtailjump_il.reflect.xml set ReflectionRoots=-x _dbgtailjump_il.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgtailjump_il.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgtailjump_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgtailjump_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
