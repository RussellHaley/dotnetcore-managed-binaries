...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_reltailjump_cs _reltailjump_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_reltailjump_cs _reltailjump_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_reltailjump_cs.dasm.il _reltailjump_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_reltailjump_cs.dasm.il _reltailjump_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_reltailjump_cs.asm.exe  _reltailjump_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_reltailjump_cs.asm.exe  _reltailjump_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_reltailjump_cs
    if [ ! -f _reltailjump_cs.org ]; then
        TakeLock
        if [ ! -f _reltailjump_cs.org ]; then
          mkdir IL
          cp _reltailjump_cs.exe IL/_reltailjump_cs.exe
          mv _reltailjump_cs.exe _reltailjump_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reltailjump_cs.org /out _reltailjump_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reltailjump_cs.org /out _reltailjump_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _reltailjump_cs.reflect.xml
  if [ -f _reltailjump_cs.reflect.xml ]
then
    ReflectionRoots="-x _reltailjump_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_reltailjump_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _reltailjump_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _reltailjump_cs.asm.exe $CLRTestExecutionArguments
