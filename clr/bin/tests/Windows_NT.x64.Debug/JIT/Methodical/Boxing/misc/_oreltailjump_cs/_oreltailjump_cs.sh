...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_oreltailjump_cs _oreltailjump_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_oreltailjump_cs _oreltailjump_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_oreltailjump_cs.dasm.il _oreltailjump_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_oreltailjump_cs.dasm.il _oreltailjump_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_oreltailjump_cs.asm.exe  _oreltailjump_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_oreltailjump_cs.asm.exe  _oreltailjump_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_oreltailjump_cs
    if [ ! -f _oreltailjump_cs.org ]; then
        TakeLock
        if [ ! -f _oreltailjump_cs.org ]; then
          mkdir IL
          cp _oreltailjump_cs.exe IL/_oreltailjump_cs.exe
          mv _oreltailjump_cs.exe _oreltailjump_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _oreltailjump_cs.org /out _oreltailjump_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _oreltailjump_cs.org /out _oreltailjump_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _oreltailjump_cs.reflect.xml
  if [ -f _oreltailjump_cs.reflect.xml ]
then
    ReflectionRoots="-x _oreltailjump_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_oreltailjump_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _oreltailjump_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _oreltailjump_cs.asm.exe $CLRTestExecutionArguments
