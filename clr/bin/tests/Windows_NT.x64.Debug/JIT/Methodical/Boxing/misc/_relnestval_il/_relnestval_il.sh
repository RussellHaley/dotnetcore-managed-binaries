...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_relnestval_il _relnestval_il.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_relnestval_il _relnestval_il.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relnestval_il.dasm.il _relnestval_il.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relnestval_il.dasm.il _relnestval_il.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relnestval_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relnestval_il.dasm.il
    "$CORE_ROOT/ilasm" -output=_relnestval_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relnestval_il.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relnestval_il
    if [ ! -f _relnestval_il.org ]; then
        TakeLock
        if [ ! -f _relnestval_il.org ]; then
          mkdir IL
          cp _relnestval_il.exe IL/_relnestval_il.exe
          mv _relnestval_il.exe _relnestval_il.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relnestval_il.org /out _relnestval_il.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relnestval_il.org /out _relnestval_il.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relnestval_il.reflect.xml
  if [ -f _relnestval_il.reflect.xml ]
then
    ReflectionRoots="-x _relnestval_il.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relnestval_il.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relnestval_il.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relnestval_il.asm.exe $CLRTestExecutionArguments
