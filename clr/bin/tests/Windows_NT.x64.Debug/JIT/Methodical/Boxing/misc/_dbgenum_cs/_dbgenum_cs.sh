...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_dbgenum_cs _dbgenum_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_dbgenum_cs _dbgenum_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgenum_cs.dasm.il _dbgenum_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgenum_cs.dasm.il _dbgenum_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgenum_cs.asm.exe  _dbgenum_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgenum_cs.asm.exe  _dbgenum_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgenum_cs
    if [ ! -f _dbgenum_cs.org ]; then
        TakeLock
        if [ ! -f _dbgenum_cs.org ]; then
          mkdir IL
          cp _dbgenum_cs.exe IL/_dbgenum_cs.exe
          mv _dbgenum_cs.exe _dbgenum_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgenum_cs.org /out _dbgenum_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgenum_cs.org /out _dbgenum_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgenum_cs.reflect.xml
  if [ -f _dbgenum_cs.reflect.xml ]
then
    ReflectionRoots="-x _dbgenum_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgenum_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgenum_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgenum_cs.asm.exe $CLRTestExecutionArguments
