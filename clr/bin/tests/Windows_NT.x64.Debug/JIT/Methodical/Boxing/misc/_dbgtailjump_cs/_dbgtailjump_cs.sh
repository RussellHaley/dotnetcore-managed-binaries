...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_dbgtailjump_cs _dbgtailjump_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_dbgtailjump_cs _dbgtailjump_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgtailjump_cs.dasm.il _dbgtailjump_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgtailjump_cs.dasm.il _dbgtailjump_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgtailjump_cs.asm.exe  _dbgtailjump_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgtailjump_cs.asm.exe  _dbgtailjump_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgtailjump_cs
    if [ ! -f _dbgtailjump_cs.org ]; then
        TakeLock
        if [ ! -f _dbgtailjump_cs.org ]; then
          mkdir IL
          cp _dbgtailjump_cs.exe IL/_dbgtailjump_cs.exe
          mv _dbgtailjump_cs.exe _dbgtailjump_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgtailjump_cs.org /out _dbgtailjump_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgtailjump_cs.org /out _dbgtailjump_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgtailjump_cs.reflect.xml
  if [ -f _dbgtailjump_cs.reflect.xml ]
then
    ReflectionRoots="-x _dbgtailjump_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgtailjump_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgtailjump_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgtailjump_cs.asm.exe $CLRTestExecutionArguments
