...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_dbgnestval_il _dbgnestval_il.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_dbgnestval_il _dbgnestval_il.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgnestval_il.dasm.il _dbgnestval_il.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgnestval_il.dasm.il _dbgnestval_il.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgnestval_il.asm.exe -QUIET -NOLOGO -DEBUG _dbgnestval_il.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgnestval_il.asm.exe -QUIET -NOLOGO -DEBUG _dbgnestval_il.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgnestval_il
    if [ ! -f _dbgnestval_il.org ]; then
        TakeLock
        if [ ! -f _dbgnestval_il.org ]; then
          mkdir IL
          cp _dbgnestval_il.exe IL/_dbgnestval_il.exe
          mv _dbgnestval_il.exe _dbgnestval_il.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgnestval_il.org /out _dbgnestval_il.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgnestval_il.org /out _dbgnestval_il.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgnestval_il.reflect.xml
  if [ -f _dbgnestval_il.reflect.xml ]
then
    ReflectionRoots="-x _dbgnestval_il.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgnestval_il.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgnestval_il.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgnestval_il.asm.exe $CLRTestExecutionArguments
