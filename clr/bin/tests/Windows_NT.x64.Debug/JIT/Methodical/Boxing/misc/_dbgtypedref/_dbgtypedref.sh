...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_dbgtypedref _dbgtypedref.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_dbgtypedref _dbgtypedref.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgtypedref.dasm.il _dbgtypedref.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgtypedref.dasm.il _dbgtypedref.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgtypedref.asm.exe -QUIET -NOLOGO -DEBUG _dbgtypedref.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgtypedref.asm.exe -QUIET -NOLOGO -DEBUG _dbgtypedref.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgtypedref
    if [ ! -f _dbgtypedref.org ]; then
        TakeLock
        if [ ! -f _dbgtypedref.org ]; then
          mkdir IL
          cp _dbgtypedref.exe IL/_dbgtypedref.exe
          mv _dbgtypedref.exe _dbgtypedref.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgtypedref.org /out _dbgtypedref.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgtypedref.org /out _dbgtypedref.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgtypedref.reflect.xml
  if [ -f _dbgtypedref.reflect.xml ]
then
    ReflectionRoots="-x _dbgtypedref.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgtypedref.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgtypedref.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgtypedref.asm.exe $CLRTestExecutionArguments
