...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_relenum_il _relenum_il.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_relenum_il _relenum_il.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relenum_il.dasm.il _relenum_il.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relenum_il.dasm.il _relenum_il.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relenum_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relenum_il.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relenum_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relenum_il.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relenum_il
    if not exist "_relenum_il.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relenum_il.org" (
            mkdir IL
            copy _relenum_il.exe IL\_relenum_il.exe 
            ren _relenum_il.exe _relenum_il.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relenum_il.org /out _relenum_il.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relenum_il.org /out _relenum_il.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relenum_il.reflect.xml
    IF EXIST _relenum_il.reflect.xml set ReflectionRoots=-x _relenum_il.reflect.xml
    ...
    set ExePath=%LinkBin%\_relenum_il.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relenum_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relenum_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
