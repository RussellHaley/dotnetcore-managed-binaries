...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_reltypedref _reltypedref.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/misc/_reltypedref _reltypedref.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_reltypedref.dasm.il _reltypedref.exe
    "$CORE_ROOT/ildasm" -raweh -out=_reltypedref.dasm.il _reltypedref.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_reltypedref.asm.exe -QUIET -NOLOGO -DEBUG=OPT _reltypedref.dasm.il
    "$CORE_ROOT/ilasm" -output=_reltypedref.asm.exe -QUIET -NOLOGO -DEBUG=OPT _reltypedref.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_reltypedref
    if [ ! -f _reltypedref.org ]; then
        TakeLock
        if [ ! -f _reltypedref.org ]; then
          mkdir IL
          cp _reltypedref.exe IL/_reltypedref.exe
          mv _reltypedref.exe _reltypedref.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reltypedref.org /out _reltypedref.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _reltypedref.org /out _reltypedref.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _reltypedref.reflect.xml
  if [ -f _reltypedref.reflect.xml ]
then
    ReflectionRoots="-x _reltypedref.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_reltypedref.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _reltypedref.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _reltypedref.asm.exe $CLRTestExecutionArguments
