...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_reltailjump_cs _reltailjump_cs.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\misc\_reltailjump_cs _reltailjump_cs.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_reltailjump_cs.dasm.il _reltailjump_cs.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_reltailjump_cs.dasm.il _reltailjump_cs.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_reltailjump_cs.asm.exe  _reltailjump_cs.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_reltailjump_cs.asm.exe  _reltailjump_cs.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_reltailjump_cs
    if not exist "_reltailjump_cs.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_reltailjump_cs.org" (
            mkdir IL
            copy _reltailjump_cs.exe IL\_reltailjump_cs.exe 
            ren _reltailjump_cs.exe _reltailjump_cs.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _reltailjump_cs.org /out _reltailjump_cs.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _reltailjump_cs.org /out _reltailjump_cs.exe
...
set ExePath=_reltailjump_cs.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _reltailjump_cs.reflect.xml
    IF EXIST _reltailjump_cs.reflect.xml set ReflectionRoots=-x _reltailjump_cs.reflect.xml
    ...
    set ExePath=%LinkBin%\_reltailjump_cs.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _reltailjump_cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _reltailjump_cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
