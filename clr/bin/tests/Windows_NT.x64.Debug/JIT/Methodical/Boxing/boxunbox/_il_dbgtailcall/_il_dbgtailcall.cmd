...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\boxunbox\_il_dbgtailcall _il_dbgtailcall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\boxunbox\_il_dbgtailcall _il_dbgtailcall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgtailcall.dasm.il _il_dbgtailcall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgtailcall.dasm.il _il_dbgtailcall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgtailcall.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtailcall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgtailcall.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtailcall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgtailcall
    if not exist "_il_dbgtailcall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgtailcall.org" (
            mkdir IL
            copy _il_dbgtailcall.exe IL\_il_dbgtailcall.exe 
            ren _il_dbgtailcall.exe _il_dbgtailcall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgtailcall.org /out _il_dbgtailcall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgtailcall.org /out _il_dbgtailcall.exe
...
set ExePath=_il_dbgtailcall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgtailcall.reflect.xml
    IF EXIST _il_dbgtailcall.reflect.xml set ReflectionRoots=-x _il_dbgtailcall.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgtailcall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgtailcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgtailcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
