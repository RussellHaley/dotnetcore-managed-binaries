...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\boxunbox\_il_dbghuge_filter _il_dbghuge_filter.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\boxunbox\_il_dbghuge_filter _il_dbghuge_filter.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbghuge_filter.dasm.il _il_dbghuge_filter.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbghuge_filter.dasm.il _il_dbghuge_filter.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbghuge_filter.asm.exe -QUIET -NOLOGO -DEBUG _il_dbghuge_filter.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbghuge_filter.asm.exe -QUIET -NOLOGO -DEBUG _il_dbghuge_filter.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbghuge_filter
    if not exist "_il_dbghuge_filter.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbghuge_filter.org" (
            mkdir IL
            copy _il_dbghuge_filter.exe IL\_il_dbghuge_filter.exe 
            ren _il_dbghuge_filter.exe _il_dbghuge_filter.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbghuge_filter.org /out _il_dbghuge_filter.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbghuge_filter.org /out _il_dbghuge_filter.exe
...
set ExePath=_il_dbghuge_filter.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbghuge_filter.reflect.xml
    IF EXIST _il_dbghuge_filter.reflect.xml set ReflectionRoots=-x _il_dbghuge_filter.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbghuge_filter.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbghuge_filter.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbghuge_filter.asm.exe %CLRTestExecutionArguments% %Host_Args%
