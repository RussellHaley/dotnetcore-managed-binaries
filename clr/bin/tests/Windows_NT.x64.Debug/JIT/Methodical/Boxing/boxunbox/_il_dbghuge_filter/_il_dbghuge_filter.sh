...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/boxunbox/_il_dbghuge_filter _il_dbghuge_filter.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/boxunbox/_il_dbghuge_filter _il_dbghuge_filter.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbghuge_filter.dasm.il _il_dbghuge_filter.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbghuge_filter.dasm.il _il_dbghuge_filter.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbghuge_filter.asm.exe -QUIET -NOLOGO -DEBUG _il_dbghuge_filter.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbghuge_filter.asm.exe -QUIET -NOLOGO -DEBUG _il_dbghuge_filter.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbghuge_filter
    if [ ! -f _il_dbghuge_filter.org ]; then
        TakeLock
        if [ ! -f _il_dbghuge_filter.org ]; then
          mkdir IL
          cp _il_dbghuge_filter.exe IL/_il_dbghuge_filter.exe
          mv _il_dbghuge_filter.exe _il_dbghuge_filter.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbghuge_filter.org /out _il_dbghuge_filter.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbghuge_filter.org /out _il_dbghuge_filter.exe
          __cgExitCode=$?
...
ExePath=_il_dbghuge_filter.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbghuge_filter.reflect.xml
  if [ -f _il_dbghuge_filter.reflect.xml ]
then
    ReflectionRoots="-x _il_dbghuge_filter.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbghuge_filter.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbghuge_filter.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbghuge_filter.asm.exe $CLRTestExecutionArguments
