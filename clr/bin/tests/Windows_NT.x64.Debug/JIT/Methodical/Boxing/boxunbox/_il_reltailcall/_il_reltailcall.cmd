...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\boxunbox\_il_reltailcall _il_reltailcall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\boxunbox\_il_reltailcall _il_reltailcall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_reltailcall.dasm.il _il_reltailcall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_reltailcall.dasm.il _il_reltailcall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_reltailcall.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltailcall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_reltailcall.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltailcall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_reltailcall
    if not exist "_il_reltailcall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_reltailcall.org" (
            mkdir IL
            copy _il_reltailcall.exe IL\_il_reltailcall.exe 
            ren _il_reltailcall.exe _il_reltailcall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reltailcall.org /out _il_reltailcall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_reltailcall.org /out _il_reltailcall.exe
...
set ExePath=_il_reltailcall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_reltailcall.reflect.xml
    IF EXIST _il_reltailcall.reflect.xml set ReflectionRoots=-x _il_reltailcall.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_reltailcall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_reltailcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_reltailcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
