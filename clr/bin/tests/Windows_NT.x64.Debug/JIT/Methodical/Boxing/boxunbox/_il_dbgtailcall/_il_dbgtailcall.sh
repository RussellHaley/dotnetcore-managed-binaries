...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/boxunbox/_il_dbgtailcall _il_dbgtailcall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/boxunbox/_il_dbgtailcall _il_dbgtailcall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtailcall.dasm.il _il_dbgtailcall.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgtailcall.dasm.il _il_dbgtailcall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgtailcall.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtailcall.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgtailcall.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgtailcall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgtailcall
    if [ ! -f _il_dbgtailcall.org ]; then
        TakeLock
        if [ ! -f _il_dbgtailcall.org ]; then
          mkdir IL
          cp _il_dbgtailcall.exe IL/_il_dbgtailcall.exe
          mv _il_dbgtailcall.exe _il_dbgtailcall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtailcall.org /out _il_dbgtailcall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgtailcall.org /out _il_dbgtailcall.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgtailcall.reflect.xml
  if [ -f _il_dbgtailcall.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgtailcall.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgtailcall.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgtailcall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgtailcall.asm.exe $CLRTestExecutionArguments
