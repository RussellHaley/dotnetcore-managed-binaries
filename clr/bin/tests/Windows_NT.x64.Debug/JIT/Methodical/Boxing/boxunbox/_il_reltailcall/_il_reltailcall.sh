...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/boxunbox/_il_reltailcall _il_reltailcall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/boxunbox/_il_reltailcall _il_reltailcall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_reltailcall.dasm.il _il_reltailcall.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_reltailcall.dasm.il _il_reltailcall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_reltailcall.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltailcall.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_reltailcall.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_reltailcall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_reltailcall
    if [ ! -f _il_reltailcall.org ]; then
        TakeLock
        if [ ! -f _il_reltailcall.org ]; then
          mkdir IL
          cp _il_reltailcall.exe IL/_il_reltailcall.exe
          mv _il_reltailcall.exe _il_reltailcall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reltailcall.org /out _il_reltailcall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_reltailcall.org /out _il_reltailcall.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_reltailcall.reflect.xml
  if [ -f _il_reltailcall.reflect.xml ]
then
    ReflectionRoots="-x _il_reltailcall.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_reltailcall.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_reltailcall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_reltailcall.asm.exe $CLRTestExecutionArguments
