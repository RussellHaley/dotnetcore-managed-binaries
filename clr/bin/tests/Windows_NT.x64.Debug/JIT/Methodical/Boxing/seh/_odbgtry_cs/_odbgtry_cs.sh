...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/seh/_odbgtry_cs _odbgtry_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/seh/_odbgtry_cs _odbgtry_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_odbgtry_cs.dasm.il _odbgtry_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_odbgtry_cs.dasm.il _odbgtry_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_odbgtry_cs.asm.exe  _odbgtry_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_odbgtry_cs.asm.exe  _odbgtry_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_odbgtry_cs
    if [ ! -f _odbgtry_cs.org ]; then
        TakeLock
        if [ ! -f _odbgtry_cs.org ]; then
          mkdir IL
          cp _odbgtry_cs.exe IL/_odbgtry_cs.exe
          mv _odbgtry_cs.exe _odbgtry_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _odbgtry_cs.org /out _odbgtry_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _odbgtry_cs.org /out _odbgtry_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _odbgtry_cs.reflect.xml
  if [ -f _odbgtry_cs.reflect.xml ]
then
    ReflectionRoots="-x _odbgtry_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_odbgtry_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _odbgtry_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _odbgtry_cs.asm.exe $CLRTestExecutionArguments
