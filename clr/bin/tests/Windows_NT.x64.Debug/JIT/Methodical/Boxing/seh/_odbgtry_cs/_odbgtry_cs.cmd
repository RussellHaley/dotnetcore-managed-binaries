...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\seh\_odbgtry_cs _odbgtry_cs.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\seh\_odbgtry_cs _odbgtry_cs.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_odbgtry_cs.dasm.il _odbgtry_cs.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_odbgtry_cs.dasm.il _odbgtry_cs.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_odbgtry_cs.asm.exe  _odbgtry_cs.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_odbgtry_cs.asm.exe  _odbgtry_cs.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_odbgtry_cs
    if not exist "_odbgtry_cs.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_odbgtry_cs.org" (
            mkdir IL
            copy _odbgtry_cs.exe IL\_odbgtry_cs.exe 
            ren _odbgtry_cs.exe _odbgtry_cs.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _odbgtry_cs.org /out _odbgtry_cs.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _odbgtry_cs.org /out _odbgtry_cs.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _odbgtry_cs.reflect.xml
    IF EXIST _odbgtry_cs.reflect.xml set ReflectionRoots=-x _odbgtry_cs.reflect.xml
    ...
    set ExePath=%LinkBin%\_odbgtry_cs.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _odbgtry_cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _odbgtry_cs.asm.exe %CLRTestExecutionArguments% %Host_Args%
