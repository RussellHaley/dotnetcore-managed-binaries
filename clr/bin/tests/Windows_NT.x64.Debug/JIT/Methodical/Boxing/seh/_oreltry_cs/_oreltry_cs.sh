...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/seh/_oreltry_cs _oreltry_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/seh/_oreltry_cs _oreltry_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_oreltry_cs.dasm.il _oreltry_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_oreltry_cs.dasm.il _oreltry_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_oreltry_cs.asm.exe  _oreltry_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_oreltry_cs.asm.exe  _oreltry_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_oreltry_cs
    if [ ! -f _oreltry_cs.org ]; then
        TakeLock
        if [ ! -f _oreltry_cs.org ]; then
          mkdir IL
          cp _oreltry_cs.exe IL/_oreltry_cs.exe
          mv _oreltry_cs.exe _oreltry_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _oreltry_cs.org /out _oreltry_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _oreltry_cs.org /out _oreltry_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _oreltry_cs.reflect.xml
  if [ -f _oreltry_cs.reflect.xml ]
then
    ReflectionRoots="-x _oreltry_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_oreltry_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _oreltry_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _oreltry_cs.asm.exe $CLRTestExecutionArguments
