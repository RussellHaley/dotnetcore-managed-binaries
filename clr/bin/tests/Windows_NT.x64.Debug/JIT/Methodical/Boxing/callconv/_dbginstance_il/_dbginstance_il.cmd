...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\callconv\_dbginstance_il _dbginstance_il.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\callconv\_dbginstance_il _dbginstance_il.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbginstance_il.dasm.il _dbginstance_il.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbginstance_il.dasm.il _dbginstance_il.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbginstance_il.asm.exe -QUIET -NOLOGO -DEBUG _dbginstance_il.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbginstance_il.asm.exe -QUIET -NOLOGO -DEBUG _dbginstance_il.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbginstance_il
    if not exist "_dbginstance_il.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbginstance_il.org" (
            mkdir IL
            copy _dbginstance_il.exe IL\_dbginstance_il.exe 
            ren _dbginstance_il.exe _dbginstance_il.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbginstance_il.org /out _dbginstance_il.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbginstance_il.org /out _dbginstance_il.exe
...
set ExePath=_dbginstance_il.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbginstance_il.reflect.xml
    IF EXIST _dbginstance_il.reflect.xml set ReflectionRoots=-x _dbginstance_il.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbginstance_il.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbginstance_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbginstance_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
