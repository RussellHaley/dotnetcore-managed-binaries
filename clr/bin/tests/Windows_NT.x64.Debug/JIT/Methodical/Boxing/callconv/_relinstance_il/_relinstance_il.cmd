...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\callconv\_relinstance_il _relinstance_il.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\Boxing\callconv\_relinstance_il _relinstance_il.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relinstance_il.dasm.il _relinstance_il.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relinstance_il.dasm.il _relinstance_il.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relinstance_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relinstance_il.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relinstance_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relinstance_il.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relinstance_il
    if not exist "_relinstance_il.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relinstance_il.org" (
            mkdir IL
            copy _relinstance_il.exe IL\_relinstance_il.exe 
            ren _relinstance_il.exe _relinstance_il.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relinstance_il.org /out _relinstance_il.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relinstance_il.org /out _relinstance_il.exe
...
set ExePath=_relinstance_il.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relinstance_il.reflect.xml
    IF EXIST _relinstance_il.reflect.xml set ReflectionRoots=-x _relinstance_il.reflect.xml
    ...
    set ExePath=%LinkBin%\_relinstance_il.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relinstance_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relinstance_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
