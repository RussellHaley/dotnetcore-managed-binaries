...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/callconv/_dbginstance_cs _dbginstance_cs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/callconv/_dbginstance_cs _dbginstance_cs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbginstance_cs.dasm.il _dbginstance_cs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbginstance_cs.dasm.il _dbginstance_cs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbginstance_cs.asm.exe  _dbginstance_cs.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbginstance_cs.asm.exe  _dbginstance_cs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbginstance_cs
    if [ ! -f _dbginstance_cs.org ]; then
        TakeLock
        if [ ! -f _dbginstance_cs.org ]; then
          mkdir IL
          cp _dbginstance_cs.exe IL/_dbginstance_cs.exe
          mv _dbginstance_cs.exe _dbginstance_cs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbginstance_cs.org /out _dbginstance_cs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbginstance_cs.org /out _dbginstance_cs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbginstance_cs.reflect.xml
  if [ -f _dbginstance_cs.reflect.xml ]
then
    ReflectionRoots="-x _dbginstance_cs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbginstance_cs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbginstance_cs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbginstance_cs.asm.exe $CLRTestExecutionArguments
