...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/callconv/_dbginstance_il _dbginstance_il.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/callconv/_dbginstance_il _dbginstance_il.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbginstance_il.dasm.il _dbginstance_il.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbginstance_il.dasm.il _dbginstance_il.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbginstance_il.asm.exe -QUIET -NOLOGO -DEBUG _dbginstance_il.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbginstance_il.asm.exe -QUIET -NOLOGO -DEBUG _dbginstance_il.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbginstance_il
    if [ ! -f _dbginstance_il.org ]; then
        TakeLock
        if [ ! -f _dbginstance_il.org ]; then
          mkdir IL
          cp _dbginstance_il.exe IL/_dbginstance_il.exe
          mv _dbginstance_il.exe _dbginstance_il.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbginstance_il.org /out _dbginstance_il.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbginstance_il.org /out _dbginstance_il.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbginstance_il.reflect.xml
  if [ -f _dbginstance_il.reflect.xml ]
then
    ReflectionRoots="-x _dbginstance_il.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbginstance_il.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbginstance_il.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbginstance_il.asm.exe $CLRTestExecutionArguments
