...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/callconv/_relinstance_il _relinstance_il.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/Boxing/callconv/_relinstance_il _relinstance_il.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relinstance_il.dasm.il _relinstance_il.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relinstance_il.dasm.il _relinstance_il.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relinstance_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relinstance_il.dasm.il
    "$CORE_ROOT/ilasm" -output=_relinstance_il.asm.exe -QUIET -NOLOGO -DEBUG=OPT _relinstance_il.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relinstance_il
    if [ ! -f _relinstance_il.org ]; then
        TakeLock
        if [ ! -f _relinstance_il.org ]; then
          mkdir IL
          cp _relinstance_il.exe IL/_relinstance_il.exe
          mv _relinstance_il.exe _relinstance_il.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relinstance_il.org /out _relinstance_il.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relinstance_il.org /out _relinstance_il.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relinstance_il.reflect.xml
  if [ -f _relinstance_il.reflect.xml ]
then
    ReflectionRoots="-x _relinstance_il.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relinstance_il.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relinstance_il.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relinstance_il.asm.exe $CLRTestExecutionArguments
