...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_dbggcreport _dbggcreport.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_dbggcreport _dbggcreport.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbggcreport.dasm.il _dbggcreport.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbggcreport.dasm.il _dbggcreport.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbggcreport.asm.exe  _dbggcreport.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbggcreport.asm.exe  _dbggcreport.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbggcreport
    if [ ! -f _dbggcreport.org ]; then
        TakeLock
        if [ ! -f _dbggcreport.org ]; then
          mkdir IL
          cp _dbggcreport.exe IL/_dbggcreport.exe
          mv _dbggcreport.exe _dbggcreport.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbggcreport.org /out _dbggcreport.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbggcreport.org /out _dbggcreport.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbggcreport.reflect.xml
  if [ -f _dbggcreport.reflect.xml ]
then
    ReflectionRoots="-x _dbggcreport.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbggcreport.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbggcreport.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbggcreport.asm.exe $CLRTestExecutionArguments
