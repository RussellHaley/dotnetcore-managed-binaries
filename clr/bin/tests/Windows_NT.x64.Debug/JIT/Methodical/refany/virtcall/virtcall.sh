...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/virtcall virtcall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/virtcall virtcall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=virtcall.dasm.il virtcall.exe
    "$CORE_ROOT/ildasm" -raweh -out=virtcall.dasm.il virtcall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=virtcall.asm.exe  virtcall.dasm.il
    "$CORE_ROOT/ilasm" -output=virtcall.asm.exe  virtcall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=virtcall
    if [ ! -f virtcall.org ]; then
        TakeLock
        if [ ! -f virtcall.org ]; then
          mkdir IL
          cp virtcall.exe IL/virtcall.exe
          mv virtcall.exe virtcall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in virtcall.org /out virtcall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in virtcall.org /out virtcall.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in virtcall.reflect.xml
  if [ -f virtcall.reflect.xml ]
then
    ReflectionRoots="-x virtcall.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" virtcall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" virtcall.asm.exe $CLRTestExecutionArguments
