...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\virtcall virtcall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\virtcall virtcall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=virtcall.dasm.il virtcall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=virtcall.dasm.il virtcall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=virtcall.asm.exe  virtcall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=virtcall.asm.exe  virtcall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=virtcall
    if not exist "virtcall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "virtcall.org" (
            mkdir IL
            copy virtcall.exe IL\virtcall.exe 
            ren virtcall.exe virtcall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in virtcall.org /out virtcall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in virtcall.org /out virtcall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in virtcall.reflect.xml
    IF EXIST virtcall.reflect.xml set ReflectionRoots=-x virtcall.reflect.xml
    ...
    set ExePath=%LinkBin%\virtcall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% virtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% virtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
