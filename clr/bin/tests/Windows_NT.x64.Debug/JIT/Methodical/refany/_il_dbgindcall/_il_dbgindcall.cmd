...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_il_dbgindcall _il_dbgindcall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_il_dbgindcall _il_dbgindcall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgindcall.dasm.il _il_dbgindcall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_dbgindcall.dasm.il _il_dbgindcall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_dbgindcall.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgindcall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_dbgindcall.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgindcall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_dbgindcall
    if not exist "_il_dbgindcall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_dbgindcall.org" (
            mkdir IL
            copy _il_dbgindcall.exe IL\_il_dbgindcall.exe 
            ren _il_dbgindcall.exe _il_dbgindcall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgindcall.org /out _il_dbgindcall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_dbgindcall.org /out _il_dbgindcall.exe
...
set ExePath=_il_dbgindcall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_dbgindcall.reflect.xml
    IF EXIST _il_dbgindcall.reflect.xml set ReflectionRoots=-x _il_dbgindcall.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_dbgindcall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_dbgindcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_dbgindcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
