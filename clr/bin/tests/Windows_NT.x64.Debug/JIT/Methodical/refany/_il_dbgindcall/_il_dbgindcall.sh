...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_il_dbgindcall _il_dbgindcall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_il_dbgindcall _il_dbgindcall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_dbgindcall.dasm.il _il_dbgindcall.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_dbgindcall.dasm.il _il_dbgindcall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_dbgindcall.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgindcall.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_dbgindcall.asm.exe -QUIET -NOLOGO -DEBUG _il_dbgindcall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_dbgindcall
    if [ ! -f _il_dbgindcall.org ]; then
        TakeLock
        if [ ! -f _il_dbgindcall.org ]; then
          mkdir IL
          cp _il_dbgindcall.exe IL/_il_dbgindcall.exe
          mv _il_dbgindcall.exe _il_dbgindcall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgindcall.org /out _il_dbgindcall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_dbgindcall.org /out _il_dbgindcall.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_dbgindcall.reflect.xml
  if [ -f _il_dbgindcall.reflect.xml ]
then
    ReflectionRoots="-x _il_dbgindcall.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_dbgindcall.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_dbgindcall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_dbgindcall.asm.exe $CLRTestExecutionArguments
