...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_relstress1 _relstress1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_relstress1 _relstress1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relstress1.dasm.il _relstress1.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relstress1.dasm.il _relstress1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relstress1.asm.exe  _relstress1.dasm.il
    "$CORE_ROOT/ilasm" -output=_relstress1.asm.exe  _relstress1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relstress1
    if [ ! -f _relstress1.org ]; then
        TakeLock
        if [ ! -f _relstress1.org ]; then
          mkdir IL
          cp _relstress1.exe IL/_relstress1.exe
          mv _relstress1.exe _relstress1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relstress1.org /out _relstress1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relstress1.org /out _relstress1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relstress1.reflect.xml
  if [ -f _relstress1.reflect.xml ]
then
    ReflectionRoots="-x _relstress1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relstress1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relstress1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relstress1.asm.exe $CLRTestExecutionArguments
