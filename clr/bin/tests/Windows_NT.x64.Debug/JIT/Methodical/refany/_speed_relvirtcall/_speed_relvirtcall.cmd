...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_speed_relvirtcall _speed_relvirtcall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_speed_relvirtcall _speed_relvirtcall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relvirtcall.dasm.il _speed_relvirtcall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_relvirtcall.dasm.il _speed_relvirtcall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_relvirtcall.asm.exe  _speed_relvirtcall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_relvirtcall.asm.exe  _speed_relvirtcall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_relvirtcall
    if not exist "_speed_relvirtcall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_relvirtcall.org" (
            mkdir IL
            copy _speed_relvirtcall.exe IL\_speed_relvirtcall.exe 
            ren _speed_relvirtcall.exe _speed_relvirtcall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relvirtcall.org /out _speed_relvirtcall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_relvirtcall.org /out _speed_relvirtcall.exe
...
set ExePath=_speed_relvirtcall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_relvirtcall.reflect.xml
    IF EXIST _speed_relvirtcall.reflect.xml set ReflectionRoots=-x _speed_relvirtcall.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_relvirtcall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_relvirtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_relvirtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
