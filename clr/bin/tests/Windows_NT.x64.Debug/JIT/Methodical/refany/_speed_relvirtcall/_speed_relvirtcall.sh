...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_speed_relvirtcall _speed_relvirtcall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_speed_relvirtcall _speed_relvirtcall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relvirtcall.dasm.il _speed_relvirtcall.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relvirtcall.dasm.il _speed_relvirtcall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relvirtcall.asm.exe  _speed_relvirtcall.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relvirtcall.asm.exe  _speed_relvirtcall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relvirtcall
    if [ ! -f _speed_relvirtcall.org ]; then
        TakeLock
        if [ ! -f _speed_relvirtcall.org ]; then
          mkdir IL
          cp _speed_relvirtcall.exe IL/_speed_relvirtcall.exe
          mv _speed_relvirtcall.exe _speed_relvirtcall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relvirtcall.org /out _speed_relvirtcall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relvirtcall.org /out _speed_relvirtcall.exe
          __cgExitCode=$?
...
ExePath=_speed_relvirtcall.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relvirtcall.reflect.xml
  if [ -f _speed_relvirtcall.reflect.xml ]
then
    ReflectionRoots="-x _speed_relvirtcall.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relvirtcall.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relvirtcall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relvirtcall.asm.exe $CLRTestExecutionArguments
