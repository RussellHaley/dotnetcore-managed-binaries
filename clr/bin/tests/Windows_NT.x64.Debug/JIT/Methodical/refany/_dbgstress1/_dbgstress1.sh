...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_dbgstress1 _dbgstress1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_dbgstress1 _dbgstress1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgstress1.dasm.il _dbgstress1.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgstress1.dasm.il _dbgstress1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgstress1.asm.exe  _dbgstress1.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgstress1.asm.exe  _dbgstress1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgstress1
    if [ ! -f _dbgstress1.org ]; then
        TakeLock
        if [ ! -f _dbgstress1.org ]; then
          mkdir IL
          cp _dbgstress1.exe IL/_dbgstress1.exe
          mv _dbgstress1.exe _dbgstress1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgstress1.org /out _dbgstress1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgstress1.org /out _dbgstress1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgstress1.reflect.xml
  if [ -f _dbgstress1.reflect.xml ]
then
    ReflectionRoots="-x _dbgstress1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgstress1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgstress1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgstress1.asm.exe $CLRTestExecutionArguments
