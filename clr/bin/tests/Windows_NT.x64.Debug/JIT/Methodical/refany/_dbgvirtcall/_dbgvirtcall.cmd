...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_dbgvirtcall _dbgvirtcall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_dbgvirtcall _dbgvirtcall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_dbgvirtcall.dasm.il _dbgvirtcall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_dbgvirtcall.dasm.il _dbgvirtcall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_dbgvirtcall.asm.exe  _dbgvirtcall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_dbgvirtcall.asm.exe  _dbgvirtcall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_dbgvirtcall
    if not exist "_dbgvirtcall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_dbgvirtcall.org" (
            mkdir IL
            copy _dbgvirtcall.exe IL\_dbgvirtcall.exe 
            ren _dbgvirtcall.exe _dbgvirtcall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgvirtcall.org /out _dbgvirtcall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _dbgvirtcall.org /out _dbgvirtcall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _dbgvirtcall.reflect.xml
    IF EXIST _dbgvirtcall.reflect.xml set ReflectionRoots=-x _dbgvirtcall.reflect.xml
    ...
    set ExePath=%LinkBin%\_dbgvirtcall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _dbgvirtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _dbgvirtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
