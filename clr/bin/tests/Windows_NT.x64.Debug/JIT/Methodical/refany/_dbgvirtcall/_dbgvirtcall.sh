...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_dbgvirtcall _dbgvirtcall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_dbgvirtcall _dbgvirtcall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_dbgvirtcall.dasm.il _dbgvirtcall.exe
    "$CORE_ROOT/ildasm" -raweh -out=_dbgvirtcall.dasm.il _dbgvirtcall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_dbgvirtcall.asm.exe  _dbgvirtcall.dasm.il
    "$CORE_ROOT/ilasm" -output=_dbgvirtcall.asm.exe  _dbgvirtcall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_dbgvirtcall
    if [ ! -f _dbgvirtcall.org ]; then
        TakeLock
        if [ ! -f _dbgvirtcall.org ]; then
          mkdir IL
          cp _dbgvirtcall.exe IL/_dbgvirtcall.exe
          mv _dbgvirtcall.exe _dbgvirtcall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgvirtcall.org /out _dbgvirtcall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _dbgvirtcall.org /out _dbgvirtcall.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _dbgvirtcall.reflect.xml
  if [ -f _dbgvirtcall.reflect.xml ]
then
    ReflectionRoots="-x _dbgvirtcall.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_dbgvirtcall.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _dbgvirtcall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _dbgvirtcall.asm.exe $CLRTestExecutionArguments
