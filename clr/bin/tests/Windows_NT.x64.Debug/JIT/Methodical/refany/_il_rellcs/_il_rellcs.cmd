...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_il_rellcs _il_rellcs.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_il_rellcs _il_rellcs.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_il_rellcs.dasm.il _il_rellcs.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_il_rellcs.dasm.il _il_rellcs.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_il_rellcs.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_rellcs.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_il_rellcs.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_rellcs.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_il_rellcs
    if not exist "_il_rellcs.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_il_rellcs.org" (
            mkdir IL
            copy _il_rellcs.exe IL\_il_rellcs.exe 
            ren _il_rellcs.exe _il_rellcs.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_rellcs.org /out _il_rellcs.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _il_rellcs.org /out _il_rellcs.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _il_rellcs.reflect.xml
    IF EXIST _il_rellcs.reflect.xml set ReflectionRoots=-x _il_rellcs.reflect.xml
    ...
    set ExePath=%LinkBin%\_il_rellcs.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _il_rellcs.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _il_rellcs.asm.exe %CLRTestExecutionArguments% %Host_Args%
