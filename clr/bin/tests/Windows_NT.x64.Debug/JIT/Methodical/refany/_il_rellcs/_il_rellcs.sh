...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_il_rellcs _il_rellcs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_il_rellcs _il_rellcs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_il_rellcs.dasm.il _il_rellcs.exe
    "$CORE_ROOT/ildasm" -raweh -out=_il_rellcs.dasm.il _il_rellcs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_il_rellcs.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_rellcs.dasm.il
    "$CORE_ROOT/ilasm" -output=_il_rellcs.asm.exe -QUIET -NOLOGO -DEBUG=OPT _il_rellcs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_il_rellcs
    if [ ! -f _il_rellcs.org ]; then
        TakeLock
        if [ ! -f _il_rellcs.org ]; then
          mkdir IL
          cp _il_rellcs.exe IL/_il_rellcs.exe
          mv _il_rellcs.exe _il_rellcs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_rellcs.org /out _il_rellcs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _il_rellcs.org /out _il_rellcs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _il_rellcs.reflect.xml
  if [ -f _il_rellcs.reflect.xml ]
then
    ReflectionRoots="-x _il_rellcs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_il_rellcs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _il_rellcs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _il_rellcs.asm.exe $CLRTestExecutionArguments
