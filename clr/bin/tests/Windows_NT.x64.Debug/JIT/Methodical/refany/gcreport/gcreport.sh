...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/gcreport gcreport.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/gcreport gcreport.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gcreport.dasm.il gcreport.exe
    "$CORE_ROOT/ildasm" -raweh -out=gcreport.dasm.il gcreport.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gcreport.asm.exe  gcreport.dasm.il
    "$CORE_ROOT/ilasm" -output=gcreport.asm.exe  gcreport.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gcreport
    if [ ! -f gcreport.org ]; then
        TakeLock
        if [ ! -f gcreport.org ]; then
          mkdir IL
          cp gcreport.exe IL/gcreport.exe
          mv gcreport.exe gcreport.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcreport.org /out gcreport.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcreport.org /out gcreport.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gcreport.reflect.xml
  if [ -f gcreport.reflect.xml ]
then
    ReflectionRoots="-x gcreport.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gcreport.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gcreport.asm.exe $CLRTestExecutionArguments
