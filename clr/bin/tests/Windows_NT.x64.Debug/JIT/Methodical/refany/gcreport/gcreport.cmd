...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\gcreport gcreport.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\gcreport gcreport.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gcreport.dasm.il gcreport.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gcreport.dasm.il gcreport.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gcreport.asm.exe  gcreport.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gcreport.asm.exe  gcreport.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gcreport
    if not exist "gcreport.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gcreport.org" (
            mkdir IL
            copy gcreport.exe IL\gcreport.exe 
            ren gcreport.exe gcreport.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcreport.org /out gcreport.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcreport.org /out gcreport.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gcreport.reflect.xml
    IF EXIST gcreport.reflect.xml set ReflectionRoots=-x gcreport.reflect.xml
    ...
    set ExePath=%LinkBin%\gcreport.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gcreport.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gcreport.asm.exe %CLRTestExecutionArguments% %Host_Args%
