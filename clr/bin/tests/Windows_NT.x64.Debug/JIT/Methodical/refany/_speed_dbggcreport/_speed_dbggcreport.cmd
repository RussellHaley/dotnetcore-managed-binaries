...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_speed_dbggcreport _speed_dbggcreport.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_speed_dbggcreport _speed_dbggcreport.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbggcreport.dasm.il _speed_dbggcreport.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbggcreport.dasm.il _speed_dbggcreport.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbggcreport.asm.exe  _speed_dbggcreport.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbggcreport.asm.exe  _speed_dbggcreport.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbggcreport
    if not exist "_speed_dbggcreport.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbggcreport.org" (
            mkdir IL
            copy _speed_dbggcreport.exe IL\_speed_dbggcreport.exe 
            ren _speed_dbggcreport.exe _speed_dbggcreport.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbggcreport.org /out _speed_dbggcreport.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbggcreport.org /out _speed_dbggcreport.exe
...
set ExePath=_speed_dbggcreport.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbggcreport.reflect.xml
    IF EXIST _speed_dbggcreport.reflect.xml set ReflectionRoots=-x _speed_dbggcreport.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbggcreport.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbggcreport.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbggcreport.asm.exe %CLRTestExecutionArguments% %Host_Args%
