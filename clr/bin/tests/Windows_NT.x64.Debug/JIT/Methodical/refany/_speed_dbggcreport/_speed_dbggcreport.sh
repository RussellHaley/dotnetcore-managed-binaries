...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_speed_dbggcreport _speed_dbggcreport.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_speed_dbggcreport _speed_dbggcreport.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbggcreport.dasm.il _speed_dbggcreport.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbggcreport.dasm.il _speed_dbggcreport.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbggcreport.asm.exe  _speed_dbggcreport.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbggcreport.asm.exe  _speed_dbggcreport.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbggcreport
    if [ ! -f _speed_dbggcreport.org ]; then
        TakeLock
        if [ ! -f _speed_dbggcreport.org ]; then
          mkdir IL
          cp _speed_dbggcreport.exe IL/_speed_dbggcreport.exe
          mv _speed_dbggcreport.exe _speed_dbggcreport.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbggcreport.org /out _speed_dbggcreport.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbggcreport.org /out _speed_dbggcreport.exe
          __cgExitCode=$?
...
ExePath=_speed_dbggcreport.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbggcreport.reflect.xml
  if [ -f _speed_dbggcreport.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbggcreport.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbggcreport.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbggcreport.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbggcreport.asm.exe $CLRTestExecutionArguments
