...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_relvirtcall _relvirtcall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_relvirtcall _relvirtcall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relvirtcall.dasm.il _relvirtcall.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relvirtcall.dasm.il _relvirtcall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relvirtcall.asm.exe  _relvirtcall.dasm.il
    "$CORE_ROOT/ilasm" -output=_relvirtcall.asm.exe  _relvirtcall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relvirtcall
    if [ ! -f _relvirtcall.org ]; then
        TakeLock
        if [ ! -f _relvirtcall.org ]; then
          mkdir IL
          cp _relvirtcall.exe IL/_relvirtcall.exe
          mv _relvirtcall.exe _relvirtcall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relvirtcall.org /out _relvirtcall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relvirtcall.org /out _relvirtcall.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relvirtcall.reflect.xml
  if [ -f _relvirtcall.reflect.xml ]
then
    ReflectionRoots="-x _relvirtcall.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relvirtcall.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relvirtcall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relvirtcall.asm.exe $CLRTestExecutionArguments
