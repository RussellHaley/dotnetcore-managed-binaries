...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_relvirtcall _relvirtcall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_relvirtcall _relvirtcall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relvirtcall.dasm.il _relvirtcall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relvirtcall.dasm.il _relvirtcall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relvirtcall.asm.exe  _relvirtcall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relvirtcall.asm.exe  _relvirtcall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relvirtcall
    if not exist "_relvirtcall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relvirtcall.org" (
            mkdir IL
            copy _relvirtcall.exe IL\_relvirtcall.exe 
            ren _relvirtcall.exe _relvirtcall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relvirtcall.org /out _relvirtcall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relvirtcall.org /out _relvirtcall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relvirtcall.reflect.xml
    IF EXIST _relvirtcall.reflect.xml set ReflectionRoots=-x _relvirtcall.reflect.xml
    ...
    set ExePath=%LinkBin%\_relvirtcall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relvirtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relvirtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
