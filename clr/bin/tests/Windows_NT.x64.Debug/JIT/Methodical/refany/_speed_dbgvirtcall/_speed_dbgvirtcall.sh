...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_speed_dbgvirtcall _speed_dbgvirtcall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_speed_dbgvirtcall _speed_dbgvirtcall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgvirtcall.dasm.il _speed_dbgvirtcall.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_dbgvirtcall.dasm.il _speed_dbgvirtcall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_dbgvirtcall.asm.exe  _speed_dbgvirtcall.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_dbgvirtcall.asm.exe  _speed_dbgvirtcall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_dbgvirtcall
    if [ ! -f _speed_dbgvirtcall.org ]; then
        TakeLock
        if [ ! -f _speed_dbgvirtcall.org ]; then
          mkdir IL
          cp _speed_dbgvirtcall.exe IL/_speed_dbgvirtcall.exe
          mv _speed_dbgvirtcall.exe _speed_dbgvirtcall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgvirtcall.org /out _speed_dbgvirtcall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_dbgvirtcall.org /out _speed_dbgvirtcall.exe
          __cgExitCode=$?
...
ExePath=_speed_dbgvirtcall.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_dbgvirtcall.reflect.xml
  if [ -f _speed_dbgvirtcall.reflect.xml ]
then
    ReflectionRoots="-x _speed_dbgvirtcall.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_dbgvirtcall.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_dbgvirtcall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_dbgvirtcall.asm.exe $CLRTestExecutionArguments
