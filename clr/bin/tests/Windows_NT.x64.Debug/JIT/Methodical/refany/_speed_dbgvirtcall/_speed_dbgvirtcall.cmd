...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_speed_dbgvirtcall _speed_dbgvirtcall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_speed_dbgvirtcall _speed_dbgvirtcall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgvirtcall.dasm.il _speed_dbgvirtcall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_speed_dbgvirtcall.dasm.il _speed_dbgvirtcall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_speed_dbgvirtcall.asm.exe  _speed_dbgvirtcall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_speed_dbgvirtcall.asm.exe  _speed_dbgvirtcall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_speed_dbgvirtcall
    if not exist "_speed_dbgvirtcall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_speed_dbgvirtcall.org" (
            mkdir IL
            copy _speed_dbgvirtcall.exe IL\_speed_dbgvirtcall.exe 
            ren _speed_dbgvirtcall.exe _speed_dbgvirtcall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgvirtcall.org /out _speed_dbgvirtcall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _speed_dbgvirtcall.org /out _speed_dbgvirtcall.exe
...
set ExePath=_speed_dbgvirtcall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _speed_dbgvirtcall.reflect.xml
    IF EXIST _speed_dbgvirtcall.reflect.xml set ReflectionRoots=-x _speed_dbgvirtcall.reflect.xml
    ...
    set ExePath=%LinkBin%\_speed_dbgvirtcall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _speed_dbgvirtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _speed_dbgvirtcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
