...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_relgcreport _relgcreport.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\Methodical\refany\_relgcreport _relgcreport.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=_relgcreport.dasm.il _relgcreport.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=_relgcreport.dasm.il _relgcreport.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=_relgcreport.asm.exe  _relgcreport.dasm.il
    %CORE_ROOT%\ilasm.exe /output=_relgcreport.asm.exe  _relgcreport.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=_relgcreport
    if not exist "_relgcreport.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "_relgcreport.org" (
            mkdir IL
            copy _relgcreport.exe IL\_relgcreport.exe 
            ren _relgcreport.exe _relgcreport.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relgcreport.org /out _relgcreport.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in _relgcreport.org /out _relgcreport.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in _relgcreport.reflect.xml
    IF EXIST _relgcreport.reflect.xml set ReflectionRoots=-x _relgcreport.reflect.xml
    ...
    set ExePath=%LinkBin%\_relgcreport.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% _relgcreport.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% _relgcreport.asm.exe %CLRTestExecutionArguments% %Host_Args%
