...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_relgcreport _relgcreport.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_relgcreport _relgcreport.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_relgcreport.dasm.il _relgcreport.exe
    "$CORE_ROOT/ildasm" -raweh -out=_relgcreport.dasm.il _relgcreport.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_relgcreport.asm.exe  _relgcreport.dasm.il
    "$CORE_ROOT/ilasm" -output=_relgcreport.asm.exe  _relgcreport.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_relgcreport
    if [ ! -f _relgcreport.org ]; then
        TakeLock
        if [ ! -f _relgcreport.org ]; then
          mkdir IL
          cp _relgcreport.exe IL/_relgcreport.exe
          mv _relgcreport.exe _relgcreport.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relgcreport.org /out _relgcreport.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _relgcreport.org /out _relgcreport.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in _relgcreport.reflect.xml
  if [ -f _relgcreport.reflect.xml ]
then
    ReflectionRoots="-x _relgcreport.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_relgcreport.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _relgcreport.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _relgcreport.asm.exe $CLRTestExecutionArguments
