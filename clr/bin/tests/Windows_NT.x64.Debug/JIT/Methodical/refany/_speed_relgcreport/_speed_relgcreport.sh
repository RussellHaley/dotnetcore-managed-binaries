...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_speed_relgcreport _speed_relgcreport.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/Methodical/refany/_speed_relgcreport _speed_relgcreport.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=_speed_relgcreport.dasm.il _speed_relgcreport.exe
    "$CORE_ROOT/ildasm" -raweh -out=_speed_relgcreport.dasm.il _speed_relgcreport.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=_speed_relgcreport.asm.exe  _speed_relgcreport.dasm.il
    "$CORE_ROOT/ilasm" -output=_speed_relgcreport.asm.exe  _speed_relgcreport.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=_speed_relgcreport
    if [ ! -f _speed_relgcreport.org ]; then
        TakeLock
        if [ ! -f _speed_relgcreport.org ]; then
          mkdir IL
          cp _speed_relgcreport.exe IL/_speed_relgcreport.exe
          mv _speed_relgcreport.exe _speed_relgcreport.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relgcreport.org /out _speed_relgcreport.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in _speed_relgcreport.org /out _speed_relgcreport.exe
          __cgExitCode=$?
...
ExePath=_speed_relgcreport.exe
...
then
  ...
  # Use hints for reflection roots, if provided in _speed_relgcreport.reflect.xml
  if [ -f _speed_relgcreport.reflect.xml ]
then
    ReflectionRoots="-x _speed_relgcreport.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/_speed_relgcreport.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" _speed_relgcreport.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" _speed_relgcreport.asm.exe $CLRTestExecutionArguments
