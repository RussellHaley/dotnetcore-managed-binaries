...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/strswitchfinal_do strswitchfinal_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/strswitchfinal_do strswitchfinal_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=strswitchfinal_do.dasm.il strswitchfinal_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=strswitchfinal_do.dasm.il strswitchfinal_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=strswitchfinal_do.asm.exe  strswitchfinal_do.dasm.il
    "$CORE_ROOT/ilasm" -output=strswitchfinal_do.asm.exe  strswitchfinal_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=strswitchfinal_do
    if [ ! -f strswitchfinal_do.org ]; then
        TakeLock
        if [ ! -f strswitchfinal_do.org ]; then
          mkdir IL
          cp strswitchfinal_do.exe IL/strswitchfinal_do.exe
          mv strswitchfinal_do.exe strswitchfinal_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in strswitchfinal_do.org /out strswitchfinal_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in strswitchfinal_do.org /out strswitchfinal_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in strswitchfinal_do.reflect.xml
  if [ -f strswitchfinal_do.reflect.xml ]
then
    ReflectionRoots="-x strswitchfinal_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/strswitchfinal_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" strswitchfinal_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" strswitchfinal_do.asm.exe $CLRTestExecutionArguments
