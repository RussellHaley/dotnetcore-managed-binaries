...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\switchinfinally_r switchinfinally_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\switchinfinally_r switchinfinally_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=switchinfinally_r.dasm.il switchinfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=switchinfinally_r.dasm.il switchinfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=switchinfinally_r.asm.exe  switchinfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=switchinfinally_r.asm.exe  switchinfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=switchinfinally_r
    if not exist "switchinfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "switchinfinally_r.org" (
            mkdir IL
            copy switchinfinally_r.exe IL\switchinfinally_r.exe 
            ren switchinfinally_r.exe switchinfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switchinfinally_r.org /out switchinfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switchinfinally_r.org /out switchinfinally_r.exe
...
set ExePath=switchinfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in switchinfinally_r.reflect.xml
    IF EXIST switchinfinally_r.reflect.xml set ReflectionRoots=-x switchinfinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\switchinfinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% switchinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% switchinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
