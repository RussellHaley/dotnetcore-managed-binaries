...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/switchinfinally_do switchinfinally_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/switchinfinally_do switchinfinally_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=switchinfinally_do.dasm.il switchinfinally_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=switchinfinally_do.dasm.il switchinfinally_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=switchinfinally_do.asm.exe  switchinfinally_do.dasm.il
    "$CORE_ROOT/ilasm" -output=switchinfinally_do.asm.exe  switchinfinally_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=switchinfinally_do
    if [ ! -f switchinfinally_do.org ]; then
        TakeLock
        if [ ! -f switchinfinally_do.org ]; then
          mkdir IL
          cp switchinfinally_do.exe IL/switchinfinally_do.exe
          mv switchinfinally_do.exe switchinfinally_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switchinfinally_do.org /out switchinfinally_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switchinfinally_do.org /out switchinfinally_do.exe
          __cgExitCode=$?
...
ExePath=switchinfinally_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in switchinfinally_do.reflect.xml
  if [ -f switchinfinally_do.reflect.xml ]
then
    ReflectionRoots="-x switchinfinally_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/switchinfinally_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" switchinfinally_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" switchinfinally_do.asm.exe $CLRTestExecutionArguments
