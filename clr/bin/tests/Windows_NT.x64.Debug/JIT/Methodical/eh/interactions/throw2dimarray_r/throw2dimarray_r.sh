...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/throw2dimarray_r throw2dimarray_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/throw2dimarray_r throw2dimarray_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throw2dimarray_r.dasm.il throw2dimarray_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throw2dimarray_r.dasm.il throw2dimarray_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throw2dimarray_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throw2dimarray_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throw2dimarray_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throw2dimarray_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throw2dimarray_r
    if [ ! -f throw2dimarray_r.org ]; then
        TakeLock
        if [ ! -f throw2dimarray_r.org ]; then
          mkdir IL
          cp throw2dimarray_r.exe IL/throw2dimarray_r.exe
          mv throw2dimarray_r.exe throw2dimarray_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw2dimarray_r.org /out throw2dimarray_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw2dimarray_r.org /out throw2dimarray_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throw2dimarray_r.reflect.xml
  if [ -f throw2dimarray_r.reflect.xml ]
then
    ReflectionRoots="-x throw2dimarray_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throw2dimarray_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throw2dimarray_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throw2dimarray_r.asm.exe $CLRTestExecutionArguments
