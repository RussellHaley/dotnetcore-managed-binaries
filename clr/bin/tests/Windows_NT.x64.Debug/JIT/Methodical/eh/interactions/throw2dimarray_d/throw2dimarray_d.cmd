...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\throw2dimarray_d throw2dimarray_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\throw2dimarray_d throw2dimarray_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throw2dimarray_d.dasm.il throw2dimarray_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throw2dimarray_d.dasm.il throw2dimarray_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throw2dimarray_d.asm.exe -QUIET -NOLOGO -DEBUG throw2dimarray_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throw2dimarray_d.asm.exe -QUIET -NOLOGO -DEBUG throw2dimarray_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throw2dimarray_d
    if not exist "throw2dimarray_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throw2dimarray_d.org" (
            mkdir IL
            copy throw2dimarray_d.exe IL\throw2dimarray_d.exe 
            ren throw2dimarray_d.exe throw2dimarray_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw2dimarray_d.org /out throw2dimarray_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw2dimarray_d.org /out throw2dimarray_d.exe
...
set ExePath=throw2dimarray_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throw2dimarray_d.reflect.xml
    IF EXIST throw2dimarray_d.reflect.xml set ReflectionRoots=-x throw2dimarray_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throw2dimarray_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throw2dimarray_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throw2dimarray_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
