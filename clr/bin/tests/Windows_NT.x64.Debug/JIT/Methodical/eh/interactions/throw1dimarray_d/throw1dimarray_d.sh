...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/throw1dimarray_d throw1dimarray_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/throw1dimarray_d throw1dimarray_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throw1dimarray_d.dasm.il throw1dimarray_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throw1dimarray_d.dasm.il throw1dimarray_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throw1dimarray_d.asm.exe -QUIET -NOLOGO -DEBUG throw1dimarray_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throw1dimarray_d.asm.exe -QUIET -NOLOGO -DEBUG throw1dimarray_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throw1dimarray_d
    if [ ! -f throw1dimarray_d.org ]; then
        TakeLock
        if [ ! -f throw1dimarray_d.org ]; then
          mkdir IL
          cp throw1dimarray_d.exe IL/throw1dimarray_d.exe
          mv throw1dimarray_d.exe throw1dimarray_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw1dimarray_d.org /out throw1dimarray_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw1dimarray_d.org /out throw1dimarray_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throw1dimarray_d.reflect.xml
  if [ -f throw1dimarray_d.reflect.xml ]
then
    ReflectionRoots="-x throw1dimarray_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throw1dimarray_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throw1dimarray_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throw1dimarray_d.asm.exe $CLRTestExecutionArguments
