...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/strswitchfinal_r strswitchfinal_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/strswitchfinal_r strswitchfinal_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=strswitchfinal_r.dasm.il strswitchfinal_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=strswitchfinal_r.dasm.il strswitchfinal_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=strswitchfinal_r.asm.exe  strswitchfinal_r.dasm.il
    "$CORE_ROOT/ilasm" -output=strswitchfinal_r.asm.exe  strswitchfinal_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=strswitchfinal_r
    if [ ! -f strswitchfinal_r.org ]; then
        TakeLock
        if [ ! -f strswitchfinal_r.org ]; then
          mkdir IL
          cp strswitchfinal_r.exe IL/strswitchfinal_r.exe
          mv strswitchfinal_r.exe strswitchfinal_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in strswitchfinal_r.org /out strswitchfinal_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in strswitchfinal_r.org /out strswitchfinal_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in strswitchfinal_r.reflect.xml
  if [ -f strswitchfinal_r.reflect.xml ]
then
    ReflectionRoots="-x strswitchfinal_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/strswitchfinal_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" strswitchfinal_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" strswitchfinal_r.asm.exe $CLRTestExecutionArguments
