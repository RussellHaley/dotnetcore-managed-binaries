...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\throw1dimarray_r throw1dimarray_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\throw1dimarray_r throw1dimarray_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throw1dimarray_r.dasm.il throw1dimarray_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throw1dimarray_r.dasm.il throw1dimarray_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throw1dimarray_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throw1dimarray_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throw1dimarray_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throw1dimarray_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throw1dimarray_r
    if not exist "throw1dimarray_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throw1dimarray_r.org" (
            mkdir IL
            copy throw1dimarray_r.exe IL\throw1dimarray_r.exe 
            ren throw1dimarray_r.exe throw1dimarray_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw1dimarray_r.org /out throw1dimarray_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw1dimarray_r.org /out throw1dimarray_r.exe
...
set ExePath=throw1dimarray_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throw1dimarray_r.reflect.xml
    IF EXIST throw1dimarray_r.reflect.xml set ReflectionRoots=-x throw1dimarray_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throw1dimarray_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throw1dimarray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throw1dimarray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
