...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/switchinfinally_d switchinfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/switchinfinally_d switchinfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=switchinfinally_d.dasm.il switchinfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=switchinfinally_d.dasm.il switchinfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=switchinfinally_d.asm.exe  switchinfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=switchinfinally_d.asm.exe  switchinfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=switchinfinally_d
    if [ ! -f switchinfinally_d.org ]; then
        TakeLock
        if [ ! -f switchinfinally_d.org ]; then
          mkdir IL
          cp switchinfinally_d.exe IL/switchinfinally_d.exe
          mv switchinfinally_d.exe switchinfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switchinfinally_d.org /out switchinfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switchinfinally_d.org /out switchinfinally_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in switchinfinally_d.reflect.xml
  if [ -f switchinfinally_d.reflect.xml ]
then
    ReflectionRoots="-x switchinfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/switchinfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" switchinfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" switchinfinally_d.asm.exe $CLRTestExecutionArguments
