...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/rangecheckinfinally_d rangecheckinfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/rangecheckinfinally_d rangecheckinfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rangecheckinfinally_d.dasm.il rangecheckinfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=rangecheckinfinally_d.dasm.il rangecheckinfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rangecheckinfinally_d.asm.exe  rangecheckinfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=rangecheckinfinally_d.asm.exe  rangecheckinfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rangecheckinfinally_d
    if [ ! -f rangecheckinfinally_d.org ]; then
        TakeLock
        if [ ! -f rangecheckinfinally_d.org ]; then
          mkdir IL
          cp rangecheckinfinally_d.exe IL/rangecheckinfinally_d.exe
          mv rangecheckinfinally_d.exe rangecheckinfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rangecheckinfinally_d.org /out rangecheckinfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rangecheckinfinally_d.org /out rangecheckinfinally_d.exe
          __cgExitCode=$?
...
ExePath=rangecheckinfinally_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in rangecheckinfinally_d.reflect.xml
  if [ -f rangecheckinfinally_d.reflect.xml ]
then
    ReflectionRoots="-x rangecheckinfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/rangecheckinfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rangecheckinfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rangecheckinfinally_d.asm.exe $CLRTestExecutionArguments
