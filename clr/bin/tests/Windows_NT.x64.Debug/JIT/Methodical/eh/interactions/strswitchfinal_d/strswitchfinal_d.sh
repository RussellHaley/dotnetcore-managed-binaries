...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/strswitchfinal_d strswitchfinal_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/strswitchfinal_d strswitchfinal_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=strswitchfinal_d.dasm.il strswitchfinal_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=strswitchfinal_d.dasm.il strswitchfinal_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=strswitchfinal_d.asm.exe  strswitchfinal_d.dasm.il
    "$CORE_ROOT/ilasm" -output=strswitchfinal_d.asm.exe  strswitchfinal_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=strswitchfinal_d
    if [ ! -f strswitchfinal_d.org ]; then
        TakeLock
        if [ ! -f strswitchfinal_d.org ]; then
          mkdir IL
          cp strswitchfinal_d.exe IL/strswitchfinal_d.exe
          mv strswitchfinal_d.exe strswitchfinal_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in strswitchfinal_d.org /out strswitchfinal_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in strswitchfinal_d.org /out strswitchfinal_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in strswitchfinal_d.reflect.xml
  if [ -f strswitchfinal_d.reflect.xml ]
then
    ReflectionRoots="-x strswitchfinal_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/strswitchfinal_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" strswitchfinal_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" strswitchfinal_d.asm.exe $CLRTestExecutionArguments
