...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\strswitchfinal_ro strswitchfinal_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\strswitchfinal_ro strswitchfinal_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=strswitchfinal_ro.dasm.il strswitchfinal_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=strswitchfinal_ro.dasm.il strswitchfinal_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=strswitchfinal_ro.asm.exe  strswitchfinal_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=strswitchfinal_ro.asm.exe  strswitchfinal_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=strswitchfinal_ro
    if not exist "strswitchfinal_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "strswitchfinal_ro.org" (
            mkdir IL
            copy strswitchfinal_ro.exe IL\strswitchfinal_ro.exe 
            ren strswitchfinal_ro.exe strswitchfinal_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in strswitchfinal_ro.org /out strswitchfinal_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in strswitchfinal_ro.org /out strswitchfinal_ro.exe
...
set ExePath=strswitchfinal_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in strswitchfinal_ro.reflect.xml
    IF EXIST strswitchfinal_ro.reflect.xml set ReflectionRoots=-x strswitchfinal_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\strswitchfinal_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% strswitchfinal_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% strswitchfinal_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
