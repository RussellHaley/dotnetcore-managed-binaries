...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/rangecheckinfinally_r rangecheckinfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/rangecheckinfinally_r rangecheckinfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rangecheckinfinally_r.dasm.il rangecheckinfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=rangecheckinfinally_r.dasm.il rangecheckinfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rangecheckinfinally_r.asm.exe  rangecheckinfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=rangecheckinfinally_r.asm.exe  rangecheckinfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rangecheckinfinally_r
    if [ ! -f rangecheckinfinally_r.org ]; then
        TakeLock
        if [ ! -f rangecheckinfinally_r.org ]; then
          mkdir IL
          cp rangecheckinfinally_r.exe IL/rangecheckinfinally_r.exe
          mv rangecheckinfinally_r.exe rangecheckinfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rangecheckinfinally_r.org /out rangecheckinfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rangecheckinfinally_r.org /out rangecheckinfinally_r.exe
          __cgExitCode=$?
...
ExePath=rangecheckinfinally_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in rangecheckinfinally_r.reflect.xml
  if [ -f rangecheckinfinally_r.reflect.xml ]
then
    ReflectionRoots="-x rangecheckinfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/rangecheckinfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rangecheckinfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rangecheckinfinally_r.asm.exe $CLRTestExecutionArguments
