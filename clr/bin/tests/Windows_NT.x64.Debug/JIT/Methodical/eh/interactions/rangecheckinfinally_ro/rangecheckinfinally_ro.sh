...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/rangecheckinfinally_ro rangecheckinfinally_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/rangecheckinfinally_ro rangecheckinfinally_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rangecheckinfinally_ro.dasm.il rangecheckinfinally_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=rangecheckinfinally_ro.dasm.il rangecheckinfinally_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rangecheckinfinally_ro.asm.exe  rangecheckinfinally_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=rangecheckinfinally_ro.asm.exe  rangecheckinfinally_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rangecheckinfinally_ro
    if [ ! -f rangecheckinfinally_ro.org ]; then
        TakeLock
        if [ ! -f rangecheckinfinally_ro.org ]; then
          mkdir IL
          cp rangecheckinfinally_ro.exe IL/rangecheckinfinally_ro.exe
          mv rangecheckinfinally_ro.exe rangecheckinfinally_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rangecheckinfinally_ro.org /out rangecheckinfinally_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rangecheckinfinally_ro.org /out rangecheckinfinally_ro.exe
          __cgExitCode=$?
...
ExePath=rangecheckinfinally_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in rangecheckinfinally_ro.reflect.xml
  if [ -f rangecheckinfinally_ro.reflect.xml ]
then
    ReflectionRoots="-x rangecheckinfinally_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/rangecheckinfinally_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rangecheckinfinally_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rangecheckinfinally_ro.asm.exe $CLRTestExecutionArguments
