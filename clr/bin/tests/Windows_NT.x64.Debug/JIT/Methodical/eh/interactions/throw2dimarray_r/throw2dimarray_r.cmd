...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\throw2dimarray_r throw2dimarray_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\throw2dimarray_r throw2dimarray_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throw2dimarray_r.dasm.il throw2dimarray_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throw2dimarray_r.dasm.il throw2dimarray_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throw2dimarray_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throw2dimarray_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throw2dimarray_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throw2dimarray_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throw2dimarray_r
    if not exist "throw2dimarray_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throw2dimarray_r.org" (
            mkdir IL
            copy throw2dimarray_r.exe IL\throw2dimarray_r.exe 
            ren throw2dimarray_r.exe throw2dimarray_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw2dimarray_r.org /out throw2dimarray_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw2dimarray_r.org /out throw2dimarray_r.exe
...
set ExePath=throw2dimarray_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throw2dimarray_r.reflect.xml
    IF EXIST throw2dimarray_r.reflect.xml set ReflectionRoots=-x throw2dimarray_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throw2dimarray_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throw2dimarray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throw2dimarray_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
