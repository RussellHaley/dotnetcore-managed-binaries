...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/switchinfinally_r switchinfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/switchinfinally_r switchinfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=switchinfinally_r.dasm.il switchinfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=switchinfinally_r.dasm.il switchinfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=switchinfinally_r.asm.exe  switchinfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=switchinfinally_r.asm.exe  switchinfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=switchinfinally_r
    if [ ! -f switchinfinally_r.org ]; then
        TakeLock
        if [ ! -f switchinfinally_r.org ]; then
          mkdir IL
          cp switchinfinally_r.exe IL/switchinfinally_r.exe
          mv switchinfinally_r.exe switchinfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switchinfinally_r.org /out switchinfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switchinfinally_r.org /out switchinfinally_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in switchinfinally_r.reflect.xml
  if [ -f switchinfinally_r.reflect.xml ]
then
    ReflectionRoots="-x switchinfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/switchinfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" switchinfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" switchinfinally_r.asm.exe $CLRTestExecutionArguments
