...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\strswitchfinal_do strswitchfinal_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\strswitchfinal_do strswitchfinal_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=strswitchfinal_do.dasm.il strswitchfinal_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=strswitchfinal_do.dasm.il strswitchfinal_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=strswitchfinal_do.asm.exe  strswitchfinal_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=strswitchfinal_do.asm.exe  strswitchfinal_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=strswitchfinal_do
    if not exist "strswitchfinal_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "strswitchfinal_do.org" (
            mkdir IL
            copy strswitchfinal_do.exe IL\strswitchfinal_do.exe 
            ren strswitchfinal_do.exe strswitchfinal_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in strswitchfinal_do.org /out strswitchfinal_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in strswitchfinal_do.org /out strswitchfinal_do.exe
...
set ExePath=strswitchfinal_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in strswitchfinal_do.reflect.xml
    IF EXIST strswitchfinal_do.reflect.xml set ReflectionRoots=-x strswitchfinal_do.reflect.xml
    ...
    set ExePath=%LinkBin%\strswitchfinal_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% strswitchfinal_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% strswitchfinal_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
