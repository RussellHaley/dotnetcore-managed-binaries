...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/rangecheckinfinally_do rangecheckinfinally_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/rangecheckinfinally_do rangecheckinfinally_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rangecheckinfinally_do.dasm.il rangecheckinfinally_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=rangecheckinfinally_do.dasm.il rangecheckinfinally_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rangecheckinfinally_do.asm.exe  rangecheckinfinally_do.dasm.il
    "$CORE_ROOT/ilasm" -output=rangecheckinfinally_do.asm.exe  rangecheckinfinally_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rangecheckinfinally_do
    if [ ! -f rangecheckinfinally_do.org ]; then
        TakeLock
        if [ ! -f rangecheckinfinally_do.org ]; then
          mkdir IL
          cp rangecheckinfinally_do.exe IL/rangecheckinfinally_do.exe
          mv rangecheckinfinally_do.exe rangecheckinfinally_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rangecheckinfinally_do.org /out rangecheckinfinally_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rangecheckinfinally_do.org /out rangecheckinfinally_do.exe
          __cgExitCode=$?
...
ExePath=rangecheckinfinally_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in rangecheckinfinally_do.reflect.xml
  if [ -f rangecheckinfinally_do.reflect.xml ]
then
    ReflectionRoots="-x rangecheckinfinally_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/rangecheckinfinally_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rangecheckinfinally_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rangecheckinfinally_do.asm.exe $CLRTestExecutionArguments
