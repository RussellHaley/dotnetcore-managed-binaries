...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\strswitchfinal_r strswitchfinal_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\strswitchfinal_r strswitchfinal_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=strswitchfinal_r.dasm.il strswitchfinal_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=strswitchfinal_r.dasm.il strswitchfinal_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=strswitchfinal_r.asm.exe  strswitchfinal_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=strswitchfinal_r.asm.exe  strswitchfinal_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=strswitchfinal_r
    if not exist "strswitchfinal_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "strswitchfinal_r.org" (
            mkdir IL
            copy strswitchfinal_r.exe IL\strswitchfinal_r.exe 
            ren strswitchfinal_r.exe strswitchfinal_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in strswitchfinal_r.org /out strswitchfinal_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in strswitchfinal_r.org /out strswitchfinal_r.exe
...
set ExePath=strswitchfinal_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in strswitchfinal_r.reflect.xml
    IF EXIST strswitchfinal_r.reflect.xml set ReflectionRoots=-x strswitchfinal_r.reflect.xml
    ...
    set ExePath=%LinkBin%\strswitchfinal_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% strswitchfinal_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% strswitchfinal_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
