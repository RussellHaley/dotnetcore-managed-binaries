...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\rangecheckinfinally_r rangecheckinfinally_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\rangecheckinfinally_r rangecheckinfinally_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=rangecheckinfinally_r.dasm.il rangecheckinfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=rangecheckinfinally_r.dasm.il rangecheckinfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=rangecheckinfinally_r.asm.exe  rangecheckinfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=rangecheckinfinally_r.asm.exe  rangecheckinfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=rangecheckinfinally_r
    if not exist "rangecheckinfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "rangecheckinfinally_r.org" (
            mkdir IL
            copy rangecheckinfinally_r.exe IL\rangecheckinfinally_r.exe 
            ren rangecheckinfinally_r.exe rangecheckinfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rangecheckinfinally_r.org /out rangecheckinfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rangecheckinfinally_r.org /out rangecheckinfinally_r.exe
...
set ExePath=rangecheckinfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in rangecheckinfinally_r.reflect.xml
    IF EXIST rangecheckinfinally_r.reflect.xml set ReflectionRoots=-x rangecheckinfinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\rangecheckinfinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% rangecheckinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% rangecheckinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
