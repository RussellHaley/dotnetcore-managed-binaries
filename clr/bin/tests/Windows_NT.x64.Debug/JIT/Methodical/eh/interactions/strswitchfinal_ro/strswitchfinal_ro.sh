...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/strswitchfinal_ro strswitchfinal_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/strswitchfinal_ro strswitchfinal_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=strswitchfinal_ro.dasm.il strswitchfinal_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=strswitchfinal_ro.dasm.il strswitchfinal_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=strswitchfinal_ro.asm.exe  strswitchfinal_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=strswitchfinal_ro.asm.exe  strswitchfinal_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=strswitchfinal_ro
    if [ ! -f strswitchfinal_ro.org ]; then
        TakeLock
        if [ ! -f strswitchfinal_ro.org ]; then
          mkdir IL
          cp strswitchfinal_ro.exe IL/strswitchfinal_ro.exe
          mv strswitchfinal_ro.exe strswitchfinal_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in strswitchfinal_ro.org /out strswitchfinal_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in strswitchfinal_ro.org /out strswitchfinal_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in strswitchfinal_ro.reflect.xml
  if [ -f strswitchfinal_ro.reflect.xml ]
then
    ReflectionRoots="-x strswitchfinal_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/strswitchfinal_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" strswitchfinal_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" strswitchfinal_ro.asm.exe $CLRTestExecutionArguments
