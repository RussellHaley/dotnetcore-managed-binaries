...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\switchinfinally_d switchinfinally_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\switchinfinally_d switchinfinally_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=switchinfinally_d.dasm.il switchinfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=switchinfinally_d.dasm.il switchinfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=switchinfinally_d.asm.exe  switchinfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=switchinfinally_d.asm.exe  switchinfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=switchinfinally_d
    if not exist "switchinfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "switchinfinally_d.org" (
            mkdir IL
            copy switchinfinally_d.exe IL\switchinfinally_d.exe 
            ren switchinfinally_d.exe switchinfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switchinfinally_d.org /out switchinfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switchinfinally_d.org /out switchinfinally_d.exe
...
set ExePath=switchinfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in switchinfinally_d.reflect.xml
    IF EXIST switchinfinally_d.reflect.xml set ReflectionRoots=-x switchinfinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\switchinfinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% switchinfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% switchinfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
