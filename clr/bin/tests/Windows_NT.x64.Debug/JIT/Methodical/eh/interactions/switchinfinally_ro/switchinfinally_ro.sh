...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/switchinfinally_ro switchinfinally_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/switchinfinally_ro switchinfinally_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=switchinfinally_ro.dasm.il switchinfinally_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=switchinfinally_ro.dasm.il switchinfinally_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=switchinfinally_ro.asm.exe  switchinfinally_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=switchinfinally_ro.asm.exe  switchinfinally_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=switchinfinally_ro
    if [ ! -f switchinfinally_ro.org ]; then
        TakeLock
        if [ ! -f switchinfinally_ro.org ]; then
          mkdir IL
          cp switchinfinally_ro.exe IL/switchinfinally_ro.exe
          mv switchinfinally_ro.exe switchinfinally_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switchinfinally_ro.org /out switchinfinally_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in switchinfinally_ro.org /out switchinfinally_ro.exe
          __cgExitCode=$?
...
ExePath=switchinfinally_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in switchinfinally_ro.reflect.xml
  if [ -f switchinfinally_ro.reflect.xml ]
then
    ReflectionRoots="-x switchinfinally_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/switchinfinally_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" switchinfinally_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" switchinfinally_ro.asm.exe $CLRTestExecutionArguments
