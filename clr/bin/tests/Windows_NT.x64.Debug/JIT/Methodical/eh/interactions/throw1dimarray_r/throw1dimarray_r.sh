...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/throw1dimarray_r throw1dimarray_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/interactions/throw1dimarray_r throw1dimarray_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throw1dimarray_r.dasm.il throw1dimarray_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throw1dimarray_r.dasm.il throw1dimarray_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throw1dimarray_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throw1dimarray_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throw1dimarray_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throw1dimarray_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throw1dimarray_r
    if [ ! -f throw1dimarray_r.org ]; then
        TakeLock
        if [ ! -f throw1dimarray_r.org ]; then
          mkdir IL
          cp throw1dimarray_r.exe IL/throw1dimarray_r.exe
          mv throw1dimarray_r.exe throw1dimarray_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw1dimarray_r.org /out throw1dimarray_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw1dimarray_r.org /out throw1dimarray_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throw1dimarray_r.reflect.xml
  if [ -f throw1dimarray_r.reflect.xml ]
then
    ReflectionRoots="-x throw1dimarray_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throw1dimarray_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throw1dimarray_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throw1dimarray_r.asm.exe $CLRTestExecutionArguments
