...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\strswitchfinal_d strswitchfinal_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\strswitchfinal_d strswitchfinal_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=strswitchfinal_d.dasm.il strswitchfinal_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=strswitchfinal_d.dasm.il strswitchfinal_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=strswitchfinal_d.asm.exe  strswitchfinal_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=strswitchfinal_d.asm.exe  strswitchfinal_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=strswitchfinal_d
    if not exist "strswitchfinal_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "strswitchfinal_d.org" (
            mkdir IL
            copy strswitchfinal_d.exe IL\strswitchfinal_d.exe 
            ren strswitchfinal_d.exe strswitchfinal_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in strswitchfinal_d.org /out strswitchfinal_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in strswitchfinal_d.org /out strswitchfinal_d.exe
...
set ExePath=strswitchfinal_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in strswitchfinal_d.reflect.xml
    IF EXIST strswitchfinal_d.reflect.xml set ReflectionRoots=-x strswitchfinal_d.reflect.xml
    ...
    set ExePath=%LinkBin%\strswitchfinal_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% strswitchfinal_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% strswitchfinal_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
