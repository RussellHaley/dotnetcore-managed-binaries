...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\switchinfinally_do switchinfinally_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\switchinfinally_do switchinfinally_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=switchinfinally_do.dasm.il switchinfinally_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=switchinfinally_do.dasm.il switchinfinally_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=switchinfinally_do.asm.exe  switchinfinally_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=switchinfinally_do.asm.exe  switchinfinally_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=switchinfinally_do
    if not exist "switchinfinally_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "switchinfinally_do.org" (
            mkdir IL
            copy switchinfinally_do.exe IL\switchinfinally_do.exe 
            ren switchinfinally_do.exe switchinfinally_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switchinfinally_do.org /out switchinfinally_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switchinfinally_do.org /out switchinfinally_do.exe
...
set ExePath=switchinfinally_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in switchinfinally_do.reflect.xml
    IF EXIST switchinfinally_do.reflect.xml set ReflectionRoots=-x switchinfinally_do.reflect.xml
    ...
    set ExePath=%LinkBin%\switchinfinally_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% switchinfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% switchinfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
