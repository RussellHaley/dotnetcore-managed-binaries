...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\switchinfinally_ro switchinfinally_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\interactions\switchinfinally_ro switchinfinally_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=switchinfinally_ro.dasm.il switchinfinally_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=switchinfinally_ro.dasm.il switchinfinally_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=switchinfinally_ro.asm.exe  switchinfinally_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=switchinfinally_ro.asm.exe  switchinfinally_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=switchinfinally_ro
    if not exist "switchinfinally_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "switchinfinally_ro.org" (
            mkdir IL
            copy switchinfinally_ro.exe IL\switchinfinally_ro.exe 
            ren switchinfinally_ro.exe switchinfinally_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switchinfinally_ro.org /out switchinfinally_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in switchinfinally_ro.org /out switchinfinally_ro.exe
...
set ExePath=switchinfinally_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in switchinfinally_ro.reflect.xml
    IF EXIST switchinfinally_ro.reflect.xml set ReflectionRoots=-x switchinfinally_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\switchinfinally_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% switchinfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% switchinfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
