...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samethrowtwice_do samethrowtwice_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samethrowtwice_do samethrowtwice_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=samethrowtwice_do.dasm.il samethrowtwice_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=samethrowtwice_do.dasm.il samethrowtwice_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=samethrowtwice_do.asm.exe  samethrowtwice_do.dasm.il
    "$CORE_ROOT/ilasm" -output=samethrowtwice_do.asm.exe  samethrowtwice_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=samethrowtwice_do
    if [ ! -f samethrowtwice_do.org ]; then
        TakeLock
        if [ ! -f samethrowtwice_do.org ]; then
          mkdir IL
          cp samethrowtwice_do.exe IL/samethrowtwice_do.exe
          mv samethrowtwice_do.exe samethrowtwice_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samethrowtwice_do.org /out samethrowtwice_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samethrowtwice_do.org /out samethrowtwice_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in samethrowtwice_do.reflect.xml
  if [ -f samethrowtwice_do.reflect.xml ]
then
    ReflectionRoots="-x samethrowtwice_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/samethrowtwice_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" samethrowtwice_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" samethrowtwice_do.asm.exe $CLRTestExecutionArguments
