...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samerethrowtwice_do samerethrowtwice_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samerethrowtwice_do samerethrowtwice_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=samerethrowtwice_do.dasm.il samerethrowtwice_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=samerethrowtwice_do.dasm.il samerethrowtwice_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=samerethrowtwice_do.asm.exe  samerethrowtwice_do.dasm.il
    "$CORE_ROOT/ilasm" -output=samerethrowtwice_do.asm.exe  samerethrowtwice_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=samerethrowtwice_do
    if [ ! -f samerethrowtwice_do.org ]; then
        TakeLock
        if [ ! -f samerethrowtwice_do.org ]; then
          mkdir IL
          cp samerethrowtwice_do.exe IL/samerethrowtwice_do.exe
          mv samerethrowtwice_do.exe samerethrowtwice_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samerethrowtwice_do.org /out samerethrowtwice_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samerethrowtwice_do.org /out samerethrowtwice_do.exe
          __cgExitCode=$?
...
ExePath=samerethrowtwice_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in samerethrowtwice_do.reflect.xml
  if [ -f samerethrowtwice_do.reflect.xml ]
then
    ReflectionRoots="-x samerethrowtwice_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/samerethrowtwice_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" samerethrowtwice_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" samerethrowtwice_do.asm.exe $CLRTestExecutionArguments
