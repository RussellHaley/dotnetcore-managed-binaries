...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/throwwithhandlerscatchingbase_do throwwithhandlerscatchingbase_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/throwwithhandlerscatchingbase_do throwwithhandlerscatchingbase_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwwithhandlerscatchingbase_do.dasm.il throwwithhandlerscatchingbase_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwwithhandlerscatchingbase_do.dasm.il throwwithhandlerscatchingbase_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwwithhandlerscatchingbase_do.asm.exe  throwwithhandlerscatchingbase_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwwithhandlerscatchingbase_do.asm.exe  throwwithhandlerscatchingbase_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwwithhandlerscatchingbase_do
    if [ ! -f throwwithhandlerscatchingbase_do.org ]; then
        TakeLock
        if [ ! -f throwwithhandlerscatchingbase_do.org ]; then
          mkdir IL
          cp throwwithhandlerscatchingbase_do.exe IL/throwwithhandlerscatchingbase_do.exe
          mv throwwithhandlerscatchingbase_do.exe throwwithhandlerscatchingbase_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwwithhandlerscatchingbase_do.org /out throwwithhandlerscatchingbase_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwwithhandlerscatchingbase_do.org /out throwwithhandlerscatchingbase_do.exe
          __cgExitCode=$?
...
ExePath=throwwithhandlerscatchingbase_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwwithhandlerscatchingbase_do.reflect.xml
  if [ -f throwwithhandlerscatchingbase_do.reflect.xml ]
then
    ReflectionRoots="-x throwwithhandlerscatchingbase_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwwithhandlerscatchingbase_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwwithhandlerscatchingbase_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwwithhandlerscatchingbase_do.asm.exe $CLRTestExecutionArguments
