...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samerethrowtwice_d samerethrowtwice_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samerethrowtwice_d samerethrowtwice_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=samerethrowtwice_d.dasm.il samerethrowtwice_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=samerethrowtwice_d.dasm.il samerethrowtwice_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=samerethrowtwice_d.asm.exe  samerethrowtwice_d.dasm.il
    "$CORE_ROOT/ilasm" -output=samerethrowtwice_d.asm.exe  samerethrowtwice_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=samerethrowtwice_d
    if [ ! -f samerethrowtwice_d.org ]; then
        TakeLock
        if [ ! -f samerethrowtwice_d.org ]; then
          mkdir IL
          cp samerethrowtwice_d.exe IL/samerethrowtwice_d.exe
          mv samerethrowtwice_d.exe samerethrowtwice_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samerethrowtwice_d.org /out samerethrowtwice_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samerethrowtwice_d.org /out samerethrowtwice_d.exe
          __cgExitCode=$?
...
ExePath=samerethrowtwice_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in samerethrowtwice_d.reflect.xml
  if [ -f samerethrowtwice_d.reflect.xml ]
then
    ReflectionRoots="-x samerethrowtwice_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/samerethrowtwice_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" samerethrowtwice_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" samerethrowtwice_d.asm.exe $CLRTestExecutionArguments
