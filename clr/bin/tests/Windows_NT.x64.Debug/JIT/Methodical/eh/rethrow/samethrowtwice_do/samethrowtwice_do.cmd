...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samethrowtwice_do samethrowtwice_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samethrowtwice_do samethrowtwice_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=samethrowtwice_do.dasm.il samethrowtwice_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=samethrowtwice_do.dasm.il samethrowtwice_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=samethrowtwice_do.asm.exe  samethrowtwice_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=samethrowtwice_do.asm.exe  samethrowtwice_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=samethrowtwice_do
    if not exist "samethrowtwice_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "samethrowtwice_do.org" (
            mkdir IL
            copy samethrowtwice_do.exe IL\samethrowtwice_do.exe 
            ren samethrowtwice_do.exe samethrowtwice_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samethrowtwice_do.org /out samethrowtwice_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samethrowtwice_do.org /out samethrowtwice_do.exe
...
set ExePath=samethrowtwice_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in samethrowtwice_do.reflect.xml
    IF EXIST samethrowtwice_do.reflect.xml set ReflectionRoots=-x samethrowtwice_do.reflect.xml
    ...
    set ExePath=%LinkBin%\samethrowtwice_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% samethrowtwice_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% samethrowtwice_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
