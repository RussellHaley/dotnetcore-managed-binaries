...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samerethrowtwice_d samerethrowtwice_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samerethrowtwice_d samerethrowtwice_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=samerethrowtwice_d.dasm.il samerethrowtwice_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=samerethrowtwice_d.dasm.il samerethrowtwice_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=samerethrowtwice_d.asm.exe  samerethrowtwice_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=samerethrowtwice_d.asm.exe  samerethrowtwice_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=samerethrowtwice_d
    if not exist "samerethrowtwice_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "samerethrowtwice_d.org" (
            mkdir IL
            copy samerethrowtwice_d.exe IL\samerethrowtwice_d.exe 
            ren samerethrowtwice_d.exe samerethrowtwice_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samerethrowtwice_d.org /out samerethrowtwice_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samerethrowtwice_d.org /out samerethrowtwice_d.exe
...
set ExePath=samerethrowtwice_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in samerethrowtwice_d.reflect.xml
    IF EXIST samerethrowtwice_d.reflect.xml set ReflectionRoots=-x samerethrowtwice_d.reflect.xml
    ...
    set ExePath=%LinkBin%\samerethrowtwice_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% samerethrowtwice_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% samerethrowtwice_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
