...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samethrowtwice_r samethrowtwice_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samethrowtwice_r samethrowtwice_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=samethrowtwice_r.dasm.il samethrowtwice_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=samethrowtwice_r.dasm.il samethrowtwice_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=samethrowtwice_r.asm.exe  samethrowtwice_r.dasm.il
    "$CORE_ROOT/ilasm" -output=samethrowtwice_r.asm.exe  samethrowtwice_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=samethrowtwice_r
    if [ ! -f samethrowtwice_r.org ]; then
        TakeLock
        if [ ! -f samethrowtwice_r.org ]; then
          mkdir IL
          cp samethrowtwice_r.exe IL/samethrowtwice_r.exe
          mv samethrowtwice_r.exe samethrowtwice_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samethrowtwice_r.org /out samethrowtwice_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samethrowtwice_r.org /out samethrowtwice_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in samethrowtwice_r.reflect.xml
  if [ -f samethrowtwice_r.reflect.xml ]
then
    ReflectionRoots="-x samethrowtwice_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/samethrowtwice_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" samethrowtwice_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" samethrowtwice_r.asm.exe $CLRTestExecutionArguments
