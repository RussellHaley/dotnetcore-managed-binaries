...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samerethrowtwice_ro samerethrowtwice_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samerethrowtwice_ro samerethrowtwice_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=samerethrowtwice_ro.dasm.il samerethrowtwice_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=samerethrowtwice_ro.dasm.il samerethrowtwice_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=samerethrowtwice_ro.asm.exe  samerethrowtwice_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=samerethrowtwice_ro.asm.exe  samerethrowtwice_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=samerethrowtwice_ro
    if not exist "samerethrowtwice_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "samerethrowtwice_ro.org" (
            mkdir IL
            copy samerethrowtwice_ro.exe IL\samerethrowtwice_ro.exe 
            ren samerethrowtwice_ro.exe samerethrowtwice_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samerethrowtwice_ro.org /out samerethrowtwice_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samerethrowtwice_ro.org /out samerethrowtwice_ro.exe
...
set ExePath=samerethrowtwice_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in samerethrowtwice_ro.reflect.xml
    IF EXIST samerethrowtwice_ro.reflect.xml set ReflectionRoots=-x samerethrowtwice_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\samerethrowtwice_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% samerethrowtwice_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% samerethrowtwice_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
