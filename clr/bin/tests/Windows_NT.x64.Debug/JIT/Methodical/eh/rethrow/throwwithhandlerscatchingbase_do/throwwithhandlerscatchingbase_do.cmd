...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\throwwithhandlerscatchingbase_do throwwithhandlerscatchingbase_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\throwwithhandlerscatchingbase_do throwwithhandlerscatchingbase_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwwithhandlerscatchingbase_do.dasm.il throwwithhandlerscatchingbase_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwwithhandlerscatchingbase_do.dasm.il throwwithhandlerscatchingbase_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwwithhandlerscatchingbase_do.asm.exe  throwwithhandlerscatchingbase_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwwithhandlerscatchingbase_do.asm.exe  throwwithhandlerscatchingbase_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwwithhandlerscatchingbase_do
    if not exist "throwwithhandlerscatchingbase_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwwithhandlerscatchingbase_do.org" (
            mkdir IL
            copy throwwithhandlerscatchingbase_do.exe IL\throwwithhandlerscatchingbase_do.exe 
            ren throwwithhandlerscatchingbase_do.exe throwwithhandlerscatchingbase_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwwithhandlerscatchingbase_do.org /out throwwithhandlerscatchingbase_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwwithhandlerscatchingbase_do.org /out throwwithhandlerscatchingbase_do.exe
...
set ExePath=throwwithhandlerscatchingbase_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwwithhandlerscatchingbase_do.reflect.xml
    IF EXIST throwwithhandlerscatchingbase_do.reflect.xml set ReflectionRoots=-x throwwithhandlerscatchingbase_do.reflect.xml
    ...
    set ExePath=%LinkBin%\throwwithhandlerscatchingbase_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwwithhandlerscatchingbase_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwwithhandlerscatchingbase_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
