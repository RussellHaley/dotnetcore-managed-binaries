...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samerethrowtwice_ro samerethrowtwice_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samerethrowtwice_ro samerethrowtwice_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=samerethrowtwice_ro.dasm.il samerethrowtwice_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=samerethrowtwice_ro.dasm.il samerethrowtwice_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=samerethrowtwice_ro.asm.exe  samerethrowtwice_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=samerethrowtwice_ro.asm.exe  samerethrowtwice_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=samerethrowtwice_ro
    if [ ! -f samerethrowtwice_ro.org ]; then
        TakeLock
        if [ ! -f samerethrowtwice_ro.org ]; then
          mkdir IL
          cp samerethrowtwice_ro.exe IL/samerethrowtwice_ro.exe
          mv samerethrowtwice_ro.exe samerethrowtwice_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samerethrowtwice_ro.org /out samerethrowtwice_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samerethrowtwice_ro.org /out samerethrowtwice_ro.exe
          __cgExitCode=$?
...
ExePath=samerethrowtwice_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in samerethrowtwice_ro.reflect.xml
  if [ -f samerethrowtwice_ro.reflect.xml ]
then
    ReflectionRoots="-x samerethrowtwice_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/samerethrowtwice_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" samerethrowtwice_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" samerethrowtwice_ro.asm.exe $CLRTestExecutionArguments
