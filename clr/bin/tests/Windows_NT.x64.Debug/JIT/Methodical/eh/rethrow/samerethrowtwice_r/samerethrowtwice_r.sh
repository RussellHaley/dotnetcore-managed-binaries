...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samerethrowtwice_r samerethrowtwice_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samerethrowtwice_r samerethrowtwice_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=samerethrowtwice_r.dasm.il samerethrowtwice_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=samerethrowtwice_r.dasm.il samerethrowtwice_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=samerethrowtwice_r.asm.exe  samerethrowtwice_r.dasm.il
    "$CORE_ROOT/ilasm" -output=samerethrowtwice_r.asm.exe  samerethrowtwice_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=samerethrowtwice_r
    if [ ! -f samerethrowtwice_r.org ]; then
        TakeLock
        if [ ! -f samerethrowtwice_r.org ]; then
          mkdir IL
          cp samerethrowtwice_r.exe IL/samerethrowtwice_r.exe
          mv samerethrowtwice_r.exe samerethrowtwice_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samerethrowtwice_r.org /out samerethrowtwice_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samerethrowtwice_r.org /out samerethrowtwice_r.exe
          __cgExitCode=$?
...
ExePath=samerethrowtwice_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in samerethrowtwice_r.reflect.xml
  if [ -f samerethrowtwice_r.reflect.xml ]
then
    ReflectionRoots="-x samerethrowtwice_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/samerethrowtwice_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" samerethrowtwice_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" samerethrowtwice_r.asm.exe $CLRTestExecutionArguments
