...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samerethrowtwice_r samerethrowtwice_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samerethrowtwice_r samerethrowtwice_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=samerethrowtwice_r.dasm.il samerethrowtwice_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=samerethrowtwice_r.dasm.il samerethrowtwice_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=samerethrowtwice_r.asm.exe  samerethrowtwice_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=samerethrowtwice_r.asm.exe  samerethrowtwice_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=samerethrowtwice_r
    if not exist "samerethrowtwice_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "samerethrowtwice_r.org" (
            mkdir IL
            copy samerethrowtwice_r.exe IL\samerethrowtwice_r.exe 
            ren samerethrowtwice_r.exe samerethrowtwice_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samerethrowtwice_r.org /out samerethrowtwice_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samerethrowtwice_r.org /out samerethrowtwice_r.exe
...
set ExePath=samerethrowtwice_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in samerethrowtwice_r.reflect.xml
    IF EXIST samerethrowtwice_r.reflect.xml set ReflectionRoots=-x samerethrowtwice_r.reflect.xml
    ...
    set ExePath=%LinkBin%\samerethrowtwice_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% samerethrowtwice_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% samerethrowtwice_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
