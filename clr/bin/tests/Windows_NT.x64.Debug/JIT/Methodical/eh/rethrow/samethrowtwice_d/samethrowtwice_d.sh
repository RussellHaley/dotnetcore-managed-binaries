...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samethrowtwice_d samethrowtwice_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samethrowtwice_d samethrowtwice_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=samethrowtwice_d.dasm.il samethrowtwice_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=samethrowtwice_d.dasm.il samethrowtwice_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=samethrowtwice_d.asm.exe  samethrowtwice_d.dasm.il
    "$CORE_ROOT/ilasm" -output=samethrowtwice_d.asm.exe  samethrowtwice_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=samethrowtwice_d
    if [ ! -f samethrowtwice_d.org ]; then
        TakeLock
        if [ ! -f samethrowtwice_d.org ]; then
          mkdir IL
          cp samethrowtwice_d.exe IL/samethrowtwice_d.exe
          mv samethrowtwice_d.exe samethrowtwice_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samethrowtwice_d.org /out samethrowtwice_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samethrowtwice_d.org /out samethrowtwice_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in samethrowtwice_d.reflect.xml
  if [ -f samethrowtwice_d.reflect.xml ]
then
    ReflectionRoots="-x samethrowtwice_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/samethrowtwice_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" samethrowtwice_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" samethrowtwice_d.asm.exe $CLRTestExecutionArguments
