...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samerethrowtwice_do samerethrowtwice_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samerethrowtwice_do samerethrowtwice_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=samerethrowtwice_do.dasm.il samerethrowtwice_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=samerethrowtwice_do.dasm.il samerethrowtwice_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=samerethrowtwice_do.asm.exe  samerethrowtwice_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=samerethrowtwice_do.asm.exe  samerethrowtwice_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=samerethrowtwice_do
    if not exist "samerethrowtwice_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "samerethrowtwice_do.org" (
            mkdir IL
            copy samerethrowtwice_do.exe IL\samerethrowtwice_do.exe 
            ren samerethrowtwice_do.exe samerethrowtwice_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samerethrowtwice_do.org /out samerethrowtwice_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samerethrowtwice_do.org /out samerethrowtwice_do.exe
...
set ExePath=samerethrowtwice_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in samerethrowtwice_do.reflect.xml
    IF EXIST samerethrowtwice_do.reflect.xml set ReflectionRoots=-x samerethrowtwice_do.reflect.xml
    ...
    set ExePath=%LinkBin%\samerethrowtwice_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% samerethrowtwice_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% samerethrowtwice_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
