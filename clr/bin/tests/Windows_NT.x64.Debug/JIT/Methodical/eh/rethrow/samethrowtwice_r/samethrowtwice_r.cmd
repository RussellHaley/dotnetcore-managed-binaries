...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samethrowtwice_r samethrowtwice_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samethrowtwice_r samethrowtwice_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=samethrowtwice_r.dasm.il samethrowtwice_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=samethrowtwice_r.dasm.il samethrowtwice_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=samethrowtwice_r.asm.exe  samethrowtwice_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=samethrowtwice_r.asm.exe  samethrowtwice_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=samethrowtwice_r
    if not exist "samethrowtwice_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "samethrowtwice_r.org" (
            mkdir IL
            copy samethrowtwice_r.exe IL\samethrowtwice_r.exe 
            ren samethrowtwice_r.exe samethrowtwice_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samethrowtwice_r.org /out samethrowtwice_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samethrowtwice_r.org /out samethrowtwice_r.exe
...
set ExePath=samethrowtwice_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in samethrowtwice_r.reflect.xml
    IF EXIST samethrowtwice_r.reflect.xml set ReflectionRoots=-x samethrowtwice_r.reflect.xml
    ...
    set ExePath=%LinkBin%\samethrowtwice_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% samethrowtwice_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% samethrowtwice_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
