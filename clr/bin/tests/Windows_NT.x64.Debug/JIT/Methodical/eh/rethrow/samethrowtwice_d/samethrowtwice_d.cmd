...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samethrowtwice_d samethrowtwice_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samethrowtwice_d samethrowtwice_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=samethrowtwice_d.dasm.il samethrowtwice_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=samethrowtwice_d.dasm.il samethrowtwice_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=samethrowtwice_d.asm.exe  samethrowtwice_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=samethrowtwice_d.asm.exe  samethrowtwice_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=samethrowtwice_d
    if not exist "samethrowtwice_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "samethrowtwice_d.org" (
            mkdir IL
            copy samethrowtwice_d.exe IL\samethrowtwice_d.exe 
            ren samethrowtwice_d.exe samethrowtwice_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samethrowtwice_d.org /out samethrowtwice_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samethrowtwice_d.org /out samethrowtwice_d.exe
...
set ExePath=samethrowtwice_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in samethrowtwice_d.reflect.xml
    IF EXIST samethrowtwice_d.reflect.xml set ReflectionRoots=-x samethrowtwice_d.reflect.xml
    ...
    set ExePath=%LinkBin%\samethrowtwice_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% samethrowtwice_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% samethrowtwice_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
