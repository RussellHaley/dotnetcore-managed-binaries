...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samethrowtwice_ro samethrowtwice_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/rethrow/samethrowtwice_ro samethrowtwice_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=samethrowtwice_ro.dasm.il samethrowtwice_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=samethrowtwice_ro.dasm.il samethrowtwice_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=samethrowtwice_ro.asm.exe  samethrowtwice_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=samethrowtwice_ro.asm.exe  samethrowtwice_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=samethrowtwice_ro
    if [ ! -f samethrowtwice_ro.org ]; then
        TakeLock
        if [ ! -f samethrowtwice_ro.org ]; then
          mkdir IL
          cp samethrowtwice_ro.exe IL/samethrowtwice_ro.exe
          mv samethrowtwice_ro.exe samethrowtwice_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samethrowtwice_ro.org /out samethrowtwice_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in samethrowtwice_ro.org /out samethrowtwice_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in samethrowtwice_ro.reflect.xml
  if [ -f samethrowtwice_ro.reflect.xml ]
then
    ReflectionRoots="-x samethrowtwice_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/samethrowtwice_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" samethrowtwice_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" samethrowtwice_ro.asm.exe $CLRTestExecutionArguments
