...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samethrowtwice_ro samethrowtwice_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\rethrow\samethrowtwice_ro samethrowtwice_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=samethrowtwice_ro.dasm.il samethrowtwice_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=samethrowtwice_ro.dasm.il samethrowtwice_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=samethrowtwice_ro.asm.exe  samethrowtwice_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=samethrowtwice_ro.asm.exe  samethrowtwice_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=samethrowtwice_ro
    if not exist "samethrowtwice_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "samethrowtwice_ro.org" (
            mkdir IL
            copy samethrowtwice_ro.exe IL\samethrowtwice_ro.exe 
            ren samethrowtwice_ro.exe samethrowtwice_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samethrowtwice_ro.org /out samethrowtwice_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in samethrowtwice_ro.org /out samethrowtwice_ro.exe
...
set ExePath=samethrowtwice_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in samethrowtwice_ro.reflect.xml
    IF EXIST samethrowtwice_ro.reflect.xml set ReflectionRoots=-x samethrowtwice_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\samethrowtwice_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% samethrowtwice_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% samethrowtwice_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
