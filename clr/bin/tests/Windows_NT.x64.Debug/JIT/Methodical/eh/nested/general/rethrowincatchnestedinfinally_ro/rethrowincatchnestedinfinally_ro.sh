...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/rethrowincatchnestedinfinally_ro rethrowincatchnestedinfinally_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/rethrowincatchnestedinfinally_ro rethrowincatchnestedinfinally_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rethrowincatchnestedinfinally_ro.dasm.il rethrowincatchnestedinfinally_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=rethrowincatchnestedinfinally_ro.dasm.il rethrowincatchnestedinfinally_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rethrowincatchnestedinfinally_ro.asm.exe  rethrowincatchnestedinfinally_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=rethrowincatchnestedinfinally_ro.asm.exe  rethrowincatchnestedinfinally_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rethrowincatchnestedinfinally_ro
    if [ ! -f rethrowincatchnestedinfinally_ro.org ]; then
        TakeLock
        if [ ! -f rethrowincatchnestedinfinally_ro.org ]; then
          mkdir IL
          cp rethrowincatchnestedinfinally_ro.exe IL/rethrowincatchnestedinfinally_ro.exe
          mv rethrowincatchnestedinfinally_ro.exe rethrowincatchnestedinfinally_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrowincatchnestedinfinally_ro.org /out rethrowincatchnestedinfinally_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrowincatchnestedinfinally_ro.org /out rethrowincatchnestedinfinally_ro.exe
          __cgExitCode=$?
...
ExePath=rethrowincatchnestedinfinally_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in rethrowincatchnestedinfinally_ro.reflect.xml
  if [ -f rethrowincatchnestedinfinally_ro.reflect.xml ]
then
    ReflectionRoots="-x rethrowincatchnestedinfinally_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/rethrowincatchnestedinfinally_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rethrowincatchnestedinfinally_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rethrowincatchnestedinfinally_ro.asm.exe $CLRTestExecutionArguments
