...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallyrecursive_20_r throwinfinallyrecursive_20_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallyrecursive_20_r throwinfinallyrecursive_20_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyrecursive_20_r.dasm.il throwinfinallyrecursive_20_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyrecursive_20_r.dasm.il throwinfinallyrecursive_20_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallyrecursive_20_r.asm.exe  throwinfinallyrecursive_20_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallyrecursive_20_r.asm.exe  throwinfinallyrecursive_20_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallyrecursive_20_r
    if [ ! -f throwinfinallyrecursive_20_r.org ]; then
        TakeLock
        if [ ! -f throwinfinallyrecursive_20_r.org ]; then
          mkdir IL
          cp throwinfinallyrecursive_20_r.exe IL/throwinfinallyrecursive_20_r.exe
          mv throwinfinallyrecursive_20_r.exe throwinfinallyrecursive_20_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyrecursive_20_r.org /out throwinfinallyrecursive_20_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyrecursive_20_r.org /out throwinfinallyrecursive_20_r.exe
          __cgExitCode=$?
...
ExePath=throwinfinallyrecursive_20_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallyrecursive_20_r.reflect.xml
  if [ -f throwinfinallyrecursive_20_r.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallyrecursive_20_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallyrecursive_20_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallyrecursive_20_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallyrecursive_20_r.asm.exe $CLRTestExecutionArguments
