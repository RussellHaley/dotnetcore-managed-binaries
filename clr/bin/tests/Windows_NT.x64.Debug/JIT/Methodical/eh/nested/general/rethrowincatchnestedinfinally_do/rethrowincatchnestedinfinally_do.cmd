...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=rethrowincatchnestedinfinally_do.dasm.il rethrowincatchnestedinfinally_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=rethrowincatchnestedinfinally_do.dasm.il rethrowincatchnestedinfinally_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=rethrowincatchnestedinfinally_do.asm.exe  rethrowincatchnestedinfinally_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=rethrowincatchnestedinfinally_do.asm.exe  rethrowincatchnestedinfinally_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=rethrowincatchnestedinfinally_do
    if not exist "rethrowincatchnestedinfinally_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "rethrowincatchnestedinfinally_do.org" (
            mkdir IL
            copy rethrowincatchnestedinfinally_do.exe IL\rethrowincatchnestedinfinally_do.exe 
            ren rethrowincatchnestedinfinally_do.exe rethrowincatchnestedinfinally_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrowincatchnestedinfinally_do.org /out rethrowincatchnestedinfinally_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrowincatchnestedinfinally_do.org /out rethrowincatchnestedinfinally_do.exe
...
set ExePath=rethrowincatchnestedinfinally_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in rethrowincatchnestedinfinally_do.reflect.xml
    IF EXIST rethrowincatchnestedinfinally_do.reflect.xml set ReflectionRoots=-x rethrowincatchnestedinfinally_do.reflect.xml
    ...
    set ExePath=%LinkBin%\rethrowincatchnestedinfinally_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% rethrowincatchnestedinfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% rethrowincatchnestedinfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
