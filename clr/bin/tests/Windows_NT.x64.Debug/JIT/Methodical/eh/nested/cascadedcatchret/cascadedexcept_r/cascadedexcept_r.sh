...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/cascadedexcept_r cascadedexcept_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/cascadedexcept_r cascadedexcept_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=cascadedexcept_r.dasm.il cascadedexcept_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=cascadedexcept_r.dasm.il cascadedexcept_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=cascadedexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT cascadedexcept_r.dasm.il
    "$CORE_ROOT/ilasm" -output=cascadedexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT cascadedexcept_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=cascadedexcept_r
    if [ ! -f cascadedexcept_r.org ]; then
        TakeLock
        if [ ! -f cascadedexcept_r.org ]; then
          mkdir IL
          cp cascadedexcept_r.exe IL/cascadedexcept_r.exe
          mv cascadedexcept_r.exe cascadedexcept_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cascadedexcept_r.org /out cascadedexcept_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cascadedexcept_r.org /out cascadedexcept_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in cascadedexcept_r.reflect.xml
  if [ -f cascadedexcept_r.reflect.xml ]
then
    ReflectionRoots="-x cascadedexcept_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/cascadedexcept_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" cascadedexcept_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" cascadedexcept_r.asm.exe $CLRTestExecutionArguments
