...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/cascadedcatch_r cascadedcatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/cascadedcatch_r cascadedcatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=cascadedcatch_r.dasm.il cascadedcatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=cascadedcatch_r.dasm.il cascadedcatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=cascadedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT cascadedcatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=cascadedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT cascadedcatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=cascadedcatch_r
    if [ ! -f cascadedcatch_r.org ]; then
        TakeLock
        if [ ! -f cascadedcatch_r.org ]; then
          mkdir IL
          cp cascadedcatch_r.exe IL/cascadedcatch_r.exe
          mv cascadedcatch_r.exe cascadedcatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cascadedcatch_r.org /out cascadedcatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cascadedcatch_r.org /out cascadedcatch_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in cascadedcatch_r.reflect.xml
  if [ -f cascadedcatch_r.reflect.xml ]
then
    ReflectionRoots="-x cascadedcatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/cascadedcatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" cascadedcatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" cascadedcatch_r.asm.exe $CLRTestExecutionArguments
