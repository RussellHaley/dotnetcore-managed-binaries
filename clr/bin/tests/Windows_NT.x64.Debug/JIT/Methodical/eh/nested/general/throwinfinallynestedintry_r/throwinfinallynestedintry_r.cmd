...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_r.dasm.il throwinfinallynestedintry_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_r.dasm.il throwinfinallynestedintry_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_r.asm.exe  throwinfinallynestedintry_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_r.asm.exe  throwinfinallynestedintry_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinallynestedintry_r
    if not exist "throwinfinallynestedintry_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinallynestedintry_r.org" (
            mkdir IL
            copy throwinfinallynestedintry_r.exe IL\throwinfinallynestedintry_r.exe 
            ren throwinfinallynestedintry_r.exe throwinfinallynestedintry_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_r.org /out throwinfinallynestedintry_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_r.org /out throwinfinallynestedintry_r.exe
...
set ExePath=throwinfinallynestedintry_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinallynestedintry_r.reflect.xml
    IF EXIST throwinfinallynestedintry_r.reflect.xml set ReflectionRoots=-x throwinfinallynestedintry_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinallynestedintry_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinallynestedintry_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinallynestedintry_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
