...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_50_d.dasm.il throwinfinally_50_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_50_d.dasm.il throwinfinally_50_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinally_50_d.asm.exe  throwinfinally_50_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinally_50_d.asm.exe  throwinfinally_50_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinally_50_d
    if not exist "throwinfinally_50_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinally_50_d.org" (
            mkdir IL
            copy throwinfinally_50_d.exe IL\throwinfinally_50_d.exe 
            ren throwinfinally_50_d.exe throwinfinally_50_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_50_d.org /out throwinfinally_50_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_50_d.org /out throwinfinally_50_d.exe
...
set ExePath=throwinfinally_50_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinally_50_d.reflect.xml
    IF EXIST throwinfinally_50_d.reflect.xml set ReflectionRoots=-x throwinfinally_50_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinally_50_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinally_50_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinally_50_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
