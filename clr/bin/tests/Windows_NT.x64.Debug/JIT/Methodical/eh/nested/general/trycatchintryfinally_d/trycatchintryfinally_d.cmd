...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchintryfinally_d.dasm.il trycatchintryfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchintryfinally_d.dasm.il trycatchintryfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchintryfinally_d.asm.exe  trycatchintryfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchintryfinally_d.asm.exe  trycatchintryfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchintryfinally_d
    if not exist "trycatchintryfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchintryfinally_d.org" (
            mkdir IL
            copy trycatchintryfinally_d.exe IL\trycatchintryfinally_d.exe 
            ren trycatchintryfinally_d.exe trycatchintryfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchintryfinally_d.org /out trycatchintryfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchintryfinally_d.org /out trycatchintryfinally_d.exe
...
set ExePath=trycatchintryfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchintryfinally_d.reflect.xml
    IF EXIST trycatchintryfinally_d.reflect.xml set ReflectionRoots=-x trycatchintryfinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchintryfinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchintryfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchintryfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
