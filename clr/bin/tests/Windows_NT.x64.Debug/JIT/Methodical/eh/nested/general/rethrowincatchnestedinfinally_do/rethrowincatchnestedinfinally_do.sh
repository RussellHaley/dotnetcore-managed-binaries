...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/rethrowincatchnestedinfinally_do rethrowincatchnestedinfinally_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/rethrowincatchnestedinfinally_do rethrowincatchnestedinfinally_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rethrowincatchnestedinfinally_do.dasm.il rethrowincatchnestedinfinally_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=rethrowincatchnestedinfinally_do.dasm.il rethrowincatchnestedinfinally_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rethrowincatchnestedinfinally_do.asm.exe  rethrowincatchnestedinfinally_do.dasm.il
    "$CORE_ROOT/ilasm" -output=rethrowincatchnestedinfinally_do.asm.exe  rethrowincatchnestedinfinally_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rethrowincatchnestedinfinally_do
    if [ ! -f rethrowincatchnestedinfinally_do.org ]; then
        TakeLock
        if [ ! -f rethrowincatchnestedinfinally_do.org ]; then
          mkdir IL
          cp rethrowincatchnestedinfinally_do.exe IL/rethrowincatchnestedinfinally_do.exe
          mv rethrowincatchnestedinfinally_do.exe rethrowincatchnestedinfinally_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrowincatchnestedinfinally_do.org /out rethrowincatchnestedinfinally_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrowincatchnestedinfinally_do.org /out rethrowincatchnestedinfinally_do.exe
          __cgExitCode=$?
...
ExePath=rethrowincatchnestedinfinally_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in rethrowincatchnestedinfinally_do.reflect.xml
  if [ -f rethrowincatchnestedinfinally_do.reflect.xml ]
then
    ReflectionRoots="-x rethrowincatchnestedinfinally_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/rethrowincatchnestedinfinally_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rethrowincatchnestedinfinally_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rethrowincatchnestedinfinally_do.asm.exe $CLRTestExecutionArguments
