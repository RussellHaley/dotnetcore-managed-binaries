...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedcatchnofin_d throwincascadedcatchnofin_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedcatchnofin_d throwincascadedcatchnofin_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincascadedcatchnofin_d.dasm.il throwincascadedcatchnofin_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincascadedcatchnofin_d.dasm.il throwincascadedcatchnofin_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincascadedcatchnofin_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedcatchnofin_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincascadedcatchnofin_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedcatchnofin_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincascadedcatchnofin_d
    if [ ! -f throwincascadedcatchnofin_d.org ]; then
        TakeLock
        if [ ! -f throwincascadedcatchnofin_d.org ]; then
          mkdir IL
          cp throwincascadedcatchnofin_d.exe IL/throwincascadedcatchnofin_d.exe
          mv throwincascadedcatchnofin_d.exe throwincascadedcatchnofin_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedcatchnofin_d.org /out throwincascadedcatchnofin_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedcatchnofin_d.org /out throwincascadedcatchnofin_d.exe
          __cgExitCode=$?
...
ExePath=throwincascadedcatchnofin_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwincascadedcatchnofin_d.reflect.xml
  if [ -f throwincascadedcatchnofin_d.reflect.xml ]
then
    ReflectionRoots="-x throwincascadedcatchnofin_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincascadedcatchnofin_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincascadedcatchnofin_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincascadedcatchnofin_d.asm.exe $CLRTestExecutionArguments
