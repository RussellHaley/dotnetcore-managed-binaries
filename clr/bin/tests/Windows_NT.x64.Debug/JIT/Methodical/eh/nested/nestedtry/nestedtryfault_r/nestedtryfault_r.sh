...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/nestedtryfault_r nestedtryfault_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/nestedtryfault_r nestedtryfault_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nestedtryfault_r.dasm.il nestedtryfault_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=nestedtryfault_r.dasm.il nestedtryfault_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nestedtryfault_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedtryfault_r.dasm.il
    "$CORE_ROOT/ilasm" -output=nestedtryfault_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedtryfault_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nestedtryfault_r
    if [ ! -f nestedtryfault_r.org ]; then
        TakeLock
        if [ ! -f nestedtryfault_r.org ]; then
          mkdir IL
          cp nestedtryfault_r.exe IL/nestedtryfault_r.exe
          mv nestedtryfault_r.exe nestedtryfault_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedtryfault_r.org /out nestedtryfault_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedtryfault_r.org /out nestedtryfault_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in nestedtryfault_r.reflect.xml
  if [ -f nestedtryfault_r.reflect.xml ]
then
    ReflectionRoots="-x nestedtryfault_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nestedtryfault_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nestedtryfault_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nestedtryfault_r.asm.exe $CLRTestExecutionArguments
