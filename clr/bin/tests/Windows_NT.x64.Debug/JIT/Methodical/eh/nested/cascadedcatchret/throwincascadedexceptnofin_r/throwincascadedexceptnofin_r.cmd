...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedexceptnofin_r.dasm.il throwincascadedexceptnofin_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedexceptnofin_r.dasm.il throwincascadedexceptnofin_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincascadedexceptnofin_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedexceptnofin_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincascadedexceptnofin_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedexceptnofin_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincascadedexceptnofin_r
    if not exist "throwincascadedexceptnofin_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincascadedexceptnofin_r.org" (
            mkdir IL
            copy throwincascadedexceptnofin_r.exe IL\throwincascadedexceptnofin_r.exe 
            ren throwincascadedexceptnofin_r.exe throwincascadedexceptnofin_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedexceptnofin_r.org /out throwincascadedexceptnofin_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedexceptnofin_r.org /out throwincascadedexceptnofin_r.exe
...
set ExePath=throwincascadedexceptnofin_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincascadedexceptnofin_r.reflect.xml
    IF EXIST throwincascadedexceptnofin_r.reflect.xml set ReflectionRoots=-x throwincascadedexceptnofin_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincascadedexceptnofin_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincascadedexceptnofin_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincascadedexceptnofin_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
