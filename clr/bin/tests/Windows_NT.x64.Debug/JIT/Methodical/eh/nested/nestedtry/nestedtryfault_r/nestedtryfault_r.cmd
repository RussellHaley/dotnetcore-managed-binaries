...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nestedtryfault_r.dasm.il nestedtryfault_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nestedtryfault_r.dasm.il nestedtryfault_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nestedtryfault_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedtryfault_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nestedtryfault_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedtryfault_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nestedtryfault_r
    if not exist "nestedtryfault_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nestedtryfault_r.org" (
            mkdir IL
            copy nestedtryfault_r.exe IL\nestedtryfault_r.exe 
            ren nestedtryfault_r.exe nestedtryfault_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedtryfault_r.org /out nestedtryfault_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedtryfault_r.org /out nestedtryfault_r.exe
...
set ExePath=nestedtryfault_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nestedtryfault_r.reflect.xml
    IF EXIST nestedtryfault_r.reflect.xml set ReflectionRoots=-x nestedtryfault_r.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nestedtryfault_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nestedtryfault_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
