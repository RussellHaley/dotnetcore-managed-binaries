...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/localvarincatch_d localvarincatch_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/localvarincatch_d localvarincatch_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=localvarincatch_d.dasm.il localvarincatch_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=localvarincatch_d.dasm.il localvarincatch_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=localvarincatch_d.asm.exe -QUIET -NOLOGO -DEBUG localvarincatch_d.dasm.il
    "$CORE_ROOT/ilasm" -output=localvarincatch_d.asm.exe -QUIET -NOLOGO -DEBUG localvarincatch_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=localvarincatch_d
    if [ ! -f localvarincatch_d.org ]; then
        TakeLock
        if [ ! -f localvarincatch_d.org ]; then
          mkdir IL
          cp localvarincatch_d.exe IL/localvarincatch_d.exe
          mv localvarincatch_d.exe localvarincatch_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in localvarincatch_d.org /out localvarincatch_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in localvarincatch_d.org /out localvarincatch_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in localvarincatch_d.reflect.xml
  if [ -f localvarincatch_d.reflect.xml ]
then
    ReflectionRoots="-x localvarincatch_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/localvarincatch_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" localvarincatch_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" localvarincatch_d.asm.exe $CLRTestExecutionArguments
