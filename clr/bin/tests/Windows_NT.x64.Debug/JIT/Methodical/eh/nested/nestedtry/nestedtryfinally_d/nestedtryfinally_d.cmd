...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nestedtryfinally_d.dasm.il nestedtryfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nestedtryfinally_d.dasm.il nestedtryfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nestedtryfinally_d.asm.exe -QUIET -NOLOGO -DEBUG nestedtryfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nestedtryfinally_d.asm.exe -QUIET -NOLOGO -DEBUG nestedtryfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nestedtryfinally_d
    if not exist "nestedtryfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nestedtryfinally_d.org" (
            mkdir IL
            copy nestedtryfinally_d.exe IL\nestedtryfinally_d.exe 
            ren nestedtryfinally_d.exe nestedtryfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedtryfinally_d.org /out nestedtryfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedtryfinally_d.org /out nestedtryfinally_d.exe
...
set ExePath=nestedtryfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nestedtryfinally_d.reflect.xml
    IF EXIST nestedtryfinally_d.reflect.xml set ReflectionRoots=-x nestedtryfinally_d.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nestedtryfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nestedtryfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
