...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedfinally_r throwinnestedfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedfinally_r throwinnestedfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinnestedfinally_r.dasm.il throwinnestedfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinnestedfinally_r.dasm.il throwinnestedfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinnestedfinally_r.asm.exe  throwinnestedfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinnestedfinally_r.asm.exe  throwinnestedfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinnestedfinally_r
    if [ ! -f throwinnestedfinally_r.org ]; then
        TakeLock
        if [ ! -f throwinnestedfinally_r.org ]; then
          mkdir IL
          cp throwinnestedfinally_r.exe IL/throwinnestedfinally_r.exe
          mv throwinnestedfinally_r.exe throwinnestedfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedfinally_r.org /out throwinnestedfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedfinally_r.org /out throwinnestedfinally_r.exe
          __cgExitCode=$?
...
ExePath=throwinnestedfinally_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinnestedfinally_r.reflect.xml
  if [ -f throwinnestedfinally_r.reflect.xml ]
then
    ReflectionRoots="-x throwinnestedfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinnestedfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinnestedfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinnestedfinally_r.asm.exe $CLRTestExecutionArguments
