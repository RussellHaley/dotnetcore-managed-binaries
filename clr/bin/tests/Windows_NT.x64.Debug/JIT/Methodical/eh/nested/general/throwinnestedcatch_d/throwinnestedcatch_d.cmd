...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedcatch_d.dasm.il throwinnestedcatch_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedcatch_d.dasm.il throwinnestedcatch_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinnestedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedcatch_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinnestedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedcatch_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinnestedcatch_d
    if not exist "throwinnestedcatch_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinnestedcatch_d.org" (
            mkdir IL
            copy throwinnestedcatch_d.exe IL\throwinnestedcatch_d.exe 
            ren throwinnestedcatch_d.exe throwinnestedcatch_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedcatch_d.org /out throwinnestedcatch_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedcatch_d.org /out throwinnestedcatch_d.exe
...
set ExePath=throwinnestedcatch_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinnestedcatch_d.reflect.xml
    IF EXIST throwinnestedcatch_d.reflect.xml set ReflectionRoots=-x throwinnestedcatch_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinnestedcatch_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinnestedcatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinnestedcatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
