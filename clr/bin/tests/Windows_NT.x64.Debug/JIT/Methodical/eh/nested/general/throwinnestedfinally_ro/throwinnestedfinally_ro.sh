...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedfinally_ro throwinnestedfinally_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedfinally_ro throwinnestedfinally_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinnestedfinally_ro.dasm.il throwinnestedfinally_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinnestedfinally_ro.dasm.il throwinnestedfinally_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinnestedfinally_ro.asm.exe  throwinnestedfinally_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinnestedfinally_ro.asm.exe  throwinnestedfinally_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinnestedfinally_ro
    if [ ! -f throwinnestedfinally_ro.org ]; then
        TakeLock
        if [ ! -f throwinnestedfinally_ro.org ]; then
          mkdir IL
          cp throwinnestedfinally_ro.exe IL/throwinnestedfinally_ro.exe
          mv throwinnestedfinally_ro.exe throwinnestedfinally_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedfinally_ro.org /out throwinnestedfinally_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedfinally_ro.org /out throwinnestedfinally_ro.exe
          __cgExitCode=$?
...
ExePath=throwinnestedfinally_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinnestedfinally_ro.reflect.xml
  if [ -f throwinnestedfinally_ro.reflect.xml ]
then
    ReflectionRoots="-x throwinnestedfinally_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinnestedfinally_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinnestedfinally_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinnestedfinally_ro.asm.exe $CLRTestExecutionArguments
