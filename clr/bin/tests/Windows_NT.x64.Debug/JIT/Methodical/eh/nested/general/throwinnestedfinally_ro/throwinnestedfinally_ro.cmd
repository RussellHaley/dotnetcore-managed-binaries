...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedfinally_ro.dasm.il throwinnestedfinally_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedfinally_ro.dasm.il throwinnestedfinally_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinnestedfinally_ro.asm.exe  throwinnestedfinally_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinnestedfinally_ro.asm.exe  throwinnestedfinally_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinnestedfinally_ro
    if not exist "throwinnestedfinally_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinnestedfinally_ro.org" (
            mkdir IL
            copy throwinnestedfinally_ro.exe IL\throwinnestedfinally_ro.exe 
            ren throwinnestedfinally_ro.exe throwinnestedfinally_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedfinally_ro.org /out throwinnestedfinally_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedfinally_ro.org /out throwinnestedfinally_ro.exe
...
set ExePath=throwinnestedfinally_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinnestedfinally_ro.reflect.xml
    IF EXIST throwinnestedfinally_ro.reflect.xml set ReflectionRoots=-x throwinnestedfinally_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinnestedfinally_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinnestedfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinnestedfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
