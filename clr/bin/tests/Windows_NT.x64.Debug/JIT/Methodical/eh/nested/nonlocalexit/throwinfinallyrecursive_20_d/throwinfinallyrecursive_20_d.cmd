...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallyrecursive_20_d.dasm.il throwinfinallyrecursive_20_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallyrecursive_20_d.dasm.il throwinfinallyrecursive_20_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinallyrecursive_20_d.asm.exe  throwinfinallyrecursive_20_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinallyrecursive_20_d.asm.exe  throwinfinallyrecursive_20_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinallyrecursive_20_d
    if not exist "throwinfinallyrecursive_20_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinallyrecursive_20_d.org" (
            mkdir IL
            copy throwinfinallyrecursive_20_d.exe IL\throwinfinallyrecursive_20_d.exe 
            ren throwinfinallyrecursive_20_d.exe throwinfinallyrecursive_20_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallyrecursive_20_d.org /out throwinfinallyrecursive_20_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallyrecursive_20_d.org /out throwinfinallyrecursive_20_d.exe
...
set ExePath=throwinfinallyrecursive_20_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinallyrecursive_20_d.reflect.xml
    IF EXIST throwinfinallyrecursive_20_d.reflect.xml set ReflectionRoots=-x throwinfinallyrecursive_20_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinallyrecursive_20_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinallyrecursive_20_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinallyrecursive_20_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
