...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=rethrowincatchnestedinfinally_d.dasm.il rethrowincatchnestedinfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=rethrowincatchnestedinfinally_d.dasm.il rethrowincatchnestedinfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=rethrowincatchnestedinfinally_d.asm.exe  rethrowincatchnestedinfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=rethrowincatchnestedinfinally_d.asm.exe  rethrowincatchnestedinfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=rethrowincatchnestedinfinally_d
    if not exist "rethrowincatchnestedinfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "rethrowincatchnestedinfinally_d.org" (
            mkdir IL
            copy rethrowincatchnestedinfinally_d.exe IL\rethrowincatchnestedinfinally_d.exe 
            ren rethrowincatchnestedinfinally_d.exe rethrowincatchnestedinfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrowincatchnestedinfinally_d.org /out rethrowincatchnestedinfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrowincatchnestedinfinally_d.org /out rethrowincatchnestedinfinally_d.exe
...
set ExePath=rethrowincatchnestedinfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in rethrowincatchnestedinfinally_d.reflect.xml
    IF EXIST rethrowincatchnestedinfinally_d.reflect.xml set ReflectionRoots=-x rethrowincatchnestedinfinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\rethrowincatchnestedinfinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% rethrowincatchnestedinfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% rethrowincatchnestedinfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
