...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/nestedtryfinally_d nestedtryfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/nestedtryfinally_d nestedtryfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nestedtryfinally_d.dasm.il nestedtryfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=nestedtryfinally_d.dasm.il nestedtryfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nestedtryfinally_d.asm.exe -QUIET -NOLOGO -DEBUG nestedtryfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=nestedtryfinally_d.asm.exe -QUIET -NOLOGO -DEBUG nestedtryfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nestedtryfinally_d
    if [ ! -f nestedtryfinally_d.org ]; then
        TakeLock
        if [ ! -f nestedtryfinally_d.org ]; then
          mkdir IL
          cp nestedtryfinally_d.exe IL/nestedtryfinally_d.exe
          mv nestedtryfinally_d.exe nestedtryfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedtryfinally_d.org /out nestedtryfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedtryfinally_d.org /out nestedtryfinally_d.exe
          __cgExitCode=$?
...
ExePath=nestedtryfinally_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in nestedtryfinally_d.reflect.xml
  if [ -f nestedtryfinally_d.reflect.xml ]
then
    ReflectionRoots="-x nestedtryfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nestedtryfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nestedtryfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nestedtryfinally_d.asm.exe $CLRTestExecutionArguments
