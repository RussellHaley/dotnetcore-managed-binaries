...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedexcept_d throwincascadedexcept_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedexcept_d throwincascadedexcept_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincascadedexcept_d.dasm.il throwincascadedexcept_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincascadedexcept_d.dasm.il throwincascadedexcept_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincascadedexcept_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedexcept_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincascadedexcept_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedexcept_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincascadedexcept_d
    if [ ! -f throwincascadedexcept_d.org ]; then
        TakeLock
        if [ ! -f throwincascadedexcept_d.org ]; then
          mkdir IL
          cp throwincascadedexcept_d.exe IL/throwincascadedexcept_d.exe
          mv throwincascadedexcept_d.exe throwincascadedexcept_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedexcept_d.org /out throwincascadedexcept_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedexcept_d.org /out throwincascadedexcept_d.exe
          __cgExitCode=$?
...
ExePath=throwincascadedexcept_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwincascadedexcept_d.reflect.xml
  if [ -f throwincascadedexcept_d.reflect.xml ]
then
    ReflectionRoots="-x throwincascadedexcept_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincascadedexcept_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincascadedexcept_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincascadedexcept_d.asm.exe $CLRTestExecutionArguments
