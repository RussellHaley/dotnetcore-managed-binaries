...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=cascadedcatch_r.dasm.il cascadedcatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=cascadedcatch_r.dasm.il cascadedcatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=cascadedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT cascadedcatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=cascadedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT cascadedcatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=cascadedcatch_r
    if not exist "cascadedcatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "cascadedcatch_r.org" (
            mkdir IL
            copy cascadedcatch_r.exe IL\cascadedcatch_r.exe 
            ren cascadedcatch_r.exe cascadedcatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cascadedcatch_r.org /out cascadedcatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cascadedcatch_r.org /out cascadedcatch_r.exe
...
set ExePath=cascadedcatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in cascadedcatch_r.reflect.xml
    IF EXIST cascadedcatch_r.reflect.xml set ReflectionRoots=-x cascadedcatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\cascadedcatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% cascadedcatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% cascadedcatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
