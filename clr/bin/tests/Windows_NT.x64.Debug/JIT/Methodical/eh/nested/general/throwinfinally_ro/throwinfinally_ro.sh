...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinally_ro throwinfinally_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinally_ro throwinfinally_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_ro.dasm.il throwinfinally_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_ro.dasm.il throwinfinally_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinally_ro.asm.exe  throwinfinally_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinally_ro.asm.exe  throwinfinally_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinally_ro
    if [ ! -f throwinfinally_ro.org ]; then
        TakeLock
        if [ ! -f throwinfinally_ro.org ]; then
          mkdir IL
          cp throwinfinally_ro.exe IL/throwinfinally_ro.exe
          mv throwinfinally_ro.exe throwinfinally_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_ro.org /out throwinfinally_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_ro.org /out throwinfinally_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinally_ro.reflect.xml
  if [ -f throwinfinally_ro.reflect.xml ]
then
    ReflectionRoots="-x throwinfinally_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinally_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinally_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinally_ro.asm.exe $CLRTestExecutionArguments
