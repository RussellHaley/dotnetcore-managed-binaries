...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedfinally_d throwinnestedfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedfinally_d throwinnestedfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinnestedfinally_d.dasm.il throwinnestedfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinnestedfinally_d.dasm.il throwinnestedfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinnestedfinally_d.asm.exe  throwinnestedfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinnestedfinally_d.asm.exe  throwinnestedfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinnestedfinally_d
    if [ ! -f throwinnestedfinally_d.org ]; then
        TakeLock
        if [ ! -f throwinnestedfinally_d.org ]; then
          mkdir IL
          cp throwinnestedfinally_d.exe IL/throwinnestedfinally_d.exe
          mv throwinnestedfinally_d.exe throwinnestedfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedfinally_d.org /out throwinnestedfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedfinally_d.org /out throwinnestedfinally_d.exe
          __cgExitCode=$?
...
ExePath=throwinnestedfinally_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinnestedfinally_d.reflect.xml
  if [ -f throwinnestedfinally_d.reflect.xml ]
then
    ReflectionRoots="-x throwinnestedfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinnestedfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinnestedfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinnestedfinally_d.asm.exe $CLRTestExecutionArguments
