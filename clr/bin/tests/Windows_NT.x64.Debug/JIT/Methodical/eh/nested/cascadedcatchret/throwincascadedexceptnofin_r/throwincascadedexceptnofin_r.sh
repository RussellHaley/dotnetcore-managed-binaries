...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedexceptnofin_r throwincascadedexceptnofin_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedexceptnofin_r throwincascadedexceptnofin_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincascadedexceptnofin_r.dasm.il throwincascadedexceptnofin_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincascadedexceptnofin_r.dasm.il throwincascadedexceptnofin_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincascadedexceptnofin_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedexceptnofin_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincascadedexceptnofin_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedexceptnofin_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincascadedexceptnofin_r
    if [ ! -f throwincascadedexceptnofin_r.org ]; then
        TakeLock
        if [ ! -f throwincascadedexceptnofin_r.org ]; then
          mkdir IL
          cp throwincascadedexceptnofin_r.exe IL/throwincascadedexceptnofin_r.exe
          mv throwincascadedexceptnofin_r.exe throwincascadedexceptnofin_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedexceptnofin_r.org /out throwincascadedexceptnofin_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedexceptnofin_r.org /out throwincascadedexceptnofin_r.exe
          __cgExitCode=$?
...
ExePath=throwincascadedexceptnofin_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwincascadedexceptnofin_r.reflect.xml
  if [ -f throwincascadedexceptnofin_r.reflect.xml ]
then
    ReflectionRoots="-x throwincascadedexceptnofin_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincascadedexceptnofin_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincascadedexceptnofin_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincascadedexceptnofin_r.asm.exe $CLRTestExecutionArguments
