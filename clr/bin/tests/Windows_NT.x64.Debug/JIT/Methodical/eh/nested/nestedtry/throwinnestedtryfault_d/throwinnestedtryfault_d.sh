...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/throwinnestedtryfault_d throwinnestedtryfault_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/throwinnestedtryfault_d throwinnestedtryfault_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinnestedtryfault_d.dasm.il throwinnestedtryfault_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinnestedtryfault_d.dasm.il throwinnestedtryfault_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinnestedtryfault_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedtryfault_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinnestedtryfault_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedtryfault_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinnestedtryfault_d
    if [ ! -f throwinnestedtryfault_d.org ]; then
        TakeLock
        if [ ! -f throwinnestedtryfault_d.org ]; then
          mkdir IL
          cp throwinnestedtryfault_d.exe IL/throwinnestedtryfault_d.exe
          mv throwinnestedtryfault_d.exe throwinnestedtryfault_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedtryfault_d.org /out throwinnestedtryfault_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedtryfault_d.org /out throwinnestedtryfault_d.exe
          __cgExitCode=$?
...
ExePath=throwinnestedtryfault_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinnestedtryfault_d.reflect.xml
  if [ -f throwinnestedtryfault_d.reflect.xml ]
then
    ReflectionRoots="-x throwinnestedtryfault_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinnestedtryfault_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinnestedtryfault_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinnestedtryfault_d.asm.exe $CLRTestExecutionArguments
