...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedcatch_r throwinnestedcatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedcatch_r throwinnestedcatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinnestedcatch_r.dasm.il throwinnestedcatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinnestedcatch_r.dasm.il throwinnestedcatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinnestedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinnestedcatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinnestedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinnestedcatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinnestedcatch_r
    if [ ! -f throwinnestedcatch_r.org ]; then
        TakeLock
        if [ ! -f throwinnestedcatch_r.org ]; then
          mkdir IL
          cp throwinnestedcatch_r.exe IL/throwinnestedcatch_r.exe
          mv throwinnestedcatch_r.exe throwinnestedcatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedcatch_r.org /out throwinnestedcatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedcatch_r.org /out throwinnestedcatch_r.exe
          __cgExitCode=$?
...
ExePath=throwinnestedcatch_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinnestedcatch_r.reflect.xml
  if [ -f throwinnestedcatch_r.reflect.xml ]
then
    ReflectionRoots="-x throwinnestedcatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinnestedcatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinnestedcatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinnestedcatch_r.asm.exe $CLRTestExecutionArguments
