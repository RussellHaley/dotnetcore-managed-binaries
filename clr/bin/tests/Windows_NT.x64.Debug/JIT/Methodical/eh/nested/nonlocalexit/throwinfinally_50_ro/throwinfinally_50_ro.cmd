...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_50_ro.dasm.il throwinfinally_50_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_50_ro.dasm.il throwinfinally_50_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinally_50_ro.asm.exe  throwinfinally_50_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinally_50_ro.asm.exe  throwinfinally_50_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinally_50_ro
    if not exist "throwinfinally_50_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinally_50_ro.org" (
            mkdir IL
            copy throwinfinally_50_ro.exe IL\throwinfinally_50_ro.exe 
            ren throwinfinally_50_ro.exe throwinfinally_50_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_50_ro.org /out throwinfinally_50_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_50_ro.org /out throwinfinally_50_ro.exe
...
set ExePath=throwinfinally_50_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinally_50_ro.reflect.xml
    IF EXIST throwinfinally_50_ro.reflect.xml set ReflectionRoots=-x throwinfinally_50_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinally_50_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinally_50_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinally_50_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
