...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedcatch_d throwinnestedcatch_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedcatch_d throwinnestedcatch_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinnestedcatch_d.dasm.il throwinnestedcatch_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinnestedcatch_d.dasm.il throwinnestedcatch_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinnestedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedcatch_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinnestedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedcatch_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinnestedcatch_d
    if [ ! -f throwinnestedcatch_d.org ]; then
        TakeLock
        if [ ! -f throwinnestedcatch_d.org ]; then
          mkdir IL
          cp throwinnestedcatch_d.exe IL/throwinnestedcatch_d.exe
          mv throwinnestedcatch_d.exe throwinnestedcatch_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedcatch_d.org /out throwinnestedcatch_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedcatch_d.org /out throwinnestedcatch_d.exe
          __cgExitCode=$?
...
ExePath=throwinnestedcatch_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinnestedcatch_d.reflect.xml
  if [ -f throwinnestedcatch_d.reflect.xml ]
then
    ReflectionRoots="-x throwinnestedcatch_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinnestedcatch_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinnestedcatch_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinnestedcatch_d.asm.exe $CLRTestExecutionArguments
