...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_50_do.dasm.il throwinfinally_50_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_50_do.dasm.il throwinfinally_50_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinally_50_do.asm.exe  throwinfinally_50_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinally_50_do.asm.exe  throwinfinally_50_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinally_50_do
    if not exist "throwinfinally_50_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinally_50_do.org" (
            mkdir IL
            copy throwinfinally_50_do.exe IL\throwinfinally_50_do.exe 
            ren throwinfinally_50_do.exe throwinfinally_50_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_50_do.org /out throwinfinally_50_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_50_do.org /out throwinfinally_50_do.exe
...
set ExePath=throwinfinally_50_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinally_50_do.reflect.xml
    IF EXIST throwinfinally_50_do.reflect.xml set ReflectionRoots=-x throwinfinally_50_do.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinally_50_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinally_50_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinally_50_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
