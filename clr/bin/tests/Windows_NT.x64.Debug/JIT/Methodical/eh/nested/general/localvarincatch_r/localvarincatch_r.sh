...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/localvarincatch_r localvarincatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/localvarincatch_r localvarincatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=localvarincatch_r.dasm.il localvarincatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=localvarincatch_r.dasm.il localvarincatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=localvarincatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT localvarincatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=localvarincatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT localvarincatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=localvarincatch_r
    if [ ! -f localvarincatch_r.org ]; then
        TakeLock
        if [ ! -f localvarincatch_r.org ]; then
          mkdir IL
          cp localvarincatch_r.exe IL/localvarincatch_r.exe
          mv localvarincatch_r.exe localvarincatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in localvarincatch_r.org /out localvarincatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in localvarincatch_r.org /out localvarincatch_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in localvarincatch_r.reflect.xml
  if [ -f localvarincatch_r.reflect.xml ]
then
    ReflectionRoots="-x localvarincatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/localvarincatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" localvarincatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" localvarincatch_r.asm.exe $CLRTestExecutionArguments
