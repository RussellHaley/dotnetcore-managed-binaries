...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinallynestedintry_r throwinfinallynestedintry_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinallynestedintry_r throwinfinallynestedintry_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_r.dasm.il throwinfinallynestedintry_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_r.dasm.il throwinfinallynestedintry_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_r.asm.exe  throwinfinallynestedintry_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_r.asm.exe  throwinfinallynestedintry_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallynestedintry_r
    if [ ! -f throwinfinallynestedintry_r.org ]; then
        TakeLock
        if [ ! -f throwinfinallynestedintry_r.org ]; then
          mkdir IL
          cp throwinfinallynestedintry_r.exe IL/throwinfinallynestedintry_r.exe
          mv throwinfinallynestedintry_r.exe throwinfinallynestedintry_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_r.org /out throwinfinallynestedintry_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_r.org /out throwinfinallynestedintry_r.exe
          __cgExitCode=$?
...
ExePath=throwinfinallynestedintry_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallynestedintry_r.reflect.xml
  if [ -f throwinfinallynestedintry_r.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallynestedintry_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallynestedintry_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallynestedintry_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallynestedintry_r.asm.exe $CLRTestExecutionArguments
