...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=rethrowincatchnestedinfinally_r.dasm.il rethrowincatchnestedinfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=rethrowincatchnestedinfinally_r.dasm.il rethrowincatchnestedinfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=rethrowincatchnestedinfinally_r.asm.exe  rethrowincatchnestedinfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=rethrowincatchnestedinfinally_r.asm.exe  rethrowincatchnestedinfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=rethrowincatchnestedinfinally_r
    if not exist "rethrowincatchnestedinfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "rethrowincatchnestedinfinally_r.org" (
            mkdir IL
            copy rethrowincatchnestedinfinally_r.exe IL\rethrowincatchnestedinfinally_r.exe 
            ren rethrowincatchnestedinfinally_r.exe rethrowincatchnestedinfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrowincatchnestedinfinally_r.org /out rethrowincatchnestedinfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrowincatchnestedinfinally_r.org /out rethrowincatchnestedinfinally_r.exe
...
set ExePath=rethrowincatchnestedinfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in rethrowincatchnestedinfinally_r.reflect.xml
    IF EXIST rethrowincatchnestedinfinally_r.reflect.xml set ReflectionRoots=-x rethrowincatchnestedinfinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\rethrowincatchnestedinfinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% rethrowincatchnestedinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% rethrowincatchnestedinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
