...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedexceptnofin_d throwincascadedexceptnofin_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedexceptnofin_d throwincascadedexceptnofin_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincascadedexceptnofin_d.dasm.il throwincascadedexceptnofin_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincascadedexceptnofin_d.dasm.il throwincascadedexceptnofin_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincascadedexceptnofin_d.asm.exe -QUIET -NOLOGO -STACK=2000000 -DEBUG throwincascadedexceptnofin_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincascadedexceptnofin_d.asm.exe -QUIET -NOLOGO -STACK=2000000 -DEBUG throwincascadedexceptnofin_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincascadedexceptnofin_d
    if [ ! -f throwincascadedexceptnofin_d.org ]; then
        TakeLock
        if [ ! -f throwincascadedexceptnofin_d.org ]; then
          mkdir IL
          cp throwincascadedexceptnofin_d.exe IL/throwincascadedexceptnofin_d.exe
          mv throwincascadedexceptnofin_d.exe throwincascadedexceptnofin_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedexceptnofin_d.org /out throwincascadedexceptnofin_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedexceptnofin_d.org /out throwincascadedexceptnofin_d.exe
          __cgExitCode=$?
...
ExePath=throwincascadedexceptnofin_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwincascadedexceptnofin_d.reflect.xml
  if [ -f throwincascadedexceptnofin_d.reflect.xml ]
then
    ReflectionRoots="-x throwincascadedexceptnofin_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincascadedexceptnofin_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincascadedexceptnofin_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincascadedexceptnofin_d.asm.exe $CLRTestExecutionArguments
