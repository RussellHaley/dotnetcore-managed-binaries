...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedfinally_d.dasm.il throwinnestedfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedfinally_d.dasm.il throwinnestedfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinnestedfinally_d.asm.exe  throwinnestedfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinnestedfinally_d.asm.exe  throwinnestedfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinnestedfinally_d
    if not exist "throwinnestedfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinnestedfinally_d.org" (
            mkdir IL
            copy throwinnestedfinally_d.exe IL\throwinnestedfinally_d.exe 
            ren throwinnestedfinally_d.exe throwinnestedfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedfinally_d.org /out throwinnestedfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedfinally_d.org /out throwinnestedfinally_d.exe
...
set ExePath=throwinnestedfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinnestedfinally_d.reflect.xml
    IF EXIST throwinnestedfinally_d.reflect.xml set ReflectionRoots=-x throwinnestedfinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinnestedfinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinnestedfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinnestedfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
