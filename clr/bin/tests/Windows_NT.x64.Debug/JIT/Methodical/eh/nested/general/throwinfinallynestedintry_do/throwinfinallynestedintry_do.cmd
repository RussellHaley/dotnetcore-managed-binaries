...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_do.dasm.il throwinfinallynestedintry_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_do.dasm.il throwinfinallynestedintry_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_do.asm.exe  throwinfinallynestedintry_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_do.asm.exe  throwinfinallynestedintry_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinallynestedintry_do
    if not exist "throwinfinallynestedintry_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinallynestedintry_do.org" (
            mkdir IL
            copy throwinfinallynestedintry_do.exe IL\throwinfinallynestedintry_do.exe 
            ren throwinfinallynestedintry_do.exe throwinfinallynestedintry_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_do.org /out throwinfinallynestedintry_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_do.org /out throwinfinallynestedintry_do.exe
...
set ExePath=throwinfinallynestedintry_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinallynestedintry_do.reflect.xml
    IF EXIST throwinfinallynestedintry_do.reflect.xml set ReflectionRoots=-x throwinfinallynestedintry_do.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinallynestedintry_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinallynestedintry_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinallynestedintry_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
