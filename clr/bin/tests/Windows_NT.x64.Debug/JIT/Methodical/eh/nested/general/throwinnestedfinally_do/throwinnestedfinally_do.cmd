...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedfinally_do.dasm.il throwinnestedfinally_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedfinally_do.dasm.il throwinnestedfinally_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinnestedfinally_do.asm.exe  throwinnestedfinally_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinnestedfinally_do.asm.exe  throwinnestedfinally_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinnestedfinally_do
    if not exist "throwinnestedfinally_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinnestedfinally_do.org" (
            mkdir IL
            copy throwinnestedfinally_do.exe IL\throwinnestedfinally_do.exe 
            ren throwinnestedfinally_do.exe throwinnestedfinally_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedfinally_do.org /out throwinnestedfinally_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedfinally_do.org /out throwinnestedfinally_do.exe
...
set ExePath=throwinnestedfinally_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinnestedfinally_do.reflect.xml
    IF EXIST throwinnestedfinally_do.reflect.xml set ReflectionRoots=-x throwinnestedfinally_do.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinnestedfinally_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinnestedfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinnestedfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
