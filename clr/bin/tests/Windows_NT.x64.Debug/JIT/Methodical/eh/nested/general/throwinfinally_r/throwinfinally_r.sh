...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinally_r throwinfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinally_r throwinfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_r.dasm.il throwinfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_r.dasm.il throwinfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinally_r.asm.exe  throwinfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinally_r.asm.exe  throwinfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinally_r
    if [ ! -f throwinfinally_r.org ]; then
        TakeLock
        if [ ! -f throwinfinally_r.org ]; then
          mkdir IL
          cp throwinfinally_r.exe IL/throwinfinally_r.exe
          mv throwinfinally_r.exe throwinfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_r.org /out throwinfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_r.org /out throwinfinally_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinally_r.reflect.xml
  if [ -f throwinfinally_r.reflect.xml ]
then
    ReflectionRoots="-x throwinfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinally_r.asm.exe $CLRTestExecutionArguments
