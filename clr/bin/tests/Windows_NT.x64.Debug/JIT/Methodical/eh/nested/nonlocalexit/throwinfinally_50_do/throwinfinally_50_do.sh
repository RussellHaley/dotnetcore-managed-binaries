...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinally_50_do throwinfinally_50_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinally_50_do throwinfinally_50_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_50_do.dasm.il throwinfinally_50_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_50_do.dasm.il throwinfinally_50_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinally_50_do.asm.exe  throwinfinally_50_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinally_50_do.asm.exe  throwinfinally_50_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinally_50_do
    if [ ! -f throwinfinally_50_do.org ]; then
        TakeLock
        if [ ! -f throwinfinally_50_do.org ]; then
          mkdir IL
          cp throwinfinally_50_do.exe IL/throwinfinally_50_do.exe
          mv throwinfinally_50_do.exe throwinfinally_50_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_50_do.org /out throwinfinally_50_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_50_do.org /out throwinfinally_50_do.exe
          __cgExitCode=$?
...
ExePath=throwinfinally_50_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinally_50_do.reflect.xml
  if [ -f throwinfinally_50_do.reflect.xml ]
then
    ReflectionRoots="-x throwinfinally_50_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinally_50_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinally_50_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinally_50_do.asm.exe $CLRTestExecutionArguments
