...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=localvarincatch_r.dasm.il localvarincatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=localvarincatch_r.dasm.il localvarincatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=localvarincatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT localvarincatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=localvarincatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT localvarincatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=localvarincatch_r
    if not exist "localvarincatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "localvarincatch_r.org" (
            mkdir IL
            copy localvarincatch_r.exe IL\localvarincatch_r.exe 
            ren localvarincatch_r.exe localvarincatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in localvarincatch_r.org /out localvarincatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in localvarincatch_r.org /out localvarincatch_r.exe
...
set ExePath=localvarincatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in localvarincatch_r.reflect.xml
    IF EXIST localvarincatch_r.reflect.xml set ReflectionRoots=-x localvarincatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\localvarincatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% localvarincatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% localvarincatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
