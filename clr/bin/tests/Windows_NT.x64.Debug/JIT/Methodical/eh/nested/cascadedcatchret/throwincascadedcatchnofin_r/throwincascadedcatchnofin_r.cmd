...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedcatchnofin_r.dasm.il throwincascadedcatchnofin_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedcatchnofin_r.dasm.il throwincascadedcatchnofin_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincascadedcatchnofin_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedcatchnofin_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincascadedcatchnofin_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedcatchnofin_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincascadedcatchnofin_r
    if not exist "throwincascadedcatchnofin_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincascadedcatchnofin_r.org" (
            mkdir IL
            copy throwincascadedcatchnofin_r.exe IL\throwincascadedcatchnofin_r.exe 
            ren throwincascadedcatchnofin_r.exe throwincascadedcatchnofin_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedcatchnofin_r.org /out throwincascadedcatchnofin_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedcatchnofin_r.org /out throwincascadedcatchnofin_r.exe
...
set ExePath=throwincascadedcatchnofin_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincascadedcatchnofin_r.reflect.xml
    IF EXIST throwincascadedcatchnofin_r.reflect.xml set ReflectionRoots=-x throwincascadedcatchnofin_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincascadedcatchnofin_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincascadedcatchnofin_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincascadedcatchnofin_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
