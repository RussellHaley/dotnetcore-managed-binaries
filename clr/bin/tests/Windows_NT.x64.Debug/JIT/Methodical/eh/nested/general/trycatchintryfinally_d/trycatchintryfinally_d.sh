...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/trycatchintryfinally_d trycatchintryfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/trycatchintryfinally_d trycatchintryfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchintryfinally_d.dasm.il trycatchintryfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchintryfinally_d.dasm.il trycatchintryfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchintryfinally_d.asm.exe  trycatchintryfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchintryfinally_d.asm.exe  trycatchintryfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchintryfinally_d
    if [ ! -f trycatchintryfinally_d.org ]; then
        TakeLock
        if [ ! -f trycatchintryfinally_d.org ]; then
          mkdir IL
          cp trycatchintryfinally_d.exe IL/trycatchintryfinally_d.exe
          mv trycatchintryfinally_d.exe trycatchintryfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchintryfinally_d.org /out trycatchintryfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchintryfinally_d.org /out trycatchintryfinally_d.exe
          __cgExitCode=$?
...
ExePath=trycatchintryfinally_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchintryfinally_d.reflect.xml
  if [ -f trycatchintryfinally_d.reflect.xml ]
then
    ReflectionRoots="-x trycatchintryfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchintryfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchintryfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchintryfinally_d.asm.exe $CLRTestExecutionArguments
