...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedcatchnofin_d.dasm.il throwincascadedcatchnofin_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedcatchnofin_d.dasm.il throwincascadedcatchnofin_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincascadedcatchnofin_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedcatchnofin_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincascadedcatchnofin_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedcatchnofin_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincascadedcatchnofin_d
    if not exist "throwincascadedcatchnofin_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincascadedcatchnofin_d.org" (
            mkdir IL
            copy throwincascadedcatchnofin_d.exe IL\throwincascadedcatchnofin_d.exe 
            ren throwincascadedcatchnofin_d.exe throwincascadedcatchnofin_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedcatchnofin_d.org /out throwincascadedcatchnofin_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedcatchnofin_d.org /out throwincascadedcatchnofin_d.exe
...
set ExePath=throwincascadedcatchnofin_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincascadedcatchnofin_d.reflect.xml
    IF EXIST throwincascadedcatchnofin_d.reflect.xml set ReflectionRoots=-x throwincascadedcatchnofin_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincascadedcatchnofin_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincascadedcatchnofin_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincascadedcatchnofin_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
