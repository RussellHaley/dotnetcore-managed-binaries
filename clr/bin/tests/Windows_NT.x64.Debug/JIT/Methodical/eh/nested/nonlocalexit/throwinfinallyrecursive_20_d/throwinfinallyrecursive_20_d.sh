...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallyrecursive_20_d throwinfinallyrecursive_20_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallyrecursive_20_d throwinfinallyrecursive_20_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyrecursive_20_d.dasm.il throwinfinallyrecursive_20_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyrecursive_20_d.dasm.il throwinfinallyrecursive_20_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallyrecursive_20_d.asm.exe  throwinfinallyrecursive_20_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallyrecursive_20_d.asm.exe  throwinfinallyrecursive_20_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallyrecursive_20_d
    if [ ! -f throwinfinallyrecursive_20_d.org ]; then
        TakeLock
        if [ ! -f throwinfinallyrecursive_20_d.org ]; then
          mkdir IL
          cp throwinfinallyrecursive_20_d.exe IL/throwinfinallyrecursive_20_d.exe
          mv throwinfinallyrecursive_20_d.exe throwinfinallyrecursive_20_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyrecursive_20_d.org /out throwinfinallyrecursive_20_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyrecursive_20_d.org /out throwinfinallyrecursive_20_d.exe
          __cgExitCode=$?
...
ExePath=throwinfinallyrecursive_20_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallyrecursive_20_d.reflect.xml
  if [ -f throwinfinallyrecursive_20_d.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallyrecursive_20_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallyrecursive_20_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallyrecursive_20_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallyrecursive_20_d.asm.exe $CLRTestExecutionArguments
