...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/trycatchintryfinally_r trycatchintryfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/trycatchintryfinally_r trycatchintryfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchintryfinally_r.dasm.il trycatchintryfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchintryfinally_r.dasm.il trycatchintryfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchintryfinally_r.asm.exe  trycatchintryfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchintryfinally_r.asm.exe  trycatchintryfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchintryfinally_r
    if [ ! -f trycatchintryfinally_r.org ]; then
        TakeLock
        if [ ! -f trycatchintryfinally_r.org ]; then
          mkdir IL
          cp trycatchintryfinally_r.exe IL/trycatchintryfinally_r.exe
          mv trycatchintryfinally_r.exe trycatchintryfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchintryfinally_r.org /out trycatchintryfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchintryfinally_r.org /out trycatchintryfinally_r.exe
          __cgExitCode=$?
...
ExePath=trycatchintryfinally_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchintryfinally_r.reflect.xml
  if [ -f trycatchintryfinally_r.reflect.xml ]
then
    ReflectionRoots="-x trycatchintryfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchintryfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchintryfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchintryfinally_r.asm.exe $CLRTestExecutionArguments
