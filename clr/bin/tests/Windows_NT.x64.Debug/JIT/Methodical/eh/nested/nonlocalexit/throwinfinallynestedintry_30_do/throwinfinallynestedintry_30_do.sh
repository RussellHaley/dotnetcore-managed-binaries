...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallynestedintry_30_do throwinfinallynestedintry_30_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallynestedintry_30_do throwinfinallynestedintry_30_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_30_do.dasm.il throwinfinallynestedintry_30_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_30_do.dasm.il throwinfinallynestedintry_30_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_30_do.asm.exe  throwinfinallynestedintry_30_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_30_do.asm.exe  throwinfinallynestedintry_30_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallynestedintry_30_do
    if [ ! -f throwinfinallynestedintry_30_do.org ]; then
        TakeLock
        if [ ! -f throwinfinallynestedintry_30_do.org ]; then
          mkdir IL
          cp throwinfinallynestedintry_30_do.exe IL/throwinfinallynestedintry_30_do.exe
          mv throwinfinallynestedintry_30_do.exe throwinfinallynestedintry_30_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_30_do.org /out throwinfinallynestedintry_30_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_30_do.org /out throwinfinallynestedintry_30_do.exe
          __cgExitCode=$?
...
ExePath=throwinfinallynestedintry_30_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallynestedintry_30_do.reflect.xml
  if [ -f throwinfinallynestedintry_30_do.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallynestedintry_30_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallynestedintry_30_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallynestedintry_30_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallynestedintry_30_do.asm.exe $CLRTestExecutionArguments
