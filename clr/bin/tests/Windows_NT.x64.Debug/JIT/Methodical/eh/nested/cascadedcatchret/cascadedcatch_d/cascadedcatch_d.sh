...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/cascadedcatch_d cascadedcatch_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/cascadedcatch_d cascadedcatch_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=cascadedcatch_d.dasm.il cascadedcatch_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=cascadedcatch_d.dasm.il cascadedcatch_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=cascadedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG cascadedcatch_d.dasm.il
    "$CORE_ROOT/ilasm" -output=cascadedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG cascadedcatch_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=cascadedcatch_d
    if [ ! -f cascadedcatch_d.org ]; then
        TakeLock
        if [ ! -f cascadedcatch_d.org ]; then
          mkdir IL
          cp cascadedcatch_d.exe IL/cascadedcatch_d.exe
          mv cascadedcatch_d.exe cascadedcatch_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cascadedcatch_d.org /out cascadedcatch_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cascadedcatch_d.org /out cascadedcatch_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in cascadedcatch_d.reflect.xml
  if [ -f cascadedcatch_d.reflect.xml ]
then
    ReflectionRoots="-x cascadedcatch_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/cascadedcatch_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" cascadedcatch_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" cascadedcatch_d.asm.exe $CLRTestExecutionArguments
