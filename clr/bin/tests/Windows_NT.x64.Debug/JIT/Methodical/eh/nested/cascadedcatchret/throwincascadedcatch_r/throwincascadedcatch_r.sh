...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedcatch_r throwincascadedcatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedcatch_r throwincascadedcatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincascadedcatch_r.dasm.il throwincascadedcatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincascadedcatch_r.dasm.il throwincascadedcatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincascadedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedcatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincascadedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedcatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincascadedcatch_r
    if [ ! -f throwincascadedcatch_r.org ]; then
        TakeLock
        if [ ! -f throwincascadedcatch_r.org ]; then
          mkdir IL
          cp throwincascadedcatch_r.exe IL/throwincascadedcatch_r.exe
          mv throwincascadedcatch_r.exe throwincascadedcatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedcatch_r.org /out throwincascadedcatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedcatch_r.org /out throwincascadedcatch_r.exe
          __cgExitCode=$?
...
ExePath=throwincascadedcatch_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwincascadedcatch_r.reflect.xml
  if [ -f throwincascadedcatch_r.reflect.xml ]
then
    ReflectionRoots="-x throwincascadedcatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincascadedcatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincascadedcatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincascadedcatch_r.asm.exe $CLRTestExecutionArguments
