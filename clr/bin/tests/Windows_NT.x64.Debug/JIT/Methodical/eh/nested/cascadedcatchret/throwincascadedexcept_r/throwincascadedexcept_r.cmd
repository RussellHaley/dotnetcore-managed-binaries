...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedexcept_r.dasm.il throwincascadedexcept_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedexcept_r.dasm.il throwincascadedexcept_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincascadedexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedexcept_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincascadedexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedexcept_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincascadedexcept_r
    if not exist "throwincascadedexcept_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincascadedexcept_r.org" (
            mkdir IL
            copy throwincascadedexcept_r.exe IL\throwincascadedexcept_r.exe 
            ren throwincascadedexcept_r.exe throwincascadedexcept_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedexcept_r.org /out throwincascadedexcept_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedexcept_r.org /out throwincascadedexcept_r.exe
...
set ExePath=throwincascadedexcept_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincascadedexcept_r.reflect.xml
    IF EXIST throwincascadedexcept_r.reflect.xml set ReflectionRoots=-x throwincascadedexcept_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincascadedexcept_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincascadedexcept_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincascadedexcept_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
