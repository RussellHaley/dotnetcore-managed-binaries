...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedexceptnofin_d.dasm.il throwincascadedexceptnofin_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedexceptnofin_d.dasm.il throwincascadedexceptnofin_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincascadedexceptnofin_d.asm.exe -QUIET -NOLOGO -STACK=2000000 -DEBUG throwincascadedexceptnofin_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincascadedexceptnofin_d.asm.exe -QUIET -NOLOGO -STACK=2000000 -DEBUG throwincascadedexceptnofin_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincascadedexceptnofin_d
    if not exist "throwincascadedexceptnofin_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincascadedexceptnofin_d.org" (
            mkdir IL
            copy throwincascadedexceptnofin_d.exe IL\throwincascadedexceptnofin_d.exe 
            ren throwincascadedexceptnofin_d.exe throwincascadedexceptnofin_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedexceptnofin_d.org /out throwincascadedexceptnofin_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedexceptnofin_d.org /out throwincascadedexceptnofin_d.exe
...
set ExePath=throwincascadedexceptnofin_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincascadedexceptnofin_d.reflect.xml
    IF EXIST throwincascadedexceptnofin_d.reflect.xml set ReflectionRoots=-x throwincascadedexceptnofin_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincascadedexceptnofin_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincascadedexceptnofin_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincascadedexceptnofin_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
