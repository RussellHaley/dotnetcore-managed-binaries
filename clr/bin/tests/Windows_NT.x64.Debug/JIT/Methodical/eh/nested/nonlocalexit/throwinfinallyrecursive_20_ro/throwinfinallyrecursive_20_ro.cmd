...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallyrecursive_20_ro.dasm.il throwinfinallyrecursive_20_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallyrecursive_20_ro.dasm.il throwinfinallyrecursive_20_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinallyrecursive_20_ro.asm.exe  throwinfinallyrecursive_20_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinallyrecursive_20_ro.asm.exe  throwinfinallyrecursive_20_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinallyrecursive_20_ro
    if not exist "throwinfinallyrecursive_20_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinallyrecursive_20_ro.org" (
            mkdir IL
            copy throwinfinallyrecursive_20_ro.exe IL\throwinfinallyrecursive_20_ro.exe 
            ren throwinfinallyrecursive_20_ro.exe throwinfinallyrecursive_20_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallyrecursive_20_ro.org /out throwinfinallyrecursive_20_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallyrecursive_20_ro.org /out throwinfinallyrecursive_20_ro.exe
...
set ExePath=throwinfinallyrecursive_20_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinallyrecursive_20_ro.reflect.xml
    IF EXIST throwinfinallyrecursive_20_ro.reflect.xml set ReflectionRoots=-x throwinfinallyrecursive_20_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinallyrecursive_20_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinallyrecursive_20_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinallyrecursive_20_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
