...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=localvarincatch_d.dasm.il localvarincatch_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=localvarincatch_d.dasm.il localvarincatch_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=localvarincatch_d.asm.exe -QUIET -NOLOGO -DEBUG localvarincatch_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=localvarincatch_d.asm.exe -QUIET -NOLOGO -DEBUG localvarincatch_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=localvarincatch_d
    if not exist "localvarincatch_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "localvarincatch_d.org" (
            mkdir IL
            copy localvarincatch_d.exe IL\localvarincatch_d.exe 
            ren localvarincatch_d.exe localvarincatch_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in localvarincatch_d.org /out localvarincatch_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in localvarincatch_d.org /out localvarincatch_d.exe
...
set ExePath=localvarincatch_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in localvarincatch_d.reflect.xml
    IF EXIST localvarincatch_d.reflect.xml set ReflectionRoots=-x localvarincatch_d.reflect.xml
    ...
    set ExePath=%LinkBin%\localvarincatch_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% localvarincatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% localvarincatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
