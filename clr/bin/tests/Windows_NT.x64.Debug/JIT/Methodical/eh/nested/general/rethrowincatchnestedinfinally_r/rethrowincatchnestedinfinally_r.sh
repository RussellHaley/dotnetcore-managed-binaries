...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/rethrowincatchnestedinfinally_r rethrowincatchnestedinfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/rethrowincatchnestedinfinally_r rethrowincatchnestedinfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rethrowincatchnestedinfinally_r.dasm.il rethrowincatchnestedinfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=rethrowincatchnestedinfinally_r.dasm.il rethrowincatchnestedinfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rethrowincatchnestedinfinally_r.asm.exe  rethrowincatchnestedinfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=rethrowincatchnestedinfinally_r.asm.exe  rethrowincatchnestedinfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rethrowincatchnestedinfinally_r
    if [ ! -f rethrowincatchnestedinfinally_r.org ]; then
        TakeLock
        if [ ! -f rethrowincatchnestedinfinally_r.org ]; then
          mkdir IL
          cp rethrowincatchnestedinfinally_r.exe IL/rethrowincatchnestedinfinally_r.exe
          mv rethrowincatchnestedinfinally_r.exe rethrowincatchnestedinfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrowincatchnestedinfinally_r.org /out rethrowincatchnestedinfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrowincatchnestedinfinally_r.org /out rethrowincatchnestedinfinally_r.exe
          __cgExitCode=$?
...
ExePath=rethrowincatchnestedinfinally_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in rethrowincatchnestedinfinally_r.reflect.xml
  if [ -f rethrowincatchnestedinfinally_r.reflect.xml ]
then
    ReflectionRoots="-x rethrowincatchnestedinfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/rethrowincatchnestedinfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rethrowincatchnestedinfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rethrowincatchnestedinfinally_r.asm.exe $CLRTestExecutionArguments
