...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinally_do throwinfinally_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinally_do throwinfinally_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_do.dasm.il throwinfinally_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_do.dasm.il throwinfinally_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinally_do.asm.exe  throwinfinally_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinally_do.asm.exe  throwinfinally_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinally_do
    if [ ! -f throwinfinally_do.org ]; then
        TakeLock
        if [ ! -f throwinfinally_do.org ]; then
          mkdir IL
          cp throwinfinally_do.exe IL/throwinfinally_do.exe
          mv throwinfinally_do.exe throwinfinally_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_do.org /out throwinfinally_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_do.org /out throwinfinally_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinally_do.reflect.xml
  if [ -f throwinfinally_do.reflect.xml ]
then
    ReflectionRoots="-x throwinfinally_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinally_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinally_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinally_do.asm.exe $CLRTestExecutionArguments
