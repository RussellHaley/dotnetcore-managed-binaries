...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedcatch_r.dasm.il throwinnestedcatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedcatch_r.dasm.il throwinnestedcatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinnestedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinnestedcatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinnestedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinnestedcatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinnestedcatch_r
    if not exist "throwinnestedcatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinnestedcatch_r.org" (
            mkdir IL
            copy throwinnestedcatch_r.exe IL\throwinnestedcatch_r.exe 
            ren throwinnestedcatch_r.exe throwinnestedcatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedcatch_r.org /out throwinnestedcatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedcatch_r.org /out throwinnestedcatch_r.exe
...
set ExePath=throwinnestedcatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinnestedcatch_r.reflect.xml
    IF EXIST throwinnestedcatch_r.reflect.xml set ReflectionRoots=-x throwinnestedcatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinnestedcatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinnestedcatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinnestedcatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
