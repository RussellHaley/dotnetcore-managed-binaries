...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/trycatchintryfinally_do trycatchintryfinally_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/trycatchintryfinally_do trycatchintryfinally_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchintryfinally_do.dasm.il trycatchintryfinally_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchintryfinally_do.dasm.il trycatchintryfinally_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchintryfinally_do.asm.exe  trycatchintryfinally_do.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchintryfinally_do.asm.exe  trycatchintryfinally_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchintryfinally_do
    if [ ! -f trycatchintryfinally_do.org ]; then
        TakeLock
        if [ ! -f trycatchintryfinally_do.org ]; then
          mkdir IL
          cp trycatchintryfinally_do.exe IL/trycatchintryfinally_do.exe
          mv trycatchintryfinally_do.exe trycatchintryfinally_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchintryfinally_do.org /out trycatchintryfinally_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchintryfinally_do.org /out trycatchintryfinally_do.exe
          __cgExitCode=$?
...
ExePath=trycatchintryfinally_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchintryfinally_do.reflect.xml
  if [ -f trycatchintryfinally_do.reflect.xml ]
then
    ReflectionRoots="-x trycatchintryfinally_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchintryfinally_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchintryfinally_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchintryfinally_do.asm.exe $CLRTestExecutionArguments
