...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinallynestedintry_d throwinfinallynestedintry_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinallynestedintry_d throwinfinallynestedintry_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_d.dasm.il throwinfinallynestedintry_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_d.dasm.il throwinfinallynestedintry_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_d.asm.exe  throwinfinallynestedintry_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_d.asm.exe  throwinfinallynestedintry_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallynestedintry_d
    if [ ! -f throwinfinallynestedintry_d.org ]; then
        TakeLock
        if [ ! -f throwinfinallynestedintry_d.org ]; then
          mkdir IL
          cp throwinfinallynestedintry_d.exe IL/throwinfinallynestedintry_d.exe
          mv throwinfinallynestedintry_d.exe throwinfinallynestedintry_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_d.org /out throwinfinallynestedintry_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_d.org /out throwinfinallynestedintry_d.exe
          __cgExitCode=$?
...
ExePath=throwinfinallynestedintry_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallynestedintry_d.reflect.xml
  if [ -f throwinfinallynestedintry_d.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallynestedintry_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallynestedintry_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallynestedintry_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallynestedintry_d.asm.exe $CLRTestExecutionArguments
