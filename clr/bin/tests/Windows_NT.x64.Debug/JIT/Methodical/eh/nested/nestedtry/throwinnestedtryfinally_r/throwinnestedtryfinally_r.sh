...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/throwinnestedtryfinally_r throwinnestedtryfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/throwinnestedtryfinally_r throwinnestedtryfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinnestedtryfinally_r.dasm.il throwinnestedtryfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinnestedtryfinally_r.dasm.il throwinnestedtryfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinnestedtryfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinnestedtryfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinnestedtryfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinnestedtryfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinnestedtryfinally_r
    if [ ! -f throwinnestedtryfinally_r.org ]; then
        TakeLock
        if [ ! -f throwinnestedtryfinally_r.org ]; then
          mkdir IL
          cp throwinnestedtryfinally_r.exe IL/throwinnestedtryfinally_r.exe
          mv throwinnestedtryfinally_r.exe throwinnestedtryfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedtryfinally_r.org /out throwinnestedtryfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedtryfinally_r.org /out throwinnestedtryfinally_r.exe
          __cgExitCode=$?
...
ExePath=throwinnestedtryfinally_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinnestedtryfinally_r.reflect.xml
  if [ -f throwinnestedtryfinally_r.reflect.xml ]
then
    ReflectionRoots="-x throwinnestedtryfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinnestedtryfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinnestedtryfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinnestedtryfinally_r.asm.exe $CLRTestExecutionArguments
