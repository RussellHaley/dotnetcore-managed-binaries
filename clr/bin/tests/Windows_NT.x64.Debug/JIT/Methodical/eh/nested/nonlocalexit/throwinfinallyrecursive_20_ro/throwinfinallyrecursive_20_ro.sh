...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallyrecursive_20_ro throwinfinallyrecursive_20_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallyrecursive_20_ro throwinfinallyrecursive_20_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyrecursive_20_ro.dasm.il throwinfinallyrecursive_20_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyrecursive_20_ro.dasm.il throwinfinallyrecursive_20_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallyrecursive_20_ro.asm.exe  throwinfinallyrecursive_20_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallyrecursive_20_ro.asm.exe  throwinfinallyrecursive_20_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallyrecursive_20_ro
    if [ ! -f throwinfinallyrecursive_20_ro.org ]; then
        TakeLock
        if [ ! -f throwinfinallyrecursive_20_ro.org ]; then
          mkdir IL
          cp throwinfinallyrecursive_20_ro.exe IL/throwinfinallyrecursive_20_ro.exe
          mv throwinfinallyrecursive_20_ro.exe throwinfinallyrecursive_20_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyrecursive_20_ro.org /out throwinfinallyrecursive_20_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyrecursive_20_ro.org /out throwinfinallyrecursive_20_ro.exe
          __cgExitCode=$?
...
ExePath=throwinfinallyrecursive_20_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallyrecursive_20_ro.reflect.xml
  if [ -f throwinfinallyrecursive_20_ro.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallyrecursive_20_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallyrecursive_20_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallyrecursive_20_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallyrecursive_20_ro.asm.exe $CLRTestExecutionArguments
