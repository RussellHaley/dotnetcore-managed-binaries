...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Methodical\eh\nested\nonlocalexit\throwinfinallynestedintry_30_do throwinfinallynestedintry_30_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\Methodical\eh\nested\nonlocalexit\throwinfinallynestedintry_30_do throwinfinallynestedintry_30_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_30_do.dasm.il throwinfinallynestedintry_30_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_30_do.dasm.il throwinfinallynestedintry_30_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_30_do.asm.exe  throwinfinallynestedintry_30_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_30_do.asm.exe  throwinfinallynestedintry_30_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinallynestedintry_30_do
    if not exist "throwinfinallynestedintry_30_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinallynestedintry_30_do.org" (
            mkdir IL
            copy throwinfinallynestedintry_30_do.exe IL\throwinfinallynestedintry_30_do.exe 
            ren throwinfinallynestedintry_30_do.exe throwinfinallynestedintry_30_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_30_do.org /out throwinfinallynestedintry_30_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_30_do.org /out throwinfinallynestedintry_30_do.exe
...
set ExePath=throwinfinallynestedintry_30_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinallynestedintry_30_do.reflect.xml
    IF EXIST throwinfinallynestedintry_30_do.reflect.xml set ReflectionRoots=-x throwinfinallynestedintry_30_do.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinallynestedintry_30_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinallynestedintry_30_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinallynestedintry_30_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
