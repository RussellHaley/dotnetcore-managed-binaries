...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedtryfinally_d.dasm.il throwinnestedtryfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedtryfinally_d.dasm.il throwinnestedtryfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinnestedtryfinally_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedtryfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinnestedtryfinally_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedtryfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinnestedtryfinally_d
    if not exist "throwinnestedtryfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinnestedtryfinally_d.org" (
            mkdir IL
            copy throwinnestedtryfinally_d.exe IL\throwinnestedtryfinally_d.exe 
            ren throwinnestedtryfinally_d.exe throwinnestedtryfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedtryfinally_d.org /out throwinnestedtryfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedtryfinally_d.org /out throwinnestedtryfinally_d.exe
...
set ExePath=throwinnestedtryfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinnestedtryfinally_d.reflect.xml
    IF EXIST throwinnestedtryfinally_d.reflect.xml set ReflectionRoots=-x throwinnestedtryfinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinnestedtryfinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinnestedtryfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinnestedtryfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
