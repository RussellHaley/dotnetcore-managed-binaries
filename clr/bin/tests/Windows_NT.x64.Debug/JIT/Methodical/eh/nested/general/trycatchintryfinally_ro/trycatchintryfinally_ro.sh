...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/trycatchintryfinally_ro trycatchintryfinally_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/trycatchintryfinally_ro trycatchintryfinally_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchintryfinally_ro.dasm.il trycatchintryfinally_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchintryfinally_ro.dasm.il trycatchintryfinally_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchintryfinally_ro.asm.exe  trycatchintryfinally_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchintryfinally_ro.asm.exe  trycatchintryfinally_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchintryfinally_ro
    if [ ! -f trycatchintryfinally_ro.org ]; then
        TakeLock
        if [ ! -f trycatchintryfinally_ro.org ]; then
          mkdir IL
          cp trycatchintryfinally_ro.exe IL/trycatchintryfinally_ro.exe
          mv trycatchintryfinally_ro.exe trycatchintryfinally_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchintryfinally_ro.org /out trycatchintryfinally_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchintryfinally_ro.org /out trycatchintryfinally_ro.exe
          __cgExitCode=$?
...
ExePath=trycatchintryfinally_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchintryfinally_ro.reflect.xml
  if [ -f trycatchintryfinally_ro.reflect.xml ]
then
    ReflectionRoots="-x trycatchintryfinally_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchintryfinally_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchintryfinally_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchintryfinally_ro.asm.exe $CLRTestExecutionArguments
