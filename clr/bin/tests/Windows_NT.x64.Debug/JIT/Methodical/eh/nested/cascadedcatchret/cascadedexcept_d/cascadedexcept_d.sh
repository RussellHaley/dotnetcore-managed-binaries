...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/cascadedexcept_d cascadedexcept_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/cascadedexcept_d cascadedexcept_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=cascadedexcept_d.dasm.il cascadedexcept_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=cascadedexcept_d.dasm.il cascadedexcept_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=cascadedexcept_d.asm.exe -QUIET -NOLOGO -DEBUG cascadedexcept_d.dasm.il
    "$CORE_ROOT/ilasm" -output=cascadedexcept_d.asm.exe -QUIET -NOLOGO -DEBUG cascadedexcept_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=cascadedexcept_d
    if [ ! -f cascadedexcept_d.org ]; then
        TakeLock
        if [ ! -f cascadedexcept_d.org ]; then
          mkdir IL
          cp cascadedexcept_d.exe IL/cascadedexcept_d.exe
          mv cascadedexcept_d.exe cascadedexcept_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cascadedexcept_d.org /out cascadedexcept_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in cascadedexcept_d.org /out cascadedexcept_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in cascadedexcept_d.reflect.xml
  if [ -f cascadedexcept_d.reflect.xml ]
then
    ReflectionRoots="-x cascadedexcept_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/cascadedexcept_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" cascadedexcept_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" cascadedexcept_d.asm.exe $CLRTestExecutionArguments
