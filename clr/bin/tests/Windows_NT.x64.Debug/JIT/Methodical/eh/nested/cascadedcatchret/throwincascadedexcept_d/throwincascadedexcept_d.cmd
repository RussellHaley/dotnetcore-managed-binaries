...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedexcept_d.dasm.il throwincascadedexcept_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedexcept_d.dasm.il throwincascadedexcept_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincascadedexcept_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedexcept_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincascadedexcept_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedexcept_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincascadedexcept_d
    if not exist "throwincascadedexcept_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincascadedexcept_d.org" (
            mkdir IL
            copy throwincascadedexcept_d.exe IL\throwincascadedexcept_d.exe 
            ren throwincascadedexcept_d.exe throwincascadedexcept_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedexcept_d.org /out throwincascadedexcept_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedexcept_d.org /out throwincascadedexcept_d.exe
...
set ExePath=throwincascadedexcept_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincascadedexcept_d.reflect.xml
    IF EXIST throwincascadedexcept_d.reflect.xml set ReflectionRoots=-x throwincascadedexcept_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincascadedexcept_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincascadedexcept_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincascadedexcept_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
