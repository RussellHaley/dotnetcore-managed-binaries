...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinally_50_ro throwinfinally_50_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinally_50_ro throwinfinally_50_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_50_ro.dasm.il throwinfinally_50_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_50_ro.dasm.il throwinfinally_50_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinally_50_ro.asm.exe  throwinfinally_50_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinally_50_ro.asm.exe  throwinfinally_50_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinally_50_ro
    if [ ! -f throwinfinally_50_ro.org ]; then
        TakeLock
        if [ ! -f throwinfinally_50_ro.org ]; then
          mkdir IL
          cp throwinfinally_50_ro.exe IL/throwinfinally_50_ro.exe
          mv throwinfinally_50_ro.exe throwinfinally_50_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_50_ro.org /out throwinfinally_50_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_50_ro.org /out throwinfinally_50_ro.exe
          __cgExitCode=$?
...
ExePath=throwinfinally_50_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinally_50_ro.reflect.xml
  if [ -f throwinfinally_50_ro.reflect.xml ]
then
    ReflectionRoots="-x throwinfinally_50_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinally_50_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinally_50_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinally_50_ro.asm.exe $CLRTestExecutionArguments
