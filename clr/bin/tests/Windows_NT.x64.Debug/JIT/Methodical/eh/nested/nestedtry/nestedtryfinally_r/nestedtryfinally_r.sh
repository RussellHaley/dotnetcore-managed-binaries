...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/nestedtryfinally_r nestedtryfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/nestedtryfinally_r nestedtryfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nestedtryfinally_r.dasm.il nestedtryfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=nestedtryfinally_r.dasm.il nestedtryfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nestedtryfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedtryfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=nestedtryfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedtryfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nestedtryfinally_r
    if [ ! -f nestedtryfinally_r.org ]; then
        TakeLock
        if [ ! -f nestedtryfinally_r.org ]; then
          mkdir IL
          cp nestedtryfinally_r.exe IL/nestedtryfinally_r.exe
          mv nestedtryfinally_r.exe nestedtryfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedtryfinally_r.org /out nestedtryfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedtryfinally_r.org /out nestedtryfinally_r.exe
          __cgExitCode=$?
...
ExePath=nestedtryfinally_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in nestedtryfinally_r.reflect.xml
  if [ -f nestedtryfinally_r.reflect.xml ]
then
    ReflectionRoots="-x nestedtryfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nestedtryfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nestedtryfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nestedtryfinally_r.asm.exe $CLRTestExecutionArguments
