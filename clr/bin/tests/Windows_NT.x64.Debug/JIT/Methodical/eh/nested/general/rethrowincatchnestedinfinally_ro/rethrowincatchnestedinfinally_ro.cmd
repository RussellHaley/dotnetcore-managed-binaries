...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=rethrowincatchnestedinfinally_ro.dasm.il rethrowincatchnestedinfinally_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=rethrowincatchnestedinfinally_ro.dasm.il rethrowincatchnestedinfinally_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=rethrowincatchnestedinfinally_ro.asm.exe  rethrowincatchnestedinfinally_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=rethrowincatchnestedinfinally_ro.asm.exe  rethrowincatchnestedinfinally_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=rethrowincatchnestedinfinally_ro
    if not exist "rethrowincatchnestedinfinally_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "rethrowincatchnestedinfinally_ro.org" (
            mkdir IL
            copy rethrowincatchnestedinfinally_ro.exe IL\rethrowincatchnestedinfinally_ro.exe 
            ren rethrowincatchnestedinfinally_ro.exe rethrowincatchnestedinfinally_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrowincatchnestedinfinally_ro.org /out rethrowincatchnestedinfinally_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rethrowincatchnestedinfinally_ro.org /out rethrowincatchnestedinfinally_ro.exe
...
set ExePath=rethrowincatchnestedinfinally_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in rethrowincatchnestedinfinally_ro.reflect.xml
    IF EXIST rethrowincatchnestedinfinally_ro.reflect.xml set ReflectionRoots=-x rethrowincatchnestedinfinally_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\rethrowincatchnestedinfinally_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% rethrowincatchnestedinfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% rethrowincatchnestedinfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
