...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedtryfault_d.dasm.il throwinnestedtryfault_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedtryfault_d.dasm.il throwinnestedtryfault_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinnestedtryfault_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedtryfault_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinnestedtryfault_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedtryfault_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinnestedtryfault_d
    if not exist "throwinnestedtryfault_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinnestedtryfault_d.org" (
            mkdir IL
            copy throwinnestedtryfault_d.exe IL\throwinnestedtryfault_d.exe 
            ren throwinnestedtryfault_d.exe throwinnestedtryfault_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedtryfault_d.org /out throwinnestedtryfault_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedtryfault_d.org /out throwinnestedtryfault_d.exe
...
set ExePath=throwinnestedtryfault_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinnestedtryfault_d.reflect.xml
    IF EXIST throwinnestedtryfault_d.reflect.xml set ReflectionRoots=-x throwinnestedtryfault_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinnestedtryfault_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinnestedtryfault_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinnestedtryfault_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
