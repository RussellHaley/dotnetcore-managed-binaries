...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallyrecursive_20_do throwinfinallyrecursive_20_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinallyrecursive_20_do throwinfinallyrecursive_20_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyrecursive_20_do.dasm.il throwinfinallyrecursive_20_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyrecursive_20_do.dasm.il throwinfinallyrecursive_20_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallyrecursive_20_do.asm.exe  throwinfinallyrecursive_20_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallyrecursive_20_do.asm.exe  throwinfinallyrecursive_20_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallyrecursive_20_do
    if [ ! -f throwinfinallyrecursive_20_do.org ]; then
        TakeLock
        if [ ! -f throwinfinallyrecursive_20_do.org ]; then
          mkdir IL
          cp throwinfinallyrecursive_20_do.exe IL/throwinfinallyrecursive_20_do.exe
          mv throwinfinallyrecursive_20_do.exe throwinfinallyrecursive_20_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyrecursive_20_do.org /out throwinfinallyrecursive_20_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyrecursive_20_do.org /out throwinfinallyrecursive_20_do.exe
          __cgExitCode=$?
...
ExePath=throwinfinallyrecursive_20_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallyrecursive_20_do.reflect.xml
  if [ -f throwinfinallyrecursive_20_do.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallyrecursive_20_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallyrecursive_20_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallyrecursive_20_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallyrecursive_20_do.asm.exe $CLRTestExecutionArguments
