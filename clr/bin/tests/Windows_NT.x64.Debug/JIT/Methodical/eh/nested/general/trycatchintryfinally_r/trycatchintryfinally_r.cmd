...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchintryfinally_r.dasm.il trycatchintryfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchintryfinally_r.dasm.il trycatchintryfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchintryfinally_r.asm.exe  trycatchintryfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchintryfinally_r.asm.exe  trycatchintryfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchintryfinally_r
    if not exist "trycatchintryfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchintryfinally_r.org" (
            mkdir IL
            copy trycatchintryfinally_r.exe IL\trycatchintryfinally_r.exe 
            ren trycatchintryfinally_r.exe trycatchintryfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchintryfinally_r.org /out trycatchintryfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchintryfinally_r.org /out trycatchintryfinally_r.exe
...
set ExePath=trycatchintryfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchintryfinally_r.reflect.xml
    IF EXIST trycatchintryfinally_r.reflect.xml set ReflectionRoots=-x trycatchintryfinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchintryfinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchintryfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchintryfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
