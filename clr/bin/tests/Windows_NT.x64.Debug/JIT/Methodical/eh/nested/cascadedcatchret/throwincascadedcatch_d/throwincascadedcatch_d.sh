...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedcatch_d throwincascadedcatch_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedcatch_d throwincascadedcatch_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincascadedcatch_d.dasm.il throwincascadedcatch_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincascadedcatch_d.dasm.il throwincascadedcatch_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincascadedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedcatch_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincascadedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedcatch_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincascadedcatch_d
    if [ ! -f throwincascadedcatch_d.org ]; then
        TakeLock
        if [ ! -f throwincascadedcatch_d.org ]; then
          mkdir IL
          cp throwincascadedcatch_d.exe IL/throwincascadedcatch_d.exe
          mv throwincascadedcatch_d.exe throwincascadedcatch_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedcatch_d.org /out throwincascadedcatch_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedcatch_d.org /out throwincascadedcatch_d.exe
          __cgExitCode=$?
...
ExePath=throwincascadedcatch_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwincascadedcatch_d.reflect.xml
  if [ -f throwincascadedcatch_d.reflect.xml ]
then
    ReflectionRoots="-x throwincascadedcatch_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincascadedcatch_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincascadedcatch_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincascadedcatch_d.asm.exe $CLRTestExecutionArguments
