...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchintryfinally_do.dasm.il trycatchintryfinally_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchintryfinally_do.dasm.il trycatchintryfinally_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchintryfinally_do.asm.exe  trycatchintryfinally_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchintryfinally_do.asm.exe  trycatchintryfinally_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchintryfinally_do
    if not exist "trycatchintryfinally_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchintryfinally_do.org" (
            mkdir IL
            copy trycatchintryfinally_do.exe IL\trycatchintryfinally_do.exe 
            ren trycatchintryfinally_do.exe trycatchintryfinally_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchintryfinally_do.org /out trycatchintryfinally_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchintryfinally_do.org /out trycatchintryfinally_do.exe
...
set ExePath=trycatchintryfinally_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchintryfinally_do.reflect.xml
    IF EXIST trycatchintryfinally_do.reflect.xml set ReflectionRoots=-x trycatchintryfinally_do.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchintryfinally_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchintryfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchintryfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
