...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedcatchnofin_r throwincascadedcatchnofin_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedcatchnofin_r throwincascadedcatchnofin_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincascadedcatchnofin_r.dasm.il throwincascadedcatchnofin_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincascadedcatchnofin_r.dasm.il throwincascadedcatchnofin_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincascadedcatchnofin_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedcatchnofin_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincascadedcatchnofin_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedcatchnofin_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincascadedcatchnofin_r
    if [ ! -f throwincascadedcatchnofin_r.org ]; then
        TakeLock
        if [ ! -f throwincascadedcatchnofin_r.org ]; then
          mkdir IL
          cp throwincascadedcatchnofin_r.exe IL/throwincascadedcatchnofin_r.exe
          mv throwincascadedcatchnofin_r.exe throwincascadedcatchnofin_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedcatchnofin_r.org /out throwincascadedcatchnofin_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedcatchnofin_r.org /out throwincascadedcatchnofin_r.exe
          __cgExitCode=$?
...
ExePath=throwincascadedcatchnofin_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwincascadedcatchnofin_r.reflect.xml
  if [ -f throwincascadedcatchnofin_r.reflect.xml ]
then
    ReflectionRoots="-x throwincascadedcatchnofin_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincascadedcatchnofin_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincascadedcatchnofin_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincascadedcatchnofin_r.asm.exe $CLRTestExecutionArguments
