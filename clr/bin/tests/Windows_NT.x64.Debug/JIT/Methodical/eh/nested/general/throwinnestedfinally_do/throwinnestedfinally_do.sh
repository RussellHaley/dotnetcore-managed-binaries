...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedfinally_do throwinnestedfinally_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinnestedfinally_do throwinnestedfinally_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinnestedfinally_do.dasm.il throwinnestedfinally_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinnestedfinally_do.dasm.il throwinnestedfinally_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinnestedfinally_do.asm.exe  throwinnestedfinally_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinnestedfinally_do.asm.exe  throwinnestedfinally_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinnestedfinally_do
    if [ ! -f throwinnestedfinally_do.org ]; then
        TakeLock
        if [ ! -f throwinnestedfinally_do.org ]; then
          mkdir IL
          cp throwinnestedfinally_do.exe IL/throwinnestedfinally_do.exe
          mv throwinnestedfinally_do.exe throwinnestedfinally_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedfinally_do.org /out throwinnestedfinally_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedfinally_do.org /out throwinnestedfinally_do.exe
          __cgExitCode=$?
...
ExePath=throwinnestedfinally_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinnestedfinally_do.reflect.xml
  if [ -f throwinnestedfinally_do.reflect.xml ]
then
    ReflectionRoots="-x throwinnestedfinally_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinnestedfinally_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinnestedfinally_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinnestedfinally_do.asm.exe $CLRTestExecutionArguments
