...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_ro.dasm.il throwinfinallynestedintry_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_ro.dasm.il throwinfinallynestedintry_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_ro.asm.exe  throwinfinallynestedintry_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_ro.asm.exe  throwinfinallynestedintry_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinallynestedintry_ro
    if not exist "throwinfinallynestedintry_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinallynestedintry_ro.org" (
            mkdir IL
            copy throwinfinallynestedintry_ro.exe IL\throwinfinallynestedintry_ro.exe 
            ren throwinfinallynestedintry_ro.exe throwinfinallynestedintry_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_ro.org /out throwinfinallynestedintry_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_ro.org /out throwinfinallynestedintry_ro.exe
...
set ExePath=throwinfinallynestedintry_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinallynestedintry_ro.reflect.xml
    IF EXIST throwinfinallynestedintry_ro.reflect.xml set ReflectionRoots=-x throwinfinallynestedintry_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinallynestedintry_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinallynestedintry_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinallynestedintry_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
