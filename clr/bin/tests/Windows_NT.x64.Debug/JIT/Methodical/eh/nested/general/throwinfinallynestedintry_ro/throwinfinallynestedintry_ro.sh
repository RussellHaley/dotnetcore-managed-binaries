...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinallynestedintry_ro throwinfinallynestedintry_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinallynestedintry_ro throwinfinallynestedintry_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_ro.dasm.il throwinfinallynestedintry_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_ro.dasm.il throwinfinallynestedintry_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_ro.asm.exe  throwinfinallynestedintry_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_ro.asm.exe  throwinfinallynestedintry_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallynestedintry_ro
    if [ ! -f throwinfinallynestedintry_ro.org ]; then
        TakeLock
        if [ ! -f throwinfinallynestedintry_ro.org ]; then
          mkdir IL
          cp throwinfinallynestedintry_ro.exe IL/throwinfinallynestedintry_ro.exe
          mv throwinfinallynestedintry_ro.exe throwinfinallynestedintry_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_ro.org /out throwinfinallynestedintry_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_ro.org /out throwinfinallynestedintry_ro.exe
          __cgExitCode=$?
...
ExePath=throwinfinallynestedintry_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallynestedintry_ro.reflect.xml
  if [ -f throwinfinallynestedintry_ro.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallynestedintry_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallynestedintry_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallynestedintry_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallynestedintry_ro.asm.exe $CLRTestExecutionArguments
