...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinally_50_d throwinfinally_50_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nonlocalexit/throwinfinally_50_d throwinfinally_50_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_50_d.dasm.il throwinfinally_50_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_50_d.dasm.il throwinfinally_50_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinally_50_d.asm.exe  throwinfinally_50_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinally_50_d.asm.exe  throwinfinally_50_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinally_50_d
    if [ ! -f throwinfinally_50_d.org ]; then
        TakeLock
        if [ ! -f throwinfinally_50_d.org ]; then
          mkdir IL
          cp throwinfinally_50_d.exe IL/throwinfinally_50_d.exe
          mv throwinfinally_50_d.exe throwinfinally_50_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_50_d.org /out throwinfinally_50_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_50_d.org /out throwinfinally_50_d.exe
          __cgExitCode=$?
...
ExePath=throwinfinally_50_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinally_50_d.reflect.xml
  if [ -f throwinfinally_50_d.reflect.xml ]
then
    ReflectionRoots="-x throwinfinally_50_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinally_50_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinally_50_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinally_50_d.asm.exe $CLRTestExecutionArguments
