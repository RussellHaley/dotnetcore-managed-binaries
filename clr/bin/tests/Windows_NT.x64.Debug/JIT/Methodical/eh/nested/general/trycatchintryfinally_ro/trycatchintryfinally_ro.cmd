...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchintryfinally_ro.dasm.il trycatchintryfinally_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchintryfinally_ro.dasm.il trycatchintryfinally_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchintryfinally_ro.asm.exe  trycatchintryfinally_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchintryfinally_ro.asm.exe  trycatchintryfinally_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchintryfinally_ro
    if not exist "trycatchintryfinally_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchintryfinally_ro.org" (
            mkdir IL
            copy trycatchintryfinally_ro.exe IL\trycatchintryfinally_ro.exe 
            ren trycatchintryfinally_ro.exe trycatchintryfinally_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchintryfinally_ro.org /out trycatchintryfinally_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchintryfinally_ro.org /out trycatchintryfinally_ro.exe
...
set ExePath=trycatchintryfinally_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchintryfinally_ro.reflect.xml
    IF EXIST trycatchintryfinally_ro.reflect.xml set ReflectionRoots=-x trycatchintryfinally_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchintryfinally_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchintryfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchintryfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
