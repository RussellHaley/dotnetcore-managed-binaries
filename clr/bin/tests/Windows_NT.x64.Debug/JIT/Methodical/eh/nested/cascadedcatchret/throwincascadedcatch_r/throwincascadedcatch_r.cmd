...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedcatch_r.dasm.il throwincascadedcatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedcatch_r.dasm.il throwincascadedcatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincascadedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedcatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincascadedcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedcatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincascadedcatch_r
    if not exist "throwincascadedcatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincascadedcatch_r.org" (
            mkdir IL
            copy throwincascadedcatch_r.exe IL\throwincascadedcatch_r.exe 
            ren throwincascadedcatch_r.exe throwincascadedcatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedcatch_r.org /out throwincascadedcatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedcatch_r.org /out throwincascadedcatch_r.exe
...
set ExePath=throwincascadedcatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincascadedcatch_r.reflect.xml
    IF EXIST throwincascadedcatch_r.reflect.xml set ReflectionRoots=-x throwincascadedcatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincascadedcatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincascadedcatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincascadedcatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
