...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinally_d throwinfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinally_d throwinfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_d.dasm.il throwinfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinally_d.dasm.il throwinfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinally_d.asm.exe  throwinfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinally_d.asm.exe  throwinfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinally_d
    if [ ! -f throwinfinally_d.org ]; then
        TakeLock
        if [ ! -f throwinfinally_d.org ]; then
          mkdir IL
          cp throwinfinally_d.exe IL/throwinfinally_d.exe
          mv throwinfinally_d.exe throwinfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_d.org /out throwinfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinally_d.org /out throwinfinally_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinally_d.reflect.xml
  if [ -f throwinfinally_d.reflect.xml ]
then
    ReflectionRoots="-x throwinfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinally_d.asm.exe $CLRTestExecutionArguments
