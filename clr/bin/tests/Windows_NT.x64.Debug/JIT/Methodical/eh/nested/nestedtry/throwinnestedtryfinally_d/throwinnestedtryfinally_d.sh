...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/throwinnestedtryfinally_d throwinnestedtryfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/nestedtry/throwinnestedtryfinally_d throwinnestedtryfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinnestedtryfinally_d.dasm.il throwinnestedtryfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinnestedtryfinally_d.dasm.il throwinnestedtryfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinnestedtryfinally_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedtryfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinnestedtryfinally_d.asm.exe -QUIET -NOLOGO -DEBUG throwinnestedtryfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinnestedtryfinally_d
    if [ ! -f throwinnestedtryfinally_d.org ]; then
        TakeLock
        if [ ! -f throwinnestedtryfinally_d.org ]; then
          mkdir IL
          cp throwinnestedtryfinally_d.exe IL/throwinnestedtryfinally_d.exe
          mv throwinnestedtryfinally_d.exe throwinnestedtryfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedtryfinally_d.org /out throwinnestedtryfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinnestedtryfinally_d.org /out throwinnestedtryfinally_d.exe
          __cgExitCode=$?
...
ExePath=throwinnestedtryfinally_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinnestedtryfinally_d.reflect.xml
  if [ -f throwinnestedtryfinally_d.reflect.xml ]
then
    ReflectionRoots="-x throwinnestedtryfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinnestedtryfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinnestedtryfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinnestedtryfinally_d.asm.exe $CLRTestExecutionArguments
