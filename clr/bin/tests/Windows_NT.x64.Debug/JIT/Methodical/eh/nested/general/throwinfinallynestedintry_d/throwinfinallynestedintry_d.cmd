...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_d.dasm.il throwinfinallynestedintry_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallynestedintry_d.dasm.il throwinfinallynestedintry_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_d.asm.exe  throwinfinallynestedintry_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinallynestedintry_d.asm.exe  throwinfinallynestedintry_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinallynestedintry_d
    if not exist "throwinfinallynestedintry_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinallynestedintry_d.org" (
            mkdir IL
            copy throwinfinallynestedintry_d.exe IL\throwinfinallynestedintry_d.exe 
            ren throwinfinallynestedintry_d.exe throwinfinallynestedintry_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_d.org /out throwinfinallynestedintry_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallynestedintry_d.org /out throwinfinallynestedintry_d.exe
...
set ExePath=throwinfinallynestedintry_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinallynestedintry_d.reflect.xml
    IF EXIST throwinfinallynestedintry_d.reflect.xml set ReflectionRoots=-x throwinfinallynestedintry_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinallynestedintry_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinallynestedintry_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinallynestedintry_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
