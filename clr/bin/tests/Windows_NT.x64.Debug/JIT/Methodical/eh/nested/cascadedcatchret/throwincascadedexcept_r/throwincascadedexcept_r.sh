...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedexcept_r throwincascadedexcept_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/cascadedcatchret/throwincascadedexcept_r throwincascadedexcept_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincascadedexcept_r.dasm.il throwincascadedexcept_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincascadedexcept_r.dasm.il throwincascadedexcept_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincascadedexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedexcept_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincascadedexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwincascadedexcept_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincascadedexcept_r
    if [ ! -f throwincascadedexcept_r.org ]; then
        TakeLock
        if [ ! -f throwincascadedexcept_r.org ]; then
          mkdir IL
          cp throwincascadedexcept_r.exe IL/throwincascadedexcept_r.exe
          mv throwincascadedexcept_r.exe throwincascadedexcept_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedexcept_r.org /out throwincascadedexcept_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincascadedexcept_r.org /out throwincascadedexcept_r.exe
          __cgExitCode=$?
...
ExePath=throwincascadedexcept_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwincascadedexcept_r.reflect.xml
  if [ -f throwincascadedexcept_r.reflect.xml ]
then
    ReflectionRoots="-x throwincascadedexcept_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincascadedexcept_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincascadedexcept_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincascadedexcept_r.asm.exe $CLRTestExecutionArguments
