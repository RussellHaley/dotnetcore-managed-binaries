...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/rethrowincatchnestedinfinally_d rethrowincatchnestedinfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/rethrowincatchnestedinfinally_d rethrowincatchnestedinfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rethrowincatchnestedinfinally_d.dasm.il rethrowincatchnestedinfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=rethrowincatchnestedinfinally_d.dasm.il rethrowincatchnestedinfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rethrowincatchnestedinfinally_d.asm.exe  rethrowincatchnestedinfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=rethrowincatchnestedinfinally_d.asm.exe  rethrowincatchnestedinfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rethrowincatchnestedinfinally_d
    if [ ! -f rethrowincatchnestedinfinally_d.org ]; then
        TakeLock
        if [ ! -f rethrowincatchnestedinfinally_d.org ]; then
          mkdir IL
          cp rethrowincatchnestedinfinally_d.exe IL/rethrowincatchnestedinfinally_d.exe
          mv rethrowincatchnestedinfinally_d.exe rethrowincatchnestedinfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrowincatchnestedinfinally_d.org /out rethrowincatchnestedinfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rethrowincatchnestedinfinally_d.org /out rethrowincatchnestedinfinally_d.exe
          __cgExitCode=$?
...
ExePath=rethrowincatchnestedinfinally_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in rethrowincatchnestedinfinally_d.reflect.xml
  if [ -f rethrowincatchnestedinfinally_d.reflect.xml ]
then
    ReflectionRoots="-x rethrowincatchnestedinfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/rethrowincatchnestedinfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rethrowincatchnestedinfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rethrowincatchnestedinfinally_d.asm.exe $CLRTestExecutionArguments
