...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nestedtryfinally_r.dasm.il nestedtryfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nestedtryfinally_r.dasm.il nestedtryfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nestedtryfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedtryfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nestedtryfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedtryfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nestedtryfinally_r
    if not exist "nestedtryfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nestedtryfinally_r.org" (
            mkdir IL
            copy nestedtryfinally_r.exe IL\nestedtryfinally_r.exe 
            ren nestedtryfinally_r.exe nestedtryfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedtryfinally_r.org /out nestedtryfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedtryfinally_r.org /out nestedtryfinally_r.exe
...
set ExePath=nestedtryfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nestedtryfinally_r.reflect.xml
    IF EXIST nestedtryfinally_r.reflect.xml set ReflectionRoots=-x nestedtryfinally_r.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nestedtryfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nestedtryfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
