...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinallynestedintry_do throwinfinallynestedintry_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/Methodical/eh/nested/general/throwinfinallynestedintry_do throwinfinallynestedintry_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_do.dasm.il throwinfinallynestedintry_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallynestedintry_do.dasm.il throwinfinallynestedintry_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_do.asm.exe  throwinfinallynestedintry_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallynestedintry_do.asm.exe  throwinfinallynestedintry_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallynestedintry_do
    if [ ! -f throwinfinallynestedintry_do.org ]; then
        TakeLock
        if [ ! -f throwinfinallynestedintry_do.org ]; then
          mkdir IL
          cp throwinfinallynestedintry_do.exe IL/throwinfinallynestedintry_do.exe
          mv throwinfinallynestedintry_do.exe throwinfinallynestedintry_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_do.org /out throwinfinallynestedintry_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallynestedintry_do.org /out throwinfinallynestedintry_do.exe
          __cgExitCode=$?
...
ExePath=throwinfinallynestedintry_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallynestedintry_do.reflect.xml
  if [ -f throwinfinallynestedintry_do.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallynestedintry_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallynestedintry_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallynestedintry_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallynestedintry_do.asm.exe $CLRTestExecutionArguments
