...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=cascadedexcept_d.dasm.il cascadedexcept_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=cascadedexcept_d.dasm.il cascadedexcept_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=cascadedexcept_d.asm.exe -QUIET -NOLOGO -DEBUG cascadedexcept_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=cascadedexcept_d.asm.exe -QUIET -NOLOGO -DEBUG cascadedexcept_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=cascadedexcept_d
    if not exist "cascadedexcept_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "cascadedexcept_d.org" (
            mkdir IL
            copy cascadedexcept_d.exe IL\cascadedexcept_d.exe 
            ren cascadedexcept_d.exe cascadedexcept_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cascadedexcept_d.org /out cascadedexcept_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cascadedexcept_d.org /out cascadedexcept_d.exe
...
set ExePath=cascadedexcept_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in cascadedexcept_d.reflect.xml
    IF EXIST cascadedexcept_d.reflect.xml set ReflectionRoots=-x cascadedexcept_d.reflect.xml
    ...
    set ExePath=%LinkBin%\cascadedexcept_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% cascadedexcept_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% cascadedexcept_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
