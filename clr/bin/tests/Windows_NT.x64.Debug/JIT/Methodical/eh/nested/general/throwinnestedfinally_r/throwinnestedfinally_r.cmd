...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedfinally_r.dasm.il throwinnestedfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedfinally_r.dasm.il throwinnestedfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinnestedfinally_r.asm.exe  throwinnestedfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinnestedfinally_r.asm.exe  throwinnestedfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinnestedfinally_r
    if not exist "throwinnestedfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinnestedfinally_r.org" (
            mkdir IL
            copy throwinnestedfinally_r.exe IL\throwinnestedfinally_r.exe 
            ren throwinnestedfinally_r.exe throwinnestedfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedfinally_r.org /out throwinnestedfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedfinally_r.org /out throwinnestedfinally_r.exe
...
set ExePath=throwinnestedfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinnestedfinally_r.reflect.xml
    IF EXIST throwinnestedfinally_r.reflect.xml set ReflectionRoots=-x throwinnestedfinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinnestedfinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinnestedfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinnestedfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
