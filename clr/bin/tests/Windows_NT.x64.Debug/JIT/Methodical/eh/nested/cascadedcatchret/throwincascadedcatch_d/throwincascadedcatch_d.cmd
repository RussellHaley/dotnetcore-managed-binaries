...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedcatch_d.dasm.il throwincascadedcatch_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincascadedcatch_d.dasm.il throwincascadedcatch_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincascadedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedcatch_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincascadedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG throwincascadedcatch_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincascadedcatch_d
    if not exist "throwincascadedcatch_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincascadedcatch_d.org" (
            mkdir IL
            copy throwincascadedcatch_d.exe IL\throwincascadedcatch_d.exe 
            ren throwincascadedcatch_d.exe throwincascadedcatch_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedcatch_d.org /out throwincascadedcatch_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincascadedcatch_d.org /out throwincascadedcatch_d.exe
...
set ExePath=throwincascadedcatch_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincascadedcatch_d.reflect.xml
    IF EXIST throwincascadedcatch_d.reflect.xml set ReflectionRoots=-x throwincascadedcatch_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincascadedcatch_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincascadedcatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincascadedcatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
