...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=cascadedcatch_d.dasm.il cascadedcatch_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=cascadedcatch_d.dasm.il cascadedcatch_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=cascadedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG cascadedcatch_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=cascadedcatch_d.asm.exe -QUIET -NOLOGO -DEBUG cascadedcatch_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=cascadedcatch_d
    if not exist "cascadedcatch_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "cascadedcatch_d.org" (
            mkdir IL
            copy cascadedcatch_d.exe IL\cascadedcatch_d.exe 
            ren cascadedcatch_d.exe cascadedcatch_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cascadedcatch_d.org /out cascadedcatch_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cascadedcatch_d.org /out cascadedcatch_d.exe
...
set ExePath=cascadedcatch_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in cascadedcatch_d.reflect.xml
    IF EXIST cascadedcatch_d.reflect.xml set ReflectionRoots=-x cascadedcatch_d.reflect.xml
    ...
    set ExePath=%LinkBin%\cascadedcatch_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% cascadedcatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% cascadedcatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
