...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=cascadedexcept_r.dasm.il cascadedexcept_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=cascadedexcept_r.dasm.il cascadedexcept_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=cascadedexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT cascadedexcept_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=cascadedexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT cascadedexcept_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=cascadedexcept_r
    if not exist "cascadedexcept_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "cascadedexcept_r.org" (
            mkdir IL
            copy cascadedexcept_r.exe IL\cascadedexcept_r.exe 
            ren cascadedexcept_r.exe cascadedexcept_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cascadedexcept_r.org /out cascadedexcept_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in cascadedexcept_r.org /out cascadedexcept_r.exe
...
set ExePath=cascadedexcept_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in cascadedexcept_r.reflect.xml
    IF EXIST cascadedexcept_r.reflect.xml set ReflectionRoots=-x cascadedexcept_r.reflect.xml
    ...
    set ExePath=%LinkBin%\cascadedexcept_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% cascadedexcept_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% cascadedexcept_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
