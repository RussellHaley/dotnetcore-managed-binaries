...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedtryfinally_r.dasm.il throwinnestedtryfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinnestedtryfinally_r.dasm.il throwinnestedtryfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinnestedtryfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinnestedtryfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinnestedtryfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinnestedtryfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinnestedtryfinally_r
    if not exist "throwinnestedtryfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinnestedtryfinally_r.org" (
            mkdir IL
            copy throwinnestedtryfinally_r.exe IL\throwinnestedtryfinally_r.exe 
            ren throwinnestedtryfinally_r.exe throwinnestedtryfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedtryfinally_r.org /out throwinnestedtryfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinnestedtryfinally_r.org /out throwinnestedtryfinally_r.exe
...
set ExePath=throwinnestedtryfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinnestedtryfinally_r.reflect.xml
    IF EXIST throwinnestedtryfinally_r.reflect.xml set ReflectionRoots=-x throwinnestedtryfinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinnestedtryfinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinnestedtryfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinnestedtryfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
