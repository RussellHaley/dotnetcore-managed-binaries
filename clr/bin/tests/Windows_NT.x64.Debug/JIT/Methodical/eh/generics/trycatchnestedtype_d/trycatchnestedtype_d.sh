...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchnestedtype_d trycatchnestedtype_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchnestedtype_d trycatchnestedtype_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchnestedtype_d.dasm.il trycatchnestedtype_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchnestedtype_d.dasm.il trycatchnestedtype_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchnestedtype_d.asm.exe  trycatchnestedtype_d.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchnestedtype_d.asm.exe  trycatchnestedtype_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchnestedtype_d
    if [ ! -f trycatchnestedtype_d.org ]; then
        TakeLock
        if [ ! -f trycatchnestedtype_d.org ]; then
          mkdir IL
          cp trycatchnestedtype_d.exe IL/trycatchnestedtype_d.exe
          mv trycatchnestedtype_d.exe trycatchnestedtype_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchnestedtype_d.org /out trycatchnestedtype_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchnestedtype_d.org /out trycatchnestedtype_d.exe
          __cgExitCode=$?
...
ExePath=trycatchnestedtype_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchnestedtype_d.reflect.xml
  if [ -f trycatchnestedtype_d.reflect.xml ]
then
    ReflectionRoots="-x trycatchnestedtype_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchnestedtype_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchnestedtype_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchnestedtype_d.asm.exe $CLRTestExecutionArguments
