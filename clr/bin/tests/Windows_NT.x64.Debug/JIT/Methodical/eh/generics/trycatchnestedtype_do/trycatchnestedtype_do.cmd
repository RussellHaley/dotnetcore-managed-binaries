...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchnestedtype_do trycatchnestedtype_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchnestedtype_do trycatchnestedtype_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchnestedtype_do.dasm.il trycatchnestedtype_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchnestedtype_do.dasm.il trycatchnestedtype_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchnestedtype_do.asm.exe  trycatchnestedtype_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchnestedtype_do.asm.exe  trycatchnestedtype_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchnestedtype_do
    if not exist "trycatchnestedtype_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchnestedtype_do.org" (
            mkdir IL
            copy trycatchnestedtype_do.exe IL\trycatchnestedtype_do.exe 
            ren trycatchnestedtype_do.exe trycatchnestedtype_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchnestedtype_do.org /out trycatchnestedtype_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchnestedtype_do.org /out trycatchnestedtype_do.exe
...
set ExePath=trycatchnestedtype_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchnestedtype_do.reflect.xml
    IF EXIST trycatchnestedtype_do.reflect.xml set ReflectionRoots=-x trycatchnestedtype_do.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchnestedtype_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchnestedtype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchnestedtype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
