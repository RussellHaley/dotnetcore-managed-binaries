...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchnestedtype_do trycatchnestedtype_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchnestedtype_do trycatchnestedtype_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchnestedtype_do.dasm.il trycatchnestedtype_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchnestedtype_do.dasm.il trycatchnestedtype_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchnestedtype_do.asm.exe  trycatchnestedtype_do.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchnestedtype_do.asm.exe  trycatchnestedtype_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchnestedtype_do
    if [ ! -f trycatchnestedtype_do.org ]; then
        TakeLock
        if [ ! -f trycatchnestedtype_do.org ]; then
          mkdir IL
          cp trycatchnestedtype_do.exe IL/trycatchnestedtype_do.exe
          mv trycatchnestedtype_do.exe trycatchnestedtype_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchnestedtype_do.org /out trycatchnestedtype_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchnestedtype_do.org /out trycatchnestedtype_do.exe
          __cgExitCode=$?
...
ExePath=trycatchnestedtype_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchnestedtype_do.reflect.xml
  if [ -f trycatchnestedtype_do.reflect.xml ]
then
    ReflectionRoots="-x trycatchnestedtype_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchnestedtype_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchnestedtype_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchnestedtype_do.asm.exe $CLRTestExecutionArguments
