...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/throwincatch_do throwincatch_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/throwincatch_do throwincatch_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincatch_do.dasm.il throwincatch_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincatch_do.dasm.il throwincatch_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincatch_do.asm.exe  throwincatch_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincatch_do.asm.exe  throwincatch_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincatch_do
    if [ ! -f throwincatch_do.org ]; then
        TakeLock
        if [ ! -f throwincatch_do.org ]; then
          mkdir IL
          cp throwincatch_do.exe IL/throwincatch_do.exe
          mv throwincatch_do.exe throwincatch_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincatch_do.org /out throwincatch_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincatch_do.org /out throwincatch_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwincatch_do.reflect.xml
  if [ -f throwincatch_do.reflect.xml ]
then
    ReflectionRoots="-x throwincatch_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincatch_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincatch_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincatch_do.asm.exe $CLRTestExecutionArguments
