...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\throwincatch_ro throwincatch_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\throwincatch_ro throwincatch_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincatch_ro.dasm.il throwincatch_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincatch_ro.dasm.il throwincatch_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincatch_ro.asm.exe  throwincatch_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincatch_ro.asm.exe  throwincatch_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincatch_ro
    if not exist "throwincatch_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincatch_ro.org" (
            mkdir IL
            copy throwincatch_ro.exe IL\throwincatch_ro.exe 
            ren throwincatch_ro.exe throwincatch_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincatch_ro.org /out throwincatch_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincatch_ro.org /out throwincatch_ro.exe
...
set ExePath=throwincatch_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincatch_ro.reflect.xml
    IF EXIST throwincatch_ro.reflect.xml set ReflectionRoots=-x throwincatch_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincatch_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincatch_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincatch_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
