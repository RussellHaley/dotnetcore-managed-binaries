...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchsimpletype_r trycatchsimpletype_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchsimpletype_r trycatchsimpletype_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchsimpletype_r.dasm.il trycatchsimpletype_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchsimpletype_r.dasm.il trycatchsimpletype_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchsimpletype_r.asm.exe  trycatchsimpletype_r.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchsimpletype_r.asm.exe  trycatchsimpletype_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchsimpletype_r
    if [ ! -f trycatchsimpletype_r.org ]; then
        TakeLock
        if [ ! -f trycatchsimpletype_r.org ]; then
          mkdir IL
          cp trycatchsimpletype_r.exe IL/trycatchsimpletype_r.exe
          mv trycatchsimpletype_r.exe trycatchsimpletype_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchsimpletype_r.org /out trycatchsimpletype_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchsimpletype_r.org /out trycatchsimpletype_r.exe
          __cgExitCode=$?
...
ExePath=trycatchsimpletype_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchsimpletype_r.reflect.xml
  if [ -f trycatchsimpletype_r.reflect.xml ]
then
    ReflectionRoots="-x trycatchsimpletype_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchsimpletype_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchsimpletype_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchsimpletype_r.asm.exe $CLRTestExecutionArguments
