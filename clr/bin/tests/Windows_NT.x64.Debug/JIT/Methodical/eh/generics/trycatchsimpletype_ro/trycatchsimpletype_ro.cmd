...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchsimpletype_ro trycatchsimpletype_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchsimpletype_ro trycatchsimpletype_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchsimpletype_ro.dasm.il trycatchsimpletype_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchsimpletype_ro.dasm.il trycatchsimpletype_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchsimpletype_ro.asm.exe  trycatchsimpletype_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchsimpletype_ro.asm.exe  trycatchsimpletype_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchsimpletype_ro
    if not exist "trycatchsimpletype_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchsimpletype_ro.org" (
            mkdir IL
            copy trycatchsimpletype_ro.exe IL\trycatchsimpletype_ro.exe 
            ren trycatchsimpletype_ro.exe trycatchsimpletype_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchsimpletype_ro.org /out trycatchsimpletype_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchsimpletype_ro.org /out trycatchsimpletype_ro.exe
...
set ExePath=trycatchsimpletype_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchsimpletype_ro.reflect.xml
    IF EXIST trycatchsimpletype_ro.reflect.xml set ReflectionRoots=-x trycatchsimpletype_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchsimpletype_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchsimpletype_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchsimpletype_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
