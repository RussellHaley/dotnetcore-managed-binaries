...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchnestedtype_d trycatchnestedtype_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchnestedtype_d trycatchnestedtype_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchnestedtype_d.dasm.il trycatchnestedtype_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchnestedtype_d.dasm.il trycatchnestedtype_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchnestedtype_d.asm.exe  trycatchnestedtype_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchnestedtype_d.asm.exe  trycatchnestedtype_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchnestedtype_d
    if not exist "trycatchnestedtype_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchnestedtype_d.org" (
            mkdir IL
            copy trycatchnestedtype_d.exe IL\trycatchnestedtype_d.exe 
            ren trycatchnestedtype_d.exe trycatchnestedtype_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchnestedtype_d.org /out trycatchnestedtype_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchnestedtype_d.org /out trycatchnestedtype_d.exe
...
set ExePath=trycatchnestedtype_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchnestedtype_d.reflect.xml
    IF EXIST trycatchnestedtype_d.reflect.xml set ReflectionRoots=-x trycatchnestedtype_d.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchnestedtype_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchnestedtype_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchnestedtype_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
