...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchsimpletype_do trycatchsimpletype_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchsimpletype_do trycatchsimpletype_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchsimpletype_do.dasm.il trycatchsimpletype_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchsimpletype_do.dasm.il trycatchsimpletype_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchsimpletype_do.asm.exe  trycatchsimpletype_do.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchsimpletype_do.asm.exe  trycatchsimpletype_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchsimpletype_do
    if [ ! -f trycatchsimpletype_do.org ]; then
        TakeLock
        if [ ! -f trycatchsimpletype_do.org ]; then
          mkdir IL
          cp trycatchsimpletype_do.exe IL/trycatchsimpletype_do.exe
          mv trycatchsimpletype_do.exe trycatchsimpletype_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchsimpletype_do.org /out trycatchsimpletype_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchsimpletype_do.org /out trycatchsimpletype_do.exe
          __cgExitCode=$?
...
ExePath=trycatchsimpletype_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchsimpletype_do.reflect.xml
  if [ -f trycatchsimpletype_do.reflect.xml ]
then
    ReflectionRoots="-x trycatchsimpletype_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchsimpletype_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchsimpletype_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchsimpletype_do.asm.exe $CLRTestExecutionArguments
