...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchnestedtype_r trycatchnestedtype_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchnestedtype_r trycatchnestedtype_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchnestedtype_r.dasm.il trycatchnestedtype_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchnestedtype_r.dasm.il trycatchnestedtype_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchnestedtype_r.asm.exe  trycatchnestedtype_r.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchnestedtype_r.asm.exe  trycatchnestedtype_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchnestedtype_r
    if [ ! -f trycatchnestedtype_r.org ]; then
        TakeLock
        if [ ! -f trycatchnestedtype_r.org ]; then
          mkdir IL
          cp trycatchnestedtype_r.exe IL/trycatchnestedtype_r.exe
          mv trycatchnestedtype_r.exe trycatchnestedtype_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchnestedtype_r.org /out trycatchnestedtype_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchnestedtype_r.org /out trycatchnestedtype_r.exe
          __cgExitCode=$?
...
ExePath=trycatchnestedtype_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchnestedtype_r.reflect.xml
  if [ -f trycatchnestedtype_r.reflect.xml ]
then
    ReflectionRoots="-x trycatchnestedtype_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchnestedtype_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchnestedtype_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchnestedtype_r.asm.exe $CLRTestExecutionArguments
