...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/throwincatch_d throwincatch_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/throwincatch_d throwincatch_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincatch_d.dasm.il throwincatch_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincatch_d.dasm.il throwincatch_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincatch_d.asm.exe  throwincatch_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincatch_d.asm.exe  throwincatch_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincatch_d
    if [ ! -f throwincatch_d.org ]; then
        TakeLock
        if [ ! -f throwincatch_d.org ]; then
          mkdir IL
          cp throwincatch_d.exe IL/throwincatch_d.exe
          mv throwincatch_d.exe throwincatch_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincatch_d.org /out throwincatch_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincatch_d.org /out throwincatch_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwincatch_d.reflect.xml
  if [ -f throwincatch_d.reflect.xml ]
then
    ReflectionRoots="-x throwincatch_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincatch_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincatch_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincatch_d.asm.exe $CLRTestExecutionArguments
