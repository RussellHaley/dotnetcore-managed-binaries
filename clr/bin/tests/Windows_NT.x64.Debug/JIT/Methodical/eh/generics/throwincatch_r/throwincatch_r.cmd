...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\throwincatch_r throwincatch_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\throwincatch_r throwincatch_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincatch_r.dasm.il throwincatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincatch_r.dasm.il throwincatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincatch_r.asm.exe  throwincatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincatch_r.asm.exe  throwincatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincatch_r
    if not exist "throwincatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincatch_r.org" (
            mkdir IL
            copy throwincatch_r.exe IL\throwincatch_r.exe 
            ren throwincatch_r.exe throwincatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincatch_r.org /out throwincatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincatch_r.org /out throwincatch_r.exe
...
set ExePath=throwincatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincatch_r.reflect.xml
    IF EXIST throwincatch_r.reflect.xml set ReflectionRoots=-x throwincatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
