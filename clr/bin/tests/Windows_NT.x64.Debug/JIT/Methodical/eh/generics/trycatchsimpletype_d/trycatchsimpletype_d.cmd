...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchsimpletype_d trycatchsimpletype_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchsimpletype_d trycatchsimpletype_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchsimpletype_d.dasm.il trycatchsimpletype_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchsimpletype_d.dasm.il trycatchsimpletype_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchsimpletype_d.asm.exe  trycatchsimpletype_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchsimpletype_d.asm.exe  trycatchsimpletype_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchsimpletype_d
    if not exist "trycatchsimpletype_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchsimpletype_d.org" (
            mkdir IL
            copy trycatchsimpletype_d.exe IL\trycatchsimpletype_d.exe 
            ren trycatchsimpletype_d.exe trycatchsimpletype_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchsimpletype_d.org /out trycatchsimpletype_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchsimpletype_d.org /out trycatchsimpletype_d.exe
...
set ExePath=trycatchsimpletype_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchsimpletype_d.reflect.xml
    IF EXIST trycatchsimpletype_d.reflect.xml set ReflectionRoots=-x trycatchsimpletype_d.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchsimpletype_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchsimpletype_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchsimpletype_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
