...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchsimpletype_ro trycatchsimpletype_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchsimpletype_ro trycatchsimpletype_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchsimpletype_ro.dasm.il trycatchsimpletype_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchsimpletype_ro.dasm.il trycatchsimpletype_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchsimpletype_ro.asm.exe  trycatchsimpletype_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchsimpletype_ro.asm.exe  trycatchsimpletype_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchsimpletype_ro
    if [ ! -f trycatchsimpletype_ro.org ]; then
        TakeLock
        if [ ! -f trycatchsimpletype_ro.org ]; then
          mkdir IL
          cp trycatchsimpletype_ro.exe IL/trycatchsimpletype_ro.exe
          mv trycatchsimpletype_ro.exe trycatchsimpletype_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchsimpletype_ro.org /out trycatchsimpletype_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchsimpletype_ro.org /out trycatchsimpletype_ro.exe
          __cgExitCode=$?
...
ExePath=trycatchsimpletype_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchsimpletype_ro.reflect.xml
  if [ -f trycatchsimpletype_ro.reflect.xml ]
then
    ReflectionRoots="-x trycatchsimpletype_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchsimpletype_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchsimpletype_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchsimpletype_ro.asm.exe $CLRTestExecutionArguments
