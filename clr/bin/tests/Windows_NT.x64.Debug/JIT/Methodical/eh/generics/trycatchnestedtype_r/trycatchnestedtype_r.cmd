...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchnestedtype_r trycatchnestedtype_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchnestedtype_r trycatchnestedtype_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchnestedtype_r.dasm.il trycatchnestedtype_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchnestedtype_r.dasm.il trycatchnestedtype_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchnestedtype_r.asm.exe  trycatchnestedtype_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchnestedtype_r.asm.exe  trycatchnestedtype_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchnestedtype_r
    if not exist "trycatchnestedtype_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchnestedtype_r.org" (
            mkdir IL
            copy trycatchnestedtype_r.exe IL\trycatchnestedtype_r.exe 
            ren trycatchnestedtype_r.exe trycatchnestedtype_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchnestedtype_r.org /out trycatchnestedtype_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchnestedtype_r.org /out trycatchnestedtype_r.exe
...
set ExePath=trycatchnestedtype_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchnestedtype_r.reflect.xml
    IF EXIST trycatchnestedtype_r.reflect.xml set ReflectionRoots=-x trycatchnestedtype_r.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchnestedtype_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchnestedtype_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchnestedtype_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
