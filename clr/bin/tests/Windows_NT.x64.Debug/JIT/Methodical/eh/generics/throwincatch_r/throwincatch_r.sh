...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/throwincatch_r throwincatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/throwincatch_r throwincatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincatch_r.dasm.il throwincatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincatch_r.dasm.il throwincatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincatch_r.asm.exe  throwincatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincatch_r.asm.exe  throwincatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincatch_r
    if [ ! -f throwincatch_r.org ]; then
        TakeLock
        if [ ! -f throwincatch_r.org ]; then
          mkdir IL
          cp throwincatch_r.exe IL/throwincatch_r.exe
          mv throwincatch_r.exe throwincatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincatch_r.org /out throwincatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincatch_r.org /out throwincatch_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwincatch_r.reflect.xml
  if [ -f throwincatch_r.reflect.xml ]
then
    ReflectionRoots="-x throwincatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincatch_r.asm.exe $CLRTestExecutionArguments
