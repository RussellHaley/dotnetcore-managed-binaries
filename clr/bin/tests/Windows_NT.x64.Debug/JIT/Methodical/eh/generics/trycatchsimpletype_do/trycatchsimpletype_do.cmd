...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchsimpletype_do trycatchsimpletype_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchsimpletype_do trycatchsimpletype_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchsimpletype_do.dasm.il trycatchsimpletype_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchsimpletype_do.dasm.il trycatchsimpletype_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchsimpletype_do.asm.exe  trycatchsimpletype_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchsimpletype_do.asm.exe  trycatchsimpletype_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchsimpletype_do
    if not exist "trycatchsimpletype_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchsimpletype_do.org" (
            mkdir IL
            copy trycatchsimpletype_do.exe IL\trycatchsimpletype_do.exe 
            ren trycatchsimpletype_do.exe trycatchsimpletype_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchsimpletype_do.org /out trycatchsimpletype_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchsimpletype_do.org /out trycatchsimpletype_do.exe
...
set ExePath=trycatchsimpletype_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchsimpletype_do.reflect.xml
    IF EXIST trycatchsimpletype_do.reflect.xml set ReflectionRoots=-x trycatchsimpletype_do.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchsimpletype_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchsimpletype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchsimpletype_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
