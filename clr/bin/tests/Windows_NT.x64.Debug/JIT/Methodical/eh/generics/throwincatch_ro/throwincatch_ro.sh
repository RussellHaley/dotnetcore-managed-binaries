...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/throwincatch_ro throwincatch_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/throwincatch_ro throwincatch_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwincatch_ro.dasm.il throwincatch_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwincatch_ro.dasm.il throwincatch_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwincatch_ro.asm.exe  throwincatch_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=throwincatch_ro.asm.exe  throwincatch_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwincatch_ro
    if [ ! -f throwincatch_ro.org ]; then
        TakeLock
        if [ ! -f throwincatch_ro.org ]; then
          mkdir IL
          cp throwincatch_ro.exe IL/throwincatch_ro.exe
          mv throwincatch_ro.exe throwincatch_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincatch_ro.org /out throwincatch_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwincatch_ro.org /out throwincatch_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwincatch_ro.reflect.xml
  if [ -f throwincatch_ro.reflect.xml ]
then
    ReflectionRoots="-x throwincatch_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwincatch_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwincatch_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwincatch_ro.asm.exe $CLRTestExecutionArguments
