...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\throwincatch_do throwincatch_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\throwincatch_do throwincatch_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincatch_do.dasm.il throwincatch_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincatch_do.dasm.il throwincatch_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincatch_do.asm.exe  throwincatch_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincatch_do.asm.exe  throwincatch_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincatch_do
    if not exist "throwincatch_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincatch_do.org" (
            mkdir IL
            copy throwincatch_do.exe IL\throwincatch_do.exe 
            ren throwincatch_do.exe throwincatch_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincatch_do.org /out throwincatch_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincatch_do.org /out throwincatch_do.exe
...
set ExePath=throwincatch_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincatch_do.reflect.xml
    IF EXIST throwincatch_do.reflect.xml set ReflectionRoots=-x throwincatch_do.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincatch_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincatch_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincatch_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
