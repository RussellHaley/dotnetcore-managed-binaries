...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchnestedtype_ro trycatchnestedtype_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchnestedtype_ro trycatchnestedtype_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchnestedtype_ro.dasm.il trycatchnestedtype_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchnestedtype_ro.dasm.il trycatchnestedtype_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchnestedtype_ro.asm.exe  trycatchnestedtype_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchnestedtype_ro.asm.exe  trycatchnestedtype_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchnestedtype_ro
    if not exist "trycatchnestedtype_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchnestedtype_ro.org" (
            mkdir IL
            copy trycatchnestedtype_ro.exe IL\trycatchnestedtype_ro.exe 
            ren trycatchnestedtype_ro.exe trycatchnestedtype_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchnestedtype_ro.org /out trycatchnestedtype_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchnestedtype_ro.org /out trycatchnestedtype_ro.exe
...
set ExePath=trycatchnestedtype_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchnestedtype_ro.reflect.xml
    IF EXIST trycatchnestedtype_ro.reflect.xml set ReflectionRoots=-x trycatchnestedtype_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchnestedtype_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchnestedtype_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchnestedtype_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
