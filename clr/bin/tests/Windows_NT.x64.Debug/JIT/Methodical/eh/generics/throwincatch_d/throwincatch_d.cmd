...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\throwincatch_d throwincatch_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\throwincatch_d throwincatch_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwincatch_d.dasm.il throwincatch_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwincatch_d.dasm.il throwincatch_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwincatch_d.asm.exe  throwincatch_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwincatch_d.asm.exe  throwincatch_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwincatch_d
    if not exist "throwincatch_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwincatch_d.org" (
            mkdir IL
            copy throwincatch_d.exe IL\throwincatch_d.exe 
            ren throwincatch_d.exe throwincatch_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincatch_d.org /out throwincatch_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwincatch_d.org /out throwincatch_d.exe
...
set ExePath=throwincatch_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwincatch_d.reflect.xml
    IF EXIST throwincatch_d.reflect.xml set ReflectionRoots=-x throwincatch_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwincatch_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwincatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwincatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
