...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchsimpletype_d trycatchsimpletype_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchsimpletype_d trycatchsimpletype_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchsimpletype_d.dasm.il trycatchsimpletype_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchsimpletype_d.dasm.il trycatchsimpletype_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchsimpletype_d.asm.exe  trycatchsimpletype_d.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchsimpletype_d.asm.exe  trycatchsimpletype_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchsimpletype_d
    if [ ! -f trycatchsimpletype_d.org ]; then
        TakeLock
        if [ ! -f trycatchsimpletype_d.org ]; then
          mkdir IL
          cp trycatchsimpletype_d.exe IL/trycatchsimpletype_d.exe
          mv trycatchsimpletype_d.exe trycatchsimpletype_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchsimpletype_d.org /out trycatchsimpletype_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchsimpletype_d.org /out trycatchsimpletype_d.exe
          __cgExitCode=$?
...
ExePath=trycatchsimpletype_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchsimpletype_d.reflect.xml
  if [ -f trycatchsimpletype_d.reflect.xml ]
then
    ReflectionRoots="-x trycatchsimpletype_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchsimpletype_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchsimpletype_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchsimpletype_d.asm.exe $CLRTestExecutionArguments
