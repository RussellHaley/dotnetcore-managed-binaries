...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchsimpletype_r trycatchsimpletype_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\generics\trycatchsimpletype_r trycatchsimpletype_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatchsimpletype_r.dasm.il trycatchsimpletype_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatchsimpletype_r.dasm.il trycatchsimpletype_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatchsimpletype_r.asm.exe  trycatchsimpletype_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatchsimpletype_r.asm.exe  trycatchsimpletype_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatchsimpletype_r
    if not exist "trycatchsimpletype_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatchsimpletype_r.org" (
            mkdir IL
            copy trycatchsimpletype_r.exe IL\trycatchsimpletype_r.exe 
            ren trycatchsimpletype_r.exe trycatchsimpletype_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchsimpletype_r.org /out trycatchsimpletype_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatchsimpletype_r.org /out trycatchsimpletype_r.exe
...
set ExePath=trycatchsimpletype_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatchsimpletype_r.reflect.xml
    IF EXIST trycatchsimpletype_r.reflect.xml set ReflectionRoots=-x trycatchsimpletype_r.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatchsimpletype_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatchsimpletype_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatchsimpletype_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
