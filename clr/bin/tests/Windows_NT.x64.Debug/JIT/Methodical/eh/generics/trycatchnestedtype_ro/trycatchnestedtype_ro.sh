...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchnestedtype_ro trycatchnestedtype_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/generics/trycatchnestedtype_ro trycatchnestedtype_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatchnestedtype_ro.dasm.il trycatchnestedtype_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatchnestedtype_ro.dasm.il trycatchnestedtype_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatchnestedtype_ro.asm.exe  trycatchnestedtype_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatchnestedtype_ro.asm.exe  trycatchnestedtype_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatchnestedtype_ro
    if [ ! -f trycatchnestedtype_ro.org ]; then
        TakeLock
        if [ ! -f trycatchnestedtype_ro.org ]; then
          mkdir IL
          cp trycatchnestedtype_ro.exe IL/trycatchnestedtype_ro.exe
          mv trycatchnestedtype_ro.exe trycatchnestedtype_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchnestedtype_ro.org /out trycatchnestedtype_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatchnestedtype_ro.org /out trycatchnestedtype_ro.exe
          __cgExitCode=$?
...
ExePath=trycatchnestedtype_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in trycatchnestedtype_ro.reflect.xml
  if [ -f trycatchnestedtype_ro.reflect.xml ]
then
    ReflectionRoots="-x trycatchnestedtype_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatchnestedtype_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatchnestedtype_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatchnestedtype_ro.asm.exe $CLRTestExecutionArguments
