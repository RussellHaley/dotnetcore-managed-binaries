...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/multihandler_r multihandler_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/multihandler_r multihandler_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=multihandler_r.dasm.il multihandler_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=multihandler_r.dasm.il multihandler_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=multihandler_r.asm.exe  multihandler_r.dasm.il
    "$CORE_ROOT/ilasm" -output=multihandler_r.asm.exe  multihandler_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=multihandler_r
    if [ ! -f multihandler_r.org ]; then
        TakeLock
        if [ ! -f multihandler_r.org ]; then
          mkdir IL
          cp multihandler_r.exe IL/multihandler_r.exe
          mv multihandler_r.exe multihandler_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multihandler_r.org /out multihandler_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multihandler_r.org /out multihandler_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in multihandler_r.reflect.xml
  if [ -f multihandler_r.reflect.xml ]
then
    ReflectionRoots="-x multihandler_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/multihandler_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" multihandler_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" multihandler_r.asm.exe $CLRTestExecutionArguments
