...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinfault_r throwinfault_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinfault_r throwinfault_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfault_r.dasm.il throwinfault_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfault_r.dasm.il throwinfault_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfault_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinfault_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfault_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinfault_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfault_r
    if [ ! -f throwinfault_r.org ]; then
        TakeLock
        if [ ! -f throwinfault_r.org ]; then
          mkdir IL
          cp throwinfault_r.exe IL/throwinfault_r.exe
          mv throwinfault_r.exe throwinfault_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfault_r.org /out throwinfault_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfault_r.org /out throwinfault_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfault_r.reflect.xml
  if [ -f throwinfault_r.reflect.xml ]
then
    ReflectionRoots="-x throwinfault_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfault_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfault_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfault_r.asm.exe $CLRTestExecutionArguments
