...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinfilter_d throwinfilter_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinfilter_d throwinfilter_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfilter_d.dasm.il throwinfilter_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfilter_d.dasm.il throwinfilter_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfilter_d.asm.exe -QUIET -NOLOGO -DEBUG throwinfilter_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfilter_d.asm.exe -QUIET -NOLOGO -DEBUG throwinfilter_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfilter_d
    if [ ! -f throwinfilter_d.org ]; then
        TakeLock
        if [ ! -f throwinfilter_d.org ]; then
          mkdir IL
          cp throwinfilter_d.exe IL/throwinfilter_d.exe
          mv throwinfilter_d.exe throwinfilter_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfilter_d.org /out throwinfilter_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfilter_d.org /out throwinfilter_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfilter_d.reflect.xml
  if [ -f throwinfilter_d.reflect.xml ]
then
    ReflectionRoots="-x throwinfilter_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfilter_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfilter_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfilter_d.asm.exe $CLRTestExecutionArguments
