...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfaulttrycatchfn_d tryfaulttrycatchfn_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfaulttrycatchfn_d tryfaulttrycatchfn_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tryfaulttrycatchfn_d.dasm.il tryfaulttrycatchfn_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=tryfaulttrycatchfn_d.dasm.il tryfaulttrycatchfn_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tryfaulttrycatchfn_d.asm.exe -QUIET -NOLOGO -DEBUG tryfaulttrycatchfn_d.dasm.il
    "$CORE_ROOT/ilasm" -output=tryfaulttrycatchfn_d.asm.exe -QUIET -NOLOGO -DEBUG tryfaulttrycatchfn_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tryfaulttrycatchfn_d
    if [ ! -f tryfaulttrycatchfn_d.org ]; then
        TakeLock
        if [ ! -f tryfaulttrycatchfn_d.org ]; then
          mkdir IL
          cp tryfaulttrycatchfn_d.exe IL/tryfaulttrycatchfn_d.exe
          mv tryfaulttrycatchfn_d.exe tryfaulttrycatchfn_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfaulttrycatchfn_d.org /out tryfaulttrycatchfn_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfaulttrycatchfn_d.org /out tryfaulttrycatchfn_d.exe
          __cgExitCode=$?
...
ExePath=tryfaulttrycatchfn_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in tryfaulttrycatchfn_d.reflect.xml
  if [ -f tryfaulttrycatchfn_d.reflect.xml ]
then
    ReflectionRoots="-x tryfaulttrycatchfn_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/tryfaulttrycatchfn_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tryfaulttrycatchfn_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tryfaulttrycatchfn_d.asm.exe $CLRTestExecutionArguments
