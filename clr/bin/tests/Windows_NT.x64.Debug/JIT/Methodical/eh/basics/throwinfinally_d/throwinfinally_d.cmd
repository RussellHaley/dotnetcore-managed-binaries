...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfinally_d throwinfinally_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfinally_d throwinfinally_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_d.dasm.il throwinfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_d.dasm.il throwinfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinally_d.asm.exe  throwinfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinally_d.asm.exe  throwinfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinally_d
    if not exist "throwinfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinally_d.org" (
            mkdir IL
            copy throwinfinally_d.exe IL\throwinfinally_d.exe 
            ren throwinfinally_d.exe throwinfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_d.org /out throwinfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_d.org /out throwinfinally_d.exe
...
set ExePath=throwinfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinally_d.reflect.xml
    IF EXIST throwinfinally_d.reflect.xml set ReflectionRoots=-x throwinfinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
