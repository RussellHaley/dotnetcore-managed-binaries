...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfault_r throwinfault_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfault_r throwinfault_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfault_r.dasm.il throwinfault_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfault_r.dasm.il throwinfault_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfault_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinfault_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfault_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinfault_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfault_r
    if not exist "throwinfault_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfault_r.org" (
            mkdir IL
            copy throwinfault_r.exe IL\throwinfault_r.exe 
            ren throwinfault_r.exe throwinfault_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfault_r.org /out throwinfault_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfault_r.org /out throwinfault_r.exe
...
set ExePath=throwinfault_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfault_r.reflect.xml
    IF EXIST throwinfault_r.reflect.xml set ReflectionRoots=-x throwinfault_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfault_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfault_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfault_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
