...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfilter_d throwinfilter_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfilter_d throwinfilter_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfilter_d.dasm.il throwinfilter_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfilter_d.dasm.il throwinfilter_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfilter_d.asm.exe -QUIET -NOLOGO -DEBUG throwinfilter_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfilter_d.asm.exe -QUIET -NOLOGO -DEBUG throwinfilter_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfilter_d
    if not exist "throwinfilter_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfilter_d.org" (
            mkdir IL
            copy throwinfilter_d.exe IL\throwinfilter_d.exe 
            ren throwinfilter_d.exe throwinfilter_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfilter_d.org /out throwinfilter_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfilter_d.org /out throwinfilter_d.exe
...
set ExePath=throwinfilter_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfilter_d.reflect.xml
    IF EXIST throwinfilter_d.reflect.xml set ReflectionRoots=-x throwinfilter_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfilter_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfilter_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfilter_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
