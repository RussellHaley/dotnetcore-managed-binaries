...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/trythrowexcept_r trythrowexcept_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/trythrowexcept_r trythrowexcept_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trythrowexcept_r.dasm.il trythrowexcept_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=trythrowexcept_r.dasm.il trythrowexcept_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trythrowexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT trythrowexcept_r.dasm.il
    "$CORE_ROOT/ilasm" -output=trythrowexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT trythrowexcept_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trythrowexcept_r
    if [ ! -f trythrowexcept_r.org ]; then
        TakeLock
        if [ ! -f trythrowexcept_r.org ]; then
          mkdir IL
          cp trythrowexcept_r.exe IL/trythrowexcept_r.exe
          mv trythrowexcept_r.exe trythrowexcept_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trythrowexcept_r.org /out trythrowexcept_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trythrowexcept_r.org /out trythrowexcept_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in trythrowexcept_r.reflect.xml
  if [ -f trythrowexcept_r.reflect.xml ]
then
    ReflectionRoots="-x trythrowexcept_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trythrowexcept_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trythrowexcept_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trythrowexcept_r.asm.exe $CLRTestExecutionArguments
