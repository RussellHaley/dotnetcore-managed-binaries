...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfinallytrycatch_r tryfinallytrycatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfinallytrycatch_r tryfinallytrycatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tryfinallytrycatch_r.dasm.il tryfinallytrycatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=tryfinallytrycatch_r.dasm.il tryfinallytrycatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tryfinallytrycatch_r.asm.exe  tryfinallytrycatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=tryfinallytrycatch_r.asm.exe  tryfinallytrycatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tryfinallytrycatch_r
    if [ ! -f tryfinallytrycatch_r.org ]; then
        TakeLock
        if [ ! -f tryfinallytrycatch_r.org ]; then
          mkdir IL
          cp tryfinallytrycatch_r.exe IL/tryfinallytrycatch_r.exe
          mv tryfinallytrycatch_r.exe tryfinallytrycatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfinallytrycatch_r.org /out tryfinallytrycatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfinallytrycatch_r.org /out tryfinallytrycatch_r.exe
          __cgExitCode=$?
...
ExePath=tryfinallytrycatch_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in tryfinallytrycatch_r.reflect.xml
  if [ -f tryfinallytrycatch_r.reflect.xml ]
then
    ReflectionRoots="-x tryfinallytrycatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/tryfinallytrycatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tryfinallytrycatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tryfinallytrycatch_r.asm.exe $CLRTestExecutionArguments
