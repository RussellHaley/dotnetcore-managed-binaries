...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/trythrowexcept_d trythrowexcept_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/trythrowexcept_d trythrowexcept_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trythrowexcept_d.dasm.il trythrowexcept_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=trythrowexcept_d.dasm.il trythrowexcept_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trythrowexcept_d.asm.exe -QUIET -NOLOGO -DEBUG trythrowexcept_d.dasm.il
    "$CORE_ROOT/ilasm" -output=trythrowexcept_d.asm.exe -QUIET -NOLOGO -DEBUG trythrowexcept_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trythrowexcept_d
    if [ ! -f trythrowexcept_d.org ]; then
        TakeLock
        if [ ! -f trythrowexcept_d.org ]; then
          mkdir IL
          cp trythrowexcept_d.exe IL/trythrowexcept_d.exe
          mv trythrowexcept_d.exe trythrowexcept_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trythrowexcept_d.org /out trythrowexcept_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trythrowexcept_d.org /out trythrowexcept_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in trythrowexcept_d.reflect.xml
  if [ -f trythrowexcept_d.reflect.xml ]
then
    ReflectionRoots="-x trythrowexcept_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trythrowexcept_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trythrowexcept_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trythrowexcept_d.asm.exe $CLRTestExecutionArguments
