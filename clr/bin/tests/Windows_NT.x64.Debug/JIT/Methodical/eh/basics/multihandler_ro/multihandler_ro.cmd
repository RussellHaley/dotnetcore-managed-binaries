...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\multihandler_ro multihandler_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\multihandler_ro multihandler_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=multihandler_ro.dasm.il multihandler_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=multihandler_ro.dasm.il multihandler_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=multihandler_ro.asm.exe  multihandler_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=multihandler_ro.asm.exe  multihandler_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=multihandler_ro
    if not exist "multihandler_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "multihandler_ro.org" (
            mkdir IL
            copy multihandler_ro.exe IL\multihandler_ro.exe 
            ren multihandler_ro.exe multihandler_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multihandler_ro.org /out multihandler_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multihandler_ro.org /out multihandler_ro.exe
...
set ExePath=multihandler_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in multihandler_ro.reflect.xml
    IF EXIST multihandler_ro.reflect.xml set ReflectionRoots=-x multihandler_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\multihandler_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% multihandler_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% multihandler_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
