...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfaulttrycatch_r tryfaulttrycatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfaulttrycatch_r tryfaulttrycatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tryfaulttrycatch_r.dasm.il tryfaulttrycatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=tryfaulttrycatch_r.dasm.il tryfaulttrycatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tryfaulttrycatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT tryfaulttrycatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=tryfaulttrycatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT tryfaulttrycatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tryfaulttrycatch_r
    if [ ! -f tryfaulttrycatch_r.org ]; then
        TakeLock
        if [ ! -f tryfaulttrycatch_r.org ]; then
          mkdir IL
          cp tryfaulttrycatch_r.exe IL/tryfaulttrycatch_r.exe
          mv tryfaulttrycatch_r.exe tryfaulttrycatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfaulttrycatch_r.org /out tryfaulttrycatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfaulttrycatch_r.org /out tryfaulttrycatch_r.exe
          __cgExitCode=$?
...
ExePath=tryfaulttrycatch_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in tryfaulttrycatch_r.reflect.xml
  if [ -f tryfaulttrycatch_r.reflect.xml ]
then
    ReflectionRoots="-x tryfaulttrycatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/tryfaulttrycatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tryfaulttrycatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tryfaulttrycatch_r.asm.exe $CLRTestExecutionArguments
