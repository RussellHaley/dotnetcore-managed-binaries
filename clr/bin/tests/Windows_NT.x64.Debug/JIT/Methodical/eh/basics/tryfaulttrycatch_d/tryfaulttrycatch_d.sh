...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfaulttrycatch_d tryfaulttrycatch_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfaulttrycatch_d tryfaulttrycatch_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tryfaulttrycatch_d.dasm.il tryfaulttrycatch_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=tryfaulttrycatch_d.dasm.il tryfaulttrycatch_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tryfaulttrycatch_d.asm.exe -QUIET -NOLOGO -DEBUG tryfaulttrycatch_d.dasm.il
    "$CORE_ROOT/ilasm" -output=tryfaulttrycatch_d.asm.exe -QUIET -NOLOGO -DEBUG tryfaulttrycatch_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tryfaulttrycatch_d
    if [ ! -f tryfaulttrycatch_d.org ]; then
        TakeLock
        if [ ! -f tryfaulttrycatch_d.org ]; then
          mkdir IL
          cp tryfaulttrycatch_d.exe IL/tryfaulttrycatch_d.exe
          mv tryfaulttrycatch_d.exe tryfaulttrycatch_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfaulttrycatch_d.org /out tryfaulttrycatch_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfaulttrycatch_d.org /out tryfaulttrycatch_d.exe
          __cgExitCode=$?
...
ExePath=tryfaulttrycatch_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in tryfaulttrycatch_d.reflect.xml
  if [ -f tryfaulttrycatch_d.reflect.xml ]
then
    ReflectionRoots="-x tryfaulttrycatch_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/tryfaulttrycatch_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tryfaulttrycatch_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tryfaulttrycatch_d.asm.exe $CLRTestExecutionArguments
