...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\trythrowexcept_r trythrowexcept_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\trythrowexcept_r trythrowexcept_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trythrowexcept_r.dasm.il trythrowexcept_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trythrowexcept_r.dasm.il trythrowexcept_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trythrowexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT trythrowexcept_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trythrowexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT trythrowexcept_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trythrowexcept_r
    if not exist "trythrowexcept_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trythrowexcept_r.org" (
            mkdir IL
            copy trythrowexcept_r.exe IL\trythrowexcept_r.exe 
            ren trythrowexcept_r.exe trythrowexcept_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trythrowexcept_r.org /out trythrowexcept_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trythrowexcept_r.org /out trythrowexcept_r.exe
...
set ExePath=trythrowexcept_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trythrowexcept_r.reflect.xml
    IF EXIST trythrowexcept_r.reflect.xml set ReflectionRoots=-x trythrowexcept_r.reflect.xml
    ...
    set ExePath=%LinkBin%\trythrowexcept_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trythrowexcept_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trythrowexcept_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
