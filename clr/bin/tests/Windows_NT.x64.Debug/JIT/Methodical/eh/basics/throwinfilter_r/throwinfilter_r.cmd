...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfilter_r throwinfilter_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfilter_r throwinfilter_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfilter_r.dasm.il throwinfilter_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfilter_r.dasm.il throwinfilter_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfilter_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinfilter_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfilter_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinfilter_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfilter_r
    if not exist "throwinfilter_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfilter_r.org" (
            mkdir IL
            copy throwinfilter_r.exe IL\throwinfilter_r.exe 
            ren throwinfilter_r.exe throwinfilter_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfilter_r.org /out throwinfilter_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfilter_r.org /out throwinfilter_r.exe
...
set ExePath=throwinfilter_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfilter_r.reflect.xml
    IF EXIST throwinfilter_r.reflect.xml set ReflectionRoots=-x throwinfilter_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfilter_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfilter_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfilter_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
