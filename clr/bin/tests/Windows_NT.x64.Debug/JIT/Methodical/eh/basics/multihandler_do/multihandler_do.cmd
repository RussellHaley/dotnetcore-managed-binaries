...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\multihandler_do multihandler_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\multihandler_do multihandler_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=multihandler_do.dasm.il multihandler_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=multihandler_do.dasm.il multihandler_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=multihandler_do.asm.exe  multihandler_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=multihandler_do.asm.exe  multihandler_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=multihandler_do
    if not exist "multihandler_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "multihandler_do.org" (
            mkdir IL
            copy multihandler_do.exe IL\multihandler_do.exe 
            ren multihandler_do.exe multihandler_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multihandler_do.org /out multihandler_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multihandler_do.org /out multihandler_do.exe
...
set ExePath=multihandler_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in multihandler_do.reflect.xml
    IF EXIST multihandler_do.reflect.xml set ReflectionRoots=-x multihandler_do.reflect.xml
    ...
    set ExePath=%LinkBin%\multihandler_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% multihandler_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% multihandler_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
