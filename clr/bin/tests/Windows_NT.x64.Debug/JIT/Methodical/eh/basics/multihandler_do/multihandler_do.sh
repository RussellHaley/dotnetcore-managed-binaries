...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/multihandler_do multihandler_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/multihandler_do multihandler_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=multihandler_do.dasm.il multihandler_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=multihandler_do.dasm.il multihandler_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=multihandler_do.asm.exe  multihandler_do.dasm.il
    "$CORE_ROOT/ilasm" -output=multihandler_do.asm.exe  multihandler_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=multihandler_do
    if [ ! -f multihandler_do.org ]; then
        TakeLock
        if [ ! -f multihandler_do.org ]; then
          mkdir IL
          cp multihandler_do.exe IL/multihandler_do.exe
          mv multihandler_do.exe multihandler_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multihandler_do.org /out multihandler_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multihandler_do.org /out multihandler_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in multihandler_do.reflect.xml
  if [ -f multihandler_do.reflect.xml ]
then
    ReflectionRoots="-x multihandler_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/multihandler_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" multihandler_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" multihandler_do.asm.exe $CLRTestExecutionArguments
