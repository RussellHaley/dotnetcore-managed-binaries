...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfaulttrycatch_r tryfaulttrycatch_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfaulttrycatch_r tryfaulttrycatch_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tryfaulttrycatch_r.dasm.il tryfaulttrycatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tryfaulttrycatch_r.dasm.il tryfaulttrycatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tryfaulttrycatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT tryfaulttrycatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tryfaulttrycatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT tryfaulttrycatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tryfaulttrycatch_r
    if not exist "tryfaulttrycatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tryfaulttrycatch_r.org" (
            mkdir IL
            copy tryfaulttrycatch_r.exe IL\tryfaulttrycatch_r.exe 
            ren tryfaulttrycatch_r.exe tryfaulttrycatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfaulttrycatch_r.org /out tryfaulttrycatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfaulttrycatch_r.org /out tryfaulttrycatch_r.exe
...
set ExePath=tryfaulttrycatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tryfaulttrycatch_r.reflect.xml
    IF EXIST tryfaulttrycatch_r.reflect.xml set ReflectionRoots=-x tryfaulttrycatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\tryfaulttrycatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tryfaulttrycatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tryfaulttrycatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
