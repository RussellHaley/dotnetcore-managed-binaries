...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinexcept_d throwinexcept_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinexcept_d throwinexcept_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinexcept_d.dasm.il throwinexcept_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinexcept_d.dasm.il throwinexcept_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinexcept_d.asm.exe -QUIET -NOLOGO -DEBUG throwinexcept_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinexcept_d.asm.exe -QUIET -NOLOGO -DEBUG throwinexcept_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinexcept_d
    if [ ! -f throwinexcept_d.org ]; then
        TakeLock
        if [ ! -f throwinexcept_d.org ]; then
          mkdir IL
          cp throwinexcept_d.exe IL/throwinexcept_d.exe
          mv throwinexcept_d.exe throwinexcept_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinexcept_d.org /out throwinexcept_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinexcept_d.org /out throwinexcept_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinexcept_d.reflect.xml
  if [ -f throwinexcept_d.reflect.xml ]
then
    ReflectionRoots="-x throwinexcept_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinexcept_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinexcept_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinexcept_d.asm.exe $CLRTestExecutionArguments
