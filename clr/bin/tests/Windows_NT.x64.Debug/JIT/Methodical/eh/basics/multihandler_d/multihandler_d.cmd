...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\multihandler_d multihandler_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\multihandler_d multihandler_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=multihandler_d.dasm.il multihandler_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=multihandler_d.dasm.il multihandler_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=multihandler_d.asm.exe  multihandler_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=multihandler_d.asm.exe  multihandler_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=multihandler_d
    if not exist "multihandler_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "multihandler_d.org" (
            mkdir IL
            copy multihandler_d.exe IL\multihandler_d.exe 
            ren multihandler_d.exe multihandler_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multihandler_d.org /out multihandler_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multihandler_d.org /out multihandler_d.exe
...
set ExePath=multihandler_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in multihandler_d.reflect.xml
    IF EXIST multihandler_d.reflect.xml set ReflectionRoots=-x multihandler_d.reflect.xml
    ...
    set ExePath=%LinkBin%\multihandler_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% multihandler_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% multihandler_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
