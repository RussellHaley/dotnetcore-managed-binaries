...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfaulttrycatchfn_r tryfaulttrycatchfn_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/tryfaulttrycatchfn_r tryfaulttrycatchfn_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tryfaulttrycatchfn_r.dasm.il tryfaulttrycatchfn_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=tryfaulttrycatchfn_r.dasm.il tryfaulttrycatchfn_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tryfaulttrycatchfn_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT tryfaulttrycatchfn_r.dasm.il
    "$CORE_ROOT/ilasm" -output=tryfaulttrycatchfn_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT tryfaulttrycatchfn_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tryfaulttrycatchfn_r
    if [ ! -f tryfaulttrycatchfn_r.org ]; then
        TakeLock
        if [ ! -f tryfaulttrycatchfn_r.org ]; then
          mkdir IL
          cp tryfaulttrycatchfn_r.exe IL/tryfaulttrycatchfn_r.exe
          mv tryfaulttrycatchfn_r.exe tryfaulttrycatchfn_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfaulttrycatchfn_r.org /out tryfaulttrycatchfn_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfaulttrycatchfn_r.org /out tryfaulttrycatchfn_r.exe
          __cgExitCode=$?
...
ExePath=tryfaulttrycatchfn_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in tryfaulttrycatchfn_r.reflect.xml
  if [ -f tryfaulttrycatchfn_r.reflect.xml ]
then
    ReflectionRoots="-x tryfaulttrycatchfn_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/tryfaulttrycatchfn_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tryfaulttrycatchfn_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tryfaulttrycatchfn_r.asm.exe $CLRTestExecutionArguments
