...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinexcept_r throwinexcept_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinexcept_r throwinexcept_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinexcept_r.dasm.il throwinexcept_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinexcept_r.dasm.il throwinexcept_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinexcept_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinexcept_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinexcept_r
    if [ ! -f throwinexcept_r.org ]; then
        TakeLock
        if [ ! -f throwinexcept_r.org ]; then
          mkdir IL
          cp throwinexcept_r.exe IL/throwinexcept_r.exe
          mv throwinexcept_r.exe throwinexcept_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinexcept_r.org /out throwinexcept_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinexcept_r.org /out throwinexcept_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinexcept_r.reflect.xml
  if [ -f throwinexcept_r.reflect.xml ]
then
    ReflectionRoots="-x throwinexcept_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinexcept_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinexcept_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinexcept_r.asm.exe $CLRTestExecutionArguments
