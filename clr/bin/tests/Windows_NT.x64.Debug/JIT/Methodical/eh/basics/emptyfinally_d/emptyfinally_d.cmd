...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\emptyfinally_d emptyfinally_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\emptyfinally_d emptyfinally_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=emptyfinally_d.dasm.il emptyfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=emptyfinally_d.dasm.il emptyfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=emptyfinally_d.asm.exe -QUIET -NOLOGO -DEBUG emptyfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=emptyfinally_d.asm.exe -QUIET -NOLOGO -DEBUG emptyfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=emptyfinally_d
    if not exist "emptyfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "emptyfinally_d.org" (
            mkdir IL
            copy emptyfinally_d.exe IL\emptyfinally_d.exe 
            ren emptyfinally_d.exe emptyfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in emptyfinally_d.org /out emptyfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in emptyfinally_d.org /out emptyfinally_d.exe
...
set ExePath=emptyfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in emptyfinally_d.reflect.xml
    IF EXIST emptyfinally_d.reflect.xml set ReflectionRoots=-x emptyfinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\emptyfinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% emptyfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% emptyfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
