...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinclassconstructor_d throwinclassconstructor_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinclassconstructor_d throwinclassconstructor_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinclassconstructor_d.dasm.il throwinclassconstructor_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinclassconstructor_d.dasm.il throwinclassconstructor_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinclassconstructor_d.asm.exe  throwinclassconstructor_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinclassconstructor_d.asm.exe  throwinclassconstructor_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinclassconstructor_d
    if not exist "throwinclassconstructor_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinclassconstructor_d.org" (
            mkdir IL
            copy throwinclassconstructor_d.exe IL\throwinclassconstructor_d.exe 
            ren throwinclassconstructor_d.exe throwinclassconstructor_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinclassconstructor_d.org /out throwinclassconstructor_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinclassconstructor_d.org /out throwinclassconstructor_d.exe
...
set ExePath=throwinclassconstructor_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinclassconstructor_d.reflect.xml
    IF EXIST throwinclassconstructor_d.reflect.xml set ReflectionRoots=-x throwinclassconstructor_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinclassconstructor_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinclassconstructor_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinclassconstructor_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
