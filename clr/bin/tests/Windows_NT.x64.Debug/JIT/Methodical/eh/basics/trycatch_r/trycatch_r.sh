...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/trycatch_r trycatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/trycatch_r trycatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=trycatch_r.dasm.il trycatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=trycatch_r.dasm.il trycatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=trycatch_r.asm.exe  trycatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=trycatch_r.asm.exe  trycatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=trycatch_r
    if [ ! -f trycatch_r.org ]; then
        TakeLock
        if [ ! -f trycatch_r.org ]; then
          mkdir IL
          cp trycatch_r.exe IL/trycatch_r.exe
          mv trycatch_r.exe trycatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatch_r.org /out trycatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in trycatch_r.org /out trycatch_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in trycatch_r.reflect.xml
  if [ -f trycatch_r.reflect.xml ]
then
    ReflectionRoots="-x trycatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/trycatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" trycatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" trycatch_r.asm.exe $CLRTestExecutionArguments
