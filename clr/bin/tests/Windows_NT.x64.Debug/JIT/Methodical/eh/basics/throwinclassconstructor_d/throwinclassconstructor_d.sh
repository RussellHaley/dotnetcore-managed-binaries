...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinclassconstructor_d throwinclassconstructor_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinclassconstructor_d throwinclassconstructor_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinclassconstructor_d.dasm.il throwinclassconstructor_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinclassconstructor_d.dasm.il throwinclassconstructor_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinclassconstructor_d.asm.exe  throwinclassconstructor_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinclassconstructor_d.asm.exe  throwinclassconstructor_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinclassconstructor_d
    if [ ! -f throwinclassconstructor_d.org ]; then
        TakeLock
        if [ ! -f throwinclassconstructor_d.org ]; then
          mkdir IL
          cp throwinclassconstructor_d.exe IL/throwinclassconstructor_d.exe
          mv throwinclassconstructor_d.exe throwinclassconstructor_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinclassconstructor_d.org /out throwinclassconstructor_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinclassconstructor_d.org /out throwinclassconstructor_d.exe
          __cgExitCode=$?
...
ExePath=throwinclassconstructor_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinclassconstructor_d.reflect.xml
  if [ -f throwinclassconstructor_d.reflect.xml ]
then
    ReflectionRoots="-x throwinclassconstructor_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinclassconstructor_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinclassconstructor_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinclassconstructor_d.asm.exe $CLRTestExecutionArguments
