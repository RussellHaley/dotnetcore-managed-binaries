...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfaulttrycatch_d tryfaulttrycatch_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfaulttrycatch_d tryfaulttrycatch_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tryfaulttrycatch_d.dasm.il tryfaulttrycatch_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tryfaulttrycatch_d.dasm.il tryfaulttrycatch_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tryfaulttrycatch_d.asm.exe -QUIET -NOLOGO -DEBUG tryfaulttrycatch_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tryfaulttrycatch_d.asm.exe -QUIET -NOLOGO -DEBUG tryfaulttrycatch_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tryfaulttrycatch_d
    if not exist "tryfaulttrycatch_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tryfaulttrycatch_d.org" (
            mkdir IL
            copy tryfaulttrycatch_d.exe IL\tryfaulttrycatch_d.exe 
            ren tryfaulttrycatch_d.exe tryfaulttrycatch_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfaulttrycatch_d.org /out tryfaulttrycatch_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfaulttrycatch_d.org /out tryfaulttrycatch_d.exe
...
set ExePath=tryfaulttrycatch_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tryfaulttrycatch_d.reflect.xml
    IF EXIST tryfaulttrycatch_d.reflect.xml set ReflectionRoots=-x tryfaulttrycatch_d.reflect.xml
    ...
    set ExePath=%LinkBin%\tryfaulttrycatch_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tryfaulttrycatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tryfaulttrycatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
