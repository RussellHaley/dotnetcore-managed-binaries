...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfaulttrycatchfn_r tryfaulttrycatchfn_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfaulttrycatchfn_r tryfaulttrycatchfn_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tryfaulttrycatchfn_r.dasm.il tryfaulttrycatchfn_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tryfaulttrycatchfn_r.dasm.il tryfaulttrycatchfn_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tryfaulttrycatchfn_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT tryfaulttrycatchfn_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tryfaulttrycatchfn_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT tryfaulttrycatchfn_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tryfaulttrycatchfn_r
    if not exist "tryfaulttrycatchfn_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tryfaulttrycatchfn_r.org" (
            mkdir IL
            copy tryfaulttrycatchfn_r.exe IL\tryfaulttrycatchfn_r.exe 
            ren tryfaulttrycatchfn_r.exe tryfaulttrycatchfn_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfaulttrycatchfn_r.org /out tryfaulttrycatchfn_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfaulttrycatchfn_r.org /out tryfaulttrycatchfn_r.exe
...
set ExePath=tryfaulttrycatchfn_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tryfaulttrycatchfn_r.reflect.xml
    IF EXIST tryfaulttrycatchfn_r.reflect.xml set ReflectionRoots=-x tryfaulttrycatchfn_r.reflect.xml
    ...
    set ExePath=%LinkBin%\tryfaulttrycatchfn_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tryfaulttrycatchfn_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tryfaulttrycatchfn_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
