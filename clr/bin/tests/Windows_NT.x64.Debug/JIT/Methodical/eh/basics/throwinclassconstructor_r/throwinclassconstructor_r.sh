...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinclassconstructor_r throwinclassconstructor_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinclassconstructor_r throwinclassconstructor_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinclassconstructor_r.dasm.il throwinclassconstructor_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinclassconstructor_r.dasm.il throwinclassconstructor_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinclassconstructor_r.asm.exe  throwinclassconstructor_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinclassconstructor_r.asm.exe  throwinclassconstructor_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinclassconstructor_r
    if [ ! -f throwinclassconstructor_r.org ]; then
        TakeLock
        if [ ! -f throwinclassconstructor_r.org ]; then
          mkdir IL
          cp throwinclassconstructor_r.exe IL/throwinclassconstructor_r.exe
          mv throwinclassconstructor_r.exe throwinclassconstructor_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinclassconstructor_r.org /out throwinclassconstructor_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinclassconstructor_r.org /out throwinclassconstructor_r.exe
          __cgExitCode=$?
...
ExePath=throwinclassconstructor_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinclassconstructor_r.reflect.xml
  if [ -f throwinclassconstructor_r.reflect.xml ]
then
    ReflectionRoots="-x throwinclassconstructor_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinclassconstructor_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinclassconstructor_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinclassconstructor_r.asm.exe $CLRTestExecutionArguments
