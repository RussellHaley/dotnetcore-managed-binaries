...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/multihandler_ro multihandler_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/multihandler_ro multihandler_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=multihandler_ro.dasm.il multihandler_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=multihandler_ro.dasm.il multihandler_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=multihandler_ro.asm.exe  multihandler_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=multihandler_ro.asm.exe  multihandler_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=multihandler_ro
    if [ ! -f multihandler_ro.org ]; then
        TakeLock
        if [ ! -f multihandler_ro.org ]; then
          mkdir IL
          cp multihandler_ro.exe IL/multihandler_ro.exe
          mv multihandler_ro.exe multihandler_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multihandler_ro.org /out multihandler_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multihandler_ro.org /out multihandler_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in multihandler_ro.reflect.xml
  if [ -f multihandler_ro.reflect.xml ]
then
    ReflectionRoots="-x multihandler_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/multihandler_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" multihandler_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" multihandler_ro.asm.exe $CLRTestExecutionArguments
