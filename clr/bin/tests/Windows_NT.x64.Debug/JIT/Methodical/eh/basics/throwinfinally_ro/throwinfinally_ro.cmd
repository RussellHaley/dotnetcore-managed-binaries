...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfinally_ro throwinfinally_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfinally_ro throwinfinally_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_ro.dasm.il throwinfinally_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_ro.dasm.il throwinfinally_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinally_ro.asm.exe  throwinfinally_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinally_ro.asm.exe  throwinfinally_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinally_ro
    if not exist "throwinfinally_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinally_ro.org" (
            mkdir IL
            copy throwinfinally_ro.exe IL\throwinfinally_ro.exe 
            ren throwinfinally_ro.exe throwinfinally_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_ro.org /out throwinfinally_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_ro.org /out throwinfinally_ro.exe
...
set ExePath=throwinfinally_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinally_ro.reflect.xml
    IF EXIST throwinfinally_ro.reflect.xml set ReflectionRoots=-x throwinfinally_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinally_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
