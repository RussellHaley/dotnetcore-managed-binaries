...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\multihandler_r multihandler_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\multihandler_r multihandler_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=multihandler_r.dasm.il multihandler_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=multihandler_r.dasm.il multihandler_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=multihandler_r.asm.exe  multihandler_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=multihandler_r.asm.exe  multihandler_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=multihandler_r
    if not exist "multihandler_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "multihandler_r.org" (
            mkdir IL
            copy multihandler_r.exe IL\multihandler_r.exe 
            ren multihandler_r.exe multihandler_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multihandler_r.org /out multihandler_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multihandler_r.org /out multihandler_r.exe
...
set ExePath=multihandler_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in multihandler_r.reflect.xml
    IF EXIST multihandler_r.reflect.xml set ReflectionRoots=-x multihandler_r.reflect.xml
    ...
    set ExePath=%LinkBin%\multihandler_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% multihandler_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% multihandler_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
