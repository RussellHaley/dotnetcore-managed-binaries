...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinclassconstructor_do throwinclassconstructor_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinclassconstructor_do throwinclassconstructor_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinclassconstructor_do.dasm.il throwinclassconstructor_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinclassconstructor_do.dasm.il throwinclassconstructor_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinclassconstructor_do.asm.exe  throwinclassconstructor_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinclassconstructor_do.asm.exe  throwinclassconstructor_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinclassconstructor_do
    if not exist "throwinclassconstructor_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinclassconstructor_do.org" (
            mkdir IL
            copy throwinclassconstructor_do.exe IL\throwinclassconstructor_do.exe 
            ren throwinclassconstructor_do.exe throwinclassconstructor_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinclassconstructor_do.org /out throwinclassconstructor_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinclassconstructor_do.org /out throwinclassconstructor_do.exe
...
set ExePath=throwinclassconstructor_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinclassconstructor_do.reflect.xml
    IF EXIST throwinclassconstructor_do.reflect.xml set ReflectionRoots=-x throwinclassconstructor_do.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinclassconstructor_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinclassconstructor_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinclassconstructor_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
