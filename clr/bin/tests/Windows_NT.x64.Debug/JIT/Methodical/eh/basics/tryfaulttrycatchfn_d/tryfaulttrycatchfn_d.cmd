...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfaulttrycatchfn_d tryfaulttrycatchfn_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfaulttrycatchfn_d tryfaulttrycatchfn_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tryfaulttrycatchfn_d.dasm.il tryfaulttrycatchfn_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tryfaulttrycatchfn_d.dasm.il tryfaulttrycatchfn_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tryfaulttrycatchfn_d.asm.exe -QUIET -NOLOGO -DEBUG tryfaulttrycatchfn_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tryfaulttrycatchfn_d.asm.exe -QUIET -NOLOGO -DEBUG tryfaulttrycatchfn_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tryfaulttrycatchfn_d
    if not exist "tryfaulttrycatchfn_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tryfaulttrycatchfn_d.org" (
            mkdir IL
            copy tryfaulttrycatchfn_d.exe IL\tryfaulttrycatchfn_d.exe 
            ren tryfaulttrycatchfn_d.exe tryfaulttrycatchfn_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfaulttrycatchfn_d.org /out tryfaulttrycatchfn_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfaulttrycatchfn_d.org /out tryfaulttrycatchfn_d.exe
...
set ExePath=tryfaulttrycatchfn_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tryfaulttrycatchfn_d.reflect.xml
    IF EXIST tryfaulttrycatchfn_d.reflect.xml set ReflectionRoots=-x tryfaulttrycatchfn_d.reflect.xml
    ...
    set ExePath=%LinkBin%\tryfaulttrycatchfn_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tryfaulttrycatchfn_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tryfaulttrycatchfn_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
