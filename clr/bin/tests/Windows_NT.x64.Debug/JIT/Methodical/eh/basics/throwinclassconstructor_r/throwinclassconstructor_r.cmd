...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinclassconstructor_r throwinclassconstructor_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinclassconstructor_r throwinclassconstructor_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinclassconstructor_r.dasm.il throwinclassconstructor_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinclassconstructor_r.dasm.il throwinclassconstructor_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinclassconstructor_r.asm.exe  throwinclassconstructor_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinclassconstructor_r.asm.exe  throwinclassconstructor_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinclassconstructor_r
    if not exist "throwinclassconstructor_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinclassconstructor_r.org" (
            mkdir IL
            copy throwinclassconstructor_r.exe IL\throwinclassconstructor_r.exe 
            ren throwinclassconstructor_r.exe throwinclassconstructor_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinclassconstructor_r.org /out throwinclassconstructor_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinclassconstructor_r.org /out throwinclassconstructor_r.exe
...
set ExePath=throwinclassconstructor_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinclassconstructor_r.reflect.xml
    IF EXIST throwinclassconstructor_r.reflect.xml set ReflectionRoots=-x throwinclassconstructor_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinclassconstructor_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinclassconstructor_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinclassconstructor_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
