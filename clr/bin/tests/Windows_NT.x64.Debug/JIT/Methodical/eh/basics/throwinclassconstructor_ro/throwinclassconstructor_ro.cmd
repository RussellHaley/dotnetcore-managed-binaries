...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinclassconstructor_ro throwinclassconstructor_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinclassconstructor_ro throwinclassconstructor_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinclassconstructor_ro.dasm.il throwinclassconstructor_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinclassconstructor_ro.dasm.il throwinclassconstructor_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinclassconstructor_ro.asm.exe  throwinclassconstructor_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinclassconstructor_ro.asm.exe  throwinclassconstructor_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinclassconstructor_ro
    if not exist "throwinclassconstructor_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinclassconstructor_ro.org" (
            mkdir IL
            copy throwinclassconstructor_ro.exe IL\throwinclassconstructor_ro.exe 
            ren throwinclassconstructor_ro.exe throwinclassconstructor_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinclassconstructor_ro.org /out throwinclassconstructor_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinclassconstructor_ro.org /out throwinclassconstructor_ro.exe
...
set ExePath=throwinclassconstructor_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinclassconstructor_ro.reflect.xml
    IF EXIST throwinclassconstructor_ro.reflect.xml set ReflectionRoots=-x throwinclassconstructor_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinclassconstructor_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinclassconstructor_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinclassconstructor_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
