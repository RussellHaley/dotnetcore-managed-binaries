...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwoutside_do throwoutside_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwoutside_do throwoutside_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwoutside_do.dasm.il throwoutside_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwoutside_do.dasm.il throwoutside_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwoutside_do.asm.exe  throwoutside_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwoutside_do.asm.exe  throwoutside_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwoutside_do
    if [ ! -f throwoutside_do.org ]; then
        TakeLock
        if [ ! -f throwoutside_do.org ]; then
          mkdir IL
          cp throwoutside_do.exe IL/throwoutside_do.exe
          mv throwoutside_do.exe throwoutside_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwoutside_do.org /out throwoutside_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwoutside_do.org /out throwoutside_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwoutside_do.reflect.xml
  if [ -f throwoutside_do.reflect.xml ]
then
    ReflectionRoots="-x throwoutside_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwoutside_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwoutside_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwoutside_do.asm.exe $CLRTestExecutionArguments
