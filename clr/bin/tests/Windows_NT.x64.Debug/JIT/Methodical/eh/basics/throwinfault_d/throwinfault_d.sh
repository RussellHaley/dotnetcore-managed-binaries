...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinfault_d throwinfault_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinfault_d throwinfault_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfault_d.dasm.il throwinfault_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfault_d.dasm.il throwinfault_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfault_d.asm.exe -QUIET -NOLOGO -DEBUG throwinfault_d.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfault_d.asm.exe -QUIET -NOLOGO -DEBUG throwinfault_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfault_d
    if [ ! -f throwinfault_d.org ]; then
        TakeLock
        if [ ! -f throwinfault_d.org ]; then
          mkdir IL
          cp throwinfault_d.exe IL/throwinfault_d.exe
          mv throwinfault_d.exe throwinfault_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfault_d.org /out throwinfault_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfault_d.org /out throwinfault_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfault_d.reflect.xml
  if [ -f throwinfault_d.reflect.xml ]
then
    ReflectionRoots="-x throwinfault_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfault_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfault_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfault_d.asm.exe $CLRTestExecutionArguments
