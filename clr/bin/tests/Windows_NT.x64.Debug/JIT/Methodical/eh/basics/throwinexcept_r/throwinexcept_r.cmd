...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinexcept_r throwinexcept_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinexcept_r throwinexcept_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinexcept_r.dasm.il throwinexcept_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinexcept_r.dasm.il throwinexcept_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinexcept_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinexcept_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinexcept_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinexcept_r
    if not exist "throwinexcept_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinexcept_r.org" (
            mkdir IL
            copy throwinexcept_r.exe IL\throwinexcept_r.exe 
            ren throwinexcept_r.exe throwinexcept_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinexcept_r.org /out throwinexcept_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinexcept_r.org /out throwinexcept_r.exe
...
set ExePath=throwinexcept_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinexcept_r.reflect.xml
    IF EXIST throwinexcept_r.reflect.xml set ReflectionRoots=-x throwinexcept_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinexcept_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinexcept_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinexcept_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
