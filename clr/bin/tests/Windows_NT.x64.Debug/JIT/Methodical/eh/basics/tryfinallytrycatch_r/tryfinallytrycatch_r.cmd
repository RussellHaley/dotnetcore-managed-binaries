...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfinallytrycatch_r tryfinallytrycatch_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\tryfinallytrycatch_r tryfinallytrycatch_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tryfinallytrycatch_r.dasm.il tryfinallytrycatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tryfinallytrycatch_r.dasm.il tryfinallytrycatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tryfinallytrycatch_r.asm.exe  tryfinallytrycatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tryfinallytrycatch_r.asm.exe  tryfinallytrycatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tryfinallytrycatch_r
    if not exist "tryfinallytrycatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tryfinallytrycatch_r.org" (
            mkdir IL
            copy tryfinallytrycatch_r.exe IL\tryfinallytrycatch_r.exe 
            ren tryfinallytrycatch_r.exe tryfinallytrycatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfinallytrycatch_r.org /out tryfinallytrycatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfinallytrycatch_r.org /out tryfinallytrycatch_r.exe
...
set ExePath=tryfinallytrycatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tryfinallytrycatch_r.reflect.xml
    IF EXIST tryfinallytrycatch_r.reflect.xml set ReflectionRoots=-x tryfinallytrycatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\tryfinallytrycatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tryfinallytrycatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tryfinallytrycatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
