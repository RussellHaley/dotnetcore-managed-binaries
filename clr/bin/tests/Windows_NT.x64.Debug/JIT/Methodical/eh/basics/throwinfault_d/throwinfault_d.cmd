...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfault_d throwinfault_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfault_d throwinfault_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfault_d.dasm.il throwinfault_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfault_d.dasm.il throwinfault_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfault_d.asm.exe -QUIET -NOLOGO -DEBUG throwinfault_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfault_d.asm.exe -QUIET -NOLOGO -DEBUG throwinfault_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfault_d
    if not exist "throwinfault_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfault_d.org" (
            mkdir IL
            copy throwinfault_d.exe IL\throwinfault_d.exe 
            ren throwinfault_d.exe throwinfault_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfault_d.org /out throwinfault_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfault_d.org /out throwinfault_d.exe
...
set ExePath=throwinfault_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfault_d.reflect.xml
    IF EXIST throwinfault_d.reflect.xml set ReflectionRoots=-x throwinfault_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfault_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfault_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfault_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
