...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwoutside_do throwoutside_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwoutside_do throwoutside_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwoutside_do.dasm.il throwoutside_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwoutside_do.dasm.il throwoutside_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwoutside_do.asm.exe  throwoutside_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwoutside_do.asm.exe  throwoutside_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwoutside_do
    if not exist "throwoutside_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwoutside_do.org" (
            mkdir IL
            copy throwoutside_do.exe IL\throwoutside_do.exe 
            ren throwoutside_do.exe throwoutside_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwoutside_do.org /out throwoutside_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwoutside_do.org /out throwoutside_do.exe
...
set ExePath=throwoutside_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwoutside_do.reflect.xml
    IF EXIST throwoutside_do.reflect.xml set ReflectionRoots=-x throwoutside_do.reflect.xml
    ...
    set ExePath=%LinkBin%\throwoutside_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwoutside_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwoutside_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
