...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfinally_r throwinfinally_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfinally_r throwinfinally_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_r.dasm.il throwinfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_r.dasm.il throwinfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinally_r.asm.exe  throwinfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinally_r.asm.exe  throwinfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinally_r
    if not exist "throwinfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinally_r.org" (
            mkdir IL
            copy throwinfinally_r.exe IL\throwinfinally_r.exe 
            ren throwinfinally_r.exe throwinfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_r.org /out throwinfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_r.org /out throwinfinally_r.exe
...
set ExePath=throwinfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinally_r.reflect.xml
    IF EXIST throwinfinally_r.reflect.xml set ReflectionRoots=-x throwinfinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
