...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinclassconstructor_ro throwinclassconstructor_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinclassconstructor_ro throwinclassconstructor_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinclassconstructor_ro.dasm.il throwinclassconstructor_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinclassconstructor_ro.dasm.il throwinclassconstructor_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinclassconstructor_ro.asm.exe  throwinclassconstructor_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinclassconstructor_ro.asm.exe  throwinclassconstructor_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinclassconstructor_ro
    if [ ! -f throwinclassconstructor_ro.org ]; then
        TakeLock
        if [ ! -f throwinclassconstructor_ro.org ]; then
          mkdir IL
          cp throwinclassconstructor_ro.exe IL/throwinclassconstructor_ro.exe
          mv throwinclassconstructor_ro.exe throwinclassconstructor_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinclassconstructor_ro.org /out throwinclassconstructor_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinclassconstructor_ro.org /out throwinclassconstructor_ro.exe
          __cgExitCode=$?
...
ExePath=throwinclassconstructor_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinclassconstructor_ro.reflect.xml
  if [ -f throwinclassconstructor_ro.reflect.xml ]
then
    ReflectionRoots="-x throwinclassconstructor_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinclassconstructor_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinclassconstructor_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinclassconstructor_ro.asm.exe $CLRTestExecutionArguments
