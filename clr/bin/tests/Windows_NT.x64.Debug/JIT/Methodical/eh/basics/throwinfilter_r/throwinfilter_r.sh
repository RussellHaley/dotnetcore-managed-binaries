...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinfilter_r throwinfilter_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinfilter_r throwinfilter_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfilter_r.dasm.il throwinfilter_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfilter_r.dasm.il throwinfilter_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfilter_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinfilter_r.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfilter_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT throwinfilter_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfilter_r
    if [ ! -f throwinfilter_r.org ]; then
        TakeLock
        if [ ! -f throwinfilter_r.org ]; then
          mkdir IL
          cp throwinfilter_r.exe IL/throwinfilter_r.exe
          mv throwinfilter_r.exe throwinfilter_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfilter_r.org /out throwinfilter_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfilter_r.org /out throwinfilter_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfilter_r.reflect.xml
  if [ -f throwinfilter_r.reflect.xml ]
then
    ReflectionRoots="-x throwinfilter_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfilter_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfilter_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfilter_r.asm.exe $CLRTestExecutionArguments
