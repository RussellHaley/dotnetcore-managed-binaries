...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\trycatch_r trycatch_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\trycatch_r trycatch_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trycatch_r.dasm.il trycatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trycatch_r.dasm.il trycatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trycatch_r.asm.exe  trycatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trycatch_r.asm.exe  trycatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trycatch_r
    if not exist "trycatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trycatch_r.org" (
            mkdir IL
            copy trycatch_r.exe IL\trycatch_r.exe 
            ren trycatch_r.exe trycatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatch_r.org /out trycatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trycatch_r.org /out trycatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trycatch_r.reflect.xml
    IF EXIST trycatch_r.reflect.xml set ReflectionRoots=-x trycatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\trycatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trycatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trycatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
