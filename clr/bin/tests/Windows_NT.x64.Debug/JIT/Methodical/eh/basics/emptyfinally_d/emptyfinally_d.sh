...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/emptyfinally_d emptyfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/emptyfinally_d emptyfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=emptyfinally_d.dasm.il emptyfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=emptyfinally_d.dasm.il emptyfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=emptyfinally_d.asm.exe -QUIET -NOLOGO -DEBUG emptyfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=emptyfinally_d.asm.exe -QUIET -NOLOGO -DEBUG emptyfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=emptyfinally_d
    if [ ! -f emptyfinally_d.org ]; then
        TakeLock
        if [ ! -f emptyfinally_d.org ]; then
          mkdir IL
          cp emptyfinally_d.exe IL/emptyfinally_d.exe
          mv emptyfinally_d.exe emptyfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in emptyfinally_d.org /out emptyfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in emptyfinally_d.org /out emptyfinally_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in emptyfinally_d.reflect.xml
  if [ -f emptyfinally_d.reflect.xml ]
then
    ReflectionRoots="-x emptyfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/emptyfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" emptyfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" emptyfinally_d.asm.exe $CLRTestExecutionArguments
