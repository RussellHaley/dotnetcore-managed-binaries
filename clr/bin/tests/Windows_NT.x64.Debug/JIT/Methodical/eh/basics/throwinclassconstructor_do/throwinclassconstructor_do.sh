...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinclassconstructor_do throwinclassconstructor_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/throwinclassconstructor_do throwinclassconstructor_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinclassconstructor_do.dasm.il throwinclassconstructor_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinclassconstructor_do.dasm.il throwinclassconstructor_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinclassconstructor_do.asm.exe  throwinclassconstructor_do.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinclassconstructor_do.asm.exe  throwinclassconstructor_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinclassconstructor_do
    if [ ! -f throwinclassconstructor_do.org ]; then
        TakeLock
        if [ ! -f throwinclassconstructor_do.org ]; then
          mkdir IL
          cp throwinclassconstructor_do.exe IL/throwinclassconstructor_do.exe
          mv throwinclassconstructor_do.exe throwinclassconstructor_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinclassconstructor_do.org /out throwinclassconstructor_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinclassconstructor_do.org /out throwinclassconstructor_do.exe
          __cgExitCode=$?
...
ExePath=throwinclassconstructor_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinclassconstructor_do.reflect.xml
  if [ -f throwinclassconstructor_do.reflect.xml ]
then
    ReflectionRoots="-x throwinclassconstructor_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinclassconstructor_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinclassconstructor_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinclassconstructor_do.asm.exe $CLRTestExecutionArguments
