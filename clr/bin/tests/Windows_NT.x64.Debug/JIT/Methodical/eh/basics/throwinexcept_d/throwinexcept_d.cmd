...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinexcept_d throwinexcept_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinexcept_d throwinexcept_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinexcept_d.dasm.il throwinexcept_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinexcept_d.dasm.il throwinexcept_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinexcept_d.asm.exe -QUIET -NOLOGO -DEBUG throwinexcept_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinexcept_d.asm.exe -QUIET -NOLOGO -DEBUG throwinexcept_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinexcept_d
    if not exist "throwinexcept_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinexcept_d.org" (
            mkdir IL
            copy throwinexcept_d.exe IL\throwinexcept_d.exe 
            ren throwinexcept_d.exe throwinexcept_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinexcept_d.org /out throwinexcept_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinexcept_d.org /out throwinexcept_d.exe
...
set ExePath=throwinexcept_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinexcept_d.reflect.xml
    IF EXIST throwinexcept_d.reflect.xml set ReflectionRoots=-x throwinexcept_d.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinexcept_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinexcept_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinexcept_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
