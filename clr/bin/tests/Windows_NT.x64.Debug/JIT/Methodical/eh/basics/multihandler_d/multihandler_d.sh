...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/multihandler_d multihandler_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/basics/multihandler_d multihandler_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=multihandler_d.dasm.il multihandler_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=multihandler_d.dasm.il multihandler_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=multihandler_d.asm.exe  multihandler_d.dasm.il
    "$CORE_ROOT/ilasm" -output=multihandler_d.asm.exe  multihandler_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=multihandler_d
    if [ ! -f multihandler_d.org ]; then
        TakeLock
        if [ ! -f multihandler_d.org ]; then
          mkdir IL
          cp multihandler_d.exe IL/multihandler_d.exe
          mv multihandler_d.exe multihandler_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multihandler_d.org /out multihandler_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multihandler_d.org /out multihandler_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in multihandler_d.reflect.xml
  if [ -f multihandler_d.reflect.xml ]
then
    ReflectionRoots="-x multihandler_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/multihandler_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" multihandler_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" multihandler_d.asm.exe $CLRTestExecutionArguments
