...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\trythrowexcept_d trythrowexcept_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\trythrowexcept_d trythrowexcept_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=trythrowexcept_d.dasm.il trythrowexcept_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=trythrowexcept_d.dasm.il trythrowexcept_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=trythrowexcept_d.asm.exe -QUIET -NOLOGO -DEBUG trythrowexcept_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=trythrowexcept_d.asm.exe -QUIET -NOLOGO -DEBUG trythrowexcept_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=trythrowexcept_d
    if not exist "trythrowexcept_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "trythrowexcept_d.org" (
            mkdir IL
            copy trythrowexcept_d.exe IL\trythrowexcept_d.exe 
            ren trythrowexcept_d.exe trythrowexcept_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trythrowexcept_d.org /out trythrowexcept_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in trythrowexcept_d.org /out trythrowexcept_d.exe
...
set ExePath=trythrowexcept_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in trythrowexcept_d.reflect.xml
    IF EXIST trythrowexcept_d.reflect.xml set ReflectionRoots=-x trythrowexcept_d.reflect.xml
    ...
    set ExePath=%LinkBin%\trythrowexcept_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% trythrowexcept_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% trythrowexcept_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
