...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfinally_do throwinfinally_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\basics\throwinfinally_do throwinfinally_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_do.dasm.il throwinfinally_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinally_do.dasm.il throwinfinally_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinally_do.asm.exe  throwinfinally_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinally_do.asm.exe  throwinfinally_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinally_do
    if not exist "throwinfinally_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinally_do.org" (
            mkdir IL
            copy throwinfinally_do.exe IL\throwinfinally_do.exe 
            ren throwinfinally_do.exe throwinfinally_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_do.org /out throwinfinally_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinally_do.org /out throwinfinally_do.exe
...
set ExePath=throwinfinally_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinally_do.reflect.xml
    IF EXIST throwinfinally_do.reflect.xml set ReflectionRoots=-x throwinfinally_do.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinally_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
