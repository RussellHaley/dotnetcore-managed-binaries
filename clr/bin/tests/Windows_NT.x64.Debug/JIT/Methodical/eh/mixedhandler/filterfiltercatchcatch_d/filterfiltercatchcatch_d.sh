...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/mixedhandler/filterfiltercatchcatch_d filterfiltercatchcatch_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/mixedhandler/filterfiltercatchcatch_d filterfiltercatchcatch_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=filterfiltercatchcatch_d.dasm.il filterfiltercatchcatch_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=filterfiltercatchcatch_d.dasm.il filterfiltercatchcatch_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=filterfiltercatchcatch_d.asm.exe -QUIET -NOLOGO -DEBUG filterfiltercatchcatch_d.dasm.il
    "$CORE_ROOT/ilasm" -output=filterfiltercatchcatch_d.asm.exe -QUIET -NOLOGO -DEBUG filterfiltercatchcatch_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=filterfiltercatchcatch_d
    if [ ! -f filterfiltercatchcatch_d.org ]; then
        TakeLock
        if [ ! -f filterfiltercatchcatch_d.org ]; then
          mkdir IL
          cp filterfiltercatchcatch_d.exe IL/filterfiltercatchcatch_d.exe
          mv filterfiltercatchcatch_d.exe filterfiltercatchcatch_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in filterfiltercatchcatch_d.org /out filterfiltercatchcatch_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in filterfiltercatchcatch_d.org /out filterfiltercatchcatch_d.exe
          __cgExitCode=$?
...
ExePath=filterfiltercatchcatch_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in filterfiltercatchcatch_d.reflect.xml
  if [ -f filterfiltercatchcatch_d.reflect.xml ]
then
    ReflectionRoots="-x filterfiltercatchcatch_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/filterfiltercatchcatch_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" filterfiltercatchcatch_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" filterfiltercatchcatch_d.asm.exe $CLRTestExecutionArguments
