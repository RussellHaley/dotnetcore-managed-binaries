...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\mixedhandler\catchfiltercatch_d catchfiltercatch_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\mixedhandler\catchfiltercatch_d catchfiltercatch_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=catchfiltercatch_d.dasm.il catchfiltercatch_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=catchfiltercatch_d.dasm.il catchfiltercatch_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=catchfiltercatch_d.asm.exe -QUIET -NOLOGO -DEBUG catchfiltercatch_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=catchfiltercatch_d.asm.exe -QUIET -NOLOGO -DEBUG catchfiltercatch_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=catchfiltercatch_d
    if not exist "catchfiltercatch_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "catchfiltercatch_d.org" (
            mkdir IL
            copy catchfiltercatch_d.exe IL\catchfiltercatch_d.exe 
            ren catchfiltercatch_d.exe catchfiltercatch_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchfiltercatch_d.org /out catchfiltercatch_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchfiltercatch_d.org /out catchfiltercatch_d.exe
...
set ExePath=catchfiltercatch_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in catchfiltercatch_d.reflect.xml
    IF EXIST catchfiltercatch_d.reflect.xml set ReflectionRoots=-x catchfiltercatch_d.reflect.xml
    ...
    set ExePath=%LinkBin%\catchfiltercatch_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% catchfiltercatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% catchfiltercatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
