...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\mixedhandler\catchfiltercatch_r catchfiltercatch_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\mixedhandler\catchfiltercatch_r catchfiltercatch_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=catchfiltercatch_r.dasm.il catchfiltercatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=catchfiltercatch_r.dasm.il catchfiltercatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=catchfiltercatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT catchfiltercatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=catchfiltercatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT catchfiltercatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=catchfiltercatch_r
    if not exist "catchfiltercatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "catchfiltercatch_r.org" (
            mkdir IL
            copy catchfiltercatch_r.exe IL\catchfiltercatch_r.exe 
            ren catchfiltercatch_r.exe catchfiltercatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchfiltercatch_r.org /out catchfiltercatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchfiltercatch_r.org /out catchfiltercatch_r.exe
...
set ExePath=catchfiltercatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in catchfiltercatch_r.reflect.xml
    IF EXIST catchfiltercatch_r.reflect.xml set ReflectionRoots=-x catchfiltercatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\catchfiltercatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% catchfiltercatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% catchfiltercatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
