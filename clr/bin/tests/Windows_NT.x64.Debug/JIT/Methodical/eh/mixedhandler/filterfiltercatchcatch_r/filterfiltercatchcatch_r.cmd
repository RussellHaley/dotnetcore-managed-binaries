...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\mixedhandler\filterfiltercatchcatch_r filterfiltercatchcatch_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\mixedhandler\filterfiltercatchcatch_r filterfiltercatchcatch_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=filterfiltercatchcatch_r.dasm.il filterfiltercatchcatch_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=filterfiltercatchcatch_r.dasm.il filterfiltercatchcatch_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=filterfiltercatchcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT filterfiltercatchcatch_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=filterfiltercatchcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT filterfiltercatchcatch_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=filterfiltercatchcatch_r
    if not exist "filterfiltercatchcatch_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "filterfiltercatchcatch_r.org" (
            mkdir IL
            copy filterfiltercatchcatch_r.exe IL\filterfiltercatchcatch_r.exe 
            ren filterfiltercatchcatch_r.exe filterfiltercatchcatch_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in filterfiltercatchcatch_r.org /out filterfiltercatchcatch_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in filterfiltercatchcatch_r.org /out filterfiltercatchcatch_r.exe
...
set ExePath=filterfiltercatchcatch_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in filterfiltercatchcatch_r.reflect.xml
    IF EXIST filterfiltercatchcatch_r.reflect.xml set ReflectionRoots=-x filterfiltercatchcatch_r.reflect.xml
    ...
    set ExePath=%LinkBin%\filterfiltercatchcatch_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% filterfiltercatchcatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% filterfiltercatchcatch_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
