...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\mixedhandler\filterfiltercatchcatch_d filterfiltercatchcatch_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\mixedhandler\filterfiltercatchcatch_d filterfiltercatchcatch_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=filterfiltercatchcatch_d.dasm.il filterfiltercatchcatch_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=filterfiltercatchcatch_d.dasm.il filterfiltercatchcatch_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=filterfiltercatchcatch_d.asm.exe -QUIET -NOLOGO -DEBUG filterfiltercatchcatch_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=filterfiltercatchcatch_d.asm.exe -QUIET -NOLOGO -DEBUG filterfiltercatchcatch_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=filterfiltercatchcatch_d
    if not exist "filterfiltercatchcatch_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "filterfiltercatchcatch_d.org" (
            mkdir IL
            copy filterfiltercatchcatch_d.exe IL\filterfiltercatchcatch_d.exe 
            ren filterfiltercatchcatch_d.exe filterfiltercatchcatch_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in filterfiltercatchcatch_d.org /out filterfiltercatchcatch_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in filterfiltercatchcatch_d.org /out filterfiltercatchcatch_d.exe
...
set ExePath=filterfiltercatchcatch_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in filterfiltercatchcatch_d.reflect.xml
    IF EXIST filterfiltercatchcatch_d.reflect.xml set ReflectionRoots=-x filterfiltercatchcatch_d.reflect.xml
    ...
    set ExePath=%LinkBin%\filterfiltercatchcatch_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% filterfiltercatchcatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% filterfiltercatchcatch_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
