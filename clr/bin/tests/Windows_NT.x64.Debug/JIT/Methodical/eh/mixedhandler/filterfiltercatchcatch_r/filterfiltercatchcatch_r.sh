...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/mixedhandler/filterfiltercatchcatch_r filterfiltercatchcatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/mixedhandler/filterfiltercatchcatch_r filterfiltercatchcatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=filterfiltercatchcatch_r.dasm.il filterfiltercatchcatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=filterfiltercatchcatch_r.dasm.il filterfiltercatchcatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=filterfiltercatchcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT filterfiltercatchcatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=filterfiltercatchcatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT filterfiltercatchcatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=filterfiltercatchcatch_r
    if [ ! -f filterfiltercatchcatch_r.org ]; then
        TakeLock
        if [ ! -f filterfiltercatchcatch_r.org ]; then
          mkdir IL
          cp filterfiltercatchcatch_r.exe IL/filterfiltercatchcatch_r.exe
          mv filterfiltercatchcatch_r.exe filterfiltercatchcatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in filterfiltercatchcatch_r.org /out filterfiltercatchcatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in filterfiltercatchcatch_r.org /out filterfiltercatchcatch_r.exe
          __cgExitCode=$?
...
ExePath=filterfiltercatchcatch_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in filterfiltercatchcatch_r.reflect.xml
  if [ -f filterfiltercatchcatch_r.reflect.xml ]
then
    ReflectionRoots="-x filterfiltercatchcatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/filterfiltercatchcatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" filterfiltercatchcatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" filterfiltercatchcatch_r.asm.exe $CLRTestExecutionArguments
