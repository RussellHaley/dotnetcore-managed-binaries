...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/mixedhandler/catchfiltercatch_r catchfiltercatch_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/mixedhandler/catchfiltercatch_r catchfiltercatch_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=catchfiltercatch_r.dasm.il catchfiltercatch_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=catchfiltercatch_r.dasm.il catchfiltercatch_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=catchfiltercatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT catchfiltercatch_r.dasm.il
    "$CORE_ROOT/ilasm" -output=catchfiltercatch_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT catchfiltercatch_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=catchfiltercatch_r
    if [ ! -f catchfiltercatch_r.org ]; then
        TakeLock
        if [ ! -f catchfiltercatch_r.org ]; then
          mkdir IL
          cp catchfiltercatch_r.exe IL/catchfiltercatch_r.exe
          mv catchfiltercatch_r.exe catchfiltercatch_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchfiltercatch_r.org /out catchfiltercatch_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchfiltercatch_r.org /out catchfiltercatch_r.exe
          __cgExitCode=$?
...
ExePath=catchfiltercatch_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in catchfiltercatch_r.reflect.xml
  if [ -f catchfiltercatch_r.reflect.xml ]
then
    ReflectionRoots="-x catchfiltercatch_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/catchfiltercatch_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" catchfiltercatch_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" catchfiltercatch_r.asm.exe $CLRTestExecutionArguments
