...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/mixedhandler/catchfiltercatch_d catchfiltercatch_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/mixedhandler/catchfiltercatch_d catchfiltercatch_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=catchfiltercatch_d.dasm.il catchfiltercatch_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=catchfiltercatch_d.dasm.il catchfiltercatch_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=catchfiltercatch_d.asm.exe -QUIET -NOLOGO -DEBUG catchfiltercatch_d.dasm.il
    "$CORE_ROOT/ilasm" -output=catchfiltercatch_d.asm.exe -QUIET -NOLOGO -DEBUG catchfiltercatch_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=catchfiltercatch_d
    if [ ! -f catchfiltercatch_d.org ]; then
        TakeLock
        if [ ! -f catchfiltercatch_d.org ]; then
          mkdir IL
          cp catchfiltercatch_d.exe IL/catchfiltercatch_d.exe
          mv catchfiltercatch_d.exe catchfiltercatch_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchfiltercatch_d.org /out catchfiltercatch_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchfiltercatch_d.org /out catchfiltercatch_d.exe
          __cgExitCode=$?
...
ExePath=catchfiltercatch_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in catchfiltercatch_d.reflect.xml
  if [ -f catchfiltercatch_d.reflect.xml ]
then
    ReflectionRoots="-x catchfiltercatch_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/catchfiltercatch_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" catchfiltercatch_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" catchfiltercatch_d.asm.exe $CLRTestExecutionArguments
