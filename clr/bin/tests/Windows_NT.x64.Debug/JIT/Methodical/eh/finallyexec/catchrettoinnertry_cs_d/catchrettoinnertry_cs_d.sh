...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/catchrettoinnertry_cs_d catchrettoinnertry_cs_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/catchrettoinnertry_cs_d catchrettoinnertry_cs_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=catchrettoinnertry_cs_d.dasm.il catchrettoinnertry_cs_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=catchrettoinnertry_cs_d.dasm.il catchrettoinnertry_cs_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=catchrettoinnertry_cs_d.asm.exe  catchrettoinnertry_cs_d.dasm.il
    "$CORE_ROOT/ilasm" -output=catchrettoinnertry_cs_d.asm.exe  catchrettoinnertry_cs_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=catchrettoinnertry_cs_d
    if [ ! -f catchrettoinnertry_cs_d.org ]; then
        TakeLock
        if [ ! -f catchrettoinnertry_cs_d.org ]; then
          mkdir IL
          cp catchrettoinnertry_cs_d.exe IL/catchrettoinnertry_cs_d.exe
          mv catchrettoinnertry_cs_d.exe catchrettoinnertry_cs_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchrettoinnertry_cs_d.org /out catchrettoinnertry_cs_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchrettoinnertry_cs_d.org /out catchrettoinnertry_cs_d.exe
          __cgExitCode=$?
...
ExePath=catchrettoinnertry_cs_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in catchrettoinnertry_cs_d.reflect.xml
  if [ -f catchrettoinnertry_cs_d.reflect.xml ]
then
    ReflectionRoots="-x catchrettoinnertry_cs_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/catchrettoinnertry_cs_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" catchrettoinnertry_cs_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" catchrettoinnertry_cs_d.asm.exe $CLRTestExecutionArguments
