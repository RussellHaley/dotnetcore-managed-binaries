...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/loopinfinally_ro loopinfinally_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/loopinfinally_ro loopinfinally_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=loopinfinally_ro.dasm.il loopinfinally_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=loopinfinally_ro.dasm.il loopinfinally_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=loopinfinally_ro.asm.exe  loopinfinally_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=loopinfinally_ro.asm.exe  loopinfinally_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=loopinfinally_ro
    if [ ! -f loopinfinally_ro.org ]; then
        TakeLock
        if [ ! -f loopinfinally_ro.org ]; then
          mkdir IL
          cp loopinfinally_ro.exe IL/loopinfinally_ro.exe
          mv loopinfinally_ro.exe loopinfinally_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopinfinally_ro.org /out loopinfinally_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopinfinally_ro.org /out loopinfinally_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in loopinfinally_ro.reflect.xml
  if [ -f loopinfinally_ro.reflect.xml ]
then
    ReflectionRoots="-x loopinfinally_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/loopinfinally_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" loopinfinally_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" loopinfinally_ro.asm.exe $CLRTestExecutionArguments
