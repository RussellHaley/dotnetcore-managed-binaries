...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nestedfinallycall_d.dasm.il nestedfinallycall_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nestedfinallycall_d.dasm.il nestedfinallycall_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nestedfinallycall_d.asm.exe -QUIET -NOLOGO -DEBUG nestedfinallycall_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nestedfinallycall_d.asm.exe -QUIET -NOLOGO -DEBUG nestedfinallycall_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nestedfinallycall_d
    if not exist "nestedfinallycall_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nestedfinallycall_d.org" (
            mkdir IL
            copy nestedfinallycall_d.exe IL\nestedfinallycall_d.exe 
            ren nestedfinallycall_d.exe nestedfinallycall_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedfinallycall_d.org /out nestedfinallycall_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedfinallycall_d.org /out nestedfinallycall_d.exe
...
set ExePath=nestedfinallycall_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nestedfinallycall_d.reflect.xml
    IF EXIST nestedfinallycall_d.reflect.xml set ReflectionRoots=-x nestedfinallycall_d.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nestedfinallycall_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nestedfinallycall_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
