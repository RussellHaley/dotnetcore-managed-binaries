...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/nonlocalexittonestedsibling_d nonlocalexittonestedsibling_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/nonlocalexittonestedsibling_d nonlocalexittonestedsibling_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nonlocalexittonestedsibling_d.dasm.il nonlocalexittonestedsibling_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=nonlocalexittonestedsibling_d.dasm.il nonlocalexittonestedsibling_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nonlocalexittonestedsibling_d.asm.exe -QUIET -NOLOGO -DEBUG nonlocalexittonestedsibling_d.dasm.il
    "$CORE_ROOT/ilasm" -output=nonlocalexittonestedsibling_d.asm.exe -QUIET -NOLOGO -DEBUG nonlocalexittonestedsibling_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nonlocalexittonestedsibling_d
    if [ ! -f nonlocalexittonestedsibling_d.org ]; then
        TakeLock
        if [ ! -f nonlocalexittonestedsibling_d.org ]; then
          mkdir IL
          cp nonlocalexittonestedsibling_d.exe IL/nonlocalexittonestedsibling_d.exe
          mv nonlocalexittonestedsibling_d.exe nonlocalexittonestedsibling_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nonlocalexittonestedsibling_d.org /out nonlocalexittonestedsibling_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nonlocalexittonestedsibling_d.org /out nonlocalexittonestedsibling_d.exe
          __cgExitCode=$?
...
ExePath=nonlocalexittonestedsibling_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in nonlocalexittonestedsibling_d.reflect.xml
  if [ -f nonlocalexittonestedsibling_d.reflect.xml ]
then
    ReflectionRoots="-x nonlocalexittonestedsibling_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nonlocalexittonestedsibling_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nonlocalexittonestedsibling_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nonlocalexittonestedsibling_d.asm.exe $CLRTestExecutionArguments
