...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\catchrettoinnertry_cs_r catchrettoinnertry_cs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\catchrettoinnertry_cs_r catchrettoinnertry_cs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=catchrettoinnertry_cs_r.dasm.il catchrettoinnertry_cs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=catchrettoinnertry_cs_r.dasm.il catchrettoinnertry_cs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=catchrettoinnertry_cs_r.asm.exe  catchrettoinnertry_cs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=catchrettoinnertry_cs_r.asm.exe  catchrettoinnertry_cs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=catchrettoinnertry_cs_r
    if not exist "catchrettoinnertry_cs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "catchrettoinnertry_cs_r.org" (
            mkdir IL
            copy catchrettoinnertry_cs_r.exe IL\catchrettoinnertry_cs_r.exe 
            ren catchrettoinnertry_cs_r.exe catchrettoinnertry_cs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchrettoinnertry_cs_r.org /out catchrettoinnertry_cs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchrettoinnertry_cs_r.org /out catchrettoinnertry_cs_r.exe
...
set ExePath=catchrettoinnertry_cs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in catchrettoinnertry_cs_r.reflect.xml
    IF EXIST catchrettoinnertry_cs_r.reflect.xml set ReflectionRoots=-x catchrettoinnertry_cs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\catchrettoinnertry_cs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% catchrettoinnertry_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% catchrettoinnertry_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
