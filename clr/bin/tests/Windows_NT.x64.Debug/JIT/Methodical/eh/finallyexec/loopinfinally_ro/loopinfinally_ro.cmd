...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\loopinfinally_ro loopinfinally_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\loopinfinally_ro loopinfinally_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=loopinfinally_ro.dasm.il loopinfinally_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=loopinfinally_ro.dasm.il loopinfinally_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=loopinfinally_ro.asm.exe  loopinfinally_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=loopinfinally_ro.asm.exe  loopinfinally_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=loopinfinally_ro
    if not exist "loopinfinally_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "loopinfinally_ro.org" (
            mkdir IL
            copy loopinfinally_ro.exe IL\loopinfinally_ro.exe 
            ren loopinfinally_ro.exe loopinfinally_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopinfinally_ro.org /out loopinfinally_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopinfinally_ro.org /out loopinfinally_ro.exe
...
set ExePath=loopinfinally_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in loopinfinally_ro.reflect.xml
    IF EXIST loopinfinally_ro.reflect.xml set ReflectionRoots=-x loopinfinally_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\loopinfinally_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% loopinfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% loopinfinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
