...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/nestedfinallycall_d nestedfinallycall_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/nestedfinallycall_d nestedfinallycall_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nestedfinallycall_d.dasm.il nestedfinallycall_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=nestedfinallycall_d.dasm.il nestedfinallycall_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nestedfinallycall_d.asm.exe -QUIET -NOLOGO -DEBUG nestedfinallycall_d.dasm.il
    "$CORE_ROOT/ilasm" -output=nestedfinallycall_d.asm.exe -QUIET -NOLOGO -DEBUG nestedfinallycall_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nestedfinallycall_d
    if [ ! -f nestedfinallycall_d.org ]; then
        TakeLock
        if [ ! -f nestedfinallycall_d.org ]; then
          mkdir IL
          cp nestedfinallycall_d.exe IL/nestedfinallycall_d.exe
          mv nestedfinallycall_d.exe nestedfinallycall_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedfinallycall_d.org /out nestedfinallycall_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedfinallycall_d.org /out nestedfinallycall_d.exe
          __cgExitCode=$?
...
ExePath=nestedfinallycall_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in nestedfinallycall_d.reflect.xml
  if [ -f nestedfinallycall_d.reflect.xml ]
then
    ReflectionRoots="-x nestedfinallycall_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nestedfinallycall_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nestedfinallycall_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nestedfinallycall_d.asm.exe $CLRTestExecutionArguments
