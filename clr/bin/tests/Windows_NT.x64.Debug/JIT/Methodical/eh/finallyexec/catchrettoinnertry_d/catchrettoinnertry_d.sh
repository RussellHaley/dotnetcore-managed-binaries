...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/catchrettoinnertry_d catchrettoinnertry_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/catchrettoinnertry_d catchrettoinnertry_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=catchrettoinnertry_d.dasm.il catchrettoinnertry_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=catchrettoinnertry_d.dasm.il catchrettoinnertry_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=catchrettoinnertry_d.asm.exe -QUIET -NOLOGO -DEBUG catchrettoinnertry_d.dasm.il
    "$CORE_ROOT/ilasm" -output=catchrettoinnertry_d.asm.exe -QUIET -NOLOGO -DEBUG catchrettoinnertry_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=catchrettoinnertry_d
    if [ ! -f catchrettoinnertry_d.org ]; then
        TakeLock
        if [ ! -f catchrettoinnertry_d.org ]; then
          mkdir IL
          cp catchrettoinnertry_d.exe IL/catchrettoinnertry_d.exe
          mv catchrettoinnertry_d.exe catchrettoinnertry_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchrettoinnertry_d.org /out catchrettoinnertry_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchrettoinnertry_d.org /out catchrettoinnertry_d.exe
          __cgExitCode=$?
...
ExePath=catchrettoinnertry_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in catchrettoinnertry_d.reflect.xml
  if [ -f catchrettoinnertry_d.reflect.xml ]
then
    ReflectionRoots="-x catchrettoinnertry_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/catchrettoinnertry_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" catchrettoinnertry_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" catchrettoinnertry_d.asm.exe $CLRTestExecutionArguments
