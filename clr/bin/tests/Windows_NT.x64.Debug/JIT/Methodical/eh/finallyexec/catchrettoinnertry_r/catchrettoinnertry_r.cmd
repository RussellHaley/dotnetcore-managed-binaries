...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\catchrettoinnertry_r catchrettoinnertry_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\catchrettoinnertry_r catchrettoinnertry_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=catchrettoinnertry_r.dasm.il catchrettoinnertry_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=catchrettoinnertry_r.dasm.il catchrettoinnertry_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=catchrettoinnertry_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT catchrettoinnertry_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=catchrettoinnertry_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT catchrettoinnertry_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=catchrettoinnertry_r
    if not exist "catchrettoinnertry_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "catchrettoinnertry_r.org" (
            mkdir IL
            copy catchrettoinnertry_r.exe IL\catchrettoinnertry_r.exe 
            ren catchrettoinnertry_r.exe catchrettoinnertry_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchrettoinnertry_r.org /out catchrettoinnertry_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchrettoinnertry_r.org /out catchrettoinnertry_r.exe
...
set ExePath=catchrettoinnertry_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in catchrettoinnertry_r.reflect.xml
    IF EXIST catchrettoinnertry_r.reflect.xml set ReflectionRoots=-x catchrettoinnertry_r.reflect.xml
    ...
    set ExePath=%LinkBin%\catchrettoinnertry_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% catchrettoinnertry_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% catchrettoinnertry_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
