...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/loopinfinally_r loopinfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/loopinfinally_r loopinfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=loopinfinally_r.dasm.il loopinfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=loopinfinally_r.dasm.il loopinfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=loopinfinally_r.asm.exe  loopinfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=loopinfinally_r.asm.exe  loopinfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=loopinfinally_r
    if [ ! -f loopinfinally_r.org ]; then
        TakeLock
        if [ ! -f loopinfinally_r.org ]; then
          mkdir IL
          cp loopinfinally_r.exe IL/loopinfinally_r.exe
          mv loopinfinally_r.exe loopinfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopinfinally_r.org /out loopinfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopinfinally_r.org /out loopinfinally_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in loopinfinally_r.reflect.xml
  if [ -f loopinfinally_r.reflect.xml ]
then
    ReflectionRoots="-x loopinfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/loopinfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" loopinfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" loopinfinally_r.asm.exe $CLRTestExecutionArguments
