...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/nonlocalexittonestedsibling_r nonlocalexittonestedsibling_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/nonlocalexittonestedsibling_r nonlocalexittonestedsibling_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nonlocalexittonestedsibling_r.dasm.il nonlocalexittonestedsibling_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=nonlocalexittonestedsibling_r.dasm.il nonlocalexittonestedsibling_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nonlocalexittonestedsibling_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nonlocalexittonestedsibling_r.dasm.il
    "$CORE_ROOT/ilasm" -output=nonlocalexittonestedsibling_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nonlocalexittonestedsibling_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nonlocalexittonestedsibling_r
    if [ ! -f nonlocalexittonestedsibling_r.org ]; then
        TakeLock
        if [ ! -f nonlocalexittonestedsibling_r.org ]; then
          mkdir IL
          cp nonlocalexittonestedsibling_r.exe IL/nonlocalexittonestedsibling_r.exe
          mv nonlocalexittonestedsibling_r.exe nonlocalexittonestedsibling_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nonlocalexittonestedsibling_r.org /out nonlocalexittonestedsibling_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nonlocalexittonestedsibling_r.org /out nonlocalexittonestedsibling_r.exe
          __cgExitCode=$?
...
ExePath=nonlocalexittonestedsibling_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in nonlocalexittonestedsibling_r.reflect.xml
  if [ -f nonlocalexittonestedsibling_r.reflect.xml ]
then
    ReflectionRoots="-x nonlocalexittonestedsibling_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nonlocalexittonestedsibling_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nonlocalexittonestedsibling_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nonlocalexittonestedsibling_r.asm.exe $CLRTestExecutionArguments
