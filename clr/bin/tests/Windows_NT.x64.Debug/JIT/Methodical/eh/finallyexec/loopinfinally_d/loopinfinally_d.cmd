...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\loopinfinally_d loopinfinally_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\loopinfinally_d loopinfinally_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=loopinfinally_d.dasm.il loopinfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=loopinfinally_d.dasm.il loopinfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=loopinfinally_d.asm.exe  loopinfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=loopinfinally_d.asm.exe  loopinfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=loopinfinally_d
    if not exist "loopinfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "loopinfinally_d.org" (
            mkdir IL
            copy loopinfinally_d.exe IL\loopinfinally_d.exe 
            ren loopinfinally_d.exe loopinfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopinfinally_d.org /out loopinfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopinfinally_d.org /out loopinfinally_d.exe
...
set ExePath=loopinfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in loopinfinally_d.reflect.xml
    IF EXIST loopinfinally_d.reflect.xml set ReflectionRoots=-x loopinfinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\loopinfinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% loopinfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% loopinfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
