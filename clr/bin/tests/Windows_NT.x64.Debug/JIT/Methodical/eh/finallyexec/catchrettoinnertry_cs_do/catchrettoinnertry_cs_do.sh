...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/catchrettoinnertry_cs_do catchrettoinnertry_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/catchrettoinnertry_cs_do catchrettoinnertry_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=catchrettoinnertry_cs_do.dasm.il catchrettoinnertry_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=catchrettoinnertry_cs_do.dasm.il catchrettoinnertry_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=catchrettoinnertry_cs_do.asm.exe  catchrettoinnertry_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=catchrettoinnertry_cs_do.asm.exe  catchrettoinnertry_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=catchrettoinnertry_cs_do
    if [ ! -f catchrettoinnertry_cs_do.org ]; then
        TakeLock
        if [ ! -f catchrettoinnertry_cs_do.org ]; then
          mkdir IL
          cp catchrettoinnertry_cs_do.exe IL/catchrettoinnertry_cs_do.exe
          mv catchrettoinnertry_cs_do.exe catchrettoinnertry_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchrettoinnertry_cs_do.org /out catchrettoinnertry_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchrettoinnertry_cs_do.org /out catchrettoinnertry_cs_do.exe
          __cgExitCode=$?
...
ExePath=catchrettoinnertry_cs_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in catchrettoinnertry_cs_do.reflect.xml
  if [ -f catchrettoinnertry_cs_do.reflect.xml ]
then
    ReflectionRoots="-x catchrettoinnertry_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/catchrettoinnertry_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" catchrettoinnertry_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" catchrettoinnertry_cs_do.asm.exe $CLRTestExecutionArguments
