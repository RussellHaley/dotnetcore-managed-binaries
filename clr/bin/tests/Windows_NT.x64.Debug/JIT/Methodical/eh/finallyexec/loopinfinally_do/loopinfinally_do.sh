...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/loopinfinally_do loopinfinally_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/loopinfinally_do loopinfinally_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=loopinfinally_do.dasm.il loopinfinally_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=loopinfinally_do.dasm.il loopinfinally_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=loopinfinally_do.asm.exe  loopinfinally_do.dasm.il
    "$CORE_ROOT/ilasm" -output=loopinfinally_do.asm.exe  loopinfinally_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=loopinfinally_do
    if [ ! -f loopinfinally_do.org ]; then
        TakeLock
        if [ ! -f loopinfinally_do.org ]; then
          mkdir IL
          cp loopinfinally_do.exe IL/loopinfinally_do.exe
          mv loopinfinally_do.exe loopinfinally_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopinfinally_do.org /out loopinfinally_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopinfinally_do.org /out loopinfinally_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in loopinfinally_do.reflect.xml
  if [ -f loopinfinally_do.reflect.xml ]
then
    ReflectionRoots="-x loopinfinally_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/loopinfinally_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" loopinfinally_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" loopinfinally_do.asm.exe $CLRTestExecutionArguments
