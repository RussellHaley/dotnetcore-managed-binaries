...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nestedfinallycall_r.dasm.il nestedfinallycall_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nestedfinallycall_r.dasm.il nestedfinallycall_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nestedfinallycall_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedfinallycall_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nestedfinallycall_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedfinallycall_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nestedfinallycall_r
    if not exist "nestedfinallycall_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nestedfinallycall_r.org" (
            mkdir IL
            copy nestedfinallycall_r.exe IL\nestedfinallycall_r.exe 
            ren nestedfinallycall_r.exe nestedfinallycall_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedfinallycall_r.org /out nestedfinallycall_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nestedfinallycall_r.org /out nestedfinallycall_r.exe
...
set ExePath=nestedfinallycall_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nestedfinallycall_r.reflect.xml
    IF EXIST nestedfinallycall_r.reflect.xml set ReflectionRoots=-x nestedfinallycall_r.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nestedfinallycall_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nestedfinallycall_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
