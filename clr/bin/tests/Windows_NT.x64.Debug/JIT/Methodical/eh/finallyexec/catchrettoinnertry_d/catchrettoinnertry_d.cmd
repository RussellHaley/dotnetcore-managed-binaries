...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\catchrettoinnertry_d catchrettoinnertry_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\catchrettoinnertry_d catchrettoinnertry_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=catchrettoinnertry_d.dasm.il catchrettoinnertry_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=catchrettoinnertry_d.dasm.il catchrettoinnertry_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=catchrettoinnertry_d.asm.exe -QUIET -NOLOGO -DEBUG catchrettoinnertry_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=catchrettoinnertry_d.asm.exe -QUIET -NOLOGO -DEBUG catchrettoinnertry_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=catchrettoinnertry_d
    if not exist "catchrettoinnertry_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "catchrettoinnertry_d.org" (
            mkdir IL
            copy catchrettoinnertry_d.exe IL\catchrettoinnertry_d.exe 
            ren catchrettoinnertry_d.exe catchrettoinnertry_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchrettoinnertry_d.org /out catchrettoinnertry_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchrettoinnertry_d.org /out catchrettoinnertry_d.exe
...
set ExePath=catchrettoinnertry_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in catchrettoinnertry_d.reflect.xml
    IF EXIST catchrettoinnertry_d.reflect.xml set ReflectionRoots=-x catchrettoinnertry_d.reflect.xml
    ...
    set ExePath=%LinkBin%\catchrettoinnertry_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% catchrettoinnertry_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% catchrettoinnertry_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
