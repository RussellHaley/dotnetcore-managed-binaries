...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/loopinfinally_d loopinfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/loopinfinally_d loopinfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=loopinfinally_d.dasm.il loopinfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=loopinfinally_d.dasm.il loopinfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=loopinfinally_d.asm.exe  loopinfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=loopinfinally_d.asm.exe  loopinfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=loopinfinally_d
    if [ ! -f loopinfinally_d.org ]; then
        TakeLock
        if [ ! -f loopinfinally_d.org ]; then
          mkdir IL
          cp loopinfinally_d.exe IL/loopinfinally_d.exe
          mv loopinfinally_d.exe loopinfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopinfinally_d.org /out loopinfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in loopinfinally_d.org /out loopinfinally_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in loopinfinally_d.reflect.xml
  if [ -f loopinfinally_d.reflect.xml ]
then
    ReflectionRoots="-x loopinfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/loopinfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" loopinfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" loopinfinally_d.asm.exe $CLRTestExecutionArguments
