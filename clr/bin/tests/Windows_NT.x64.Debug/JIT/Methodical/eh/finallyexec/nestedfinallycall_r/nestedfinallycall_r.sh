...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/nestedfinallycall_r nestedfinallycall_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/finallyexec/nestedfinallycall_r nestedfinallycall_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nestedfinallycall_r.dasm.il nestedfinallycall_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=nestedfinallycall_r.dasm.il nestedfinallycall_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nestedfinallycall_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedfinallycall_r.dasm.il
    "$CORE_ROOT/ilasm" -output=nestedfinallycall_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT nestedfinallycall_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nestedfinallycall_r
    if [ ! -f nestedfinallycall_r.org ]; then
        TakeLock
        if [ ! -f nestedfinallycall_r.org ]; then
          mkdir IL
          cp nestedfinallycall_r.exe IL/nestedfinallycall_r.exe
          mv nestedfinallycall_r.exe nestedfinallycall_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedfinallycall_r.org /out nestedfinallycall_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nestedfinallycall_r.org /out nestedfinallycall_r.exe
          __cgExitCode=$?
...
ExePath=nestedfinallycall_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in nestedfinallycall_r.reflect.xml
  if [ -f nestedfinallycall_r.reflect.xml ]
then
    ReflectionRoots="-x nestedfinallycall_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nestedfinallycall_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nestedfinallycall_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nestedfinallycall_r.asm.exe $CLRTestExecutionArguments
