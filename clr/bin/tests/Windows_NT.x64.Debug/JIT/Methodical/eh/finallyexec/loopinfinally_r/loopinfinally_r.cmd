...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\loopinfinally_r loopinfinally_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\loopinfinally_r loopinfinally_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=loopinfinally_r.dasm.il loopinfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=loopinfinally_r.dasm.il loopinfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=loopinfinally_r.asm.exe  loopinfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=loopinfinally_r.asm.exe  loopinfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=loopinfinally_r
    if not exist "loopinfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "loopinfinally_r.org" (
            mkdir IL
            copy loopinfinally_r.exe IL\loopinfinally_r.exe 
            ren loopinfinally_r.exe loopinfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopinfinally_r.org /out loopinfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopinfinally_r.org /out loopinfinally_r.exe
...
set ExePath=loopinfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in loopinfinally_r.reflect.xml
    IF EXIST loopinfinally_r.reflect.xml set ReflectionRoots=-x loopinfinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\loopinfinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% loopinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% loopinfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
