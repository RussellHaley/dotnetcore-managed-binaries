...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\loopinfinally_do loopinfinally_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\finallyexec\loopinfinally_do loopinfinally_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=loopinfinally_do.dasm.il loopinfinally_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=loopinfinally_do.dasm.il loopinfinally_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=loopinfinally_do.asm.exe  loopinfinally_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=loopinfinally_do.asm.exe  loopinfinally_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=loopinfinally_do
    if not exist "loopinfinally_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "loopinfinally_do.org" (
            mkdir IL
            copy loopinfinally_do.exe IL\loopinfinally_do.exe 
            ren loopinfinally_do.exe loopinfinally_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopinfinally_do.org /out loopinfinally_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in loopinfinally_do.org /out loopinfinally_do.exe
...
set ExePath=loopinfinally_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in loopinfinally_do.reflect.xml
    IF EXIST loopinfinally_do.reflect.xml set ReflectionRoots=-x loopinfinally_do.reflect.xml
    ...
    set ExePath=%LinkBin%\loopinfinally_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% loopinfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% loopinfinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
