...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\labelbeforefinally_r labelbeforefinally_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\labelbeforefinally_r labelbeforefinally_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=labelbeforefinally_r.dasm.il labelbeforefinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=labelbeforefinally_r.dasm.il labelbeforefinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=labelbeforefinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT labelbeforefinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=labelbeforefinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT labelbeforefinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=labelbeforefinally_r
    if not exist "labelbeforefinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "labelbeforefinally_r.org" (
            mkdir IL
            copy labelbeforefinally_r.exe IL\labelbeforefinally_r.exe 
            ren labelbeforefinally_r.exe labelbeforefinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in labelbeforefinally_r.org /out labelbeforefinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in labelbeforefinally_r.org /out labelbeforefinally_r.exe
...
set ExePath=labelbeforefinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in labelbeforefinally_r.reflect.xml
    IF EXIST labelbeforefinally_r.reflect.xml set ReflectionRoots=-x labelbeforefinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\labelbeforefinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% labelbeforefinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% labelbeforefinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
