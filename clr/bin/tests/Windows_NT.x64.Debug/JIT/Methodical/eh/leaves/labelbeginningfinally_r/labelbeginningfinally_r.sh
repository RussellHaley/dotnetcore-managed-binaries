...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/labelbeginningfinally_r labelbeginningfinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/labelbeginningfinally_r labelbeginningfinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=labelbeginningfinally_r.dasm.il labelbeginningfinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=labelbeginningfinally_r.dasm.il labelbeginningfinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=labelbeginningfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT labelbeginningfinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=labelbeginningfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT labelbeginningfinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=labelbeginningfinally_r
    if [ ! -f labelbeginningfinally_r.org ]; then
        TakeLock
        if [ ! -f labelbeginningfinally_r.org ]; then
          mkdir IL
          cp labelbeginningfinally_r.exe IL/labelbeginningfinally_r.exe
          mv labelbeginningfinally_r.exe labelbeginningfinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in labelbeginningfinally_r.org /out labelbeginningfinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in labelbeginningfinally_r.org /out labelbeginningfinally_r.exe
          __cgExitCode=$?
...
ExePath=labelbeginningfinally_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in labelbeginningfinally_r.reflect.xml
  if [ -f labelbeginningfinally_r.reflect.xml ]
then
    ReflectionRoots="-x labelbeginningfinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/labelbeginningfinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" labelbeginningfinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" labelbeginningfinally_r.asm.exe $CLRTestExecutionArguments
