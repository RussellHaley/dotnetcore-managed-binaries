...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/labelbeforefinally_r labelbeforefinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/labelbeforefinally_r labelbeforefinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=labelbeforefinally_r.dasm.il labelbeforefinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=labelbeforefinally_r.dasm.il labelbeforefinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=labelbeforefinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT labelbeforefinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=labelbeforefinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT labelbeforefinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=labelbeforefinally_r
    if [ ! -f labelbeforefinally_r.org ]; then
        TakeLock
        if [ ! -f labelbeforefinally_r.org ]; then
          mkdir IL
          cp labelbeforefinally_r.exe IL/labelbeforefinally_r.exe
          mv labelbeforefinally_r.exe labelbeforefinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in labelbeforefinally_r.org /out labelbeforefinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in labelbeforefinally_r.org /out labelbeforefinally_r.exe
          __cgExitCode=$?
...
ExePath=labelbeforefinally_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in labelbeforefinally_r.reflect.xml
  if [ -f labelbeforefinally_r.reflect.xml ]
then
    ReflectionRoots="-x labelbeforefinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/labelbeforefinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" labelbeforefinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" labelbeforefinally_r.asm.exe $CLRTestExecutionArguments
