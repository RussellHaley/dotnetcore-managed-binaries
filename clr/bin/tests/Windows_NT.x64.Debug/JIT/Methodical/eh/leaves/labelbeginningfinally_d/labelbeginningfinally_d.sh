...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/labelbeginningfinally_d labelbeginningfinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/labelbeginningfinally_d labelbeginningfinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=labelbeginningfinally_d.dasm.il labelbeginningfinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=labelbeginningfinally_d.dasm.il labelbeginningfinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=labelbeginningfinally_d.asm.exe -QUIET -NOLOGO -DEBUG labelbeginningfinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=labelbeginningfinally_d.asm.exe -QUIET -NOLOGO -DEBUG labelbeginningfinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=labelbeginningfinally_d
    if [ ! -f labelbeginningfinally_d.org ]; then
        TakeLock
        if [ ! -f labelbeginningfinally_d.org ]; then
          mkdir IL
          cp labelbeginningfinally_d.exe IL/labelbeginningfinally_d.exe
          mv labelbeginningfinally_d.exe labelbeginningfinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in labelbeginningfinally_d.org /out labelbeginningfinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in labelbeginningfinally_d.org /out labelbeginningfinally_d.exe
          __cgExitCode=$?
...
ExePath=labelbeginningfinally_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in labelbeginningfinally_d.reflect.xml
  if [ -f labelbeginningfinally_d.reflect.xml ]
then
    ReflectionRoots="-x labelbeginningfinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/labelbeginningfinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" labelbeginningfinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" labelbeginningfinally_d.asm.exe $CLRTestExecutionArguments
