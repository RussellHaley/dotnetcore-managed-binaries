...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\catchretnonlocalexitinfunclet_d catchretnonlocalexitinfunclet_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\catchretnonlocalexitinfunclet_d catchretnonlocalexitinfunclet_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=catchretnonlocalexitinfunclet_d.dasm.il catchretnonlocalexitinfunclet_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=catchretnonlocalexitinfunclet_d.dasm.il catchretnonlocalexitinfunclet_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=catchretnonlocalexitinfunclet_d.asm.exe  catchretnonlocalexitinfunclet_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=catchretnonlocalexitinfunclet_d.asm.exe  catchretnonlocalexitinfunclet_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=catchretnonlocalexitinfunclet_d
    if not exist "catchretnonlocalexitinfunclet_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "catchretnonlocalexitinfunclet_d.org" (
            mkdir IL
            copy catchretnonlocalexitinfunclet_d.exe IL\catchretnonlocalexitinfunclet_d.exe 
            ren catchretnonlocalexitinfunclet_d.exe catchretnonlocalexitinfunclet_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchretnonlocalexitinfunclet_d.org /out catchretnonlocalexitinfunclet_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchretnonlocalexitinfunclet_d.org /out catchretnonlocalexitinfunclet_d.exe
...
set ExePath=catchretnonlocalexitinfunclet_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in catchretnonlocalexitinfunclet_d.reflect.xml
    IF EXIST catchretnonlocalexitinfunclet_d.reflect.xml set ReflectionRoots=-x catchretnonlocalexitinfunclet_d.reflect.xml
    ...
    set ExePath=%LinkBin%\catchretnonlocalexitinfunclet_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% catchretnonlocalexitinfunclet_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% catchretnonlocalexitinfunclet_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
