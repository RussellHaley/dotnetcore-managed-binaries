...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/oponerror_d oponerror_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/oponerror_d oponerror_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=oponerror_d.dasm.il oponerror_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=oponerror_d.dasm.il oponerror_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=oponerror_d.asm.exe  oponerror_d.dasm.il
    "$CORE_ROOT/ilasm" -output=oponerror_d.asm.exe  oponerror_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=oponerror_d
    if [ ! -f oponerror_d.org ]; then
        TakeLock
        if [ ! -f oponerror_d.org ]; then
          mkdir IL
          cp oponerror_d.exe IL/oponerror_d.exe
          mv oponerror_d.exe oponerror_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in oponerror_d.org /out oponerror_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in oponerror_d.org /out oponerror_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in oponerror_d.reflect.xml
  if [ -f oponerror_d.reflect.xml ]
then
    ReflectionRoots="-x oponerror_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/oponerror_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" oponerror_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" oponerror_d.asm.exe $CLRTestExecutionArguments
