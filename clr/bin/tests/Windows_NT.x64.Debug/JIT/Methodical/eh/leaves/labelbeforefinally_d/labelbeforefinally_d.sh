...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/labelbeforefinally_d labelbeforefinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/labelbeforefinally_d labelbeforefinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=labelbeforefinally_d.dasm.il labelbeforefinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=labelbeforefinally_d.dasm.il labelbeforefinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=labelbeforefinally_d.asm.exe -QUIET -NOLOGO -DEBUG labelbeforefinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=labelbeforefinally_d.asm.exe -QUIET -NOLOGO -DEBUG labelbeforefinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=labelbeforefinally_d
    if [ ! -f labelbeforefinally_d.org ]; then
        TakeLock
        if [ ! -f labelbeforefinally_d.org ]; then
          mkdir IL
          cp labelbeforefinally_d.exe IL/labelbeforefinally_d.exe
          mv labelbeforefinally_d.exe labelbeforefinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in labelbeforefinally_d.org /out labelbeforefinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in labelbeforefinally_d.org /out labelbeforefinally_d.exe
          __cgExitCode=$?
...
ExePath=labelbeforefinally_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in labelbeforefinally_d.reflect.xml
  if [ -f labelbeforefinally_d.reflect.xml ]
then
    ReflectionRoots="-x labelbeforefinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/labelbeforefinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" labelbeforefinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" labelbeforefinally_d.asm.exe $CLRTestExecutionArguments
