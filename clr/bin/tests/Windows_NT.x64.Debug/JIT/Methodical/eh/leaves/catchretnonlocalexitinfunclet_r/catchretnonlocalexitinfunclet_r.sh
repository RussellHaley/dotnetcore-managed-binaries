...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/catchretnonlocalexitinfunclet_r catchretnonlocalexitinfunclet_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/catchretnonlocalexitinfunclet_r catchretnonlocalexitinfunclet_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=catchretnonlocalexitinfunclet_r.dasm.il catchretnonlocalexitinfunclet_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=catchretnonlocalexitinfunclet_r.dasm.il catchretnonlocalexitinfunclet_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=catchretnonlocalexitinfunclet_r.asm.exe  catchretnonlocalexitinfunclet_r.dasm.il
    "$CORE_ROOT/ilasm" -output=catchretnonlocalexitinfunclet_r.asm.exe  catchretnonlocalexitinfunclet_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=catchretnonlocalexitinfunclet_r
    if [ ! -f catchretnonlocalexitinfunclet_r.org ]; then
        TakeLock
        if [ ! -f catchretnonlocalexitinfunclet_r.org ]; then
          mkdir IL
          cp catchretnonlocalexitinfunclet_r.exe IL/catchretnonlocalexitinfunclet_r.exe
          mv catchretnonlocalexitinfunclet_r.exe catchretnonlocalexitinfunclet_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchretnonlocalexitinfunclet_r.org /out catchretnonlocalexitinfunclet_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchretnonlocalexitinfunclet_r.org /out catchretnonlocalexitinfunclet_r.exe
          __cgExitCode=$?
...
ExePath=catchretnonlocalexitinfunclet_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in catchretnonlocalexitinfunclet_r.reflect.xml
  if [ -f catchretnonlocalexitinfunclet_r.reflect.xml ]
then
    ReflectionRoots="-x catchretnonlocalexitinfunclet_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/catchretnonlocalexitinfunclet_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" catchretnonlocalexitinfunclet_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" catchretnonlocalexitinfunclet_r.asm.exe $CLRTestExecutionArguments
