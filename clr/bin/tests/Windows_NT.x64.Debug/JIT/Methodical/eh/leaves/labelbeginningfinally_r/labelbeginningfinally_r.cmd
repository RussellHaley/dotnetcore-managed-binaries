...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\labelbeginningfinally_r labelbeginningfinally_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\labelbeginningfinally_r labelbeginningfinally_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=labelbeginningfinally_r.dasm.il labelbeginningfinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=labelbeginningfinally_r.dasm.il labelbeginningfinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=labelbeginningfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT labelbeginningfinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=labelbeginningfinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT labelbeginningfinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=labelbeginningfinally_r
    if not exist "labelbeginningfinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "labelbeginningfinally_r.org" (
            mkdir IL
            copy labelbeginningfinally_r.exe IL\labelbeginningfinally_r.exe 
            ren labelbeginningfinally_r.exe labelbeginningfinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in labelbeginningfinally_r.org /out labelbeginningfinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in labelbeginningfinally_r.org /out labelbeginningfinally_r.exe
...
set ExePath=labelbeginningfinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in labelbeginningfinally_r.reflect.xml
    IF EXIST labelbeginningfinally_r.reflect.xml set ReflectionRoots=-x labelbeginningfinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\labelbeginningfinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% labelbeginningfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% labelbeginningfinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
