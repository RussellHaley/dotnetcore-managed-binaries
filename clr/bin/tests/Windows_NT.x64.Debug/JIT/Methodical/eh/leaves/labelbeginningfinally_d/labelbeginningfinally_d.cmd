...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\labelbeginningfinally_d labelbeginningfinally_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\labelbeginningfinally_d labelbeginningfinally_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=labelbeginningfinally_d.dasm.il labelbeginningfinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=labelbeginningfinally_d.dasm.il labelbeginningfinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=labelbeginningfinally_d.asm.exe -QUIET -NOLOGO -DEBUG labelbeginningfinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=labelbeginningfinally_d.asm.exe -QUIET -NOLOGO -DEBUG labelbeginningfinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=labelbeginningfinally_d
    if not exist "labelbeginningfinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "labelbeginningfinally_d.org" (
            mkdir IL
            copy labelbeginningfinally_d.exe IL\labelbeginningfinally_d.exe 
            ren labelbeginningfinally_d.exe labelbeginningfinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in labelbeginningfinally_d.org /out labelbeginningfinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in labelbeginningfinally_d.org /out labelbeginningfinally_d.exe
...
set ExePath=labelbeginningfinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in labelbeginningfinally_d.reflect.xml
    IF EXIST labelbeginningfinally_d.reflect.xml set ReflectionRoots=-x labelbeginningfinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\labelbeginningfinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% labelbeginningfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% labelbeginningfinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
