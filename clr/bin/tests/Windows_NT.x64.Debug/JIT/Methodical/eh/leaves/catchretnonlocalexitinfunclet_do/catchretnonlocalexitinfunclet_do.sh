...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/catchretnonlocalexitinfunclet_do catchretnonlocalexitinfunclet_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/catchretnonlocalexitinfunclet_do catchretnonlocalexitinfunclet_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=catchretnonlocalexitinfunclet_do.dasm.il catchretnonlocalexitinfunclet_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=catchretnonlocalexitinfunclet_do.dasm.il catchretnonlocalexitinfunclet_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=catchretnonlocalexitinfunclet_do.asm.exe  catchretnonlocalexitinfunclet_do.dasm.il
    "$CORE_ROOT/ilasm" -output=catchretnonlocalexitinfunclet_do.asm.exe  catchretnonlocalexitinfunclet_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=catchretnonlocalexitinfunclet_do
    if [ ! -f catchretnonlocalexitinfunclet_do.org ]; then
        TakeLock
        if [ ! -f catchretnonlocalexitinfunclet_do.org ]; then
          mkdir IL
          cp catchretnonlocalexitinfunclet_do.exe IL/catchretnonlocalexitinfunclet_do.exe
          mv catchretnonlocalexitinfunclet_do.exe catchretnonlocalexitinfunclet_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchretnonlocalexitinfunclet_do.org /out catchretnonlocalexitinfunclet_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchretnonlocalexitinfunclet_do.org /out catchretnonlocalexitinfunclet_do.exe
          __cgExitCode=$?
...
ExePath=catchretnonlocalexitinfunclet_do.exe
...
then
  ...
  # Use hints for reflection roots, if provided in catchretnonlocalexitinfunclet_do.reflect.xml
  if [ -f catchretnonlocalexitinfunclet_do.reflect.xml ]
then
    ReflectionRoots="-x catchretnonlocalexitinfunclet_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/catchretnonlocalexitinfunclet_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" catchretnonlocalexitinfunclet_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" catchretnonlocalexitinfunclet_do.asm.exe $CLRTestExecutionArguments
