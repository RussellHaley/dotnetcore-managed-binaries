...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\catchretnonlocalexitinfunclet_do catchretnonlocalexitinfunclet_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\catchretnonlocalexitinfunclet_do catchretnonlocalexitinfunclet_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=catchretnonlocalexitinfunclet_do.dasm.il catchretnonlocalexitinfunclet_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=catchretnonlocalexitinfunclet_do.dasm.il catchretnonlocalexitinfunclet_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=catchretnonlocalexitinfunclet_do.asm.exe  catchretnonlocalexitinfunclet_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=catchretnonlocalexitinfunclet_do.asm.exe  catchretnonlocalexitinfunclet_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=catchretnonlocalexitinfunclet_do
    if not exist "catchretnonlocalexitinfunclet_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "catchretnonlocalexitinfunclet_do.org" (
            mkdir IL
            copy catchretnonlocalexitinfunclet_do.exe IL\catchretnonlocalexitinfunclet_do.exe 
            ren catchretnonlocalexitinfunclet_do.exe catchretnonlocalexitinfunclet_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchretnonlocalexitinfunclet_do.org /out catchretnonlocalexitinfunclet_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchretnonlocalexitinfunclet_do.org /out catchretnonlocalexitinfunclet_do.exe
...
set ExePath=catchretnonlocalexitinfunclet_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in catchretnonlocalexitinfunclet_do.reflect.xml
    IF EXIST catchretnonlocalexitinfunclet_do.reflect.xml set ReflectionRoots=-x catchretnonlocalexitinfunclet_do.reflect.xml
    ...
    set ExePath=%LinkBin%\catchretnonlocalexitinfunclet_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% catchretnonlocalexitinfunclet_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% catchretnonlocalexitinfunclet_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
