...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/catchretnonlocalexitinfunclet_d catchretnonlocalexitinfunclet_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/catchretnonlocalexitinfunclet_d catchretnonlocalexitinfunclet_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=catchretnonlocalexitinfunclet_d.dasm.il catchretnonlocalexitinfunclet_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=catchretnonlocalexitinfunclet_d.dasm.il catchretnonlocalexitinfunclet_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=catchretnonlocalexitinfunclet_d.asm.exe  catchretnonlocalexitinfunclet_d.dasm.il
    "$CORE_ROOT/ilasm" -output=catchretnonlocalexitinfunclet_d.asm.exe  catchretnonlocalexitinfunclet_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=catchretnonlocalexitinfunclet_d
    if [ ! -f catchretnonlocalexitinfunclet_d.org ]; then
        TakeLock
        if [ ! -f catchretnonlocalexitinfunclet_d.org ]; then
          mkdir IL
          cp catchretnonlocalexitinfunclet_d.exe IL/catchretnonlocalexitinfunclet_d.exe
          mv catchretnonlocalexitinfunclet_d.exe catchretnonlocalexitinfunclet_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchretnonlocalexitinfunclet_d.org /out catchretnonlocalexitinfunclet_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchretnonlocalexitinfunclet_d.org /out catchretnonlocalexitinfunclet_d.exe
          __cgExitCode=$?
...
ExePath=catchretnonlocalexitinfunclet_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in catchretnonlocalexitinfunclet_d.reflect.xml
  if [ -f catchretnonlocalexitinfunclet_d.reflect.xml ]
then
    ReflectionRoots="-x catchretnonlocalexitinfunclet_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/catchretnonlocalexitinfunclet_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" catchretnonlocalexitinfunclet_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" catchretnonlocalexitinfunclet_d.asm.exe $CLRTestExecutionArguments
