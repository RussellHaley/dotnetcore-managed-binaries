...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\labelbeforefinally_d labelbeforefinally_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\labelbeforefinally_d labelbeforefinally_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=labelbeforefinally_d.dasm.il labelbeforefinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=labelbeforefinally_d.dasm.il labelbeforefinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=labelbeforefinally_d.asm.exe -QUIET -NOLOGO -DEBUG labelbeforefinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=labelbeforefinally_d.asm.exe -QUIET -NOLOGO -DEBUG labelbeforefinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=labelbeforefinally_d
    if not exist "labelbeforefinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "labelbeforefinally_d.org" (
            mkdir IL
            copy labelbeforefinally_d.exe IL\labelbeforefinally_d.exe 
            ren labelbeforefinally_d.exe labelbeforefinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in labelbeforefinally_d.org /out labelbeforefinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in labelbeforefinally_d.org /out labelbeforefinally_d.exe
...
set ExePath=labelbeforefinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in labelbeforefinally_d.reflect.xml
    IF EXIST labelbeforefinally_d.reflect.xml set ReflectionRoots=-x labelbeforefinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\labelbeforefinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% labelbeforefinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% labelbeforefinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
