...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/oponerror_do oponerror_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/oponerror_do oponerror_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=oponerror_do.dasm.il oponerror_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=oponerror_do.dasm.il oponerror_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=oponerror_do.asm.exe  oponerror_do.dasm.il
    "$CORE_ROOT/ilasm" -output=oponerror_do.asm.exe  oponerror_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=oponerror_do
    if [ ! -f oponerror_do.org ]; then
        TakeLock
        if [ ! -f oponerror_do.org ]; then
          mkdir IL
          cp oponerror_do.exe IL/oponerror_do.exe
          mv oponerror_do.exe oponerror_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in oponerror_do.org /out oponerror_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in oponerror_do.org /out oponerror_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in oponerror_do.reflect.xml
  if [ -f oponerror_do.reflect.xml ]
then
    ReflectionRoots="-x oponerror_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/oponerror_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" oponerror_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" oponerror_do.asm.exe $CLRTestExecutionArguments
