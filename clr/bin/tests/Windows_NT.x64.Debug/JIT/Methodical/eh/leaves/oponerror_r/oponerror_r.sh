...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/oponerror_r oponerror_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/oponerror_r oponerror_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=oponerror_r.dasm.il oponerror_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=oponerror_r.dasm.il oponerror_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=oponerror_r.asm.exe  oponerror_r.dasm.il
    "$CORE_ROOT/ilasm" -output=oponerror_r.asm.exe  oponerror_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=oponerror_r
    if [ ! -f oponerror_r.org ]; then
        TakeLock
        if [ ! -f oponerror_r.org ]; then
          mkdir IL
          cp oponerror_r.exe IL/oponerror_r.exe
          mv oponerror_r.exe oponerror_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in oponerror_r.org /out oponerror_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in oponerror_r.org /out oponerror_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in oponerror_r.reflect.xml
  if [ -f oponerror_r.reflect.xml ]
then
    ReflectionRoots="-x oponerror_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/oponerror_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" oponerror_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" oponerror_r.asm.exe $CLRTestExecutionArguments
