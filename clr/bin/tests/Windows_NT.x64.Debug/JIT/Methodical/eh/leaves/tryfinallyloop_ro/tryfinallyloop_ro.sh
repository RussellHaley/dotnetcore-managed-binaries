...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/tryfinallyloop_ro tryfinallyloop_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/tryfinallyloop_ro tryfinallyloop_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tryfinallyloop_ro.dasm.il tryfinallyloop_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=tryfinallyloop_ro.dasm.il tryfinallyloop_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tryfinallyloop_ro.asm.exe  tryfinallyloop_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=tryfinallyloop_ro.asm.exe  tryfinallyloop_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tryfinallyloop_ro
    if [ ! -f tryfinallyloop_ro.org ]; then
        TakeLock
        if [ ! -f tryfinallyloop_ro.org ]; then
          mkdir IL
          cp tryfinallyloop_ro.exe IL/tryfinallyloop_ro.exe
          mv tryfinallyloop_ro.exe tryfinallyloop_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfinallyloop_ro.org /out tryfinallyloop_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tryfinallyloop_ro.org /out tryfinallyloop_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in tryfinallyloop_ro.reflect.xml
  if [ -f tryfinallyloop_ro.reflect.xml ]
then
    ReflectionRoots="-x tryfinallyloop_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/tryfinallyloop_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tryfinallyloop_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tryfinallyloop_ro.asm.exe $CLRTestExecutionArguments
