...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\catchretnonlocalexitinfunclet_r catchretnonlocalexitinfunclet_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\catchretnonlocalexitinfunclet_r catchretnonlocalexitinfunclet_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=catchretnonlocalexitinfunclet_r.dasm.il catchretnonlocalexitinfunclet_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=catchretnonlocalexitinfunclet_r.dasm.il catchretnonlocalexitinfunclet_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=catchretnonlocalexitinfunclet_r.asm.exe  catchretnonlocalexitinfunclet_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=catchretnonlocalexitinfunclet_r.asm.exe  catchretnonlocalexitinfunclet_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=catchretnonlocalexitinfunclet_r
    if not exist "catchretnonlocalexitinfunclet_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "catchretnonlocalexitinfunclet_r.org" (
            mkdir IL
            copy catchretnonlocalexitinfunclet_r.exe IL\catchretnonlocalexitinfunclet_r.exe 
            ren catchretnonlocalexitinfunclet_r.exe catchretnonlocalexitinfunclet_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchretnonlocalexitinfunclet_r.org /out catchretnonlocalexitinfunclet_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in catchretnonlocalexitinfunclet_r.org /out catchretnonlocalexitinfunclet_r.exe
...
set ExePath=catchretnonlocalexitinfunclet_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in catchretnonlocalexitinfunclet_r.reflect.xml
    IF EXIST catchretnonlocalexitinfunclet_r.reflect.xml set ReflectionRoots=-x catchretnonlocalexitinfunclet_r.reflect.xml
    ...
    set ExePath=%LinkBin%\catchretnonlocalexitinfunclet_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% catchretnonlocalexitinfunclet_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% catchretnonlocalexitinfunclet_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
