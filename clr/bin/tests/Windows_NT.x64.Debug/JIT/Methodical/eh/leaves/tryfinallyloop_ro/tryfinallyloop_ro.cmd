...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\tryfinallyloop_ro tryfinallyloop_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\leaves\tryfinallyloop_ro tryfinallyloop_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tryfinallyloop_ro.dasm.il tryfinallyloop_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tryfinallyloop_ro.dasm.il tryfinallyloop_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tryfinallyloop_ro.asm.exe  tryfinallyloop_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tryfinallyloop_ro.asm.exe  tryfinallyloop_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tryfinallyloop_ro
    if not exist "tryfinallyloop_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tryfinallyloop_ro.org" (
            mkdir IL
            copy tryfinallyloop_ro.exe IL\tryfinallyloop_ro.exe 
            ren tryfinallyloop_ro.exe tryfinallyloop_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfinallyloop_ro.org /out tryfinallyloop_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tryfinallyloop_ro.org /out tryfinallyloop_ro.exe
...
set ExePath=tryfinallyloop_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tryfinallyloop_ro.reflect.xml
    IF EXIST tryfinallyloop_ro.reflect.xml set ReflectionRoots=-x tryfinallyloop_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\tryfinallyloop_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tryfinallyloop_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tryfinallyloop_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
