...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/catchretnonlocalexitinfunclet_ro catchretnonlocalexitinfunclet_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/leaves/catchretnonlocalexitinfunclet_ro catchretnonlocalexitinfunclet_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=catchretnonlocalexitinfunclet_ro.dasm.il catchretnonlocalexitinfunclet_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=catchretnonlocalexitinfunclet_ro.dasm.il catchretnonlocalexitinfunclet_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=catchretnonlocalexitinfunclet_ro.asm.exe  catchretnonlocalexitinfunclet_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=catchretnonlocalexitinfunclet_ro.asm.exe  catchretnonlocalexitinfunclet_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=catchretnonlocalexitinfunclet_ro
    if [ ! -f catchretnonlocalexitinfunclet_ro.org ]; then
        TakeLock
        if [ ! -f catchretnonlocalexitinfunclet_ro.org ]; then
          mkdir IL
          cp catchretnonlocalexitinfunclet_ro.exe IL/catchretnonlocalexitinfunclet_ro.exe
          mv catchretnonlocalexitinfunclet_ro.exe catchretnonlocalexitinfunclet_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchretnonlocalexitinfunclet_ro.org /out catchretnonlocalexitinfunclet_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in catchretnonlocalexitinfunclet_ro.org /out catchretnonlocalexitinfunclet_ro.exe
          __cgExitCode=$?
...
ExePath=catchretnonlocalexitinfunclet_ro.exe
...
then
  ...
  # Use hints for reflection roots, if provided in catchretnonlocalexitinfunclet_ro.reflect.xml
  if [ -f catchretnonlocalexitinfunclet_ro.reflect.xml ]
then
    ReflectionRoots="-x catchretnonlocalexitinfunclet_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/catchretnonlocalexitinfunclet_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" catchretnonlocalexitinfunclet_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" catchretnonlocalexitinfunclet_ro.asm.exe $CLRTestExecutionArguments
