...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/reversedtryblock_d reversedtryblock_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/reversedtryblock_d reversedtryblock_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=reversedtryblock_d.dasm.il reversedtryblock_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=reversedtryblock_d.dasm.il reversedtryblock_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=reversedtryblock_d.asm.exe -QUIET -NOLOGO -DEBUG reversedtryblock_d.dasm.il
    "$CORE_ROOT/ilasm" -output=reversedtryblock_d.asm.exe -QUIET -NOLOGO -DEBUG reversedtryblock_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=reversedtryblock_d
    if [ ! -f reversedtryblock_d.org ]; then
        TakeLock
        if [ ! -f reversedtryblock_d.org ]; then
          mkdir IL
          cp reversedtryblock_d.exe IL/reversedtryblock_d.exe
          mv reversedtryblock_d.exe reversedtryblock_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reversedtryblock_d.org /out reversedtryblock_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reversedtryblock_d.org /out reversedtryblock_d.exe
          __cgExitCode=$?
...
ExePath=reversedtryblock_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in reversedtryblock_d.reflect.xml
  if [ -f reversedtryblock_d.reflect.xml ]
then
    ReflectionRoots="-x reversedtryblock_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/reversedtryblock_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" reversedtryblock_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" reversedtryblock_d.asm.exe $CLRTestExecutionArguments
