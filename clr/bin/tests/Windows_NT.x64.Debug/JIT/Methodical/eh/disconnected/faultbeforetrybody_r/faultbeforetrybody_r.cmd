...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\faultbeforetrybody_r faultbeforetrybody_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\faultbeforetrybody_r faultbeforetrybody_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=faultbeforetrybody_r.dasm.il faultbeforetrybody_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=faultbeforetrybody_r.dasm.il faultbeforetrybody_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=faultbeforetrybody_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT faultbeforetrybody_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=faultbeforetrybody_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT faultbeforetrybody_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=faultbeforetrybody_r
    if not exist "faultbeforetrybody_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "faultbeforetrybody_r.org" (
            mkdir IL
            copy faultbeforetrybody_r.exe IL\faultbeforetrybody_r.exe 
            ren faultbeforetrybody_r.exe faultbeforetrybody_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in faultbeforetrybody_r.org /out faultbeforetrybody_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in faultbeforetrybody_r.org /out faultbeforetrybody_r.exe
...
set ExePath=faultbeforetrybody_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in faultbeforetrybody_r.reflect.xml
    IF EXIST faultbeforetrybody_r.reflect.xml set ReflectionRoots=-x faultbeforetrybody_r.reflect.xml
    ...
    set ExePath=%LinkBin%\faultbeforetrybody_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% faultbeforetrybody_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% faultbeforetrybody_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
