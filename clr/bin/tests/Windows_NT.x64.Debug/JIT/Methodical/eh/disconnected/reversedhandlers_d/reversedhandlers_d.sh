...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/reversedhandlers_d reversedhandlers_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/reversedhandlers_d reversedhandlers_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=reversedhandlers_d.dasm.il reversedhandlers_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=reversedhandlers_d.dasm.il reversedhandlers_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=reversedhandlers_d.asm.exe -QUIET -NOLOGO -DEBUG reversedhandlers_d.dasm.il
    "$CORE_ROOT/ilasm" -output=reversedhandlers_d.asm.exe -QUIET -NOLOGO -DEBUG reversedhandlers_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=reversedhandlers_d
    if [ ! -f reversedhandlers_d.org ]; then
        TakeLock
        if [ ! -f reversedhandlers_d.org ]; then
          mkdir IL
          cp reversedhandlers_d.exe IL/reversedhandlers_d.exe
          mv reversedhandlers_d.exe reversedhandlers_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reversedhandlers_d.org /out reversedhandlers_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reversedhandlers_d.org /out reversedhandlers_d.exe
          __cgExitCode=$?
...
ExePath=reversedhandlers_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in reversedhandlers_d.reflect.xml
  if [ -f reversedhandlers_d.reflect.xml ]
then
    ReflectionRoots="-x reversedhandlers_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/reversedhandlers_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" reversedhandlers_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" reversedhandlers_d.asm.exe $CLRTestExecutionArguments
