...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\reversedtryblock_d reversedtryblock_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\reversedtryblock_d reversedtryblock_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=reversedtryblock_d.dasm.il reversedtryblock_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=reversedtryblock_d.dasm.il reversedtryblock_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=reversedtryblock_d.asm.exe -QUIET -NOLOGO -DEBUG reversedtryblock_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=reversedtryblock_d.asm.exe -QUIET -NOLOGO -DEBUG reversedtryblock_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=reversedtryblock_d
    if not exist "reversedtryblock_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "reversedtryblock_d.org" (
            mkdir IL
            copy reversedtryblock_d.exe IL\reversedtryblock_d.exe 
            ren reversedtryblock_d.exe reversedtryblock_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reversedtryblock_d.org /out reversedtryblock_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reversedtryblock_d.org /out reversedtryblock_d.exe
...
set ExePath=reversedtryblock_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in reversedtryblock_d.reflect.xml
    IF EXIST reversedtryblock_d.reflect.xml set ReflectionRoots=-x reversedtryblock_d.reflect.xml
    ...
    set ExePath=%LinkBin%\reversedtryblock_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% reversedtryblock_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% reversedtryblock_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
