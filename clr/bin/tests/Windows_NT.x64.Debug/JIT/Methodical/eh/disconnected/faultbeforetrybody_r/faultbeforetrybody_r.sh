...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/faultbeforetrybody_r faultbeforetrybody_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/faultbeforetrybody_r faultbeforetrybody_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=faultbeforetrybody_r.dasm.il faultbeforetrybody_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=faultbeforetrybody_r.dasm.il faultbeforetrybody_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=faultbeforetrybody_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT faultbeforetrybody_r.dasm.il
    "$CORE_ROOT/ilasm" -output=faultbeforetrybody_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT faultbeforetrybody_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=faultbeforetrybody_r
    if [ ! -f faultbeforetrybody_r.org ]; then
        TakeLock
        if [ ! -f faultbeforetrybody_r.org ]; then
          mkdir IL
          cp faultbeforetrybody_r.exe IL/faultbeforetrybody_r.exe
          mv faultbeforetrybody_r.exe faultbeforetrybody_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in faultbeforetrybody_r.org /out faultbeforetrybody_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in faultbeforetrybody_r.org /out faultbeforetrybody_r.exe
          __cgExitCode=$?
...
ExePath=faultbeforetrybody_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in faultbeforetrybody_r.reflect.xml
  if [ -f faultbeforetrybody_r.reflect.xml ]
then
    ReflectionRoots="-x faultbeforetrybody_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/faultbeforetrybody_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" faultbeforetrybody_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" faultbeforetrybody_r.asm.exe $CLRTestExecutionArguments
