...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/reversedtryblock_r reversedtryblock_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/reversedtryblock_r reversedtryblock_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=reversedtryblock_r.dasm.il reversedtryblock_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=reversedtryblock_r.dasm.il reversedtryblock_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=reversedtryblock_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT reversedtryblock_r.dasm.il
    "$CORE_ROOT/ilasm" -output=reversedtryblock_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT reversedtryblock_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=reversedtryblock_r
    if [ ! -f reversedtryblock_r.org ]; then
        TakeLock
        if [ ! -f reversedtryblock_r.org ]; then
          mkdir IL
          cp reversedtryblock_r.exe IL/reversedtryblock_r.exe
          mv reversedtryblock_r.exe reversedtryblock_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reversedtryblock_r.org /out reversedtryblock_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reversedtryblock_r.org /out reversedtryblock_r.exe
          __cgExitCode=$?
...
ExePath=reversedtryblock_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in reversedtryblock_r.reflect.xml
  if [ -f reversedtryblock_r.reflect.xml ]
then
    ReflectionRoots="-x reversedtryblock_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/reversedtryblock_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" reversedtryblock_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" reversedtryblock_r.asm.exe $CLRTestExecutionArguments
