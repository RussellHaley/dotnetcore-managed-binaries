...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/reversedhandlers_r reversedhandlers_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/reversedhandlers_r reversedhandlers_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=reversedhandlers_r.dasm.il reversedhandlers_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=reversedhandlers_r.dasm.il reversedhandlers_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=reversedhandlers_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT reversedhandlers_r.dasm.il
    "$CORE_ROOT/ilasm" -output=reversedhandlers_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT reversedhandlers_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=reversedhandlers_r
    if [ ! -f reversedhandlers_r.org ]; then
        TakeLock
        if [ ! -f reversedhandlers_r.org ]; then
          mkdir IL
          cp reversedhandlers_r.exe IL/reversedhandlers_r.exe
          mv reversedhandlers_r.exe reversedhandlers_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reversedhandlers_r.org /out reversedhandlers_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reversedhandlers_r.org /out reversedhandlers_r.exe
          __cgExitCode=$?
...
ExePath=reversedhandlers_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in reversedhandlers_r.reflect.xml
  if [ -f reversedhandlers_r.reflect.xml ]
then
    ReflectionRoots="-x reversedhandlers_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/reversedhandlers_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" reversedhandlers_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" reversedhandlers_r.asm.exe $CLRTestExecutionArguments
