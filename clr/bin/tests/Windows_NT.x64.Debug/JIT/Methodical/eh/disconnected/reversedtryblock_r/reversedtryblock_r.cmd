...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\reversedtryblock_r reversedtryblock_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\reversedtryblock_r reversedtryblock_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=reversedtryblock_r.dasm.il reversedtryblock_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=reversedtryblock_r.dasm.il reversedtryblock_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=reversedtryblock_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT reversedtryblock_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=reversedtryblock_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT reversedtryblock_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=reversedtryblock_r
    if not exist "reversedtryblock_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "reversedtryblock_r.org" (
            mkdir IL
            copy reversedtryblock_r.exe IL\reversedtryblock_r.exe 
            ren reversedtryblock_r.exe reversedtryblock_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reversedtryblock_r.org /out reversedtryblock_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reversedtryblock_r.org /out reversedtryblock_r.exe
...
set ExePath=reversedtryblock_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in reversedtryblock_r.reflect.xml
    IF EXIST reversedtryblock_r.reflect.xml set ReflectionRoots=-x reversedtryblock_r.reflect.xml
    ...
    set ExePath=%LinkBin%\reversedtryblock_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% reversedtryblock_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% reversedtryblock_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
