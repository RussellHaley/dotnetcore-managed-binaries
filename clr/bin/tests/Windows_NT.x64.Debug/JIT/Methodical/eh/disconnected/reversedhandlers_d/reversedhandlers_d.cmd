...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\reversedhandlers_d reversedhandlers_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\reversedhandlers_d reversedhandlers_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=reversedhandlers_d.dasm.il reversedhandlers_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=reversedhandlers_d.dasm.il reversedhandlers_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=reversedhandlers_d.asm.exe -QUIET -NOLOGO -DEBUG reversedhandlers_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=reversedhandlers_d.asm.exe -QUIET -NOLOGO -DEBUG reversedhandlers_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=reversedhandlers_d
    if not exist "reversedhandlers_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "reversedhandlers_d.org" (
            mkdir IL
            copy reversedhandlers_d.exe IL\reversedhandlers_d.exe 
            ren reversedhandlers_d.exe reversedhandlers_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reversedhandlers_d.org /out reversedhandlers_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reversedhandlers_d.org /out reversedhandlers_d.exe
...
set ExePath=reversedhandlers_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in reversedhandlers_d.reflect.xml
    IF EXIST reversedhandlers_d.reflect.xml set ReflectionRoots=-x reversedhandlers_d.reflect.xml
    ...
    set ExePath=%LinkBin%\reversedhandlers_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% reversedhandlers_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% reversedhandlers_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
