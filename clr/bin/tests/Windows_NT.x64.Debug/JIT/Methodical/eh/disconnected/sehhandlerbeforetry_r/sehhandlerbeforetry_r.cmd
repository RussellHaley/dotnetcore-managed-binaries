...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\sehhandlerbeforetry_r sehhandlerbeforetry_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\sehhandlerbeforetry_r sehhandlerbeforetry_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=sehhandlerbeforetry_r.dasm.il sehhandlerbeforetry_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=sehhandlerbeforetry_r.dasm.il sehhandlerbeforetry_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=sehhandlerbeforetry_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT sehhandlerbeforetry_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=sehhandlerbeforetry_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT sehhandlerbeforetry_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=sehhandlerbeforetry_r
    if not exist "sehhandlerbeforetry_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "sehhandlerbeforetry_r.org" (
            mkdir IL
            copy sehhandlerbeforetry_r.exe IL\sehhandlerbeforetry_r.exe 
            ren sehhandlerbeforetry_r.exe sehhandlerbeforetry_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in sehhandlerbeforetry_r.org /out sehhandlerbeforetry_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in sehhandlerbeforetry_r.org /out sehhandlerbeforetry_r.exe
...
set ExePath=sehhandlerbeforetry_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in sehhandlerbeforetry_r.reflect.xml
    IF EXIST sehhandlerbeforetry_r.reflect.xml set ReflectionRoots=-x sehhandlerbeforetry_r.reflect.xml
    ...
    set ExePath=%LinkBin%\sehhandlerbeforetry_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% sehhandlerbeforetry_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% sehhandlerbeforetry_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
