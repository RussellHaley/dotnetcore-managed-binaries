...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\reversedhandlers_r reversedhandlers_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\disconnected\reversedhandlers_r reversedhandlers_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=reversedhandlers_r.dasm.il reversedhandlers_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=reversedhandlers_r.dasm.il reversedhandlers_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=reversedhandlers_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT reversedhandlers_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=reversedhandlers_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT reversedhandlers_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=reversedhandlers_r
    if not exist "reversedhandlers_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "reversedhandlers_r.org" (
            mkdir IL
            copy reversedhandlers_r.exe IL\reversedhandlers_r.exe 
            ren reversedhandlers_r.exe reversedhandlers_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reversedhandlers_r.org /out reversedhandlers_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reversedhandlers_r.org /out reversedhandlers_r.exe
...
set ExePath=reversedhandlers_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in reversedhandlers_r.reflect.xml
    IF EXIST reversedhandlers_r.reflect.xml set ReflectionRoots=-x reversedhandlers_r.reflect.xml
    ...
    set ExePath=%LinkBin%\reversedhandlers_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% reversedhandlers_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% reversedhandlers_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
