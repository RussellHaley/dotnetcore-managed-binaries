...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/sehhandlerbeforetry_r sehhandlerbeforetry_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/disconnected/sehhandlerbeforetry_r sehhandlerbeforetry_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=sehhandlerbeforetry_r.dasm.il sehhandlerbeforetry_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=sehhandlerbeforetry_r.dasm.il sehhandlerbeforetry_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=sehhandlerbeforetry_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT sehhandlerbeforetry_r.dasm.il
    "$CORE_ROOT/ilasm" -output=sehhandlerbeforetry_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT sehhandlerbeforetry_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=sehhandlerbeforetry_r
    if [ ! -f sehhandlerbeforetry_r.org ]; then
        TakeLock
        if [ ! -f sehhandlerbeforetry_r.org ]; then
          mkdir IL
          cp sehhandlerbeforetry_r.exe IL/sehhandlerbeforetry_r.exe
          mv sehhandlerbeforetry_r.exe sehhandlerbeforetry_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in sehhandlerbeforetry_r.org /out sehhandlerbeforetry_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in sehhandlerbeforetry_r.org /out sehhandlerbeforetry_r.exe
          __cgExitCode=$?
...
ExePath=sehhandlerbeforetry_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in sehhandlerbeforetry_r.reflect.xml
  if [ -f sehhandlerbeforetry_r.reflect.xml ]
then
    ReflectionRoots="-x sehhandlerbeforetry_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/sehhandlerbeforetry_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" sehhandlerbeforetry_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" sehhandlerbeforetry_r.asm.exe $CLRTestExecutionArguments
