...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/140713/innerFinally_d innerFinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/140713/innerFinally_d innerFinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=innerFinally_d.dasm.il innerFinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=innerFinally_d.dasm.il innerFinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=innerFinally_d.asm.exe  innerFinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=innerFinally_d.asm.exe  innerFinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=innerFinally_d
    if [ ! -f innerFinally_d.org ]; then
        TakeLock
        if [ ! -f innerFinally_d.org ]; then
          mkdir IL
          cp innerFinally_d.exe IL/innerFinally_d.exe
          mv innerFinally_d.exe innerFinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in innerFinally_d.org /out innerFinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in innerFinally_d.org /out innerFinally_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in innerFinally_d.reflect.xml
  if [ -f innerFinally_d.reflect.xml ]
then
    ReflectionRoots="-x innerFinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/innerFinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" innerFinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" innerFinally_d.asm.exe $CLRTestExecutionArguments
