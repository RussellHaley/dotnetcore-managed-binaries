...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/140713/innerFinally_r innerFinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/140713/innerFinally_r innerFinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=innerFinally_r.dasm.il innerFinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=innerFinally_r.dasm.il innerFinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=innerFinally_r.asm.exe  innerFinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=innerFinally_r.asm.exe  innerFinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=innerFinally_r
    if [ ! -f innerFinally_r.org ]; then
        TakeLock
        if [ ! -f innerFinally_r.org ]; then
          mkdir IL
          cp innerFinally_r.exe IL/innerFinally_r.exe
          mv innerFinally_r.exe innerFinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in innerFinally_r.org /out innerFinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in innerFinally_r.org /out innerFinally_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in innerFinally_r.reflect.xml
  if [ -f innerFinally_r.reflect.xml ]
then
    ReflectionRoots="-x innerFinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/innerFinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" innerFinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" innerFinally_r.asm.exe $CLRTestExecutionArguments
