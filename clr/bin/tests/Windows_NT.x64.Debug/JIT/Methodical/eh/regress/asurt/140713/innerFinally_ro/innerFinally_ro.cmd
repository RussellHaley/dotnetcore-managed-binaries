...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\140713\innerFinally_ro innerFinally_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\140713\innerFinally_ro innerFinally_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=innerFinally_ro.dasm.il innerFinally_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=innerFinally_ro.dasm.il innerFinally_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=innerFinally_ro.asm.exe  innerFinally_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=innerFinally_ro.asm.exe  innerFinally_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=innerFinally_ro
    if not exist "innerFinally_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "innerFinally_ro.org" (
            mkdir IL
            copy innerFinally_ro.exe IL\innerFinally_ro.exe 
            ren innerFinally_ro.exe innerFinally_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in innerFinally_ro.org /out innerFinally_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in innerFinally_ro.org /out innerFinally_ro.exe
...
set ExePath=innerFinally_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in innerFinally_ro.reflect.xml
    IF EXIST innerFinally_ro.reflect.xml set ReflectionRoots=-x innerFinally_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\innerFinally_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% innerFinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% innerFinally_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
