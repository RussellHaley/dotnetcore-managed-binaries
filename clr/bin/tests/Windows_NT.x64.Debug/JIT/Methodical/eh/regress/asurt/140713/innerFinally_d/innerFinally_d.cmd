...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\140713\innerFinally_d innerFinally_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\140713\innerFinally_d innerFinally_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=innerFinally_d.dasm.il innerFinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=innerFinally_d.dasm.il innerFinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=innerFinally_d.asm.exe  innerFinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=innerFinally_d.asm.exe  innerFinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=innerFinally_d
    if not exist "innerFinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "innerFinally_d.org" (
            mkdir IL
            copy innerFinally_d.exe IL\innerFinally_d.exe 
            ren innerFinally_d.exe innerFinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in innerFinally_d.org /out innerFinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in innerFinally_d.org /out innerFinally_d.exe
...
set ExePath=innerFinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in innerFinally_d.reflect.xml
    IF EXIST innerFinally_d.reflect.xml set ReflectionRoots=-x innerFinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\innerFinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% innerFinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% innerFinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
