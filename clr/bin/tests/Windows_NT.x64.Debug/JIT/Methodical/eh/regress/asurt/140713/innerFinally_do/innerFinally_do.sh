...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/140713/innerFinally_do innerFinally_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/140713/innerFinally_do innerFinally_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=innerFinally_do.dasm.il innerFinally_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=innerFinally_do.dasm.il innerFinally_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=innerFinally_do.asm.exe  innerFinally_do.dasm.il
    "$CORE_ROOT/ilasm" -output=innerFinally_do.asm.exe  innerFinally_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=innerFinally_do
    if [ ! -f innerFinally_do.org ]; then
        TakeLock
        if [ ! -f innerFinally_do.org ]; then
          mkdir IL
          cp innerFinally_do.exe IL/innerFinally_do.exe
          mv innerFinally_do.exe innerFinally_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in innerFinally_do.org /out innerFinally_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in innerFinally_do.org /out innerFinally_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in innerFinally_do.reflect.xml
  if [ -f innerFinally_do.reflect.xml ]
then
    ReflectionRoots="-x innerFinally_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/innerFinally_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" innerFinally_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" innerFinally_do.asm.exe $CLRTestExecutionArguments
