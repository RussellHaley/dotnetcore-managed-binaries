...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\140713\innerFinally_r innerFinally_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\140713\innerFinally_r innerFinally_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=innerFinally_r.dasm.il innerFinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=innerFinally_r.dasm.il innerFinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=innerFinally_r.asm.exe  innerFinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=innerFinally_r.asm.exe  innerFinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=innerFinally_r
    if not exist "innerFinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "innerFinally_r.org" (
            mkdir IL
            copy innerFinally_r.exe IL\innerFinally_r.exe 
            ren innerFinally_r.exe innerFinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in innerFinally_r.org /out innerFinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in innerFinally_r.org /out innerFinally_r.exe
...
set ExePath=innerFinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in innerFinally_r.reflect.xml
    IF EXIST innerFinally_r.reflect.xml set ReflectionRoots=-x innerFinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\innerFinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% innerFinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% innerFinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
