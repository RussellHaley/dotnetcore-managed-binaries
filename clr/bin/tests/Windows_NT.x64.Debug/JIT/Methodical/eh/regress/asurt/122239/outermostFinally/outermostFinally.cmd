...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\122239\outermostFinally outermostFinally.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\122239\outermostFinally outermostFinally.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=outermostFinally.dasm.il outermostFinally.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=outermostFinally.dasm.il outermostFinally.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=outermostFinally.asm.exe  outermostFinally.dasm.il
    %CORE_ROOT%\ilasm.exe /output=outermostFinally.asm.exe  outermostFinally.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=outermostFinally
    if not exist "outermostFinally.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "outermostFinally.org" (
            mkdir IL
            copy outermostFinally.exe IL\outermostFinally.exe 
            ren outermostFinally.exe outermostFinally.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in outermostFinally.org /out outermostFinally.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in outermostFinally.org /out outermostFinally.exe
...
set ExePath=outermostFinally.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in outermostFinally.reflect.xml
    IF EXIST outermostFinally.reflect.xml set ReflectionRoots=-x outermostFinally.reflect.xml
    ...
    set ExePath=%LinkBin%\outermostFinally.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% outermostFinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% outermostFinally.asm.exe %CLRTestExecutionArguments% %Host_Args%
