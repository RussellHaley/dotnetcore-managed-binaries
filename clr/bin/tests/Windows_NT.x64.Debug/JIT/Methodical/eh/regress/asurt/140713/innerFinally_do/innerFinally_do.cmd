...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\140713\innerFinally_do innerFinally_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\Methodical\eh\regress\asurt\140713\innerFinally_do innerFinally_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=innerFinally_do.dasm.il innerFinally_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=innerFinally_do.dasm.il innerFinally_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=innerFinally_do.asm.exe  innerFinally_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=innerFinally_do.asm.exe  innerFinally_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=innerFinally_do
    if not exist "innerFinally_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "innerFinally_do.org" (
            mkdir IL
            copy innerFinally_do.exe IL\innerFinally_do.exe 
            ren innerFinally_do.exe innerFinally_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in innerFinally_do.org /out innerFinally_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in innerFinally_do.org /out innerFinally_do.exe
...
set ExePath=innerFinally_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in innerFinally_do.reflect.xml
    IF EXIST innerFinally_do.reflect.xml set ReflectionRoots=-x innerFinally_do.reflect.xml
    ...
    set ExePath=%LinkBin%\innerFinally_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% innerFinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% innerFinally_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
