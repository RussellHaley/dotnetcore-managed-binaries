...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/122239/outermostFinally outermostFinally.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/122239/outermostFinally outermostFinally.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=outermostFinally.dasm.il outermostFinally.exe
    "$CORE_ROOT/ildasm" -raweh -out=outermostFinally.dasm.il outermostFinally.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=outermostFinally.asm.exe  outermostFinally.dasm.il
    "$CORE_ROOT/ilasm" -output=outermostFinally.asm.exe  outermostFinally.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=outermostFinally
    if [ ! -f outermostFinally.org ]; then
        TakeLock
        if [ ! -f outermostFinally.org ]; then
          mkdir IL
          cp outermostFinally.exe IL/outermostFinally.exe
          mv outermostFinally.exe outermostFinally.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in outermostFinally.org /out outermostFinally.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in outermostFinally.org /out outermostFinally.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in outermostFinally.reflect.xml
  if [ -f outermostFinally.reflect.xml ]
then
    ReflectionRoots="-x outermostFinally.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/outermostFinally.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" outermostFinally.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" outermostFinally.asm.exe $CLRTestExecutionArguments
