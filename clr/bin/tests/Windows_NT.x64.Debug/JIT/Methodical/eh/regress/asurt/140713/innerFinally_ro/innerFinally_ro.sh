...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/140713/innerFinally_ro innerFinally_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/Methodical/eh/regress/asurt/140713/innerFinally_ro innerFinally_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=innerFinally_ro.dasm.il innerFinally_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=innerFinally_ro.dasm.il innerFinally_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=innerFinally_ro.asm.exe  innerFinally_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=innerFinally_ro.asm.exe  innerFinally_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=innerFinally_ro
    if [ ! -f innerFinally_ro.org ]; then
        TakeLock
        if [ ! -f innerFinally_ro.org ]; then
          mkdir IL
          cp innerFinally_ro.exe IL/innerFinally_ro.exe
          mv innerFinally_ro.exe innerFinally_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in innerFinally_ro.org /out innerFinally_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in innerFinally_ro.org /out innerFinally_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in innerFinally_ro.reflect.xml
  if [ -f innerFinally_ro.reflect.xml ]
then
    ReflectionRoots="-x innerFinally_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/innerFinally_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" innerFinally_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" innerFinally_ro.asm.exe $CLRTestExecutionArguments
