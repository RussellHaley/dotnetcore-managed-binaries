...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\cs\unsafe_ro unsafe_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\cs\unsafe_ro unsafe_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=unsafe_ro.dasm.il unsafe_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=unsafe_ro.dasm.il unsafe_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=unsafe_ro.asm.exe  unsafe_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=unsafe_ro.asm.exe  unsafe_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=unsafe_ro
    if not exist "unsafe_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "unsafe_ro.org" (
            mkdir IL
            copy unsafe_ro.exe IL\unsafe_ro.exe 
            ren unsafe_ro.exe unsafe_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unsafe_ro.org /out unsafe_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in unsafe_ro.org /out unsafe_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in unsafe_ro.reflect.xml
    IF EXIST unsafe_ro.reflect.xml set ReflectionRoots=-x unsafe_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\unsafe_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% unsafe_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% unsafe_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
