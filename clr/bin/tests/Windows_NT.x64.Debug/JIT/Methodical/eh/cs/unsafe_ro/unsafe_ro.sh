...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/cs/unsafe_ro unsafe_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/cs/unsafe_ro unsafe_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=unsafe_ro.dasm.il unsafe_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=unsafe_ro.dasm.il unsafe_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=unsafe_ro.asm.exe  unsafe_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=unsafe_ro.asm.exe  unsafe_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=unsafe_ro
    if [ ! -f unsafe_ro.org ]; then
        TakeLock
        if [ ! -f unsafe_ro.org ]; then
          mkdir IL
          cp unsafe_ro.exe IL/unsafe_ro.exe
          mv unsafe_ro.exe unsafe_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unsafe_ro.org /out unsafe_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in unsafe_ro.org /out unsafe_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in unsafe_ro.reflect.xml
  if [ -f unsafe_ro.reflect.xml ]
then
    ReflectionRoots="-x unsafe_ro.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" unsafe_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" unsafe_ro.asm.exe $CLRTestExecutionArguments
