...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/deadcode/badcodeinsidefinally_r badcodeinsidefinally_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/deadcode/badcodeinsidefinally_r badcodeinsidefinally_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=badcodeinsidefinally_r.dasm.il badcodeinsidefinally_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=badcodeinsidefinally_r.dasm.il badcodeinsidefinally_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=badcodeinsidefinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badcodeinsidefinally_r.dasm.il
    "$CORE_ROOT/ilasm" -output=badcodeinsidefinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badcodeinsidefinally_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=badcodeinsidefinally_r
    if [ ! -f badcodeinsidefinally_r.org ]; then
        TakeLock
        if [ ! -f badcodeinsidefinally_r.org ]; then
          mkdir IL
          cp badcodeinsidefinally_r.exe IL/badcodeinsidefinally_r.exe
          mv badcodeinsidefinally_r.exe badcodeinsidefinally_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badcodeinsidefinally_r.org /out badcodeinsidefinally_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badcodeinsidefinally_r.org /out badcodeinsidefinally_r.exe
          __cgExitCode=$?
...
ExePath=badcodeinsidefinally_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in badcodeinsidefinally_r.reflect.xml
  if [ -f badcodeinsidefinally_r.reflect.xml ]
then
    ReflectionRoots="-x badcodeinsidefinally_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/badcodeinsidefinally_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" badcodeinsidefinally_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" badcodeinsidefinally_r.asm.exe $CLRTestExecutionArguments
