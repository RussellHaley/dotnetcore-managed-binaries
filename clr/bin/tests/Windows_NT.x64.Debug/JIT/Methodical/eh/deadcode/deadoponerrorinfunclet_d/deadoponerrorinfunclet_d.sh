...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/deadcode/deadoponerrorinfunclet_d deadoponerrorinfunclet_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/deadcode/deadoponerrorinfunclet_d deadoponerrorinfunclet_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=deadoponerrorinfunclet_d.dasm.il deadoponerrorinfunclet_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=deadoponerrorinfunclet_d.dasm.il deadoponerrorinfunclet_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=deadoponerrorinfunclet_d.asm.exe -QUIET -NOLOGO -DEBUG deadoponerrorinfunclet_d.dasm.il
    "$CORE_ROOT/ilasm" -output=deadoponerrorinfunclet_d.asm.exe -QUIET -NOLOGO -DEBUG deadoponerrorinfunclet_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=deadoponerrorinfunclet_d
    if [ ! -f deadoponerrorinfunclet_d.org ]; then
        TakeLock
        if [ ! -f deadoponerrorinfunclet_d.org ]; then
          mkdir IL
          cp deadoponerrorinfunclet_d.exe IL/deadoponerrorinfunclet_d.exe
          mv deadoponerrorinfunclet_d.exe deadoponerrorinfunclet_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in deadoponerrorinfunclet_d.org /out deadoponerrorinfunclet_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in deadoponerrorinfunclet_d.org /out deadoponerrorinfunclet_d.exe
          __cgExitCode=$?
...
ExePath=deadoponerrorinfunclet_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in deadoponerrorinfunclet_d.reflect.xml
  if [ -f deadoponerrorinfunclet_d.reflect.xml ]
then
    ReflectionRoots="-x deadoponerrorinfunclet_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/deadoponerrorinfunclet_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" deadoponerrorinfunclet_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" deadoponerrorinfunclet_d.asm.exe $CLRTestExecutionArguments
