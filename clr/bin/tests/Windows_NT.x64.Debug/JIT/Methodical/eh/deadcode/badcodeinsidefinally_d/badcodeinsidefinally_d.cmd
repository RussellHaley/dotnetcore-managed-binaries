...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\deadcode\badcodeinsidefinally_d badcodeinsidefinally_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\deadcode\badcodeinsidefinally_d badcodeinsidefinally_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=badcodeinsidefinally_d.dasm.il badcodeinsidefinally_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=badcodeinsidefinally_d.dasm.il badcodeinsidefinally_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=badcodeinsidefinally_d.asm.exe -QUIET -NOLOGO -DEBUG badcodeinsidefinally_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=badcodeinsidefinally_d.asm.exe -QUIET -NOLOGO -DEBUG badcodeinsidefinally_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=badcodeinsidefinally_d
    if not exist "badcodeinsidefinally_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "badcodeinsidefinally_d.org" (
            mkdir IL
            copy badcodeinsidefinally_d.exe IL\badcodeinsidefinally_d.exe 
            ren badcodeinsidefinally_d.exe badcodeinsidefinally_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badcodeinsidefinally_d.org /out badcodeinsidefinally_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badcodeinsidefinally_d.org /out badcodeinsidefinally_d.exe
...
set ExePath=badcodeinsidefinally_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in badcodeinsidefinally_d.reflect.xml
    IF EXIST badcodeinsidefinally_d.reflect.xml set ReflectionRoots=-x badcodeinsidefinally_d.reflect.xml
    ...
    set ExePath=%LinkBin%\badcodeinsidefinally_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% badcodeinsidefinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% badcodeinsidefinally_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
