...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\deadcode\badcodeinsidefinally_r badcodeinsidefinally_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\Methodical\eh\deadcode\badcodeinsidefinally_r badcodeinsidefinally_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=badcodeinsidefinally_r.dasm.il badcodeinsidefinally_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=badcodeinsidefinally_r.dasm.il badcodeinsidefinally_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=badcodeinsidefinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badcodeinsidefinally_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=badcodeinsidefinally_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT badcodeinsidefinally_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=badcodeinsidefinally_r
    if not exist "badcodeinsidefinally_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "badcodeinsidefinally_r.org" (
            mkdir IL
            copy badcodeinsidefinally_r.exe IL\badcodeinsidefinally_r.exe 
            ren badcodeinsidefinally_r.exe badcodeinsidefinally_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badcodeinsidefinally_r.org /out badcodeinsidefinally_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in badcodeinsidefinally_r.org /out badcodeinsidefinally_r.exe
...
set ExePath=badcodeinsidefinally_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in badcodeinsidefinally_r.reflect.xml
    IF EXIST badcodeinsidefinally_r.reflect.xml set ReflectionRoots=-x badcodeinsidefinally_r.reflect.xml
    ...
    set ExePath=%LinkBin%\badcodeinsidefinally_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% badcodeinsidefinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% badcodeinsidefinally_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
