...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/deadcode/badcodeinsidefinally_d badcodeinsidefinally_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/deadcode/badcodeinsidefinally_d badcodeinsidefinally_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=badcodeinsidefinally_d.dasm.il badcodeinsidefinally_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=badcodeinsidefinally_d.dasm.il badcodeinsidefinally_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=badcodeinsidefinally_d.asm.exe -QUIET -NOLOGO -DEBUG badcodeinsidefinally_d.dasm.il
    "$CORE_ROOT/ilasm" -output=badcodeinsidefinally_d.asm.exe -QUIET -NOLOGO -DEBUG badcodeinsidefinally_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=badcodeinsidefinally_d
    if [ ! -f badcodeinsidefinally_d.org ]; then
        TakeLock
        if [ ! -f badcodeinsidefinally_d.org ]; then
          mkdir IL
          cp badcodeinsidefinally_d.exe IL/badcodeinsidefinally_d.exe
          mv badcodeinsidefinally_d.exe badcodeinsidefinally_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badcodeinsidefinally_d.org /out badcodeinsidefinally_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in badcodeinsidefinally_d.org /out badcodeinsidefinally_d.exe
          __cgExitCode=$?
...
ExePath=badcodeinsidefinally_d.exe
...
then
  ...
  # Use hints for reflection roots, if provided in badcodeinsidefinally_d.reflect.xml
  if [ -f badcodeinsidefinally_d.reflect.xml ]
then
    ReflectionRoots="-x badcodeinsidefinally_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/badcodeinsidefinally_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" badcodeinsidefinally_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" badcodeinsidefinally_d.asm.exe $CLRTestExecutionArguments
