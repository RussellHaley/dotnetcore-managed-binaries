...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/deadcode/deadoponerrorinfunclet_r deadoponerrorinfunclet_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/Methodical/eh/deadcode/deadoponerrorinfunclet_r deadoponerrorinfunclet_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=deadoponerrorinfunclet_r.dasm.il deadoponerrorinfunclet_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=deadoponerrorinfunclet_r.dasm.il deadoponerrorinfunclet_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=deadoponerrorinfunclet_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT deadoponerrorinfunclet_r.dasm.il
    "$CORE_ROOT/ilasm" -output=deadoponerrorinfunclet_r.asm.exe -QUIET -NOLOGO -DEBUG=OPT deadoponerrorinfunclet_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=deadoponerrorinfunclet_r
    if [ ! -f deadoponerrorinfunclet_r.org ]; then
        TakeLock
        if [ ! -f deadoponerrorinfunclet_r.org ]; then
          mkdir IL
          cp deadoponerrorinfunclet_r.exe IL/deadoponerrorinfunclet_r.exe
          mv deadoponerrorinfunclet_r.exe deadoponerrorinfunclet_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in deadoponerrorinfunclet_r.org /out deadoponerrorinfunclet_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in deadoponerrorinfunclet_r.org /out deadoponerrorinfunclet_r.exe
          __cgExitCode=$?
...
ExePath=deadoponerrorinfunclet_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in deadoponerrorinfunclet_r.reflect.xml
  if [ -f deadoponerrorinfunclet_r.reflect.xml ]
then
    ReflectionRoots="-x deadoponerrorinfunclet_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/deadoponerrorinfunclet_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" deadoponerrorinfunclet_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" deadoponerrorinfunclet_r.asm.exe $CLRTestExecutionArguments
