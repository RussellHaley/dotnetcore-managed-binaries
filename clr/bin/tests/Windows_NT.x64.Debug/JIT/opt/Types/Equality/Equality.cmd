...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\Types\Equality Equality.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\Types\Equality Equality.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Equality.dasm.il Equality.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Equality.dasm.il Equality.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Equality.asm.exe  Equality.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Equality.asm.exe  Equality.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Equality
    if not exist "Equality.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Equality.org" (
            mkdir IL
            copy Equality.exe IL\Equality.exe 
            ren Equality.exe Equality.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Equality.org /out Equality.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Equality.org /out Equality.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Equality.reflect.xml
    IF EXIST Equality.reflect.xml set ReflectionRoots=-x Equality.reflect.xml
    ...
    set ExePath=%LinkBin%\Equality.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Equality.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Equality.asm.exe %CLRTestExecutionArguments% %Host_Args%
