...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/Types/Equality Equality.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/Types/Equality Equality.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Equality.dasm.il Equality.exe
    "$CORE_ROOT/ildasm" -raweh -out=Equality.dasm.il Equality.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Equality.asm.exe  Equality.dasm.il
    "$CORE_ROOT/ilasm" -output=Equality.asm.exe  Equality.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Equality
    if [ ! -f Equality.org ]; then
        TakeLock
        if [ ! -f Equality.org ]; then
          mkdir IL
          cp Equality.exe IL/Equality.exe
          mv Equality.exe Equality.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Equality.org /out Equality.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Equality.org /out Equality.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Equality.reflect.xml
  if [ -f Equality.reflect.xml ]
then
    ReflectionRoots="-x Equality.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Equality.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Equality.asm.exe $CLRTestExecutionArguments
