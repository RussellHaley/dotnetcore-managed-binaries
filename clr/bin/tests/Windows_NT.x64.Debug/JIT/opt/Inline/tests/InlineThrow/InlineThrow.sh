...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/Inline/tests/InlineThrow InlineThrow.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/Inline/tests/InlineThrow InlineThrow.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=InlineThrow.dasm.il InlineThrow.exe
    "$CORE_ROOT/ildasm" -raweh -out=InlineThrow.dasm.il InlineThrow.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=InlineThrow.asm.exe  InlineThrow.dasm.il
    "$CORE_ROOT/ilasm" -output=InlineThrow.asm.exe  InlineThrow.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=InlineThrow
    if [ ! -f InlineThrow.org ]; then
        TakeLock
        if [ ! -f InlineThrow.org ]; then
          mkdir IL
          cp InlineThrow.exe IL/InlineThrow.exe
          mv InlineThrow.exe InlineThrow.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InlineThrow.org /out InlineThrow.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InlineThrow.org /out InlineThrow.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in InlineThrow.reflect.xml
  if [ -f InlineThrow.reflect.xml ]
then
    ReflectionRoots="-x InlineThrow.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/InlineThrow.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" InlineThrow.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" InlineThrow.asm.exe $CLRTestExecutionArguments
