...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\Inline\tests\calli calli.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\Inline\tests\calli calli.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=calli.dasm.il calli.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=calli.dasm.il calli.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=calli.asm.exe -QUIET -NOLOGO -DEBUG=OPT calli.dasm.il
    %CORE_ROOT%\ilasm.exe /output=calli.asm.exe -QUIET -NOLOGO -DEBUG=OPT calli.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=calli
    if not exist "calli.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "calli.org" (
            mkdir IL
            copy calli.exe IL\calli.exe 
            ren calli.exe calli.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in calli.org /out calli.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in calli.org /out calli.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in calli.reflect.xml
    IF EXIST calli.reflect.xml set ReflectionRoots=-x calli.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% calli.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% calli.asm.exe %CLRTestExecutionArguments% %Host_Args%
