...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\Inline\tests\InlineThrow InlineThrow.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\Inline\tests\InlineThrow InlineThrow.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=InlineThrow.dasm.il InlineThrow.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=InlineThrow.dasm.il InlineThrow.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=InlineThrow.asm.exe  InlineThrow.dasm.il
    %CORE_ROOT%\ilasm.exe /output=InlineThrow.asm.exe  InlineThrow.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=InlineThrow
    if not exist "InlineThrow.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "InlineThrow.org" (
            mkdir IL
            copy InlineThrow.exe IL\InlineThrow.exe 
            ren InlineThrow.exe InlineThrow.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InlineThrow.org /out InlineThrow.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InlineThrow.org /out InlineThrow.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in InlineThrow.reflect.xml
    IF EXIST InlineThrow.reflect.xml set ReflectionRoots=-x InlineThrow.reflect.xml
    ...
    set ExePath=%LinkBin%\InlineThrow.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% InlineThrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% InlineThrow.asm.exe %CLRTestExecutionArguments% %Host_Args%
