...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/Inline/tests/mathfunc mathfunc.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/Inline/tests/mathfunc mathfunc.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mathfunc.dasm.il mathfunc.exe
    "$CORE_ROOT/ildasm" -raweh -out=mathfunc.dasm.il mathfunc.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mathfunc.asm.exe  mathfunc.dasm.il
    "$CORE_ROOT/ilasm" -output=mathfunc.asm.exe  mathfunc.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mathfunc
    if [ ! -f mathfunc.org ]; then
        TakeLock
        if [ ! -f mathfunc.org ]; then
          mkdir IL
          cp mathfunc.exe IL/mathfunc.exe
          mv mathfunc.exe mathfunc.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mathfunc.org /out mathfunc.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mathfunc.org /out mathfunc.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mathfunc.reflect.xml
  if [ -f mathfunc.reflect.xml ]
then
    ReflectionRoots="-x mathfunc.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mathfunc.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mathfunc.asm.exe $CLRTestExecutionArguments
