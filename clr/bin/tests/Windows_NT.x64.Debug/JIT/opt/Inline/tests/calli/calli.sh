...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/Inline/tests/calli calli.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/Inline/tests/calli calli.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=calli.dasm.il calli.exe
    "$CORE_ROOT/ildasm" -raweh -out=calli.dasm.il calli.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=calli.asm.exe -QUIET -NOLOGO -DEBUG=OPT calli.dasm.il
    "$CORE_ROOT/ilasm" -output=calli.asm.exe -QUIET -NOLOGO -DEBUG=OPT calli.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=calli
    if [ ! -f calli.org ]; then
        TakeLock
        if [ ! -f calli.org ]; then
          mkdir IL
          cp calli.exe IL/calli.exe
          mv calli.exe calli.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in calli.org /out calli.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in calli.org /out calli.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in calli.reflect.xml
  if [ -f calli.reflect.xml ]
then
    ReflectionRoots="-x calli.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" calli.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" calli.asm.exe $CLRTestExecutionArguments
