...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\Inline\tests\mathfunc mathfunc.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\Inline\tests\mathfunc mathfunc.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mathfunc.dasm.il mathfunc.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mathfunc.dasm.il mathfunc.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mathfunc.asm.exe  mathfunc.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mathfunc.asm.exe  mathfunc.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mathfunc
    if not exist "mathfunc.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mathfunc.org" (
            mkdir IL
            copy mathfunc.exe IL\mathfunc.exe 
            ren mathfunc.exe mathfunc.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mathfunc.org /out mathfunc.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mathfunc.org /out mathfunc.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mathfunc.reflect.xml
    IF EXIST mathfunc.reflect.xml set ReflectionRoots=-x mathfunc.reflect.xml
    ...
    set ExePath=%LinkBin%\mathfunc.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mathfunc.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mathfunc.asm.exe %CLRTestExecutionArguments% %Host_Args%
