...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/Tailcall/TailcallVerifyWithPrefix TailcallVerifyWithPrefix.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/Tailcall/TailcallVerifyWithPrefix TailcallVerifyWithPrefix.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=TailcallVerifyWithPrefix.dasm.il TailcallVerifyWithPrefix.exe
    "$CORE_ROOT/ildasm" -raweh -out=TailcallVerifyWithPrefix.dasm.il TailcallVerifyWithPrefix.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=TailcallVerifyWithPrefix.asm.exe -QUIET -NOLOGO -OPTIMIZE TailcallVerifyWithPrefix.dasm.il
    "$CORE_ROOT/ilasm" -output=TailcallVerifyWithPrefix.asm.exe -QUIET -NOLOGO -OPTIMIZE TailcallVerifyWithPrefix.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=TailcallVerifyWithPrefix
    if [ ! -f TailcallVerifyWithPrefix.org ]; then
        TakeLock
        if [ ! -f TailcallVerifyWithPrefix.org ]; then
          mkdir IL
          cp TailcallVerifyWithPrefix.exe IL/TailcallVerifyWithPrefix.exe
          mv TailcallVerifyWithPrefix.exe TailcallVerifyWithPrefix.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TailcallVerifyWithPrefix.org /out TailcallVerifyWithPrefix.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TailcallVerifyWithPrefix.org /out TailcallVerifyWithPrefix.exe
          __cgExitCode=$?
...
ExePath=TailcallVerifyWithPrefix.exe
...
then
  ...
  # Use hints for reflection roots, if provided in TailcallVerifyWithPrefix.reflect.xml
  if [ -f TailcallVerifyWithPrefix.reflect.xml ]
then
    ReflectionRoots="-x TailcallVerifyWithPrefix.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/TailcallVerifyWithPrefix.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" TailcallVerifyWithPrefix.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" TailcallVerifyWithPrefix.asm.exe $CLRTestExecutionArguments
