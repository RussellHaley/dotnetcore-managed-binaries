...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\Tailcall\TailcallVerifyWithPrefix TailcallVerifyWithPrefix.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\Tailcall\TailcallVerifyWithPrefix TailcallVerifyWithPrefix.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=TailcallVerifyWithPrefix.dasm.il TailcallVerifyWithPrefix.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=TailcallVerifyWithPrefix.dasm.il TailcallVerifyWithPrefix.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=TailcallVerifyWithPrefix.asm.exe -QUIET -NOLOGO -OPTIMIZE TailcallVerifyWithPrefix.dasm.il
    %CORE_ROOT%\ilasm.exe /output=TailcallVerifyWithPrefix.asm.exe -QUIET -NOLOGO -OPTIMIZE TailcallVerifyWithPrefix.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=TailcallVerifyWithPrefix
    if not exist "TailcallVerifyWithPrefix.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "TailcallVerifyWithPrefix.org" (
            mkdir IL
            copy TailcallVerifyWithPrefix.exe IL\TailcallVerifyWithPrefix.exe 
            ren TailcallVerifyWithPrefix.exe TailcallVerifyWithPrefix.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TailcallVerifyWithPrefix.org /out TailcallVerifyWithPrefix.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TailcallVerifyWithPrefix.org /out TailcallVerifyWithPrefix.exe
...
set ExePath=TailcallVerifyWithPrefix.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in TailcallVerifyWithPrefix.reflect.xml
    IF EXIST TailcallVerifyWithPrefix.reflect.xml set ReflectionRoots=-x TailcallVerifyWithPrefix.reflect.xml
    ...
    set ExePath=%LinkBin%\TailcallVerifyWithPrefix.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% TailcallVerifyWithPrefix.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% TailcallVerifyWithPrefix.asm.exe %CLRTestExecutionArguments% %Host_Args%
