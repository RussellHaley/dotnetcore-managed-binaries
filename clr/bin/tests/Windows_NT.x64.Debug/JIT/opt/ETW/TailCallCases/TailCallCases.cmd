...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\ETW\TailCallCases TailCallCases.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\ETW\TailCallCases TailCallCases.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=TailCallCases.dasm.il TailCallCases.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=TailCallCases.dasm.il TailCallCases.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=TailCallCases.asm.exe -QUIET -NOLOGO -DEBUG TailCallCases.dasm.il
    %CORE_ROOT%\ilasm.exe /output=TailCallCases.asm.exe -QUIET -NOLOGO -DEBUG TailCallCases.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=TailCallCases
    if not exist "TailCallCases.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "TailCallCases.org" (
            mkdir IL
            copy TailCallCases.exe IL\TailCallCases.exe 
            ren TailCallCases.exe TailCallCases.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TailCallCases.org /out TailCallCases.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in TailCallCases.org /out TailCallCases.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in TailCallCases.reflect.xml
    IF EXIST TailCallCases.reflect.xml set ReflectionRoots=-x TailCallCases.reflect.xml
    ...
    set ExePath=%LinkBin%\TailCallCases.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% TailCallCases.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% TailCallCases.asm.exe %CLRTestExecutionArguments% %Host_Args%
