...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/ETW/TailCallCases TailCallCases.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/ETW/TailCallCases TailCallCases.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=TailCallCases.dasm.il TailCallCases.exe
    "$CORE_ROOT/ildasm" -raweh -out=TailCallCases.dasm.il TailCallCases.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=TailCallCases.asm.exe -QUIET -NOLOGO -DEBUG TailCallCases.dasm.il
    "$CORE_ROOT/ilasm" -output=TailCallCases.asm.exe -QUIET -NOLOGO -DEBUG TailCallCases.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=TailCallCases
    if [ ! -f TailCallCases.org ]; then
        TakeLock
        if [ ! -f TailCallCases.org ]; then
          mkdir IL
          cp TailCallCases.exe IL/TailCallCases.exe
          mv TailCallCases.exe TailCallCases.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TailCallCases.org /out TailCallCases.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in TailCallCases.org /out TailCallCases.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in TailCallCases.reflect.xml
  if [ -f TailCallCases.reflect.xml ]
then
    ReflectionRoots="-x TailCallCases.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/TailCallCases.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" TailCallCases.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" TailCallCases.asm.exe $CLRTestExecutionArguments
