...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\virtualstubdispatch\bigvtbl\bigvtbl_cs_r bigvtbl_cs_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\virtualstubdispatch\bigvtbl\bigvtbl_cs_r bigvtbl_cs_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=bigvtbl_cs_r.dasm.il bigvtbl_cs_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=bigvtbl_cs_r.dasm.il bigvtbl_cs_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=bigvtbl_cs_r.asm.exe  bigvtbl_cs_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=bigvtbl_cs_r.asm.exe  bigvtbl_cs_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=bigvtbl_cs_r
    if not exist "bigvtbl_cs_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "bigvtbl_cs_r.org" (
            mkdir IL
            copy bigvtbl_cs_r.exe IL\bigvtbl_cs_r.exe 
            ren bigvtbl_cs_r.exe bigvtbl_cs_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bigvtbl_cs_r.org /out bigvtbl_cs_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bigvtbl_cs_r.org /out bigvtbl_cs_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in bigvtbl_cs_r.reflect.xml
    IF EXIST bigvtbl_cs_r.reflect.xml set ReflectionRoots=-x bigvtbl_cs_r.reflect.xml
    ...
    set ExePath=%LinkBin%\bigvtbl_cs_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% bigvtbl_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% bigvtbl_cs_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
