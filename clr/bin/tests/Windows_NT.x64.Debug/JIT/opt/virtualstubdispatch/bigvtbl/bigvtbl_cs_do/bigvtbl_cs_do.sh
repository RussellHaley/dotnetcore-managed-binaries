...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/virtualstubdispatch/bigvtbl/bigvtbl_cs_do bigvtbl_cs_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/virtualstubdispatch/bigvtbl/bigvtbl_cs_do bigvtbl_cs_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=bigvtbl_cs_do.dasm.il bigvtbl_cs_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=bigvtbl_cs_do.dasm.il bigvtbl_cs_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=bigvtbl_cs_do.asm.exe  bigvtbl_cs_do.dasm.il
    "$CORE_ROOT/ilasm" -output=bigvtbl_cs_do.asm.exe  bigvtbl_cs_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=bigvtbl_cs_do
    if [ ! -f bigvtbl_cs_do.org ]; then
        TakeLock
        if [ ! -f bigvtbl_cs_do.org ]; then
          mkdir IL
          cp bigvtbl_cs_do.exe IL/bigvtbl_cs_do.exe
          mv bigvtbl_cs_do.exe bigvtbl_cs_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bigvtbl_cs_do.org /out bigvtbl_cs_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bigvtbl_cs_do.org /out bigvtbl_cs_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in bigvtbl_cs_do.reflect.xml
  if [ -f bigvtbl_cs_do.reflect.xml ]
then
    ReflectionRoots="-x bigvtbl_cs_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/bigvtbl_cs_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" bigvtbl_cs_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" bigvtbl_cs_do.asm.exe $CLRTestExecutionArguments
