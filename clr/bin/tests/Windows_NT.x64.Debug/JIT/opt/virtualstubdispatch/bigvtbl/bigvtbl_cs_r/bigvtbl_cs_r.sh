...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/virtualstubdispatch/bigvtbl/bigvtbl_cs_r bigvtbl_cs_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/virtualstubdispatch/bigvtbl/bigvtbl_cs_r bigvtbl_cs_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=bigvtbl_cs_r.dasm.il bigvtbl_cs_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=bigvtbl_cs_r.dasm.il bigvtbl_cs_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=bigvtbl_cs_r.asm.exe  bigvtbl_cs_r.dasm.il
    "$CORE_ROOT/ilasm" -output=bigvtbl_cs_r.asm.exe  bigvtbl_cs_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=bigvtbl_cs_r
    if [ ! -f bigvtbl_cs_r.org ]; then
        TakeLock
        if [ ! -f bigvtbl_cs_r.org ]; then
          mkdir IL
          cp bigvtbl_cs_r.exe IL/bigvtbl_cs_r.exe
          mv bigvtbl_cs_r.exe bigvtbl_cs_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bigvtbl_cs_r.org /out bigvtbl_cs_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bigvtbl_cs_r.org /out bigvtbl_cs_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in bigvtbl_cs_r.reflect.xml
  if [ -f bigvtbl_cs_r.reflect.xml ]
then
    ReflectionRoots="-x bigvtbl_cs_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/bigvtbl_cs_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" bigvtbl_cs_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" bigvtbl_cs_r.asm.exe $CLRTestExecutionArguments
