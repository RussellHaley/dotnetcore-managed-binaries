...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\DumpDisasm\JitMinOpts\InstrCnt1 InstrCnt1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\DumpDisasm\JitMinOpts\InstrCnt1 InstrCnt1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=InstrCnt1.dasm.il InstrCnt1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=InstrCnt1.dasm.il InstrCnt1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=InstrCnt1.asm.exe -QUIET -NOLOGO -DEBUG InstrCnt1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=InstrCnt1.asm.exe -QUIET -NOLOGO -DEBUG InstrCnt1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=InstrCnt1
    if not exist "InstrCnt1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "InstrCnt1.org" (
            mkdir IL
            copy InstrCnt1.exe IL\InstrCnt1.exe 
            ren InstrCnt1.exe InstrCnt1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InstrCnt1.org /out InstrCnt1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InstrCnt1.org /out InstrCnt1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in InstrCnt1.reflect.xml
    IF EXIST InstrCnt1.reflect.xml set ReflectionRoots=-x InstrCnt1.reflect.xml
    ...
    set ExePath=%LinkBin%\InstrCnt1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% InstrCnt1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% InstrCnt1.asm.exe %CLRTestExecutionArguments% %Host_Args%
