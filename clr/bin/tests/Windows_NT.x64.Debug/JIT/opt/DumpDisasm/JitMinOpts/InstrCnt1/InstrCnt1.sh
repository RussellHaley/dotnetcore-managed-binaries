...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/DumpDisasm/JitMinOpts/InstrCnt1 InstrCnt1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/DumpDisasm/JitMinOpts/InstrCnt1 InstrCnt1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=InstrCnt1.dasm.il InstrCnt1.exe
    "$CORE_ROOT/ildasm" -raweh -out=InstrCnt1.dasm.il InstrCnt1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=InstrCnt1.asm.exe -QUIET -NOLOGO -DEBUG InstrCnt1.dasm.il
    "$CORE_ROOT/ilasm" -output=InstrCnt1.asm.exe -QUIET -NOLOGO -DEBUG InstrCnt1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=InstrCnt1
    if [ ! -f InstrCnt1.org ]; then
        TakeLock
        if [ ! -f InstrCnt1.org ]; then
          mkdir IL
          cp InstrCnt1.exe IL/InstrCnt1.exe
          mv InstrCnt1.exe InstrCnt1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InstrCnt1.org /out InstrCnt1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InstrCnt1.org /out InstrCnt1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in InstrCnt1.reflect.xml
  if [ -f InstrCnt1.reflect.xml ]
then
    ReflectionRoots="-x InstrCnt1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" InstrCnt1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" InstrCnt1.asm.exe $CLRTestExecutionArguments
