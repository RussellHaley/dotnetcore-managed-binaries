...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\StackFixup StackFixup.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\StackFixup StackFixup.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=StackFixup.dasm.il StackFixup.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=StackFixup.dasm.il StackFixup.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=StackFixup.asm.exe  StackFixup.dasm.il
    %CORE_ROOT%\ilasm.exe /output=StackFixup.asm.exe  StackFixup.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=StackFixup
    if not exist "StackFixup.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "StackFixup.org" (
            mkdir IL
            copy StackFixup.exe IL\StackFixup.exe 
            ren StackFixup.exe StackFixup.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in StackFixup.org /out StackFixup.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in StackFixup.org /out StackFixup.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in StackFixup.reflect.xml
    IF EXIST StackFixup.reflect.xml set ReflectionRoots=-x StackFixup.reflect.xml
    ...
    set ExePath=%LinkBin%\StackFixup.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% StackFixup.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% StackFixup.asm.exe %CLRTestExecutionArguments% %Host_Args%
