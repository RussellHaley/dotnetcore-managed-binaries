...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/StackFixup StackFixup.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/StackFixup StackFixup.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=StackFixup.dasm.il StackFixup.exe
    "$CORE_ROOT/ildasm" -raweh -out=StackFixup.dasm.il StackFixup.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=StackFixup.asm.exe  StackFixup.dasm.il
    "$CORE_ROOT/ilasm" -output=StackFixup.asm.exe  StackFixup.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=StackFixup
    if [ ! -f StackFixup.org ]; then
        TakeLock
        if [ ! -f StackFixup.org ]; then
          mkdir IL
          cp StackFixup.exe IL/StackFixup.exe
          mv StackFixup.exe StackFixup.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in StackFixup.org /out StackFixup.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in StackFixup.org /out StackFixup.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in StackFixup.reflect.xml
  if [ -f StackFixup.reflect.xml ]
then
    ReflectionRoots="-x StackFixup.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/StackFixup.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" StackFixup.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" StackFixup.asm.exe $CLRTestExecutionArguments
