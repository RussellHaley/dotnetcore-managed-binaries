...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\FastTailCallCandidates FastTailCallCandidates.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\FastTailCallCandidates FastTailCallCandidates.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=FastTailCallCandidates.dasm.il FastTailCallCandidates.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=FastTailCallCandidates.dasm.il FastTailCallCandidates.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=FastTailCallCandidates.asm.exe  FastTailCallCandidates.dasm.il
    %CORE_ROOT%\ilasm.exe /output=FastTailCallCandidates.asm.exe  FastTailCallCandidates.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=FastTailCallCandidates
    if not exist "FastTailCallCandidates.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "FastTailCallCandidates.org" (
            mkdir IL
            copy FastTailCallCandidates.exe IL\FastTailCallCandidates.exe 
            ren FastTailCallCandidates.exe FastTailCallCandidates.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FastTailCallCandidates.org /out FastTailCallCandidates.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FastTailCallCandidates.org /out FastTailCallCandidates.exe
...
set ExePath=FastTailCallCandidates.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in FastTailCallCandidates.reflect.xml
    IF EXIST FastTailCallCandidates.reflect.xml set ReflectionRoots=-x FastTailCallCandidates.reflect.xml
    ...
    set ExePath=%LinkBin%\FastTailCallCandidates.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% FastTailCallCandidates.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% FastTailCallCandidates.asm.exe %CLRTestExecutionArguments% %Host_Args%
