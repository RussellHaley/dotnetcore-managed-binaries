...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/FastTailCallCandidates FastTailCallCandidates.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/FastTailCallCandidates FastTailCallCandidates.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=FastTailCallCandidates.dasm.il FastTailCallCandidates.exe
    "$CORE_ROOT/ildasm" -raweh -out=FastTailCallCandidates.dasm.il FastTailCallCandidates.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=FastTailCallCandidates.asm.exe  FastTailCallCandidates.dasm.il
    "$CORE_ROOT/ilasm" -output=FastTailCallCandidates.asm.exe  FastTailCallCandidates.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=FastTailCallCandidates
    if [ ! -f FastTailCallCandidates.org ]; then
        TakeLock
        if [ ! -f FastTailCallCandidates.org ]; then
          mkdir IL
          cp FastTailCallCandidates.exe IL/FastTailCallCandidates.exe
          mv FastTailCallCandidates.exe FastTailCallCandidates.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FastTailCallCandidates.org /out FastTailCallCandidates.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FastTailCallCandidates.org /out FastTailCallCandidates.exe
          __cgExitCode=$?
...
ExePath=FastTailCallCandidates.exe
...
then
  ...
  # Use hints for reflection roots, if provided in FastTailCallCandidates.reflect.xml
  if [ -f FastTailCallCandidates.reflect.xml ]
then
    ReflectionRoots="-x FastTailCallCandidates.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/FastTailCallCandidates.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" FastTailCallCandidates.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" FastTailCallCandidates.asm.exe $CLRTestExecutionArguments
