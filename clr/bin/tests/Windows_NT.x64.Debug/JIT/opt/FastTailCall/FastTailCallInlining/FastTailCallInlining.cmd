...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\FastTailCallInlining FastTailCallInlining.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\FastTailCallInlining FastTailCallInlining.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=FastTailCallInlining.dasm.il FastTailCallInlining.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=FastTailCallInlining.dasm.il FastTailCallInlining.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=FastTailCallInlining.asm.exe  FastTailCallInlining.dasm.il
    %CORE_ROOT%\ilasm.exe /output=FastTailCallInlining.asm.exe  FastTailCallInlining.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=FastTailCallInlining
    if not exist "FastTailCallInlining.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "FastTailCallInlining.org" (
            mkdir IL
            copy FastTailCallInlining.exe IL\FastTailCallInlining.exe 
            ren FastTailCallInlining.exe FastTailCallInlining.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FastTailCallInlining.org /out FastTailCallInlining.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FastTailCallInlining.org /out FastTailCallInlining.exe
...
set ExePath=FastTailCallInlining.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in FastTailCallInlining.reflect.xml
    IF EXIST FastTailCallInlining.reflect.xml set ReflectionRoots=-x FastTailCallInlining.reflect.xml
    ...
    set ExePath=%LinkBin%\FastTailCallInlining.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% FastTailCallInlining.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% FastTailCallInlining.asm.exe %CLRTestExecutionArguments% %Host_Args%
