...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/FastTailCallInlining FastTailCallInlining.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/FastTailCallInlining FastTailCallInlining.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=FastTailCallInlining.dasm.il FastTailCallInlining.exe
    "$CORE_ROOT/ildasm" -raweh -out=FastTailCallInlining.dasm.il FastTailCallInlining.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=FastTailCallInlining.asm.exe  FastTailCallInlining.dasm.il
    "$CORE_ROOT/ilasm" -output=FastTailCallInlining.asm.exe  FastTailCallInlining.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=FastTailCallInlining
    if [ ! -f FastTailCallInlining.org ]; then
        TakeLock
        if [ ! -f FastTailCallInlining.org ]; then
          mkdir IL
          cp FastTailCallInlining.exe IL/FastTailCallInlining.exe
          mv FastTailCallInlining.exe FastTailCallInlining.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FastTailCallInlining.org /out FastTailCallInlining.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FastTailCallInlining.org /out FastTailCallInlining.exe
          __cgExitCode=$?
...
ExePath=FastTailCallInlining.exe
...
then
  ...
  # Use hints for reflection roots, if provided in FastTailCallInlining.reflect.xml
  if [ -f FastTailCallInlining.reflect.xml ]
then
    ReflectionRoots="-x FastTailCallInlining.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/FastTailCallInlining.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" FastTailCallInlining.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" FastTailCallInlining.asm.exe $CLRTestExecutionArguments
