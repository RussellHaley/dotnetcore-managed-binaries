...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/GitHubIssue12479 GitHubIssue12479.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/GitHubIssue12479 GitHubIssue12479.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GitHubIssue12479.dasm.il GitHubIssue12479.exe
    "$CORE_ROOT/ildasm" -raweh -out=GitHubIssue12479.dasm.il GitHubIssue12479.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GitHubIssue12479.asm.exe  GitHubIssue12479.dasm.il
    "$CORE_ROOT/ilasm" -output=GitHubIssue12479.asm.exe  GitHubIssue12479.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GitHubIssue12479
    if [ ! -f GitHubIssue12479.org ]; then
        TakeLock
        if [ ! -f GitHubIssue12479.org ]; then
          mkdir IL
          cp GitHubIssue12479.exe IL/GitHubIssue12479.exe
          mv GitHubIssue12479.exe GitHubIssue12479.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHubIssue12479.org /out GitHubIssue12479.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GitHubIssue12479.org /out GitHubIssue12479.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GitHubIssue12479.reflect.xml
  if [ -f GitHubIssue12479.reflect.xml ]
then
    ReflectionRoots="-x GitHubIssue12479.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GitHubIssue12479.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GitHubIssue12479.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GitHubIssue12479.asm.exe $CLRTestExecutionArguments
