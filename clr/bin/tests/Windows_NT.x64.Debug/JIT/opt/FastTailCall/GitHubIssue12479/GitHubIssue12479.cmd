...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\GitHubIssue12479 GitHubIssue12479.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\GitHubIssue12479 GitHubIssue12479.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GitHubIssue12479.dasm.il GitHubIssue12479.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GitHubIssue12479.dasm.il GitHubIssue12479.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GitHubIssue12479.asm.exe  GitHubIssue12479.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GitHubIssue12479.asm.exe  GitHubIssue12479.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GitHubIssue12479
    if not exist "GitHubIssue12479.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GitHubIssue12479.org" (
            mkdir IL
            copy GitHubIssue12479.exe IL\GitHubIssue12479.exe 
            ren GitHubIssue12479.exe GitHubIssue12479.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHubIssue12479.org /out GitHubIssue12479.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GitHubIssue12479.org /out GitHubIssue12479.exe
...
set ExePath=GitHubIssue12479.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GitHubIssue12479.reflect.xml
    IF EXIST GitHubIssue12479.reflect.xml set ReflectionRoots=-x GitHubIssue12479.reflect.xml
    ...
    set ExePath=%LinkBin%\GitHubIssue12479.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GitHubIssue12479.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GitHubIssue12479.asm.exe %CLRTestExecutionArguments% %Host_Args%
