...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\StructPassingSimple StructPassingSimple.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\FastTailCall\StructPassingSimple StructPassingSimple.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=StructPassingSimple.dasm.il StructPassingSimple.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=StructPassingSimple.dasm.il StructPassingSimple.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=StructPassingSimple.asm.exe  StructPassingSimple.dasm.il
    %CORE_ROOT%\ilasm.exe /output=StructPassingSimple.asm.exe  StructPassingSimple.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=StructPassingSimple
    if not exist "StructPassingSimple.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "StructPassingSimple.org" (
            mkdir IL
            copy StructPassingSimple.exe IL\StructPassingSimple.exe 
            ren StructPassingSimple.exe StructPassingSimple.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in StructPassingSimple.org /out StructPassingSimple.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in StructPassingSimple.org /out StructPassingSimple.exe
...
set ExePath=StructPassingSimple.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in StructPassingSimple.reflect.xml
    IF EXIST StructPassingSimple.reflect.xml set ReflectionRoots=-x StructPassingSimple.reflect.xml
    ...
    set ExePath=%LinkBin%\StructPassingSimple.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% StructPassingSimple.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% StructPassingSimple.asm.exe %CLRTestExecutionArguments% %Host_Args%
