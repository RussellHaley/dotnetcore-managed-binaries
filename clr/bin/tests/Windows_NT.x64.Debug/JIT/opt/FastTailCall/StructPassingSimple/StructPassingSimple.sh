...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/StructPassingSimple StructPassingSimple.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/FastTailCall/StructPassingSimple StructPassingSimple.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=StructPassingSimple.dasm.il StructPassingSimple.exe
    "$CORE_ROOT/ildasm" -raweh -out=StructPassingSimple.dasm.il StructPassingSimple.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=StructPassingSimple.asm.exe  StructPassingSimple.dasm.il
    "$CORE_ROOT/ilasm" -output=StructPassingSimple.asm.exe  StructPassingSimple.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=StructPassingSimple
    if [ ! -f StructPassingSimple.org ]; then
        TakeLock
        if [ ! -f StructPassingSimple.org ]; then
          mkdir IL
          cp StructPassingSimple.exe IL/StructPassingSimple.exe
          mv StructPassingSimple.exe StructPassingSimple.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in StructPassingSimple.org /out StructPassingSimple.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in StructPassingSimple.org /out StructPassingSimple.exe
          __cgExitCode=$?
...
ExePath=StructPassingSimple.exe
...
then
  ...
  # Use hints for reflection roots, if provided in StructPassingSimple.reflect.xml
  if [ -f StructPassingSimple.reflect.xml ]
then
    ReflectionRoots="-x StructPassingSimple.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/StructPassingSimple.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" StructPassingSimple.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" StructPassingSimple.asm.exe $CLRTestExecutionArguments
