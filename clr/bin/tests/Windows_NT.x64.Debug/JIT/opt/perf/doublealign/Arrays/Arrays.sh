...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/perf/doublealign/Arrays Arrays.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/opt/perf/doublealign/Arrays Arrays.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Arrays.dasm.il Arrays.exe
    "$CORE_ROOT/ildasm" -raweh -out=Arrays.dasm.il Arrays.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Arrays.asm.exe  Arrays.dasm.il
    "$CORE_ROOT/ilasm" -output=Arrays.asm.exe  Arrays.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Arrays
    if [ ! -f Arrays.org ]; then
        TakeLock
        if [ ! -f Arrays.org ]; then
          mkdir IL
          cp Arrays.exe IL/Arrays.exe
          mv Arrays.exe Arrays.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Arrays.org /out Arrays.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Arrays.org /out Arrays.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Arrays.reflect.xml
  if [ -f Arrays.reflect.xml ]
then
    ReflectionRoots="-x Arrays.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Arrays.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Arrays.asm.exe $CLRTestExecutionArguments
