...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\perf\doublealign\Arrays Arrays.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\opt\perf\doublealign\Arrays Arrays.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Arrays.dasm.il Arrays.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Arrays.dasm.il Arrays.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Arrays.asm.exe  Arrays.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Arrays.asm.exe  Arrays.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Arrays
    if not exist "Arrays.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Arrays.org" (
            mkdir IL
            copy Arrays.exe IL\Arrays.exe 
            ren Arrays.exe Arrays.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Arrays.org /out Arrays.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Arrays.org /out Arrays.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Arrays.reflect.xml
    IF EXIST Arrays.reflect.xml set ReflectionRoots=-x Arrays.reflect.xml
    ...
    set ExePath=%LinkBin%\Arrays.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Arrays.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Arrays.asm.exe %CLRTestExecutionArguments% %Host_Args%
