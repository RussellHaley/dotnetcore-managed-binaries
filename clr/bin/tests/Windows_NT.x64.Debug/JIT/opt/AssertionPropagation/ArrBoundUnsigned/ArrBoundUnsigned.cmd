...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\AssertionPropagation\ArrBoundUnsigned ArrBoundUnsigned.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\JIT\opt\AssertionPropagation\ArrBoundUnsigned ArrBoundUnsigned.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ArrBoundUnsigned.dasm.il ArrBoundUnsigned.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ArrBoundUnsigned.dasm.il ArrBoundUnsigned.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ArrBoundUnsigned.asm.exe  ArrBoundUnsigned.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ArrBoundUnsigned.asm.exe  ArrBoundUnsigned.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ArrBoundUnsigned
    if not exist "ArrBoundUnsigned.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ArrBoundUnsigned.org" (
            mkdir IL
            copy ArrBoundUnsigned.exe IL\ArrBoundUnsigned.exe 
            ren ArrBoundUnsigned.exe ArrBoundUnsigned.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrBoundUnsigned.org /out ArrBoundUnsigned.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrBoundUnsigned.org /out ArrBoundUnsigned.exe
...
set ExePath=ArrBoundUnsigned.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ArrBoundUnsigned.reflect.xml
    IF EXIST ArrBoundUnsigned.reflect.xml set ReflectionRoots=-x ArrBoundUnsigned.reflect.xml
    ...
    set ExePath=%LinkBin%\ArrBoundUnsigned.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ArrBoundUnsigned.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ArrBoundUnsigned.asm.exe %CLRTestExecutionArguments% %Host_Args%
