...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/AssertionPropagation/ArrBoundUnsigned ArrBoundUnsigned.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/JIT/opt/AssertionPropagation/ArrBoundUnsigned ArrBoundUnsigned.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ArrBoundUnsigned.dasm.il ArrBoundUnsigned.exe
    "$CORE_ROOT/ildasm" -raweh -out=ArrBoundUnsigned.dasm.il ArrBoundUnsigned.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ArrBoundUnsigned.asm.exe  ArrBoundUnsigned.dasm.il
    "$CORE_ROOT/ilasm" -output=ArrBoundUnsigned.asm.exe  ArrBoundUnsigned.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ArrBoundUnsigned
    if [ ! -f ArrBoundUnsigned.org ]; then
        TakeLock
        if [ ! -f ArrBoundUnsigned.org ]; then
          mkdir IL
          cp ArrBoundUnsigned.exe IL/ArrBoundUnsigned.exe
          mv ArrBoundUnsigned.exe ArrBoundUnsigned.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrBoundUnsigned.org /out ArrBoundUnsigned.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrBoundUnsigned.org /out ArrBoundUnsigned.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ArrBoundUnsigned.reflect.xml
  if [ -f ArrBoundUnsigned.reflect.xml ]
then
    ReflectionRoots="-x ArrBoundUnsigned.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ArrBoundUnsigned.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ArrBoundUnsigned.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ArrBoundUnsigned.asm.exe $CLRTestExecutionArguments
