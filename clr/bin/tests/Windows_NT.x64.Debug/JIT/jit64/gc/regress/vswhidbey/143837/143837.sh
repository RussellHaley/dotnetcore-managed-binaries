...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/gc/regress/vswhidbey/143837 143837.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/gc/regress/vswhidbey/143837 143837.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=143837.dasm.il 143837.exe
    "$CORE_ROOT/ildasm" -raweh -out=143837.dasm.il 143837.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=143837.asm.exe  143837.dasm.il
    "$CORE_ROOT/ilasm" -output=143837.asm.exe  143837.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=143837
    if [ ! -f 143837.org ]; then
        TakeLock
        if [ ! -f 143837.org ]; then
          mkdir IL
          cp 143837.exe IL/143837.exe
          mv 143837.exe 143837.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 143837.org /out 143837.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 143837.org /out 143837.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 143837.reflect.xml
  if [ -f 143837.reflect.xml ]
then
    ReflectionRoots="-x 143837.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 143837.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 143837.asm.exe $CLRTestExecutionArguments
