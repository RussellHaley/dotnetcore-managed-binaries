...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\gc\regress\vswhidbey\143837 143837.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\gc\regress\vswhidbey\143837 143837.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=143837.dasm.il 143837.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=143837.dasm.il 143837.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=143837.asm.exe  143837.dasm.il
    %CORE_ROOT%\ilasm.exe /output=143837.asm.exe  143837.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=143837
    if not exist "143837.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "143837.org" (
            mkdir IL
            copy 143837.exe IL\143837.exe 
            ren 143837.exe 143837.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 143837.org /out 143837.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 143837.org /out 143837.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 143837.reflect.xml
    IF EXIST 143837.reflect.xml set ReflectionRoots=-x 143837.reflect.xml
    ...
    set ExePath=%LinkBin%\143837.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 143837.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 143837.asm.exe %CLRTestExecutionArguments% %Host_Args%
