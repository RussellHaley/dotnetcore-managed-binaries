...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\gc\misc\struct6_4 struct6_4.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\gc\misc\struct6_4 struct6_4.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=struct6_4.dasm.il struct6_4.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=struct6_4.dasm.il struct6_4.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=struct6_4.asm.exe  struct6_4.dasm.il
    %CORE_ROOT%\ilasm.exe /output=struct6_4.asm.exe  struct6_4.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=struct6_4
    if not exist "struct6_4.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "struct6_4.org" (
            mkdir IL
            copy struct6_4.exe IL\struct6_4.exe 
            ren struct6_4.exe struct6_4.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct6_4.org /out struct6_4.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in struct6_4.org /out struct6_4.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in struct6_4.reflect.xml
    IF EXIST struct6_4.reflect.xml set ReflectionRoots=-x struct6_4.reflect.xml
    ...
    set ExePath=%LinkBin%\struct6_4.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% struct6_4.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% struct6_4.asm.exe %CLRTestExecutionArguments% %Host_Args%
