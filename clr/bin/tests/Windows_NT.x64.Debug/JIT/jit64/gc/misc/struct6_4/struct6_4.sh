...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/gc/misc/struct6_4 struct6_4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/gc/misc/struct6_4 struct6_4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=struct6_4.dasm.il struct6_4.exe
    "$CORE_ROOT/ildasm" -raweh -out=struct6_4.dasm.il struct6_4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=struct6_4.asm.exe  struct6_4.dasm.il
    "$CORE_ROOT/ilasm" -output=struct6_4.asm.exe  struct6_4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=struct6_4
    if [ ! -f struct6_4.org ]; then
        TakeLock
        if [ ! -f struct6_4.org ]; then
          mkdir IL
          cp struct6_4.exe IL/struct6_4.exe
          mv struct6_4.exe struct6_4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct6_4.org /out struct6_4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in struct6_4.org /out struct6_4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in struct6_4.reflect.xml
  if [ -f struct6_4.reflect.xml ]
then
    ReflectionRoots="-x struct6_4.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" struct6_4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" struct6_4.asm.exe $CLRTestExecutionArguments
