...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\inl\caninline_do caninline_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\inl\caninline_do caninline_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=caninline_do.dasm.il caninline_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=caninline_do.dasm.il caninline_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=caninline_do.asm.exe  caninline_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=caninline_do.asm.exe  caninline_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=caninline_do
    if not exist "caninline_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "caninline_do.org" (
            mkdir IL
            copy caninline_do.exe IL\caninline_do.exe 
            ren caninline_do.exe caninline_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in caninline_do.org /out caninline_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in caninline_do.org /out caninline_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in caninline_do.reflect.xml
    IF EXIST caninline_do.reflect.xml set ReflectionRoots=-x caninline_do.reflect.xml
    ...
    set ExePath=%LinkBin%\caninline_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% caninline_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% caninline_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
