...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/inl/caninline_r caninline_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/inl/caninline_r caninline_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=caninline_r.dasm.il caninline_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=caninline_r.dasm.il caninline_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=caninline_r.asm.exe  caninline_r.dasm.il
    "$CORE_ROOT/ilasm" -output=caninline_r.asm.exe  caninline_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=caninline_r
    if [ ! -f caninline_r.org ]; then
        TakeLock
        if [ ! -f caninline_r.org ]; then
          mkdir IL
          cp caninline_r.exe IL/caninline_r.exe
          mv caninline_r.exe caninline_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in caninline_r.org /out caninline_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in caninline_r.org /out caninline_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in caninline_r.reflect.xml
  if [ -f caninline_r.reflect.xml ]
then
    ReflectionRoots="-x caninline_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/caninline_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" caninline_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" caninline_r.asm.exe $CLRTestExecutionArguments
