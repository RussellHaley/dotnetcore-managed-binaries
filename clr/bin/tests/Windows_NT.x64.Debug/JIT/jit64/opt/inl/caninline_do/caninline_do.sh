...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/inl/caninline_do caninline_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/inl/caninline_do caninline_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=caninline_do.dasm.il caninline_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=caninline_do.dasm.il caninline_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=caninline_do.asm.exe  caninline_do.dasm.il
    "$CORE_ROOT/ilasm" -output=caninline_do.asm.exe  caninline_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=caninline_do
    if [ ! -f caninline_do.org ]; then
        TakeLock
        if [ ! -f caninline_do.org ]; then
          mkdir IL
          cp caninline_do.exe IL/caninline_do.exe
          mv caninline_do.exe caninline_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in caninline_do.org /out caninline_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in caninline_do.org /out caninline_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in caninline_do.reflect.xml
  if [ -f caninline_do.reflect.xml ]
then
    ReflectionRoots="-x caninline_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/caninline_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" caninline_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" caninline_do.asm.exe $CLRTestExecutionArguments
