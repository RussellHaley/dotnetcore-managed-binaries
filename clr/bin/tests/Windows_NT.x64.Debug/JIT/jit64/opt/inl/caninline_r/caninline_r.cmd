...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\inl\caninline_r caninline_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\inl\caninline_r caninline_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=caninline_r.dasm.il caninline_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=caninline_r.dasm.il caninline_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=caninline_r.asm.exe  caninline_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=caninline_r.asm.exe  caninline_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=caninline_r
    if not exist "caninline_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "caninline_r.org" (
            mkdir IL
            copy caninline_r.exe IL\caninline_r.exe 
            ren caninline_r.exe caninline_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in caninline_r.org /out caninline_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in caninline_r.org /out caninline_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in caninline_r.reflect.xml
    IF EXIST caninline_r.reflect.xml set ReflectionRoots=-x caninline_r.reflect.xml
    ...
    set ExePath=%LinkBin%\caninline_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% caninline_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% caninline_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
