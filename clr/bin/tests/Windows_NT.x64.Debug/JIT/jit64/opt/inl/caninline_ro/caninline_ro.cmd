...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\inl\caninline_ro caninline_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\inl\caninline_ro caninline_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=caninline_ro.dasm.il caninline_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=caninline_ro.dasm.il caninline_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=caninline_ro.asm.exe  caninline_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=caninline_ro.asm.exe  caninline_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=caninline_ro
    if not exist "caninline_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "caninline_ro.org" (
            mkdir IL
            copy caninline_ro.exe IL\caninline_ro.exe 
            ren caninline_ro.exe caninline_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in caninline_ro.org /out caninline_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in caninline_ro.org /out caninline_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in caninline_ro.reflect.xml
    IF EXIST caninline_ro.reflect.xml set ReflectionRoots=-x caninline_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\caninline_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% caninline_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% caninline_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
