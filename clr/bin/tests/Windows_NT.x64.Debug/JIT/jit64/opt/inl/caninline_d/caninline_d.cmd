...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\inl\caninline_d caninline_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\inl\caninline_d caninline_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=caninline_d.dasm.il caninline_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=caninline_d.dasm.il caninline_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=caninline_d.asm.exe  caninline_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=caninline_d.asm.exe  caninline_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=caninline_d
    if not exist "caninline_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "caninline_d.org" (
            mkdir IL
            copy caninline_d.exe IL\caninline_d.exe 
            ren caninline_d.exe caninline_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in caninline_d.org /out caninline_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in caninline_d.org /out caninline_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in caninline_d.reflect.xml
    IF EXIST caninline_d.reflect.xml set ReflectionRoots=-x caninline_d.reflect.xml
    ...
    set ExePath=%LinkBin%\caninline_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% caninline_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% caninline_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
