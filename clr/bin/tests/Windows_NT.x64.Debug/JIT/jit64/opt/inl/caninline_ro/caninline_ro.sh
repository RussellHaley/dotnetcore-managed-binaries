...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/inl/caninline_ro caninline_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/inl/caninline_ro caninline_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=caninline_ro.dasm.il caninline_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=caninline_ro.dasm.il caninline_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=caninline_ro.asm.exe  caninline_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=caninline_ro.asm.exe  caninline_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=caninline_ro
    if [ ! -f caninline_ro.org ]; then
        TakeLock
        if [ ! -f caninline_ro.org ]; then
          mkdir IL
          cp caninline_ro.exe IL/caninline_ro.exe
          mv caninline_ro.exe caninline_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in caninline_ro.org /out caninline_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in caninline_ro.org /out caninline_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in caninline_ro.reflect.xml
  if [ -f caninline_ro.reflect.xml ]
then
    ReflectionRoots="-x caninline_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/caninline_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" caninline_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" caninline_ro.asm.exe $CLRTestExecutionArguments
