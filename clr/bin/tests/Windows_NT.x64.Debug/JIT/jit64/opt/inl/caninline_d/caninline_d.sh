...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/inl/caninline_d caninline_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/inl/caninline_d caninline_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=caninline_d.dasm.il caninline_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=caninline_d.dasm.il caninline_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=caninline_d.asm.exe  caninline_d.dasm.il
    "$CORE_ROOT/ilasm" -output=caninline_d.asm.exe  caninline_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=caninline_d
    if [ ! -f caninline_d.org ]; then
        TakeLock
        if [ ! -f caninline_d.org ]; then
          mkdir IL
          cp caninline_d.exe IL/caninline_d.exe
          mv caninline_d.exe caninline_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in caninline_d.org /out caninline_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in caninline_d.org /out caninline_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in caninline_d.reflect.xml
  if [ -f caninline_d.reflect.xml ]
then
    ReflectionRoots="-x caninline_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/caninline_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" caninline_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" caninline_d.asm.exe $CLRTestExecutionArguments
