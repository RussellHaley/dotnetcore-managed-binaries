...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress3_ro CgStress3_ro.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress3_ro CgStress3_ro.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CgStress3_ro.dasm.il CgStress3_ro.exe
    "$CORE_ROOT/ildasm" -raweh -out=CgStress3_ro.dasm.il CgStress3_ro.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CgStress3_ro.asm.exe  CgStress3_ro.dasm.il
    "$CORE_ROOT/ilasm" -output=CgStress3_ro.asm.exe  CgStress3_ro.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CgStress3_ro
    if [ ! -f CgStress3_ro.org ]; then
        TakeLock
        if [ ! -f CgStress3_ro.org ]; then
          mkdir IL
          cp CgStress3_ro.exe IL/CgStress3_ro.exe
          mv CgStress3_ro.exe CgStress3_ro.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress3_ro.org /out CgStress3_ro.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress3_ro.org /out CgStress3_ro.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CgStress3_ro.reflect.xml
  if [ -f CgStress3_ro.reflect.xml ]
then
    ReflectionRoots="-x CgStress3_ro.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CgStress3_ro.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CgStress3_ro.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CgStress3_ro.asm.exe $CLRTestExecutionArguments
