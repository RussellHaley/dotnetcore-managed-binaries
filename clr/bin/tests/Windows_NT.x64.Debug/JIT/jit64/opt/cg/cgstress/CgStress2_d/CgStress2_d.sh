...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress2_d CgStress2_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress2_d CgStress2_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CgStress2_d.dasm.il CgStress2_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=CgStress2_d.dasm.il CgStress2_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CgStress2_d.asm.exe  CgStress2_d.dasm.il
    "$CORE_ROOT/ilasm" -output=CgStress2_d.asm.exe  CgStress2_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CgStress2_d
    if [ ! -f CgStress2_d.org ]; then
        TakeLock
        if [ ! -f CgStress2_d.org ]; then
          mkdir IL
          cp CgStress2_d.exe IL/CgStress2_d.exe
          mv CgStress2_d.exe CgStress2_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress2_d.org /out CgStress2_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress2_d.org /out CgStress2_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CgStress2_d.reflect.xml
  if [ -f CgStress2_d.reflect.xml ]
then
    ReflectionRoots="-x CgStress2_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CgStress2_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CgStress2_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CgStress2_d.asm.exe $CLRTestExecutionArguments
