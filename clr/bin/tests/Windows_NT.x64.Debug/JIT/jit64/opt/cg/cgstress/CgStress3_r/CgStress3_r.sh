...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress3_r CgStress3_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress3_r CgStress3_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CgStress3_r.dasm.il CgStress3_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=CgStress3_r.dasm.il CgStress3_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CgStress3_r.asm.exe  CgStress3_r.dasm.il
    "$CORE_ROOT/ilasm" -output=CgStress3_r.asm.exe  CgStress3_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CgStress3_r
    if [ ! -f CgStress3_r.org ]; then
        TakeLock
        if [ ! -f CgStress3_r.org ]; then
          mkdir IL
          cp CgStress3_r.exe IL/CgStress3_r.exe
          mv CgStress3_r.exe CgStress3_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress3_r.org /out CgStress3_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress3_r.org /out CgStress3_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CgStress3_r.reflect.xml
  if [ -f CgStress3_r.reflect.xml ]
then
    ReflectionRoots="-x CgStress3_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CgStress3_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CgStress3_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CgStress3_r.asm.exe $CLRTestExecutionArguments
