...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress1_r CgStress1_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress1_r CgStress1_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CgStress1_r.dasm.il CgStress1_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CgStress1_r.dasm.il CgStress1_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CgStress1_r.asm.exe  CgStress1_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CgStress1_r.asm.exe  CgStress1_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CgStress1_r
    if not exist "CgStress1_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CgStress1_r.org" (
            mkdir IL
            copy CgStress1_r.exe IL\CgStress1_r.exe 
            ren CgStress1_r.exe CgStress1_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress1_r.org /out CgStress1_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress1_r.org /out CgStress1_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CgStress1_r.reflect.xml
    IF EXIST CgStress1_r.reflect.xml set ReflectionRoots=-x CgStress1_r.reflect.xml
    ...
    set ExePath=%LinkBin%\CgStress1_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CgStress1_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CgStress1_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
