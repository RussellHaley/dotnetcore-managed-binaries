...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress3_ro CgStress3_ro.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress3_ro CgStress3_ro.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CgStress3_ro.dasm.il CgStress3_ro.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CgStress3_ro.dasm.il CgStress3_ro.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CgStress3_ro.asm.exe  CgStress3_ro.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CgStress3_ro.asm.exe  CgStress3_ro.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CgStress3_ro
    if not exist "CgStress3_ro.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CgStress3_ro.org" (
            mkdir IL
            copy CgStress3_ro.exe IL\CgStress3_ro.exe 
            ren CgStress3_ro.exe CgStress3_ro.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress3_ro.org /out CgStress3_ro.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress3_ro.org /out CgStress3_ro.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CgStress3_ro.reflect.xml
    IF EXIST CgStress3_ro.reflect.xml set ReflectionRoots=-x CgStress3_ro.reflect.xml
    ...
    set ExePath=%LinkBin%\CgStress3_ro.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CgStress3_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CgStress3_ro.asm.exe %CLRTestExecutionArguments% %Host_Args%
