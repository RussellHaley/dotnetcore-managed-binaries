...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress2_r CgStress2_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress2_r CgStress2_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CgStress2_r.dasm.il CgStress2_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=CgStress2_r.dasm.il CgStress2_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CgStress2_r.asm.exe  CgStress2_r.dasm.il
    "$CORE_ROOT/ilasm" -output=CgStress2_r.asm.exe  CgStress2_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CgStress2_r
    if [ ! -f CgStress2_r.org ]; then
        TakeLock
        if [ ! -f CgStress2_r.org ]; then
          mkdir IL
          cp CgStress2_r.exe IL/CgStress2_r.exe
          mv CgStress2_r.exe CgStress2_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress2_r.org /out CgStress2_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress2_r.org /out CgStress2_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CgStress2_r.reflect.xml
  if [ -f CgStress2_r.reflect.xml ]
then
    ReflectionRoots="-x CgStress2_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CgStress2_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CgStress2_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CgStress2_r.asm.exe $CLRTestExecutionArguments
