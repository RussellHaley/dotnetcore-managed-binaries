...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress3_r CgStress3_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress3_r CgStress3_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CgStress3_r.dasm.il CgStress3_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CgStress3_r.dasm.il CgStress3_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CgStress3_r.asm.exe  CgStress3_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CgStress3_r.asm.exe  CgStress3_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CgStress3_r
    if not exist "CgStress3_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CgStress3_r.org" (
            mkdir IL
            copy CgStress3_r.exe IL\CgStress3_r.exe 
            ren CgStress3_r.exe CgStress3_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress3_r.org /out CgStress3_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress3_r.org /out CgStress3_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CgStress3_r.reflect.xml
    IF EXIST CgStress3_r.reflect.xml set ReflectionRoots=-x CgStress3_r.reflect.xml
    ...
    set ExePath=%LinkBin%\CgStress3_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CgStress3_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CgStress3_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
