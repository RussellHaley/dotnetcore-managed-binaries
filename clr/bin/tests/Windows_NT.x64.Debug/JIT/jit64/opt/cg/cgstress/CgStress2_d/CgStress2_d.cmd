...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress2_d CgStress2_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress2_d CgStress2_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CgStress2_d.dasm.il CgStress2_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CgStress2_d.dasm.il CgStress2_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CgStress2_d.asm.exe  CgStress2_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CgStress2_d.asm.exe  CgStress2_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CgStress2_d
    if not exist "CgStress2_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CgStress2_d.org" (
            mkdir IL
            copy CgStress2_d.exe IL\CgStress2_d.exe 
            ren CgStress2_d.exe CgStress2_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress2_d.org /out CgStress2_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress2_d.org /out CgStress2_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CgStress2_d.reflect.xml
    IF EXIST CgStress2_d.reflect.xml set ReflectionRoots=-x CgStress2_d.reflect.xml
    ...
    set ExePath=%LinkBin%\CgStress2_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CgStress2_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CgStress2_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
