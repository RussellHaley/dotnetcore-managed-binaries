...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress1_d CgStress1_d.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress1_d CgStress1_d.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CgStress1_d.dasm.il CgStress1_d.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CgStress1_d.dasm.il CgStress1_d.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CgStress1_d.asm.exe  CgStress1_d.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CgStress1_d.asm.exe  CgStress1_d.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CgStress1_d
    if not exist "CgStress1_d.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CgStress1_d.org" (
            mkdir IL
            copy CgStress1_d.exe IL\CgStress1_d.exe 
            ren CgStress1_d.exe CgStress1_d.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress1_d.org /out CgStress1_d.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress1_d.org /out CgStress1_d.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CgStress1_d.reflect.xml
    IF EXIST CgStress1_d.reflect.xml set ReflectionRoots=-x CgStress1_d.reflect.xml
    ...
    set ExePath=%LinkBin%\CgStress1_d.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CgStress1_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CgStress1_d.asm.exe %CLRTestExecutionArguments% %Host_Args%
