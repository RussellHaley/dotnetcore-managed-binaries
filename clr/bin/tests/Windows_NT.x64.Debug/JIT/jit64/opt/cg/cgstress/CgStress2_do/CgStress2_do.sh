...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress2_do CgStress2_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress2_do CgStress2_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CgStress2_do.dasm.il CgStress2_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=CgStress2_do.dasm.il CgStress2_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CgStress2_do.asm.exe  CgStress2_do.dasm.il
    "$CORE_ROOT/ilasm" -output=CgStress2_do.asm.exe  CgStress2_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CgStress2_do
    if [ ! -f CgStress2_do.org ]; then
        TakeLock
        if [ ! -f CgStress2_do.org ]; then
          mkdir IL
          cp CgStress2_do.exe IL/CgStress2_do.exe
          mv CgStress2_do.exe CgStress2_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress2_do.org /out CgStress2_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress2_do.org /out CgStress2_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CgStress2_do.reflect.xml
  if [ -f CgStress2_do.reflect.xml ]
then
    ReflectionRoots="-x CgStress2_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CgStress2_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CgStress2_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CgStress2_do.asm.exe $CLRTestExecutionArguments
