...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress2_do CgStress2_do.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress2_do CgStress2_do.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CgStress2_do.dasm.il CgStress2_do.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CgStress2_do.dasm.il CgStress2_do.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CgStress2_do.asm.exe  CgStress2_do.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CgStress2_do.asm.exe  CgStress2_do.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CgStress2_do
    if not exist "CgStress2_do.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CgStress2_do.org" (
            mkdir IL
            copy CgStress2_do.exe IL\CgStress2_do.exe 
            ren CgStress2_do.exe CgStress2_do.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress2_do.org /out CgStress2_do.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress2_do.org /out CgStress2_do.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CgStress2_do.reflect.xml
    IF EXIST CgStress2_do.reflect.xml set ReflectionRoots=-x CgStress2_do.reflect.xml
    ...
    set ExePath=%LinkBin%\CgStress2_do.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CgStress2_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CgStress2_do.asm.exe %CLRTestExecutionArguments% %Host_Args%
