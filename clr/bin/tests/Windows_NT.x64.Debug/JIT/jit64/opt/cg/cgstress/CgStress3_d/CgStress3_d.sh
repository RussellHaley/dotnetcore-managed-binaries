...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress3_d CgStress3_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress3_d CgStress3_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CgStress3_d.dasm.il CgStress3_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=CgStress3_d.dasm.il CgStress3_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CgStress3_d.asm.exe  CgStress3_d.dasm.il
    "$CORE_ROOT/ilasm" -output=CgStress3_d.asm.exe  CgStress3_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CgStress3_d
    if [ ! -f CgStress3_d.org ]; then
        TakeLock
        if [ ! -f CgStress3_d.org ]; then
          mkdir IL
          cp CgStress3_d.exe IL/CgStress3_d.exe
          mv CgStress3_d.exe CgStress3_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress3_d.org /out CgStress3_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress3_d.org /out CgStress3_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CgStress3_d.reflect.xml
  if [ -f CgStress3_d.reflect.xml ]
then
    ReflectionRoots="-x CgStress3_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CgStress3_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CgStress3_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CgStress3_d.asm.exe $CLRTestExecutionArguments
