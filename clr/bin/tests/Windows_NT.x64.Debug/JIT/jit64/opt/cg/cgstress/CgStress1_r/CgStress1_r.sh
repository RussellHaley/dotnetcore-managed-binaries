...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress1_r CgStress1_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress1_r CgStress1_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CgStress1_r.dasm.il CgStress1_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=CgStress1_r.dasm.il CgStress1_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CgStress1_r.asm.exe  CgStress1_r.dasm.il
    "$CORE_ROOT/ilasm" -output=CgStress1_r.asm.exe  CgStress1_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CgStress1_r
    if [ ! -f CgStress1_r.org ]; then
        TakeLock
        if [ ! -f CgStress1_r.org ]; then
          mkdir IL
          cp CgStress1_r.exe IL/CgStress1_r.exe
          mv CgStress1_r.exe CgStress1_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress1_r.org /out CgStress1_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress1_r.org /out CgStress1_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CgStress1_r.reflect.xml
  if [ -f CgStress1_r.reflect.xml ]
then
    ReflectionRoots="-x CgStress1_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CgStress1_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CgStress1_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CgStress1_r.asm.exe $CLRTestExecutionArguments
