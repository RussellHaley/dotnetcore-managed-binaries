...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress1_do CgStress1_do.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress1_do CgStress1_do.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CgStress1_do.dasm.il CgStress1_do.exe
    "$CORE_ROOT/ildasm" -raweh -out=CgStress1_do.dasm.il CgStress1_do.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CgStress1_do.asm.exe  CgStress1_do.dasm.il
    "$CORE_ROOT/ilasm" -output=CgStress1_do.asm.exe  CgStress1_do.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CgStress1_do
    if [ ! -f CgStress1_do.org ]; then
        TakeLock
        if [ ! -f CgStress1_do.org ]; then
          mkdir IL
          cp CgStress1_do.exe IL/CgStress1_do.exe
          mv CgStress1_do.exe CgStress1_do.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress1_do.org /out CgStress1_do.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress1_do.org /out CgStress1_do.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CgStress1_do.reflect.xml
  if [ -f CgStress1_do.reflect.xml ]
then
    ReflectionRoots="-x CgStress1_do.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CgStress1_do.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CgStress1_do.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CgStress1_do.asm.exe $CLRTestExecutionArguments
