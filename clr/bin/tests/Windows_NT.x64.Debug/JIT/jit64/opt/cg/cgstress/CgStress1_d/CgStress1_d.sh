...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress1_d CgStress1_d.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/opt/cg/cgstress/CgStress1_d CgStress1_d.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CgStress1_d.dasm.il CgStress1_d.exe
    "$CORE_ROOT/ildasm" -raweh -out=CgStress1_d.dasm.il CgStress1_d.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CgStress1_d.asm.exe  CgStress1_d.dasm.il
    "$CORE_ROOT/ilasm" -output=CgStress1_d.asm.exe  CgStress1_d.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CgStress1_d
    if [ ! -f CgStress1_d.org ]; then
        TakeLock
        if [ ! -f CgStress1_d.org ]; then
          mkdir IL
          cp CgStress1_d.exe IL/CgStress1_d.exe
          mv CgStress1_d.exe CgStress1_d.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress1_d.org /out CgStress1_d.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CgStress1_d.org /out CgStress1_d.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CgStress1_d.reflect.xml
  if [ -f CgStress1_d.reflect.xml ]
then
    ReflectionRoots="-x CgStress1_d.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CgStress1_d.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CgStress1_d.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CgStress1_d.asm.exe $CLRTestExecutionArguments
