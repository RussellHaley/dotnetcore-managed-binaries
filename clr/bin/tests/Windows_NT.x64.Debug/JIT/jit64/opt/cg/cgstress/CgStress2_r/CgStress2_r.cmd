...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress2_r CgStress2_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\opt\cg\cgstress\CgStress2_r CgStress2_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CgStress2_r.dasm.il CgStress2_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CgStress2_r.dasm.il CgStress2_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CgStress2_r.asm.exe  CgStress2_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CgStress2_r.asm.exe  CgStress2_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CgStress2_r
    if not exist "CgStress2_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CgStress2_r.org" (
            mkdir IL
            copy CgStress2_r.exe IL\CgStress2_r.exe 
            ren CgStress2_r.exe CgStress2_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress2_r.org /out CgStress2_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CgStress2_r.org /out CgStress2_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CgStress2_r.reflect.xml
    IF EXIST CgStress2_r.reflect.xml set ReflectionRoots=-x CgStress2_r.reflect.xml
    ...
    set ExePath=%LinkBin%\CgStress2_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CgStress2_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CgStress2_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
