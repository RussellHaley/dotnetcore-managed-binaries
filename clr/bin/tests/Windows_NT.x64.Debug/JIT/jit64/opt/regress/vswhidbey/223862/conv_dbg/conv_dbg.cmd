...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\jit64\opt\regress\vswhidbey\223862\conv_dbg conv_dbg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\jit64\opt\regress\vswhidbey\223862\conv_dbg conv_dbg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=conv_dbg.dasm.il conv_dbg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=conv_dbg.dasm.il conv_dbg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=conv_dbg.asm.exe -QUIET -NOLOGO -DEBUG conv_dbg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=conv_dbg.asm.exe -QUIET -NOLOGO -DEBUG conv_dbg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=conv_dbg
    if not exist "conv_dbg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "conv_dbg.org" (
            mkdir IL
            copy conv_dbg.exe IL\conv_dbg.exe 
            ren conv_dbg.exe conv_dbg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_dbg.org /out conv_dbg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_dbg.org /out conv_dbg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in conv_dbg.reflect.xml
    IF EXIST conv_dbg.reflect.xml set ReflectionRoots=-x conv_dbg.reflect.xml
    ...
    set ExePath=%LinkBin%\conv_dbg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% conv_dbg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% conv_dbg.asm.exe %CLRTestExecutionArguments% %Host_Args%
