...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\jit64\opt\regress\vswhidbey\223862\mul_exception_dbg mul_exception_dbg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\jit64\opt\regress\vswhidbey\223862\mul_exception_dbg mul_exception_dbg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mul_exception_dbg.dasm.il mul_exception_dbg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mul_exception_dbg.dasm.il mul_exception_dbg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mul_exception_dbg.asm.exe -QUIET -NOLOGO -DEBUG mul_exception_dbg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mul_exception_dbg.asm.exe -QUIET -NOLOGO -DEBUG mul_exception_dbg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mul_exception_dbg
    if not exist "mul_exception_dbg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mul_exception_dbg.org" (
            mkdir IL
            copy mul_exception_dbg.exe IL\mul_exception_dbg.exe 
            ren mul_exception_dbg.exe mul_exception_dbg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mul_exception_dbg.org /out mul_exception_dbg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mul_exception_dbg.org /out mul_exception_dbg.exe
...
set ExePath=mul_exception_dbg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mul_exception_dbg.reflect.xml
    IF EXIST mul_exception_dbg.reflect.xml set ReflectionRoots=-x mul_exception_dbg.reflect.xml
    ...
    set ExePath=%LinkBin%\mul_exception_dbg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mul_exception_dbg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mul_exception_dbg.asm.exe %CLRTestExecutionArguments% %Host_Args%
