...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\jit64\opt\regress\vswhidbey\228572\conv_opt conv_opt.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\jit64\opt\regress\vswhidbey\228572\conv_opt conv_opt.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=conv_opt.dasm.il conv_opt.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=conv_opt.dasm.il conv_opt.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=conv_opt.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_opt.dasm.il
    %CORE_ROOT%\ilasm.exe /output=conv_opt.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_opt.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=conv_opt
    if not exist "conv_opt.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "conv_opt.org" (
            mkdir IL
            copy conv_opt.exe IL\conv_opt.exe 
            ren conv_opt.exe conv_opt.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_opt.org /out conv_opt.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_opt.org /out conv_opt.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in conv_opt.reflect.xml
    IF EXIST conv_opt.reflect.xml set ReflectionRoots=-x conv_opt.reflect.xml
    ...
    set ExePath=%LinkBin%\conv_opt.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% conv_opt.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% conv_opt.asm.exe %CLRTestExecutionArguments% %Host_Args%
