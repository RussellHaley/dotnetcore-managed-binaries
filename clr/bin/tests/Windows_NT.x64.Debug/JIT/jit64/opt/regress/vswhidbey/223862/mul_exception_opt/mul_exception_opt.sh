...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/jit64/opt/regress/vswhidbey/223862/mul_exception_opt mul_exception_opt.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/jit64/opt/regress/vswhidbey/223862/mul_exception_opt mul_exception_opt.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mul_exception_opt.dasm.il mul_exception_opt.exe
    "$CORE_ROOT/ildasm" -raweh -out=mul_exception_opt.dasm.il mul_exception_opt.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mul_exception_opt.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mul_exception_opt.dasm.il
    "$CORE_ROOT/ilasm" -output=mul_exception_opt.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mul_exception_opt.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mul_exception_opt
    if [ ! -f mul_exception_opt.org ]; then
        TakeLock
        if [ ! -f mul_exception_opt.org ]; then
          mkdir IL
          cp mul_exception_opt.exe IL/mul_exception_opt.exe
          mv mul_exception_opt.exe mul_exception_opt.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mul_exception_opt.org /out mul_exception_opt.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mul_exception_opt.org /out mul_exception_opt.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mul_exception_opt.reflect.xml
  if [ -f mul_exception_opt.reflect.xml ]
then
    ReflectionRoots="-x mul_exception_opt.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/mul_exception_opt.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mul_exception_opt.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mul_exception_opt.asm.exe $CLRTestExecutionArguments
