...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/jit64/opt/regress/vswhidbey/228572/conv_dbg conv_dbg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/jit64/opt/regress/vswhidbey/228572/conv_dbg conv_dbg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=conv_dbg.dasm.il conv_dbg.exe
    "$CORE_ROOT/ildasm" -raweh -out=conv_dbg.dasm.il conv_dbg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=conv_dbg.asm.exe -QUIET -NOLOGO -DEBUG conv_dbg.dasm.il
    "$CORE_ROOT/ilasm" -output=conv_dbg.asm.exe -QUIET -NOLOGO -DEBUG conv_dbg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=conv_dbg
    if [ ! -f conv_dbg.org ]; then
        TakeLock
        if [ ! -f conv_dbg.org ]; then
          mkdir IL
          cp conv_dbg.exe IL/conv_dbg.exe
          mv conv_dbg.exe conv_dbg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in conv_dbg.org /out conv_dbg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in conv_dbg.org /out conv_dbg.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in conv_dbg.reflect.xml
  if [ -f conv_dbg.reflect.xml ]
then
    ReflectionRoots="-x conv_dbg.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" conv_dbg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" conv_dbg.asm.exe $CLRTestExecutionArguments
