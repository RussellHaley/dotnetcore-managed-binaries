...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/jit64/opt/regress/vswhidbey/228572/conv_opt conv_opt.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/jit64/opt/regress/vswhidbey/228572/conv_opt conv_opt.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=conv_opt.dasm.il conv_opt.exe
    "$CORE_ROOT/ildasm" -raweh -out=conv_opt.dasm.il conv_opt.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=conv_opt.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_opt.dasm.il
    "$CORE_ROOT/ilasm" -output=conv_opt.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_opt.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=conv_opt
    if [ ! -f conv_opt.org ]; then
        TakeLock
        if [ ! -f conv_opt.org ]; then
          mkdir IL
          cp conv_opt.exe IL/conv_opt.exe
          mv conv_opt.exe conv_opt.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in conv_opt.org /out conv_opt.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in conv_opt.org /out conv_opt.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in conv_opt.reflect.xml
  if [ -f conv_opt.reflect.xml ]
then
    ReflectionRoots="-x conv_opt.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" conv_opt.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" conv_opt.asm.exe $CLRTestExecutionArguments
