...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/VolatileTest_op_mul VolatileTest_op_mul.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/VolatileTest_op_mul VolatileTest_op_mul.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=VolatileTest_op_mul.dasm.il VolatileTest_op_mul.exe
    "$CORE_ROOT/ildasm" -raweh -out=VolatileTest_op_mul.dasm.il VolatileTest_op_mul.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=VolatileTest_op_mul.asm.exe  VolatileTest_op_mul.dasm.il
    "$CORE_ROOT/ilasm" -output=VolatileTest_op_mul.asm.exe  VolatileTest_op_mul.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=VolatileTest_op_mul
    if [ ! -f VolatileTest_op_mul.org ]; then
        TakeLock
        if [ ! -f VolatileTest_op_mul.org ]; then
          mkdir IL
          cp VolatileTest_op_mul.exe IL/VolatileTest_op_mul.exe
          mv VolatileTest_op_mul.exe VolatileTest_op_mul.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VolatileTest_op_mul.org /out VolatileTest_op_mul.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in VolatileTest_op_mul.org /out VolatileTest_op_mul.exe
          __cgExitCode=$?
...
ExePath=VolatileTest_op_mul.exe
...
then
  ...
  # Use hints for reflection roots, if provided in VolatileTest_op_mul.reflect.xml
  if [ -f VolatileTest_op_mul.reflect.xml ]
then
    ReflectionRoots="-x VolatileTest_op_mul.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/VolatileTest_op_mul.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" VolatileTest_op_mul.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" VolatileTest_op_mul.asm.exe $CLRTestExecutionArguments
