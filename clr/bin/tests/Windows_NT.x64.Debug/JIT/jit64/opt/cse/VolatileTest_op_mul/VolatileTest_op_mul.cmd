...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\VolatileTest_op_mul VolatileTest_op_mul.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\VolatileTest_op_mul VolatileTest_op_mul.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=VolatileTest_op_mul.dasm.il VolatileTest_op_mul.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=VolatileTest_op_mul.dasm.il VolatileTest_op_mul.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=VolatileTest_op_mul.asm.exe  VolatileTest_op_mul.dasm.il
    %CORE_ROOT%\ilasm.exe /output=VolatileTest_op_mul.asm.exe  VolatileTest_op_mul.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=VolatileTest_op_mul
    if not exist "VolatileTest_op_mul.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "VolatileTest_op_mul.org" (
            mkdir IL
            copy VolatileTest_op_mul.exe IL\VolatileTest_op_mul.exe 
            ren VolatileTest_op_mul.exe VolatileTest_op_mul.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VolatileTest_op_mul.org /out VolatileTest_op_mul.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in VolatileTest_op_mul.org /out VolatileTest_op_mul.exe
...
set ExePath=VolatileTest_op_mul.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in VolatileTest_op_mul.reflect.xml
    IF EXIST VolatileTest_op_mul.reflect.xml set ReflectionRoots=-x VolatileTest_op_mul.reflect.xml
    ...
    set ExePath=%LinkBin%\VolatileTest_op_mul.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% VolatileTest_op_mul.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% VolatileTest_op_mul.asm.exe %CLRTestExecutionArguments% %Host_Args%
