...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/fieldExprUnchecked1 fieldExprUnchecked1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/fieldExprUnchecked1 fieldExprUnchecked1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=fieldExprUnchecked1.dasm.il fieldExprUnchecked1.exe
    "$CORE_ROOT/ildasm" -raweh -out=fieldExprUnchecked1.dasm.il fieldExprUnchecked1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=fieldExprUnchecked1.asm.exe  fieldExprUnchecked1.dasm.il
    "$CORE_ROOT/ilasm" -output=fieldExprUnchecked1.asm.exe  fieldExprUnchecked1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=fieldExprUnchecked1
    if [ ! -f fieldExprUnchecked1.org ]; then
        TakeLock
        if [ ! -f fieldExprUnchecked1.org ]; then
          mkdir IL
          cp fieldExprUnchecked1.exe IL/fieldExprUnchecked1.exe
          mv fieldExprUnchecked1.exe fieldExprUnchecked1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fieldExprUnchecked1.org /out fieldExprUnchecked1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in fieldExprUnchecked1.org /out fieldExprUnchecked1.exe
          __cgExitCode=$?
...
ExePath=fieldExprUnchecked1.exe
...
then
  ...
  # Use hints for reflection roots, if provided in fieldExprUnchecked1.reflect.xml
  if [ -f fieldExprUnchecked1.reflect.xml ]
then
    ReflectionRoots="-x fieldExprUnchecked1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/fieldExprUnchecked1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" fieldExprUnchecked1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" fieldExprUnchecked1.asm.exe $CLRTestExecutionArguments
