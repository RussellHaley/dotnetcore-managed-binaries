...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\fieldExprUnchecked1 fieldExprUnchecked1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\fieldExprUnchecked1 fieldExprUnchecked1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=fieldExprUnchecked1.dasm.il fieldExprUnchecked1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=fieldExprUnchecked1.dasm.il fieldExprUnchecked1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=fieldExprUnchecked1.asm.exe  fieldExprUnchecked1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=fieldExprUnchecked1.asm.exe  fieldExprUnchecked1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=fieldExprUnchecked1
    if not exist "fieldExprUnchecked1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "fieldExprUnchecked1.org" (
            mkdir IL
            copy fieldExprUnchecked1.exe IL\fieldExprUnchecked1.exe 
            ren fieldExprUnchecked1.exe fieldExprUnchecked1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fieldExprUnchecked1.org /out fieldExprUnchecked1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in fieldExprUnchecked1.org /out fieldExprUnchecked1.exe
...
set ExePath=fieldExprUnchecked1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in fieldExprUnchecked1.reflect.xml
    IF EXIST fieldExprUnchecked1.reflect.xml set ReflectionRoots=-x fieldExprUnchecked1.reflect.xml
    ...
    set ExePath=%LinkBin%\fieldExprUnchecked1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% fieldExprUnchecked1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% fieldExprUnchecked1.asm.exe %CLRTestExecutionArguments% %Host_Args%
