...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/staticFieldExprUnchecked1_ro_loop staticFieldExprUnchecked1_ro_loop.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/staticFieldExprUnchecked1_ro_loop staticFieldExprUnchecked1_ro_loop.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=staticFieldExprUnchecked1_ro_loop.dasm.il staticFieldExprUnchecked1_ro_loop.exe
    "$CORE_ROOT/ildasm" -raweh -out=staticFieldExprUnchecked1_ro_loop.dasm.il staticFieldExprUnchecked1_ro_loop.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=staticFieldExprUnchecked1_ro_loop.asm.exe  staticFieldExprUnchecked1_ro_loop.dasm.il
    "$CORE_ROOT/ilasm" -output=staticFieldExprUnchecked1_ro_loop.asm.exe  staticFieldExprUnchecked1_ro_loop.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=staticFieldExprUnchecked1_ro_loop
    if [ ! -f staticFieldExprUnchecked1_ro_loop.org ]; then
        TakeLock
        if [ ! -f staticFieldExprUnchecked1_ro_loop.org ]; then
          mkdir IL
          cp staticFieldExprUnchecked1_ro_loop.exe IL/staticFieldExprUnchecked1_ro_loop.exe
          mv staticFieldExprUnchecked1_ro_loop.exe staticFieldExprUnchecked1_ro_loop.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in staticFieldExprUnchecked1_ro_loop.org /out staticFieldExprUnchecked1_ro_loop.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in staticFieldExprUnchecked1_ro_loop.org /out staticFieldExprUnchecked1_ro_loop.exe
          __cgExitCode=$?
...
ExePath=staticFieldExprUnchecked1_ro_loop.exe
...
then
  ...
  # Use hints for reflection roots, if provided in staticFieldExprUnchecked1_ro_loop.reflect.xml
  if [ -f staticFieldExprUnchecked1_ro_loop.reflect.xml ]
then
    ReflectionRoots="-x staticFieldExprUnchecked1_ro_loop.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/staticFieldExprUnchecked1_ro_loop.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" staticFieldExprUnchecked1_ro_loop.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" staticFieldExprUnchecked1_ro_loop.asm.exe $CLRTestExecutionArguments
