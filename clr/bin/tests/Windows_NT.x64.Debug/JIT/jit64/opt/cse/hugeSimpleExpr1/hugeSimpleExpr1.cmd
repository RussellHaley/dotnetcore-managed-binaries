...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\hugeSimpleExpr1 hugeSimpleExpr1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\hugeSimpleExpr1 hugeSimpleExpr1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=hugeSimpleExpr1.dasm.il hugeSimpleExpr1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=hugeSimpleExpr1.dasm.il hugeSimpleExpr1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=hugeSimpleExpr1.asm.exe  hugeSimpleExpr1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=hugeSimpleExpr1.asm.exe  hugeSimpleExpr1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=hugeSimpleExpr1
    if not exist "hugeSimpleExpr1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "hugeSimpleExpr1.org" (
            mkdir IL
            copy hugeSimpleExpr1.exe IL\hugeSimpleExpr1.exe 
            ren hugeSimpleExpr1.exe hugeSimpleExpr1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hugeSimpleExpr1.org /out hugeSimpleExpr1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hugeSimpleExpr1.org /out hugeSimpleExpr1.exe
...
set ExePath=hugeSimpleExpr1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in hugeSimpleExpr1.reflect.xml
    IF EXIST hugeSimpleExpr1.reflect.xml set ReflectionRoots=-x hugeSimpleExpr1.reflect.xml
    ...
    set ExePath=%LinkBin%\hugeSimpleExpr1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% hugeSimpleExpr1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% hugeSimpleExpr1.asm.exe %CLRTestExecutionArguments% %Host_Args%
