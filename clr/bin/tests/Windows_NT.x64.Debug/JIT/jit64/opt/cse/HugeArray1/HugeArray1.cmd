...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\HugeArray1 HugeArray1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\HugeArray1 HugeArray1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=HugeArray1.dasm.il HugeArray1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=HugeArray1.dasm.il HugeArray1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=HugeArray1.asm.exe  HugeArray1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=HugeArray1.asm.exe  HugeArray1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=HugeArray1
    if not exist "HugeArray1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "HugeArray1.org" (
            mkdir IL
            copy HugeArray1.exe IL\HugeArray1.exe 
            ren HugeArray1.exe HugeArray1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in HugeArray1.org /out HugeArray1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in HugeArray1.org /out HugeArray1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in HugeArray1.reflect.xml
    IF EXIST HugeArray1.reflect.xml set ReflectionRoots=-x HugeArray1.reflect.xml
    ...
    set ExePath=%LinkBin%\HugeArray1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% HugeArray1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% HugeArray1.asm.exe %CLRTestExecutionArguments% %Host_Args%
