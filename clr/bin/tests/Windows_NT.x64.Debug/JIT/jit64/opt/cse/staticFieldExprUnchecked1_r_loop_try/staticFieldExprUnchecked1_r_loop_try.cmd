...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\staticFieldExprUnchecked1_r_loop_try staticFieldExprUnchecked1_r_loop_try.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\staticFieldExprUnchecked1_r_loop_try staticFieldExprUnchecked1_r_loop_try.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=staticFieldExprUnchecked1_r_loop_try.dasm.il staticFieldExprUnchecked1_r_loop_try.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=staticFieldExprUnchecked1_r_loop_try.dasm.il staticFieldExprUnchecked1_r_loop_try.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=staticFieldExprUnchecked1_r_loop_try.asm.exe  staticFieldExprUnchecked1_r_loop_try.dasm.il
    %CORE_ROOT%\ilasm.exe /output=staticFieldExprUnchecked1_r_loop_try.asm.exe  staticFieldExprUnchecked1_r_loop_try.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=staticFieldExprUnchecked1_r_loop_try
    if not exist "staticFieldExprUnchecked1_r_loop_try.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "staticFieldExprUnchecked1_r_loop_try.org" (
            mkdir IL
            copy staticFieldExprUnchecked1_r_loop_try.exe IL\staticFieldExprUnchecked1_r_loop_try.exe 
            ren staticFieldExprUnchecked1_r_loop_try.exe staticFieldExprUnchecked1_r_loop_try.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in staticFieldExprUnchecked1_r_loop_try.org /out staticFieldExprUnchecked1_r_loop_try.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in staticFieldExprUnchecked1_r_loop_try.org /out staticFieldExprUnchecked1_r_loop_try.exe
...
set ExePath=staticFieldExprUnchecked1_r_loop_try.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in staticFieldExprUnchecked1_r_loop_try.reflect.xml
    IF EXIST staticFieldExprUnchecked1_r_loop_try.reflect.xml set ReflectionRoots=-x staticFieldExprUnchecked1_r_loop_try.reflect.xml
    ...
    set ExePath=%LinkBin%\staticFieldExprUnchecked1_r_loop_try.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% staticFieldExprUnchecked1_r_loop_try.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% staticFieldExprUnchecked1_r_loop_try.asm.exe %CLRTestExecutionArguments% %Host_Args%
