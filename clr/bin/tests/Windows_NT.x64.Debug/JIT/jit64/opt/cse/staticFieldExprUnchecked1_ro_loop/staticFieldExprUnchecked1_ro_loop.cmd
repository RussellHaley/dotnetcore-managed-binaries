...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\staticFieldExprUnchecked1_ro_loop staticFieldExprUnchecked1_ro_loop.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\cse\staticFieldExprUnchecked1_ro_loop staticFieldExprUnchecked1_ro_loop.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=staticFieldExprUnchecked1_ro_loop.dasm.il staticFieldExprUnchecked1_ro_loop.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=staticFieldExprUnchecked1_ro_loop.dasm.il staticFieldExprUnchecked1_ro_loop.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=staticFieldExprUnchecked1_ro_loop.asm.exe  staticFieldExprUnchecked1_ro_loop.dasm.il
    %CORE_ROOT%\ilasm.exe /output=staticFieldExprUnchecked1_ro_loop.asm.exe  staticFieldExprUnchecked1_ro_loop.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=staticFieldExprUnchecked1_ro_loop
    if not exist "staticFieldExprUnchecked1_ro_loop.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "staticFieldExprUnchecked1_ro_loop.org" (
            mkdir IL
            copy staticFieldExprUnchecked1_ro_loop.exe IL\staticFieldExprUnchecked1_ro_loop.exe 
            ren staticFieldExprUnchecked1_ro_loop.exe staticFieldExprUnchecked1_ro_loop.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in staticFieldExprUnchecked1_ro_loop.org /out staticFieldExprUnchecked1_ro_loop.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in staticFieldExprUnchecked1_ro_loop.org /out staticFieldExprUnchecked1_ro_loop.exe
...
set ExePath=staticFieldExprUnchecked1_ro_loop.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in staticFieldExprUnchecked1_ro_loop.reflect.xml
    IF EXIST staticFieldExprUnchecked1_ro_loop.reflect.xml set ReflectionRoots=-x staticFieldExprUnchecked1_ro_loop.reflect.xml
    ...
    set ExePath=%LinkBin%\staticFieldExprUnchecked1_ro_loop.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% staticFieldExprUnchecked1_ro_loop.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% staticFieldExprUnchecked1_ro_loop.asm.exe %CLRTestExecutionArguments% %Host_Args%
