...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/simpleexpr4_ro_loop simpleexpr4_ro_loop.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/simpleexpr4_ro_loop simpleexpr4_ro_loop.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=simpleexpr4_ro_loop.dasm.il simpleexpr4_ro_loop.exe
    "$CORE_ROOT/ildasm" -raweh -out=simpleexpr4_ro_loop.dasm.il simpleexpr4_ro_loop.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=simpleexpr4_ro_loop.asm.exe  simpleexpr4_ro_loop.dasm.il
    "$CORE_ROOT/ilasm" -output=simpleexpr4_ro_loop.asm.exe  simpleexpr4_ro_loop.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=simpleexpr4_ro_loop
    if [ ! -f simpleexpr4_ro_loop.org ]; then
        TakeLock
        if [ ! -f simpleexpr4_ro_loop.org ]; then
          mkdir IL
          cp simpleexpr4_ro_loop.exe IL/simpleexpr4_ro_loop.exe
          mv simpleexpr4_ro_loop.exe simpleexpr4_ro_loop.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in simpleexpr4_ro_loop.org /out simpleexpr4_ro_loop.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in simpleexpr4_ro_loop.org /out simpleexpr4_ro_loop.exe
          __cgExitCode=$?
...
ExePath=simpleexpr4_ro_loop.exe
...
then
  ...
  # Use hints for reflection roots, if provided in simpleexpr4_ro_loop.reflect.xml
  if [ -f simpleexpr4_ro_loop.reflect.xml ]
then
    ReflectionRoots="-x simpleexpr4_ro_loop.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/simpleexpr4_ro_loop.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" simpleexpr4_ro_loop.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" simpleexpr4_ro_loop.asm.exe $CLRTestExecutionArguments
