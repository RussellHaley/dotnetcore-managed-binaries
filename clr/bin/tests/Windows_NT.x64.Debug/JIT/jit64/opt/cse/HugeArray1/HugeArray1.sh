...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/HugeArray1 HugeArray1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/HugeArray1 HugeArray1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=HugeArray1.dasm.il HugeArray1.exe
    "$CORE_ROOT/ildasm" -raweh -out=HugeArray1.dasm.il HugeArray1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=HugeArray1.asm.exe  HugeArray1.dasm.il
    "$CORE_ROOT/ilasm" -output=HugeArray1.asm.exe  HugeArray1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=HugeArray1
    if [ ! -f HugeArray1.org ]; then
        TakeLock
        if [ ! -f HugeArray1.org ]; then
          mkdir IL
          cp HugeArray1.exe IL/HugeArray1.exe
          mv HugeArray1.exe HugeArray1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in HugeArray1.org /out HugeArray1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in HugeArray1.org /out HugeArray1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in HugeArray1.reflect.xml
  if [ -f HugeArray1.reflect.xml ]
then
    ReflectionRoots="-x HugeArray1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/HugeArray1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" HugeArray1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" HugeArray1.asm.exe $CLRTestExecutionArguments
