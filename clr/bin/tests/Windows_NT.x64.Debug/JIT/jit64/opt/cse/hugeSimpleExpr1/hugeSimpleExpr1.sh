...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/hugeSimpleExpr1 hugeSimpleExpr1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/hugeSimpleExpr1 hugeSimpleExpr1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=hugeSimpleExpr1.dasm.il hugeSimpleExpr1.exe
    "$CORE_ROOT/ildasm" -raweh -out=hugeSimpleExpr1.dasm.il hugeSimpleExpr1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=hugeSimpleExpr1.asm.exe  hugeSimpleExpr1.dasm.il
    "$CORE_ROOT/ilasm" -output=hugeSimpleExpr1.asm.exe  hugeSimpleExpr1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=hugeSimpleExpr1
    if [ ! -f hugeSimpleExpr1.org ]; then
        TakeLock
        if [ ! -f hugeSimpleExpr1.org ]; then
          mkdir IL
          cp hugeSimpleExpr1.exe IL/hugeSimpleExpr1.exe
          mv hugeSimpleExpr1.exe hugeSimpleExpr1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hugeSimpleExpr1.org /out hugeSimpleExpr1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hugeSimpleExpr1.org /out hugeSimpleExpr1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in hugeSimpleExpr1.reflect.xml
  if [ -f hugeSimpleExpr1.reflect.xml ]
then
    ReflectionRoots="-x hugeSimpleExpr1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/hugeSimpleExpr1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" hugeSimpleExpr1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" hugeSimpleExpr1.asm.exe $CLRTestExecutionArguments
