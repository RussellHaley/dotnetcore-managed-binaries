...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/staticFieldExprUnchecked1_r staticFieldExprUnchecked1_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/cse/staticFieldExprUnchecked1_r staticFieldExprUnchecked1_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=staticFieldExprUnchecked1_r.dasm.il staticFieldExprUnchecked1_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=staticFieldExprUnchecked1_r.dasm.il staticFieldExprUnchecked1_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=staticFieldExprUnchecked1_r.asm.exe  staticFieldExprUnchecked1_r.dasm.il
    "$CORE_ROOT/ilasm" -output=staticFieldExprUnchecked1_r.asm.exe  staticFieldExprUnchecked1_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=staticFieldExprUnchecked1_r
    if [ ! -f staticFieldExprUnchecked1_r.org ]; then
        TakeLock
        if [ ! -f staticFieldExprUnchecked1_r.org ]; then
          mkdir IL
          cp staticFieldExprUnchecked1_r.exe IL/staticFieldExprUnchecked1_r.exe
          mv staticFieldExprUnchecked1_r.exe staticFieldExprUnchecked1_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in staticFieldExprUnchecked1_r.org /out staticFieldExprUnchecked1_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in staticFieldExprUnchecked1_r.org /out staticFieldExprUnchecked1_r.exe
          __cgExitCode=$?
...
ExePath=staticFieldExprUnchecked1_r.exe
...
then
  ...
  # Use hints for reflection roots, if provided in staticFieldExprUnchecked1_r.reflect.xml
  if [ -f staticFieldExprUnchecked1_r.reflect.xml ]
then
    ReflectionRoots="-x staticFieldExprUnchecked1_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/staticFieldExprUnchecked1_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" staticFieldExprUnchecked1_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" staticFieldExprUnchecked1_r.asm.exe $CLRTestExecutionArguments
