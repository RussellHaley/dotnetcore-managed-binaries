...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/ArrayWithFunc_o ArrayWithFunc_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/ArrayWithFunc_o ArrayWithFunc_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ArrayWithFunc_o.dasm.il ArrayWithFunc_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=ArrayWithFunc_o.dasm.il ArrayWithFunc_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ArrayWithFunc_o.asm.exe  ArrayWithFunc_o.dasm.il
    "$CORE_ROOT/ilasm" -output=ArrayWithFunc_o.asm.exe  ArrayWithFunc_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ArrayWithFunc_o
    if [ ! -f ArrayWithFunc_o.org ]; then
        TakeLock
        if [ ! -f ArrayWithFunc_o.org ]; then
          mkdir IL
          cp ArrayWithFunc_o.exe IL/ArrayWithFunc_o.exe
          mv ArrayWithFunc_o.exe ArrayWithFunc_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayWithFunc_o.org /out ArrayWithFunc_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayWithFunc_o.org /out ArrayWithFunc_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ArrayWithFunc_o.reflect.xml
  if [ -f ArrayWithFunc_o.reflect.xml ]
then
    ReflectionRoots="-x ArrayWithFunc_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ArrayWithFunc_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ArrayWithFunc_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ArrayWithFunc_o.asm.exe $CLRTestExecutionArguments
