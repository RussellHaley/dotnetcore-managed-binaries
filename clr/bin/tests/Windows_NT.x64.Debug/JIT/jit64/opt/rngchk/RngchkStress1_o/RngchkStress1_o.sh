...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/RngchkStress1_o RngchkStress1_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/RngchkStress1_o RngchkStress1_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=RngchkStress1_o.dasm.il RngchkStress1_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=RngchkStress1_o.dasm.il RngchkStress1_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=RngchkStress1_o.asm.exe  RngchkStress1_o.dasm.il
    "$CORE_ROOT/ilasm" -output=RngchkStress1_o.asm.exe  RngchkStress1_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=RngchkStress1_o
    if [ ! -f RngchkStress1_o.org ]; then
        TakeLock
        if [ ! -f RngchkStress1_o.org ]; then
          mkdir IL
          cp RngchkStress1_o.exe IL/RngchkStress1_o.exe
          mv RngchkStress1_o.exe RngchkStress1_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RngchkStress1_o.org /out RngchkStress1_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RngchkStress1_o.org /out RngchkStress1_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in RngchkStress1_o.reflect.xml
  if [ -f RngchkStress1_o.reflect.xml ]
then
    ReflectionRoots="-x RngchkStress1_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/RngchkStress1_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" RngchkStress1_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" RngchkStress1_o.asm.exe $CLRTestExecutionArguments
