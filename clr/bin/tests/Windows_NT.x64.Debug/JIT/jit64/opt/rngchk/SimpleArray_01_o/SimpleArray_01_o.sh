...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/SimpleArray_01_o SimpleArray_01_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/SimpleArray_01_o SimpleArray_01_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=SimpleArray_01_o.dasm.il SimpleArray_01_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=SimpleArray_01_o.dasm.il SimpleArray_01_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=SimpleArray_01_o.asm.exe  SimpleArray_01_o.dasm.il
    "$CORE_ROOT/ilasm" -output=SimpleArray_01_o.asm.exe  SimpleArray_01_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=SimpleArray_01_o
    if [ ! -f SimpleArray_01_o.org ]; then
        TakeLock
        if [ ! -f SimpleArray_01_o.org ]; then
          mkdir IL
          cp SimpleArray_01_o.exe IL/SimpleArray_01_o.exe
          mv SimpleArray_01_o.exe SimpleArray_01_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SimpleArray_01_o.org /out SimpleArray_01_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SimpleArray_01_o.org /out SimpleArray_01_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in SimpleArray_01_o.reflect.xml
  if [ -f SimpleArray_01_o.reflect.xml ]
then
    ReflectionRoots="-x SimpleArray_01_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/SimpleArray_01_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" SimpleArray_01_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" SimpleArray_01_o.asm.exe $CLRTestExecutionArguments
