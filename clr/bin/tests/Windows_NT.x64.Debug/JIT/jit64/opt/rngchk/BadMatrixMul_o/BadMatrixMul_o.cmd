...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\BadMatrixMul_o BadMatrixMul_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\BadMatrixMul_o BadMatrixMul_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=BadMatrixMul_o.dasm.il BadMatrixMul_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=BadMatrixMul_o.dasm.il BadMatrixMul_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=BadMatrixMul_o.asm.exe  BadMatrixMul_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=BadMatrixMul_o.asm.exe  BadMatrixMul_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=BadMatrixMul_o
    if not exist "BadMatrixMul_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "BadMatrixMul_o.org" (
            mkdir IL
            copy BadMatrixMul_o.exe IL\BadMatrixMul_o.exe 
            ren BadMatrixMul_o.exe BadMatrixMul_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BadMatrixMul_o.org /out BadMatrixMul_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in BadMatrixMul_o.org /out BadMatrixMul_o.exe
...
set ExePath=BadMatrixMul_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in BadMatrixMul_o.reflect.xml
    IF EXIST BadMatrixMul_o.reflect.xml set ReflectionRoots=-x BadMatrixMul_o.reflect.xml
    ...
    set ExePath=%LinkBin%\BadMatrixMul_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% BadMatrixMul_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% BadMatrixMul_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
