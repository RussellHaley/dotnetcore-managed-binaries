...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\SimpleArray_01_o SimpleArray_01_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\SimpleArray_01_o SimpleArray_01_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=SimpleArray_01_o.dasm.il SimpleArray_01_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=SimpleArray_01_o.dasm.il SimpleArray_01_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=SimpleArray_01_o.asm.exe  SimpleArray_01_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=SimpleArray_01_o.asm.exe  SimpleArray_01_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=SimpleArray_01_o
    if not exist "SimpleArray_01_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "SimpleArray_01_o.org" (
            mkdir IL
            copy SimpleArray_01_o.exe IL\SimpleArray_01_o.exe 
            ren SimpleArray_01_o.exe SimpleArray_01_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SimpleArray_01_o.org /out SimpleArray_01_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SimpleArray_01_o.org /out SimpleArray_01_o.exe
...
set ExePath=SimpleArray_01_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in SimpleArray_01_o.reflect.xml
    IF EXIST SimpleArray_01_o.reflect.xml set ReflectionRoots=-x SimpleArray_01_o.reflect.xml
    ...
    set ExePath=%LinkBin%\SimpleArray_01_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% SimpleArray_01_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% SimpleArray_01_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
