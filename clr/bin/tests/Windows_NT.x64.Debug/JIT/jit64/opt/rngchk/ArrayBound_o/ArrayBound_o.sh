...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/ArrayBound_o ArrayBound_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/ArrayBound_o ArrayBound_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ArrayBound_o.dasm.il ArrayBound_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=ArrayBound_o.dasm.il ArrayBound_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ArrayBound_o.asm.exe  ArrayBound_o.dasm.il
    "$CORE_ROOT/ilasm" -output=ArrayBound_o.asm.exe  ArrayBound_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ArrayBound_o
    if [ ! -f ArrayBound_o.org ]; then
        TakeLock
        if [ ! -f ArrayBound_o.org ]; then
          mkdir IL
          cp ArrayBound_o.exe IL/ArrayBound_o.exe
          mv ArrayBound_o.exe ArrayBound_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayBound_o.org /out ArrayBound_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayBound_o.org /out ArrayBound_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ArrayBound_o.reflect.xml
  if [ -f ArrayBound_o.reflect.xml ]
then
    ReflectionRoots="-x ArrayBound_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ArrayBound_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ArrayBound_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ArrayBound_o.asm.exe $CLRTestExecutionArguments
