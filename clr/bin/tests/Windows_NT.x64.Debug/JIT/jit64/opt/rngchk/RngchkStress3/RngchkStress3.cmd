...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\RngchkStress3 RngchkStress3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\RngchkStress3 RngchkStress3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=RngchkStress3.dasm.il RngchkStress3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=RngchkStress3.dasm.il RngchkStress3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=RngchkStress3.asm.exe  RngchkStress3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=RngchkStress3.asm.exe  RngchkStress3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=RngchkStress3
    if not exist "RngchkStress3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "RngchkStress3.org" (
            mkdir IL
            copy RngchkStress3.exe IL\RngchkStress3.exe 
            ren RngchkStress3.exe RngchkStress3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RngchkStress3.org /out RngchkStress3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RngchkStress3.org /out RngchkStress3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in RngchkStress3.reflect.xml
    IF EXIST RngchkStress3.reflect.xml set ReflectionRoots=-x RngchkStress3.reflect.xml
    ...
    set ExePath=%LinkBin%\RngchkStress3.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% RngchkStress3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% RngchkStress3.asm.exe %CLRTestExecutionArguments% %Host_Args%
