...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\ArrayWithThread_o ArrayWithThread_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\ArrayWithThread_o ArrayWithThread_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ArrayWithThread_o.dasm.il ArrayWithThread_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ArrayWithThread_o.dasm.il ArrayWithThread_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ArrayWithThread_o.asm.exe  ArrayWithThread_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ArrayWithThread_o.asm.exe  ArrayWithThread_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ArrayWithThread_o
    if not exist "ArrayWithThread_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ArrayWithThread_o.org" (
            mkdir IL
            copy ArrayWithThread_o.exe IL\ArrayWithThread_o.exe 
            ren ArrayWithThread_o.exe ArrayWithThread_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayWithThread_o.org /out ArrayWithThread_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayWithThread_o.org /out ArrayWithThread_o.exe
...
set ExePath=ArrayWithThread_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ArrayWithThread_o.reflect.xml
    IF EXIST ArrayWithThread_o.reflect.xml set ReflectionRoots=-x ArrayWithThread_o.reflect.xml
    ...
    set ExePath=%LinkBin%\ArrayWithThread_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ArrayWithThread_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ArrayWithThread_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
