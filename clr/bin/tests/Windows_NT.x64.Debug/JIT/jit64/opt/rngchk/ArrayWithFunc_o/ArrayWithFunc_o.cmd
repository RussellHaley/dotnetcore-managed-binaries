...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\ArrayWithFunc_o ArrayWithFunc_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\ArrayWithFunc_o ArrayWithFunc_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ArrayWithFunc_o.dasm.il ArrayWithFunc_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ArrayWithFunc_o.dasm.il ArrayWithFunc_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ArrayWithFunc_o.asm.exe  ArrayWithFunc_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ArrayWithFunc_o.asm.exe  ArrayWithFunc_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ArrayWithFunc_o
    if not exist "ArrayWithFunc_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ArrayWithFunc_o.org" (
            mkdir IL
            copy ArrayWithFunc_o.exe IL\ArrayWithFunc_o.exe 
            ren ArrayWithFunc_o.exe ArrayWithFunc_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayWithFunc_o.org /out ArrayWithFunc_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayWithFunc_o.org /out ArrayWithFunc_o.exe
...
set ExePath=ArrayWithFunc_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ArrayWithFunc_o.reflect.xml
    IF EXIST ArrayWithFunc_o.reflect.xml set ReflectionRoots=-x ArrayWithFunc_o.reflect.xml
    ...
    set ExePath=%LinkBin%\ArrayWithFunc_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ArrayWithFunc_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ArrayWithFunc_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
