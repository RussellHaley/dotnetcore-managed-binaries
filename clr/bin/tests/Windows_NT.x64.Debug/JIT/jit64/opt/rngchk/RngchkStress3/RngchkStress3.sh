...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/RngchkStress3 RngchkStress3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/RngchkStress3 RngchkStress3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=RngchkStress3.dasm.il RngchkStress3.exe
    "$CORE_ROOT/ildasm" -raweh -out=RngchkStress3.dasm.il RngchkStress3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=RngchkStress3.asm.exe  RngchkStress3.dasm.il
    "$CORE_ROOT/ilasm" -output=RngchkStress3.asm.exe  RngchkStress3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=RngchkStress3
    if [ ! -f RngchkStress3.org ]; then
        TakeLock
        if [ ! -f RngchkStress3.org ]; then
          mkdir IL
          cp RngchkStress3.exe IL/RngchkStress3.exe
          mv RngchkStress3.exe RngchkStress3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RngchkStress3.org /out RngchkStress3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RngchkStress3.org /out RngchkStress3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in RngchkStress3.reflect.xml
  if [ -f RngchkStress3.reflect.xml ]
then
    ReflectionRoots="-x RngchkStress3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/RngchkStress3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" RngchkStress3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" RngchkStress3.asm.exe $CLRTestExecutionArguments
