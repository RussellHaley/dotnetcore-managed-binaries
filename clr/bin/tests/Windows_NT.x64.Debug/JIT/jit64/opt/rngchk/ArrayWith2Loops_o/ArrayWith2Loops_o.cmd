...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\ArrayWith2Loops_o ArrayWith2Loops_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\ArrayWith2Loops_o ArrayWith2Loops_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ArrayWith2Loops_o.dasm.il ArrayWith2Loops_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ArrayWith2Loops_o.dasm.il ArrayWith2Loops_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ArrayWith2Loops_o.asm.exe  ArrayWith2Loops_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ArrayWith2Loops_o.asm.exe  ArrayWith2Loops_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ArrayWith2Loops_o
    if not exist "ArrayWith2Loops_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ArrayWith2Loops_o.org" (
            mkdir IL
            copy ArrayWith2Loops_o.exe IL\ArrayWith2Loops_o.exe 
            ren ArrayWith2Loops_o.exe ArrayWith2Loops_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayWith2Loops_o.org /out ArrayWith2Loops_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayWith2Loops_o.org /out ArrayWith2Loops_o.exe
...
set ExePath=ArrayWith2Loops_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ArrayWith2Loops_o.reflect.xml
    IF EXIST ArrayWith2Loops_o.reflect.xml set ReflectionRoots=-x ArrayWith2Loops_o.reflect.xml
    ...
    set ExePath=%LinkBin%\ArrayWith2Loops_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ArrayWith2Loops_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ArrayWith2Loops_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
