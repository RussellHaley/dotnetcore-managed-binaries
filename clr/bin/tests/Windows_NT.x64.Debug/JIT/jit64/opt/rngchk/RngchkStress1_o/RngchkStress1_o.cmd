...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\RngchkStress1_o RngchkStress1_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\RngchkStress1_o RngchkStress1_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=RngchkStress1_o.dasm.il RngchkStress1_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=RngchkStress1_o.dasm.il RngchkStress1_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=RngchkStress1_o.asm.exe  RngchkStress1_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=RngchkStress1_o.asm.exe  RngchkStress1_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=RngchkStress1_o
    if not exist "RngchkStress1_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "RngchkStress1_o.org" (
            mkdir IL
            copy RngchkStress1_o.exe IL\RngchkStress1_o.exe 
            ren RngchkStress1_o.exe RngchkStress1_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RngchkStress1_o.org /out RngchkStress1_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RngchkStress1_o.org /out RngchkStress1_o.exe
...
set ExePath=RngchkStress1_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in RngchkStress1_o.reflect.xml
    IF EXIST RngchkStress1_o.reflect.xml set ReflectionRoots=-x RngchkStress1_o.reflect.xml
    ...
    set ExePath=%LinkBin%\RngchkStress1_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% RngchkStress1_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% RngchkStress1_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
