...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/ArrayWithThread_o ArrayWithThread_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/ArrayWithThread_o ArrayWithThread_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ArrayWithThread_o.dasm.il ArrayWithThread_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=ArrayWithThread_o.dasm.il ArrayWithThread_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ArrayWithThread_o.asm.exe  ArrayWithThread_o.dasm.il
    "$CORE_ROOT/ilasm" -output=ArrayWithThread_o.asm.exe  ArrayWithThread_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ArrayWithThread_o
    if [ ! -f ArrayWithThread_o.org ]; then
        TakeLock
        if [ ! -f ArrayWithThread_o.org ]; then
          mkdir IL
          cp ArrayWithThread_o.exe IL/ArrayWithThread_o.exe
          mv ArrayWithThread_o.exe ArrayWithThread_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayWithThread_o.org /out ArrayWithThread_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayWithThread_o.org /out ArrayWithThread_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ArrayWithThread_o.reflect.xml
  if [ -f ArrayWithThread_o.reflect.xml ]
then
    ReflectionRoots="-x ArrayWithThread_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ArrayWithThread_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ArrayWithThread_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ArrayWithThread_o.asm.exe $CLRTestExecutionArguments
