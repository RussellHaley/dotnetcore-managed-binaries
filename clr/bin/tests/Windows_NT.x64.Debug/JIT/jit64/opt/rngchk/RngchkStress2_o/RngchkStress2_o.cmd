...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\RngchkStress2_o RngchkStress2_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\RngchkStress2_o RngchkStress2_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=RngchkStress2_o.dasm.il RngchkStress2_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=RngchkStress2_o.dasm.il RngchkStress2_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=RngchkStress2_o.asm.exe  RngchkStress2_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=RngchkStress2_o.asm.exe  RngchkStress2_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=RngchkStress2_o
    if not exist "RngchkStress2_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "RngchkStress2_o.org" (
            mkdir IL
            copy RngchkStress2_o.exe IL\RngchkStress2_o.exe 
            ren RngchkStress2_o.exe RngchkStress2_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RngchkStress2_o.org /out RngchkStress2_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RngchkStress2_o.org /out RngchkStress2_o.exe
...
set ExePath=RngchkStress2_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in RngchkStress2_o.reflect.xml
    IF EXIST RngchkStress2_o.reflect.xml set ReflectionRoots=-x RngchkStress2_o.reflect.xml
    ...
    set ExePath=%LinkBin%\RngchkStress2_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% RngchkStress2_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% RngchkStress2_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
