...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\ArrayBound_o ArrayBound_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\ArrayBound_o ArrayBound_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ArrayBound_o.dasm.il ArrayBound_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ArrayBound_o.dasm.il ArrayBound_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ArrayBound_o.asm.exe  ArrayBound_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ArrayBound_o.asm.exe  ArrayBound_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ArrayBound_o
    if not exist "ArrayBound_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ArrayBound_o.org" (
            mkdir IL
            copy ArrayBound_o.exe IL\ArrayBound_o.exe 
            ren ArrayBound_o.exe ArrayBound_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayBound_o.org /out ArrayBound_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ArrayBound_o.org /out ArrayBound_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ArrayBound_o.reflect.xml
    IF EXIST ArrayBound_o.reflect.xml set ReflectionRoots=-x ArrayBound_o.reflect.xml
    ...
    set ExePath=%LinkBin%\ArrayBound_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ArrayBound_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ArrayBound_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
