...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\JaggedArray_o JaggedArray_o.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\rngchk\JaggedArray_o JaggedArray_o.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=JaggedArray_o.dasm.il JaggedArray_o.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=JaggedArray_o.dasm.il JaggedArray_o.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=JaggedArray_o.asm.exe  JaggedArray_o.dasm.il
    %CORE_ROOT%\ilasm.exe /output=JaggedArray_o.asm.exe  JaggedArray_o.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=JaggedArray_o
    if not exist "JaggedArray_o.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "JaggedArray_o.org" (
            mkdir IL
            copy JaggedArray_o.exe IL\JaggedArray_o.exe 
            ren JaggedArray_o.exe JaggedArray_o.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in JaggedArray_o.org /out JaggedArray_o.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in JaggedArray_o.org /out JaggedArray_o.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in JaggedArray_o.reflect.xml
    IF EXIST JaggedArray_o.reflect.xml set ReflectionRoots=-x JaggedArray_o.reflect.xml
    ...
    set ExePath=%LinkBin%\JaggedArray_o.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% JaggedArray_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% JaggedArray_o.asm.exe %CLRTestExecutionArguments% %Host_Args%
