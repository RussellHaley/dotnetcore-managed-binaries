...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/BadMatrixMul_o BadMatrixMul_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/BadMatrixMul_o BadMatrixMul_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=BadMatrixMul_o.dasm.il BadMatrixMul_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=BadMatrixMul_o.dasm.il BadMatrixMul_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=BadMatrixMul_o.asm.exe  BadMatrixMul_o.dasm.il
    "$CORE_ROOT/ilasm" -output=BadMatrixMul_o.asm.exe  BadMatrixMul_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=BadMatrixMul_o
    if [ ! -f BadMatrixMul_o.org ]; then
        TakeLock
        if [ ! -f BadMatrixMul_o.org ]; then
          mkdir IL
          cp BadMatrixMul_o.exe IL/BadMatrixMul_o.exe
          mv BadMatrixMul_o.exe BadMatrixMul_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BadMatrixMul_o.org /out BadMatrixMul_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in BadMatrixMul_o.org /out BadMatrixMul_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in BadMatrixMul_o.reflect.xml
  if [ -f BadMatrixMul_o.reflect.xml ]
then
    ReflectionRoots="-x BadMatrixMul_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/BadMatrixMul_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" BadMatrixMul_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" BadMatrixMul_o.asm.exe $CLRTestExecutionArguments
