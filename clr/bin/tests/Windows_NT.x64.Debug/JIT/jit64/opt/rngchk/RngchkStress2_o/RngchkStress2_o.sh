...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/RngchkStress2_o RngchkStress2_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/RngchkStress2_o RngchkStress2_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=RngchkStress2_o.dasm.il RngchkStress2_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=RngchkStress2_o.dasm.il RngchkStress2_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=RngchkStress2_o.asm.exe  RngchkStress2_o.dasm.il
    "$CORE_ROOT/ilasm" -output=RngchkStress2_o.asm.exe  RngchkStress2_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=RngchkStress2_o
    if [ ! -f RngchkStress2_o.org ]; then
        TakeLock
        if [ ! -f RngchkStress2_o.org ]; then
          mkdir IL
          cp RngchkStress2_o.exe IL/RngchkStress2_o.exe
          mv RngchkStress2_o.exe RngchkStress2_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RngchkStress2_o.org /out RngchkStress2_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RngchkStress2_o.org /out RngchkStress2_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in RngchkStress2_o.reflect.xml
  if [ -f RngchkStress2_o.reflect.xml ]
then
    ReflectionRoots="-x RngchkStress2_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/RngchkStress2_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" RngchkStress2_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" RngchkStress2_o.asm.exe $CLRTestExecutionArguments
