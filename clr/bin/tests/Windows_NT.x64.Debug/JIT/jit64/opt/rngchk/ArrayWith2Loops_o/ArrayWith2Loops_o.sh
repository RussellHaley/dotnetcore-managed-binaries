...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/ArrayWith2Loops_o ArrayWith2Loops_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/ArrayWith2Loops_o ArrayWith2Loops_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ArrayWith2Loops_o.dasm.il ArrayWith2Loops_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=ArrayWith2Loops_o.dasm.il ArrayWith2Loops_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ArrayWith2Loops_o.asm.exe  ArrayWith2Loops_o.dasm.il
    "$CORE_ROOT/ilasm" -output=ArrayWith2Loops_o.asm.exe  ArrayWith2Loops_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ArrayWith2Loops_o
    if [ ! -f ArrayWith2Loops_o.org ]; then
        TakeLock
        if [ ! -f ArrayWith2Loops_o.org ]; then
          mkdir IL
          cp ArrayWith2Loops_o.exe IL/ArrayWith2Loops_o.exe
          mv ArrayWith2Loops_o.exe ArrayWith2Loops_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayWith2Loops_o.org /out ArrayWith2Loops_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ArrayWith2Loops_o.org /out ArrayWith2Loops_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ArrayWith2Loops_o.reflect.xml
  if [ -f ArrayWith2Loops_o.reflect.xml ]
then
    ReflectionRoots="-x ArrayWith2Loops_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ArrayWith2Loops_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ArrayWith2Loops_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ArrayWith2Loops_o.asm.exe $CLRTestExecutionArguments
