...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/JaggedArray_o JaggedArray_o.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/rngchk/JaggedArray_o JaggedArray_o.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=JaggedArray_o.dasm.il JaggedArray_o.exe
    "$CORE_ROOT/ildasm" -raweh -out=JaggedArray_o.dasm.il JaggedArray_o.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=JaggedArray_o.asm.exe  JaggedArray_o.dasm.il
    "$CORE_ROOT/ilasm" -output=JaggedArray_o.asm.exe  JaggedArray_o.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=JaggedArray_o
    if [ ! -f JaggedArray_o.org ]; then
        TakeLock
        if [ ! -f JaggedArray_o.org ]; then
          mkdir IL
          cp JaggedArray_o.exe IL/JaggedArray_o.exe
          mv JaggedArray_o.exe JaggedArray_o.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in JaggedArray_o.org /out JaggedArray_o.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in JaggedArray_o.org /out JaggedArray_o.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in JaggedArray_o.reflect.xml
  if [ -f JaggedArray_o.reflect.xml ]
then
    ReflectionRoots="-x JaggedArray_o.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/JaggedArray_o.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" JaggedArray_o.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" JaggedArray_o.asm.exe $CLRTestExecutionArguments
