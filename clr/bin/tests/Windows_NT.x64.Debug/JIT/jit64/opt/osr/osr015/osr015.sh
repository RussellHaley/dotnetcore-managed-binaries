...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/osr/osr015 osr015.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/opt/osr/osr015 osr015.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=osr015.dasm.il osr015.exe
    "$CORE_ROOT/ildasm" -raweh -out=osr015.dasm.il osr015.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=osr015.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE osr015.dasm.il
    "$CORE_ROOT/ilasm" -output=osr015.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE osr015.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=osr015
    if [ ! -f osr015.org ]; then
        TakeLock
        if [ ! -f osr015.org ]; then
          mkdir IL
          cp osr015.exe IL/osr015.exe
          mv osr015.exe osr015.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in osr015.org /out osr015.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in osr015.org /out osr015.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in osr015.reflect.xml
  if [ -f osr015.reflect.xml ]
then
    ReflectionRoots="-x osr015.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" osr015.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" osr015.asm.exe $CLRTestExecutionArguments
