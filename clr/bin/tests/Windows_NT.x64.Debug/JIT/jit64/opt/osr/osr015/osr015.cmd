...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\osr\osr015 osr015.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\opt\osr\osr015 osr015.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=osr015.dasm.il osr015.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=osr015.dasm.il osr015.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=osr015.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE osr015.dasm.il
    %CORE_ROOT%\ilasm.exe /output=osr015.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE osr015.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=osr015
    if not exist "osr015.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "osr015.org" (
            mkdir IL
            copy osr015.exe IL\osr015.exe 
            ren osr015.exe osr015.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in osr015.org /out osr015.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in osr015.org /out osr015.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in osr015.reflect.xml
    IF EXIST osr015.reflect.xml set ReflectionRoots=-x osr015.reflect.xml
    ...
    set ExePath=%LinkBin%\osr015.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% osr015.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% osr015.asm.exe %CLRTestExecutionArguments% %Host_Args%
