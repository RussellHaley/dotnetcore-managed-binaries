...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\verif\sniff\fg\ver_fg_13 ver_fg_13.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\verif\sniff\fg\ver_fg_13 ver_fg_13.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ver_fg_13.dasm.il ver_fg_13.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ver_fg_13.dasm.il ver_fg_13.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ver_fg_13.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ver_fg_13.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ver_fg_13.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ver_fg_13.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ver_fg_13
    if not exist "ver_fg_13.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ver_fg_13.org" (
            mkdir IL
            copy ver_fg_13.exe IL\ver_fg_13.exe 
            ren ver_fg_13.exe ver_fg_13.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ver_fg_13.org /out ver_fg_13.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ver_fg_13.org /out ver_fg_13.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ver_fg_13.reflect.xml
    IF EXIST ver_fg_13.reflect.xml set ReflectionRoots=-x ver_fg_13.reflect.xml
    ...
    set ExePath=%LinkBin%\ver_fg_13.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ver_fg_13.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ver_fg_13.asm.exe %CLRTestExecutionArguments% %Host_Args%
