...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/verif/sniff/fg/ver_fg_13 ver_fg_13.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/verif/sniff/fg/ver_fg_13 ver_fg_13.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ver_fg_13.dasm.il ver_fg_13.exe
    "$CORE_ROOT/ildasm" -raweh -out=ver_fg_13.dasm.il ver_fg_13.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ver_fg_13.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ver_fg_13.dasm.il
    "$CORE_ROOT/ilasm" -output=ver_fg_13.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ver_fg_13.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ver_fg_13
    if [ ! -f ver_fg_13.org ]; then
        TakeLock
        if [ ! -f ver_fg_13.org ]; then
          mkdir IL
          cp ver_fg_13.exe IL/ver_fg_13.exe
          mv ver_fg_13.exe ver_fg_13.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ver_fg_13.org /out ver_fg_13.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ver_fg_13.org /out ver_fg_13.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ver_fg_13.reflect.xml
  if [ -f ver_fg_13.reflect.xml ]
then
    ReflectionRoots="-x ver_fg_13.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ver_fg_13.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ver_fg_13.asm.exe $CLRTestExecutionArguments
