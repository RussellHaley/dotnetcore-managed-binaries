...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow02_mul overflow02_mul.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow02_mul overflow02_mul.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=overflow02_mul.dasm.il overflow02_mul.exe
    "$CORE_ROOT/ildasm" -raweh -out=overflow02_mul.dasm.il overflow02_mul.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=overflow02_mul.asm.exe  overflow02_mul.dasm.il
    "$CORE_ROOT/ilasm" -output=overflow02_mul.asm.exe  overflow02_mul.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=overflow02_mul
    if [ ! -f overflow02_mul.org ]; then
        TakeLock
        if [ ! -f overflow02_mul.org ]; then
          mkdir IL
          cp overflow02_mul.exe IL/overflow02_mul.exe
          mv overflow02_mul.exe overflow02_mul.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow02_mul.org /out overflow02_mul.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow02_mul.org /out overflow02_mul.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in overflow02_mul.reflect.xml
  if [ -f overflow02_mul.reflect.xml ]
then
    ReflectionRoots="-x overflow02_mul.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/overflow02_mul.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" overflow02_mul.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" overflow02_mul.asm.exe $CLRTestExecutionArguments
