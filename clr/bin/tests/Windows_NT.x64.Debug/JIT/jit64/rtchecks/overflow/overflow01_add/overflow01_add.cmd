...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\rtchecks\overflow\overflow01_add overflow01_add.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\rtchecks\overflow\overflow01_add overflow01_add.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=overflow01_add.dasm.il overflow01_add.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=overflow01_add.dasm.il overflow01_add.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=overflow01_add.asm.exe  overflow01_add.dasm.il
    %CORE_ROOT%\ilasm.exe /output=overflow01_add.asm.exe  overflow01_add.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=overflow01_add
    if not exist "overflow01_add.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "overflow01_add.org" (
            mkdir IL
            copy overflow01_add.exe IL\overflow01_add.exe 
            ren overflow01_add.exe overflow01_add.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in overflow01_add.org /out overflow01_add.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in overflow01_add.org /out overflow01_add.exe
...
set ExePath=overflow01_add.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in overflow01_add.reflect.xml
    IF EXIST overflow01_add.reflect.xml set ReflectionRoots=-x overflow01_add.reflect.xml
    ...
    set ExePath=%LinkBin%\overflow01_add.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% overflow01_add.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% overflow01_add.asm.exe %CLRTestExecutionArguments% %Host_Args%
