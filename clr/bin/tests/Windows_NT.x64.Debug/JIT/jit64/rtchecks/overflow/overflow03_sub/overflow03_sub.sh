...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow03_sub overflow03_sub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow03_sub overflow03_sub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=overflow03_sub.dasm.il overflow03_sub.exe
    "$CORE_ROOT/ildasm" -raweh -out=overflow03_sub.dasm.il overflow03_sub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=overflow03_sub.asm.exe  overflow03_sub.dasm.il
    "$CORE_ROOT/ilasm" -output=overflow03_sub.asm.exe  overflow03_sub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=overflow03_sub
    if [ ! -f overflow03_sub.org ]; then
        TakeLock
        if [ ! -f overflow03_sub.org ]; then
          mkdir IL
          cp overflow03_sub.exe IL/overflow03_sub.exe
          mv overflow03_sub.exe overflow03_sub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow03_sub.org /out overflow03_sub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow03_sub.org /out overflow03_sub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in overflow03_sub.reflect.xml
  if [ -f overflow03_sub.reflect.xml ]
then
    ReflectionRoots="-x overflow03_sub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/overflow03_sub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" overflow03_sub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" overflow03_sub.asm.exe $CLRTestExecutionArguments
