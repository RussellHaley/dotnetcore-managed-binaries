...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\rtchecks\overflow\overflow03_add overflow03_add.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\rtchecks\overflow\overflow03_add overflow03_add.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=overflow03_add.dasm.il overflow03_add.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=overflow03_add.dasm.il overflow03_add.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=overflow03_add.asm.exe  overflow03_add.dasm.il
    %CORE_ROOT%\ilasm.exe /output=overflow03_add.asm.exe  overflow03_add.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=overflow03_add
    if not exist "overflow03_add.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "overflow03_add.org" (
            mkdir IL
            copy overflow03_add.exe IL\overflow03_add.exe 
            ren overflow03_add.exe overflow03_add.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in overflow03_add.org /out overflow03_add.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in overflow03_add.org /out overflow03_add.exe
...
set ExePath=overflow03_add.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in overflow03_add.reflect.xml
    IF EXIST overflow03_add.reflect.xml set ReflectionRoots=-x overflow03_add.reflect.xml
    ...
    set ExePath=%LinkBin%\overflow03_add.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% overflow03_add.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% overflow03_add.asm.exe %CLRTestExecutionArguments% %Host_Args%
