...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow04_mul overflow04_mul.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow04_mul overflow04_mul.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=overflow04_mul.dasm.il overflow04_mul.exe
    "$CORE_ROOT/ildasm" -raweh -out=overflow04_mul.dasm.il overflow04_mul.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=overflow04_mul.asm.exe  overflow04_mul.dasm.il
    "$CORE_ROOT/ilasm" -output=overflow04_mul.asm.exe  overflow04_mul.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=overflow04_mul
    if [ ! -f overflow04_mul.org ]; then
        TakeLock
        if [ ! -f overflow04_mul.org ]; then
          mkdir IL
          cp overflow04_mul.exe IL/overflow04_mul.exe
          mv overflow04_mul.exe overflow04_mul.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow04_mul.org /out overflow04_mul.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow04_mul.org /out overflow04_mul.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in overflow04_mul.reflect.xml
  if [ -f overflow04_mul.reflect.xml ]
then
    ReflectionRoots="-x overflow04_mul.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/overflow04_mul.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" overflow04_mul.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" overflow04_mul.asm.exe $CLRTestExecutionArguments
