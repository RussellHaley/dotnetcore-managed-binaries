...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow03_add overflow03_add.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow03_add overflow03_add.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=overflow03_add.dasm.il overflow03_add.exe
    "$CORE_ROOT/ildasm" -raweh -out=overflow03_add.dasm.il overflow03_add.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=overflow03_add.asm.exe  overflow03_add.dasm.il
    "$CORE_ROOT/ilasm" -output=overflow03_add.asm.exe  overflow03_add.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=overflow03_add
    if [ ! -f overflow03_add.org ]; then
        TakeLock
        if [ ! -f overflow03_add.org ]; then
          mkdir IL
          cp overflow03_add.exe IL/overflow03_add.exe
          mv overflow03_add.exe overflow03_add.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow03_add.org /out overflow03_add.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow03_add.org /out overflow03_add.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in overflow03_add.reflect.xml
  if [ -f overflow03_add.reflect.xml ]
then
    ReflectionRoots="-x overflow03_add.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/overflow03_add.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" overflow03_add.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" overflow03_add.asm.exe $CLRTestExecutionArguments
