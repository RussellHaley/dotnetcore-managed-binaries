...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow01_sub overflow01_sub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow01_sub overflow01_sub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=overflow01_sub.dasm.il overflow01_sub.exe
    "$CORE_ROOT/ildasm" -raweh -out=overflow01_sub.dasm.il overflow01_sub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=overflow01_sub.asm.exe  overflow01_sub.dasm.il
    "$CORE_ROOT/ilasm" -output=overflow01_sub.asm.exe  overflow01_sub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=overflow01_sub
    if [ ! -f overflow01_sub.org ]; then
        TakeLock
        if [ ! -f overflow01_sub.org ]; then
          mkdir IL
          cp overflow01_sub.exe IL/overflow01_sub.exe
          mv overflow01_sub.exe overflow01_sub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow01_sub.org /out overflow01_sub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow01_sub.org /out overflow01_sub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in overflow01_sub.reflect.xml
  if [ -f overflow01_sub.reflect.xml ]
then
    ReflectionRoots="-x overflow01_sub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/overflow01_sub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" overflow01_sub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" overflow01_sub.asm.exe $CLRTestExecutionArguments
