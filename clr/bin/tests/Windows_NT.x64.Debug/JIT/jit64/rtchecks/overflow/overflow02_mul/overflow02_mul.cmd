...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\rtchecks\overflow\overflow02_mul overflow02_mul.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\rtchecks\overflow\overflow02_mul overflow02_mul.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=overflow02_mul.dasm.il overflow02_mul.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=overflow02_mul.dasm.il overflow02_mul.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=overflow02_mul.asm.exe  overflow02_mul.dasm.il
    %CORE_ROOT%\ilasm.exe /output=overflow02_mul.asm.exe  overflow02_mul.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=overflow02_mul
    if not exist "overflow02_mul.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "overflow02_mul.org" (
            mkdir IL
            copy overflow02_mul.exe IL\overflow02_mul.exe 
            ren overflow02_mul.exe overflow02_mul.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in overflow02_mul.org /out overflow02_mul.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in overflow02_mul.org /out overflow02_mul.exe
...
set ExePath=overflow02_mul.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in overflow02_mul.reflect.xml
    IF EXIST overflow02_mul.reflect.xml set ReflectionRoots=-x overflow02_mul.reflect.xml
    ...
    set ExePath=%LinkBin%\overflow02_mul.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% overflow02_mul.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% overflow02_mul.asm.exe %CLRTestExecutionArguments% %Host_Args%
