...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\rtchecks\overflow\overflow04_sub overflow04_sub.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\rtchecks\overflow\overflow04_sub overflow04_sub.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=overflow04_sub.dasm.il overflow04_sub.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=overflow04_sub.dasm.il overflow04_sub.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=overflow04_sub.asm.exe  overflow04_sub.dasm.il
    %CORE_ROOT%\ilasm.exe /output=overflow04_sub.asm.exe  overflow04_sub.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=overflow04_sub
    if not exist "overflow04_sub.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "overflow04_sub.org" (
            mkdir IL
            copy overflow04_sub.exe IL\overflow04_sub.exe 
            ren overflow04_sub.exe overflow04_sub.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in overflow04_sub.org /out overflow04_sub.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in overflow04_sub.org /out overflow04_sub.exe
...
set ExePath=overflow04_sub.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in overflow04_sub.reflect.xml
    IF EXIST overflow04_sub.reflect.xml set ReflectionRoots=-x overflow04_sub.reflect.xml
    ...
    set ExePath=%LinkBin%\overflow04_sub.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% overflow04_sub.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% overflow04_sub.asm.exe %CLRTestExecutionArguments% %Host_Args%
