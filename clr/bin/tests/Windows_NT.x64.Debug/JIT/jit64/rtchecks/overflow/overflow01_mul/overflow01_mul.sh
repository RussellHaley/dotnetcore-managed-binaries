...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow01_mul overflow01_mul.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow01_mul overflow01_mul.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=overflow01_mul.dasm.il overflow01_mul.exe
    "$CORE_ROOT/ildasm" -raweh -out=overflow01_mul.dasm.il overflow01_mul.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=overflow01_mul.asm.exe  overflow01_mul.dasm.il
    "$CORE_ROOT/ilasm" -output=overflow01_mul.asm.exe  overflow01_mul.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=overflow01_mul
    if [ ! -f overflow01_mul.org ]; then
        TakeLock
        if [ ! -f overflow01_mul.org ]; then
          mkdir IL
          cp overflow01_mul.exe IL/overflow01_mul.exe
          mv overflow01_mul.exe overflow01_mul.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow01_mul.org /out overflow01_mul.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow01_mul.org /out overflow01_mul.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in overflow01_mul.reflect.xml
  if [ -f overflow01_mul.reflect.xml ]
then
    ReflectionRoots="-x overflow01_mul.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/overflow01_mul.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" overflow01_mul.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" overflow01_mul.asm.exe $CLRTestExecutionArguments
