...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow02_add overflow02_add.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow02_add overflow02_add.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=overflow02_add.dasm.il overflow02_add.exe
    "$CORE_ROOT/ildasm" -raweh -out=overflow02_add.dasm.il overflow02_add.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=overflow02_add.asm.exe  overflow02_add.dasm.il
    "$CORE_ROOT/ilasm" -output=overflow02_add.asm.exe  overflow02_add.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=overflow02_add
    if [ ! -f overflow02_add.org ]; then
        TakeLock
        if [ ! -f overflow02_add.org ]; then
          mkdir IL
          cp overflow02_add.exe IL/overflow02_add.exe
          mv overflow02_add.exe overflow02_add.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow02_add.org /out overflow02_add.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow02_add.org /out overflow02_add.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in overflow02_add.reflect.xml
  if [ -f overflow02_add.reflect.xml ]
then
    ReflectionRoots="-x overflow02_add.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/overflow02_add.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" overflow02_add.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" overflow02_add.asm.exe $CLRTestExecutionArguments
