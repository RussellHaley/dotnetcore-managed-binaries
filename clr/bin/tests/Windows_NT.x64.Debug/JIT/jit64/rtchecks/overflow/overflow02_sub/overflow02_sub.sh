...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow02_sub overflow02_sub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow02_sub overflow02_sub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=overflow02_sub.dasm.il overflow02_sub.exe
    "$CORE_ROOT/ildasm" -raweh -out=overflow02_sub.dasm.il overflow02_sub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=overflow02_sub.asm.exe  overflow02_sub.dasm.il
    "$CORE_ROOT/ilasm" -output=overflow02_sub.asm.exe  overflow02_sub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=overflow02_sub
    if [ ! -f overflow02_sub.org ]; then
        TakeLock
        if [ ! -f overflow02_sub.org ]; then
          mkdir IL
          cp overflow02_sub.exe IL/overflow02_sub.exe
          mv overflow02_sub.exe overflow02_sub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow02_sub.org /out overflow02_sub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow02_sub.org /out overflow02_sub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in overflow02_sub.reflect.xml
  if [ -f overflow02_sub.reflect.xml ]
then
    ReflectionRoots="-x overflow02_sub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/overflow02_sub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" overflow02_sub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" overflow02_sub.asm.exe $CLRTestExecutionArguments
