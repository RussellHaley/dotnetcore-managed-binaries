...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow04_sub overflow04_sub.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/rtchecks/overflow/overflow04_sub overflow04_sub.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=overflow04_sub.dasm.il overflow04_sub.exe
    "$CORE_ROOT/ildasm" -raweh -out=overflow04_sub.dasm.il overflow04_sub.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=overflow04_sub.asm.exe  overflow04_sub.dasm.il
    "$CORE_ROOT/ilasm" -output=overflow04_sub.asm.exe  overflow04_sub.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=overflow04_sub
    if [ ! -f overflow04_sub.org ]; then
        TakeLock
        if [ ! -f overflow04_sub.org ]; then
          mkdir IL
          cp overflow04_sub.exe IL/overflow04_sub.exe
          mv overflow04_sub.exe overflow04_sub.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow04_sub.org /out overflow04_sub.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in overflow04_sub.org /out overflow04_sub.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in overflow04_sub.reflect.xml
  if [ -f overflow04_sub.reflect.xml ]
then
    ReflectionRoots="-x overflow04_sub.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/overflow04_sub.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" overflow04_sub.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" overflow04_sub.asm.exe $CLRTestExecutionArguments
