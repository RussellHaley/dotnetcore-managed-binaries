...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\eh\basics\throwinfinallyintryfilter2 throwinfinallyintryfilter2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\eh\basics\throwinfinallyintryfilter2 throwinfinallyintryfilter2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallyintryfilter2.dasm.il throwinfinallyintryfilter2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throwinfinallyintryfilter2.dasm.il throwinfinallyintryfilter2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throwinfinallyintryfilter2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE throwinfinallyintryfilter2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throwinfinallyintryfilter2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE throwinfinallyintryfilter2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throwinfinallyintryfilter2
    if not exist "throwinfinallyintryfilter2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throwinfinallyintryfilter2.org" (
            mkdir IL
            copy throwinfinallyintryfilter2.exe IL\throwinfinallyintryfilter2.exe 
            ren throwinfinallyintryfilter2.exe throwinfinallyintryfilter2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallyintryfilter2.org /out throwinfinallyintryfilter2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throwinfinallyintryfilter2.org /out throwinfinallyintryfilter2.exe
...
set ExePath=throwinfinallyintryfilter2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throwinfinallyintryfilter2.reflect.xml
    IF EXIST throwinfinallyintryfilter2.reflect.xml set ReflectionRoots=-x throwinfinallyintryfilter2.reflect.xml
    ...
    set ExePath=%LinkBin%\throwinfinallyintryfilter2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throwinfinallyintryfilter2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throwinfinallyintryfilter2.asm.exe %CLRTestExecutionArguments% %Host_Args%
