...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/eh/basics/throwinfinallyintryfilter2 throwinfinallyintryfilter2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/eh/basics/throwinfinallyintryfilter2 throwinfinallyintryfilter2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyintryfilter2.dasm.il throwinfinallyintryfilter2.exe
    "$CORE_ROOT/ildasm" -raweh -out=throwinfinallyintryfilter2.dasm.il throwinfinallyintryfilter2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throwinfinallyintryfilter2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE throwinfinallyintryfilter2.dasm.il
    "$CORE_ROOT/ilasm" -output=throwinfinallyintryfilter2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE throwinfinallyintryfilter2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throwinfinallyintryfilter2
    if [ ! -f throwinfinallyintryfilter2.org ]; then
        TakeLock
        if [ ! -f throwinfinallyintryfilter2.org ]; then
          mkdir IL
          cp throwinfinallyintryfilter2.exe IL/throwinfinallyintryfilter2.exe
          mv throwinfinallyintryfilter2.exe throwinfinallyintryfilter2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyintryfilter2.org /out throwinfinallyintryfilter2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throwinfinallyintryfilter2.org /out throwinfinallyintryfilter2.exe
          __cgExitCode=$?
...
ExePath=throwinfinallyintryfilter2.exe
...
then
  ...
  # Use hints for reflection roots, if provided in throwinfinallyintryfilter2.reflect.xml
  if [ -f throwinfinallyintryfilter2.reflect.xml ]
then
    ReflectionRoots="-x throwinfinallyintryfilter2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/throwinfinallyintryfilter2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throwinfinallyintryfilter2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throwinfinallyintryfilter2.asm.exe $CLRTestExecutionArguments
