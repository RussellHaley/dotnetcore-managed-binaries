...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\jit64\valuetypes\nullable\castclass\castclass\castclass032 castclass032.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\..\dasm\JIT\jit64\valuetypes\nullable\castclass\castclass\castclass032 castclass032.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclass032.dasm.il castclass032.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclass032.dasm.il castclass032.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclass032.asm.exe  castclass032.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclass032.asm.exe  castclass032.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclass032
    if not exist "castclass032.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclass032.org" (
            mkdir IL
            copy castclass032.exe IL\castclass032.exe 
            ren castclass032.exe castclass032.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclass032.org /out castclass032.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclass032.org /out castclass032.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclass032.reflect.xml
    IF EXIST castclass032.reflect.xml set ReflectionRoots=-x castclass032.reflect.xml
    ...
    set ExePath=%LinkBin%\castclass032.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclass032.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclass032.asm.exe %CLRTestExecutionArguments% %Host_Args%
