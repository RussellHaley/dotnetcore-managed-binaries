...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/jit64/valuetypes/nullable/castclass/castclass/castclass032 castclass032.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../../dasm/JIT/jit64/valuetypes/nullable/castclass/castclass/castclass032 castclass032.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclass032.dasm.il castclass032.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclass032.dasm.il castclass032.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclass032.asm.exe  castclass032.dasm.il
    "$CORE_ROOT/ilasm" -output=castclass032.asm.exe  castclass032.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclass032
    if [ ! -f castclass032.org ]; then
        TakeLock
        if [ ! -f castclass032.org ]; then
          mkdir IL
          cp castclass032.exe IL/castclass032.exe
          mv castclass032.exe castclass032.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclass032.org /out castclass032.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclass032.org /out castclass032.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in castclass032.reflect.xml
  if [ -f castclass032.reflect.xml ]
then
    ReflectionRoots="-x castclass032.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/castclass032.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclass032.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclass032.asm.exe $CLRTestExecutionArguments
