...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\mcc\interop\mcc_i87 mcc_i87.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\mcc\interop\mcc_i87 mcc_i87.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mcc_i87.dasm.il mcc_i87.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mcc_i87.dasm.il mcc_i87.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mcc_i87.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mcc_i87.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mcc_i87.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mcc_i87.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mcc_i87
    if not exist "mcc_i87.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mcc_i87.org" (
            mkdir IL
            copy mcc_i87.exe IL\mcc_i87.exe 
            ren mcc_i87.exe mcc_i87.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mcc_i87.org /out mcc_i87.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mcc_i87.org /out mcc_i87.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mcc_i87.reflect.xml
    IF EXIST mcc_i87.reflect.xml set ReflectionRoots=-x mcc_i87.reflect.xml
    ...
    set ExePath=%LinkBin%\mcc_i87.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mcc_i87.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mcc_i87.asm.exe %CLRTestExecutionArguments% %Host_Args%
