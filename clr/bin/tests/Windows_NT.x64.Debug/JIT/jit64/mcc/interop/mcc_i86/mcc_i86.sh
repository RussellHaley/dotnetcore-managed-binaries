...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/mcc/interop/mcc_i86 mcc_i86.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/mcc/interop/mcc_i86 mcc_i86.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mcc_i86.dasm.il mcc_i86.exe
    "$CORE_ROOT/ildasm" -raweh -out=mcc_i86.dasm.il mcc_i86.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mcc_i86.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mcc_i86.dasm.il
    "$CORE_ROOT/ilasm" -output=mcc_i86.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mcc_i86.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mcc_i86
    if [ ! -f mcc_i86.org ]; then
        TakeLock
        if [ ! -f mcc_i86.org ]; then
          mkdir IL
          cp mcc_i86.exe IL/mcc_i86.exe
          mv mcc_i86.exe mcc_i86.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mcc_i86.org /out mcc_i86.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mcc_i86.org /out mcc_i86.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mcc_i86.reflect.xml
  if [ -f mcc_i86.reflect.xml ]
then
    ReflectionRoots="-x mcc_i86.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mcc_i86.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mcc_i86.asm.exe $CLRTestExecutionArguments
