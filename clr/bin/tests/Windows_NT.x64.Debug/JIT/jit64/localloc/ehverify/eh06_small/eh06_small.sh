...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh06_small eh06_small.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh06_small eh06_small.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh06_small.dasm.il eh06_small.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh06_small.dasm.il eh06_small.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh06_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh06_small.dasm.il
    "$CORE_ROOT/ilasm" -output=eh06_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh06_small.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh06_small
    if [ ! -f eh06_small.org ]; then
        TakeLock
        if [ ! -f eh06_small.org ]; then
          mkdir IL
          cp eh06_small.exe IL/eh06_small.exe
          mv eh06_small.exe eh06_small.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh06_small.org /out eh06_small.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh06_small.org /out eh06_small.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh06_small.reflect.xml
  if [ -f eh06_small.reflect.xml ]
then
    ReflectionRoots="-x eh06_small.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh06_small.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh06_small.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh06_small.asm.exe $CLRTestExecutionArguments
