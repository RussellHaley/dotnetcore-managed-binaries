...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\ehverify\eh07_large eh07_large.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\ehverify\eh07_large eh07_large.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=eh07_large.dasm.il eh07_large.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=eh07_large.dasm.il eh07_large.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=eh07_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh07_large.dasm.il
    %CORE_ROOT%\ilasm.exe /output=eh07_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh07_large.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=eh07_large
    if not exist "eh07_large.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "eh07_large.org" (
            mkdir IL
            copy eh07_large.exe IL\eh07_large.exe 
            ren eh07_large.exe eh07_large.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh07_large.org /out eh07_large.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh07_large.org /out eh07_large.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in eh07_large.reflect.xml
    IF EXIST eh07_large.reflect.xml set ReflectionRoots=-x eh07_large.reflect.xml
    ...
    set ExePath=%LinkBin%\eh07_large.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% eh07_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% eh07_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
