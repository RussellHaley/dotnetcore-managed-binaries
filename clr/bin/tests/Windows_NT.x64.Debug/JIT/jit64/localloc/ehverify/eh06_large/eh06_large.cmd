...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\ehverify\eh06_large eh06_large.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\ehverify\eh06_large eh06_large.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=eh06_large.dasm.il eh06_large.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=eh06_large.dasm.il eh06_large.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=eh06_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh06_large.dasm.il
    %CORE_ROOT%\ilasm.exe /output=eh06_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh06_large.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=eh06_large
    if not exist "eh06_large.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "eh06_large.org" (
            mkdir IL
            copy eh06_large.exe IL\eh06_large.exe 
            ren eh06_large.exe eh06_large.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh06_large.org /out eh06_large.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh06_large.org /out eh06_large.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in eh06_large.reflect.xml
    IF EXIST eh06_large.reflect.xml set ReflectionRoots=-x eh06_large.reflect.xml
    ...
    set ExePath=%LinkBin%\eh06_large.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% eh06_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% eh06_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
