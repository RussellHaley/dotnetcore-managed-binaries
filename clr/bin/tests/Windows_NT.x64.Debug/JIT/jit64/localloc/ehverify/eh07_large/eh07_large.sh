...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh07_large eh07_large.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh07_large eh07_large.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh07_large.dasm.il eh07_large.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh07_large.dasm.il eh07_large.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh07_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh07_large.dasm.il
    "$CORE_ROOT/ilasm" -output=eh07_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh07_large.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh07_large
    if [ ! -f eh07_large.org ]; then
        TakeLock
        if [ ! -f eh07_large.org ]; then
          mkdir IL
          cp eh07_large.exe IL/eh07_large.exe
          mv eh07_large.exe eh07_large.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh07_large.org /out eh07_large.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh07_large.org /out eh07_large.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh07_large.reflect.xml
  if [ -f eh07_large.reflect.xml ]
then
    ReflectionRoots="-x eh07_large.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh07_large.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh07_large.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh07_large.asm.exe $CLRTestExecutionArguments
