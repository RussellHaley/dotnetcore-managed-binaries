...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\ehverify\eh08_small eh08_small.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\ehverify\eh08_small eh08_small.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=eh08_small.dasm.il eh08_small.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=eh08_small.dasm.il eh08_small.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=eh08_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh08_small.dasm.il
    %CORE_ROOT%\ilasm.exe /output=eh08_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh08_small.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=eh08_small
    if not exist "eh08_small.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "eh08_small.org" (
            mkdir IL
            copy eh08_small.exe IL\eh08_small.exe 
            ren eh08_small.exe eh08_small.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh08_small.org /out eh08_small.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh08_small.org /out eh08_small.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in eh08_small.reflect.xml
    IF EXIST eh08_small.reflect.xml set ReflectionRoots=-x eh08_small.reflect.xml
    ...
    set ExePath=%LinkBin%\eh08_small.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% eh08_small.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% eh08_small.asm.exe %CLRTestExecutionArguments% %Host_Args%
