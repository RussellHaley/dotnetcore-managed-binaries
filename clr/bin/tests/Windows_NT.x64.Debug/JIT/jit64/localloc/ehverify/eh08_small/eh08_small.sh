...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh08_small eh08_small.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh08_small eh08_small.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh08_small.dasm.il eh08_small.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh08_small.dasm.il eh08_small.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh08_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh08_small.dasm.il
    "$CORE_ROOT/ilasm" -output=eh08_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh08_small.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh08_small
    if [ ! -f eh08_small.org ]; then
        TakeLock
        if [ ! -f eh08_small.org ]; then
          mkdir IL
          cp eh08_small.exe IL/eh08_small.exe
          mv eh08_small.exe eh08_small.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh08_small.org /out eh08_small.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh08_small.org /out eh08_small.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh08_small.reflect.xml
  if [ -f eh08_small.reflect.xml ]
then
    ReflectionRoots="-x eh08_small.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh08_small.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh08_small.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh08_small.asm.exe $CLRTestExecutionArguments
