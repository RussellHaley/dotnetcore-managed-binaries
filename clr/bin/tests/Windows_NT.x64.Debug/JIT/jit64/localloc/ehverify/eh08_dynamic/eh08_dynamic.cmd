...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\ehverify\eh08_dynamic eh08_dynamic.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\ehverify\eh08_dynamic eh08_dynamic.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=eh08_dynamic.dasm.il eh08_dynamic.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=eh08_dynamic.dasm.il eh08_dynamic.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=eh08_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh08_dynamic.dasm.il
    %CORE_ROOT%\ilasm.exe /output=eh08_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh08_dynamic.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=eh08_dynamic
    if not exist "eh08_dynamic.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "eh08_dynamic.org" (
            mkdir IL
            copy eh08_dynamic.exe IL\eh08_dynamic.exe 
            ren eh08_dynamic.exe eh08_dynamic.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh08_dynamic.org /out eh08_dynamic.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh08_dynamic.org /out eh08_dynamic.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in eh08_dynamic.reflect.xml
    IF EXIST eh08_dynamic.reflect.xml set ReflectionRoots=-x eh08_dynamic.reflect.xml
    ...
    set ExePath=%LinkBin%\eh08_dynamic.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% eh08_dynamic.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% eh08_dynamic.asm.exe %CLRTestExecutionArguments% %Host_Args%
