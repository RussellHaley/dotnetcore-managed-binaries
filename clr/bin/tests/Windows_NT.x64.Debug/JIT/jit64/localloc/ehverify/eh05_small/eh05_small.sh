...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh05_small eh05_small.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh05_small eh05_small.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh05_small.dasm.il eh05_small.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh05_small.dasm.il eh05_small.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh05_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh05_small.dasm.il
    "$CORE_ROOT/ilasm" -output=eh05_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh05_small.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh05_small
    if [ ! -f eh05_small.org ]; then
        TakeLock
        if [ ! -f eh05_small.org ]; then
          mkdir IL
          cp eh05_small.exe IL/eh05_small.exe
          mv eh05_small.exe eh05_small.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh05_small.org /out eh05_small.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh05_small.org /out eh05_small.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh05_small.reflect.xml
  if [ -f eh05_small.reflect.xml ]
then
    ReflectionRoots="-x eh05_small.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh05_small.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh05_small.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh05_small.asm.exe $CLRTestExecutionArguments
