...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh05_large eh05_large.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh05_large eh05_large.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh05_large.dasm.il eh05_large.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh05_large.dasm.il eh05_large.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh05_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh05_large.dasm.il
    "$CORE_ROOT/ilasm" -output=eh05_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh05_large.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh05_large
    if [ ! -f eh05_large.org ]; then
        TakeLock
        if [ ! -f eh05_large.org ]; then
          mkdir IL
          cp eh05_large.exe IL/eh05_large.exe
          mv eh05_large.exe eh05_large.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh05_large.org /out eh05_large.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh05_large.org /out eh05_large.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh05_large.reflect.xml
  if [ -f eh05_large.reflect.xml ]
then
    ReflectionRoots="-x eh05_large.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh05_large.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh05_large.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh05_large.asm.exe $CLRTestExecutionArguments
