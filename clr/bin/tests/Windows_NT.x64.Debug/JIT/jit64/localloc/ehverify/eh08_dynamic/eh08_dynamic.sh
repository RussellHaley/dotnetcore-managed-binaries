...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh08_dynamic eh08_dynamic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh08_dynamic eh08_dynamic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh08_dynamic.dasm.il eh08_dynamic.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh08_dynamic.dasm.il eh08_dynamic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh08_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh08_dynamic.dasm.il
    "$CORE_ROOT/ilasm" -output=eh08_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh08_dynamic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh08_dynamic
    if [ ! -f eh08_dynamic.org ]; then
        TakeLock
        if [ ! -f eh08_dynamic.org ]; then
          mkdir IL
          cp eh08_dynamic.exe IL/eh08_dynamic.exe
          mv eh08_dynamic.exe eh08_dynamic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh08_dynamic.org /out eh08_dynamic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh08_dynamic.org /out eh08_dynamic.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh08_dynamic.reflect.xml
  if [ -f eh08_dynamic.reflect.xml ]
then
    ReflectionRoots="-x eh08_dynamic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh08_dynamic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh08_dynamic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh08_dynamic.asm.exe $CLRTestExecutionArguments
