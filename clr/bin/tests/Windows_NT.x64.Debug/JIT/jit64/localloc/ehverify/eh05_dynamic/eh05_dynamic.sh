...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh05_dynamic eh05_dynamic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh05_dynamic eh05_dynamic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh05_dynamic.dasm.il eh05_dynamic.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh05_dynamic.dasm.il eh05_dynamic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh05_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh05_dynamic.dasm.il
    "$CORE_ROOT/ilasm" -output=eh05_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh05_dynamic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh05_dynamic
    if [ ! -f eh05_dynamic.org ]; then
        TakeLock
        if [ ! -f eh05_dynamic.org ]; then
          mkdir IL
          cp eh05_dynamic.exe IL/eh05_dynamic.exe
          mv eh05_dynamic.exe eh05_dynamic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh05_dynamic.org /out eh05_dynamic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh05_dynamic.org /out eh05_dynamic.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh05_dynamic.reflect.xml
  if [ -f eh05_dynamic.reflect.xml ]
then
    ReflectionRoots="-x eh05_dynamic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh05_dynamic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh05_dynamic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh05_dynamic.asm.exe $CLRTestExecutionArguments
