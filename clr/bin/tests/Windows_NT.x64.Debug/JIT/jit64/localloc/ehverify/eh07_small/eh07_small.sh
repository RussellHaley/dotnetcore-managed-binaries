...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh07_small eh07_small.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh07_small eh07_small.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh07_small.dasm.il eh07_small.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh07_small.dasm.il eh07_small.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh07_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh07_small.dasm.il
    "$CORE_ROOT/ilasm" -output=eh07_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh07_small.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh07_small
    if [ ! -f eh07_small.org ]; then
        TakeLock
        if [ ! -f eh07_small.org ]; then
          mkdir IL
          cp eh07_small.exe IL/eh07_small.exe
          mv eh07_small.exe eh07_small.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh07_small.org /out eh07_small.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh07_small.org /out eh07_small.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh07_small.reflect.xml
  if [ -f eh07_small.reflect.xml ]
then
    ReflectionRoots="-x eh07_small.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh07_small.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh07_small.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh07_small.asm.exe $CLRTestExecutionArguments
