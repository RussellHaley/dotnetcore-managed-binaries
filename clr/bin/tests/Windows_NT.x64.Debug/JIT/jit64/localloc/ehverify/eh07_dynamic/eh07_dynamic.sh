...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh07_dynamic eh07_dynamic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh07_dynamic eh07_dynamic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh07_dynamic.dasm.il eh07_dynamic.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh07_dynamic.dasm.il eh07_dynamic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh07_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh07_dynamic.dasm.il
    "$CORE_ROOT/ilasm" -output=eh07_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh07_dynamic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh07_dynamic
    if [ ! -f eh07_dynamic.org ]; then
        TakeLock
        if [ ! -f eh07_dynamic.org ]; then
          mkdir IL
          cp eh07_dynamic.exe IL/eh07_dynamic.exe
          mv eh07_dynamic.exe eh07_dynamic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh07_dynamic.org /out eh07_dynamic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh07_dynamic.org /out eh07_dynamic.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh07_dynamic.reflect.xml
  if [ -f eh07_dynamic.reflect.xml ]
then
    ReflectionRoots="-x eh07_dynamic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh07_dynamic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh07_dynamic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh07_dynamic.asm.exe $CLRTestExecutionArguments
