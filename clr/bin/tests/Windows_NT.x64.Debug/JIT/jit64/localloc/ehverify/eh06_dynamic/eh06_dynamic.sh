...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh06_dynamic eh06_dynamic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/ehverify/eh06_dynamic eh06_dynamic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh06_dynamic.dasm.il eh06_dynamic.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh06_dynamic.dasm.il eh06_dynamic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh06_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh06_dynamic.dasm.il
    "$CORE_ROOT/ilasm" -output=eh06_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh06_dynamic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh06_dynamic
    if [ ! -f eh06_dynamic.org ]; then
        TakeLock
        if [ ! -f eh06_dynamic.org ]; then
          mkdir IL
          cp eh06_dynamic.exe IL/eh06_dynamic.exe
          mv eh06_dynamic.exe eh06_dynamic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh06_dynamic.org /out eh06_dynamic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh06_dynamic.org /out eh06_dynamic.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh06_dynamic.reflect.xml
  if [ -f eh06_dynamic.reflect.xml ]
then
    ReflectionRoots="-x eh06_dynamic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh06_dynamic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh06_dynamic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh06_dynamic.asm.exe $CLRTestExecutionArguments
