...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/eh/eh04_small eh04_small.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/eh/eh04_small eh04_small.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh04_small.dasm.il eh04_small.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh04_small.dasm.il eh04_small.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh04_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh04_small.dasm.il
    "$CORE_ROOT/ilasm" -output=eh04_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh04_small.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh04_small
    if [ ! -f eh04_small.org ]; then
        TakeLock
        if [ ! -f eh04_small.org ]; then
          mkdir IL
          cp eh04_small.exe IL/eh04_small.exe
          mv eh04_small.exe eh04_small.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh04_small.org /out eh04_small.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh04_small.org /out eh04_small.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh04_small.reflect.xml
  if [ -f eh04_small.reflect.xml ]
then
    ReflectionRoots="-x eh04_small.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh04_small.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh04_small.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh04_small.asm.exe $CLRTestExecutionArguments
