...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/eh/eh03_dynamic eh03_dynamic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/eh/eh03_dynamic eh03_dynamic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh03_dynamic.dasm.il eh03_dynamic.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh03_dynamic.dasm.il eh03_dynamic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh03_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh03_dynamic.dasm.il
    "$CORE_ROOT/ilasm" -output=eh03_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh03_dynamic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh03_dynamic
    if [ ! -f eh03_dynamic.org ]; then
        TakeLock
        if [ ! -f eh03_dynamic.org ]; then
          mkdir IL
          cp eh03_dynamic.exe IL/eh03_dynamic.exe
          mv eh03_dynamic.exe eh03_dynamic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh03_dynamic.org /out eh03_dynamic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh03_dynamic.org /out eh03_dynamic.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh03_dynamic.reflect.xml
  if [ -f eh03_dynamic.reflect.xml ]
then
    ReflectionRoots="-x eh03_dynamic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh03_dynamic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh03_dynamic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh03_dynamic.asm.exe $CLRTestExecutionArguments
