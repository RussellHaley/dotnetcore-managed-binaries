...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/eh/eh03_large eh03_large.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/eh/eh03_large eh03_large.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=eh03_large.dasm.il eh03_large.exe
    "$CORE_ROOT/ildasm" -raweh -out=eh03_large.dasm.il eh03_large.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=eh03_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh03_large.dasm.il
    "$CORE_ROOT/ilasm" -output=eh03_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh03_large.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=eh03_large
    if [ ! -f eh03_large.org ]; then
        TakeLock
        if [ ! -f eh03_large.org ]; then
          mkdir IL
          cp eh03_large.exe IL/eh03_large.exe
          mv eh03_large.exe eh03_large.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh03_large.org /out eh03_large.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in eh03_large.org /out eh03_large.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in eh03_large.reflect.xml
  if [ -f eh03_large.reflect.xml ]
then
    ReflectionRoots="-x eh03_large.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/eh03_large.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" eh03_large.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" eh03_large.asm.exe $CLRTestExecutionArguments
