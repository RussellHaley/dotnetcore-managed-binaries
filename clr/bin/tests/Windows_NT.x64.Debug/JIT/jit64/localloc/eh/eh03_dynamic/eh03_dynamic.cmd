...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\eh\eh03_dynamic eh03_dynamic.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\eh\eh03_dynamic eh03_dynamic.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=eh03_dynamic.dasm.il eh03_dynamic.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=eh03_dynamic.dasm.il eh03_dynamic.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=eh03_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh03_dynamic.dasm.il
    %CORE_ROOT%\ilasm.exe /output=eh03_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh03_dynamic.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=eh03_dynamic
    if not exist "eh03_dynamic.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "eh03_dynamic.org" (
            mkdir IL
            copy eh03_dynamic.exe IL\eh03_dynamic.exe 
            ren eh03_dynamic.exe eh03_dynamic.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh03_dynamic.org /out eh03_dynamic.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh03_dynamic.org /out eh03_dynamic.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in eh03_dynamic.reflect.xml
    IF EXIST eh03_dynamic.reflect.xml set ReflectionRoots=-x eh03_dynamic.reflect.xml
    ...
    set ExePath=%LinkBin%\eh03_dynamic.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% eh03_dynamic.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% eh03_dynamic.asm.exe %CLRTestExecutionArguments% %Host_Args%
