...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\eh\eh04_small eh04_small.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\eh\eh04_small eh04_small.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=eh04_small.dasm.il eh04_small.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=eh04_small.dasm.il eh04_small.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=eh04_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh04_small.dasm.il
    %CORE_ROOT%\ilasm.exe /output=eh04_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE eh04_small.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=eh04_small
    if not exist "eh04_small.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "eh04_small.org" (
            mkdir IL
            copy eh04_small.exe IL\eh04_small.exe 
            ren eh04_small.exe eh04_small.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh04_small.org /out eh04_small.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in eh04_small.org /out eh04_small.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in eh04_small.reflect.xml
    IF EXIST eh04_small.reflect.xml set ReflectionRoots=-x eh04_small.reflect.xml
    ...
    set ExePath=%LinkBin%\eh04_small.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% eh04_small.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% eh04_small.asm.exe %CLRTestExecutionArguments% %Host_Args%
