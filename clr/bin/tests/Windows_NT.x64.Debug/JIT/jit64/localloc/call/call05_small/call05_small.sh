...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call05_small call05_small.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call05_small call05_small.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=call05_small.dasm.il call05_small.exe
    "$CORE_ROOT/ildasm" -raweh -out=call05_small.dasm.il call05_small.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=call05_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call05_small.dasm.il
    "$CORE_ROOT/ilasm" -output=call05_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call05_small.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=call05_small
    if [ ! -f call05_small.org ]; then
        TakeLock
        if [ ! -f call05_small.org ]; then
          mkdir IL
          cp call05_small.exe IL/call05_small.exe
          mv call05_small.exe call05_small.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call05_small.org /out call05_small.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call05_small.org /out call05_small.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in call05_small.reflect.xml
  if [ -f call05_small.reflect.xml ]
then
    ReflectionRoots="-x call05_small.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/call05_small.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" call05_small.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" call05_small.asm.exe $CLRTestExecutionArguments
