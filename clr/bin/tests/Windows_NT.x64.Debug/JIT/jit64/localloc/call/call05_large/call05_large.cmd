...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\call\call05_large call05_large.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\call\call05_large call05_large.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=call05_large.dasm.il call05_large.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=call05_large.dasm.il call05_large.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=call05_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call05_large.dasm.il
    %CORE_ROOT%\ilasm.exe /output=call05_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call05_large.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=call05_large
    if not exist "call05_large.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "call05_large.org" (
            mkdir IL
            copy call05_large.exe IL\call05_large.exe 
            ren call05_large.exe call05_large.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in call05_large.org /out call05_large.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in call05_large.org /out call05_large.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in call05_large.reflect.xml
    IF EXIST call05_large.reflect.xml set ReflectionRoots=-x call05_large.reflect.xml
    ...
    set ExePath=%LinkBin%\call05_large.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% call05_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% call05_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
