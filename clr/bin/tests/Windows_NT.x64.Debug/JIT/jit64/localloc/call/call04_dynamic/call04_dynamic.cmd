...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\call\call04_dynamic call04_dynamic.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\call\call04_dynamic call04_dynamic.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=call04_dynamic.dasm.il call04_dynamic.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=call04_dynamic.dasm.il call04_dynamic.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=call04_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_dynamic.dasm.il
    %CORE_ROOT%\ilasm.exe /output=call04_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_dynamic.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=call04_dynamic
    if not exist "call04_dynamic.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "call04_dynamic.org" (
            mkdir IL
            copy call04_dynamic.exe IL\call04_dynamic.exe 
            ren call04_dynamic.exe call04_dynamic.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in call04_dynamic.org /out call04_dynamic.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in call04_dynamic.org /out call04_dynamic.exe
...
set ExePath=call04_dynamic.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in call04_dynamic.reflect.xml
    IF EXIST call04_dynamic.reflect.xml set ReflectionRoots=-x call04_dynamic.reflect.xml
    ...
    set ExePath=%LinkBin%\call04_dynamic.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% call04_dynamic.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% call04_dynamic.asm.exe %CLRTestExecutionArguments% %Host_Args%
