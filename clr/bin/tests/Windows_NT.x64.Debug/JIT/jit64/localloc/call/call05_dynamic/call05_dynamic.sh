...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call05_dynamic call05_dynamic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call05_dynamic call05_dynamic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=call05_dynamic.dasm.il call05_dynamic.exe
    "$CORE_ROOT/ildasm" -raweh -out=call05_dynamic.dasm.il call05_dynamic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=call05_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call05_dynamic.dasm.il
    "$CORE_ROOT/ilasm" -output=call05_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call05_dynamic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=call05_dynamic
    if [ ! -f call05_dynamic.org ]; then
        TakeLock
        if [ ! -f call05_dynamic.org ]; then
          mkdir IL
          cp call05_dynamic.exe IL/call05_dynamic.exe
          mv call05_dynamic.exe call05_dynamic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call05_dynamic.org /out call05_dynamic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call05_dynamic.org /out call05_dynamic.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in call05_dynamic.reflect.xml
  if [ -f call05_dynamic.reflect.xml ]
then
    ReflectionRoots="-x call05_dynamic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/call05_dynamic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" call05_dynamic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" call05_dynamic.asm.exe $CLRTestExecutionArguments
