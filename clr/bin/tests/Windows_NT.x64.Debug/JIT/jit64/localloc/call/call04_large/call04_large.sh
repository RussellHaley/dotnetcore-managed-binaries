...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call04_large call04_large.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call04_large call04_large.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=call04_large.dasm.il call04_large.exe
    "$CORE_ROOT/ildasm" -raweh -out=call04_large.dasm.il call04_large.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=call04_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_large.dasm.il
    "$CORE_ROOT/ilasm" -output=call04_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_large.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=call04_large
    if [ ! -f call04_large.org ]; then
        TakeLock
        if [ ! -f call04_large.org ]; then
          mkdir IL
          cp call04_large.exe IL/call04_large.exe
          mv call04_large.exe call04_large.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call04_large.org /out call04_large.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call04_large.org /out call04_large.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in call04_large.reflect.xml
  if [ -f call04_large.reflect.xml ]
then
    ReflectionRoots="-x call04_large.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/call04_large.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" call04_large.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" call04_large.asm.exe $CLRTestExecutionArguments
