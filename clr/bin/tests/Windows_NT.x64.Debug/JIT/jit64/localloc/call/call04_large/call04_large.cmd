...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\call\call04_large call04_large.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\call\call04_large call04_large.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=call04_large.dasm.il call04_large.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=call04_large.dasm.il call04_large.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=call04_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_large.dasm.il
    %CORE_ROOT%\ilasm.exe /output=call04_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_large.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=call04_large
    if not exist "call04_large.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "call04_large.org" (
            mkdir IL
            copy call04_large.exe IL\call04_large.exe 
            ren call04_large.exe call04_large.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in call04_large.org /out call04_large.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in call04_large.org /out call04_large.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in call04_large.reflect.xml
    IF EXIST call04_large.reflect.xml set ReflectionRoots=-x call04_large.reflect.xml
    ...
    set ExePath=%LinkBin%\call04_large.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% call04_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% call04_large.asm.exe %CLRTestExecutionArguments% %Host_Args%
