...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\call\call04_small call04_small.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\jit64\localloc\call\call04_small call04_small.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=call04_small.dasm.il call04_small.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=call04_small.dasm.il call04_small.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=call04_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_small.dasm.il
    %CORE_ROOT%\ilasm.exe /output=call04_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_small.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=call04_small
    if not exist "call04_small.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "call04_small.org" (
            mkdir IL
            copy call04_small.exe IL\call04_small.exe 
            ren call04_small.exe call04_small.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in call04_small.org /out call04_small.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in call04_small.org /out call04_small.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in call04_small.reflect.xml
    IF EXIST call04_small.reflect.xml set ReflectionRoots=-x call04_small.reflect.xml
    ...
    set ExePath=%LinkBin%\call04_small.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% call04_small.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% call04_small.asm.exe %CLRTestExecutionArguments% %Host_Args%
