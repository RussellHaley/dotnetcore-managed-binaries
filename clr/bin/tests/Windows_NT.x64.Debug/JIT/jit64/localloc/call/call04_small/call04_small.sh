...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call04_small call04_small.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call04_small call04_small.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=call04_small.dasm.il call04_small.exe
    "$CORE_ROOT/ildasm" -raweh -out=call04_small.dasm.il call04_small.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=call04_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_small.dasm.il
    "$CORE_ROOT/ilasm" -output=call04_small.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_small.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=call04_small
    if [ ! -f call04_small.org ]; then
        TakeLock
        if [ ! -f call04_small.org ]; then
          mkdir IL
          cp call04_small.exe IL/call04_small.exe
          mv call04_small.exe call04_small.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call04_small.org /out call04_small.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call04_small.org /out call04_small.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in call04_small.reflect.xml
  if [ -f call04_small.reflect.xml ]
then
    ReflectionRoots="-x call04_small.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/call04_small.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" call04_small.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" call04_small.asm.exe $CLRTestExecutionArguments
