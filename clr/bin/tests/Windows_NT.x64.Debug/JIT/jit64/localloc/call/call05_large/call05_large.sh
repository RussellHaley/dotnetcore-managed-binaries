...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call05_large call05_large.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call05_large call05_large.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=call05_large.dasm.il call05_large.exe
    "$CORE_ROOT/ildasm" -raweh -out=call05_large.dasm.il call05_large.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=call05_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call05_large.dasm.il
    "$CORE_ROOT/ilasm" -output=call05_large.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call05_large.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=call05_large
    if [ ! -f call05_large.org ]; then
        TakeLock
        if [ ! -f call05_large.org ]; then
          mkdir IL
          cp call05_large.exe IL/call05_large.exe
          mv call05_large.exe call05_large.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call05_large.org /out call05_large.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call05_large.org /out call05_large.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in call05_large.reflect.xml
  if [ -f call05_large.reflect.xml ]
then
    ReflectionRoots="-x call05_large.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/call05_large.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" call05_large.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" call05_large.asm.exe $CLRTestExecutionArguments
