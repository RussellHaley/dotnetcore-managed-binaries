...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call04_dynamic call04_dynamic.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/jit64/localloc/call/call04_dynamic call04_dynamic.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=call04_dynamic.dasm.il call04_dynamic.exe
    "$CORE_ROOT/ildasm" -raweh -out=call04_dynamic.dasm.il call04_dynamic.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=call04_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_dynamic.dasm.il
    "$CORE_ROOT/ilasm" -output=call04_dynamic.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE call04_dynamic.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=call04_dynamic
    if [ ! -f call04_dynamic.org ]; then
        TakeLock
        if [ ! -f call04_dynamic.org ]; then
          mkdir IL
          cp call04_dynamic.exe IL/call04_dynamic.exe
          mv call04_dynamic.exe call04_dynamic.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call04_dynamic.org /out call04_dynamic.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in call04_dynamic.org /out call04_dynamic.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in call04_dynamic.reflect.xml
  if [ -f call04_dynamic.reflect.xml ]
then
    ReflectionRoots="-x call04_dynamic.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/call04_dynamic.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" call04_dynamic.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" call04_dynamic.asm.exe $CLRTestExecutionArguments
