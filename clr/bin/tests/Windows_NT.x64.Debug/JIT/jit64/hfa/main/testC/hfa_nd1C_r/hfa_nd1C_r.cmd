...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\hfa\main\testC\hfa_nd1C_r hfa_nd1C_r.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\hfa\main\testC\hfa_nd1C_r hfa_nd1C_r.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=hfa_nd1C_r.dasm.il hfa_nd1C_r.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=hfa_nd1C_r.dasm.il hfa_nd1C_r.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=hfa_nd1C_r.asm.exe  hfa_nd1C_r.dasm.il
    %CORE_ROOT%\ilasm.exe /output=hfa_nd1C_r.asm.exe  hfa_nd1C_r.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=hfa_nd1C_r
    if not exist "hfa_nd1C_r.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "hfa_nd1C_r.org" (
            mkdir IL
            copy hfa_nd1C_r.exe IL\hfa_nd1C_r.exe 
            ren hfa_nd1C_r.exe hfa_nd1C_r.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hfa_nd1C_r.org /out hfa_nd1C_r.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in hfa_nd1C_r.org /out hfa_nd1C_r.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in hfa_nd1C_r.reflect.xml
    IF EXIST hfa_nd1C_r.reflect.xml set ReflectionRoots=-x hfa_nd1C_r.reflect.xml
    ...
    set ExePath=%LinkBin%\hfa_nd1C_r.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% hfa_nd1C_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% hfa_nd1C_r.asm.exe %CLRTestExecutionArguments% %Host_Args%
