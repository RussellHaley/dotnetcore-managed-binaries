...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/hfa/main/testC/hfa_nd1C_r hfa_nd1C_r.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/hfa/main/testC/hfa_nd1C_r hfa_nd1C_r.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=hfa_nd1C_r.dasm.il hfa_nd1C_r.exe
    "$CORE_ROOT/ildasm" -raweh -out=hfa_nd1C_r.dasm.il hfa_nd1C_r.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=hfa_nd1C_r.asm.exe  hfa_nd1C_r.dasm.il
    "$CORE_ROOT/ilasm" -output=hfa_nd1C_r.asm.exe  hfa_nd1C_r.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=hfa_nd1C_r
    if [ ! -f hfa_nd1C_r.org ]; then
        TakeLock
        if [ ! -f hfa_nd1C_r.org ]; then
          mkdir IL
          cp hfa_nd1C_r.exe IL/hfa_nd1C_r.exe
          mv hfa_nd1C_r.exe hfa_nd1C_r.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hfa_nd1C_r.org /out hfa_nd1C_r.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in hfa_nd1C_r.org /out hfa_nd1C_r.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in hfa_nd1C_r.reflect.xml
  if [ -f hfa_nd1C_r.reflect.xml ]
then
    ReflectionRoots="-x hfa_nd1C_r.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/hfa_nd1C_r.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" hfa_nd1C_r.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" hfa_nd1C_r.asm.exe $CLRTestExecutionArguments
