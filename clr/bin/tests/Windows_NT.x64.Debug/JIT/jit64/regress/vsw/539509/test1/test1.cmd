...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\vsw\539509\test1 test1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\vsw\539509\test1 test1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=test1.dasm.il test1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=test1.dasm.il test1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=test1.asm.exe  test1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=test1.asm.exe  test1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=test1
    if not exist "test1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "test1.org" (
            mkdir IL
            copy test1.exe IL\test1.exe 
            ren test1.exe test1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test1.org /out test1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test1.org /out test1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in test1.reflect.xml
    IF EXIST test1.reflect.xml set ReflectionRoots=-x test1.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% test1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% test1.asm.exe %CLRTestExecutionArguments% %Host_Args%
