...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/vsw/373472/test_il test_il.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/vsw/373472/test_il test_il.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=test_il.dasm.il test_il.exe
    "$CORE_ROOT/ildasm" -raweh -out=test_il.dasm.il test_il.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=test_il.asm.exe -QUIET -NOLOGO -DEBUG test_il.dasm.il
    "$CORE_ROOT/ilasm" -output=test_il.asm.exe -QUIET -NOLOGO -DEBUG test_il.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=test_il
    if [ ! -f test_il.org ]; then
        TakeLock
        if [ ! -f test_il.org ]; then
          mkdir IL
          cp test_il.exe IL/test_il.exe
          mv test_il.exe test_il.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test_il.org /out test_il.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test_il.org /out test_il.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in test_il.reflect.xml
  if [ -f test_il.reflect.xml ]
then
    ReflectionRoots="-x test_il.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" test_il.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" test_il.asm.exe $CLRTestExecutionArguments
