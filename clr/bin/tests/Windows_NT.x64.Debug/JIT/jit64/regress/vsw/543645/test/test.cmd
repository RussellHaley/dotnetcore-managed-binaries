...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\vsw\543645\test test.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\vsw\543645\test test.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=test.dasm.il test.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=test.dasm.il test.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=test.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE test.dasm.il
    %CORE_ROOT%\ilasm.exe /output=test.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE test.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=test
    if not exist "test.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "test.org" (
            mkdir IL
            copy test.exe IL\test.exe 
            ren test.exe test.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test.org /out test.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test.org /out test.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in test.reflect.xml
    IF EXIST test.reflect.xml set ReflectionRoots=-x test.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% test.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% test.asm.exe %CLRTestExecutionArguments% %Host_Args%
