...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\vsw\373472\test_il test_il.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\vsw\373472\test_il test_il.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=test_il.dasm.il test_il.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=test_il.dasm.il test_il.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=test_il.asm.exe -QUIET -NOLOGO -DEBUG test_il.dasm.il
    %CORE_ROOT%\ilasm.exe /output=test_il.asm.exe -QUIET -NOLOGO -DEBUG test_il.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=test_il
    if not exist "test_il.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "test_il.org" (
            mkdir IL
            copy test_il.exe IL\test_il.exe 
            ren test_il.exe test_il.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test_il.org /out test_il.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in test_il.org /out test_il.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in test_il.reflect.xml
    IF EXIST test_il.reflect.xml set ReflectionRoots=-x test_il.reflect.xml
    ...
    set ExePath=%LinkBin%\test_il.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% test_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% test_il.asm.exe %CLRTestExecutionArguments% %Host_Args%
