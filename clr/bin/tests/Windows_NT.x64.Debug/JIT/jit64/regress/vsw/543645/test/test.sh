...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/vsw/543645/test test.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/vsw/543645/test test.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=test.dasm.il test.exe
    "$CORE_ROOT/ildasm" -raweh -out=test.dasm.il test.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=test.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE test.dasm.il
    "$CORE_ROOT/ilasm" -output=test.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE test.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=test
    if [ ! -f test.org ]; then
        TakeLock
        if [ ! -f test.org ]; then
          mkdir IL
          cp test.exe IL/test.exe
          mv test.exe test.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test.org /out test.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test.org /out test.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in test.reflect.xml
  if [ -f test.reflect.xml ]
then
    ReflectionRoots="-x test.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" test.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" test.asm.exe $CLRTestExecutionArguments
