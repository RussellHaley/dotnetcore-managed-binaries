...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\vsw\373472\test test.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\vsw\373472\test test.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    ...
    ECHO %CORE_ROOT%\ilasm.exe /output=test.asm.exe  test.dasm.il
    %CORE_ROOT%\ilasm.exe /output=test.asm.exe  test.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
