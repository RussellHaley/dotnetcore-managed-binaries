...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/vsw/539509/test1 test1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/vsw/539509/test1 test1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=test1.dasm.il test1.exe
    "$CORE_ROOT/ildasm" -raweh -out=test1.dasm.il test1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=test1.asm.exe  test1.dasm.il
    "$CORE_ROOT/ilasm" -output=test1.asm.exe  test1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=test1
    if [ ! -f test1.org ]; then
        TakeLock
        if [ ! -f test1.org ]; then
          mkdir IL
          cp test1.exe IL/test1.exe
          mv test1.exe test1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test1.org /out test1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in test1.org /out test1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in test1.reflect.xml
  if [ -f test1.reflect.xml ]
then
    ReflectionRoots="-x test1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" test1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" test1.asm.exe $CLRTestExecutionArguments
