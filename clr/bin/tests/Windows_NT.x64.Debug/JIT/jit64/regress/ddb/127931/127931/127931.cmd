...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\ddb\127931\127931 127931.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\..\dasm\JIT\jit64\regress\ddb\127931\127931 127931.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=127931.dasm.il 127931.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=127931.dasm.il 127931.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=127931.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE 127931.dasm.il
    %CORE_ROOT%\ilasm.exe /output=127931.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE 127931.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=127931
    if not exist "127931.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "127931.org" (
            mkdir IL
            copy 127931.exe IL\127931.exe 
            ren 127931.exe 127931.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 127931.org /out 127931.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 127931.org /out 127931.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 127931.reflect.xml
    IF EXIST 127931.reflect.xml set ReflectionRoots=-x 127931.reflect.xml
    ...
    set ExePath=%LinkBin%\127931.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 127931.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 127931.asm.exe %CLRTestExecutionArguments% %Host_Args%
