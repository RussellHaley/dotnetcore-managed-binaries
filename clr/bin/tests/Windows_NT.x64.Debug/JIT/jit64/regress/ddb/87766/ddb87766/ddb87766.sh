...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/ddb/87766/ddb87766 ddb87766.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/ddb/87766/ddb87766 ddb87766.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ddb87766.dasm.il ddb87766.exe
    "$CORE_ROOT/ildasm" -raweh -out=ddb87766.dasm.il ddb87766.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ddb87766.asm.exe  ddb87766.dasm.il
    "$CORE_ROOT/ilasm" -output=ddb87766.asm.exe  ddb87766.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ddb87766
    if [ ! -f ddb87766.org ]; then
        TakeLock
        if [ ! -f ddb87766.org ]; then
          mkdir IL
          cp ddb87766.exe IL/ddb87766.exe
          mv ddb87766.exe ddb87766.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ddb87766.org /out ddb87766.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ddb87766.org /out ddb87766.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ddb87766.reflect.xml
  if [ -f ddb87766.reflect.xml ]
then
    ReflectionRoots="-x ddb87766.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ddb87766.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ddb87766.asm.exe $CLRTestExecutionArguments
