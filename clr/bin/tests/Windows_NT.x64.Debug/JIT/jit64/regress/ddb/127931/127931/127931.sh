...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/ddb/127931/127931 127931.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../../dasm/JIT/jit64/regress/ddb/127931/127931 127931.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=127931.dasm.il 127931.exe
    "$CORE_ROOT/ildasm" -raweh -out=127931.dasm.il 127931.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=127931.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE 127931.dasm.il
    "$CORE_ROOT/ilasm" -output=127931.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE 127931.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=127931
    if [ ! -f 127931.org ]; then
        TakeLock
        if [ ! -f 127931.org ]; then
          mkdir IL
          cp 127931.exe IL/127931.exe
          mv 127931.exe 127931.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 127931.org /out 127931.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 127931.org /out 127931.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 127931.reflect.xml
  if [ -f 127931.reflect.xml ]
then
    ReflectionRoots="-x 127931.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 127931.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 127931.asm.exe $CLRTestExecutionArguments
