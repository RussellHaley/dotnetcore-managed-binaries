...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\mul_ovf mul_ovf.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\mul_ovf mul_ovf.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mul_ovf.dasm.il mul_ovf.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mul_ovf.dasm.il mul_ovf.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mul_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mul_ovf.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mul_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mul_ovf.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mul_ovf
    if not exist "mul_ovf.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mul_ovf.org" (
            mkdir IL
            copy mul_ovf.exe IL\mul_ovf.exe 
            ren mul_ovf.exe mul_ovf.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mul_ovf.org /out mul_ovf.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mul_ovf.org /out mul_ovf.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mul_ovf.reflect.xml
    IF EXIST mul_ovf.reflect.xml set ReflectionRoots=-x mul_ovf.reflect.xml
    ...
    set ExePath=%LinkBin%\mul_ovf.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mul_ovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mul_ovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
