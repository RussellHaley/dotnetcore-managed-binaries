...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\ckfinite ckfinite.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\ckfinite ckfinite.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ckfinite.dasm.il ckfinite.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ckfinite.dasm.il ckfinite.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ckfinite.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ckfinite.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ckfinite
    if not exist "ckfinite.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ckfinite.org" (
            mkdir IL
            copy ckfinite.exe IL\ckfinite.exe 
            ren ckfinite.exe ckfinite.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ckfinite.org /out ckfinite.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ckfinite.org /out ckfinite.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ckfinite.reflect.xml
    IF EXIST ckfinite.reflect.xml set ReflectionRoots=-x ckfinite.reflect.xml
    ...
    set ExePath=%LinkBin%\ckfinite.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ckfinite.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ckfinite.asm.exe %CLRTestExecutionArguments% %Host_Args%
