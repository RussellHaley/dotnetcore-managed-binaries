...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\tailcall tailcall.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\tailcall tailcall.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=tailcall.dasm.il tailcall.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=tailcall.dasm.il tailcall.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=tailcall.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall.dasm.il
    %CORE_ROOT%\ilasm.exe /output=tailcall.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=tailcall
    if not exist "tailcall.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "tailcall.org" (
            mkdir IL
            copy tailcall.exe IL\tailcall.exe 
            ren tailcall.exe tailcall.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tailcall.org /out tailcall.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in tailcall.org /out tailcall.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in tailcall.reflect.xml
    IF EXIST tailcall.reflect.xml set ReflectionRoots=-x tailcall.reflect.xml
    ...
    set ExePath=%LinkBin%\tailcall.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% tailcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% tailcall.asm.exe %CLRTestExecutionArguments% %Host_Args%
