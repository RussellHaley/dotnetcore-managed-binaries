...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\add_ovf add_ovf.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\add_ovf add_ovf.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=add_ovf.dasm.il add_ovf.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=add_ovf.dasm.il add_ovf.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=add_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE add_ovf.dasm.il
    %CORE_ROOT%\ilasm.exe /output=add_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE add_ovf.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=add_ovf
    if not exist "add_ovf.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "add_ovf.org" (
            mkdir IL
            copy add_ovf.exe IL\add_ovf.exe 
            ren add_ovf.exe add_ovf.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in add_ovf.org /out add_ovf.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in add_ovf.org /out add_ovf.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in add_ovf.reflect.xml
    IF EXIST add_ovf.reflect.xml set ReflectionRoots=-x add_ovf.reflect.xml
    ...
    set ExePath=%LinkBin%\add_ovf.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% add_ovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% add_ovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
