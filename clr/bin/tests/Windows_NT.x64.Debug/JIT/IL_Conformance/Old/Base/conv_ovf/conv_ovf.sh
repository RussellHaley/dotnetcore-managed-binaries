...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/conv_ovf conv_ovf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/conv_ovf conv_ovf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=conv_ovf.dasm.il conv_ovf.exe
    "$CORE_ROOT/ildasm" -raweh -out=conv_ovf.dasm.il conv_ovf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=conv_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf.dasm.il
    "$CORE_ROOT/ilasm" -output=conv_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=conv_ovf
    if [ ! -f conv_ovf.org ]; then
        TakeLock
        if [ ! -f conv_ovf.org ]; then
          mkdir IL
          cp conv_ovf.exe IL/conv_ovf.exe
          mv conv_ovf.exe conv_ovf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in conv_ovf.org /out conv_ovf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in conv_ovf.org /out conv_ovf.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in conv_ovf.reflect.xml
  if [ -f conv_ovf.reflect.xml ]
then
    ReflectionRoots="-x conv_ovf.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" conv_ovf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" conv_ovf.asm.exe $CLRTestExecutionArguments
