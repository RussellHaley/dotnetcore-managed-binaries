...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/mul_ovf mul_ovf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/mul_ovf mul_ovf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mul_ovf.dasm.il mul_ovf.exe
    "$CORE_ROOT/ildasm" -raweh -out=mul_ovf.dasm.il mul_ovf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mul_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mul_ovf.dasm.il
    "$CORE_ROOT/ilasm" -output=mul_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mul_ovf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mul_ovf
    if [ ! -f mul_ovf.org ]; then
        TakeLock
        if [ ! -f mul_ovf.org ]; then
          mkdir IL
          cp mul_ovf.exe IL/mul_ovf.exe
          mv mul_ovf.exe mul_ovf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mul_ovf.org /out mul_ovf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mul_ovf.org /out mul_ovf.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mul_ovf.reflect.xml
  if [ -f mul_ovf.reflect.xml ]
then
    ReflectionRoots="-x mul_ovf.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mul_ovf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mul_ovf.asm.exe $CLRTestExecutionArguments
