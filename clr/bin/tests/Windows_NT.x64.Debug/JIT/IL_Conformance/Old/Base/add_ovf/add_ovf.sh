...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/add_ovf add_ovf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/add_ovf add_ovf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=add_ovf.dasm.il add_ovf.exe
    "$CORE_ROOT/ildasm" -raweh -out=add_ovf.dasm.il add_ovf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=add_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE add_ovf.dasm.il
    "$CORE_ROOT/ilasm" -output=add_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE add_ovf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=add_ovf
    if [ ! -f add_ovf.org ]; then
        TakeLock
        if [ ! -f add_ovf.org ]; then
          mkdir IL
          cp add_ovf.exe IL/add_ovf.exe
          mv add_ovf.exe add_ovf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in add_ovf.org /out add_ovf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in add_ovf.org /out add_ovf.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in add_ovf.reflect.xml
  if [ -f add_ovf.reflect.xml ]
then
    ReflectionRoots="-x add_ovf.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" add_ovf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" add_ovf.asm.exe $CLRTestExecutionArguments
