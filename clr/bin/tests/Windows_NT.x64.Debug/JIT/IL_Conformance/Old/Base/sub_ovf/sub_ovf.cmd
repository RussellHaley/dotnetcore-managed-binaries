...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\sub_ovf sub_ovf.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\sub_ovf sub_ovf.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=sub_ovf.dasm.il sub_ovf.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=sub_ovf.dasm.il sub_ovf.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=sub_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE sub_ovf.dasm.il
    %CORE_ROOT%\ilasm.exe /output=sub_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE sub_ovf.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=sub_ovf
    if not exist "sub_ovf.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "sub_ovf.org" (
            mkdir IL
            copy sub_ovf.exe IL\sub_ovf.exe 
            ren sub_ovf.exe sub_ovf.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in sub_ovf.org /out sub_ovf.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in sub_ovf.org /out sub_ovf.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in sub_ovf.reflect.xml
    IF EXIST sub_ovf.reflect.xml set ReflectionRoots=-x sub_ovf.reflect.xml
    ...
    set ExePath=%LinkBin%\sub_ovf.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% sub_ovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% sub_ovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
