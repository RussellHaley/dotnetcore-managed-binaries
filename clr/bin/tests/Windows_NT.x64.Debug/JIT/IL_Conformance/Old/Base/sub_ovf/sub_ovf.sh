...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/sub_ovf sub_ovf.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/sub_ovf sub_ovf.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=sub_ovf.dasm.il sub_ovf.exe
    "$CORE_ROOT/ildasm" -raweh -out=sub_ovf.dasm.il sub_ovf.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=sub_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE sub_ovf.dasm.il
    "$CORE_ROOT/ilasm" -output=sub_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE sub_ovf.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=sub_ovf
    if [ ! -f sub_ovf.org ]; then
        TakeLock
        if [ ! -f sub_ovf.org ]; then
          mkdir IL
          cp sub_ovf.exe IL/sub_ovf.exe
          mv sub_ovf.exe sub_ovf.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in sub_ovf.org /out sub_ovf.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in sub_ovf.org /out sub_ovf.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in sub_ovf.reflect.xml
  if [ -f sub_ovf.reflect.xml ]
then
    ReflectionRoots="-x sub_ovf.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" sub_ovf.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" sub_ovf.asm.exe $CLRTestExecutionArguments
