...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\conv_ovf conv_ovf.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Base\conv_ovf conv_ovf.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=conv_ovf.dasm.il conv_ovf.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=conv_ovf.dasm.il conv_ovf.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=conv_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf.dasm.il
    %CORE_ROOT%\ilasm.exe /output=conv_ovf.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=conv_ovf
    if not exist "conv_ovf.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "conv_ovf.org" (
            mkdir IL
            copy conv_ovf.exe IL\conv_ovf.exe 
            ren conv_ovf.exe conv_ovf.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_ovf.org /out conv_ovf.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_ovf.org /out conv_ovf.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in conv_ovf.reflect.xml
    IF EXIST conv_ovf.reflect.xml set ReflectionRoots=-x conv_ovf.reflect.xml
    ...
    set ExePath=%LinkBin%\conv_ovf.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% conv_ovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% conv_ovf.asm.exe %CLRTestExecutionArguments% %Host_Args%
