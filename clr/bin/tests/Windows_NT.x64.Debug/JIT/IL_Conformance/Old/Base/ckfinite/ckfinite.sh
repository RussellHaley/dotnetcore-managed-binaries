...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/ckfinite ckfinite.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/ckfinite ckfinite.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ckfinite.dasm.il ckfinite.exe
    "$CORE_ROOT/ildasm" -raweh -out=ckfinite.dasm.il ckfinite.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ckfinite.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite.dasm.il
    "$CORE_ROOT/ilasm" -output=ckfinite.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ckfinite
    if [ ! -f ckfinite.org ]; then
        TakeLock
        if [ ! -f ckfinite.org ]; then
          mkdir IL
          cp ckfinite.exe IL/ckfinite.exe
          mv ckfinite.exe ckfinite.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ckfinite.org /out ckfinite.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ckfinite.org /out ckfinite.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ckfinite.reflect.xml
  if [ -f ckfinite.reflect.xml ]
then
    ReflectionRoots="-x ckfinite.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ckfinite.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ckfinite.asm.exe $CLRTestExecutionArguments
