...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/tailcall tailcall.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Base/tailcall tailcall.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=tailcall.dasm.il tailcall.exe
    "$CORE_ROOT/ildasm" -raweh -out=tailcall.dasm.il tailcall.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=tailcall.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall.dasm.il
    "$CORE_ROOT/ilasm" -output=tailcall.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE tailcall.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=tailcall
    if [ ! -f tailcall.org ]; then
        TakeLock
        if [ ! -f tailcall.org ]; then
          mkdir IL
          cp tailcall.exe IL/tailcall.exe
          mv tailcall.exe tailcall.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tailcall.org /out tailcall.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in tailcall.org /out tailcall.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in tailcall.reflect.xml
  if [ -f tailcall.reflect.xml ]
then
    ReflectionRoots="-x tailcall.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" tailcall.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" tailcall.asm.exe $CLRTestExecutionArguments
