...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\conv_ovf_r8_i8 conv_ovf_r8_i8.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\conv_ovf_r8_i8 conv_ovf_r8_i8.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=conv_ovf_r8_i8.dasm.il conv_ovf_r8_i8.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=conv_ovf_r8_i8.dasm.il conv_ovf_r8_i8.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=conv_ovf_r8_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf_r8_i8.dasm.il
    %CORE_ROOT%\ilasm.exe /output=conv_ovf_r8_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf_r8_i8.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=conv_ovf_r8_i8
    if not exist "conv_ovf_r8_i8.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "conv_ovf_r8_i8.org" (
            mkdir IL
            copy conv_ovf_r8_i8.exe IL\conv_ovf_r8_i8.exe 
            ren conv_ovf_r8_i8.exe conv_ovf_r8_i8.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_ovf_r8_i8.org /out conv_ovf_r8_i8.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_ovf_r8_i8.org /out conv_ovf_r8_i8.exe
...
set ExePath=conv_ovf_r8_i8.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in conv_ovf_r8_i8.reflect.xml
    IF EXIST conv_ovf_r8_i8.reflect.xml set ReflectionRoots=-x conv_ovf_r8_i8.reflect.xml
    ...
    set ExePath=%LinkBin%\conv_ovf_r8_i8.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% conv_ovf_r8_i8.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% conv_ovf_r8_i8.asm.exe %CLRTestExecutionArguments% %Host_Args%
