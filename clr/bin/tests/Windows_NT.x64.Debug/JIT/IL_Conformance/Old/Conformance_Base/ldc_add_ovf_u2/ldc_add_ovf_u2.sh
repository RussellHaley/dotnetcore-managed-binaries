...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldc_add_ovf_u2 ldc_add_ovf_u2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldc_add_ovf_u2 ldc_add_ovf_u2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ldc_add_ovf_u2.dasm.il ldc_add_ovf_u2.exe
    "$CORE_ROOT/ildasm" -raweh -out=ldc_add_ovf_u2.dasm.il ldc_add_ovf_u2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ldc_add_ovf_u2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldc_add_ovf_u2.dasm.il
    "$CORE_ROOT/ilasm" -output=ldc_add_ovf_u2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldc_add_ovf_u2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ldc_add_ovf_u2
    if [ ! -f ldc_add_ovf_u2.org ]; then
        TakeLock
        if [ ! -f ldc_add_ovf_u2.org ]; then
          mkdir IL
          cp ldc_add_ovf_u2.exe IL/ldc_add_ovf_u2.exe
          mv ldc_add_ovf_u2.exe ldc_add_ovf_u2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldc_add_ovf_u2.org /out ldc_add_ovf_u2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldc_add_ovf_u2.org /out ldc_add_ovf_u2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ldc_add_ovf_u2.reflect.xml
  if [ -f ldc_add_ovf_u2.reflect.xml ]
then
    ReflectionRoots="-x ldc_add_ovf_u2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ldc_add_ovf_u2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ldc_add_ovf_u2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ldc_add_ovf_u2.asm.exe $CLRTestExecutionArguments
