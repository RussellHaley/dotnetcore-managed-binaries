...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/conv_ovf_i4_i2 conv_ovf_i4_i2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/conv_ovf_i4_i2 conv_ovf_i4_i2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=conv_ovf_i4_i2.dasm.il conv_ovf_i4_i2.exe
    "$CORE_ROOT/ildasm" -raweh -out=conv_ovf_i4_i2.dasm.il conv_ovf_i4_i2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=conv_ovf_i4_i2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf_i4_i2.dasm.il
    "$CORE_ROOT/ilasm" -output=conv_ovf_i4_i2.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf_i4_i2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=conv_ovf_i4_i2
    if [ ! -f conv_ovf_i4_i2.org ]; then
        TakeLock
        if [ ! -f conv_ovf_i4_i2.org ]; then
          mkdir IL
          cp conv_ovf_i4_i2.exe IL/conv_ovf_i4_i2.exe
          mv conv_ovf_i4_i2.exe conv_ovf_i4_i2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in conv_ovf_i4_i2.org /out conv_ovf_i4_i2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in conv_ovf_i4_i2.org /out conv_ovf_i4_i2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in conv_ovf_i4_i2.reflect.xml
  if [ -f conv_ovf_i4_i2.reflect.xml ]
then
    ReflectionRoots="-x conv_ovf_i4_i2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/conv_ovf_i4_i2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" conv_ovf_i4_i2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" conv_ovf_i4_i2.asm.exe $CLRTestExecutionArguments
