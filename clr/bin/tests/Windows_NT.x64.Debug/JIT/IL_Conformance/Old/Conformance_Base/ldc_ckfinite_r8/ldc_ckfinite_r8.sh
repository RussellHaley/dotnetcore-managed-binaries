...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldc_ckfinite_r8 ldc_ckfinite_r8.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldc_ckfinite_r8 ldc_ckfinite_r8.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ldc_ckfinite_r8.dasm.il ldc_ckfinite_r8.exe
    "$CORE_ROOT/ildasm" -raweh -out=ldc_ckfinite_r8.dasm.il ldc_ckfinite_r8.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ldc_ckfinite_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldc_ckfinite_r8.dasm.il
    "$CORE_ROOT/ilasm" -output=ldc_ckfinite_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldc_ckfinite_r8.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ldc_ckfinite_r8
    if [ ! -f ldc_ckfinite_r8.org ]; then
        TakeLock
        if [ ! -f ldc_ckfinite_r8.org ]; then
          mkdir IL
          cp ldc_ckfinite_r8.exe IL/ldc_ckfinite_r8.exe
          mv ldc_ckfinite_r8.exe ldc_ckfinite_r8.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldc_ckfinite_r8.org /out ldc_ckfinite_r8.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldc_ckfinite_r8.org /out ldc_ckfinite_r8.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ldc_ckfinite_r8.reflect.xml
  if [ -f ldc_ckfinite_r8.reflect.xml ]
then
    ReflectionRoots="-x ldc_ckfinite_r8.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ldc_ckfinite_r8.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ldc_ckfinite_r8.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ldc_ckfinite_r8.asm.exe $CLRTestExecutionArguments
