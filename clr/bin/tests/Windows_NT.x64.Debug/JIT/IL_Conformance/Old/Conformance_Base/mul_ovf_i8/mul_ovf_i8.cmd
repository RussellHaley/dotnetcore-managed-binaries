...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\mul_ovf_i8 mul_ovf_i8.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\mul_ovf_i8 mul_ovf_i8.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mul_ovf_i8.dasm.il mul_ovf_i8.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mul_ovf_i8.dasm.il mul_ovf_i8.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mul_ovf_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mul_ovf_i8.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mul_ovf_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE mul_ovf_i8.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mul_ovf_i8
    if not exist "mul_ovf_i8.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mul_ovf_i8.org" (
            mkdir IL
            copy mul_ovf_i8.exe IL\mul_ovf_i8.exe 
            ren mul_ovf_i8.exe mul_ovf_i8.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mul_ovf_i8.org /out mul_ovf_i8.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mul_ovf_i8.org /out mul_ovf_i8.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mul_ovf_i8.reflect.xml
    IF EXIST mul_ovf_i8.reflect.xml set ReflectionRoots=-x mul_ovf_i8.reflect.xml
    ...
    set ExePath=%LinkBin%\mul_ovf_i8.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mul_ovf_i8.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mul_ovf_i8.asm.exe %CLRTestExecutionArguments% %Host_Args%
