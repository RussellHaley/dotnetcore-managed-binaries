...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=neg_r8.dasm.il neg_r8.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=neg_r8.dasm.il neg_r8.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=neg_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE neg_r8.dasm.il
    %CORE_ROOT%\ilasm.exe /output=neg_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE neg_r8.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=neg_r8
    if not exist "neg_r8.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "neg_r8.org" (
            mkdir IL
            copy neg_r8.exe IL\neg_r8.exe 
            ren neg_r8.exe neg_r8.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in neg_r8.org /out neg_r8.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in neg_r8.org /out neg_r8.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in neg_r8.reflect.xml
    IF EXIST neg_r8.reflect.xml set ReflectionRoots=-x neg_r8.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% neg_r8.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% neg_r8.asm.exe %CLRTestExecutionArguments% %Host_Args%
