...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ckfinite_r4 ckfinite_r4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ckfinite_r4 ckfinite_r4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ckfinite_r4.dasm.il ckfinite_r4.exe
    "$CORE_ROOT/ildasm" -raweh -out=ckfinite_r4.dasm.il ckfinite_r4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ckfinite_r4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite_r4.dasm.il
    "$CORE_ROOT/ilasm" -output=ckfinite_r4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite_r4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ckfinite_r4
    if [ ! -f ckfinite_r4.org ]; then
        TakeLock
        if [ ! -f ckfinite_r4.org ]; then
          mkdir IL
          cp ckfinite_r4.exe IL/ckfinite_r4.exe
          mv ckfinite_r4.exe ckfinite_r4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ckfinite_r4.org /out ckfinite_r4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ckfinite_r4.org /out ckfinite_r4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ckfinite_r4.reflect.xml
  if [ -f ckfinite_r4.reflect.xml ]
then
    ReflectionRoots="-x ckfinite_r4.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ckfinite_r4.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ckfinite_r4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ckfinite_r4.asm.exe $CLRTestExecutionArguments
