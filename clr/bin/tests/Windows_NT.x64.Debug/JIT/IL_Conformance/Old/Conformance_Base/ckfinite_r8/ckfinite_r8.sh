...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ckfinite_r8 ckfinite_r8.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ckfinite_r8 ckfinite_r8.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ckfinite_r8.dasm.il ckfinite_r8.exe
    "$CORE_ROOT/ildasm" -raweh -out=ckfinite_r8.dasm.il ckfinite_r8.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ckfinite_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite_r8.dasm.il
    "$CORE_ROOT/ilasm" -output=ckfinite_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite_r8.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ckfinite_r8
    if [ ! -f ckfinite_r8.org ]; then
        TakeLock
        if [ ! -f ckfinite_r8.org ]; then
          mkdir IL
          cp ckfinite_r8.exe IL/ckfinite_r8.exe
          mv ckfinite_r8.exe ckfinite_r8.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ckfinite_r8.org /out ckfinite_r8.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ckfinite_r8.org /out ckfinite_r8.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ckfinite_r8.reflect.xml
  if [ -f ckfinite_r8.reflect.xml ]
then
    ReflectionRoots="-x ckfinite_r8.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ckfinite_r8.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ckfinite_r8.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ckfinite_r8.asm.exe $CLRTestExecutionArguments
