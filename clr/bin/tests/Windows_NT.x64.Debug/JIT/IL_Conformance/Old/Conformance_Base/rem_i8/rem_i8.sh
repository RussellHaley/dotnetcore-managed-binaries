...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/rem_i8 rem_i8.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/rem_i8 rem_i8.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rem_i8.dasm.il rem_i8.exe
    "$CORE_ROOT/ildasm" -raweh -out=rem_i8.dasm.il rem_i8.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rem_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rem_i8.dasm.il
    "$CORE_ROOT/ilasm" -output=rem_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rem_i8.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rem_i8
    if [ ! -f rem_i8.org ]; then
        TakeLock
        if [ ! -f rem_i8.org ]; then
          mkdir IL
          cp rem_i8.exe IL/rem_i8.exe
          mv rem_i8.exe rem_i8.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rem_i8.org /out rem_i8.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rem_i8.org /out rem_i8.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in rem_i8.reflect.xml
  if [ -f rem_i8.reflect.xml ]
then
    ReflectionRoots="-x rem_i8.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rem_i8.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rem_i8.asm.exe $CLRTestExecutionArguments
