...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\ckfinite_r8 ckfinite_r8.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\ckfinite_r8 ckfinite_r8.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ckfinite_r8.dasm.il ckfinite_r8.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ckfinite_r8.dasm.il ckfinite_r8.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ckfinite_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite_r8.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ckfinite_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ckfinite_r8.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ckfinite_r8
    if not exist "ckfinite_r8.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ckfinite_r8.org" (
            mkdir IL
            copy ckfinite_r8.exe IL\ckfinite_r8.exe 
            ren ckfinite_r8.exe ckfinite_r8.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ckfinite_r8.org /out ckfinite_r8.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ckfinite_r8.org /out ckfinite_r8.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ckfinite_r8.reflect.xml
    IF EXIST ckfinite_r8.reflect.xml set ReflectionRoots=-x ckfinite_r8.reflect.xml
    ...
    set ExePath=%LinkBin%\ckfinite_r8.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ckfinite_r8.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ckfinite_r8.asm.exe %CLRTestExecutionArguments% %Host_Args%
