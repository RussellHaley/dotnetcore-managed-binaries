...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\conv_ovf_r8_i conv_ovf_r8_i.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\conv_ovf_r8_i conv_ovf_r8_i.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=conv_ovf_r8_i.dasm.il conv_ovf_r8_i.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=conv_ovf_r8_i.dasm.il conv_ovf_r8_i.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=conv_ovf_r8_i.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf_r8_i.dasm.il
    %CORE_ROOT%\ilasm.exe /output=conv_ovf_r8_i.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE conv_ovf_r8_i.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=conv_ovf_r8_i
    if not exist "conv_ovf_r8_i.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "conv_ovf_r8_i.org" (
            mkdir IL
            copy conv_ovf_r8_i.exe IL\conv_ovf_r8_i.exe 
            ren conv_ovf_r8_i.exe conv_ovf_r8_i.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_ovf_r8_i.org /out conv_ovf_r8_i.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in conv_ovf_r8_i.org /out conv_ovf_r8_i.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in conv_ovf_r8_i.reflect.xml
    IF EXIST conv_ovf_r8_i.reflect.xml set ReflectionRoots=-x conv_ovf_r8_i.reflect.xml
    ...
    set ExePath=%LinkBin%\conv_ovf_r8_i.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% conv_ovf_r8_i.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% conv_ovf_r8_i.asm.exe %CLRTestExecutionArguments% %Host_Args%
