...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/div_i8 div_i8.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/div_i8 div_i8.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=div_i8.dasm.il div_i8.exe
    "$CORE_ROOT/ildasm" -raweh -out=div_i8.dasm.il div_i8.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=div_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE div_i8.dasm.il
    "$CORE_ROOT/ilasm" -output=div_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE div_i8.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=div_i8
    if [ ! -f div_i8.org ]; then
        TakeLock
        if [ ! -f div_i8.org ]; then
          mkdir IL
          cp div_i8.exe IL/div_i8.exe
          mv div_i8.exe div_i8.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in div_i8.org /out div_i8.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in div_i8.org /out div_i8.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in div_i8.reflect.xml
  if [ -f div_i8.reflect.xml ]
then
    ReflectionRoots="-x div_i8.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" div_i8.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" div_i8.asm.exe $CLRTestExecutionArguments
