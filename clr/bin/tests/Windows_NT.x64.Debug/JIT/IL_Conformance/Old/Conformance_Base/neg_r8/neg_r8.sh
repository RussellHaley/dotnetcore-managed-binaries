...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/neg_r8 neg_r8.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/neg_r8 neg_r8.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=neg_r8.dasm.il neg_r8.exe
    "$CORE_ROOT/ildasm" -raweh -out=neg_r8.dasm.il neg_r8.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=neg_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE neg_r8.dasm.il
    "$CORE_ROOT/ilasm" -output=neg_r8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE neg_r8.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=neg_r8
    if [ ! -f neg_r8.org ]; then
        TakeLock
        if [ ! -f neg_r8.org ]; then
          mkdir IL
          cp neg_r8.exe IL/neg_r8.exe
          mv neg_r8.exe neg_r8.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in neg_r8.org /out neg_r8.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in neg_r8.org /out neg_r8.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in neg_r8.reflect.xml
  if [ -f neg_r8.reflect.xml ]
then
    ReflectionRoots="-x neg_r8.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" neg_r8.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" neg_r8.asm.exe $CLRTestExecutionArguments
