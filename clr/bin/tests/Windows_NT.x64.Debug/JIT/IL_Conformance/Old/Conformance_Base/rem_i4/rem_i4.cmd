...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\rem_i4 rem_i4.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\rem_i4 rem_i4.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=rem_i4.dasm.il rem_i4.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=rem_i4.dasm.il rem_i4.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=rem_i4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rem_i4.dasm.il
    %CORE_ROOT%\ilasm.exe /output=rem_i4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rem_i4.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=rem_i4
    if not exist "rem_i4.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "rem_i4.org" (
            mkdir IL
            copy rem_i4.exe IL\rem_i4.exe 
            ren rem_i4.exe rem_i4.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rem_i4.org /out rem_i4.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rem_i4.org /out rem_i4.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in rem_i4.reflect.xml
    IF EXIST rem_i4.reflect.xml set ReflectionRoots=-x rem_i4.reflect.xml
    ...
    set ExePath=%LinkBin%\rem_i4.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% rem_i4.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% rem_i4.asm.exe %CLRTestExecutionArguments% %Host_Args%
