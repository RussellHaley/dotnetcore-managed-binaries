...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\ldind_i ldind_i.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\ldind_i ldind_i.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ldind_i.dasm.il ldind_i.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ldind_i.dasm.il ldind_i.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ldind_i.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldind_i.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ldind_i.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldind_i.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ldind_i
    if not exist "ldind_i.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ldind_i.org" (
            mkdir IL
            copy ldind_i.exe IL\ldind_i.exe 
            ren ldind_i.exe ldind_i.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ldind_i.org /out ldind_i.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ldind_i.org /out ldind_i.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ldind_i.reflect.xml
    IF EXIST ldind_i.reflect.xml set ReflectionRoots=-x ldind_i.reflect.xml
    ...
    set ExePath=%LinkBin%\ldind_i.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ldind_i.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ldind_i.asm.exe %CLRTestExecutionArguments% %Host_Args%
