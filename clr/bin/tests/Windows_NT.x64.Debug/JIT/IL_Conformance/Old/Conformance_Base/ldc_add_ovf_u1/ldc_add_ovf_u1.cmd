...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\ldc_add_ovf_u1 ldc_add_ovf_u1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\ldc_add_ovf_u1 ldc_add_ovf_u1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ldc_add_ovf_u1.dasm.il ldc_add_ovf_u1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ldc_add_ovf_u1.dasm.il ldc_add_ovf_u1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ldc_add_ovf_u1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldc_add_ovf_u1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ldc_add_ovf_u1.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldc_add_ovf_u1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ldc_add_ovf_u1
    if not exist "ldc_add_ovf_u1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ldc_add_ovf_u1.org" (
            mkdir IL
            copy ldc_add_ovf_u1.exe IL\ldc_add_ovf_u1.exe 
            ren ldc_add_ovf_u1.exe ldc_add_ovf_u1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ldc_add_ovf_u1.org /out ldc_add_ovf_u1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ldc_add_ovf_u1.org /out ldc_add_ovf_u1.exe
...
set ExePath=ldc_add_ovf_u1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ldc_add_ovf_u1.reflect.xml
    IF EXIST ldc_add_ovf_u1.reflect.xml set ReflectionRoots=-x ldc_add_ovf_u1.reflect.xml
    ...
    set ExePath=%LinkBin%\ldc_add_ovf_u1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ldc_add_ovf_u1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ldc_add_ovf_u1.asm.exe %CLRTestExecutionArguments% %Host_Args%
