...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/rem_u4 rem_u4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/rem_u4 rem_u4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rem_u4.dasm.il rem_u4.exe
    "$CORE_ROOT/ildasm" -raweh -out=rem_u4.dasm.il rem_u4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rem_u4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rem_u4.dasm.il
    "$CORE_ROOT/ilasm" -output=rem_u4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE rem_u4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rem_u4
    if [ ! -f rem_u4.org ]; then
        TakeLock
        if [ ! -f rem_u4.org ]; then
          mkdir IL
          cp rem_u4.exe IL/rem_u4.exe
          mv rem_u4.exe rem_u4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rem_u4.org /out rem_u4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rem_u4.org /out rem_u4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in rem_u4.reflect.xml
  if [ -f rem_u4.reflect.xml ]
then
    ReflectionRoots="-x rem_u4.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rem_u4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rem_u4.asm.exe $CLRTestExecutionArguments
