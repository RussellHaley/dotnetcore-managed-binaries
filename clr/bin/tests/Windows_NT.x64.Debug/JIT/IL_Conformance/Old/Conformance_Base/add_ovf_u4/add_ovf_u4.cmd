...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\add_ovf_u4 add_ovf_u4.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\add_ovf_u4 add_ovf_u4.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=add_ovf_u4.dasm.il add_ovf_u4.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=add_ovf_u4.dasm.il add_ovf_u4.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=add_ovf_u4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE add_ovf_u4.dasm.il
    %CORE_ROOT%\ilasm.exe /output=add_ovf_u4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE add_ovf_u4.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=add_ovf_u4
    if not exist "add_ovf_u4.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "add_ovf_u4.org" (
            mkdir IL
            copy add_ovf_u4.exe IL\add_ovf_u4.exe 
            ren add_ovf_u4.exe add_ovf_u4.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in add_ovf_u4.org /out add_ovf_u4.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in add_ovf_u4.org /out add_ovf_u4.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in add_ovf_u4.reflect.xml
    IF EXIST add_ovf_u4.reflect.xml set ReflectionRoots=-x add_ovf_u4.reflect.xml
    ...
    set ExePath=%LinkBin%\add_ovf_u4.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% add_ovf_u4.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% add_ovf_u4.asm.exe %CLRTestExecutionArguments% %Host_Args%
