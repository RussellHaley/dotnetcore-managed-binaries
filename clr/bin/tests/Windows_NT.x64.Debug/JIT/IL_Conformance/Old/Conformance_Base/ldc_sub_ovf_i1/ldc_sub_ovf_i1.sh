...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldc_sub_ovf_i1 ldc_sub_ovf_i1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldc_sub_ovf_i1 ldc_sub_ovf_i1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ldc_sub_ovf_i1.dasm.il ldc_sub_ovf_i1.exe
    "$CORE_ROOT/ildasm" -raweh -out=ldc_sub_ovf_i1.dasm.il ldc_sub_ovf_i1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ldc_sub_ovf_i1.asm.exe -QUIET -NOLOGO -DEBUG ldc_sub_ovf_i1.dasm.il
    "$CORE_ROOT/ilasm" -output=ldc_sub_ovf_i1.asm.exe -QUIET -NOLOGO -DEBUG ldc_sub_ovf_i1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ldc_sub_ovf_i1
    if [ ! -f ldc_sub_ovf_i1.org ]; then
        TakeLock
        if [ ! -f ldc_sub_ovf_i1.org ]; then
          mkdir IL
          cp ldc_sub_ovf_i1.exe IL/ldc_sub_ovf_i1.exe
          mv ldc_sub_ovf_i1.exe ldc_sub_ovf_i1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldc_sub_ovf_i1.org /out ldc_sub_ovf_i1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldc_sub_ovf_i1.org /out ldc_sub_ovf_i1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ldc_sub_ovf_i1.reflect.xml
  if [ -f ldc_sub_ovf_i1.reflect.xml ]
then
    ReflectionRoots="-x ldc_sub_ovf_i1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ldc_sub_ovf_i1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ldc_sub_ovf_i1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ldc_sub_ovf_i1.asm.exe $CLRTestExecutionArguments
