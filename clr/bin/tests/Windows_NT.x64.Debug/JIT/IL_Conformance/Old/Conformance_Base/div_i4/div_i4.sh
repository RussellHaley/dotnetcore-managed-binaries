...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/div_i4 div_i4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/div_i4 div_i4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=div_i4.dasm.il div_i4.exe
    "$CORE_ROOT/ildasm" -raweh -out=div_i4.dasm.il div_i4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=div_i4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE div_i4.dasm.il
    "$CORE_ROOT/ilasm" -output=div_i4.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE div_i4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=div_i4
    if [ ! -f div_i4.org ]; then
        TakeLock
        if [ ! -f div_i4.org ]; then
          mkdir IL
          cp div_i4.exe IL/div_i4.exe
          mv div_i4.exe div_i4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in div_i4.org /out div_i4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in div_i4.org /out div_i4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in div_i4.reflect.xml
  if [ -f div_i4.reflect.xml ]
then
    ReflectionRoots="-x div_i4.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" div_i4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" div_i4.asm.exe $CLRTestExecutionArguments
