...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldind_i ldind_i.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldind_i ldind_i.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ldind_i.dasm.il ldind_i.exe
    "$CORE_ROOT/ildasm" -raweh -out=ldind_i.dasm.il ldind_i.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ldind_i.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldind_i.dasm.il
    "$CORE_ROOT/ilasm" -output=ldind_i.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE ldind_i.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ldind_i
    if [ ! -f ldind_i.org ]; then
        TakeLock
        if [ ! -f ldind_i.org ]; then
          mkdir IL
          cp ldind_i.exe IL/ldind_i.exe
          mv ldind_i.exe ldind_i.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldind_i.org /out ldind_i.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldind_i.org /out ldind_i.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ldind_i.reflect.xml
  if [ -f ldind_i.reflect.xml ]
then
    ReflectionRoots="-x ldind_i.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ldind_i.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ldind_i.asm.exe $CLRTestExecutionArguments
