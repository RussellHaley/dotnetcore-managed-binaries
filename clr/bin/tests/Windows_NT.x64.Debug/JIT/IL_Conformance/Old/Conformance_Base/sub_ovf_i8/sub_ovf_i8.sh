...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/sub_ovf_i8 sub_ovf_i8.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/sub_ovf_i8 sub_ovf_i8.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=sub_ovf_i8.dasm.il sub_ovf_i8.exe
    "$CORE_ROOT/ildasm" -raweh -out=sub_ovf_i8.dasm.il sub_ovf_i8.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=sub_ovf_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE sub_ovf_i8.dasm.il
    "$CORE_ROOT/ilasm" -output=sub_ovf_i8.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE sub_ovf_i8.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=sub_ovf_i8
    if [ ! -f sub_ovf_i8.org ]; then
        TakeLock
        if [ ! -f sub_ovf_i8.org ]; then
          mkdir IL
          cp sub_ovf_i8.exe IL/sub_ovf_i8.exe
          mv sub_ovf_i8.exe sub_ovf_i8.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in sub_ovf_i8.org /out sub_ovf_i8.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in sub_ovf_i8.org /out sub_ovf_i8.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in sub_ovf_i8.reflect.xml
  if [ -f sub_ovf_i8.reflect.xml ]
then
    ReflectionRoots="-x sub_ovf_i8.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/sub_ovf_i8.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" sub_ovf_i8.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" sub_ovf_i8.asm.exe $CLRTestExecutionArguments
