...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\ldc_conv_ovf_i8_u8 ldc_conv_ovf_i8_u8.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\Conformance_Base\ldc_conv_ovf_i8_u8 ldc_conv_ovf_i8_u8.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ldc_conv_ovf_i8_u8.dasm.il ldc_conv_ovf_i8_u8.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ldc_conv_ovf_i8_u8.dasm.il ldc_conv_ovf_i8_u8.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ldc_conv_ovf_i8_u8.asm.exe -QUIET -NOLOGO -DEBUG ldc_conv_ovf_i8_u8.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ldc_conv_ovf_i8_u8.asm.exe -QUIET -NOLOGO -DEBUG ldc_conv_ovf_i8_u8.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ldc_conv_ovf_i8_u8
    if not exist "ldc_conv_ovf_i8_u8.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ldc_conv_ovf_i8_u8.org" (
            mkdir IL
            copy ldc_conv_ovf_i8_u8.exe IL\ldc_conv_ovf_i8_u8.exe 
            ren ldc_conv_ovf_i8_u8.exe ldc_conv_ovf_i8_u8.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ldc_conv_ovf_i8_u8.org /out ldc_conv_ovf_i8_u8.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ldc_conv_ovf_i8_u8.org /out ldc_conv_ovf_i8_u8.exe
...
set ExePath=ldc_conv_ovf_i8_u8.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ldc_conv_ovf_i8_u8.reflect.xml
    IF EXIST ldc_conv_ovf_i8_u8.reflect.xml set ReflectionRoots=-x ldc_conv_ovf_i8_u8.reflect.xml
    ...
    set ExePath=%LinkBin%\ldc_conv_ovf_i8_u8.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ldc_conv_ovf_i8_u8.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ldc_conv_ovf_i8_u8.asm.exe %CLRTestExecutionArguments% %Host_Args%
