...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldc_conv_ovf_r8_i4 ldc_conv_ovf_r8_i4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/Conformance_Base/ldc_conv_ovf_r8_i4 ldc_conv_ovf_r8_i4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ldc_conv_ovf_r8_i4.dasm.il ldc_conv_ovf_r8_i4.exe
    "$CORE_ROOT/ildasm" -raweh -out=ldc_conv_ovf_r8_i4.dasm.il ldc_conv_ovf_r8_i4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ldc_conv_ovf_r8_i4.asm.exe -QUIET -NOLOGO -DEBUG ldc_conv_ovf_r8_i4.dasm.il
    "$CORE_ROOT/ilasm" -output=ldc_conv_ovf_r8_i4.asm.exe -QUIET -NOLOGO -DEBUG ldc_conv_ovf_r8_i4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ldc_conv_ovf_r8_i4
    if [ ! -f ldc_conv_ovf_r8_i4.org ]; then
        TakeLock
        if [ ! -f ldc_conv_ovf_r8_i4.org ]; then
          mkdir IL
          cp ldc_conv_ovf_r8_i4.exe IL/ldc_conv_ovf_r8_i4.exe
          mv ldc_conv_ovf_r8_i4.exe ldc_conv_ovf_r8_i4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldc_conv_ovf_r8_i4.org /out ldc_conv_ovf_r8_i4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ldc_conv_ovf_r8_i4.org /out ldc_conv_ovf_r8_i4.exe
          __cgExitCode=$?
...
ExePath=ldc_conv_ovf_r8_i4.exe
...
then
  ...
  # Use hints for reflection roots, if provided in ldc_conv_ovf_r8_i4.reflect.xml
  if [ -f ldc_conv_ovf_r8_i4.reflect.xml ]
then
    ReflectionRoots="-x ldc_conv_ovf_r8_i4.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ldc_conv_ovf_r8_i4.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ldc_conv_ovf_r8_i4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ldc_conv_ovf_r8_i4.asm.exe $CLRTestExecutionArguments
