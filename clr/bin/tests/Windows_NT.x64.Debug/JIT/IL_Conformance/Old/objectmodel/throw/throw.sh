...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/objectmodel/throw throw.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/objectmodel/throw throw.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=throw.dasm.il throw.exe
    "$CORE_ROOT/ildasm" -raweh -out=throw.dasm.il throw.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=throw.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE throw.dasm.il
    "$CORE_ROOT/ilasm" -output=throw.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE throw.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=throw
    if [ ! -f throw.org ]; then
        TakeLock
        if [ ! -f throw.org ]; then
          mkdir IL
          cp throw.exe IL/throw.exe
          mv throw.exe throw.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw.org /out throw.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in throw.org /out throw.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in throw.reflect.xml
  if [ -f throw.reflect.xml ]
then
    ReflectionRoots="-x throw.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" throw.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" throw.asm.exe $CLRTestExecutionArguments
