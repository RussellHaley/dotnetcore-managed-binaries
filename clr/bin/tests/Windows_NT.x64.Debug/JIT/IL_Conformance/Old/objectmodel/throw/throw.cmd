...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\objectmodel\throw throw.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\objectmodel\throw throw.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=throw.dasm.il throw.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=throw.dasm.il throw.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=throw.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE throw.dasm.il
    %CORE_ROOT%\ilasm.exe /output=throw.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE throw.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=throw
    if not exist "throw.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "throw.org" (
            mkdir IL
            copy throw.exe IL\throw.exe 
            ren throw.exe throw.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw.org /out throw.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in throw.org /out throw.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in throw.reflect.xml
    IF EXIST throw.reflect.xml set ReflectionRoots=-x throw.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% throw.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% throw.asm.exe %CLRTestExecutionArguments% %Host_Args%
