...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\objectmodel\castclass castclass.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\JIT\IL_Conformance\Old\objectmodel\castclass castclass.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=castclass.dasm.il castclass.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=castclass.dasm.il castclass.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=castclass.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE castclass.dasm.il
    %CORE_ROOT%\ilasm.exe /output=castclass.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE castclass.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=castclass
    if not exist "castclass.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "castclass.org" (
            mkdir IL
            copy castclass.exe IL\castclass.exe 
            ren castclass.exe castclass.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclass.org /out castclass.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in castclass.org /out castclass.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in castclass.reflect.xml
    IF EXIST castclass.reflect.xml set ReflectionRoots=-x castclass.reflect.xml
    ...
    set ExePath=%LinkBin%\castclass.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% castclass.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% castclass.asm.exe %CLRTestExecutionArguments% %Host_Args%
