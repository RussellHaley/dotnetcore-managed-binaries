...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/objectmodel/castclass castclass.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/JIT/IL_Conformance/Old/objectmodel/castclass castclass.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=castclass.dasm.il castclass.exe
    "$CORE_ROOT/ildasm" -raweh -out=castclass.dasm.il castclass.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=castclass.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE castclass.dasm.il
    "$CORE_ROOT/ilasm" -output=castclass.asm.exe -QUIET -NOLOGO -DEBUG=OPT -OPTIMIZE castclass.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=castclass
    if [ ! -f castclass.org ]; then
        TakeLock
        if [ ! -f castclass.org ]; then
          mkdir IL
          cp castclass.exe IL/castclass.exe
          mv castclass.exe castclass.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclass.org /out castclass.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in castclass.org /out castclass.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in castclass.reflect.xml
  if [ -f castclass.reflect.xml ]
then
    ReflectionRoots="-x castclass.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" castclass.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" castclass.asm.exe $CLRTestExecutionArguments
