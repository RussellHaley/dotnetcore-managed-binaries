...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Exceptions/ForeignThread/ForeignThreadExceptions ForeignThreadExceptions.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Exceptions/ForeignThread/ForeignThreadExceptions ForeignThreadExceptions.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ForeignThreadExceptions.dasm.il ForeignThreadExceptions.exe
    "$CORE_ROOT/ildasm" -raweh -out=ForeignThreadExceptions.dasm.il ForeignThreadExceptions.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ForeignThreadExceptions.asm.exe  ForeignThreadExceptions.dasm.il
    "$CORE_ROOT/ilasm" -output=ForeignThreadExceptions.asm.exe  ForeignThreadExceptions.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ForeignThreadExceptions
    if [ ! -f ForeignThreadExceptions.org ]; then
        TakeLock
        if [ ! -f ForeignThreadExceptions.org ]; then
          mkdir IL
          cp ForeignThreadExceptions.exe IL/ForeignThreadExceptions.exe
          mv ForeignThreadExceptions.exe ForeignThreadExceptions.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ForeignThreadExceptions.org /out ForeignThreadExceptions.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ForeignThreadExceptions.org /out ForeignThreadExceptions.exe
          __cgExitCode=$?
...
ExePath=ForeignThreadExceptions.exe
...
then
  ...
  # Use hints for reflection roots, if provided in ForeignThreadExceptions.reflect.xml
  if [ -f ForeignThreadExceptions.reflect.xml ]
then
    ReflectionRoots="-x ForeignThreadExceptions.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ForeignThreadExceptions.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ForeignThreadExceptions.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ForeignThreadExceptions.asm.exe $CLRTestExecutionArguments
