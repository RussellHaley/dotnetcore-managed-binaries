...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Exceptions\ForeignThread\ForeignThreadExceptions ForeignThreadExceptions.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Exceptions\ForeignThread\ForeignThreadExceptions ForeignThreadExceptions.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ForeignThreadExceptions.dasm.il ForeignThreadExceptions.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ForeignThreadExceptions.dasm.il ForeignThreadExceptions.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ForeignThreadExceptions.asm.exe  ForeignThreadExceptions.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ForeignThreadExceptions.asm.exe  ForeignThreadExceptions.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ForeignThreadExceptions
    if not exist "ForeignThreadExceptions.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ForeignThreadExceptions.org" (
            mkdir IL
            copy ForeignThreadExceptions.exe IL\ForeignThreadExceptions.exe 
            ren ForeignThreadExceptions.exe ForeignThreadExceptions.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ForeignThreadExceptions.org /out ForeignThreadExceptions.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ForeignThreadExceptions.org /out ForeignThreadExceptions.exe
...
set ExePath=ForeignThreadExceptions.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ForeignThreadExceptions.reflect.xml
    IF EXIST ForeignThreadExceptions.reflect.xml set ReflectionRoots=-x ForeignThreadExceptions.reflect.xml
    ...
    set ExePath=%LinkBin%\ForeignThreadExceptions.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ForeignThreadExceptions.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ForeignThreadExceptions.asm.exe %CLRTestExecutionArguments% %Host_Args%
