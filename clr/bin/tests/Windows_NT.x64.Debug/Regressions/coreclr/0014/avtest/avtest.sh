...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Regressions/coreclr/0014/avtest avtest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/Regressions/coreclr/0014/avtest avtest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=avtest.dasm.il avtest.exe
    "$CORE_ROOT/ildasm" -raweh -out=avtest.dasm.il avtest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=avtest.asm.exe  avtest.dasm.il
    "$CORE_ROOT/ilasm" -output=avtest.asm.exe  avtest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=avtest
    if [ ! -f avtest.org ]; then
        TakeLock
        if [ ! -f avtest.org ]; then
          mkdir IL
          cp avtest.exe IL/avtest.exe
          mv avtest.exe avtest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in avtest.org /out avtest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in avtest.org /out avtest.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in avtest.reflect.xml
  if [ -f avtest.reflect.xml ]
then
    ReflectionRoots="-x avtest.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" avtest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" avtest.asm.exe $CLRTestExecutionArguments
