...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Regressions\coreclr\0014\avtest avtest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\Regressions\coreclr\0014\avtest avtest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=avtest.dasm.il avtest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=avtest.dasm.il avtest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=avtest.asm.exe  avtest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=avtest.asm.exe  avtest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=avtest
    if not exist "avtest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "avtest.org" (
            mkdir IL
            copy avtest.exe IL\avtest.exe 
            ren avtest.exe avtest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in avtest.org /out avtest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in avtest.org /out avtest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in avtest.reflect.xml
    IF EXIST avtest.reflect.xml set ReflectionRoots=-x avtest.reflect.xml
    ...
    set ExePath=%LinkBin%\avtest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% avtest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% avtest.asm.exe %CLRTestExecutionArguments% %Host_Args%
