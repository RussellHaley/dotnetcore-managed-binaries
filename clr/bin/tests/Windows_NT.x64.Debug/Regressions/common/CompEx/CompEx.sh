...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/CompEx CompEx.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/CompEx CompEx.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CompEx.dasm.il CompEx.exe
    "$CORE_ROOT/ildasm" -raweh -out=CompEx.dasm.il CompEx.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CompEx.asm.exe  CompEx.dasm.il
    "$CORE_ROOT/ilasm" -output=CompEx.asm.exe  CompEx.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CompEx
    if [ ! -f CompEx.org ]; then
        TakeLock
        if [ ! -f CompEx.org ]; then
          mkdir IL
          cp CompEx.exe IL/CompEx.exe
          mv CompEx.exe CompEx.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CompEx.org /out CompEx.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CompEx.org /out CompEx.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in CompEx.reflect.xml
  if [ -f CompEx.reflect.xml ]
then
    ReflectionRoots="-x CompEx.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CompEx.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CompEx.asm.exe $CLRTestExecutionArguments
