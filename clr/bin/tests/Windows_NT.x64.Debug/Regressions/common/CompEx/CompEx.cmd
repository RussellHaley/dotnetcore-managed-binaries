...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\CompEx CompEx.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\CompEx CompEx.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CompEx.dasm.il CompEx.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CompEx.dasm.il CompEx.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CompEx.asm.exe  CompEx.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CompEx.asm.exe  CompEx.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CompEx
    if not exist "CompEx.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CompEx.org" (
            mkdir IL
            copy CompEx.exe IL\CompEx.exe 
            ren CompEx.exe CompEx.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CompEx.org /out CompEx.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CompEx.org /out CompEx.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CompEx.reflect.xml
    IF EXIST CompEx.reflect.xml set ReflectionRoots=-x CompEx.reflect.xml
    ...
    set ExePath=%LinkBin%\CompEx.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CompEx.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CompEx.asm.exe %CLRTestExecutionArguments% %Host_Args%
