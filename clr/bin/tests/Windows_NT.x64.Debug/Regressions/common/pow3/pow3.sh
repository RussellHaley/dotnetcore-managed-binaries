...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/pow3 pow3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/pow3 pow3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=pow3.dasm.il pow3.exe
    "$CORE_ROOT/ildasm" -raweh -out=pow3.dasm.il pow3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=pow3.asm.exe  pow3.dasm.il
    "$CORE_ROOT/ilasm" -output=pow3.asm.exe  pow3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=pow3
    if [ ! -f pow3.org ]; then
        TakeLock
        if [ ! -f pow3.org ]; then
          mkdir IL
          cp pow3.exe IL/pow3.exe
          mv pow3.exe pow3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in pow3.org /out pow3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in pow3.org /out pow3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in pow3.reflect.xml
  if [ -f pow3.reflect.xml ]
then
    ReflectionRoots="-x pow3.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" pow3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" pow3.asm.exe $CLRTestExecutionArguments
