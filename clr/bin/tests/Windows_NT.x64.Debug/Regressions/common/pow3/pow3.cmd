...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\pow3 pow3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\pow3 pow3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=pow3.dasm.il pow3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=pow3.dasm.il pow3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=pow3.asm.exe  pow3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=pow3.asm.exe  pow3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=pow3
    if not exist "pow3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "pow3.org" (
            mkdir IL
            copy pow3.exe IL\pow3.exe 
            ren pow3.exe pow3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in pow3.org /out pow3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in pow3.org /out pow3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in pow3.reflect.xml
    IF EXIST pow3.reflect.xml set ReflectionRoots=-x pow3.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% pow3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% pow3.asm.exe %CLRTestExecutionArguments% %Host_Args%
