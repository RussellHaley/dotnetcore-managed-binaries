...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\Unsafe Unsafe.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\Unsafe Unsafe.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Unsafe.dasm.il Unsafe.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Unsafe.dasm.il Unsafe.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Unsafe.asm.exe  Unsafe.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Unsafe.asm.exe  Unsafe.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Unsafe
    if not exist "Unsafe.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Unsafe.org" (
            mkdir IL
            copy Unsafe.exe IL\Unsafe.exe 
            ren Unsafe.exe Unsafe.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unsafe.org /out Unsafe.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Unsafe.org /out Unsafe.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Unsafe.reflect.xml
    IF EXIST Unsafe.reflect.xml set ReflectionRoots=-x Unsafe.reflect.xml
    ...
    set ExePath=%LinkBin%\Unsafe.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Unsafe.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Unsafe.asm.exe %CLRTestExecutionArguments% %Host_Args%
