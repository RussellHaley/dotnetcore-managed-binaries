...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/Unsafe Unsafe.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/Unsafe Unsafe.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Unsafe.dasm.il Unsafe.exe
    "$CORE_ROOT/ildasm" -raweh -out=Unsafe.dasm.il Unsafe.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Unsafe.asm.exe  Unsafe.dasm.il
    "$CORE_ROOT/ilasm" -output=Unsafe.asm.exe  Unsafe.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Unsafe
    if [ ! -f Unsafe.org ]; then
        TakeLock
        if [ ! -f Unsafe.org ]; then
          mkdir IL
          cp Unsafe.exe IL/Unsafe.exe
          mv Unsafe.exe Unsafe.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unsafe.org /out Unsafe.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Unsafe.org /out Unsafe.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Unsafe.reflect.xml
  if [ -f Unsafe.reflect.xml ]
then
    ReflectionRoots="-x Unsafe.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Unsafe.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Unsafe.asm.exe $CLRTestExecutionArguments
