...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\date date.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\date date.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=date.dasm.il date.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=date.dasm.il date.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=date.asm.exe  date.dasm.il
    %CORE_ROOT%\ilasm.exe /output=date.asm.exe  date.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=date
    if not exist "date.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "date.org" (
            mkdir IL
            copy date.exe IL\date.exe 
            ren date.exe date.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in date.org /out date.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in date.org /out date.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in date.reflect.xml
    IF EXIST date.reflect.xml set ReflectionRoots=-x date.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% date.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% date.asm.exe %CLRTestExecutionArguments% %Host_Args%
