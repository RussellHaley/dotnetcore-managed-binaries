...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/date date.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/date date.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=date.dasm.il date.exe
    "$CORE_ROOT/ildasm" -raweh -out=date.dasm.il date.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=date.asm.exe  date.dasm.il
    "$CORE_ROOT/ilasm" -output=date.asm.exe  date.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=date
    if [ ! -f date.org ]; then
        TakeLock
        if [ ! -f date.org ]; then
          mkdir IL
          cp date.exe IL/date.exe
          mv date.exe date.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in date.org /out date.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in date.org /out date.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in date.reflect.xml
  if [ -f date.reflect.xml ]
then
    ReflectionRoots="-x date.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" date.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" date.asm.exe $CLRTestExecutionArguments
