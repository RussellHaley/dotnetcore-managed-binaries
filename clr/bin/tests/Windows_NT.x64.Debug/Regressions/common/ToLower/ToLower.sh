...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/ToLower ToLower.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/ToLower ToLower.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ToLower.dasm.il ToLower.exe
    "$CORE_ROOT/ildasm" -raweh -out=ToLower.dasm.il ToLower.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ToLower.asm.exe  ToLower.dasm.il
    "$CORE_ROOT/ilasm" -output=ToLower.asm.exe  ToLower.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ToLower
    if [ ! -f ToLower.org ]; then
        TakeLock
        if [ ! -f ToLower.org ]; then
          mkdir IL
          cp ToLower.exe IL/ToLower.exe
          mv ToLower.exe ToLower.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ToLower.org /out ToLower.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ToLower.org /out ToLower.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ToLower.reflect.xml
  if [ -f ToLower.reflect.xml ]
then
    ReflectionRoots="-x ToLower.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ToLower.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ToLower.asm.exe $CLRTestExecutionArguments
