...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\ToLower ToLower.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\ToLower ToLower.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ToLower.dasm.il ToLower.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ToLower.dasm.il ToLower.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ToLower.asm.exe  ToLower.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ToLower.asm.exe  ToLower.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ToLower
    if not exist "ToLower.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ToLower.org" (
            mkdir IL
            copy ToLower.exe IL\ToLower.exe 
            ren ToLower.exe ToLower.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ToLower.org /out ToLower.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ToLower.org /out ToLower.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ToLower.reflect.xml
    IF EXIST ToLower.reflect.xml set ReflectionRoots=-x ToLower.reflect.xml
    ...
    set ExePath=%LinkBin%\ToLower.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ToLower.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ToLower.asm.exe %CLRTestExecutionArguments% %Host_Args%
