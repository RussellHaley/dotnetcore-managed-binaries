...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\Marshal Marshal.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\Marshal Marshal.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Marshal.dasm.il Marshal.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Marshal.dasm.il Marshal.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Marshal.asm.exe  Marshal.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Marshal.asm.exe  Marshal.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Marshal
    if not exist "Marshal.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Marshal.org" (
            mkdir IL
            copy Marshal.exe IL\Marshal.exe 
            ren Marshal.exe Marshal.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Marshal.org /out Marshal.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Marshal.org /out Marshal.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Marshal.reflect.xml
    IF EXIST Marshal.reflect.xml set ReflectionRoots=-x Marshal.reflect.xml
    ...
    set ExePath=%LinkBin%\Marshal.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Marshal.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Marshal.asm.exe %CLRTestExecutionArguments% %Host_Args%
