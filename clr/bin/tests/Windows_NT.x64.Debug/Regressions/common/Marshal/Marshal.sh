...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/Marshal Marshal.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/Marshal Marshal.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Marshal.dasm.il Marshal.exe
    "$CORE_ROOT/ildasm" -raweh -out=Marshal.dasm.il Marshal.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Marshal.asm.exe  Marshal.dasm.il
    "$CORE_ROOT/ilasm" -output=Marshal.asm.exe  Marshal.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Marshal
    if [ ! -f Marshal.org ]; then
        TakeLock
        if [ ! -f Marshal.org ]; then
          mkdir IL
          cp Marshal.exe IL/Marshal.exe
          mv Marshal.exe Marshal.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Marshal.org /out Marshal.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Marshal.org /out Marshal.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Marshal.reflect.xml
  if [ -f Marshal.reflect.xml ]
then
    ReflectionRoots="-x Marshal.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Marshal.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Marshal.asm.exe $CLRTestExecutionArguments
