...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\AboveStackLimit AboveStackLimit.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\Regressions\common\AboveStackLimit AboveStackLimit.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AboveStackLimit.dasm.il AboveStackLimit.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AboveStackLimit.dasm.il AboveStackLimit.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AboveStackLimit.asm.exe  AboveStackLimit.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AboveStackLimit.asm.exe  AboveStackLimit.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AboveStackLimit
    if not exist "AboveStackLimit.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AboveStackLimit.org" (
            mkdir IL
            copy AboveStackLimit.exe IL\AboveStackLimit.exe 
            ren AboveStackLimit.exe AboveStackLimit.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AboveStackLimit.org /out AboveStackLimit.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AboveStackLimit.org /out AboveStackLimit.exe
...
set ExePath=AboveStackLimit.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AboveStackLimit.reflect.xml
    IF EXIST AboveStackLimit.reflect.xml set ReflectionRoots=-x AboveStackLimit.reflect.xml
    ...
    set ExePath=%LinkBin%\AboveStackLimit.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AboveStackLimit.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AboveStackLimit.asm.exe %CLRTestExecutionArguments% %Host_Args%
