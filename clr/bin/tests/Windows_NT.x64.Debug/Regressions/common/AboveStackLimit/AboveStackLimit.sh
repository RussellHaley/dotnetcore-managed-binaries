...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/AboveStackLimit AboveStackLimit.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/Regressions/common/AboveStackLimit AboveStackLimit.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AboveStackLimit.dasm.il AboveStackLimit.exe
    "$CORE_ROOT/ildasm" -raweh -out=AboveStackLimit.dasm.il AboveStackLimit.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AboveStackLimit.asm.exe  AboveStackLimit.dasm.il
    "$CORE_ROOT/ilasm" -output=AboveStackLimit.asm.exe  AboveStackLimit.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AboveStackLimit
    if [ ! -f AboveStackLimit.org ]; then
        TakeLock
        if [ ! -f AboveStackLimit.org ]; then
          mkdir IL
          cp AboveStackLimit.exe IL/AboveStackLimit.exe
          mv AboveStackLimit.exe AboveStackLimit.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AboveStackLimit.org /out AboveStackLimit.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AboveStackLimit.org /out AboveStackLimit.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AboveStackLimit.reflect.xml
  if [ -f AboveStackLimit.reflect.xml ]
then
    ReflectionRoots="-x AboveStackLimit.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AboveStackLimit.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AboveStackLimit.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AboveStackLimit.asm.exe $CLRTestExecutionArguments
