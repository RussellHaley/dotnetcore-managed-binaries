...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/readytorun/tests/mainv1 mainv1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/readytorun/tests/mainv1 mainv1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mainv1.dasm.il mainv1.exe
    "$CORE_ROOT/ildasm" -raweh -out=mainv1.dasm.il mainv1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mainv1.asm.exe  mainv1.dasm.il
    "$CORE_ROOT/ilasm" -output=mainv1.asm.exe  mainv1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mainv1
    if [ ! -f mainv1.org ]; then
        TakeLock
        if [ ! -f mainv1.org ]; then
          mkdir IL
          cp mainv1.exe IL/mainv1.exe
          mv mainv1.exe mainv1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mainv1.org /out mainv1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mainv1.org /out mainv1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mainv1.reflect.xml
  if [ -f mainv1.reflect.xml ]
then
    ReflectionRoots="-x mainv1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mainv1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mainv1.asm.exe $CLRTestExecutionArguments
