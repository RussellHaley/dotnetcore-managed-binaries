...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\readytorun\tests\mainv1 mainv1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\readytorun\tests\mainv1 mainv1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mainv1.dasm.il mainv1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mainv1.dasm.il mainv1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mainv1.asm.exe  mainv1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mainv1.asm.exe  mainv1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
%Core_Root%\crossgen /readytorun /platform_assemblies_paths %Core_Root%;%CD% /out test.ni.dll test.dll 
%Core_Root%\crossgen /readytorun /platform_assemblies_paths %Core_Root%;%CD% /out fieldgetter.ni.dll fieldgetter.dll 
%Core_Root%\crossgen /readytorun /platform_assemblies_paths %Core_Root%;%CD% /out mainv1.ni.exe mainv1.exe
REM Long GC script
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mainv1
    if not exist "mainv1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mainv1.org" (
            mkdir IL
            copy mainv1.exe IL\mainv1.exe 
            ren mainv1.exe mainv1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mainv1.org /out mainv1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mainv1.org /out mainv1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mainv1.reflect.xml
    IF EXIST mainv1.reflect.xml set ReflectionRoots=-x mainv1.reflect.xml
    ...
    set ExePath=%LinkBin%\mainv1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mainv1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mainv1.asm.exe %CLRTestExecutionArguments% %Host_Args%
