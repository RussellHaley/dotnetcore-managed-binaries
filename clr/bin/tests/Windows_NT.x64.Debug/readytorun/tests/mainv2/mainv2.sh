...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/readytorun/tests/mainv2 mainv2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/readytorun/tests/mainv2 mainv2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=mainv2.dasm.il mainv2.exe
    "$CORE_ROOT/ildasm" -raweh -out=mainv2.dasm.il mainv2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=mainv2.asm.exe  mainv2.dasm.il
    "$CORE_ROOT/ilasm" -output=mainv2.asm.exe  mainv2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=mainv2
    if [ ! -f mainv2.org ]; then
        TakeLock
        if [ ! -f mainv2.org ]; then
          mkdir IL
          cp mainv2.exe IL/mainv2.exe
          mv mainv2.exe mainv2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mainv2.org /out mainv2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in mainv2.org /out mainv2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in mainv2.reflect.xml
  if [ -f mainv2.reflect.xml ]
then
    ReflectionRoots="-x mainv2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" mainv2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" mainv2.asm.exe $CLRTestExecutionArguments
