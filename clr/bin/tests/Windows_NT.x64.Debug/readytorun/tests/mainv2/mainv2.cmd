...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\readytorun\tests\mainv2 mainv2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\readytorun\tests\mainv2 mainv2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=mainv2.dasm.il mainv2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=mainv2.dasm.il mainv2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=mainv2.asm.exe  mainv2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=mainv2.asm.exe  mainv2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
%Core_Root%\crossgen /readytorun /platform_assemblies_paths %Core_Root%;%CD% /out mainv2.ni.exe mainv2.exe
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=mainv2
    if not exist "mainv2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "mainv2.org" (
            mkdir IL
            copy mainv2.exe IL\mainv2.exe 
            ren mainv2.exe mainv2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mainv2.org /out mainv2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in mainv2.org /out mainv2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in mainv2.reflect.xml
    IF EXIST mainv2.reflect.xml set ReflectionRoots=-x mainv2.reflect.xml
    ...
    set ExePath=%LinkBin%\mainv2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% mainv2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% mainv2.asm.exe %CLRTestExecutionArguments% %Host_Args%
