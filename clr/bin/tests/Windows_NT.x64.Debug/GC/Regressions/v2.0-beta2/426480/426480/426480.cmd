...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta2\426480\426480 426480.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta2\426480\426480 426480.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=426480.dasm.il 426480.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=426480.dasm.il 426480.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=426480.asm.exe  426480.dasm.il
    %CORE_ROOT%\ilasm.exe /output=426480.asm.exe  426480.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=426480
    if not exist "426480.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "426480.org" (
            mkdir IL
            copy 426480.exe IL\426480.exe 
            ren 426480.exe 426480.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 426480.org /out 426480.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 426480.org /out 426480.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 426480.reflect.xml
    IF EXIST 426480.reflect.xml set ReflectionRoots=-x 426480.reflect.xml
    ...
    set ExePath=%LinkBin%\426480.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 426480.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 426480.asm.exe %CLRTestExecutionArguments% %Host_Args%
