...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta2/426480/426480 426480.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta2/426480/426480 426480.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=426480.dasm.il 426480.exe
    "$CORE_ROOT/ildasm" -raweh -out=426480.dasm.il 426480.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=426480.asm.exe  426480.dasm.il
    "$CORE_ROOT/ilasm" -output=426480.asm.exe  426480.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=426480
    if [ ! -f 426480.org ]; then
        TakeLock
        if [ ! -f 426480.org ]; then
          mkdir IL
          cp 426480.exe IL/426480.exe
          mv 426480.exe 426480.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 426480.org /out 426480.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 426480.org /out 426480.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 426480.reflect.xml
  if [ -f 426480.reflect.xml ]
then
    ReflectionRoots="-x 426480.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 426480.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 426480.asm.exe $CLRTestExecutionArguments
