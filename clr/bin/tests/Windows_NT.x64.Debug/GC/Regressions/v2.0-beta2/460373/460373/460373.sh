...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta2/460373/460373 460373.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta2/460373/460373 460373.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=460373.dasm.il 460373.exe
    "$CORE_ROOT/ildasm" -raweh -out=460373.dasm.il 460373.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=460373.asm.exe  460373.dasm.il
    "$CORE_ROOT/ilasm" -output=460373.asm.exe  460373.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=460373
    if [ ! -f 460373.org ]; then
        TakeLock
        if [ ! -f 460373.org ]; then
          mkdir IL
          cp 460373.exe IL/460373.exe
          mv 460373.exe 460373.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 460373.org /out 460373.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 460373.org /out 460373.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 460373.reflect.xml
  if [ -f 460373.reflect.xml ]
then
    ReflectionRoots="-x 460373.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 460373.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 460373.asm.exe $CLRTestExecutionArguments
