...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta2\460373\460373 460373.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta2\460373\460373 460373.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=460373.dasm.il 460373.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=460373.dasm.il 460373.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=460373.asm.exe  460373.dasm.il
    %CORE_ROOT%\ilasm.exe /output=460373.asm.exe  460373.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=460373
    if not exist "460373.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "460373.org" (
            mkdir IL
            copy 460373.exe IL\460373.exe 
            ren 460373.exe 460373.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 460373.org /out 460373.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 460373.org /out 460373.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 460373.reflect.xml
    IF EXIST 460373.reflect.xml set ReflectionRoots=-x 460373.reflect.xml
    ...
    set ExePath=%LinkBin%\460373.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 460373.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 460373.asm.exe %CLRTestExecutionArguments% %Host_Args%
