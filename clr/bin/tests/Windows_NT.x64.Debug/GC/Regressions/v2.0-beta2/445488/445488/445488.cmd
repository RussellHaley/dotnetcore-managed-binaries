...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta2\445488\445488 445488.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta2\445488\445488 445488.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=445488.dasm.il 445488.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=445488.dasm.il 445488.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=445488.asm.exe  445488.dasm.il
    %CORE_ROOT%\ilasm.exe /output=445488.asm.exe  445488.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=445488
    if not exist "445488.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "445488.org" (
            mkdir IL
            copy 445488.exe IL\445488.exe 
            ren 445488.exe 445488.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 445488.org /out 445488.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 445488.org /out 445488.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 445488.reflect.xml
    IF EXIST 445488.reflect.xml set ReflectionRoots=-x 445488.reflect.xml
    ...
    set ExePath=%LinkBin%\445488.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 445488.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 445488.asm.exe %CLRTestExecutionArguments% %Host_Args%
