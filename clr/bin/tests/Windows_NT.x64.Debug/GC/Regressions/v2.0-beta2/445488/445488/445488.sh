...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta2/445488/445488 445488.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta2/445488/445488 445488.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=445488.dasm.il 445488.exe
    "$CORE_ROOT/ildasm" -raweh -out=445488.dasm.il 445488.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=445488.asm.exe  445488.dasm.il
    "$CORE_ROOT/ilasm" -output=445488.asm.exe  445488.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=445488
    if [ ! -f 445488.org ]; then
        TakeLock
        if [ ! -f 445488.org ]; then
          mkdir IL
          cp 445488.exe IL/445488.exe
          mv 445488.exe 445488.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 445488.org /out 445488.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 445488.org /out 445488.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 445488.reflect.xml
  if [ -f 445488.reflect.xml ]
then
    ReflectionRoots="-x 445488.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 445488.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 445488.asm.exe $CLRTestExecutionArguments
