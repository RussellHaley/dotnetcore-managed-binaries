...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta2/452950/452950 452950.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta2/452950/452950 452950.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=452950.dasm.il 452950.exe
    "$CORE_ROOT/ildasm" -raweh -out=452950.dasm.il 452950.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=452950.asm.exe  452950.dasm.il
    "$CORE_ROOT/ilasm" -output=452950.asm.exe  452950.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=452950
    if [ ! -f 452950.org ]; then
        TakeLock
        if [ ! -f 452950.org ]; then
          mkdir IL
          cp 452950.exe IL/452950.exe
          mv 452950.exe 452950.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 452950.org /out 452950.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 452950.org /out 452950.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 452950.reflect.xml
  if [ -f 452950.reflect.xml ]
then
    ReflectionRoots="-x 452950.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 452950.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 452950.asm.exe $CLRTestExecutionArguments
