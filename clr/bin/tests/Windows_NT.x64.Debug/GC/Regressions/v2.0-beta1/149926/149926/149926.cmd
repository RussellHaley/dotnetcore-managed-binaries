...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta1\149926\149926 149926.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta1\149926\149926 149926.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=149926.dasm.il 149926.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=149926.dasm.il 149926.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=149926.asm.exe  149926.dasm.il
    %CORE_ROOT%\ilasm.exe /output=149926.asm.exe  149926.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=149926
    if not exist "149926.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "149926.org" (
            mkdir IL
            copy 149926.exe IL\149926.exe 
            ren 149926.exe 149926.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 149926.org /out 149926.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 149926.org /out 149926.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 149926.reflect.xml
    IF EXIST 149926.reflect.xml set ReflectionRoots=-x 149926.reflect.xml
    ...
    set ExePath=%LinkBin%\149926.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 149926.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 149926.asm.exe %CLRTestExecutionArguments% %Host_Args%
