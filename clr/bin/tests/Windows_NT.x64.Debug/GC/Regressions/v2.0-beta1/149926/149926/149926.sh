...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta1/149926/149926 149926.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta1/149926/149926 149926.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=149926.dasm.il 149926.exe
    "$CORE_ROOT/ildasm" -raweh -out=149926.dasm.il 149926.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=149926.asm.exe  149926.dasm.il
    "$CORE_ROOT/ilasm" -output=149926.asm.exe  149926.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=149926
    if [ ! -f 149926.org ]; then
        TakeLock
        if [ ! -f 149926.org ]; then
          mkdir IL
          cp 149926.exe IL/149926.exe
          mv 149926.exe 149926.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 149926.org /out 149926.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 149926.org /out 149926.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 149926.reflect.xml
  if [ -f 149926.reflect.xml ]
then
    ReflectionRoots="-x 149926.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 149926.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 149926.asm.exe $CLRTestExecutionArguments
