...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta1\289745\302560 302560.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta1\289745\302560 302560.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=302560.dasm.il 302560.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=302560.dasm.il 302560.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=302560.asm.exe  302560.dasm.il
    %CORE_ROOT%\ilasm.exe /output=302560.asm.exe  302560.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=302560
    if not exist "302560.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "302560.org" (
            mkdir IL
            copy 302560.exe IL\302560.exe 
            ren 302560.exe 302560.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 302560.org /out 302560.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 302560.org /out 302560.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 302560.reflect.xml
    IF EXIST 302560.reflect.xml set ReflectionRoots=-x 302560.reflect.xml
    ...
    set ExePath=%LinkBin%\302560.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 302560.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 302560.asm.exe %CLRTestExecutionArguments% %Host_Args%
