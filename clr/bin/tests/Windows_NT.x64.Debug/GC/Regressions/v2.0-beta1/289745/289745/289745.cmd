...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta1\289745\289745 289745.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-beta1\289745\289745 289745.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=289745.dasm.il 289745.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=289745.dasm.il 289745.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=289745.asm.exe  289745.dasm.il
    %CORE_ROOT%\ilasm.exe /output=289745.asm.exe  289745.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=289745
    if not exist "289745.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "289745.org" (
            mkdir IL
            copy 289745.exe IL\289745.exe 
            ren 289745.exe 289745.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 289745.org /out 289745.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 289745.org /out 289745.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 289745.reflect.xml
    IF EXIST 289745.reflect.xml set ReflectionRoots=-x 289745.reflect.xml
    ...
    set ExePath=%LinkBin%\289745.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 289745.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 289745.asm.exe %CLRTestExecutionArguments% %Host_Args%
