...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta1/289745/302560 302560.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta1/289745/302560 302560.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=302560.dasm.il 302560.exe
    "$CORE_ROOT/ildasm" -raweh -out=302560.dasm.il 302560.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=302560.asm.exe  302560.dasm.il
    "$CORE_ROOT/ilasm" -output=302560.asm.exe  302560.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=302560
    if [ ! -f 302560.org ]; then
        TakeLock
        if [ ! -f 302560.org ]; then
          mkdir IL
          cp 302560.exe IL/302560.exe
          mv 302560.exe 302560.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 302560.org /out 302560.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 302560.org /out 302560.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 302560.reflect.xml
  if [ -f 302560.reflect.xml ]
then
    ReflectionRoots="-x 302560.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 302560.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 302560.asm.exe $CLRTestExecutionArguments
