...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta1/289745/289745 289745.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-beta1/289745/289745 289745.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=289745.dasm.il 289745.exe
    "$CORE_ROOT/ildasm" -raweh -out=289745.dasm.il 289745.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=289745.asm.exe  289745.dasm.il
    "$CORE_ROOT/ilasm" -output=289745.asm.exe  289745.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=289745
    if [ ! -f 289745.org ]; then
        TakeLock
        if [ ! -f 289745.org ]; then
          mkdir IL
          cp 289745.exe IL/289745.exe
          mv 289745.exe 289745.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 289745.org /out 289745.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 289745.org /out 289745.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 289745.reflect.xml
  if [ -f 289745.reflect.xml ]
then
    ReflectionRoots="-x 289745.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 289745.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 289745.asm.exe $CLRTestExecutionArguments
