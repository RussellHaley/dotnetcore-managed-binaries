...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-rtm/494226/494226 494226.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Regressions/v2.0-rtm/494226/494226 494226.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=494226.dasm.il 494226.exe
    "$CORE_ROOT/ildasm" -raweh -out=494226.dasm.il 494226.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=494226.asm.exe  494226.dasm.il
    "$CORE_ROOT/ilasm" -output=494226.asm.exe  494226.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=494226
    if [ ! -f 494226.org ]; then
        TakeLock
        if [ ! -f 494226.org ]; then
          mkdir IL
          cp 494226.exe IL/494226.exe
          mv 494226.exe 494226.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 494226.org /out 494226.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in 494226.org /out 494226.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in 494226.reflect.xml
  if [ -f 494226.reflect.xml ]
then
    ReflectionRoots="-x 494226.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" 494226.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" 494226.asm.exe $CLRTestExecutionArguments
