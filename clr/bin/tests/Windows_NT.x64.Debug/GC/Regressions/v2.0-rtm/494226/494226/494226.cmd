...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-rtm\494226\494226 494226.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Regressions\v2.0-rtm\494226\494226 494226.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=494226.dasm.il 494226.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=494226.dasm.il 494226.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=494226.asm.exe  494226.dasm.il
    %CORE_ROOT%\ilasm.exe /output=494226.asm.exe  494226.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=494226
    if not exist "494226.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "494226.org" (
            mkdir IL
            copy 494226.exe IL\494226.exe 
            ren 494226.exe 494226.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 494226.org /out 494226.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in 494226.org /out 494226.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in 494226.reflect.xml
    IF EXIST 494226.reflect.xml set ReflectionRoots=-x 494226.reflect.xml
    ...
    set ExePath=%LinkBin%\494226.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% 494226.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% 494226.asm.exe %CLRTestExecutionArguments% %Host_Args%
