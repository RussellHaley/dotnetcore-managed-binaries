...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\GC\Coverage\LargeObjectAlloc LargeObjectAlloc.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\GC\Coverage\LargeObjectAlloc LargeObjectAlloc.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LargeObjectAlloc.dasm.il LargeObjectAlloc.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LargeObjectAlloc.dasm.il LargeObjectAlloc.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LargeObjectAlloc.asm.exe  LargeObjectAlloc.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LargeObjectAlloc.asm.exe  LargeObjectAlloc.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LargeObjectAlloc
    if not exist "LargeObjectAlloc.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LargeObjectAlloc.org" (
            mkdir IL
            copy LargeObjectAlloc.exe IL\LargeObjectAlloc.exe 
            ren LargeObjectAlloc.exe LargeObjectAlloc.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LargeObjectAlloc.org /out LargeObjectAlloc.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LargeObjectAlloc.org /out LargeObjectAlloc.exe
...
set ExePath=LargeObjectAlloc.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LargeObjectAlloc.reflect.xml
    IF EXIST LargeObjectAlloc.reflect.xml set ReflectionRoots=-x LargeObjectAlloc.reflect.xml
    ...
    set ExePath=%LinkBin%\LargeObjectAlloc.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LargeObjectAlloc.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LargeObjectAlloc.asm.exe %CLRTestExecutionArguments% %Host_Args%
