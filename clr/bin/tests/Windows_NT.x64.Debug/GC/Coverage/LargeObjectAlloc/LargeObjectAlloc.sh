...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/GC/Coverage/LargeObjectAlloc LargeObjectAlloc.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/GC/Coverage/LargeObjectAlloc LargeObjectAlloc.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LargeObjectAlloc.dasm.il LargeObjectAlloc.exe
    "$CORE_ROOT/ildasm" -raweh -out=LargeObjectAlloc.dasm.il LargeObjectAlloc.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LargeObjectAlloc.asm.exe  LargeObjectAlloc.dasm.il
    "$CORE_ROOT/ilasm" -output=LargeObjectAlloc.asm.exe  LargeObjectAlloc.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LargeObjectAlloc
    if [ ! -f LargeObjectAlloc.org ]; then
        TakeLock
        if [ ! -f LargeObjectAlloc.org ]; then
          mkdir IL
          cp LargeObjectAlloc.exe IL/LargeObjectAlloc.exe
          mv LargeObjectAlloc.exe LargeObjectAlloc.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LargeObjectAlloc.org /out LargeObjectAlloc.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LargeObjectAlloc.org /out LargeObjectAlloc.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LargeObjectAlloc.reflect.xml
  if [ -f LargeObjectAlloc.reflect.xml ]
then
    ReflectionRoots="-x LargeObjectAlloc.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/LargeObjectAlloc.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LargeObjectAlloc.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LargeObjectAlloc.asm.exe $CLRTestExecutionArguments
