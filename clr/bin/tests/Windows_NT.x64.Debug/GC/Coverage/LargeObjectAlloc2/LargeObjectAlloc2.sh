...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/GC/Coverage/LargeObjectAlloc2 LargeObjectAlloc2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/GC/Coverage/LargeObjectAlloc2 LargeObjectAlloc2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=LargeObjectAlloc2.dasm.il LargeObjectAlloc2.exe
    "$CORE_ROOT/ildasm" -raweh -out=LargeObjectAlloc2.dasm.il LargeObjectAlloc2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=LargeObjectAlloc2.asm.exe  LargeObjectAlloc2.dasm.il
    "$CORE_ROOT/ilasm" -output=LargeObjectAlloc2.asm.exe  LargeObjectAlloc2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=LargeObjectAlloc2
    if [ ! -f LargeObjectAlloc2.org ]; then
        TakeLock
        if [ ! -f LargeObjectAlloc2.org ]; then
          mkdir IL
          cp LargeObjectAlloc2.exe IL/LargeObjectAlloc2.exe
          mv LargeObjectAlloc2.exe LargeObjectAlloc2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LargeObjectAlloc2.org /out LargeObjectAlloc2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in LargeObjectAlloc2.org /out LargeObjectAlloc2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in LargeObjectAlloc2.reflect.xml
  if [ -f LargeObjectAlloc2.reflect.xml ]
then
    ReflectionRoots="-x LargeObjectAlloc2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/LargeObjectAlloc2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" LargeObjectAlloc2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" LargeObjectAlloc2.asm.exe $CLRTestExecutionArguments
