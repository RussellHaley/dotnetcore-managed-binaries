...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\GC\Coverage\LargeObjectAlloc2 LargeObjectAlloc2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\GC\Coverage\LargeObjectAlloc2 LargeObjectAlloc2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=LargeObjectAlloc2.dasm.il LargeObjectAlloc2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=LargeObjectAlloc2.dasm.il LargeObjectAlloc2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=LargeObjectAlloc2.asm.exe  LargeObjectAlloc2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=LargeObjectAlloc2.asm.exe  LargeObjectAlloc2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=LargeObjectAlloc2
    if not exist "LargeObjectAlloc2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "LargeObjectAlloc2.org" (
            mkdir IL
            copy LargeObjectAlloc2.exe IL\LargeObjectAlloc2.exe 
            ren LargeObjectAlloc2.exe LargeObjectAlloc2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LargeObjectAlloc2.org /out LargeObjectAlloc2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in LargeObjectAlloc2.org /out LargeObjectAlloc2.exe
...
set ExePath=LargeObjectAlloc2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in LargeObjectAlloc2.reflect.xml
    IF EXIST LargeObjectAlloc2.reflect.xml set ReflectionRoots=-x LargeObjectAlloc2.reflect.xml
    ...
    set ExePath=%LinkBin%\LargeObjectAlloc2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% LargeObjectAlloc2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% LargeObjectAlloc2.asm.exe %CLRTestExecutionArguments% %Host_Args%
