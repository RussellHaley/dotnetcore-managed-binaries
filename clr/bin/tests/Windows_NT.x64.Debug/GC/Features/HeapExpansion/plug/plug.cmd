...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\plug plug.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\plug plug.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=plug.dasm.il plug.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=plug.dasm.il plug.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=plug.asm.exe  plug.dasm.il
    %CORE_ROOT%\ilasm.exe /output=plug.asm.exe  plug.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=plug
    if not exist "plug.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "plug.org" (
            mkdir IL
            copy plug.exe IL\plug.exe 
            ren plug.exe plug.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in plug.org /out plug.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in plug.org /out plug.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in plug.reflect.xml
    IF EXIST plug.reflect.xml set ReflectionRoots=-x plug.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% plug.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% plug.asm.exe %CLRTestExecutionArguments% %Host_Args%
