...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/plug plug.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/plug plug.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=plug.dasm.il plug.exe
    "$CORE_ROOT/ildasm" -raweh -out=plug.dasm.il plug.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=plug.asm.exe  plug.dasm.il
    "$CORE_ROOT/ilasm" -output=plug.asm.exe  plug.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=plug
    if [ ! -f plug.org ]; then
        TakeLock
        if [ ! -f plug.org ]; then
          mkdir IL
          cp plug.exe IL/plug.exe
          mv plug.exe plug.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in plug.org /out plug.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in plug.org /out plug.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in plug.reflect.xml
  if [ -f plug.reflect.xml ]
then
    ReflectionRoots="-x plug.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" plug.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" plug.asm.exe $CLRTestExecutionArguments
