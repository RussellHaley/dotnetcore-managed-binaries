...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\bestfit-threaded bestfit-threaded.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\bestfit-threaded bestfit-threaded.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=bestfit-threaded.dasm.il bestfit-threaded.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=bestfit-threaded.dasm.il bestfit-threaded.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=bestfit-threaded.asm.exe  bestfit-threaded.dasm.il
    %CORE_ROOT%\ilasm.exe /output=bestfit-threaded.asm.exe  bestfit-threaded.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=bestfit-threaded
    if not exist "bestfit-threaded.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "bestfit-threaded.org" (
            mkdir IL
            copy bestfit-threaded.exe IL\bestfit-threaded.exe 
            ren bestfit-threaded.exe bestfit-threaded.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bestfit-threaded.org /out bestfit-threaded.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bestfit-threaded.org /out bestfit-threaded.exe
...
set ExePath=bestfit-threaded.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in bestfit-threaded.reflect.xml
    IF EXIST bestfit-threaded.reflect.xml set ReflectionRoots=-x bestfit-threaded.reflect.xml
    ...
    set ExePath=%LinkBin%\bestfit-threaded.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% bestfit-threaded.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% bestfit-threaded.asm.exe %CLRTestExecutionArguments% %Host_Args%
