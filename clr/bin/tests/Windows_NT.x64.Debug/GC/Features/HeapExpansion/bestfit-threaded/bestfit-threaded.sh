...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/bestfit-threaded bestfit-threaded.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/bestfit-threaded bestfit-threaded.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=bestfit-threaded.dasm.il bestfit-threaded.exe
    "$CORE_ROOT/ildasm" -raweh -out=bestfit-threaded.dasm.il bestfit-threaded.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=bestfit-threaded.asm.exe  bestfit-threaded.dasm.il
    "$CORE_ROOT/ilasm" -output=bestfit-threaded.asm.exe  bestfit-threaded.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=bestfit-threaded
    if [ ! -f bestfit-threaded.org ]; then
        TakeLock
        if [ ! -f bestfit-threaded.org ]; then
          mkdir IL
          cp bestfit-threaded.exe IL/bestfit-threaded.exe
          mv bestfit-threaded.exe bestfit-threaded.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bestfit-threaded.org /out bestfit-threaded.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bestfit-threaded.org /out bestfit-threaded.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in bestfit-threaded.reflect.xml
  if [ -f bestfit-threaded.reflect.xml ]
then
    ReflectionRoots="-x bestfit-threaded.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/bestfit-threaded.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" bestfit-threaded.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" bestfit-threaded.asm.exe $CLRTestExecutionArguments
