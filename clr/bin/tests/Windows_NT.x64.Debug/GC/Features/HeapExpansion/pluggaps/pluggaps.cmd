...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\pluggaps pluggaps.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\pluggaps pluggaps.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=pluggaps.dasm.il pluggaps.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=pluggaps.dasm.il pluggaps.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=pluggaps.asm.exe  pluggaps.dasm.il
    %CORE_ROOT%\ilasm.exe /output=pluggaps.asm.exe  pluggaps.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=pluggaps
    if not exist "pluggaps.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "pluggaps.org" (
            mkdir IL
            copy pluggaps.exe IL\pluggaps.exe 
            ren pluggaps.exe pluggaps.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in pluggaps.org /out pluggaps.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in pluggaps.org /out pluggaps.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in pluggaps.reflect.xml
    IF EXIST pluggaps.reflect.xml set ReflectionRoots=-x pluggaps.reflect.xml
    ...
    set ExePath=%LinkBin%\pluggaps.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% pluggaps.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% pluggaps.asm.exe %CLRTestExecutionArguments% %Host_Args%
