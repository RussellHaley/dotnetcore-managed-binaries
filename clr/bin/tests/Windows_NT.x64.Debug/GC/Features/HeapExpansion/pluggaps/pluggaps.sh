...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/pluggaps pluggaps.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/pluggaps pluggaps.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=pluggaps.dasm.il pluggaps.exe
    "$CORE_ROOT/ildasm" -raweh -out=pluggaps.dasm.il pluggaps.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=pluggaps.asm.exe  pluggaps.dasm.il
    "$CORE_ROOT/ilasm" -output=pluggaps.asm.exe  pluggaps.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=pluggaps
    if [ ! -f pluggaps.org ]; then
        TakeLock
        if [ ! -f pluggaps.org ]; then
          mkdir IL
          cp pluggaps.exe IL/pluggaps.exe
          mv pluggaps.exe pluggaps.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in pluggaps.org /out pluggaps.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in pluggaps.org /out pluggaps.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in pluggaps.reflect.xml
  if [ -f pluggaps.reflect.xml ]
then
    ReflectionRoots="-x pluggaps.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" pluggaps.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" pluggaps.asm.exe $CLRTestExecutionArguments
