...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\bestfit-finalize bestfit-finalize.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\bestfit-finalize bestfit-finalize.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=bestfit-finalize.dasm.il bestfit-finalize.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=bestfit-finalize.dasm.il bestfit-finalize.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=bestfit-finalize.asm.exe  bestfit-finalize.dasm.il
    %CORE_ROOT%\ilasm.exe /output=bestfit-finalize.asm.exe  bestfit-finalize.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=bestfit-finalize
    if not exist "bestfit-finalize.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "bestfit-finalize.org" (
            mkdir IL
            copy bestfit-finalize.exe IL\bestfit-finalize.exe 
            ren bestfit-finalize.exe bestfit-finalize.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bestfit-finalize.org /out bestfit-finalize.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bestfit-finalize.org /out bestfit-finalize.exe
...
set ExePath=bestfit-finalize.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in bestfit-finalize.reflect.xml
    IF EXIST bestfit-finalize.reflect.xml set ReflectionRoots=-x bestfit-finalize.reflect.xml
    ...
    set ExePath=%LinkBin%\bestfit-finalize.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% bestfit-finalize.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% bestfit-finalize.asm.exe %CLRTestExecutionArguments% %Host_Args%
