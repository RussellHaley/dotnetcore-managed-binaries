...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/bestfit-finalize bestfit-finalize.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/bestfit-finalize bestfit-finalize.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=bestfit-finalize.dasm.il bestfit-finalize.exe
    "$CORE_ROOT/ildasm" -raweh -out=bestfit-finalize.dasm.il bestfit-finalize.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=bestfit-finalize.asm.exe  bestfit-finalize.dasm.il
    "$CORE_ROOT/ilasm" -output=bestfit-finalize.asm.exe  bestfit-finalize.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=bestfit-finalize
    if [ ! -f bestfit-finalize.org ]; then
        TakeLock
        if [ ! -f bestfit-finalize.org ]; then
          mkdir IL
          cp bestfit-finalize.exe IL/bestfit-finalize.exe
          mv bestfit-finalize.exe bestfit-finalize.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bestfit-finalize.org /out bestfit-finalize.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bestfit-finalize.org /out bestfit-finalize.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in bestfit-finalize.reflect.xml
  if [ -f bestfit-finalize.reflect.xml ]
then
    ReflectionRoots="-x bestfit-finalize.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/bestfit-finalize.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" bestfit-finalize.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" bestfit-finalize.asm.exe $CLRTestExecutionArguments
