...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\Handles Handles.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\Handles Handles.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Handles.dasm.il Handles.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Handles.dasm.il Handles.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Handles.asm.exe  Handles.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Handles.asm.exe  Handles.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Handles
    if not exist "Handles.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Handles.org" (
            mkdir IL
            copy Handles.exe IL\Handles.exe 
            ren Handles.exe Handles.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Handles.org /out Handles.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Handles.org /out Handles.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Handles.reflect.xml
    IF EXIST Handles.reflect.xml set ReflectionRoots=-x Handles.reflect.xml
    ...
    set ExePath=%LinkBin%\Handles.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Handles.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Handles.asm.exe %CLRTestExecutionArguments% %Host_Args%
