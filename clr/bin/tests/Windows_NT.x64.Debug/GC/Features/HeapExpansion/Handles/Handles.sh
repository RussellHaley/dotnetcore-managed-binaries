...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/Handles Handles.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/Handles Handles.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Handles.dasm.il Handles.exe
    "$CORE_ROOT/ildasm" -raweh -out=Handles.dasm.il Handles.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Handles.asm.exe  Handles.dasm.il
    "$CORE_ROOT/ilasm" -output=Handles.asm.exe  Handles.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Handles
    if [ ! -f Handles.org ]; then
        TakeLock
        if [ ! -f Handles.org ]; then
          mkdir IL
          cp Handles.exe IL/Handles.exe
          mv Handles.exe Handles.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Handles.org /out Handles.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Handles.org /out Handles.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Handles.reflect.xml
  if [ -f Handles.reflect.xml ]
then
    ReflectionRoots="-x Handles.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Handles.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Handles.asm.exe $CLRTestExecutionArguments
