...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\bestfit bestfit.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\bestfit bestfit.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=bestfit.dasm.il bestfit.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=bestfit.dasm.il bestfit.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=bestfit.asm.exe  bestfit.dasm.il
    %CORE_ROOT%\ilasm.exe /output=bestfit.asm.exe  bestfit.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=bestfit
    if not exist "bestfit.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "bestfit.org" (
            mkdir IL
            copy bestfit.exe IL\bestfit.exe 
            ren bestfit.exe bestfit.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bestfit.org /out bestfit.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bestfit.org /out bestfit.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in bestfit.reflect.xml
    IF EXIST bestfit.reflect.xml set ReflectionRoots=-x bestfit.reflect.xml
    ...
    set ExePath=%LinkBin%\bestfit.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% bestfit.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% bestfit.asm.exe %CLRTestExecutionArguments% %Host_Args%
