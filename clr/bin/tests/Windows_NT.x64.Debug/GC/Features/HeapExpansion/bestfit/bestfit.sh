...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/bestfit bestfit.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/bestfit bestfit.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=bestfit.dasm.il bestfit.exe
    "$CORE_ROOT/ildasm" -raweh -out=bestfit.dasm.il bestfit.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=bestfit.asm.exe  bestfit.dasm.il
    "$CORE_ROOT/ilasm" -output=bestfit.asm.exe  bestfit.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=bestfit
    if [ ! -f bestfit.org ]; then
        TakeLock
        if [ ! -f bestfit.org ]; then
          mkdir IL
          cp bestfit.exe IL/bestfit.exe
          mv bestfit.exe bestfit.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bestfit.org /out bestfit.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bestfit.org /out bestfit.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in bestfit.reflect.xml
  if [ -f bestfit.reflect.xml ]
then
    ReflectionRoots="-x bestfit.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" bestfit.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" bestfit.asm.exe $CLRTestExecutionArguments
