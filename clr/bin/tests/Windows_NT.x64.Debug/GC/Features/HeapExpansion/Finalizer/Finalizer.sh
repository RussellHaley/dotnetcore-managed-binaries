...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/Finalizer Finalizer.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/Finalizer Finalizer.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Finalizer.dasm.il Finalizer.exe
    "$CORE_ROOT/ildasm" -raweh -out=Finalizer.dasm.il Finalizer.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Finalizer.asm.exe  Finalizer.dasm.il
    "$CORE_ROOT/ilasm" -output=Finalizer.asm.exe  Finalizer.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Finalizer
    if [ ! -f Finalizer.org ]; then
        TakeLock
        if [ ! -f Finalizer.org ]; then
          mkdir IL
          cp Finalizer.exe IL/Finalizer.exe
          mv Finalizer.exe Finalizer.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Finalizer.org /out Finalizer.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Finalizer.org /out Finalizer.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Finalizer.reflect.xml
  if [ -f Finalizer.reflect.xml ]
then
    ReflectionRoots="-x Finalizer.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Finalizer.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Finalizer.asm.exe $CLRTestExecutionArguments
