...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\Finalizer Finalizer.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\Finalizer Finalizer.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Finalizer.dasm.il Finalizer.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Finalizer.dasm.il Finalizer.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Finalizer.asm.exe  Finalizer.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Finalizer.asm.exe  Finalizer.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Finalizer
    if not exist "Finalizer.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Finalizer.org" (
            mkdir IL
            copy Finalizer.exe IL\Finalizer.exe 
            ren Finalizer.exe Finalizer.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Finalizer.org /out Finalizer.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Finalizer.org /out Finalizer.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Finalizer.reflect.xml
    IF EXIST Finalizer.reflect.xml set ReflectionRoots=-x Finalizer.reflect.xml
    ...
    set ExePath=%LinkBin%\Finalizer.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Finalizer.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Finalizer.asm.exe %CLRTestExecutionArguments% %Host_Args%
