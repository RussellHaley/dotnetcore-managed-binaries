...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\expandheap expandheap.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\expandheap expandheap.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=expandheap.dasm.il expandheap.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=expandheap.dasm.il expandheap.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=expandheap.asm.exe  expandheap.dasm.il
    %CORE_ROOT%\ilasm.exe /output=expandheap.asm.exe  expandheap.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=expandheap
    if not exist "expandheap.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "expandheap.org" (
            mkdir IL
            copy expandheap.exe IL\expandheap.exe 
            ren expandheap.exe expandheap.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in expandheap.org /out expandheap.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in expandheap.org /out expandheap.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in expandheap.reflect.xml
    IF EXIST expandheap.reflect.xml set ReflectionRoots=-x expandheap.reflect.xml
    ...
    set ExePath=%LinkBin%\expandheap.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% expandheap.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% expandheap.asm.exe %CLRTestExecutionArguments% %Host_Args%
