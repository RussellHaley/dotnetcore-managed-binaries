...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/expandheap expandheap.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/expandheap expandheap.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=expandheap.dasm.il expandheap.exe
    "$CORE_ROOT/ildasm" -raweh -out=expandheap.dasm.il expandheap.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=expandheap.asm.exe  expandheap.dasm.il
    "$CORE_ROOT/ilasm" -output=expandheap.asm.exe  expandheap.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=expandheap
    if [ ! -f expandheap.org ]; then
        TakeLock
        if [ ! -f expandheap.org ]; then
          mkdir IL
          cp expandheap.exe IL/expandheap.exe
          mv expandheap.exe expandheap.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in expandheap.org /out expandheap.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in expandheap.org /out expandheap.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in expandheap.reflect.xml
  if [ -f expandheap.reflect.xml ]
then
    ReflectionRoots="-x expandheap.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/expandheap.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" expandheap.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" expandheap.asm.exe $CLRTestExecutionArguments
