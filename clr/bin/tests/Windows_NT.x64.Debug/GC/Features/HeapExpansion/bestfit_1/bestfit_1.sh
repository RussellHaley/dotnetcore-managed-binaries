...
if [ -z ${CLRTestExecutionArguments+x} ]; then export CLRTestExecutionArguments='1 1000 50000'; fi
...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/bestfit_1 bestfit_1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/HeapExpansion/bestfit_1 bestfit_1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=bestfit_1.dasm.il bestfit_1.exe
    "$CORE_ROOT/ildasm" -raweh -out=bestfit_1.dasm.il bestfit_1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=bestfit_1.asm.exe  bestfit_1.dasm.il
    "$CORE_ROOT/ilasm" -output=bestfit_1.asm.exe  bestfit_1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=bestfit_1
    if [ ! -f bestfit_1.org ]; then
        TakeLock
        if [ ! -f bestfit_1.org ]; then
          mkdir IL
          cp bestfit_1.exe IL/bestfit_1.exe
          mv bestfit_1.exe bestfit_1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bestfit_1.org /out bestfit_1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in bestfit_1.org /out bestfit_1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in bestfit_1.reflect.xml
  if [ -f bestfit_1.reflect.xml ]
then
    ReflectionRoots="-x bestfit_1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" bestfit_1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" bestfit_1.asm.exe $CLRTestExecutionArguments
