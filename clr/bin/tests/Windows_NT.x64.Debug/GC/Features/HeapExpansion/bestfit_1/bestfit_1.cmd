...
if not defined CLRTestExecutionArguments (set CLRTestExecutionArguments=1 1000 50000 )
...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\bestfit_1 bestfit_1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\HeapExpansion\bestfit_1 bestfit_1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=bestfit_1.dasm.il bestfit_1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=bestfit_1.dasm.il bestfit_1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=bestfit_1.asm.exe  bestfit_1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=bestfit_1.asm.exe  bestfit_1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=bestfit_1
    if not exist "bestfit_1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "bestfit_1.org" (
            mkdir IL
            copy bestfit_1.exe IL\bestfit_1.exe 
            ren bestfit_1.exe bestfit_1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bestfit_1.org /out bestfit_1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in bestfit_1.org /out bestfit_1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in bestfit_1.reflect.xml
    IF EXIST bestfit_1.reflect.xml set ReflectionRoots=-x bestfit_1.reflect.xml
    ...
    set ExePath=%LinkBin%\bestfit_1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% bestfit_1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% bestfit_1.asm.exe %CLRTestExecutionArguments% %Host_Args%
