...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedCollect PinnedCollect.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedCollect PinnedCollect.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=PinnedCollect.dasm.il PinnedCollect.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=PinnedCollect.dasm.il PinnedCollect.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=PinnedCollect.asm.exe  PinnedCollect.dasm.il
    %CORE_ROOT%\ilasm.exe /output=PinnedCollect.asm.exe  PinnedCollect.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=PinnedCollect
    if not exist "PinnedCollect.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "PinnedCollect.org" (
            mkdir IL
            copy PinnedCollect.exe IL\PinnedCollect.exe 
            ren PinnedCollect.exe PinnedCollect.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedCollect.org /out PinnedCollect.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedCollect.org /out PinnedCollect.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in PinnedCollect.reflect.xml
    IF EXIST PinnedCollect.reflect.xml set ReflectionRoots=-x PinnedCollect.reflect.xml
    ...
    set ExePath=%LinkBin%\PinnedCollect.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% PinnedCollect.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% PinnedCollect.asm.exe %CLRTestExecutionArguments% %Host_Args%
