...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedHandle PinnedHandle.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedHandle PinnedHandle.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=PinnedHandle.dasm.il PinnedHandle.exe
    "$CORE_ROOT/ildasm" -raweh -out=PinnedHandle.dasm.il PinnedHandle.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=PinnedHandle.asm.exe  PinnedHandle.dasm.il
    "$CORE_ROOT/ilasm" -output=PinnedHandle.asm.exe  PinnedHandle.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=PinnedHandle
    if [ ! -f PinnedHandle.org ]; then
        TakeLock
        if [ ! -f PinnedHandle.org ]; then
          mkdir IL
          cp PinnedHandle.exe IL/PinnedHandle.exe
          mv PinnedHandle.exe PinnedHandle.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedHandle.org /out PinnedHandle.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedHandle.org /out PinnedHandle.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in PinnedHandle.reflect.xml
  if [ -f PinnedHandle.reflect.xml ]
then
    ReflectionRoots="-x PinnedHandle.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/PinnedHandle.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" PinnedHandle.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" PinnedHandle.asm.exe $CLRTestExecutionArguments
