...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedMultiple PinnedMultiple.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedMultiple PinnedMultiple.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=PinnedMultiple.dasm.il PinnedMultiple.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=PinnedMultiple.dasm.il PinnedMultiple.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=PinnedMultiple.asm.exe  PinnedMultiple.dasm.il
    %CORE_ROOT%\ilasm.exe /output=PinnedMultiple.asm.exe  PinnedMultiple.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=PinnedMultiple
    if not exist "PinnedMultiple.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "PinnedMultiple.org" (
            mkdir IL
            copy PinnedMultiple.exe IL\PinnedMultiple.exe 
            ren PinnedMultiple.exe PinnedMultiple.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedMultiple.org /out PinnedMultiple.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedMultiple.org /out PinnedMultiple.exe
...
set ExePath=PinnedMultiple.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in PinnedMultiple.reflect.xml
    IF EXIST PinnedMultiple.reflect.xml set ReflectionRoots=-x PinnedMultiple.reflect.xml
    ...
    set ExePath=%LinkBin%\PinnedMultiple.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% PinnedMultiple.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% PinnedMultiple.asm.exe %CLRTestExecutionArguments% %Host_Args%
