...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedObject PinnedObject.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedObject PinnedObject.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=PinnedObject.dasm.il PinnedObject.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=PinnedObject.dasm.il PinnedObject.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=PinnedObject.asm.exe  PinnedObject.dasm.il
    %CORE_ROOT%\ilasm.exe /output=PinnedObject.asm.exe  PinnedObject.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=PinnedObject
    if not exist "PinnedObject.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "PinnedObject.org" (
            mkdir IL
            copy PinnedObject.exe IL\PinnedObject.exe 
            ren PinnedObject.exe PinnedObject.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedObject.org /out PinnedObject.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedObject.org /out PinnedObject.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in PinnedObject.reflect.xml
    IF EXIST PinnedObject.reflect.xml set ReflectionRoots=-x PinnedObject.reflect.xml
    ...
    set ExePath=%LinkBin%\PinnedObject.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% PinnedObject.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% PinnedObject.asm.exe %CLRTestExecutionArguments% %Host_Args%
