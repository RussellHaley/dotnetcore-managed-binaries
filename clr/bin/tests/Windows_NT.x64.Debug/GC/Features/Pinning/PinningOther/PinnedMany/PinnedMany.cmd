...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedMany PinnedMany.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedMany PinnedMany.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=PinnedMany.dasm.il PinnedMany.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=PinnedMany.dasm.il PinnedMany.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=PinnedMany.asm.exe  PinnedMany.dasm.il
    %CORE_ROOT%\ilasm.exe /output=PinnedMany.asm.exe  PinnedMany.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=PinnedMany
    if not exist "PinnedMany.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "PinnedMany.org" (
            mkdir IL
            copy PinnedMany.exe IL\PinnedMany.exe 
            ren PinnedMany.exe PinnedMany.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedMany.org /out PinnedMany.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedMany.org /out PinnedMany.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in PinnedMany.reflect.xml
    IF EXIST PinnedMany.reflect.xml set ReflectionRoots=-x PinnedMany.reflect.xml
    ...
    set ExePath=%LinkBin%\PinnedMany.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% PinnedMany.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% PinnedMany.asm.exe %CLRTestExecutionArguments% %Host_Args%
