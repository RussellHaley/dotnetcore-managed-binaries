...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedHandle PinnedHandle.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Pinning\PinningOther\PinnedHandle PinnedHandle.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=PinnedHandle.dasm.il PinnedHandle.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=PinnedHandle.dasm.il PinnedHandle.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=PinnedHandle.asm.exe  PinnedHandle.dasm.il
    %CORE_ROOT%\ilasm.exe /output=PinnedHandle.asm.exe  PinnedHandle.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=PinnedHandle
    if not exist "PinnedHandle.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "PinnedHandle.org" (
            mkdir IL
            copy PinnedHandle.exe IL\PinnedHandle.exe 
            ren PinnedHandle.exe PinnedHandle.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedHandle.org /out PinnedHandle.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinnedHandle.org /out PinnedHandle.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in PinnedHandle.reflect.xml
    IF EXIST PinnedHandle.reflect.xml set ReflectionRoots=-x PinnedHandle.reflect.xml
    ...
    set ExePath=%LinkBin%\PinnedHandle.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% PinnedHandle.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% PinnedHandle.asm.exe %CLRTestExecutionArguments% %Host_Args%
