...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedMany PinnedMany.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedMany PinnedMany.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=PinnedMany.dasm.il PinnedMany.exe
    "$CORE_ROOT/ildasm" -raweh -out=PinnedMany.dasm.il PinnedMany.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=PinnedMany.asm.exe  PinnedMany.dasm.il
    "$CORE_ROOT/ilasm" -output=PinnedMany.asm.exe  PinnedMany.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=PinnedMany
    if [ ! -f PinnedMany.org ]; then
        TakeLock
        if [ ! -f PinnedMany.org ]; then
          mkdir IL
          cp PinnedMany.exe IL/PinnedMany.exe
          mv PinnedMany.exe PinnedMany.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedMany.org /out PinnedMany.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedMany.org /out PinnedMany.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in PinnedMany.reflect.xml
  if [ -f PinnedMany.reflect.xml ]
then
    ReflectionRoots="-x PinnedMany.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/PinnedMany.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" PinnedMany.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" PinnedMany.asm.exe $CLRTestExecutionArguments
