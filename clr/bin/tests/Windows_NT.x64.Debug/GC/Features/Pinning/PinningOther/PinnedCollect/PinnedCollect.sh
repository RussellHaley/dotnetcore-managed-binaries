...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedCollect PinnedCollect.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedCollect PinnedCollect.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=PinnedCollect.dasm.il PinnedCollect.exe
    "$CORE_ROOT/ildasm" -raweh -out=PinnedCollect.dasm.il PinnedCollect.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=PinnedCollect.asm.exe  PinnedCollect.dasm.il
    "$CORE_ROOT/ilasm" -output=PinnedCollect.asm.exe  PinnedCollect.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=PinnedCollect
    if [ ! -f PinnedCollect.org ]; then
        TakeLock
        if [ ! -f PinnedCollect.org ]; then
          mkdir IL
          cp PinnedCollect.exe IL/PinnedCollect.exe
          mv PinnedCollect.exe PinnedCollect.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedCollect.org /out PinnedCollect.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedCollect.org /out PinnedCollect.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in PinnedCollect.reflect.xml
  if [ -f PinnedCollect.reflect.xml ]
then
    ReflectionRoots="-x PinnedCollect.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/PinnedCollect.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" PinnedCollect.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" PinnedCollect.asm.exe $CLRTestExecutionArguments
