...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedObject PinnedObject.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedObject PinnedObject.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=PinnedObject.dasm.il PinnedObject.exe
    "$CORE_ROOT/ildasm" -raweh -out=PinnedObject.dasm.il PinnedObject.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=PinnedObject.asm.exe  PinnedObject.dasm.il
    "$CORE_ROOT/ilasm" -output=PinnedObject.asm.exe  PinnedObject.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=PinnedObject
    if [ ! -f PinnedObject.org ]; then
        TakeLock
        if [ ! -f PinnedObject.org ]; then
          mkdir IL
          cp PinnedObject.exe IL/PinnedObject.exe
          mv PinnedObject.exe PinnedObject.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedObject.org /out PinnedObject.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedObject.org /out PinnedObject.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in PinnedObject.reflect.xml
  if [ -f PinnedObject.reflect.xml ]
then
    ReflectionRoots="-x PinnedObject.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/PinnedObject.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" PinnedObject.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" PinnedObject.asm.exe $CLRTestExecutionArguments
