...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedMultiple PinnedMultiple.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Pinning/PinningOther/PinnedMultiple PinnedMultiple.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=PinnedMultiple.dasm.il PinnedMultiple.exe
    "$CORE_ROOT/ildasm" -raweh -out=PinnedMultiple.dasm.il PinnedMultiple.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=PinnedMultiple.asm.exe  PinnedMultiple.dasm.il
    "$CORE_ROOT/ilasm" -output=PinnedMultiple.asm.exe  PinnedMultiple.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=PinnedMultiple
    if [ ! -f PinnedMultiple.org ]; then
        TakeLock
        if [ ! -f PinnedMultiple.org ]; then
          mkdir IL
          cp PinnedMultiple.exe IL/PinnedMultiple.exe
          mv PinnedMultiple.exe PinnedMultiple.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedMultiple.org /out PinnedMultiple.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinnedMultiple.org /out PinnedMultiple.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in PinnedMultiple.reflect.xml
  if [ -f PinnedMultiple.reflect.xml ]
then
    ReflectionRoots="-x PinnedMultiple.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/PinnedMultiple.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" PinnedMultiple.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" PinnedMultiple.asm.exe $CLRTestExecutionArguments
