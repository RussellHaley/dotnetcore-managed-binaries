...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\KeepAlive\keepaliveother\keepalivedirectedgraph keepalivedirectedgraph.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\KeepAlive\keepaliveother\keepalivedirectedgraph keepalivedirectedgraph.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=keepalivedirectedgraph.dasm.il keepalivedirectedgraph.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=keepalivedirectedgraph.dasm.il keepalivedirectedgraph.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=keepalivedirectedgraph.asm.exe  keepalivedirectedgraph.dasm.il
    %CORE_ROOT%\ilasm.exe /output=keepalivedirectedgraph.asm.exe  keepalivedirectedgraph.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=keepalivedirectedgraph
    if not exist "keepalivedirectedgraph.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "keepalivedirectedgraph.org" (
            mkdir IL
            copy keepalivedirectedgraph.exe IL\keepalivedirectedgraph.exe 
            ren keepalivedirectedgraph.exe keepalivedirectedgraph.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in keepalivedirectedgraph.org /out keepalivedirectedgraph.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in keepalivedirectedgraph.org /out keepalivedirectedgraph.exe
...
set ExePath=keepalivedirectedgraph.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in keepalivedirectedgraph.reflect.xml
    IF EXIST keepalivedirectedgraph.reflect.xml set ReflectionRoots=-x keepalivedirectedgraph.reflect.xml
    ...
    set ExePath=%LinkBin%\keepalivedirectedgraph.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% keepalivedirectedgraph.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% keepalivedirectedgraph.asm.exe %CLRTestExecutionArguments% %Host_Args%
