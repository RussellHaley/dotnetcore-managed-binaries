...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/KeepAlive/keepaliveother/keepalivetry keepalivetry.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/KeepAlive/keepaliveother/keepalivetry keepalivetry.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=keepalivetry.dasm.il keepalivetry.exe
    "$CORE_ROOT/ildasm" -raweh -out=keepalivetry.dasm.il keepalivetry.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=keepalivetry.asm.exe  keepalivetry.dasm.il
    "$CORE_ROOT/ilasm" -output=keepalivetry.asm.exe  keepalivetry.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=keepalivetry
    if [ ! -f keepalivetry.org ]; then
        TakeLock
        if [ ! -f keepalivetry.org ]; then
          mkdir IL
          cp keepalivetry.exe IL/keepalivetry.exe
          mv keepalivetry.exe keepalivetry.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in keepalivetry.org /out keepalivetry.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in keepalivetry.org /out keepalivetry.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in keepalivetry.reflect.xml
  if [ -f keepalivetry.reflect.xml ]
then
    ReflectionRoots="-x keepalivetry.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/keepalivetry.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" keepalivetry.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" keepalivetry.asm.exe $CLRTestExecutionArguments
