...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\KeepAlive\keepaliveother\keepalivetry keepalivetry.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\KeepAlive\keepaliveother\keepalivetry keepalivetry.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=keepalivetry.dasm.il keepalivetry.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=keepalivetry.dasm.il keepalivetry.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=keepalivetry.asm.exe  keepalivetry.dasm.il
    %CORE_ROOT%\ilasm.exe /output=keepalivetry.asm.exe  keepalivetry.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=keepalivetry
    if not exist "keepalivetry.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "keepalivetry.org" (
            mkdir IL
            copy keepalivetry.exe IL\keepalivetry.exe 
            ren keepalivetry.exe keepalivetry.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in keepalivetry.org /out keepalivetry.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in keepalivetry.org /out keepalivetry.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in keepalivetry.reflect.xml
    IF EXIST keepalivetry.reflect.xml set ReflectionRoots=-x keepalivetry.reflect.xml
    ...
    set ExePath=%LinkBin%\keepalivetry.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% keepalivetry.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% keepalivetry.asm.exe %CLRTestExecutionArguments% %Host_Args%
