...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/KeepAlive/keepaliveother/keepalivedirectedgraph keepalivedirectedgraph.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/KeepAlive/keepaliveother/keepalivedirectedgraph keepalivedirectedgraph.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=keepalivedirectedgraph.dasm.il keepalivedirectedgraph.exe
    "$CORE_ROOT/ildasm" -raweh -out=keepalivedirectedgraph.dasm.il keepalivedirectedgraph.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=keepalivedirectedgraph.asm.exe  keepalivedirectedgraph.dasm.il
    "$CORE_ROOT/ilasm" -output=keepalivedirectedgraph.asm.exe  keepalivedirectedgraph.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=keepalivedirectedgraph
    if [ ! -f keepalivedirectedgraph.org ]; then
        TakeLock
        if [ ! -f keepalivedirectedgraph.org ]; then
          mkdir IL
          cp keepalivedirectedgraph.exe IL/keepalivedirectedgraph.exe
          mv keepalivedirectedgraph.exe keepalivedirectedgraph.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in keepalivedirectedgraph.org /out keepalivedirectedgraph.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in keepalivedirectedgraph.org /out keepalivedirectedgraph.exe
          __cgExitCode=$?
...
ExePath=keepalivedirectedgraph.exe
...
then
  ...
  # Use hints for reflection roots, if provided in keepalivedirectedgraph.reflect.xml
  if [ -f keepalivedirectedgraph.reflect.xml ]
then
    ReflectionRoots="-x keepalivedirectedgraph.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/keepalivedirectedgraph.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" keepalivedirectedgraph.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" keepalivedirectedgraph.asm.exe $CLRTestExecutionArguments
