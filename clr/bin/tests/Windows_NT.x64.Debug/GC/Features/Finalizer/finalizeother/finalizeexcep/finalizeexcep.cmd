...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Finalizer\finalizeother\finalizeexcep finalizeexcep.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Finalizer\finalizeother\finalizeexcep finalizeexcep.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=finalizeexcep.dasm.il finalizeexcep.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=finalizeexcep.dasm.il finalizeexcep.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=finalizeexcep.asm.exe  finalizeexcep.dasm.il
    %CORE_ROOT%\ilasm.exe /output=finalizeexcep.asm.exe  finalizeexcep.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=finalizeexcep
    if not exist "finalizeexcep.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "finalizeexcep.org" (
            mkdir IL
            copy finalizeexcep.exe IL\finalizeexcep.exe 
            ren finalizeexcep.exe finalizeexcep.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in finalizeexcep.org /out finalizeexcep.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in finalizeexcep.org /out finalizeexcep.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in finalizeexcep.reflect.xml
    IF EXIST finalizeexcep.reflect.xml set ReflectionRoots=-x finalizeexcep.reflect.xml
    ...
    set ExePath=%LinkBin%\finalizeexcep.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% finalizeexcep.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% finalizeexcep.asm.exe %CLRTestExecutionArguments% %Host_Args%
