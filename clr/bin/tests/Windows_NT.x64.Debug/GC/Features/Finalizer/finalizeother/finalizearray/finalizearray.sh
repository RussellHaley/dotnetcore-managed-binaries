...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Finalizer/finalizeother/finalizearray finalizearray.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Finalizer/finalizeother/finalizearray finalizearray.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=finalizearray.dasm.il finalizearray.exe
    "$CORE_ROOT/ildasm" -raweh -out=finalizearray.dasm.il finalizearray.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=finalizearray.asm.exe  finalizearray.dasm.il
    "$CORE_ROOT/ilasm" -output=finalizearray.asm.exe  finalizearray.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=finalizearray
    if [ ! -f finalizearray.org ]; then
        TakeLock
        if [ ! -f finalizearray.org ]; then
          mkdir IL
          cp finalizearray.exe IL/finalizearray.exe
          mv finalizearray.exe finalizearray.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in finalizearray.org /out finalizearray.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in finalizearray.org /out finalizearray.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in finalizearray.reflect.xml
  if [ -f finalizearray.reflect.xml ]
then
    ReflectionRoots="-x finalizearray.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/finalizearray.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" finalizearray.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" finalizearray.asm.exe $CLRTestExecutionArguments
