...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Finalizer\finalizeother\finalizearray finalizearray.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\Features\Finalizer\finalizeother\finalizearray finalizearray.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=finalizearray.dasm.il finalizearray.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=finalizearray.dasm.il finalizearray.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=finalizearray.asm.exe  finalizearray.dasm.il
    %CORE_ROOT%\ilasm.exe /output=finalizearray.asm.exe  finalizearray.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=finalizearray
    if not exist "finalizearray.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "finalizearray.org" (
            mkdir IL
            copy finalizearray.exe IL\finalizearray.exe 
            ren finalizearray.exe finalizearray.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in finalizearray.org /out finalizearray.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in finalizearray.org /out finalizearray.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in finalizearray.reflect.xml
    IF EXIST finalizearray.reflect.xml set ReflectionRoots=-x finalizearray.reflect.xml
    ...
    set ExePath=%LinkBin%\finalizearray.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% finalizearray.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% finalizearray.asm.exe %CLRTestExecutionArguments% %Host_Args%
