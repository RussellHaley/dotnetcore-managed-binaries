...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Finalizer/finalizeother/finalizeexcep finalizeexcep.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/Features/Finalizer/finalizeother/finalizeexcep finalizeexcep.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=finalizeexcep.dasm.il finalizeexcep.exe
    "$CORE_ROOT/ildasm" -raweh -out=finalizeexcep.dasm.il finalizeexcep.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=finalizeexcep.asm.exe  finalizeexcep.dasm.il
    "$CORE_ROOT/ilasm" -output=finalizeexcep.asm.exe  finalizeexcep.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=finalizeexcep
    if [ ! -f finalizeexcep.org ]; then
        TakeLock
        if [ ! -f finalizeexcep.org ]; then
          mkdir IL
          cp finalizeexcep.exe IL/finalizeexcep.exe
          mv finalizeexcep.exe finalizeexcep.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in finalizeexcep.org /out finalizeexcep.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in finalizeexcep.org /out finalizeexcep.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in finalizeexcep.reflect.xml
  if [ -f finalizeexcep.reflect.xml ]
then
    ReflectionRoots="-x finalizeexcep.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/finalizeexcep.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" finalizeexcep.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" finalizeexcep.asm.exe $CLRTestExecutionArguments
