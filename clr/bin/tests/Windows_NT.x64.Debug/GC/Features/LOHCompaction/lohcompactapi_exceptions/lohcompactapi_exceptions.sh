...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/LOHCompaction/lohcompactapi_exceptions lohcompactapi_exceptions.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/LOHCompaction/lohcompactapi_exceptions lohcompactapi_exceptions.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=lohcompactapi_exceptions.dasm.il lohcompactapi_exceptions.exe
    "$CORE_ROOT/ildasm" -raweh -out=lohcompactapi_exceptions.dasm.il lohcompactapi_exceptions.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=lohcompactapi_exceptions.asm.exe  lohcompactapi_exceptions.dasm.il
    "$CORE_ROOT/ilasm" -output=lohcompactapi_exceptions.asm.exe  lohcompactapi_exceptions.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=lohcompactapi_exceptions
    if [ ! -f lohcompactapi_exceptions.org ]; then
        TakeLock
        if [ ! -f lohcompactapi_exceptions.org ]; then
          mkdir IL
          cp lohcompactapi_exceptions.exe IL/lohcompactapi_exceptions.exe
          mv lohcompactapi_exceptions.exe lohcompactapi_exceptions.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in lohcompactapi_exceptions.org /out lohcompactapi_exceptions.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in lohcompactapi_exceptions.org /out lohcompactapi_exceptions.exe
          __cgExitCode=$?
...
ExePath=lohcompactapi_exceptions.exe
...
then
  ...
  # Use hints for reflection roots, if provided in lohcompactapi_exceptions.reflect.xml
  if [ -f lohcompactapi_exceptions.reflect.xml ]
then
    ReflectionRoots="-x lohcompactapi_exceptions.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/lohcompactapi_exceptions.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" lohcompactapi_exceptions.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" lohcompactapi_exceptions.asm.exe $CLRTestExecutionArguments
