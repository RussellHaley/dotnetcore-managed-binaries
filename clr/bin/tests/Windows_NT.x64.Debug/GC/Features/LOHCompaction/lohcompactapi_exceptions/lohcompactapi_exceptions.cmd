...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\LOHCompaction\lohcompactapi_exceptions lohcompactapi_exceptions.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\LOHCompaction\lohcompactapi_exceptions lohcompactapi_exceptions.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=lohcompactapi_exceptions.dasm.il lohcompactapi_exceptions.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=lohcompactapi_exceptions.dasm.il lohcompactapi_exceptions.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=lohcompactapi_exceptions.asm.exe  lohcompactapi_exceptions.dasm.il
    %CORE_ROOT%\ilasm.exe /output=lohcompactapi_exceptions.asm.exe  lohcompactapi_exceptions.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=lohcompactapi_exceptions
    if not exist "lohcompactapi_exceptions.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "lohcompactapi_exceptions.org" (
            mkdir IL
            copy lohcompactapi_exceptions.exe IL\lohcompactapi_exceptions.exe 
            ren lohcompactapi_exceptions.exe lohcompactapi_exceptions.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in lohcompactapi_exceptions.org /out lohcompactapi_exceptions.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in lohcompactapi_exceptions.org /out lohcompactapi_exceptions.exe
...
set ExePath=lohcompactapi_exceptions.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in lohcompactapi_exceptions.reflect.xml
    IF EXIST lohcompactapi_exceptions.reflect.xml set ReflectionRoots=-x lohcompactapi_exceptions.reflect.xml
    ...
    set ExePath=%LinkBin%\lohcompactapi_exceptions.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% lohcompactapi_exceptions.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% lohcompactapi_exceptions.asm.exe %CLRTestExecutionArguments% %Host_Args%
