...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/LOHCompaction/lohcompactapi lohcompactapi.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/LOHCompaction/lohcompactapi lohcompactapi.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=lohcompactapi.dasm.il lohcompactapi.exe
    "$CORE_ROOT/ildasm" -raweh -out=lohcompactapi.dasm.il lohcompactapi.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=lohcompactapi.asm.exe  lohcompactapi.dasm.il
    "$CORE_ROOT/ilasm" -output=lohcompactapi.asm.exe  lohcompactapi.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=lohcompactapi
    if [ ! -f lohcompactapi.org ]; then
        TakeLock
        if [ ! -f lohcompactapi.org ]; then
          mkdir IL
          cp lohcompactapi.exe IL/lohcompactapi.exe
          mv lohcompactapi.exe lohcompactapi.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in lohcompactapi.org /out lohcompactapi.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in lohcompactapi.org /out lohcompactapi.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in lohcompactapi.reflect.xml
  if [ -f lohcompactapi.reflect.xml ]
then
    ReflectionRoots="-x lohcompactapi.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/lohcompactapi.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" lohcompactapi.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" lohcompactapi.asm.exe $CLRTestExecutionArguments
