...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\LOHCompaction\lohcompactapi lohcompactapi.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\LOHCompaction\lohcompactapi lohcompactapi.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=lohcompactapi.dasm.il lohcompactapi.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=lohcompactapi.dasm.il lohcompactapi.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=lohcompactapi.asm.exe  lohcompactapi.dasm.il
    %CORE_ROOT%\ilasm.exe /output=lohcompactapi.asm.exe  lohcompactapi.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=lohcompactapi
    if not exist "lohcompactapi.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "lohcompactapi.org" (
            mkdir IL
            copy lohcompactapi.exe IL\lohcompactapi.exe 
            ren lohcompactapi.exe lohcompactapi.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in lohcompactapi.org /out lohcompactapi.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in lohcompactapi.org /out lohcompactapi.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in lohcompactapi.reflect.xml
    IF EXIST lohcompactapi.reflect.xml set ReflectionRoots=-x lohcompactapi.reflect.xml
    ...
    set ExePath=%LinkBin%\lohcompactapi.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% lohcompactapi.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% lohcompactapi.asm.exe %CLRTestExecutionArguments% %Host_Args%
