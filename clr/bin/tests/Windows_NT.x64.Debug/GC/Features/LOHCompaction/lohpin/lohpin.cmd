...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\LOHCompaction\lohpin lohpin.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Features\LOHCompaction\lohpin lohpin.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=lohpin.dasm.il lohpin.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=lohpin.dasm.il lohpin.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=lohpin.asm.exe  lohpin.dasm.il
    %CORE_ROOT%\ilasm.exe /output=lohpin.asm.exe  lohpin.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=lohpin
    if not exist "lohpin.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "lohpin.org" (
            mkdir IL
            copy lohpin.exe IL\lohpin.exe 
            ren lohpin.exe lohpin.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in lohpin.org /out lohpin.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in lohpin.org /out lohpin.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in lohpin.reflect.xml
    IF EXIST lohpin.reflect.xml set ReflectionRoots=-x lohpin.reflect.xml
    ...
    set ExePath=%LinkBin%\lohpin.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% lohpin.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% lohpin.asm.exe %CLRTestExecutionArguments% %Host_Args%
