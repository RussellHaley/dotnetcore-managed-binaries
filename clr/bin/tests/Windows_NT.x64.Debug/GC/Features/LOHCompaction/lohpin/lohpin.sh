...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/LOHCompaction/lohpin lohpin.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Features/LOHCompaction/lohpin lohpin.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=lohpin.dasm.il lohpin.exe
    "$CORE_ROOT/ildasm" -raweh -out=lohpin.dasm.il lohpin.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=lohpin.asm.exe  lohpin.dasm.il
    "$CORE_ROOT/ilasm" -output=lohpin.asm.exe  lohpin.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=lohpin
    if [ ! -f lohpin.org ]; then
        TakeLock
        if [ ! -f lohpin.org ]; then
          mkdir IL
          cp lohpin.exe IL/lohpin.exe
          mv lohpin.exe lohpin.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in lohpin.org /out lohpin.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in lohpin.org /out lohpin.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in lohpin.reflect.xml
  if [ -f lohpin.reflect.xml ]
then
    ReflectionRoots="-x lohpin.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" lohpin.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" lohpin.asm.exe $CLRTestExecutionArguments
