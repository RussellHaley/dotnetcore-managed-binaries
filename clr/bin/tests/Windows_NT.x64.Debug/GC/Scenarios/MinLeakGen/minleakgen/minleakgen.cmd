...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\MinLeakGen\minleakgen minleakgen.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\MinLeakGen\minleakgen minleakgen.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=minleakgen.dasm.il minleakgen.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=minleakgen.dasm.il minleakgen.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=minleakgen.asm.exe  minleakgen.dasm.il
    %CORE_ROOT%\ilasm.exe /output=minleakgen.asm.exe  minleakgen.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=minleakgen
    if not exist "minleakgen.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "minleakgen.org" (
            mkdir IL
            copy minleakgen.exe IL\minleakgen.exe 
            ren minleakgen.exe minleakgen.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in minleakgen.org /out minleakgen.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in minleakgen.org /out minleakgen.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in minleakgen.reflect.xml
    IF EXIST minleakgen.reflect.xml set ReflectionRoots=-x minleakgen.reflect.xml
    ...
    set ExePath=%LinkBin%\minleakgen.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% minleakgen.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% minleakgen.asm.exe %CLRTestExecutionArguments% %Host_Args%
