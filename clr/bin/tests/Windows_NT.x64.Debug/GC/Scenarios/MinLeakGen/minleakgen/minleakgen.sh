...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/MinLeakGen/minleakgen minleakgen.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/MinLeakGen/minleakgen minleakgen.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=minleakgen.dasm.il minleakgen.exe
    "$CORE_ROOT/ildasm" -raweh -out=minleakgen.dasm.il minleakgen.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=minleakgen.asm.exe  minleakgen.dasm.il
    "$CORE_ROOT/ilasm" -output=minleakgen.asm.exe  minleakgen.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=minleakgen
    if [ ! -f minleakgen.org ]; then
        TakeLock
        if [ ! -f minleakgen.org ]; then
          mkdir IL
          cp minleakgen.exe IL/minleakgen.exe
          mv minleakgen.exe minleakgen.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in minleakgen.org /out minleakgen.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in minleakgen.org /out minleakgen.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in minleakgen.reflect.xml
  if [ -f minleakgen.reflect.xml ]
then
    ReflectionRoots="-x minleakgen.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/minleakgen.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" minleakgen.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" minleakgen.asm.exe $CLRTestExecutionArguments
