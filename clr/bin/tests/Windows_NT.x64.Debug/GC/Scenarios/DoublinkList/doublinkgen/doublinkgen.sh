...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/doublinkgen doublinkgen.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/doublinkgen doublinkgen.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=doublinkgen.dasm.il doublinkgen.exe
    "$CORE_ROOT/ildasm" -raweh -out=doublinkgen.dasm.il doublinkgen.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=doublinkgen.asm.exe  doublinkgen.dasm.il
    "$CORE_ROOT/ilasm" -output=doublinkgen.asm.exe  doublinkgen.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=doublinkgen
    if [ ! -f doublinkgen.org ]; then
        TakeLock
        if [ ! -f doublinkgen.org ]; then
          mkdir IL
          cp doublinkgen.exe IL/doublinkgen.exe
          mv doublinkgen.exe doublinkgen.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublinkgen.org /out doublinkgen.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublinkgen.org /out doublinkgen.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in doublinkgen.reflect.xml
  if [ -f doublinkgen.reflect.xml ]
then
    ReflectionRoots="-x doublinkgen.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/doublinkgen.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" doublinkgen.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" doublinkgen.asm.exe $CLRTestExecutionArguments
