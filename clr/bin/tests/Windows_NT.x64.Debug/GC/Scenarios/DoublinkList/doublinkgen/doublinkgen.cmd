...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\doublinkgen doublinkgen.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\doublinkgen doublinkgen.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=doublinkgen.dasm.il doublinkgen.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=doublinkgen.dasm.il doublinkgen.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=doublinkgen.asm.exe  doublinkgen.dasm.il
    %CORE_ROOT%\ilasm.exe /output=doublinkgen.asm.exe  doublinkgen.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=doublinkgen
    if not exist "doublinkgen.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "doublinkgen.org" (
            mkdir IL
            copy doublinkgen.exe IL\doublinkgen.exe 
            ren doublinkgen.exe doublinkgen.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublinkgen.org /out doublinkgen.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublinkgen.org /out doublinkgen.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in doublinkgen.reflect.xml
    IF EXIST doublinkgen.reflect.xml set ReflectionRoots=-x doublinkgen.reflect.xml
    ...
    set ExePath=%LinkBin%\doublinkgen.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% doublinkgen.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% doublinkgen.asm.exe %CLRTestExecutionArguments% %Host_Args%
