...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\dlcollect dlcollect.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\dlcollect dlcollect.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=dlcollect.dasm.il dlcollect.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=dlcollect.dasm.il dlcollect.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=dlcollect.asm.exe  dlcollect.dasm.il
    %CORE_ROOT%\ilasm.exe /output=dlcollect.asm.exe  dlcollect.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=dlcollect
    if not exist "dlcollect.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "dlcollect.org" (
            mkdir IL
            copy dlcollect.exe IL\dlcollect.exe 
            ren dlcollect.exe dlcollect.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dlcollect.org /out dlcollect.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dlcollect.org /out dlcollect.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in dlcollect.reflect.xml
    IF EXIST dlcollect.reflect.xml set ReflectionRoots=-x dlcollect.reflect.xml
    ...
    set ExePath=%LinkBin%\dlcollect.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% dlcollect.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% dlcollect.asm.exe %CLRTestExecutionArguments% %Host_Args%
