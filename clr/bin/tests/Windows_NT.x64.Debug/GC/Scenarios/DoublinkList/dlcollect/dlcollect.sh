...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/dlcollect dlcollect.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/dlcollect dlcollect.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=dlcollect.dasm.il dlcollect.exe
    "$CORE_ROOT/ildasm" -raweh -out=dlcollect.dasm.il dlcollect.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=dlcollect.asm.exe  dlcollect.dasm.il
    "$CORE_ROOT/ilasm" -output=dlcollect.asm.exe  dlcollect.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=dlcollect
    if [ ! -f dlcollect.org ]; then
        TakeLock
        if [ ! -f dlcollect.org ]; then
          mkdir IL
          cp dlcollect.exe IL/dlcollect.exe
          mv dlcollect.exe dlcollect.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dlcollect.org /out dlcollect.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dlcollect.org /out dlcollect.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in dlcollect.reflect.xml
  if [ -f dlcollect.reflect.xml ]
then
    ReflectionRoots="-x dlcollect.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" dlcollect.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" dlcollect.asm.exe $CLRTestExecutionArguments
