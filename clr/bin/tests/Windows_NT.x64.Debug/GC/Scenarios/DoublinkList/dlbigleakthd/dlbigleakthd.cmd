...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\dlbigleakthd dlbigleakthd.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\dlbigleakthd dlbigleakthd.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=dlbigleakthd.dasm.il dlbigleakthd.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=dlbigleakthd.dasm.il dlbigleakthd.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=dlbigleakthd.asm.exe  dlbigleakthd.dasm.il
    %CORE_ROOT%\ilasm.exe /output=dlbigleakthd.asm.exe  dlbigleakthd.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=dlbigleakthd
    if not exist "dlbigleakthd.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "dlbigleakthd.org" (
            mkdir IL
            copy dlbigleakthd.exe IL\dlbigleakthd.exe 
            ren dlbigleakthd.exe dlbigleakthd.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dlbigleakthd.org /out dlbigleakthd.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dlbigleakthd.org /out dlbigleakthd.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in dlbigleakthd.reflect.xml
    IF EXIST dlbigleakthd.reflect.xml set ReflectionRoots=-x dlbigleakthd.reflect.xml
    ...
    set ExePath=%LinkBin%\dlbigleakthd.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% dlbigleakthd.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% dlbigleakthd.asm.exe %CLRTestExecutionArguments% %Host_Args%
