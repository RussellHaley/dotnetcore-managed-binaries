...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/dlbigleakthd dlbigleakthd.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/dlbigleakthd dlbigleakthd.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=dlbigleakthd.dasm.il dlbigleakthd.exe
    "$CORE_ROOT/ildasm" -raweh -out=dlbigleakthd.dasm.il dlbigleakthd.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=dlbigleakthd.asm.exe  dlbigleakthd.dasm.il
    "$CORE_ROOT/ilasm" -output=dlbigleakthd.asm.exe  dlbigleakthd.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=dlbigleakthd
    if [ ! -f dlbigleakthd.org ]; then
        TakeLock
        if [ ! -f dlbigleakthd.org ]; then
          mkdir IL
          cp dlbigleakthd.exe IL/dlbigleakthd.exe
          mv dlbigleakthd.exe dlbigleakthd.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dlbigleakthd.org /out dlbigleakthd.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dlbigleakthd.org /out dlbigleakthd.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in dlbigleakthd.reflect.xml
  if [ -f dlbigleakthd.reflect.xml ]
then
    ReflectionRoots="-x dlbigleakthd.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/dlbigleakthd.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" dlbigleakthd.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" dlbigleakthd.asm.exe $CLRTestExecutionArguments
