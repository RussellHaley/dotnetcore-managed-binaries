...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\doublinkstay doublinkstay.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\doublinkstay doublinkstay.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=doublinkstay.dasm.il doublinkstay.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=doublinkstay.dasm.il doublinkstay.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=doublinkstay.asm.exe  doublinkstay.dasm.il
    %CORE_ROOT%\ilasm.exe /output=doublinkstay.asm.exe  doublinkstay.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=doublinkstay
    if not exist "doublinkstay.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "doublinkstay.org" (
            mkdir IL
            copy doublinkstay.exe IL\doublinkstay.exe 
            ren doublinkstay.exe doublinkstay.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublinkstay.org /out doublinkstay.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublinkstay.org /out doublinkstay.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in doublinkstay.reflect.xml
    IF EXIST doublinkstay.reflect.xml set ReflectionRoots=-x doublinkstay.reflect.xml
    ...
    set ExePath=%LinkBin%\doublinkstay.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% doublinkstay.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% doublinkstay.asm.exe %CLRTestExecutionArguments% %Host_Args%
