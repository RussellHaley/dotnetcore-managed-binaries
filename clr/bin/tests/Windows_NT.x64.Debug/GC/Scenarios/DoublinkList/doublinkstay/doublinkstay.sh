...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/doublinkstay doublinkstay.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/doublinkstay doublinkstay.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=doublinkstay.dasm.il doublinkstay.exe
    "$CORE_ROOT/ildasm" -raweh -out=doublinkstay.dasm.il doublinkstay.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=doublinkstay.asm.exe  doublinkstay.dasm.il
    "$CORE_ROOT/ilasm" -output=doublinkstay.asm.exe  doublinkstay.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=doublinkstay
    if [ ! -f doublinkstay.org ]; then
        TakeLock
        if [ ! -f doublinkstay.org ]; then
          mkdir IL
          cp doublinkstay.exe IL/doublinkstay.exe
          mv doublinkstay.exe doublinkstay.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublinkstay.org /out doublinkstay.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublinkstay.org /out doublinkstay.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in doublinkstay.reflect.xml
  if [ -f doublinkstay.reflect.xml ]
then
    ReflectionRoots="-x doublinkstay.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/doublinkstay.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" doublinkstay.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" doublinkstay.asm.exe $CLRTestExecutionArguments
