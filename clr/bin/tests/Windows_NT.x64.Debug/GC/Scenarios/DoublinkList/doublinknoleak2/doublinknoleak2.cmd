...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\doublinknoleak2 doublinknoleak2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\doublinknoleak2 doublinknoleak2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=doublinknoleak2.dasm.il doublinknoleak2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=doublinknoleak2.dasm.il doublinknoleak2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=doublinknoleak2.asm.exe  doublinknoleak2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=doublinknoleak2.asm.exe  doublinknoleak2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=doublinknoleak2
    if not exist "doublinknoleak2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "doublinknoleak2.org" (
            mkdir IL
            copy doublinknoleak2.exe IL\doublinknoleak2.exe 
            ren doublinknoleak2.exe doublinknoleak2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublinknoleak2.org /out doublinknoleak2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in doublinknoleak2.org /out doublinknoleak2.exe
...
set ExePath=doublinknoleak2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in doublinknoleak2.reflect.xml
    IF EXIST doublinknoleak2.reflect.xml set ReflectionRoots=-x doublinknoleak2.reflect.xml
    ...
    set ExePath=%LinkBin%\doublinknoleak2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% doublinknoleak2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% doublinknoleak2.asm.exe %CLRTestExecutionArguments% %Host_Args%
