...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/doublinknoleak2 doublinknoleak2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/doublinknoleak2 doublinknoleak2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=doublinknoleak2.dasm.il doublinknoleak2.exe
    "$CORE_ROOT/ildasm" -raweh -out=doublinknoleak2.dasm.il doublinknoleak2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=doublinknoleak2.asm.exe  doublinknoleak2.dasm.il
    "$CORE_ROOT/ilasm" -output=doublinknoleak2.asm.exe  doublinknoleak2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=doublinknoleak2
    if [ ! -f doublinknoleak2.org ]; then
        TakeLock
        if [ ! -f doublinknoleak2.org ]; then
          mkdir IL
          cp doublinknoleak2.exe IL/doublinknoleak2.exe
          mv doublinknoleak2.exe doublinknoleak2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublinknoleak2.org /out doublinknoleak2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in doublinknoleak2.org /out doublinknoleak2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in doublinknoleak2.reflect.xml
  if [ -f doublinknoleak2.reflect.xml ]
then
    ReflectionRoots="-x doublinknoleak2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/doublinknoleak2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" doublinknoleak2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" doublinknoleak2.asm.exe $CLRTestExecutionArguments
