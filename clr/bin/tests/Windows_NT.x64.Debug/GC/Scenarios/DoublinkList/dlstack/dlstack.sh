...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/dlstack dlstack.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/DoublinkList/dlstack dlstack.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=dlstack.dasm.il dlstack.exe
    "$CORE_ROOT/ildasm" -raweh -out=dlstack.dasm.il dlstack.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=dlstack.asm.exe  dlstack.dasm.il
    "$CORE_ROOT/ilasm" -output=dlstack.asm.exe  dlstack.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=dlstack
    if [ ! -f dlstack.org ]; then
        TakeLock
        if [ ! -f dlstack.org ]; then
          mkdir IL
          cp dlstack.exe IL/dlstack.exe
          mv dlstack.exe dlstack.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dlstack.org /out dlstack.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dlstack.org /out dlstack.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in dlstack.reflect.xml
  if [ -f dlstack.reflect.xml ]
then
    ReflectionRoots="-x dlstack.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" dlstack.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" dlstack.asm.exe $CLRTestExecutionArguments
