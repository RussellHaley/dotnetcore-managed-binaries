...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\dlstack dlstack.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\DoublinkList\dlstack dlstack.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=dlstack.dasm.il dlstack.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=dlstack.dasm.il dlstack.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=dlstack.asm.exe  dlstack.dasm.il
    %CORE_ROOT%\ilasm.exe /output=dlstack.asm.exe  dlstack.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=dlstack
    if not exist "dlstack.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "dlstack.org" (
            mkdir IL
            copy dlstack.exe IL\dlstack.exe 
            ren dlstack.exe dlstack.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dlstack.org /out dlstack.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dlstack.org /out dlstack.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in dlstack.reflect.xml
    IF EXIST dlstack.reflect.xml set ReflectionRoots=-x dlstack.reflect.xml
    ...
    set ExePath=%LinkBin%\dlstack.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% dlstack.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% dlstack.asm.exe %CLRTestExecutionArguments% %Host_Args%
