...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\GCStress\gcstress gcstress.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\GCStress\gcstress gcstress.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gcstress.dasm.il gcstress.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gcstress.dasm.il gcstress.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gcstress.asm.exe  gcstress.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gcstress.asm.exe  gcstress.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gcstress
    if not exist "gcstress.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gcstress.org" (
            mkdir IL
            copy gcstress.exe IL\gcstress.exe 
            ren gcstress.exe gcstress.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcstress.org /out gcstress.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcstress.org /out gcstress.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gcstress.reflect.xml
    IF EXIST gcstress.reflect.xml set ReflectionRoots=-x gcstress.reflect.xml
    ...
    set ExePath=%LinkBin%\gcstress.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gcstress.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gcstress.asm.exe %CLRTestExecutionArguments% %Host_Args%
