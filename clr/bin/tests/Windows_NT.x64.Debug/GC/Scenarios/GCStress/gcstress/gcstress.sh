...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/GCStress/gcstress gcstress.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/GCStress/gcstress gcstress.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gcstress.dasm.il gcstress.exe
    "$CORE_ROOT/ildasm" -raweh -out=gcstress.dasm.il gcstress.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gcstress.asm.exe  gcstress.dasm.il
    "$CORE_ROOT/ilasm" -output=gcstress.asm.exe  gcstress.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gcstress
    if [ ! -f gcstress.org ]; then
        TakeLock
        if [ ! -f gcstress.org ]; then
          mkdir IL
          cp gcstress.exe IL/gcstress.exe
          mv gcstress.exe gcstress.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcstress.org /out gcstress.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcstress.org /out gcstress.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gcstress.reflect.xml
  if [ -f gcstress.reflect.xml ]
then
    ReflectionRoots="-x gcstress.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gcstress.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gcstress.asm.exe $CLRTestExecutionArguments
