...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/GCBench/gcbench gcbench.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/GCBench/gcbench gcbench.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gcbench.dasm.il gcbench.exe
    "$CORE_ROOT/ildasm" -raweh -out=gcbench.dasm.il gcbench.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gcbench.asm.exe  gcbench.dasm.il
    "$CORE_ROOT/ilasm" -output=gcbench.asm.exe  gcbench.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gcbench
    if [ ! -f gcbench.org ]; then
        TakeLock
        if [ ! -f gcbench.org ]; then
          mkdir IL
          cp gcbench.exe IL/gcbench.exe
          mv gcbench.exe gcbench.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcbench.org /out gcbench.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcbench.org /out gcbench.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gcbench.reflect.xml
  if [ -f gcbench.reflect.xml ]
then
    ReflectionRoots="-x gcbench.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gcbench.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gcbench.asm.exe $CLRTestExecutionArguments
