...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\GCBench\gcbench gcbench.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\GCBench\gcbench gcbench.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gcbench.dasm.il gcbench.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gcbench.dasm.il gcbench.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gcbench.asm.exe  gcbench.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gcbench.asm.exe  gcbench.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gcbench
    if not exist "gcbench.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gcbench.org" (
            mkdir IL
            copy gcbench.exe IL\gcbench.exe 
            ren gcbench.exe gcbench.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcbench.org /out gcbench.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcbench.org /out gcbench.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gcbench.reflect.xml
    IF EXIST gcbench.reflect.xml set ReflectionRoots=-x gcbench.reflect.xml
    ...
    set ExePath=%LinkBin%\gcbench.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gcbench.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gcbench.asm.exe %CLRTestExecutionArguments% %Host_Args%
