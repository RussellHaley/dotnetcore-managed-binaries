...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/gcvariant2 gcvariant2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/gcvariant2 gcvariant2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gcvariant2.dasm.il gcvariant2.exe
    "$CORE_ROOT/ildasm" -raweh -out=gcvariant2.dasm.il gcvariant2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gcvariant2.asm.exe  gcvariant2.dasm.il
    "$CORE_ROOT/ilasm" -output=gcvariant2.asm.exe  gcvariant2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gcvariant2
    if [ ! -f gcvariant2.org ]; then
        TakeLock
        if [ ! -f gcvariant2.org ]; then
          mkdir IL
          cp gcvariant2.exe IL/gcvariant2.exe
          mv gcvariant2.exe gcvariant2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcvariant2.org /out gcvariant2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcvariant2.org /out gcvariant2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gcvariant2.reflect.xml
  if [ -f gcvariant2.reflect.xml ]
then
    ReflectionRoots="-x gcvariant2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/gcvariant2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gcvariant2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gcvariant2.asm.exe $CLRTestExecutionArguments
