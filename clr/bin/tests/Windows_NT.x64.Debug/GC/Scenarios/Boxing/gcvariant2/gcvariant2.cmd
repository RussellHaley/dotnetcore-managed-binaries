...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\gcvariant2 gcvariant2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\gcvariant2 gcvariant2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gcvariant2.dasm.il gcvariant2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gcvariant2.dasm.il gcvariant2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gcvariant2.asm.exe  gcvariant2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gcvariant2.asm.exe  gcvariant2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gcvariant2
    if not exist "gcvariant2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gcvariant2.org" (
            mkdir IL
            copy gcvariant2.exe IL\gcvariant2.exe 
            ren gcvariant2.exe gcvariant2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcvariant2.org /out gcvariant2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcvariant2.org /out gcvariant2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gcvariant2.reflect.xml
    IF EXIST gcvariant2.reflect.xml set ReflectionRoots=-x gcvariant2.reflect.xml
    ...
    set ExePath=%LinkBin%\gcvariant2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gcvariant2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gcvariant2.asm.exe %CLRTestExecutionArguments% %Host_Args%
