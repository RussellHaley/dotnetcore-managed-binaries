...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\gcvariant4 gcvariant4.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\gcvariant4 gcvariant4.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gcvariant4.dasm.il gcvariant4.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gcvariant4.dasm.il gcvariant4.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gcvariant4.asm.exe  gcvariant4.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gcvariant4.asm.exe  gcvariant4.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gcvariant4
    if not exist "gcvariant4.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gcvariant4.org" (
            mkdir IL
            copy gcvariant4.exe IL\gcvariant4.exe 
            ren gcvariant4.exe gcvariant4.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcvariant4.org /out gcvariant4.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcvariant4.org /out gcvariant4.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gcvariant4.reflect.xml
    IF EXIST gcvariant4.reflect.xml set ReflectionRoots=-x gcvariant4.reflect.xml
    ...
    set ExePath=%LinkBin%\gcvariant4.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gcvariant4.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gcvariant4.asm.exe %CLRTestExecutionArguments% %Host_Args%
