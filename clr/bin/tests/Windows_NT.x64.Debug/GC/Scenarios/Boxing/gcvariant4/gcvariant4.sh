...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/gcvariant4 gcvariant4.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/gcvariant4 gcvariant4.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gcvariant4.dasm.il gcvariant4.exe
    "$CORE_ROOT/ildasm" -raweh -out=gcvariant4.dasm.il gcvariant4.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gcvariant4.asm.exe  gcvariant4.dasm.il
    "$CORE_ROOT/ilasm" -output=gcvariant4.asm.exe  gcvariant4.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gcvariant4
    if [ ! -f gcvariant4.org ]; then
        TakeLock
        if [ ! -f gcvariant4.org ]; then
          mkdir IL
          cp gcvariant4.exe IL/gcvariant4.exe
          mv gcvariant4.exe gcvariant4.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcvariant4.org /out gcvariant4.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcvariant4.org /out gcvariant4.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gcvariant4.reflect.xml
  if [ -f gcvariant4.reflect.xml ]
then
    ReflectionRoots="-x gcvariant4.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/gcvariant4.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gcvariant4.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gcvariant4.asm.exe $CLRTestExecutionArguments
