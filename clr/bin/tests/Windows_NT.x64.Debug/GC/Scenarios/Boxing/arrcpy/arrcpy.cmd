...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\arrcpy arrcpy.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\arrcpy arrcpy.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=arrcpy.dasm.il arrcpy.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=arrcpy.dasm.il arrcpy.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=arrcpy.asm.exe  arrcpy.dasm.il
    %CORE_ROOT%\ilasm.exe /output=arrcpy.asm.exe  arrcpy.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=arrcpy
    if not exist "arrcpy.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "arrcpy.org" (
            mkdir IL
            copy arrcpy.exe IL\arrcpy.exe 
            ren arrcpy.exe arrcpy.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in arrcpy.org /out arrcpy.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in arrcpy.org /out arrcpy.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in arrcpy.reflect.xml
    IF EXIST arrcpy.reflect.xml set ReflectionRoots=-x arrcpy.reflect.xml
    ...
    set ExePath=%LinkBin%\arrcpy.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% arrcpy.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% arrcpy.asm.exe %CLRTestExecutionArguments% %Host_Args%
