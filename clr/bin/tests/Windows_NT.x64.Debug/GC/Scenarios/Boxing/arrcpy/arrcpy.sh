...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/arrcpy arrcpy.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/arrcpy arrcpy.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=arrcpy.dasm.il arrcpy.exe
    "$CORE_ROOT/ildasm" -raweh -out=arrcpy.dasm.il arrcpy.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=arrcpy.asm.exe  arrcpy.dasm.il
    "$CORE_ROOT/ilasm" -output=arrcpy.asm.exe  arrcpy.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=arrcpy
    if [ ! -f arrcpy.org ]; then
        TakeLock
        if [ ! -f arrcpy.org ]; then
          mkdir IL
          cp arrcpy.exe IL/arrcpy.exe
          mv arrcpy.exe arrcpy.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in arrcpy.org /out arrcpy.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in arrcpy.org /out arrcpy.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in arrcpy.reflect.xml
  if [ -f arrcpy.reflect.xml ]
then
    ReflectionRoots="-x arrcpy.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" arrcpy.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" arrcpy.asm.exe $CLRTestExecutionArguments
