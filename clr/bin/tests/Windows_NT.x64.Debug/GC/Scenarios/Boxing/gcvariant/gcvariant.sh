...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/gcvariant gcvariant.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/gcvariant gcvariant.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gcvariant.dasm.il gcvariant.exe
    "$CORE_ROOT/ildasm" -raweh -out=gcvariant.dasm.il gcvariant.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gcvariant.asm.exe  gcvariant.dasm.il
    "$CORE_ROOT/ilasm" -output=gcvariant.asm.exe  gcvariant.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gcvariant
    if [ ! -f gcvariant.org ]; then
        TakeLock
        if [ ! -f gcvariant.org ]; then
          mkdir IL
          cp gcvariant.exe IL/gcvariant.exe
          mv gcvariant.exe gcvariant.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcvariant.org /out gcvariant.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcvariant.org /out gcvariant.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gcvariant.reflect.xml
  if [ -f gcvariant.reflect.xml ]
then
    ReflectionRoots="-x gcvariant.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gcvariant.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gcvariant.asm.exe $CLRTestExecutionArguments
