...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\gcvariant gcvariant.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\gcvariant gcvariant.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gcvariant.dasm.il gcvariant.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gcvariant.dasm.il gcvariant.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gcvariant.asm.exe  gcvariant.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gcvariant.asm.exe  gcvariant.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gcvariant
    if not exist "gcvariant.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gcvariant.org" (
            mkdir IL
            copy gcvariant.exe IL\gcvariant.exe 
            ren gcvariant.exe gcvariant.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcvariant.org /out gcvariant.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcvariant.org /out gcvariant.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gcvariant.reflect.xml
    IF EXIST gcvariant.reflect.xml set ReflectionRoots=-x gcvariant.reflect.xml
    ...
    set ExePath=%LinkBin%\gcvariant.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gcvariant.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gcvariant.asm.exe %CLRTestExecutionArguments% %Host_Args%
