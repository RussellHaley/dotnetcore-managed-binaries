...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\variantint variantint.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\variantint variantint.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=variantint.dasm.il variantint.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=variantint.dasm.il variantint.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=variantint.asm.exe  variantint.dasm.il
    %CORE_ROOT%\ilasm.exe /output=variantint.asm.exe  variantint.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=variantint
    if not exist "variantint.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "variantint.org" (
            mkdir IL
            copy variantint.exe IL\variantint.exe 
            ren variantint.exe variantint.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in variantint.org /out variantint.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in variantint.org /out variantint.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in variantint.reflect.xml
    IF EXIST variantint.reflect.xml set ReflectionRoots=-x variantint.reflect.xml
    ...
    set ExePath=%LinkBin%\variantint.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% variantint.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% variantint.asm.exe %CLRTestExecutionArguments% %Host_Args%
