...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/variantint variantint.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/variantint variantint.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=variantint.dasm.il variantint.exe
    "$CORE_ROOT/ildasm" -raweh -out=variantint.dasm.il variantint.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=variantint.asm.exe  variantint.dasm.il
    "$CORE_ROOT/ilasm" -output=variantint.asm.exe  variantint.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=variantint
    if [ ! -f variantint.org ]; then
        TakeLock
        if [ ! -f variantint.org ]; then
          mkdir IL
          cp variantint.exe IL/variantint.exe
          mv variantint.exe variantint.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in variantint.org /out variantint.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in variantint.org /out variantint.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in variantint.reflect.xml
  if [ -f variantint.reflect.xml ]
then
    ReflectionRoots="-x variantint.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/variantint.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" variantint.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" variantint.asm.exe $CLRTestExecutionArguments
