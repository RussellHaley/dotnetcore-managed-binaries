...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\gcvariant3 gcvariant3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\gcvariant3 gcvariant3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gcvariant3.dasm.il gcvariant3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gcvariant3.dasm.il gcvariant3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gcvariant3.asm.exe  gcvariant3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gcvariant3.asm.exe  gcvariant3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gcvariant3
    if not exist "gcvariant3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gcvariant3.org" (
            mkdir IL
            copy gcvariant3.exe IL\gcvariant3.exe 
            ren gcvariant3.exe gcvariant3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcvariant3.org /out gcvariant3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gcvariant3.org /out gcvariant3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gcvariant3.reflect.xml
    IF EXIST gcvariant3.reflect.xml set ReflectionRoots=-x gcvariant3.reflect.xml
    ...
    set ExePath=%LinkBin%\gcvariant3.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gcvariant3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gcvariant3.asm.exe %CLRTestExecutionArguments% %Host_Args%
