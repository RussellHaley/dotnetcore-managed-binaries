...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/gcvariant3 gcvariant3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/gcvariant3 gcvariant3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gcvariant3.dasm.il gcvariant3.exe
    "$CORE_ROOT/ildasm" -raweh -out=gcvariant3.dasm.il gcvariant3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gcvariant3.asm.exe  gcvariant3.dasm.il
    "$CORE_ROOT/ilasm" -output=gcvariant3.asm.exe  gcvariant3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gcvariant3
    if [ ! -f gcvariant3.org ]; then
        TakeLock
        if [ ! -f gcvariant3.org ]; then
          mkdir IL
          cp gcvariant3.exe IL/gcvariant3.exe
          mv gcvariant3.exe gcvariant3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcvariant3.org /out gcvariant3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gcvariant3.org /out gcvariant3.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gcvariant3.reflect.xml
  if [ -f gcvariant3.reflect.xml ]
then
    ReflectionRoots="-x gcvariant3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/gcvariant3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gcvariant3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gcvariant3.asm.exe $CLRTestExecutionArguments
