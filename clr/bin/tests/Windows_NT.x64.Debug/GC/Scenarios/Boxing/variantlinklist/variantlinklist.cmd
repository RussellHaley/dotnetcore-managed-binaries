...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\variantlinklist variantlinklist.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Boxing\variantlinklist variantlinklist.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=variantlinklist.dasm.il variantlinklist.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=variantlinklist.dasm.il variantlinklist.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=variantlinklist.asm.exe  variantlinklist.dasm.il
    %CORE_ROOT%\ilasm.exe /output=variantlinklist.asm.exe  variantlinklist.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=variantlinklist
    if not exist "variantlinklist.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "variantlinklist.org" (
            mkdir IL
            copy variantlinklist.exe IL\variantlinklist.exe 
            ren variantlinklist.exe variantlinklist.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in variantlinklist.org /out variantlinklist.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in variantlinklist.org /out variantlinklist.exe
...
set ExePath=variantlinklist.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in variantlinklist.reflect.xml
    IF EXIST variantlinklist.reflect.xml set ReflectionRoots=-x variantlinklist.reflect.xml
    ...
    set ExePath=%LinkBin%\variantlinklist.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% variantlinklist.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% variantlinklist.asm.exe %CLRTestExecutionArguments% %Host_Args%
