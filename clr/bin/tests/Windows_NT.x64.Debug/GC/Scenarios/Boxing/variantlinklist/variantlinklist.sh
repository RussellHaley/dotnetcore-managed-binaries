...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/variantlinklist variantlinklist.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Boxing/variantlinklist variantlinklist.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=variantlinklist.dasm.il variantlinklist.exe
    "$CORE_ROOT/ildasm" -raweh -out=variantlinklist.dasm.il variantlinklist.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=variantlinklist.asm.exe  variantlinklist.dasm.il
    "$CORE_ROOT/ilasm" -output=variantlinklist.asm.exe  variantlinklist.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=variantlinklist
    if [ ! -f variantlinklist.org ]; then
        TakeLock
        if [ ! -f variantlinklist.org ]; then
          mkdir IL
          cp variantlinklist.exe IL/variantlinklist.exe
          mv variantlinklist.exe variantlinklist.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in variantlinklist.org /out variantlinklist.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in variantlinklist.org /out variantlinklist.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in variantlinklist.reflect.xml
  if [ -f variantlinklist.reflect.xml ]
then
    ReflectionRoots="-x variantlinklist.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/variantlinklist.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" variantlinklist.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" variantlinklist.asm.exe $CLRTestExecutionArguments
