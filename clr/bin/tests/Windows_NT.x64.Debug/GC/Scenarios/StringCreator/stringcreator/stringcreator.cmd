...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\StringCreator\stringcreator stringcreator.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\StringCreator\stringcreator stringcreator.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=stringcreator.dasm.il stringcreator.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=stringcreator.dasm.il stringcreator.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=stringcreator.asm.exe  stringcreator.dasm.il
    %CORE_ROOT%\ilasm.exe /output=stringcreator.asm.exe  stringcreator.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=stringcreator
    if not exist "stringcreator.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "stringcreator.org" (
            mkdir IL
            copy stringcreator.exe IL\stringcreator.exe 
            ren stringcreator.exe stringcreator.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in stringcreator.org /out stringcreator.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in stringcreator.org /out stringcreator.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in stringcreator.reflect.xml
    IF EXIST stringcreator.reflect.xml set ReflectionRoots=-x stringcreator.reflect.xml
    ...
    set ExePath=%LinkBin%\stringcreator.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% stringcreator.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% stringcreator.asm.exe %CLRTestExecutionArguments% %Host_Args%
