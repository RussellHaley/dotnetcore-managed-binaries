...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/StringCreator/stringcreator stringcreator.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/StringCreator/stringcreator stringcreator.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=stringcreator.dasm.il stringcreator.exe
    "$CORE_ROOT/ildasm" -raweh -out=stringcreator.dasm.il stringcreator.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=stringcreator.asm.exe  stringcreator.dasm.il
    "$CORE_ROOT/ilasm" -output=stringcreator.asm.exe  stringcreator.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=stringcreator
    if [ ! -f stringcreator.org ]; then
        TakeLock
        if [ ! -f stringcreator.org ]; then
          mkdir IL
          cp stringcreator.exe IL/stringcreator.exe
          mv stringcreator.exe stringcreator.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in stringcreator.org /out stringcreator.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in stringcreator.org /out stringcreator.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in stringcreator.reflect.xml
  if [ -f stringcreator.reflect.xml ]
then
    ReflectionRoots="-x stringcreator.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/stringcreator.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" stringcreator.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" stringcreator.asm.exe $CLRTestExecutionArguments
