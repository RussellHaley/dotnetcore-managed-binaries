...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\NDPin\ndpinfinal ndpinfinal.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\NDPin\ndpinfinal ndpinfinal.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ndpinfinal.dasm.il ndpinfinal.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ndpinfinal.dasm.il ndpinfinal.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ndpinfinal.asm.exe  ndpinfinal.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ndpinfinal.asm.exe  ndpinfinal.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ndpinfinal
    if not exist "ndpinfinal.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ndpinfinal.org" (
            mkdir IL
            copy ndpinfinal.exe IL\ndpinfinal.exe 
            ren ndpinfinal.exe ndpinfinal.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ndpinfinal.org /out ndpinfinal.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ndpinfinal.org /out ndpinfinal.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ndpinfinal.reflect.xml
    IF EXIST ndpinfinal.reflect.xml set ReflectionRoots=-x ndpinfinal.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ndpinfinal.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ndpinfinal.asm.exe %CLRTestExecutionArguments% %Host_Args%
