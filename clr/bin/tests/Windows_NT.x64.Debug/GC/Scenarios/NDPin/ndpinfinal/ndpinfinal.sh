...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/NDPin/ndpinfinal ndpinfinal.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/NDPin/ndpinfinal ndpinfinal.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ndpinfinal.dasm.il ndpinfinal.exe
    "$CORE_ROOT/ildasm" -raweh -out=ndpinfinal.dasm.il ndpinfinal.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ndpinfinal.asm.exe  ndpinfinal.dasm.il
    "$CORE_ROOT/ilasm" -output=ndpinfinal.asm.exe  ndpinfinal.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ndpinfinal
    if [ ! -f ndpinfinal.org ]; then
        TakeLock
        if [ ! -f ndpinfinal.org ]; then
          mkdir IL
          cp ndpinfinal.exe IL/ndpinfinal.exe
          mv ndpinfinal.exe ndpinfinal.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ndpinfinal.org /out ndpinfinal.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ndpinfinal.org /out ndpinfinal.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ndpinfinal.reflect.xml
  if [ -f ndpinfinal.reflect.xml ]
then
    ReflectionRoots="-x ndpinfinal.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ndpinfinal.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ndpinfinal.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ndpinfinal.asm.exe $CLRTestExecutionArguments
