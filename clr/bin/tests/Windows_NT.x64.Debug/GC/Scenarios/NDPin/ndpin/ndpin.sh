...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/NDPin/ndpin ndpin.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/NDPin/ndpin ndpin.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ndpin.dasm.il ndpin.exe
    "$CORE_ROOT/ildasm" -raweh -out=ndpin.dasm.il ndpin.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ndpin.asm.exe  ndpin.dasm.il
    "$CORE_ROOT/ilasm" -output=ndpin.asm.exe  ndpin.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ndpin
    if [ ! -f ndpin.org ]; then
        TakeLock
        if [ ! -f ndpin.org ]; then
          mkdir IL
          cp ndpin.exe IL/ndpin.exe
          mv ndpin.exe ndpin.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ndpin.org /out ndpin.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ndpin.org /out ndpin.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ndpin.reflect.xml
  if [ -f ndpin.reflect.xml ]
then
    ReflectionRoots="-x ndpin.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ndpin.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ndpin.asm.exe $CLRTestExecutionArguments
