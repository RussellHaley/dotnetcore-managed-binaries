...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ndpin.dasm.il ndpin.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ndpin.dasm.il ndpin.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ndpin.asm.exe  ndpin.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ndpin.asm.exe  ndpin.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ndpin
    if not exist "ndpin.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ndpin.org" (
            mkdir IL
            copy ndpin.exe IL\ndpin.exe 
            ren ndpin.exe ndpin.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ndpin.org /out ndpin.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ndpin.org /out ndpin.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ndpin.reflect.xml
    IF EXIST ndpin.reflect.xml set ReflectionRoots=-x ndpin.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ndpin.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ndpin.asm.exe %CLRTestExecutionArguments% %Host_Args%
