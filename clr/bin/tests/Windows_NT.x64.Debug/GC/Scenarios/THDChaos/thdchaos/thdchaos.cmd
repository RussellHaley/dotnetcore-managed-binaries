...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\THDChaos\thdchaos thdchaos.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\THDChaos\thdchaos thdchaos.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=thdchaos.dasm.il thdchaos.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=thdchaos.dasm.il thdchaos.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=thdchaos.asm.exe  thdchaos.dasm.il
    %CORE_ROOT%\ilasm.exe /output=thdchaos.asm.exe  thdchaos.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=thdchaos
    if not exist "thdchaos.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "thdchaos.org" (
            mkdir IL
            copy thdchaos.exe IL\thdchaos.exe 
            ren thdchaos.exe thdchaos.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdchaos.org /out thdchaos.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdchaos.org /out thdchaos.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in thdchaos.reflect.xml
    IF EXIST thdchaos.reflect.xml set ReflectionRoots=-x thdchaos.reflect.xml
    ...
    set ExePath=%LinkBin%\thdchaos.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% thdchaos.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% thdchaos.asm.exe %CLRTestExecutionArguments% %Host_Args%
