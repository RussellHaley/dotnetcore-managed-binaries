...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/THDChaos/thdchaos thdchaos.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/THDChaos/thdchaos thdchaos.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=thdchaos.dasm.il thdchaos.exe
    "$CORE_ROOT/ildasm" -raweh -out=thdchaos.dasm.il thdchaos.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=thdchaos.asm.exe  thdchaos.dasm.il
    "$CORE_ROOT/ilasm" -output=thdchaos.asm.exe  thdchaos.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=thdchaos
    if [ ! -f thdchaos.org ]; then
        TakeLock
        if [ ! -f thdchaos.org ]; then
          mkdir IL
          cp thdchaos.exe IL/thdchaos.exe
          mv thdchaos.exe thdchaos.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdchaos.org /out thdchaos.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdchaos.org /out thdchaos.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in thdchaos.reflect.xml
  if [ -f thdchaos.reflect.xml ]
then
    ReflectionRoots="-x thdchaos.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" thdchaos.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" thdchaos.asm.exe $CLRTestExecutionArguments
