...
if [ -z ${CLRTestExecutionArguments+x} ]; then export CLRTestExecutionArguments='1000 40 191919'; fi
...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Dynamo/dynamo dynamo.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Dynamo/dynamo dynamo.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=dynamo.dasm.il dynamo.exe
    "$CORE_ROOT/ildasm" -raweh -out=dynamo.dasm.il dynamo.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=dynamo.asm.exe  dynamo.dasm.il
    "$CORE_ROOT/ilasm" -output=dynamo.asm.exe  dynamo.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=dynamo
    if [ ! -f dynamo.org ]; then
        TakeLock
        if [ ! -f dynamo.org ]; then
          mkdir IL
          cp dynamo.exe IL/dynamo.exe
          mv dynamo.exe dynamo.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dynamo.org /out dynamo.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in dynamo.org /out dynamo.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in dynamo.reflect.xml
  if [ -f dynamo.reflect.xml ]
then
    ReflectionRoots="-x dynamo.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" dynamo.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" dynamo.asm.exe $CLRTestExecutionArguments
