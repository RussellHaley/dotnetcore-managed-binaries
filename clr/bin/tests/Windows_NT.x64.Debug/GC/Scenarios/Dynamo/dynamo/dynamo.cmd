...
if not defined CLRTestExecutionArguments (set CLRTestExecutionArguments=1000 40 191919 )
...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Dynamo\dynamo dynamo.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Dynamo\dynamo dynamo.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=dynamo.dasm.il dynamo.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=dynamo.dasm.il dynamo.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=dynamo.asm.exe  dynamo.dasm.il
    %CORE_ROOT%\ilasm.exe /output=dynamo.asm.exe  dynamo.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=dynamo
    if not exist "dynamo.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "dynamo.org" (
            mkdir IL
            copy dynamo.exe IL\dynamo.exe 
            ren dynamo.exe dynamo.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dynamo.org /out dynamo.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in dynamo.org /out dynamo.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in dynamo.reflect.xml
    IF EXIST dynamo.reflect.xml set ReflectionRoots=-x dynamo.reflect.xml
    ...
    set ExePath=%LinkBin%\dynamo.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% dynamo.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% dynamo.asm.exe %CLRTestExecutionArguments% %Host_Args%
