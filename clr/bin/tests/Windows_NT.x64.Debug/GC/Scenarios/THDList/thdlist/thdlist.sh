...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/THDList/thdlist thdlist.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/THDList/thdlist thdlist.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=thdlist.dasm.il thdlist.exe
    "$CORE_ROOT/ildasm" -raweh -out=thdlist.dasm.il thdlist.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=thdlist.asm.exe  thdlist.dasm.il
    "$CORE_ROOT/ilasm" -output=thdlist.asm.exe  thdlist.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=thdlist
    if [ ! -f thdlist.org ]; then
        TakeLock
        if [ ! -f thdlist.org ]; then
          mkdir IL
          cp thdlist.exe IL/thdlist.exe
          mv thdlist.exe thdlist.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdlist.org /out thdlist.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdlist.org /out thdlist.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in thdlist.reflect.xml
  if [ -f thdlist.reflect.xml ]
then
    ReflectionRoots="-x thdlist.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" thdlist.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" thdlist.asm.exe $CLRTestExecutionArguments
