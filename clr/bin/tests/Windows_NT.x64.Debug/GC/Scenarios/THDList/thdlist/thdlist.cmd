...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\THDList\thdlist thdlist.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\THDList\thdlist thdlist.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=thdlist.dasm.il thdlist.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=thdlist.dasm.il thdlist.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=thdlist.asm.exe  thdlist.dasm.il
    %CORE_ROOT%\ilasm.exe /output=thdlist.asm.exe  thdlist.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=thdlist
    if not exist "thdlist.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "thdlist.org" (
            mkdir IL
            copy thdlist.exe IL\thdlist.exe 
            ren thdlist.exe thdlist.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdlist.org /out thdlist.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdlist.org /out thdlist.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in thdlist.reflect.xml
    IF EXIST thdlist.reflect.xml set ReflectionRoots=-x thdlist.reflect.xml
    ...
    set ExePath=%LinkBin%\thdlist.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% thdlist.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% thdlist.asm.exe %CLRTestExecutionArguments% %Host_Args%
