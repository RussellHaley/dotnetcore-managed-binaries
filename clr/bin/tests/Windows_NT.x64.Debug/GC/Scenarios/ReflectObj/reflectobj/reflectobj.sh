...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/ReflectObj/reflectobj reflectobj.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/ReflectObj/reflectobj reflectobj.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=reflectobj.dasm.il reflectobj.exe
    "$CORE_ROOT/ildasm" -raweh -out=reflectobj.dasm.il reflectobj.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=reflectobj.asm.exe  reflectobj.dasm.il
    "$CORE_ROOT/ilasm" -output=reflectobj.asm.exe  reflectobj.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=reflectobj
    if [ ! -f reflectobj.org ]; then
        TakeLock
        if [ ! -f reflectobj.org ]; then
          mkdir IL
          cp reflectobj.exe IL/reflectobj.exe
          mv reflectobj.exe reflectobj.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reflectobj.org /out reflectobj.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in reflectobj.org /out reflectobj.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in reflectobj.reflect.xml
  if [ -f reflectobj.reflect.xml ]
then
    ReflectionRoots="-x reflectobj.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/reflectobj.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" reflectobj.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" reflectobj.asm.exe $CLRTestExecutionArguments
