...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\ReflectObj\reflectobj reflectobj.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\ReflectObj\reflectobj reflectobj.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=reflectobj.dasm.il reflectobj.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=reflectobj.dasm.il reflectobj.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=reflectobj.asm.exe  reflectobj.dasm.il
    %CORE_ROOT%\ilasm.exe /output=reflectobj.asm.exe  reflectobj.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=reflectobj
    if not exist "reflectobj.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "reflectobj.org" (
            mkdir IL
            copy reflectobj.exe IL\reflectobj.exe 
            ren reflectobj.exe reflectobj.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reflectobj.org /out reflectobj.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in reflectobj.org /out reflectobj.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in reflectobj.reflect.xml
    IF EXIST reflectobj.reflect.xml set ReflectionRoots=-x reflectobj.reflect.xml
    ...
    set ExePath=%LinkBin%\reflectobj.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% reflectobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% reflectobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
