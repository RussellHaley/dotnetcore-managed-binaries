...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\LeakWheel\leakwheel leakwheel.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\LeakWheel\leakwheel leakwheel.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=leakwheel.dasm.il leakwheel.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=leakwheel.dasm.il leakwheel.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=leakwheel.asm.exe  leakwheel.dasm.il
    %CORE_ROOT%\ilasm.exe /output=leakwheel.asm.exe  leakwheel.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=leakwheel
    if not exist "leakwheel.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "leakwheel.org" (
            mkdir IL
            copy leakwheel.exe IL\leakwheel.exe 
            ren leakwheel.exe leakwheel.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in leakwheel.org /out leakwheel.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in leakwheel.org /out leakwheel.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in leakwheel.reflect.xml
    IF EXIST leakwheel.reflect.xml set ReflectionRoots=-x leakwheel.reflect.xml
    ...
    set ExePath=%LinkBin%\leakwheel.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% leakwheel.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% leakwheel.asm.exe %CLRTestExecutionArguments% %Host_Args%
