...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/LeakWheel/leakwheel leakwheel.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/LeakWheel/leakwheel leakwheel.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=leakwheel.dasm.il leakwheel.exe
    "$CORE_ROOT/ildasm" -raweh -out=leakwheel.dasm.il leakwheel.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=leakwheel.asm.exe  leakwheel.dasm.il
    "$CORE_ROOT/ilasm" -output=leakwheel.asm.exe  leakwheel.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=leakwheel
    if [ ! -f leakwheel.org ]; then
        TakeLock
        if [ ! -f leakwheel.org ]; then
          mkdir IL
          cp leakwheel.exe IL/leakwheel.exe
          mv leakwheel.exe leakwheel.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in leakwheel.org /out leakwheel.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in leakwheel.org /out leakwheel.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in leakwheel.reflect.xml
  if [ -f leakwheel.reflect.xml ]
then
    ReflectionRoots="-x leakwheel.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" leakwheel.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" leakwheel.asm.exe $CLRTestExecutionArguments
