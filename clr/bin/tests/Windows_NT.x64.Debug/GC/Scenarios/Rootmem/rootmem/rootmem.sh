...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Rootmem/rootmem rootmem.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Rootmem/rootmem rootmem.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rootmem.dasm.il rootmem.exe
    "$CORE_ROOT/ildasm" -raweh -out=rootmem.dasm.il rootmem.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rootmem.asm.exe  rootmem.dasm.il
    "$CORE_ROOT/ilasm" -output=rootmem.asm.exe  rootmem.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rootmem
    if [ ! -f rootmem.org ]; then
        TakeLock
        if [ ! -f rootmem.org ]; then
          mkdir IL
          cp rootmem.exe IL/rootmem.exe
          mv rootmem.exe rootmem.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rootmem.org /out rootmem.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rootmem.org /out rootmem.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in rootmem.reflect.xml
  if [ -f rootmem.reflect.xml ]
then
    ReflectionRoots="-x rootmem.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rootmem.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rootmem.asm.exe $CLRTestExecutionArguments
