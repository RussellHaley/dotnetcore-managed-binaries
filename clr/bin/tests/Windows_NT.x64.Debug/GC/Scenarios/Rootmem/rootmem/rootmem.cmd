...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Rootmem\rootmem rootmem.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Rootmem\rootmem rootmem.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=rootmem.dasm.il rootmem.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=rootmem.dasm.il rootmem.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=rootmem.asm.exe  rootmem.dasm.il
    %CORE_ROOT%\ilasm.exe /output=rootmem.asm.exe  rootmem.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=rootmem
    if not exist "rootmem.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "rootmem.org" (
            mkdir IL
            copy rootmem.exe IL\rootmem.exe 
            ren rootmem.exe rootmem.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rootmem.org /out rootmem.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rootmem.org /out rootmem.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in rootmem.reflect.xml
    IF EXIST rootmem.reflect.xml set ReflectionRoots=-x rootmem.reflect.xml
    ...
    set ExePath=%LinkBin%\rootmem.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% rootmem.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% rootmem.asm.exe %CLRTestExecutionArguments% %Host_Args%
