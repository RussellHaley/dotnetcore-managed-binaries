...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\SingLinkList\singlinkgen singlinkgen.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\SingLinkList\singlinkgen singlinkgen.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=singlinkgen.dasm.il singlinkgen.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=singlinkgen.dasm.il singlinkgen.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=singlinkgen.asm.exe  singlinkgen.dasm.il
    %CORE_ROOT%\ilasm.exe /output=singlinkgen.asm.exe  singlinkgen.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=singlinkgen
    if not exist "singlinkgen.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "singlinkgen.org" (
            mkdir IL
            copy singlinkgen.exe IL\singlinkgen.exe 
            ren singlinkgen.exe singlinkgen.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in singlinkgen.org /out singlinkgen.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in singlinkgen.org /out singlinkgen.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in singlinkgen.reflect.xml
    IF EXIST singlinkgen.reflect.xml set ReflectionRoots=-x singlinkgen.reflect.xml
    ...
    set ExePath=%LinkBin%\singlinkgen.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% singlinkgen.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% singlinkgen.asm.exe %CLRTestExecutionArguments% %Host_Args%
