...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/SingLinkList/singlinkgen singlinkgen.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/SingLinkList/singlinkgen singlinkgen.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=singlinkgen.dasm.il singlinkgen.exe
    "$CORE_ROOT/ildasm" -raweh -out=singlinkgen.dasm.il singlinkgen.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=singlinkgen.asm.exe  singlinkgen.dasm.il
    "$CORE_ROOT/ilasm" -output=singlinkgen.asm.exe  singlinkgen.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=singlinkgen
    if [ ! -f singlinkgen.org ]; then
        TakeLock
        if [ ! -f singlinkgen.org ]; then
          mkdir IL
          cp singlinkgen.exe IL/singlinkgen.exe
          mv singlinkgen.exe singlinkgen.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in singlinkgen.org /out singlinkgen.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in singlinkgen.org /out singlinkgen.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in singlinkgen.reflect.xml
  if [ -f singlinkgen.reflect.xml ]
then
    ReflectionRoots="-x singlinkgen.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/singlinkgen.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" singlinkgen.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" singlinkgen.asm.exe $CLRTestExecutionArguments
