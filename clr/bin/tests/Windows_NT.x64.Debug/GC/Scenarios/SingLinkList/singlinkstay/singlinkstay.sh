...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/SingLinkList/singlinkstay singlinkstay.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/SingLinkList/singlinkstay singlinkstay.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=singlinkstay.dasm.il singlinkstay.exe
    "$CORE_ROOT/ildasm" -raweh -out=singlinkstay.dasm.il singlinkstay.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=singlinkstay.asm.exe  singlinkstay.dasm.il
    "$CORE_ROOT/ilasm" -output=singlinkstay.asm.exe  singlinkstay.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=singlinkstay
    if [ ! -f singlinkstay.org ]; then
        TakeLock
        if [ ! -f singlinkstay.org ]; then
          mkdir IL
          cp singlinkstay.exe IL/singlinkstay.exe
          mv singlinkstay.exe singlinkstay.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in singlinkstay.org /out singlinkstay.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in singlinkstay.org /out singlinkstay.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in singlinkstay.reflect.xml
  if [ -f singlinkstay.reflect.xml ]
then
    ReflectionRoots="-x singlinkstay.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/singlinkstay.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" singlinkstay.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" singlinkstay.asm.exe $CLRTestExecutionArguments
