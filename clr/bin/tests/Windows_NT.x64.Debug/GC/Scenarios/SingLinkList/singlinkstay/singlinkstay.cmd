...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\SingLinkList\singlinkstay singlinkstay.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\SingLinkList\singlinkstay singlinkstay.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=singlinkstay.dasm.il singlinkstay.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=singlinkstay.dasm.il singlinkstay.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=singlinkstay.asm.exe  singlinkstay.dasm.il
    %CORE_ROOT%\ilasm.exe /output=singlinkstay.asm.exe  singlinkstay.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=singlinkstay
    if not exist "singlinkstay.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "singlinkstay.org" (
            mkdir IL
            copy singlinkstay.exe IL\singlinkstay.exe 
            ren singlinkstay.exe singlinkstay.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in singlinkstay.org /out singlinkstay.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in singlinkstay.org /out singlinkstay.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in singlinkstay.reflect.xml
    IF EXIST singlinkstay.reflect.xml set ReflectionRoots=-x singlinkstay.reflect.xml
    ...
    set ExePath=%LinkBin%\singlinkstay.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% singlinkstay.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% singlinkstay.asm.exe %CLRTestExecutionArguments% %Host_Args%
