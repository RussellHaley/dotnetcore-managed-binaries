...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/FinalNStruct/finalnstruct finalnstruct.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/FinalNStruct/finalnstruct finalnstruct.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=finalnstruct.dasm.il finalnstruct.exe
    "$CORE_ROOT/ildasm" -raweh -out=finalnstruct.dasm.il finalnstruct.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=finalnstruct.asm.exe  finalnstruct.dasm.il
    "$CORE_ROOT/ilasm" -output=finalnstruct.asm.exe  finalnstruct.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=finalnstruct
    if [ ! -f finalnstruct.org ]; then
        TakeLock
        if [ ! -f finalnstruct.org ]; then
          mkdir IL
          cp finalnstruct.exe IL/finalnstruct.exe
          mv finalnstruct.exe finalnstruct.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in finalnstruct.org /out finalnstruct.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in finalnstruct.org /out finalnstruct.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in finalnstruct.reflect.xml
  if [ -f finalnstruct.reflect.xml ]
then
    ReflectionRoots="-x finalnstruct.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/finalnstruct.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" finalnstruct.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" finalnstruct.asm.exe $CLRTestExecutionArguments
