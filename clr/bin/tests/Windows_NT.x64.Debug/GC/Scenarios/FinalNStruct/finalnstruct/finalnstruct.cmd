...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\FinalNStruct\finalnstruct finalnstruct.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\FinalNStruct\finalnstruct finalnstruct.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=finalnstruct.dasm.il finalnstruct.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=finalnstruct.dasm.il finalnstruct.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=finalnstruct.asm.exe  finalnstruct.dasm.il
    %CORE_ROOT%\ilasm.exe /output=finalnstruct.asm.exe  finalnstruct.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=finalnstruct
    if not exist "finalnstruct.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "finalnstruct.org" (
            mkdir IL
            copy finalnstruct.exe IL\finalnstruct.exe 
            ren finalnstruct.exe finalnstruct.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in finalnstruct.org /out finalnstruct.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in finalnstruct.org /out finalnstruct.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in finalnstruct.reflect.xml
    IF EXIST finalnstruct.reflect.xml set ReflectionRoots=-x finalnstruct.reflect.xml
    ...
    set ExePath=%LinkBin%\finalnstruct.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% finalnstruct.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% finalnstruct.asm.exe %CLRTestExecutionArguments% %Host_Args%
