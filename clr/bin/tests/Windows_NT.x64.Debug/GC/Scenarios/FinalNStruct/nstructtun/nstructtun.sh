...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/FinalNStruct/nstructtun nstructtun.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/FinalNStruct/nstructtun nstructtun.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nstructtun.dasm.il nstructtun.exe
    "$CORE_ROOT/ildasm" -raweh -out=nstructtun.dasm.il nstructtun.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nstructtun.asm.exe  nstructtun.dasm.il
    "$CORE_ROOT/ilasm" -output=nstructtun.asm.exe  nstructtun.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nstructtun
    if [ ! -f nstructtun.org ]; then
        TakeLock
        if [ ! -f nstructtun.org ]; then
          mkdir IL
          cp nstructtun.exe IL/nstructtun.exe
          mv nstructtun.exe nstructtun.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nstructtun.org /out nstructtun.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nstructtun.org /out nstructtun.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in nstructtun.reflect.xml
  if [ -f nstructtun.reflect.xml ]
then
    ReflectionRoots="-x nstructtun.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nstructtun.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nstructtun.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nstructtun.asm.exe $CLRTestExecutionArguments
