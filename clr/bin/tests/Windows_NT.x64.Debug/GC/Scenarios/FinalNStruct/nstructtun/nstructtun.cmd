...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nstructtun.dasm.il nstructtun.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nstructtun.dasm.il nstructtun.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nstructtun.asm.exe  nstructtun.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nstructtun.asm.exe  nstructtun.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nstructtun
    if not exist "nstructtun.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nstructtun.org" (
            mkdir IL
            copy nstructtun.exe IL\nstructtun.exe 
            ren nstructtun.exe nstructtun.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nstructtun.org /out nstructtun.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nstructtun.org /out nstructtun.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nstructtun.reflect.xml
    IF EXIST nstructtun.reflect.xml set ReflectionRoots=-x nstructtun.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nstructtun.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nstructtun.asm.exe %CLRTestExecutionArguments% %Host_Args%
