...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\FinalNStruct\nstructresur nstructresur.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\FinalNStruct\nstructresur nstructresur.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=nstructresur.dasm.il nstructresur.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=nstructresur.dasm.il nstructresur.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=nstructresur.asm.exe  nstructresur.dasm.il
    %CORE_ROOT%\ilasm.exe /output=nstructresur.asm.exe  nstructresur.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=nstructresur
    if not exist "nstructresur.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "nstructresur.org" (
            mkdir IL
            copy nstructresur.exe IL\nstructresur.exe 
            ren nstructresur.exe nstructresur.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nstructresur.org /out nstructresur.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in nstructresur.org /out nstructresur.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in nstructresur.reflect.xml
    IF EXIST nstructresur.reflect.xml set ReflectionRoots=-x nstructresur.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% nstructresur.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% nstructresur.asm.exe %CLRTestExecutionArguments% %Host_Args%
