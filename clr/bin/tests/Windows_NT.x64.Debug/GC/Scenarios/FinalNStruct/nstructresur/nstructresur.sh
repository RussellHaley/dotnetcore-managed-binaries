...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/FinalNStruct/nstructresur nstructresur.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/FinalNStruct/nstructresur nstructresur.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=nstructresur.dasm.il nstructresur.exe
    "$CORE_ROOT/ildasm" -raweh -out=nstructresur.dasm.il nstructresur.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=nstructresur.asm.exe  nstructresur.dasm.il
    "$CORE_ROOT/ilasm" -output=nstructresur.asm.exe  nstructresur.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=nstructresur
    if [ ! -f nstructresur.org ]; then
        TakeLock
        if [ ! -f nstructresur.org ]; then
          mkdir IL
          cp nstructresur.exe IL/nstructresur.exe
          mv nstructresur.exe nstructresur.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nstructresur.org /out nstructresur.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in nstructresur.org /out nstructresur.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in nstructresur.reflect.xml
  if [ -f nstructresur.reflect.xml ]
then
    ReflectionRoots="-x nstructresur.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/nstructresur.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" nstructresur.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" nstructresur.asm.exe $CLRTestExecutionArguments
