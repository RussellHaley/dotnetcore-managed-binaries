...
if [ -z ${CLRTestExecutionArguments+x} ]; then export CLRTestExecutionArguments='/numrequests:100'; fi
...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/ServerModel/servermodel servermodel.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/ServerModel/servermodel servermodel.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=servermodel.dasm.il servermodel.exe
    "$CORE_ROOT/ildasm" -raweh -out=servermodel.dasm.il servermodel.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=servermodel.asm.exe  servermodel.dasm.il
    "$CORE_ROOT/ilasm" -output=servermodel.asm.exe  servermodel.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=servermodel
    if [ ! -f servermodel.org ]; then
        TakeLock
        if [ ! -f servermodel.org ]; then
          mkdir IL
          cp servermodel.exe IL/servermodel.exe
          mv servermodel.exe servermodel.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in servermodel.org /out servermodel.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in servermodel.org /out servermodel.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in servermodel.reflect.xml
  if [ -f servermodel.reflect.xml ]
then
    ReflectionRoots="-x servermodel.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/servermodel.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" servermodel.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" servermodel.asm.exe $CLRTestExecutionArguments
