...
if not defined CLRTestExecutionArguments (set CLRTestExecutionArguments=/numrequests:100 )
...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\ServerModel\servermodel servermodel.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\ServerModel\servermodel servermodel.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=servermodel.dasm.il servermodel.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=servermodel.dasm.il servermodel.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=servermodel.asm.exe  servermodel.dasm.il
    %CORE_ROOT%\ilasm.exe /output=servermodel.asm.exe  servermodel.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=servermodel
    if not exist "servermodel.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "servermodel.org" (
            mkdir IL
            copy servermodel.exe IL\servermodel.exe 
            ren servermodel.exe servermodel.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in servermodel.org /out servermodel.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in servermodel.org /out servermodel.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in servermodel.reflect.xml
    IF EXIST servermodel.reflect.xml set ReflectionRoots=-x servermodel.reflect.xml
    ...
    set ExePath=%LinkBin%\servermodel.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% servermodel.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% servermodel.asm.exe %CLRTestExecutionArguments% %Host_Args%
