...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\BinTree\thdtreelivingobj thdtreelivingobj.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\BinTree\thdtreelivingobj thdtreelivingobj.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=thdtreelivingobj.dasm.il thdtreelivingobj.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=thdtreelivingobj.dasm.il thdtreelivingobj.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=thdtreelivingobj.asm.exe  thdtreelivingobj.dasm.il
    %CORE_ROOT%\ilasm.exe /output=thdtreelivingobj.asm.exe  thdtreelivingobj.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=thdtreelivingobj
    if not exist "thdtreelivingobj.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "thdtreelivingobj.org" (
            mkdir IL
            copy thdtreelivingobj.exe IL\thdtreelivingobj.exe 
            ren thdtreelivingobj.exe thdtreelivingobj.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdtreelivingobj.org /out thdtreelivingobj.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdtreelivingobj.org /out thdtreelivingobj.exe
...
set ExePath=thdtreelivingobj.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in thdtreelivingobj.reflect.xml
    IF EXIST thdtreelivingobj.reflect.xml set ReflectionRoots=-x thdtreelivingobj.reflect.xml
    ...
    set ExePath=%LinkBin%\thdtreelivingobj.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% thdtreelivingobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% thdtreelivingobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
