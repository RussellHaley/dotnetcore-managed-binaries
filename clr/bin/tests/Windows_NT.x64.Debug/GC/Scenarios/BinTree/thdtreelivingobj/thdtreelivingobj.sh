...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/BinTree/thdtreelivingobj thdtreelivingobj.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/BinTree/thdtreelivingobj thdtreelivingobj.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=thdtreelivingobj.dasm.il thdtreelivingobj.exe
    "$CORE_ROOT/ildasm" -raweh -out=thdtreelivingobj.dasm.il thdtreelivingobj.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=thdtreelivingobj.asm.exe  thdtreelivingobj.dasm.il
    "$CORE_ROOT/ilasm" -output=thdtreelivingobj.asm.exe  thdtreelivingobj.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=thdtreelivingobj
    if [ ! -f thdtreelivingobj.org ]; then
        TakeLock
        if [ ! -f thdtreelivingobj.org ]; then
          mkdir IL
          cp thdtreelivingobj.exe IL/thdtreelivingobj.exe
          mv thdtreelivingobj.exe thdtreelivingobj.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdtreelivingobj.org /out thdtreelivingobj.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdtreelivingobj.org /out thdtreelivingobj.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in thdtreelivingobj.reflect.xml
  if [ -f thdtreelivingobj.reflect.xml ]
then
    ReflectionRoots="-x thdtreelivingobj.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/thdtreelivingobj.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" thdtreelivingobj.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" thdtreelivingobj.asm.exe $CLRTestExecutionArguments
