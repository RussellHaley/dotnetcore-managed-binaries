...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\BinTree\thdtree thdtree.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\BinTree\thdtree thdtree.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=thdtree.dasm.il thdtree.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=thdtree.dasm.il thdtree.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=thdtree.asm.exe  thdtree.dasm.il
    %CORE_ROOT%\ilasm.exe /output=thdtree.asm.exe  thdtree.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=thdtree
    if not exist "thdtree.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "thdtree.org" (
            mkdir IL
            copy thdtree.exe IL\thdtree.exe 
            ren thdtree.exe thdtree.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdtree.org /out thdtree.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdtree.org /out thdtree.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in thdtree.reflect.xml
    IF EXIST thdtree.reflect.xml set ReflectionRoots=-x thdtree.reflect.xml
    ...
    set ExePath=%LinkBin%\thdtree.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% thdtree.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% thdtree.asm.exe %CLRTestExecutionArguments% %Host_Args%
