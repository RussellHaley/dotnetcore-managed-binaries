...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/BinTree/thdtree thdtree.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/BinTree/thdtree thdtree.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=thdtree.dasm.il thdtree.exe
    "$CORE_ROOT/ildasm" -raweh -out=thdtree.dasm.il thdtree.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=thdtree.asm.exe  thdtree.dasm.il
    "$CORE_ROOT/ilasm" -output=thdtree.asm.exe  thdtree.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=thdtree
    if [ ! -f thdtree.org ]; then
        TakeLock
        if [ ! -f thdtree.org ]; then
          mkdir IL
          cp thdtree.exe IL/thdtree.exe
          mv thdtree.exe thdtree.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdtree.org /out thdtree.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdtree.org /out thdtree.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in thdtree.reflect.xml
  if [ -f thdtree.reflect.xml ]
then
    ReflectionRoots="-x thdtree.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" thdtree.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" thdtree.asm.exe $CLRTestExecutionArguments
