...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/BinTree/thdtreegrowingobj thdtreegrowingobj.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/BinTree/thdtreegrowingobj thdtreegrowingobj.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=thdtreegrowingobj.dasm.il thdtreegrowingobj.exe
    "$CORE_ROOT/ildasm" -raweh -out=thdtreegrowingobj.dasm.il thdtreegrowingobj.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=thdtreegrowingobj.asm.exe  thdtreegrowingobj.dasm.il
    "$CORE_ROOT/ilasm" -output=thdtreegrowingobj.asm.exe  thdtreegrowingobj.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=thdtreegrowingobj
    if [ ! -f thdtreegrowingobj.org ]; then
        TakeLock
        if [ ! -f thdtreegrowingobj.org ]; then
          mkdir IL
          cp thdtreegrowingobj.exe IL/thdtreegrowingobj.exe
          mv thdtreegrowingobj.exe thdtreegrowingobj.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdtreegrowingobj.org /out thdtreegrowingobj.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in thdtreegrowingobj.org /out thdtreegrowingobj.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in thdtreegrowingobj.reflect.xml
  if [ -f thdtreegrowingobj.reflect.xml ]
then
    ReflectionRoots="-x thdtreegrowingobj.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/thdtreegrowingobj.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" thdtreegrowingobj.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" thdtreegrowingobj.asm.exe $CLRTestExecutionArguments
