...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\BinTree\thdtreegrowingobj thdtreegrowingobj.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\BinTree\thdtreegrowingobj thdtreegrowingobj.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=thdtreegrowingobj.dasm.il thdtreegrowingobj.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=thdtreegrowingobj.dasm.il thdtreegrowingobj.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=thdtreegrowingobj.asm.exe  thdtreegrowingobj.dasm.il
    %CORE_ROOT%\ilasm.exe /output=thdtreegrowingobj.asm.exe  thdtreegrowingobj.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=thdtreegrowingobj
    if not exist "thdtreegrowingobj.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "thdtreegrowingobj.org" (
            mkdir IL
            copy thdtreegrowingobj.exe IL\thdtreegrowingobj.exe 
            ren thdtreegrowingobj.exe thdtreegrowingobj.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdtreegrowingobj.org /out thdtreegrowingobj.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in thdtreegrowingobj.org /out thdtreegrowingobj.exe
...
set ExePath=thdtreegrowingobj.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in thdtreegrowingobj.reflect.xml
    IF EXIST thdtreegrowingobj.reflect.xml set ReflectionRoots=-x thdtreegrowingobj.reflect.xml
    ...
    set ExePath=%LinkBin%\thdtreegrowingobj.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% thdtreegrowingobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% thdtreegrowingobj.asm.exe %CLRTestExecutionArguments% %Host_Args%
