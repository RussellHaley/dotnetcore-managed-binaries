...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\FinalizeTimeout\FinalizeTimeout FinalizeTimeout.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\FinalizeTimeout\FinalizeTimeout FinalizeTimeout.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=FinalizeTimeout.dasm.il FinalizeTimeout.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=FinalizeTimeout.dasm.il FinalizeTimeout.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=FinalizeTimeout.asm.exe  FinalizeTimeout.dasm.il
    %CORE_ROOT%\ilasm.exe /output=FinalizeTimeout.asm.exe  FinalizeTimeout.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=FinalizeTimeout
    if not exist "FinalizeTimeout.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "FinalizeTimeout.org" (
            mkdir IL
            copy FinalizeTimeout.exe IL\FinalizeTimeout.exe 
            ren FinalizeTimeout.exe FinalizeTimeout.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FinalizeTimeout.org /out FinalizeTimeout.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in FinalizeTimeout.org /out FinalizeTimeout.exe
...
set ExePath=FinalizeTimeout.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in FinalizeTimeout.reflect.xml
    IF EXIST FinalizeTimeout.reflect.xml set ReflectionRoots=-x FinalizeTimeout.reflect.xml
    ...
    set ExePath=%LinkBin%\FinalizeTimeout.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% FinalizeTimeout.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% FinalizeTimeout.asm.exe %CLRTestExecutionArguments% %Host_Args%
