...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/FinalizeTimeout/FinalizeTimeout FinalizeTimeout.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/FinalizeTimeout/FinalizeTimeout FinalizeTimeout.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=FinalizeTimeout.dasm.il FinalizeTimeout.exe
    "$CORE_ROOT/ildasm" -raweh -out=FinalizeTimeout.dasm.il FinalizeTimeout.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=FinalizeTimeout.asm.exe  FinalizeTimeout.dasm.il
    "$CORE_ROOT/ilasm" -output=FinalizeTimeout.asm.exe  FinalizeTimeout.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=FinalizeTimeout
    if [ ! -f FinalizeTimeout.org ]; then
        TakeLock
        if [ ! -f FinalizeTimeout.org ]; then
          mkdir IL
          cp FinalizeTimeout.exe IL/FinalizeTimeout.exe
          mv FinalizeTimeout.exe FinalizeTimeout.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FinalizeTimeout.org /out FinalizeTimeout.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in FinalizeTimeout.org /out FinalizeTimeout.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in FinalizeTimeout.reflect.xml
  if [ -f FinalizeTimeout.reflect.xml ]
then
    ReflectionRoots="-x FinalizeTimeout.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/FinalizeTimeout.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" FinalizeTimeout.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" FinalizeTimeout.asm.exe $CLRTestExecutionArguments
