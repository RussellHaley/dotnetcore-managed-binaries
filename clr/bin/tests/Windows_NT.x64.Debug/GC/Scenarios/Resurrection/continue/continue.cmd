...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Resurrection\continue continue.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\Resurrection\continue continue.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=continue.dasm.il continue.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=continue.dasm.il continue.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=continue.asm.exe  continue.dasm.il
    %CORE_ROOT%\ilasm.exe /output=continue.asm.exe  continue.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=continue
    if not exist "continue.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "continue.org" (
            mkdir IL
            copy continue.exe IL\continue.exe 
            ren continue.exe continue.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in continue.org /out continue.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in continue.org /out continue.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in continue.reflect.xml
    IF EXIST continue.reflect.xml set ReflectionRoots=-x continue.reflect.xml
    ...
    set ExePath=%LinkBin%\continue.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% continue.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% continue.asm.exe %CLRTestExecutionArguments% %Host_Args%
