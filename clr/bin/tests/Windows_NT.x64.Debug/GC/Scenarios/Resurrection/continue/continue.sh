...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Resurrection/continue continue.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/Resurrection/continue continue.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=continue.dasm.il continue.exe
    "$CORE_ROOT/ildasm" -raweh -out=continue.dasm.il continue.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=continue.asm.exe  continue.dasm.il
    "$CORE_ROOT/ilasm" -output=continue.asm.exe  continue.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=continue
    if [ ! -f continue.org ]; then
        TakeLock
        if [ ! -f continue.org ]; then
          mkdir IL
          cp continue.exe IL/continue.exe
          mv continue.exe continue.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in continue.org /out continue.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in continue.org /out continue.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in continue.reflect.xml
  if [ -f continue.reflect.xml ]
then
    ReflectionRoots="-x continue.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" continue.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" continue.asm.exe $CLRTestExecutionArguments
