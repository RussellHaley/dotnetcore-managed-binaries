...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\muldimjagary\muldimjagary muldimjagary.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\muldimjagary\muldimjagary muldimjagary.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=muldimjagary.dasm.il muldimjagary.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=muldimjagary.dasm.il muldimjagary.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=muldimjagary.asm.exe  muldimjagary.dasm.il
    %CORE_ROOT%\ilasm.exe /output=muldimjagary.asm.exe  muldimjagary.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=muldimjagary
    if not exist "muldimjagary.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "muldimjagary.org" (
            mkdir IL
            copy muldimjagary.exe IL\muldimjagary.exe 
            ren muldimjagary.exe muldimjagary.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in muldimjagary.org /out muldimjagary.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in muldimjagary.org /out muldimjagary.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in muldimjagary.reflect.xml
    IF EXIST muldimjagary.reflect.xml set ReflectionRoots=-x muldimjagary.reflect.xml
    ...
    set ExePath=%LinkBin%\muldimjagary.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% muldimjagary.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% muldimjagary.asm.exe %CLRTestExecutionArguments% %Host_Args%
