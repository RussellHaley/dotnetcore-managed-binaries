...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/muldimjagary/muldimjagary muldimjagary.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/muldimjagary/muldimjagary muldimjagary.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=muldimjagary.dasm.il muldimjagary.exe
    "$CORE_ROOT/ildasm" -raweh -out=muldimjagary.dasm.il muldimjagary.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=muldimjagary.asm.exe  muldimjagary.dasm.il
    "$CORE_ROOT/ilasm" -output=muldimjagary.asm.exe  muldimjagary.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=muldimjagary
    if [ ! -f muldimjagary.org ]; then
        TakeLock
        if [ ! -f muldimjagary.org ]; then
          mkdir IL
          cp muldimjagary.exe IL/muldimjagary.exe
          mv muldimjagary.exe muldimjagary.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in muldimjagary.org /out muldimjagary.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in muldimjagary.org /out muldimjagary.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in muldimjagary.reflect.xml
  if [ -f muldimjagary.reflect.xml ]
then
    ReflectionRoots="-x muldimjagary.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/muldimjagary.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" muldimjagary.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" muldimjagary.asm.exe $CLRTestExecutionArguments
