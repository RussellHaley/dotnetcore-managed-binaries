...
if [ -z ${CLRTestExecutionArguments+x} ]; then export CLRTestExecutionArguments='7 40 4 77'; fi
...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/RanCollect/rancollect rancollect.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/Scenarios/RanCollect/rancollect rancollect.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=rancollect.dasm.il rancollect.exe
    "$CORE_ROOT/ildasm" -raweh -out=rancollect.dasm.il rancollect.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=rancollect.asm.exe  rancollect.dasm.il
    "$CORE_ROOT/ilasm" -output=rancollect.asm.exe  rancollect.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=rancollect
    if [ ! -f rancollect.org ]; then
        TakeLock
        if [ ! -f rancollect.org ]; then
          mkdir IL
          cp rancollect.exe IL/rancollect.exe
          mv rancollect.exe rancollect.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rancollect.org /out rancollect.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in rancollect.org /out rancollect.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in rancollect.reflect.xml
  if [ -f rancollect.reflect.xml ]
then
    ReflectionRoots="-x rancollect.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/rancollect.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" rancollect.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" rancollect.asm.exe $CLRTestExecutionArguments
