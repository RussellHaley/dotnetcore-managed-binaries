...
if not defined CLRTestExecutionArguments (set CLRTestExecutionArguments=7 40 4 77 )
...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\RanCollect\rancollect rancollect.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\Scenarios\RanCollect\rancollect rancollect.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=rancollect.dasm.il rancollect.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=rancollect.dasm.il rancollect.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=rancollect.asm.exe  rancollect.dasm.il
    %CORE_ROOT%\ilasm.exe /output=rancollect.asm.exe  rancollect.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=rancollect
    if not exist "rancollect.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "rancollect.org" (
            mkdir IL
            copy rancollect.exe IL\rancollect.exe 
            ren rancollect.exe rancollect.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rancollect.org /out rancollect.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in rancollect.org /out rancollect.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in rancollect.reflect.xml
    IF EXIST rancollect.reflect.xml set ReflectionRoots=-x rancollect.reflect.xml
    ...
    set ExePath=%LinkBin%\rancollect.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% rancollect.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% rancollect.asm.exe %CLRTestExecutionArguments% %Host_Args%
