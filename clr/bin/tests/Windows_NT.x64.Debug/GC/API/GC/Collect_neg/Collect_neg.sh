...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect_neg Collect_neg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect_neg Collect_neg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Collect_neg.dasm.il Collect_neg.exe
    "$CORE_ROOT/ildasm" -raweh -out=Collect_neg.dasm.il Collect_neg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Collect_neg.asm.exe  Collect_neg.dasm.il
    "$CORE_ROOT/ilasm" -output=Collect_neg.asm.exe  Collect_neg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Collect_neg
    if [ ! -f Collect_neg.org ]; then
        TakeLock
        if [ ! -f Collect_neg.org ]; then
          mkdir IL
          cp Collect_neg.exe IL/Collect_neg.exe
          mv Collect_neg.exe Collect_neg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect_neg.org /out Collect_neg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect_neg.org /out Collect_neg.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Collect_neg.reflect.xml
  if [ -f Collect_neg.reflect.xml ]
then
    ReflectionRoots="-x Collect_neg.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Collect_neg.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Collect_neg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Collect_neg.asm.exe $CLRTestExecutionArguments
