...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect_neg Collect_neg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect_neg Collect_neg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Collect_neg.dasm.il Collect_neg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Collect_neg.dasm.il Collect_neg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Collect_neg.asm.exe  Collect_neg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Collect_neg.asm.exe  Collect_neg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Collect_neg
    if not exist "Collect_neg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Collect_neg.org" (
            mkdir IL
            copy Collect_neg.exe IL\Collect_neg.exe 
            ren Collect_neg.exe Collect_neg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect_neg.org /out Collect_neg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect_neg.org /out Collect_neg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Collect_neg.reflect.xml
    IF EXIST Collect_neg.reflect.xml set ReflectionRoots=-x Collect_neg.reflect.xml
    ...
    set ExePath=%LinkBin%\Collect_neg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Collect_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Collect_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
