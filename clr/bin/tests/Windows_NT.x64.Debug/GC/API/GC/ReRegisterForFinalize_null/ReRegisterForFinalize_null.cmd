...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\ReRegisterForFinalize_null ReRegisterForFinalize_null.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\ReRegisterForFinalize_null ReRegisterForFinalize_null.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ReRegisterForFinalize_null.dasm.il ReRegisterForFinalize_null.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ReRegisterForFinalize_null.dasm.il ReRegisterForFinalize_null.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ReRegisterForFinalize_null.asm.exe  ReRegisterForFinalize_null.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ReRegisterForFinalize_null.asm.exe  ReRegisterForFinalize_null.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ReRegisterForFinalize_null
    if not exist "ReRegisterForFinalize_null.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ReRegisterForFinalize_null.org" (
            mkdir IL
            copy ReRegisterForFinalize_null.exe IL\ReRegisterForFinalize_null.exe 
            ren ReRegisterForFinalize_null.exe ReRegisterForFinalize_null.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReRegisterForFinalize_null.org /out ReRegisterForFinalize_null.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ReRegisterForFinalize_null.org /out ReRegisterForFinalize_null.exe
...
set ExePath=ReRegisterForFinalize_null.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ReRegisterForFinalize_null.reflect.xml
    IF EXIST ReRegisterForFinalize_null.reflect.xml set ReflectionRoots=-x ReRegisterForFinalize_null.reflect.xml
    ...
    set ExePath=%LinkBin%\ReRegisterForFinalize_null.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ReRegisterForFinalize_null.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ReRegisterForFinalize_null.asm.exe %CLRTestExecutionArguments% %Host_Args%
