...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/ReRegisterForFinalize_null ReRegisterForFinalize_null.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/ReRegisterForFinalize_null ReRegisterForFinalize_null.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ReRegisterForFinalize_null.dasm.il ReRegisterForFinalize_null.exe
    "$CORE_ROOT/ildasm" -raweh -out=ReRegisterForFinalize_null.dasm.il ReRegisterForFinalize_null.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ReRegisterForFinalize_null.asm.exe  ReRegisterForFinalize_null.dasm.il
    "$CORE_ROOT/ilasm" -output=ReRegisterForFinalize_null.asm.exe  ReRegisterForFinalize_null.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ReRegisterForFinalize_null
    if [ ! -f ReRegisterForFinalize_null.org ]; then
        TakeLock
        if [ ! -f ReRegisterForFinalize_null.org ]; then
          mkdir IL
          cp ReRegisterForFinalize_null.exe IL/ReRegisterForFinalize_null.exe
          mv ReRegisterForFinalize_null.exe ReRegisterForFinalize_null.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReRegisterForFinalize_null.org /out ReRegisterForFinalize_null.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ReRegisterForFinalize_null.org /out ReRegisterForFinalize_null.exe
          __cgExitCode=$?
...
ExePath=ReRegisterForFinalize_null.exe
...
then
  ...
  # Use hints for reflection roots, if provided in ReRegisterForFinalize_null.reflect.xml
  if [ -f ReRegisterForFinalize_null.reflect.xml ]
then
    ReflectionRoots="-x ReRegisterForFinalize_null.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ReRegisterForFinalize_null.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ReRegisterForFinalize_null.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ReRegisterForFinalize_null.asm.exe $CLRTestExecutionArguments
