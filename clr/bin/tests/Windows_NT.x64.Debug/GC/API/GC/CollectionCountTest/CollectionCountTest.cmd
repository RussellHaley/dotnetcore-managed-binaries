...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\CollectionCountTest CollectionCountTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\CollectionCountTest CollectionCountTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=CollectionCountTest.dasm.il CollectionCountTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=CollectionCountTest.dasm.il CollectionCountTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=CollectionCountTest.asm.exe  CollectionCountTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=CollectionCountTest.asm.exe  CollectionCountTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=CollectionCountTest
    if not exist "CollectionCountTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "CollectionCountTest.org" (
            mkdir IL
            copy CollectionCountTest.exe IL\CollectionCountTest.exe 
            ren CollectionCountTest.exe CollectionCountTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CollectionCountTest.org /out CollectionCountTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in CollectionCountTest.org /out CollectionCountTest.exe
...
set ExePath=CollectionCountTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in CollectionCountTest.reflect.xml
    IF EXIST CollectionCountTest.reflect.xml set ReflectionRoots=-x CollectionCountTest.reflect.xml
    ...
    set ExePath=%LinkBin%\CollectionCountTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% CollectionCountTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% CollectionCountTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
