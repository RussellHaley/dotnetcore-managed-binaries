...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/CollectionCountTest CollectionCountTest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/CollectionCountTest CollectionCountTest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=CollectionCountTest.dasm.il CollectionCountTest.exe
    "$CORE_ROOT/ildasm" -raweh -out=CollectionCountTest.dasm.il CollectionCountTest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=CollectionCountTest.asm.exe  CollectionCountTest.dasm.il
    "$CORE_ROOT/ilasm" -output=CollectionCountTest.asm.exe  CollectionCountTest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=CollectionCountTest
    if [ ! -f CollectionCountTest.org ]; then
        TakeLock
        if [ ! -f CollectionCountTest.org ]; then
          mkdir IL
          cp CollectionCountTest.exe IL/CollectionCountTest.exe
          mv CollectionCountTest.exe CollectionCountTest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CollectionCountTest.org /out CollectionCountTest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in CollectionCountTest.org /out CollectionCountTest.exe
          __cgExitCode=$?
...
ExePath=CollectionCountTest.exe
...
then
  ...
  # Use hints for reflection roots, if provided in CollectionCountTest.reflect.xml
  if [ -f CollectionCountTest.reflect.xml ]
then
    ReflectionRoots="-x CollectionCountTest.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/CollectionCountTest.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" CollectionCountTest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" CollectionCountTest.asm.exe $CLRTestExecutionArguments
