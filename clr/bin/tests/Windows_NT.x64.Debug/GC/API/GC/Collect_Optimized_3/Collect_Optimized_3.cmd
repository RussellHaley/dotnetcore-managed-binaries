...
if not defined CLRTestExecutionArguments (set CLRTestExecutionArguments=2 )
...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect_Optimized_3 Collect_Optimized_3.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect_Optimized_3 Collect_Optimized_3.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Collect_Optimized_3.dasm.il Collect_Optimized_3.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Collect_Optimized_3.dasm.il Collect_Optimized_3.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Collect_Optimized_3.asm.exe  Collect_Optimized_3.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Collect_Optimized_3.asm.exe  Collect_Optimized_3.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Collect_Optimized_3
    if not exist "Collect_Optimized_3.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Collect_Optimized_3.org" (
            mkdir IL
            copy Collect_Optimized_3.exe IL\Collect_Optimized_3.exe 
            ren Collect_Optimized_3.exe Collect_Optimized_3.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect_Optimized_3.org /out Collect_Optimized_3.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect_Optimized_3.org /out Collect_Optimized_3.exe
...
set ExePath=Collect_Optimized_3.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Collect_Optimized_3.reflect.xml
    IF EXIST Collect_Optimized_3.reflect.xml set ReflectionRoots=-x Collect_Optimized_3.reflect.xml
    ...
    set ExePath=%LinkBin%\Collect_Optimized_3.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Collect_Optimized_3.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Collect_Optimized_3.asm.exe %CLRTestExecutionArguments% %Host_Args%
