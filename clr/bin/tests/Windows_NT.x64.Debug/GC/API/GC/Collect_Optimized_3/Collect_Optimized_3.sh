...
if [ -z ${CLRTestExecutionArguments+x} ]; then export CLRTestExecutionArguments='2'; fi
...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect_Optimized_3 Collect_Optimized_3.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect_Optimized_3 Collect_Optimized_3.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Collect_Optimized_3.dasm.il Collect_Optimized_3.exe
    "$CORE_ROOT/ildasm" -raweh -out=Collect_Optimized_3.dasm.il Collect_Optimized_3.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Collect_Optimized_3.asm.exe  Collect_Optimized_3.dasm.il
    "$CORE_ROOT/ilasm" -output=Collect_Optimized_3.asm.exe  Collect_Optimized_3.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Collect_Optimized_3
    if [ ! -f Collect_Optimized_3.org ]; then
        TakeLock
        if [ ! -f Collect_Optimized_3.org ]; then
          mkdir IL
          cp Collect_Optimized_3.exe IL/Collect_Optimized_3.exe
          mv Collect_Optimized_3.exe Collect_Optimized_3.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect_Optimized_3.org /out Collect_Optimized_3.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect_Optimized_3.org /out Collect_Optimized_3.exe
          __cgExitCode=$?
...
ExePath=Collect_Optimized_3.exe
...
then
  ...
  # Use hints for reflection roots, if provided in Collect_Optimized_3.reflect.xml
  if [ -f Collect_Optimized_3.reflect.xml ]
then
    ReflectionRoots="-x Collect_Optimized_3.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Collect_Optimized_3.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Collect_Optimized_3.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Collect_Optimized_3.asm.exe $CLRTestExecutionArguments
