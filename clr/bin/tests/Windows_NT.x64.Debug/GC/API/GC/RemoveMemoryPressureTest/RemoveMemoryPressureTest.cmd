...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\RemoveMemoryPressureTest RemoveMemoryPressureTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\RemoveMemoryPressureTest RemoveMemoryPressureTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=RemoveMemoryPressureTest.dasm.il RemoveMemoryPressureTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=RemoveMemoryPressureTest.dasm.il RemoveMemoryPressureTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=RemoveMemoryPressureTest.asm.exe  RemoveMemoryPressureTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=RemoveMemoryPressureTest.asm.exe  RemoveMemoryPressureTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=RemoveMemoryPressureTest
    if not exist "RemoveMemoryPressureTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "RemoveMemoryPressureTest.org" (
            mkdir IL
            copy RemoveMemoryPressureTest.exe IL\RemoveMemoryPressureTest.exe 
            ren RemoveMemoryPressureTest.exe RemoveMemoryPressureTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RemoveMemoryPressureTest.org /out RemoveMemoryPressureTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RemoveMemoryPressureTest.org /out RemoveMemoryPressureTest.exe
...
set ExePath=RemoveMemoryPressureTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in RemoveMemoryPressureTest.reflect.xml
    IF EXIST RemoveMemoryPressureTest.reflect.xml set ReflectionRoots=-x RemoveMemoryPressureTest.reflect.xml
    ...
    set ExePath=%LinkBin%\RemoveMemoryPressureTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% RemoveMemoryPressureTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% RemoveMemoryPressureTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
