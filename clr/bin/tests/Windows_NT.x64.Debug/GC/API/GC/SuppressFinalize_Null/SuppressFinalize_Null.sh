...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/SuppressFinalize_Null SuppressFinalize_Null.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/SuppressFinalize_Null SuppressFinalize_Null.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=SuppressFinalize_Null.dasm.il SuppressFinalize_Null.exe
    "$CORE_ROOT/ildasm" -raweh -out=SuppressFinalize_Null.dasm.il SuppressFinalize_Null.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=SuppressFinalize_Null.asm.exe  SuppressFinalize_Null.dasm.il
    "$CORE_ROOT/ilasm" -output=SuppressFinalize_Null.asm.exe  SuppressFinalize_Null.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=SuppressFinalize_Null
    if [ ! -f SuppressFinalize_Null.org ]; then
        TakeLock
        if [ ! -f SuppressFinalize_Null.org ]; then
          mkdir IL
          cp SuppressFinalize_Null.exe IL/SuppressFinalize_Null.exe
          mv SuppressFinalize_Null.exe SuppressFinalize_Null.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SuppressFinalize_Null.org /out SuppressFinalize_Null.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in SuppressFinalize_Null.org /out SuppressFinalize_Null.exe
          __cgExitCode=$?
...
ExePath=SuppressFinalize_Null.exe
...
then
  ...
  # Use hints for reflection roots, if provided in SuppressFinalize_Null.reflect.xml
  if [ -f SuppressFinalize_Null.reflect.xml ]
then
    ReflectionRoots="-x SuppressFinalize_Null.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/SuppressFinalize_Null.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" SuppressFinalize_Null.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" SuppressFinalize_Null.asm.exe $CLRTestExecutionArguments
