...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\SuppressFinalize_Null SuppressFinalize_Null.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\SuppressFinalize_Null SuppressFinalize_Null.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=SuppressFinalize_Null.dasm.il SuppressFinalize_Null.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=SuppressFinalize_Null.dasm.il SuppressFinalize_Null.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=SuppressFinalize_Null.asm.exe  SuppressFinalize_Null.dasm.il
    %CORE_ROOT%\ilasm.exe /output=SuppressFinalize_Null.asm.exe  SuppressFinalize_Null.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=SuppressFinalize_Null
    if not exist "SuppressFinalize_Null.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "SuppressFinalize_Null.org" (
            mkdir IL
            copy SuppressFinalize_Null.exe IL\SuppressFinalize_Null.exe 
            ren SuppressFinalize_Null.exe SuppressFinalize_Null.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SuppressFinalize_Null.org /out SuppressFinalize_Null.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in SuppressFinalize_Null.org /out SuppressFinalize_Null.exe
...
set ExePath=SuppressFinalize_Null.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in SuppressFinalize_Null.reflect.xml
    IF EXIST SuppressFinalize_Null.reflect.xml set ReflectionRoots=-x SuppressFinalize_Null.reflect.xml
    ...
    set ExePath=%LinkBin%\SuppressFinalize_Null.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% SuppressFinalize_Null.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% SuppressFinalize_Null.asm.exe %CLRTestExecutionArguments% %Host_Args%
