...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/GetGenerationWR GetGenerationWR.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/GetGenerationWR GetGenerationWR.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GetGenerationWR.dasm.il GetGenerationWR.exe
    "$CORE_ROOT/ildasm" -raweh -out=GetGenerationWR.dasm.il GetGenerationWR.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GetGenerationWR.asm.exe  GetGenerationWR.dasm.il
    "$CORE_ROOT/ilasm" -output=GetGenerationWR.asm.exe  GetGenerationWR.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GetGenerationWR
    if [ ! -f GetGenerationWR.org ]; then
        TakeLock
        if [ ! -f GetGenerationWR.org ]; then
          mkdir IL
          cp GetGenerationWR.exe IL/GetGenerationWR.exe
          mv GetGenerationWR.exe GetGenerationWR.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetGenerationWR.org /out GetGenerationWR.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetGenerationWR.org /out GetGenerationWR.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GetGenerationWR.reflect.xml
  if [ -f GetGenerationWR.reflect.xml ]
then
    ReflectionRoots="-x GetGenerationWR.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GetGenerationWR.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GetGenerationWR.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GetGenerationWR.asm.exe $CLRTestExecutionArguments
