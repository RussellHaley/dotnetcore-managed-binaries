...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\GetGenerationWR GetGenerationWR.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\GetGenerationWR GetGenerationWR.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GetGenerationWR.dasm.il GetGenerationWR.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GetGenerationWR.dasm.il GetGenerationWR.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GetGenerationWR.asm.exe  GetGenerationWR.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GetGenerationWR.asm.exe  GetGenerationWR.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GetGenerationWR
    if not exist "GetGenerationWR.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GetGenerationWR.org" (
            mkdir IL
            copy GetGenerationWR.exe IL\GetGenerationWR.exe 
            ren GetGenerationWR.exe GetGenerationWR.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetGenerationWR.org /out GetGenerationWR.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetGenerationWR.org /out GetGenerationWR.exe
...
set ExePath=GetGenerationWR.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GetGenerationWR.reflect.xml
    IF EXIST GetGenerationWR.reflect.xml set ReflectionRoots=-x GetGenerationWR.reflect.xml
    ...
    set ExePath=%LinkBin%\GetGenerationWR.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GetGenerationWR.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GetGenerationWR.asm.exe %CLRTestExecutionArguments% %Host_Args%
