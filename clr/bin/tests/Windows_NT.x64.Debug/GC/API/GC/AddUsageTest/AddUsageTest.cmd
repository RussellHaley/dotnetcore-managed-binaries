...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\AddUsageTest AddUsageTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\AddUsageTest AddUsageTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AddUsageTest.dasm.il AddUsageTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AddUsageTest.dasm.il AddUsageTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AddUsageTest.asm.exe  AddUsageTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AddUsageTest.asm.exe  AddUsageTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AddUsageTest
    if not exist "AddUsageTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AddUsageTest.org" (
            mkdir IL
            copy AddUsageTest.exe IL\AddUsageTest.exe 
            ren AddUsageTest.exe AddUsageTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddUsageTest.org /out AddUsageTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddUsageTest.org /out AddUsageTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AddUsageTest.reflect.xml
    IF EXIST AddUsageTest.reflect.xml set ReflectionRoots=-x AddUsageTest.reflect.xml
    ...
    set ExePath=%LinkBin%\AddUsageTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AddUsageTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AddUsageTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
