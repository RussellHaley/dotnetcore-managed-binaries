...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/AddUsageTest AddUsageTest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/AddUsageTest AddUsageTest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AddUsageTest.dasm.il AddUsageTest.exe
    "$CORE_ROOT/ildasm" -raweh -out=AddUsageTest.dasm.il AddUsageTest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AddUsageTest.asm.exe  AddUsageTest.dasm.il
    "$CORE_ROOT/ilasm" -output=AddUsageTest.asm.exe  AddUsageTest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AddUsageTest
    if [ ! -f AddUsageTest.org ]; then
        TakeLock
        if [ ! -f AddUsageTest.org ]; then
          mkdir IL
          cp AddUsageTest.exe IL/AddUsageTest.exe
          mv AddUsageTest.exe AddUsageTest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddUsageTest.org /out AddUsageTest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddUsageTest.org /out AddUsageTest.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AddUsageTest.reflect.xml
  if [ -f AddUsageTest.reflect.xml ]
then
    ReflectionRoots="-x AddUsageTest.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AddUsageTest.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AddUsageTest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AddUsageTest.asm.exe $CLRTestExecutionArguments
