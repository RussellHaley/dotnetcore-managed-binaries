...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/GetGeneration GetGeneration.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/GetGeneration GetGeneration.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GetGeneration.dasm.il GetGeneration.exe
    "$CORE_ROOT/ildasm" -raweh -out=GetGeneration.dasm.il GetGeneration.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GetGeneration.asm.exe  GetGeneration.dasm.il
    "$CORE_ROOT/ilasm" -output=GetGeneration.asm.exe  GetGeneration.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GetGeneration
    if [ ! -f GetGeneration.org ]; then
        TakeLock
        if [ ! -f GetGeneration.org ]; then
          mkdir IL
          cp GetGeneration.exe IL/GetGeneration.exe
          mv GetGeneration.exe GetGeneration.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetGeneration.org /out GetGeneration.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetGeneration.org /out GetGeneration.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in GetGeneration.reflect.xml
  if [ -f GetGeneration.reflect.xml ]
then
    ReflectionRoots="-x GetGeneration.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GetGeneration.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GetGeneration.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GetGeneration.asm.exe $CLRTestExecutionArguments
