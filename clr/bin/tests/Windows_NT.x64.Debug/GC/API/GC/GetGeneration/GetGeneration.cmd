...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\GetGeneration GetGeneration.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\GetGeneration GetGeneration.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GetGeneration.dasm.il GetGeneration.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GetGeneration.dasm.il GetGeneration.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GetGeneration.asm.exe  GetGeneration.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GetGeneration.asm.exe  GetGeneration.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GetGeneration
    if not exist "GetGeneration.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GetGeneration.org" (
            mkdir IL
            copy GetGeneration.exe IL\GetGeneration.exe 
            ren GetGeneration.exe GetGeneration.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetGeneration.org /out GetGeneration.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetGeneration.org /out GetGeneration.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GetGeneration.reflect.xml
    IF EXIST GetGeneration.reflect.xml set ReflectionRoots=-x GetGeneration.reflect.xml
    ...
    set ExePath=%LinkBin%\GetGeneration.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GetGeneration.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GetGeneration.asm.exe %CLRTestExecutionArguments% %Host_Args%
