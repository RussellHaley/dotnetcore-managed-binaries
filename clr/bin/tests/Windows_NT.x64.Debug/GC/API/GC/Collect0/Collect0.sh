...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect0 Collect0.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect0 Collect0.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Collect0.dasm.il Collect0.exe
    "$CORE_ROOT/ildasm" -raweh -out=Collect0.dasm.il Collect0.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Collect0.asm.exe  Collect0.dasm.il
    "$CORE_ROOT/ilasm" -output=Collect0.asm.exe  Collect0.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Collect0
    if [ ! -f Collect0.org ]; then
        TakeLock
        if [ ! -f Collect0.org ]; then
          mkdir IL
          cp Collect0.exe IL/Collect0.exe
          mv Collect0.exe Collect0.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect0.org /out Collect0.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect0.org /out Collect0.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Collect0.reflect.xml
  if [ -f Collect0.reflect.xml ]
then
    ReflectionRoots="-x Collect0.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Collect0.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Collect0.asm.exe $CLRTestExecutionArguments
