...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect0 Collect0.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect0 Collect0.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Collect0.dasm.il Collect0.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Collect0.dasm.il Collect0.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Collect0.asm.exe  Collect0.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Collect0.asm.exe  Collect0.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Collect0
    if not exist "Collect0.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Collect0.org" (
            mkdir IL
            copy Collect0.exe IL\Collect0.exe 
            ren Collect0.exe Collect0.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect0.org /out Collect0.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect0.org /out Collect0.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Collect0.reflect.xml
    IF EXIST Collect0.reflect.xml set ReflectionRoots=-x Collect0.reflect.xml
    ...
    set ExePath=%LinkBin%\Collect0.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Collect0.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Collect0.asm.exe %CLRTestExecutionArguments% %Host_Args%
