...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\RemoveUsageTest RemoveUsageTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\RemoveUsageTest RemoveUsageTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=RemoveUsageTest.dasm.il RemoveUsageTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=RemoveUsageTest.dasm.il RemoveUsageTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=RemoveUsageTest.asm.exe  RemoveUsageTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=RemoveUsageTest.asm.exe  RemoveUsageTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=RemoveUsageTest
    if not exist "RemoveUsageTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "RemoveUsageTest.org" (
            mkdir IL
            copy RemoveUsageTest.exe IL\RemoveUsageTest.exe 
            ren RemoveUsageTest.exe RemoveUsageTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RemoveUsageTest.org /out RemoveUsageTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in RemoveUsageTest.org /out RemoveUsageTest.exe
...
set ExePath=RemoveUsageTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in RemoveUsageTest.reflect.xml
    IF EXIST RemoveUsageTest.reflect.xml set ReflectionRoots=-x RemoveUsageTest.reflect.xml
    ...
    set ExePath=%LinkBin%\RemoveUsageTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% RemoveUsageTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% RemoveUsageTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
