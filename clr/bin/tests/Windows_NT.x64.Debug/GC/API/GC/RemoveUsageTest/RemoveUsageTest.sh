...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/RemoveUsageTest RemoveUsageTest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/RemoveUsageTest RemoveUsageTest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=RemoveUsageTest.dasm.il RemoveUsageTest.exe
    "$CORE_ROOT/ildasm" -raweh -out=RemoveUsageTest.dasm.il RemoveUsageTest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=RemoveUsageTest.asm.exe  RemoveUsageTest.dasm.il
    "$CORE_ROOT/ilasm" -output=RemoveUsageTest.asm.exe  RemoveUsageTest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=RemoveUsageTest
    if [ ! -f RemoveUsageTest.org ]; then
        TakeLock
        if [ ! -f RemoveUsageTest.org ]; then
          mkdir IL
          cp RemoveUsageTest.exe IL/RemoveUsageTest.exe
          mv RemoveUsageTest.exe RemoveUsageTest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RemoveUsageTest.org /out RemoveUsageTest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in RemoveUsageTest.org /out RemoveUsageTest.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in RemoveUsageTest.reflect.xml
  if [ -f RemoveUsageTest.reflect.xml ]
then
    ReflectionRoots="-x RemoveUsageTest.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/RemoveUsageTest.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" RemoveUsageTest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" RemoveUsageTest.asm.exe $CLRTestExecutionArguments
