...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect1 Collect1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect1 Collect1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Collect1.dasm.il Collect1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Collect1.dasm.il Collect1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Collect1.asm.exe  Collect1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Collect1.asm.exe  Collect1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Collect1
    if not exist "Collect1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Collect1.org" (
            mkdir IL
            copy Collect1.exe IL\Collect1.exe 
            ren Collect1.exe Collect1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect1.org /out Collect1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect1.org /out Collect1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Collect1.reflect.xml
    IF EXIST Collect1.reflect.xml set ReflectionRoots=-x Collect1.reflect.xml
    ...
    set ExePath=%LinkBin%\Collect1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Collect1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Collect1.asm.exe %CLRTestExecutionArguments% %Host_Args%
