...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect1 Collect1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect1 Collect1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Collect1.dasm.il Collect1.exe
    "$CORE_ROOT/ildasm" -raweh -out=Collect1.dasm.il Collect1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Collect1.asm.exe  Collect1.dasm.il
    "$CORE_ROOT/ilasm" -output=Collect1.asm.exe  Collect1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Collect1
    if [ ! -f Collect1.org ]; then
        TakeLock
        if [ ! -f Collect1.org ]; then
          mkdir IL
          cp Collect1.exe IL/Collect1.exe
          mv Collect1.exe Collect1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect1.org /out Collect1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect1.org /out Collect1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Collect1.reflect.xml
  if [ -f Collect1.reflect.xml ]
then
    ReflectionRoots="-x Collect1.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Collect1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Collect1.asm.exe $CLRTestExecutionArguments
