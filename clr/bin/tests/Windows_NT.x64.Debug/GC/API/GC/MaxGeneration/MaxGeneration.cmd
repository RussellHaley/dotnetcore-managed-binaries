...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\MaxGeneration MaxGeneration.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\MaxGeneration MaxGeneration.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=MaxGeneration.dasm.il MaxGeneration.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=MaxGeneration.dasm.il MaxGeneration.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=MaxGeneration.asm.exe  MaxGeneration.dasm.il
    %CORE_ROOT%\ilasm.exe /output=MaxGeneration.asm.exe  MaxGeneration.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=MaxGeneration
    if not exist "MaxGeneration.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "MaxGeneration.org" (
            mkdir IL
            copy MaxGeneration.exe IL\MaxGeneration.exe 
            ren MaxGeneration.exe MaxGeneration.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MaxGeneration.org /out MaxGeneration.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in MaxGeneration.org /out MaxGeneration.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in MaxGeneration.reflect.xml
    IF EXIST MaxGeneration.reflect.xml set ReflectionRoots=-x MaxGeneration.reflect.xml
    ...
    set ExePath=%LinkBin%\MaxGeneration.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% MaxGeneration.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% MaxGeneration.asm.exe %CLRTestExecutionArguments% %Host_Args%
