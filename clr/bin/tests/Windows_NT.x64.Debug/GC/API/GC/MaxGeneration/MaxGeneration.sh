...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/MaxGeneration MaxGeneration.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/MaxGeneration MaxGeneration.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=MaxGeneration.dasm.il MaxGeneration.exe
    "$CORE_ROOT/ildasm" -raweh -out=MaxGeneration.dasm.il MaxGeneration.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=MaxGeneration.asm.exe  MaxGeneration.dasm.il
    "$CORE_ROOT/ilasm" -output=MaxGeneration.asm.exe  MaxGeneration.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=MaxGeneration
    if [ ! -f MaxGeneration.org ]; then
        TakeLock
        if [ ! -f MaxGeneration.org ]; then
          mkdir IL
          cp MaxGeneration.exe IL/MaxGeneration.exe
          mv MaxGeneration.exe MaxGeneration.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MaxGeneration.org /out MaxGeneration.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in MaxGeneration.org /out MaxGeneration.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in MaxGeneration.reflect.xml
  if [ -f MaxGeneration.reflect.xml ]
then
    ReflectionRoots="-x MaxGeneration.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/MaxGeneration.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" MaxGeneration.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" MaxGeneration.asm.exe $CLRTestExecutionArguments
