...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/AddThresholdTest AddThresholdTest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/AddThresholdTest AddThresholdTest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AddThresholdTest.dasm.il AddThresholdTest.exe
    "$CORE_ROOT/ildasm" -raweh -out=AddThresholdTest.dasm.il AddThresholdTest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AddThresholdTest.asm.exe  AddThresholdTest.dasm.il
    "$CORE_ROOT/ilasm" -output=AddThresholdTest.asm.exe  AddThresholdTest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AddThresholdTest
    if [ ! -f AddThresholdTest.org ]; then
        TakeLock
        if [ ! -f AddThresholdTest.org ]; then
          mkdir IL
          cp AddThresholdTest.exe IL/AddThresholdTest.exe
          mv AddThresholdTest.exe AddThresholdTest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddThresholdTest.org /out AddThresholdTest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddThresholdTest.org /out AddThresholdTest.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in AddThresholdTest.reflect.xml
  if [ -f AddThresholdTest.reflect.xml ]
then
    ReflectionRoots="-x AddThresholdTest.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AddThresholdTest.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AddThresholdTest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AddThresholdTest.asm.exe $CLRTestExecutionArguments
