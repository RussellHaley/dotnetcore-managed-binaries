...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\AddThresholdTest AddThresholdTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\AddThresholdTest AddThresholdTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AddThresholdTest.dasm.il AddThresholdTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AddThresholdTest.dasm.il AddThresholdTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AddThresholdTest.asm.exe  AddThresholdTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AddThresholdTest.asm.exe  AddThresholdTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AddThresholdTest
    if not exist "AddThresholdTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AddThresholdTest.org" (
            mkdir IL
            copy AddThresholdTest.exe IL\AddThresholdTest.exe 
            ren AddThresholdTest.exe AddThresholdTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddThresholdTest.org /out AddThresholdTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddThresholdTest.org /out AddThresholdTest.exe
...
set ExePath=AddThresholdTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AddThresholdTest.reflect.xml
    IF EXIST AddThresholdTest.reflect.xml set ReflectionRoots=-x AddThresholdTest.reflect.xml
    ...
    set ExePath=%LinkBin%\AddThresholdTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AddThresholdTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AddThresholdTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
