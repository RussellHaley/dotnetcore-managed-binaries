...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\AddMemoryPressureTest AddMemoryPressureTest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\AddMemoryPressureTest AddMemoryPressureTest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AddMemoryPressureTest.dasm.il AddMemoryPressureTest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AddMemoryPressureTest.dasm.il AddMemoryPressureTest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AddMemoryPressureTest.asm.exe  AddMemoryPressureTest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AddMemoryPressureTest.asm.exe  AddMemoryPressureTest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AddMemoryPressureTest
    if not exist "AddMemoryPressureTest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AddMemoryPressureTest.org" (
            mkdir IL
            copy AddMemoryPressureTest.exe IL\AddMemoryPressureTest.exe 
            ren AddMemoryPressureTest.exe AddMemoryPressureTest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddMemoryPressureTest.org /out AddMemoryPressureTest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddMemoryPressureTest.org /out AddMemoryPressureTest.exe
...
set ExePath=AddMemoryPressureTest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AddMemoryPressureTest.reflect.xml
    IF EXIST AddMemoryPressureTest.reflect.xml set ReflectionRoots=-x AddMemoryPressureTest.reflect.xml
    ...
    set ExePath=%LinkBin%\AddMemoryPressureTest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AddMemoryPressureTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AddMemoryPressureTest.asm.exe %CLRTestExecutionArguments% %Host_Args%
