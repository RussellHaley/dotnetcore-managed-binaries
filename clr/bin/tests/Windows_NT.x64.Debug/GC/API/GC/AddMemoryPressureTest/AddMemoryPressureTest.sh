...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/AddMemoryPressureTest AddMemoryPressureTest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/AddMemoryPressureTest AddMemoryPressureTest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AddMemoryPressureTest.dasm.il AddMemoryPressureTest.exe
    "$CORE_ROOT/ildasm" -raweh -out=AddMemoryPressureTest.dasm.il AddMemoryPressureTest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AddMemoryPressureTest.asm.exe  AddMemoryPressureTest.dasm.il
    "$CORE_ROOT/ilasm" -output=AddMemoryPressureTest.asm.exe  AddMemoryPressureTest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AddMemoryPressureTest
    if [ ! -f AddMemoryPressureTest.org ]; then
        TakeLock
        if [ ! -f AddMemoryPressureTest.org ]; then
          mkdir IL
          cp AddMemoryPressureTest.exe IL/AddMemoryPressureTest.exe
          mv AddMemoryPressureTest.exe AddMemoryPressureTest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddMemoryPressureTest.org /out AddMemoryPressureTest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddMemoryPressureTest.org /out AddMemoryPressureTest.exe
          __cgExitCode=$?
...
ExePath=AddMemoryPressureTest.exe
...
then
  ...
  # Use hints for reflection roots, if provided in AddMemoryPressureTest.reflect.xml
  if [ -f AddMemoryPressureTest.reflect.xml ]
then
    ReflectionRoots="-x AddMemoryPressureTest.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AddMemoryPressureTest.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AddMemoryPressureTest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AddMemoryPressureTest.asm.exe $CLRTestExecutionArguments
