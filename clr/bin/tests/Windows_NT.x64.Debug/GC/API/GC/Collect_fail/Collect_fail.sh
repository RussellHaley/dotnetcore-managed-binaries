...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect_fail Collect_fail.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/Collect_fail Collect_fail.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Collect_fail.dasm.il Collect_fail.exe
    "$CORE_ROOT/ildasm" -raweh -out=Collect_fail.dasm.il Collect_fail.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Collect_fail.asm.exe  Collect_fail.dasm.il
    "$CORE_ROOT/ilasm" -output=Collect_fail.asm.exe  Collect_fail.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Collect_fail
    if [ ! -f Collect_fail.org ]; then
        TakeLock
        if [ ! -f Collect_fail.org ]; then
          mkdir IL
          cp Collect_fail.exe IL/Collect_fail.exe
          mv Collect_fail.exe Collect_fail.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect_fail.org /out Collect_fail.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Collect_fail.org /out Collect_fail.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Collect_fail.reflect.xml
  if [ -f Collect_fail.reflect.xml ]
then
    ReflectionRoots="-x Collect_fail.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Collect_fail.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Collect_fail.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Collect_fail.asm.exe $CLRTestExecutionArguments
