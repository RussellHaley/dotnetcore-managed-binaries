...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect_fail Collect_fail.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\Collect_fail Collect_fail.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Collect_fail.dasm.il Collect_fail.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Collect_fail.dasm.il Collect_fail.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Collect_fail.asm.exe  Collect_fail.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Collect_fail.asm.exe  Collect_fail.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Collect_fail
    if not exist "Collect_fail.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Collect_fail.org" (
            mkdir IL
            copy Collect_fail.exe IL\Collect_fail.exe 
            ren Collect_fail.exe Collect_fail.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect_fail.org /out Collect_fail.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Collect_fail.org /out Collect_fail.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Collect_fail.reflect.xml
    IF EXIST Collect_fail.reflect.xml set ReflectionRoots=-x Collect_fail.reflect.xml
    ...
    set ExePath=%LinkBin%\Collect_fail.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Collect_fail.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Collect_fail.asm.exe %CLRTestExecutionArguments% %Host_Args%
