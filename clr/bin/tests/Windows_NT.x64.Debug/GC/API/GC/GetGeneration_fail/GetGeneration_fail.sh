...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/GetGeneration_fail GetGeneration_fail.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GC/GetGeneration_fail GetGeneration_fail.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=GetGeneration_fail.dasm.il GetGeneration_fail.exe
    "$CORE_ROOT/ildasm" -raweh -out=GetGeneration_fail.dasm.il GetGeneration_fail.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=GetGeneration_fail.asm.exe  GetGeneration_fail.dasm.il
    "$CORE_ROOT/ilasm" -output=GetGeneration_fail.asm.exe  GetGeneration_fail.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=GetGeneration_fail
    if [ ! -f GetGeneration_fail.org ]; then
        TakeLock
        if [ ! -f GetGeneration_fail.org ]; then
          mkdir IL
          cp GetGeneration_fail.exe IL/GetGeneration_fail.exe
          mv GetGeneration_fail.exe GetGeneration_fail.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetGeneration_fail.org /out GetGeneration_fail.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in GetGeneration_fail.org /out GetGeneration_fail.exe
          __cgExitCode=$?
...
ExePath=GetGeneration_fail.exe
...
then
  ...
  # Use hints for reflection roots, if provided in GetGeneration_fail.reflect.xml
  if [ -f GetGeneration_fail.reflect.xml ]
then
    ReflectionRoots="-x GetGeneration_fail.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/GetGeneration_fail.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" GetGeneration_fail.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" GetGeneration_fail.asm.exe $CLRTestExecutionArguments
