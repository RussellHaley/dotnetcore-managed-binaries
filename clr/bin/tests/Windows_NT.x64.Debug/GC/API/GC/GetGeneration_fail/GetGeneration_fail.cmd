...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\GetGeneration_fail GetGeneration_fail.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GC\GetGeneration_fail GetGeneration_fail.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=GetGeneration_fail.dasm.il GetGeneration_fail.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=GetGeneration_fail.dasm.il GetGeneration_fail.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=GetGeneration_fail.asm.exe  GetGeneration_fail.dasm.il
    %CORE_ROOT%\ilasm.exe /output=GetGeneration_fail.asm.exe  GetGeneration_fail.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=GetGeneration_fail
    if not exist "GetGeneration_fail.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "GetGeneration_fail.org" (
            mkdir IL
            copy GetGeneration_fail.exe IL\GetGeneration_fail.exe 
            ren GetGeneration_fail.exe GetGeneration_fail.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetGeneration_fail.org /out GetGeneration_fail.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in GetGeneration_fail.org /out GetGeneration_fail.exe
...
set ExePath=GetGeneration_fail.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in GetGeneration_fail.reflect.xml
    IF EXIST GetGeneration_fail.reflect.xml set ReflectionRoots=-x GetGeneration_fail.reflect.xml
    ...
    set ExePath=%LinkBin%\GetGeneration_fail.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% GetGeneration_fail.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% GetGeneration_fail.asm.exe %CLRTestExecutionArguments% %Host_Args%
