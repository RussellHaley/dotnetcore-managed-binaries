...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandleCollector\Usage Usage.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandleCollector\Usage Usage.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Usage.dasm.il Usage.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Usage.dasm.il Usage.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Usage.asm.exe  Usage.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Usage.asm.exe  Usage.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Usage
    if not exist "Usage.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Usage.org" (
            mkdir IL
            copy Usage.exe IL\Usage.exe 
            ren Usage.exe Usage.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Usage.org /out Usage.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Usage.org /out Usage.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Usage.reflect.xml
    IF EXIST Usage.reflect.xml set ReflectionRoots=-x Usage.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Usage.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Usage.asm.exe %CLRTestExecutionArguments% %Host_Args%
