...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandleCollector/Usage Usage.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandleCollector/Usage Usage.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Usage.dasm.il Usage.exe
    "$CORE_ROOT/ildasm" -raweh -out=Usage.dasm.il Usage.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Usage.asm.exe  Usage.dasm.il
    "$CORE_ROOT/ilasm" -output=Usage.asm.exe  Usage.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Usage
    if [ ! -f Usage.org ]; then
        TakeLock
        if [ ! -f Usage.org ]; then
          mkdir IL
          cp Usage.exe IL/Usage.exe
          mv Usage.exe Usage.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Usage.org /out Usage.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Usage.org /out Usage.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Usage.reflect.xml
  if [ -f Usage.reflect.xml ]
then
    ReflectionRoots="-x Usage.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Usage.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Usage.asm.exe $CLRTestExecutionArguments
