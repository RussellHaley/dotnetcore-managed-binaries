...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandleCollector\Count Count.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandleCollector\Count Count.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Count.dasm.il Count.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Count.dasm.il Count.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Count.asm.exe  Count.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Count.asm.exe  Count.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Count
    if not exist "Count.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Count.org" (
            mkdir IL
            copy Count.exe IL\Count.exe 
            ren Count.exe Count.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Count.org /out Count.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Count.org /out Count.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Count.reflect.xml
    IF EXIST Count.reflect.xml set ReflectionRoots=-x Count.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Count.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Count.asm.exe %CLRTestExecutionArguments% %Host_Args%
