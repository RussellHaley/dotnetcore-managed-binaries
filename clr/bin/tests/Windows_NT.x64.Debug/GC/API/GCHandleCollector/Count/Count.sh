...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandleCollector/Count Count.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandleCollector/Count Count.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Count.dasm.il Count.exe
    "$CORE_ROOT/ildasm" -raweh -out=Count.dasm.il Count.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Count.asm.exe  Count.dasm.il
    "$CORE_ROOT/ilasm" -output=Count.asm.exe  Count.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Count
    if [ ! -f Count.org ]; then
        TakeLock
        if [ ! -f Count.org ]; then
          mkdir IL
          cp Count.exe IL/Count.exe
          mv Count.exe Count.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Count.org /out Count.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Count.org /out Count.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Count.reflect.xml
  if [ -f Count.reflect.xml ]
then
    ReflectionRoots="-x Count.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Count.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Count.asm.exe $CLRTestExecutionArguments
