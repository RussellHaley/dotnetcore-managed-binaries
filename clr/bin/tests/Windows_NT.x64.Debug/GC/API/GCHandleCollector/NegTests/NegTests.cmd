...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandleCollector\NegTests NegTests.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandleCollector\NegTests NegTests.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NegTests.dasm.il NegTests.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NegTests.dasm.il NegTests.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NegTests.asm.exe  NegTests.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NegTests.asm.exe  NegTests.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NegTests
    if not exist "NegTests.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NegTests.org" (
            mkdir IL
            copy NegTests.exe IL\NegTests.exe 
            ren NegTests.exe NegTests.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NegTests.org /out NegTests.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NegTests.org /out NegTests.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NegTests.reflect.xml
    IF EXIST NegTests.reflect.xml set ReflectionRoots=-x NegTests.reflect.xml
    ...
    set ExePath=%LinkBin%\NegTests.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NegTests.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NegTests.asm.exe %CLRTestExecutionArguments% %Host_Args%
