...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandleCollector/NegTests NegTests.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandleCollector/NegTests NegTests.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NegTests.dasm.il NegTests.exe
    "$CORE_ROOT/ildasm" -raweh -out=NegTests.dasm.il NegTests.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NegTests.asm.exe  NegTests.dasm.il
    "$CORE_ROOT/ilasm" -output=NegTests.asm.exe  NegTests.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NegTests
    if [ ! -f NegTests.org ]; then
        TakeLock
        if [ ! -f NegTests.org ]; then
          mkdir IL
          cp NegTests.exe IL/NegTests.exe
          mv NegTests.exe NegTests.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NegTests.org /out NegTests.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NegTests.org /out NegTests.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NegTests.reflect.xml
  if [ -f NegTests.reflect.xml ]
then
    ReflectionRoots="-x NegTests.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NegTests.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NegTests.asm.exe $CLRTestExecutionArguments
