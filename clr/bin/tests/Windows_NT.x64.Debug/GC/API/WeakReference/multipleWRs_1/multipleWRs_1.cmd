...
if not defined CLRTestExecutionArguments (set CLRTestExecutionArguments=10000 track )
...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\WeakReference\multipleWRs_1 multipleWRs_1.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\WeakReference\multipleWRs_1 multipleWRs_1.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=multipleWRs_1.dasm.il multipleWRs_1.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=multipleWRs_1.dasm.il multipleWRs_1.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=multipleWRs_1.asm.exe  multipleWRs_1.dasm.il
    %CORE_ROOT%\ilasm.exe /output=multipleWRs_1.asm.exe  multipleWRs_1.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=multipleWRs_1
    if not exist "multipleWRs_1.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "multipleWRs_1.org" (
            mkdir IL
            copy multipleWRs_1.exe IL\multipleWRs_1.exe 
            ren multipleWRs_1.exe multipleWRs_1.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multipleWRs_1.org /out multipleWRs_1.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multipleWRs_1.org /out multipleWRs_1.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in multipleWRs_1.reflect.xml
    IF EXIST multipleWRs_1.reflect.xml set ReflectionRoots=-x multipleWRs_1.reflect.xml
    ...
    set ExePath=%LinkBin%\multipleWRs_1.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% multipleWRs_1.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% multipleWRs_1.asm.exe %CLRTestExecutionArguments% %Host_Args%
