...
if [ -z ${CLRTestExecutionArguments+x} ]; then export CLRTestExecutionArguments='10000 track'; fi
...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/WeakReference/multipleWRs_1 multipleWRs_1.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/WeakReference/multipleWRs_1 multipleWRs_1.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=multipleWRs_1.dasm.il multipleWRs_1.exe
    "$CORE_ROOT/ildasm" -raweh -out=multipleWRs_1.dasm.il multipleWRs_1.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=multipleWRs_1.asm.exe  multipleWRs_1.dasm.il
    "$CORE_ROOT/ilasm" -output=multipleWRs_1.asm.exe  multipleWRs_1.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=multipleWRs_1
    if [ ! -f multipleWRs_1.org ]; then
        TakeLock
        if [ ! -f multipleWRs_1.org ]; then
          mkdir IL
          cp multipleWRs_1.exe IL/multipleWRs_1.exe
          mv multipleWRs_1.exe multipleWRs_1.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multipleWRs_1.org /out multipleWRs_1.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multipleWRs_1.org /out multipleWRs_1.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in multipleWRs_1.reflect.xml
  if [ -f multipleWRs_1.reflect.xml ]
then
    ReflectionRoots="-x multipleWRs_1.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/multipleWRs_1.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" multipleWRs_1.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" multipleWRs_1.asm.exe $CLRTestExecutionArguments
