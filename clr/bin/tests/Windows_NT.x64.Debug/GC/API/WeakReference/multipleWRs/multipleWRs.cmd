...
if not defined CLRTestExecutionArguments (set CLRTestExecutionArguments=10000 )
...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\WeakReference\multipleWRs multipleWRs.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\WeakReference\multipleWRs multipleWRs.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=multipleWRs.dasm.il multipleWRs.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=multipleWRs.dasm.il multipleWRs.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=multipleWRs.asm.exe  multipleWRs.dasm.il
    %CORE_ROOT%\ilasm.exe /output=multipleWRs.asm.exe  multipleWRs.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=multipleWRs
    if not exist "multipleWRs.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "multipleWRs.org" (
            mkdir IL
            copy multipleWRs.exe IL\multipleWRs.exe 
            ren multipleWRs.exe multipleWRs.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multipleWRs.org /out multipleWRs.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in multipleWRs.org /out multipleWRs.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in multipleWRs.reflect.xml
    IF EXIST multipleWRs.reflect.xml set ReflectionRoots=-x multipleWRs.reflect.xml
    ...
    set ExePath=%LinkBin%\multipleWRs.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% multipleWRs.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% multipleWRs.asm.exe %CLRTestExecutionArguments% %Host_Args%
