...
if [ -z ${CLRTestExecutionArguments+x} ]; then export CLRTestExecutionArguments='10000'; fi
...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/WeakReference/multipleWRs multipleWRs.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/WeakReference/multipleWRs multipleWRs.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=multipleWRs.dasm.il multipleWRs.exe
    "$CORE_ROOT/ildasm" -raweh -out=multipleWRs.dasm.il multipleWRs.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=multipleWRs.asm.exe  multipleWRs.dasm.il
    "$CORE_ROOT/ilasm" -output=multipleWRs.asm.exe  multipleWRs.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=multipleWRs
    if [ ! -f multipleWRs.org ]; then
        TakeLock
        if [ ! -f multipleWRs.org ]; then
          mkdir IL
          cp multipleWRs.exe IL/multipleWRs.exe
          mv multipleWRs.exe multipleWRs.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multipleWRs.org /out multipleWRs.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in multipleWRs.org /out multipleWRs.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in multipleWRs.reflect.xml
  if [ -f multipleWRs.reflect.xml ]
then
    ReflectionRoots="-x multipleWRs.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/multipleWRs.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" multipleWRs.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" multipleWRs.asm.exe $CLRTestExecutionArguments
