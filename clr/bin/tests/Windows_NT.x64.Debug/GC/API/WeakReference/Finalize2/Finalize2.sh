...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/WeakReference/Finalize2 Finalize2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/WeakReference/Finalize2 Finalize2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Finalize2.dasm.il Finalize2.exe
    "$CORE_ROOT/ildasm" -raweh -out=Finalize2.dasm.il Finalize2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Finalize2.asm.exe  Finalize2.dasm.il
    "$CORE_ROOT/ilasm" -output=Finalize2.asm.exe  Finalize2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Finalize2
    if [ ! -f Finalize2.org ]; then
        TakeLock
        if [ ! -f Finalize2.org ]; then
          mkdir IL
          cp Finalize2.exe IL/Finalize2.exe
          mv Finalize2.exe Finalize2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Finalize2.org /out Finalize2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Finalize2.org /out Finalize2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Finalize2.reflect.xml
  if [ -f Finalize2.reflect.xml ]
then
    ReflectionRoots="-x Finalize2.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Finalize2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Finalize2.asm.exe $CLRTestExecutionArguments
