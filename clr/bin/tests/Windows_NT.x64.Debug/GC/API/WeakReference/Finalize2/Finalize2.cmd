...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\WeakReference\Finalize2 Finalize2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\WeakReference\Finalize2 Finalize2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Finalize2.dasm.il Finalize2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Finalize2.dasm.il Finalize2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Finalize2.asm.exe  Finalize2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Finalize2.asm.exe  Finalize2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Finalize2
    if not exist "Finalize2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Finalize2.org" (
            mkdir IL
            copy Finalize2.exe IL\Finalize2.exe 
            ren Finalize2.exe Finalize2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Finalize2.org /out Finalize2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Finalize2.org /out Finalize2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Finalize2.reflect.xml
    IF EXIST Finalize2.reflect.xml set ReflectionRoots=-x Finalize2.reflect.xml
    ...
    set ExePath=%LinkBin%\Finalize2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Finalize2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Finalize2.asm.exe %CLRTestExecutionArguments% %Host_Args%
