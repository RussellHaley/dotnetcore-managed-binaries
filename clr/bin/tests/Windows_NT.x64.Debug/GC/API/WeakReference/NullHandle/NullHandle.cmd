...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\WeakReference\NullHandle NullHandle.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\WeakReference\NullHandle NullHandle.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NullHandle.dasm.il NullHandle.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NullHandle.dasm.il NullHandle.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NullHandle.asm.exe  NullHandle.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NullHandle.asm.exe  NullHandle.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NullHandle
    if not exist "NullHandle.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NullHandle.org" (
            mkdir IL
            copy NullHandle.exe IL\NullHandle.exe 
            ren NullHandle.exe NullHandle.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NullHandle.org /out NullHandle.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NullHandle.org /out NullHandle.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NullHandle.reflect.xml
    IF EXIST NullHandle.reflect.xml set ReflectionRoots=-x NullHandle.reflect.xml
    ...
    set ExePath=%LinkBin%\NullHandle.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NullHandle.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NullHandle.asm.exe %CLRTestExecutionArguments% %Host_Args%
