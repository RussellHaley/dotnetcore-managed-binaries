...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/WeakReference/NullHandle NullHandle.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/WeakReference/NullHandle NullHandle.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NullHandle.dasm.il NullHandle.exe
    "$CORE_ROOT/ildasm" -raweh -out=NullHandle.dasm.il NullHandle.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NullHandle.asm.exe  NullHandle.dasm.il
    "$CORE_ROOT/ilasm" -output=NullHandle.asm.exe  NullHandle.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NullHandle
    if [ ! -f NullHandle.org ]; then
        TakeLock
        if [ ! -f NullHandle.org ]; then
          mkdir IL
          cp NullHandle.exe IL/NullHandle.exe
          mv NullHandle.exe NullHandle.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NullHandle.org /out NullHandle.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NullHandle.org /out NullHandle.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NullHandle.reflect.xml
  if [ -f NullHandle.reflect.xml ]
then
    ReflectionRoots="-x NullHandle.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/NullHandle.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NullHandle.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NullHandle.asm.exe $CLRTestExecutionArguments
