...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\PinObj_neg PinObj_neg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\PinObj_neg PinObj_neg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=PinObj_neg.dasm.il PinObj_neg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=PinObj_neg.dasm.il PinObj_neg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=PinObj_neg.asm.exe  PinObj_neg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=PinObj_neg.asm.exe  PinObj_neg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=PinObj_neg
    if not exist "PinObj_neg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "PinObj_neg.org" (
            mkdir IL
            copy PinObj_neg.exe IL\PinObj_neg.exe 
            ren PinObj_neg.exe PinObj_neg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinObj_neg.org /out PinObj_neg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in PinObj_neg.org /out PinObj_neg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in PinObj_neg.reflect.xml
    IF EXIST PinObj_neg.reflect.xml set ReflectionRoots=-x PinObj_neg.reflect.xml
    ...
    set ExePath=%LinkBin%\PinObj_neg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% PinObj_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% PinObj_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
