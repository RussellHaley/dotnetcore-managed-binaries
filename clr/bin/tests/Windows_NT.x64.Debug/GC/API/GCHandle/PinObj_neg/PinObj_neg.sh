...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/PinObj_neg PinObj_neg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/PinObj_neg PinObj_neg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=PinObj_neg.dasm.il PinObj_neg.exe
    "$CORE_ROOT/ildasm" -raweh -out=PinObj_neg.dasm.il PinObj_neg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=PinObj_neg.asm.exe  PinObj_neg.dasm.il
    "$CORE_ROOT/ilasm" -output=PinObj_neg.asm.exe  PinObj_neg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=PinObj_neg
    if [ ! -f PinObj_neg.org ]; then
        TakeLock
        if [ ! -f PinObj_neg.org ]; then
          mkdir IL
          cp PinObj_neg.exe IL/PinObj_neg.exe
          mv PinObj_neg.exe PinObj_neg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinObj_neg.org /out PinObj_neg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in PinObj_neg.org /out PinObj_neg.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in PinObj_neg.reflect.xml
  if [ -f PinObj_neg.reflect.xml ]
then
    ReflectionRoots="-x PinObj_neg.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/PinObj_neg.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" PinObj_neg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" PinObj_neg.asm.exe $CLRTestExecutionArguments
