...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\Target_neg Target_neg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\Target_neg Target_neg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Target_neg.dasm.il Target_neg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Target_neg.dasm.il Target_neg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Target_neg.asm.exe  Target_neg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Target_neg.asm.exe  Target_neg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Target_neg
    if not exist "Target_neg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Target_neg.org" (
            mkdir IL
            copy Target_neg.exe IL\Target_neg.exe 
            ren Target_neg.exe Target_neg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Target_neg.org /out Target_neg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Target_neg.org /out Target_neg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Target_neg.reflect.xml
    IF EXIST Target_neg.reflect.xml set ReflectionRoots=-x Target_neg.reflect.xml
    ...
    set ExePath=%LinkBin%\Target_neg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Target_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Target_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
