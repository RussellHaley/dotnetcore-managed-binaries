...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/Target_neg Target_neg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/Target_neg Target_neg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Target_neg.dasm.il Target_neg.exe
    "$CORE_ROOT/ildasm" -raweh -out=Target_neg.dasm.il Target_neg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Target_neg.asm.exe  Target_neg.dasm.il
    "$CORE_ROOT/ilasm" -output=Target_neg.asm.exe  Target_neg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Target_neg
    if [ ! -f Target_neg.org ]; then
        TakeLock
        if [ ! -f Target_neg.org ]; then
          mkdir IL
          cp Target_neg.exe IL/Target_neg.exe
          mv Target_neg.exe Target_neg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Target_neg.org /out Target_neg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Target_neg.org /out Target_neg.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Target_neg.reflect.xml
  if [ -f Target_neg.reflect.xml ]
then
    ReflectionRoots="-x Target_neg.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Target_neg.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Target_neg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Target_neg.asm.exe $CLRTestExecutionArguments
