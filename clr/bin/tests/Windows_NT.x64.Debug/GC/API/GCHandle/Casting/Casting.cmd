...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\Casting Casting.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\Casting Casting.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Casting.dasm.il Casting.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Casting.dasm.il Casting.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Casting.asm.exe  Casting.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Casting.asm.exe  Casting.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Casting
    if not exist "Casting.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Casting.org" (
            mkdir IL
            copy Casting.exe IL\Casting.exe 
            ren Casting.exe Casting.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Casting.org /out Casting.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Casting.org /out Casting.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Casting.reflect.xml
    IF EXIST Casting.reflect.xml set ReflectionRoots=-x Casting.reflect.xml
    ...
    set ExePath=%LinkBin%\Casting.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Casting.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Casting.asm.exe %CLRTestExecutionArguments% %Host_Args%
