...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/Casting Casting.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/Casting Casting.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Casting.dasm.il Casting.exe
    "$CORE_ROOT/ildasm" -raweh -out=Casting.dasm.il Casting.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Casting.asm.exe  Casting.dasm.il
    "$CORE_ROOT/ilasm" -output=Casting.asm.exe  Casting.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Casting
    if [ ! -f Casting.org ]; then
        TakeLock
        if [ ! -f Casting.org ]; then
          mkdir IL
          cp Casting.exe IL/Casting.exe
          mv Casting.exe Casting.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Casting.org /out Casting.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Casting.org /out Casting.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Casting.reflect.xml
  if [ -f Casting.reflect.xml ]
then
    ReflectionRoots="-x Casting.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Casting.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Casting.asm.exe $CLRTestExecutionArguments
