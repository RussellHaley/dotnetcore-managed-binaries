...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/Alloc_neg2 Alloc_neg2.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/Alloc_neg2 Alloc_neg2.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Alloc_neg2.dasm.il Alloc_neg2.exe
    "$CORE_ROOT/ildasm" -raweh -out=Alloc_neg2.dasm.il Alloc_neg2.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Alloc_neg2.asm.exe  Alloc_neg2.dasm.il
    "$CORE_ROOT/ilasm" -output=Alloc_neg2.asm.exe  Alloc_neg2.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Alloc_neg2
    if [ ! -f Alloc_neg2.org ]; then
        TakeLock
        if [ ! -f Alloc_neg2.org ]; then
          mkdir IL
          cp Alloc_neg2.exe IL/Alloc_neg2.exe
          mv Alloc_neg2.exe Alloc_neg2.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Alloc_neg2.org /out Alloc_neg2.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Alloc_neg2.org /out Alloc_neg2.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Alloc_neg2.reflect.xml
  if [ -f Alloc_neg2.reflect.xml ]
then
    ReflectionRoots="-x Alloc_neg2.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Alloc_neg2.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Alloc_neg2.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Alloc_neg2.asm.exe $CLRTestExecutionArguments
