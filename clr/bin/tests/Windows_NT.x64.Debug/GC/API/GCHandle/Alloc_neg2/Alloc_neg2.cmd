...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\Alloc_neg2 Alloc_neg2.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\Alloc_neg2 Alloc_neg2.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Alloc_neg2.dasm.il Alloc_neg2.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Alloc_neg2.dasm.il Alloc_neg2.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Alloc_neg2.asm.exe  Alloc_neg2.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Alloc_neg2.asm.exe  Alloc_neg2.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Alloc_neg2
    if not exist "Alloc_neg2.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Alloc_neg2.org" (
            mkdir IL
            copy Alloc_neg2.exe IL\Alloc_neg2.exe 
            ren Alloc_neg2.exe Alloc_neg2.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Alloc_neg2.org /out Alloc_neg2.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Alloc_neg2.org /out Alloc_neg2.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Alloc_neg2.reflect.xml
    IF EXIST Alloc_neg2.reflect.xml set ReflectionRoots=-x Alloc_neg2.reflect.xml
    ...
    set ExePath=%LinkBin%\Alloc_neg2.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Alloc_neg2.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Alloc_neg2.asm.exe %CLRTestExecutionArguments% %Host_Args%
