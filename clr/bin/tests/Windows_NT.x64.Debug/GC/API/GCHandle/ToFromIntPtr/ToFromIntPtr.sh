...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/ToFromIntPtr ToFromIntPtr.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/ToFromIntPtr ToFromIntPtr.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=ToFromIntPtr.dasm.il ToFromIntPtr.exe
    "$CORE_ROOT/ildasm" -raweh -out=ToFromIntPtr.dasm.il ToFromIntPtr.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=ToFromIntPtr.asm.exe  ToFromIntPtr.dasm.il
    "$CORE_ROOT/ilasm" -output=ToFromIntPtr.asm.exe  ToFromIntPtr.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=ToFromIntPtr
    if [ ! -f ToFromIntPtr.org ]; then
        TakeLock
        if [ ! -f ToFromIntPtr.org ]; then
          mkdir IL
          cp ToFromIntPtr.exe IL/ToFromIntPtr.exe
          mv ToFromIntPtr.exe ToFromIntPtr.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ToFromIntPtr.org /out ToFromIntPtr.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in ToFromIntPtr.org /out ToFromIntPtr.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in ToFromIntPtr.reflect.xml
  if [ -f ToFromIntPtr.reflect.xml ]
then
    ReflectionRoots="-x ToFromIntPtr.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/ToFromIntPtr.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" ToFromIntPtr.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" ToFromIntPtr.asm.exe $CLRTestExecutionArguments
