...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\ToFromIntPtr ToFromIntPtr.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\ToFromIntPtr ToFromIntPtr.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=ToFromIntPtr.dasm.il ToFromIntPtr.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=ToFromIntPtr.dasm.il ToFromIntPtr.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=ToFromIntPtr.asm.exe  ToFromIntPtr.dasm.il
    %CORE_ROOT%\ilasm.exe /output=ToFromIntPtr.asm.exe  ToFromIntPtr.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=ToFromIntPtr
    if not exist "ToFromIntPtr.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "ToFromIntPtr.org" (
            mkdir IL
            copy ToFromIntPtr.exe IL\ToFromIntPtr.exe 
            ren ToFromIntPtr.exe ToFromIntPtr.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ToFromIntPtr.org /out ToFromIntPtr.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in ToFromIntPtr.org /out ToFromIntPtr.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in ToFromIntPtr.reflect.xml
    IF EXIST ToFromIntPtr.reflect.xml set ReflectionRoots=-x ToFromIntPtr.reflect.xml
    ...
    set ExePath=%LinkBin%\ToFromIntPtr.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% ToFromIntPtr.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% ToFromIntPtr.asm.exe %CLRTestExecutionArguments% %Host_Args%
