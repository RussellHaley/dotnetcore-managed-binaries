...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\Free_neg Free_neg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\Free_neg Free_neg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Free_neg.dasm.il Free_neg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Free_neg.dasm.il Free_neg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Free_neg.asm.exe  Free_neg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Free_neg.asm.exe  Free_neg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Free_neg
    if not exist "Free_neg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Free_neg.org" (
            mkdir IL
            copy Free_neg.exe IL\Free_neg.exe 
            ren Free_neg.exe Free_neg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Free_neg.org /out Free_neg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Free_neg.org /out Free_neg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Free_neg.reflect.xml
    IF EXIST Free_neg.reflect.xml set ReflectionRoots=-x Free_neg.reflect.xml
    ...
    set ExePath=%LinkBin%\Free_neg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Free_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Free_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
