...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/Free_neg Free_neg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/Free_neg Free_neg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Free_neg.dasm.il Free_neg.exe
    "$CORE_ROOT/ildasm" -raweh -out=Free_neg.dasm.il Free_neg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Free_neg.asm.exe  Free_neg.dasm.il
    "$CORE_ROOT/ilasm" -output=Free_neg.asm.exe  Free_neg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Free_neg
    if [ ! -f Free_neg.org ]; then
        TakeLock
        if [ ! -f Free_neg.org ]; then
          mkdir IL
          cp Free_neg.exe IL/Free_neg.exe
          mv Free_neg.exe Free_neg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Free_neg.org /out Free_neg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Free_neg.org /out Free_neg.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Free_neg.reflect.xml
  if [ -f Free_neg.reflect.xml ]
then
    ReflectionRoots="-x Free_neg.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Free_neg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Free_neg.asm.exe $CLRTestExecutionArguments
