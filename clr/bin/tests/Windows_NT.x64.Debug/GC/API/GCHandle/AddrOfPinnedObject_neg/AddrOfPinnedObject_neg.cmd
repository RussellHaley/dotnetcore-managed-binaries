...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\AddrOfPinnedObject_neg AddrOfPinnedObject_neg.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCHandle\AddrOfPinnedObject_neg AddrOfPinnedObject_neg.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=AddrOfPinnedObject_neg.dasm.il AddrOfPinnedObject_neg.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=AddrOfPinnedObject_neg.dasm.il AddrOfPinnedObject_neg.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=AddrOfPinnedObject_neg.asm.exe  AddrOfPinnedObject_neg.dasm.il
    %CORE_ROOT%\ilasm.exe /output=AddrOfPinnedObject_neg.asm.exe  AddrOfPinnedObject_neg.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=AddrOfPinnedObject_neg
    if not exist "AddrOfPinnedObject_neg.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "AddrOfPinnedObject_neg.org" (
            mkdir IL
            copy AddrOfPinnedObject_neg.exe IL\AddrOfPinnedObject_neg.exe 
            ren AddrOfPinnedObject_neg.exe AddrOfPinnedObject_neg.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddrOfPinnedObject_neg.org /out AddrOfPinnedObject_neg.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in AddrOfPinnedObject_neg.org /out AddrOfPinnedObject_neg.exe
...
set ExePath=AddrOfPinnedObject_neg.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in AddrOfPinnedObject_neg.reflect.xml
    IF EXIST AddrOfPinnedObject_neg.reflect.xml set ReflectionRoots=-x AddrOfPinnedObject_neg.reflect.xml
    ...
    set ExePath=%LinkBin%\AddrOfPinnedObject_neg.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% AddrOfPinnedObject_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% AddrOfPinnedObject_neg.asm.exe %CLRTestExecutionArguments% %Host_Args%
