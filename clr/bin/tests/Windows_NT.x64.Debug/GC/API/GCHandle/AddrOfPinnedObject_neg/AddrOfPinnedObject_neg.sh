...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/AddrOfPinnedObject_neg AddrOfPinnedObject_neg.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCHandle/AddrOfPinnedObject_neg AddrOfPinnedObject_neg.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=AddrOfPinnedObject_neg.dasm.il AddrOfPinnedObject_neg.exe
    "$CORE_ROOT/ildasm" -raweh -out=AddrOfPinnedObject_neg.dasm.il AddrOfPinnedObject_neg.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=AddrOfPinnedObject_neg.asm.exe  AddrOfPinnedObject_neg.dasm.il
    "$CORE_ROOT/ilasm" -output=AddrOfPinnedObject_neg.asm.exe  AddrOfPinnedObject_neg.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=AddrOfPinnedObject_neg
    if [ ! -f AddrOfPinnedObject_neg.org ]; then
        TakeLock
        if [ ! -f AddrOfPinnedObject_neg.org ]; then
          mkdir IL
          cp AddrOfPinnedObject_neg.exe IL/AddrOfPinnedObject_neg.exe
          mv AddrOfPinnedObject_neg.exe AddrOfPinnedObject_neg.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddrOfPinnedObject_neg.org /out AddrOfPinnedObject_neg.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in AddrOfPinnedObject_neg.org /out AddrOfPinnedObject_neg.exe
          __cgExitCode=$?
...
ExePath=AddrOfPinnedObject_neg.exe
...
then
  ...
  # Use hints for reflection roots, if provided in AddrOfPinnedObject_neg.reflect.xml
  if [ -f AddrOfPinnedObject_neg.reflect.xml ]
then
    ReflectionRoots="-x AddrOfPinnedObject_neg.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/AddrOfPinnedObject_neg.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" AddrOfPinnedObject_neg.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" AddrOfPinnedObject_neg.asm.exe $CLRTestExecutionArguments
