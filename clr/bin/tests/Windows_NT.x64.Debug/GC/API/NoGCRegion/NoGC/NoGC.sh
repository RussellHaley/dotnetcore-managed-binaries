...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/NoGCRegion/NoGC NoGC.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/NoGCRegion/NoGC NoGC.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=NoGC.dasm.il NoGC.exe
    "$CORE_ROOT/ildasm" -raweh -out=NoGC.dasm.il NoGC.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=NoGC.asm.exe  NoGC.dasm.il
    "$CORE_ROOT/ilasm" -output=NoGC.asm.exe  NoGC.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=NoGC
    if [ ! -f NoGC.org ]; then
        TakeLock
        if [ ! -f NoGC.org ]; then
          mkdir IL
          cp NoGC.exe IL/NoGC.exe
          mv NoGC.exe NoGC.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NoGC.org /out NoGC.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in NoGC.org /out NoGC.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in NoGC.reflect.xml
  if [ -f NoGC.reflect.xml ]
then
    ReflectionRoots="-x NoGC.reflect.xml"
  fi
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" NoGC.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" NoGC.asm.exe $CLRTestExecutionArguments
