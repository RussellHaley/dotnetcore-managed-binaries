...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\NoGCRegion\NoGC NoGC.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\NoGCRegion\NoGC NoGC.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=NoGC.dasm.il NoGC.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=NoGC.dasm.il NoGC.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=NoGC.asm.exe  NoGC.dasm.il
    %CORE_ROOT%\ilasm.exe /output=NoGC.asm.exe  NoGC.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=NoGC
    if not exist "NoGC.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "NoGC.org" (
            mkdir IL
            copy NoGC.exe IL\NoGC.exe 
            ren NoGC.exe NoGC.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NoGC.org /out NoGC.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in NoGC.org /out NoGC.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in NoGC.reflect.xml
    IF EXIST NoGC.reflect.xml set ReflectionRoots=-x NoGC.reflect.xml
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% NoGC.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% NoGC.asm.exe %CLRTestExecutionArguments% %Host_Args%
