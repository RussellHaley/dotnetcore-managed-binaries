...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCSettings/InputValidation InputValidation.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/API/GCSettings/InputValidation InputValidation.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=InputValidation.dasm.il InputValidation.exe
    "$CORE_ROOT/ildasm" -raweh -out=InputValidation.dasm.il InputValidation.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=InputValidation.asm.exe  InputValidation.dasm.il
    "$CORE_ROOT/ilasm" -output=InputValidation.asm.exe  InputValidation.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=InputValidation
    if [ ! -f InputValidation.org ]; then
        TakeLock
        if [ ! -f InputValidation.org ]; then
          mkdir IL
          cp InputValidation.exe IL/InputValidation.exe
          mv InputValidation.exe InputValidation.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InputValidation.org /out InputValidation.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in InputValidation.org /out InputValidation.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in InputValidation.reflect.xml
  if [ -f InputValidation.reflect.xml ]
then
    ReflectionRoots="-x InputValidation.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/InputValidation.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" InputValidation.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" InputValidation.asm.exe $CLRTestExecutionArguments
