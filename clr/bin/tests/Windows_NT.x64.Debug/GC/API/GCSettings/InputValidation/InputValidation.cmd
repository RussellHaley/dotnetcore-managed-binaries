...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCSettings\InputValidation InputValidation.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\API\GCSettings\InputValidation InputValidation.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=InputValidation.dasm.il InputValidation.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=InputValidation.dasm.il InputValidation.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=InputValidation.asm.exe  InputValidation.dasm.il
    %CORE_ROOT%\ilasm.exe /output=InputValidation.asm.exe  InputValidation.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=InputValidation
    if not exist "InputValidation.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "InputValidation.org" (
            mkdir IL
            copy InputValidation.exe IL\InputValidation.exe 
            ren InputValidation.exe InputValidation.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InputValidation.org /out InputValidation.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in InputValidation.org /out InputValidation.exe
...
set ExePath=InputValidation.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in InputValidation.reflect.xml
    IF EXIST InputValidation.reflect.xml set ReflectionRoots=-x InputValidation.reflect.xml
    ...
    set ExePath=%LinkBin%\InputValidation.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% InputValidation.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% InputValidation.asm.exe %CLRTestExecutionArguments% %Host_Args%
