...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\LargeMemory\Regressions\pressureoverflow pressureoverflow.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\LargeMemory\Regressions\pressureoverflow pressureoverflow.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=pressureoverflow.dasm.il pressureoverflow.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=pressureoverflow.dasm.il pressureoverflow.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=pressureoverflow.asm.exe  pressureoverflow.dasm.il
    %CORE_ROOT%\ilasm.exe /output=pressureoverflow.asm.exe  pressureoverflow.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=pressureoverflow
    if not exist "pressureoverflow.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "pressureoverflow.org" (
            mkdir IL
            copy pressureoverflow.exe IL\pressureoverflow.exe 
            ren pressureoverflow.exe pressureoverflow.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in pressureoverflow.org /out pressureoverflow.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in pressureoverflow.org /out pressureoverflow.exe
...
set ExePath=pressureoverflow.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in pressureoverflow.reflect.xml
    IF EXIST pressureoverflow.reflect.xml set ReflectionRoots=-x pressureoverflow.reflect.xml
    ...
    set ExePath=%LinkBin%\pressureoverflow.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% pressureoverflow.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% pressureoverflow.asm.exe %CLRTestExecutionArguments% %Host_Args%
