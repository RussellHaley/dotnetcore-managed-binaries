...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/LargeMemory/Regressions/largearraytest largearraytest.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../dasm/GC/LargeMemory/Regressions/largearraytest largearraytest.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=largearraytest.dasm.il largearraytest.exe
    "$CORE_ROOT/ildasm" -raweh -out=largearraytest.dasm.il largearraytest.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=largearraytest.asm.exe  largearraytest.dasm.il
    "$CORE_ROOT/ilasm" -output=largearraytest.asm.exe  largearraytest.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=largearraytest
    if [ ! -f largearraytest.org ]; then
        TakeLock
        if [ ! -f largearraytest.org ]; then
          mkdir IL
          cp largearraytest.exe IL/largearraytest.exe
          mv largearraytest.exe largearraytest.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in largearraytest.org /out largearraytest.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in largearraytest.org /out largearraytest.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in largearraytest.reflect.xml
  if [ -f largearraytest.reflect.xml ]
then
    ReflectionRoots="-x largearraytest.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/largearraytest.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" largearraytest.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" largearraytest.asm.exe $CLRTestExecutionArguments
