...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\LargeMemory\Regressions\largearraytest largearraytest.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\dasm\GC\LargeMemory\Regressions\largearraytest largearraytest.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=largearraytest.dasm.il largearraytest.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=largearraytest.dasm.il largearraytest.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=largearraytest.asm.exe  largearraytest.dasm.il
    %CORE_ROOT%\ilasm.exe /output=largearraytest.asm.exe  largearraytest.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=largearraytest
    if not exist "largearraytest.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "largearraytest.org" (
            mkdir IL
            copy largearraytest.exe IL\largearraytest.exe 
            ren largearraytest.exe largearraytest.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in largearraytest.org /out largearraytest.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in largearraytest.org /out largearraytest.exe
...
set ExePath=largearraytest.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in largearraytest.reflect.xml
    IF EXIST largearraytest.reflect.xml set ReflectionRoots=-x largearraytest.reflect.xml
    ...
    set ExePath=%LinkBin%\largearraytest.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% largearraytest.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% largearraytest.asm.exe %CLRTestExecutionArguments% %Host_Args%
