...
if not defined CLRTestExecutionArguments (set CLRTestExecutionArguments=2048 )
...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\LargeMemory\API\gc\gettotalmemory gettotalmemory.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\..\..\dasm\GC\LargeMemory\API\gc\gettotalmemory gettotalmemory.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=gettotalmemory.dasm.il gettotalmemory.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=gettotalmemory.dasm.il gettotalmemory.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=gettotalmemory.asm.exe  gettotalmemory.dasm.il
    %CORE_ROOT%\ilasm.exe /output=gettotalmemory.asm.exe  gettotalmemory.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=gettotalmemory
    if not exist "gettotalmemory.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "gettotalmemory.org" (
            mkdir IL
            copy gettotalmemory.exe IL\gettotalmemory.exe 
            ren gettotalmemory.exe gettotalmemory.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gettotalmemory.org /out gettotalmemory.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in gettotalmemory.org /out gettotalmemory.exe
...
set ExePath=gettotalmemory.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in gettotalmemory.reflect.xml
    IF EXIST gettotalmemory.reflect.xml set ReflectionRoots=-x gettotalmemory.reflect.xml
    ...
    set ExePath=%LinkBin%\gettotalmemory.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% gettotalmemory.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% gettotalmemory.asm.exe %CLRTestExecutionArguments% %Host_Args%
