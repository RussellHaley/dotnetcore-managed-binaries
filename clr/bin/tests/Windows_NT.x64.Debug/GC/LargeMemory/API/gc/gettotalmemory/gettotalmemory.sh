...
if [ -z ${CLRTestExecutionArguments+x} ]; then export CLRTestExecutionArguments='2048'; fi
...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/LargeMemory/API/gc/gettotalmemory gettotalmemory.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../../../dasm/GC/LargeMemory/API/gc/gettotalmemory gettotalmemory.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=gettotalmemory.dasm.il gettotalmemory.exe
    "$CORE_ROOT/ildasm" -raweh -out=gettotalmemory.dasm.il gettotalmemory.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=gettotalmemory.asm.exe  gettotalmemory.dasm.il
    "$CORE_ROOT/ilasm" -output=gettotalmemory.asm.exe  gettotalmemory.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=gettotalmemory
    if [ ! -f gettotalmemory.org ]; then
        TakeLock
        if [ ! -f gettotalmemory.org ]; then
          mkdir IL
          cp gettotalmemory.exe IL/gettotalmemory.exe
          mv gettotalmemory.exe gettotalmemory.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gettotalmemory.org /out gettotalmemory.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in gettotalmemory.org /out gettotalmemory.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in gettotalmemory.reflect.xml
  if [ -f gettotalmemory.reflect.xml ]
then
    ReflectionRoots="-x gettotalmemory.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/gettotalmemory.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" gettotalmemory.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" gettotalmemory.asm.exe $CLRTestExecutionArguments
