...
then
    echo $CORE_ROOT/corerun "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/managed/Compilation/Compilation Compilation.exe
    "$CORE_ROOT/corerun" "$CORE_ROOT/jit-dasm.dll" --crossgen $CORE_ROOT/crossgen --platform $CORE_ROOT --output ../../../dasm/managed/Compilation/Compilation Compilation.exe
    _jdExitCode=$?
    if [ $_jdExitCode -ne 0 ]
    then
...
then
    echo "$CORE_ROOT/ildasm" -raweh -out=Compilation.dasm.il Compilation.exe
    "$CORE_ROOT/ildasm" -raweh -out=Compilation.dasm.il Compilation.exe
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
    ...
    echo "$CORE_ROOT/ilasm" -output=Compilation.asm.exe  Compilation.dasm.il
    "$CORE_ROOT/ilasm" -output=Compilation.asm.exe  Compilation.dasm.il
    ERRORLEVEL=$?
    if [ $ERRORLEVEL -ne 0 ]
    then
...
if [ ! -z ${RunCrossGen+x} ]; then
    ...
    export COMPlus_ZapRequireList=Compilation
    if [ ! -f Compilation.org ]; then
        TakeLock
        if [ ! -f Compilation.org ]; then
          mkdir IL
          cp Compilation.exe IL/Compilation.exe
          mv Compilation.exe Compilation.org
          echo $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Compilation.org /out Compilation.exe
          $_DebuggerFullPath "$CORE_ROOT/crossgen" /Platform_Assemblies_Paths $CORE_ROOT:$PWD/IL:$PWD /in Compilation.org /out Compilation.exe
          __cgExitCode=$?
...
then
  ...
  # Use hints for reflection roots, if provided in Compilation.reflect.xml
  if [ -f Compilation.reflect.xml ]
then
    ReflectionRoots="-x Compilation.reflect.xml"
  fi
...
do
  ...
  ExePath=$LinkBin/Compilation.exe
...
if [ -z "$DoLink" -a ! -z "$RunningIlasmRoundTrip" ]
then
  echo "$CORE_ROOT/corerun" Compilation.asm.exe $CLRTestExecutionArguments
  "$CORE_ROOT/corerun" Compilation.asm.exe $CLRTestExecutionArguments
