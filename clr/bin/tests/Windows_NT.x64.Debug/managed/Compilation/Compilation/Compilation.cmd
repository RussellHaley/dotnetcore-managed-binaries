...
if defined RunningJitDisasm (
  echo %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\managed\Compilation\Compilation Compilation.exe
  %CORE_ROOT%\corerun %CORE_ROOT%\jit-dasm.dll --crossgen %CORE_ROOT%/crossgen.exe --platform %CORE_ROOT%;%cd% --output ..\..\..\dasm\managed\Compilation\Compilation Compilation.exe
  IF NOT "!ERRORLEVEL!"=="0" (
...
IF NOT DEFINED DoLink (
  IF DEFINED RunningIlasmRoundTrip (
    echo %CORE_ROOT%\ildasm.exe /raweh /out=Compilation.dasm.il Compilation.exe
    %CORE_ROOT%\ildasm.exe /raweh /out=Compilation.dasm.il Compilation.exe
    IF NOT "!ERRORLEVEL!"=="0" (
      ECHO EXECUTION OF ILDASM - FAILED !ERRORLEVEL!
      Exit /b 1
    )
    ECHO %CORE_ROOT%\ilasm.exe /output=Compilation.asm.exe  Compilation.dasm.il
    %CORE_ROOT%\ilasm.exe /output=Compilation.asm.exe  Compilation.dasm.il
    IF NOT "!ERRORLEVEL!"=="0" (
...
if defined RunCrossGen ( 
    ...
    set COMPlus_ZapRequireList=Compilation
    if not exist "Compilation.org" (
        call :TakeLock
        set CrossGenStatus=0
        if not exist "Compilation.org" (
            mkdir IL
            copy Compilation.exe IL\Compilation.exe 
            ren Compilation.exe Compilation.org
            echo "%_DebuggerFullPath% %CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Compilation.org /out Compilation.exe
            %_DebuggerFullPath% "%CORE_ROOT%\crossgen.exe" /Platform_Assemblies_Paths %CORE_ROOT%;%cd%\IL;%cd% /in Compilation.org /out Compilation.exe
...
IF defined DoLink ( 
    ...
    REM Use hints for reflection roots, if provided in Compilation.reflect.xml
    IF EXIST Compilation.reflect.xml set ReflectionRoots=-x Compilation.reflect.xml
    ...
    set ExePath=%LinkBin%\Compilation.exe
...
IF NOT DEFINED DoLink (
  if defined RunningIlasmRoundTrip (
    ECHO %LAUNCHER% Compilation.asm.exe %CLRTestExecutionArguments% %Host_Args%
    %LAUNCHER% Compilation.asm.exe %CLRTestExecutionArguments% %Host_Args%
